-- Schema for Domain: monetization | Business: Gaming | Version: v1_mvm
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`subscription_plan` (
    ...
    `economy_config_id` BIGINT COMMENT 'Foreign key linking to licensing.economy_config. Business justification: Subscription plans granting virtual currency (virtual_currency_grant attribute exists) must reference the economy_config to apply correct earn rates, balance caps, and currency type for subscribers. L',
    ...
    `game_title_id` BIGINT COMMENT 'Foreign key linking to title.game_title. Business justification: Single-game subscription plans (e.g., FFXIV, WoW monthly sub) are scoped to a specific game title. Game-level subscription revenue attribution, MRR reporting, and entitlement management all require kn',
    ...
    `price_point_id` BIGINT COMMENT 'Foreign key linking to monetization.price_point. Business justification: A subscription_plan defines a recurring subscription product with a specific price. price_point is the master catalog of standardized IAP price points including platform fees, tax rates, and currency ',
    `pricing_id` BIGINT COMMENT 'Foreign key linking to platform.pricing. Business justification: Subscription plans sold through platform storefronts (e.g., console subscription services) require platform-approved pricing tiers. platform.pricing governs revenue share percentage and promotional pr',
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Subscription regulatory compliance: subscription plans must comply with specific regulatory obligations (EU auto-renewal disclosure laws, UK Consumer Contracts Regulations, cancellation rights). The p',
    `title_season_id` BIGINT COMMENT 'Foreign key linking to title.title_season. Business justification: Battle pass and seasonal subscription plans are scoped to a specific title_season with defined start/end dates. Season-specific subscription catalog management, seasonal revenue planning, and plan sun',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`player_subscription` (
    ...
    `billing_account_id` BIGINT COMMENT 'Foreign key linking to billing.billing_account. Business justification: Subscription charges are applied against a players billing account. Linking player_subscription to billing_account enables dunning management, lifetime spend tracking, and fraud risk assessment for r',
    `campaign_id` BIGINT COMMENT 'The identifier of the marketing campaign that led to the player subscribing. Used for campaign ROI analysis and budget allocation optimization.',
    ...
    `device_id` BIGINT COMMENT 'Foreign key linking to player.device. Business justification: Subscription fraud detection and platform-specific billing compliance require tracking the device on which a subscription was initiated. Trust and safety teams flag subscriptions originating from root',
    `drm_entitlement_id` BIGINT COMMENT 'Foreign key linking to platform.drm_entitlement. Business justification: Active subscriptions grant platform DRM entitlements for game library access. When a subscription lapses or is cancelled, the corresponding drm_entitlement must be revoked. Subscription lifecycle mana',
    `esports_season_id` BIGINT COMMENT 'Foreign key linking to esports.esports_season. Business justification: Seasonal battle passes and esports season subscriptions are a core gaming monetization product. Linking player_subscription to esports_season enables season-over-season subscriber retention reporting ',
    `funnel_definition_id` BIGINT COMMENT 'Foreign key linking to analytics.funnel_definition. Business justification: Subscription conversion funnel attribution: each player subscription record must be attributed to the funnel through which the player converted (trial funnel, promotional funnel, organic). This enable',
    `game_title_id` BIGINT COMMENT 'Foreign key linking to title.game_title. Business justification: Player subscriptions for single-game services (MMO monthly subs, game-specific passes) must reference the game title for churn analysis, game-level MRR reporting, and entitlement activation. No existi',
    ...
    `storefront_order_id` BIGINT COMMENT 'Foreign key linking to billing.storefront_order. Business justification: Subscriptions acquired through the storefront must trace back to the originating order for acquisition channel attribution, first-payment reconciliation, and promotional code validation. Gaming growth',
    `platform_sku_id` BIGINT COMMENT 'Foreign key linking to platform.platform_sku. Business justification: Subscriptions are sold as platform SKUs on storefronts (e.g., a subscription listed as a distinct SKU on PlayStation Store). Platform revenue reconciliation, subscription renewal billing through platf',
    `price_point_id` BIGINT COMMENT 'Foreign key linking to monetization.price_point. Business justification: A player subscription is activated at a specific price point from the master price catalog. Linking player_subscription.price_point_id → price_point.price_point_id enables subscription revenue analyti',
    `promotion_id` BIGINT COMMENT 'Foreign key linking to licensing.promotion. Business justification: Player subscriptions acquired through a promotional offer (discounted first month, trial-to-paid promotion) must reference the promotion for revenue recognition, promotional liability tracking, and re',
    `seasonal_event_id` BIGINT COMMENT 'Foreign key linking to content.seasonal_event. Business justification: Battle pass and seasonal subscriptions are scoped to a specific seasonal event (e.g., Season 5 Battle Pass tied to Season 5 event). Subscription analytics and churn reporting require attributing playe',
    `session_id` BIGINT COMMENT 'Foreign key linking to player.session. Business justification: Subscription acquisition funnel analysis requires knowing which session context (session type, matchmaking state, game mode) converted a player to a paid subscription. Growth and monetization teams us',
    `spend_limit_control_id` BIGINT COMMENT 'Foreign key linking to compliance.spend_limit_control. Business justification: Subscription spend limit enforcement: in regulated markets, recurring subscription charges must be checked against the players active spend_limit_control. This FK records which control was applied at',
    ...
    `storefront_item_id` BIGINT COMMENT 'Foreign key linking to monetization.storefront_item. Business justification: A player subscription is initiated through the purchase of a subscription product listed as a storefront_item. Linking player_subscription.storefront_item_id → storefront_item.storefront_item_id conne',
    `subscription_id` BIGINT COMMENT 'Foreign key linking to billing.subscription. Business justification: Subscription dunning, renewal billing, and payment failure handling require linking the monetization player_subscription record to the billing.subscription lifecycle record. Revenue operations teams r',
    ...
    `team_id` BIGINT COMMENT 'Foreign key linking to esports.team. Business justification: Team fan subscription passes (e.g., team-branded premium memberships) are a real esports monetization product. Linking player_subscription to team enables fan monetization reporting, team-level subscr',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` (
    ...
    `ab_experiment_id` BIGINT COMMENT 'Foreign key linking to analytics.ab_experiment. Business justification: DLC offer A/B test revenue attribution: DLC entitlements acquired through A/B tested pricing or offer experiments must be attributed to the experiment for revenue impact analysis. Gaming studios requi',
    `age_rating_cert_id` BIGINT COMMENT 'Foreign key linking to compliance.age_rating_cert. Business justification: Age-gated DLC sale compliance: dlc_entitlement has content_rating as a plain attribute — a denormalization of the age_rating_cert. The entitlement must reference the cert that authorized the sale in t',
    ...
    `campaign_id` BIGINT COMMENT 'Reference to the marketing campaign if this entitlement was granted as a promotion. Null if not promotional.',
    `content_release_id` BIGINT COMMENT 'Foreign key linking to content.content_release. Business justification: DLC entitlements are granted as part of a content release (season pass drops, patch-bundled DLC). Entitlement auditing and rollback procedures require knowing which content_release delivered each enti',
    `device_id` BIGINT COMMENT 'Foreign key linking to player.device. Business justification: DLC download management and device-limit enforcement require knowing which device holds an entitlement download. dlc_entitlement tracks download_status and download_completed_timestamp — these are mea',
    ...
    `drm_entitlement_id` BIGINT COMMENT 'Foreign key linking to platform.drm_entitlement. Business justification: When a player purchases DLC, the monetization entitlement record must reference the platform DRM entitlement that enforces the license. Refund processing, entitlement revocation, and audit workflows r',
    `entitlement_id` BIGINT COMMENT 'Foreign key linking to licensing.entitlement. Business justification: Player-facing DLC entitlements are granted under B2B licensing entitlements covering IP rights. Compliance teams must verify that each DLC grant is covered by an active licensing entitlement record, s',
    ...
    `league_id` BIGINT COMMENT 'Foreign key linking to esports.league. Business justification: League-branded DLC bundles (cosmetics, content drops tied to a specific league) are a real esports monetization product. Entitlement records must reference the league for IP licensing compliance, reve',
    `live_ops_cycle_id` BIGINT COMMENT 'Foreign key linking to studio.live_ops_cycle. Business justification: Live Ops DLC Revenue Recognition: Battle pass rewards, seasonal DLC, and cycle-specific entitlements must be attributed to the generating live_ops_cycle for revenue recognition, content delivery audit',
    `loot_box_disclosure_id` BIGINT COMMENT 'Foreign key linking to compliance.loot_box_disclosure. Business justification: DLC loot box acquisition audit: DLC entitlements acquired via gacha/loot box mechanics must reference the loot_box_disclosure active at acquisition_timestamp. Regulators require proof that odds were d',
    `platform_sku_id` BIGINT COMMENT 'Foreign key linking to platform.platform_sku. Business justification: DLC entitlements are granted for specific platform SKUs (the DLC as listed on a given platform storefront). Platform-side entitlement verification, regional availability enforcement, and DRM type dete',
    `price_point_id` BIGINT COMMENT 'Foreign key linking to monetization.price_point. Business justification: A DLC entitlement is acquired at a specific price point. Linking dlc_entitlement.price_point_id → price_point.price_point_id allows monetization analytics to track which price tiers drive DLC adoption',
    `player_account_id` BIGINT COMMENT 'Reference to the player who owns this DLC entitlement.',
    `promotion_id` BIGINT COMMENT 'Foreign key linking to licensing.promotion. Business justification: DLC entitlements acquired through a promotional offer must reference the promotion for revenue recognition (discounted vs. full price), promotional discount accounting, and redemption tracking against',
    `spend_limit_control_id` BIGINT COMMENT 'Foreign key linking to compliance.spend_limit_control. Business justification: DLC purchase spend limit audit: the spend_limit_control enforced at DLC acquisition must be recorded for regulatory compliance reporting. Jurisdictions with mandatory spend limits require each purchas',
    `storefront_id` BIGINT COMMENT 'Reference to the gaming platform where this entitlement is valid (console, PC, mobile).',
    `storefront_item_id` BIGINT COMMENT 'Foreign key linking to monetization.storefront_item. Business justification: A DLC entitlement represents a players ownership of a specific DLC product, which is listed and sold as a storefront_item. Linking dlc_entitlement.storefront_item_id → storefront_item.storefront_item',
    `storefront_order_line_id` BIGINT COMMENT 'Foreign key linking to billing.storefront_order_line. Business justification: DLC entitlements granted via storefront purchase must reference the specific order line for refund processing, entitlement revocation on chargeback, and content access audit. Gaming platform operation',
    ...
    `team_id` BIGINT COMMENT 'Foreign key linking to esports.team. Business justification: Team-branded DLC (team skins, team cosmetics, jersey bundles) is a major esports monetization revenue stream. Entitlement records must reference the team for IP licensing compliance, team revenue shar',
    `title_season_id` BIGINT COMMENT 'Foreign key linking to title.title_season. Business justification: Season pass entitlements grant access to a specific title_seasons content. Season pass fulfillment, entitlement validation, and seasonal content access control require linking dlc_entitlement to the ',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` (
    ...
    `consent_policy_id` BIGINT COMMENT 'Foreign key linking to compliance.consent_policy. Business justification: GDPR profiling consent: LTV segmentation involves automated profiling of player spending behavior. Under GDPR Art. 22, the processing must reference the consent_policy that authorized this profiling a',
    `league_id` BIGINT COMMENT 'Foreign key linking to esports.league. Business justification: Esports league viewership is a proven LTV driver. Segmenting player LTV by league affiliation enables targeted monetization campaigns, league-specific ARPU reporting for sponsor valuation, and identif',
    `ml_model_registry_id` BIGINT COMMENT 'Foreign key linking to analytics.ml_model_registry. Business justification: ML model lineage for LTV scoring: LTV segments are computed by ML models; the segment record must reference the model that produced it for model governance, auditability, and retraining decisions. Gam',
    ...
    `player_behavior_segment_id` BIGINT COMMENT 'Foreign key linking to analytics.player_behavior_segment. Business justification: LTV-to-behavior segment reconciliation: gaming studios cross-reference monetization LTV segments with analytics behavior segments in unified player intelligence dashboards. This join is essential for ',
    ...
    `processing_activity_id` BIGINT COMMENT 'Foreign key linking to compliance.processing_activity. Business justification: GDPR Records of Processing Activities (RoPA): LTV segmentation is a documented processing activity under GDPR Article 30. The segment record must reference the processing_activity that documents this ',
    `spend_limit_control_id` BIGINT COMMENT 'Foreign key linking to compliance.spend_limit_control. Business justification: Responsible gambling monitoring: LTV segmentation is used to identify high-spend players approaching regulatory spend limits. The segment record references the applicable spend_limit_control to enable',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`ad_placement` (
    ...
    `ab_experiment_id` BIGINT COMMENT 'Foreign key linking to analytics.ab_experiment. Business justification: Ad placement A/B testing: placements are A/B tested on format, frequency caps, and location to optimize eCPM and fill rates. The existing ab_test_variant column is a denormalized variant label; a prop',
    `ad_network_id` BIGINT COMMENT 'Foreign key linking to marketing.ad_network. Business justification: Ad placements are configured per ad network for mediation waterfall setup. The existing `ad_network_partner` plain-text field is a denormalized reference to ad_network. Gaming ad monetization teams ne',
    ...
    `age_rating_cert_id` BIGINT COMMENT 'Foreign key linking to compliance.age_rating_cert. Business justification: Age-gated ad serving: ad placements have age_gate_minimum and coppa_compliant flags. The placement must reference the age_rating_cert that governs permissible ad content and audience targeting rules. ',
    `consent_policy_id` BIGINT COMMENT 'Foreign key linking to compliance.consent_policy. Business justification: GDPR/COPPA ad consent governance: ad_placement has gdpr_consent_required as a plain flag — a denormalization of the consent_policy. The placement must reference the consent_policy that defines what co',
    ...
    `event_id` BIGINT COMMENT 'Foreign key linking to community.community_event. Business justification: Ad placements scoped to specific community events (event-exclusive ad slots, sponsor integrations during tournaments/events) require this FK for event-specific ad targeting configuration and sponsor r',
    ...
    `league_id` BIGINT COMMENT 'Foreign key linking to esports.league. Business justification: Ad inventory is managed at the league level in esports broadcast operations. League-scoped ad placements define the inventory available to sponsors and ad networks for a specific league, enabling leag',
    `level_map_id` BIGINT COMMENT 'Foreign key linking to content.level_map. Business justification: In-game advertising (billboards, banners) is placed within specific level maps. Ad revenue attribution by map, fill rate analysis per level, and advertiser reporting on placement context all require t',
    `licensed_ip_id` BIGINT COMMENT 'Foreign key linking to licensing.licensed_ip. Business justification: Ad placements featuring licensed IP content (branded in-game placements using licensed franchises or characters) must reference the licensed IP for royalty tracking and compliance. Gaming companies wi',
    ...
    `player_segment_id` BIGINT COMMENT 'Foreign key linking to marketing.player_segment. Business justification: Ad ops teams configure which player segments are eligible for each ad placement (e.g., exclude paying players from rewarded ads, target churned users). The existing `player_segment_eligibility` plain-',
    ...
    `asset_bundle_id` BIGINT COMMENT 'Foreign key linking to content.asset_bundle. Business justification: Rewarded ads grant asset bundles as rewards (e.g., watch ad, receive starter pack). The existing reward_asset_id covers single-asset rewards, but bundle rewards require a separate FK. Ad monetizatio',
    ...
    `sdk_integration_id` BIGINT COMMENT 'Foreign key linking to platform.sdk_integration. Business justification: Ad placements are served through specific SDK integrations (ad mediation SDKs) on each platform. Ad monetization operations require knowing which SDK integration serves each placement for fill rate tr',
    ...
    `sponsorship_id` BIGINT COMMENT 'Foreign key linking to esports.sponsorship. Business justification: Sponsor-integrated ad placements (branded overlays, sponsor bumpers) are contractually defined in sponsorship agreements. Linking ad_placement to sponsorship enables delivery tracking against contract',
    `storefront_id` BIGINT COMMENT 'Reference to the platform where this ad placement is served (iOS, Android, Web).',
    `tournament_id` BIGINT COMMENT 'Foreign key linking to esports.tournament. Business justification: Tournament-specific ad placements (e.g., grand final broadcast ad units) are a distinct inventory category with premium eCPM floors. Linking ad_placement to tournament enables tournament-level ad reve',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`ad_impression` (
    ...
    `campaign_id` BIGINT COMMENT 'The advertising campaign identifier from the ad network. Used for campaign-level performance tracking and attribution.',
    `consent_policy_id` BIGINT COMMENT 'Foreign key linking to compliance.consent_policy. Business justification: GDPR ad impression audit trail: each ad impression must record the consent_policy under which it was served. Data protection authorities require proof that impressions were served only to players who ',
    `device_id` BIGINT COMMENT 'Foreign key linking to player.device. Business justification: Ad fraud detection and per-device frequency capping require linking impressions to the registered device record. Ad ops teams run device-level impression reports to identify emulator fraud and enforce',
    `event_id` BIGINT COMMENT 'Foreign key linking to community.community_event. Business justification: Ad impressions served during community events require event attribution for sponsor campaign performance reporting and event monetization analytics. Gaming ad ops teams report eCPM and fill rates per ',
    ...
    `player_behavior_segment_id` BIGINT COMMENT 'Foreign key linking to analytics.player_behavior_segment. Business justification: Segment-level ad revenue reporting: ad impressions are served to players based on behavior segments; attributing impressions to the formal segment definition enables segment-level eCPM and fill rate a',
    `player_segment_id` BIGINT COMMENT 'Foreign key linking to marketing.player_segment. Business justification: Ad impression records carry `player_segment` as a plain-text denormalized field. Normalizing to a FK enables segment-level ad revenue reporting (eCPM by segment, rewarded ad completion by LTV tier) — ',
    ...
    `sponsorship_id` BIGINT COMMENT 'Foreign key linking to esports.sponsorship. Business justification: Sponsor impression delivery reporting is a contractual obligation in esports sponsorship deals. Linking ad_impression to sponsorship enables real-time tracking of contracted impression guarantees, per',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`price_point` (
    ...
    `commercial_term_id` BIGINT COMMENT 'Foreign key linking to licensing.commercial_term. Business justification: Price points for licensed IP content must comply with commercial terms governing minimum guarantees, royalty rates, and territory restrictions. Finance and licensing teams link price points to commerc',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Price regulation compliance: price points must comply with jurisdiction-specific regulatory obligations (VAT display requirements, loot box price disclosure laws, consumer protection pricing rules). T',
    `superseded_price_point_id` BIGINT COMMENT 'Self-referencing FK on price_point (superseded_price_point_id)',
    `title_sku_id` BIGINT COMMENT 'Foreign key linking to title.title_sku. Business justification: Price points are defined at the SKU level for platform+region pricing (e.g., PS5 US SKU at $69.99). SKU-level price management, regional pricing compliance, and storefront submission workflows all req',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`ad_unit` (
    ...
    `ab_experiment_id` BIGINT COMMENT 'Foreign key linking to analytics.ab_experiment. Business justification: Ad unit configuration A/B testing: ad unit parameters (format, skip offset, reward amount, floor price) are A/B tested to optimize eCPM and player experience. Gaming ad monetization teams require this',
    `ad_network_id` BIGINT COMMENT 'Reference to the advertising network or mediation platform serving ads to this unit.',
    `consent_policy_id` BIGINT COMMENT 'Foreign key linking to compliance.consent_policy. Business justification: Ad unit consent framework: ad_unit has gdpr_consent_required and coppa_compliant as plain flags — denormalizations of the consent_policy. The ad unit must reference the consent_policy defining what co',
    `game_title_id` BIGINT COMMENT 'Reference to the game title where this ad unit is integrated.',
    `localization_string_id` BIGINT COMMENT 'Foreign key linking to content.localization_string. Business justification: ad_unit_name and ad_unit_description are plain text denormalized strings. Ad units deployed on global platforms require localized names/descriptions for compliance and partner reporting. FK to localiz',
    `parent_ad_unit_id` BIGINT COMMENT 'Self-referencing FK on ad_unit (parent_ad_unit_id)',
    `policy_id` BIGINT COMMENT 'Foreign key linking to compliance.policy. Business justification: Ad unit policy governance: ad units must reference the compliance policy governing their content, targeting, and frequency rules (COPPA, GDPR ad targeting policies). Ad operations teams use this to en',
    `sdk_integration_id` BIGINT COMMENT 'Foreign key linking to platform.sdk_integration. Business justification: Ad units are registered and configured within specific platform SDK integrations. Ad network mediation, floor price enforcement, and platform certification compliance all require knowing which sdk_int',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`storefront_transaction` (
    ...
    `ab_experiment_id` BIGINT COMMENT 'Foreign key linking to analytics.ab_experiment. Business justification: Storefront layout and offer A/B test revenue attribution: transactions must be attributed to the experiment variant (e.g., storefront UI test, bundle offer test) that influenced the purchase. Gaming s',
    `billing_account_id` BIGINT COMMENT 'Foreign key linking to billing.billing_account. Business justification: Each storefront purchase is charged to a players billing account. Linking storefront_transaction to billing_account enables lifetime spend aggregation, fraud scoring, and chargeback risk analysis — s',
    `campaign_id` BIGINT COMMENT 'Identifier of the marketing campaign that influenced or attributed to this transaction.',
    `device_id` BIGINT COMMENT 'Unique identifier of the device (console, mobile, PC) from which the transaction was initiated.',
    `event_id` BIGINT COMMENT 'Foreign key linking to community.community_event. Business justification: Event shop purchases and event-triggered transactions need event attribution for ROI reporting, event revenue dashboards, and prize fund accounting. Gaming operators track revenue generated by commun',
    `funnel_definition_id` BIGINT COMMENT 'Foreign key linking to analytics.funnel_definition. Business justification: Purchase funnel conversion analysis: every completed storefront transaction is the terminal event of a purchase funnel. Gaming monetization teams attribute revenue to specific funnel definitions to me',
    `game_title_id` BIGINT COMMENT 'Identifier of the game title in which this storefront transaction occurred.',
    `iap_transaction_id` BIGINT COMMENT 'Foreign key linking to licensing.iap_transaction. Business justification: Storefront transactions (platform-level) and IAP transactions (licensing/economy-level) represent the same purchase from two domain perspectives. Finance teams reconcile these records for royalty repo',
    `infrastructure_incident_id` BIGINT COMMENT 'Foreign key linking to infrastructure.infrastructure_incident. Business justification: Incident revenue impact quantification: linking failed/affected storefront transactions to the causative infrastructure_incident enables precise revenue loss calculation for SLA breach reporting and p',
    `live_ops_cycle_id` BIGINT COMMENT 'Foreign key linking to studio.live_ops_cycle. Business justification: Live Ops Seasonal Revenue Reporting: GaaS studios track revenue per season/cycle for P&L attribution. Linking storefront transactions to the active live_ops_cycle enables direct seasonal revenue dashb',
    `loot_box_disclosure_id` BIGINT COMMENT 'Foreign key linking to compliance.loot_box_disclosure. Business justification: Loot box purchase audit trail: when a storefront transaction involves a loot box/gacha purchase, regulators (Belgium, Netherlands, UK) require proof that the active disclosure was presented at point o',
    `offer_campaign_id` BIGINT COMMENT 'Foreign key linking to licensing.offer_campaign. Business justification: Storefront transactions resulting from targeted offer campaigns must reference the offer_campaign for conversion tracking, campaign ROI reporting, and revenue attribution. Monetization and live-ops te',
    ...
    `platform_sku_id` BIGINT COMMENT 'Foreign key linking to platform.platform_sku. Business justification: Every platform storefront transaction is associated with a specific platform SKU for revenue reconciliation and platform fee calculation. Platform holders (Steam, PlayStation, Xbox) require transactio',
    `price_point_id` BIGINT COMMENT 'Foreign key linking to monetization.price_point. Business justification: A storefront_transaction is executed at a specific price point from the master price catalog. Linking storefront_transaction.price_point_id → price_point.price_point_id enables revenue analytics by pr',
    `promotion_id` BIGINT COMMENT 'Foreign key linking to licensing.promotion. Business justification: Storefront transactions that applied a promotion must reference the promotion record for revenue recognition (net vs. gross), promotional liability accounting, and redemption cap enforcement. This is ',
    `server_session_id` BIGINT COMMENT 'Foreign key linking to infrastructure.server_session. Business justification: Revenue-per-session attribution: gaming ops teams correlate IAP/storefront purchases with the active server_session to attribute infrastructure costs against revenue, detect session-context fraud patt',
    `session_id` BIGINT COMMENT 'Unique identifier of the player game session during which the transaction occurred.',
    `spend_limit_control_id` BIGINT COMMENT 'Foreign key linking to compliance.spend_limit_control. Business justification: Real-time spend limit enforcement: when processing a storefront transaction in regulated markets (UK GC, Netherlands), the system must check and record which spend_limit_control was active. Regulators',
    ...
    `storefront_order_id` BIGINT COMMENT 'Foreign key linking to billing.storefront_order. Business justification: Storefront transactions in monetization must reconcile with billing storefront orders for revenue recognition, refund processing, and financial close. Gaming finance teams match these records daily; t',
    `subscription_id` BIGINT COMMENT 'Foreign key linking to billing.subscription. Business justification: Storefront transactions that initiate or renew a subscription must reference the billing subscription record for acquisition tracking, first-payment attribution, and subscription lifecycle reporting. ',
    `tournament_id` BIGINT COMMENT 'Foreign key linking to esports.tournament. Business justification: Tournament entry fee transactions and tournament pass purchases must be attributed to the specific tournament for revenue reporting, prize pool contribution tracking, and financial reconciliation. Thi',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`storefront_item` (
    ...
    `ab_experiment_id` BIGINT COMMENT 'Foreign key linking to analytics.ab_experiment. Business justification: Storefront item A/B testing: item prices, descriptions, images, and bundle compositions are A/B tested to optimize conversion rates. Gaming monetization teams require this FK to attribute item-level r',
    `asset_bundle_id` BIGINT COMMENT 'Foreign key linking to content.asset_bundle. Business justification: Bundle storefront items (is_bundle=true) are delivered as asset bundles. Platform fulfillment must know which asset_bundle to download on purchase. This link drives download orchestration and bundle d',
    `asset_id` BIGINT COMMENT 'Foreign key linking to content.asset. Business justification: Storefront items (skins, cosmetics, icons) are rendered using a specific content asset. Merchandising and live-ops teams must link each storefront item to its display asset for store rendering, asset ',
    `content_release_id` BIGINT COMMENT 'Foreign key linking to content.content_release. Business justification: Storefront items are introduced via content releases (patches, updates). Release managers need to know which items ship with each release for rollback decisions and launch revenue attribution. This FK',
    `dlc_bundle_id` BIGINT COMMENT 'Foreign key linking to title.dlc_bundle. Business justification: A storefront item often represents a DLC bundle listing for sale. Linking storefront_item to dlc_bundle enables DLC sales reporting, entitlement fulfillment reconciliation, and catalog alignment betwe',
    `esports_season_id` BIGINT COMMENT 'Foreign key linking to esports.esports_season. Business justification: Season battle passes and season-exclusive cosmetics are the highest-revenue storefront items in live-service games. Linking storefront_item to esports_season enables season pass revenue reporting, sea',
    `gacha_pool_id` BIGINT COMMENT 'Foreign key linking to licensing.gacha_pool. Business justification: Storefront items that are gacha/loot box purchases must reference the specific gacha pool for regulatory loot box disclosure compliance (drop rate disclosure requirements in Belgium, Netherlands, Sout',
    `game_edition_id` BIGINT COMMENT 'Foreign key linking to title.game_edition. Business justification: A storefront item can represent a game edition (Standard, Deluxe, Ultimate). Linking storefront_item to game_edition enables edition-level sales reporting and catalog reconciliation between title mana',
    `game_title_id` BIGINT COMMENT 'Foreign key linking to title.game_title. Business justification: Every storefront item (skin, weapon, cosmetic, bundle) belongs to a specific game title. Game-level item catalog management, title-scoped revenue reporting, and storefront curation all require knowing',
    `licensed_ip_id` BIGINT COMMENT 'Foreign key linking to licensing.licensed_ip. Business justification: Storefront items based on licensed IP (branded cosmetics, licensed character skins, franchise-themed bundles) must reference the licensed IP for royalty calculation and compliance. Revenue from these ',
    `live_ops_cycle_id` BIGINT COMMENT 'Foreign key linking to studio.live_ops_cycle. Business justification: GaaS Seasonal Content Scheduling: storefront items (battle passes, limited-time offers, seasonal bundles) are scoped to a live ops cycle. Monetization and live ops teams must attribute storefront item',
    `loot_box_disclosure_id` BIGINT COMMENT 'Foreign key linking to compliance.loot_box_disclosure. Business justification: Loot box item compliance: a storefront item that is a loot box/gacha must reference its governing loot_box_disclosure. Regulators require the item listing itself to be tied to its probability disclosu',
    `mtx_catalog_id` BIGINT COMMENT 'Foreign key linking to licensing.mtx_catalog. Business justification: Storefront items are the platform-facing representation of MTX catalog entries. Revenue operations and royalty teams must map storefront_item records to mtx_catalog to reconcile platform sales against',
    `localization_string_id` BIGINT COMMENT 'Foreign key linking to content.localization_string. Business justification: item_name and item_description are denormalized localization strings. Global storefronts require localized item names/descriptions across all supported languages. Proper FK to localization_string enfo',
    `parent_storefront_item_id` BIGINT COMMENT 'Self-referencing FK on storefront_item (parent_storefront_item_id)',
    `platform_sku_id` BIGINT COMMENT 'Foreign key linking to platform.platform_sku. Business justification: Storefront items (IAPs, DLC packs) are listed under specific platform SKUs on each storefront. Platform revenue share calculation, certification compliance, and platform-side product reconciliation al',
    `price_point_id` BIGINT COMMENT 'Foreign key linking to monetization.price_point. Business justification: storefront_item is the master catalog of items sold in the store. price_point is the authoritative master catalog of standardized IAP price points. Linking storefront_item.price_point_id → price_point',
    `seasonal_event_id` BIGINT COMMENT 'Foreign key linking to content.seasonal_event. Business justification: Time-limited storefront items (is_time_limited=true) are gated to seasonal events (Halloween skins, holiday bundles). Live-ops revenue reporting requires attributing item sales to specific events. Thi',
    `team_id` BIGINT COMMENT 'Foreign key linking to esports.team. Business justification: Team-branded storefront items (team skins, cosmetics, fan bundles) are a primary esports merchandise revenue stream. Linking storefront_item to team enables team merchandise revenue reporting, IP lice',
    `tournament_id` BIGINT COMMENT 'Foreign key linking to esports.tournament. Business justification: Tournament-specific storefront items (tournament passes, commemorative bundles, championship cosmetics) are time-limited products tied to a specific tournament. This link enables tournament merchandis',
...
CREATE OR REPLACE TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` (
    `plan_content_inclusion_id` BIGINT COMMENT 'Primary key for the plan content inclusion association record',
    `dlc_entitlement_id` BIGINT COMMENT 'Foreign key linking to the DLC content that is included in the subscription plan library',
    `subscription_plan_id` BIGINT COMMENT 'Foreign key linking to the subscription plan that includes this DLC content in its library',
    `access_level` STRING COMMENT 'Level of access granted to subscribers for this DLC content (FULL = complete access, TRIAL = limited time trial, PREVIEW = demo mode, TIMED = time-limited sessions). Explicitly identified in detection reasoning.',
    `content_unlock_sequence` STRING COMMENT 'Ordinal position in which this DLC content unlocks for subscribers (e.g., episodic content released over subscription tenure). Null if all content available immediately. Explicitly identified in detection reasoning.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this plan content inclusion record was first created in the system. Used for audit and change tracking.',
    `exclusion_date` DATE COMMENT 'Date when this DLC content will be or was removed from the subscription plan library. Null if content is permanent. Used for content rotation management and subscriber notifications.',
    `included_since_date` DATE COMMENT 'Date when this DLC content was added to the subscription plan library. Used for content lifecycle tracking and subscriber communication. Explicitly identified in detection reasoning.',
    `inclusion_status` STRING COMMENT 'Current lifecycle status of this content inclusion (ACTIVE = currently available, SCHEDULED = future inclusion, EXPIRED = removed from library, SUSPENDED = temporarily unavailable). Used for access enforcement and content catalog display.',
    `is_exclusive_to_plan` BOOLEAN COMMENT 'Indicates whether this DLC content is exclusive to this subscription plan tier (not available in lower tiers or standalone purchase). Used for marketing differentiation and access control. Explicitly identified in detection reasoning.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this plan content inclusion record was last modified. Used for audit and change tracking.',
    CONSTRAINT pk_plan_content_inclusion PRIMARY KEY(`plan_content_inclusion_id`)
) COMMENT 'This association product represents the inclusion of DLC content within subscription plan libraries. It captures which DLC entitlements are available to subscribers of each plan, when content enters or exits the library, access restrictions, and exclusivity arrangements. Each record links one subscription_plan to one DLC content offering with attributes that govern subscriber access rights and content lifecycle within the subscription service.. Existence Justification: In gaming subscription services (Xbox Game Pass, PlayStation Plus, EA Play model), subscription plans include curated libraries of DLC content, where each plan tier includes multiple DLC items and each DLC can be included across multiple subscription tiers with different access levels. The business actively manages content library composition, tracking when content enters/exits plans, enforcing tier-specific access rules, and managing exclusivity arrangements. This is an operational M:N relationship representing subscription content library management.';

-- ========= FOREIGN KEYS =========
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ADD CONSTRAINT `fk_monetization_subscription_plan_price_point_id` FOREIGN KEY (`price_point_id`) REFERENCES `gaming_ecm`.`monetization`.`price_point`(`price_point_id`);
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ADD CONSTRAINT `fk_monetization_player_subscription_price_point_id` FOREIGN KEY (`price_point_id`) REFERENCES `gaming_ecm`.`monetization`.`price_point`(`price_point_id`);
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ADD CONSTRAINT `fk_monetization_player_subscription_storefront_item_id` FOREIGN KEY (`storefront_item_id`) REFERENCES `gaming_ecm`.`monetization`.`storefront_item`(`storefront_item_id`);
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ADD CONSTRAINT `fk_monetization_dlc_entitlement_price_point_id` FOREIGN KEY (`price_point_id`) REFERENCES `gaming_ecm`.`monetization`.`price_point`(`price_point_id`);
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ADD CONSTRAINT `fk_monetization_dlc_entitlement_storefront_item_id` FOREIGN KEY (`storefront_item_id`) REFERENCES `gaming_ecm`.`monetization`.`storefront_item`(`storefront_item_id`);
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ADD CONSTRAINT `fk_monetization_storefront_transaction_price_point_id` FOREIGN KEY (`price_point_id`) REFERENCES `gaming_ecm`.`monetization`.`price_point`(`price_point_id`);
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ADD CONSTRAINT `fk_monetization_storefront_item_price_point_id` FOREIGN KEY (`price_point_id`) REFERENCES `gaming_ecm`.`monetization`.`price_point`(`price_point_id`);
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ADD CONSTRAINT `fk_monetization_plan_content_inclusion_dlc_entitlement_id` FOREIGN KEY (`dlc_entitlement_id`) REFERENCES `gaming_ecm`.`monetization`.`dlc_entitlement`(`dlc_entitlement_id`);
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ADD CONSTRAINT `fk_monetization_plan_content_inclusion_subscription_plan_id` FOREIGN KEY (`subscription_plan_id`) REFERENCES `gaming_ecm`.`monetization`.`subscription_plan`(`subscription_plan_id`);
...
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` SET TAGS ('dbx_subdomain' = 'subscription_services');
...
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ALTER COLUMN `economy_config_id` SET TAGS ('dbx_business_glossary_term' = 'Economy Config Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ALTER COLUMN `game_title_id` SET TAGS ('dbx_business_glossary_term' = 'Game Title Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ALTER COLUMN `price_point_id` SET TAGS ('dbx_business_glossary_term' = 'Price Point Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ALTER COLUMN `pricing_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`subscription_plan` ALTER COLUMN `title_season_id` SET TAGS ('dbx_business_glossary_term' = 'Title Season Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` SET TAGS ('dbx_subdomain' = 'subscription_services');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `billing_account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Acquisition Campaign ID');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `device_id` SET TAGS ('dbx_business_glossary_term' = 'Device Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `device_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `device_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `drm_entitlement_id` SET TAGS ('dbx_business_glossary_term' = 'Drm Entitlement Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `esports_season_id` SET TAGS ('dbx_business_glossary_term' = 'Esports Season Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `funnel_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Funnel Definition Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `game_title_id` SET TAGS ('dbx_business_glossary_term' = 'Game Title Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `storefront_order_id` SET TAGS ('dbx_business_glossary_term' = 'Originating Storefront Order Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `platform_sku_id` SET TAGS ('dbx_business_glossary_term' = 'Platform Sku Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `price_point_id` SET TAGS ('dbx_business_glossary_term' = 'Price Point Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `seasonal_event_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Event Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `session_id` SET TAGS ('dbx_business_glossary_term' = 'Session Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `spend_limit_control_id` SET TAGS ('dbx_business_glossary_term' = 'Spend Limit Control Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `storefront_item_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Item Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `subscription_id` SET TAGS ('dbx_business_glossary_term' = 'Subscription Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_subscription` ALTER COLUMN `team_id` SET TAGS ('dbx_business_glossary_term' = 'Team Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` SET TAGS ('dbx_subdomain' = 'subscription_services');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `ab_experiment_id` SET TAGS ('dbx_business_glossary_term' = 'Ab Experiment Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `age_rating_cert_id` SET TAGS ('dbx_business_glossary_term' = 'Age Rating Cert Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotional Campaign ID');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `content_release_id` SET TAGS ('dbx_business_glossary_term' = 'Content Release Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `device_id` SET TAGS ('dbx_business_glossary_term' = 'Device Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `device_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `device_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `drm_entitlement_id` SET TAGS ('dbx_business_glossary_term' = 'Drm Entitlement Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `entitlement_id` SET TAGS ('dbx_business_glossary_term' = 'Entitlement Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `league_id` SET TAGS ('dbx_business_glossary_term' = 'League Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `live_ops_cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Live Ops Cycle Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `loot_box_disclosure_id` SET TAGS ('dbx_business_glossary_term' = 'Loot Box Disclosure Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `platform_sku_id` SET TAGS ('dbx_business_glossary_term' = 'Platform Sku Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `price_point_id` SET TAGS ('dbx_business_glossary_term' = 'Price Point Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `spend_limit_control_id` SET TAGS ('dbx_business_glossary_term' = 'Spend Limit Control Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `storefront_item_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Item Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `storefront_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Order Line Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `team_id` SET TAGS ('dbx_business_glossary_term' = 'Team Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`dlc_entitlement` ALTER COLUMN `title_season_id` SET TAGS ('dbx_business_glossary_term' = 'Title Season Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` SET TAGS ('dbx_subdomain' = 'player_monetization');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` ALTER COLUMN `consent_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Policy Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` ALTER COLUMN `league_id` SET TAGS ('dbx_business_glossary_term' = 'League Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` ALTER COLUMN `ml_model_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Ml Model Registry Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` ALTER COLUMN `player_behavior_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Player Behavior Segment Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` ALTER COLUMN `processing_activity_id` SET TAGS ('dbx_business_glossary_term' = 'Processing Activity Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`player_ltv_segment` ALTER COLUMN `spend_limit_control_id` SET TAGS ('dbx_business_glossary_term' = 'Spend Limit Control Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` SET TAGS ('dbx_subdomain' = 'advertising_revenue');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `ab_experiment_id` SET TAGS ('dbx_business_glossary_term' = 'Ab Experiment Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `ad_network_id` SET TAGS ('dbx_business_glossary_term' = 'Ad Network Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `ad_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Ad Unit ID');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `age_rating_cert_id` SET TAGS ('dbx_business_glossary_term' = 'Age Rating Cert Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `consent_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Policy Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Community Event Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `league_id` SET TAGS ('dbx_business_glossary_term' = 'League Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `level_map_id` SET TAGS ('dbx_business_glossary_term' = 'Level Map Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `licensed_ip_id` SET TAGS ('dbx_business_glossary_term' = 'Licensed Ip Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `player_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Player Segment Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `asset_bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Reward Asset Bundle Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `sdk_integration_id` SET TAGS ('dbx_business_glossary_term' = 'Sdk Integration Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `sponsorship_id` SET TAGS ('dbx_business_glossary_term' = 'Sponsorship Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_placement` ALTER COLUMN `tournament_id` SET TAGS ('dbx_business_glossary_term' = 'Tournament Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` SET TAGS ('dbx_subdomain' = 'advertising_revenue');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign ID');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `consent_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Policy Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `device_id` SET TAGS ('dbx_business_glossary_term' = 'Device Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `device_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `device_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Community Event Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `player_behavior_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Player Behavior Segment Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `player_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Player Segment Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_impression` ALTER COLUMN `sponsorship_id` SET TAGS ('dbx_business_glossary_term' = 'Sponsorship Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`price_point` SET TAGS ('dbx_subdomain' = 'player_monetization');
...
ALTER TABLE `gaming_ecm`.`monetization`.`price_point` ALTER COLUMN `commercial_term_id` SET TAGS ('dbx_business_glossary_term' = 'Commercial Term Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`price_point` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`price_point` ALTER COLUMN `superseded_price_point_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`price_point` ALTER COLUMN `title_sku_id` SET TAGS ('dbx_business_glossary_term' = 'Title Sku Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` SET TAGS ('dbx_subdomain' = 'advertising_revenue');
...
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` ALTER COLUMN `ab_experiment_id` SET TAGS ('dbx_business_glossary_term' = 'Ab Experiment Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` ALTER COLUMN `consent_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Policy Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` ALTER COLUMN `localization_string_id` SET TAGS ('dbx_business_glossary_term' = 'Name Localization String Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` ALTER COLUMN `parent_ad_unit_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`ad_unit` ALTER COLUMN `sdk_integration_id` SET TAGS ('dbx_business_glossary_term' = 'Sdk Integration Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` SET TAGS ('dbx_subdomain' = 'player_monetization');
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `ab_experiment_id` SET TAGS ('dbx_business_glossary_term' = 'Ab Experiment Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `billing_account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Community Event Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `funnel_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Funnel Definition Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `iap_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Iap Transaction Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `infrastructure_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Infrastructure Incident Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `live_ops_cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Live Ops Cycle Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `loot_box_disclosure_id` SET TAGS ('dbx_business_glossary_term' = 'Loot Box Disclosure Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `offer_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Campaign Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `platform_sku_id` SET TAGS ('dbx_business_glossary_term' = 'Platform Sku Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `price_point_id` SET TAGS ('dbx_business_glossary_term' = 'Price Point Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `server_session_id` SET TAGS ('dbx_business_glossary_term' = 'Server Session Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `spend_limit_control_id` SET TAGS ('dbx_business_glossary_term' = 'Spend Limit Control Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `storefront_order_id` SET TAGS ('dbx_business_glossary_term' = 'Storefront Order Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `subscription_id` SET TAGS ('dbx_business_glossary_term' = 'Subscription Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_transaction` ALTER COLUMN `tournament_id` SET TAGS ('dbx_business_glossary_term' = 'Tournament Id (Foreign Key)');
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` SET TAGS ('dbx_subdomain' = 'player_monetization');
...
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `ab_experiment_id` SET TAGS ('dbx_business_glossary_term' = 'Ab Experiment Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `asset_bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Bundle Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `asset_id` SET TAGS ('dbx_business_glossary_term' = 'Content Asset Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `content_release_id` SET TAGS ('dbx_business_glossary_term' = 'Content Release Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `dlc_bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Dlc Bundle Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `esports_season_id` SET TAGS ('dbx_business_glossary_term' = 'Esports Season Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `gacha_pool_id` SET TAGS ('dbx_business_glossary_term' = 'Gacha Pool Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `game_edition_id` SET TAGS ('dbx_business_glossary_term' = 'Game Edition Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `game_title_id` SET TAGS ('dbx_business_glossary_term' = 'Game Title Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `licensed_ip_id` SET TAGS ('dbx_business_glossary_term' = 'Licensed Ip Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `live_ops_cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Live Ops Cycle Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `loot_box_disclosure_id` SET TAGS ('dbx_business_glossary_term' = 'Loot Box Disclosure Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `mtx_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Mtx Catalog Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `localization_string_id` SET TAGS ('dbx_business_glossary_term' = 'Name Localization String Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `parent_storefront_item_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `platform_sku_id` SET TAGS ('dbx_business_glossary_term' = 'Platform Sku Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `price_point_id` SET TAGS ('dbx_business_glossary_term' = 'Price Point Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `seasonal_event_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Event Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `team_id` SET TAGS ('dbx_business_glossary_term' = 'Team Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `tournament_id` SET TAGS ('dbx_business_glossary_term' = 'Tournament Id (Foreign Key)');
ALTER TABLE `gaming_ecm`.`monetization`.`storefront_item` ALTER COLUMN `total_revenue` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` SET TAGS ('dbx_subdomain' = 'subscription_services');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` SET TAGS ('dbx_association_edges' = 'monetization.subscription_plan,monetization.dlc_entitlement');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `plan_content_inclusion_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Content Inclusion Identifier');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `dlc_entitlement_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Content Inclusion - Dlc Entitlement Id');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `subscription_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Plan Content Inclusion - Subscription Plan Id');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `access_level` SET TAGS ('dbx_business_glossary_term' = 'Subscriber Access Level');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `content_unlock_sequence` SET TAGS ('dbx_business_glossary_term' = 'Content Unlock Sequence Number');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `exclusion_date` SET TAGS ('dbx_business_glossary_term' = 'Content Exclusion Date');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `included_since_date` SET TAGS ('dbx_business_glossary_term' = 'Content Inclusion Start Date');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `inclusion_status` SET TAGS ('dbx_business_glossary_term' = 'Inclusion Status');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `is_exclusive_to_plan` SET TAGS ('dbx_business_glossary_term' = 'Plan Exclusivity Flag');
ALTER TABLE `gaming_ecm`.`monetization`.`plan_content_inclusion` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Modification Timestamp');
