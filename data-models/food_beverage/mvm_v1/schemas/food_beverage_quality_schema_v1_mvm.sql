-- Schema for Domain: quality | Business: Food Beverage | Version: v1_mvm
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`haccp_plan` (
    ...
    `bill_of_materials_id` BIGINT COMMENT 'Foreign key linking to product.bill_of_materials. Business justification: HACCP hazard analysis requires ingredient-level review via the BOM — each component is assessed for biological, chemical, and physical hazards. Codex Alimentarius HACCP Principle 1 mandates ingredient',
    ...
    `center_id` BIGINT COMMENT 'Foreign key linking to distribution.center. Business justification: Distribution centers handling refrigerated, frozen, or allergen-containing products must operate under a facility-specific HACCP plan (FDA 21 CFR Part 117, FSMA PCHF rule). Linking haccp_plan directly',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: HACCP plan development, validation studies, and revalidation activities incur costs (consultant fees, lab studies, documentation) charged to cost centers. F&B food safety managers track HACCP program ',
    ...
    `food_safety_plan_id` BIGINT COMMENT 'Foreign key linking to regulatory.food_safety_plan. Business justification: HACCP plans implement the regulatory food safety plan (FSMA Preventive Controls). FDA requires HACCP plans to be traceable to the food safety plan on file at the establishment. A food safety expert wo',
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: HACCP plans are formulation-dependent — processing parameters, CCPs, and hazard controls are defined per formulation version. Any formulation change triggers mandatory HACCP revalidation (FDA FSMA 21 ',
    `network_node_id` BIGINT COMMENT 'Foreign key linking to supply.network_node. Business justification: HACCP plan scoping: HACCP plans are developed for specific processing facilities, including co-packers and contract manufacturing nodes in the supply network. Linking haccp_plan to network_node enable',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: FDA FSMA and Codex Alimentarius require HACCP plans to be scoped to specific products. F&B regulatory teams link HACCP plans to SKUs for product-specific hazard analysis, regulatory submissions, and a',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`critical_control_point` (
    ...
    `center_id` BIGINT COMMENT 'Foreign key linking to distribution.center. Business justification: Distribution centers have defined CCPs for receiving dock temperature checks, cold storage monitoring, and cross-contamination prevention (FSMA Sanitary Transportation, GFSI standards). critical_contr',
    ...
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: CCPs are defined based on formulation-specific processing parameters — pasteurization temperature for a dairy formulation differs from a juice formulation. Formulation changes require CCP revalidation',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`inspection_lot` (
    ...
    `bill_of_materials_id` BIGINT COMMENT 'Foreign key linking to product.bill_of_materials. Business justification: Inspection lots in F&B manufacturing are tied to specific BOM versions — the BOM defines which components and quantities were used in the inspected batch. This link is essential for ingredient-level t',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Inspection activities (lab testing, analyst time, sampling materials) are charged to production or quality cost centers in F&B operations. Cost of quality reporting requires inspection lot costs to be',
    `critical_control_point_id` BIGINT COMMENT 'Foreign key linking to quality.critical_control_point. Business justification: In SAP QM-aligned food safety systems, inspection lots are often created at specific Critical Control Points in the production process (e.g., incoming material inspection at a CCP, in-process inspecti',
    ...
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.goods_receipt. Business justification: Incoming inspection lots in F&B are created directly from goods receipts (goods_receipt.quality_inspection_lot_number is a denormalized reference). A proper FK enables traceability from inspection res',
    ...
    `production_order_id` BIGINT COMMENT 'Foreign key linking to manufacturing.production_order. Business justification: In F&B MES/QMS integration, inspection lots are created for specific production orders (in-process and finished goods inspection). Linking inspection_lot to production_order enables all inspection re',
    `shipment_id` BIGINT COMMENT 'Foreign key linking to distribution.shipment. Business justification: Inspection lots are created for inbound/outbound shipments to verify product quality at receiving or dispatch (GMP, FSMA receiving verification). Linking inspection_lot to the specific shipment being ',
    ...
    `specification_id` BIGINT COMMENT 'Foreign key linking to quality.specification. Business justification: An inspection lot is evaluated against a quality specification (finished goods, raw material, or WIP spec). Adding specification_id normalizes the relationship. The existing specification_version STRI',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`non_conformance` (
    ...
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: NCRs in F&B are raised at batch level — more granular than production order. non_conformance has production_order_id but not batch_record_id. Batch-level NCR traceability is required for CAPA investig',
    `capacity_constraint_id` BIGINT COMMENT 'Foreign key linking to supply.capacity_constraint. Business justification: Quality-driven capacity impact: a non-conformance event (CCP deviation, contamination, allergen breach) triggers a production hold that manifests as a capacity constraint in the supply plan. Linking n',
    ...
    `customer_complaint_id` BIGINT COMMENT 'Foreign key linking to quality.customer_complaint. Business justification: Customer complaints frequently trigger formal non-conformance investigations. Adding customer_complaint_id to non_conformance provides direct traceability from the NCR back to the originating complain',
    `food_safety_audit_id` BIGINT COMMENT 'Foreign key linking to quality.food_safety_audit. Business justification: Food safety audits (SQF, BRC, FSSC 22000) generate non-conformance findings that are recorded as NCRs. Adding food_safety_audit_id to non_conformance enables direct traceability from an NCR back to th',
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: NCRs in F&B frequently trace to formulation deviations (incorrect ingredient ratio, wrong processing parameter). Linking NCR to formulation version enables PLM teams to identify which formulation vers',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Non-conformances generate scrap write-offs, rework charges, and quality loss postings to GL accounts in F&B ERP systems. Quality cost accounting requires direct GL account reference on NCR records for',
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.goods_receipt. Business justification: Incoming material NCRs in F&B are raised against specific goods receipts — linking enables supplier debit note processing, COA reconciliation, FSMA traceability, and temperature/condition-at-receipt e',
    ...
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: F&B deduction management links NCRs directly to the disputed invoice for credit memo issuance and financial resolution. Trade deduction workflows require NCR-to-invoice traceability to validate custom',
    `lot_trace_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_trace. Business justification: Non-conformances are raised against specific inventory lots. Linking NCR to lot_trace enables lot disposition workflows, recall impact analysis, and FSMA traceability reporting. non_conformance has ba',
    `micro_test_result_id` BIGINT COMMENT 'Foreign key linking to quality.micro_test_result. Business justification: A non-conformance event can be directly triggered by a failed microbiological test result (e.g., Listeria or Salmonella detection). Adding micro_test_result_id to non_conformance provides direct trace',
    `order_id` BIGINT COMMENT 'Foreign key linking to sales.order. Business justification: In F&B operations, non-conformances are frequently raised against a specific sales order (e.g., customer return, short-ship, quality defect on a delivered order). Order-level NCR tracking drives corre',
    `otif_record_id` BIGINT COMMENT 'Foreign key linking to distribution.otif_record. Business justification: Major retailer OTIF failures (Walmart, Target, Kroger) trigger formal non-conformance records and corrective action plans due to chargeback penalties and compliance requirements. Linking non_conforman',
    ...
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.purchase_order. Business justification: NCRs for purchased materials reference the PO directly for supplier claim/debit note processing, procurement corrective action, and spend impact reporting. F&B quality teams require PO reference on NC',
    ...
    `specification_id` BIGINT COMMENT 'Foreign key linking to quality.specification. Business justification: A non-conformance event is defined by a deviation from a quality specification. Adding specification_id to non_conformance normalizes the reference to the violated specification, enabling analysis of ',
    ...
    `work_center_id` BIGINT COMMENT 'Foreign key linking to manufacturing.work_center. Business justification: Non-conformances in F&B manufacturing are localized to specific work centers (e.g., metal contamination at filling WC, CIP failure at mixing WC). NCR frequency by work center is a standard quality o',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`capa` (
    ...
    `claim_id` BIGINT COMMENT 'Foreign key linking to marketing.claim. Business justification: In F&B, CAPAs are frequently raised to correct marketing claim substantiation failures (e.g., gluten-free claim compromised by cross-contact). The CAPA must reference the specific claim being correc',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: CAPA execution incurs labor, materials, and process change costs charged to cost centers for departmental quality cost reporting. F&B quality managers track CAPA spend by cost center to measure cost o',
    `customer_complaint_id` BIGINT COMMENT 'Identifier of the customer complaint that initiated this CAPA, if applicable.',
    `facility_inspection_id` BIGINT COMMENT 'Foreign key linking to regulatory.facility_inspection. Business justification: CAPAs are routinely initiated in response to FDA 483 observations or regulatory facility inspection findings. The CAPA must reference the triggering inspection for regulatory response tracking and clo',
    `food_safety_audit_id` BIGINT COMMENT 'Foreign key linking to quality.food_safety_audit. Business justification: The CAPA product description explicitly states it is linked to non-conformances, audit findings — confirming that CAPAs are directly triggered by food safety audit findings. Adding food_safety_audit',
    `food_safety_plan_id` BIGINT COMMENT 'Foreign key linking to regulatory.food_safety_plan. Business justification: FSMA requires corrective actions to reference the food safety plan under which the hazard was identified. CAPAs triggered by food safety plan deviations must be traceable to the specific plan. Regulat',
    ...
    `inbound_shipment_id` BIGINT COMMENT 'Foreign key linking to supply.inbound_shipment. Business justification: Supplier CAPA traceability: CAPAs triggered by inbound material failures (out-of-spec delivery, contamination) must reference the specific inbound shipment for root cause investigation and supplier co',
    `non_conformance_id` BIGINT COMMENT 'Identifier of the non‑conformance that triggered this CAPA, if applicable.',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: CAPAs in F&B are product-specific — effectiveness verification, regulatory reporting (FDA 483 responses), and CAPA closure require the affected SKU to be formally identified. capa.product_code is a de',
    `supplier_audit_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_audit. Business justification: CAPAs in F&B are frequently raised from supplier audit findings (capa_required_flag on supplier_audit). Linking CAPA to the triggering supplier audit enables audit closure tracking, CAPA effectiveness',
    `supplier_quality_assessment_id` BIGINT COMMENT 'Foreign key linking to quality.supplier_quality_assessment. Business justification: CAPAs are triggered by supplier quality assessment findings (e.g., failed supplier audits, low quality scores). Adding supplier_quality_assessment_id to capa enables traceability from corrective actio',
    `work_center_id` BIGINT COMMENT 'Foreign key linking to manufacturing.work_center. Business justification: CAPAs in F&B manufacturing are frequently tied to the work center where the root cause occurred (e.g., CIP failure, metal detector malfunction). All open CAPAs by work center is a standard quality o',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`micro_test_result` (
    ...
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: Microbiological test results must be traceable to the specific batch sampled — a core FDA/FSMA requirement for environmental monitoring and finished product testing. batch_number is denormalized on mi',
    `cold_chain_event_id` BIGINT COMMENT 'Foreign key linking to distribution.cold_chain_event. Business justification: Temperature abuse during distribution accelerates microbial growth — when a cold chain excursion is detected, microbiological testing is mandated to assess product safety before disposition (FSMA, HAC',
    `critical_control_point_id` BIGINT COMMENT 'Foreign key linking to quality.critical_control_point. Business justification: Microbiological sampling in food manufacturing is frequently performed at Critical Control Points (e.g., pasteurization, cooking, chilling). Adding critical_control_point_id to micro_test_result links',
    `equipment_master_id` BIGINT COMMENT 'Identifier of the equipment used for the analysis.',
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: Microbiological testing targets (APC, Listeria, Salmonella limits) are formulation-specific — water activity, pH, and preservative levels in the formulation determine micro risk thresholds. F&B lab te',
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.goods_receipt. Business justification: Microbiological testing in F&B is routinely performed on incoming raw materials at goods receipt (pathogen testing of produce, dairy, proteins). Linking micro results to goods receipt provides FSMA FS',
    `inbound_shipment_id` BIGINT COMMENT 'Foreign key linking to supply.inbound_shipment. Business justification: Inbound microbiological testing: raw materials and ingredients require micro testing (Salmonella, Listeria, TPC) upon receipt before release to production. micro_test_result.shipment_id references dis',
    `inspection_lot_id` BIGINT COMMENT 'Foreign key linking to quality.inspection_lot. Business justification: In SAP QM-aligned food safety systems, microbiological test results are generated as part of an inspection lot. A micro test result belongs to a specific inspection lot, enabling traceability from lot',
    `lot_trace_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_trace. Business justification: Microbial test results are performed on samples drawn from specific inventory lots. Lab managers and food safety teams need to link micro results to lot_trace for FSMA one-up-one-down traceability and',
    ...
    `stock_position_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_position. Business justification: Micro test samples are drawn from specific stock positions. Lab managers need to link micro results to the sampled stock position to trigger quantity_quality_inspection updates and disposition decisio',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` (
    ...
    `carrier_id` BIGINT COMMENT 'Foreign key linking to distribution.carrier. Business justification: FSMA Sanitary Transportation rule (21 CFR Part 1, Subpart O) requires shippers to audit carriers for food safety compliance. carrier already tracks last_audit_date and fsma_compliant_flag — linking fo',
    ...
    `claim_id` BIGINT COMMENT 'Foreign key linking to marketing.claim. Business justification: Food safety audits (BRC, SQF, FSSC 22000) directly validate the substantiation basis for certification-based marketing claims. Audit outcomes determine whether claims like GFSI certified or food sa',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Food safety audits incur direct costs (auditor fees, travel, certification fees) charged to cost centers for departmental audit budget tracking. F&B quality finance reporting requires cost center assi',
    ...
    `food_safety_plan_id` BIGINT COMMENT 'Foreign key linking to regulatory.food_safety_plan. Business justification: Food safety audits (BRC, SQF, FSSC 22000) directly audit compliance against the facilitys food safety plan. The audit record must reference which food safety plan was audited. Certification bodies an',
    ...
    `haccp_plan_id` BIGINT COMMENT 'Foreign key linking to quality.haccp_plan. Business justification: Food safety audits (SQF, BRC, FSSC 22000) explicitly review and verify HACCP plans as a core audit element. Adding haccp_plan_id to food_safety_audit links the audit to the specific HACCP plan being r',
    `network_node_id` BIGINT COMMENT 'Foreign key linking to supply.network_node. Business justification: Network node qualification: food safety audits (BRC, SQF, FSSC 22000) are conducted at specific supply network nodes (DCs, co-packers, 3PLs). Audit certification status determines whether a node is ap',
    `plant_id` BIGINT COMMENT 'Identifier of the production or distribution facility being audited.',
    `production_line_id` BIGINT COMMENT 'Foreign key linking to manufacturing.production_line. Business justification: BRC, SQF, and FSSC 22000 audits in F&B are frequently scoped to specific production lines (e.g., audit of aseptic filling line). food_safety_audit has plant_id but no production_line_id. Line-level ',
    `storage_location_id` BIGINT COMMENT 'Foreign key linking to inventory.storage_location. Business justification: Food safety audits (BRC, SQF, FSSC 22000) are conducted at specific storage locations (warehouses, cold stores, DCs). Linking food_safety_audit to storage_location enables audit scheduling per locatio',
    `supplier_audit_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_audit. Business justification: Food safety audits conducted at supplier facilities (FSSC 22000, SQF) are coordinated with procurements supplier audit program. Linking these records enables joint audit scheduling, shared finding ma',
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_site. Business justification: Food safety audits in F&B are conducted at specific supplier sites. Certificate numbers, HACCP verification, and corrective action due dates are site-specific. A domain expert expects food_safety_audi',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`specification` (
    ...
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: Quality specifications (pH, brix, moisture, fat%) are derived directly from formulation targets. When a formulation version changes, specifications must be updated and reapproved. F&B PLM/quality team',
    `nutrition_label_id` BIGINT COMMENT 'Foreign key linking to regulatory.nutrition_label. Business justification: Specification nutritional attributes (fat_percent, protein_percent, sodium_mg_per_kg, brix_percent) must reconcile with the approved nutrition label. Label-spec reconciliation is a mandatory pre-launc',
    `packaging_spec_id` BIGINT COMMENT 'Foreign key linking to product.packaging_spec. Business justification: Packaging quality specifications (seal integrity, barrier properties, headspace) are defined per packaging spec. F&B quality teams conduct incoming packaging inspection against these specs. Linking qu',
    `product_registration_id` BIGINT COMMENT 'Foreign key linking to regulatory.product_registration. Business justification: Product specifications must align with regulatory product registrations — allergen declarations, shelf life, and storage conditions on the spec must match the registered product dossier. Regulatory co',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`customer_complaint` (
    ...
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: Customer complaints in F&B are investigated at batch level — which batch caused this complaint? batch_number is denormalized on customer_complaint. A proper FK to batch_record enables structured com',
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Customer complaint handling costs (replacement product, investigation labor, credit processing) are charged to cost centers for quality KPI and cost-of-poor-quality reporting. F&B quality teams track ',
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: Customer complaints about taste, texture, or appearance trace back to specific formulation versions. F&B PLM and quality teams track complaint rates by formulation version to identify whether a formul',
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: Customer complaints in F&B frequently reference a specific invoice for financial deduction or credit memo processing. Complaint-to-invoice linkage is required for accounts receivable dispute resolutio',
    `lot_trace_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_trace. Business justification: Customer complaints reference specific product lots for root cause investigation and potential recall triggering. Linking customer_complaint to lot_trace enables complaint-to-lot traceability required',
    ...
    `ship_to_location_id` BIGINT COMMENT 'Foreign key linking to customer.ship_to_location. Business justification: Complaint investigation requires knowing the specific delivery location (retail store, DC) where defective product was received or consumed. Field quality teams use this to conduct on-site investigati',
    ...
    `store_id` BIGINT COMMENT 'Foreign key linking to sales.store. Business justification: F&B complaint investigations require knowing the specific retail store where the product was purchased. Store-level complaint clustering drives field quality reviews, retailer scorecards, and root-cau',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`product_recall` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: FDA 21 CFR Part 7 recall regulations require direct notification to consignee customer accounts. Recall managers need to identify which accounts received affected lots to issue notifications, track ac',
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: FDA/USDA recall regulations require tracing affected product to specific manufacturing batches. product_recall has affected_lot_codes as plain text but no FK to batch_record. A proper FK enables struc',
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Product recalls incur direct costs (logistics, destruction, customer credits) charged to specific cost centers for recall cost management reporting. F&B finance teams require cost center assignment on',
    `customer_complaint_id` BIGINT COMMENT 'Foreign key linking to quality.customer_complaint. Business justification: Product recalls can be initiated directly by consumer complaints (e.g., illness reports, foreign object complaints). Adding customer_complaint_id to product_recall enables traceability from the recall',
    `formulation_id` BIGINT COMMENT 'Foreign key linking to product.formulation. Business justification: Product recalls in F&B are frequently caused by formulation issues — undeclared allergens, incorrect ingredient substitution, or formulation errors. FDA 21 CFR Part 7 recall reporting requires identif',
    ...
    `inbound_shipment_id` BIGINT COMMENT 'Foreign key linking to supply.inbound_shipment. Business justification: Recall inbound traceability: a recall may require intercepting in-transit inbound shipments of affected raw materials or finished goods. Linking product_recall to inbound_shipment enables recall coord',
    `lot_trace_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_trace. Business justification: Product recalls target specific inventory lots. Linking product_recall to lot_trace enables automated lot quarantine triggering and recall scope quantification. affected_lot_codes is a denormalized pl',
    `non_conformance_id` BIGINT COMMENT 'Foreign key linking to quality.non_conformance. Business justification: Product recalls are initiated based on non-conformance events (e.g., contamination, allergen mislabeling, foreign material). Adding non_conformance_id to product_recall provides the causal link from t',
    `quarantine_hold_id` BIGINT COMMENT 'Foreign key linking to inventory.quarantine_hold. Business justification: When a recall is initiated, inventory quarantine holds are placed on affected stock. Linking product_recall to quarantine_hold provides the recall-to-quarantine audit trail required for FDA/USDA recal',
    `recall_event_id` BIGINT COMMENT 'Foreign key linking to regulatory.recall_event. Business justification: Internal quality recall records must be linked to the official regulatory recall event filed with FDA/USDA. Recall closure and effectiveness reporting requires tracing the internal quality action to t',
    ...
    `supplier_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier. Business justification: Product recalls in F&B frequently originate from supplier-sourced ingredient defects. Linking recall to the responsible supplier enables FDA/USDA regulatory reporting (21 CFR Part 7), supplier liabili',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` (
    ...
    `food_safety_audit_id` BIGINT COMMENT 'Foreign key linking to quality.food_safety_audit. Business justification: Supplier quality assessments are often conducted in conjunction with or reference food safety audits of the supplier facility. Adding food_safety_audit_id normalizes the relationship. The existing aud',
    `gfsi_certification_id` BIGINT COMMENT 'Foreign key linking to regulatory.gfsi_certification. Business justification: Supplier quality assessments evaluate and reference the suppliers GFSI certification (BRC, SQF, FSSC 22000) to verify validity and scope. Approved supplier list management and supplier risk tiering d',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Supplier quality assessments incur audit fees, third-party certification costs, and travel expenses posted to GL accounts. F&B procurement quality teams post assessment costs to specific GL accounts f',
    ...
    `supplier_audit_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_audit. Business justification: In F&B, the quality teams supplier quality assessment is directly linked to the procurement supplier audit event. Linking these enables unified audit-to-assessment traceability, CAPA closure tracking',
    `supplier_id` BIGINT COMMENT 'Unique identifier of the supplier being assessed.',
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_site. Business justification: GFSI/SQF/BRC assessments in F&B are conducted at a specific supplier manufacturing site, not just the supplier header. Site-level assessment drives approved vendor list status per site and regulatory ',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`hold_record` (
    ...
    `batch_record_id` BIGINT COMMENT 'Foreign key linking to manufacturing.batch_record. Business justification: Quality holds in F&B are placed on specific batches. hold_record has batch_number as a denormalized plain attribute but no FK to batch_record. Linking enables which batch is under hold? — critical f',
    `bill_of_materials_id` BIGINT COMMENT 'Foreign key linking to product.bill_of_materials. Business justification: Inventory holds in F&B are often triggered by BOM-level issues — a suspect ingredient lot affects all batches produced with that BOM version. Linking hold_record to bill_of_materials enables quality t',
    ...
    `cold_chain_event_id` BIGINT COMMENT 'Foreign key linking to distribution.cold_chain_event. Business justification: Cold chain excursion events are the most common trigger for quality holds on food products in distribution. Linking hold_record directly to the initiating cold_chain_event supports FSMA disposition de',
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Inventory holds result in write-offs, destruction costs, or rework charges posted to GL accounts in F&B ERP. Financial posting of hold-related losses requires direct GL account reference on hold recor',
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.goods_receipt. Business justification: Holds placed on incoming materials at goods receipt require reference to the receipt event for supplier notification, return-to-vendor processing, and regulatory documentation. F&B quality operations ',
    `inspection_lot_id` BIGINT COMMENT 'Foreign key linking to quality.inspection_lot. Business justification: A hold is often placed on a specific inspection lot that failed quality inspection. Adding inspection_lot_id to hold_record links the hold directly to the inspection lot, enabling traceability from lo',
    `lot_trace_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_trace. Business justification: Quality holds are placed on specific inventory lots. Linking hold_record to lot_trace enables FSMA one-up-one-down traceability and lot-level hold disposition reporting. hold_record has lot_number as ',
    `non_conformance_id` BIGINT COMMENT 'Foreign key linking to quality.non_conformance. Business justification: Product holds are frequently placed as a direct result of a non-conformance event. Adding non_conformance_id to hold_record provides the causal link between the quality event and the inventory action,',
    ...
    `product_recall_id` BIGINT COMMENT 'Foreign key linking to quality.product_recall. Business justification: During a product recall event, holds are placed on affected inventory. Adding product_recall_id to hold_record links the hold to the recall that triggered it, enabling complete recall scope tracking a',
    `quarantine_hold_id` BIGINT COMMENT 'Foreign key linking to inventory.quarantine_hold. Business justification: Quality hold_record (quality management layer) and inventory quarantine_hold (WMS/inventory layer) represent the same physical hold at different system layers. QA managers reconciling quality holds ag',
    `shipment_id` BIGINT COMMENT 'Foreign key linking to distribution.shipment. Business justification: Quality holds are routinely placed on in-transit or staged shipments (e.g., temperature excursion detected mid-route, regulatory stop-ship). A direct FK from hold_record to shipment enables the distri',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` (
    `haccp_plan_specification_id` BIGINT COMMENT 'Primary key for the HACCP plan specification linkage record',
    `haccp_plan_id` BIGINT COMMENT 'Foreign key linking to haccp_plan',
    `specification_id` BIGINT COMMENT 'Foreign key linking to specification',
    `applicability_scope` STRING COMMENT 'Scope of how this specification applies to the HACCP plan (e.g., critical limit, monitoring parameter, verification requirement). Identified in detection phase relationship data.',
    `critical_limit_reference` STRING COMMENT 'Reference to the specific critical limit parameter in the specification (e.g., pH, temperature, microbial count) that this HACCP plan monitors. Identified in detection phase relationship data.',
    `effective_date` DATE COMMENT 'Date when this specification became applicable to this HACCP plan. Identified in detection phase relationship data.',
    `expiration_date` DATE COMMENT 'Date when this linkage expires or is superseded (nullable for ongoing linkages)',
    `hazard_category_mapping` STRING COMMENT 'Mapping of which hazard categories (biological, chemical, physical, radiological) from the HACCP plan are controlled by this specification. Identified in detection phase relationship data.',
    `linkage_status` STRING COMMENT 'Current status of this HACCP plan-specification linkage (active, superseded, under review, deprecated)',
    `linked_by` STRING COMMENT 'Name or ID of the food safety professional who established this linkage',
    `linked_timestamp` TIMESTAMP COMMENT 'Timestamp when this HACCP plan-specification linkage was created',
    `notes` STRING COMMENT 'Free-form notes about this specific HACCP plan-specification linkage',
    `regulatory_citation` STRING COMMENT 'Specific regulatory requirement (FDA FSMA, USDA FSIS, Codex) that mandates this linkage',
    `version_at_plan_approval` STRING COMMENT 'Version number of the specification that was in effect when the HACCP plan was approved. Critical for regulatory audit trail. Identified in detection phase relationship data.',
    CONSTRAINT pk_haccp_plan_specification PRIMARY KEY(`haccp_plan_specification_id`)
) COMMENT 'This association product represents the regulatory linkage between HACCP plans and quality specifications. It captures the formal relationship between hazard control plans and the critical limit specifications they reference. Each record links one HACCP plan to one specification with effective dates, version tracking, and hazard category mapping that exist only in the context of this regulatory compliance relationship.. Existence Justification: In food manufacturing operations, HACCP plans reference multiple quality specifications to define critical limits for different hazard control points (e.g., temperature specs for thermal processing, pH specs for acidified foods, microbial limits for finished products), and each specification is referenced by multiple HACCP plans across different facilities, product lines, and process stages. The FDA FSMA Preventive Controls rule and USDA HACCP regulations require documented, version-controlled linkage between HACCP hazard analyses and the specifications that define critical limits, making this a regulatory compliance requirement that food safety teams actively manage.';
...
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ADD CONSTRAINT `fk_quality_inspection_lot_critical_control_point_id` FOREIGN KEY (`critical_control_point_id`) REFERENCES `food_beverage_ecm`.`quality`.`critical_control_point`(`critical_control_point_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ADD CONSTRAINT `fk_quality_inspection_lot_specification_id` FOREIGN KEY (`specification_id`) REFERENCES `food_beverage_ecm`.`quality`.`specification`(`specification_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ADD CONSTRAINT `fk_quality_non_conformance_customer_complaint_id` FOREIGN KEY (`customer_complaint_id`) REFERENCES `food_beverage_ecm`.`quality`.`customer_complaint`(`customer_complaint_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ADD CONSTRAINT `fk_quality_non_conformance_food_safety_audit_id` FOREIGN KEY (`food_safety_audit_id`) REFERENCES `food_beverage_ecm`.`quality`.`food_safety_audit`(`food_safety_audit_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ADD CONSTRAINT `fk_quality_non_conformance_micro_test_result_id` FOREIGN KEY (`micro_test_result_id`) REFERENCES `food_beverage_ecm`.`quality`.`micro_test_result`(`micro_test_result_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ADD CONSTRAINT `fk_quality_non_conformance_specification_id` FOREIGN KEY (`specification_id`) REFERENCES `food_beverage_ecm`.`quality`.`specification`(`specification_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ADD CONSTRAINT `fk_quality_capa_food_safety_audit_id` FOREIGN KEY (`food_safety_audit_id`) REFERENCES `food_beverage_ecm`.`quality`.`food_safety_audit`(`food_safety_audit_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ADD CONSTRAINT `fk_quality_capa_supplier_quality_assessment_id` FOREIGN KEY (`supplier_quality_assessment_id`) REFERENCES `food_beverage_ecm`.`quality`.`supplier_quality_assessment`(`supplier_quality_assessment_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ADD CONSTRAINT `fk_quality_micro_test_result_critical_control_point_id` FOREIGN KEY (`critical_control_point_id`) REFERENCES `food_beverage_ecm`.`quality`.`critical_control_point`(`critical_control_point_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ADD CONSTRAINT `fk_quality_micro_test_result_inspection_lot_id` FOREIGN KEY (`inspection_lot_id`) REFERENCES `food_beverage_ecm`.`quality`.`inspection_lot`(`inspection_lot_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ADD CONSTRAINT `fk_quality_food_safety_audit_haccp_plan_id` FOREIGN KEY (`haccp_plan_id`) REFERENCES `food_beverage_ecm`.`quality`.`haccp_plan`(`haccp_plan_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ADD CONSTRAINT `fk_quality_product_recall_customer_complaint_id` FOREIGN KEY (`customer_complaint_id`) REFERENCES `food_beverage_ecm`.`quality`.`customer_complaint`(`customer_complaint_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ADD CONSTRAINT `fk_quality_product_recall_non_conformance_id` FOREIGN KEY (`non_conformance_id`) REFERENCES `food_beverage_ecm`.`quality`.`non_conformance`(`non_conformance_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` ADD CONSTRAINT `fk_quality_supplier_quality_assessment_food_safety_audit_id` FOREIGN KEY (`food_safety_audit_id`) REFERENCES `food_beverage_ecm`.`quality`.`food_safety_audit`(`food_safety_audit_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ADD CONSTRAINT `fk_quality_hold_record_inspection_lot_id` FOREIGN KEY (`inspection_lot_id`) REFERENCES `food_beverage_ecm`.`quality`.`inspection_lot`(`inspection_lot_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ADD CONSTRAINT `fk_quality_hold_record_non_conformance_id` FOREIGN KEY (`non_conformance_id`) REFERENCES `food_beverage_ecm`.`quality`.`non_conformance`(`non_conformance_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ADD CONSTRAINT `fk_quality_hold_record_product_recall_id` FOREIGN KEY (`product_recall_id`) REFERENCES `food_beverage_ecm`.`quality`.`product_recall`(`product_recall_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ADD CONSTRAINT `fk_quality_haccp_plan_specification_haccp_plan_id` FOREIGN KEY (`haccp_plan_id`) REFERENCES `food_beverage_ecm`.`quality`.`haccp_plan`(`haccp_plan_id`);
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ADD CONSTRAINT `fk_quality_haccp_plan_specification_specification_id` FOREIGN KEY (`specification_id`) REFERENCES `food_beverage_ecm`.`quality`.`specification`(`specification_id`);
...
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` SET TAGS ('dbx_subdomain' = 'safety_compliance');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `bill_of_materials_id` SET TAGS ('dbx_business_glossary_term' = 'Bill Of Materials Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `center_id` SET TAGS ('dbx_business_glossary_term' = 'Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `food_safety_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `network_node_id` SET TAGS ('dbx_business_glossary_term' = 'Network Node Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`critical_control_point` SET TAGS ('dbx_subdomain' = 'safety_compliance');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`critical_control_point` ALTER COLUMN `center_id` SET TAGS ('dbx_business_glossary_term' = 'Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`critical_control_point` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` SET TAGS ('dbx_subdomain' = 'testing_evaluation');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `bill_of_materials_id` SET TAGS ('dbx_business_glossary_term' = 'Bill Of Materials Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `critical_control_point_id` SET TAGS ('dbx_business_glossary_term' = 'Critical Control Point Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Goods Receipt Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `production_order_id` SET TAGS ('dbx_business_glossary_term' = 'Production Order Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Shipment Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`inspection_lot` ALTER COLUMN `specification_id` SET TAGS ('dbx_business_glossary_term' = 'Specification Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` SET TAGS ('dbx_subdomain' = 'issue_resolution');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `batch_record_id` SET TAGS ('dbx_business_glossary_term' = 'Batch Record Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `capacity_constraint_id` SET TAGS ('dbx_business_glossary_term' = 'Capacity Constraint Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `customer_complaint_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Complaint Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `food_safety_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Audit Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Goods Receipt Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `lot_trace_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Trace Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `micro_test_result_id` SET TAGS ('dbx_business_glossary_term' = 'Micro Test Result Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Order Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `otif_record_id` SET TAGS ('dbx_business_glossary_term' = 'Otif Record Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `specification_id` SET TAGS ('dbx_business_glossary_term' = 'Specification Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`non_conformance` ALTER COLUMN `work_center_id` SET TAGS ('dbx_business_glossary_term' = 'Work Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` SET TAGS ('dbx_subdomain' = 'issue_resolution');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `capa_id` SET TAGS ('dbx_business_glossary_term' = 'CAPA Identifier');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `facility_inspection_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Inspection Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `food_safety_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Audit Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `food_safety_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `inbound_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Shipment Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `supplier_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Audit Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `supplier_quality_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Quality Assessment Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`capa` ALTER COLUMN `work_center_id` SET TAGS ('dbx_business_glossary_term' = 'Work Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` SET TAGS ('dbx_subdomain' = 'testing_evaluation');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `batch_record_id` SET TAGS ('dbx_business_glossary_term' = 'Batch Record Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `cold_chain_event_id` SET TAGS ('dbx_business_glossary_term' = 'Cold Chain Event Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `critical_control_point_id` SET TAGS ('dbx_business_glossary_term' = 'Critical Control Point Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Goods Receipt Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `inbound_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Shipment Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `inspection_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Lot Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `lot_trace_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Trace Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`micro_test_result` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` SET TAGS ('dbx_subdomain' = 'safety_compliance');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `food_safety_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Plan Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `haccp_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Haccp Plan Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `network_node_id` SET TAGS ('dbx_business_glossary_term' = 'Network Node Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `production_line_id` SET TAGS ('dbx_business_glossary_term' = 'Production Line Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `storage_location_id` SET TAGS ('dbx_business_glossary_term' = 'Storage Location Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `supplier_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Audit Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`food_safety_audit` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`specification` SET TAGS ('dbx_subdomain' = 'testing_evaluation');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`specification` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`specification` ALTER COLUMN `nutrition_label_id` SET TAGS ('dbx_business_glossary_term' = 'Nutrition Label Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`specification` ALTER COLUMN `packaging_spec_id` SET TAGS ('dbx_business_glossary_term' = 'Packaging Spec Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`specification` ALTER COLUMN `product_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Product Registration Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` SET TAGS ('dbx_subdomain' = 'issue_resolution');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `batch_record_id` SET TAGS ('dbx_business_glossary_term' = 'Batch Record Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `lot_trace_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Trace Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `ship_to_location_id` SET TAGS ('dbx_business_glossary_term' = 'Ship To Location Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`customer_complaint` ALTER COLUMN `store_id` SET TAGS ('dbx_business_glossary_term' = 'Store Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` SET TAGS ('dbx_subdomain' = 'safety_compliance');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `batch_record_id` SET TAGS ('dbx_business_glossary_term' = 'Batch Record Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `customer_complaint_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Complaint Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `formulation_id` SET TAGS ('dbx_business_glossary_term' = 'Formulation Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `inbound_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Shipment Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `lot_trace_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Trace Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `non_conformance_id` SET TAGS ('dbx_business_glossary_term' = 'Non Conformance Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `quarantine_hold_id` SET TAGS ('dbx_business_glossary_term' = 'Quarantine Hold Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `recall_event_id` SET TAGS ('dbx_business_glossary_term' = 'Recall Event Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`product_recall` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` SET TAGS ('dbx_subdomain' = 'testing_evaluation');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` ALTER COLUMN `food_safety_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Food Safety Audit Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` ALTER COLUMN `gfsi_certification_id` SET TAGS ('dbx_business_glossary_term' = 'Gfsi Certification Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` ALTER COLUMN `supplier_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Audit Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`supplier_quality_assessment` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` SET TAGS ('dbx_subdomain' = 'issue_resolution');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `batch_record_id` SET TAGS ('dbx_business_glossary_term' = 'Batch Record Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `bill_of_materials_id` SET TAGS ('dbx_business_glossary_term' = 'Bill Of Materials Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `cold_chain_event_id` SET TAGS ('dbx_business_glossary_term' = 'Cold Chain Event Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Goods Receipt Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `inspection_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Lot Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `lot_trace_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Trace Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `non_conformance_id` SET TAGS ('dbx_business_glossary_term' = 'Non Conformance Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `product_recall_id` SET TAGS ('dbx_business_glossary_term' = 'Product Recall Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `quarantine_hold_id` SET TAGS ('dbx_business_glossary_term' = 'Quarantine Hold Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`quality`.`hold_record` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Shipment Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` SET TAGS ('dbx_subdomain' = 'safety_compliance');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` SET TAGS ('dbx_association_edges' = 'quality.haccp_plan,quality.specification');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `haccp_plan_specification_id` SET TAGS ('dbx_business_glossary_term' = 'HACCP Plan Specification Linkage ID');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `haccp_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Haccp Plan Specification - Haccp Plan Id');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `specification_id` SET TAGS ('dbx_business_glossary_term' = 'Haccp Plan Specification - Specification Id');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `applicability_scope` SET TAGS ('dbx_business_glossary_term' = 'Applicability Scope');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `critical_limit_reference` SET TAGS ('dbx_business_glossary_term' = 'Critical Limit Reference');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Linkage Effective Date');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Linkage Expiration Date');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `hazard_category_mapping` SET TAGS ('dbx_business_glossary_term' = 'Hazard Category Mapping');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `linkage_status` SET TAGS ('dbx_business_glossary_term' = 'Linkage Status');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `linked_by` SET TAGS ('dbx_business_glossary_term' = 'Linked By');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `linked_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Linkage Timestamp');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Linkage Notes');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `regulatory_citation` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Citation');
ALTER TABLE `food_beverage_ecm`.`quality`.`haccp_plan_specification` ALTER COLUMN `version_at_plan_approval` SET TAGS ('dbx_business_glossary_term' = 'Specification Version at Plan Approval');
