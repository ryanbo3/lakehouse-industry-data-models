-- Schema for Domain: finance | Business: Food Beverage | Version: v1_mvm
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`gl_account` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: GL accounts in a chart of accounts are defined per company code (legal entity). The existing company_code (STRING) column in gl_account is a denormalized reference to the company_code master. Adding c',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`cost_center` (
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Cost centers are assigned to profit centers for internal P&L reporting — costs incurred by a cost center are attributed to its profit center. The existing profit_center_code (STRING) in cost_center is',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`journal_entry` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Journal entries are posted within a specific fiscal period managed by the fiscal_period product. Adding fiscal_period_id normalizes the fiscal_period (INT) and fiscal_year (INT) denormalized columns i',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`ap_invoice` (
    ...
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to finance.bank_account. Business justification: AP invoices are settled via a specific corporate bank account. Linking ap_invoice to bank_account enables cash flow forecasting, bank reconciliation, and payment run analysis by bank account. The exis',
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: AP invoices are posted and accrued within a fiscal period. The existing fiscal_period (STRING) and fiscal_year (STRING) columns are denormalized references to the fiscal_period master. Adding fiscal_p',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: AP invoices are posted to a GL account for expense recognition. The existing gl_account_code (STRING) is a denormalized reference to the gl_account master. Adding gl_account_id normalizes this to a pr',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`ar_invoice` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: AR invoices are recognized and reported within a fiscal period. The existing fiscal_period (STRING) column is a denormalized reference. Adding fiscal_period_id enables revenue recognition period valid',
    ...
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: The AR invoice is the financial representation of the sales invoice in F&B ERP systems. Linking ar_invoice to sales.invoice enables AR-to-billing reconciliation, dispute resolution, and deduction mana',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`finance_standard_cost` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Standard costs are defined and released per fiscal period. The existing fiscal_period (INT) column is a denormalized reference. Adding fiscal_period_id links standard cost records to the authoritative',
    ...
    `product_registration_id` BIGINT COMMENT 'Foreign key linking to regulatory.product_registration. Business justification: Standard costing for registered products requires linking cost records to regulatory-approved SKUs for export pricing, tariff classification, transfer pricing compliance, and duty drawback calculation',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Finance standard cost records are maintained per SKU for price release, inventory valuation, and COGS calculation. material_number is a denormalized SKU reference; replacing with sku_id FK enables sta',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`budget` (
    `budget_id` BIGINT COMMENT 'Unique identifier for the budget record. Primary key for the budget data product.',
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: F&B financial budgets are set at product category level (snacks, beverages, dairy). product_category_code is a denormalized category reference; replacing with category_id FK enables category-level bud',
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Budget records are defined per fiscal period. The existing fiscal_period (INT) column is a denormalized reference. Adding fiscal_period_id links budget records to the authoritative fiscal period maste',
    ...
    `territory_id` BIGINT COMMENT 'Foreign key linking to sales.territory. Business justification: Revenue budgets in F&B are set by sales territory for quota allocation and performance management. Linking finance_budget to territory enables budget-vs-actual revenue analysis by territory, a standar',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`forecast` (
    ...
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: F&B financial forecasts (rolling forecasts, IBP) are structured by product category. product_category_code is a denormalized reference; replacing with category_id FK enables category-level forecast ac',
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Forecasts are projected per fiscal period. The existing fiscal_period (STRING) column is a denormalized reference. Adding fiscal_period_id links forecast records to the authoritative fiscal period mas',
    ...
    `territory_id` BIGINT COMMENT 'Foreign key linking to sales.territory. Business justification: F&B financial forecasts are built bottom-up from territory-level sales forecasts. Linking finance.forecast to sales.territory enables territory-level forecast-vs-actual revenue reporting and S&OP alig',
...
CREATE OR REPLACE TABLE `food_beverage_ecm`.`finance`.`tax_record` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Tax records are attributed to cost centers for internal cost allocation and reporting. The existing cost_center_code (STRING) in tax_record is a denormalized reference to the cost_center master. Addin',
    `establishment_registration_id` BIGINT COMMENT 'Foreign key linking to regulatory.establishment_registration. Business justification: Facility-level taxes (property tax, local business tax, environmental fees) require direct establishment link for jurisdiction-specific compliance reporting, tax authority audits, and reconciliation o',
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Tax records (VAT, GST, excise, sugar tax) are posted and reported within a fiscal period. The existing fiscal_period (INT) column is a denormalized reference. Adding fiscal_period_id links tax posting',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Tax postings (VAT, GST, excise, sugar tax) are recorded against specific GL accounts in the chart of accounts. The existing gl_account_code (STRING) in tax_record is a denormalized reference to the gl',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Tax records are attributed to profit centers for internal P&L reporting and tax allocation by business segment. The existing profit_center_code (STRING) in tax_record is a denormalized reference to th',
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: Tax records in F&B are generated from sales invoices (VAT, GST on customer billings). tax_record links to ar_invoice but not directly to sales.invoice, which is the source billing document for custome',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `food_beverage_ecm`.`finance`.`gl_account` ADD CONSTRAINT `fk_finance_gl_account_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `food_beverage_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`cost_center` ADD CONSTRAINT `fk_finance_cost_center_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `food_beverage_ecm`.`finance`.`profit_center`(`profit_center_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`journal_entry` ADD CONSTRAINT `fk_finance_journal_entry_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_bank_account_id` FOREIGN KEY (`bank_account_id`) REFERENCES `food_beverage_ecm`.`finance`.`bank_account`(`bank_account_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_gl_account_id` FOREIGN KEY (`gl_account_id`) REFERENCES `food_beverage_ecm`.`finance`.`gl_account`(`gl_account_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ar_invoice` ADD CONSTRAINT `fk_finance_ar_invoice_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`finance_standard_cost` ADD CONSTRAINT `fk_finance_finance_standard_cost_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ADD CONSTRAINT `fk_finance_budget_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `food_beverage_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ADD CONSTRAINT `fk_finance_budget_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `food_beverage_ecm`.`finance`.`cost_center`(`cost_center_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ADD CONSTRAINT `fk_finance_budget_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ADD CONSTRAINT `fk_finance_budget_gl_account_id` FOREIGN KEY (`gl_account_id`) REFERENCES `food_beverage_ecm`.`finance`.`gl_account`(`gl_account_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ADD CONSTRAINT `fk_finance_budget_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `food_beverage_ecm`.`finance`.`profit_center`(`profit_center_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`forecast` ADD CONSTRAINT `fk_finance_forecast_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ADD CONSTRAINT `fk_finance_tax_record_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `food_beverage_ecm`.`finance`.`cost_center`(`cost_center_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ADD CONSTRAINT `fk_finance_tax_record_fiscal_period_id` FOREIGN KEY (`fiscal_period_id`) REFERENCES `food_beverage_ecm`.`finance`.`fiscal_period`(`fiscal_period_id`);
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ADD CONSTRAINT `fk_finance_tax_record_gl_account_id` FOREIGN KEY (`gl_account_id`) REFERENCES `food_beverage_ecm`.`finance`.`gl_account`(`gl_account_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ADD CONSTRAINT `fk_finance_tax_record_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `food_beverage_ecm`.`finance`.`profit_center`(`profit_center_id`);
...
ALTER TABLE `food_beverage_ecm`.`finance`.`gl_account` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`gl_account` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`cost_center` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`cost_center` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`profit_center` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`company_code` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`journal_entry` SET TAGS ('dbx_subdomain' = 'transaction_processing');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`journal_entry` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` SET TAGS ('dbx_subdomain' = 'transaction_processing');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`ap_invoice` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ar_invoice` SET TAGS ('dbx_subdomain' = 'transaction_processing');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ar_invoice` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`ar_invoice` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Invoice Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`finance_standard_cost` SET TAGS ('dbx_subdomain' = 'asset_management');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`finance_standard_cost` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`finance_standard_cost` ALTER COLUMN `product_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Product Registration Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`finance_standard_cost` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` SET TAGS ('dbx_subdomain' = 'accounting_structure');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `budget_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Identifier (ID)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Amount');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Budget Approval Date');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Budget Approval Status');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'DRAFT|SUBMITTED|APPROVED|REJECTED|LOCKED');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `budget_category` SET TAGS ('dbx_business_glossary_term' = 'Budget Category');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `budget_category` SET TAGS ('dbx_value_regex' = 'ANNUAL|ROLLING|SUPPLEMENTAL|CONTINGENCY');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `budget_type` SET TAGS ('dbx_business_glossary_term' = 'Budget Type');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `budget_type` SET TAGS ('dbx_value_regex' = 'OPEX|CAPEX|REVENUE|HEADCOUNT|COGS|TRADE_SPEND');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `business_area_code` SET TAGS ('dbx_business_glossary_term' = 'Business Area Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `business_area_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `controlling_area_code` SET TAGS ('dbx_business_glossary_term' = 'Controlling Area Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `controlling_area_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `distribution_channel_code` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `distribution_channel_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `functional_area_code` SET TAGS ('dbx_business_glossary_term' = 'Functional Area Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `functional_area_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,6}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `geographic_region_code` SET TAGS ('dbx_business_glossary_term' = 'Geographic Region Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `geographic_region_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,6}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `is_carry_forward` SET TAGS ('dbx_business_glossary_term' = 'Is Carry Forward Flag');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `is_locked` SET TAGS ('dbx_business_glossary_term' = 'Is Budget Locked Flag');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Budget Notes');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `plant_code` SET TAGS ('dbx_business_glossary_term' = 'Plant Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `plant_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `quantity` SET TAGS ('dbx_business_glossary_term' = 'Budget Quantity');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `quantity_unit` SET TAGS ('dbx_business_glossary_term' = 'Quantity Unit of Measure');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `sales_org_code` SET TAGS ('dbx_business_glossary_term' = 'Sales Organization Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `sales_org_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `source_record_key` SET TAGS ('dbx_business_glossary_term' = 'Source Record Key');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Identifier (ID)');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `version_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Version Code');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `version_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{1,10}$');
ALTER TABLE `food_beverage_ecm`.`finance`.`budget` ALTER COLUMN `version_name` SET TAGS ('dbx_business_glossary_term' = 'Budget Version Name');
ALTER TABLE `food_beverage_ecm`.`finance`.`forecast` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `food_beverage_ecm`.`finance`.`forecast` SET TAGS ('dbx_subdomain' = 'asset_management');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`forecast` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`forecast` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`forecast` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`fixed_asset` SET TAGS ('dbx_subdomain' = 'asset_management');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`fiscal_period` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` SET TAGS ('dbx_subdomain' = 'transaction_processing');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ALTER COLUMN `establishment_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Establishment Registration Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `food_beverage_ecm`.`finance`.`tax_record` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Invoice Id (Foreign Key)');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`bank_account` SET TAGS ('dbx_subdomain' = 'transaction_processing');
...
ALTER TABLE `food_beverage_ecm`.`finance`.`bank_account` ALTER COLUMN `cash_pool_header_bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `food_beverage_ecm`.`finance`.`bank_account` ALTER COLUMN `cash_pool_header_bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
