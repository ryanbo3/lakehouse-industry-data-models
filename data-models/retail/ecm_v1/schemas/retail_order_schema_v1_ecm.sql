-- Schema for Domain: order | Business: Retail | Version: v1_ecm
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`order_line` (
    ...
    `replenishment_plan_id` BIGINT COMMENT 'Foreign key linking to supplychain.replenishment_plan. Business justification: Promotional merchandise pre-positioning and seasonal demand planning require linking actual order lines to the replenishment plans that forecasted them. Enables forecast accuracy measurement, safety s',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`status_history` (
    ...
    `associate_id` BIGINT COMMENT 'Identifier of the user or service account that triggered the status change. Null for automated system transitions. Supports audit and accountability requirements.',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`line_status_history` (
    `line_status_history_id` BIGINT COMMENT 'Unique identifier for each status transition event in the order line lifecycle. Primary key for the status history log.',
    `carrier_id` BIGINT COMMENT 'Identifier of the shipping carrier assigned to this line for delivery (e.g., FedEx, UPS, USPS, regional courier). Populated when status transitions to shipped or in_transit.',
    `carrier_service_id` BIGINT COMMENT 'Foreign key linking to fulfillment.carrier_service. Business justification: Line-level status events capture service tier at granular pick/pack/ship stages. Business uses carrier_service_id for service-level SLA compliance reporting, carrier performance analysis by service ti',
    `fulfillment_node_id` BIGINT COMMENT 'Identifier of the fulfillment location (DC, store, dark store, MFC) responsible for this line at the time of the status transition. Tracks which node is handling each stage of fulfillment.',
    `header_id` BIGINT COMMENT 'Reference to the parent order header for context and cross-referencing. Enables quick lookup of all line status events for a given order.',
    `order_line_id` BIGINT COMMENT 'Reference to the specific order line item (SKU line) whose status transition is being recorded. Links this status event to the parent order line entity.',
    `associate_id` BIGINT COMMENT 'Identifier of the warehouse associate who picked this line. Populated when status transitions to picked. Enables picker productivity and accuracy analysis.',
    `shipment_id` BIGINT COMMENT 'Reference to the shipment container that this line is part of. Enables grouping of lines that shipped together and tracking of split shipments.',
    `tertiary_line_user_associate_id` BIGINT COMMENT 'Identifier of the user (customer service rep, warehouse manager, system admin) who manually triggered this status change, if applicable. Null for automated system transitions.',
    `pos_transaction_id` BIGINT COMMENT 'The unique transaction or message identifier from the source system that triggered this status change. Enables end-to-end traceability and troubleshooting.. Valid values are `^[A-Z0-9-]{10,50}$`',
    `wave_id` BIGINT COMMENT 'Reference to the warehouse picking wave that processed this line. Links status transitions to warehouse execution batches for operational analysis.',
    `actual_delivery_timestamp` TIMESTAMP COMMENT 'The actual date and time the line was delivered to the customer. Populated when status transitions to delivered. Used for on-time delivery SLA measurement.',
    `carrier_service_level` STRING COMMENT 'The shipping service level selected for this line (e.g., standard ground, 2-day air, overnight). Impacts delivery SLA and cost.. Valid values are `standard|expedited|overnight|same_day|two_day|economy`',
    `current_status` STRING COMMENT 'The new status that the order line transitioned TO at this event. Represents the current state of the line after this transition. [ENUM-REF-CANDIDATE: pending|confirmed|allocated|picked|packed|shipped|in_transit|out_for_delivery|delivered|cancelled|returned|refunded — 12 candidates stripped; promote to reference product]',
    `estimated_delivery_date` DATE COMMENT 'The carrier-provided or system-calculated estimated delivery date for this line. Updated as the line progresses through fulfillment stages.',
    `exception_category` STRING COMMENT 'High-level categorization of the exception type when exception_flag is True. Enables aggregated exception reporting and trend analysis. [ENUM-REF-CANDIDATE: inventory_shortage|customer_cancellation|quality_issue|carrier_delay|address_issue|payment_failure|system_error — 7 candidates stripped; promote to reference product]',
    `exception_flag` BOOLEAN COMMENT 'Indicates whether this status transition represents an exception or deviation from normal fulfillment flow (e.g., cancellation, return, inventory shortage). True if exception, False if normal progression.',
    `fulfillment_node_type` STRING COMMENT 'The type of fulfillment location handling this line. Enables analysis of fulfillment performance by node type (e.g., ship-from-store vs DC).. Valid values are `distribution_center|store|dark_store|micro_fulfillment_center|vendor_direct|third_party_logistics`',
    `line_sequence_number` STRING COMMENT 'The line item number within the parent order (e.g., line 1, line 2). Denormalized for quick identification without joining to order_line.',
    `notes` STRING COMMENT 'Free-text notes or comments added by users or systems to provide additional context about this status transition. Used for exception documentation and customer service reference.',
    `previous_status` STRING COMMENT 'The status that the order line transitioned FROM. Enables reconstruction of the full status flow and identification of status reversals or exceptions. [ENUM-REF-CANDIDATE: pending|confirmed|allocated|picked|packed|shipped|in_transit|out_for_delivery|delivered|cancelled|returned|refunded — 12 candidates stripped; promote to reference product]',
    `quantity_affected` DECIMAL(18,2) COMMENT 'The quantity of the SKU affected by this status transition. Supports partial fulfillment scenarios where a line is split across multiple shipments with different status progressions.',
    `record_created_timestamp` TIMESTAMP COMMENT 'The timestamp when this status history record was inserted into the data platform. Distinct from status_transition_timestamp; used for data pipeline monitoring and late-arriving data detection.',
    `record_updated_timestamp` TIMESTAMP COMMENT 'The timestamp when this status history record was last modified in the data platform. Supports change data capture and data quality auditing.',
    `sku` STRING COMMENT 'The product SKU identifier for the line item. Denormalized to enable direct analysis of status transitions by product without joining to product master.. Valid values are `^[A-Z0-9]{8,14}$`',
    `sla_met_flag` BOOLEAN COMMENT 'Indicates whether the status transition occurred within the SLA target window. True if on-time or early, False if late. Critical for fulfillment performance KPIs.',
    `sla_target_timestamp` TIMESTAMP COMMENT 'The target date and time by which this status transition should have occurred per the order SLA. Used to calculate on-time performance at each fulfillment stage.',
    `status_reason_code` STRING COMMENT 'Standardized code explaining WHY the status transition occurred (e.g., CUST_CANCEL, INV_SHORTAGE, CARRIER_DELAY, QUALITY_ISSUE). Critical for root-cause analysis of fulfillment exceptions.. Valid values are `^[A-Z0-9_]{2,20}$`',
    `status_reason_description` STRING COMMENT 'Human-readable explanation of the status transition reason. Provides additional context beyond the reason code for customer service and operations teams.',
    `status_transition_timestamp` TIMESTAMP COMMENT 'The precise date and time when the order line transitioned to the current status. Business event timestamp for SLA measurement and fulfillment analytics.',
    `system_source` STRING COMMENT 'The source system that generated this status transition event (e.g., OMS, WMS, TMS, carrier API). Enables data lineage tracking and system integration monitoring. [ENUM-REF-CANDIDATE: oms|wms|tms|pos|ecommerce|customer_service|carrier_api|manual — 8 candidates stripped; promote to reference product]',
    `tracking_number` STRING COMMENT 'The carrier-provided tracking number for this line shipment. Enables customer self-service tracking and last-mile visibility. Populated when status transitions to shipped.. Valid values are `^[A-Z0-9]{10,35}$`',
    `unit_of_measure` STRING COMMENT 'The unit of measure for the quantity affected (e.g., each, case, pound). Denormalized from product master for direct interpretation of quantity. [ENUM-REF-CANDIDATE: each|case|pallet|pound|kilogram|liter|gallon — 7 candidates stripped; promote to reference product]',
    CONSTRAINT pk_line_status_history PRIMARY KEY(`line_status_history_id`)
) COMMENT 'Granular status transition log at the order line level, tracking each SKU line through its own fulfillment lifecycle independently of the header. Captures line status, previous status, transition timestamp, fulfillment node, carrier, and reason code. Critical for split-shipment scenarios where individual lines progress at different rates.';
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`pos_transaction` (
    ...
    `associate_id` BIGINT COMMENT 'Identifier of the store associate or cashier who processed the transaction.',
    ...
    `stock_ledger_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_ledger. Business justification: POS transactions create inventory movement records in stock ledger for perpetual inventory accounting, shrinkage analysis, and financial reconciliation. Retail operations require linking each sale to ',
    `store_location_id` BIGINT COMMENT 'FK to store.store_location.store_location_id — POS transaction to store location is required for all store-level sales reporting, comparable sales calculations, and store P&L. Every retail operational report needs this join.',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`pos_transaction_line` (
    ...
    `associate_id` BIGINT COMMENT 'Foreign key reference to the employee who processed this transaction line. Used for cashier performance tracking and audit trails.',
    ...
    `coupon_code` STRING COMMENT 'Manufacturer or store coupon code applied to this line item. Used for coupon redemption tracking and vendor chargebacks.. Valid values are `^[A-Z0-9]{6,20}$`',
    ...
    `department_code` STRING COMMENT 'Retail department or category code for this product. Used for departmental sales reporting and category management.. Valid values are `^[A-Z0-9]{2,10}$`',
    ...
    `promotion_code` STRING COMMENT 'Promotional campaign code applied to this line item. Links to promotion management system for campaign tracking and analysis.. Valid values are `^[A-Z0-9]{4,20}$`',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`payment` (
    ...
    `associate_id` BIGINT COMMENT 'Foreign key linking to workforce.associate. Business justification: Retail associates process payments at POS terminals and customer service desks. Tracking which associate processed each payment enables cash drawer reconciliation, fraud investigation, refund authoriz',
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to finance.bank_account. Business justification: Payment settlement requires linking to specific bank accounts for cash management, daily reconciliation, treasury operations, and bank statement matching. Essential for payment processing, fraud detec',
    `plan_id` BIGINT COMMENT 'Unique identifier for the BNPL installment plan associated with this payment. Used for tracking payment schedules and reconciliation with BNPL provider.',
    ...
    `pci_assessment_id` BIGINT COMMENT 'Foreign key linking to compliance.pci_assessment. Business justification: Payment processing is core PCI scope. QSAs sample payment records during assessments to validate requirement compliance (encryption at rest/transit, tokenization, key management). Links payments to as',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`discount` (
    ...
    `associate_id` BIGINT COMMENT 'Reference to the user (manager or supervisor) who approved the discount application. Null if auto-applied or pending approval.',
    `discount_cashier_associate_id` BIGINT COMMENT 'Reference to the cashier or sales associate who applied the discount at the Point of Sale (POS). Null for online or automated discount applications.',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`cancellation` (
    ...
    `cancellation_associate_id` BIGINT COMMENT 'Reference to the customer service agent or employee who processed or approved the cancellation. Null for system-initiated cancellations.',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`hold` (
    `hold_id` BIGINT COMMENT 'Unique identifier for the order hold record. Primary key.',
    `associate_id` BIGINT COMMENT 'Reference to the customer service agent or fraud analyst assigned to review and resolve this hold. Nullable if not yet assigned.',
    `audit_finding_id` BIGINT COMMENT 'Foreign key linking to compliance.audit_finding. Business justification: Holds placed due to compliance control failures (fraud screening, sanctions hits, address verification) link to audit findings. Enables auditors to validate hold procedures, measure control effectiven',
    `header_id` BIGINT COMMENT 'Reference to the order that is subject to this hold. Links to the order header.',
    `reservation_id` BIGINT COMMENT 'Reference to the inventory reservation record associated with this hold, particularly for Buy Online Pick Up In Store (BOPIS) or Reserve Online Pick Up In Store (ROPIS) orders.',
    `order_line_id` BIGINT COMMENT 'Reference to the specific order line item subject to this hold. Nullable if the hold applies to the entire order rather than a specific line.',
    `location_id` BIGINT COMMENT 'Reference to the store or fulfillment node where the customer will pick up the order for Buy Online Pick Up In Store (BOPIS) or Reserve Online Pick Up In Store (ROPIS) holds.',
    `superseded_hold_id` BIGINT COMMENT 'Self-referencing FK on hold (superseded_hold_id)',
    `address_verification_status` STRING COMMENT 'Result of the address verification process indicating whether the shipping address passed validation checks. Relevant for address verification failure holds.. Valid values are `verified|failed|partial_match|not_verified`',
    `assigned_to_team_code` STRING COMMENT 'Code identifying the team or department responsible for resolving this hold (e.g., fraud team, payment operations, customer service).',
    `auto_release_eligible_flag` BOOLEAN COMMENT 'Indicates whether the hold is eligible for automatic release by system rules without manual intervention.',
    `auto_release_timestamp` TIMESTAMP COMMENT 'Scheduled date and time when the system will automatically release the hold if no manual action is taken. Nullable if auto-release is not configured.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this hold record was first created in the data warehouse. Used for audit trail and data lineage tracking.',
    `customer_notification_channel` STRING COMMENT 'The communication channel used to notify the customer about the hold status (email, SMS, push notification, phone call, or in-app message).. Valid values are `email|sms|push_notification|phone|in_app`',
    `customer_notification_sent_flag` BOOLEAN COMMENT 'Indicates whether the customer has been notified about the hold via email, SMS, or other communication channel.',
    `customer_notification_timestamp` TIMESTAMP COMMENT 'Date and time when the customer notification about the hold was sent. Nullable if no notification has been sent.',
    `duration_minutes` STRING COMMENT 'Calculated duration of the hold in minutes from start to end timestamp. Used for performance analysis and SLA compliance reporting.',
    `end_timestamp` TIMESTAMP COMMENT 'Date and time when the hold was released or resolved, allowing order processing to resume. Nullable if hold is still active.',
    `expected_resolution_timestamp` TIMESTAMP COMMENT 'Anticipated date and time when the hold is expected to be resolved based on business rules and Service Level Agreement (SLA) targets.',
    `fraud_decision` STRING COMMENT 'Decision rendered by the fraud detection system or fraud analyst regarding the order: approve, decline, manual review, or customer challenge required.. Valid values are `approve|decline|review|challenge`',
    `fraud_score` DECIMAL(18,2) COMMENT 'Numerical risk score assigned by the fraud detection system indicating the likelihood of fraudulent activity. Higher scores indicate higher risk.',
    `hold_number` STRING COMMENT 'Business-readable unique identifier for the hold, used for tracking and customer service reference.',
    `hold_status` STRING COMMENT 'Current lifecycle status of the hold indicating whether it is active, resolved, expired, cancelled, or escalated for further review.. Valid values are `active|resolved|expired|cancelled|escalated`',
    `hold_type` STRING COMMENT 'Classification of the hold indicating the reason category for the suspension. Includes fraud review, payment issues, pickup reservations (BOPIS/ROPIS), and compliance holds. [ENUM-REF-CANDIDATE: fraud_review|payment_authorization_failure|address_verification_failure|inventory_reservation|bopis_pickup_reservation|ropis_pickup_reservation|credit_limit_exceeded|compliance_review — 8 candidates stripped; promote to reference product]',
    `manual_review_required_flag` BOOLEAN COMMENT 'Indicates whether the hold requires manual review and approval by a customer service agent or fraud analyst before release.',
    `payment_authorization_code` STRING COMMENT 'Authorization code from the payment processor associated with this hold. Relevant for payment authorization failure holds.',
    `payment_processor_response` STRING COMMENT 'Response message or code returned by the payment processor explaining the reason for payment authorization failure.',
    `pickup_expiration_timestamp` TIMESTAMP COMMENT 'Date and time after which the pickup reservation expires and the order may be cancelled or returned to inventory if not collected.',
    `pickup_ready_timestamp` TIMESTAMP COMMENT 'Date and time when the order is ready for customer pickup at the designated location. Relevant for Buy Online Pick Up In Store (BOPIS) and Reserve Online Pick Up In Store (ROPIS) holds.',
    `priority_level` STRING COMMENT 'Business priority assigned to the hold resolution based on order value, customer tier, or urgency. Determines queue position for manual review.. Valid values are `low|medium|high|critical`',
    `reason_code` STRING COMMENT 'Standardized code representing the specific reason for the hold. Used for categorization and reporting.',
    `reason_description` STRING COMMENT 'Detailed human-readable explanation of why the hold was placed, providing context for customer service and operations teams.',
    `resolution_action` STRING COMMENT 'The action taken to resolve the hold, such as releasing the order, cancelling it, escalating for review, or applying a manual override.. Valid values are `released|cancelled|escalated|expired|manual_override`',
    `resolution_notes` STRING COMMENT 'Free-text notes documenting the resolution process, actions taken, and any special circumstances or customer interactions.',
    `sla_breach_flag` BOOLEAN COMMENT 'Indicator of whether the hold resolution exceeded the SLA target timestamp, triggering potential customer compensation or escalation.',
    `sla_target_timestamp` TIMESTAMP COMMENT 'The deadline by which the hold must be resolved to meet Service Level Agreement (SLA) commitments to the customer.',
    `source_system` STRING COMMENT 'Identifier of the system that created or initiated the hold (e.g., Order Management System, fraud detection system, payment gateway).',
    `source_system_hold_code` STRING COMMENT 'The unique identifier for this hold in the originating source system, used for cross-system reconciliation and traceability.',
    `start_timestamp` TIMESTAMP COMMENT 'Date and time when the hold was initiated and order processing was suspended. Critical for Service Level Agreement (SLA) tracking.',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time when this hold record was last modified in the data warehouse. Used for change tracking and audit purposes.',
    CONSTRAINT pk_hold PRIMARY KEY(`hold_id`)
) COMMENT 'Temporary suspension or reservation of order processing including BOPIS/ROPIS pickup reservations, fraud review holds, payment authorization failures, and address verification holds. Captures hold type, hold reason, hold start/end timestamps, resolution action, and associated order reference.';

CREATE OR REPLACE TABLE `retail_ecm`.`order`.`promise` (
    `promise_id` BIGINT COMMENT 'Unique identifier for the order promise record. Primary key.',
    `carrier_id` BIGINT COMMENT 'Foreign key linking to fulfillment.carrier. Business justification: Promise calculation engine uses carrier-specific transit times as primary input. Business needs carrier_id to trace which carriers SLA drove the customer promise for promise accuracy analysis, carrie',
    `carrier_service_id` BIGINT COMMENT 'Foreign key linking to fulfillment.carrier_service. Business justification: Promise dates calculated from carrier service level transit times (2-day vs. overnight). Business requires carrier_service_id to validate promise accuracy against service-level SLAs, analyze promise v',
    `fulfillment_node_id` BIGINT COMMENT 'Identifier of the fulfillment node (DC, store, dark store) from which the order will be shipped.',
    `header_id` BIGINT COMMENT 'Reference to the order header for which this delivery promise was made.',
    `order_line_id` BIGINT COMMENT 'Reference to the specific order line if the promise is line-level; null if promise is order-level.',
    `revised_promise_id` BIGINT COMMENT 'Self-referencing FK on promise (revised_promise_id)',
    `accuracy_flag` BOOLEAN COMMENT 'Indicates whether the delivery was completed within the promised window (true = on-time, false = missed).',
    `actual_delivery_timestamp` TIMESTAMP COMMENT 'Actual timestamp when the order was delivered to the customer, used for promise accuracy measurement.',
    `calculation_engine_version` STRING COMMENT 'Version identifier of the promise calculation engine or algorithm used.',
    `calculation_source` STRING COMMENT 'System or method used to calculate the delivery promise (carrier API, static SLA table, DOM engine, manual override).. Valid values are `carrier_api|static_sla|dom_engine|manual_override|inventory_availability`',
    `carrier_service_level` STRING COMMENT 'Specific service level offered by the carrier (e.g., Ground, Express, Priority).',
    `confidence_score` DECIMAL(18,2) COMMENT 'Confidence score (0-100) indicating the likelihood of meeting the delivery promise based on historical performance.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the delivery promise was initially created and communicated to the customer.',
    `customer_notification_channel` STRING COMMENT 'Channel through which the customer was notified of the delivery promise.. Valid values are `email|sms|push_notification|in_app|phone_call`',
    `customer_notification_sent_flag` BOOLEAN COMMENT 'Indicates whether the customer was notified of the delivery promise or any revisions.',
    `customer_notification_timestamp` TIMESTAMP COMMENT 'Timestamp when the customer was notified of the delivery promise or revision.',
    `cutoff_time_applied` STRING COMMENT 'Order cutoff time applied for same-day or next-day promise calculation (e.g., 2:00 PM local time).',
    `fulfillment_method` STRING COMMENT 'Method by which the order will be fulfilled (ship-to-home, BOPIS, ROPIS, ship-from-store, curbside pickup).. Valid values are `ship_to_home|bopis|ropis|ship_from_store|curbside_pickup|locker_pickup`',
    `inventory_availability_timestamp` TIMESTAMP COMMENT 'Timestamp when inventory availability was confirmed for promise calculation.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this promise record was last updated in the data platform.',
    `last_revision_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent revision to the delivery promise.',
    `order_channel` STRING COMMENT 'Channel through which the order was placed, influencing promise calculation logic.. Valid values are `web|mobile_app|store|call_center|marketplace`',
    `peak_season_flag` BOOLEAN COMMENT 'Indicates whether the promise was made during a peak season (e.g., holiday shopping period) affecting delivery times.',
    `promise_number` STRING COMMENT 'Business-readable identifier for the delivery promise, used for customer communication and tracking.',
    `promise_status` STRING COMMENT 'Current lifecycle status of the delivery promise.. Valid values are `active|fulfilled|breached|revised|cancelled`',
    `promise_type` STRING COMMENT 'Type of delivery promise made to the customer at checkout or order confirmation.. Valid values are `standard|expedited|same_day|next_day|two_day|scheduled`',
    `promised_delivery_date_end` DATE COMMENT 'End date of the promised delivery window communicated to the customer.',
    `promised_delivery_date_start` DATE COMMENT 'Start date of the promised delivery window communicated to the customer.',
    `promised_delivery_time_end` TIMESTAMP COMMENT 'Precise end timestamp of the promised delivery window for time-specific promises.',
    `promised_delivery_time_start` TIMESTAMP COMMENT 'Precise start timestamp of the promised delivery window for time-specific promises.',
    `revision_count` STRING COMMENT 'Number of times the delivery promise has been revised after initial creation.',
    `revision_reason_code` STRING COMMENT 'Code indicating the reason for promise revision (e.g., inventory delay, carrier delay, weather).',
    `revision_reason_description` STRING COMMENT 'Detailed description of why the delivery promise was revised.',
    `shipping_address_country_code` STRING COMMENT 'ISO 3166-1 alpha-3 country code of the delivery destination.',
    `shipping_address_postal_code` STRING COMMENT 'Postal code of the delivery destination, used for promise calculation and carrier routing.',
    `sla_breach_flag` BOOLEAN COMMENT 'Indicates whether the delivery promise was breached (actual delivery exceeded promised window).',
    `sla_target_hours` STRING COMMENT 'Target number of hours from order placement to delivery as defined by the SLA.',
    `source_system` STRING COMMENT 'Source system that created the delivery promise record (OMS, e-commerce platform, DOM engine).',
    `variance_hours` DECIMAL(18,2) COMMENT 'Difference in hours between actual delivery and promised delivery (positive = late, negative = early).',
    `weather_impact_flag` BOOLEAN COMMENT 'Indicates whether weather conditions were factored into the promise calculation or caused a delay.',
    CONSTRAINT pk_promise PRIMARY KEY(`promise_id`)
) COMMENT 'Delivery commitment made to the customer at checkout or order confirmation. Captures promise type (standard, expedited, same-day, next-day), promised delivery window (start/end), calculation source (carrier API, static SLA table, DOM engine), revision history when promises are updated, actual vs promised variance, and SLA breach flag. Enables promise accuracy KPI tracking, customer experience measurement, and carrier performance evaluation.';
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`gift_card` (
    ...
    `replacement_for_card_gift_card_id` BIGINT COMMENT 'Identifier of the original gift card that this card replaces (e.g., lost or damaged card replacement). Nullable for original issuances. Balance is typically transferred from original card.',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`gift_card_transaction` (
    `gift_card_transaction_id` BIGINT COMMENT 'Unique identifier for the gift card transaction record.',
    `associate_id` BIGINT COMMENT 'Identifier of the cashier or associate who processed the transaction at POS.',
    `gift_card_id` BIGINT COMMENT 'Unique identifier of the gift card involved in this transaction. May be masked for security.',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Each gift card transaction (issuance, redemption, reload, escheatment) posts to specific GL accounts for proper accounting treatment, revenue recognition, liability management, and financial reporting',
    ...
    `original_transaction_gift_card_transaction_id` BIGINT COMMENT 'Reference to the original gift card transaction that is being reversed or refunded.',
    ...
    `reversal_gift_card_transaction_id` BIGINT COMMENT 'Self-referencing FK on gift_card_transaction (reversal_gift_card_transaction_id)',
    ...
    `source_system` STRING COMMENT 'Name of the operational system that originated this gift card transaction record (e.g., SAP CAR, Oracle Retail, Salesforce Commerce Cloud).',
...
CREATE OR REPLACE TABLE `retail_ecm`.`order`.`subscription` (
    ...
    `associate_id` BIGINT COMMENT 'Foreign key linking to workforce.associate. Business justification: Retail associates enroll customers in subscription programs (auto-replenishment, beauty boxes, meal kits) at stores and call centers. Tracking enrollment associate supports commission calculation, cus',
    ...
    `privacy_assessment_id` BIGINT COMMENT 'Foreign key linking to compliance.privacy_assessment. Business justification: Recurring subscriptions process personal data continuously, triggering GDPR Article 35 assessments for high-risk processing. DPOs assess subscription data flows, retention, and automated decision-maki',
    `profile_id` BIGINT COMMENT 'Reference to the customer who owns this subscription. Links to the customer master record.',
    `replenishment_plan_id` BIGINT COMMENT 'Foreign key linking to supplychain.replenishment_plan. Business justification: Subscription commerce (auto-replenishment programs for consumables, pet supplies, household goods) generates predictable demand patterns that drive dedicated replenishment plans. Retailers use this li',
    ...
    `promotion_code` STRING COMMENT 'Marketing promotion or coupon code applied to the subscription, providing recurring discounts or special offers.. Valid values are `^[A-Z0-9_]{3,20}$`',
...
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ADD CONSTRAINT `fk_order_line_status_history_header_id` FOREIGN KEY (`header_id`) REFERENCES `retail_ecm`.`order`.`header`(`header_id`);
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ADD CONSTRAINT `fk_order_line_status_history_order_line_id` FOREIGN KEY (`order_line_id`) REFERENCES `retail_ecm`.`order`.`order_line`(`order_line_id`);
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ADD CONSTRAINT `fk_order_line_status_history_pos_transaction_id` FOREIGN KEY (`pos_transaction_id`) REFERENCES `retail_ecm`.`order`.`pos_transaction`(`pos_transaction_id`);
...
ALTER TABLE `retail_ecm`.`order`.`hold` ADD CONSTRAINT `fk_order_hold_header_id` FOREIGN KEY (`header_id`) REFERENCES `retail_ecm`.`order`.`header`(`header_id`);
ALTER TABLE `retail_ecm`.`order`.`hold` ADD CONSTRAINT `fk_order_hold_order_line_id` FOREIGN KEY (`order_line_id`) REFERENCES `retail_ecm`.`order`.`order_line`(`order_line_id`);
ALTER TABLE `retail_ecm`.`order`.`hold` ADD CONSTRAINT `fk_order_hold_superseded_hold_id` FOREIGN KEY (`superseded_hold_id`) REFERENCES `retail_ecm`.`order`.`hold`(`hold_id`);
ALTER TABLE `retail_ecm`.`order`.`promise` ADD CONSTRAINT `fk_order_promise_header_id` FOREIGN KEY (`header_id`) REFERENCES `retail_ecm`.`order`.`header`(`header_id`);
ALTER TABLE `retail_ecm`.`order`.`promise` ADD CONSTRAINT `fk_order_promise_order_line_id` FOREIGN KEY (`order_line_id`) REFERENCES `retail_ecm`.`order`.`order_line`(`order_line_id`);
ALTER TABLE `retail_ecm`.`order`.`promise` ADD CONSTRAINT `fk_order_promise_revised_promise_id` FOREIGN KEY (`revised_promise_id`) REFERENCES `retail_ecm`.`order`.`promise`(`promise_id`);
...
ALTER TABLE `retail_ecm`.`order`.`gift_card` ADD CONSTRAINT `fk_order_gift_card_replacement_for_card_gift_card_id` FOREIGN KEY (`replacement_for_card_gift_card_id`) REFERENCES `retail_ecm`.`order`.`gift_card`(`gift_card_id`);
ALTER TABLE `retail_ecm`.`order`.`gift_card` ADD CONSTRAINT `fk_order_gift_card_replacement_for_gift_card_id` FOREIGN KEY (`replacement_for_gift_card_id`) REFERENCES `retail_ecm`.`order`.`gift_card`(`gift_card_id`);
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ADD CONSTRAINT `fk_order_gift_card_transaction_gift_card_id` FOREIGN KEY (`gift_card_id`) REFERENCES `retail_ecm`.`order`.`gift_card`(`gift_card_id`);
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ADD CONSTRAINT `fk_order_gift_card_transaction_header_id` FOREIGN KEY (`header_id`) REFERENCES `retail_ecm`.`order`.`header`(`header_id`);
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ADD CONSTRAINT `fk_order_gift_card_transaction_original_transaction_gift_card_transaction_id` FOREIGN KEY (`original_transaction_gift_card_transaction_id`) REFERENCES `retail_ecm`.`order`.`gift_card_transaction`(`gift_card_transaction_id`);
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ADD CONSTRAINT `fk_order_gift_card_transaction_pos_transaction_id` FOREIGN KEY (`pos_transaction_id`) REFERENCES `retail_ecm`.`order`.`pos_transaction`(`pos_transaction_id`);
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ADD CONSTRAINT `fk_order_gift_card_transaction_reversal_gift_card_transaction_id` FOREIGN KEY (`reversal_gift_card_transaction_id`) REFERENCES `retail_ecm`.`order`.`gift_card_transaction`(`gift_card_transaction_id`);
...
ALTER TABLE `retail_ecm`.`order`.`header` SET TAGS ('dbx_subdomain' = 'transaction_capture');
...
ALTER TABLE `retail_ecm`.`order`.`header` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`header` ALTER COLUMN `corporate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Corporate Account Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`header` ALTER COLUMN `privacy_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Privacy Assessment Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`order_line` SET TAGS ('dbx_subdomain' = 'transaction_capture');
...
ALTER TABLE `retail_ecm`.`order`.`order_line` ALTER COLUMN `replenishment_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Replenishment Plan Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`order_line` ALTER COLUMN `sku` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU)');
ALTER TABLE `retail_ecm`.`order`.`order_line` ALTER COLUMN `sku` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,14}$');
...
ALTER TABLE `retail_ecm`.`order`.`status_history` SET TAGS ('dbx_subdomain' = 'transaction_capture');
...
ALTER TABLE `retail_ecm`.`order`.`status_history` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Triggering User ID');
ALTER TABLE `retail_ecm`.`order`.`status_history` ALTER COLUMN `associate_id` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `retail_ecm`.`order`.`status_history` ALTER COLUMN `tracking_number` SET TAGS ('dbx_business_glossary_term' = 'Shipment Tracking Number');
...
ALTER TABLE `retail_ecm`.`order`.`line_status_history` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` SET TAGS ('dbx_subdomain' = 'transaction_capture');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `line_status_history_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line Status History ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `carrier_service_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Service Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `fulfillment_node_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Picker ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Shipment ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `tertiary_line_user_associate_id` SET TAGS ('dbx_business_glossary_term' = 'User ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `pos_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Source System Transaction ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `pos_transaction_id` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]{10,50}$');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `wave_id` SET TAGS ('dbx_business_glossary_term' = 'Warehouse Wave ID');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `actual_delivery_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Actual Delivery Timestamp');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `carrier_service_level` SET TAGS ('dbx_business_glossary_term' = 'Carrier Service Level');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `carrier_service_level` SET TAGS ('dbx_value_regex' = 'standard|expedited|overnight|same_day|two_day|economy');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `current_status` SET TAGS ('dbx_business_glossary_term' = 'Current Line Status');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `estimated_delivery_date` SET TAGS ('dbx_business_glossary_term' = 'Estimated Delivery Date');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `exception_category` SET TAGS ('dbx_business_glossary_term' = 'Exception Category');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `exception_flag` SET TAGS ('dbx_business_glossary_term' = 'Exception Flag');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `fulfillment_node_type` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node Type');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `fulfillment_node_type` SET TAGS ('dbx_value_regex' = 'distribution_center|store|dark_store|micro_fulfillment_center|vendor_direct|third_party_logistics');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `line_sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Line Sequence Number');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Status Transition Notes');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `previous_status` SET TAGS ('dbx_business_glossary_term' = 'Previous Line Status');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `quantity_affected` SET TAGS ('dbx_business_glossary_term' = 'Quantity Affected by Status Change');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `record_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `sku` SET TAGS ('dbx_business_glossary_term' = 'Stock Keeping Unit (SKU)');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `sku` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,14}$');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `sla_met_flag` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Met Flag');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `sla_target_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Target Timestamp');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `status_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Status Reason Code');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `status_reason_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,20}$');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `status_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Status Reason Description');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `status_transition_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Status Transition Timestamp');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `system_source` SET TAGS ('dbx_business_glossary_term' = 'System Source');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `tracking_number` SET TAGS ('dbx_business_glossary_term' = 'Shipment Tracking Number');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `tracking_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{10,35}$');
ALTER TABLE `retail_ecm`.`order`.`line_status_history` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure (UOM)');
ALTER TABLE `retail_ecm`.`order`.`pos_transaction` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `retail_ecm`.`order`.`pos_transaction` SET TAGS ('dbx_subdomain' = 'store_sales');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction` ALTER COLUMN `pci_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Pci Assessment Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Cashier ID');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction` ALTER COLUMN `stock_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Ledger Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction` ALTER COLUMN `primary_payment_method` SET TAGS ('dbx_business_glossary_term' = 'Primary Payment Method');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` SET TAGS ('dbx_subdomain' = 'store_sales');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Cashier ID');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `category_code` SET TAGS ('dbx_business_glossary_term' = 'Category Code');
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `category_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `coupon_code` SET TAGS ('dbx_business_glossary_term' = 'Coupon Code');
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `coupon_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,20}$');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `department_code` SET TAGS ('dbx_business_glossary_term' = 'Department Code');
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `department_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
...
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `retail_ecm`.`order`.`pos_transaction_line` ALTER COLUMN `promotion_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4,20}$');
...
ALTER TABLE `retail_ecm`.`order`.`payment` SET TAGS ('dbx_subdomain' = 'financial_settlement');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `payment_id` SET TAGS ('dbx_business_glossary_term' = 'Payment ID');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `plan_id` SET TAGS ('dbx_business_glossary_term' = 'BNPL (Buy Now Pay Later) Installment Plan ID');
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `plan_id` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `retail_ecm`.`order`.`payment` ALTER COLUMN `pci_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Pci Assessment Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`discount` SET TAGS ('dbx_subdomain' = 'financial_settlement');
ALTER TABLE `retail_ecm`.`order`.`discount` ALTER COLUMN `discount_id` SET TAGS ('dbx_business_glossary_term' = 'Discount Identifier');
ALTER TABLE `retail_ecm`.`order`.`discount` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`discount` ALTER COLUMN `discount_cashier_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Cashier Identifier (ID)');
...
ALTER TABLE `retail_ecm`.`order`.`cancellation` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `retail_ecm`.`order`.`cancellation` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Approver ID');
ALTER TABLE `retail_ecm`.`order`.`cancellation` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`cancellation` ALTER COLUMN `cancellation_associate_id` SET TAGS ('dbx_business_glossary_term' = 'Processing Agent ID');
...
ALTER TABLE `retail_ecm`.`order`.`hold` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `retail_ecm`.`order`.`hold` SET TAGS ('dbx_subdomain' = 'transaction_capture');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `hold_id` SET TAGS ('dbx_business_glossary_term' = 'Order Hold Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To User Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `reservation_id` SET TAGS ('dbx_business_glossary_term' = 'Inventory Reservation Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Pickup Location Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `superseded_hold_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `address_verification_status` SET TAGS ('dbx_business_glossary_term' = 'Address Verification Status');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `address_verification_status` SET TAGS ('dbx_value_regex' = 'verified|failed|partial_match|not_verified');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `address_verification_status` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `address_verification_status` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `assigned_to_team_code` SET TAGS ('dbx_business_glossary_term' = 'Assigned To Team Code');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `auto_release_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Auto Release Eligible Flag');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `auto_release_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Auto Release Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `customer_notification_channel` SET TAGS ('dbx_business_glossary_term' = 'Customer Notification Channel');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `customer_notification_channel` SET TAGS ('dbx_value_regex' = 'email|sms|push_notification|phone|in_app');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `customer_notification_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Customer Notification Sent Flag');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `customer_notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Customer Notification Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Hold Duration in Minutes');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Hold End Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `expected_resolution_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Expected Resolution Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `fraud_decision` SET TAGS ('dbx_business_glossary_term' = 'Fraud Decision');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `fraud_decision` SET TAGS ('dbx_value_regex' = 'approve|decline|review|challenge');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `fraud_score` SET TAGS ('dbx_business_glossary_term' = 'Fraud Score');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `hold_number` SET TAGS ('dbx_business_glossary_term' = 'Hold Number');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `hold_status` SET TAGS ('dbx_business_glossary_term' = 'Hold Status');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `hold_status` SET TAGS ('dbx_value_regex' = 'active|resolved|expired|cancelled|escalated');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `hold_type` SET TAGS ('dbx_business_glossary_term' = 'Hold Type');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `manual_review_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Manual Review Required Flag');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `payment_authorization_code` SET TAGS ('dbx_business_glossary_term' = 'Payment Authorization Code');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `payment_processor_response` SET TAGS ('dbx_business_glossary_term' = 'Payment Processor Response');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `pickup_expiration_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Pickup Expiration Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `pickup_ready_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Pickup Ready Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Hold Reason Code');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Hold Reason Description');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `resolution_action` SET TAGS ('dbx_business_glossary_term' = 'Resolution Action');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `resolution_action` SET TAGS ('dbx_value_regex' = 'released|cancelled|escalated|expired|manual_override');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `resolution_notes` SET TAGS ('dbx_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `sla_breach_flag` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Breach Flag');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `sla_target_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Target Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `source_system_hold_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Hold Identifier (ID)');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Hold Start Timestamp');
ALTER TABLE `retail_ecm`.`order`.`hold` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `retail_ecm`.`order`.`promise` SET TAGS ('dbx_subdomain' = 'transaction_capture');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promise_id` SET TAGS ('dbx_business_glossary_term' = 'Order Promise ID');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `carrier_service_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Service Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `fulfillment_node_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node ID');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line ID');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `revised_promise_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `accuracy_flag` SET TAGS ('dbx_business_glossary_term' = 'Promise Accuracy Flag');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `actual_delivery_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Actual Delivery Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `calculation_engine_version` SET TAGS ('dbx_business_glossary_term' = 'Calculation Engine Version');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `calculation_source` SET TAGS ('dbx_business_glossary_term' = 'Promise Calculation Source');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `calculation_source` SET TAGS ('dbx_value_regex' = 'carrier_api|static_sla|dom_engine|manual_override|inventory_availability');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `carrier_service_level` SET TAGS ('dbx_business_glossary_term' = 'Carrier Service Level');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `confidence_score` SET TAGS ('dbx_business_glossary_term' = 'Promise Confidence Score');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Promise Created Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `customer_notification_channel` SET TAGS ('dbx_business_glossary_term' = 'Customer Notification Channel');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `customer_notification_channel` SET TAGS ('dbx_value_regex' = 'email|sms|push_notification|in_app|phone_call');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `customer_notification_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Customer Notification Sent Flag');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `customer_notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Customer Notification Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `cutoff_time_applied` SET TAGS ('dbx_business_glossary_term' = 'Cutoff Time Applied');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `fulfillment_method` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Method');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `fulfillment_method` SET TAGS ('dbx_value_regex' = 'ship_to_home|bopis|ropis|ship_from_store|curbside_pickup|locker_pickup');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `inventory_availability_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Inventory Availability Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `last_revision_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Revision Timestamp');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `order_channel` SET TAGS ('dbx_business_glossary_term' = 'Order Channel');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `order_channel` SET TAGS ('dbx_value_regex' = 'web|mobile_app|store|call_center|marketplace');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `peak_season_flag` SET TAGS ('dbx_business_glossary_term' = 'Peak Season Flag');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promise_number` SET TAGS ('dbx_business_glossary_term' = 'Promise Number');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promise_status` SET TAGS ('dbx_business_glossary_term' = 'Promise Status');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promise_status` SET TAGS ('dbx_value_regex' = 'active|fulfilled|breached|revised|cancelled');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promise_type` SET TAGS ('dbx_business_glossary_term' = 'Promise Type');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promise_type` SET TAGS ('dbx_value_regex' = 'standard|expedited|same_day|next_day|two_day|scheduled');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promised_delivery_date_end` SET TAGS ('dbx_business_glossary_term' = 'Promised Delivery Date End');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promised_delivery_date_start` SET TAGS ('dbx_business_glossary_term' = 'Promised Delivery Date Start');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promised_delivery_time_end` SET TAGS ('dbx_business_glossary_term' = 'Promised Delivery Time End');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `promised_delivery_time_start` SET TAGS ('dbx_business_glossary_term' = 'Promised Delivery Time Start');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `revision_count` SET TAGS ('dbx_business_glossary_term' = 'Revision Count');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `revision_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Revision Reason Code');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `revision_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Revision Reason Description');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `shipping_address_country_code` SET TAGS ('dbx_business_glossary_term' = 'Shipping Address Country Code');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `shipping_address_country_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `shipping_address_country_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `shipping_address_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Shipping Address Postal Code');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `shipping_address_postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `shipping_address_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `sla_breach_flag` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Breach Flag');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `sla_target_hours` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Target Hours');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `variance_hours` SET TAGS ('dbx_business_glossary_term' = 'Variance Hours');
ALTER TABLE `retail_ecm`.`order`.`promise` ALTER COLUMN `weather_impact_flag` SET TAGS ('dbx_business_glossary_term' = 'Weather Impact Flag');
ALTER TABLE `retail_ecm`.`order`.`gift_card` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `retail_ecm`.`order`.`gift_card` SET TAGS ('dbx_subdomain' = 'store_sales');
...
ALTER TABLE `retail_ecm`.`order`.`gift_card` ALTER COLUMN `pci_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Pci Assessment Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`gift_card` ALTER COLUMN `replacement_for_card_gift_card_id` SET TAGS ('dbx_business_glossary_term' = 'Replacement For Gift Card ID');
...
ALTER TABLE `retail_ecm`.`order`.`gift_card` ALTER COLUMN `replacement_for_gift_card_id` SET TAGS ('dbx_self_ref_fk' = 'true');
...
ALTER TABLE `retail_ecm`.`order`.`gift_card` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Gift Card Program Code');
...
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` SET TAGS ('dbx_subdomain' = 'store_sales');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `gift_card_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Gift Card Transaction ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Cashier ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `gift_card_id` SET TAGS ('dbx_business_glossary_term' = 'Gift Card Number');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `gift_card_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `gift_card_id` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `original_transaction_gift_card_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Original Transaction ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `pos_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `pos_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Point of Sale (POS) Transaction ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `reversal_gift_card_transaction_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `activation_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Activation Fee Amount');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `authorization_code` SET TAGS ('dbx_business_glossary_term' = 'Authorization Code');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `balance_after` SET TAGS ('dbx_business_glossary_term' = 'Balance After Transaction');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `balance_before` SET TAGS ('dbx_business_glossary_term' = 'Balance Before Transaction');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `business_date` SET TAGS ('dbx_business_glossary_term' = 'Business Date');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `channel` SET TAGS ('dbx_business_glossary_term' = 'Transaction Channel');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `channel` SET TAGS ('dbx_value_regex' = 'pos|ecommerce|mobile_app|call_center|kiosk|third_party');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `escheatment_flag` SET TAGS ('dbx_business_glossary_term' = 'Escheatment Flag');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `escheatment_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Escheatment Jurisdiction');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Gift Card Expiration Date');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `fraud_decision` SET TAGS ('dbx_business_glossary_term' = 'Fraud Decision');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `fraud_decision` SET TAGS ('dbx_value_regex' = 'approved|declined|review|challenge');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `fraud_score` SET TAGS ('dbx_business_glossary_term' = 'Fraud Score');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Transaction Notes');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `payment_method_type` SET TAGS ('dbx_business_glossary_term' = 'Payment Method Type');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `payment_method_type` SET TAGS ('dbx_value_regex' = 'credit_card|debit_card|cash|bank_transfer|digital_wallet|loyalty_points');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `processor_reference_code` SET TAGS ('dbx_business_glossary_term' = 'Processor Reference ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `reversal_flag` SET TAGS ('dbx_business_glossary_term' = 'Reversal Flag');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `reversal_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Reversal Reason Code');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `source_system_transaction_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Transaction ID');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_amount` SET TAGS ('dbx_business_glossary_term' = 'Transaction Amount');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_number` SET TAGS ('dbx_business_glossary_term' = 'Transaction Number');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_status` SET TAGS ('dbx_business_glossary_term' = 'Transaction Status');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_status` SET TAGS ('dbx_value_regex' = 'pending|approved|declined|reversed|completed|failed');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Transaction Timestamp');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_type` SET TAGS ('dbx_business_glossary_term' = 'Transaction Type');
ALTER TABLE `retail_ecm`.`order`.`gift_card_transaction` ALTER COLUMN `transaction_type` SET TAGS ('dbx_value_regex' = 'activation|load|redemption|balance_inquiry|escheatment|reversal');
ALTER TABLE `retail_ecm`.`order`.`subscription` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `retail_ecm`.`order`.`subscription` SET TAGS ('dbx_subdomain' = 'store_sales');
...
ALTER TABLE `retail_ecm`.`order`.`subscription` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Associate Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`subscription` ALTER COLUMN `privacy_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Privacy Assessment Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`subscription` ALTER COLUMN `replenishment_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Replenishment Plan Id (Foreign Key)');
...
ALTER TABLE `retail_ecm`.`order`.`subscription` ALTER COLUMN `promotion_code` SET TAGS ('dbx_business_glossary_term' = 'Promotion Code');
ALTER TABLE `retail_ecm`.`order`.`subscription` ALTER COLUMN `promotion_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{3,20}$');
