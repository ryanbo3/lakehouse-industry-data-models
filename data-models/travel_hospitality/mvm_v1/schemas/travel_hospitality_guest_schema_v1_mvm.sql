-- Schema for Domain: guest | Business: Travel Hospitality | Version: v1_mvm
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`profile` (
    ...
    `property_id` BIGINT COMMENT 'Foreign key linking to property.property. Business justification: Guest profile origin tracking: the property where a guest profile was first created is used in loyalty enrollment attribution, source-of-business reporting, and data governance audits. Travel-hospital',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`preference` (
    ...
    `fnb_outlet_id` BIGINT COMMENT 'Foreign key linking to fnb.fnb_outlet. Business justification: Guest dietary restrictions and allergy preferences must be routed to specific F&B outlets for pre-arrival briefing and VIP amenity setup. Hospitality operations teams use outlet-scoped preference reco',
    ...
    `room_type_id` BIGINT COMMENT 'Foreign key linking to inventory.room_type. Business justification: Pre-arrival room assignment and upgrade eligibility workflows match guest preferences (bed type, view, ADA, floor) against specific room types. Revenue management and front-desk teams use this link da',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`corporate_account` (
    ...
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to revenue.market_segment. Business justification: Corporate production reporting and rate strategy: corporate accounts are classified into revenue market segments to apply negotiated rate programs, commission structures, and displacement analysis. Re',
    `booking_source_id` BIGINT COMMENT 'Foreign key linking to channel.booking_source. Business justification: Corporate travel programs designate a primary booking channel (GDS, direct API, TMC portal). This FK supports corporate rate loading validation, preferred-channel compliance reporting, and commission ',
    `program_config_id` BIGINT COMMENT 'Foreign key linking to loyalty.program_config. Business justification: Corporate accounts are enrolled under specific loyalty program configurations governing earning rules, redemption terms, and data retention policies. Contract management and loyalty operations teams n',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` (
    ...
    `booking_source_id` BIGINT COMMENT 'Foreign key linking to channel.booking_source. Business justification: Group blocks originate through a specific booking source (group desk, GDS group segment, direct sales portal). This FK supports group channel attribution, commission calculation on group bookings, and',
    ...
    `cancellation_policy_id` BIGINT COMMENT 'Foreign key linking to reservation.cancellation_policy. Business justification: Group block cancellation and attrition penalty management: group contracts reference a specific cancellation policy governing penalty calculations and deposit forfeiture. The existing cancellation_pol',
    ...
    `event_booking_id` BIGINT COMMENT 'Foreign key linking to event.event_booking. Business justification: MICE operations require linking a PMS guest room block to its originating event booking for master billing, attrition tracking, and group pickup reporting. Revenue managers and event coordinators reco',
    ...
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to revenue.market_segment. Business justification: Group production reporting and rate application: group blocks are assigned market segments for revenue reporting, rate strategy application, and group displacement analysis. Revenue managers track gro',
    `promotion_id` BIGINT COMMENT 'Foreign key linking to loyalty.promotion. Business justification: Group blocks are frequently tied to loyalty promotions (double points for MICE groups, bonus nights for group bookings). Loyalty and revenue teams must link group blocks to the driving promotion for a',
    ...
    `reservation_group_block_id` BIGINT COMMENT 'Foreign key linking to reservation.reservation_group_block. Business justification: Group block reconciliation: the guest domains contracted block must link to the reservation domains pickup block for attrition reporting, rooming list management, and pickup tracking. Group sales an',
    `reservation_rate_plan_id` BIGINT COMMENT 'Foreign key linking to reservation.reservation_rate_plan. Business justification: Group contracted rate plan management: group blocks are sold against a specific rate plan governing commissions, LOS requirements, and channel restrictions. The existing group_rate_code is denormalize',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`vip_designation` (
    ...
    `room_type_id` BIGINT COMMENT 'Foreign key linking to inventory.room_type. Business justification: VIP designation drives room assignment priority and upgrade eligibility to specific room types. Pre-arrival VIP checklists and front-desk upgrade workflows require knowing which room type a VIP is ent',
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: VIP designation rules are tier-dependent: only Platinum members receive GM greeting, security escort, or incognito check-in. Operations and loyalty teams need to know which tier threshold governs or t',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`stay_history` (
    ...
    `booking_source_id` BIGINT COMMENT 'Foreign key linking to channel.booking_source. Business justification: Channel attribution reporting and commission reconciliation require linking each stay to its originating booking_source. Revenue managers and finance teams run source-of-business reports at stay level',
    `campaign_offer_id` BIGINT COMMENT 'Foreign key linking to marketing.campaign_offer. Business justification: A stay booked under a promotional campaign offer must reference that offer for offer performance measurement, incremental revenue attribution, and loyalty points multiplier application. rate_plan_code',
    ...
    `event_booking_id` BIGINT COMMENT 'Foreign key linking to event.event_booking. Business justification: Event revenue attribution requires linking actual guest stays to the originating event booking for group pickup analysis, post-event room revenue reporting, and attrition penalty calculation. Hospital',
    ...
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to revenue.market_segment. Business justification: Market segment production reporting: revenue managers join stay history to market segment definitions for ADR-by-segment, LOS analysis, and displacement studies. The current market_segment_code is a d',
    `member_id` BIGINT COMMENT 'Foreign key linking to loyalty.member. Business justification: Points accrual posting, stay credit reconciliation, and tier qualification tracking all require linking each stay record directly to the loyalty member account. Revenue and loyalty analysts run stay-l',
    ...
    `room_id` BIGINT COMMENT 'Foreign key linking to inventory.room. Business justification: Stay history must reference the specific physical room occupied for service recovery correlation, maintenance history cross-referencing, and guest complaint tracking by room. room_number is a denormal',
    `room_type_id` BIGINT COMMENT 'Foreign key linking to inventory.room_type. Business justification: Stay history records which room type was occupied — essential for loyalty analytics, repeat-guest preference capture, ADR-by-room-type reporting, and USALI revenue reporting. room_type_code is a denor',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`note` (
    ...
    `appointment_id` BIGINT COMMENT 'Foreign key linking to spa.appointment. Business justification: Spa pre-arrival traces and VIP setup notes are linked to specific appointments in hotel operations — allergy alerts, therapist briefing notes, amenity placement instructions. Guest notes scoped to a s',
    `fnb_outlet_id` BIGINT COMMENT 'Foreign key linking to fnb.fnb_outlet. Business justification: F&B-specific guest notes (allergy alerts, VIP amenity instructions, dietary requirements) are operationally routed to specific outlets for pre-service briefing. Direct FK enables outlet managers to fi',
    `member_id` BIGINT COMMENT 'Foreign key linking to loyalty.member. Business justification: Loyalty-specific service recovery notes and VIP alerts displayed at check-in must be associated with the member account for loyalty service workflows and member communication audit trails. Front desk ',
    ...
    `reservation_booking_id` BIGINT COMMENT 'Foreign key linking to reservation.reservation_booking. Business justification: Pre-arrival trace management and check-in alert display require notes attached directly to a reservation booking. In Opera PMS, traces are reservation-level constructs. The existing stay_history_id on',
    `room_id` BIGINT COMMENT 'Foreign key linking to inventory.room. Business justification: PMS traces/notes are frequently room-specific (e.g., noise complaints, maintenance requests, guest incidents in a specific room). Room-level note reporting supports service recovery, housekeeping brie',
    `stay_history_id` BIGINT COMMENT 'Foreign key linking to guest.stay_history. Business justification: A guest note is frequently written in the context of a specific completed or in-progress stay (e.g., service recovery notes, VIP alerts, housekeeping instructions tied to a particular visit). Adding s',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` (
    ...
    `booking_source_id` BIGINT COMMENT 'Foreign key linking to channel.booking_source. Business justification: GDPR and CCPA audit trails require recording which booking channel collected the guests communication consent. Regulators and DPOs need to verify consent provenance by channel. consent_source is a pl',
    `campaign_id` BIGINT COMMENT 'Foreign key linking to marketing.campaign. Business justification: Communication consent is frequently captured via a specific marketing campaign (e.g., a consent-capture or double-opt-in campaign). Linking to the originating campaign enables GDPR accountability repo',
    ...
    `member_id` BIGINT COMMENT 'Foreign key linking to loyalty.member. Business justification: Loyalty program communications (points statements, tier upgrade notifications, promotion emails) require consent records tied directly to the member account for GDPR/CAN-SPAM compliance audits. Loyalt',
...
CREATE OR REPLACE TABLE `travel_hospitality_ecm`.`guest`.`segment` (
    ...
    `cancellation_policy_id` BIGINT COMMENT 'Foreign key linking to reservation.cancellation_policy. Business justification: Segment-based cancellation policy configuration: revenue management assigns specific cancellation policies to guest segments (e.g., corporate flexible, OTA non-refundable). The existing cancellation_p',
    `hierarchy_id` BIGINT COMMENT 'Foreign key linking to property.hierarchy. Business justification: Segment scope alignment to organizational hierarchy (brand, region, cluster) is essential for chain-level revenue management rollup reporting. A segment defined at a brand or regional hierarchy node a',
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to revenue.market_segment. Business justification: Rate strategy and displacement analysis: guest segments must map to revenue market segments so revenue managers can apply correct pricing strategies, yield rules, and LRA eligibility. This mapping dri',
    ...
    `property_id` BIGINT COMMENT 'Foreign key linking to property.property. Business justification: Property-level guest segmentation is a core revenue management process: segments defined at a specific property (vs. chain-wide) drive rate strategy, displacement analysis, and RevPAR reporting. Reven',
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: Guest segments are mapped to loyalty tier eligibility rules for rate strategy and yield management. Revenue management systems join segment to tier to apply correct points multipliers, determine LRA e',
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ADD CONSTRAINT `fk_guest_note_stay_history_id` FOREIGN KEY (`stay_history_id`) REFERENCES `travel_hospitality_ecm`.`guest`.`stay_history`(`stay_history_id`);
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`profile` SET TAGS ('dbx_subdomain' = 'guest_identity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`profile` ALTER COLUMN `property_id` SET TAGS ('dbx_business_glossary_term' = 'Creation Property Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`contact_info` SET TAGS ('dbx_subdomain' = 'guest_identity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`preference` SET TAGS ('dbx_subdomain' = 'service_personalization');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`preference` ALTER COLUMN `fnb_outlet_id` SET TAGS ('dbx_business_glossary_term' = 'Fnb Outlet Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`preference` ALTER COLUMN `room_type_id` SET TAGS ('dbx_business_glossary_term' = 'Room Type Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`corporate_account` SET TAGS ('dbx_subdomain' = 'stay_activity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`corporate_account` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`corporate_account` ALTER COLUMN `booking_source_id` SET TAGS ('dbx_business_glossary_term' = 'Preferred Booking Source Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`corporate_account` ALTER COLUMN `program_config_id` SET TAGS ('dbx_business_glossary_term' = 'Program Config Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` SET TAGS ('dbx_subdomain' = 'stay_activity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `booking_source_id` SET TAGS ('dbx_business_glossary_term' = 'Booking Source Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `cancellation_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `event_booking_id` SET TAGS ('dbx_business_glossary_term' = 'Event Booking Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `reservation_group_block_id` SET TAGS ('dbx_business_glossary_term' = 'Reservation Group Block Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`guest_group_block` ALTER COLUMN `reservation_rate_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Reservation Rate Plan Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`vip_designation` SET TAGS ('dbx_subdomain' = 'service_personalization');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`vip_designation` ALTER COLUMN `room_type_id` SET TAGS ('dbx_business_glossary_term' = 'Preferred Room Type Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`vip_designation` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` SET TAGS ('dbx_subdomain' = 'stay_activity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `booking_source_id` SET TAGS ('dbx_business_glossary_term' = 'Booking Source Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `campaign_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Offer Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `event_booking_id` SET TAGS ('dbx_business_glossary_term' = 'Event Booking Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `member_id` SET TAGS ('dbx_business_glossary_term' = 'Member Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Room Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`stay_history` ALTER COLUMN `room_type_id` SET TAGS ('dbx_business_glossary_term' = 'Room Type Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` SET TAGS ('dbx_subdomain' = 'service_personalization');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `note_id` SET TAGS ('dbx_business_glossary_term' = 'Guest Note ID');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Appointment Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `fnb_outlet_id` SET TAGS ('dbx_business_glossary_term' = 'Fnb Outlet Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `member_id` SET TAGS ('dbx_business_glossary_term' = 'Member Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `reservation_booking_id` SET TAGS ('dbx_business_glossary_term' = 'Reservation Booking Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Room Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`note` ALTER COLUMN `stay_history_id` SET TAGS ('dbx_business_glossary_term' = 'Stay History Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` SET TAGS ('dbx_subdomain' = 'guest_identity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` ALTER COLUMN `booking_source_id` SET TAGS ('dbx_business_glossary_term' = 'Booking Source Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` ALTER COLUMN `member_id` SET TAGS ('dbx_business_glossary_term' = 'Member Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` ALTER COLUMN `member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`communication_consent` ALTER COLUMN `member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`identity_document` SET TAGS ('dbx_subdomain' = 'guest_identity');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` SET TAGS ('dbx_subdomain' = 'service_personalization');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Segment ID');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` ALTER COLUMN `cancellation_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Policy Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` ALTER COLUMN `hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` ALTER COLUMN `property_id` SET TAGS ('dbx_business_glossary_term' = 'Property Id (Foreign Key)');
ALTER TABLE `travel_hospitality_ecm`.`guest`.`segment` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier Id (Foreign Key)');
...
ALTER TABLE `travel_hospitality_ecm`.`guest`.`relationship` SET TAGS ('dbx_subdomain' = 'guest_identity');
