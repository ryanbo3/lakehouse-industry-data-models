-- Schema for Domain: masterdata | Business: Pharmaceuticals | Version: v1_ecm
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` (
    ...
    `jurisdiction_of_incorporation` STRING COMMENT 'The country or jurisdiction where the legal entity is incorporated or registered, represented as a 3-letter ISO country code.. Valid values are `^[A-Z]{3}$`',
    ...
    `registered_address_line1` STRING COMMENT 'The first line of the official registered address of the legal entity as recorded with the jurisdiction of incorporation.',
    `registered_address_line2` STRING COMMENT 'The second line of the official registered address of the legal entity, typically containing suite, floor, or building information.',
    `registered_city` STRING COMMENT 'The city or municipality of the registered address.',
    `registered_country` STRING COMMENT 'The country of the registered address, represented as a 3-letter ISO country code.. Valid values are `^[A-Z]{3}$`',
    `registered_postal_code` STRING COMMENT 'The postal code or ZIP code of the registered address.',
    `registered_state_province` STRING COMMENT 'The state, province, or administrative region of the registered address.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` (
    ...
    `address_line_1` STRING COMMENT 'Primary street address line for the organizational unit location. Used for correspondence, regulatory submissions, and facility audits.',
    `address_line_2` STRING COMMENT 'Secondary address line for suite, building, or floor information. Optional field for additional location detail.',
    ...
    `city` STRING COMMENT 'City or municipality where the organizational unit is located. Used for geographic reporting and regulatory submissions.',
    ...
    `postal_code` STRING COMMENT 'Postal or ZIP code for the organizational unit address. Used for mail delivery and geographic analysis.',
    ...
    `state_province` STRING COMMENT 'State, province, or administrative region for the organizational unit location. Used for regional compliance and tax reporting.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`cost_center` (
    ...
    `employee_id` BIGINT COMMENT 'Employee identifier of the person responsible for managing and controlling costs within this cost center. Used for accountability and approval workflows.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` (
    ...
    `address_line_1` STRING COMMENT 'Primary street address line of the plant facility including building number and street name.',
    `address_line_2` STRING COMMENT 'Secondary address line for additional location details such as suite, floor, or building designation.',
    ...
    `capacity_unit_of_measure` STRING COMMENT 'Unit of measure for the annual capacity metric (e.g., kg, liters, dosage units, batches) aligned with the plants primary output type.',
    `city` STRING COMMENT 'City or municipality where the plant facility is located.',
    ...
    `postal_code` STRING COMMENT 'Postal or ZIP code for the plant facility address used for mail delivery and geographic reference.',
    ...
    `state_province` STRING COMMENT 'State, province, or administrative region where the plant facility is located.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` (
    ...
    `address_line_1` STRING COMMENT 'First line of the physical address for this storage location. Includes building number, street name, and suite/unit if applicable. Required for GDP traceability and regulatory inspections.',
    `address_line_2` STRING COMMENT 'Second line of the physical address for this storage location. Optional field for additional address details such as building name or department.',
    ...
    `capacity_unit_of_measure` STRING COMMENT 'Unit of measure for the capacity_storage_units field. Standardizes capacity reporting across heterogeneous storage types.. Valid values are `pallet|bin|cubic_meter|square_meter|shelf|location`',
    `city` STRING COMMENT 'City where this storage location is physically located. Required for regulatory reporting and distribution planning.',
    `country_code` STRING COMMENT 'Three-letter ISO 3166-1 alpha-3 country code for the storage location. Examples: USA, GBR, DEU, JPN, CHN. Required for multi-country regulatory reporting and GDP compliance.. Valid values are `^[A-Z]{3}$`',
    ...
    `postal_code` STRING COMMENT 'Postal or ZIP code for this storage location. Required for logistics planning and regulatory correspondence.',
    ...
    `state_province` STRING COMMENT 'State or province where this storage location is physically located. Required for state-level regulatory reporting and DEA registration.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`inn_registry` (
    ...
    `employee_id` BIGINT COMMENT 'The identifier of the data steward or data governance role responsible for maintaining the quality and accuracy of this INN registry record. Supports accountability and data quality management.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` (
    `system_crosswalk_id` BIGINT COMMENT 'Unique identifier for the system crosswalk mapping record. Primary key for the cross-system identifier mapping table.',
    `employee_id` BIGINT COMMENT 'Identifier of the data steward responsible for approving and maintaining this identifier mapping. Used for data governance accountability and audit trail.',
    `mapping_rule_id` BIGINT COMMENT 'Reference to the business rule or transformation logic used to establish this identifier mapping. Links to MDM governance documentation and mapping specifications.',
    `merger_acquisition_event_id` BIGINT COMMENT 'Reference to the specific merger, acquisition, or divestiture event that triggered the creation of this identifier mapping. Null for mappings created during normal operations.',
    `confidence_score` DECIMAL(18,2) COMMENT 'Numeric score (0.00 to 100.00) representing the confidence level in the accuracy of this identifier mapping. Higher scores indicate greater certainty in the mapping relationship. Used for data quality management and reconciliation prioritization.',
    `created_by_user` STRING COMMENT 'User identifier of the person or system process that created this identifier mapping record. Required for audit trail and 21 CFR Part 11 electronic records compliance.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this identifier mapping record was first created in the system. Required for audit trail and regulatory compliance.',
    `effective_end_date` DATE COMMENT 'Date after which this identifier mapping is no longer valid. Null indicates the mapping is currently active with no planned expiration.',
    `effective_start_date` DATE COMMENT 'Date from which this identifier mapping becomes valid and should be used for cross-system integration and Master Data Management (MDM) golden record resolution.',
    `entity_type` STRING COMMENT 'Type of master data entity being mapped across systems. Indicates whether the mapping is for materials, business partners, sites, compounds, organizational units, or products.. Valid values are `MATERIAL|BUSINESS_PARTNER|SITE|COMPOUND|ORG_UNIT|PRODUCT`',
    `golden_record_flag` BOOLEAN COMMENT 'Indicates whether this mapping represents the authoritative Master Data Management (MDM) golden record relationship for this entity. True for the primary system-of-record mapping.',
    `gxp_regulated_flag` BOOLEAN COMMENT 'Indicates whether this identifier mapping is subject to Good Manufacturing Practice (GMP), Good Clinical Practice (GCP), or Good Laboratory Practice (GLP) regulatory requirements. True for mappings involving regulated master data.',
    `last_modified_by_user` STRING COMMENT 'User identifier of the person or system process that most recently modified this identifier mapping record. Required for audit trail and 21 CFR Part 11 electronic records compliance.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this identifier mapping record was most recently modified. Required for audit trail and regulatory compliance.',
    `last_reconciliation_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent reconciliation process that validated this identifier mapping against source and target systems. Used for audit trail and data quality monitoring.',
    `mapping_direction` STRING COMMENT 'Indicates whether the mapping is unidirectional (source to target only) or bidirectional (can be used in both directions for identifier resolution).. Valid values are `ONE_WAY|BIDIRECTIONAL`',
    `mapping_method` STRING COMMENT 'Method by which the identifier mapping was established. Manual indicates data steward creation; automated indicates system-generated; ML-derived indicates machine learning algorithm output.. Valid values are `MANUAL|AUTOMATED|SEMI_AUTOMATED|ML_DERIVED`',
    `mapping_notes` STRING COMMENT 'Free-text field for data stewards to document special considerations, exceptions, or context about this identifier mapping. Used for knowledge transfer and troubleshooting.',
    `mapping_status` STRING COMMENT 'Current lifecycle status of the identifier mapping. Active mappings are used for integration; deprecated mappings are retained for historical audit; pending validation mappings await data steward approval.. Valid values are `ACTIVE|DEPRECATED|PENDING_VALIDATION|SUSPENDED|RETIRED`',
    `merger_acquisition_flag` BOOLEAN COMMENT 'Indicates whether this identifier mapping was created as part of a merger, acquisition, or divestiture data harmonization effort. True for M&A-related mappings requiring special governance.',
    `reconciliation_status` STRING COMMENT 'Result of the most recent reconciliation check. Matched indicates both identifiers exist and are consistent; mismatch indicates data quality issue requiring investigation.. Valid values are `MATCHED|MISMATCH|NOT_FOUND_SOURCE|NOT_FOUND_TARGET|PENDING`',
    `source_entity_description` STRING COMMENT 'Human-readable description or name of the entity as it appears in the source system. Aids in manual validation and troubleshooting of identifier mappings.',
    `source_identifier` STRING COMMENT 'The identifier value as it exists in the source system. This is the original identifier that needs to be mapped to the target system.',
    `source_system_code` STRING COMMENT 'Code identifying the source system from which the identifier originates. Examples: SAP_S4, VEEVA_VAULT, LIMS_LABWARE, ARGUS, MEDIDATA, MES, VEEVA_CRM.. Valid values are `^[A-Z0-9_]{2,20}$`',
    `source_system_instance` STRING COMMENT 'Specific instance or environment of the source system (e.g., production, quality assurance, development). Important for multi-instance SAP S/4HANA or Veeva Vault deployments.',
    `sox_relevant_flag` BOOLEAN COMMENT 'Indicates whether this identifier mapping is relevant for Sarbanes-Oxley Act financial reporting compliance. True for mappings involving financial master data such as cost centers, profit centers, or general ledger accounts.',
    `target_entity_description` STRING COMMENT 'Human-readable description or name of the entity as it appears in the target system. Aids in manual validation and troubleshooting of identifier mappings.',
    `target_identifier` STRING COMMENT 'The corresponding identifier value in the target system. This is the mapped identifier that the source identifier resolves to.',
    `target_system_code` STRING COMMENT 'Code identifying the target system to which the identifier is mapped. Examples: SAP_S4, VEEVA_VAULT, LIMS_LABWARE, ARGUS, MEDIDATA, MES, VEEVA_CRM.. Valid values are `^[A-Z0-9_]{2,20}$`',
    `target_system_instance` STRING COMMENT 'Specific instance or environment of the target system (e.g., production, quality assurance, development). Important for multi-instance SAP S/4HANA or Veeva Vault deployments.',
    `validation_timestamp` TIMESTAMP COMMENT 'Timestamp when this identifier mapping was last validated or approved by a data steward. Critical for regulatory compliance and data quality assurance.',
    CONSTRAINT pk_system_crosswalk PRIMARY KEY(`system_crosswalk_id`)
) COMMENT 'Cross-system identifier mapping table maintaining authoritative golden-record mappings between master data entity identifiers across the pharmaceutical technology landscape. Maps materials, business partners, sites, compounds, and organizational units between SAP S/4HANA, Veeva Vault, LIMS, MES, Argus Safety, and Medidata Rave. Captures source system, target system, entity type, source identifier, target identifier, mapping direction, mapping status (active, deprecated, pending validation), effective/expiry dates, confidence score, and last reconciliation timestamp. Enables MDM golden record resolution, system integration reconciliation, merger/acquisition data harmonization, and audit trail for identifier lifecycle.';
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`address` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the data steward or data owner responsible for maintaining the accuracy and quality of this address record. Used for data governance, accountability, and escalation of data quality issues.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` (
    `supply_agreement_id` BIGINT COMMENT 'Primary key uniquely identifying each supply agreement record between a business partner and a material',
    `business_partner_id` BIGINT COMMENT 'Foreign key linking to the business partner acting in supplier role (API supplier, CMO, CDMO, CRO, excipient supplier, packaging supplier, laboratory reagent vendor)',
    `material_id` BIGINT COMMENT 'Foreign key linking to the material being supplied (API, excipient, drug substance, drug product, packaging material, laboratory reagent, raw material)',
    `avl_status` STRING COMMENT 'Current AVL qualification status for this supplier-material combination. Approved = fully qualified, Conditional = approved with restrictions, Suspended = temporarily not allowed, Disqualified = removed from AVL. GMP regulatory requirement.',
    `contract_number` STRING COMMENT 'Reference to the legal contract or purchase agreement governing this supply relationship. Links to contract management system for terms and conditions.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this supply agreement record was first created in the system. Audit trail for data governance.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the unit price. Required for multi-currency procurement operations.',
    `gmp_certification_status` STRING COMMENT 'GMP certification status specific to this suppliers capability to manufacture or supply this material. Material-specific because GMP requirements vary by material type (API vs excipient vs packaging).',
    `last_audit_date` DATE COMMENT 'Date of the most recent quality or GMP compliance audit conducted at this suppliers facility for this material. Supplier-material specific because audit scope varies by material criticality and GMP classification.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to this supply agreement record. Audit trail for data governance.',
    `lead_time_days` STRING COMMENT 'Standard procurement lead time in days from purchase order to delivery for this material from this supplier. Critical for MRP planning and inventory optimization.',
    `maximum_order_quantity` DECIMAL(18,2) COMMENT 'Maximum order quantity that this supplier can fulfill for this material in a single order, expressed in the materials base unit of measure. Reflects supplier capacity constraints.',
    `minimum_order_quantity` DECIMAL(18,2) COMMENT 'Minimum order quantity required by this supplier for this material, expressed in the materials base unit of measure. Supplier-specific commercial term.',
    `next_audit_due_date` DATE COMMENT 'Scheduled date for the next audit of this supplier for this material. Audit frequency varies by material risk classification and supplier performance history.',
    `preferred_supplier_flag` BOOLEAN COMMENT 'Indicates whether this supplier is the preferred source for this material based on quality, reliability, cost, and strategic relationship. Used in automated source determination during procurement.',
    `qualification_date` DATE COMMENT 'Date when this supplier was qualified for this specific material through the supplier qualification process including audits, sample testing, and documentation review. Required for regulatory traceability.',
    `supply_agreement_status` STRING COMMENT 'Current lifecycle status of this supply agreement. Active = currently valid for procurement, Inactive = not available for new orders, Pending = under negotiation, Expired = past valid_to_date.',
    `unit_price` DECIMAL(18,2) COMMENT 'Negotiated price per base unit of measure for this material from this supplier. Varies by supplier-material combination based on volume commitments, contract terms, and market conditions.',
    `valid_from_date` DATE COMMENT 'Effective start date for this supply agreement. Pricing, lead times, and qualification are valid from this date forward.',
    `valid_to_date` DATE COMMENT 'Expiration date for this supply agreement. Null indicates an open-ended agreement. Triggers contract renewal workflow when approaching.',
    CONSTRAINT pk_supply_agreement PRIMARY KEY(`supply_agreement_id`)
) COMMENT 'This association product represents the contractual supply relationship between a business partner (in supplier role) and a material. It captures the Approved Vendor List (AVL) qualification, pricing, lead times, and GMP compliance requirements mandated by pharmaceutical regulations. Each record links one business partner to one material with supplier-specific terms that exist only in the context of this supply relationship. Critical for procurement, quality assurance, and regulatory compliance in pharmaceutical supply chain management.. Existence Justification: In pharmaceutical supply chain operations, a single material (e.g., an API or excipient) is routinely sourced from multiple qualified suppliers to ensure supply continuity, competitive pricing, and regulatory compliance. Conversely, each supplier provides multiple different materials to the pharmaceutical manufacturer. The Approved Vendor List (AVL) is a regulatory requirement under GMP that mandates qualification and ongoing monitoring of each supplier-material combination, with relationship-specific data including pricing, lead times, qualification dates, audit schedules, and GMP certification status that cannot be stored on either the business partner or material entity alone.';

CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` (
    `entity_registration_id` BIGINT COMMENT 'Unique identifier for this legal entity country registration record. Primary key.',
    `country_id` BIGINT COMMENT 'Foreign key linking to the country jurisdiction where this registration is held',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to the legal entity that holds this country-specific registration',
    `branch_type` STRING COMMENT 'Classification of the type of presence or establishment the legal entity maintains in this country',
    `establishment_identifier` STRING COMMENT 'Country-specific establishment identifier assigned by the regulatory authority for pharmaceutical operations in this jurisdiction',
    `license_expiry_date` DATE COMMENT 'Expiration date of the pharmaceutical operating license in this country jurisdiction',
    `license_number` STRING COMMENT 'The pharmaceutical operating license number issued by this countrys regulatory authority to this legal entity',
    `registration_date` DATE COMMENT 'The date on which the legal entity was officially registered to operate in this country jurisdiction',
    `registration_number` STRING COMMENT 'The official registration or incorporation number assigned by the jurisdiction of this country for this legal entitys operations in that country',
    `regulatory_authority_code` STRING COMMENT 'Code or identifier assigned by this countrys regulatory authority for tracking this legal entitys regulatory submissions and compliance',
    `regulatory_status` STRING COMMENT 'Current regulatory compliance status of the legal entity in this country jurisdiction',
    `tax_identification_number` STRING COMMENT 'The tax identification number assigned by the tax authority in this country for this legal entitys tax obligations in that jurisdiction',
    `valid_from_date` DATE COMMENT 'Date from which this country registration record is effective and valid for the legal entity',
    `valid_to_date` DATE COMMENT 'Date after which this country registration record is no longer valid. Null indicates currently active registration.',
    `vat_registration_number` STRING COMMENT 'The VAT registration number assigned by this countrys tax authority for this legal entity, if VAT-registered in that jurisdiction',
    CONSTRAINT pk_entity_registration PRIMARY KEY(`entity_registration_id`)
) COMMENT 'This association product represents the country-specific registration and regulatory compliance record for a legal entity operating in a particular jurisdiction. It captures registration numbers, tax identifiers, regulatory licenses, and compliance status that exist only in the context of a specific legal entity operating in a specific country. Each record links one legal entity to one country with jurisdiction-specific regulatory and tax attributes.. Existence Justification: In pharmaceutical operations, a single legal entity (e.g., Pfizer Inc.) operates in multiple countries with country-specific registrations, tax IDs, and regulatory licenses in each jurisdiction. Conversely, each country hosts multiple legal entities from different pharmaceutical companies. The business actively manages these country-specific registrations as distinct compliance records with jurisdiction-specific attributes (registration numbers, tax IDs, regulatory status, license expiry dates) that cannot be stored on either the legal entity or country master alone.';

CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` (
    `material_plant_data_id` BIGINT COMMENT 'Unique identifier for the material plant data record. Primary key.',
    `material_id` BIGINT COMMENT 'Foreign key linking to the material master record for which plant-specific parameters are defined.',
    `plant_id` BIGINT COMMENT 'Foreign key linking to the plant facility where the material-specific operational parameters apply.',
    `abc_indicator` STRING COMMENT 'ABC classification of the material at this plant based on consumption value or strategic importance. A=High value/critical, B=Medium value/importance, C=Low value/routine. Used for inventory control prioritization.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this material plant data record was first created in the system.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this material plant data record was last updated, used for change tracking and data synchronization.',
    `lot_size` STRING COMMENT 'Lot-sizing procedure that determines how order quantities are calculated during MRP runs. EX=Lot-for-lot, FX=Fixed lot size, HB=Replenishment to maximum stock, LZ=Economic order quantity, WB=Weekly lot size.',
    `mrp_type` STRING COMMENT 'Material Requirements Planning procedure type that determines how the material is planned at this plant. PD=MRP, VB=Manual reorder point, VM=Forecast-based, VV=Reorder point planning, ND=No planning.',
    `plant_specific_material_number` STRING COMMENT 'Plant-specific material identifier that may differ from the enterprise material number, used for local operations and legacy system integration.',
    `procurement_type` STRING COMMENT 'Indicates how the material is procured at this specific plant. E=External procurement (purchased), F=In-house production (manufactured), X=Both external and in-house.',
    `production_scheduler` STRING COMMENT 'Identifier or name of the production scheduler or planner responsible for this material at this plant, used for MRP exception management and planning coordination.',
    `quality_inspection_group` STRING COMMENT 'Quality management inspection group code that defines which QC laboratory team is responsible for testing this material at this plant, linking to quality inspection procedures.',
    `reorder_point` DECIMAL(18,2) COMMENT 'Inventory level at which a procurement or production order should be triggered for this material at this plant, expressed in the material base unit of measure.',
    `safety_stock_quantity` DECIMAL(18,2) COMMENT 'Minimum inventory quantity that must be maintained at this plant to buffer against demand variability and supply disruptions, expressed in the material base unit of measure.',
    `storage_location_default` STRING COMMENT 'Default storage location code within the plant where this material is typically stored, used for automatic goods receipt and inventory management.',
    `valid_from_date` DATE COMMENT 'Date from which this material plant data record is effective and should be used for planning and operations at this plant.',
    `valid_to_date` DATE COMMENT 'Date until which this material plant data record is effective. Null indicates the record is currently active with no planned end date.',
    CONSTRAINT pk_material_plant_data PRIMARY KEY(`material_plant_data_id`)
) COMMENT 'This association product represents the plant-specific configuration and planning parameters for materials in pharmaceutical manufacturing operations. It captures MRP settings, quality inspection procedures, storage locations, and procurement rules that exist only in the context of a specific material-plant combination. Each record links one material to one plant with operational parameters that govern how that material is managed at that specific facility.. Existence Justification: In pharmaceutical manufacturing operations, materials are managed across multiple plant facilities with plant-specific operational parameters, and each plant manages thousands of different materials with unique MRP settings, quality procedures, and storage configurations. Material Plant Data is a foundational SAP MM business concept that captures the intersection of material master data and plant master data, representing how a specific material is procured, planned, stored, and quality-controlled at a specific manufacturing site. This is a textbook many-to-many relationship where the association itself carries rich operational configuration data that belongs to neither the material nor the plant alone.';

CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` (
    `material_storage_location_data_id` BIGINT COMMENT 'Unique identifier for this material-storage location configuration record. Primary key.',
    `material_id` BIGINT COMMENT 'Foreign key linking to the material master record',
    `storage_location_id` BIGINT COMMENT 'Foreign key linking to the storage location master record',
    `batch_management_indicator` BOOLEAN COMMENT 'Flag indicating whether batch-level tracking is required for this material at this storage location. May differ from material master if location-specific requirements apply.',
    `bin_location` STRING COMMENT 'Specific bin or shelf location within the storage location where this material is stored. Used for picking and putaway operations.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this material-storage location configuration record was first created in the system.',
    `hazmat_storage_class` STRING COMMENT 'Hazardous material storage classification for this material at this storage location. Ensures segregation and compliance with OSHA/DOT requirements.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this material-storage location configuration record was last modified.',
    `maximum_stock_quantity` DECIMAL(18,2) COMMENT 'Maximum inventory level allowed for this material at this storage location. Prevents overstocking and ensures capacity compliance.',
    `minimum_stock_quantity` DECIMAL(18,2) COMMENT 'Minimum inventory level to maintain for this material at this storage location. Safety stock requirement.',
    `picking_area` STRING COMMENT 'Designated picking zone within the storage location for this material. Optimizes warehouse operations and order fulfillment.',
    `putaway_strategy` STRING COMMENT 'Warehouse management strategy for placing incoming goods of this material into this storage location. Examples: FIFO, FEFO, fixed bin, bulk storage.',
    `reorder_point` DECIMAL(18,2) COMMENT 'Inventory level at which replenishment should be triggered for this material at this storage location. Used for automated procurement planning.',
    `storage_bin_type` STRING COMMENT 'Classification of the bin type used for this material at this storage location. Examples: pallet rack, shelf, bulk bin, cold storage bin.',
    `temperature_monitoring_required` BOOLEAN COMMENT 'Flag indicating whether continuous temperature monitoring is required for this material at this storage location. Critical for cold-chain integrity and regulatory compliance.',
    `valid_from_date` DATE COMMENT 'Date from which this material-storage location configuration becomes effective. Supports time-dependent storage configurations.',
    `valid_to_date` DATE COMMENT 'Date until which this material-storage location configuration remains effective. Null indicates indefinite validity.',
    CONSTRAINT pk_material_storage_location_data PRIMARY KEY(`material_storage_location_data_id`)
) COMMENT 'This association product represents the storage configuration and inventory management parameters for a specific material at a specific storage location within pharmaceutical operations. It captures bin assignments, reorder points, stock limits, handling strategies, and environmental monitoring requirements that exist only in the context of a material being stored at a particular location. Each record links one material to one storage location with operational parameters required for SAP WM/EWM, cold-chain integrity, controlled substance accountability, and GMP compliance.. Existence Justification: In pharmaceutical operations, a single material (e.g., an API or finished drug product) is routinely stored across multiple storage locations (e.g., main warehouse, cold room, quarantine zone, controlled substance vault) to support manufacturing, quality control, and distribution processes. Conversely, a single storage location houses many different materials. Each material-storage location combination requires specific operational parameters including bin assignments, reorder points, stock limits, putaway strategies, and environmental monitoring requirements that are unique to that combination and cannot be stored on either the material master or storage location master alone.';

CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`mapping_rule` (
    `mapping_rule_id` BIGINT COMMENT 'Primary key for mapping_rule',
    `parent_mapping_rule_id` BIGINT COMMENT 'Self-referencing FK on mapping_rule (parent_mapping_rule_id)',
    `approval_date` DATE COMMENT 'Date on which this mapping rule was formally approved by data governance or stewardship.',
    `approval_status` STRING COMMENT 'Approval status of the mapping rule indicating whether it has been reviewed and approved by data governance.',
    `approved_by` STRING COMMENT 'Name of the user or role who approved this mapping rule for production use.',
    `business_owner` STRING COMMENT 'Name of the business user or team responsible for defining and maintaining this mapping rule.',
    `compliance_requirement` STRING COMMENT 'Reference to regulatory or compliance requirement that this mapping rule supports (e.g., FDA 21 CFR Part 11, GDPR, GxP).',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this mapping rule record was first created in the system.',
    `data_domain` STRING COMMENT 'Business data domain to which this mapping rule belongs (e.g., masterdata, clinical, manufacturing, finance).',
    `data_quality_rule` STRING COMMENT 'Data quality validation rule or constraint applied during the mapping process to ensure data integrity.',
    `default_value` STRING COMMENT 'Default value to be used when source data is null, missing, or does not meet mapping criteria.',
    `mapping_rule_description` STRING COMMENT 'Detailed business description of the mapping rule, its purpose, and the business context in which it is applied.',
    `documentation_url` STRING COMMENT 'URL link to detailed documentation, specification, or design document for this mapping rule.',
    `effective_end_date` DATE COMMENT 'Date after which this mapping rule is no longer active and should not be applied. Null indicates open-ended validity.',
    `effective_start_date` DATE COMMENT 'Date from which this mapping rule becomes active and should be applied in data integration processes.',
    `error_handling_strategy` STRING COMMENT 'Strategy for handling errors encountered during mapping execution (fail process, log and continue, skip record, or retry).',
    `is_active` BOOLEAN COMMENT 'Boolean flag indicating whether this mapping rule is currently active and should be applied in data integration processes.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this mapping rule record was last updated or modified.',
    `last_validation_date` DATE COMMENT 'Date on which this mapping rule was last validated or tested for correctness and performance.',
    `mapping_priority` STRING COMMENT 'Numeric priority or sequence order for applying this mapping rule when multiple rules may apply to the same target.',
    `notes` STRING COMMENT 'Additional notes, comments, or special instructions related to the mapping rule for data stewards and developers.',
    `null_handling_strategy` STRING COMMENT 'Strategy for handling null or missing values in source data (reject record, apply default, skip field, or apply transformation).',
    `rule_code` STRING COMMENT 'Business-assigned unique code for the mapping rule, used for external reference and integration.',
    `rule_name` STRING COMMENT 'Human-readable name of the mapping rule describing its purpose.',
    `rule_type` STRING COMMENT 'Classification of the mapping rule based on its transformation logic (direct mapping, transformation, conditional logic, lookup-based, aggregation, or derivation).',
    `source_attribute` STRING COMMENT 'Name of the source field or attribute being mapped.',
    `source_entity` STRING COMMENT 'Name of the source entity, table, or object from which data originates.',
    `source_system` STRING COMMENT 'Name of the source system from which data is being mapped (e.g., SAP S/4HANA, Veeva, LIMS).',
    `mapping_rule_status` STRING COMMENT 'Current lifecycle status of the mapping rule indicating whether it is in use or has been retired.',
    `target_attribute` STRING COMMENT 'Name of the target field or attribute receiving the mapped data.',
    `target_entity` STRING COMMENT 'Name of the target entity, table, or data product to which data is mapped.',
    `target_system` STRING COMMENT 'Name of the target system or platform to which data is being mapped (e.g., Databricks Lakehouse, Data Warehouse).',
    `technical_owner` STRING COMMENT 'Name of the technical user or team responsible for implementing and supporting this mapping rule.',
    `transformation_logic` STRING COMMENT 'Detailed expression or formula defining the transformation logic applied during mapping (e.g., SQL expression, business rule, calculation).',
    `validation_result` STRING COMMENT 'Result of the most recent validation test indicating whether the mapping rule passed quality checks.',
    `version_number` STRING COMMENT 'Semantic version number of the mapping rule following major.minor.patch convention for change tracking.',
    CONSTRAINT pk_mapping_rule PRIMARY KEY(`mapping_rule_id`)
) COMMENT 'Master reference table for mapping_rule. Referenced by mapping_rule_id.';

CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` (
    `merger_acquisition_event_id` BIGINT COMMENT 'Primary key for merger_acquisition_event',
    `acquirer_legal_entity_id` BIGINT COMMENT 'Foreign key linking to masterdata.legal_entity. Business justification: M&A events have an acquiring legal entity. The existing acquirer_organization_id BIGINT attribute should be renamed to acquirer_legal_entity_id and made a FK to legal_entity.legal_entity_id for re',
    `legal_entity_id` BIGINT COMMENT 'Reference to the organization entity that is being acquired or merged. Links to organization master data.',
    `preceding_merger_acquisition_event_id` BIGINT COMMENT 'Self-referencing FK on merger_acquisition_event (preceding_merger_acquisition_event_id)',
    `announcement_date` DATE COMMENT 'Date when the merger or acquisition was publicly announced to stakeholders, investors, and regulatory bodies.',
    `antitrust_clearance_status` STRING COMMENT 'Status of antitrust regulatory review and clearance from competition authorities (FTC, DOJ, EC) to ensure the transaction does not create monopolistic conditions.',
    `closing_date` DATE COMMENT 'Date when all transaction conditions are satisfied, final documents are signed, and the deal is formally closed.',
    `confidentiality_agreement_date` DATE COMMENT 'Date when the non-disclosure agreement (NDA) or confidentiality agreement was executed between parties to protect sensitive transaction information.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this merger and acquisition event record was first created in the master data system.',
    `data_steward_name` STRING COMMENT 'Name of the data steward or master data management owner responsible for maintaining the accuracy and completeness of this merger and acquisition event record.',
    `deal_advisor_firm` STRING COMMENT 'Name of the primary investment banking or advisory firm providing merger and acquisition advisory services, valuation, and transaction structuring.',
    `due_diligence_completion_date` DATE COMMENT 'Date when the comprehensive due diligence review (financial, legal, operational, regulatory, intellectual property) was completed by the acquiring entity.',
    `effective_date` DATE COMMENT 'Date when the merger or acquisition transaction legally takes effect and ownership transfer is completed.',
    `employee_transfer_count` STRING COMMENT 'Number of employees transferring from the target organization to the acquiring organization as part of the transaction.',
    `event_code` STRING COMMENT 'Business identifier code for the merger or acquisition event, used for external reference and reporting to regulatory bodies.',
    `event_name` STRING COMMENT 'Full descriptive name of the merger or acquisition event, typically including the names of the entities involved.',
    `event_type` STRING COMMENT 'Classification of the corporate transaction type: merger (combination of equals), acquisition (one entity purchases another), divestiture (sale of business unit), spin-off (separation of subsidiary), joint venture (collaborative entity formation), or asset purchase (specific asset acquisition).',
    `expected_synergy_amount` DECIMAL(18,2) COMMENT 'Projected annual cost savings and revenue synergies expected to be realized from the merger or acquisition, typically measured over a 3-5 year period.',
    `integration_status` STRING COMMENT 'Current status of post-merger integration activities, tracking the operational, systems, and organizational consolidation progress.',
    `intellectual_property_included_flag` BOOLEAN COMMENT 'Indicates whether the transaction includes transfer of intellectual property assets such as patents, trademarks, trade secrets, or proprietary research and development.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this merger and acquisition event record was most recently updated or modified.',
    `legal_counsel_firm` STRING COMMENT 'Name of the primary law firm providing legal counsel and representation for the merger or acquisition transaction.',
    `manufacturing_facilities_included_flag` BOOLEAN COMMENT 'Indicates whether the transaction includes transfer of pharmaceutical manufacturing plants, production facilities, or Good Manufacturing Practice (GMP) certified sites.',
    `notes` STRING COMMENT 'Additional free-text notes, comments, or contextual information about the merger or acquisition event that do not fit into structured fields.',
    `payment_method` STRING COMMENT 'Method by which the transaction consideration is paid: cash (direct payment), stock (equity exchange), mixed (combination of cash and stock), debt assumption (acquirer assumes target debt), or earnout (performance-based future payments).',
    `product_portfolio_count` STRING COMMENT 'Number of pharmaceutical products, drug formulations, or medical devices included in the transaction scope.',
    `record_source_system` STRING COMMENT 'Name of the source system or application from which this merger and acquisition event record originated (e.g., SAP S/4HANA, corporate development system, legal management system).',
    `regulatory_approval_required_flag` BOOLEAN COMMENT 'Indicates whether the merger or acquisition requires regulatory approval from antitrust authorities, securities regulators, or industry-specific bodies (FDA, EMA).',
    `regulatory_jurisdiction` STRING COMMENT 'Primary regulatory jurisdictions overseeing the merger or acquisition, including country codes and regulatory body names (e.g., USA-FTC, EUR-EC, CHN-SAMR).',
    `research_pipeline_included_flag` BOOLEAN COMMENT 'Indicates whether the transaction includes transfer of drug development pipeline, clinical trial programs, or investigational new drug (IND) applications.',
    `shareholder_approval_date` DATE COMMENT 'Date when shareholders formally approved the merger or acquisition transaction through voting at a special or annual meeting.',
    `shareholder_approval_required_flag` BOOLEAN COMMENT 'Indicates whether the transaction requires approval from shareholders of the acquiring or target organization through a formal vote.',
    `merger_acquisition_event_status` STRING COMMENT 'Current lifecycle status of the merger or acquisition event, tracking progression from announcement through regulatory approval to completion or termination.',
    `strategic_rationale` STRING COMMENT 'Business justification and strategic reasoning for the merger or acquisition, including synergies, market expansion, technology acquisition, or portfolio optimization objectives.',
    `termination_date` DATE COMMENT 'Date when the merger or acquisition event was terminated or cancelled, if applicable. Null for completed or active transactions.',
    `therapeutic_area` STRING COMMENT 'Primary therapeutic area or disease category relevant to the merger or acquisition, such as oncology, immunology, rare diseases, vaccines, or consumer health.',
    `transaction_structure` STRING COMMENT 'Legal and financial structure of the transaction, defining how ownership and assets are transferred between parties.',
    `transaction_value_amount` DECIMAL(18,2) COMMENT 'Total monetary value of the merger or acquisition transaction, representing the purchase price or combined entity valuation.',
    `transaction_value_currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the transaction value amount (e.g., USD, EUR, GBP).',
    CONSTRAINT pk_merger_acquisition_event PRIMARY KEY(`merger_acquisition_event_id`)
) COMMENT 'Master reference table for merger_acquisition_event. Referenced by merger_acquisition_event_id.';

CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`functional_area` (
    `functional_area_id` BIGINT COMMENT 'Primary key for functional_area',
    `approval_status` STRING COMMENT 'Current approval status of this functional area master data record in the MDM workflow.',
    `approval_timestamp` TIMESTAMP COMMENT 'Timestamp when this functional area record was approved in the MDM governance workflow.',
    `approved_by_user_id` BIGINT COMMENT 'Reference to the user who approved this functional area record in the MDM governance process.',
    `budget_allocation_amount` DECIMAL(18,2) COMMENT 'Annual budget allocated to this functional area in the organizations base currency for operational expenses and investments.',
    `business_continuity_tier` STRING COMMENT 'Priority tier for business continuity and disaster recovery planning, where Tier 1 represents mission-critical functions requiring immediate recovery.',
    `compliance_audit_frequency` STRING COMMENT 'Required frequency of compliance audits for this functional area based on regulatory requirements and internal quality standards.',
    `cost_center_id` BIGINT COMMENT 'Reference to the primary cost center associated with this functional area for financial accounting and budget tracking.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this functional area record was first created in the master data system.',
    `data_integrity_level` STRING COMMENT 'Classification of data integrity requirements for systems and processes within this functional area, based on regulatory risk and business impact.',
    `data_steward_id` BIGINT COMMENT 'Reference to the employee responsible for data governance, data quality, and master data management for this functional area.',
    `functional_area_description` STRING COMMENT 'Detailed description of the functional areas purpose, scope, and responsibilities within the pharmaceutical organization.',
    `effective_end_date` DATE COMMENT 'Date when this functional area ceased operations or was retired. Null for currently active functional areas.',
    `effective_start_date` DATE COMMENT 'Date when this functional area became active and operational within the organization.',
    `external_system_code` STRING COMMENT 'Functional area identifier used in external or legacy systems for cross-system data integration and reconciliation.',
    `functional_area_code` STRING COMMENT 'Short alphanumeric code representing the functional area. Used as a business key for cross-system integration and reporting (e.g., RD for Research & Development, MFG for Manufacturing, QA for Quality Assurance).',
    `functional_area_type` STRING COMMENT 'Classification of the functional area by its primary business function within the pharmaceutical value chain.',
    `gxp_classification` STRING COMMENT 'Classification indicating which Good Practice standards apply to this functional area (Good Manufacturing Practice, Good Clinical Practice, Good Laboratory Practice, Good Distribution Practice, Good Pharmacovigilance Practice).',
    `headcount` STRING COMMENT 'Total number of full-time equivalent (FTE) employees assigned to this functional area.',
    `hierarchy_level` STRING COMMENT 'Numeric level in the functional area hierarchy where 1 represents the top level (e.g., 1=Division, 2=Department, 3=Section).',
    `last_audit_date` DATE COMMENT 'Date of the most recent compliance or quality audit conducted for this functional area.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this functional area record was last updated in the master data system.',
    `location_id` BIGINT COMMENT 'Reference to the primary physical location or site where this functional area operates.',
    `modified_by_user_id` BIGINT COMMENT 'Reference to the user who last modified this functional area record, supporting audit trail and data governance requirements.',
    `functional_area_name` STRING COMMENT 'Full business name of the functional area (e.g., Research and Development, Clinical Operations, Regulatory Affairs).',
    `next_audit_due_date` DATE COMMENT 'Scheduled date for the next compliance or quality audit of this functional area.',
    `notes` STRING COMMENT 'Additional free-text notes or comments about this functional area for internal reference and documentation purposes.',
    ...
    `quality_critical_flag` BOOLEAN COMMENT 'Indicates whether this functional area performs quality-critical activities that directly impact product quality, patient safety, or regulatory compliance.',
    `regulatory_scope` STRING COMMENT 'Geographic scope of regulatory responsibility for this functional area (e.g., global regulatory affairs vs. country-specific regulatory teams).',
    `responsible_person_id` BIGINT COMMENT 'Reference to the employee or manager who is accountable for this functional areas operations and performance.',
    `risk_classification` STRING COMMENT 'Overall risk classification of this functional area based on its impact on patient safety, product quality, regulatory compliance, and business continuity.',
    `sap_functional_area_code` STRING COMMENT 'Functional area code as defined in SAP S/4HANA system for financial accounting and controlling purposes.',
    `functional_area_status` STRING COMMENT 'Current lifecycle status of the functional area indicating whether it is actively used in business operations.',
    `therapeutic_area_alignment` STRING COMMENT 'Primary therapeutic area that this functional area supports or specializes in, relevant for research, clinical, and commercial functions.',
...
CREATE OR REPLACE TABLE `pharmaceuticals_ecm`.`masterdata`.`business_area` (
    `business_area_id` BIGINT COMMENT 'Primary key for business_area',
    `budget_amount` DECIMAL(18,2) COMMENT 'Annual budget allocation for this business area in the companys reporting currency.',
    `budget_currency_code` STRING COMMENT 'ISO 4217 three-letter currency code for the budget amount.',
    `business_area_code` STRING COMMENT 'Short alphanumeric code uniquely identifying the business area across enterprise systems. Used as the business identifier in SAP S/4HANA and other operational systems.',
    `company_code` STRING COMMENT 'SAP company code to which this business area belongs, linking business area to legal entity for financial reporting and consolidation.',
    `consolidation_unit` STRING COMMENT 'Financial consolidation unit code used for group reporting and statutory consolidation purposes.',
    `controlling_area` STRING COMMENT 'SAP controlling area code used for internal cost accounting and management reporting purposes.',
    `cost_center_id` BIGINT COMMENT 'Reference to the primary cost center associated with this business area for expense tracking and allocation.',
    `country_code` STRING COMMENT 'ISO 3166-1 alpha-3 country code for country-specific business areas.',
    `created_by_user_id` STRING COMMENT 'User identifier of the person or system account that created this business area record.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this business area record was first created in the master data system.',
    `data_integrity_level` STRING COMMENT 'Classification of data integrity requirements for this business area based on regulatory risk and compliance obligations (ALCOA+ principles).',
    `data_steward_employee_id` BIGINT COMMENT 'Reference to the employee designated as the data steward responsible for master data quality and governance for this business area.',
    `business_area_description` STRING COMMENT 'Detailed description of the business areas purpose, scope, and operational responsibilities within the pharmaceutical enterprise.',
    `effective_end_date` DATE COMMENT 'Date when this business area ceased or will cease operations. Null for currently active business areas.',
    `effective_start_date` DATE COMMENT 'Date when this business area became or will become operational and active in the enterprise structure.',
    `external_reference_id` STRING COMMENT 'External system identifier or reference code used by third-party systems, partners, or legacy systems to identify this business area.',
    `geographic_scope` STRING COMMENT 'Geographic coverage level of the business area indicating whether it operates globally, regionally, at country level, or at a specific site.',
    `gxp_applicable` BOOLEAN COMMENT 'Indicates whether Good Practice regulations (GMP, GLP, GCP, GDP, GVP) apply to this business areas operations.',
    `gxp_type` STRING COMMENT 'Specific type of Good Practice regulation applicable: GMP (Manufacturing), GLP (Laboratory), GCP (Clinical), GDP (Distribution), GVP (Pharmacovigilance), GAMP (Automated Systems).',
    `hierarchy_level` STRING COMMENT 'Numeric level in the business area hierarchy where 1 represents the top level and higher numbers represent deeper nesting.',
    `is_deleted` BOOLEAN COMMENT 'Soft delete flag indicating whether this business area record has been logically deleted but retained for audit and historical reference purposes.',
    `is_research_development` BOOLEAN COMMENT 'Indicates whether this business area is classified as Research and Development for financial reporting and tax credit purposes.',
    `is_revenue_generating` BOOLEAN COMMENT 'Indicates whether this business area directly generates revenue through product sales or service delivery.',
    `manager_employee_id` BIGINT COMMENT 'Reference to the employee who serves as the business area manager or head, responsible for operational oversight.',
    `modified_by_user_id` STRING COMMENT 'User identifier of the person or system account that last modified this business area record.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this business area record was last modified in the master data system.',
    `business_area_name` STRING COMMENT 'Full descriptive name of the business area as recognized across the enterprise.',
    `notes` STRING COMMENT 'Free-text field for additional information, special instructions, or contextual details about the business area.',
    `parent_business_area_id` BIGINT COMMENT 'Reference to the parent business area in the organizational hierarchy, enabling multi-level business area structures. Null for top-level business areas.',
    `profit_center_id` BIGINT COMMENT 'Reference to the profit center responsible for financial performance of this business area.',
    `region_code` STRING COMMENT 'Code representing the primary geographic region where this business area operates (e.g., EMEA, APAC, AMER, LATAM).',
    `regulatory_authority` STRING COMMENT 'Primary regulatory authority governing this business area (e.g., FDA, EMA, PMDA, Health Canada). Critical for regulatory affairs and compliance business areas.',
    `sap_business_area` STRING COMMENT 'Native SAP S/4HANA business area code used in financial postings and segment reporting.',
    `sort_order` STRING COMMENT 'Numeric value used to control the display sequence of business areas in reports and user interfaces.',
    `business_area_status` STRING COMMENT 'Current lifecycle status of the business area indicating whether it is operational, planned, or decommissioned.',
    `therapeutic_area` STRING COMMENT 'Primary therapeutic area focus for this business area (e.g., Oncology, Immunology, Rare Diseases, Vaccines). Applicable to research and commercial business areas.',
    `business_area_type` STRING COMMENT 'Classification of the business area by its primary function within the pharmaceutical value chain.',
    CONSTRAINT pk_business_area PRIMARY KEY(`business_area_id`)
) COMMENT 'Master reference table for business_area. Referenced by business_area_id.';
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ADD CONSTRAINT `fk_masterdata_system_crosswalk_mapping_rule_id` FOREIGN KEY (`mapping_rule_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`mapping_rule`(`mapping_rule_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ADD CONSTRAINT `fk_masterdata_system_crosswalk_merger_acquisition_event_id` FOREIGN KEY (`merger_acquisition_event_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event`(`merger_acquisition_event_id`);
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ADD CONSTRAINT `fk_masterdata_supply_agreement_business_partner_id` FOREIGN KEY (`business_partner_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`business_partner`(`business_partner_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ADD CONSTRAINT `fk_masterdata_supply_agreement_material_id` FOREIGN KEY (`material_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`material`(`material_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ADD CONSTRAINT `fk_masterdata_entity_registration_country_id` FOREIGN KEY (`country_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`country`(`country_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ADD CONSTRAINT `fk_masterdata_entity_registration_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`legal_entity`(`legal_entity_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ADD CONSTRAINT `fk_masterdata_material_plant_data_material_id` FOREIGN KEY (`material_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`material`(`material_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ADD CONSTRAINT `fk_masterdata_material_plant_data_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`plant`(`plant_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ADD CONSTRAINT `fk_masterdata_material_storage_location_data_material_id` FOREIGN KEY (`material_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`material`(`material_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ADD CONSTRAINT `fk_masterdata_material_storage_location_data_storage_location_id` FOREIGN KEY (`storage_location_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`storage_location`(`storage_location_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`mapping_rule` ADD CONSTRAINT `fk_masterdata_mapping_rule_parent_mapping_rule_id` FOREIGN KEY (`parent_mapping_rule_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`mapping_rule`(`mapping_rule_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ADD CONSTRAINT `fk_masterdata_merger_acquisition_event_acquirer_legal_entity_id` FOREIGN KEY (`acquirer_legal_entity_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`legal_entity`(`legal_entity_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ADD CONSTRAINT `fk_masterdata_merger_acquisition_event_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`legal_entity`(`legal_entity_id`);
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ADD CONSTRAINT `fk_masterdata_merger_acquisition_event_preceding_merger_acquisition_event_id` FOREIGN KEY (`preceding_merger_acquisition_event_id`) REFERENCES `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event`(`merger_acquisition_event_id`);
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` SET TAGS ('dbx_subdomain' = 'corporate_structure');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `jurisdiction_of_incorporation` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction of Incorporation');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `jurisdiction_of_incorporation` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Registered Address Line 1');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_address_line1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Registered Address Line 2');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_address_line2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_city` SET TAGS ('dbx_business_glossary_term' = 'Registered City');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_country` SET TAGS ('dbx_business_glossary_term' = 'Registered Country Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_country` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Registered Postal Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_state_province` SET TAGS ('dbx_business_glossary_term' = 'Registered State or Province');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`legal_entity` ALTER COLUMN `registered_state_province` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` SET TAGS ('dbx_subdomain' = 'corporate_structure');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `address_line_1` SET TAGS ('dbx_business_glossary_term' = 'Address Line 1');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `address_line_1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `address_line_2` SET TAGS ('dbx_business_glossary_term' = 'Address Line 2');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `address_line_2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `city` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `postal_code` SET TAGS ('dbx_business_glossary_term' = 'Postal Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `state_province` SET TAGS ('dbx_business_glossary_term' = 'State or Province');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`org_unit` ALTER COLUMN `state_province` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`cost_center` SET TAGS ('dbx_subdomain' = 'corporate_structure');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`cost_center` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Person Identifier (ID)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`cost_center` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`profit_center` SET TAGS ('dbx_subdomain' = 'corporate_structure');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`chart_of_accounts` SET TAGS ('dbx_subdomain' = 'corporate_structure');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material` SET TAGS ('dbx_subdomain' = 'product_catalog');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` SET TAGS ('dbx_subdomain' = 'facility_network');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `address_line_1` SET TAGS ('dbx_business_glossary_term' = 'Address Line 1');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `address_line_1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `address_line_2` SET TAGS ('dbx_business_glossary_term' = 'Address Line 2');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `address_line_2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `capacity_unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Capacity Unit of Measure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `city` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `postal_code` SET TAGS ('dbx_business_glossary_term' = 'Postal Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `state_province` SET TAGS ('dbx_business_glossary_term' = 'State or Province');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`plant` ALTER COLUMN `state_province` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` SET TAGS ('dbx_subdomain' = 'facility_network');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `address_line_1` SET TAGS ('dbx_business_glossary_term' = 'Address Line 1');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `address_line_1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `address_line_2` SET TAGS ('dbx_business_glossary_term' = 'Address Line 2');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `address_line_2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `capacity_unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Capacity Unit of Measure (UOM)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `capacity_unit_of_measure` SET TAGS ('dbx_value_regex' = 'pallet|bin|cubic_meter|square_meter|shelf|location');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `city` SET TAGS ('dbx_business_glossary_term' = 'City');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `postal_code` SET TAGS ('dbx_business_glossary_term' = 'Postal Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `state_province` SET TAGS ('dbx_business_glossary_term' = 'State or Province');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`storage_location` ALTER COLUMN `state_province` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`unit_of_measure` SET TAGS ('dbx_subdomain' = 'product_catalog');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`country` SET TAGS ('dbx_subdomain' = 'partner_management');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`atc_classification` SET TAGS ('dbx_subdomain' = 'product_catalog');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`masterdata_ndc_code` SET TAGS ('dbx_subdomain' = 'product_catalog');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`inn_registry` SET TAGS ('dbx_subdomain' = 'product_catalog');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`inn_registry` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Data Steward Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`inn_registry` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`inn_registry` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` SET TAGS ('dbx_subdomain' = 'corporate_structure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `system_crosswalk_id` SET TAGS ('dbx_business_glossary_term' = 'System Crosswalk Identifier (ID)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Data Steward Identifier (ID)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Mapping Rule Identifier (ID)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `merger_acquisition_event_id` SET TAGS ('dbx_business_glossary_term' = 'Merger and Acquisition (M&A) Event Identifier (ID)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `confidence_score` SET TAGS ('dbx_business_glossary_term' = 'Mapping Confidence Score');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `created_by_user` SET TAGS ('dbx_business_glossary_term' = 'Created By User');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `entity_type` SET TAGS ('dbx_business_glossary_term' = 'Entity Type');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `entity_type` SET TAGS ('dbx_value_regex' = 'MATERIAL|BUSINESS_PARTNER|SITE|COMPOUND|ORG_UNIT|PRODUCT');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `golden_record_flag` SET TAGS ('dbx_business_glossary_term' = 'Golden Record Flag');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `gxp_regulated_flag` SET TAGS ('dbx_business_glossary_term' = 'Good Practice (GxP) Regulated Flag');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `last_modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `last_reconciliation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Reconciliation Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_direction` SET TAGS ('dbx_business_glossary_term' = 'Mapping Direction');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_direction` SET TAGS ('dbx_value_regex' = 'ONE_WAY|BIDIRECTIONAL');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_method` SET TAGS ('dbx_business_glossary_term' = 'Mapping Method');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_method` SET TAGS ('dbx_value_regex' = 'MANUAL|AUTOMATED|SEMI_AUTOMATED|ML_DERIVED');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_notes` SET TAGS ('dbx_business_glossary_term' = 'Mapping Notes');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_status` SET TAGS ('dbx_business_glossary_term' = 'Mapping Status');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `mapping_status` SET TAGS ('dbx_value_regex' = 'ACTIVE|DEPRECATED|PENDING_VALIDATION|SUSPENDED|RETIRED');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `merger_acquisition_flag` SET TAGS ('dbx_business_glossary_term' = 'Merger and Acquisition (M&A) Flag');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Status');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_value_regex' = 'MATCHED|MISMATCH|NOT_FOUND_SOURCE|NOT_FOUND_TARGET|PENDING');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `source_entity_description` SET TAGS ('dbx_business_glossary_term' = 'Source Entity Description');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `source_identifier` SET TAGS ('dbx_business_glossary_term' = 'Source System Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `source_system_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,20}$');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `source_system_instance` SET TAGS ('dbx_business_glossary_term' = 'Source System Instance');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `sox_relevant_flag` SET TAGS ('dbx_business_glossary_term' = 'Sarbanes-Oxley (SOX) Relevant Flag');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `target_entity_description` SET TAGS ('dbx_business_glossary_term' = 'Target Entity Description');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `target_identifier` SET TAGS ('dbx_business_glossary_term' = 'Target System Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `target_system_code` SET TAGS ('dbx_business_glossary_term' = 'Target System Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `target_system_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,20}$');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `target_system_instance` SET TAGS ('dbx_business_glossary_term' = 'Target System Instance');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`system_crosswalk` ALTER COLUMN `validation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Validation Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`address` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`address` SET TAGS ('dbx_subdomain' = 'partner_management');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`address` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Data Steward Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`address` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`address` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`business_partner` SET TAGS ('dbx_subdomain' = 'partner_management');
...
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` SET TAGS ('dbx_subdomain' = 'partner_management');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` SET TAGS ('dbx_association_edges' = 'masterdata.business_partner,masterdata.material');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `supply_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Agreement Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `business_partner_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Agreement - Business Partner Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `material_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Agreement - Masterdata Material Master Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `avl_status` SET TAGS ('dbx_business_glossary_term' = 'Approved Vendor List Status');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `contract_number` SET TAGS ('dbx_business_glossary_term' = 'Contract Number');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `gmp_certification_status` SET TAGS ('dbx_business_glossary_term' = 'GMP Certification Status');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `last_audit_date` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Lead Time Days');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `maximum_order_quantity` SET TAGS ('dbx_business_glossary_term' = 'Maximum Order Quantity');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `minimum_order_quantity` SET TAGS ('dbx_business_glossary_term' = 'Minimum Order Quantity');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `next_audit_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Audit Due Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `preferred_supplier_flag` SET TAGS ('dbx_business_glossary_term' = 'Preferred Supplier Flag');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `qualification_date` SET TAGS ('dbx_business_glossary_term' = 'Qualification Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `supply_agreement_status` SET TAGS ('dbx_business_glossary_term' = 'Supply Agreement Status');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `unit_price` SET TAGS ('dbx_business_glossary_term' = 'Unit Price');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `unit_price` SET TAGS ('dbx_commercially_sensitive' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `valid_from_date` SET TAGS ('dbx_business_glossary_term' = 'Valid From Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`supply_agreement` ALTER COLUMN `valid_to_date` SET TAGS ('dbx_business_glossary_term' = 'Valid To Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` SET TAGS ('dbx_subdomain' = 'corporate_structure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` SET TAGS ('dbx_association_edges' = 'masterdata.legal_entity,masterdata.country');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `entity_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Entity Registration Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Entity Registration - Country Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Entity Registration - Legal Entity Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `branch_type` SET TAGS ('dbx_business_glossary_term' = 'Branch Type');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `establishment_identifier` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Establishment Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `license_expiry_date` SET TAGS ('dbx_business_glossary_term' = 'License Expiry Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `license_number` SET TAGS ('dbx_business_glossary_term' = 'Operating License Number');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `registration_date` SET TAGS ('dbx_business_glossary_term' = 'Registration Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `registration_number` SET TAGS ('dbx_business_glossary_term' = 'Country-Specific Registration Number');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `regulatory_authority_code` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Authority Code');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `regulatory_status` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Status');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `tax_identification_number` SET TAGS ('dbx_business_glossary_term' = 'Country-Specific Tax ID');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `tax_identification_number` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `valid_from_date` SET TAGS ('dbx_business_glossary_term' = 'Valid From Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `valid_to_date` SET TAGS ('dbx_business_glossary_term' = 'Valid To Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`entity_registration` ALTER COLUMN `vat_registration_number` SET TAGS ('dbx_business_glossary_term' = 'VAT Registration Number');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` SET TAGS ('dbx_subdomain' = 'facility_network');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` SET TAGS ('dbx_association_edges' = 'masterdata.material,masterdata.plant');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `material_plant_data_id` SET TAGS ('dbx_business_glossary_term' = 'Material Plant Data ID');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `material_id` SET TAGS ('dbx_business_glossary_term' = 'Material Plant Data - Masterdata Material Master Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Material Plant Data - Plant Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `abc_indicator` SET TAGS ('dbx_business_glossary_term' = 'ABC Indicator');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `lot_size` SET TAGS ('dbx_business_glossary_term' = 'Lot Size Procedure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `mrp_type` SET TAGS ('dbx_business_glossary_term' = 'MRP Type');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `plant_specific_material_number` SET TAGS ('dbx_business_glossary_term' = 'Plant-Specific Material Number');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `procurement_type` SET TAGS ('dbx_business_glossary_term' = 'Procurement Type');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `production_scheduler` SET TAGS ('dbx_business_glossary_term' = 'Production Scheduler');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `quality_inspection_group` SET TAGS ('dbx_business_glossary_term' = 'Quality Inspection Group');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `reorder_point` SET TAGS ('dbx_business_glossary_term' = 'Reorder Point');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `safety_stock_quantity` SET TAGS ('dbx_business_glossary_term' = 'Safety Stock Quantity');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `storage_location_default` SET TAGS ('dbx_business_glossary_term' = 'Default Storage Location');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `valid_from_date` SET TAGS ('dbx_business_glossary_term' = 'Valid From Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_plant_data` ALTER COLUMN `valid_to_date` SET TAGS ('dbx_business_glossary_term' = 'Valid To Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` SET TAGS ('dbx_subdomain' = 'facility_network');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` SET TAGS ('dbx_association_edges' = 'masterdata.material,masterdata.storage_location');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `material_storage_location_data_id` SET TAGS ('dbx_business_glossary_term' = 'Material Storage Location Data ID');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `material_id` SET TAGS ('dbx_business_glossary_term' = 'Material Storage Location Data - Masterdata Material Master Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `storage_location_id` SET TAGS ('dbx_business_glossary_term' = 'Material Storage Location Data - Storage Location Id');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `batch_management_indicator` SET TAGS ('dbx_business_glossary_term' = 'Batch Management Indicator');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `bin_location` SET TAGS ('dbx_business_glossary_term' = 'Bin Location');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `hazmat_storage_class` SET TAGS ('dbx_business_glossary_term' = 'Hazmat Storage Class');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `maximum_stock_quantity` SET TAGS ('dbx_business_glossary_term' = 'Maximum Stock Quantity');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `minimum_stock_quantity` SET TAGS ('dbx_business_glossary_term' = 'Minimum Stock Quantity');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `picking_area` SET TAGS ('dbx_business_glossary_term' = 'Picking Area');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `putaway_strategy` SET TAGS ('dbx_business_glossary_term' = 'Putaway Strategy');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `reorder_point` SET TAGS ('dbx_business_glossary_term' = 'Reorder Point');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `storage_bin_type` SET TAGS ('dbx_business_glossary_term' = 'Storage Bin Type');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `temperature_monitoring_required` SET TAGS ('dbx_business_glossary_term' = 'Temperature Monitoring Required');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `valid_from_date` SET TAGS ('dbx_business_glossary_term' = 'Valid From Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`material_storage_location_data` ALTER COLUMN `valid_to_date` SET TAGS ('dbx_business_glossary_term' = 'Valid To Date');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`mapping_rule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`mapping_rule` SET TAGS ('dbx_subdomain' = 'corporate_structure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`mapping_rule` ALTER COLUMN `mapping_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Mapping Rule Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`mapping_rule` ALTER COLUMN `parent_mapping_rule_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` SET TAGS ('dbx_subdomain' = 'corporate_structure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ALTER COLUMN `merger_acquisition_event_id` SET TAGS ('dbx_business_glossary_term' = 'Merger Acquisition Event Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ALTER COLUMN `acquirer_legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Acquirer Legal Entity Id (Foreign Key)');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ALTER COLUMN `preceding_merger_acquisition_event_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ALTER COLUMN `expected_synergy_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`merger_acquisition_event` ALTER COLUMN `transaction_value_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`functional_area` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`functional_area` SET TAGS ('dbx_subdomain' = 'corporate_structure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`functional_area` ALTER COLUMN `functional_area_id` SET TAGS ('dbx_business_glossary_term' = 'Functional Area Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`functional_area` ALTER COLUMN `budget_allocation_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`functional_area` ALTER COLUMN `headcount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`business_area` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`business_area` SET TAGS ('dbx_subdomain' = 'corporate_structure');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`business_area` ALTER COLUMN `business_area_id` SET TAGS ('dbx_business_glossary_term' = 'Business Area Identifier');
ALTER TABLE `pharmaceuticals_ecm`.`masterdata`.`business_area` ALTER COLUMN `budget_amount` SET TAGS ('dbx_confidential' = 'true');
