-- Schema for Domain: sales | Business: Mining | Version: v1_ecm
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`offtake_agreement` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Long-term offtake agreements are often secured during project financing and tied to specific mine development projects. Required for project bankability assessments, debt covenant compliance, and demo',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Offtake agreements require a designated commercial manager responsible for negotiation, execution, and ongoing management. Critical for contract accountability, approval workflows, renewal management,',
    ...
    `destination_port` STRING COMMENT 'Name of the destination port or delivery location specified in the agreement (e.g., Qingdao, Rotterdam, Yokohama).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`commodity_order` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Orders may reference the source capital project (specific mine phase or expansion) for production traceability and project revenue attribution. Useful for tracking which capital investments are genera',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Commodity orders require a sales coordinator or trader responsible for order execution, customer communication, and delivery coordination. Essential for operational accountability, customer relationsh',
    ...
    `spot_trade_id` BIGINT COMMENT 'Foreign key linking to sales.spot_trade. Business justification: commodity_order has spot_market_reference (STRING) which for spot orders should link to the spot_trade record. Adding spot_trade_id FK establishes the relationship between spot orders and their trade ',
    ...
    `delivery_destination` STRING COMMENT 'Named port, terminal, or facility where the commodity is to be delivered. Examples include Port Hedland, Qingdao Port, Rotterdam.',
    ...
    `pricing_basis` STRING COMMENT 'Mechanism by which the order price is determined. Fixed pricing is agreed upfront; provisional pricing is subject to final assay and market adjustment; index-linked pricing references a benchmark (e.g., Platts, TSI); formula pricing applies a contractual calculation.. Valid values are `fixed|provisional|index_linked|formula`',
    ...
    `quality_specification` STRING COMMENT 'Detailed quality parameters and tolerances agreed for this order (e.g., Fe content, moisture limits, impurity thresholds). Used for quality assurance and contract compliance.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`shipment_nomination` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the sales or logistics user who created and submitted this shipment nomination.',
    ...
    `discharge_port_code` STRING COMMENT 'Five-character UN/LOCODE for the destination port or terminal where the commodity will be discharged.. Valid values are `^[A-Z]{5}$`',
    `discharge_port_name` STRING COMMENT 'Human-readable name of the destination port or terminal where the commodity will be discharged.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`benchmark_price` (
    ...
    `commodity` STRING COMMENT 'The mineral commodity that this benchmark price index represents (e.g., Iron Ore, Copper, Coal, Lithium, Nickel). [ENUM-REF-CANDIDATE: Iron Ore|Copper|Coal|Lithium|Nickel|Zinc|Lead|Aluminum|Gold|Silver — 10 candidates stripped; promote to reference product]',
    `commodity_grade` STRING COMMENT 'The specific grade or quality specification of the commodity (e.g., 62% Fe, 65% Fe, Grade A Copper Cathode, Thermal Coal 6000 kcal/kg NAR, Battery Grade Lithium Carbonate).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`invoice` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Sales invoices require approval by authorized commercial or finance personnel for financial controls, segregation of duties, and audit compliance. Critical for revenue recognition controls, fraud prev',
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Revenue invoices attributed to specific capital projects enable project-level P&L tracking, NPV realization monitoring, and post-investment reviews. Mining companies need to track actual revenue again',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`provisional_adjustment` (
    ...
    `pricing_formula` STRING COMMENT 'Text description of the contractual pricing formula used to calculate the final price from the benchmark (e.g., Average of Platts IODEX CFR China 62% Fe over QP minus USD 2.50 discount).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`revenue_forecast` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Revenue forecasts in mining are directly tied to specific capital projects (new mines, expansions, brownfield developments) that will generate the forecast production volumes. Essential for project NP',
    ...
    `plan_scenario_id` BIGINT COMMENT 'Foreign key linking to mine.mine_plan_scenario. Business justification: Revenue forecasts are scenario-specific, modeling different price/volume/cost assumptions. Finance generates multiple revenue forecasts aligned with mine plan scenarios for sensitivity analysis, risk ',
    ...
    `employee_id` BIGINT COMMENT 'Reference to the user (typically CFO, Commercial Director, or authorized approver) who approved this forecast version.',
    ...
    `revenue_prepared_by_user_employee_id` BIGINT COMMENT 'Reference to the user (typically Sales Analyst, Commercial Planner, or Finance Analyst) who prepared this forecast.',
    ...
    `product_grade` STRING COMMENT 'The grade or quality specification of the commodity being forecast (e.g., 62% Fe Iron Ore Fines, Coking Coal Premium Low Vol, Copper Concentrate 28% Cu).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`volume_plan` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Sales volume plans must align with capital project delivery schedules (mine commissioning dates, plant expansion completions). Critical for production-sales alignment, LOM planning, and ensuring offta',
    ...
    `commodity_type` STRING COMMENT 'Type of mineral commodity covered by this volume plan.. Valid values are `IRON_ORE|COPPER|COAL|LITHIUM|NICKEL|GOLD`',
    ...
    `product_grade` STRING COMMENT 'Specification grade of the commodity product (e.g., Fe 62%, Cu 25%, Thermal Coal 6000 kcal/kg). Defines quality and pricing tier.',
    `product_specification_code` STRING COMMENT 'Reference code for the detailed product specification document defining quality parameters, impurity limits, and physical characteristics.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`cargo_shipment` (
    ...
    `freight_contract_id` BIGINT COMMENT 'Foreign key linking to sales.freight_contract. Business justification: Each cargo shipment is executed under a governing freight contract. Adding freight_contract_id links the shipment to its contractual terms, enabling freight cost validation and contract utilization tr',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Cargo shipments require a logistics coordinator responsible for vessel operations, port coordination, documentation, and demurrage management. Critical for operational accountability, dispute resoluti',
    ...
    `voyage_number` STRING COMMENT 'Unique voyage number assigned by the carrier for this specific sailing or trip.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`bill_of_lading` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Bills of lading require issuance by authorized shipping personnel for legal validity, customs clearance, and title transfer. Essential for accountability, regulatory compliance, and dispute resolution',
    ...
    `vendor_id` BIGINT COMMENT 'Reference to the shipping carrier or freight company issuing the bill of lading.',
    ...
    `vessel_name` STRING COMMENT 'Name of the vessel carrying the commodity cargo as stated on the bill of lading.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`quality_certificate` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Quality certificates require sign-off by authorized personnel (chief geologist, quality manager) for regulatory compliance and legal validity. Essential for dispute resolution, audit trails, and ensur',
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Quality certificates drive pricing adjustments that impact cost centre revenue and AISC calculations. Essential for accurate mine-level financial performance tracking and quality-based revenue adjustm',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`spot_trade` (
    `spot_trade_id` BIGINT COMMENT 'Unique identifier for the spot commodity trade transaction. Primary key.',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Spot trades are allocated to cost centres for immediate revenue recognition and variance-to-plan analysis at mine level. Required for operational financial tracking and opportunistic sales management.',
    `counterparty_id` BIGINT COMMENT 'Reference to the buyer or seller counterparty involved in this spot trade transaction.',
    `price_index_id` BIGINT COMMENT 'Foreign key linking to sales.price_index. Business justification: spot_trade has pricing_index_reference (STRING) which should be normalized to a FK to price_index master. This allows proper referential integrity for commodity pricing indices used in spot trades. Th',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Spot trades must be allocated to profit centres for segment reporting and commodity-level performance tracking. Essential for financial reporting and management analysis of spot market activity.',
    `saleable_product_id` BIGINT COMMENT 'Foreign key linking to product.saleable_product. Business justification: Spot trades specify exact product grade being traded on the spot market. Essential for pricing accuracy, delivery specification clarity, quality commitment definition, and ensuring trade execution mat',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Spot trades require attribution to the specific trader who executed the transaction for performance measurement, risk management, and trading desk accountability. Replaces denormalized trader_name wit',
    `agreed_price_per_tonne` DECIMAL(18,2) COMMENT 'The negotiated unit price per metric tonne for this spot trade. Business-confidential pricing information.',
    `broker_commission_rate` DECIMAL(18,2) COMMENT 'Commission rate (as a decimal percentage) paid to the broker for facilitating the trade. Business-confidential commercial information.',
    `broker_involved` BOOLEAN COMMENT 'Indicates whether a third-party broker or intermediary was involved in facilitating this spot trade transaction.',
    `broker_name` STRING COMMENT 'Name of the brokerage firm or intermediary that facilitated the spot trade, if applicable.',
    `contract_reference` STRING COMMENT 'Reference number of the formal sales contract or confirmation document governing this spot trade.',
    `created_timestamp` TIMESTAMP COMMENT 'System timestamp when this spot trade record was first created in the data system.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the trade transaction (typically USD for commodity trades).. Valid values are `USD|EUR|AUD|CNY|GBP|JPY`',
    `delivery_location` STRING COMMENT 'Named port, terminal, or location where commodity delivery will occur as per the delivery terms (e.g., Port Hedland, Qingdao Port, Rotterdam).',
    `delivery_terms` STRING COMMENT 'International Commercial Terms (Incoterms) defining delivery obligations, risk transfer, and cost allocation between buyer and seller (e.g., FOB - Free on Board, CIF - Cost Insurance and Freight, CFR - Cost and Freight).. Valid values are `FOB|CFR|CIF|DAP|EXW|FCA`',
    `delivery_window_end` DATE COMMENT 'The latest date within the agreed delivery period for this spot trade. Defines the delivery deadline.',
    `delivery_window_start` DATE COMMENT 'The earliest date within the agreed delivery period for this spot trade. Spot trades typically have near-term delivery windows.',
    `last_modified_by` STRING COMMENT 'User identifier or name of the person who last modified this spot trade record.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'System timestamp when this spot trade record was most recently updated or modified.',
    `payment_terms` STRING COMMENT 'Agreed payment conditions for the spot trade (e.g., 30 days from bill of lading, letter of credit, advance payment, cash against documents).',
    `pricing_basis` STRING COMMENT 'The benchmark or pricing mechanism used to determine the trade price (e.g., Platts IODEX, TSI, Metal Bulletin, Argus, fixed price, index-linked).. Valid values are `platts|tsi|metal_bulletin|argus|fixed|index_linked`',
    `quality_adjustment_clause` STRING COMMENT 'Terms defining price adjustments based on actual delivered quality versus specification (e.g., penalties for off-spec material, bonuses for premium quality).',
    `quality_specification` STRING COMMENT 'Detailed quality parameters and specifications agreed for the commodity (e.g., Fe content, moisture, impurities, size distribution). May reference standard specifications or custom requirements.',
    `settlement_date` DATE COMMENT 'The date on which financial settlement and payment for the spot trade is due or completed.',
    `total_trade_value` DECIMAL(18,2) COMMENT 'Total monetary value of the spot trade, calculated as trade volume multiplied by agreed price. Business-confidential financial information.',
    `trade_date` DATE COMMENT 'The date on which the spot trade was executed and agreed between parties. Distinct from settlement or delivery dates.',
    `trade_notes` STRING COMMENT 'Free-text field for additional notes, special conditions, or remarks related to this spot trade transaction.',
    `trade_number` STRING COMMENT 'Externally-known unique business identifier for the spot trade, used in contracts and communications.. Valid values are `^ST-[0-9]{8}-[A-Z0-9]{6}$`',
    `trade_status` STRING COMMENT 'Current lifecycle status of the spot trade transaction. Tracks progression from draft through settlement or cancellation.. Valid values are `draft|confirmed|executed|settled|cancelled|disputed`',
    `trade_timestamp` TIMESTAMP COMMENT 'Precise date and time when the spot trade was executed, including timezone information.',
    `trade_volume_tonnes` DECIMAL(18,2) COMMENT 'The quantity of commodity being traded, measured in metric tonnes (dry metric tonnes for bulk commodities).',
    `created_by` STRING COMMENT 'User identifier or name of the person who created this spot trade record in the system.',
    CONSTRAINT pk_spot_trade PRIMARY KEY(`spot_trade_id`)
) COMMENT 'Record of a spot commodity trade executed outside of a term offtake agreement. Captures trade date, commodity, volume, agreed price, pricing basis, delivery terms, counterparty, settlement date, and trade status. Distinct from term offtake orders — spot trades are one-off market transactions with immediate or near-term delivery.';
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`offtake_schedule` (
    ...
    `benchmark_index` STRING COMMENT 'Commodity pricing benchmark index used for this scheduled lifting. Platts, TSI (The Steel Index), Argus, LME (London Metal Exchange), Metal Bulletin.. Valid values are `platts|tsi|argus|lme|metal_bulletin`',
    ...
    `destination_port` STRING COMMENT 'Port facility where the commodity is scheduled to be delivered.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`performance_actual` (
    `performance_actual_id` BIGINT COMMENT 'Primary key for performance_actual',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Actual sales performance is recorded against cost centres for variance analysis, AISC calculation, and mine-level profitability tracking. Core requirement for operational performance reporting and cos',
    `counterparty_id` BIGINT COMMENT 'Reference to the customer or counterparty to whom the commodity was sold.',
    `offtake_agreement_id` BIGINT COMMENT 'Reference to the offtake agreement under which this sale was executed, if applicable.',
    `orebody_id` BIGINT COMMENT 'Foreign key linking to geology.orebody. Business justification: Actual sales performance is tracked and reconciled by orebody to compare realized revenue against orebody-specific forecasts and resource models. Essential for orebody-level P&L analysis, reserve depl',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Sales performance actuals require preparation by commercial analysts for management reporting, variance analysis, and board reporting. Critical for accountability, audit trails, and ensuring data qual',
    `price_index_id` BIGINT COMMENT 'Foreign key linking to sales.price_index. Business justification: performance_actual has benchmark_index (STRING) which is a code reference to the price_index master used for performance measurement. Adding price_index_id FK allows proper normalization. The benchmar',
    `revenue_forecast_id` BIGINT COMMENT 'Foreign key linking to sales.revenue_forecast. Business justification: Performance actuals should reference the revenue forecast they are measured against. This enables variance analysis between forecast and actual. The FK allows tracking which forecast version the actua',
    `saleable_product_id` BIGINT COMMENT 'Foreign key linking to product.saleable_product. Business justification: Performance tracking by specific product grade is essential for product line profitability analysis, pricing strategy optimization, portfolio management decisions, and understanding margin contributio',
    `actual_revenue_amount` DECIMAL(18,2) COMMENT 'The total revenue realized from commodity sales during the period, in the reporting currency.',
    `actual_volume_tonnes` DECIMAL(18,2) COMMENT 'The actual volume of commodity sold during the period, measured in metric tonnes.',
    `aisc_per_tonne` DECIMAL(18,2) COMMENT 'The All-In Sustaining Cost per tonne for the commodity during the period, used for margin calculation.',
    `approval_date` DATE COMMENT 'The date on which the actuals record was approved for final posting.',
    `benchmark_price_per_tonne` DECIMAL(18,2) COMMENT 'The benchmark or reference price per tonne for the commodity during the period (e.g., Platts, TSI index price).',
    `created_timestamp` TIMESTAMP COMMENT 'The timestamp when this actuals record was first created in the system.',
    `currency_code` STRING COMMENT 'The three-letter ISO 4217 currency code for all monetary amounts in this record (e.g., USD, AUD, EUR).. Valid values are `^[A-Z]{3}$`',
    `delivery_basis` STRING COMMENT 'The Incoterms delivery basis for the sale (e.g., FOB - Free on Board, CIF - Cost Insurance and Freight, CFR - Cost and Freight).. Valid values are `fob|cif|cfr|exw|dap|ddp`',
    `destination_country_code` STRING COMMENT 'The three-letter ISO 3166-1 alpha-3 country code of the destination country for the commodity shipment.. Valid values are `^[A-Z]{3}$`',
    `last_modified_timestamp` TIMESTAMP COMMENT 'The timestamp when this actuals record was last modified or updated.',
    `lom_forecast_alignment_flag` BOOLEAN COMMENT 'Indicates whether the actual performance aligns with the Life of Mine production forecast (True/False).',
    `margin_per_tonne` DECIMAL(18,2) COMMENT 'The AISC-adjusted margin per tonne, calculated as realised price minus AISC per tonne.',
    `notes` STRING COMMENT 'Free-text notes or comments regarding the actual performance, including explanations for significant variances or adjustments.',
    `performance_status` STRING COMMENT 'The status of the actuals record (e.g., confirmed, provisional, adjusted, final) indicating the level of finalization.. Valid values are `confirmed|provisional|adjusted|final`',
    `period_end_date` DATE COMMENT 'The last date of the reporting period for which actuals are posted.',
    `period_month` STRING COMMENT 'The calendar month (1-12) for which this actual performance is recorded.',
    `period_quarter` STRING COMMENT 'The fiscal quarter (1-4) for which this actual performance is recorded.',
    `period_year` STRING COMMENT 'The calendar year for which this actual performance is recorded (e.g., 2024).',
    `planned_revenue_amount` DECIMAL(18,2) COMMENT 'The planned or budgeted revenue for the period, in the reporting currency, used for variance calculation.',
    `planned_volume_tonnes` DECIMAL(18,2) COMMENT 'The planned or budgeted volume of commodity for the period, measured in metric tonnes, used for variance calculation.',
    `posting_date` DATE COMMENT 'The date on which the actual performance record was posted to the system.',
    `price_variance_per_tonne` DECIMAL(18,2) COMMENT 'The variance between realised price and benchmark price per tonne (realised minus benchmark).',
    `price_variance_percentage` DECIMAL(18,2) COMMENT 'The percentage variance between realised and benchmark price, calculated as (realised - benchmark) / benchmark * 100.',
    `realised_price_per_tonne` DECIMAL(18,2) COMMENT 'The actual average price achieved per tonne of commodity sold during the period, calculated as actual revenue divided by actual volume.',
    `revenue_variance_amount` DECIMAL(18,2) COMMENT 'The variance between actual and planned revenue (actual minus planned), in the reporting currency.',
    `revenue_variance_percentage` DECIMAL(18,2) COMMENT 'The percentage variance between actual and planned revenue, calculated as (actual - planned) / planned * 100.',
    `target_market` STRING COMMENT 'The primary geographic market or region to which the commodity was sold (e.g., China, Europe, Domestic).',
    `total_margin_amount` DECIMAL(18,2) COMMENT 'The total AISC-adjusted margin for the period, calculated as margin per tonne multiplied by actual volume.',
    `version_number` STRING COMMENT 'The version number of this actuals record, incremented with each modification to support audit trail and change tracking.',
    `volume_variance_percentage` DECIMAL(18,2) COMMENT 'The percentage variance between actual and planned volume, calculated as (actual - planned) / planned * 100.',
    `volume_variance_tonnes` DECIMAL(18,2) COMMENT 'The variance between actual and planned volume (actual minus planned), measured in metric tonnes.',
    CONSTRAINT pk_performance_actual PRIMARY KEY(`performance_actual_id`)
) COMMENT 'Periodic (monthly/quarterly) actuals record of commodity sales performance against plan, capturing actual volumes sold, actual revenue, realised price, benchmark price achieved, variance to plan, and AISC-adjusted margin. Operational record — not an aggregation layer — representing the confirmed actuals posted per period per commodity.';

CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`freight_contract` (
    `freight_contract_id` BIGINT COMMENT 'Unique identifier for the freight contract record. Primary key.',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Freight contracts require a commercial manager responsible for negotiation, execution, and ongoing management. Critical for contract accountability, renewal management, freight cost optimization, and ',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Freight contracts impact net-back pricing and are allocated to cost centres for AISC calculation, delivered cost analysis, and mine-level profitability. Essential for cost management and pricing decis',
    `counterparty_id` BIGINT COMMENT 'Reference to the shipping company, freight forwarder, or vessel operator engaged under this contract.',
    `approval_date` DATE COMMENT 'Date when internal approval was granted for this freight contract.',
    `approved_by` STRING COMMENT 'Name or identifier of the sales or procurement executive who approved this freight contract.',
    `arbitration_clause` STRING COMMENT 'Terms defining dispute resolution mechanism, including arbitration venue and rules (e.g., London Maritime Arbitrators Association, Singapore Chamber of Maritime Arbitration).',
    `benchmark_index` STRING COMMENT 'Reference freight index used for pricing or settlement (e.g., Baltic Capesize Index, Baltic Panamax Index, Platts freight assessments).',
    `bunker_adjustment_clause` STRING COMMENT 'Terms defining how changes in bunker fuel prices affect the freight rate, including base fuel price and adjustment mechanism.',
    `commodity_type` STRING COMMENT 'Type of mineral commodity covered by this freight contract, aligned with the mining companys product portfolio.. Valid values are `iron_ore|copper_concentrate|coal|lithium|nickel|mixed`',
    `contract_effective_date` DATE COMMENT 'Date when the freight contract becomes binding and operative.',
    `contract_expiry_date` DATE COMMENT 'Date when the freight contract terminates. Nullable for spot charters or open-ended arrangements.',
    `contract_number` STRING COMMENT 'Externally-known unique business identifier for the freight contract, used for reference in commercial documentation and communications.',
    `contract_period_months` STRING COMMENT 'Duration of the contract in months. Applicable primarily to time charters and contracts of affreightment.',
    `contract_status` STRING COMMENT 'Current lifecycle status of the freight contract. Draft indicates under negotiation, active indicates in force, suspended indicates temporarily paused, completed indicates all obligations fulfilled, terminated indicates early cancellation, expired indicates natural end of term.. Valid values are `draft|active|suspended|completed|terminated|expired`',
    `contract_type` STRING COMMENT 'Classification of the freight contract structure. Voyage charter is for a single voyage, time charter is for a period, contract of affreightment (COA) is for multiple shipments over time, freight forward agreement (FFA) is a derivative instrument, and spot charter is an ad-hoc arrangement.. Valid values are `voyage_charter|time_charter|contract_of_affreightment|freight_forward_agreement|spot_charter`',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this freight contract record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for all monetary values in this contract.. Valid values are `USD|AUD|EUR|CNY|GBP|JPY`',
    `demurrage_rate` DECIMAL(18,2) COMMENT 'Daily penalty rate charged when loading or discharge operations exceed the agreed laytime. Typically expressed in USD per day.',
    `despatch_rate` DECIMAL(18,2) COMMENT 'Daily bonus rate paid when loading or discharge operations are completed ahead of the agreed laytime. Typically expressed in USD per day, often 50% of demurrage rate.',
    `discharge_port` STRING COMMENT 'Destination port or port range where the commodity cargo will be discharged from the vessel.',
    `force_majeure_clause` STRING COMMENT 'Contractual terms defining events beyond parties control that excuse performance (e.g., natural disasters, war, strikes, port closures).',
    `freight_basis` STRING COMMENT 'INCOTERM defining the freight responsibility. FOB (Free on Board) - buyer arranges freight, CFR (Cost and Freight) - seller pays freight, CIF (Cost Insurance and Freight) - seller pays freight and insurance, DAP (Delivered at Place), EXW (Ex Works).. Valid values are `fob|cfr|cif|dap|exw`',
    `freight_rate` DECIMAL(18,2) COMMENT 'Contracted freight rate per unit. The unit is defined in freight_rate_unit (e.g., USD per tonne, USD per day for time charter).',
    `freight_rate_unit` STRING COMMENT 'Unit of measure for the freight rate. Per tonne for voyage charters, per day for time charters, per voyage for lump sum arrangements.. Valid values are `usd_per_tonne|usd_per_day|usd_per_voyage|aud_per_tonne|aud_per_day`',
    `governing_law` STRING COMMENT 'Legal jurisdiction whose laws govern the interpretation and enforcement of this contract (e.g., English Law, New York Law, Australian Law).',
    `insurance_responsibility` STRING COMMENT 'Party responsible for arranging and paying for cargo insurance during transit.. Valid values are `seller|buyer|shared`',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this freight contract record was last updated in the system.',
    `laytime_allowed_hours` DECIMAL(18,2) COMMENT 'Total time allowed for loading and discharge operations before demurrage applies, expressed in hours.',
    `loading_port` STRING COMMENT 'Port or port range where the commodity cargo will be loaded onto the vessel.',
    `maximum_cargo_quantity_tonnes` DECIMAL(18,2) COMMENT 'Maximum cargo quantity in metric tonnes that can be shipped per voyage or contract period.',
    `minimum_cargo_quantity_tonnes` DECIMAL(18,2) COMMENT 'Minimum cargo quantity in metric tonnes that must be shipped per voyage or contract period.',
    `net_back_impact_usd_per_tonne` DECIMAL(18,2) COMMENT 'Calculated impact of this freight contract on net-back realisation per tonne of commodity sold. Critical for sales margin analysis.',
    `notes` STRING COMMENT 'Free-text field for additional comments, special terms, or operational notes related to this freight contract.',
    `payment_due_days` STRING COMMENT 'Number of days after invoice date or completion of voyage when payment is due.',
    `payment_terms` STRING COMMENT 'Description of payment terms and conditions, including timing, method, and any advance payment requirements.',
    `port_range` STRING COMMENT 'Geographic range of ports covered by this contract (e.g., Australian East Coast, Chinese North Ports, Rotterdam-Hamburg range).',
    `price_adjustment_formula` STRING COMMENT 'Formula or mechanism for adjusting freight rates based on fuel prices, market indices, or other variables (e.g., bunker adjustment factor).',
    `signed_date` DATE COMMENT 'Date when the freight contract was formally executed by all parties.',
    `version_number` STRING COMMENT 'Version number tracking amendments and revisions to the freight contract over its lifecycle.',
    `vessel_type` STRING COMMENT 'Classification of vessel size and capability. Capesize (150,000+ DWT), Panamax (60,000-80,000 DWT), Handymax (40,000-60,000 DWT), Handysize (10,000-40,000 DWT), VLOC (Very Large Ore Carrier 200,000+ DWT), Supramax (50,000-60,000 DWT).. Valid values are `capesize|panamax|handymax|handysize|vloc|supramax`',
    CONSTRAINT pk_freight_contract PRIMARY KEY(`freight_contract_id`)
) COMMENT 'Master record of freight and shipping contracts engaged to transport commodity cargoes, including voyage charters, time charters, and freight forward agreements. Captures vessel type, freight rate, freight basis (FOB/CFR/CIF), contract period, port range, demurrage rate, and despatch terms. Owned by sales as freight cost directly impacts net-back realisation.';

CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`demurrage_claim` (
    `demurrage_claim_id` BIGINT COMMENT 'Unique identifier for the demurrage or despatch claim record. Primary key for the demurrage claim entity.',
    `cargo_shipment_id` BIGINT COMMENT 'Foreign key linking to sales.cargo_shipment. Business justification: demurrage_claim has bill_of_lading_number (STRING) which identifies the shipment for which demurrage is claimed. Adding cargo_shipment_id FK establishes proper relationship. The bill_of_lading_number ',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Demurrage claims require a designated claims manager responsible for dispute resolution, negotiation, and recovery. Essential for accountability, performance tracking, legal compliance, and optimizing',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Demurrage claims affect net revenue and are charged to cost centres for AISC impact assessment and operational performance tracking. Required for accurate mine-level cost and revenue reporting.',
    `counterparty_id` BIGINT COMMENT 'Reference to the counterparty (buyer, shipping line, or freight forwarder) involved in the demurrage or despatch claim.',
    `freight_contract_id` BIGINT COMMENT 'Foreign key linking to sales.freight_contract. Business justification: demurrage_claim has freight_contract_reference (STRING) which identifies the freight contract under which the claim arises. Adding freight_contract_id FK establishes proper relationship between claim ',
    `offtake_agreement_id` BIGINT COMMENT 'Reference to the parent offtake agreement under which this shipment and claim occurred, if applicable.',
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: Demurrage claims result in settlement invoices (either demurrage charges to customer or despatch credits from carrier). Adding settlement_invoice_id (FK to invoice.sales_invoice_id) links the claim to',
    `berth_name` STRING COMMENT 'Specific berth or wharf at the port where the vessel was berthed during loading or discharge operations.',
    `cargo_quantity_tonnes` DECIMAL(18,2) COMMENT 'Total quantity of commodity loaded or discharged, measured in metric tonnes.',
    `claim_agreed_date` DATE COMMENT 'Date on which the counterparty agreed to the claim amount and terms.',
    `claim_amount_usd` DECIMAL(18,2) COMMENT 'Total monetary value of the demurrage or despatch claim in USD, calculated from laytime variance and applicable rate.',
    `claim_number` STRING COMMENT 'Externally-known unique business identifier for the demurrage or despatch claim, used in correspondence with counterparties and freight forwarders.',
    `claim_raised_date` DATE COMMENT 'Date on which the demurrage or despatch claim was formally raised with the counterparty.',
    `claim_settled_date` DATE COMMENT 'Date on which the demurrage or despatch claim was financially settled (payment received or credit applied).',
    `claim_status` STRING COMMENT 'Current lifecycle status of the demurrage or despatch claim in the settlement workflow.. Valid values are `open|agreed|disputed|settled|withdrawn|cancelled`',
    `claim_type` STRING COMMENT 'Type of claim: demurrage (penalty for exceeding laytime) or despatch (credit for completing within laytime).. Valid values are `demurrage|despatch`',
    `commodity_type` STRING COMMENT 'Type of mineral commodity shipped (e.g., iron ore, copper concentrate, coal, lithium, nickel).',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this demurrage claim record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the claim amount (typically USD for international commodity trade).. Valid values are `^[A-Z]{3}$`',
    `demurrage_rate_usd_per_day` DECIMAL(18,2) COMMENT 'Contractual demurrage rate in USD per day charged for exceeding the allowed laytime.',
    `despatch_rate_usd_per_day` DECIMAL(18,2) COMMENT 'Contractual despatch rate in USD per day credited for completing operations within the allowed laytime.',
    `dispute_flag` BOOLEAN COMMENT 'Indicates whether the claim is currently under dispute with the counterparty (True) or not (False).',
    `dispute_reason` STRING COMMENT 'Description of the reason for dispute if the claim is contested by the counterparty (e.g., laytime calculation disagreement, force majeure event, weather delays).',
    `dispute_resolution_notes` STRING COMMENT 'Detailed notes on dispute resolution activities, arbitration proceedings, or settlement negotiations.',
    `imo_number` STRING COMMENT 'Unique seven-digit IMO ship identification number assigned to the vessel for permanent identification.. Valid values are `^IMO[0-9]{7}$`',
    `incoterm` STRING COMMENT 'Incoterm defining the delivery terms and responsibility for freight costs (e.g., FOB, CIF, CFR).. Valid values are `^(EXW|FCA|CPT|CIP|DAP|DPU|DDP|FAS|FOB|CFR|CIF)$`',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Date and time when this demurrage claim record was last updated.',
    `laytime_allowed_hours` DECIMAL(18,2) COMMENT 'Total laytime (in hours) contractually allowed for loading or discharge operations as per the charter party or freight contract.',
    `laytime_used_hours` DECIMAL(18,2) COMMENT 'Actual laytime (in hours) consumed during loading or discharge operations, calculated from Notice of Readiness (NOR) to completion.',
    `laytime_variance_hours` DECIMAL(18,2) COMMENT 'Difference between laytime used and laytime allowed (positive indicates demurrage, negative indicates despatch).',
    `loading_commenced_timestamp` TIMESTAMP COMMENT 'Date and time when actual loading or discharge operations commenced.',
    `loading_completed_timestamp` TIMESTAMP COMMENT 'Date and time when loading or discharge operations were completed and the vessel was ready to depart.',
    `net_back_impact_usd` DECIMAL(18,2) COMMENT 'Financial impact of the demurrage or despatch claim on the sales net-back realization (negative for demurrage, positive for despatch).',
    `notes` STRING COMMENT 'Additional free-text notes or comments related to the demurrage or despatch claim, including operational context or special circumstances.',
    `notice_of_readiness_timestamp` TIMESTAMP COMMENT 'Date and time when the vessel tendered Notice of Readiness (NOR) to commence loading or discharge operations.',
    `port_code` STRING COMMENT 'Five-character UN/LOCODE code identifying the port of loading or discharge.. Valid values are `^[A-Z]{5}$`',
    `port_of_loading` STRING COMMENT 'Name of the port where the commodity was loaded onto the vessel.',
    `responsible_party` STRING COMMENT 'Party responsible for the demurrage or despatch claim (seller, buyer, shipping line, or freight forwarder) as per contract terms.. Valid values are `seller|buyer|shipping_line|freight_forwarder`',
    `vessel_name` STRING COMMENT 'Name of the vessel involved in the loading or discharge operation that triggered the demurrage or despatch claim.',
    CONSTRAINT pk_demurrage_claim PRIMARY KEY(`demurrage_claim_id`)
) COMMENT 'Record of demurrage and despatch claims arising from vessel loading or discharge operations exceeding or completing within the agreed laytime at port. Captures vessel name, port, berth, laytime allowed (hours), laytime used, demurrage/despatch rate (USD/day), claim amount, claim status (open/agreed/disputed/settled), counterparty, and dispute resolution notes. Directly impacts sales net-back realisation and is reconciled against freight contracts.';
...
CREATE OR REPLACE TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` (
    `agreement_stakeholder_consultation_id` BIGINT COMMENT 'Primary key for the agreement_stakeholder_consultation association',
    `offtake_agreement_id` BIGINT COMMENT 'Foreign key linking to the offtake agreement subject to stakeholder consultation',
    `stakeholder_id` BIGINT COMMENT 'Foreign key linking to the community stakeholder being consulted',
    `agreement_acceptance_date` DATE COMMENT 'Date on which this stakeholder formally accepted or provided consent for this offtake agreement. Null if consent not yet obtained or not required.',
    `benefit_entitlement_percentage` DECIMAL(18,2) COMMENT 'Percentage of revenue or benefits from this specific offtake agreement that this stakeholder is entitled to receive under benefit-sharing arrangements. Null if stakeholder has no financial entitlement.',
    `conditions_imposed` STRING COMMENT 'Free-text description of any conditions or requirements this stakeholder imposed as part of their consent or consultation response for this agreement. May include environmental conditions, benefit requirements, or operational constraints.',
    `consent_status` STRING COMMENT 'Current status of consent or approval from this stakeholder for this specific offtake agreement. Tracks FPIC status, regulatory approvals, or community consent processes.',
    `consultation_date` DATE COMMENT 'Date on which formal consultation with this stakeholder regarding this offtake agreement was conducted. Records when engagement occurred.',
    `consultation_method` STRING COMMENT 'The method or process used to consult this stakeholder on this agreement. Captures whether traditional protocols, FPIC processes, or regulatory hearings were used.',
    `consultation_status` STRING COMMENT 'Current status of the consultation process with this stakeholder for this agreement. Tracks progress through the engagement lifecycle.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this consultation record was first created in the system.',
    `stakeholder_role` STRING COMMENT 'The specific role this stakeholder plays in relation to this offtake agreement. Consulted party indicates formal consultation requirement, consent authority indicates FPIC or regulatory approval authority, beneficiary indicates entitled to benefits from the agreement.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this consultation record was last modified.',
    CONSTRAINT pk_agreement_stakeholder_consultation PRIMARY KEY(`agreement_stakeholder_consultation_id`)
) COMMENT 'This association product represents the consultation and consent relationship between offtake agreements and community stakeholders. It captures the formal engagement process where stakeholders (indigenous groups, local communities, government bodies) are consulted on, provide consent for, or hold benefit entitlements in specific offtake agreements. Each record links one offtake agreement to one stakeholder with attributes that exist only in the context of this consultation relationship.. Existence Justification: In mining operations, a single offtake agreement can require consultation, consent, or benefit-sharing arrangements with multiple community stakeholders (indigenous groups with native title, local communities affected by transport routes, government regulatory bodies, traditional owners). Conversely, a single stakeholder may be consulted on or hold entitlements in multiple offtake agreements across different commodities, mine sites, or time periods. The consultation relationship itself carries critical operational data including consent status, benefit entitlements, consultation dates, and conditions imposed.';
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ADD CONSTRAINT `fk_sales_commodity_order_spot_trade_id` FOREIGN KEY (`spot_trade_id`) REFERENCES `mining_ecm`.`sales`.`spot_trade`(`spot_trade_id`);
...
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` ADD CONSTRAINT `fk_sales_cargo_shipment_freight_contract_id` FOREIGN KEY (`freight_contract_id`) REFERENCES `mining_ecm`.`sales`.`freight_contract`(`freight_contract_id`);
...
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ADD CONSTRAINT `fk_sales_spot_trade_price_index_id` FOREIGN KEY (`price_index_id`) REFERENCES `mining_ecm`.`sales`.`price_index`(`price_index_id`);
...
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ADD CONSTRAINT `fk_sales_performance_actual_offtake_agreement_id` FOREIGN KEY (`offtake_agreement_id`) REFERENCES `mining_ecm`.`sales`.`offtake_agreement`(`offtake_agreement_id`);
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ADD CONSTRAINT `fk_sales_performance_actual_price_index_id` FOREIGN KEY (`price_index_id`) REFERENCES `mining_ecm`.`sales`.`price_index`(`price_index_id`);
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ADD CONSTRAINT `fk_sales_performance_actual_revenue_forecast_id` FOREIGN KEY (`revenue_forecast_id`) REFERENCES `mining_ecm`.`sales`.`revenue_forecast`(`revenue_forecast_id`);
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ADD CONSTRAINT `fk_sales_demurrage_claim_cargo_shipment_id` FOREIGN KEY (`cargo_shipment_id`) REFERENCES `mining_ecm`.`sales`.`cargo_shipment`(`cargo_shipment_id`);
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ADD CONSTRAINT `fk_sales_demurrage_claim_freight_contract_id` FOREIGN KEY (`freight_contract_id`) REFERENCES `mining_ecm`.`sales`.`freight_contract`(`freight_contract_id`);
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ADD CONSTRAINT `fk_sales_demurrage_claim_offtake_agreement_id` FOREIGN KEY (`offtake_agreement_id`) REFERENCES `mining_ecm`.`sales`.`offtake_agreement`(`offtake_agreement_id`);
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ADD CONSTRAINT `fk_sales_demurrage_claim_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `mining_ecm`.`sales`.`invoice`(`invoice_id`);
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ADD CONSTRAINT `fk_sales_agreement_stakeholder_consultation_offtake_agreement_id` FOREIGN KEY (`offtake_agreement_id`) REFERENCES `mining_ecm`.`sales`.`offtake_agreement`(`offtake_agreement_id`);
...
ALTER TABLE `mining_ecm`.`sales`.`offtake_agreement` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`offtake_agreement` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Manager Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`offtake_agreement` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`offtake_agreement` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`offtake_agreement` ALTER COLUMN `destination_port` SET TAGS ('dbx_business_glossary_term' = 'Destination Port');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` SET TAGS ('dbx_subdomain' = 'contract_management');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Order Manager Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `spot_trade_id` SET TAGS ('dbx_business_glossary_term' = 'Spot Trade Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `delivery_destination` SET TAGS ('dbx_business_glossary_term' = 'Delivery Destination');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `pricing_basis` SET TAGS ('dbx_business_glossary_term' = 'Pricing Basis');
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `pricing_basis` SET TAGS ('dbx_value_regex' = 'fixed|provisional|index_linked|formula');
...
ALTER TABLE `mining_ecm`.`sales`.`commodity_order` ALTER COLUMN `quality_specification` SET TAGS ('dbx_business_glossary_term' = 'Quality Specification');
...
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` SET TAGS ('dbx_subdomain' = 'contract_management');
...
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Nominated By User Identifier (ID)');
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` ALTER COLUMN `discharge_port_code` SET TAGS ('dbx_business_glossary_term' = 'Discharge Port Code');
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` ALTER COLUMN `discharge_port_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{5}$');
ALTER TABLE `mining_ecm`.`sales`.`shipment_nomination` ALTER COLUMN `discharge_port_name` SET TAGS ('dbx_business_glossary_term' = 'Discharge Port Name');
...
ALTER TABLE `mining_ecm`.`sales`.`benchmark_price` SET TAGS ('dbx_subdomain' = 'contract_management');
...
ALTER TABLE `mining_ecm`.`sales`.`benchmark_price` ALTER COLUMN `commodity` SET TAGS ('dbx_business_glossary_term' = 'Commodity');
ALTER TABLE `mining_ecm`.`sales`.`benchmark_price` ALTER COLUMN `commodity_grade` SET TAGS ('dbx_business_glossary_term' = 'Commodity Grade');
...
ALTER TABLE `mining_ecm`.`sales`.`invoice` SET TAGS ('dbx_subdomain' = 'revenue_settlement');
ALTER TABLE `mining_ecm`.`sales`.`invoice` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier');
ALTER TABLE `mining_ecm`.`sales`.`invoice` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`invoice` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`invoice` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`invoice` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`provisional_adjustment` SET TAGS ('dbx_subdomain' = 'revenue_settlement');
...
ALTER TABLE `mining_ecm`.`sales`.`provisional_adjustment` ALTER COLUMN `pricing_formula` SET TAGS ('dbx_business_glossary_term' = 'Pricing Formula');
...
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` SET TAGS ('dbx_subdomain' = 'revenue_settlement');
...
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `plan_scenario_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Plan Scenario Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User ID');
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `revenue_prepared_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Forecast Prepared By User ID');
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `revenue_prepared_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `revenue_prepared_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`revenue_forecast` ALTER COLUMN `product_grade` SET TAGS ('dbx_business_glossary_term' = 'Product Grade');
...
ALTER TABLE `mining_ecm`.`sales`.`volume_plan` SET TAGS ('dbx_subdomain' = 'revenue_settlement');
...
ALTER TABLE `mining_ecm`.`sales`.`volume_plan` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`volume_plan` ALTER COLUMN `commodity_type` SET TAGS ('dbx_business_glossary_term' = 'Commodity Type');
ALTER TABLE `mining_ecm`.`sales`.`volume_plan` ALTER COLUMN `commodity_type` SET TAGS ('dbx_value_regex' = 'IRON_ORE|COPPER|COAL|LITHIUM|NICKEL|GOLD');
...
ALTER TABLE `mining_ecm`.`sales`.`volume_plan` ALTER COLUMN `product_grade` SET TAGS ('dbx_business_glossary_term' = 'Product Grade');
ALTER TABLE `mining_ecm`.`sales`.`volume_plan` ALTER COLUMN `product_specification_code` SET TAGS ('dbx_business_glossary_term' = 'Product Specification Code');
...
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` SET TAGS ('dbx_subdomain' = 'logistics_operations');
...
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` ALTER COLUMN `freight_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Logistics Coordinator Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`cargo_shipment` ALTER COLUMN `voyage_number` SET TAGS ('dbx_business_glossary_term' = 'Voyage Number');
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` SET TAGS ('dbx_subdomain' = 'logistics_operations');
...
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Issued By Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Identifier');
...
ALTER TABLE `mining_ecm`.`sales`.`bill_of_lading` ALTER COLUMN `vessel_name` SET TAGS ('dbx_business_glossary_term' = 'Vessel Name');
...
ALTER TABLE `mining_ecm`.`sales`.`quality_certificate` SET TAGS ('dbx_subdomain' = 'logistics_operations');
...
ALTER TABLE `mining_ecm`.`sales`.`quality_certificate` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Authorized Signatory Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`quality_certificate` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`quality_certificate` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`sales`.`quality_certificate` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` SET TAGS ('dbx_subdomain' = 'contract_management');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `spot_trade_id` SET TAGS ('dbx_business_glossary_term' = 'Spot Trade Identifier (ID)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `counterparty_id` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Identifier (ID)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `price_index_id` SET TAGS ('dbx_business_glossary_term' = 'Price Index Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `saleable_product_id` SET TAGS ('dbx_business_glossary_term' = 'Saleable Product Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Trader Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `agreed_price_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'Agreed Price Per Tonne');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `agreed_price_per_tonne` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `broker_commission_rate` SET TAGS ('dbx_business_glossary_term' = 'Broker Commission Rate');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `broker_commission_rate` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `broker_involved` SET TAGS ('dbx_business_glossary_term' = 'Broker Involved Flag');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `broker_name` SET TAGS ('dbx_business_glossary_term' = 'Broker Name');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `contract_reference` SET TAGS ('dbx_business_glossary_term' = 'Contract Reference Number');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = 'USD|EUR|AUD|CNY|GBP|JPY');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `delivery_location` SET TAGS ('dbx_business_glossary_term' = 'Delivery Location');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `delivery_terms` SET TAGS ('dbx_business_glossary_term' = 'Delivery Terms (Incoterms)');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `delivery_terms` SET TAGS ('dbx_value_regex' = 'FOB|CFR|CIF|DAP|EXW|FCA');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `delivery_window_end` SET TAGS ('dbx_business_glossary_term' = 'Delivery Window End Date');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `delivery_window_start` SET TAGS ('dbx_business_glossary_term' = 'Delivery Window Start Date');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified By User');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payment Terms');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `pricing_basis` SET TAGS ('dbx_business_glossary_term' = 'Pricing Basis');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `pricing_basis` SET TAGS ('dbx_value_regex' = 'platts|tsi|metal_bulletin|argus|fixed|index_linked');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `quality_adjustment_clause` SET TAGS ('dbx_business_glossary_term' = 'Quality Adjustment Clause');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `quality_specification` SET TAGS ('dbx_business_glossary_term' = 'Quality Specification');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `total_trade_value` SET TAGS ('dbx_business_glossary_term' = 'Total Trade Value');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `total_trade_value` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_date` SET TAGS ('dbx_business_glossary_term' = 'Trade Date');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_notes` SET TAGS ('dbx_business_glossary_term' = 'Trade Notes');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_number` SET TAGS ('dbx_business_glossary_term' = 'Spot Trade Number');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_number` SET TAGS ('dbx_value_regex' = '^ST-[0-9]{8}-[A-Z0-9]{6}$');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_status` SET TAGS ('dbx_business_glossary_term' = 'Trade Status');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_status` SET TAGS ('dbx_value_regex' = 'draft|confirmed|executed|settled|cancelled|disputed');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Trade Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `trade_volume_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Trade Volume in Tonnes');
ALTER TABLE `mining_ecm`.`sales`.`spot_trade` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Record Created By User');
...
ALTER TABLE `mining_ecm`.`sales`.`offtake_schedule` ALTER COLUMN `benchmark_index` SET TAGS ('dbx_business_glossary_term' = 'Benchmark Index');
ALTER TABLE `mining_ecm`.`sales`.`offtake_schedule` ALTER COLUMN `benchmark_index` SET TAGS ('dbx_value_regex' = 'platts|tsi|argus|lme|metal_bulletin');
...
ALTER TABLE `mining_ecm`.`sales`.`offtake_schedule` ALTER COLUMN `destination_port` SET TAGS ('dbx_business_glossary_term' = 'Destination Port');
...
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` SET TAGS ('dbx_subdomain' = 'revenue_settlement');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `performance_actual_id` SET TAGS ('dbx_business_glossary_term' = 'Performance Actual Identifier');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `counterparty_id` SET TAGS ('dbx_business_glossary_term' = 'Counterparty ID');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `offtake_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Offtake Agreement ID');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `orebody_id` SET TAGS ('dbx_business_glossary_term' = 'Orebody Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Prepared By Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `price_index_id` SET TAGS ('dbx_business_glossary_term' = 'Price Index Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `revenue_forecast_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Forecast Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `saleable_product_id` SET TAGS ('dbx_business_glossary_term' = 'Saleable Product Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `actual_revenue_amount` SET TAGS ('dbx_business_glossary_term' = 'Actual Revenue Amount');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `actual_volume_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Actual Volume Tonnes');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `aisc_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'All-In Sustaining Cost (AISC) Per Tonne');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `aisc_per_tonne` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `benchmark_price_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'Benchmark Price Per Tonne');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `delivery_basis` SET TAGS ('dbx_business_glossary_term' = 'Delivery Basis');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `delivery_basis` SET TAGS ('dbx_value_regex' = 'fob|cif|cfr|exw|dap|ddp');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `destination_country_code` SET TAGS ('dbx_business_glossary_term' = 'Destination Country Code');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `destination_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `lom_forecast_alignment_flag` SET TAGS ('dbx_business_glossary_term' = 'Life of Mine (LOM) Forecast Alignment Flag');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `margin_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'Margin Per Tonne');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `margin_per_tonne` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `performance_status` SET TAGS ('dbx_business_glossary_term' = 'Performance Status');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `performance_status` SET TAGS ('dbx_value_regex' = 'confirmed|provisional|adjusted|final');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Period End Date');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `period_month` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Month');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `period_quarter` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Quarter');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `period_year` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Year');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `planned_revenue_amount` SET TAGS ('dbx_business_glossary_term' = 'Planned Revenue Amount');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `planned_volume_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Planned Volume Tonnes');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `posting_date` SET TAGS ('dbx_business_glossary_term' = 'Posting Date');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `price_variance_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'Price Variance Per Tonne');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `price_variance_percentage` SET TAGS ('dbx_business_glossary_term' = 'Price Variance Percentage');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `realised_price_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'Realised Price Per Tonne');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `revenue_variance_amount` SET TAGS ('dbx_business_glossary_term' = 'Revenue Variance Amount');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `revenue_variance_percentage` SET TAGS ('dbx_business_glossary_term' = 'Revenue Variance Percentage');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `target_market` SET TAGS ('dbx_business_glossary_term' = 'Target Market');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `total_margin_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Margin Amount');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `total_margin_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `volume_variance_percentage` SET TAGS ('dbx_business_glossary_term' = 'Volume Variance Percentage');
ALTER TABLE `mining_ecm`.`sales`.`performance_actual` ALTER COLUMN `volume_variance_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Volume Variance Tonnes');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` SET TAGS ('dbx_subdomain' = 'contract_management');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `freight_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract ID');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Manager Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `counterparty_id` SET TAGS ('dbx_business_glossary_term' = 'Counterparty ID');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Approval Date');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `arbitration_clause` SET TAGS ('dbx_business_glossary_term' = 'Arbitration Clause');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `benchmark_index` SET TAGS ('dbx_business_glossary_term' = 'Freight Benchmark Index');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `bunker_adjustment_clause` SET TAGS ('dbx_business_glossary_term' = 'Bunker Adjustment Clause');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `commodity_type` SET TAGS ('dbx_business_glossary_term' = 'Commodity Type');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `commodity_type` SET TAGS ('dbx_value_regex' = 'iron_ore|copper_concentrate|coal|lithium|nickel|mixed');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Effective Date');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Expiry Date');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_number` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract Number');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_period_months` SET TAGS ('dbx_business_glossary_term' = 'Contract Period (Months)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_status` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract Status');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_status` SET TAGS ('dbx_value_regex' = 'draft|active|suspended|completed|terminated|expired');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_type` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract Type');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `contract_type` SET TAGS ('dbx_value_regex' = 'voyage_charter|time_charter|contract_of_affreightment|freight_forward_agreement|spot_charter');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = 'USD|AUD|EUR|CNY|GBP|JPY');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `demurrage_rate` SET TAGS ('dbx_business_glossary_term' = 'Demurrage Rate');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `despatch_rate` SET TAGS ('dbx_business_glossary_term' = 'Despatch Rate');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `discharge_port` SET TAGS ('dbx_business_glossary_term' = 'Port of Discharge');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `force_majeure_clause` SET TAGS ('dbx_business_glossary_term' = 'Force Majeure Clause');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `freight_basis` SET TAGS ('dbx_business_glossary_term' = 'Freight Basis (INCOTERM)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `freight_basis` SET TAGS ('dbx_value_regex' = 'fob|cfr|cif|dap|exw');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `freight_rate` SET TAGS ('dbx_business_glossary_term' = 'Freight Rate');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `freight_rate_unit` SET TAGS ('dbx_business_glossary_term' = 'Freight Rate Unit of Measure');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `freight_rate_unit` SET TAGS ('dbx_value_regex' = 'usd_per_tonne|usd_per_day|usd_per_voyage|aud_per_tonne|aud_per_day');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `governing_law` SET TAGS ('dbx_business_glossary_term' = 'Governing Law');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `insurance_responsibility` SET TAGS ('dbx_business_glossary_term' = 'Insurance Responsibility');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `insurance_responsibility` SET TAGS ('dbx_value_regex' = 'seller|buyer|shared');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `laytime_allowed_hours` SET TAGS ('dbx_business_glossary_term' = 'Laytime Allowed (Hours)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `loading_port` SET TAGS ('dbx_business_glossary_term' = 'Port of Loading');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `maximum_cargo_quantity_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Maximum Cargo Quantity (Tonnes)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `minimum_cargo_quantity_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Minimum Cargo Quantity (Tonnes)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `net_back_impact_usd_per_tonne` SET TAGS ('dbx_business_glossary_term' = 'Net-Back Impact (USD per Tonne)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract Notes');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `payment_due_days` SET TAGS ('dbx_business_glossary_term' = 'Payment Due (Days)');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payment Terms');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `port_range` SET TAGS ('dbx_business_glossary_term' = 'Port Range');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `price_adjustment_formula` SET TAGS ('dbx_business_glossary_term' = 'Freight Price Adjustment Formula');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `signed_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Signed Date');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Contract Version Number');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `vessel_type` SET TAGS ('dbx_business_glossary_term' = 'Vessel Type');
ALTER TABLE `mining_ecm`.`sales`.`freight_contract` ALTER COLUMN `vessel_type` SET TAGS ('dbx_value_regex' = 'capesize|panamax|handymax|handysize|vloc|supramax');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` SET TAGS ('dbx_subdomain' = 'logistics_operations');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `demurrage_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Demurrage Claim Identifier (ID)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `cargo_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Shipment Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Claims Manager Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `counterparty_id` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Identifier (ID)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `freight_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Freight Contract Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `offtake_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Offtake Agreement Identifier (ID)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Settlement Invoice Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `berth_name` SET TAGS ('dbx_business_glossary_term' = 'Berth Name');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `cargo_quantity_tonnes` SET TAGS ('dbx_business_glossary_term' = 'Cargo Quantity (Tonnes)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_agreed_date` SET TAGS ('dbx_business_glossary_term' = 'Claim Agreed Date');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_amount_usd` SET TAGS ('dbx_business_glossary_term' = 'Claim Amount (United States Dollars [USD])');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_number` SET TAGS ('dbx_business_glossary_term' = 'Claim Number');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_raised_date` SET TAGS ('dbx_business_glossary_term' = 'Claim Raised Date');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_settled_date` SET TAGS ('dbx_business_glossary_term' = 'Claim Settled Date');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_status` SET TAGS ('dbx_business_glossary_term' = 'Claim Status');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_status` SET TAGS ('dbx_value_regex' = 'open|agreed|disputed|settled|withdrawn|cancelled');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_type` SET TAGS ('dbx_business_glossary_term' = 'Claim Type');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `claim_type` SET TAGS ('dbx_value_regex' = 'demurrage|despatch');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `commodity_type` SET TAGS ('dbx_business_glossary_term' = 'Commodity Type');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `demurrage_rate_usd_per_day` SET TAGS ('dbx_business_glossary_term' = 'Demurrage Rate (United States Dollars [USD] per Day)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `despatch_rate_usd_per_day` SET TAGS ('dbx_business_glossary_term' = 'Despatch Rate (United States Dollars [USD] per Day)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `dispute_flag` SET TAGS ('dbx_business_glossary_term' = 'Dispute Flag');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `dispute_reason` SET TAGS ('dbx_business_glossary_term' = 'Dispute Reason');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `dispute_resolution_notes` SET TAGS ('dbx_business_glossary_term' = 'Dispute Resolution Notes');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `imo_number` SET TAGS ('dbx_business_glossary_term' = 'International Maritime Organization (IMO) Number');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `imo_number` SET TAGS ('dbx_value_regex' = '^IMO[0-9]{7}$');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `incoterm` SET TAGS ('dbx_business_glossary_term' = 'International Commercial Terms (Incoterm)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `incoterm` SET TAGS ('dbx_value_regex' = '^(EXW|FCA|CPT|CIP|DAP|DPU|DDP|FAS|FOB|CFR|CIF)$');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `laytime_allowed_hours` SET TAGS ('dbx_business_glossary_term' = 'Laytime Allowed (Hours)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `laytime_used_hours` SET TAGS ('dbx_business_glossary_term' = 'Laytime Used (Hours)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `laytime_variance_hours` SET TAGS ('dbx_business_glossary_term' = 'Laytime Variance (Hours)');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `loading_commenced_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Loading Commenced Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `loading_completed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Loading Completed Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `net_back_impact_usd` SET TAGS ('dbx_business_glossary_term' = 'Net-Back Impact (United States Dollars [USD])');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `notice_of_readiness_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Notice of Readiness (NOR) Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `port_code` SET TAGS ('dbx_business_glossary_term' = 'Port Code');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `port_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{5}$');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `port_of_loading` SET TAGS ('dbx_business_glossary_term' = 'Port of Loading');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `responsible_party` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `responsible_party` SET TAGS ('dbx_value_regex' = 'seller|buyer|shipping_line|freight_forwarder');
ALTER TABLE `mining_ecm`.`sales`.`demurrage_claim` ALTER COLUMN `vessel_name` SET TAGS ('dbx_business_glossary_term' = 'Vessel Name');
ALTER TABLE `mining_ecm`.`sales`.`price_index` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `mining_ecm`.`sales`.`price_index` SET TAGS ('dbx_subdomain' = 'contract_management');
...
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` SET TAGS ('dbx_subdomain' = 'contract_management');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` SET TAGS ('dbx_association_edges' = 'sales.offtake_agreement,community.stakeholder');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `agreement_stakeholder_consultation_id` SET TAGS ('dbx_business_glossary_term' = 'Agreement Stakeholder Consultation - Agreement Stakeholder Consultation Id');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `offtake_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Agreement Stakeholder Consultation - Sales Offtake Agreement Id');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `stakeholder_id` SET TAGS ('dbx_business_glossary_term' = 'Agreement Stakeholder Consultation - Stakeholder Id');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `agreement_acceptance_date` SET TAGS ('dbx_business_glossary_term' = 'Agreement Acceptance Date');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `benefit_entitlement_percentage` SET TAGS ('dbx_business_glossary_term' = 'Benefit Entitlement Percentage');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `conditions_imposed` SET TAGS ('dbx_business_glossary_term' = 'Conditions Imposed');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `consent_status` SET TAGS ('dbx_business_glossary_term' = 'Consent Status');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `consultation_date` SET TAGS ('dbx_business_glossary_term' = 'Consultation Date');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `consultation_method` SET TAGS ('dbx_business_glossary_term' = 'Consultation Method');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `consultation_status` SET TAGS ('dbx_business_glossary_term' = 'Consultation Status');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `stakeholder_role` SET TAGS ('dbx_business_glossary_term' = 'Stakeholder Role');
ALTER TABLE `mining_ecm`.`sales`.`agreement_stakeholder_consultation` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `mining_ecm`.`sales`.`vessel` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `mining_ecm`.`sales`.`vessel` SET TAGS ('dbx_subdomain' = 'logistics_operations');
