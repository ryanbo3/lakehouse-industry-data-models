-- Schema for Domain: hse | Business: Mining | Version: v1_ecm
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`incident` (
    ...
    `stakeholder_id` BIGINT COMMENT 'Foreign key linking to community.stakeholder. Business justification: Mining incidents (dust, noise, water contamination, blasting) frequently affect nearby community stakeholders. Incident management requires tracking affected parties for notification, consultation, an',
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Mining capital projects track HSE incidents during construction/commissioning for safety performance reporting, regulatory compliance (ESIA commitments), stage-gate reviews, and handover certification',
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the person leading the incident investigation team.',
    `incident_injured_person_employee_id` BIGINT COMMENT 'Identifier of the person injured in the incident. Applicable only for injury incidents.',
    `management_of_change_id` BIGINT COMMENT 'Foreign key linking to hse.management_of_change. Business justification: Incidents may occur during or shortly after a Management of Change implementation, indicating that the change introduced new risks or that risk controls were inadequate. Linking incidents to MOC helps',
    ...
    `location_description` STRING COMMENT 'Detailed description of the specific location within the site where the incident occurred (e.g., pit bench, processing plant, ROM pad, workshop).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`corrective_action` (
    ...
    `stakeholder_id` BIGINT COMMENT 'Foreign key linking to community.stakeholder. Business justification: Corrective actions addressing community impacts (dust suppression upgrades, noise barriers, water quality improvements) require stakeholder tracking for communication, verification, and grievance clos',
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Corrective actions from project-phase incidents, audits, or inspections must be tracked and closed before project handover. Handover certificates verify all critical corrective actions are complete. S',
    ...
    `legal_register_id` BIGINT COMMENT 'Foreign key linking to hse.legal_register. Business justification: Corrective actions may be triggered by legal compliance gaps identified through audits, inspections, or incidents. CAPAs should reference the specific legal requirement they address, creating traceabi',
    `management_of_change_id` BIGINT COMMENT 'Foreign key linking to hse.management_of_change. Business justification: Corrective actions may be triggered by Management of Change processes, particularly from PSSR (Pre-Startup Safety Review) findings, MOC risk assessments, or post-implementation reviews. CAPAs should l',
    ...
    `procurement_vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor. Business justification: Corrective actions triggered by vendor-supplied defective materials, equipment failures, or vendor HSE performance issues require vendor tracking for vendor performance management, quality non-conform',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Corrective actions assign accountability to specific employees for completion tracking, escalation, and performance metrics. Current responsible_owner is denormalized text; FK enables workforce integr',
    ...
    `site_location` STRING COMMENT 'Mine site, processing plant, or facility location where the corrective action is to be implemented.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`risk_assessment` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the manager or HSE (Health Safety and Environment) authority who approved the risk assessment.',
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Capital projects require HSE risk assessments for ESIA approvals, stage-gate decisions, design reviews, and PSSR (pre-startup safety review). Regulatory authorities require project-specific risk asses',
    ...
    `orebody_id` BIGINT COMMENT 'Foreign key linking to geology.orebody. Business justification: Hazard assessments are orebody-specific due to varying geotechnical conditions, mining methods, ground control requirements, and commodity-specific hazards (coal dust, radioactive minerals). JORC/regu',
    `primary_risk_employee_id` BIGINT COMMENT 'Reference to the employee who conducted the risk assessment.',
    `reviewer_employee_id` BIGINT COMMENT 'Reference to the employee who reviewed the risk assessment for technical accuracy and completeness.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`safety_observation` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Safety observations during project construction/commissioning are tracked separately for project safety performance metrics, contractor management, and leading indicator reporting. Standard mining pro',
    `crew_id` BIGINT COMMENT 'Identifier of the crew or team involved in the observed activity.',
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the employee who made the safety observation (supervisor, safety officer, or worker).',
    ...
    `shift_schedule_id` BIGINT COMMENT 'Identifier of the shift during which the observation was made.',
    ...
    `tertiary_safety_assigned_to_employee_id` BIGINT COMMENT 'Identifier of the employee assigned to complete follow-up actions.',
    ...
    `hazard_category` STRING COMMENT 'Category of hazard or risk observed (e.g., fall from height, mobile equipment interaction, electrical, confined space, fatigue, ergonomic).',
    ...
    `work_area` STRING COMMENT 'Functional work area or zone where the observation occurred (e.g., drilling, blasting, haulage, processing, maintenance).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`environmental_monitoring` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Mining projects require baseline and construction-phase environmental monitoring for ESIA compliance, permit conditions, and impact verification. Regulatory authorities require project-specific monito',
    ...
    `delivery_destination_id` BIGINT COMMENT 'Foreign key linking to customer.delivery_destination. Business justification: Mining companies monitor environmental parameters (dust emissions, noise levels, water quality, air quality) at customer delivery points—especially ports and rail terminals—to comply with environmenta',
    ...
    `stakeholder_id` BIGINT COMMENT 'Foreign key linking to community.stakeholder. Business justification: Monitoring points near communities require stakeholder notification when permit limits are exceeded (air quality, water quality, noise). Regulatory requirement in most mining jurisdictions and critica',
    ...
    `monitoring_equipment_code` BIGINT COMMENT 'Reference to the monitoring instrument or equipment used (e.g., air quality analyzer, water quality meter, noise meter).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`environmental_permit` (
    ...
    `legal_register_id` BIGINT COMMENT 'Foreign key linking to hse.legal_register. Business justification: Environmental permits are issued under specific legal and regulatory requirements documented in the legal register. Each permit should reference the legal obligation(s) it fulfills, creating traceabil',
    `programme_of_work_id` BIGINT COMMENT 'Foreign key linking to tenement.programme_of_work. Business justification: Environmental permits authorize programme of work activities. Compliance verification, operational authorization, and regulatory tracking require this link. Mining operations obtain permits to execute',
    ...
    `financial_assurance_amount` DECIMAL(18,2) COMMENT 'Monetary value of the financial assurance, bond, or guarantee required under the permit conditions. Expressed in the operating currency of the mining operation.',
    `financial_assurance_currency` STRING COMMENT 'Three-letter ISO 4217 currency code for the financial assurance amount.. Valid values are `^[A-Z]{3}$`',
    ...
    `site_name` STRING COMMENT 'Name of the mining site, operation, or facility covered by this permit. Provides human-readable context for the permit location.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`regulatory_submission` (
    ...
    `cargo_shipment_id` BIGINT COMMENT 'Foreign key linking to sales.cargo_shipment. Business justification: Export compliance submissions (environmental clearances, hazardous cargo declarations, customs filings) reference specific bill of lading numbers and shipment details. Regulatory bodies require shipme',
    `stakeholder_id` BIGINT COMMENT 'Foreign key linking to community.stakeholder. Business justification: Mining regulatory submissions (EIS, permit applications, compliance reports) require documented stakeholder consultation. Mandatory in most jurisdictions for environmental and social impact assessment',
    ...
    `legal_register_id` BIGINT COMMENT 'Foreign key linking to hse.legal_register. Business justification: Regulatory submissions are made to fulfill specific legal compliance obligations documented in the legal register. Each submission should reference the legal requirement it satisfies. This is a new FK',
    `lom_plan_id` BIGINT COMMENT 'Identifier of the specific mine operation covered by this regulatory submission.',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`ppe_issuance` (
    `ppe_issuance_id` BIGINT COMMENT 'Unique identifier for the PPE issuance transaction record.',
    `general_ledger_account_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger_account. Business justification: PPE issuance tracks unit_cost and total_cost with cost_centre_code but needs GL account for proper financial posting. Enables correct expense classification (PPE inventory vs. expense) and supports fi',
    `material_master_id` BIGINT COMMENT 'Identifier of the specific PPE item from the master catalogue (hard hat, respirator, safety boots, etc.).',
    `employee_id` BIGINT COMMENT 'Identifier of the worker or contractor to whom the PPE was issued.',
    `supply_material_master_id` BIGINT COMMENT 'Foreign key linking to procurement.material_master. Business justification: PPE items (hard hats, safety boots, respirators, gloves) are materials in procurement system. Linking issuance to material master enables PPE inventory management, cost allocation to cost centres, reo',
    `acknowledgement_signature` STRING COMMENT 'Digital or scanned signature of the worker acknowledging receipt of the PPE and understanding of its proper use and care requirements.',
    `acknowledgement_timestamp` TIMESTAMP COMMENT 'The date and time when the worker signed the acknowledgement of PPE receipt and training.',
    `actual_return_date` DATE COMMENT 'The actual date on which the PPE was returned by the worker, used to track compliance and lifecycle completion.',
    `batch_number` STRING COMMENT 'The manufacturing batch or lot number of the PPE item, used for quality control and recall management.',
    `compliance_status` STRING COMMENT 'Indicates whether the worker is compliant with role-based PPE requirements following this issuance, used for ISO 45001 and MSHA audit readiness.. Valid values are `compliant|non_compliant|pending_inspection|overdue`',
    `condition_at_issue` STRING COMMENT 'The physical condition of the PPE at the time of issuance to the worker.. Valid values are `new|refurbished|serviceable|inspected`',
    `cost_centre_code` STRING COMMENT 'The cost centre to which the PPE issuance cost is allocated, typically the workers department or project.',
    `created_timestamp` TIMESTAMP COMMENT 'The date and time when this issuance record was first created in the lakehouse, used for audit trail and data lineage.',
    `currency_code` STRING COMMENT 'The three-letter ISO 4217 currency code for the cost values (e.g., USD, AUD, CAD).. Valid values are `^[A-Z]{3}$`',
    `expected_return_date` DATE COMMENT 'The date by which the PPE is expected to be returned for inspection, replacement, or end of assignment, applicable to reusable or high-value items.',
    `expiry_date` DATE COMMENT 'The date on which the issued PPE item reaches the end of its certified service life or manufacturer warranty, applicable to items such as respirator cartridges, harnesses, and helmets.',
    `fit_test_date` DATE COMMENT 'The date on which the worker completed a fit test for the issued PPE, applicable to respirators and breathing apparatus.',
    `fit_test_required_flag` BOOLEAN COMMENT 'Indicates whether a fit test is required for the issued PPE (e.g., respirators), per regulatory and manufacturer requirements.',
    `issuance_number` STRING COMMENT 'Business-readable unique transaction number for the PPE issuance event, typically generated by IsoMetrix or stores system.',
    `issue_date` DATE COMMENT 'The date on which the PPE was issued to the worker.',
    `issue_timestamp` TIMESTAMP COMMENT 'The precise date and time when the PPE was issued to the worker, captured from the stores or HSE system.',
    `issuing_location` STRING COMMENT 'The mine site, warehouse, or stores location from which the PPE was issued.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'The date and time when this issuance record was last updated in the lakehouse, used for change tracking and audit compliance.',
    `notes` STRING COMMENT 'Free-text notes recorded by the issuing officer regarding special circumstances, worker requests, or equipment condition observations at the time of issuance.',
    `quantity_issued` STRING COMMENT 'The number of units of the PPE item issued in this transaction.',
    `replacement_reason` STRING COMMENT 'The reason for issuing replacement PPE, used for cost tracking, compliance reporting, and trend analysis.. Valid values are `end_of_life|damaged|lost|stolen|upgrade|size_change`',
    `return_condition` STRING COMMENT 'The physical condition of the PPE at the time of return, used to determine whether it can be reissued, requires repair, or must be disposed of.. Valid values are `serviceable|damaged|expired|lost|destroyed`',
    `serial_number` STRING COMMENT 'The unique serial or asset number of the specific PPE unit issued, used for tracking high-value or calibrated equipment such as gas detectors or respirators.',
    `source_record_reference` STRING COMMENT 'The unique identifier of this issuance record in the source operational system, used for data lineage and reconciliation.',
    `source_system` STRING COMMENT 'The operational system from which this issuance record originated (e.g., IsoMetrix, Pronto Xi, SAP MM).',
    `total_cost` DECIMAL(18,2) COMMENT 'The total cost of the PPE issuance transaction, calculated as quantity issued multiplied by unit cost.',
    `training_completed_flag` BOOLEAN COMMENT 'Indicates whether the worker has completed mandatory training on the proper use, maintenance, and limitations of the issued PPE.',
    `training_date` DATE COMMENT 'The date on which the worker completed PPE-specific training, used for compliance verification and audit trails.',
    `unit_cost` DECIMAL(18,2) COMMENT 'The cost per unit of the PPE item at the time of issuance, used for financial tracking and OPEX reporting.',
    CONSTRAINT pk_ppe_issuance PRIMARY KEY(`ppe_issuance_id`)
) COMMENT 'Manages the complete Personal Protective Equipment (PPE) lifecycle including the master catalogue of approved PPE types (hard hats, respirators, hearing protection, safety boots, high-visibility garments, gas detection equipment) with standards/certifications (AS/NZS, ANSI), applicable hazard classes, inspection frequencies, service life, and approved suppliers. Tracks transactional issuance of PPE to individual workers and contractors capturing PPE type, quantity issued, issue date, condition at issue, return date, and reason for replacement. Monitors PPE compliance per worker against role-based requirements. SSOT for PPE catalogue, issuance records, and compliance tracking supporting ISO 45001 and MSHA inspections.';

CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`health_surveillance` (
    `health_surveillance_id` BIGINT COMMENT 'Unique identifier for the health surveillance record. Primary key for the health surveillance data product.',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Health surveillance programs (medical examinations, audiometry, spirometry, biological monitoring) incur significant costs that must be allocated to site cost centres for budget planning, variance tra',
    `employee_id` BIGINT COMMENT 'Reference to the mine worker who is the subject of this health surveillance record. Links to workforce master data.',
    `incident_id` BIGINT COMMENT 'Reference to the HSE incident record if this health surveillance assessment was triggered by a specific workplace incident or exposure event. Links to incident management data.',
    `action_level_value` DECIMAL(18,2) COMMENT 'Action level threshold value that triggers additional monitoring, medical surveillance, or control measures. Typically set at 50% of the OEL/PEL or as defined by regulatory standards.',
    `baseline_test_flag` BOOLEAN COMMENT 'Boolean flag indicating whether this is a baseline health surveillance test conducted at the start of employment or upon first exposure to a specific hazard. Baseline tests establish reference values for future comparison.',
    `comments` STRING COMMENT 'Additional comments, observations, or notes related to the health surveillance test, examination findings, or follow-up actions. May include clinical observations or contextual information.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this health surveillance record was first created in the system. Audit trail field for record creation tracking.',
    `examining_physician_name` STRING COMMENT 'Name of the occupational physician or medical professional who conducted the health surveillance examination or reviewed the exposure monitoring results.',
    `exceedance_flag` BOOLEAN COMMENT 'Boolean flag indicating whether the measured value exceeded the applicable OEL, PEL, or action level threshold. True indicates an exceedance requiring follow-up action.',
    `exposure_duration_hours` DECIMAL(18,2) COMMENT 'Duration of exposure to the occupational hazard during the monitoring period, measured in hours. Used for calculating time-weighted average exposures and cumulative dose assessments.',
    `fitness_classification` STRING COMMENT 'Classification of the workers fitness for work based on the medical examination or health assessment. Determines whether the worker can perform their assigned duties without restrictions, with restrictions, or is unfit for work.. Valid values are `fit_unrestricted|fit_with_restrictions|temporarily_unfit|permanently_unfit|pending_review`',
    `follow_up_action` STRING COMMENT 'Follow-up action required based on the health surveillance results. May include repeat testing, referral to specialist, workplace exposure assessment, engineering controls implementation, or administrative controls.',
    `follow_up_due_date` DATE COMMENT 'Date by which the follow-up action must be completed. Used for tracking compliance with medical surveillance schedules and ensuring timely follow-up on abnormal findings.',
    `hazard_type` STRING COMMENT 'Type of occupational hazard being monitored or assessed. Includes noise exposure, airborne dust (respirable silica, coal dust, diesel particulate matter), chemical exposure, biological agents, ergonomic risks, and heat stress.. Valid values are `noise|dust|chemical|biological|ergonomic|heat_stress`',
    `hearing_protection_required_flag` BOOLEAN COMMENT 'Boolean flag indicating whether hearing protection (Personal Protective Equipment - PPE) is required for this worker based on noise exposure monitoring or audiometric test results. True indicates mandatory hearing protection.',
    `incident_related_flag` BOOLEAN COMMENT 'Boolean flag indicating whether this health surveillance record is related to a specific workplace incident, injury, or exposure event requiring immediate medical assessment.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this health surveillance record was last modified or updated. Audit trail field for tracking record changes and medical review updates.',
    `measurement_unit` STRING COMMENT 'Unit of measure for the surveillance measurement value. Common units: dB (decibels for audiometry), dB TWA (time-weighted average noise exposure), mg/m³ (airborne dust concentration), µg/dL (blood lead level), L (liters for spirometry volume), L/min (spirometry flow rate), percent (lung function percentage), mmHg (blood pressure). [ENUM-REF-CANDIDATE: dB|dB_TWA|mg_m3|ug_dL|L|L_min|percent|mmHg — 8 candidates stripped; promote to reference product]',
    `measurement_value` DECIMAL(18,2) COMMENT 'Quantitative result of the health surveillance test or exposure monitoring. For audiometry: hearing threshold in dB; for spirometry: lung function values; for noise dosimetry: dB TWA (Time-Weighted Average); for dust sampling: concentration in mg/m³; for blood lead: µg/dL.',
    `medical_facility_name` STRING COMMENT 'Name of the medical facility, clinic, or occupational health service provider where the health surveillance test or examination was conducted.',
    `next_test_due_date` DATE COMMENT 'Date when the next health surveillance test or assessment is due for this worker. Calculated based on test frequency requirements and used for scheduling periodic medical examinations.',
    `oel_value` DECIMAL(18,2) COMMENT 'Occupational Exposure Limit (OEL) or Permissible Exposure Limit (PEL) threshold value applicable to this hazard type and measurement. Regulatory limit against which the measurement is compared to determine compliance.',
    `periodic_examination_flag` BOOLEAN COMMENT 'Boolean flag indicating whether this is a periodic (routine) medical examination conducted at regular intervals as part of the ongoing health surveillance program.',
    `physician_recommendation` STRING COMMENT 'Recommendation provided by the occupational physician or medical professional based on the health surveillance results. May include follow-up testing, work restrictions, referral to specialist, or clearance for duty.',
    `ppe_type_required` STRING COMMENT 'Type of Personal Protective Equipment (PPE) required based on the health surveillance findings. May include hearing protection (earplugs, earmuffs), respiratory protection (dust masks, respirators), or other protective equipment.',
    `pre_employment_flag` BOOLEAN COMMENT 'Boolean flag indicating whether this health surveillance record is part of a pre-employment medical examination conducted before the worker commences employment at the mine site.',
    `record_status` STRING COMMENT 'Current lifecycle status of the health surveillance record. Tracks the record through draft, completion, medical review, approval, and archival stages.. Valid values are `draft|completed|reviewed|approved|archived`',
    `regulatory_submission_required_flag` BOOLEAN COMMENT 'Boolean flag indicating whether this health surveillance record must be submitted to regulatory authorities such as MSHA or state mining regulators as part of mandatory health reporting requirements.',
    `submission_date` DATE COMMENT 'Date when this health surveillance record was submitted to regulatory authorities for compliance reporting purposes.',
    `surveillance_type` STRING COMMENT 'Type of health surveillance or medical assessment conducted. Discriminates between audiometric testing, spirometry, blood lead testing, medical examinations, fitness-for-work assessments, and exposure monitoring.. Valid values are `audiometric|spirometry|blood_lead|medical_examination|fitness_assessment|exposure_monitoring`',
    `test_date` DATE COMMENT 'Date when the health surveillance test, medical examination, or exposure monitoring was conducted. Principal business event timestamp for the surveillance record.',
    `test_frequency_months` STRING COMMENT 'Required frequency for this type of health surveillance test or assessment, measured in months. Determines the interval between periodic medical examinations or exposure monitoring based on regulatory requirements and risk assessment.',
    `test_location` STRING COMMENT 'Location where the health surveillance test or assessment was conducted. May be mine site medical facility, external clinic, or field location for exposure monitoring.',
    `work_restrictions` STRING COMMENT 'Specific work restrictions or limitations imposed based on the health surveillance findings. May include restrictions on noise exposure, dust exposure, heavy lifting, confined space entry, or specific tasks.',
    CONSTRAINT pk_health_surveillance PRIMARY KEY(`health_surveillance_id`)
) COMMENT 'Records of occupational health surveillance, fitness-for-work assessments, and exposure monitoring for mine workers. Covers audiometric testing, spirometry, personal noise dosimetry (dB TWA), airborne dust sampling (respirable silica, coal dust, DPM in mg/m³), blood lead levels, heat stress assessments, fatigue alertness testing (PVT), and pre-employment/periodic medical examinations. Captures worker reference, test/monitoring type, hazard type discriminator (noise, dust, chemical, biological, ergonomic), measurement value, units, exceedance flags against OEL/PEL/action levels, exposure duration, fitness classification, occupational physician recommendation, restrictions, hearing protection requirements, and follow-up actions. Supports MSHA health and noise standards, Coal Mine Health and Safety Act, and ISO 45001 worker health monitoring.';
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`emergency_response_plan` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Capital projects require construction-phase emergency response plans distinct from operational plans, covering construction scenarios, contractor coordination, and temporary facilities. Regulatory app',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`training_record` (
    ...
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Project-specific inductions, PSSR training, commissioning competency training, and contractor safety training must be tracked for handover readiness verification. Handover certificates require confirm',
    ...
    `employee_id` BIGINT COMMENT 'Reference to the mine worker or contractor who received the training.',
    `procurement_vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor. Business justification: External training providers are vendors. Linking training records to vendor master enables vendor-delivered training cost allocation, vendor performance evaluation for training services, contract mana',
    `saleable_product_id` BIGINT COMMENT 'Foreign key linking to product.saleable_product. Business justification: Product-specific handling training is mandatory for hazmat-classified products, moisture-sensitive materials (TML training), and products with special packaging requirements. Mining operations track c',
    `tenement_id` BIGINT COMMENT 'Reference to the mine site or facility where the training was delivered or is applicable.',
    `training_course_id` BIGINT COMMENT 'Reference to the HSE training program or course catalog entry.',
    ...
    `training_location` STRING COMMENT 'Physical or virtual location where the training was conducted (e.g., training room, online platform, field site).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`chemical_register` (
    ...
    `procurement_vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor. Business justification: Regulatory requirement to maintain chemical supplier information for SDS management, emergency contact during chemical incidents, supply chain transparency, and vendor HSE performance tracking. Remove',
    ...
    `permit_reference_number` STRING COMMENT 'Reference number of the environmental or operational permit authorizing the use or storage of this chemical at the site.',
    ...
    `storage_location` STRING COMMENT 'Physical location or facility code where the chemical is stored at the mine site (e.g., Reagent Storage Shed A, Explosives Magazine 3, Fuel Farm).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`audit` (
    ...
    `stakeholder_id` BIGINT COMMENT 'Foreign key linking to community.stakeholder. Business justification: Community engagement audits and social performance audits assess relationships with specific stakeholder groups (indigenous communities, local villages). Audit scope often focuses on particular stakeh',
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: Mining capital projects undergo contractor safety audits, pre-commissioning safety reviews, environmental compliance audits, and PSSR audits. Project handover requires audit closure verification. Stan',
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the lead auditor or inspector in the workforce or contractor system.',
    `legal_register_id` BIGINT COMMENT 'Foreign key linking to hse.legal_register. Business justification: HSE audits often verify compliance with specific legal and regulatory requirements. Audit records should link to the legal obligations being assessed, creating traceability between audit findings and ',
    `procurement_vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor. Business justification: HSE audits of vendor facilities, contractor HSE performance audits, and vendor prequalification audits require vendor linkage for vendor performance management, supply chain risk management, and vendo',
    ...
    `area_inspected` STRING COMMENT 'Specific operational area, department, or location within the site that was inspected (e.g., underground mine, processing plant, tailings storage facility).',
...
CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`legal_register` (
    `legal_register_id` BIGINT COMMENT 'Primary key for legal_register',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Legal register tracks compliance obligations that incur costs (legal advice, compliance activities, permit fees, reporting). Cost centre allocation enables tracking legal compliance costs against resp',
    `tenement_id` BIGINT COMMENT 'Identifier of the mine site or facility to which this legal requirement applies.',
    `amendment_date` DATE COMMENT 'Date of the most recent amendment or update to the legislation that affects this compliance obligation.',
    `amendment_summary` STRING COMMENT 'Brief summary of the most recent amendment or change to the legislation and its impact on compliance obligations.',
    `applicability_notes` STRING COMMENT 'Additional notes or context regarding the applicability of this legal requirement to the specific site, operation, or activity.',
    `compliance_evidence_location` STRING COMMENT 'Reference to the location or system where evidence of compliance with this legal requirement is stored (e.g., document management system path, IsoMetrix record ID, file server location).',
    `compliance_obligation_summary` STRING COMMENT 'Concise summary of the specific compliance obligation or requirement imposed by the legislation on the mine site (e.g., submit annual environmental report, maintain dust monitoring records, conduct quarterly safety audits).',
    `compliance_status` STRING COMMENT 'Current compliance status of the site against this legal requirement (compliant, non-compliant, under review, not applicable, pending).. Valid values are `compliant|non_compliant|under_review|not_applicable|pending`',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this legal register entry was first created in the system.',
    `effective_date` DATE COMMENT 'Date when the legislation or legal requirement came into force and became applicable to the site.',
    `is_active` BOOLEAN COMMENT 'Indicates whether this legal requirement is currently active and applicable (true) or has been superseded, repealed, or is no longer applicable (false).',
    `jurisdiction` STRING COMMENT 'Geographic or administrative jurisdiction where the legislation applies (e.g., Queensland, Western Australia, Federal, New South Wales).',
    `last_review_date` DATE COMMENT 'Date when this legal requirement was last reviewed for applicability and compliance status.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this legal register entry was last modified or updated.',
    `legislation_name` STRING COMMENT 'Full name or title of the applicable legislation, act, regulation, code of practice, or standard (e.g., Mine Safety and Health Act 1996, Environmental Protection Act 1994).',
    `legislation_reference_number` STRING COMMENT 'Official reference number, citation, or identifier for the legislation (e.g., Act No. 82 of 1996, Regulation 2019/123).',
    `legislation_type` STRING COMMENT 'Classification of the legal instrument (act, regulation, code of practice, standard, licence condition, permit condition).. Valid values are `act|regulation|code_of_practice|standard|licence_condition|permit_condition`',
    `legislation_url` STRING COMMENT 'Web link or URL to the official source or full text of the legislation for reference.',
    `licence_number` STRING COMMENT 'Mining licence, environmental permit, or authorization number to which this legal requirement is attached (if applicable).',
    `penalty_description` STRING COMMENT 'Description of penalties, fines, or sanctions that may be imposed for non-compliance with this legal requirement.',
    `regulatory_body` STRING COMMENT 'Name of the government agency or regulatory authority responsible for administering and enforcing the legislation (e.g., Mine Safety and Health Administration (MSHA), Department of Environment and Science, Resources Safety and Health Queensland).',
    `reporting_frequency` STRING COMMENT 'Frequency at which compliance reporting or submission is required under this legal obligation (daily, weekly, monthly, quarterly, annually, as required, not applicable). [ENUM-REF-CANDIDATE: daily|weekly|monthly|quarterly|annually|as_required|not_applicable — 7 candidates stripped; promote to reference product]',
    `responsible_owner` STRING COMMENT 'Name or identifier of the person, role, or department responsible for ensuring compliance with this legal requirement (e.g., HSE Manager, Environmental Superintendent, Site Manager).',
    `review_date` DATE COMMENT 'Scheduled date for the next review of this legal requirement to ensure continued applicability and compliance.',
    `risk_rating` STRING COMMENT 'Risk rating assigned to non-compliance with this legal requirement based on potential impact to health, safety, environment, or business operations (critical, high, medium, low).. Valid values are `critical|high|medium|low`',
    `superseded_by_legislation` STRING COMMENT 'Name or reference of the legislation that has superseded or replaced this legal requirement (if applicable).',
    CONSTRAINT pk_legal_register PRIMARY KEY(`legal_register_id`)
) COMMENT 'Master register of applicable HSE legal and regulatory requirements for each mine site and jurisdiction, including acts, regulations, codes of practice, standards, and licence conditions. Records legislation name, jurisdiction, regulatory body, applicable site, compliance obligation summary, review date, compliance status, and responsible owner. SSOT for legal compliance obligations supporting ISO 45001 Clause 6.1.3 and ISO 14001 Clause 6.1.3.';

CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`management_of_change` (
    `management_of_change_id` BIGINT COMMENT 'Unique identifier for the Management of Change request record. Primary key.',
    `employee_id` BIGINT COMMENT 'Identifier of the authorized manager or executive who granted final approval for the change.',
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to project.capital_project. Business justification: MOC process is triggered by project design changes, scope modifications, or commissioning changes affecting HSE risk profile. PSSR requires MOC closure verification. Standard mining project change man',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: MOC tracks estimated_cost and actual_cost in currency_code but needs cost centre allocation for budget approval workflows, variance tracking, and financial accountability. Essential for managing chang',
    `primary_management_initiator_employee_id` BIGINT COMMENT 'Identifier of the employee who originated the MOC request.',
    `reviewer_employee_id` BIGINT COMMENT 'Identifier of the HSE or technical specialist currently reviewing the MOC request.',
    `risk_assessment_id` BIGINT COMMENT 'Foreign key linking to the formal risk assessment conducted to evaluate HSE hazards introduced or modified by this change.',
    `saleable_product_id` BIGINT COMMENT 'Foreign key linking to product.saleable_product. Business justification: MOC processes are triggered by changes to product specifications, blending recipes, or handling procedures. Mining operations require risk assessment and approval workflow for any product-related chan',
    `specification_id` BIGINT COMMENT 'Foreign key linking to product.product_specification. Business justification: Product specification changes are a primary MOC trigger in mining. Direct link to the specific spec revision enables traceability of HSE risk assessments, training updates, and regulatory notification',
    `tenement_id` BIGINT COMMENT 'Identifier of the mine site or facility where the change will be implemented.',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: When MOC involves capital projects (equipment modifications, facility upgrades, process changes requiring capex), costs must be tracked against WBS elements for proper capex accounting, project cost c',
    `superseded_management_of_change_id` BIGINT COMMENT 'Self-referencing FK on management_of_change (superseded_management_of_change_id)',
    `actual_cost` DECIMAL(18,2) COMMENT 'Final actual cost incurred for implementing the change.',
    `affected_area` STRING COMMENT 'Specific operational area, work zone, or department impacted by the change (e.g., underground stope, processing plant, tailings storage facility, maintenance workshop).',
    `affected_process` STRING COMMENT 'Name or code of the operational process being modified (e.g., blasting sequence, ventilation system, ore crushing, chemical leaching, ground support installation).',
    `approval_date` DATE COMMENT 'Date when the MOC request received final authorization to proceed with implementation.',
    `approval_workflow_stage` STRING COMMENT 'Current stage in the multi-level approval process (e.g., supervisor review, HSE review, engineering review, site manager approval, general manager approval).',
    `approver_name` STRING COMMENT 'Full name of the authorized manager or executive who granted final approval for the change.',
    `change_category` STRING COMMENT 'Primary classification of what is being changed: equipment (machinery, tools), process (operational workflow), chemical (hazardous substances), organizational (structure, roles), facility (infrastructure, buildings), or procedure (Standard Operating Procedure / SOP).. Valid values are `equipment|process|chemical|organizational|facility|procedure`',
    `change_description` STRING COMMENT 'Detailed narrative description of the proposed modification to mine operations, equipment, processes, chemicals, or organizational structures, including scope, rationale, and expected outcomes.',
    ...
    `change_type` STRING COMMENT 'Classification of the change duration and intent: temporary (time-limited), permanent (indefinite), emergency (immediate safety response), or trial (pilot/testing phase).. Valid values are `temporary|permanent|emergency|trial`',
    `closure_verification_date` DATE COMMENT 'Date when the MOC was formally verified as complete, all actions closed, and the change fully integrated into normal operations.',
    `closure_verified_by` STRING COMMENT 'Name of the HSE or operations manager who verified and authorized closure of the MOC.',
    `comments` STRING COMMENT 'Additional notes, observations, or context relevant to the MOC request, review, or implementation.',
    `control_measures` STRING COMMENT 'Summary of risk controls and mitigation actions to be implemented as part of the change, following hierarchy of controls (elimination, substitution, engineering, administrative, Personal Protective Equipment / PPE).',
    `created_timestamp` TIMESTAMP COMMENT 'System timestamp when the MOC record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for cost amounts (e.g., USD, AUD, CAD, ZAR).. Valid values are `^[A-Z]{3}$`',
    `estimated_cost` DECIMAL(18,2) COMMENT 'Projected total cost of implementing the change, including materials, labor, equipment, and downtime.',
    `hazard_categories` STRING COMMENT 'Comma-separated list of hazard types introduced or affected by the change (e.g., ground instability, chemical exposure, confined space, electrical, mobile equipment interaction).',
    `implementation_completion_date` DATE COMMENT 'Actual date when all implementation activities were completed and the change became operational.',
    `implementation_plan` STRING COMMENT 'Detailed plan outlining steps, resources, timeline, and responsibilities for executing the approved change.',
    `implementation_start_date` DATE COMMENT 'Actual date when implementation activities commenced.',
    `inherent_risk_level` STRING COMMENT 'Risk severity rating before implementation of control measures, derived from the linked risk assessment.. Valid values are `low|medium|high|critical`',
    `initiator_department` STRING COMMENT 'Department or functional area of the initiating employee (e.g., Mine Operations, Processing, Maintenance, Engineering).',
    `initiator_name` STRING COMMENT 'Full name of the employee who originated the MOC request.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'System timestamp when the MOC record was last updated.',
    `moc_number` STRING COMMENT 'Business-facing unique reference number for the MOC request, used for tracking and communication across departments and regulatory submissions.',
    `moc_status` STRING COMMENT 'Current lifecycle status of the MOC request in the approval and implementation workflow. [ENUM-REF-CANDIDATE: draft|submitted|under_review|approved|rejected|implementation|completed|closed|cancelled — 9 candidates stripped; promote to reference product]',
    `pssr_completion_date` DATE COMMENT 'Date when the Pre-Startup Safety Review was completed and the change was verified safe to commission.',
    `pssr_outcome` STRING COMMENT 'Result of the Pre-Startup Safety Review: passed (safe to commission), failed (deficiencies must be corrected), or conditional (minor items to be closed post-startup).. Valid values are `passed|failed|conditional`',
    `pssr_required_flag` BOOLEAN COMMENT 'Indicates whether a formal Pre-Startup Safety Review is mandated before the change can be commissioned, typically required for high-risk modifications to critical systems (ventilation, ground support, blasting, chemical processes).',
    `regulatory_notification_date` DATE COMMENT 'Date when the regulatory authority was formally notified of the change.',
    `regulatory_notification_required_flag` BOOLEAN COMMENT 'Indicates whether the change must be reported to regulatory authorities (e.g., Mine Safety and Health Administration / MSHA, state mining inspectorate) due to its nature or risk level.',
    `rejection_reason` STRING COMMENT 'Explanation provided if the MOC request was rejected, including specific concerns or deficiencies that must be addressed.',
    `required_implementation_date` DATE COMMENT 'Target date by which the change must be implemented to meet operational or regulatory requirements.',
    `residual_risk_level` STRING COMMENT 'Risk severity rating after implementation of proposed control measures, derived from the linked risk assessment.. Valid values are `low|medium|high|critical`',
    `review_date` DATE COMMENT 'Date when the technical or HSE review was completed.',
    `reviewer_name` STRING COMMENT 'Full name of the HSE or technical specialist currently reviewing the MOC request.',
    `sop_updated_flag` BOOLEAN COMMENT 'Indicates whether relevant Standard Operating Procedures have been revised to reflect the change.',
    `submission_date` DATE COMMENT 'Date when the MOC request was formally submitted for review and approval.',
    `training_completion_date` DATE COMMENT 'Date when all required training for affected personnel was completed.',
    `training_required_flag` BOOLEAN COMMENT 'Indicates whether workforce training or competency verification is required before the change can be implemented.',
    CONSTRAINT pk_management_of_change PRIMARY KEY(`management_of_change_id`)
) COMMENT 'Master record of formal Management of Change (MOC) requests for modifications to mine operations, equipment, processes, chemicals, or organizational structures that may introduce new HSE risks. Captures change description, change type (temporary/permanent), risk assessment linkage, affected areas, approval workflow status, implementation plan, pre-startup safety review, and closure verification. Required by ICMM Performance Expectations, ISO 45001 Clause 8.1.3, and critical for mining operations where changes to ventilation, ground support, blasting patterns, or chemical processes can introduce catastrophic risk.';

CREATE OR REPLACE TABLE `mining_ecm`.`hse`.`investigation` (
    `investigation_id` BIGINT COMMENT 'Primary key for investigation',
    `approved_by_employee_id` BIGINT COMMENT 'Reference to the manager or HSE authority who approved and signed off on the investigation report.',
    `employee_id` BIGINT COMMENT 'Reference to the employee assigned as the primary investigator responsible for leading the investigation.',
    `incident_id` BIGINT COMMENT 'Reference to the HSE incident that triggered this investigation.',
    `site_id` BIGINT COMMENT 'Reference to the mining site or facility where the investigated event occurred.',
    ...
    `investigation_team_members` STRING COMMENT 'Comma-separated list of employee names or identifiers who are part of the investigation team.',
    ...
    `location_description` STRING COMMENT 'Detailed textual description of the specific location within the site where the event occurred (e.g., pit level, processing plant area, haul road section).',
...
ALTER TABLE `mining_ecm`.`hse`.`incident` ADD CONSTRAINT `fk_hse_incident_management_of_change_id` FOREIGN KEY (`management_of_change_id`) REFERENCES `mining_ecm`.`hse`.`management_of_change`(`management_of_change_id`);
...
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ADD CONSTRAINT `fk_hse_corrective_action_legal_register_id` FOREIGN KEY (`legal_register_id`) REFERENCES `mining_ecm`.`hse`.`legal_register`(`legal_register_id`);
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ADD CONSTRAINT `fk_hse_corrective_action_management_of_change_id` FOREIGN KEY (`management_of_change_id`) REFERENCES `mining_ecm`.`hse`.`management_of_change`(`management_of_change_id`);
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ADD CONSTRAINT `fk_hse_environmental_permit_legal_register_id` FOREIGN KEY (`legal_register_id`) REFERENCES `mining_ecm`.`hse`.`legal_register`(`legal_register_id`);
...
ALTER TABLE `mining_ecm`.`hse`.`regulatory_submission` ADD CONSTRAINT `fk_hse_regulatory_submission_legal_register_id` FOREIGN KEY (`legal_register_id`) REFERENCES `mining_ecm`.`hse`.`legal_register`(`legal_register_id`);
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ADD CONSTRAINT `fk_hse_health_surveillance_incident_id` FOREIGN KEY (`incident_id`) REFERENCES `mining_ecm`.`hse`.`incident`(`incident_id`);
...
ALTER TABLE `mining_ecm`.`hse`.`audit` ADD CONSTRAINT `fk_hse_audit_legal_register_id` FOREIGN KEY (`legal_register_id`) REFERENCES `mining_ecm`.`hse`.`legal_register`(`legal_register_id`);
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ADD CONSTRAINT `fk_hse_management_of_change_risk_assessment_id` FOREIGN KEY (`risk_assessment_id`) REFERENCES `mining_ecm`.`hse`.`risk_assessment`(`risk_assessment_id`);
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ADD CONSTRAINT `fk_hse_management_of_change_superseded_management_of_change_id` FOREIGN KEY (`superseded_management_of_change_id`) REFERENCES `mining_ecm`.`hse`.`management_of_change`(`management_of_change_id`);
...
ALTER TABLE `mining_ecm`.`hse`.`investigation` ADD CONSTRAINT `fk_hse_investigation_related_investigation_id` FOREIGN KEY (`related_investigation_id`) REFERENCES `mining_ecm`.`hse`.`investigation`(`investigation_id`);
...
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `stakeholder_id` SET TAGS ('dbx_business_glossary_term' = 'Affected Stakeholder Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `asset_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Investigation Lead Person ID');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `incident_injured_person_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Injured Person ID');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `incident_injured_person_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `incident_injured_person_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `management_of_change_id` SET TAGS ('dbx_business_glossary_term' = 'Management Of Change Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`incident` ALTER COLUMN `location_description` SET TAGS ('dbx_business_glossary_term' = 'Location Description');
...
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `stakeholder_id` SET TAGS ('dbx_business_glossary_term' = 'Affected Stakeholder Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `legal_register_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Register Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `management_of_change_id` SET TAGS ('dbx_business_glossary_term' = 'Management Of Change Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `procurement_vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Vendor Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Owner Employee Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`hse`.`corrective_action` ALTER COLUMN `site_location` SET TAGS ('dbx_business_glossary_term' = 'Site Location');
...
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approver ID');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `orebody_id` SET TAGS ('dbx_business_glossary_term' = 'Orebody Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `primary_risk_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assessor ID');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `primary_risk_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `primary_risk_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer ID');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`risk_assessment` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `crew_id` SET TAGS ('dbx_business_glossary_term' = 'Crew ID');
...
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Observer Employee ID');
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `shift_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Shift ID');
...
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `tertiary_safety_assigned_to_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To Employee ID');
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `tertiary_safety_assigned_to_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `tertiary_safety_assigned_to_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `hazard_category` SET TAGS ('dbx_business_glossary_term' = 'Hazard Category');
...
ALTER TABLE `mining_ecm`.`hse`.`safety_observation` ALTER COLUMN `work_area` SET TAGS ('dbx_business_glossary_term' = 'Work Area');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_monitoring` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_monitoring` ALTER COLUMN `delivery_destination_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Destination Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_monitoring` ALTER COLUMN `stakeholder_id` SET TAGS ('dbx_business_glossary_term' = 'Notification Stakeholder Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_monitoring` ALTER COLUMN `monitoring_equipment_code` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Equipment ID');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `legal_register_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Register Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `programme_of_work_id` SET TAGS ('dbx_business_glossary_term' = 'Programme Of Work Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `financial_assurance_amount` SET TAGS ('dbx_business_glossary_term' = 'Financial Assurance Amount');
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `financial_assurance_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `financial_assurance_currency` SET TAGS ('dbx_business_glossary_term' = 'Financial Assurance Currency');
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `financial_assurance_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `mining_ecm`.`hse`.`environmental_permit` ALTER COLUMN `site_name` SET TAGS ('dbx_business_glossary_term' = 'Site Name');
...
ALTER TABLE `mining_ecm`.`hse`.`regulatory_submission` ALTER COLUMN `cargo_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Shipment Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`regulatory_submission` ALTER COLUMN `stakeholder_id` SET TAGS ('dbx_business_glossary_term' = 'Consulted Stakeholder Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`regulatory_submission` ALTER COLUMN `legal_register_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Register Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`regulatory_submission` ALTER COLUMN `lom_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Identifier (ID)');
...
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` SET TAGS ('dbx_subdomain' = 'workforce_health');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `ppe_issuance_id` SET TAGS ('dbx_business_glossary_term' = 'Personal Protective Equipment (PPE) Issuance ID');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `general_ledger_account_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Account Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Personal Protective Equipment (PPE) Item ID');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Worker ID');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `supply_material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Material Master Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `acknowledgement_signature` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgement Signature');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `acknowledgement_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgement Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `actual_return_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Return Date');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `batch_number` SET TAGS ('dbx_business_glossary_term' = 'Batch Number');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `compliance_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|pending_inspection|overdue');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `condition_at_issue` SET TAGS ('dbx_business_glossary_term' = 'Condition at Issue');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `condition_at_issue` SET TAGS ('dbx_value_regex' = 'new|refurbished|serviceable|inspected');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `cost_centre_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Code');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `expected_return_date` SET TAGS ('dbx_business_glossary_term' = 'Expected Return Date');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Expiry Date');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `fit_test_date` SET TAGS ('dbx_business_glossary_term' = 'Fit Test Date');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `fit_test_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Fit Test Required Flag');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `issuance_number` SET TAGS ('dbx_business_glossary_term' = 'Issuance Transaction Number');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `issue_date` SET TAGS ('dbx_business_glossary_term' = 'Issue Date');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `issue_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Issue Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `issuing_location` SET TAGS ('dbx_business_glossary_term' = 'Issuing Location');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Issuance Notes');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `quantity_issued` SET TAGS ('dbx_business_glossary_term' = 'Quantity Issued');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `replacement_reason` SET TAGS ('dbx_business_glossary_term' = 'Replacement Reason');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `replacement_reason` SET TAGS ('dbx_value_regex' = 'end_of_life|damaged|lost|stolen|upgrade|size_change');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `return_condition` SET TAGS ('dbx_business_glossary_term' = 'Return Condition');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `return_condition` SET TAGS ('dbx_value_regex' = 'serviceable|damaged|expired|lost|destroyed');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `serial_number` SET TAGS ('dbx_business_glossary_term' = 'Serial Number');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `source_record_reference` SET TAGS ('dbx_business_glossary_term' = 'Source Record ID');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `total_cost` SET TAGS ('dbx_business_glossary_term' = 'Total Cost');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `training_completed_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Completed Flag');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `training_date` SET TAGS ('dbx_business_glossary_term' = 'Training Date');
ALTER TABLE `mining_ecm`.`hse`.`ppe_issuance` ALTER COLUMN `unit_cost` SET TAGS ('dbx_business_glossary_term' = 'Unit Cost');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` SET TAGS ('dbx_subdomain' = 'workforce_health');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `health_surveillance_id` SET TAGS ('dbx_business_glossary_term' = 'Health Surveillance Record Identifier');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `health_surveillance_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `health_surveillance_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Worker Identifier');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `incident_id` SET TAGS ('dbx_business_glossary_term' = 'Related Incident Identifier');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `action_level_value` SET TAGS ('dbx_business_glossary_term' = 'Action Level Threshold Value');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `baseline_test_flag` SET TAGS ('dbx_business_glossary_term' = 'Baseline Test Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `comments` SET TAGS ('dbx_business_glossary_term' = 'Health Surveillance Comments');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `comments` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `examining_physician_name` SET TAGS ('dbx_business_glossary_term' = 'Examining Physician Name');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `examining_physician_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `exceedance_flag` SET TAGS ('dbx_business_glossary_term' = 'Exposure Limit Exceedance Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `exposure_duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Exposure Duration in Hours');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `fitness_classification` SET TAGS ('dbx_business_glossary_term' = 'Fitness for Work Classification');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `fitness_classification` SET TAGS ('dbx_value_regex' = 'fit_unrestricted|fit_with_restrictions|temporarily_unfit|permanently_unfit|pending_review');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `follow_up_action` SET TAGS ('dbx_business_glossary_term' = 'Follow-up Action Required');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `follow_up_due_date` SET TAGS ('dbx_business_glossary_term' = 'Follow-up Action Due Date');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `hazard_type` SET TAGS ('dbx_business_glossary_term' = 'Occupational Hazard Type');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `hazard_type` SET TAGS ('dbx_value_regex' = 'noise|dust|chemical|biological|ergonomic|heat_stress');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `hearing_protection_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Hearing Protection Required Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `incident_related_flag` SET TAGS ('dbx_business_glossary_term' = 'Incident-Related Assessment Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `measurement_unit` SET TAGS ('dbx_business_glossary_term' = 'Measurement Unit of Measure');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `measurement_value` SET TAGS ('dbx_business_glossary_term' = 'Measurement Value');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `medical_facility_name` SET TAGS ('dbx_business_glossary_term' = 'Medical Facility Name');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `medical_facility_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `medical_facility_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `next_test_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Test Due Date');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `oel_value` SET TAGS ('dbx_business_glossary_term' = 'Occupational Exposure Limit (OEL) Value');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `periodic_examination_flag` SET TAGS ('dbx_business_glossary_term' = 'Periodic Medical Examination Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `physician_recommendation` SET TAGS ('dbx_business_glossary_term' = 'Occupational Physician Recommendation');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `physician_recommendation` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `ppe_type_required` SET TAGS ('dbx_business_glossary_term' = 'Personal Protective Equipment (PPE) Type Required');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `pre_employment_flag` SET TAGS ('dbx_business_glossary_term' = 'Pre-employment Medical Examination Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Health Surveillance Record Status');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `record_status` SET TAGS ('dbx_value_regex' = 'draft|completed|reviewed|approved|archived');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `regulatory_submission_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Required Flag');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Date');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `surveillance_type` SET TAGS ('dbx_business_glossary_term' = 'Health Surveillance Type');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `surveillance_type` SET TAGS ('dbx_value_regex' = 'audiometric|spirometry|blood_lead|medical_examination|fitness_assessment|exposure_monitoring');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `test_date` SET TAGS ('dbx_business_glossary_term' = 'Test or Assessment Date');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `test_frequency_months` SET TAGS ('dbx_business_glossary_term' = 'Test Frequency in Months');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `test_location` SET TAGS ('dbx_business_glossary_term' = 'Test Location');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `work_restrictions` SET TAGS ('dbx_business_glossary_term' = 'Work Restrictions or Limitations');
ALTER TABLE `mining_ecm`.`hse`.`health_surveillance` ALTER COLUMN `work_restrictions` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`emergency_response_plan` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `mining_ecm`.`hse`.`emergency_response_plan` SET TAGS ('dbx_subdomain' = 'safety_operations');
...
ALTER TABLE `mining_ecm`.`hse`.`emergency_response_plan` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`training_record` SET TAGS ('dbx_subdomain' = 'workforce_health');
...
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Worker ID');
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `procurement_vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Training Vendor Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `saleable_product_id` SET TAGS ('dbx_business_glossary_term' = 'Product Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `tenement_id` SET TAGS ('dbx_business_glossary_term' = 'Site ID');
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `training_course_id` SET TAGS ('dbx_business_glossary_term' = 'Training Program ID');
...
ALTER TABLE `mining_ecm`.`hse`.`training_record` ALTER COLUMN `training_location` SET TAGS ('dbx_business_glossary_term' = 'Training Location');
...
ALTER TABLE `mining_ecm`.`hse`.`chemical_register` SET TAGS ('dbx_subdomain' = 'environmental_compliance');
...
ALTER TABLE `mining_ecm`.`hse`.`chemical_register` ALTER COLUMN `procurement_vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Vendor Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`chemical_register` ALTER COLUMN `permit_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Permit Reference Number');
...
ALTER TABLE `mining_ecm`.`hse`.`chemical_register` ALTER COLUMN `storage_location` SET TAGS ('dbx_business_glossary_term' = 'Storage Location');
...
ALTER TABLE `mining_ecm`.`hse`.`audit` SET TAGS ('dbx_subdomain' = 'environmental_compliance');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Identifier');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `stakeholder_id` SET TAGS ('dbx_business_glossary_term' = 'Audited Stakeholder Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Lead Auditor ID');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `legal_register_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Register Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `procurement_vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Audited Vendor Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`hse`.`audit` ALTER COLUMN `area_inspected` SET TAGS ('dbx_business_glossary_term' = 'Area Inspected');
...
ALTER TABLE `mining_ecm`.`hse`.`legal_register` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` SET TAGS ('dbx_subdomain' = 'environmental_compliance');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `legal_register_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Register Identifier');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `tenement_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site ID');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `amendment_date` SET TAGS ('dbx_business_glossary_term' = 'Amendment Date');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `amendment_summary` SET TAGS ('dbx_business_glossary_term' = 'Amendment Summary');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `applicability_notes` SET TAGS ('dbx_business_glossary_term' = 'Applicability Notes');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `compliance_evidence_location` SET TAGS ('dbx_business_glossary_term' = 'Compliance Evidence Location');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `compliance_obligation_summary` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Summary');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `compliance_status` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|under_review|not_applicable|pending');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `legislation_name` SET TAGS ('dbx_business_glossary_term' = 'Legislation Name');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `legislation_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Legislation Reference Number');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `legislation_type` SET TAGS ('dbx_business_glossary_term' = 'Legislation Type');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `legislation_type` SET TAGS ('dbx_value_regex' = 'act|regulation|code_of_practice|standard|licence_condition|permit_condition');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `legislation_url` SET TAGS ('dbx_business_glossary_term' = 'Legislation Uniform Resource Locator (URL)');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `licence_number` SET TAGS ('dbx_business_glossary_term' = 'Licence Number');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `licence_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `licence_number` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `penalty_description` SET TAGS ('dbx_business_glossary_term' = 'Penalty Description');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `reporting_frequency` SET TAGS ('dbx_business_glossary_term' = 'Reporting Frequency');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `responsible_owner` SET TAGS ('dbx_business_glossary_term' = 'Responsible Owner');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `mining_ecm`.`hse`.`legal_register` ALTER COLUMN `superseded_by_legislation` SET TAGS ('dbx_business_glossary_term' = 'Superseded By Legislation');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` SET TAGS ('dbx_subdomain' = 'safety_operations');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `management_of_change_id` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) ID');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approver Employee ID');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `primary_management_initiator_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Initiator Employee ID');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `primary_management_initiator_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `primary_management_initiator_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Employee ID');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `risk_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment ID');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `saleable_product_id` SET TAGS ('dbx_business_glossary_term' = 'Product Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `specification_id` SET TAGS ('dbx_business_glossary_term' = 'Product Specification Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `tenement_id` SET TAGS ('dbx_business_glossary_term' = 'Site ID');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `superseded_management_of_change_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `actual_cost` SET TAGS ('dbx_business_glossary_term' = 'Actual Cost');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `affected_area` SET TAGS ('dbx_business_glossary_term' = 'Affected Area');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `affected_process` SET TAGS ('dbx_business_glossary_term' = 'Affected Process');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `approval_workflow_stage` SET TAGS ('dbx_business_glossary_term' = 'Approval Workflow Stage');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `approver_name` SET TAGS ('dbx_business_glossary_term' = 'Approver Name');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `change_category` SET TAGS ('dbx_business_glossary_term' = 'Change Category');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `change_category` SET TAGS ('dbx_value_regex' = 'equipment|process|chemical|organizational|facility|procedure');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `change_description` SET TAGS ('dbx_business_glossary_term' = 'Change Description');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `change_title` SET TAGS ('dbx_business_glossary_term' = 'Change Title');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `change_type` SET TAGS ('dbx_business_glossary_term' = 'Change Type');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `change_type` SET TAGS ('dbx_value_regex' = 'temporary|permanent|emergency|trial');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `closure_verification_date` SET TAGS ('dbx_business_glossary_term' = 'Closure Verification Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `closure_verified_by` SET TAGS ('dbx_business_glossary_term' = 'Closure Verified By');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `comments` SET TAGS ('dbx_business_glossary_term' = 'Comments');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `control_measures` SET TAGS ('dbx_business_glossary_term' = 'Control Measures');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Cost');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `hazard_categories` SET TAGS ('dbx_business_glossary_term' = 'Hazard Categories');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `implementation_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Implementation Completion Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `implementation_plan` SET TAGS ('dbx_business_glossary_term' = 'Implementation Plan');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `implementation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Implementation Start Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `inherent_risk_level` SET TAGS ('dbx_business_glossary_term' = 'Inherent Risk Level');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `inherent_risk_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `initiator_department` SET TAGS ('dbx_business_glossary_term' = 'Initiator Department');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `initiator_name` SET TAGS ('dbx_business_glossary_term' = 'Initiator Name');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `moc_number` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Number');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `moc_status` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Status');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `pssr_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Pre-Startup Safety Review (PSSR) Completion Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `pssr_outcome` SET TAGS ('dbx_business_glossary_term' = 'Pre-Startup Safety Review (PSSR) Outcome');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `pssr_outcome` SET TAGS ('dbx_value_regex' = 'passed|failed|conditional');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `pssr_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Pre-Startup Safety Review (PSSR) Required Flag');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `regulatory_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `regulatory_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Required Flag');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `required_implementation_date` SET TAGS ('dbx_business_glossary_term' = 'Required Implementation Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `residual_risk_level` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Level');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `residual_risk_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `reviewer_name` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Name');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `sop_updated_flag` SET TAGS ('dbx_business_glossary_term' = 'Standard Operating Procedure (SOP) Updated Flag');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Submission Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `training_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Date');
ALTER TABLE `mining_ecm`.`hse`.`management_of_change` ALTER COLUMN `training_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Required Flag');
...
ALTER TABLE `mining_ecm`.`hse`.`investigation` ALTER COLUMN `related_investigation_id` SET TAGS ('dbx_self_ref_fk' = 'true');
