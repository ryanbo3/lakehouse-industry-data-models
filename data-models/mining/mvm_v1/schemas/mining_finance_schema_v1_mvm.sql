-- Schema for Domain: finance | Business: Mining | Version: v1_mvm
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`cost_centre` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: SAP CO cost centres are defined within a company code which maps to a legal entity. The cost_centre table currently stores company_code as a denormalized string. Adding legal_entity_id as a FK normali',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`general_ledger_account` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: GL accounts in SAP FI are defined per company code (legal entity) via the chart of accounts assignment. The general_ledger_account table currently stores company_code as a denormalized string. Adding ',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`profit_centre` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: SAP CO profit centres are assigned to company codes (legal entities) for segment reporting under IFRS. The profit_centre table currently stores company_code as a denormalized string. Adding legal_enti',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`wbs_element` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: WBS elements in SAP PS are assigned to company codes (legal entities) for project accounting and IFRS 6 exploration accounting. The wbs_element table currently stores company_code as a denormalized st',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`journal_entry` (
    ...
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: SAP FI journal entry headers for asset transactions reference the fixed asset via asset number and subnumber. The journal_entry table currently stores asset_number and asset_subnumber as denormalized ',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: In SAP FI, every journal entry is posted to a company code which maps 1:1 to a legal entity. The journal_entry table currently stores company_code as a denormalized string. Adding legal_entity_id as a',
    `mine_site_id` BIGINT COMMENT 'Foreign key linking to mine.mine_site. Business justification: Journal entries are posted against mine sites for site-level P&L reporting and cost allocation. Every mining journal entry is coded to a mine site — a fundamental financial posting requirement for sit',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`journal_entry_line` (
    ...
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: In SAP FI Asset Accounting, journal entry lines for asset transactions (acquisitions, depreciation, disposals) reference specific fixed assets via asset number and subnumber. The journal_entry_line ta',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`capex_budget` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_contract. Business justification: Major mining capital projects are executed under procurement contracts (EPC, EPCM). Linking capex_budget to the delivery contract enables contract vs. budget variance monitoring — a core requirement i',
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: CAPEX budgets are approved and managed at the legal entity level for regulatory financial disclosures and IFRS reporting. Adding legal_entity_id to capex_budget enables entity-level CAPEX portfolio ma',
    ...
    `project_valuation_id` BIGINT COMMENT 'Foreign key linking to finance.project_valuation. Business justification: CAPEX budgets in mining are derived from and approved based on project valuations (NPV/IRR/DCF models). Linking capex_budget to project_valuation enables traceability from approved investment decision',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`opex_budget` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_contract. Business justification: Mining OPEX budgets for contracted services (maintenance, reagents, haulage) are executed under standing procurement contracts. This FK enables contract spend vs. budget tracking and supports C1 cash ',
    ...
    `mine_site_id` BIGINT COMMENT 'Foreign key linking to mine.mine_site. Business justification: OPEX budgets are prepared at mine site level for site-level cost management and reporting. opex_budget.mine_site_code is a denormalized plain-text reference; replacing with mine_site_id FK enforces re',
    ...
    `project_valuation_id` BIGINT COMMENT 'Foreign key linking to finance.project_valuation. Business justification: OPEX budgets for mining operations are informed by project valuations which include opex_unit_usd and production rate assumptions. Linking opex_budget to project_valuation enables comparison of budget',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`cost_allocation` (
    ...
    `general_ledger_account_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger_account. Business justification: In SAP CO, cost elements are directly mapped to GL accounts. The cost_allocation table currently stores cost_element_code and cost_element_name as denormalized strings. Adding general_ledger_account_i',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Every SAP CO cost allocation cycle generates a corresponding FI journal entry (the allocation document). The cost_allocation table currently stores allocation_document_number and original_document_num',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: SAP CO cost allocations are executed within a controlling area and company code (legal entity). The cost_allocation table currently stores company_code as a denormalized string. Adding legal_entity_id',
    `mine_site_id` BIGINT COMMENT 'Foreign key linking to mine.mine_site. Business justification: Cost allocations are performed between mine sites (shared services, inter-site equipment). cost_allocation.mine_site_code is a denormalized reference; replacing with mine_site_id FK enables proper int',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`cost_performance_report` (
    ...
    `mine_site_id` BIGINT COMMENT 'Reference to the mine site for which this cost performance report is generated. Enables site-level AISC benchmarking and multi-site portfolio cost analysis.',
    `production_schedule_id` BIGINT COMMENT 'Foreign key linking to mine.production_schedule. Business justification: Cost performance reports reference the production schedule as their production basis for unit cost calculation (actual cost / scheduled tonnes). Finance teams link CPRs to the governing production sch',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`project_valuation` (
    ...
    `business_unit_id` BIGINT COMMENT 'Foreign key linking to finance.business_unit. Business justification: Project valuations are conducted for specific mining business units (e.g., a specific mine or processing operation). Linking to business_unit enables business-unit-level investment portfolio analysis ',
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Mining project valuations are prepared at the legal entity level for IFRS 6 exploration accounting and regulatory financial disclosures. Linking to legal_entity enables entity-level consolidation of N',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Project valuations represent investment decisions for autonomous business units (profit centres). Linking project_valuation to profit_centre enables P&L-level investment analysis and AISC/C1 cost repo',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Project valuations (NPV/IRR/DCF models) in SAP PS are directly tied to WBS elements representing the hierarchical project cost structure. Adding wbs_element_id normalizes the project dimension and ena',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`royalty_obligation` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Royalty obligations are legal commitments of specific legal entities to government authorities and landowners. Linking royalty_obligation to legal_entity enables entity-level royalty liability reporti',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`rehabilitation_provision` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Mine rehabilitation provisions are recognized as liabilities on the balance sheet of specific legal entities under IFRS (IAS 37). Linking rehabilitation_provision to legal_entity enables entity-level ',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`fixed_asset` (
    ...
    `asset_class_id` BIGINT COMMENT 'FK to equipment.asset_class',
    `capex_budget_id` BIGINT COMMENT 'Foreign key linking to finance.capex_budget. Business justification: Fixed assets are capitalised from CAPEX budgets — the asset acquisition is funded by and tracked against a specific CAPEX budget line. The fixed_asset table currently stores capex_budget_code as a den',
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Fixed assets in SAP FI Asset Accounting are managed per company code (legal entity). The fixed_asset table currently stores company_code as a denormalized string. Adding legal_entity_id as a FK normal',
    `mine_site_id` BIGINT COMMENT 'Foreign key linking to mine.mine_site. Business justification: Fixed assets (plant, equipment, infrastructure) are located at mine sites for depreciation, insurance, and asset management purposes. Finance teams maintain site-level fixed asset registers — required',
    ...
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_purchase_order. Business justification: In mining, fixed assets (mobile fleet, processing equipment) are capitalised from purchase orders. Linking fixed_asset to its originating PO supports asset audit trails, warranty tracking, and CAPEX-t',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`vendor_invoice` (
    ...
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_goods_receipt. Business justification: Three-way match in mining AP requires linking each invoice to the specific goods receipt confirming delivery. goods_receipt_number is a denormalized reference; this FK enables automated invoice verifi',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Vendor invoices in SAP FI are posted to a company code (legal entity). The vendor_invoice table currently stores company_code as a denormalized string. Adding legal_entity_id as a FK normalizes this a',
    `mine_site_id` BIGINT COMMENT 'Foreign key linking to mine.mine_site. Business justification: Vendor invoices are coded to mine sites for site-level cost tracking and accounts payable management. vendor_invoice.mine_site_code is a denormalized reference; replacing with mine_site_id FK enables ',
    ...
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_purchase_order. Business justification: Three-way match (PO + GR + invoice) is the core AP process in mining. Every vendor invoice must be matched to a purchase order for payment approval and budget commitment relief. purchase_order_number ',
    `royalty_obligation_id` BIGINT COMMENT 'Foreign key linking to finance.royalty_obligation. Business justification: Royalty payments to government authorities and landowners are processed as vendor invoices in SAP FI. Linking vendor_invoice to royalty_obligation enables tracking of actual payments against accrued r',
...
CREATE OR REPLACE TABLE `mining_ecm`.`finance`.`legal_entity` (
    ...
    `parent_entity_legal_entity_id` BIGINT COMMENT 'Reference to the immediate parent legal entity in the corporate ownership hierarchy. Null for ultimate parent entities.',
    `parent_legal_entity_id` BIGINT COMMENT 'Self-referencing FK on legal_entity (parent_legal_entity_id)',
    `primary_ultimate_parent_entity_legal_entity_id` BIGINT COMMENT 'Reference to the top-level parent legal entity in the corporate ownership hierarchy. Self-referencing for ultimate parent entities.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `mining_ecm`.`finance`.`cost_centre` ADD CONSTRAINT `fk_finance_cost_centre_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`general_ledger_account` ADD CONSTRAINT `fk_finance_general_ledger_account_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
ALTER TABLE `mining_ecm`.`finance`.`profit_centre` ADD CONSTRAINT `fk_finance_profit_centre_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`wbs_element` ADD CONSTRAINT `fk_finance_wbs_element_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`journal_entry` ADD CONSTRAINT `fk_finance_journal_entry_fixed_asset_id` FOREIGN KEY (`fixed_asset_id`) REFERENCES `mining_ecm`.`finance`.`fixed_asset`(`fixed_asset_id`);
ALTER TABLE `mining_ecm`.`finance`.`journal_entry` ADD CONSTRAINT `fk_finance_journal_entry_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`journal_entry_line` ADD CONSTRAINT `fk_finance_journal_entry_line_fixed_asset_id` FOREIGN KEY (`fixed_asset_id`) REFERENCES `mining_ecm`.`finance`.`fixed_asset`(`fixed_asset_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`capex_budget` ADD CONSTRAINT `fk_finance_capex_budget_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`capex_budget` ADD CONSTRAINT `fk_finance_capex_budget_project_valuation_id` FOREIGN KEY (`project_valuation_id`) REFERENCES `mining_ecm`.`finance`.`project_valuation`(`project_valuation_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`opex_budget` ADD CONSTRAINT `fk_finance_opex_budget_project_valuation_id` FOREIGN KEY (`project_valuation_id`) REFERENCES `mining_ecm`.`finance`.`project_valuation`(`project_valuation_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ADD CONSTRAINT `fk_finance_cost_allocation_general_ledger_account_id` FOREIGN KEY (`general_ledger_account_id`) REFERENCES `mining_ecm`.`finance`.`general_ledger_account`(`general_ledger_account_id`);
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ADD CONSTRAINT `fk_finance_cost_allocation_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `mining_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ADD CONSTRAINT `fk_finance_cost_allocation_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ADD CONSTRAINT `fk_finance_project_valuation_business_unit_id` FOREIGN KEY (`business_unit_id`) REFERENCES `mining_ecm`.`finance`.`business_unit`(`business_unit_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ADD CONSTRAINT `fk_finance_project_valuation_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ADD CONSTRAINT `fk_finance_project_valuation_profit_centre_id` FOREIGN KEY (`profit_centre_id`) REFERENCES `mining_ecm`.`finance`.`profit_centre`(`profit_centre_id`);
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ADD CONSTRAINT `fk_finance_project_valuation_wbs_element_id` FOREIGN KEY (`wbs_element_id`) REFERENCES `mining_ecm`.`finance`.`wbs_element`(`wbs_element_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`royalty_obligation` ADD CONSTRAINT `fk_finance_royalty_obligation_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`rehabilitation_provision` ADD CONSTRAINT `fk_finance_rehabilitation_provision_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` ADD CONSTRAINT `fk_finance_fixed_asset_capex_budget_id` FOREIGN KEY (`capex_budget_id`) REFERENCES `mining_ecm`.`finance`.`capex_budget`(`capex_budget_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` ADD CONSTRAINT `fk_finance_fixed_asset_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ADD CONSTRAINT `fk_finance_vendor_invoice_legal_entity_id` FOREIGN KEY (`legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ADD CONSTRAINT `fk_finance_vendor_invoice_royalty_obligation_id` FOREIGN KEY (`royalty_obligation_id`) REFERENCES `mining_ecm`.`finance`.`royalty_obligation`(`royalty_obligation_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`legal_entity` ADD CONSTRAINT `fk_finance_legal_entity_parent_entity_legal_entity_id` FOREIGN KEY (`parent_entity_legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`legal_entity` ADD CONSTRAINT `fk_finance_legal_entity_primary_ultimate_parent_entity_legal_entity_id` FOREIGN KEY (`primary_ultimate_parent_entity_legal_entity_id`) REFERENCES `mining_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `mining_ecm`.`finance`.`cost_centre` SET TAGS ('dbx_subdomain' = 'cost_management');
...
ALTER TABLE `mining_ecm`.`finance`.`cost_centre` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`general_ledger_account` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `mining_ecm`.`finance`.`general_ledger_account` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`profit_centre` SET TAGS ('dbx_subdomain' = 'cost_management');
...
ALTER TABLE `mining_ecm`.`finance`.`profit_centre` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`wbs_element` SET TAGS ('dbx_subdomain' = 'capital_projects');
...
ALTER TABLE `mining_ecm`.`finance`.`wbs_element` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`journal_entry` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `mining_ecm`.`finance`.`journal_entry` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`journal_entry` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`journal_entry` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`journal_entry_line` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `mining_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`capex_budget` SET TAGS ('dbx_subdomain' = 'capital_projects');
...
ALTER TABLE `mining_ecm`.`finance`.`capex_budget` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`capex_budget` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`capex_budget` ALTER COLUMN `project_valuation_id` SET TAGS ('dbx_business_glossary_term' = 'Project Valuation Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`opex_budget` SET TAGS ('dbx_subdomain' = 'cost_management');
...
ALTER TABLE `mining_ecm`.`finance`.`opex_budget` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`opex_budget` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`opex_budget` ALTER COLUMN `project_valuation_id` SET TAGS ('dbx_business_glossary_term' = 'Project Valuation Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` SET TAGS ('dbx_subdomain' = 'cost_management');
...
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ALTER COLUMN `general_ledger_account_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Account Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`cost_allocation` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`cost_performance_report` SET TAGS ('dbx_subdomain' = 'cost_management');
...
ALTER TABLE `mining_ecm`.`finance`.`cost_performance_report` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site ID');
ALTER TABLE `mining_ecm`.`finance`.`cost_performance_report` ALTER COLUMN `production_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Production Schedule Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` SET TAGS ('dbx_subdomain' = 'capital_projects');
...
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ALTER COLUMN `business_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Business Unit Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`project_valuation` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`royalty_obligation` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`finance`.`royalty_obligation` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`rehabilitation_provision` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`finance`.`rehabilitation_provision` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` SET TAGS ('dbx_subdomain' = 'capital_projects');
...
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` ALTER COLUMN `capex_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Capex Budget Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`fixed_asset` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Purchase Order Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Goods Receipt Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Purchase Order Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ALTER COLUMN `royalty_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Royalty Obligation Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`finance`.`vendor_invoice` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
...
ALTER TABLE `mining_ecm`.`finance`.`legal_entity` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`finance`.`business_unit` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`finance`.`business_unit` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `mining_ecm`.`finance`.`business_unit` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `mining_ecm`.`finance`.`business_unit` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `mining_ecm`.`finance`.`business_unit` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
