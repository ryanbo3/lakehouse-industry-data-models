-- Schema for Domain: tenement | Business: Mining | Version: v1_mvm
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`tenement` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Tenements are held by specific legal entities — this is a fundamental regulatory requirement (mining licences are granted to legal entities). Finance teams need legal entity linkage for consolidation ',
    ...
    `rehabilitation_provision_id` BIGINT COMMENT 'Foreign key linking to finance.rehabilitation_provision. Business justification: Rehabilitation provisions under IFRS/AASB 137 are calculated per tenement area under mining legislation. Finance teams must link each tenement to its rehabilitation provision for closure cost estimati',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Tenements are represented as WBS elements in mining ERP systems for IFRS 6 exploration expenditure capitalisation. Finance teams link tenements to WBS for exploration cost tracking, impairment testing',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`boundary` (
    ...
    `heritage_clearance_id` BIGINT COMMENT 'Foreign key linking to tenement.heritage_clearance. Business justification: boundary.heritage_area_overlap_flag indicates that a boundary polygon overlaps with a heritage area requiring clearance. Adding heritage_clearance_id links the specific boundary version to the heritag',
    `native_title_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.native_title_agreement. Business justification: boundary.native_title_overlap_flag indicates that a boundary polygon overlaps with a native title claim or determination area. Adding native_title_agreement_id links the specific boundary version to t',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`expenditure_commitment` (
    ...
    `general_ledger_account_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger_account. Business justification: Expenditure commitments require GL account coding for financial reporting and commitment management in mining ERP. Finance teams post commitment accruals to specific GL accounts (ifrs6_exploration_fla',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.purchase_order. Business justification: Statutory expenditure commitments on mining tenements are fulfilled through actual procurement spend. Mining companies must demonstrate regulatory compliance by linking committed amounts to actual POs',
    `regulatory_condition_id` BIGINT COMMENT 'Foreign key linking to tenement.regulatory_condition. Business justification: Expenditure commitments in mining are frequently mandated by specific regulatory conditions attached to a tenement (e.g., minimum work program requirements, environmental rehabilitation bonds). Linkin',
    ...
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Tenement expenditure commitments (minimum spend conditions) are tracked against WBS elements in mining ERP for IFRS 6 exploration capitalisation. Finance teams require WBS coding on expenditure commit',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`renewal_obligation` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_contract. Business justification: Renewal obligations require minimum work programs (drilling, geophysical surveys, environmental studies) fulfilled through procurement contracts. Tenement managers must link the renewal obligation to ',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Renewal fees, lodgement costs, and expenditure commitments on renewal obligations are charged to cost centres for AISC and operational cost reporting. Mining finance teams require cost centre coding o',
    `expenditure_commitment_id` BIGINT COMMENT 'Foreign key linking to tenement.expenditure_commitment. Business justification: renewal_obligation carries denormalized expenditure_commitment_amount and expenditure_commitment_currency_code fields, indicating a direct relationship to the expenditure_commitment entity. Linking re',
    `heritage_clearance_id` BIGINT COMMENT 'Foreign key linking to tenement.heritage_clearance. Business justification: renewal_obligation.heritage_clearance_required_flag and heritage_clearance_status indicate that tenement renewals require heritage clearance. Adding heritage_clearance_id links the renewal obligation ',
    `native_title_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.native_title_agreement. Business justification: renewal_obligation.native_title_clearance_required_flag and native_title_clearance_status indicate that tenement renewals must satisfy native title requirements. Adding native_title_agreement_id links',
    ...
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Renewal programs are managed as WBS projects in mining ERP systems (SAP). Renewal expenditure commitments require WBS coding for IFRS 6 exploration capitalisation decisions and LOM capital planning. M',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`regulatory_condition` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_contract. Business justification: Regulatory conditions (environmental monitoring, rehabilitation, water management) are fulfilled through specialist contracted services. Compliance officers must link each condition to its delivery co',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Remediation, monitoring, and penalty costs arising from regulatory conditions are charged to cost centres. Mining finance teams track regulatory compliance costs by cost centre for AISC reporting and ',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`native_title_agreement` (
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Native title compensation payments and compliance costs are charged to cost centres for AISC reporting. Mining finance teams track native title obligations by cost centre for social investment reporti',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Native title agreements are executed by specific legal entities within a mining group. The contracting legal entity is a fundamental regulatory and financial reporting requirement — compensation oblig',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`heritage_clearance` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.contract. Business justification: Heritage surveys are contracted services from specialized archaeological/anthropological firms. Real mining operations link clearance records to service contracts for heritage survey procurement track',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Heritage survey and clearance costs (survey_cost_aud exists as denormalized amount) are charged to cost centres. Mining finance teams track heritage compliance costs by cost centre for AISC reporting ',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`surface_right` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.contract. Business justification: Surface rights agreements (land access, landholder compensation) are formalized as procurement contracts with landowners treated as vendors for payment processing. Business process: land access paymen',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Surface right compensation payments are charged to cost centres for AISC and operational cost reporting. Mining finance teams track land access costs by cost centre. A domain expert would expect surfa',
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Long-term surface rights are capitalised as right-of-use assets or intangible assets under IFRS 16/IAS 38. Mining finance teams link surface rights to fixed asset records for depreciation, impairment ',
    `counterparty_id` BIGINT COMMENT 'Foreign key linking to customer.counterparty. Business justification: Surface right grantors (landowners, pastoral lessees) are counterparties requiring KYC, contract management, and compensation payment processing. Mining land access teams register grantors as counterp',
    `heritage_clearance_id` BIGINT COMMENT 'Foreign key linking to tenement.heritage_clearance. Business justification: surface_right.heritage_clearance_flag indicates that a heritage clearance is required before ground disturbance on the surface right area. Adding heritage_clearance_id normalizes this boolean flag int',
    `native_title_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.native_title_agreement. Business justification: surface_right.native_title_clearance_flag indicates that surface rights may be subject to native title obligations. Adding native_title_agreement_id links the surface right to the specific native titl',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`royalty_agreement` (
    ...
    `commodity_id` BIGINT COMMENT 'Foreign key linking to product.commodity. Business justification: Royalty agreements in mining are commodity-specific — the royalty rate and formula depend on the commodity being extracted. Linking enables royalty reporting by commodity class and regulatory return p',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Royalty agreement costs are charged to cost centres for AISC reporting and operational cost management. Mining finance teams need cost centre coding on royalty agreements for accrual budgeting and var',
    `general_ledger_account_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger_account. Business justification: Royalty agreements drive accrual postings to specific GL accounts (royalty_account_flag exists on GL). Finance teams need GL account linkage on royalty agreements for automated accrual generation, fin',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Royalty agreements are executed by specific legal entities. Finance teams need legal entity linkage for consolidation, transfer pricing, tax reporting, and regulatory disclosure of royalty obligations',
    `native_title_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.native_title_agreement. Business justification: native_title_agreement carries royalty_basis and royalty_rate_percent fields, indicating that royalty agreements can originate from or be governed by native title agreements. Adding native_title_agree',
    ...
    `payment_term_id` BIGINT COMMENT 'Foreign key linking to customer.payment_term. Business justification: Royalty agreements are financial contracts requiring standardized payment terms (LC type, grace period, penalty rates) from the master payment_term catalogue. Mining companies align royalty payment sc',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`royalty_payment` (
    ...
    `cargo_shipment_id` BIGINT COMMENT 'Foreign key linking to sales.cargo_shipment. Business justification: Royalty payments in mining are triggered by and calculated against specific cargo shipments (shipped quantity × realized price). Royalty reconciliation and regulatory returns require tracing each paym',
    ...
    `metallurgical_balance_id` BIGINT COMMENT 'Foreign key linking to processing.metallurgical_balance. Business justification: Royalty payments are calculated from the certified metallurgical balance for the production period — production tonnage, grade, and recovery data flow directly from the balance to the royalty calculat',
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to customer.bank_account. Business justification: Royalty payment disbursement requires linking to the payees verified bank account for treasury execution and audit. Mining finance teams must record which bank_account received each royalty payment. ',
    ...
    `royalty_obligation_id` BIGINT COMMENT 'Foreign key linking to finance.royalty_obligation. Business justification: Royalty payments must be reconciled against royalty obligations for financial reporting and regulatory compliance. Finance teams need direct payment-to-obligation linkage for liability clearance, paym',
    `saleable_product_id` BIGINT COMMENT 'Foreign key linking to product.saleable_product. Business justification: Royalty payments are calculated on production value of specific saleable products sold from a tenement. Linking enables product-level royalty reconciliation reports and audit trails — a core mining fi',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`application` (
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Application fees, environmental assessment costs, and legal costs for tenement applications are charged to cost centres. Mining finance teams track application costs by cost centre for exploration bud',
    `expenditure_commitment_id` BIGINT COMMENT 'Foreign key linking to tenement.expenditure_commitment. Business justification: application.expenditure_commitment_amount and expenditure_commitment_currency indicate that tenement applications carry expenditure commitment obligations as conditions of grant. Adding expenditure_co',
    ...
    `heritage_clearance_id` BIGINT COMMENT 'Foreign key linking to tenement.heritage_clearance. Business justification: application.heritage_clearance_required_flag and heritage_clearance_status indicate that tenement applications are directly linked to heritage clearance requirements. Adding heritage_clearance_id link',
    `native_title_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.native_title_agreement. Business justification: application.native_title_clearance_required_flag and native_title_clearance_status indicate that tenement applications must satisfy native title requirements before grant. Adding native_title_agreemen',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: New tenement application costs (legal fees, environmental assessments, heritage surveys) are tracked under IFRS 6 WBS elements for capitalisation vs expensing decisions. Mining finance teams require W',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`holder` (
    ...
    `application_id` BIGINT COMMENT 'Foreign key linking to tenement.application. Business justification: A holder record is established when a tenement application is granted or when an interest is transferred via a new application. Linking holder to the originating application provides a complete audit ',
    ...
    `royalty_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.royalty_agreement. Business justification: holder.royalty_obligation_flag and royalty_rate_percentage indicate that specific holders may be subject to royalty obligations. Adding royalty_agreement_id links the holder record to the specific roy',
...
CREATE OR REPLACE TABLE `mining_ecm`.`tenement`.`relinquishment` (
    ...
    `boundary_id` BIGINT COMMENT 'Foreign key linking to tenement.boundary. Business justification: A relinquishment involves surrendering a specific portion of a tenements GIS boundary. relinquishment.gis_boundary_file_reference is a denormalized file path reference to the boundary being relinquis',
    `contract_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_contract. Business justification: Relinquishment triggers rehabilitation and environmental remediation work executed via procurement contracts. Mining compliance managers must trace which rehabilitation contracts are associated with e',
    ...
    `heritage_clearance_id` BIGINT COMMENT 'Foreign key linking to tenement.heritage_clearance. Business justification: relinquishment.heritage_clearance_flag indicates that a heritage clearance is required before a relinquishment can be executed (to ensure no heritage sites are disturbed during the surrender process).',
    `native_title_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.native_title_agreement. Business justification: relinquishment.native_title_clearance_flag indicates that native title clearance is required before a relinquishment can proceed. Adding native_title_agreement_id links the relinquishment to the gover',
    `orebody_id` BIGINT COMMENT 'Foreign key linking to geology.orebody. Business justification: Tenement relinquishment decisions are driven by orebody depletion, uneconomic resource classification, or exhausted reserves. Tenement managers must record which orebodys geological status (depleted,',
    `rehabilitation_provision_id` BIGINT COMMENT 'Foreign key linking to finance.rehabilitation_provision. Business justification: Relinquishment directly triggers rehabilitation provision adjustments under IFRS/AASB 137 — reduced disturbance area reduces the provision balance. Mining finance teams must update rehabilitation prov',
    `renewal_obligation_id` BIGINT COMMENT 'Foreign key linking to tenement.renewal_obligation. Business justification: A relinquishment event is frequently triggered by a renewal obligation that requires area surrender (renewal_obligation.area_relinquishment_required_flag). Linking relinquishment to its originating re',
    `tenement_id` BIGINT COMMENT 'Reference to the tenement from which area is being relinquished.',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Relinquishment activities (surveys, regulatory lodgements, rehabilitation works) are tracked as WBS elements in mining ERP for cost capture and project close-out. Finance teams need WBS coding on reli',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `mining_ecm`.`tenement`.`boundary` ADD CONSTRAINT `fk_tenement_boundary_heritage_clearance_id` FOREIGN KEY (`heritage_clearance_id`) REFERENCES `mining_ecm`.`tenement`.`heritage_clearance`(`heritage_clearance_id`);
ALTER TABLE `mining_ecm`.`tenement`.`boundary` ADD CONSTRAINT `fk_tenement_boundary_native_title_agreement_id` FOREIGN KEY (`native_title_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`native_title_agreement`(`native_title_agreement_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`expenditure_commitment` ADD CONSTRAINT `fk_tenement_expenditure_commitment_regulatory_condition_id` FOREIGN KEY (`regulatory_condition_id`) REFERENCES `mining_ecm`.`tenement`.`regulatory_condition`(`regulatory_condition_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ADD CONSTRAINT `fk_tenement_renewal_obligation_expenditure_commitment_id` FOREIGN KEY (`expenditure_commitment_id`) REFERENCES `mining_ecm`.`tenement`.`expenditure_commitment`(`expenditure_commitment_id`);
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ADD CONSTRAINT `fk_tenement_renewal_obligation_heritage_clearance_id` FOREIGN KEY (`heritage_clearance_id`) REFERENCES `mining_ecm`.`tenement`.`heritage_clearance`(`heritage_clearance_id`);
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ADD CONSTRAINT `fk_tenement_renewal_obligation_native_title_agreement_id` FOREIGN KEY (`native_title_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`native_title_agreement`(`native_title_agreement_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ADD CONSTRAINT `fk_tenement_surface_right_heritage_clearance_id` FOREIGN KEY (`heritage_clearance_id`) REFERENCES `mining_ecm`.`tenement`.`heritage_clearance`(`heritage_clearance_id`);
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ADD CONSTRAINT `fk_tenement_surface_right_native_title_agreement_id` FOREIGN KEY (`native_title_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`native_title_agreement`(`native_title_agreement_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ADD CONSTRAINT `fk_tenement_royalty_agreement_native_title_agreement_id` FOREIGN KEY (`native_title_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`native_title_agreement`(`native_title_agreement_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`application` ADD CONSTRAINT `fk_tenement_application_expenditure_commitment_id` FOREIGN KEY (`expenditure_commitment_id`) REFERENCES `mining_ecm`.`tenement`.`expenditure_commitment`(`expenditure_commitment_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`application` ADD CONSTRAINT `fk_tenement_application_heritage_clearance_id` FOREIGN KEY (`heritage_clearance_id`) REFERENCES `mining_ecm`.`tenement`.`heritage_clearance`(`heritage_clearance_id`);
ALTER TABLE `mining_ecm`.`tenement`.`application` ADD CONSTRAINT `fk_tenement_application_native_title_agreement_id` FOREIGN KEY (`native_title_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`native_title_agreement`(`native_title_agreement_id`);
ALTER TABLE `mining_ecm`.`tenement`.`holder` ADD CONSTRAINT `fk_tenement_holder_application_id` FOREIGN KEY (`application_id`) REFERENCES `mining_ecm`.`tenement`.`application`(`application_id`);
ALTER TABLE `mining_ecm`.`tenement`.`holder` ADD CONSTRAINT `fk_tenement_holder_royalty_agreement_id` FOREIGN KEY (`royalty_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`royalty_agreement`(`royalty_agreement_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ADD CONSTRAINT `fk_tenement_relinquishment_boundary_id` FOREIGN KEY (`boundary_id`) REFERENCES `mining_ecm`.`tenement`.`boundary`(`boundary_id`);
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ADD CONSTRAINT `fk_tenement_relinquishment_heritage_clearance_id` FOREIGN KEY (`heritage_clearance_id`) REFERENCES `mining_ecm`.`tenement`.`heritage_clearance`(`heritage_clearance_id`);
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ADD CONSTRAINT `fk_tenement_relinquishment_native_title_agreement_id` FOREIGN KEY (`native_title_agreement_id`) REFERENCES `mining_ecm`.`tenement`.`native_title_agreement`(`native_title_agreement_id`);
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ADD CONSTRAINT `fk_tenement_relinquishment_renewal_obligation_id` FOREIGN KEY (`renewal_obligation_id`) REFERENCES `mining_ecm`.`tenement`.`renewal_obligation`(`renewal_obligation_id`);
...
ALTER TABLE `mining_ecm`.`tenement`.`tenement` SET TAGS ('dbx_subdomain' = 'tenement_registry');
...
ALTER TABLE `mining_ecm`.`tenement`.`tenement` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`tenement` ALTER COLUMN `rehabilitation_provision_id` SET TAGS ('dbx_business_glossary_term' = 'Rehabilitation Provision Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`tenement` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`boundary` SET TAGS ('dbx_subdomain' = 'tenement_registry');
...
ALTER TABLE `mining_ecm`.`tenement`.`boundary` ALTER COLUMN `heritage_clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Heritage Clearance Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`boundary` ALTER COLUMN `native_title_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Native Title Agreement Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`expenditure_commitment` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`tenement`.`expenditure_commitment` ALTER COLUMN `general_ledger_account_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Account Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`expenditure_commitment` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Purchase Order Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`expenditure_commitment` ALTER COLUMN `regulatory_condition_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Condition Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`expenditure_commitment` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ALTER COLUMN `expenditure_commitment_id` SET TAGS ('dbx_business_glossary_term' = 'Expenditure Commitment Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ALTER COLUMN `heritage_clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Heritage Clearance Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ALTER COLUMN `native_title_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Native Title Agreement Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`renewal_obligation` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`regulatory_condition` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`tenement`.`regulatory_condition` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`regulatory_condition` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`native_title_agreement` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`tenement`.`native_title_agreement` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`native_title_agreement` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`heritage_clearance` SET TAGS ('dbx_subdomain' = 'compliance_obligations');
...
ALTER TABLE `mining_ecm`.`tenement`.`heritage_clearance` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`heritage_clearance` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` SET TAGS ('dbx_subdomain' = 'royalty_rights');
...
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ALTER COLUMN `counterparty_id` SET TAGS ('dbx_business_glossary_term' = 'Grantor Counterparty Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ALTER COLUMN `heritage_clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Heritage Clearance Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`surface_right` ALTER COLUMN `native_title_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Native Title Agreement Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` SET TAGS ('dbx_subdomain' = 'royalty_rights');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ALTER COLUMN `general_ledger_account_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Account Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ALTER COLUMN `native_title_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Native Title Agreement Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_agreement` ALTER COLUMN `payment_term_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Term Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` SET TAGS ('dbx_subdomain' = 'royalty_rights');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `cargo_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Shipment Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `metallurgical_balance_id` SET TAGS ('dbx_business_glossary_term' = 'Metallurgical Balance Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Payee Bank Account Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `royalty_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Royalty Obligation Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`royalty_payment` ALTER COLUMN `saleable_product_id` SET TAGS ('dbx_business_glossary_term' = 'Saleable Product Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`application` SET TAGS ('dbx_subdomain' = 'tenement_registry');
...
ALTER TABLE `mining_ecm`.`tenement`.`application` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`application` ALTER COLUMN `expenditure_commitment_id` SET TAGS ('dbx_business_glossary_term' = 'Expenditure Commitment Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`application` ALTER COLUMN `heritage_clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Heritage Clearance Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`application` ALTER COLUMN `native_title_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Native Title Agreement Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`application` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`holder` SET TAGS ('dbx_subdomain' = 'tenement_registry');
...
ALTER TABLE `mining_ecm`.`tenement`.`holder` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Application Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`holder` ALTER COLUMN `royalty_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Royalty Agreement Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` SET TAGS ('dbx_subdomain' = 'tenement_registry');
...
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `boundary_id` SET TAGS ('dbx_business_glossary_term' = 'Boundary Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `heritage_clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Heritage Clearance Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `native_title_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Native Title Agreement Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `orebody_id` SET TAGS ('dbx_business_glossary_term' = 'Orebody Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `rehabilitation_provision_id` SET TAGS ('dbx_business_glossary_term' = 'Rehabilitation Provision Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `renewal_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Renewal Obligation Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `tenement_id` SET TAGS ('dbx_business_glossary_term' = 'Tenement ID');
ALTER TABLE `mining_ecm`.`tenement`.`relinquishment` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
