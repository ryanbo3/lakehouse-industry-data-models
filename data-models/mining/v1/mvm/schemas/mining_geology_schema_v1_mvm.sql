-- Schema for Domain: geology | Business: Mining | Version: v1_mvm
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`orebody` (
    `orebody_id` BIGINT COMMENT 'Primary key for orebody',
    `area_id` BIGINT COMMENT 'Unique identifier for the orebody or mineralised zone within the mining tenement portfolio. Primary key.',
    ...
    `grade_parameter_id` BIGINT COMMENT 'Foreign key linking to product.grade_parameter. Business justification: Orebody average_grade and cutoff_grade are defined for a specific grade parameter (e.g., Fe% for iron ore, Cu% for copper). Reserve classification, economic cut-off determination, and JORC reporting a',
    `mine_site_id` BIGINT COMMENT 'Reference to the mine site or operational location where this orebody is being or will be extracted.',
    ...
    `rehabilitation_provision_id` BIGINT COMMENT 'Foreign key linking to finance.rehabilitation_provision. Business justification: Rehabilitation provisions are calculated per orebody/mining area based on disturbance footprint and closure liability. IFRS/AASB requires rehabilitation provisions tied to specific assets being mined.',
    `royalty_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.royalty_agreement. Business justification: Royalty agreements in mining are frequently negotiated at the orebody level — particularly native title royalties and State Agreement royalties tied to a specific mineral deposit. Finance and legal te',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`geological_unit` (
    `geological_unit_id` BIGINT COMMENT 'Primary key for geological_unit',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`production_drillhole` (
    ...
    `bench_id` BIGINT COMMENT 'Foreign key linking to mine.bench. Business justification: Production drillholes (blast holes) are drilled on specific benches. Blast hole drilling is organized and reported by bench. Drill and blast planning, drill metre tracking, and bench completion report',
    `blast_execution_id` BIGINT COMMENT 'Foreign key linking to mine.blast_execution. Business justification: Production drillholes are loaded and fired in a specific blast execution. Misfire management, blast performance analysis, and explosive consumption reporting require traceability from individual drill',
    `blast_pattern_id` BIGINT COMMENT 'Foreign key linking to mine.blast_pattern. Business justification: Production drillholes are drilled according to a specific blast pattern design. Blast pattern compliance reporting (actual vs designed hole positions, depths, diameters) requires the link between indi',
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_vendor. Business justification: Drilling contractor invoice reconciliation, HSE incident attribution, and vendor performance tracking all require linking each drillhole to the contracted procurement vendor. The existing drilling_co',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_purchase_order. Business justification: Drilling programs are executed under purchase orders issued to drilling contractors. Linking each drillhole to its governing PO enables cost-per-metre reconciliation, invoice verification against actu',
    `mine_site_id` BIGINT COMMENT 'Reference to the mine site where the drillhole is located.',
    `mining_block_id` BIGINT COMMENT 'Foreign key linking to mine.mining_block. Business justification: Production drillholes are drilled within specific mining blocks for grade control and blast design. Drill hole density reporting, grade control sample coverage, and mining block completion tracking al',
    ...
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Drilling program costs are reported by profit centre for P&L segment reporting and AISC cost disclosure. Mining finance teams require drill costs attributed to profit centres for commodity-level finan',
    `sample_program_id` BIGINT COMMENT 'Foreign key linking to laboratory.sample_program. Business justification: Production drillholes are drilled under a grade control sample program that governs QAQC protocols, analytical suites, and sampling intervals. A grade control geologist expects each production drillho',
    `tenement_id` BIGINT COMMENT 'Foreign key linking to tenement.tenement. Business justification: Drilling permits are issued per tenement in all mining jurisdictions. Regulatory reporting of drilling activity (metres drilled, expenditure) must be attributed to the specific tenement under which th',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Production drilling is a capital or exploration project activity tracked on WBS elements in ERP systems for CAPEX/OPEX classification and project cost control. Standard SAP-based mining finance practi',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`production_drillhole_survey` (
    ...
    `asset_id` BIGINT COMMENT 'Foreign key linking to equipment.asset. Business justification: Downhole survey instruments (gyroscopes, EM tools) are registered EAM assets requiring calibration tracking and maintenance history. JORC compliance and survey accuracy reporting require traceability ',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`lithology_log` (
    ...
    `laboratory_sample_id` BIGINT COMMENT 'Foreign key linking to laboratory.laboratory_sample. Business justification: Lithology log intervals are physically sampled for assay during resource and grade control drilling. Tracing a logged interval to its laboratory sample is required for JORC Table 1 data verification a',
    `production_drillhole_id` BIGINT COMMENT 'Foreign key linking to geology.production_drillhole. Business justification: Lithology log intervals are captured from drillhole core and belong to a specific production drillhole in the geology domain. The existing drill_hole_id links to the cross-domain exploration.drill_hol',
    `sample_program_id` BIGINT COMMENT 'Foreign key linking to laboratory.sample_program. Business justification: Lithology logs are recorded during drilling campaigns governed by a sample program that defines logging protocols, sampling requirements, and QAQC standards. A resource geologist expects to trace lith',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`structural_measurement` (
    ...
    `production_drillhole_id` BIGINT COMMENT 'Foreign key linking to geology.production_drillhole. Business justification: Structural measurements are captured from drillhole core (alpha/beta angles, dip, dip direction) and are directly associated with a specific production drillhole interval. The existing drill_hole_id l',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`geotechnical_log` (
    ...
    `laboratory_sample_id` BIGINT COMMENT 'Foreign key linking to laboratory.laboratory_sample. Business justification: Geotechnical log entries (UCS, point load, density) are measured on physical core samples submitted to a laboratory. The geotechnical_log already links to laboratory_id but not the specific sample. A ',
    `production_drillhole_id` BIGINT COMMENT 'Foreign key linking to geology.production_drillhole. Business justification: Geotechnical logs (RQD, UCS, RMR, fracture frequency, density) are captured from diamond drillhole core and belong to a specific production drillhole. The existing drill_hole_id links to the cross-dom',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`block_model` (
    ...
    `commodity_id` BIGINT COMMENT 'Foreign key linking to product.commodity. Business justification: Block models are built for a specific commodity (iron ore, copper, coal). Reserve/resource reporting, mine scheduling, and economic evaluation all depend on knowing which commodity a block model cover',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`block_model_cell` (
    ...
    `mining_block_id` BIGINT COMMENT 'Foreign key linking to mine.mining_block. Business justification: Block model cells are aggregated into operational mining blocks for extraction scheduling. Grade control and reconciliation require knowing which mining block each block model cell belongs to. This li',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`grade_control_sample` (
    ...
    `analytical_method_id` BIGINT COMMENT 'Foreign key linking to laboratory.analytical_method. Business justification: Grade control samples are assayed using a specific analytical method (e.g., XRF for Fe, fire assay for Au) that directly drives ore/waste classification and mine reconciliation. The existing plain-tex',
    `bench_id` BIGINT COMMENT 'Foreign key linking to mine.bench. Business justification: Grade control samples are organized by bench for blast hole sampling and ore classification. Bench-level grade control reporting requires this link. bench on grade_control_sample is a denormalized tex',
    ...
    `rom_stockpile_id` BIGINT COMMENT 'Foreign key linking to mine.rom_stockpile. Business justification: Grade control samples determine material destination (which ROM stockpile receives the ore). This is the core ore routing decision in grade control. material_destination on grade_control_sample is a d',
    ...
    `geological_domain_id` BIGINT COMMENT 'Foreign key linking to geology.geological_domain. Business justification: Grade control samples are collected and classified within geological domains for grade estimation and resource reconciliation. Grade control geologists assign each sample to a geological domain to app',
    `geological_unit_id` BIGINT COMMENT 'Foreign key linking to geology.geological_unit. Business justification: Grade control samples are logged against geological units (rock types/lithological formations) as part of standard grade control practice. Each sample interval is assigned a geological unit to enable ',
    ...
    `mining_block_id` BIGINT COMMENT 'Foreign key linking to mine.mining_block. Business justification: Grade control samples are collected within specific mining blocks to classify ore/waste and determine material routing. This is the core grade control process in open pit and underground mining. minin',
    ...
    `pit_or_level_id` BIGINT COMMENT 'Foreign key linking to mine.pit_or_level. Business justification: Grade control samples are collected within specific pits or underground levels. Pit-level grade control reporting and ore classification require this link. pit_name on grade_control_sample is a denorm',
    ...
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Grade control costs are reported by profit centre for AISC and C1 cash cost reporting by commodity/segment. Mining finance requires grade control expenditure attributed to profit centres for investor-',
    ...
    `sample_program_id` BIGINT COMMENT 'Foreign key linking to laboratory.sample_program. Business justification: Grade control samples are collected under a sample program defining QAQC insertion rates, analytical suites, and chain-of-custody requirements. A grade control geologist expects each sample to be trac',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`geological_domain` (
    `geological_domain_id` BIGINT COMMENT 'Primary key for geological_domain',
    `bench_id` BIGINT COMMENT 'Unique identifier for the geological domain. Primary key.',
    `commodity_id` BIGINT COMMENT 'Foreign key linking to product.commodity. Business justification: Geological domains are defined per commodity (e.g., iron ore estimation domain vs copper domain). Resource estimation, variogram modelling, and grade interpolation are all commodity-specific processes',
    ...
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Geological domains define resource estimation zones reported by profit centre/business segment in JORC resource statements and annual reports. Resource reporting by segment requires geological domains',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`wireframe` (
    `wireframe_id` BIGINT COMMENT 'Primary key for wireframe',
    `area_id` BIGINT COMMENT 'Foreign key linking to mine.mine_area. Business justification: Mine areas are spatially delineated based on geological wireframe boundaries defining the ore envelope. Mine area planning and resource classification require the link between the operational mine are',
    `commodity_id` BIGINT COMMENT 'Foreign key linking to product.commodity. Business justification: Wireframes model 3D mineralisation boundaries for specific commodities (iron ore wireframe, copper wireframe). Resource estimation, JORC/NI 43-101 reporting, and mine planning all require wireframes t',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Wireframe modelling is a billable geological activity (geologist time, software licences) tracked against cost centres for resource definition project cost control. Mining companies track geological m',
    ...
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Wireframe modelling is a project deliverable within resource definition WBS projects. Mining ERP systems charge geological modelling work to WBS elements for CAPEX/exploration expenditure tracking und',
...
CREATE OR REPLACE TABLE `mining_ecm`.`geology`.`resource_statement` (
    `resource_statement_id` BIGINT COMMENT 'Primary key for resource_statement',
    ...
    `grade_parameter_id` BIGINT COMMENT 'Foreign key linking to product.grade_parameter. Business justification: Resource statements report a grade value (grade_value) for a specific grade parameter (Fe%, Cu%, Li ppm). JORC/NI 43-101 regulatory reporting requires explicit identification of the reported grade par',
    `analytical_method_id` BIGINT COMMENT 'Foreign key linking to laboratory.analytical_method. Business justification: JORC Code and NI43-101 mandate that resource statements disclose the primary analytical method used for grade determination. A Competent Person must reference the specific analytical method in the res',
    `primary_previous_statement_resource_statement_id` BIGINT COMMENT 'Reference to the previous version of the resource statement that this statement supersedes, enabling version tracking and reconciliation.',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Resource statements are published by profit centre/business segment for JORC compliance, ASX/SEC investor reporting, and financial segment disclosure. Auditors and regulators require resource statemen',
    `project_valuation_id` BIGINT COMMENT 'Foreign key linking to finance.project_valuation. Business justification: Resource statements are the primary input to project valuations — NPV/IRR models consume resource tonnage, grade, and classification directly from resource statements. Feasibility studies and investme',
    ...
    `royalty_agreement_id` BIGINT COMMENT 'Foreign key linking to tenement.royalty_agreement. Business justification: Royalty agreements in mining are negotiated and calculated against specific resource/reserve statements — the grade, tonnage, and commodity price assumptions in a resource_statement directly underpin ',
    `tenement_id` BIGINT COMMENT 'Foreign key linking to tenement.tenement. Business justification: JORC/NI43-101 resource statements are lodged with regulatory authorities against specific tenement identifiers. Regulatory compliance teams must directly associate each resource statement with the ten',
...
ALTER TABLE `mining_ecm`.`geology`.`lithology_log` ADD CONSTRAINT `fk_geology_lithology_log_production_drillhole_id` FOREIGN KEY (`production_drillhole_id`) REFERENCES `mining_ecm`.`geology`.`production_drillhole`(`production_drillhole_id`);
ALTER TABLE `mining_ecm`.`geology`.`structural_measurement` ADD CONSTRAINT `fk_geology_structural_measurement_production_drillhole_id` FOREIGN KEY (`production_drillhole_id`) REFERENCES `mining_ecm`.`geology`.`production_drillhole`(`production_drillhole_id`);
...
ALTER TABLE `mining_ecm`.`geology`.`geotechnical_log` ADD CONSTRAINT `fk_geology_geotechnical_log_production_drillhole_id` FOREIGN KEY (`production_drillhole_id`) REFERENCES `mining_ecm`.`geology`.`production_drillhole`(`production_drillhole_id`);
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ADD CONSTRAINT `fk_geology_grade_control_sample_geological_domain_id` FOREIGN KEY (`geological_domain_id`) REFERENCES `mining_ecm`.`geology`.`geological_domain`(`geological_domain_id`);
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ADD CONSTRAINT `fk_geology_grade_control_sample_geological_unit_id` FOREIGN KEY (`geological_unit_id`) REFERENCES `mining_ecm`.`geology`.`geological_unit`(`geological_unit_id`);
...
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ADD CONSTRAINT `fk_geology_resource_statement_primary_previous_statement_resource_statement_id` FOREIGN KEY (`primary_previous_statement_resource_statement_id`) REFERENCES `mining_ecm`.`geology`.`resource_statement`(`resource_statement_id`);
...
ALTER TABLE `mining_ecm`.`geology`.`orebody` SET TAGS ('dbx_subdomain' = 'geological_characterisation');
ALTER TABLE `mining_ecm`.`geology`.`orebody` ALTER COLUMN `orebody_id` SET TAGS ('dbx_business_glossary_term' = 'Orebody Identifier');
ALTER TABLE `mining_ecm`.`geology`.`orebody` ALTER COLUMN `area_id` SET TAGS ('dbx_business_glossary_term' = 'Orebody Identifier');
...
ALTER TABLE `mining_ecm`.`geology`.`orebody` ALTER COLUMN `grade_parameter_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Parameter Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`orebody` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Site Identifier');
...
ALTER TABLE `mining_ecm`.`geology`.`orebody` ALTER COLUMN `rehabilitation_provision_id` SET TAGS ('dbx_business_glossary_term' = 'Rehabilitation Provision Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`orebody` ALTER COLUMN `royalty_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Royalty Agreement Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`geological_unit` SET TAGS ('dbx_subdomain' = 'geological_characterisation');
ALTER TABLE `mining_ecm`.`geology`.`geological_unit` ALTER COLUMN `geological_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Geological Unit Identifier');
...
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` SET TAGS ('dbx_subdomain' = 'drillhole_logging');
...
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `bench_id` SET TAGS ('dbx_business_glossary_term' = 'Bench Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `blast_execution_id` SET TAGS ('dbx_business_glossary_term' = 'Blast Execution Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `blast_pattern_id` SET TAGS ('dbx_business_glossary_term' = 'Blast Pattern Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Drilling Contractor Procurement Vendor Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Drilling Procurement Purchase Order Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `mine_site_id` SET TAGS ('dbx_business_glossary_term' = 'Mine Site Identifier (ID)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `mining_block_id` SET TAGS ('dbx_business_glossary_term' = 'Mining Block Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `sample_program_id` SET TAGS ('dbx_business_glossary_term' = 'Sample Program Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `tenement_id` SET TAGS ('dbx_business_glossary_term' = 'Tenement Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole_survey` SET TAGS ('dbx_subdomain' = 'drillhole_logging');
...
ALTER TABLE `mining_ecm`.`geology`.`production_drillhole_survey` ALTER COLUMN `asset_id` SET TAGS ('dbx_business_glossary_term' = 'Survey Instrument Asset Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`lithology_log` SET TAGS ('dbx_subdomain' = 'drillhole_logging');
...
ALTER TABLE `mining_ecm`.`geology`.`lithology_log` ALTER COLUMN `laboratory_sample_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Sample Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`lithology_log` ALTER COLUMN `production_drillhole_id` SET TAGS ('dbx_business_glossary_term' = 'Production Drillhole Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`lithology_log` ALTER COLUMN `sample_program_id` SET TAGS ('dbx_business_glossary_term' = 'Sample Program Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`structural_measurement` SET TAGS ('dbx_subdomain' = 'drillhole_logging');
...
ALTER TABLE `mining_ecm`.`geology`.`structural_measurement` ALTER COLUMN `production_drillhole_id` SET TAGS ('dbx_business_glossary_term' = 'Production Drillhole Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`geotechnical_log` SET TAGS ('dbx_subdomain' = 'drillhole_logging');
...
ALTER TABLE `mining_ecm`.`geology`.`geotechnical_log` ALTER COLUMN `laboratory_sample_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Sample Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`geotechnical_log` ALTER COLUMN `production_drillhole_id` SET TAGS ('dbx_business_glossary_term' = 'Production Drillhole Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`block_model` SET TAGS ('dbx_subdomain' = 'resource_estimation');
...
ALTER TABLE `mining_ecm`.`geology`.`block_model` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`block_model_cell` SET TAGS ('dbx_subdomain' = 'resource_estimation');
...
ALTER TABLE `mining_ecm`.`geology`.`block_model_cell` ALTER COLUMN `mining_block_id` SET TAGS ('dbx_business_glossary_term' = 'Mining Block Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` SET TAGS ('dbx_subdomain' = 'resource_estimation');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `analytical_method_id` SET TAGS ('dbx_business_glossary_term' = 'Analytical Method Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `bench_id` SET TAGS ('dbx_business_glossary_term' = 'Bench Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `rom_stockpile_id` SET TAGS ('dbx_business_glossary_term' = 'Destination Rom Stockpile Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `geological_domain_id` SET TAGS ('dbx_business_glossary_term' = 'Geological Domain Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `geological_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Geological Unit Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `mining_block_id` SET TAGS ('dbx_business_glossary_term' = 'Mining Block Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `pit_or_level_id` SET TAGS ('dbx_business_glossary_term' = 'Pit Or Level Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`grade_control_sample` ALTER COLUMN `sample_program_id` SET TAGS ('dbx_business_glossary_term' = 'Sample Program Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`geological_domain` SET TAGS ('dbx_subdomain' = 'geological_characterisation');
ALTER TABLE `mining_ecm`.`geology`.`geological_domain` ALTER COLUMN `geological_domain_id` SET TAGS ('dbx_business_glossary_term' = 'Geological Domain Identifier');
ALTER TABLE `mining_ecm`.`geology`.`geological_domain` ALTER COLUMN `bench_id` SET TAGS ('dbx_business_glossary_term' = 'Geological Domain Identifier (ID)');
ALTER TABLE `mining_ecm`.`geology`.`geological_domain` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`geological_domain` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`wireframe` SET TAGS ('dbx_subdomain' = 'geological_characterisation');
ALTER TABLE `mining_ecm`.`geology`.`wireframe` ALTER COLUMN `wireframe_id` SET TAGS ('dbx_business_glossary_term' = 'Wireframe Identifier');
ALTER TABLE `mining_ecm`.`geology`.`wireframe` ALTER COLUMN `area_id` SET TAGS ('dbx_business_glossary_term' = 'Wireframe Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`wireframe` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`wireframe` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`wireframe` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` SET TAGS ('dbx_subdomain' = 'resource_estimation');
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `resource_statement_id` SET TAGS ('dbx_business_glossary_term' = 'Resource Statement Identifier');
...
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `grade_parameter_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Parameter Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `analytical_method_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Analytical Method Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `primary_previous_statement_resource_statement_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Statement ID');
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `project_valuation_id` SET TAGS ('dbx_business_glossary_term' = 'Project Valuation Id (Foreign Key)');
...
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `royalty_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Royalty Agreement Id (Foreign Key)');
ALTER TABLE `mining_ecm`.`geology`.`resource_statement` ALTER COLUMN `tenement_id` SET TAGS ('dbx_business_glossary_term' = 'Tenement Id (Foreign Key)');
