-- Schema for Domain: service | Business: Ecommerce | Version: v1_mvm
...
-- ========= TABLES =========
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`support_case` (
    `support_case_id` BIGINT COMMENT 'Unique identifier for the customer support case or service ticket. Primary key for the service support case entity.',
    ...
    `accounts_receivable_id` BIGINT COMMENT 'Foreign key linking to finance.accounts_receivable. Business justification: Billing dispute cases require direct reference to the AR record for resolution, dunning holds, and dispute status updates. E-commerce customer service teams routinely open cases against specific invoi',
    ...
    `team_id` BIGINT COMMENT 'Foreign key linking to service.team. Business justification: service_support_case currently stores assigned_queue as a denormalized STRING representing the team or queue a case is routed to. Adding assigned_team_id → service.team.team_id normalizes this relatio',
    `bundle_id` BIGINT COMMENT 'Foreign key linking to product.bundle. Business justification: Support cases are raised for bundle-specific issues (missing component, bundle pricing dispute, partial return eligibility). Agents need to identify the bundle to apply correct return policy and calcu',
    ...
    `catalog_item_id` BIGINT COMMENT 'Foreign key linking to product.catalog_item. Business justification: Product quality reporting, CPSC compliance tracking, and recall management require grouping all support cases by catalog item across all SKUs. Agents investigating product-level defects need this link',
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to marketing.channel. Business justification: Omnichannel support routing and channel-level SLA assignment require linking each case to its originating marketing channel. E-commerce operations report CSAT and resolution time by channel; `originat',
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: In B2B e-commerce, support cases are raised by a specific contact (buyer persona within an account). Contact-level case tracking drives SLA reporting per contact, escalation routing, and communication',
    ...
    `coupon_redemption_id` BIGINT COMMENT 'Foreign key linking to pricing.coupon_redemption. Business justification: Coupon failure and misapplication are among the top support case drivers in e-commerce. Linking the case directly to the coupon_redemption record allows agents to inspect redemption status, fraud flag',
    `customer_profile_id` BIGINT COMMENT 'Foreign key linking to customer.customer_profile. Business justification: GDPR subject access requests and B2C NPS attribution require direct case-to-profile linkage. Service platforms universally link cases to the individual customer profile for personalization, data subje',
    `fulfillment_order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: Support cases for missing items, wrong items, or late delivery are opened against a specific fulfillment order. Agents need direct access to fulfillment order status, method, and SLA tier during case ',
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_goods_receipt. Business justification: Goods receipt discrepancy case management: when buyers report receiving wrong or damaged goods, the service case must trace to the GR record for three-way match investigation and discrepancy resolutio',
    ...
    `line_id` BIGINT COMMENT 'Foreign key linking to order.order_line. Business justification: Support cases for wrong/damaged/missing items require line-level order linkage for accurate case routing, seller accountability reporting, and SLA tracking. The existing header_id identifies the order',
    `logistics_shipment_id` BIGINT COMMENT 'Foreign key linking to logistics.logistics_shipment. Business justification: Customer service agents handling delivery complaints, lost packages, or SLA breach disputes need direct access to the logistics_shipment record for tracking, proof of delivery, and declared value — a ',
    `loyalty_enrollment_id` BIGINT COMMENT 'Foreign key linking to customer.loyalty_enrollment. Business justification: customer_entitlement_tier on service_support_case requires real-time loyalty tier validation for SLA routing and VIP case handling. Direct FK to loyalty_enrollment enables tier-based case prioritizati',
    `marketplace_promotion_id` BIGINT COMMENT 'Foreign key linking to marketplace.marketplace_promotion. Business justification: Support cases frequently arise from marketplace promotion disputes (promotional price not honored, eligibility errors). Direct FK to marketplace_promotion enables promotion-specific case routing, FTC ',
    ...
    `price_override_id` BIGINT COMMENT 'Foreign key linking to pricing.price_override. Business justification: Pricing dispute cases — where customers question why they were charged a non-standard price — require agents to directly inspect the price_override record. This is one of the most common case types in',
    `product_listing_id` BIGINT COMMENT 'Foreign key linking to product.product_listing. Business justification: Cases originating from listing-level issues (incorrect price displayed, suppressed listing, wrong description) require direct listing linkage for listing quality reporting, suppression root-cause anal',
    ...
    `proof_of_delivery_id` BIGINT COMMENT 'Foreign key linking to fulfillment.proof_of_delivery. Business justification: Delivery dispute resolution is a named e-commerce support process: when a customer claims non-delivery, the agent must access the POD record (photo, GPS, signature) directly from the case. No existing',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to procurement.purchase_order. Business justification: PO dispute case management: buyers raise service cases referencing a specific purchase order for unfulfilled, incorrect, or billing-disputed POs. This link enables procurement dispute resolution workf',
    ...
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to service.sla_policy. Business justification: service_support_case currently stores sla_policy_code as a denormalized STRING reference to the governing SLA policy. Adding a proper FK sla_policy_id → service.sla_policy.sla_policy_id normalizes thi',
    `supplier_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier. Business justification: Supplier dispute/complaint case management: procurement teams raise service cases against suppliers for SLA breaches, defective goods, or compliance violations. Linking service_support_case to supplie',
    `supplier_invoice_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_invoice. Business justification: Invoice dispute case management: procurement teams raise service cases for supplier invoice discrepancies (overbilling, incorrect amounts, payment disputes). Linking service_support_case to supplier_i',
    `vendor_contract_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor_contract. Business justification: Contract dispute/SLA breach case management: service cases raised for vendor contract violations (missed SLAs, exclusivity breaches, penalty triggers) must reference the governing vendor contract. E-c',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`case_interaction` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to marketing.channel. Business justification: Channel-level interaction analytics (cost-per-interaction, handle time by channel) are standard e-commerce service KPIs. Each interaction occurs on a specific channel; `channel` plain string is a deno',
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: Case interactions (calls, chats, emails) are conducted with a specific contact person in B2B e-commerce. Linking interaction to contact enables contact-level service history, communication preference ',
    ...
    `support_case_id` BIGINT COMMENT 'Reference to the parent support case that this interaction belongs to. Links to the service case entity.',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`rma` (
    ...
    `accounts_receivable_id` BIGINT COMMENT 'Foreign key linking to finance.accounts_receivable. Business justification: RMA approval triggers an AR credit memo against the original invoice. E-commerce returns processing requires linking the RMA to the AR record to apply credits, update aging buckets, and reconcile open',
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: In B2B e-commerce, returns are authorized and coordinated by a specific contact (procurement or operations contact). Linking RMA to contact enables return authorization tracking, contact-level return ',
    ...
    `delivery_confirmation_id` BIGINT COMMENT 'Foreign key linking to order.delivery_confirmation. Business justification: RMA return window eligibility is calculated from confirmed delivery date; linking RMA to delivery_confirmation enables automated return eligibility validation, proof-of-delivery dispute resolution, an',
    ...
    `fulfillment_shipment_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_shipment. Business justification: Returns management requires validating the original shipments actual delivery date to confirm return window eligibility and generate return labels. E-commerce ops teams link every RMA to its originat',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: RMA processing generates journal entries for inventory write-backs, restocking fees, and refund liabilities. Linking rma to journal_entry enables financial impact tracking of returns, supports invento',
    `marketplace_transaction_id` BIGINT COMMENT 'Foreign key linking to marketplace.marketplace_transaction. Business justification: RMAs are initiated against marketplace transactions in e-commerce. Seller settlement deduction, commission reversal, and dispute resolution all require tracing which marketplace transaction generated ',
    ...
    `promotional_campaign_id` BIGINT COMMENT 'Foreign key linking to pricing.promotional_campaign. Business justification: Items purchased under a promotional campaign often have special return policies (e.g., final sale, reduced return window, non-refundable). Linking RMA to promotional_campaign enables return eligibilit',
    `customer_address_id` BIGINT COMMENT 'Foreign key linking to customer.customer_address. Business justification: Return label generation and carrier pickup scheduling require the customers return-from address. The rma table has no address FK currently. Role-prefixed return_from_ distinguishes pickup address fro',
    `seller_profile_id` BIGINT COMMENT 'Foreign key linking to seller.seller_profile. Business justification: Marketplace RMAs for seller-fulfilled orders must reference the responsible seller for return routing, seller-funded refund chargebacks, and violation tracking. Returns management and seller performan',
    `tax_record_id` BIGINT COMMENT 'Foreign key linking to finance.tax_record. Business justification: Product returns require tax reversals on the original transaction. E-commerce tax compliance (marketplace facilitator laws) requires RMAs to reference the originating tax record to process accurate ta',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`rma_line` (
    ...
    `bundle_id` BIGINT COMMENT 'Foreign key linking to product.bundle. Business justification: Bundle returns require component-level disposition rules and restocking eligibility checks defined on the bundle. Existing rma_line FKs cover catalog_item and sku but not bundle, leaving bundle return',
    ...
    `line_id` BIGINT COMMENT 'Foreign key linking to order.order_line. Business justification: Return eligibility validation, return_window_days enforcement, and unit_price reconciliation require tracing each RMA line back to the specific order line. E-commerce return processing and seller char',
    `order_line_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order_line. Business justification: Each RMA line must trace back to the original fulfillment order line for refund amount validation, inventory restocking disposition, and return fraud detection. E-commerce returns processing always re',
    `price_list_item_id` BIGINT COMMENT 'Foreign key linking to pricing.price_list_item. Business justification: RMA line processing requires the original price_list_item to validate unit_price for refund computation, determine restocking fee eligibility based on pricing tier, and confirm return window policy. R',
    ...
    `stock_position_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_position. Business justification: After RMA inspection, disposition (restock, quarantine, scrap) maps to a specific stock_position. rma_line has restocking_eligible_flag and disposition_code but no FK to stock_position. Returns proces',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`service_refund` (
    ...
    `accounts_receivable_id` BIGINT COMMENT 'Foreign key linking to finance.accounts_receivable. Business justification: Refund issuance directly reduces or offsets an AR balance. E-commerce financial reconciliation requires linking each service refund to the originating AR record to apply credit memos, update open amou',
    `coupon_redemption_id` BIGINT COMMENT 'Foreign key linking to pricing.coupon_redemption. Business justification: When a refund reverses a coupon discount, the service agent must reference the specific coupon_redemption record to determine the discount amount to reverse and update redemption status. Coupon refund',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Every refund disbursement generates a journal entry (debit revenue/liability, credit cash). Linking service_refund to journal_entry enables financial audit trail, SOX compliance, and period-close reco',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: In multi-entity e-commerce platforms, refunds must be processed and reported under the correct legal entity for regulatory compliance, VAT/GST reporting, and intercompany settlement. Domain experts ex',
    `line_id` BIGINT COMMENT 'Foreign key linking to order.order_line. Business justification: Partial refunds on specific line items require direct line-level linkage for financial reconciliation, tax recalculation, and seller payout adjustments. Traversing via rma_id is insufficient for goodw',
    `logistics_shipment_id` BIGINT COMMENT 'Foreign key linking to logistics.logistics_shipment. Business justification: Refunds issued for shipping failures (lost, damaged, late delivery) must reference the logistics_shipment to support carrier chargeback processing and cost-recovery workflows. service_refund has heade',
    `loyalty_enrollment_id` BIGINT COMMENT 'Foreign key linking to customer.loyalty_enrollment. Business justification: service_refund.loyalty_points_granted requires knowing which loyalty enrollment to credit. Direct FK to loyalty_enrollment enables automated point posting for refund compensation — a standard loyalty ',
    ...
    `price_list_item_id` BIGINT COMMENT 'Foreign key linking to pricing.price_list_item. Business justification: Refund calculation validation requires referencing the exact price_list_item applied to the original sale to confirm the unit price, tier pricing, and discount percent used. E-commerce refund audits a',
    `price_override_id` BIGINT COMMENT 'Foreign key linking to pricing.price_override. Business justification: Refunds on orders with price overrides must reference the override record to validate the overridden price used and ensure the refund amount matches the actual amount charged. Price override audit tra',
    ...
    `seller_profile_id` BIGINT COMMENT 'Foreign key linking to seller.seller_profile. Business justification: Marketplace refunds are often charged back to the responsible seller (seller-funded refunds). Finance and seller operations teams require seller attribution on refund records for chargeback processing',
    `seller_settlement_id` BIGINT COMMENT 'Foreign key linking to marketplace.seller_settlement. Business justification: Refunds issued on marketplace transactions must be deducted from the sellers settlement disbursement. Finance and marketplace ops require this link for seller payout reconciliation, net disbursement ',
    `settlement_id` BIGINT COMMENT 'Foreign key linking to payment.settlement. Business justification: Finance teams require direct linkage between service-initiated refunds and payment settlement batches for period-end reconciliation and GL posting. settlement_reference_number is a plain-text denormal',
    `support_case_id` BIGINT COMMENT 'Reference to the customer service case that authorized this refund or compensation. Links to the support ticket or inquiry.',
    `tax_record_id` BIGINT COMMENT 'Foreign key linking to finance.tax_record. Business justification: Refunds require tax reversals or credit adjustments against the original tax record. E-commerce marketplace facilitator tax compliance mandates linking refund transactions to their originating tax rec',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`team` (
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Service teams are P&L-accountable units in e-commerce operations. Linking team to profit_center enables revenue and cost attribution per service team for management reporting, budgeting, and profitabi',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`sla_policy` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to marketing.channel. Business justification: SLA policies in e-commerce are channel-specific (social media: 1-hour response; email: 24-hour). Linking sla_policy to marketing.channel enables proper policy lookup by channel and channel-level SLA c',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: SLA policies carry breach_penalty_amount and breach_penalty_currency. Linking to cost_center enables allocation of SLA breach penalty costs to the responsible cost center, supporting operational cost ',
    `segment_id` BIGINT COMMENT 'Foreign key linking to customer.segment. Business justification: sla_policy.customer_segment is a denormalized text field. SLA policies are defined per customer segment (VIP, B2B, standard). Proper FK to segment enables automated SLA assignment during case creation',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`escalation` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to marketing.channel. Business justification: Channel-level escalation rate reporting is a core e-commerce service metric; escalations may occur on a different channel than the originating case. `channel` plain string is a denormalization of mark',
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: Escalations in B2B e-commerce are initiated by or directed to a specific contact. Contact-level escalation history drives SLA accountability reporting, executive escalation routing, and contact satisf',
    `dispute_id` BIGINT COMMENT 'Foreign key linking to marketplace.dispute. Business justification: Marketplace disputes that exceed standard resolution thresholds trigger service escalations. Escalation management and SLA compliance reporting require direct traceability from the escalation record t',
    `agent_id` BIGINT COMMENT 'Reference to the support agent who initiated or triggered the escalation.',
    `fulfillment_shipment_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_shipment. Business justification: Escalations triggered by shipment failures (SLA breach, lost package, repeated delivery failures) require direct reference to the shipment for carrier chargeback filing and root-cause analysis. Escala',
    `header_id` BIGINT COMMENT 'Foreign key linking to order.header. Business justification: Escalations triggered by order fulfillment failures (late delivery, wrong item, fraud) require direct order linkage for SLA breach root-cause analysis, seller performance scorecards, and customer comp',
    ...
    `segment_id` BIGINT COMMENT 'Foreign key linking to customer.segment. Business justification: escalation.customer_segment is a denormalized text field. Proper FK to segment enables segment-level escalation rate reporting, SLA breach analysis by customer tier, and escalation routing rules by se',
    `seller_profile_id` BIGINT COMMENT 'Foreign key linking to seller.seller_profile. Business justification: Marketplace escalations are frequently triggered by seller behavior (late shipment, item not as described, policy violations). Seller performance management, scorecard updates, and violation workflows',
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to service.sla_policy. Business justification: escalation tracks sla_breach_flag and sla_target_resolution_minutes but has no FK to the governing SLA policy. Adding sla_policy_id → service.sla_policy.sla_policy_id directly links each escalation ev',
    `support_case_id` BIGINT COMMENT 'Reference to the originating support case that was escalated.',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`feedback_response` (
    ...
    `campaign_id` BIGINT COMMENT 'Foreign key linking to marketing.campaign. Business justification: Post-purchase NPS/CSAT surveys are frequently triggered by marketing campaigns in e-commerce. Linking feedback_response to the triggering campaign enables campaign-level satisfaction measurement, a cr',
    `catalog_item_id` BIGINT COMMENT 'Foreign key linking to product.catalog_item. Business justification: Post-purchase NPS and CSAT surveys are triggered by specific product purchases. Linking feedback to catalog_item enables product quality scoring dashboards, PDP improvement decisions, and brand-level ',
    `channel_id` BIGINT COMMENT 'Foreign key linking to marketing.channel. Business justification: Post-purchase surveys are delivered via specific channels; channel-level survey response rate and NPS analysis are standard e-commerce CX metrics. `delivery_channel` plain string is a denormalization ',
    ...
    `fulfillment_shipment_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_shipment. Business justification: Post-delivery CSAT/NPS surveys are triggered by shipment delivery events. Linking feedback_response to fulfillment_shipment enables delivery experience analytics — correlating satisfaction scores with',
    ...
    `loyalty_enrollment_id` BIGINT COMMENT 'Foreign key linking to customer.loyalty_enrollment. Business justification: feedback_response.incentive_offered_flag indicates loyalty point incentives for survey completion. Direct FK to loyalty_enrollment enables automated point crediting for survey participation — a standa',
    `marketplace_transaction_id` BIGINT COMMENT 'Foreign key linking to marketplace.marketplace_transaction. Business justification: Post-purchase feedback surveys are triggered by marketplace transactions. Seller performance dashboards, marketplace quality scoring, and FTC compliance reporting all require linking feedback response',
    ...
    `segment_id` BIGINT COMMENT 'Foreign key linking to customer.segment. Business justification: feedback_response.customer_segment is a denormalized text field. Proper FK to segment enables segment-level NPS/CSAT reporting, regulatory consent-aware survey targeting, and CX performance benchmarki',
    `support_case_id` BIGINT COMMENT 'Reference to the service case or support ticket that triggered this feedback survey. Links the feedback to the specific service interaction being evaluated.',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`service`.`knowledge_article` (
    ...
    `bundle_id` BIGINT COMMENT 'Foreign key linking to product.bundle. Business justification: Bundle-specific knowledge articles (assembly guides, component compatibility, bundle return procedures) are a distinct content type in e-commerce support. Direct FK enables agents to retrieve bundle a',
    `campaign_id` BIGINT COMMENT 'Foreign key linking to marketing.campaign. Business justification: Campaign-specific knowledge articles (promotional FAQs, offer terms, campaign issue resolutions) are standard in e-commerce support operations. Linking knowledge_article to campaign enables agents to ',
    `catalog_item_id` BIGINT COMMENT 'Foreign key linking to product.catalog_item. Business justification: Agents surface product-specific troubleshooting and setup articles when a case is opened for a catalog item. Direct FK enables article coverage gap analysis by product and powers automated article sug',
    ...
    `listing_category_id` BIGINT COMMENT 'Foreign key linking to marketplace.listing_category. Business justification: Knowledge articles are scoped to marketplace listing categories (e.g., hazmat return policies, age-restricted item handling, CPSC compliance guides). Support agents need category-specific articles sur',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ADD CONSTRAINT `fk_service_support_case_agent_id` FOREIGN KEY (`agent_id`) REFERENCES `ecommerce_ecm`.`service`.`agent`(`agent_id`);
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ADD CONSTRAINT `fk_service_support_case_team_id` FOREIGN KEY (`team_id`) REFERENCES `ecommerce_ecm`.`service`.`team`(`team_id`);
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ADD CONSTRAINT `fk_service_support_case_knowledge_article_id` FOREIGN KEY (`knowledge_article_id`) REFERENCES `ecommerce_ecm`.`service`.`knowledge_article`(`knowledge_article_id`);
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ADD CONSTRAINT `fk_service_support_case_parent_case_service_support_case_id` FOREIGN KEY (`parent_case_service_support_case_id`) REFERENCES `ecommerce_ecm`.`service`.`support_case`(`support_case_id`);
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ADD CONSTRAINT `fk_service_support_case_rma_id` FOREIGN KEY (`rma_id`) REFERENCES `ecommerce_ecm`.`service`.`rma`(`rma_id`);
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ADD CONSTRAINT `fk_service_support_case_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `ecommerce_ecm`.`service`.`sla_policy`(`sla_policy_id`);
...
ALTER TABLE `ecommerce_ecm`.`service`.`case_interaction` ADD CONSTRAINT `fk_service_case_interaction_support_case_id` FOREIGN KEY (`support_case_id`) REFERENCES `ecommerce_ecm`.`service`.`support_case`(`support_case_id`);
...
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ADD CONSTRAINT `fk_service_service_refund_support_case_id` FOREIGN KEY (`support_case_id`) REFERENCES `ecommerce_ecm`.`service`.`support_case`(`support_case_id`);
...
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ADD CONSTRAINT `fk_service_escalation_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `ecommerce_ecm`.`service`.`sla_policy`(`sla_policy_id`);
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ADD CONSTRAINT `fk_service_escalation_support_case_id` FOREIGN KEY (`support_case_id`) REFERENCES `ecommerce_ecm`.`service`.`support_case`(`support_case_id`);
...
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ADD CONSTRAINT `fk_service_feedback_response_support_case_id` FOREIGN KEY (`support_case_id`) REFERENCES `ecommerce_ecm`.`service`.`support_case`(`support_case_id`);
...
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` SET TAGS ('dbx_subdomain' = 'case_management');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `support_case_id` SET TAGS ('dbx_business_glossary_term' = 'Service Support Case ID');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `accounts_receivable_id` SET TAGS ('dbx_business_glossary_term' = 'Accounts Receivable Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `agent_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Agent ID');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `team_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Team Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Bundle Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `carrier_service_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Service Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `catalog_item_id` SET TAGS ('dbx_business_glossary_term' = 'Catalog Item Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `center_id` SET TAGS ('dbx_business_glossary_term' = 'Center Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `coupon_redemption_id` SET TAGS ('dbx_business_glossary_term' = 'Coupon Redemption Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `customer_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Profile Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `fulfillment_order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Goods Receipt Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `knowledge_article_id` SET TAGS ('dbx_business_glossary_term' = 'Knowledge Article ID');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `logistics_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Logistics Shipment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `loyalty_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Enrollment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `marketplace_promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Promotion Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `marketplace_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Transaction Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `parent_case_service_support_case_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Case ID');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `price_override_id` SET TAGS ('dbx_business_glossary_term' = 'Price Override Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `product_listing_id` SET TAGS ('dbx_business_glossary_term' = 'Product Listing Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `promotional_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotional Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `proof_of_delivery_id` SET TAGS ('dbx_business_glossary_term' = 'Proof Of Delivery Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `rma_id` SET TAGS ('dbx_business_glossary_term' = 'Return Merchandise Authorization (RMA) ID');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Product Stock Keeping Unit (SKU)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `supplier_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Invoice Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `vendor_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Contract Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `case_number` SET TAGS ('dbx_business_glossary_term' = 'Case Number');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `case_number` SET TAGS ('dbx_value_regex' = '^[A-Z]{2,4}-[0-9]{6,10}$');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `case_type` SET TAGS ('dbx_business_glossary_term' = 'Case Type');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `closed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Case Closed Timestamp');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `compensation_amount` SET TAGS ('dbx_business_glossary_term' = 'Compensation Amount');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `compensation_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `contact_attempts` SET TAGS ('dbx_business_glossary_term' = 'Contact Attempts Count');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Case Created Timestamp');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `csat_score` SET TAGS ('dbx_business_glossary_term' = 'Customer Satisfaction (CSAT) Score');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `customer_entitlement_tier` SET TAGS ('dbx_business_glossary_term' = 'Customer Entitlement Tier');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `customer_entitlement_tier` SET TAGS ('dbx_value_regex' = 'standard|premium|vip|enterprise');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `escalation_status` SET TAGS ('dbx_business_glossary_term' = 'Escalation Status');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `escalation_status` SET TAGS ('dbx_value_regex' = 'not_escalated|tier2|tier3|management|executive');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `feedback_comments` SET TAGS ('dbx_business_glossary_term' = 'Customer Feedback Comments');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `first_response_timestamp` SET TAGS ('dbx_business_glossary_term' = 'First Response Timestamp');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `interaction_count` SET TAGS ('dbx_business_glossary_term' = 'Interaction Count');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `is_vip_case` SET TAGS ('dbx_business_glossary_term' = 'VIP Case Flag');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Language Code');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `language_code` SET TAGS ('dbx_value_regex' = '^[a-z]{2}$');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `nps_score` SET TAGS ('dbx_business_glossary_term' = 'Net Promoter Score (NPS)');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `outcome_disposition` SET TAGS ('dbx_business_glossary_term' = 'Outcome Disposition');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `outcome_disposition` SET TAGS ('dbx_value_regex' = 'resolved|unresolved|duplicate|transferred|withdrawn');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'P1|P2|P3|P4');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `reopened_count` SET TAGS ('dbx_business_glossary_term' = 'Reopened Count');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `resolution_notes` SET TAGS ('dbx_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `resolution_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Resolution Timestamp');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `root_cause_category` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Category');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `sentiment_score` SET TAGS ('dbx_business_glossary_term' = 'Customer Sentiment Score');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `service_support_case_description` SET TAGS ('dbx_business_glossary_term' = 'Case Description');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `service_support_case_status` SET TAGS ('dbx_business_glossary_term' = 'Case Status');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `sla_breach_flag` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Breach Flag');
ALTER TABLE `ecommerce_ecm`.`service`.`support_case` ALTER COLUMN `subject` SET TAGS ('dbx_business_glossary_term' = 'Case Subject');
ALTER TABLE `ecommerce_ecm`.`service`.`case_interaction` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `ecommerce_ecm`.`service`.`case_interaction` SET TAGS ('dbx_subdomain' = 'case_management');
...
ALTER TABLE `ecommerce_ecm`.`service`.`case_interaction` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`case_interaction` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`case_interaction` ALTER COLUMN `support_case_id` SET TAGS ('dbx_business_glossary_term' = 'Case ID');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma` SET TAGS ('dbx_subdomain' = 'returns_remediation');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `accounts_receivable_id` SET TAGS ('dbx_business_glossary_term' = 'Accounts Receivable Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `delivery_confirmation_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Confirmation Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `fulfillment_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Shipment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `marketplace_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Transaction Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `promotional_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotional Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `customer_address_id` SET TAGS ('dbx_business_glossary_term' = 'Return From Customer Address Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `customer_address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `customer_address_id` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `seller_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Seller Profile Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma` ALTER COLUMN `tax_record_id` SET TAGS ('dbx_business_glossary_term' = 'Tax Record Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma_line` SET TAGS ('dbx_subdomain' = 'returns_remediation');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma_line` ALTER COLUMN `bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Bundle Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma_line` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma_line` ALTER COLUMN `order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Line Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`rma_line` ALTER COLUMN `price_list_item_id` SET TAGS ('dbx_business_glossary_term' = 'Price List Item Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`rma_line` ALTER COLUMN `stock_position_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Position Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` SET TAGS ('dbx_subdomain' = 'returns_remediation');
...
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `accounts_receivable_id` SET TAGS ('dbx_business_glossary_term' = 'Accounts Receivable Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `coupon_redemption_id` SET TAGS ('dbx_business_glossary_term' = 'Coupon Redemption Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Order Line Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `logistics_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Logistics Shipment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `loyalty_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Enrollment Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `price_list_item_id` SET TAGS ('dbx_business_glossary_term' = 'Price List Item Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `price_override_id` SET TAGS ('dbx_business_glossary_term' = 'Price Override Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `seller_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Seller Profile Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `seller_settlement_id` SET TAGS ('dbx_business_glossary_term' = 'Seller Settlement Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `settlement_id` SET TAGS ('dbx_business_glossary_term' = 'Settlement Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `support_case_id` SET TAGS ('dbx_business_glossary_term' = 'Service Case ID');
ALTER TABLE `ecommerce_ecm`.`service`.`service_refund` ALTER COLUMN `tax_record_id` SET TAGS ('dbx_business_glossary_term' = 'Tax Record Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`agent` SET TAGS ('dbx_subdomain' = 'agent_operations');
...
ALTER TABLE `ecommerce_ecm`.`service`.`team` SET TAGS ('dbx_subdomain' = 'agent_operations');
...
ALTER TABLE `ecommerce_ecm`.`service`.`team` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`sla_policy` SET TAGS ('dbx_subdomain' = 'case_management');
...
ALTER TABLE `ecommerce_ecm`.`service`.`sla_policy` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`sla_policy` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`sla_policy` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Segment Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` SET TAGS ('dbx_subdomain' = 'case_management');
...
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `dispute_id` SET TAGS ('dbx_business_glossary_term' = 'Dispute Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `fulfillment_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Shipment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Header Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Segment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `seller_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Seller Profile Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`escalation` ALTER COLUMN `support_case_id` SET TAGS ('dbx_business_glossary_term' = 'Case ID');
...
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` SET TAGS ('dbx_subdomain' = 'returns_remediation');
...
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `catalog_item_id` SET TAGS ('dbx_business_glossary_term' = 'Catalog Item Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `fulfillment_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Shipment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `loyalty_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Loyalty Enrollment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `marketplace_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Transaction Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Segment Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`feedback_response` ALTER COLUMN `support_case_id` SET TAGS ('dbx_business_glossary_term' = 'Service Case ID');
...
ALTER TABLE `ecommerce_ecm`.`service`.`knowledge_article` SET TAGS ('dbx_subdomain' = 'agent_operations');
...
ALTER TABLE `ecommerce_ecm`.`service`.`knowledge_article` ALTER COLUMN `bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Bundle Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`knowledge_article` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`service`.`knowledge_article` ALTER COLUMN `catalog_item_id` SET TAGS ('dbx_business_glossary_term' = 'Catalog Item Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`service`.`knowledge_article` ALTER COLUMN `listing_category_id` SET TAGS ('dbx_business_glossary_term' = 'Listing Category Id (Foreign Key)');
