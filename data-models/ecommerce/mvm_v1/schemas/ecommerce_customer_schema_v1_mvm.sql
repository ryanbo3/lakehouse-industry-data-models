-- Schema for Domain: customer | Business: Ecommerce | Version: v1_mvm
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`account` (
    ...
    `wallet_id` BIGINT COMMENT 'Foreign key linking to payment.wallet. Business justification: Checkout wallet selection: e-commerce accounts need a designated default digital wallet for one-click checkout and balance-based payments. This FK supports wallet balance display at checkout, auto-deb',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`customer_address` (
    ...
    `delivery_zone_id` BIGINT COMMENT 'Foreign key linking to logistics.delivery_zone. Business justification: Address serviceability check at checkout: the system must determine which delivery zone a customer address falls into to display accurate SLA, apply residential/remote surcharges, and validate carrier',
    `price_zone_id` BIGINT COMMENT 'Foreign key linking to pricing.price_zone. Business justification: Customer addresses determine applicable price zones for geographic pricing, regional tax rates, and delivery surcharges. E-commerce pricing engines resolve the correct price list by mapping a customer',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`contact` (
    ...
    `customer_address_id` BIGINT COMMENT 'Foreign key linking to customer.customer_address. Business justification: In B2B e-commerce, individual contacts (buyers, procurement officers, decision-makers) associated with an account often have their own mailing, shipping, or billing addresses distinct from the account',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`loyalty_program` (
    ...
    `campaign_id` BIGINT COMMENT 'Foreign key linking to marketing.campaign. Business justification: Loyalty programs are launched and sustained via marketing campaigns. Linking loyalty_program to its associated campaign enables spend attribution, enrollment ROI, and campaign budget allocation report',
    `promotional_campaign_id` BIGINT COMMENT 'Foreign key linking to pricing.promotional_campaign. Business justification: Loyalty programs are backed by promotional campaigns for launch periods, bonus-point events, and seasonal multipliers. Linking enables finance to track promotional spend attributed to loyalty programs',
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: Category-scoped loyalty programs (e.g., double points on Electronics) are a standard e-commerce promotional mechanic. Program managers configure which product category earns bonus points. A domain exp',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: A loyalty enrollment should be tied to the customers commercial account entity, not just the profile. In B2B and B2C e-commerce, the account is the transactional relationship holder — it carries loya',
    `channel_id` BIGINT COMMENT 'Foreign key linking to marketing.channel. Business justification: Channel performance analysis for loyalty acquisition: which marketing channels (email, push, social, in-app) drive loyalty enrollments is a standard e-commerce KPI. Replaces denormalized string enrol',
    ...
    `marketplace_id` BIGINT COMMENT 'Foreign key linking to marketplace.marketplace. Business justification: Loyalty program enrollments are marketplace-scoped in multi-marketplace platforms (e.g., Prime on Amazon.com vs Amazon.co.uk). Benefit application, points earning rules, and settlement reporting requi',
    ...
    `promotional_campaign_id` BIGINT COMMENT 'Foreign key linking to pricing.promotional_campaign. Business justification: Loyalty enrollments are driven by specific promotional campaigns (e.g., double-points enrollment bonuses). Linking enables campaign ROI reporting on enrollment lift and replaces the denormalized enrol',
    `campaign_id` BIGINT COMMENT 'Foreign key linking to marketing.campaign. Business justification: Loyalty acquisition attribution: linking enrollment to the originating campaign enables campaign ROI measurement (cost-per-enrollment, LTV of campaign-acquired loyalty members). E-commerce marketers r',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`preference` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to product.brand. Business justification: Brand preferences drive personalized homepage recommendations, email campaigns, and search ranking in e-commerce. Distinct from item-level preference (preferred_catalog_item_id already exists). A pers',
    ...
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: Category preferences (e.g., prefers Electronics, Sports) are a foundational personalization signal used in recommendation engines, browse personalization, and targeted email campaigns. Distinct from',
    `promotional_campaign_id` BIGINT COMMENT 'Foreign key linking to pricing.promotional_campaign. Business justification: Customers opt in or out of specific promotional campaigns (GDPR-compliant promotional pricing consent). Linking preference to promotional_campaign enables the pricing engine to check customer consent ',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`consent_event` (
    ...
    `seller_profile_id` BIGINT COMMENT 'Foreign key linking to seller.seller_profile. Business justification: GDPR/CCPA require tracking customer consent per data processor. In marketplace e-commerce, individual sellers are separate data controllers/processors. consent_event.seller_profile_id identifies which',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`wishlist` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to product.brand. Business justification: Brand-curated wishlists (e.g., My Nike Collection, brand gift registries) are a real e-commerce feature enabling brand-specific marketing outreach and conversion campaigns. Retailers use brand-scope',
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: Category-scoped wishlists (e.g., My Electronics Wishlist, Baby Registry - Clothing) are a standard e-commerce feature. Category association enables back-in-stock notifications by category, categor',
    ...
    `marketplace_id` BIGINT COMMENT 'Foreign key linking to marketplace.marketplace. Business justification: In multi-marketplace platforms, wishlists are marketplace-scoped: sharing URLs, currency display, and item availability are marketplace-specific. Operational reports on wishlist conversion rates and c',
...
CREATE OR REPLACE TABLE `ecommerce_ecm`.`customer`.`wishlist_item` (
    ...
    `bundle_id` BIGINT COMMENT 'Foreign key linking to product.bundle. Business justification: Customers add product bundles (gift sets, starter kits) to wishlists for gift registries and deferred purchase. Bundle wishlist items support bundle promotion conversion tracking and gift registry ful',
    `catalog_item_id` BIGINT COMMENT 'Foreign key linking to the catalog item',
    `marketplace_listing_id` BIGINT COMMENT 'Foreign key linking to marketplace.marketplace_listing. Business justification: Price-drop and back-in-stock notification workflows require knowing which specific marketplace listing a wishlist item references — including seller, condition, and current price. E-commerce platforms',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Customers save specific SKU variants (size, color) to wishlists — not just generic catalog items. SKU-level wishlist items enable precise back-in-stock alerts, accurate inventory-driven conversion nud',
...
ALTER TABLE `ecommerce_ecm`.`customer`.`contact` ADD CONSTRAINT `fk_customer_contact_customer_address_id` FOREIGN KEY (`customer_address_id`) REFERENCES `ecommerce_ecm`.`customer`.`customer_address`(`customer_address_id`);
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` ADD CONSTRAINT `fk_customer_loyalty_enrollment_account_id` FOREIGN KEY (`account_id`) REFERENCES `ecommerce_ecm`.`customer`.`account`(`account_id`);
...
ALTER TABLE `ecommerce_ecm`.`customer`.`account` ALTER COLUMN `wallet_id` SET TAGS ('dbx_business_glossary_term' = 'Default Wallet Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`customer_address` ALTER COLUMN `delivery_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Zone Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`customer_address` ALTER COLUMN `price_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Price Zone Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`contact` ALTER COLUMN `customer_address_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Customer Address Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`contact` ALTER COLUMN `customer_address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `ecommerce_ecm`.`customer`.`contact` ALTER COLUMN `customer_address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_program` SET TAGS ('dbx_subdomain' = 'loyalty_engagement');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_program` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_program` ALTER COLUMN `promotional_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotional Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_program` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Scoped Category Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` SET TAGS ('dbx_subdomain' = 'loyalty_engagement');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Channel Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` ALTER COLUMN `marketplace_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` ALTER COLUMN `promotional_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotional Campaign Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`loyalty_enrollment` ALTER COLUMN `campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Source Campaign Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`segment` SET TAGS ('dbx_subdomain' = 'loyalty_engagement');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`preference` SET TAGS ('dbx_subdomain' = 'loyalty_engagement');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`preference` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Preferred Brand Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`preference` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Preferred Category Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`preference` ALTER COLUMN `promotional_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promotional Campaign Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`consent_event` SET TAGS ('dbx_subdomain' = 'loyalty_engagement');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`consent_event` ALTER COLUMN `seller_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Seller Profile Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist` SET TAGS ('dbx_subdomain' = 'shopping_behavior');
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist` ALTER COLUMN `wishlist_id` SET TAGS ('dbx_business_glossary_term' = 'Wishlist ID');
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist` ALTER COLUMN `marketplace_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist_item` SET TAGS ('dbx_subdomain' = 'shopping_behavior');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist_item` ALTER COLUMN `bundle_id` SET TAGS ('dbx_business_glossary_term' = 'Bundle Id (Foreign Key)');
...
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist_item` ALTER COLUMN `marketplace_listing_id` SET TAGS ('dbx_business_glossary_term' = 'Marketplace Listing Id (Foreign Key)');
ALTER TABLE `ecommerce_ecm`.`customer`.`wishlist_item` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
