-- Schema for Domain: compliance | Business: Water Utilities | Version: v1_mvm
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`compliance_permit` (
    ...
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: A compliance permit (NPDES, IUP, state primacy permit) is issued by a specific regulatory agency. Linking compliance_permit to regulatory_agency establishes the issuing authority for each permit, enab',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`permit_condition` (
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: A permit_condition is an enforceable condition attached to a regulatory permit. The compliance_permit is the master permit record in this domain. This parent-child relationship is essential — permit c',
    `milestone_id` BIGINT COMMENT 'Foreign key linking to project.milestone. Business justification: Permit conditions with compliance schedule milestones (e.g., install upgrade by date X) are directly tracked as project milestones. Water utilities project managers and compliance officers use this ',
    `process_unit_id` BIGINT COMMENT 'Foreign key linking to treatment.process_unit. Business justification: Treatment permit conditions frequently specify requirements for specific process units (e.g., CT requirements for a specific disinfection unit, turbidity limits for a specific filter). Linking permit ',
    `regulatory_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_requirement. Business justification: Permit conditions are derived from and must comply with specific regulatory requirements (e.g., NPDES effluent guidelines, SDWA treatment technique requirements). This FK links each permit condition t',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Permit conditions in NPDES/drinking water permits frequently mandate specific treatment chemicals (e.g., required disinfectant type, coagulant). Linking permit_condition to material_master enables com',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`regulatory_requirement` (
    ...
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: A regulatory requirement is issued and enforced by a specific regulatory agency (EPA, state primacy agency, etc.). The regulatory_agency master table is the authoritative source. The existing issuing_',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`obligation` (
    ...
    `cip_project_id` BIGINT COMMENT 'Foreign key linking to project.cip_project. Business justification: Compliance obligations (e.g., treatment upgrades, infrastructure replacements required by permit) are delivered through CIP projects. Water utilities track which CIP project fulfills each obligation f',
    ...
    `grant_id` BIGINT COMMENT 'Foreign key linking to finance.grant. Business justification: Compliance obligations are frequently funded by federal or state grants (e.g., EPA consent decree requirements funded via SRF). Grant-funded obligations require direct linkage for grant expenditure re',
    `permit_condition_id` BIGINT COMMENT 'Foreign key linking to compliance.permit_condition. Business justification: An obligation is an operational compliance task derived from a regulatory requirement or permit condition. While obligation already has regulatory_requirement_id and compliance_permit_id, linking dire',
    ...
    `treatment_permit_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_permit. Business justification: Compliance obligations (monitoring, reporting, operational requirements) arise directly from treatment permit conditions. obligation links to compliance_permit but not to treatment_permit. Water utili',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`dmr` (
    ...
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: A DMR is submitted to a specific regulatory agency (EPA or state NPDES authority). The regulatory_agency master table is the authoritative source for agency information. The existing regulatory_author',
    ...
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: DMRs are submitted by WWTPs; the dmr product currently links to treatment.facility but not directly to wastewater.wwtp. This FK enables WWTP-level DMR management, submission history tracking, and oper',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`dmr_result` (
    ...
    `asset_meter_id` BIGINT COMMENT 'Foreign key linking to asset.asset_meter. Business justification: NPDES DMR results must be traceable to the specific monitoring instrument that produced the measurement. Linking dmr_result to asset_meter supports instrument calibration verification, data quality as',
    ...
    `dmr_id` BIGINT COMMENT 'Foreign key linking to compliance.dmr. Business justification: dmr_result is the line-item record of a Discharge Monitoring Report. Each result belongs to exactly one parent DMR. This is a critical missing parent-child FK — without it, individual parameter result',
    ...
    `permit_condition_id` BIGINT COMMENT 'Foreign key linking to compliance.permit_condition. Business justification: A DMR result measures compliance against a specific permit condition limit (e.g., effluent limit for a specific parameter). This FK links each measurement result to the exact permit condition it is te',
    `process_reading_id` BIGINT COMMENT 'Foreign key linking to treatment.process_reading. Business justification: DMR result values are sourced directly from process readings (SCADA/lab measurements). Water utilities must trace each DMR submission back to the originating process reading for regulatory audit defen',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`mor` (
    ...
    `asset_meter_id` BIGINT COMMENT 'Foreign key linking to asset.asset_meter. Business justification: Monthly Operating Reports include turbidity, flow, and residual measurements from specific plant instruments. Linking MOR to the primary flow/turbidity meter supports instrument calibration traceabili',
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: A Monthly Operating Report (MOR) is submitted under a specific drinking water operating permit. Linking mor to compliance_permit establishes which permit the MOR satisfies, enabling permit-level repor',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Monthly Operating Reports for water treatment facilities must be attributed to a cost center for operational cost reporting and rate case support. mor has no cost_center_id despite tracking operationa',
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: A Monthly Operating Report is submitted to the state primacy agency. The regulatory_agency master table is the authoritative source for agency information. This FK enables tracking which primacy agenc',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` (
    ...
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: LCRR and SDWA require asset-level violation tracking (e.g., specific lead service lines, specific monitoring points). Compliance violations must be traceable to the specific asset involved for targete',
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: A compliance violation occurs under a specific regulatory permit. Linking compliance_violation to compliance_permit establishes which permit was violated, enabling permit-level violation tracking and ',
    `permit_condition_id` BIGINT COMMENT 'Foreign key linking to compliance.permit_condition. Business justification: A compliance violation is most precisely a violation of a specific permit condition (e.g., exceeding an effluent limit set in a permit condition). This FK provides the most granular linkage — from the',
    `process_reading_id` BIGINT COMMENT 'Foreign key linking to treatment.process_reading. Business justification: Compliance violations (MCL exceedances, turbidity violations) are triggered by specific process readings. Regulators require utilities to identify the exact measurement that triggered a violation. No ',
    `regulatory_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_requirement. Business justification: A compliance violation is a violation of a specific regulatory requirement (e.g., MCL exceedance under SDWA, effluent limit exceedance under NPDES). This FK links the violation to the specific require',
    `treatment_violation_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_violation. Business justification: Compliance violations are the regulatory formalization of operational treatment violations. Water utilities must reconcile internal treatment violation records with formal regulatory compliance violat',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` (
    ...
    `finance_budget_id` BIGINT COMMENT 'Foreign key linking to finance.finance_budget. Business justification: Enforcement actions incur civil penalties, legal costs, and SEP project expenditures requiring budget appropriation. Water utility finance teams must track enforcement response spending against approv',
    ...
    `milestone_id` BIGINT COMMENT 'Foreign key linking to project.milestone. Business justification: Enforcement actions include compliance schedules with specific deadlines that map to project milestones. Regulators and utility compliance staff track whether enforcement action deadlines are met by m',
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: Enforcement actions are issued by specific regulatory agencies (EPA, state primacy agency, NPDES authority). The regulatory_agency master table is the authoritative source. The existing issuing_agency',
    ...
    `procurement_contract_id` BIGINT COMMENT 'Foreign key linking to supply.procurement_contract. Business justification: Enforcement actions with Supplemental Environmental Projects (SEP) require a governing procurement contract for the SEP work. The enforcement_action already has sep_description and sep_estimated_cost ',
    `treatment_permit_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_permit. Business justification: Enforcement actions are issued against specific treatment permit violations. enforcement_action links to compliance_permit and facility but not to treatment_permit. Water utility legal and compliance ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` (
    ...
    `ap_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ap_invoice. Business justification: Public notifications involve vendor-paid distribution costs (mailing, media). compliance_public_notification tracks distribution_cost_amount — the corresponding AP invoice must be linked for cost reco',
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: Public notifications are issued when MCL violations occur under a specific operating permit. Linking compliance_public_notification to compliance_permit establishes which permits requirements trigger',
    ...
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: Public notifications must be submitted to the state primacy agency. The regulatory_agency master table is the authoritative source. The existing primacy_agency_name (STRING) field is a denormalized ag',
    `treatment_violation_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_violation. Business justification: Public notifications are directly triggered by treatment violations (MCL exceedances, turbidity violations). Water utilities must link public notification records to the specific treatment violation t',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`ccr` (
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to treatment.facility. Business justification: CCRs are facility-specific annual reports required by the Safe Drinking Water Act. Every CCR belongs to a specific water treatment facility (identified by PWSID). ccr has pws_code and system_name as p',
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: Consumer Confidence Reports (CCR) are submitted to the state primacy agency as required by SDWA. Linking ccr to regulatory_agency establishes which primacy agency oversees the CCR submission, enabling',
    ...
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: Consumer Confidence Reports are published per service territory/water system. Linking CCR to the formal territory entity enables automated population of service area demographics, customer counts, and',
    `water_source_id` BIGINT COMMENT 'Foreign key linking to treatment.water_source. Business justification: Consumer Confidence Reports must describe the source water by regulation. CCR has source_water_description and source_water_type as denormalized plain text fields that should be sourced from the water',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` (
    ...
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: Regulatory inspections are conducted by specific regulatory agencies (EPA, state primacy agency, etc.). The regulatory_agency master table is the authoritative source. The existing inspecting_agency (',
    `treatment_permit_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_permit. Business justification: Regulatory inspections are conducted to verify compliance with treatment permit conditions. Inspectors review the specific treatment permit during facility inspections. regulatory_inspection links to ',
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: Regulatory agencies inspect WWTPs directly; regulatory_inspection currently links to treatment.facility but not wastewater.wwtp. This FK directly associates inspection findings with WWTP operational r',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`inspection_finding` (
    ...
    `compliance_violation_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_violation. Business justification: An inspection finding may identify a formal compliance violation. This FK links the inspection finding to the compliance violation record it generated, enabling end-to-end tracking from inspection fin',
    ...
    `regulatory_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_requirement. Business justification: An inspection finding cites a specific regulatory requirement that was not met. The regulatory_citation (STRING) field on inspection_finding is a denormalized text reference; this FK provides a struct',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`corrective_action` (
    ...
    `compliance_violation_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_violation. Business justification: A compliance corrective action is initiated in response to a compliance violation. This FK directly links the corrective action to the violation it addresses, enabling violation-to-resolution tracking',
    `construction_contract_id` BIGINT COMMENT 'Foreign key linking to project.construction_contract. Business justification: Compliance corrective actions requiring physical construction (e.g., sewer rehabilitation after SSO enforcement) are executed under a specific construction contract. This link enables cost tracking an',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Corrective actions with actual_cost require GL journal entry posting for financial close and audit trail. Water utilities must post remediation expenditures to the general ledger. No existing journal_',
    `milestone_id` BIGINT COMMENT 'Foreign key linking to project.milestone. Business justification: Corrective actions have regulatory completion deadlines that correspond to specific project milestones. Linking corrective actions to milestones allows compliance managers to automatically flag overdu',
    ...
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Compliance corrective actions frequently require procurement of specific materials (replacement treatment chemicals, equipment parts, sampling supplies). A direct FK to material_master enables correct',
    `schedule_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_schedule. Business justification: A corrective action may be part of a formal compliance schedule agreed upon with a regulatory agency. This FK links individual corrective action items to the compliance schedule they belong to, enabli',
    ...
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to project.wbs_element. Business justification: Compliance corrective actions are executed as specific WBS elements within CIP projects. Project controls teams track corrective action costs and schedule at the WBS level; regulators require evidence',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Regulatory submissions involve staff labor and consulting costs that must be attributed to a cost center for compliance program cost tracking and rate case O&M expense support. regulatory_submission h',
    ...
    `treatment_permit_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_permit. Business justification: Regulatory submissions (DMRs, MORs, annual reports) are made under a specific treatment permit number. regulatory_submission links to compliance_permit and facility but not to treatment_permit. Water ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` (
    ...
    `cip_project_id` BIGINT COMMENT 'Foreign key linking to project.cip_project. Business justification: Industrial user permits may require capital improvements (e.g., pretreatment facility upgrades, new connection infrastructure) tracked as CIP projects. Pretreatment program managers link IUPs to CIP p',
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: An Industrial User Permit (IUP) is issued under the utilitys pretreatment program, which itself operates under the utilitys NPDES permit. Linking pretreatment_iup to compliance_permit establishes th',
    `finance_budget_id` BIGINT COMMENT 'Foreign key linking to finance.finance_budget. Business justification: Industrial user permit programs require dedicated budget appropriations for inspection, sampling, and enforcement activities. Pretreatment program managers must track IUP-related expenditures against ',
    `industrial_user_id` BIGINT COMMENT 'Reference to the industrial user (customer/facility) to whom this permit is issued.',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Industrial User Permits specify approved pretreatment chemicals the industrial user must use. Linking pretreatment_iup to material_master enables the pretreatment program coordinator to verify that th',
    ...
    `regulatory_submission_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_submission. Business justification: An Industrial User Permit (IUP) issuance, renewal, or modification requires a formal regulatory submission. This FK links the IUP to its corresponding regulatory submission record, enabling tracking o',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`industrial_user` (
    ...
    `point_id` BIGINT COMMENT 'Foreign key linking to service.point. Business justification: Each industrial user under a pretreatment program connects at a specific service point. Pretreatment coordinators and billing staff must link the industrial user to their service connection point for ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`overflow_event` (
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: SSO/CSO overflow events must be reported under the facilitys NPDES permit. Linking overflow_event to compliance_permit establishes which permit governs the reporting obligation for the overflow, enab',
    `compliance_violation_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_violation. Business justification: An overflow event (SSO/CSO) typically constitutes or results in a compliance violation under the NPDES permit. This FK links the overflow event to the formal compliance violation record it generated, ',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Sanitary sewer overflow and combined sewer overflow events incur emergency response, cleanup, and regulatory reporting costs that must be attributed to a cost center. overflow_event has no cost_center',
    ...
    `regulatory_submission_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_submission. Business justification: Overflow events (SSO/CSO) require formal regulatory submission/notification to the permitting authority. This FK links the overflow event to its corresponding regulatory submission record, enabling tr',
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supply.vendor. Business justification: SSO/CSO overflow events require immediate emergency vendor response (vacuum trucks, bypass pumping, cleanup crews). Tracking the response vendor on the overflow event record supports emergency respons',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to asset.work_order. Business justification: SSO/CSO overflow events require emergency response work orders for repair and cleanup. Clean Water Act reporting requires documentation of response actions taken. Linking overflow_event to the respons',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`schedule` (
    `schedule_id` BIGINT COMMENT 'Unique identifier for the compliance schedule record. Primary key.',
    ...
    `grant_id` BIGINT COMMENT 'Foreign key linking to finance.grant. Business justification: Compliance schedules for infrastructure improvements are frequently funded by EPA/state grants (e.g., SRF loans, WIFIA). compliance_schedule has a plain-text funding_source column — replacing it with ',
    `milestone_id` BIGINT COMMENT 'Foreign key linking to project.milestone. Business justification: Compliance schedules negotiated with regulators map individual schedule milestones to specific CIP project milestones. This link enables compliance officers to report milestone completion status direc',
    ...
    `regulatory_agency_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_agency. Business justification: Compliance schedules are agreed upon between the utility and a specific regulatory agency. The regulatory_agency master table is the authoritative source. The existing issuing_authority (STRING) field',
    ...
    `treatment_permit_id` BIGINT COMMENT 'Foreign key linking to treatment.treatment_permit. Business justification: Compliance schedules are established as conditions of treatment permits (e.g., schedule to achieve new turbidity standard). compliance_schedule links to compliance_permit and enforcement_action but no',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to project.wbs_element. Business justification: Compliance schedules are delivered through specific WBS elements in CIP projects. Linking compliance schedules to WBS elements enables earned value tracking of compliance-driven work and supports regu',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to asset.work_order. Business justification: Compliance schedule milestones are executed through specific work orders (e.g., infrastructure replacement under consent order). Linking compliance_schedule to the executing work_order supports milest',
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: Compliance schedules for treatment upgrades, capacity improvements, and permit condition milestones are directly tied to specific WWTPs. This FK links the compliance schedule to the WWTP for milestone',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`compliance`.`regulatory_agency` (
    ...
    `parent_agency_regulatory_agency_id` BIGINT COMMENT 'Reference to the parent or umbrella agency if this agency is a regional office, division, or sub-agency (e.g., EPA Region 5 parent is EPA). Null if this is a top-level agency.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_permit` ADD CONSTRAINT `fk_compliance_compliance_permit_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ADD CONSTRAINT `fk_compliance_permit_condition_compliance_permit_id` FOREIGN KEY (`compliance_permit_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_permit`(`compliance_permit_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ADD CONSTRAINT `fk_compliance_permit_condition_regulatory_requirement_id` FOREIGN KEY (`regulatory_requirement_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_requirement`(`regulatory_requirement_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_requirement` ADD CONSTRAINT `fk_compliance_regulatory_requirement_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ADD CONSTRAINT `fk_compliance_obligation_permit_condition_id` FOREIGN KEY (`permit_condition_id`) REFERENCES `water_utilities_ecm`.`compliance`.`permit_condition`(`permit_condition_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr` ADD CONSTRAINT `fk_compliance_dmr_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` ADD CONSTRAINT `fk_compliance_dmr_result_dmr_id` FOREIGN KEY (`dmr_id`) REFERENCES `water_utilities_ecm`.`compliance`.`dmr`(`dmr_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` ADD CONSTRAINT `fk_compliance_dmr_result_permit_condition_id` FOREIGN KEY (`permit_condition_id`) REFERENCES `water_utilities_ecm`.`compliance`.`permit_condition`(`permit_condition_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` ADD CONSTRAINT `fk_compliance_mor_compliance_permit_id` FOREIGN KEY (`compliance_permit_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_permit`(`compliance_permit_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` ADD CONSTRAINT `fk_compliance_mor_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ADD CONSTRAINT `fk_compliance_compliance_violation_compliance_permit_id` FOREIGN KEY (`compliance_permit_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_permit`(`compliance_permit_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ADD CONSTRAINT `fk_compliance_compliance_violation_permit_condition_id` FOREIGN KEY (`permit_condition_id`) REFERENCES `water_utilities_ecm`.`compliance`.`permit_condition`(`permit_condition_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ADD CONSTRAINT `fk_compliance_compliance_violation_regulatory_requirement_id` FOREIGN KEY (`regulatory_requirement_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_requirement`(`regulatory_requirement_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ADD CONSTRAINT `fk_compliance_enforcement_action_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ADD CONSTRAINT `fk_compliance_compliance_public_notification_compliance_permit_id` FOREIGN KEY (`compliance_permit_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_permit`(`compliance_permit_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ADD CONSTRAINT `fk_compliance_compliance_public_notification_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`ccr` ADD CONSTRAINT `fk_compliance_ccr_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` ADD CONSTRAINT `fk_compliance_regulatory_inspection_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`inspection_finding` ADD CONSTRAINT `fk_compliance_inspection_finding_compliance_violation_id` FOREIGN KEY (`compliance_violation_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_violation`(`compliance_violation_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`inspection_finding` ADD CONSTRAINT `fk_compliance_inspection_finding_regulatory_requirement_id` FOREIGN KEY (`regulatory_requirement_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_requirement`(`regulatory_requirement_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_compliance_violation_id` FOREIGN KEY (`compliance_violation_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_violation`(`compliance_violation_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_enforcement_action_id` FOREIGN KEY (`enforcement_action_id`) REFERENCES `water_utilities_ecm`.`compliance`.`enforcement_action`(`enforcement_action_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_inspection_finding_id` FOREIGN KEY (`inspection_finding_id`) REFERENCES `water_utilities_ecm`.`compliance`.`inspection_finding`(`inspection_finding_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_overflow_event_id` FOREIGN KEY (`overflow_event_id`) REFERENCES `water_utilities_ecm`.`compliance`.`overflow_event`(`overflow_event_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ADD CONSTRAINT `fk_compliance_corrective_action_schedule_id` FOREIGN KEY (`schedule_id`) REFERENCES `water_utilities_ecm`.`compliance`.`schedule`(`schedule_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ADD CONSTRAINT `fk_compliance_pretreatment_iup_compliance_permit_id` FOREIGN KEY (`compliance_permit_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_permit`(`compliance_permit_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ADD CONSTRAINT `fk_compliance_pretreatment_iup_regulatory_submission_id` FOREIGN KEY (`regulatory_submission_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_submission`(`regulatory_submission_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ADD CONSTRAINT `fk_compliance_overflow_event_compliance_permit_id` FOREIGN KEY (`compliance_permit_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_permit`(`compliance_permit_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ADD CONSTRAINT `fk_compliance_overflow_event_compliance_violation_id` FOREIGN KEY (`compliance_violation_id`) REFERENCES `water_utilities_ecm`.`compliance`.`compliance_violation`(`compliance_violation_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ADD CONSTRAINT `fk_compliance_overflow_event_regulatory_submission_id` FOREIGN KEY (`regulatory_submission_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_submission`(`regulatory_submission_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ADD CONSTRAINT `fk_compliance_schedule_enforcement_action_id` FOREIGN KEY (`enforcement_action_id`) REFERENCES `water_utilities_ecm`.`compliance`.`enforcement_action`(`enforcement_action_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ADD CONSTRAINT `fk_compliance_schedule_obligation_id` FOREIGN KEY (`obligation_id`) REFERENCES `water_utilities_ecm`.`compliance`.`obligation`(`obligation_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ADD CONSTRAINT `fk_compliance_schedule_regulatory_agency_id` FOREIGN KEY (`regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ADD CONSTRAINT `fk_compliance_schedule_regulatory_requirement_id` FOREIGN KEY (`regulatory_requirement_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_requirement`(`regulatory_requirement_id`);
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_agency` ADD CONSTRAINT `fk_compliance_regulatory_agency_parent_agency_regulatory_agency_id` FOREIGN KEY (`parent_agency_regulatory_agency_id`) REFERENCES `water_utilities_ecm`.`compliance`.`regulatory_agency`(`regulatory_agency_id`);
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_permit` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `milestone_id` SET TAGS ('dbx_business_glossary_term' = 'Milestone Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `process_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Process Unit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `regulatory_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Chemical Material Master Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `material_master_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`permit_condition` ALTER COLUMN `material_master_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_requirement` SET TAGS ('dbx_subdomain' = 'permit_management');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_requirement` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` SET TAGS ('dbx_subdomain' = 'permit_management');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ALTER COLUMN `cip_project_id` SET TAGS ('dbx_business_glossary_term' = 'Cip Project Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ALTER COLUMN `grant_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ALTER COLUMN `permit_condition_id` SET TAGS ('dbx_business_glossary_term' = 'Permit Condition Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`obligation` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr` SET TAGS ('dbx_subdomain' = 'reporting_submissions');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` SET TAGS ('dbx_subdomain' = 'reporting_submissions');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` ALTER COLUMN `asset_meter_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Meter Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` ALTER COLUMN `dmr_id` SET TAGS ('dbx_business_glossary_term' = 'Dmr Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` ALTER COLUMN `permit_condition_id` SET TAGS ('dbx_business_glossary_term' = 'Permit Condition Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`dmr_result` ALTER COLUMN `process_reading_id` SET TAGS ('dbx_business_glossary_term' = 'Process Reading Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` SET TAGS ('dbx_subdomain' = 'reporting_submissions');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` ALTER COLUMN `asset_meter_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Meter Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`mor` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Registry Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `permit_condition_id` SET TAGS ('dbx_business_glossary_term' = 'Permit Condition Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `process_reading_id` SET TAGS ('dbx_business_glossary_term' = 'Process Reading Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `regulatory_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `treatment_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Violation Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `treatment_violation_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_violation` ALTER COLUMN `treatment_violation_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Budget Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `milestone_id` SET TAGS ('dbx_business_glossary_term' = 'Milestone Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `procurement_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Sep Procurement Contract Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`enforcement_action` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ALTER COLUMN `treatment_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Violation Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ALTER COLUMN `treatment_violation_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`compliance_public_notification` ALTER COLUMN `treatment_violation_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`ccr` SET TAGS ('dbx_subdomain' = 'reporting_submissions');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`ccr` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`ccr` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`ccr` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`ccr` ALTER COLUMN `water_source_id` SET TAGS ('dbx_business_glossary_term' = 'Water Source Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_inspection` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`inspection_finding` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`inspection_finding` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Violation Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`inspection_finding` ALTER COLUMN `regulatory_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `corrective_action_id` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action ID');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `cip_project_id` SET TAGS ('dbx_business_glossary_term' = 'Cip Project Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Violation Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `construction_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Construction Contract Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `enforcement_action_id` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Action ID');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `inspection_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Finding ID');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `milestone_id` SET TAGS ('dbx_business_glossary_term' = 'Milestone Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `overflow_event_id` SET TAGS ('dbx_business_glossary_term' = 'Overflow Event Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Required Material Master Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `action_description` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Description');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `action_number` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Number');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `action_status` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Status');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `action_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|pending_verification|completed|overdue|cancelled');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `action_title` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Title');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `actual_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Completion Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `actual_cost` SET TAGS ('dbx_business_glossary_term' = 'Actual Cost');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `actual_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `actual_start_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Start Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `agency_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Agency Notification Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `agency_notification_method` SET TAGS ('dbx_business_glossary_term' = 'Agency Notification Method');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `agency_notification_method` SET TAGS ('dbx_value_regex' = 'formal_letter|email|portal_submission|phone|in_person_meeting');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `agency_response` SET TAGS ('dbx_business_glossary_term' = 'Agency Response');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `closure_notes` SET TAGS ('dbx_business_glossary_term' = 'Closure Notes');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `effectiveness_review_date` SET TAGS ('dbx_business_glossary_term' = 'Effectiveness Review Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `effectiveness_review_outcome` SET TAGS ('dbx_business_glossary_term' = 'Effectiveness Review Outcome');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Cost');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `planned_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Completion Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `planned_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Start Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `preventive_action_description` SET TAGS ('dbx_business_glossary_term' = 'Preventive Action Description');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `preventive_action_implemented` SET TAGS ('dbx_business_glossary_term' = 'Preventive Action Implemented');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `regulatory_agency_notified` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Notified');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `responsible_department` SET TAGS ('dbx_business_glossary_term' = 'Responsible Department');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `responsible_individual` SET TAGS ('dbx_business_glossary_term' = 'Responsible Individual');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `responsible_individual` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `root_cause_analysis` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Analysis');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `supporting_evidence_location` SET TAGS ('dbx_business_glossary_term' = 'Supporting Evidence Location');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `triggering_event_description` SET TAGS ('dbx_business_glossary_term' = 'Triggering Event Description');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `triggering_event_type` SET TAGS ('dbx_business_glossary_term' = 'Triggering Event Type');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `triggering_event_type` SET TAGS ('dbx_value_regex' = 'regulatory_violation|enforcement_action|inspection_finding|internal_audit|self_disclosure|customer_complaint');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `verification_date` SET TAGS ('dbx_business_glossary_term' = 'Verification Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `verification_method` SET TAGS ('dbx_business_glossary_term' = 'Verification Method');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `verification_method` SET TAGS ('dbx_value_regex' = 'internal_audit|third_party_audit|regulatory_inspection|documentation_review|testing|monitoring');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `verification_notes` SET TAGS ('dbx_business_glossary_term' = 'Verification Notes');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `verification_status` SET TAGS ('dbx_business_glossary_term' = 'Verification Status');
ALTER TABLE `water_utilities_ecm`.`compliance`.`corrective_action` ALTER COLUMN `verification_status` SET TAGS ('dbx_value_regex' = 'pending|verified_effective|verified_ineffective|requires_additional_action');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` SET TAGS ('dbx_subdomain' = 'reporting_submissions');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_submission` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ALTER COLUMN `cip_project_id` SET TAGS ('dbx_business_glossary_term' = 'Cip Project Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Budget Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Pretreatment Chemical Material Master Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`pretreatment_iup` ALTER COLUMN `regulatory_submission_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`industrial_user` ALTER COLUMN `point_id` SET TAGS ('dbx_business_glossary_term' = 'Point Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` SET TAGS ('dbx_subdomain' = 'enforcement_oversight');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Violation Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ALTER COLUMN `regulatory_submission_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Response Vendor Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`overflow_event` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` SET TAGS ('dbx_subdomain' = 'permit_management');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Identifier (ID)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `cip_project_id` SET TAGS ('dbx_business_glossary_term' = 'Cip Project Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `enforcement_action_id` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Action Identifier (ID)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `finance_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `grant_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `milestone_id` SET TAGS ('dbx_business_glossary_term' = 'Milestone Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `procurement_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Registry Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `regulatory_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Identifier (ID)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `treatment_permit_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `actual_total_cost` SET TAGS ('dbx_business_glossary_term' = 'Actual Total Cost');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `document_url` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Document Uniform Resource Locator (URL)');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Effective Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `estimated_total_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Total Cost');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `extension_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Extension Approval Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `extension_approved_flag` SET TAGS ('dbx_business_glossary_term' = 'Extension Approved Flag');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `extension_justification` SET TAGS ('dbx_business_glossary_term' = 'Extension Request Justification');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `extension_request_date` SET TAGS ('dbx_business_glossary_term' = 'Extension Request Submission Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `extension_requested_flag` SET TAGS ('dbx_business_glossary_term' = 'Extension Requested Flag');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `final_compliance_deadline` SET TAGS ('dbx_business_glossary_term' = 'Final Compliance Deadline Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Jurisdiction');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified By User');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `last_status_update_date` SET TAGS ('dbx_business_glossary_term' = 'Last Status Update Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `milestone_count` SET TAGS ('dbx_business_glossary_term' = 'Total Milestone Count');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `milestones_completed_count` SET TAGS ('dbx_business_glossary_term' = 'Completed Milestones Count');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `milestones_overdue_count` SET TAGS ('dbx_business_glossary_term' = 'Overdue Milestones Count');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `next_status_update_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Status Update Due Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Notes');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `on_schedule_flag` SET TAGS ('dbx_business_glossary_term' = 'On Schedule Flag');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `original_final_deadline` SET TAGS ('dbx_business_glossary_term' = 'Original Final Compliance Deadline Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `overall_completion_percentage` SET TAGS ('dbx_business_glossary_term' = 'Overall Completion Percentage');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `public_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Public Notification Date');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `public_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Public Notification Required Flag');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `regulatory_correspondence_count` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Correspondence Count');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `responsible_department` SET TAGS ('dbx_business_glossary_term' = 'Responsible Department');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_description` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Description');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_name` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Name');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_number` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Number');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Status');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_type` SET TAGS ('dbx_business_glossary_term' = 'Compliance Schedule Type');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `schedule_type` SET TAGS ('dbx_value_regex' = 'consent_order|administrative_order|permit_condition|settlement_agreement|voluntary|other');
ALTER TABLE `water_utilities_ecm`.`compliance`.`schedule` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Record Created By User');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_agency` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_agency` SET TAGS ('dbx_subdomain' = 'permit_management');
...
ALTER TABLE `water_utilities_ecm`.`compliance`.`regulatory_agency` ALTER COLUMN `parent_agency_regulatory_agency_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Agency Identifier (ID)');
