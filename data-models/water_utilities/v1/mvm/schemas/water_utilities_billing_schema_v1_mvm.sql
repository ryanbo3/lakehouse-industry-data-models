-- Schema for Domain: billing | Business: Water Utilities | Version: v1_mvm
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`invoice` (
    ...
    `billing_rate_schedule_id` BIGINT COMMENT 'Foreign key linking to billing.billing_rate_schedule. Business justification: An invoice is generated under a specific rate schedule (tariff). The invoice currently stores rate_schedule_code as a denormalized string. Adding billing_rate_schedule_id FK normalizes this reference ',
    `ccr_id` BIGINT COMMENT 'Foreign key linking to compliance.ccr. Business justification: Water utilities are required to distribute the Consumer Confidence Report (CCR) with or alongside customer invoices annually. Invoice already has a ccr_included boolean flag — a denormalization of the',
    `ccr_period_id` BIGINT COMMENT 'Foreign key linking to quality.ccr_period. Business justification: EPA CCR Rule requires annual Consumer Confidence Report distribution to customers, often bundled with invoices. Invoice.ccr_included flag already signals this; a FK to ccr_period_id identifies which s',
    ...
    `compliance_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_permit. Business justification: Water utilities invoice industrial/commercial customers for permit-related surcharges and compliance fees tied to a specific NPDES or pretreatment permit. Billing staff generating invoices for regulat',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Water utilities allocate billed revenue to cost centers for departmental P&L reporting, rate case cost-of-service studies, and GASB fund accounting. A utility finance expert expects every invoice to c',
    ...
    `industrial_user_permit_id` BIGINT COMMENT 'Foreign key linking to wastewater.industrial_user_permit. Business justification: Industrial pretreatment permit holders are billed for annual permit fees, excess strength surcharges (BOD, TSS, FOG), and inspection fees. Invoices must reference the industrial user permit to support',
    ...
    `premise_id` BIGINT COMMENT 'Foreign key linking to customer.premise. Business justification: Invoices in water utilities are generated for consumption at a specific premise. Premise defines meter size, service type, pressure zone, and connection details — all needed for bill generation, consu',
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: Invoices in water utilities are generated per service agreement — the contract defining services, rates, and billing terms. Bill generation, rate schedule application, and revenue recognition all requ',
    ...
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: Wastewater invoices must be allocated to the receiving WWTP for WWTP-level revenue reporting, cost-of-service studies, and rate case filings. Utilities track which WWTP treats a customers wastewater ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`invoice_line` (
    ...
    `biosolids_batch_id` BIGINT COMMENT 'Foreign key linking to wastewater.biosolids_batch. Business justification: Biosolids disposal and hauling contracts generate invoice line items referencing specific biosolids batches (by batch number, weight, and disposition method). Utilities bill municipalities or contract',
    `conservation_program_id` BIGINT COMMENT 'Foreign key linking to service.conservation_program. Business justification: Conservation program rebates and incentive credits appear as specific line items on water utility invoices. Direct FK supports program cost tracking, regulatory reporting on incentive disbursements, a',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Invoice line items represent individual service charges that must be attributed to cost centers for departmental revenue reporting, rate case cost-of-service studies, and budget variance analysis. Uti',
    ...
    `industrial_user_permit_id` BIGINT COMMENT 'Foreign key linking to wastewater.industrial_user_permit. Business justification: Individual invoice line items for excess strength surcharges (BOD, TSS, FOG, heavy metals) reference the specific industrial user permit that defines the applicable limits and surcharge rates. This is',
    ...
    `rate_component_id` BIGINT COMMENT 'Foreign key linking to billing.rate_component. Business justification: Each invoice line represents a discrete billable charge generated by a specific rate component (e.g., base charge, volumetric tier, surcharge). invoice_line already links to billing_rate_schedule and ',
    ...
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: Wastewater service charge line items must reference the specific WWTP for line-level cost allocation, WWTP-level revenue reconciliation, and cost-of-service rate case support. Existing wtp_facility_id',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`payment` (
    ...
    `payment_invoice_id` BIGINT COMMENT 'Reference to the specific bill or invoice that this payment is intended to satisfy. May be null for advance payments or account credits.',
    ...
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: Payments are applied to balances arising from a specific service agreement. Cash application, AR reconciliation, and revenue recognition by service type all require knowing which service agreement a p',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`billing_account` (
    ...
    `affordability_plan_id` BIGINT COMMENT 'Foreign key linking to service.affordability_plan. Business justification: Affordability plan discounts and reduced rates are applied directly at billing account level during charge calculation. Low-income assistance program administration requires direct billing account to ',
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Billing accounts are assigned to cost centers to support recurring revenue tracking, aging analysis by department, and rate case cost allocation. Utility finance teams report outstanding balances and ',
    ...
    `cycle_id` BIGINT COMMENT 'Foreign key linking to billing.billing_cycle. Business justification: A billing account is assigned to a billing cycle that governs when invoices are generated. billing_account currently stores billing_cycle_code as a denormalized string. Adding billing_cycle_id FK norm',
    `premise_id` BIGINT COMMENT 'Foreign key linking to customer.premise. Business justification: A billing account in water utilities is associated with a specific premise (physical service location). Premise-level financial reporting, disconnection management, final bill processing, and service ',
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: A billing account is the financial representation of a service agreement in water utilities. The service agreement defines services being billed; the billing account tracks financial position. This li',
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: Territory-level revenue reporting, rate zone determination, and PUC/regulatory filings require direct territory association on billing accounts. Water utilities report revenue by service territory for',
    `water_system_id` BIGINT COMMENT 'Foreign key linking to quality.water_system. Business justification: Billing accounts are tied to a specific public water system (PWS), determining applicable rate schedules, CCR distribution obligations, and regulatory reporting. Water utility billing systems must ass',
    ...
    `wwtp_id` BIGINT COMMENT 'Foreign key linking to wastewater.wwtp. Business justification: A billing accounts assigned WWTP determines applicable wastewater rate schedules, service area classification, and capacity fee obligations. Utilities assign accounts to WWTPs for rate differentiatio',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`billing_rate_schedule` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Rate schedules are associated with cost centers for cost-of-service studies, rate case filings, and revenue budget preparation. Utility regulators require that each rate schedules revenue be traceabl',
    `meter_size_type_id` BIGINT COMMENT 'Foreign key linking to metering.meter_size_type. Business justification: Water utility rate schedules are meter-size-specific (e.g., 5/8" residential vs. 2" commercial base charges). The plain-text meter_size_applicability column is a denormalization of meter_size_type. A ',
    `rate_case_id` BIGINT COMMENT 'Foreign key reference to the rate case (regulatory proceeding) that established or modified this rate schedule. Links to the rate_case product.',
    `regulatory_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_requirement. Business justification: Rate schedules in water utilities are directly mandated or approved under specific regulatory requirements (e.g., conservation rate structures required by drought regulations, surcharges mandated by s',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`rate_component` (
    ...
    `conservation_program_id` BIGINT COMMENT 'Foreign key linking to service.conservation_program. Business justification: Conservation rate components (tiered pricing, drought surcharges) implement specific conservation programs. Direct FK links the billing rate structure to the program it supports, enabling regulatory r',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Rate components are assigned to cost centers for cost-of-service studies and rate case filings with regulatory commissions. The plain cost_center column is a denormalized reference; a proper FK to fin',
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Each rate component maps to a GL revenue account for financial reporting and rate case revenue requirement filings. The plain gl_account_code is a denormalized GL reference; a proper FK to general_led',
    `meter_size_type_id` BIGINT COMMENT 'Foreign key linking to metering.meter_size_type. Business justification: Individual rate components (base charges, minimum charges) vary by meter size in water utility tariff structures. The plain-text meter_size_applicability column is a denormalization of meter_size_type',
    `rate_tier_id` BIGINT COMMENT 'Foreign key linking to billing.rate_tier. Business justification: A rate component that implements tiered pricing references a specific rate tier definition from the rate_tier master table. This FK links the component-level pricing definition to the tier structure (',
    `regulatory_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_requirement. Business justification: Individual rate components (stormwater surcharges, lead service line replacement fees, low-income assistance credits) are often mandated by specific regulatory requirements. Linking rate_component to ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`adjustment` (
    ...
    `accuracy_test_id` BIGINT COMMENT 'Foreign key linking to metering.accuracy_test. Business justification: Billing adjustments issued after a meter accuracy test finding (e.g., retroactive consumption correction when a meter tested slow or fast) must reference the specific accuracy test for regulatory audi',
    `analytical_result_id` BIGINT COMMENT 'Foreign key linking to quality.analytical_result. Business justification: Quality-triggered billing adjustments (boil-water advisory credits, contamination event credits) must trace to the specific lab analytical result that justified the credit amount. Billing analysts and',
    ...
    `compliance_violation_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_violation. Business justification: Billing adjustments in water utilities are frequently issued as a direct result of compliance violations — e.g., MCL exceedances trigger customer credits, or enforcement-ordered rate relief. Adjustmen',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Billing adjustments (leak allowances, rate corrections, write-offs) must be posted to a cost center for budget impact tracking, regulatory compliance reporting, and cost-of-service studies. Utility co',
    `cso_event_id` BIGINT COMMENT 'Foreign key linking to wastewater.cso_event. Business justification: CSO events trigger billing adjustments for affected customers (service credits, damage compensation, emergency response fee waivers). The existing sso_event→adjustment link covers SSOs; CSO events req',
    ...
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Billing adjustments are posted to specific GL revenue or contra-revenue accounts. The plain gl_account_code column is a denormalized GL reference; replacing it with a proper FK to general_ledger enfor',
    `high_usage_alert_id` BIGINT COMMENT 'Foreign key linking to metering.high_usage_alert. Business justification: Billing adjustments issued in response to high-usage alert investigations (e.g., leak allowances, billing corrections) must reference the triggering alert for audit traceability. Water utilities requi',
    ...
    `invoice_line_id` BIGINT COMMENT 'Foreign key linking to billing.invoice_line. Business justification: Billing adjustments frequently target a specific invoice line item (e.g., correcting a volumetric charge, applying a leak allowance credit to a specific charge line). adjustment already has invoice_id',
    ...
    `leak_detection_survey_id` BIGINT COMMENT 'Foreign key linking to distribution.leak_detection_survey. Business justification: Leak allowance adjustments (adjustment.leak_allowance_flag) require documented evidence from a field leak detection survey. Water utilities must link the credit to the confirming survey for audit, reg',
    ...
    `premise_id` BIGINT COMMENT 'Foreign key linking to customer.premise. Business justification: Leak allowance adjustments and consumption corrections are tied to the physical premise where the leak or meter issue occurred. Premise-level adjustment tracking supports infrastructure loss analysis,',
    ...
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: Billing adjustments (leak allowances, rate corrections, consumption corrections) are applied against a specific service agreement. The service agreement defines applicable rate schedule, service type,',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to service.order. Business justification: Billing adjustments (leak credits, meter test credits, main break credits) are authorized by specific service orders. Adjustment approval workflows and audit trails require direct reference to the aut',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to asset.work_order. Business justification: In water utilities, billing adjustments for leak credits, meter replacements, or service restoration are authorized by a specific asset work order. Linking adjustment to work_order provides the audit ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`payment_plan` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to service.agreement. Business justification: Payment plan negotiation and disconnection prevention workflows directly reference the service agreement at risk. Collections staff need to know which agreement governs the service being protected. Di',
    `assistance_enrollment_id` BIGINT COMMENT 'Reference to the utility assistance program (such as Low Income Home Energy Assistance Program - LIHEAP, or local affordability program) associated with this payment plan. Nullable if plan is not tied to an assistance program.',
    `billing_account_id` BIGINT COMMENT 'Reference to the customer account enrolled in this payment plan.',
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: Payment plans are established to resolve past-due balances on a specific service agreement. Collections staff and assistance program coordinators need this link to manage delinquency, track plan compl',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`collection_notice` (
    ...
    `invoice_id` BIGINT COMMENT 'Foreign key linking to billing.invoice. Business justification: A collection notice is issued when a specific invoice becomes delinquent. Linking collection_notice directly to the triggering invoice enables traceability from delinquency notification back to the or',
    `person_id` BIGINT COMMENT 'Foreign key linking to customer.person. Business justification: Collection notices must be delivered to the responsible person on the account. Regulatory requirements (LCRR, state PUC rules) mandate advance notice to life-support equipment holders and low-income c',
    ...
    `premise_id` BIGINT COMMENT 'Foreign key linking to customer.premise. Business justification: Collection notices and disconnection orders are tied to the premise where service will be physically disconnected. Field crew dispatch, regulatory advance-notice requirements, and disconnection workfl',
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: Collection notices and disconnection proceedings in water utilities are issued against a specific service agreement. Regulatory advance-notice requirements (e.g., life-support equipment holders) and d',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`dispute` (
    ...
    `accuracy_test_id` BIGINT COMMENT 'Foreign key linking to metering.accuracy_test. Business justification: Meter accuracy dispute resolution is a named water utility process: when a customer disputes consumption charges citing meter inaccuracy, a meter accuracy test is ordered and the dispute record must r',
    ...
    `compliance_violation_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_violation. Business justification: Customer billing disputes in water utilities are frequently triggered by compliance violations — MCL exceedances, boil-water notices, or permit limit breaches cause customers to dispute charges. Dispu',
    ...
    `cso_event_id` BIGINT COMMENT 'Foreign key linking to wastewater.cso_event. Business justification: CSO events affecting customers may trigger billing disputes for service credits, property damage claims, or emergency fee waivers. Linking dispute to cso_event supports CSO-related customer complaint ',
    ...
    `high_usage_alert_id` BIGINT COMMENT 'Foreign key linking to metering.high_usage_alert. Business justification: When a customer disputes a bill triggered by a high-usage alert, the dispute record must reference the originating alert for investigation and resolution tracking. Water utility customer service teams',
    ...
    `invoice_line_id` BIGINT COMMENT 'Foreign key linking to billing.invoice_line. Business justification: A billing dispute typically contests a specific charge line on an invoice, not just the invoice header. dispute already has invoice_id for header-level reference, but line-level granularity is critica',
    `leak_detection_survey_id` BIGINT COMMENT 'Foreign key linking to distribution.leak_detection_survey. Business justification: Customers disputing high bills due to suspected leaks trigger field leak detection surveys. Dispute resolution requires linking the dispute record to the confirming or denying survey result — a standa',
    ...
    `complaint_id` BIGINT COMMENT 'Foreign key linking to customer.customer_complaint. Business justification: Billing disputes in water utilities are frequently triggered by formal customer complaints (billing errors, water quality, high-bill complaints). Linking dispute to originating complaint supports PUC ',
    `premise_id` BIGINT COMMENT 'Foreign key linking to customer.premise. Business justification: Disputes about consumption accuracy, meter performance, or service quality are tied to the physical premise. Premise-level dispute tracking supports field investigation dispatch, meter test scheduling',
    ...
    `service_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.service_agreement. Business justification: Billing disputes are tied to a specific service agreement governing the disputed charges. Rate verification, contract terms review, and regulatory compliance reporting all require knowing which servic',
    ...
    `sso_event_id` BIGINT COMMENT 'Foreign key linking to wastewater.sso_event. Business justification: Customers file billing disputes citing SSO events as cause for property damage, service disruption credits, or emergency response fee waivers. Linking dispute to sso_event supports SSO-related dispute',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to asset.work_order. Business justification: Billing disputes for meter accuracy or high consumption trigger asset work orders (meter test, field leak investigation). Linking dispute to the asset work_order supports dispute resolution tracking, ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`rate_tier` (
    ...
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Water utilities map each rate tier to a distinct GL revenue account (e.g., Tier 1 Water Revenue, Tier 2 Conservation Revenue) for GASB revenue recognition reporting and rate case revenue requirement c',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`billing`.`cycle` (
    ...
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: Billing cycles in water utilities are organized by service territory and geographic route. Territory-specific billing schedules support meter reading route optimization and territory-level billing ope',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ADD CONSTRAINT `fk_billing_invoice_billing_rate_schedule_id` FOREIGN KEY (`billing_rate_schedule_id`) REFERENCES `water_utilities_ecm`.`billing`.`billing_rate_schedule`(`billing_rate_schedule_id`);
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ADD CONSTRAINT `fk_billing_invoice_cycle_id` FOREIGN KEY (`cycle_id`) REFERENCES `water_utilities_ecm`.`billing`.`cycle`(`cycle_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ADD CONSTRAINT `fk_billing_invoice_line_rate_component_id` FOREIGN KEY (`rate_component_id`) REFERENCES `water_utilities_ecm`.`billing`.`rate_component`(`rate_component_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`payment` ADD CONSTRAINT `fk_billing_payment_payment_invoice_id` FOREIGN KEY (`payment_invoice_id`) REFERENCES `water_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ADD CONSTRAINT `fk_billing_billing_account_cycle_id` FOREIGN KEY (`cycle_id`) REFERENCES `water_utilities_ecm`.`billing`.`cycle`(`cycle_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ADD CONSTRAINT `fk_billing_rate_component_rate_tier_id` FOREIGN KEY (`rate_tier_id`) REFERENCES `water_utilities_ecm`.`billing`.`rate_tier`(`rate_tier_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_invoice_line_id` FOREIGN KEY (`invoice_line_id`) REFERENCES `water_utilities_ecm`.`billing`.`invoice_line`(`invoice_line_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`collection_notice` ADD CONSTRAINT `fk_billing_collection_notice_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `water_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ADD CONSTRAINT `fk_billing_dispute_invoice_line_id` FOREIGN KEY (`invoice_line_id`) REFERENCES `water_utilities_ecm`.`billing`.`invoice_line`(`invoice_line_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`cycle` ADD CONSTRAINT `fk_billing_cycle_previous_billing_cycle_id` FOREIGN KEY (`previous_billing_cycle_id`) REFERENCES `water_utilities_ecm`.`billing`.`cycle`(`cycle_id`);
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` SET TAGS ('dbx_subdomain' = 'invoice_management');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `billing_rate_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Rate Schedule Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `ccr_id` SET TAGS ('dbx_business_glossary_term' = 'Ccr Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `ccr_period_id` SET TAGS ('dbx_business_glossary_term' = 'Ccr Period Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `compliance_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Permit Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `industrial_user_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Industrial User Permit Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` SET TAGS ('dbx_subdomain' = 'invoice_management');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `biosolids_batch_id` SET TAGS ('dbx_business_glossary_term' = 'Biosolids Batch Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `conservation_program_id` SET TAGS ('dbx_business_glossary_term' = 'Conservation Program Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `industrial_user_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Industrial User Permit Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `rate_component_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Component Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`payment` ALTER COLUMN `payment_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Bill Identifier');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`payment` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` SET TAGS ('dbx_subdomain' = 'invoice_management');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `affordability_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Affordability Plan Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `water_system_id` SET TAGS ('dbx_business_glossary_term' = 'Water System Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_account` ALTER COLUMN `wwtp_id` SET TAGS ('dbx_business_glossary_term' = 'Wwtp Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_rate_schedule` SET TAGS ('dbx_subdomain' = 'rate_pricing');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_rate_schedule` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_rate_schedule` ALTER COLUMN `meter_size_type_id` SET TAGS ('dbx_business_glossary_term' = 'Meter Size Type Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_rate_schedule` ALTER COLUMN `rate_case_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Case ID');
ALTER TABLE `water_utilities_ecm`.`billing`.`billing_rate_schedule` ALTER COLUMN `regulatory_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` SET TAGS ('dbx_subdomain' = 'rate_pricing');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `conservation_program_id` SET TAGS ('dbx_business_glossary_term' = 'Conservation Program Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `meter_size_type_id` SET TAGS ('dbx_business_glossary_term' = 'Meter Size Type Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `rate_tier_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Tier Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `regulatory_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` SET TAGS ('dbx_subdomain' = 'invoice_management');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `accuracy_test_id` SET TAGS ('dbx_business_glossary_term' = 'Accuracy Test Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `analytical_result_id` SET TAGS ('dbx_business_glossary_term' = 'Analytical Result Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Violation Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `cso_event_id` SET TAGS ('dbx_business_glossary_term' = 'Cso Event Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `high_usage_alert_id` SET TAGS ('dbx_business_glossary_term' = 'High Usage Alert Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `invoice_line_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Line Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `leak_detection_survey_id` SET TAGS ('dbx_business_glossary_term' = 'Leak Detection Survey Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Service Order Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`payment_plan` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Agreement Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`payment_plan` ALTER COLUMN `assistance_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Assistance Program Identifier (ID)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`payment_plan` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`collection_notice` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`collection_notice` ALTER COLUMN `person_id` SET TAGS ('dbx_business_glossary_term' = 'Notified Person Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`collection_notice` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`collection_notice` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` SET TAGS ('dbx_subdomain' = 'invoice_management');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `accuracy_test_id` SET TAGS ('dbx_business_glossary_term' = 'Accuracy Test Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `compliance_violation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Violation Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `cso_event_id` SET TAGS ('dbx_business_glossary_term' = 'Cso Event Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `high_usage_alert_id` SET TAGS ('dbx_business_glossary_term' = 'High Usage Alert Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `invoice_line_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Line Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `leak_detection_survey_id` SET TAGS ('dbx_business_glossary_term' = 'Leak Detection Survey Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `complaint_id` SET TAGS ('dbx_business_glossary_term' = 'Originating Complaint Customer Complaint Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `sso_event_id` SET TAGS ('dbx_business_glossary_term' = 'Sso Event Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`dispute` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_tier` SET TAGS ('dbx_subdomain' = 'rate_pricing');
...
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_tier` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`billing`.`rate_tier` ALTER COLUMN `preceding_rate_tier_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `water_utilities_ecm`.`billing`.`cycle` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `water_utilities_ecm`.`billing`.`cycle` SET TAGS ('dbx_subdomain' = 'rate_pricing');
ALTER TABLE `water_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Identifier');
ALTER TABLE `water_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `previous_billing_cycle_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `water_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
