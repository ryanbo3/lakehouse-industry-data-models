-- Schema for Domain: finance | Business: Water Utilities | Version: v1_mvm
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`journal_entry` (
    ...
    `billing_account_id` BIGINT COMMENT 'Foreign key linking to billing.billing_account. Business justification: Journal entries for revenue recognition, bad debt write-offs, and deferred revenue adjustments reference specific billing accounts for audit trail and AR reconciliation. Utility accountants require th',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`journal_entry_line` (
    ...
    `invoice_id` BIGINT COMMENT 'Foreign key linking to billing.invoice. Business justification: Journal entry lines for revenue posting, write-offs, and adjustments reference specific invoices for audit trail, invoice-level GL reconciliation, and regulatory reporting. Utility auditors require tr',
    ...
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to project.wbs_element. Business justification: In SAP/Tyler Munis implementations for water utilities, journal entry lines carry WBS element assignments for project cost postings. This enables budget vs actual reporting at WBS level and is require',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`ap_invoice` (
    ...
    `construction_contract_id` BIGINT COMMENT 'Foreign key linking to project.construction_contract. Business justification: Construction contract payments require three-way match (contract, pay application, AP invoice) for water utilities CIP projects. Linking AP invoices to construction contracts enables contract-level in',
    ...
    `design_contract_id` BIGINT COMMENT 'Foreign key linking to project.design_contract. Business justification: Engineering firm invoices for design services are paid against design contracts in water utilities CIP programs. Linking AP invoices to design contracts enables invoiced-to-date vs contract value reco',
    ...
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to supply.goods_receipt. Business justification: Three-way match requires AP invoices to reference the goods receipt confirming delivery. ap_invoice.goods_receipt_number is a denormalized text field; a proper FK to goods_receipt enables automated re',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: AP invoices generate journal entries when posted (liability recognition). Adding journal_entry_id provides direct audit trail linkage between AP invoices and their GL postings, supporting three-way ma',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to supply.purchase_order. Business justification: Three-way match (PO/GR/Invoice) is a core AP control in water utilities. ap_invoice.purchase_order_number is a denormalized text field; a proper FK to purchase_order enables automated PO matching, enc',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`ap_payment` (
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: AP payments generate journal entries when disbursed (cash reduction, liability clearance). Adding journal_entry_id provides direct audit trail linkage between payment transactions and their GL posting',
    ...
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to supply.purchase_order. Business justification: AP payment runs in water utilities reference POs for payment authorization and encumbrance liquidation. ap_payment.purchase_order_number is a denormalized text field; a proper FK to purchase_order ena',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`ar_transaction` (
    ...
    `customer_account_id` BIGINT COMMENT 'Reference to the customer or entity responsible for payment. Links to customer master for non-utility billing customers.',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: AR transactions (connection fees, capacity charges) generate journal entries when recognized as revenue. Adding journal_entry_id provides direct audit trail linkage between AR transactions and their G',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`finance_budget` (
    ...
    `grant_id` BIGINT COMMENT 'Foreign key linking to finance.grant. Business justification: Budgets can be grant-funded, and finance_budget has grant_number as a denormalized string. Adding grant_id normalizes this reference to the grant master, enabling grant-funded budget tracking and repo',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`budget_line` (
    ...
    `grant_id` BIGINT COMMENT 'Foreign key linking to finance.grant. Business justification: Budget line items can be grant-funded, and budget_line has grant_code as a denormalized string. Adding grant_id normalizes this to the grant master, enabling granular grant-funded budget line tracking',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to project.wbs_element. Business justification: Capital budget lines in water utilities are allocated at WBS element level for granular CIP project cost control. Linking budget lines to WBS elements enables WBS-level budget vs actual variance repor',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`fixed_asset` (
    ...
    `procurement_contract_id` BIGINT COMMENT 'Foreign key linking to supply.procurement_contract. Business justification: Water utility capital assets (pumps, treatment equipment, pipelines) are frequently procured under long-term supply contracts. Linking fixed_asset to procurement_contract enables contract performance ',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to supply.purchase_order. Business justification: Capital assets in water utilities are capitalized from POs; linking fixed_asset to purchase_order enables asset capitalization workflows, capital project closeout, and GASB 34 infrastructure reporting',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor or supplier from whom the asset was purchased.',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to project.wbs_element. Business justification: Fixed assets in water utilities are capitalized from CIP projects via WBS element settlement (SAP asset accounting). The WBS element is the direct source of asset capitalization. fixed_asset already h',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`grant` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Grants are administered by specific organizational cost centers (e.g., grants management, capital projects). Adding cost_center_id links the grant to its administering cost center for SAP CO integrati',
    `fund_id` BIGINT COMMENT 'Foreign key linking to finance.fund. Business justification: Grants are managed within specific municipal funds (grant fund, capital project fund). Adding fund_id links the grant master to its associated fund for GASB fund accounting and single audit reporting.',
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: EPA, SRF, and WIFIA grants are awarded for infrastructure improvements within defined service territories. The plain-text project_location is a denormalization of territory. Grant compliance reporting',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` (
    ...
    `ap_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ap_invoice. Business justification: Grant expenditures are typically backed by AP invoices for vendor payments. invoice_number is a denormalized string reference to the AP invoice. Adding ap_invoice_id normalizes this and enables three-',
    ...
    `fund_id` BIGINT COMMENT 'Foreign key linking to finance.fund. Business justification: Grant expenditures are charged against specific municipal funds (e.g., grant fund, capital fund). Adding fund_id enables fund-level grant expenditure reporting and GASB compliance. No bidirectional co',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Grant expenditures are posted to the general ledger via journal entries. Adding journal_entry_id provides direct audit trail linkage between grant expenditure transactions and their GL postings, suppo',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to supply.purchase_order. Business justification: Grant expenditure records in water utilities must reference the originating PO for federal audit compliance (2 CFR Part 200). grant_expenditure.purchase_order_number is a denormalized text field; a pr',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Federal grant compliance (2 CFR 200 / Uniform Guidance) requires tracking which specific physical assets were funded by each grant. Linking grant_expenditure to asset registry enables grant closeout r',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor or supplier from whom goods or services were purchased, if applicable.',
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to project.wbs_element. Business justification: Federal grant reporting (SEFA, single audit) requires grant expenditures to be tracked at WBS element level. Water utilities grant administrators must report costs by WBS element to grantor agencies (',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`debt_instrument` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Debt instruments are managed by the treasury or finance cost center. Adding cost_center_id enables cost center-level debt portfolio reporting and SAP CO integration for debt management overhead. No bi',
    `fund_id` BIGINT COMMENT 'Foreign key linking to finance.fund. Business justification: Debt instruments (revenue bonds, GO bonds) are issued for and repaid from specific municipal funds (debt service fund, enterprise fund). Adding fund_id links the debt instrument to its associated fund',
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: Revenue bonds and debt instruments are secured by revenues from specific service territories. Bond covenants reference the revenue base of the territory. Debt capacity analysis, coverage ratio calcula',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` (
    ...
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to finance.bank_account. Business justification: Debt service payments are disbursed from specific treasury bank accounts. Adding bank_account_id normalizes the bank account reference and removes the denormalized bank_account_number string field. No',
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Debt service payments are associated with treasury or finance cost centers for SAP CO cost accounting. Adding cost_center_id enables cost center reporting on debt service expenditures. No bidirectiona',
    ...
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Debt service payments post to specific GL accounts (interest expense, principal reduction). Adding general_ledger_id links the payment to the authoritative GL account master. gl_document_number is a j',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Debt service payments generate journal entries when posted. gl_document_number is a denormalized reference to the journal entry document number. Adding journal_entry_id normalizes this and enables dir',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`rate_case` (
    `rate_case_id` BIGINT COMMENT 'Unique identifier for the rate case proceeding. Primary key for the rate case master record.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Rate cases are managed by specific regulatory affairs or finance cost centers. Adding cost_center_id enables cost center-level rate case cost tracking and SAP CO integration. No bidirectional conflict',
    `fund_id` BIGINT COMMENT 'Foreign key linking to finance.fund. Business justification: Rate cases are filed for specific utility funds (water enterprise fund, wastewater fund). Adding fund_id links the rate case to the applicable municipal fund, enabling fund-level rate case tracking an',
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: Rate cases are filed for specific service territories with the applicable regulatory authority. Multi-territory utilities manage separate rate cases per territory. Regulatory docket tracking, public n',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` (
    ...
    `debt_instrument_id` BIGINT COMMENT 'Foreign key linking to finance.debt_instrument. Business justification: Revenue requirements include debt service components tied to specific debt instruments (bonds, loans). Adding debt_instrument_id directly links the revenue requirement to the debt instrument driving t',
    ...
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Revenue requirements reference specific GL accounts for revenue and expense categorization in rate case preparation. Adding general_ledger_id links the revenue requirement calculation to the authorita',
    `grant_id` BIGINT COMMENT 'Foreign key linking to finance.grant. Business justification: Revenue requirements include grant_funding_amount as a revenue offset. Adding grant_id normalizes this to the grant master, enabling direct linkage between grant funding and revenue requirement calcul',
    `rate_case_id` BIGINT COMMENT 'Foreign key linking to finance.rate_case. Business justification: Revenue requirements support rate cases. The docket_number STRING should be replaced with FK to rate_case.finance_rate_case_id (note: rate_case PK is finance_rate_case_id, not rate_case_id).',
    `service_class_id` BIGINT COMMENT 'Foreign key linking to service.service_class. Business justification: Revenue requirements are allocated by customer class in cost-of-service studies — a core rate case deliverable. The plain-text customer_class column is a denormalization of service_class. A proper FK ',
    `tariff_id` BIGINT COMMENT 'Foreign key linking to service.tariff. Business justification: Approved revenue requirements are implemented via specific tariffs filed with regulators. Rate case proceedings produce a revenue requirement that directly determines tariff rates. Water utility rate ',
    `territory_id` BIGINT COMMENT 'Foreign key linking to service.territory. Business justification: Revenue requirements are calculated per service territory — the regulatory unit for rate filings. Multi-territory utilities file separate revenue requirements per territory. Regulators and rate analys',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`bank_account` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Bank accounts are managed by specific treasury or finance cost centers. Adding cost_center_id enables cost center-level bank account management reporting and SAP CO integration for treasury overhead. ',
...
CREATE OR REPLACE TABLE `water_utilities_ecm`.`finance`.`payment_run` (
    ...
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to finance.bank_account. Business justification: Payment runs are executed from specific bank accounts (operating account, payroll account). Adding bank_account_id links the payment run to the disbursement bank account, enabling bank reconciliation ',
    `cost_center_id` BIGINT COMMENT 'FK to finance.cost_center',
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `water_utilities_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_payment` ADD CONSTRAINT `fk_finance_ap_payment_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `water_utilities_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ar_transaction` ADD CONSTRAINT `fk_finance_ar_transaction_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `water_utilities_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`finance_budget` ADD CONSTRAINT `fk_finance_finance_budget_grant_id` FOREIGN KEY (`grant_id`) REFERENCES `water_utilities_ecm`.`finance`.`grant`(`grant_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`budget_line` ADD CONSTRAINT `fk_finance_budget_line_grant_id` FOREIGN KEY (`grant_id`) REFERENCES `water_utilities_ecm`.`finance`.`grant`(`grant_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant` ADD CONSTRAINT `fk_finance_grant_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `water_utilities_ecm`.`finance`.`cost_center`(`cost_center_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`grant` ADD CONSTRAINT `fk_finance_grant_fund_id` FOREIGN KEY (`fund_id`) REFERENCES `water_utilities_ecm`.`finance`.`fund`(`fund_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ADD CONSTRAINT `fk_finance_grant_expenditure_ap_invoice_id` FOREIGN KEY (`ap_invoice_id`) REFERENCES `water_utilities_ecm`.`finance`.`ap_invoice`(`ap_invoice_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ADD CONSTRAINT `fk_finance_grant_expenditure_fund_id` FOREIGN KEY (`fund_id`) REFERENCES `water_utilities_ecm`.`finance`.`fund`(`fund_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ADD CONSTRAINT `fk_finance_grant_expenditure_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `water_utilities_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_instrument` ADD CONSTRAINT `fk_finance_debt_instrument_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `water_utilities_ecm`.`finance`.`cost_center`(`cost_center_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_instrument` ADD CONSTRAINT `fk_finance_debt_instrument_fund_id` FOREIGN KEY (`fund_id`) REFERENCES `water_utilities_ecm`.`finance`.`fund`(`fund_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ADD CONSTRAINT `fk_finance_debt_service_payment_bank_account_id` FOREIGN KEY (`bank_account_id`) REFERENCES `water_utilities_ecm`.`finance`.`bank_account`(`bank_account_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ADD CONSTRAINT `fk_finance_debt_service_payment_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `water_utilities_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ADD CONSTRAINT `fk_finance_debt_service_payment_general_ledger_id` FOREIGN KEY (`general_ledger_id`) REFERENCES `water_utilities_ecm`.`finance`.`general_ledger`(`general_ledger_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ADD CONSTRAINT `fk_finance_debt_service_payment_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `water_utilities_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ADD CONSTRAINT `fk_finance_rate_case_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `water_utilities_ecm`.`finance`.`cost_center`(`cost_center_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ADD CONSTRAINT `fk_finance_rate_case_fund_id` FOREIGN KEY (`fund_id`) REFERENCES `water_utilities_ecm`.`finance`.`fund`(`fund_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ADD CONSTRAINT `fk_finance_revenue_requirement_debt_instrument_id` FOREIGN KEY (`debt_instrument_id`) REFERENCES `water_utilities_ecm`.`finance`.`debt_instrument`(`debt_instrument_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ADD CONSTRAINT `fk_finance_revenue_requirement_general_ledger_id` FOREIGN KEY (`general_ledger_id`) REFERENCES `water_utilities_ecm`.`finance`.`general_ledger`(`general_ledger_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ADD CONSTRAINT `fk_finance_revenue_requirement_grant_id` FOREIGN KEY (`grant_id`) REFERENCES `water_utilities_ecm`.`finance`.`grant`(`grant_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ADD CONSTRAINT `fk_finance_revenue_requirement_rate_case_id` FOREIGN KEY (`rate_case_id`) REFERENCES `water_utilities_ecm`.`finance`.`rate_case`(`rate_case_id`);
ALTER TABLE `water_utilities_ecm`.`finance`.`bank_account` ADD CONSTRAINT `fk_finance_bank_account_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `water_utilities_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`payment_run` ADD CONSTRAINT `fk_finance_payment_run_bank_account_id` FOREIGN KEY (`bank_account_id`) REFERENCES `water_utilities_ecm`.`finance`.`bank_account`(`bank_account_id`);
...
ALTER TABLE `water_utilities_ecm`.`finance`.`general_ledger` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`fund` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`cost_center` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`journal_entry` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`journal_entry` ALTER COLUMN `billing_account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`journal_entry_line` SET TAGS ('dbx_subdomain' = 'ledger_accounting');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` SET TAGS ('dbx_subdomain' = 'payables_receivables');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` ALTER COLUMN `construction_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Construction Contract Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` ALTER COLUMN `design_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Design Contract Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Goods Receipt Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_invoice` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_payment` SET TAGS ('dbx_subdomain' = 'payables_receivables');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_payment` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ap_payment` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ar_transaction` SET TAGS ('dbx_subdomain' = 'payables_receivables');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ar_transaction` ALTER COLUMN `customer_account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`ar_transaction` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`finance_budget` ALTER COLUMN `grant_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`budget_line` ALTER COLUMN `grant_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`budget_line` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`fixed_asset` SET TAGS ('dbx_subdomain' = 'budget_planning');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`fixed_asset` ALTER COLUMN `facility_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`fixed_asset` ALTER COLUMN `procurement_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Contract Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`fixed_asset` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`fixed_asset` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
ALTER TABLE `water_utilities_ecm`.`finance`.`fixed_asset` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant` SET TAGS ('dbx_subdomain' = 'budget_planning');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`grant` ALTER COLUMN `fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`grant` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` SET TAGS ('dbx_subdomain' = 'budget_planning');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ALTER COLUMN `fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`grant_expenditure` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_instrument` SET TAGS ('dbx_subdomain' = 'capital_funding');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_instrument` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_instrument` ALTER COLUMN `fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_instrument` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` SET TAGS ('dbx_subdomain' = 'capital_funding');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`debt_service_payment` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` SET TAGS ('dbx_subdomain' = 'capital_funding');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `rate_case_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Identifier (ID)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `fund_id` SET TAGS ('dbx_business_glossary_term' = 'Fund Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `allowed_rate_of_return` SET TAGS ('dbx_business_glossary_term' = 'Allowed Rate of Return (ROR)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `allowed_return_on_equity` SET TAGS ('dbx_business_glossary_term' = 'Allowed Return on Equity (ROE)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `appeal_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Flag');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `approved_revenue_requirement` SET TAGS ('dbx_business_glossary_term' = 'Approved Revenue Requirement');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `capital_improvement_program_amount` SET TAGS ('dbx_business_glossary_term' = 'Capital Improvement Program (CIP) Amount');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `case_status` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Status');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `case_type` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Type');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `case_type` SET TAGS ('dbx_value_regex' = 'general_rate_case|interim_rate_adjustment|surcharge|cost_of_service|rate_design|emergency_rate');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `commercial_rate_impact_percentage` SET TAGS ('dbx_business_glossary_term' = 'Commercial Rate Impact Percentage');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `consultant` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Consultant');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `customer_class_allocation_method` SET TAGS ('dbx_business_glossary_term' = 'Customer Class Allocation Method');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `customer_class_allocation_method` SET TAGS ('dbx_value_regex' = 'base_extra_capacity|commodity_demand|customer_count|meter_equivalent|peak_demand');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `debt_service_amount` SET TAGS ('dbx_business_glossary_term' = 'Debt Service Amount');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `decision_date` SET TAGS ('dbx_business_glossary_term' = 'Commission Decision Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `decision_document_url` SET TAGS ('dbx_business_glossary_term' = 'Decision Document Uniform Resource Locator (URL)');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `depreciation_expense_amount` SET TAGS ('dbx_business_glossary_term' = 'Depreciation Expense Amount');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `docket_number` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Docket Number');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `docket_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]+$');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `filing_date` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Filing Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `gasb_compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Governmental Accounting Standards Board (GASB) Compliance Notes');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `hearing_date` SET TAGS ('dbx_business_glossary_term' = 'Public Hearing Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `industrial_rate_impact_percentage` SET TAGS ('dbx_business_glossary_term' = 'Industrial Rate Impact Percentage');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `intervenor_count` SET TAGS ('dbx_business_glossary_term' = 'Intervenor Count');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `lead_attorney` SET TAGS ('dbx_business_glossary_term' = 'Lead Attorney');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `next_rate_case_anticipated_date` SET TAGS ('dbx_business_glossary_term' = 'Next Rate Case Anticipated Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Notes');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `operating_expense_amount` SET TAGS ('dbx_business_glossary_term' = 'Operating Expense (OPEX) Amount');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `public_notice_date` SET TAGS ('dbx_business_glossary_term' = 'Public Notice Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `rate_base_amount` SET TAGS ('dbx_business_glossary_term' = 'Rate Base Amount');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `rate_design_methodology` SET TAGS ('dbx_business_glossary_term' = 'Rate Design Methodology');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `rate_design_methodology` SET TAGS ('dbx_value_regex' = 'cost_of_service|value_of_service|marginal_cost|tiered_conservation|uniform_rate|lifeline');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `rate_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Rate Effective Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `regulatory_lag_days` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Lag Days');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `requested_revenue_requirement` SET TAGS ('dbx_business_glossary_term' = 'Requested Revenue Requirement');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `residential_rate_impact_percentage` SET TAGS ('dbx_business_glossary_term' = 'Residential Rate Impact Percentage');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `revenue_increase_amount` SET TAGS ('dbx_business_glossary_term' = 'Revenue Increase Amount');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `revenue_increase_percentage` SET TAGS ('dbx_business_glossary_term' = 'Revenue Increase Percentage');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `settlement_agreement_flag` SET TAGS ('dbx_business_glossary_term' = 'Settlement Agreement Flag');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `test_year_end_date` SET TAGS ('dbx_business_glossary_term' = 'Test Year End Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `test_year_start_date` SET TAGS ('dbx_business_glossary_term' = 'Test Year Start Date');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `test_year_type` SET TAGS ('dbx_business_glossary_term' = 'Test Year Type');
ALTER TABLE `water_utilities_ecm`.`finance`.`rate_case` ALTER COLUMN `test_year_type` SET TAGS ('dbx_value_regex' = 'historical|forecasted|hybrid');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` SET TAGS ('dbx_subdomain' = 'capital_funding');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `debt_instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Debt Instrument Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `grant_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `rate_case_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `service_class_id` SET TAGS ('dbx_business_glossary_term' = 'Service Class Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `tariff_id` SET TAGS ('dbx_business_glossary_term' = 'Tariff Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`revenue_requirement` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`bank_account` SET TAGS ('dbx_subdomain' = 'payables_receivables');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`bank_account` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`payment_run` SET TAGS ('dbx_subdomain' = 'payables_receivables');
...
ALTER TABLE `water_utilities_ecm`.`finance`.`payment_run` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `water_utilities_ecm`.`finance`.`payment_run` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `water_utilities_ecm`.`finance`.`payment_run` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
