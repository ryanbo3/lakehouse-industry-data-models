-- Schema for Domain: finance | Business: Shipping Ports | Version: v1_mvm
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`internal_order` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: SAP CO internal orders settle their accumulated costs to a GL account (or cost centre/asset) at period end. The internal_order table has settlement_profile, settlement_receiver_reference, and settleme',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`journal_entry` (
    ...
    `internal_order_id` BIGINT COMMENT 'Foreign key linking to finance.internal_order. Business justification: journal_entry has a string field `internal_order` that is a denormalized reference to the SAP CO internal order. Replacing this with a proper FK internal_order_id → internal_order.internal_order_id no',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`journal_entry_line` (
    ...
    `invoice_line_id` BIGINT COMMENT 'Foreign key linking to billing.invoice_line. Business justification: Journal entry lines record detailed GL postings; linking to invoice_line supports line-level audit trail for revenue recognition and SAP FI-AR sub-ledger reconciliation. Port finance auditors require ',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`ap_invoice` (
    ...
    `haulier_id` BIGINT COMMENT 'Foreign key linking to intermodal.haulier. Business justification: Port AP teams process invoices from hauliers for drayage and container transport services. Linking ap_invoice to haulier enables vendor invoice reconciliation, payment terms management, and haulier sp',
    ...
    `rail_operator_id` BIGINT COMMENT 'Foreign key linking to intermodal.rail_operator. Business justification: Port AP teams process invoices from rail operators for intermodal rail services (track access, locomotive hire, wagon handling). Linking ap_invoice to rail_operator enables rail operator spend trackin',
    `service_code_id` BIGINT COMMENT 'Foreign key linking to masterdata.service_code. Business justification: AP invoices for port services (pilotage, towage, waste reception) should reference the service code to enable cost-by-service analysis and GL account derivation. Enables direct service-level AP spend ',
    `vessel_master_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_master. Business justification: Port AP invoices for pilotage, towage, berth dues, and waste reception are raised per vessel call. Linking ap_invoice to vessel_master enables vessel-level cost analysis and reconciliation with port d',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`receivable` (
    ...
    `payment_id` BIGINT COMMENT 'Foreign key linking to billing.payment. Business justification: Receivables are cleared by customer payments; linking receivable to the clearing payment supports AR aging accuracy, cash application reporting, and SAP FI-AR open item management. Port AR teams requi',
    `commodity_code_id` BIGINT COMMENT 'Foreign key linking to masterdata.commodity_code. Business justification: Port receivables for cargo-handling charges (storage, handling, fumigation) are commodity-specific — tariffs vary by commodity type. Linking receivable to commodity_code enables commodity-level revenu',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Every AR receivable in SAP FI belongs to a specific company code (legal entity). The receivable table is missing a company_code_id FK despite all other financial posting tables (journal_entry, ap_invo',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: AR receivables in SAP FI are posted as journal entries (debit AR subledger, credit revenue). The receivable table has document_number and reference strings but no FK to journal_entry. Adding journal_e',
    ...
    `shipping_line_id` BIGINT COMMENT 'Foreign key linking to masterdata.shipping_line. Business justification: Port receivables are owed by shipping lines as the commercial operator responsible for port dues. Linking enables shipping-line-level AR aging, credit limit monitoring, and commercial account reconcil',
    `vessel_master_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_master. Business justification: Port receivables (berth dues, port dues, pilotage fees) are charged per vessel call. Linking receivable to vessel_master enables vessel-level revenue tracking, debtor aging by vessel, and regulatory p',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` (
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: SAP CO cost allocation cycles post journal entries to the GL when executed. The cost_allocation table has document_number as a string but no FK to journal_entry. Adding journal_entry_id links each all',
    `port_location_id` BIGINT COMMENT 'Foreign key linking to masterdata.port_location. Business justification: Cost allocations distribute overhead across port terminals and locations. Direct FK to port_location enables location-based cost allocation reporting and terminal profitability analysis without joinin',
    ...
    `internal_order_id` BIGINT COMMENT 'Foreign key linking to finance.internal_order. Business justification: In SAP CO, cost allocation cycles can settle costs to internal orders as receivers (in addition to cost centres and profit centres). The cost_allocation table currently only models profit centre recei',
    ...
    `service_code_id` BIGINT COMMENT 'Foreign key linking to masterdata.service_code. Business justification: Cost allocations in port finance distribute overhead across service lines (stevedoring, pilotage, terminal handling). Linking cost_allocation to service_code enables service-level profitability analys',
    `terminal_id` BIGINT COMMENT 'Foreign key linking to terminal.terminal. Business justification: Cost allocations reference terminal_code as a denormalized plain attribute. Replacing with a proper FK to terminal supports terminal-level cost allocation reporting and eliminates the denormalized ter',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`fixed_asset` (
    ...
    `internal_order_id` BIGINT COMMENT 'Foreign key linking to finance.internal_order. Business justification: Fixed assets at port authorities are typically created from capital appropriation requests (CARs) tracked as SAP CO internal orders. The fixed_asset table has no FK to internal_order, losing the trace',
    ...
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: In SAP FI-AA, fixed assets are assigned to profit centres for segment reporting and EBITDA analysis by business line (e.g., container terminal, bulk cargo, passenger). The fixed_asset table has cost_c',
    `zone_id` BIGINT COMMENT 'Foreign key linking to security.zone. Business justification: Port fixed assets (CCTV systems, access gates, perimeter fencing, barriers) are physically located within security zones. FSP compliance and zone-level security assessments require knowing which asset',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`asset_transaction` (
    ...
    `ap_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ap_invoice. Business justification: Asset acquisitions at port authorities are typically triggered by AP invoices (vendor invoices for equipment, infrastructure, etc.). The asset_transaction table has originating_document_number and ori',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`accrual` (
    ...
    `internal_order_id` BIGINT COMMENT 'Foreign key linking to finance.internal_order. Business justification: Accruals in SAP FI/CO are frequently associated with internal orders — particularly CAPEX accruals, project-phase accruals, and capital appropriation requests. The accrual table currently has no FK to',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Period-end accruals generate GL journal entries when posted in SAP FI. The accrual table has document_number and reference_document_number as string fields but no FK to journal_entry. Adding journal_e',
    ...
    `shipping_line_id` BIGINT COMMENT 'Foreign key linking to masterdata.shipping_line. Business justification: Period-end accruals for port dues and vessel services are raised against specific shipping lines. Linking enables shipping-line-level accrual reporting and reconciliation with commercial accounts — es',
    `vessel_master_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_master. Business justification: Period-end accruals for vessel services rendered but not yet invoiced (pilotage, towage, berth dues) must reference the vessel. Linking accrual to vessel_master enables accurate vessel-level accrual t',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` (
    `ap_invoice_clearing_id` BIGINT COMMENT 'Primary key for the ap_invoice_clearing association',
    `ap_invoice_id` BIGINT COMMENT 'Foreign key linking to the AP invoice being cleared by the payment',
    `ap_payment_id` BIGINT COMMENT 'Foreign key linking to the AP payment that was applied to clear the invoice',
    `applied_amount` DECIMAL(18,2) COMMENT 'The portion of the payment amount applied to this specific invoice in the clearing event. May be less than the full invoice amount for partial settlements or less than the full payment amount when one payment clears multiple invoices.',
    `clearing_date` DATE COMMENT 'The date on which this specific invoice-payment pairing was cleared in the SAP FI-AP system. May differ from the payment bank clearing date when invoices are cleared in a subsequent posting step.',
    `clearing_document_number` STRING COMMENT 'The financial document number generated during the clearing process that links the invoice to its payment transaction. [Moved from ap_invoice: clearing_document_number on ap_invoice is the SAP document number for the clearing event. In a multi-payment scenario, an invoice may have multiple clearing documents. This attribute belongs to each individual clearing record in the association, not to the invoice header.]',
    `discount_taken` DECIMAL(18,2) COMMENT 'The early payment discount amount captured at the individual invoice clearing level. Represents the cash discount deducted from this specific invoice as part of this clearing event.',
    `payment_reference` STRING COMMENT 'The SAP clearing document reference number that links this specific payment application to the invoice. Used for AP reconciliation and audit trail purposes.',
    `withholding_tax_applied` DECIMAL(18,2) COMMENT 'The withholding tax amount deducted at source for this specific invoice clearing event. Applicable for vendor payments subject to withholding tax obligations under port authority tax compliance requirements.',
    CONSTRAINT pk_ap_invoice_clearing PRIMARY KEY(`ap_invoice_clearing_id`)
) COMMENT 'This association product represents the Invoice Clearing event between ap_payment and ap_invoice in the SAP FI-AP module. It captures the operational act of applying a specific payment (or portion thereof) to a specific invoice, recording the exact amount applied, discount taken, withholding tax, and clearing reference. Each record links one ap_payment to one ap_invoice with attributes that exist only in the context of this clearing relationship — enabling full payment application traceability, partial settlement tracking, and AP reconciliation for port authority vendor obligations.. Existence Justification: In SAP FI-AP, the payment-to-invoice clearing relationship is a well-established operational concept called invoice clearing or payment application. A single payment run can clear multiple invoices (confirmed by ap_payment.invoice_count), and a single invoice can be partially settled across multiple payments (installment payments, partial settlements common in large port CAPEX procurement). This is not an analytical correlation — AP clerks actively create, manage, and reconcile these clearing records as part of the daily payment run process.';
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`internal_order` ADD CONSTRAINT `fk_finance_internal_order_gl_account_id` FOREIGN KEY (`gl_account_id`) REFERENCES `shipping_ports_ecm`.`finance`.`gl_account`(`gl_account_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`journal_entry` ADD CONSTRAINT `fk_finance_journal_entry_internal_order_id` FOREIGN KEY (`internal_order_id`) REFERENCES `shipping_ports_ecm`.`finance`.`internal_order`(`internal_order_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ADD CONSTRAINT `fk_finance_receivable_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `shipping_ports_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ADD CONSTRAINT `fk_finance_receivable_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `shipping_ports_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ADD CONSTRAINT `fk_finance_cost_allocation_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `shipping_ports_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ADD CONSTRAINT `fk_finance_cost_allocation_internal_order_id` FOREIGN KEY (`internal_order_id`) REFERENCES `shipping_ports_ecm`.`finance`.`internal_order`(`internal_order_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`fixed_asset` ADD CONSTRAINT `fk_finance_fixed_asset_internal_order_id` FOREIGN KEY (`internal_order_id`) REFERENCES `shipping_ports_ecm`.`finance`.`internal_order`(`internal_order_id`);
ALTER TABLE `shipping_ports_ecm`.`finance`.`fixed_asset` ADD CONSTRAINT `fk_finance_fixed_asset_profit_centre_id` FOREIGN KEY (`profit_centre_id`) REFERENCES `shipping_ports_ecm`.`finance`.`profit_centre`(`profit_centre_id`);
ALTER TABLE `shipping_ports_ecm`.`finance`.`asset_transaction` ADD CONSTRAINT `fk_finance_asset_transaction_ap_invoice_id` FOREIGN KEY (`ap_invoice_id`) REFERENCES `shipping_ports_ecm`.`finance`.`ap_invoice`(`ap_invoice_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` ADD CONSTRAINT `fk_finance_accrual_internal_order_id` FOREIGN KEY (`internal_order_id`) REFERENCES `shipping_ports_ecm`.`finance`.`internal_order`(`internal_order_id`);
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` ADD CONSTRAINT `fk_finance_accrual_journal_entry_id` FOREIGN KEY (`journal_entry_id`) REFERENCES `shipping_ports_ecm`.`finance`.`journal_entry`(`journal_entry_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ADD CONSTRAINT `fk_finance_ap_invoice_clearing_ap_invoice_id` FOREIGN KEY (`ap_invoice_id`) REFERENCES `shipping_ports_ecm`.`finance`.`ap_invoice`(`ap_invoice_id`);
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ADD CONSTRAINT `fk_finance_ap_invoice_clearing_ap_payment_id` FOREIGN KEY (`ap_payment_id`) REFERENCES `shipping_ports_ecm`.`finance`.`ap_payment`(`ap_payment_id`);
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`gl_account` SET TAGS ('dbx_subdomain' = 'ledger_accounts');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_centre` SET TAGS ('dbx_subdomain' = 'cost_control');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`profit_centre` SET TAGS ('dbx_subdomain' = 'cost_control');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`internal_order` SET TAGS ('dbx_subdomain' = 'cost_control');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`internal_order` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Settlement Gl Account Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`journal_entry` SET TAGS ('dbx_subdomain' = 'ledger_accounts');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`journal_entry` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`journal_entry_line` SET TAGS ('dbx_subdomain' = 'ledger_accounts');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `invoice_line_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Line Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice` SET TAGS ('dbx_subdomain' = 'payables_settlement');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice` ALTER COLUMN `haulier_id` SET TAGS ('dbx_business_glossary_term' = 'Haulier Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice` ALTER COLUMN `rail_operator_id` SET TAGS ('dbx_business_glossary_term' = 'Rail Operator Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice` ALTER COLUMN `service_code_id` SET TAGS ('dbx_business_glossary_term' = 'Service Code Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice` ALTER COLUMN `vessel_master_id` SET TAGS ('dbx_business_glossary_term' = 'Vessel Master Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_payment` SET TAGS ('dbx_subdomain' = 'payables_settlement');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` SET TAGS ('dbx_subdomain' = 'payables_settlement');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ALTER COLUMN `payment_id` SET TAGS ('dbx_business_glossary_term' = 'Clearing Payment Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ALTER COLUMN `commodity_code_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Code Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ALTER COLUMN `shipping_line_id` SET TAGS ('dbx_business_glossary_term' = 'Shipping Line Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`receivable` ALTER COLUMN `vessel_master_id` SET TAGS ('dbx_business_glossary_term' = 'Vessel Master Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` SET TAGS ('dbx_subdomain' = 'cost_control');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ALTER COLUMN `port_location_id` SET TAGS ('dbx_business_glossary_term' = 'Port Location Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Receiver Internal Order Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ALTER COLUMN `service_code_id` SET TAGS ('dbx_business_glossary_term' = 'Service Code Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`cost_allocation` ALTER COLUMN `terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`fixed_asset` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`fixed_asset` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`fixed_asset` ALTER COLUMN `zone_id` SET TAGS ('dbx_business_glossary_term' = 'Zone Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`asset_transaction` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`company_code` SET TAGS ('dbx_subdomain' = 'ledger_accounts');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` SET TAGS ('dbx_subdomain' = 'ledger_accounts');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` ALTER COLUMN `shipping_line_id` SET TAGS ('dbx_business_glossary_term' = 'Shipping Line Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`finance`.`accrual` ALTER COLUMN `vessel_master_id` SET TAGS ('dbx_business_glossary_term' = 'Vessel Master Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` SET TAGS ('dbx_subdomain' = 'payables_settlement');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` SET TAGS ('dbx_association_edges' = 'finance.ap_payment,finance.ap_invoice');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `ap_invoice_clearing_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Clearing - Ap Invoice Clearing Id');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Clearing - Ap Invoice Id');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `ap_payment_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Clearing - Ap Payment Id');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `applied_amount` SET TAGS ('dbx_business_glossary_term' = 'Applied Amount');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `applied_amount` SET TAGS ('dbx_financial_amount' = 'true');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `clearing_date` SET TAGS ('dbx_business_glossary_term' = 'Clearing Date');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `clearing_document_number` SET TAGS ('dbx_business_glossary_term' = 'Clearing Document Number');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `discount_taken` SET TAGS ('dbx_business_glossary_term' = 'Discount Taken');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `discount_taken` SET TAGS ('dbx_financial_amount' = 'true');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `payment_reference` SET TAGS ('dbx_business_glossary_term' = 'Payment Reference');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `withholding_tax_applied` SET TAGS ('dbx_business_glossary_term' = 'Withholding Tax Applied');
ALTER TABLE `shipping_ports_ecm`.`finance`.`ap_invoice_clearing` ALTER COLUMN `withholding_tax_applied` SET TAGS ('dbx_financial_amount' = 'true');
