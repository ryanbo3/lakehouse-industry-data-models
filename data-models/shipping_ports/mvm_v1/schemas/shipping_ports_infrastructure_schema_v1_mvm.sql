-- Schema for Domain: infrastructure | Business: Shipping Ports | Version: v1_mvm
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`berth` (
    ...
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Berths are designated for specific cargo categories (container berth, bulk berth, liquid berth, RoRo berth). Port tariff schedules, ISPS security levels, and vessel scheduling systems use berth-cargo_',
    ...
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Berths generate direct revenue (berth hire, port dues, wharfage). Berth-level revenue reporting, utilization vs. revenue analysis, and profit centre P&L for individual berths are standard in port fina',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` (
    ...
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Quay walls have permitted_cargo_types as a plain text attribute — a direct denormalization of cargo_category. Port engineers and planners use quay wall cargo designation for structural load planning, ',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Quay wall maintenance (fender replacement, cathodic protection, painting) generates ongoing opex tracked per quay wall cost centre. Already has fixed_asset_id for capital tracking; direct cost_centre_',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Quay walls are structurally designed to accommodate specific vessel types, determining load capacity, fender energy, and bollard SWL. Structural engineers and port planners use quay wall-vessel type c',
    ...
    `infrastructure_terminal_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal. Business justification: A quay wall is a waterfront structure that physically belongs to a terminal. Adding terminal_id to quay_wall establishes the direct ownership relationship, enabling terminal-level aggregation of quay ',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` (
    ...
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Terminal zones are designated for specific cargo handling (reefer zone, hazmat zone, container yard, bulk storage). zone_type and hazmat_approved_flag hint at this. Port operators use zone-cargo_categ',
    ...
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Terminal zones represent significant capital investment (paving, reefer infrastructure, drainage, lighting). Port authorities capitalize zone infrastructure as fixed assets for depreciation, insurance',
    `infrastructure_terminal_id` BIGINT COMMENT 'Reference to the parent terminal facility that contains this zone. Links zone to its owning terminal for hierarchical port layout management.',
    ...
    `equipment_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.equipment_type. Business justification: Terminal zones are equipped with specific handling equipment types (RTG cranes, reach stackers, forklifts). handling_equipment_type is a plain text denormalization of equipment_type. Port planners use',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Terminal zones (container yard, reefer area, dangerous goods zone) generate distinct revenue streams with separate P&L accountability. Zone-level profitability reporting and revenue allocation in term',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_audit. Business justification: Bonded and customs-controlled warehouses undergo compliance audits for customs licensing, dangerous goods storage certification, and environmental standards. The warehouse has customs_license_number a',
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Warehouses are designated for specific cargo categories (bonded, hazmat, reefer, general). Customs authorities and port operators use warehouse-cargo_category assignment for cargo routing, inspection ',
    ...
    `country_id` BIGINT COMMENT 'Foreign key linking to masterdata.country. Business justification: Warehouse country determines customs bonded status jurisdiction, import/export licensing requirements, and applicable environmental regulations. country_code is a denormalized ISO code. Linking to mas',
    ...
    `equipment_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.equipment_type. Business justification: Warehouses are equipped with specific material handling equipment types (forklifts, reach stackers, conveyor systems). material_handling_equipment is a plain text denormalization of equipment_type. Wa',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Warehouses generate rental and storage revenue. Warehouse-level P&L reporting, lease revenue tracking, and profitability analysis by warehouse are standard in port financial management. Already has co',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`navigational_aid` (
    ...
    `anchorage_area_id` BIGINT COMMENT 'Foreign key linking to infrastructure.anchorage_area. Business justification: Navigational aids (buoys, beacons, lights) are deployed to mark anchorage areas and their boundaries. The navigational_aid table already has channel_id for channel-marking aids. Adding anchorage_area_',
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Navigational aid maintenance (lamp replacement, battery servicing, inspection) generates recurring opex tracked per cost centre. Already has fixed_asset_id for capital tracking; direct cost_centre_id ',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`channel` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_audit. Business justification: Navigation channels are subject to compliance audits for depth maintenance standards (IALA, IMO), environmental compliance, and navigational safety certifications. Port state authorities audit channel',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Channel maintenance costs (dredging, hydrographic surveys, buoy maintenance) are tracked against a dedicated cost centre. Annual maintenance budget control and cost-per-metre-depth reporting require t',
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Navigation channels represent significant capital investment (dredging, maintenance). Port authorities capitalize channel infrastructure as fixed assets for balance sheet reporting, depreciation, and ',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Channels are designed and maintained to accommodate a maximum vessel type class. VTS routing, pre-arrival planning, and dredging programs are driven by the design vessel type. Role-prefix max_design_',
    `port_id` BIGINT COMMENT 'Foreign key linking to infrastructure.port. Business justification: A navigable channel belongs to a ports jurisdiction. The channel table has port_location_id (cross-domain FK to masterdata.port_location) but no in-domain FK to the port master record. Adding port_id',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` (
    ...
    `port_community_participant_id` BIGINT COMMENT 'Foreign key linking to customer.port_community_participant. Business justification: Hydrographic survey contractors are registered port_community_participants. The survey_contractor_name plain-text field is a denormalization. This FK enables contractor procurement management, payment',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Hydrographic survey costs (contractor fees, equipment, data processing) are tracked against cost centres. Port authority maintenance budget management and survey cost-per-area reporting require direct',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Depth surveys are conducted to verify navigability for specific vessel classes. Survey results (declared_depth_m, dredging_required_flag) are interpreted against the design vessel types draft require',
    `quay_wall_id` BIGINT COMMENT 'Foreign key linking to infrastructure.quay_wall. Business justification: Hydrographic depth surveys are conducted at quay wall pockets to assess navigable depth alongside the structure. The depth_survey table already has berth_id, channel_id, and anchorage_area_id — quay_w',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`dredging_campaign` (
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Dredging campaigns have opex components (mobilization, environmental monitoring, disposal) tracked against cost centres separate from the capital internal order. Maintenance dredging cost management a',
    ...
    `quay_wall_id` BIGINT COMMENT 'Foreign key linking to infrastructure.quay_wall. Business justification: Dredging campaigns are frequently conducted at quay wall pockets to maintain navigable depth alongside the wall. The dredging_campaign table already has target_berth_id, target_channel_id, and target_',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Dredging campaigns are scoped to achieve depth targets for a specific vessel type class (e.g., dredge to accommodate Post-Panamax). design_depth_target_m is meaningless without the target vessel type.',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` (
    ...
    `access_point_id` BIGINT COMMENT 'Foreign key linking to security.access_point. Business justification: Port gates are the physical manifestation of security access points. Linking gate records to their corresponding access_point enables integrated gate/access management: access event logging, credentia',
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_audit. Business justification: Port gates are ISPS-regulated access control points subject to compliance audits verifying security procedures, OCR systems, and customs inspection point operations. Port security audits specifically ',
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Port gates are designated for specific cargo categories (hazmat gate, reefer gate, general cargo). hazmat_clearance_required_flag and gate_type hint at this. Customs and ISPS compliance require gates ',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Gate operations (staffing, OCR systems, RFID, weighbridge maintenance) generate significant opex tracked per gate. Terminal gate cost management, throughput-per-cost reporting, and budget allocation r',
    ...
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Port gate infrastructure (barriers, OCR cameras, RFID readers, lane equipment) is capitalized as fixed assets. Asset depreciation, insurance, and technology refresh capex planning require gate assets ',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`project` (
    ...
    `anchorage_area_id` BIGINT COMMENT 'Foreign key linking to infrastructure.anchorage_area. Business justification: Infrastructure projects can involve development or expansion of anchorage areas (new anchorage designation, seabed preparation, mooring installation). Adding anchorage_area_id to project enables direc',
    `channel_id` BIGINT COMMENT 'Foreign key linking to infrastructure.channel. Business justification: Infrastructure projects frequently target channel deepening, widening, or realignment. The project table has channel_depth_m as an attribute (indicating channel-related projects are common) but no cha',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Infrastructure projects (berth deepening, channel widening, terminal expansion) are scoped to accommodate a target vessel type. design_vessel_dwt and design_vessel_loa_m are numeric parameters derived',
    ...
    `infrastructure_terminal_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal. Business justification: CAPEX infrastructure projects are typically scoped to a specific terminal (terminal expansion, berth construction, yard paving). The project table has no terminal_id, limiting the ability to query all',
    ...
    `port_id` BIGINT COMMENT 'Foreign key linking to infrastructure.port. Business justification: Every CAPEX infrastructure project belongs to a port. The project table is the master record for port expansion and infrastructure development but has no direct port_id FK. Adding port_id establishes ',
    ...
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Infrastructure projects (new terminals, berth upgrades, warehouse construction) are designed to handle specific cargo categories. Port authority CAPEX approval, environmental impact assessments, and r',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` (
    ...
    `anchorage_area_id` BIGINT COMMENT 'Foreign key linking to infrastructure.anchorage_area. Business justification: Structural inspections are conducted on anchorage area infrastructure (mooring buoys, anchor chains, seabed conditions). The existing structural_inspection table already covers berth, quay_wall, wareh',
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to infrastructure.channel. Business justification: Channels have structural elements including revetments, training walls, and bank protection that require periodic structural inspection. The structural_inspection tables asset_type field supports mul',
    `port_community_participant_id` BIGINT COMMENT 'Foreign key linking to customer.port_community_participant. Business justification: Structural inspection contractors are registered port_community_participants. The inspector_company plain-text field is a denormalization. Linking to the participant enables contractor invoice reconci',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Structural inspection costs (inspector fees, specialist equipment, reports) are tracked against cost centres for maintenance budget control. Already has internal_order_id for project-level tracking; d',
    `facility_security_plan_id` BIGINT COMMENT 'Foreign key linking to security.facility_security_plan. Business justification: ISPS Facility Security Plans mandate structural security inspections of port infrastructure. Linking inspections to the governing FSP enables PFSO audit compliance tracking, FSP review cycles, and dem',
    ...
    `port_gate_id` BIGINT COMMENT 'Foreign key linking to infrastructure.port_gate. Business justification: Port gates are structural assets (gate structures, barriers, access control infrastructure) that require periodic structural inspection. The existing structural_inspection table covers berth, quay_wal',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`anchorage_area` (
    ...
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Anchorage areas are designated for specific vessel type categories with maximum size constraints. Port authorities issue anchorage assignments based on vessel type. maximum_vessel_dwt and maximum_vess',
    `port_id` BIGINT COMMENT 'Foreign key linking to infrastructure.port. Business justification: An anchorage area is designated within a ports jurisdiction and waters. The anchorage_area table has port_location_id (cross-domain FK to masterdata.port_location) but no in-domain FK to the port mas',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`closure` (
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Infrastructure closures incur direct costs (emergency repairs, diversions, notifications) and revenue losses tracked against cost centres. Financial impact reporting, insurance claims, and closure cos',
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to infrastructure.facility. Business justification: Port facilities (CFS sheds, repair yards, fuel depots) can be subject to planned or unplanned closures. The closure table is the SSOT for all infrastructure restrictions and already covers the major a',
    `infrastructure_terminal_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal. Business justification: Closures can affect an entire terminal (e.g., terminal-wide emergency, industrial action, or planned maintenance shutdown). The closure table already references terminal_zone_id for zone-level closure',
    `marsec_level_change_id` BIGINT COMMENT 'Foreign key linking to security.marsec_level_change. Business justification: MARSEC level elevations routinely trigger infrastructure closures (berths, gates, channels). Port operations teams must link closures to the MARSEC change that caused them for incident reporting, regu',
    ...
    `port_id` BIGINT COMMENT 'Foreign key linking to infrastructure.port. Business justification: Port-wide closures (force majeure, severe weather, security incidents) affect the entire port rather than a specific asset. The closure table covers individual assets (berth, channel, port_gate, etc.)',
    ...
    `security_incident_id` BIGINT COMMENT 'Foreign key linking to security.security_incident. Business justification: Security incidents (breaches, threats, suspicious activity) directly trigger infrastructure closures. Port security and operations teams must link the closure record to the causative security incident',
    ...
    `weighing_station_id` BIGINT COMMENT 'Foreign key linking to infrastructure.weighing_station. Business justification: Weighing stations (weighbridges) can be closed for calibration, maintenance, or equipment failure. The closure tables infrastructure_type field and its broad coverage of port assets (berth, channel, ',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`permit` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_audit. Business justification: Infrastructure permits (environmental, construction, operational) are subject to compliance audits verifying permit conditions are met. The permit product has audit_inspection_schedule and last_inspec',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Environmental and operational permit costs (application fees, compliance monitoring, financial security bonds) are tracked against cost centres. Regulatory compliance cost management and permit renewa',
    ...
    `port_community_participant_id` BIGINT COMMENT 'Foreign key linking to customer.port_community_participant. Business justification: Environmental, construction, and operational permits are held by registered port_community_participants (terminal operators, contractors). This FK supports permit renewal notifications, compliance mon',
    `infrastructure_terminal_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal. Business justification: Operational permits are issued for terminal operations (dangerous goods handling, customs bonded area, ISPS facility designation). These permits are terminal-specific and need to be linked to the term',
    `project_id` BIGINT COMMENT 'Foreign key linking to infrastructure.project. Business justification: Regulatory permits are obtained for specific CAPEX infrastructure projects (construction permits, environmental approvals, reclamation consents). While dredging_campaign already references permit via ',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` (
    `infrastructure_terminal_id` BIGINT COMMENT 'Primary key for terminal',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Terminals are primary cost centres in port financial management. Opex budgeting, maintenance cost tracking, and terminal-level P&L reporting all require a direct terminal-to-cost-centre assignment. Ev',
    `country_id` BIGINT COMMENT 'Foreign key linking to masterdata.country. Business justification: Terminal country is required for customs jurisdiction, free trade zone eligibility determination, and regulatory compliance reporting. country_code is a denormalized ISO code. Linking to masterdata.co',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Terminals are designed and certified for specific vessel types (container, bulk, RoRo, tanker). Port planners and vessel schedulers use terminal-vessel_type compatibility for berth assignment and pre-',
    `facility_security_plan_id` BIGINT COMMENT 'Foreign key linking to security.facility_security_plan. Business justification: ISPS Code mandates every port facility (terminal) must have an approved Facility Security Plan. This direct FK enables ISPS compliance reporting, FSP audit trails, and MARSEC level activation workflow',
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Terminals are major fixed assets on the port authority balance sheet. Asset depreciation schedules, insurance valuation, and IFRS 16 capital reporting require the terminal to be registered in the fixe',
    `port_community_participant_id` BIGINT COMMENT 'Foreign key linking to customer.port_community_participant. Business justification: Terminal operator assignment is a core business relationship used in billing, SLA management, and regulatory reporting. The terminal product carries only plain-text operator_name — a clear denormaliza',
    `shipping_line_id` BIGINT COMMENT 'Foreign key linking to masterdata.shipping_line. Business justification: Terminals are frequently operated by or dedicated to specific shipping lines (e.g., APM Terminals for Maersk). operator_name is a denormalized plain text field. Port authority commercial and regulator',
    `parent_terminal_infrastructure_terminal_id` BIGINT COMMENT 'Self-referencing FK on terminal (parent_terminal_id)',
    `port_id` BIGINT COMMENT 'Reference to the parent port facility that owns or operates this terminal.',
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Terminals specialize in cargo categories (container, bulk, liquid bulk, RoRo). terminal_type is a plain text field. Port authority tariff schedules, customs declarations, and environmental permits are',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: Terminals are distinct profit-generating business units. Management accounting, EBITDA reporting by terminal, and group financial consolidation in multi-terminal port operators require each terminal t',
    ...
    CONSTRAINT pk_infrastructure_terminal PRIMARY KEY(`infrastructure_terminal_id`)
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_audit. Business justification: Weighing stations must comply with SOLAS VGM regulations and are subject to compliance audits for calibration certification and regulatory compliance. The weighing_station has solas_vgm_compliant flag',
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Weighing station calibration, maintenance, and certification costs are tracked against cost centres. SOLAS VGM compliance cost management and maintenance budget control for weighbridge assets require ',
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Weighing stations (platform, load cells, electronics, housing) are capitalized fixed assets. Asset depreciation, insurance, and technology refresh planning under SOLAS VGM compliance requirements need',
    `infrastructure_terminal_id` BIGINT COMMENT 'Reference to the marine terminal where this weighing station is located and operates.',
    `parent_weighing_station_id` BIGINT COMMENT 'Self-referencing FK on weighing_station (parent_weighing_station_id)',
    `terminal_zone_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal_zone. Business justification: A weighing station is physically located within a terminal zone. The weighing_station table already has berth_id, terminal_id, and warehouse_id but lacks a direct terminal_zone_id. Adding this FK enab',
    ...
    `zone_id` BIGINT COMMENT 'Foreign key linking to security.zone. Business justification: SOLAS VGM weighing stations must be located within ISPS-controlled security zones; access to weighing stations is governed by zone security level. Port security teams need zone assignment for access c',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`facility` (
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Facilities (repair shops, fuel stations, passenger terminals) have dedicated cost centres for opex tracking. Facility-level budget management, maintenance cost reporting, and overhead allocation in po',
    `country_id` BIGINT COMMENT 'Foreign key linking to masterdata.country. Business justification: Facility country determines applicable regulatory framework, environmental certification requirements, and customs jurisdiction. country_code is a denormalized ISO code. Linking to masterdata.country ',
    `vessel_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.vessel_type. Business justification: Facilities (dry docks, repair yards, specialized berths) are designed for specific vessel types. Port planners use facility-vessel_type compatibility for maintenance scheduling and vessel routing. A m',
    ...
    `facility_terminal_operator_port_community_participant_id` BIGINT COMMENT 'Reference to the terminal operator or concessionaire responsible for operating this facility.',
    `fixed_asset_id` BIGINT COMMENT 'Foreign key linking to finance.fixed_asset. Business justification: Facilities are capitalized infrastructure assets on the port balance sheet. Fixed asset register maintenance, depreciation calculation, insurance valuation, and CAPEX vs. NBV reporting require each fa',
    `infrastructure_terminal_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal. Business justification: A port facility (CFS shed, repair yard, fuel depot, cold store) is physically located within a terminal. The facility table currently has capex_project_id and parent_facility_id but no direct link to ',
    `isps_facility_record_id` BIGINT COMMENT 'Foreign key linking to compliance.isps_facility_record. Business justification: The facility product has isps_compliant as a plain boolean — a denormalization of the ISPS facility record. ISPS-regulated port facilities must reference their approved ISPS facility record for securi',
    `parent_facility_id` BIGINT COMMENT 'Self-referencing FK on facility (parent_facility_id)',
    `port_id` BIGINT COMMENT 'Foreign key linking to infrastructure.port. Business justification: A facility belongs to a port. While terminal_id (being added) links facility to a terminal which links to port, some facilities may be port-level assets not associated with a specific terminal (e.g., ',
    `cargo_category_id` BIGINT COMMENT 'Foreign key linking to masterdata.cargo_category. Business justification: Facilities handle specific cargo categories (dangerous goods, liquid bulk, general cargo). Port authority licensing, environmental certification, and tariff application are structured by cargo categor',
    `terminal_zone_id` BIGINT COMMENT 'Foreign key linking to infrastructure.terminal_zone. Business justification: A facility is physically located within a specific terminal zone (e.g., a CFS shed in the import zone, a reefer facility in the reefer yard zone). Adding terminal_zone_id provides precise spatial plac',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`infrastructure`.`port` (
    ...
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Port-level shared services (pilotage, VTS, port authority overhead) are tracked against a port-level cost centre. Port authority financial reporting and overhead allocation to terminals require this d',
    `country_id` BIGINT COMMENT 'Foreign key linking to masterdata.country. Business justification: Port country is required for sanctions screening, PSC targeting factor application, MARPOL jurisdiction, and customs regulatory framework determination. country_code is a denormalized ISO code. Linkin',
    `parent_port_id` BIGINT COMMENT 'Self-referencing FK on port (parent_port_id)',
    `profit_centre_id` BIGINT COMMENT 'Foreign key linking to finance.profit_centre. Business justification: In multi-port operators, each port is a top-level profit centre for group P&L consolidation and port-level revenue reporting. Port dues, pilotage fees, and ancillary revenues are aggregated at port pr',
    `un_locode_id` BIGINT COMMENT 'Foreign key linking to masterdata.un_locode. Business justification: Ports are universally identified by UN/LOCODE in international trade, EDI messages (EDIFACT IFTMCS, COPRAR), customs declarations, and shipping manifests. port_code is a direct denormalization of un_l',
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` ADD CONSTRAINT `fk_infrastructure_quay_wall_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` ADD CONSTRAINT `fk_infrastructure_terminal_zone_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`navigational_aid` ADD CONSTRAINT `fk_infrastructure_navigational_aid_anchorage_area_id` FOREIGN KEY (`anchorage_area_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`anchorage_area`(`anchorage_area_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` ADD CONSTRAINT `fk_infrastructure_channel_port_id` FOREIGN KEY (`port_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port`(`port_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` ADD CONSTRAINT `fk_infrastructure_depth_survey_quay_wall_id` FOREIGN KEY (`quay_wall_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`quay_wall`(`quay_wall_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`dredging_campaign` ADD CONSTRAINT `fk_infrastructure_dredging_campaign_quay_wall_id` FOREIGN KEY (`quay_wall_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`quay_wall`(`quay_wall_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ADD CONSTRAINT `fk_infrastructure_project_anchorage_area_id` FOREIGN KEY (`anchorage_area_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`anchorage_area`(`anchorage_area_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ADD CONSTRAINT `fk_infrastructure_project_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`channel`(`channel_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ADD CONSTRAINT `fk_infrastructure_project_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ADD CONSTRAINT `fk_infrastructure_project_port_id` FOREIGN KEY (`port_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port`(`port_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ADD CONSTRAINT `fk_infrastructure_structural_inspection_anchorage_area_id` FOREIGN KEY (`anchorage_area_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`anchorage_area`(`anchorage_area_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ADD CONSTRAINT `fk_infrastructure_structural_inspection_channel_id` FOREIGN KEY (`channel_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`channel`(`channel_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ADD CONSTRAINT `fk_infrastructure_structural_inspection_port_gate_id` FOREIGN KEY (`port_gate_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port_gate`(`port_gate_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`anchorage_area` ADD CONSTRAINT `fk_infrastructure_anchorage_area_port_id` FOREIGN KEY (`port_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port`(`port_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ADD CONSTRAINT `fk_infrastructure_closure_facility_id` FOREIGN KEY (`facility_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`facility`(`facility_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ADD CONSTRAINT `fk_infrastructure_closure_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ADD CONSTRAINT `fk_infrastructure_closure_port_id` FOREIGN KEY (`port_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port`(`port_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ADD CONSTRAINT `fk_infrastructure_closure_weighing_station_id` FOREIGN KEY (`weighing_station_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`weighing_station`(`weighing_station_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ADD CONSTRAINT `fk_infrastructure_permit_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ADD CONSTRAINT `fk_infrastructure_permit_project_id` FOREIGN KEY (`project_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`project`(`project_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ADD CONSTRAINT `fk_infrastructure_infrastructure_terminal_parent_terminal_infrastructure_terminal_id` FOREIGN KEY (`parent_terminal_infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ADD CONSTRAINT `fk_infrastructure_infrastructure_terminal_port_id` FOREIGN KEY (`port_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port`(`port_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ADD CONSTRAINT `fk_infrastructure_weighing_station_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ADD CONSTRAINT `fk_infrastructure_weighing_station_terminal_zone_id` FOREIGN KEY (`terminal_zone_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`terminal_zone`(`terminal_zone_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ADD CONSTRAINT `fk_infrastructure_facility_infrastructure_terminal_id` FOREIGN KEY (`infrastructure_terminal_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal`(`infrastructure_terminal_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ADD CONSTRAINT `fk_infrastructure_facility_port_id` FOREIGN KEY (`port_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`port`(`port_id`);
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ADD CONSTRAINT `fk_infrastructure_facility_terminal_zone_id` FOREIGN KEY (`terminal_zone_id`) REFERENCES `shipping_ports_ecm`.`infrastructure`.`terminal_zone`(`terminal_zone_id`);
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`berth` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`berth` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Category Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`berth` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Category Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Design Vessel Type Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`quay_wall` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Category Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Identifier (ID)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` ALTER COLUMN `equipment_type_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Equipment Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`terminal_zone` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Category Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` ALTER COLUMN `equipment_type_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Equipment Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`warehouse` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`navigational_aid` SET TAGS ('dbx_subdomain' = 'waterway_navigation');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`navigational_aid` ALTER COLUMN `anchorage_area_id` SET TAGS ('dbx_business_glossary_term' = 'Anchorage Area Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`navigational_aid` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` SET TAGS ('dbx_subdomain' = 'waterway_navigation');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Max Design Vessel Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`channel` ALTER COLUMN `port_id` SET TAGS ('dbx_business_glossary_term' = 'Port Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` SET TAGS ('dbx_subdomain' = 'waterway_navigation');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` ALTER COLUMN `port_community_participant_id` SET TAGS ('dbx_business_glossary_term' = 'Contractor Port Community Participant Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Design Vessel Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`depth_survey` ALTER COLUMN `quay_wall_id` SET TAGS ('dbx_business_glossary_term' = 'Quay Wall Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`dredging_campaign` SET TAGS ('dbx_subdomain' = 'waterway_navigation');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`dredging_campaign` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`dredging_campaign` ALTER COLUMN `quay_wall_id` SET TAGS ('dbx_business_glossary_term' = 'Target Quay Wall Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`dredging_campaign` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Target Vessel Type Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` ALTER COLUMN `access_point_id` SET TAGS ('dbx_business_glossary_term' = 'Access Point Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Category Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port_gate` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` SET TAGS ('dbx_subdomain' = 'capital_development');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ALTER COLUMN `anchorage_area_id` SET TAGS ('dbx_business_glossary_term' = 'Anchorage Area Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Design Vessel Type Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ALTER COLUMN `port_id` SET TAGS ('dbx_business_glossary_term' = 'Port Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`project` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Target Cargo Category Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` SET TAGS ('dbx_subdomain' = 'capital_development');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ALTER COLUMN `anchorage_area_id` SET TAGS ('dbx_business_glossary_term' = 'Anchorage Area Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ALTER COLUMN `port_community_participant_id` SET TAGS ('dbx_business_glossary_term' = 'Contractor Port Community Participant Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ALTER COLUMN `facility_security_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Security Plan Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`structural_inspection` ALTER COLUMN `port_gate_id` SET TAGS ('dbx_business_glossary_term' = 'Port Gate Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`anchorage_area` SET TAGS ('dbx_subdomain' = 'waterway_navigation');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`anchorage_area` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Max Vessel Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`anchorage_area` ALTER COLUMN `port_id` SET TAGS ('dbx_business_glossary_term' = 'Port Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` SET TAGS ('dbx_subdomain' = 'capital_development');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `marsec_level_change_id` SET TAGS ('dbx_business_glossary_term' = 'Marsec Level Change Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `port_id` SET TAGS ('dbx_business_glossary_term' = 'Port Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `security_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Security Incident Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`closure` ALTER COLUMN `weighing_station_id` SET TAGS ('dbx_business_glossary_term' = 'Weighing Station Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` SET TAGS ('dbx_subdomain' = 'capital_development');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ALTER COLUMN `port_community_participant_id` SET TAGS ('dbx_business_glossary_term' = 'Holder Port Community Participant Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`permit` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Project Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` SET TAGS ('dbx_subdomain' = 'berth_facilities');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Identifier');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Design Vessel Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `facility_security_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Security Plan Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `port_community_participant_id` SET TAGS ('dbx_business_glossary_term' = 'Operator Port Community Participant Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `shipping_line_id` SET TAGS ('dbx_business_glossary_term' = 'Operator Shipping Line Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `parent_terminal_infrastructure_terminal_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Cargo Category Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `address_line1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `address_line2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `email_address` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `email_address` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `phone_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`infrastructure_terminal` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `terminal_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Zone Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `zone_id` SET TAGS ('dbx_business_glossary_term' = 'Zone Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`weighing_station` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `vessel_type_id` SET TAGS ('dbx_business_glossary_term' = 'Design Vessel Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `infrastructure_terminal_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `isps_facility_record_id` SET TAGS ('dbx_business_glossary_term' = 'Isps Facility Record Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `parent_facility_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `port_id` SET TAGS ('dbx_business_glossary_term' = 'Port Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `cargo_category_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Cargo Category Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `terminal_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Terminal Zone Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`facility` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` SET TAGS ('dbx_subdomain' = 'berth_facilities');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Country Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `parent_port_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `un_locode_id` SET TAGS ('dbx_business_glossary_term' = 'Un Locode Id (Foreign Key)');
...
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `shipping_ports_ecm`.`infrastructure`.`port` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
