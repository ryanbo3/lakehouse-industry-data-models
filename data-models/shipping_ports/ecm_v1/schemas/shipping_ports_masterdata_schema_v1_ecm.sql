-- Schema for Domain: masterdata | Business: Shipping Ports | Version: v1_ecm
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`masterdata`.`edi_partner` (
    ...
    `port_community_participant_id` BIGINT COMMENT 'The unique identifier assigned to this partner within the Port Community System for cross-system integration and stakeholder messaging.',
...
CREATE OR REPLACE TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` (
    `packaging_type_id` BIGINT COMMENT 'Unique identifier for the packaging type record. Primary key.',
    `imdg_class_id` BIGINT COMMENT 'Foreign key linking to masterdata.imdg_class. Business justification: Packaging types for dangerous goods reference IMDG hazard classifications to determine compatibility and handling requirements. Currently stores imdg_class_code as STRING; normalizing to FK allows joi',
    `parent_packaging_type_id` BIGINT COMMENT 'Self-referencing FK on packaging_type (parent_packaging_type_id)',
    `applicable_transport_modes` STRING COMMENT 'Transport modes for which this packaging type is approved (e.g., sea, road, rail, air, intermodal). Multiple modes may be comma-separated. Used for intermodal logistics coordination.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this packaging type record was first created in the master data system. Used for audit trail and data lineage tracking.',
    `customs_declaration_required` BOOLEAN COMMENT 'Indicates whether cargo in this packaging type requires specific customs declaration or documentation beyond standard manifest entries.',
    `data_steward` STRING COMMENT 'Name or identifier of the business unit or individual responsible for maintaining this packaging type master data. Typically Safety and Compliance department.',
    `effective_from_date` DATE COMMENT 'Date from which this packaging type classification becomes effective and can be used in operational systems. Used for regulatory update management.',
    `effective_to_date` DATE COMMENT 'Date until which this packaging type classification remains valid. Null for currently valid records. Used for managing regulatory updates and deprecated packaging codes.',
    `fumigation_compatible` BOOLEAN COMMENT 'Indicates whether this packaging type can withstand fumigation treatment for quarantine and phytosanitary compliance without damage or contamination.',
    `handling_equipment_type` STRING COMMENT 'Recommended or required handling equipment for this packaging type (e.g., forklift, pallet jack, crane, Rubber Tyred Gantry (RTG), Mobile Harbour Crane (MHC)). Multiple types may be comma-separated.',
    `height_mm` STRING COMMENT 'External height dimension of the packaging in millimeters. Used for stowage planning and container loading optimization.',
    `imdg_packing_group` STRING COMMENT 'IMDG packing group compatibility indicating the degree of danger: I (high danger), II (medium danger), III (low danger), or not applicable for non-dangerous goods packaging.. Valid values are `I|II|III|not_applicable`',
    `iso_standard_version` STRING COMMENT 'Version of the ISO standard this packaging type conforms to (e.g., ISO 3394:2012, ISO 9711:1990). Used for compliance verification and audit trails.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this packaging type record was last updated. Used for change tracking and synchronization with downstream systems.',
    `length_mm` STRING COMMENT 'External length dimension of the packaging in millimeters. Used for stowage planning and container loading optimization.',
    `marpol_compliant` BOOLEAN COMMENT 'Indicates whether this packaging type meets MARPOL requirements for prevention of marine pollution, particularly relevant for hazardous substance packaging.',
    `material_type` STRING COMMENT 'Primary material composition of the packaging (e.g., steel, aluminium, wood, fibreboard, plastic, textile, composite). Critical for IMDG compliance and handling requirements. [ENUM-REF-CANDIDATE: steel|aluminium|wood|plywood|fibreboard|plastic|textile|paper|composite|glass|other — 11 candidates stripped; promote to reference product]',
    `maximum_capacity_liters` DECIMAL(18,2) COMMENT 'Maximum volumetric capacity of the packaging in liters. Used for liquid and bulk cargo capacity planning and compliance verification.',
    `maximum_gross_weight_kg` DECIMAL(18,2) COMMENT 'Maximum allowable gross weight (packaging plus contents) in kilograms. Critical for Safe Working Load (SWL) compliance and handling equipment selection.',
    `maximum_stack_height` STRING COMMENT 'Maximum number of units that can be safely stacked vertically. Null if not stackable. Used for warehouse and yard stacking operations.',
    `notes` STRING COMMENT 'Free-text field for additional remarks, clarifications, or operational notes related to this packaging type. Used for capturing context not covered by structured fields.',
    `operational_status` STRING COMMENT 'Current operational status of this packaging type in the master data system. Controls whether the packaging type can be used in new cargo bookings and manifests.. Valid values are `active|inactive|deprecated|pending_approval|suspended`',
    `packaging_category` STRING COMMENT 'High-level classification of the packaging type for operational grouping and handling instructions. [ENUM-REF-CANDIDATE: drum|box|bag|crate|ibc|cylinder|jerrycan|pallet|composite|bulk|other — 11 candidates stripped; promote to reference product]',
    `packaging_name` STRING COMMENT 'Full descriptive name of the packaging type (e.g., Steel Drum, Fibreboard Box, Intermediate Bulk Container, Wooden Crate, Plastic Jerrycan, Gas Cylinder, Pallet).',
    `reefer_compatible` BOOLEAN COMMENT 'Indicates whether this packaging type is suitable for use in refrigerated containers for temperature-controlled cargo.',
    `regulatory_update_cycle` STRING COMMENT 'Frequency and schedule of regulatory updates for this packaging type (e.g., biennial UN update, annual IMDG amendment). Used for compliance calendar management.',
    `segregation_group` STRING COMMENT 'Segregation group code for dangerous goods stowage planning. Defines which other packaging types or cargo classes this packaging must be segregated from during transport and storage.',
    `solas_vgm_applicable` BOOLEAN COMMENT 'Indicates whether SOLAS VGM verification requirements apply to cargo shipped in this packaging type. Mandatory for containerized cargo under SOLAS Convention.',
    `source_system` STRING COMMENT 'Operational system of record from which this packaging type data originates (e.g., NAVIS N4, Port Community System (PCS), SAP Materials Management (MM)).',
    `special_handling_instructions` STRING COMMENT 'Free-text field capturing any special handling, stowage, or operational instructions specific to this packaging type (e.g., keep upright, fragile, protect from moisture, segregation requirements).',
    `stackable` BOOLEAN COMMENT 'Indicates whether this packaging type can be safely stacked during storage and transport. Critical for yard management and Container Freight Station (CFS) operations.',
    `tare_weight_kg` DECIMAL(18,2) COMMENT 'Empty weight of the packaging itself in kilograms, excluding contents. Used for net weight calculations and cargo documentation.',
    `tariff_classification_code` STRING COMMENT 'Internal tariff code used for billing and Terminal Handling Charge (THC) calculation. Links packaging type to applicable port tariff schedules.',
    `un_packaging_code` STRING COMMENT 'Standard UN packaging code as defined by UN Recommendation 21 for dangerous goods transport. Two to four character alphanumeric code identifying the packaging type (e.g., 1A for steel drum, 4G for fibreboard box).. Valid values are `^[0-9A-Z]{2,4}$`',
    `un_recommendation_version` STRING COMMENT 'Version of UN Recommendation 21 or UN Model Regulations this packaging type is classified under. Updated biennially to reflect regulatory changes.',
    `un_test_certificate_required` BOOLEAN COMMENT 'Indicates whether this packaging type requires UN performance testing and certification for dangerous goods transport under IMDG regulations.',
    `width_mm` STRING COMMENT 'External width dimension of the packaging in millimeters. Used for stowage planning and container loading optimization.',
    CONSTRAINT pk_packaging_type PRIMARY KEY(`packaging_type_id`)
) COMMENT 'Reference classification of cargo packaging types as defined by UN Recommendation 21 and used in dangerous goods declarations, cargo manifests, and customs filings. Captures UN packaging code, packaging name (drum, bag, pallet, crate, IBC, cylinder, etc.), material type, IMDG packing group compatibility (FK to imdg_class for DG-rated packaging), maximum capacity, applicable transport regulations, and effectivity dates for regulatory updates. Data steward: Safety & Compliance. SSOT for packaging classification used in cargo documentation, IMDG compliance, and customs declarations.';

CREATE OR REPLACE TABLE `shipping_ports_ecm`.`masterdata`.`resource` (
    `resource_id` BIGINT COMMENT 'Primary key for resource',
    `commodity_code_id` BIGINT COMMENT 'Foreign key linking to masterdata.commodity_code. Business justification: resource has commodity_code (STRING) which matches commodity_code.hs_code business key. When resource represents cargo or commodity assets, it should FK to commodity_code master for HS classification.',
    `container_type_id` BIGINT COMMENT 'Foreign key linking to masterdata.container_type. Business justification: resource has iso_type_code (STRING) which matches container_type.iso_type_code business key. When resource represents a container asset, it should FK to container_type master for ISO type classificati',
    `edi_partner_id` BIGINT COMMENT 'EDI partner identification code for electronic messaging and data exchange related to this resource.',
    `country_id` BIGINT COMMENT 'Foreign key linking to masterdata.country. Business justification: resource has flag_country_code (STRING) which should reference country master. When resource represents a vessel or flagged asset, the flag country should FK to country table. Using flag_country_id as',
    `port_location_id` BIGINT COMMENT 'Foreign key linking to masterdata.port_location. Business justification: resource has location_code (STRING) which matches port_location.location_code business key. Resources are physically located at port locations (berths, yards, etc.). Adding port_location_id FK normali',
    `parent_resource_id` BIGINT COMMENT 'Self-referencing FK on resource (parent_resource_id)',
    `acquisition_cost` DECIMAL(18,2) COMMENT 'Original purchase or acquisition cost of the resource in the base currency.',
    `capacity_unit` STRING COMMENT 'Unit of measure for the capacity value (TEU=Twenty-foot Equivalent Unit, FEU=Forty-foot Equivalent Unit, CBM=Cubic Meter, DWT=Deadweight Tonnage, MT=Metric Ton, SQM=Square Meter).',
    `capacity_value` DECIMAL(18,2) COMMENT 'Principal quantitative measurement of the resources capacity or capability (e.g., TEU for containers, DWT for vessels, square meters for terminals).',
    `certification_expiry_date` DATE COMMENT 'Expiration date of the resources current certification or compliance documentation.',
    `commission_date` DATE COMMENT 'Date when the resource was commissioned and put into operational service.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this resource record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for monetary values associated with the resource.',
    `current_book_value` DECIMAL(18,2) COMMENT 'Current accounting book value of the resource after depreciation.',
    `customs_bond_number` STRING COMMENT 'Customs bond or guarantee number associated with the resource for international trade compliance.',
    `resource_description` STRING COMMENT 'Detailed textual description of the resource including specifications, features, and operational characteristics.',
    `height_meters` DECIMAL(18,2) COMMENT 'Physical height dimension of the resource in meters.',
    `home_port_code` STRING COMMENT 'Primary or home port location for the resource using UN/LOCODE standard.',
    `insurance_policy_number` STRING COMMENT 'Insurance policy number covering the resource for liability, damage, or loss.',
    `is_active` BOOLEAN COMMENT 'Indicates whether this resource record is currently active and available for use in operational systems.',
    `is_hazmat_certified` BOOLEAN COMMENT 'Indicates whether the resource is certified for handling or transporting hazardous materials per IMDG Code.',
    `is_reefer_capable` BOOLEAN COMMENT 'Indicates whether the resource has refrigeration capability for temperature-controlled cargo (applicable to containers, vessels, terminals).',
    `last_inspection_date` DATE COMMENT 'Date of the most recent inspection or survey conducted on the resource.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this resource record was last updated or modified.',
    `length_meters` DECIMAL(18,2) COMMENT 'Physical length dimension of the resource in meters.',
    `manufacture_date` DATE COMMENT 'Date when the resource was manufactured or built.',
    `manufacturer` STRING COMMENT 'Name of the manufacturer or builder of the resource (applicable to equipment, containers, vessels).',
    `max_gross_weight_kg` DECIMAL(18,2) COMMENT 'Maximum allowable gross weight (including cargo) for the resource in kilograms.',
    `model_number` STRING COMMENT 'Manufacturers model or series identifier for the resource.',
    `next_inspection_date` DATE COMMENT 'Scheduled date for the next mandatory inspection or survey of the resource.',
    `notes` STRING COMMENT 'Additional free-text notes, comments, or special instructions related to the resource.',
    `operator_name` STRING COMMENT 'Name of the entity responsible for operating or managing the resource (may differ from owner).',
    `owner_name` STRING COMMENT 'Legal name of the entity that owns the resource.',
    `registration_number` STRING COMMENT 'Official registration or license number issued by regulatory authorities (e.g., vessel IMO number, equipment registration).',
    `resource_code` STRING COMMENT 'Externally-known unique business identifier for the resource used across operational systems and partner communications.',
    `resource_name` STRING COMMENT 'Human-readable name or title of the resource for display and reporting purposes.',
    `resource_subtype` STRING COMMENT 'Secondary classification providing granular categorization within the resource type (e.g., container types: 20GP, 40HC, 45HC; vessel types: feeder, panamax, post-panamax).',
    `resource_type` STRING COMMENT 'Primary classification of the resource indicating its functional category within maritime logistics operations.',
    `retirement_date` DATE COMMENT 'Date when the resource was retired from active service or decommissioned.',
    `rfid_tag_code` STRING COMMENT 'Unique RFID tag identifier attached to the resource for automated tracking and identification.',
    `serial_number` STRING COMMENT 'Unique serial number assigned by the manufacturer for individual resource identification and tracking.',
    `resource_status` STRING COMMENT 'Current lifecycle status of the resource indicating its operational availability and state.',
    `tare_weight_kg` DECIMAL(18,2) COMMENT 'Empty weight of the resource in kilograms (applicable to containers, equipment, cargo units).',
    `width_meters` DECIMAL(18,2) COMMENT 'Physical width dimension of the resource in meters.',
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ADD CONSTRAINT `fk_masterdata_packaging_type_imdg_class_id` FOREIGN KEY (`imdg_class_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`imdg_class`(`imdg_class_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ADD CONSTRAINT `fk_masterdata_packaging_type_parent_packaging_type_id` FOREIGN KEY (`parent_packaging_type_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`packaging_type`(`packaging_type_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ADD CONSTRAINT `fk_masterdata_resource_commodity_code_id` FOREIGN KEY (`commodity_code_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`commodity_code`(`commodity_code_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ADD CONSTRAINT `fk_masterdata_resource_container_type_id` FOREIGN KEY (`container_type_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`container_type`(`container_type_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ADD CONSTRAINT `fk_masterdata_resource_edi_partner_id` FOREIGN KEY (`edi_partner_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`edi_partner`(`edi_partner_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ADD CONSTRAINT `fk_masterdata_resource_country_id` FOREIGN KEY (`country_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`country`(`country_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ADD CONSTRAINT `fk_masterdata_resource_port_location_id` FOREIGN KEY (`port_location_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`port_location`(`port_location_id`);
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ADD CONSTRAINT `fk_masterdata_resource_parent_resource_id` FOREIGN KEY (`parent_resource_id`) REFERENCES `shipping_ports_ecm`.`masterdata`.`resource`(`resource_id`);
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`vessel_master` SET TAGS ('dbx_subdomain' = 'maritime_assets');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`vessel_type` SET TAGS ('dbx_subdomain' = 'maritime_assets');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`container_type` ALTER COLUMN `handling_equipment_type` SET TAGS ('dbx_business_glossary_term' = 'Handling Equipment Type Required');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`port_location` SET TAGS ('dbx_subdomain' = 'terminal_operations');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`un_locode` SET TAGS ('dbx_subdomain' = 'global_references');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`equipment_type` SET TAGS ('dbx_subdomain' = 'terminal_operations');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`country` SET TAGS ('dbx_subdomain' = 'global_references');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`edi_partner` SET TAGS ('dbx_subdomain' = 'global_references');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`edi_partner` ALTER COLUMN `port_community_participant_id` SET TAGS ('dbx_business_glossary_term' = 'Port Community System (PCS) Participant Identifier');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`flag_state` SET TAGS ('dbx_subdomain' = 'maritime_assets');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`shipping_line` SET TAGS ('dbx_subdomain' = 'maritime_assets');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`service_code` SET TAGS ('dbx_subdomain' = 'terminal_operations');
...
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` SET TAGS ('dbx_subdomain' = 'cargo_classification');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `packaging_type_id` SET TAGS ('dbx_business_glossary_term' = 'Packaging Type ID');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `imdg_class_id` SET TAGS ('dbx_business_glossary_term' = 'Imdg Class Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `parent_packaging_type_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `applicable_transport_modes` SET TAGS ('dbx_business_glossary_term' = 'Applicable Transport Modes');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `customs_declaration_required` SET TAGS ('dbx_business_glossary_term' = 'Customs Declaration Required');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `data_steward` SET TAGS ('dbx_business_glossary_term' = 'Data Steward');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `effective_from_date` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `effective_to_date` SET TAGS ('dbx_business_glossary_term' = 'Effective To Date');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `fumigation_compatible` SET TAGS ('dbx_business_glossary_term' = 'Fumigation Compatible');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `handling_equipment_type` SET TAGS ('dbx_business_glossary_term' = 'Handling Equipment Type');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `height_mm` SET TAGS ('dbx_business_glossary_term' = 'Packaging Height in Millimeters (MM)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `imdg_packing_group` SET TAGS ('dbx_business_glossary_term' = 'International Maritime Dangerous Goods (IMDG) Packing Group');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `imdg_packing_group` SET TAGS ('dbx_value_regex' = 'I|II|III|not_applicable');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `iso_standard_version` SET TAGS ('dbx_business_glossary_term' = 'International Organization for Standardization (ISO) Standard Version');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `length_mm` SET TAGS ('dbx_business_glossary_term' = 'Packaging Length in Millimeters (MM)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `marpol_compliant` SET TAGS ('dbx_business_glossary_term' = 'Marine Pollution (MARPOL) Convention Compliant');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `material_type` SET TAGS ('dbx_business_glossary_term' = 'Packaging Material Type');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `maximum_capacity_liters` SET TAGS ('dbx_business_glossary_term' = 'Maximum Capacity in Liters');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `maximum_gross_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Maximum Gross Weight in Kilograms (KG)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `maximum_stack_height` SET TAGS ('dbx_business_glossary_term' = 'Maximum Stack Height');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `operational_status` SET TAGS ('dbx_business_glossary_term' = 'Operational Status');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `operational_status` SET TAGS ('dbx_value_regex' = 'active|inactive|deprecated|pending_approval|suspended');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `packaging_category` SET TAGS ('dbx_business_glossary_term' = 'Packaging Category');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `packaging_name` SET TAGS ('dbx_business_glossary_term' = 'Packaging Type Name');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `reefer_compatible` SET TAGS ('dbx_business_glossary_term' = 'Refrigerated (Reefer) Container Compatible');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `regulatory_update_cycle` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Update Cycle');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `segregation_group` SET TAGS ('dbx_business_glossary_term' = 'Segregation Group');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `solas_vgm_applicable` SET TAGS ('dbx_business_glossary_term' = 'Safety of Life at Sea (SOLAS) Verified Gross Mass (VGM) Applicable');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `special_handling_instructions` SET TAGS ('dbx_business_glossary_term' = 'Special Handling Instructions');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `stackable` SET TAGS ('dbx_business_glossary_term' = 'Stackable Indicator');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `tare_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Tare Weight in Kilograms (KG)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `tariff_classification_code` SET TAGS ('dbx_business_glossary_term' = 'Tariff Classification Code');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `un_packaging_code` SET TAGS ('dbx_business_glossary_term' = 'United Nations (UN) Packaging Code');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `un_packaging_code` SET TAGS ('dbx_value_regex' = '^[0-9A-Z]{2,4}$');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `un_recommendation_version` SET TAGS ('dbx_business_glossary_term' = 'United Nations (UN) Recommendation Version');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `un_test_certificate_required` SET TAGS ('dbx_business_glossary_term' = 'United Nations (UN) Test Certificate Required');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`packaging_type` ALTER COLUMN `width_mm` SET TAGS ('dbx_business_glossary_term' = 'Packaging Width in Millimeters (MM)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` SET TAGS ('dbx_subdomain' = 'terminal_operations');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `resource_id` SET TAGS ('dbx_business_glossary_term' = 'Resource Identifier');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `commodity_code_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Code Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `container_type_id` SET TAGS ('dbx_business_glossary_term' = 'Container Type Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `country_id` SET TAGS ('dbx_business_glossary_term' = 'Flag Country Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `port_location_id` SET TAGS ('dbx_business_glossary_term' = 'Port Location Id (Foreign Key)');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `parent_resource_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `acquisition_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `current_book_value` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `customs_bond_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `insurance_policy_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `owner_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `rfid_tag_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `shipping_ports_ecm`.`masterdata`.`resource` ALTER COLUMN `serial_number` SET TAGS ('dbx_confidential' = 'true');
