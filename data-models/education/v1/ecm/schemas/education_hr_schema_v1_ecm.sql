-- Schema for Domain: hr | Business: Education | Version: v1_ecm
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`employee` (
    ...
    `job_profile_code` STRING COMMENT 'The Workday HCM job profile identifier that defines the standard set of responsibilities, qualifications, and compensation grade for this position type across the institution.',
    ...
    `position_number` STRING COMMENT 'The unique position control number assigned to the budgeted position the employee occupies. Links to the position control system for headcount and budget management.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`position` (
    ...
    `grant_account_id` BIGINT COMMENT 'Reference to the sponsored research grant or contract funding this position, applicable when position_type is grant_funded. Links to the Kuali Research award record for effort reporting and F&A cost allocation.',
    `job_family_id` BIGINT COMMENT 'Reference to the job family grouping that classifies this position within the institutions job architecture. Supports CUPA-HR benchmarking, compensation band alignment, and career pathway analysis.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`job_profile` (
    ...
    `job_family_id` BIGINT COMMENT 'FK to hr.job_family',
    `profile_id` BIGINT COMMENT 'Native system identifier for this job profile record in Workday HCM, used for ETL reconciliation, system-of-record traceability, and API integration with downstream HR analytics platforms.',
    ...
    `cip_code` STRING COMMENT 'CIP code aligned to the functional area or academic discipline most closely associated with this job profile, enabling linkage between workforce data and academic program structures. Used for IPEDS reporting and institutional research analytics.. Valid values are `^[0-9]{2}.[0-9]{4}$`',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`org_unit` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the employee who serves as the responsible manager or unit head for this organizational unit. Used for supervisory hierarchy navigation, approval routing, and workforce planning. Sourced from Workday HCM position management.',
    ...
    `cip_code` STRING COMMENT 'U.S. Department of Education Classification of Instructional Programs (CIP) code associated with the academic units primary discipline. Used for IPEDS completions reporting, program accreditation alignment, and STEM designation tracking. Applicable to academic units only; null for administrative and auxiliary units.. Valid values are `^[0-9]{2}.[0-9]{4}$`',
    ...
    `primary_campus_location` STRING COMMENT 'Name or code of the primary campus or physical location where this organizational unit is headquartered. Used for facilities planning, space allocation in Archibus, and multi-campus reporting. Examples: Main Campus, North Campus, Downtown Center, Online.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`employee_compensation` (
    ...
    `compensation_plan_id` BIGINT COMMENT 'Reference to the institutional compensation plan (salary structure) under which this employee is compensated. Compensation plans define pay grades, ranges, and eligibility rules.',
    ...
    `pay_grade_id` BIGINT COMMENT 'Reference to the pay grade assigned to this compensation record. Pay grades define the salary band (minimum, midpoint, maximum) applicable to the employees role.',
    ...
    `event_id` BIGINT COMMENT 'Source system business identifier for the compensation change event as recorded in Workday HCM. Used for traceability and reconciliation back to the system of record.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`benefit_enrollment` (
    ...
    `benefit_program_id` BIGINT COMMENT 'Reference to the overarching benefit program grouping under which this plan falls (e.g., Faculty Benefits Program, Staff Benefits Program). Supports CUPA-HR benchmarking segmentation.',
    ...
    `event_id` BIGINT COMMENT 'Reference to the benefit enrollment event (open enrollment period, new hire event, qualifying life event) that triggered this enrollment record.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`payroll_result` (
    ...
    `payroll_run_id` BIGINT COMMENT 'Reference to the payroll run batch that produced this result, enabling reconciliation across all employees processed in the same run.',
    ...
    `cost_center_code` STRING COMMENT 'The institutional cost center (department or organizational unit) to which this payroll expense is charged. Used for budget reconciliation, departmental expense reporting, and Oracle PeopleSoft Financials general ledger integration.',
    ...
    `fund_code` STRING COMMENT 'The accounting fund (e.g., general fund, restricted grant fund, auxiliary fund) to which this payroll expense is allocated. Required for GASB fund accounting and grant-funded payroll compliance.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`absence_event` (
    ...
    `accrual_plan_id` BIGINT COMMENT 'Reference to the Workday HCM absence accrual plan governing the employees leave entitlement rules, accrual rates, and carryover caps for this leave type.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`recruitment_requisition` (
    ...
    `budget_account_code` STRING COMMENT 'The Oracle PeopleSoft Financials chart-of-accounts code (fund, department, account) to which the salary and benefits costs for this position will be charged. Enables integration between HR recruiting and financial budgeting systems.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`hr_applicant` (
    ...
    `quaternary_hr_prior_employee_id` BIGINT COMMENT 'Reference to the applicants prior employee record if they are a former employee of the institution. Populated only when is_rehire_eligible is True. Supports rehire analytics and alumni employee tracking.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`performance_review` (
    ...
    `training_record_id` BIGINT COMMENT 'FK to hr.training_record',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`grievance` (
    `grievance_id` BIGINT COMMENT 'Unique system-generated identifier for the employee grievance or disciplinary action record. Primary key for the grievance data product.',
    `employee_id` BIGINT COMMENT 'Identifier of the employee who filed the grievance or is the subject of the disciplinary action. Links to the HR employee master record.',
    `grievance_investigator_employee_id` BIGINT COMMENT 'Identifier of the HR staff member, compliance officer, or external investigator assigned to conduct the formal investigation of the grievance.',
    `grievance_respondent_employee_id` BIGINT COMMENT 'Identifier of the employee named as the respondent or subject of the grievance (e.g., the accused party in a harassment or misconduct complaint). May differ from the filing employee.',
    `org_unit_id` BIGINT COMMENT 'Identifier of the organizational department in which the grievance or disciplinary matter originated. Used for workforce analytics and compliance reporting.',
    `ada_accommodation_involved` BOOLEAN COMMENT 'Indicates whether the grievance involves a dispute related to a requested or denied Americans with Disabilities Act (ADA) reasonable accommodation.',
    `appeal_decision_date` DATE COMMENT 'The date on which the appeal review body issued its final decision. Null if appeal is pending or not filed.',
    `appeal_filed_date` DATE COMMENT 'The date on which a formal appeal of the grievance resolution was submitted. Null if no appeal was filed.',
    `appeal_status` STRING COMMENT 'Current status of any appeal filed by the grievant or respondent following the initial resolution. Tracks whether the case has been escalated through institutional or arbitration channels.. Valid values are `not_appealed|appeal_filed|appeal_under_review|appeal_upheld|appeal_denied|arbitration_requested`',
    `closure_date` DATE COMMENT 'The date on which the grievance case was fully closed, including completion of any corrective actions, appeals, or follow-up requirements. Distinct from resolution_date when appeals extend the case.',
    `created_timestamp` TIMESTAMP COMMENT 'The timestamp when the grievance record was first created in the system. Serves as the audit trail creation marker for data lineage and compliance purposes.',
    `disciplinary_action_type` STRING COMMENT 'The specific type of disciplinary action imposed on the respondent employee as a result of the grievance investigation findings. Distinct from resolution_type which captures the broader case outcome. [ENUM-REF-CANDIDATE: promote to reference product]',
    `eeoc_charge_filed` BOOLEAN COMMENT 'Indicates whether the grievant has filed or expressed intent to file a formal charge with the Equal Employment Opportunity Commission (EEOC) in connection with this matter. Triggers legal hold and counsel notification.',
    `eeoc_charge_number` STRING COMMENT 'The official charge number assigned by the EEOC if a formal charge has been filed. Used to cross-reference internal HR records with external regulatory proceedings.',
    `external_counsel_engaged` BOOLEAN COMMENT 'Indicates whether external legal counsel has been engaged to advise on or represent the institution in connection with this grievance matter.',
    `filing_date` DATE COMMENT 'The date on which the grievance or complaint was formally filed by the employee or initiated by HR. Serves as the principal business event date for timeline and statute-of-limitations tracking.',
    `filing_method` STRING COMMENT 'The channel or method through which the grievance was submitted (e.g., written form, online HR portal, email, verbal report). Relevant for procedural compliance and documentation standards.. Valid values are `written|verbal|online_portal|email|anonymous`',
    `finding_summary` STRING COMMENT 'Narrative summary of the investigation findings, documenting key facts established, evidence reviewed, and the basis for the finding determination. Restricted to authorized HR and legal personnel.',
    `fmla_related` BOOLEAN COMMENT 'Indicates whether the grievance involves a dispute related to Family and Medical Leave Act (FMLA) rights, including denial of leave, interference, or retaliation for taking FMLA leave.',
    `grievance_status` STRING COMMENT 'Current lifecycle state of the grievance record. Tracks progression from initial filing through investigation, resolution, and closure. [ENUM-REF-CANDIDATE: filed|under_investigation|pending_review|resolved|closed|appealed|withdrawn — promote to reference product]',
    `grievance_type` STRING COMMENT 'Classification of the nature of the grievance or disciplinary matter. Drives routing, investigation procedures, and regulatory reporting obligations. [ENUM-REF-CANDIDATE: harassment|discrimination|retaliation|policy_violation|workplace_conduct|labor_relations|wage_dispute|other — promote to reference product]',
    `hr_case_number` STRING COMMENT 'Externally-known alphanumeric case reference number assigned by the HR department or case management system for tracking and correspondence. Format: GRV-YYYY-NNNNNN.. Valid values are `^GRV-[0-9]{4}-[0-9]{6}$`',
    `incident_date` DATE COMMENT 'The date on which the underlying incident, policy violation, or conduct giving rise to the grievance allegedly occurred. May predate the filing date.',
    `investigation_close_date` DATE COMMENT 'The date on which the formal investigation was concluded and findings were documented. Used to calculate investigation duration and compliance with regulatory timelines.',
    `investigation_finding` STRING COMMENT 'The formal outcome determination of the investigation indicating whether the allegations were substantiated, unsubstantiated, or inconclusive. Drives resolution and disciplinary action decisions.. Valid values are `substantiated|unsubstantiated|inconclusive|no_jurisdiction|withdrawn_prior_to_finding`',
    `investigation_start_date` DATE COMMENT 'The date on which the formal investigation into the grievance was initiated. Used to measure investigation duration and compliance with institutional response-time policies.',
    `is_anonymous` BOOLEAN COMMENT 'Indicates whether the grievance was filed anonymously. Anonymous filings may limit the scope of investigation and available remedies.',
    `is_union_grievance` BOOLEAN COMMENT 'Indicates whether the grievance is filed under a collective bargaining agreement (CBA) and involves union representation. Triggers specific procedural timelines and arbitration rights under the NLRA.',
    `legal_hold_flag` BOOLEAN COMMENT 'Indicates whether a legal hold has been placed on records associated with this grievance due to pending or anticipated litigation, EEOC proceedings, or regulatory inquiry. Prevents record destruction.',
    `mediation_outcome` STRING COMMENT 'The outcome of the mediation process if mediation was conducted. Indicates whether the parties reached a settlement agreement through mediation.. Valid values are `settled|not_settled|withdrawn|not_applicable`',
    `mediation_requested` BOOLEAN COMMENT 'Indicates whether formal mediation was requested by either party as an alternative dispute resolution mechanism prior to or in lieu of a full investigation.',
    `priority_level` STRING COMMENT 'Assigned urgency level of the grievance case, used to prioritize investigator workload and escalation timelines. Critical cases may involve imminent safety or legal risk.. Valid values are `low|medium|high|critical`',
    `protected_class_basis` STRING COMMENT 'The protected class characteristic(s) alleged as the basis for discrimination or harassment in the grievance. Applicable for EEO-related grievances. [ENUM-REF-CANDIDATE: race|color|national_origin|sex|religion|age|disability|pregnancy|genetic_information|veteran_status|not_applicable|other — promote to reference product]',
    `record_retention_date` DATE COMMENT 'The date after which the grievance record may be eligible for destruction per institutional records retention policy and applicable regulatory requirements. Calculated from closure date plus required retention period.',
    `resolution_date` DATE COMMENT 'The date on which the grievance was formally resolved and a resolution determination was communicated to the involved parties.',
    `resolution_type` STRING COMMENT 'The type of resolution or corrective action taken as a result of the grievance investigation. [ENUM-REF-CANDIDATE: no_action|verbal_warning|written_warning|suspension|termination|mediation|settlement|training_required|policy_change|other — promote to reference product]',
    `settlement_amount` DECIMAL(18,2) COMMENT 'Monetary settlement amount paid to the grievant as part of the resolution, if applicable. Null if no monetary settlement was reached. Reported in USD.',
    `subtype` STRING COMMENT 'Secondary classification providing more granular categorization within the grievance type (e.g., sexual harassment under harassment, racial discrimination under discrimination, FMLA retaliation under retaliation). [ENUM-REF-CANDIDATE: promote to reference product]',
    `suspension_days` STRING COMMENT 'Number of calendar days of suspension imposed on the respondent employee as part of the disciplinary action. Applicable only when disciplinary_action_type includes a suspension.',
    `union_contract_article` STRING COMMENT 'The specific article or section of the collective bargaining agreement (CBA) cited in the union grievance. Applicable only when is_union_grievance is true.',
    `updated_timestamp` TIMESTAMP COMMENT 'The timestamp when the grievance record was most recently modified. Used for change tracking, data synchronization, and audit compliance.',
    `workday_case_reference` STRING COMMENT 'The source system case identifier from Workday HCM Employee Relations module. Used for cross-system reconciliation and audit trail between the data lakehouse and the system of record.',
    CONSTRAINT pk_grievance PRIMARY KEY(`grievance_id`)
) COMMENT 'Formal employee grievance or disciplinary action record tracking complaints, investigations, and resolutions related to workplace conduct, policy violations, harassment, discrimination, or labor relations. Captures grievance type, filing date, parties involved, investigation status, findings, resolution type, appeal status, and closure date. Supports HR compliance, Title VII, ADA, and labor relations reporting. Distinct from Title IX cases owned by the compliance domain.';
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`time_entry` (
    ...
    `pay_period_id` BIGINT COMMENT 'Reference to the pay period in which this time entry will be processed for payment. Links time entries to payroll cycles.',
    `payroll_run_id` BIGINT COMMENT 'Reference to the specific payroll run in which this time entry was processed for payment. Links time entries to payroll results.',
    ...
    `cost_center_code` STRING COMMENT 'Financial accounting code identifying the cost center to which this time should be charged. Used for labor cost allocation and budgetary reporting.',
    ...
    `fund_code` STRING COMMENT 'Financial fund code identifying the funding source for this labor. Critical for grant-funded positions and restricted fund compliance.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`onboarding_task` (
    ...
    `enterprise_application_id` BIGINT COMMENT 'Foreign key linking to technology.enterprise_application. Business justification: IT onboarding tasks in higher-ed explicitly provision access to specific enterprise applications (Banner, Workday, Canvas, email). Links HR onboarding workflows to target systems for automated provisi',
    `org_unit_id` BIGINT COMMENT 'Reference to the department the new hire is joining. Used for department-specific onboarding tasks and reporting.',
    `position_id` BIGINT COMMENT 'Reference to the position the employee is being onboarded into. Links to the position control record.',
    `employee_id` BIGINT COMMENT 'Reference to the employee undergoing onboarding. Links to the employee master record.',
    `quaternary_onboarding_approved_by_employee_id` BIGINT COMMENT 'Reference to the employee who approved the task completion. Applicable only when requires_approval is true.',
    `quinary_onboarding_waived_by_employee_id` BIGINT COMMENT 'Reference to the HR or management employee who authorized the waiver. Required for audit trail.',
    `staffing_event_id` BIGINT COMMENT 'Reference to the parent staffing event that triggered this onboarding task. Links to the hire or rehire event.',
    `tertiary_onboarding_completed_by_employee_id` BIGINT COMMENT 'Reference to the employee who marked the task as completed. May differ from assigned_to_employee_id in cases of delegation or escalation.',
    `prerequisite_onboarding_task_id` BIGINT COMMENT 'Self-referencing FK on onboarding_task (prerequisite_onboarding_task_id)',
    `access_level` STRING COMMENT 'Level or type of access being granted (e.g., Read-Only, Standard User, Administrator, Faculty, Staff). Used for role-based access control (RBAC).',
    `approved_date` DATE COMMENT 'Date when the task completion was approved. Marks final closure for tasks requiring approval.',
    `assigned_date` DATE COMMENT 'Date when the task was assigned to the responsible party. Marks the start of the task lifecycle.',
    `assigned_to_role` STRING COMMENT 'Functional role responsible for the task. Used for routing and escalation when specific employee assignment is not yet determined. [ENUM-REF-CANDIDATE: new_hire|hr_specialist|it_administrator|hiring_manager|payroll_specialist|benefits_coordinator|facilities_manager|training_coordinator — 8 candidates stripped; promote to reference product]',
    `certificate_issued_flag` BOOLEAN COMMENT 'Indicates whether a completion certificate was issued for the training. Required for certain compliance trainings.',
    `completed_date` DATE COMMENT 'Date when the task was marked as completed. Used for compliance reporting and onboarding cycle time analysis.',
    `compliance_framework` STRING COMMENT 'Name of the regulatory framework or policy that mandates this task (e.g., IRCA, FERPA, Institutional Background Check Policy, Title IX Training Requirement).',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the onboarding task record was first created in the system. Used for audit trail and data lineage.',
    `document_received_date` DATE COMMENT 'Date when the required document was received and verified. Used for compliance reporting and audit trail.',
    `document_received_flag` BOOLEAN COMMENT 'Indicates whether the required document has been received and verified. Critical for compliance tasks.',
    `document_required` BOOLEAN COMMENT 'Indicates whether the task requires submission or verification of a document (e.g., I-9 supporting documents, degree verification, certifications).',
    `document_type` STRING COMMENT 'Type of document required for task completion (e.g., I-9 List A Document, Degree Certificate, Professional License, Policy Acknowledgment Form).',
    `due_date` DATE COMMENT 'Target completion date for the onboarding task. Critical for compliance tasks that must be completed by the first day of work or within regulatory timeframes.',
    `is_compliance_required` BOOLEAN COMMENT 'Indicates whether the task is required for regulatory compliance (e.g., I-9 verification per Immigration Reform and Control Act, background checks per institutional policy).',
    `is_mandatory` BOOLEAN COMMENT 'Indicates whether the task is required for all new hires or optional. Mandatory tasks block onboarding completion if not finished.',
    `requires_approval` BOOLEAN COMMENT 'Indicates whether the task requires managerial or HR approval before being marked as complete.',
    `started_date` DATE COMMENT 'Date when work on the task began. Indicates transition from assigned to in-progress status.',
    `system_access_required` BOOLEAN COMMENT 'Indicates whether the task involves provisioning access to institutional systems (e.g., Banner, Workday, Canvas, email, network).',
    `task_instructions` STRING COMMENT 'Detailed instructions for completing the task. May include links to forms, systems, or resources needed.',
    `task_name` STRING COMMENT 'Descriptive name of the onboarding task (e.g., Complete I-9 Form, Background Check Verification, System Access Provisioning).',
    `task_notes` STRING COMMENT 'Free-text notes about task progress, issues encountered, or special circumstances. Used for communication between HR, IT, and hiring managers.',
    `task_number` STRING COMMENT 'Human-readable business identifier for the onboarding task. Used for tracking and reference in communications.',
    `task_priority` STRING COMMENT 'Priority level of the task. Critical tasks (e.g., I-9, background check) must be completed before start date; others may have flexible timelines.. Valid values are `critical|high|medium|low`',
    `task_status` STRING COMMENT 'Current lifecycle status of the onboarding task. Tracks progress from assignment through completion. [ENUM-REF-CANDIDATE: not_started|in_progress|pending_approval|completed|waived|cancelled|overdue — 7 candidates stripped; promote to reference product]',
    `task_type` STRING COMMENT 'Category of the onboarding task. Classifies tasks by functional area to support workflow routing and reporting. [ENUM-REF-CANDIDATE: compliance|administrative|system_access|orientation|training|documentation|benefits|payroll|equipment|facility_access — 10 candidates stripped; promote to reference product]',
    `training_completion_date` DATE COMMENT 'Date when the required training was completed. Used for compliance reporting and certification tracking.',
    `training_module_name` STRING COMMENT 'Name of the training module or orientation session required (e.g., FERPA Compliance Training, Title IX Awareness, Campus Safety Orientation).',
    `training_required` BOOLEAN COMMENT 'Indicates whether the task involves completion of a training module or orientation session (e.g., FERPA training, Title IX training, safety orientation).',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the onboarding task record was last modified. Used for audit trail and change tracking.',
    `waived_date` DATE COMMENT 'Date when the task waiver was approved. Used for compliance reporting and audit trail.',
    `waived_flag` BOOLEAN COMMENT 'Indicates whether the task was waived for this employee (e.g., rehire who already completed background check, transfer from another department).',
    `waiver_reason` STRING COMMENT 'Explanation for why the task was waived. Required when waived_flag is true for audit and compliance purposes.',
    `workday_task_reference` STRING COMMENT 'External identifier from Workday HCM system. Used for integration and reconciliation with source system.',
    CONSTRAINT pk_onboarding_task PRIMARY KEY(`onboarding_task_id`)
) COMMENT 'Tracks individual onboarding tasks and milestones for new hires including I-9 verification, background check completion, system access provisioning, orientation attendance, and policy acknowledgments';

CREATE OR REPLACE TABLE `education_ecm`.`hr`.`training_record` (
    `training_record_id` BIGINT COMMENT 'Unique identifier for the training record. Primary key.',
    `org_unit_id` BIGINT COMMENT 'Reference to the department or organizational unit to which the employee belonged at the time of training.',
    `employee_id` BIGINT COMMENT 'Reference to the employee who completed or is assigned the training.',
    `training_program_id` BIGINT COMMENT 'Reference to the training program or course catalog entry.',
    `renewal_training_record_id` BIGINT COMMENT 'Self-referencing FK on training_record (renewal_training_record_id)',
    `attempts_count` STRING COMMENT 'Number of attempts the employee made to complete or pass the training.',
    `certificate_issue_date` DATE COMMENT 'Date the training certificate was issued to the employee.',
    `certificate_issued_flag` BOOLEAN COMMENT 'Indicator of whether a certificate of completion was issued to the employee.',
    `certificate_number` STRING COMMENT 'Unique identifier or serial number of the issued training certificate.',
    `completion_date` DATE COMMENT 'Date the employee successfully completed the training program.',
    `compliance_framework` STRING COMMENT 'Name of the regulatory framework or policy that mandates this training such as Family Educational Rights and Privacy Act (FERPA), Occupational Safety and Health Administration (OSHA), or Title IX.',
    `compliance_requirement_flag` BOOLEAN COMMENT 'Indicator of whether this training is required for regulatory or institutional compliance.',
    `contact_hours` DECIMAL(18,2) COMMENT 'Total number of instructional contact hours for the training program.',
    `cost_amount` DECIMAL(18,2) COMMENT 'Total cost incurred for the employee to complete this training program.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this training record was first created in the system.',
    `credit_hours` DECIMAL(18,2) COMMENT 'Number of continuing education or professional development credit hours awarded for completing the training.',
    `currency_code` STRING COMMENT 'Three-letter International Organization for Standardization (ISO) 4217 currency code for the training cost.. Valid values are `^[A-Z]{3}$`',
    `delivery_method` STRING COMMENT 'The format or modality through which the training was delivered.. Valid values are `online|in_person|hybrid|webinar|self_paced|instructor_led`',
    `due_date` DATE COMMENT 'Deadline by which the employee must complete the training to remain compliant.',
    `employee_comments` STRING COMMENT 'Free-text comments or feedback provided by the employee about the training experience.',
    `enrollment_date` DATE COMMENT 'Date the employee was enrolled or assigned to the training program.',
    `expiration_date` DATE COMMENT 'Date when the training certification or completion expires and requires renewal.',
    `ferpa_training_flag` BOOLEAN COMMENT 'Indicator of whether this training covers Family Educational Rights and Privacy Act (FERPA) student data privacy requirements.',
    `funding_source` STRING COMMENT 'Source of funding for the training such as departmental budget, grant, or professional development fund.',
    `instructor_name` STRING COMMENT 'Name of the instructor or facilitator who delivered the training.',
    `passed_flag` BOOLEAN COMMENT 'Indicator of whether the employee passed the training assessment.',
    `passing_score` DECIMAL(18,2) COMMENT 'Minimum score required to successfully pass the training program.',
    `safety_training_flag` BOOLEAN COMMENT 'Indicator of whether this training covers workplace safety or Occupational Safety and Health Administration (OSHA) requirements.',
    `score` DECIMAL(18,2) COMMENT 'Numeric score or grade achieved by the employee on the training assessment.',
    `start_date` DATE COMMENT 'Date the employee began the training program or course.',
    `title_ix_training_flag` BOOLEAN COMMENT 'Indicator of whether this training covers Title IX sexual harassment and discrimination prevention.',
    `training_category` STRING COMMENT 'Broad category of the training content such as Safety, Compliance, Leadership, Technical Skills, or Diversity and Inclusion.',
    `training_location` STRING COMMENT 'Physical or virtual location where the training was conducted.',
    `training_provider` STRING COMMENT 'Name of the organization or vendor that provided the training program.',
    `training_status` STRING COMMENT 'Current lifecycle status of the training record for this employee.. Valid values are `assigned|in_progress|completed|failed|waived|expired`',
    `training_title` STRING COMMENT 'The full title or name of the training program or course.',
    `training_type` STRING COMMENT 'Classification of the training program by requirement type.. Valid values are `required|elective|professional_development|compliance|safety|orientation`',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this training record was last modified in the system.',
    `waiver_granted_flag` BOOLEAN COMMENT 'Indicator of whether the employee was granted a waiver exempting them from completing this training.',
    `waiver_reason` STRING COMMENT 'Explanation or justification for granting a training waiver.',
    `workday_training_reference` STRING COMMENT 'External identifier from Workday HCM for this training record.',
    CONSTRAINT pk_training_record PRIMARY KEY(`training_record_id`)
) COMMENT 'Records employee completion of required and elective training programs including safety training, harassment prevention, FERPA training, and professional development courses with certification tracking';

CREATE OR REPLACE TABLE `education_ecm`.`hr`.`accrual_plan` (
    `accrual_plan_id` BIGINT COMMENT 'Primary key for accrual_plan',
    `superseded_accrual_plan_id` BIGINT COMMENT 'Self-referencing FK on accrual_plan (superseded_accrual_plan_id)',
    `accrual_frequency` STRING COMMENT 'Frequency at which leave time is accrued under this plan (e.g., monthly, per pay period).',
    `accrual_rate` DECIMAL(18,2) COMMENT 'Rate at which leave time accrues per accrual period, expressed in hours or days depending on unit of measure.',
    `accrual_start_event` STRING COMMENT 'Event that triggers the start of accrual under this plan (e.g., hire date, eligibility date).',
    `accrual_unit` STRING COMMENT 'Unit of measure for accrued leave time (hours or days).',
    `advance_notice_days` STRING COMMENT 'Number of days advance notice required before taking leave under this plan. Zero if no advance notice required.',
    `approval_required` BOOLEAN COMMENT 'Indicates whether supervisor or HR approval is required before leave under this plan can be taken.',
    `carryover_allowed` BOOLEAN COMMENT 'Indicates whether unused accrued leave can be carried over to the next accrual period or fiscal year.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this accrual plan record was first created in the system.',
    `accrual_plan_description` STRING COMMENT 'Detailed description of the accrual plan including purpose, rules, and any special conditions.',
    `documentation_required` BOOLEAN COMMENT 'Indicates whether supporting documentation (e.g., medical certificate) is required for leave requests under this plan.',
    `effective_end_date` DATE COMMENT 'Date when the accrual plan is no longer active. Null for open-ended plans.',
    `effective_start_date` DATE COMMENT 'Date when the accrual plan becomes active and available for employee assignment.',
    `eligibility_criteria` STRING COMMENT 'Description of the employee eligibility requirements for this accrual plan (e.g., full-time staff, faculty with tenure).',
    `employee_group_code` STRING COMMENT 'Code identifying the employee group or classification to which this accrual plan applies (e.g., staff, faculty, administrative).',
    `fiscal_year_reset` BOOLEAN COMMENT 'Indicates whether accrual balances reset at the start of each fiscal year.',
    `integration_code` STRING COMMENT 'External system integration code used to synchronize this accrual plan with payroll or HRIS systems (e.g., Workday HCM).',
    `last_modified_by` STRING COMMENT 'Username or identifier of the user who last modified this accrual plan record.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this accrual plan record was last updated.',
    `maximum_accrual_balance` DECIMAL(18,2) COMMENT 'Maximum amount of leave time that can be accrued under this plan. Null if no cap applies.',
    `maximum_carryover_amount` DECIMAL(18,2) COMMENT 'Maximum amount of accrued leave that can be carried over to the next period if carryover is allowed. Null if no limit or carryover not allowed.',
    `minimum_usage_increment` DECIMAL(18,2) COMMENT 'Minimum increment in which leave can be taken (e.g., 0.25 for quarter-hour increments, 1.0 for full-day increments).',
    `negative_balance_allowed` BOOLEAN COMMENT 'Indicates whether employees can take leave in advance of accrual, resulting in a negative balance.',
    `payout_on_termination` BOOLEAN COMMENT 'Indicates whether unused accrued leave is paid out to the employee upon termination of employment.',
    `plan_code` STRING COMMENT 'Unique business identifier code for the accrual plan used in external communications and system integrations.',
    `plan_name` STRING COMMENT 'Human-readable name of the accrual plan (e.g., Standard Staff PTO, Faculty Sabbatical Leave).',
    `plan_type` STRING COMMENT 'Category of leave or time-off that this accrual plan governs.',
    `policy_reference` STRING COMMENT 'Reference to the institutional policy document or section that governs this accrual plan.',
    `proration_method` STRING COMMENT 'Method used to prorate accruals for part-time employees or mid-period hires.',
    `accrual_plan_status` STRING COMMENT 'Current lifecycle status of the accrual plan indicating whether it is available for assignment to employees.',
    `union_code` STRING COMMENT 'Code identifying the union or collective bargaining unit associated with this accrual plan, if applicable. Null for non-union plans.',
    `waiting_period_days` STRING COMMENT 'Number of days an employee must be employed before they begin accruing leave under this plan. Zero if accrual starts immediately.',
    `created_by` STRING COMMENT 'Username or identifier of the user who created this accrual plan record.',
    CONSTRAINT pk_accrual_plan PRIMARY KEY(`accrual_plan_id`)
) COMMENT 'Master reference table for accrual_plan. Referenced by accrual_plan_id.';

CREATE OR REPLACE TABLE `education_ecm`.`hr`.`benefit_program` (
    `benefit_program_id` BIGINT COMMENT 'Primary key for benefit_program',
    `superseded_benefit_program_id` BIGINT COMMENT 'Self-referencing FK on benefit_program (superseded_benefit_program_id)',
    `aca_reporting_code` STRING COMMENT 'Code used for ACA Form 1095-C reporting to indicate the type of coverage offered under this benefit program.',
    `annual_maximum_benefit` DECIMAL(18,2) COMMENT 'Maximum dollar amount of benefits payable per employee per plan year under this benefit program.',
    `carrier_name` STRING COMMENT 'Name of the insurance carrier or third-party administrator providing the benefit program.',
    `carrier_policy_number` STRING COMMENT 'Policy or contract number assigned by the insurance carrier for this benefit program.',
    `coinsurance_percentage` DECIMAL(18,2) COMMENT 'Percentage of costs the employee pays after meeting the deductible under this benefit program.',
    `compliance_reporting_required` BOOLEAN COMMENT 'Indicates whether this benefit program requires regulatory compliance reporting such as ACA 1095 forms or ERISA annual reports.',
    `copay_amount` DECIMAL(18,2) COMMENT 'Fixed dollar amount the employee pays per service or visit under this benefit program.',
    `coverage_level_options` STRING COMMENT 'Available coverage tiers for the benefit program such as employee-only, employee-plus-spouse, employee-plus-children, or family.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this benefit program record was first created in the system.',
    `deductible_amount` DECIMAL(18,2) COMMENT 'Annual deductible amount the employee must pay before the benefit program begins coverage.',
    `effective_end_date` DATE COMMENT 'Date when the benefit program is no longer available for new enrollments. Nullable for open-ended programs.',
    `effective_start_date` DATE COMMENT 'Date when the benefit program becomes active and available for employee enrollment.',
    `eligibility_rule_description` STRING COMMENT 'Textual description of the eligibility criteria employees must meet to enroll in this benefit program.',
    `employee_contribution_amount` DECIMAL(18,2) COMMENT 'Fixed dollar amount the employee contributes per pay period for this benefit program.',
    `employee_contribution_percentage` DECIMAL(18,2) COMMENT 'Percentage of premium or salary the employee contributes for this benefit program.',
    `employee_contribution_type` STRING COMMENT 'Tax treatment classification for employee contributions to the benefit program.',
    `employer_contribution_amount` DECIMAL(18,2) COMMENT 'Fixed dollar amount the employer contributes per employee per pay period for this benefit program.',
    `employer_contribution_percentage` DECIMAL(18,2) COMMENT 'Percentage of premium or salary the employer contributes for this benefit program.',
    `enrollment_method` STRING COMMENT 'Primary method by which employees enroll in the benefit program.',
    `erisa_plan_number` STRING COMMENT 'Three-digit plan number assigned for ERISA reporting and compliance purposes.',
    `is_cobra_eligible` BOOLEAN COMMENT 'Indicates whether this benefit program qualifies for COBRA continuation coverage upon termination of employment.',
    `is_fmla_protected` BOOLEAN COMMENT 'Indicates whether this benefit program is protected under FMLA during employee leave.',
    `is_mandatory` BOOLEAN COMMENT 'Indicates whether enrollment in this benefit program is mandatory for eligible employees.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this benefit program record was last updated in the system.',
    `lifetime_maximum_benefit` DECIMAL(18,2) COMMENT 'Maximum dollar amount of benefits payable per employee over their lifetime under this benefit program.',
    `modified_by_user` STRING COMMENT 'Username or identifier of the user who last modified this benefit program record.',
    `open_enrollment_end_date` DATE COMMENT 'Ending date of the annual open enrollment period for this benefit program.',
    `open_enrollment_start_date` DATE COMMENT 'Beginning date of the annual open enrollment period for this benefit program.',
    `out_of_pocket_maximum` DECIMAL(18,2) COMMENT 'Maximum amount the employee will pay out-of-pocket per plan year before the benefit program covers 100% of costs.',
    `plan_document_url` STRING COMMENT 'Web address or document repository location for the official benefit plan document.',
    `plan_year_end_date` DATE COMMENT 'Ending date of the benefit plan year cycle.',
    `plan_year_start_date` DATE COMMENT 'Beginning date of the benefit plan year cycle used for enrollment periods and benefit calculations.',
    `program_administrator_contact` STRING COMMENT 'Email address or phone number for the benefit program administrator.',
    `program_administrator_name` STRING COMMENT 'Name of the internal or external administrator responsible for managing this benefit program.',
    `program_code` STRING COMMENT 'Externally-known unique code for the benefit program used in HR systems and employee communications.',
    `program_name` STRING COMMENT 'Full descriptive name of the benefit program as displayed to employees and administrators.',
    `program_status` STRING COMMENT 'Current lifecycle status of the benefit program indicating availability for enrollment and administration.',
    `program_type` STRING COMMENT 'Classification of the benefit program by primary benefit category.',
    `qualifying_life_event_allowed` BOOLEAN COMMENT 'Indicates whether employees can make enrollment changes outside open enrollment due to qualifying life events.',
    `summary_plan_description_url` STRING COMMENT 'Web address or document repository location for the employee-facing summary plan description.',
    `waiting_period_days` STRING COMMENT 'Number of days an employee must wait after hire date or eligibility date before they can enroll in the benefit program.',
    CONSTRAINT pk_benefit_program PRIMARY KEY(`benefit_program_id`)
) COMMENT 'Master reference table for benefit_program. Referenced by benefit_program_id.';

CREATE OR REPLACE TABLE `education_ecm`.`hr`.`compensation_plan` (
    ...
    `annual_leave_accrual_rate` DECIMAL(18,2) COMMENT 'Rate at which annual leave hours accrue per pay period for employees on this compensation plan.',
    `approval_authority_level` STRING COMMENT 'Organizational level or role required to approve hiring or salary changes for positions on this compensation plan.',
    `bargaining_unit` STRING COMMENT 'Specific bargaining unit within a union that this compensation plan covers. Null for non-union plans.',
    `base_salary_maximum` DECIMAL(18,2) COMMENT 'Maximum base salary or wage amount for this compensation plan in institutional currency.',
    `base_salary_midpoint` DECIMAL(18,2) COMMENT 'Target midpoint salary or wage amount for this compensation plan representing market competitive rate.',
    `base_salary_minimum` DECIMAL(18,2) COMMENT 'Minimum base salary or wage amount for this compensation plan in institutional currency.',
    `benefits_eligible` BOOLEAN COMMENT 'Indicates whether employees on this compensation plan are eligible for institutional benefits programs.',
    `budget_pool_code` STRING COMMENT 'Financial budget pool or cost center code associated with funding for this compensation plan.',
    `compa_ratio_maximum` DECIMAL(18,2) COMMENT 'Maximum acceptable compa-ratio (actual salary divided by midpoint) for employees on this plan.',
    `compa_ratio_minimum` DECIMAL(18,2) COMMENT 'Minimum acceptable compa-ratio (actual salary divided by midpoint) for employees on this plan.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this compensation plan record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO currency code for all monetary amounts in this compensation plan.',
    `compensation_plan_description` STRING COMMENT 'Detailed textual description of the compensation plan including purpose, scope, and key features.',
    `effective_end_date` DATE COMMENT 'Date when the compensation plan is no longer active. Null indicates an open-ended plan.',
    `effective_start_date` DATE COMMENT 'Date when the compensation plan becomes active and available for employee assignment.',
    `flsa_status` STRING COMMENT 'FLSA classification indicating whether employees on this plan are exempt from overtime regulations.',
    `fte_standard` DECIMAL(18,2) COMMENT 'Standard FTE value for positions on this compensation plan, typically 1.0000 for full-time and fractional for part-time.',
    `job_family` STRING COMMENT 'Broad occupational job family grouping for positions assigned to this compensation plan.',
    `last_review_date` DATE COMMENT 'Date when this compensation plan was last reviewed for market competitiveness and internal equity.',
    `market_percentile_target` STRING COMMENT 'Target market percentile for compensation positioning (e.g., 50 for median, 75 for upper quartile).',
    `market_reference_source` STRING COMMENT 'External market survey or benchmark source used to establish compensation ranges for this plan.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this compensation plan record was last modified in the system.',
    `next_review_date` DATE COMMENT 'Scheduled date for the next formal review of this compensation plan.',
    `notes` STRING COMMENT 'Additional administrative notes or special instructions related to this compensation plan.',
    `overtime_eligible` BOOLEAN COMMENT 'Indicates whether employees on this compensation plan are eligible to receive overtime pay.',
    `pay_frequency` STRING COMMENT 'Standard payment cycle frequency for employees assigned to this compensation plan.',
    `pay_grade` STRING COMMENT 'Institutional pay grade or band classification associated with this compensation plan.',
    `performance_bonus_eligible` BOOLEAN COMMENT 'Indicates whether employees on this compensation plan are eligible for performance-based bonus compensation.',
    `plan_category` STRING COMMENT 'Broad employee category this compensation plan applies to, aligned with institutional workforce segmentation.',
    `plan_code` STRING COMMENT 'Unique business identifier code for the compensation plan used in HR systems and reporting.',
    `plan_name` STRING COMMENT 'Full descriptive name of the compensation plan.',
    `plan_type` STRING COMMENT 'Classification of the compensation plan structure indicating the primary payment method.',
    `retirement_plan_eligible` BOOLEAN COMMENT 'Indicates whether employees on this compensation plan are eligible to participate in institutional retirement plans.',
    `sick_leave_accrual_rate` DECIMAL(18,2) COMMENT 'Rate at which sick leave hours accrue per pay period for employees on this compensation plan.',
    `standard_hours_per_week` DECIMAL(18,2) COMMENT 'Expected number of work hours per week for employees on this compensation plan.',
    ...
    `union_affiliation` STRING COMMENT 'Name or code of the labor union associated with this compensation plan, if applicable. Null for non-union plans.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`pay_grade` (
    `pay_grade_id` BIGINT COMMENT 'Primary key for pay_grade',
    `parent_pay_grade_id` BIGINT COMMENT 'Self-referencing FK on pay_grade (parent_pay_grade_id)',
    `approval_authority_level` STRING COMMENT 'Organizational level required to approve hiring or salary actions for positions in this pay grade.',
    `benchmark_job_title` STRING COMMENT 'Representative job title used for external market benchmarking and CUPA-HR survey matching.',
    `bonus_eligible` BOOLEAN COMMENT 'Indicates whether positions in this pay grade are eligible for performance-based bonuses or incentive compensation.',
    `classification_type` STRING COMMENT 'FLSA classification determining overtime eligibility and compensation rules.',
    `pay_grade_code` STRING COMMENT 'Short alphanumeric code representing the pay grade (e.g., PG01, E5, M3). Used as the business identifier in HR systems and employee communications.',
    `compa_ratio_maximum` DECIMAL(18,2) COMMENT 'Maximum acceptable compa-ratio (actual salary divided by midpoint) for positions in this grade. Typically 1.00 to 1.20.',
    `compa_ratio_minimum` DECIMAL(18,2) COMMENT 'Minimum acceptable compa-ratio (actual salary divided by midpoint) for positions in this grade. Typically 0.80 to 1.00.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this pay grade record was first created in the system.',
    `cupa_hr_category_code` STRING COMMENT 'CUPA-HR standard occupational category code for benchmarking and survey participation.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for salary amounts (e.g., USD, CAD, GBP). Supports multi-currency institutions.',
    `pay_grade_description` STRING COMMENT 'Detailed description of the pay grade including typical job responsibilities, required qualifications, and scope of authority.',
    `effective_end_date` DATE COMMENT 'Date when this pay grade is no longer effective. Null for currently active pay grades.',
    `effective_start_date` DATE COMMENT 'Date when this pay grade becomes effective and can be assigned to positions.',
    `employee_category` STRING COMMENT 'Broad employee category this pay grade applies to, used for workforce segmentation and CUPA-HR reporting.',
    `grade_level` STRING COMMENT 'Numeric level or rank of this pay grade within the institutional compensation structure. Lower numbers typically indicate entry-level positions; higher numbers indicate senior positions.',
    `hourly_rate_maximum` DECIMAL(18,2) COMMENT 'Maximum hourly rate for this pay grade in USD. Applicable for non-exempt hourly positions.',
    `hourly_rate_minimum` DECIMAL(18,2) COMMENT 'Minimum hourly rate for this pay grade in USD. Applicable for non-exempt hourly positions.',
    `job_family` STRING COMMENT 'Job family or occupational group this pay grade belongs to (e.g., Finance & Accounting, Information Technology, Student Services).',
    `last_market_review_date` DATE COMMENT 'Date when this pay grade was last reviewed against external market data and CUPA-HR benchmarks.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this pay grade record was last updated.',
    `market_reference_percentile` STRING COMMENT 'Target market percentile for midpoint salary (e.g., 50 for median, 75 for 75th percentile). Used for competitive positioning against CUPA-HR benchmarks.',
    `maximum_annual_salary` DECIMAL(18,2) COMMENT 'Maximum annual salary amount for this pay grade in USD. Represents the ceiling of the compensation range.',
    `midpoint_annual_salary` DECIMAL(18,2) COMMENT 'Midpoint annual salary amount for this pay grade in USD. Represents the target market rate for fully competent performance.',
    `minimum_annual_salary` DECIMAL(18,2) COMMENT 'Minimum annual salary amount for this pay grade in USD. Represents the floor of the compensation range.',
    `minimum_education_level` STRING COMMENT 'Minimum educational attainment typically required for positions in this pay grade.',
    `minimum_years_experience` STRING COMMENT 'Minimum number of years of relevant professional experience typically required for positions in this pay grade.',
    `pay_grade_name` STRING COMMENT 'Full descriptive name of the pay grade (e.g., Administrative Support I, Senior Faculty, Executive Leadership).',
    `next_review_date` DATE COMMENT 'Scheduled date for the next market review and potential adjustment of this pay grade.',
    `notes` STRING COMMENT 'Additional notes, comments, or special considerations related to this pay grade.',
    `number_of_steps` STRING COMMENT 'Total number of steps or increments within this pay grade. Used in step-based compensation structures.',
    `overtime_eligible` BOOLEAN COMMENT 'Indicates whether positions in this pay grade are eligible for overtime compensation under FLSA regulations.',
    `pay_frequency` STRING COMMENT 'Standard pay frequency for positions in this grade (e.g., monthly for salaried staff, biweekly for hourly).',
    `range_spread_percentage` DECIMAL(18,2) COMMENT 'Percentage spread between minimum and maximum salary ((max - min) / min * 100). Indicates the width of the compensation range.',
    `pay_grade_status` STRING COMMENT 'Current lifecycle status of the pay grade. Active grades can be assigned to positions; inactive grades are retained for historical reference only.',
    `step_increment_amount` DECIMAL(18,2) COMMENT 'Standard dollar amount for step increases within this pay grade. Applicable for step-based progression systems.',
    `step_increment_percentage` DECIMAL(18,2) COMMENT 'Standard percentage increase for step progression within this pay grade. Applicable for percentage-based progression systems.',
    `union_covered` BOOLEAN COMMENT 'Indicates whether positions in this pay grade are covered by collective bargaining agreements.',
    CONSTRAINT pk_pay_grade PRIMARY KEY(`pay_grade_id`)
) COMMENT 'Master reference table for pay_grade. Referenced by pay_grade_id.';

CREATE OR REPLACE TABLE `education_ecm`.`hr`.`payroll_run` (
    `payroll_run_id` BIGINT COMMENT 'Primary key for payroll_run',
    `employee_id` BIGINT COMMENT 'The user identifier of the person who approved this payroll run.',
    `journal_batch_id` BIGINT COMMENT 'The batch identifier used when posting this payroll run to the general ledger system.',
    `org_unit_id` BIGINT COMMENT 'Reference to the primary department or organizational unit associated with this payroll run.',
    `original_run_id` BIGINT COMMENT 'Reference to the original payroll run if this is a correction or reversal run.',
    `payroll_calendar_id` BIGINT COMMENT 'Reference to the payroll calendar that defines the schedule for this payroll run.',
    `adjustment_payroll_run_id` BIGINT COMMENT 'Self-referencing FK on payroll_run (adjustment_payroll_run_id)',
    `approval_timestamp` TIMESTAMP COMMENT 'The date and time when the payroll run was approved for payment processing.',
    `calculation_timestamp` TIMESTAMP COMMENT 'The date and time when the payroll calculations were completed for this run.',
    `check_date` DATE COMMENT 'The date on which payment is issued to employees for this payroll run.',
    `company_code` STRING COMMENT 'The company or legal entity code for which this payroll run is processed.',
    `created_timestamp` TIMESTAMP COMMENT 'The date and time when this payroll run record was first created in the system.',
    `currency_code` STRING COMMENT 'The three-letter ISO 4217 currency code for all monetary amounts in this payroll run.',
    `employee_count` STRING COMMENT 'The total number of employees included in this payroll run.',
    `fiscal_period` STRING COMMENT 'The fiscal period within the fiscal year for this payroll run.',
    `fiscal_year` STRING COMMENT 'The fiscal year in which this payroll run is processed.',
    `modified_timestamp` TIMESTAMP COMMENT 'The date and time when this payroll run record was last modified.',
    `notes` STRING COMMENT 'Free-text notes or comments about this payroll run for audit and documentation purposes.',
    `pay_frequency` STRING COMMENT 'The frequency at which employees are paid in this payroll run.',
    `pay_group_code` STRING COMMENT 'Code identifying the pay group or employee population included in this payroll run.',
    `pay_period_end_date` DATE COMMENT 'The last day of the pay period covered by this payroll run.',
    `pay_period_start_date` DATE COMMENT 'The first day of the pay period covered by this payroll run.',
    `payment_method` STRING COMMENT 'The primary payment method used for disbursing funds in this payroll run.',
    `posting_timestamp` TIMESTAMP COMMENT 'The date and time when the payroll run was posted to the general ledger.',
    `reversal_flag` BOOLEAN COMMENT 'Indicates whether this payroll run is a reversal of a previous run.',
    `run_number` STRING COMMENT 'Business identifier for the payroll run, typically formatted as PR-YYYY-####.',
    ...
    `total_deductions` DECIMAL(18,2) COMMENT 'The total amount of all deductions (taxes, benefits, garnishments) for this payroll run.',
    `total_employer_benefits` DECIMAL(18,2) COMMENT 'The total amount of employer-paid benefits contributions for this payroll run.',
    `total_employer_taxes` DECIMAL(18,2) COMMENT 'The total amount of employer-paid taxes (FICA, FUTA, SUTA) for this payroll run.',
    `total_gross_pay` DECIMAL(18,2) COMMENT 'The total gross pay amount for all employees in this payroll run before any deductions.',
    `total_net_pay` DECIMAL(18,2) COMMENT 'The total net pay amount for all employees in this payroll run after all deductions.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`job_family` (
    `job_family_id` BIGINT COMMENT 'Primary key for job_family',
    `competency_framework_id` BIGINT COMMENT 'Reference to the competency framework or skill model associated with this job family, used for talent development and succession planning.',
    `parent_job_family_id` BIGINT COMMENT 'Reference to a parent job family if this job family is part of a hierarchical classification structure.',
    `benefits_tier` STRING COMMENT 'Default benefits tier or package level for positions within this job family.',
    `career_ladder_type` STRING COMMENT 'Type of career progression path associated with this job family (e.g., individual contributor, management track, technical specialist).',
    `job_family_category` STRING COMMENT 'High-level functional category to which this job family belongs. Used for organizational reporting and workforce segmentation. [ENUM-REF-CANDIDATE: administrative|academic_support|facilities|finance|human_resources|information_technology|student_services|research|advancement|athletics|legal|communications — promote to reference product]',
    `job_family_code` STRING COMMENT 'Short alphanumeric code used to identify the job family in human-readable format. Used in HR systems, reports, and position postings.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this job family record was first created in the system.',
    `cupa_hr_category` STRING COMMENT 'CUPA-HR job family classification code used for benchmarking and salary surveys in higher education.',
    `job_family_description` STRING COMMENT 'Detailed description of the job family, including the types of roles, responsibilities, and functional areas covered.',
    `eeo_category` STRING COMMENT 'EEO-1 reporting category for positions in this job family, used for federal compliance reporting.',
    `effective_end_date` DATE COMMENT 'Date when this job family definition was retired or superseded. Null if currently active.',
    `effective_start_date` DATE COMMENT 'Date when this job family definition became effective and available for use in position management.',
    `flsa_classification` STRING COMMENT 'Default FLSA classification for positions within this job family, indicating whether roles are typically exempt or non-exempt from overtime regulations.',
    `ipeds_category` STRING COMMENT 'IPEDS occupational category for positions in this job family, used for federal higher education reporting.',
    `minimum_education_level` STRING COMMENT 'Typical minimum education level required for entry-level positions within this job family.',
    `job_family_name` STRING COMMENT 'Full descriptive name of the job family (e.g., Information Technology, Facilities Management, Student Services).',
    `notes` STRING COMMENT 'Additional notes, comments, or special instructions related to this job family for HR administrators.',
    `occupational_group` STRING COMMENT 'Standard occupational group classification aligned with federal or industry standards (e.g., SOC code group, EEO-1 category).',
    `performance_cycle` STRING COMMENT 'Standard performance review cycle for positions in this job family.',
    `remote_work_eligible` BOOLEAN COMMENT 'Indicates whether positions in this job family are generally eligible for remote or hybrid work arrangements.',
    `salary_grade_max` STRING COMMENT 'Maximum salary grade applicable to positions within this job family, used for compensation planning.',
    `salary_grade_min` STRING COMMENT 'Minimum salary grade applicable to positions within this job family, used for compensation planning.',
    `sort_order` STRING COMMENT 'Numeric value used to control the display order of job families in reports and user interfaces.',
    `job_family_status` STRING COMMENT 'Current lifecycle status of the job family. Active families are in use; inactive families are no longer used for new positions.',
    `succession_planning_priority` STRING COMMENT 'Priority level for succession planning activities for this job family, indicating strategic importance to the institution.',
    `typical_experience_years` STRING COMMENT 'Typical number of years of relevant experience expected for mid-level positions in this job family.',
    `union_eligible` BOOLEAN COMMENT 'Indicates whether positions in this job family are typically eligible for union representation or collective bargaining.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this job family record was last modified.',
    `workforce_planning_category` STRING COMMENT 'Workforce planning classification indicating expected growth, stability, or decline in demand for this job family.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`pay_period` (
    `pay_period_id` BIGINT COMMENT 'Primary key for pay_period',
    `employee_id` BIGINT COMMENT 'User identifier of the payroll administrator who finalized this pay period.',
    `payroll_calendar_id` BIGINT COMMENT 'Reference to the payroll calendar that defines the schedule and rules for this pay period.',
    `prior_pay_period_id` BIGINT COMMENT 'Self-referencing FK on pay_period (prior_pay_period_id)',
    `approval_deadline_date` DATE COMMENT 'Final deadline date by which all supervisory approvals must be completed for timesheets and payroll adjustments.',
    `benefits_eligibility_date` DATE COMMENT 'Date used to determine benefits eligibility and accruals for employees paid during this period.',
    `calendar_year` STRING COMMENT 'Calendar year in which this pay period occurs, used for tax reporting and compliance.',
    `check_date` DATE COMMENT 'Date when paychecks or direct deposits are issued to employees for this pay period.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this pay period record was first created in the system.',
    `pay_period_description` STRING COMMENT 'Detailed description of the pay period, including any special circumstances, adjustments, or processing notes.',
    `employee_group_code` STRING COMMENT 'Code identifying the employee group or classification to which this pay period applies (e.g., staff, faculty, student workers, administrators).',
    `end_date` DATE COMMENT 'Last calendar date of the pay period when time and attendance tracking ends.',
    `finalized_timestamp` TIMESTAMP COMMENT 'Timestamp when this pay period was finalized and locked from further changes.',
    `fiscal_month` STRING COMMENT 'Fiscal month (1-12) within the fiscal year to which this pay period belongs.',
    `fiscal_quarter` STRING COMMENT 'Fiscal quarter (1-4) within the fiscal year to which this pay period belongs.',
    `fiscal_year` STRING COMMENT 'Fiscal year to which this pay period belongs, used for budgeting and financial reporting alignment.',
    `is_current_period` BOOLEAN COMMENT 'Boolean flag indicating whether this is the currently active pay period for time entry and payroll processing.',
    `is_final` BOOLEAN COMMENT 'Boolean flag indicating whether this pay period has been finalized and locked from further changes.',
    `leave_accrual_date` DATE COMMENT 'Date when leave balances (vacation, sick time, personal days) are calculated and accrued for this pay period.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this pay period record was last modified or updated.',
    `notes` STRING COMMENT 'Free-text notes or comments about this pay period, including special processing instructions, holidays, or exceptions.',
    `pay_frequency_type` STRING COMMENT 'Classification of how frequently payroll is processed for this pay period (weekly, biweekly, semimonthly, monthly, quarterly, annual).',
    `pay_group_code` STRING COMMENT 'Code identifying the pay group that determines payroll processing rules and timing for this period.',
    `pay_period_code` STRING COMMENT 'Business identifier code for the pay period, typically used in payroll processing and reporting systems.',
    `pay_period_name` STRING COMMENT 'Human-readable name or label for the pay period (e.g., January 2024 - Period 1, PP01-2024).',
    `pay_schedule_type` STRING COMMENT 'Type of pay schedule this period represents (regular recurring payroll, supplemental pay, bonus cycle, off-cycle payment, or adjustment).',
    `payroll_processing_date` DATE COMMENT 'Date when payroll calculations and processing are executed for this pay period.',
    `period_number` STRING COMMENT 'Sequential number of this pay period within the fiscal or calendar year (e.g., 1-26 for biweekly, 1-12 for monthly).',
    `retirement_contribution_date` DATE COMMENT 'Date when employer and employee retirement contributions are calculated and processed for this pay period.',
    `start_date` DATE COMMENT 'First calendar date of the pay period when time and attendance tracking begins.',
    ...
    `tax_period` STRING COMMENT 'Sequential tax period number within the tax year, used for quarterly and annual tax reporting.',
    `tax_year` STRING COMMENT 'Tax year to which this pay period belongs for tax withholding and reporting purposes (typically matches calendar year).',
    `time_entry_close_date` DATE COMMENT 'Date when the time entry system closes and no further time submissions are accepted for this pay period.',
    `time_entry_open_date` DATE COMMENT 'Date when the time entry system opens for employees to begin recording hours for this pay period.',
    `timesheet_due_date` DATE COMMENT 'Deadline date by which employees and supervisors must submit and approve timesheets for this pay period.',
    `total_days` STRING COMMENT 'Total number of calendar days in this pay period, calculated from start date to end date inclusive.',
    `working_days` STRING COMMENT 'Number of standard working days (excluding weekends and holidays) within this pay period.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`payroll_calendar` (
    ...
    `approval_deadline_days_prior` STRING COMMENT 'Number of days before the pay date that all payroll approvals must be completed by supervisors and payroll administrators.',
    `calendar_code` STRING COMMENT 'Business identifier code for the payroll calendar used in external reporting and system integration (e.g., BIWEEKLY_STAFF, MONTHLY_FACULTY).',
    `calendar_name` STRING COMMENT 'Human-readable name of the payroll calendar (e.g., Bi-Weekly Staff Payroll, Monthly Faculty Payroll).',
    `calendar_type` STRING COMMENT 'Classification of the payroll frequency type determining how often employees are paid.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this payroll calendar record was first created in the system.',
    `default_pay_day_of_month` STRING COMMENT 'The standard day of the month on which payroll is disbursed for monthly calendars (e.g., 25th of each month). Null for non-monthly calendars.',
    `default_pay_day_of_week` STRING COMMENT 'The standard day of the week on which payroll is disbursed for this calendar (e.g., Friday for biweekly staff).',
    `payroll_calendar_description` STRING COMMENT 'Detailed description of the payroll calendar including its purpose, scope, and any special considerations or exceptions.',
    `effective_end_date` DATE COMMENT 'The date when this payroll calendar ceases to be active. Null for open-ended calendars.',
    `effective_start_date` DATE COMMENT 'The date when this payroll calendar becomes active and begins being used for payroll processing.',
    `employee_group` STRING COMMENT 'The employee classification group that this payroll calendar applies to, aligning with institutional workforce segmentation.',
    `fiscal_year` STRING COMMENT 'The fiscal year to which this payroll calendar applies, following the institutions fiscal year convention.',
    `holiday_adjustment_rule` STRING COMMENT 'Rule for adjusting pay dates when the scheduled pay date falls on a holiday or weekend.',
    `includes_summer_pay` BOOLEAN COMMENT 'Indicates whether this payroll calendar includes summer pay periods, relevant for academic calendars where faculty may have optional summer compensation.',
    `is_academic_year_calendar` BOOLEAN COMMENT 'Indicates whether this payroll calendar follows the academic year schedule (True) or the fiscal year schedule (False).',
    `modified_by` STRING COMMENT 'Identifier of the user or system process that last modified this payroll calendar record.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this payroll calendar record was last modified.',
    `notes` STRING COMMENT 'Additional notes or comments about the payroll calendar for administrative reference, including historical context or special instructions.',
    `pay_frequency_days` STRING COMMENT 'Number of days between pay periods (e.g., 14 for biweekly, 7 for weekly, 30 for monthly).',
    `pay_periods_per_year` STRING COMMENT 'Total number of pay periods in a calendar year for this payroll calendar (e.g., 26 for biweekly, 12 for monthly).',
    `payroll_processing_system` STRING COMMENT 'The name or identifier of the payroll system used to process this calendar (e.g., Workday HCM, PeopleSoft, ADP).',
    `payroll_calendar_status` STRING COMMENT 'Current lifecycle status of the payroll calendar indicating whether it is currently in use for payroll processing.',
    `time_entry_cutoff_days_prior` STRING COMMENT 'Number of days before the pay date that time entry must be submitted and approved for inclusion in the payroll run.',
    `union_affiliation_code` STRING COMMENT 'Code identifying the labor union or collective bargaining unit associated with this payroll calendar, if applicable. Null for non-union calendars.',
    `created_by` STRING COMMENT 'Identifier of the user or system process that created this payroll calendar record.',
...
CREATE OR REPLACE TABLE `education_ecm`.`hr`.`competency_framework` (
    `competency_framework_id` BIGINT COMMENT 'Primary key for competency_framework',
    `applicable_job_families` STRING COMMENT 'Comma-separated list of job family codes or names to which this competency framework applies.',
    `approval_authority` STRING COMMENT 'Title or role of the individual or committee with authority to approve changes to this competency framework (e.g., Chief Human Resources Officer, Competency Governance Committee).',
    `approved_date` DATE COMMENT 'Date when the current version of the competency framework was formally approved for use.',
    `competency_count` STRING COMMENT 'Total number of individual competencies defined within this framework.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when the competency framework record was first created in the system.',
    `competency_framework_description` STRING COMMENT 'Detailed description of the competency framework including its purpose, scope, and intended application within the institution.',
    `documentation_location` STRING COMMENT 'File path, SharePoint location, or document management system reference where detailed framework documentation, competency definitions, and behavioral indicators are stored.',
    `effective_end_date` DATE COMMENT 'Date when this competency framework is no longer active or has been superseded by a newer version. Null indicates the framework is currently active with no planned end date.',
    `effective_start_date` DATE COMMENT 'Date when this competency framework becomes active and available for use in HR processes such as performance reviews, talent assessments, and development planning.',
    `external_reference_url` STRING COMMENT 'Web address linking to external documentation, industry standards, or source materials for this competency framework.',
    `framework_category` STRING COMMENT 'Organizational scope and applicability category of the competency framework.',
    `framework_code` STRING COMMENT 'Unique business identifier code for the competency framework used in external communications and system integrations.',
    `framework_name` STRING COMMENT 'Full name of the competency framework (e.g., Leadership Competency Framework, Technical Skills Framework).',
    `framework_type` STRING COMMENT 'Classification of the competency framework by its primary focus area.',
    `industry_alignment` STRING COMMENT 'Industry standards or frameworks with which this competency framework is aligned (e.g., SHRM Competency Model, O*NET, European Qualifications Framework).',
    `integration_system` STRING COMMENT 'Name of the primary HR system or platform where this competency framework is implemented (e.g., Workday HCM, SuccessFactors, PeopleSoft).',
    `language_code` STRING COMMENT 'ISO 639-1 two-letter language code (optionally with ISO 3166-1 country code) indicating the primary language of the competency framework content (e.g., en, en-US, fr, es).',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Date and time when the competency framework record was most recently updated.',
    `last_review_date` DATE COMMENT 'Date when the competency framework was most recently reviewed for accuracy, relevance, and alignment with organizational strategy.',
    `multi_language_support_flag` BOOLEAN COMMENT 'Indicates whether this competency framework has been translated and is available in multiple languages.',
    `next_review_date` DATE COMMENT 'Scheduled date for the next formal review of the competency framework.',
    `notes` STRING COMMENT 'Additional notes, comments, or contextual information about the competency framework including implementation guidance, known limitations, or special considerations.',
    `owner_department` STRING COMMENT 'Name of the department or organizational unit responsible for maintaining and governing this competency framework (e.g., Human Resources, Organizational Development).',
    `proficiency_levels` STRING COMMENT 'Number of proficiency or mastery levels defined in the framework rating scale (e.g., 3 for Basic/Intermediate/Advanced, 5 for a five-point scale).',
    `proficiency_scale_description` STRING COMMENT 'Textual description of the proficiency levels and rating scale used in this framework (e.g., 1=Awareness, 2=Basic, 3=Intermediate, 4=Advanced, 5=Expert).',
    `purpose_statement` STRING COMMENT 'Formal statement articulating the strategic purpose and business objectives the competency framework is designed to support.',
    `regulatory_compliance_flag` BOOLEAN COMMENT 'Indicates whether this competency framework is required to meet specific regulatory or accreditation requirements.',
    `regulatory_reference` STRING COMMENT 'Citation or reference to the regulatory requirement, accreditation standard, or compliance mandate that this framework addresses.',
    `review_cycle_months` STRING COMMENT 'Number of months between scheduled reviews of the competency framework to ensure continued relevance and alignment with institutional needs.',
    `source_organization` STRING COMMENT 'Name of the organization or body that developed or published the original competency framework (e.g., SHRM, CUPA-HR, Internal HR Development Team).',
    `source_type` STRING COMMENT 'Origin classification of the competency framework indicating whether it was developed internally, adopted from external sources, or customized from industry standards.',
    `competency_framework_status` STRING COMMENT 'Current lifecycle status of the competency framework indicating its availability for use in HR processes.',
    `target_audience` STRING COMMENT 'Description of the employee population or job families for which this competency framework is intended (e.g., All administrative staff, IT professionals, Senior leadership).',
    `usage_scope` STRING COMMENT 'HR processes and applications in which this competency framework is utilized. [ENUM-REF-CANDIDATE: recruitment|performance_management|succession_planning|learning_development|compensation|career_pathing|talent_assessment — promote to reference product]',
    `version_number` STRING COMMENT 'Version identifier for the competency framework following semantic versioning (e.g., 1.0, 2.1, 3.0.1).',
...
ALTER TABLE `education_ecm`.`hr`.`position` ADD CONSTRAINT `fk_hr_position_job_family_id` FOREIGN KEY (`job_family_id`) REFERENCES `education_ecm`.`hr`.`job_family`(`job_family_id`);
...
ALTER TABLE `education_ecm`.`hr`.`job_profile` ADD CONSTRAINT `fk_hr_job_profile_job_family_id` FOREIGN KEY (`job_family_id`) REFERENCES `education_ecm`.`hr`.`job_family`(`job_family_id`);
ALTER TABLE `education_ecm`.`hr`.`org_unit` ADD CONSTRAINT `fk_hr_org_unit_employee_id` FOREIGN KEY (`employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
...
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ADD CONSTRAINT `fk_hr_employee_compensation_compensation_plan_id` FOREIGN KEY (`compensation_plan_id`) REFERENCES `education_ecm`.`hr`.`compensation_plan`(`compensation_plan_id`);
...
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ADD CONSTRAINT `fk_hr_employee_compensation_pay_grade_id` FOREIGN KEY (`pay_grade_id`) REFERENCES `education_ecm`.`hr`.`pay_grade`(`pay_grade_id`);
...
ALTER TABLE `education_ecm`.`hr`.`benefit_enrollment` ADD CONSTRAINT `fk_hr_benefit_enrollment_benefit_program_id` FOREIGN KEY (`benefit_program_id`) REFERENCES `education_ecm`.`hr`.`benefit_program`(`benefit_program_id`);
...
ALTER TABLE `education_ecm`.`hr`.`payroll_result` ADD CONSTRAINT `fk_hr_payroll_result_payroll_run_id` FOREIGN KEY (`payroll_run_id`) REFERENCES `education_ecm`.`hr`.`payroll_run`(`payroll_run_id`);
...
ALTER TABLE `education_ecm`.`hr`.`absence_event` ADD CONSTRAINT `fk_hr_absence_event_accrual_plan_id` FOREIGN KEY (`accrual_plan_id`) REFERENCES `education_ecm`.`hr`.`accrual_plan`(`accrual_plan_id`);
...
ALTER TABLE `education_ecm`.`hr`.`recruitment_requisition` ADD CONSTRAINT `fk_hr_recruitment_requisition_job_family_id` FOREIGN KEY (`job_family_id`) REFERENCES `education_ecm`.`hr`.`job_family`(`job_family_id`);
...
ALTER TABLE `education_ecm`.`hr`.`hr_applicant` ADD CONSTRAINT `fk_hr_hr_applicant_quaternary_hr_prior_employee_id` FOREIGN KEY (`quaternary_hr_prior_employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
...
ALTER TABLE `education_ecm`.`hr`.`performance_review` ADD CONSTRAINT `fk_hr_performance_review_training_record_id` FOREIGN KEY (`training_record_id`) REFERENCES `education_ecm`.`hr`.`training_record`(`training_record_id`);
ALTER TABLE `education_ecm`.`hr`.`grievance` ADD CONSTRAINT `fk_hr_grievance_employee_id` FOREIGN KEY (`employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`grievance` ADD CONSTRAINT `fk_hr_grievance_grievance_investigator_employee_id` FOREIGN KEY (`grievance_investigator_employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`grievance` ADD CONSTRAINT `fk_hr_grievance_grievance_respondent_employee_id` FOREIGN KEY (`grievance_respondent_employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`grievance` ADD CONSTRAINT `fk_hr_grievance_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `education_ecm`.`hr`.`org_unit`(`org_unit_id`);
...
ALTER TABLE `education_ecm`.`hr`.`time_entry` ADD CONSTRAINT `fk_hr_time_entry_pay_period_id` FOREIGN KEY (`pay_period_id`) REFERENCES `education_ecm`.`hr`.`pay_period`(`pay_period_id`);
ALTER TABLE `education_ecm`.`hr`.`time_entry` ADD CONSTRAINT `fk_hr_time_entry_payroll_run_id` FOREIGN KEY (`payroll_run_id`) REFERENCES `education_ecm`.`hr`.`payroll_run`(`payroll_run_id`);
...
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `education_ecm`.`hr`.`org_unit`(`org_unit_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_position_id` FOREIGN KEY (`position_id`) REFERENCES `education_ecm`.`hr`.`position`(`position_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_employee_id` FOREIGN KEY (`employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_quaternary_onboarding_approved_by_employee_id` FOREIGN KEY (`quaternary_onboarding_approved_by_employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_quinary_onboarding_waived_by_employee_id` FOREIGN KEY (`quinary_onboarding_waived_by_employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_staffing_event_id` FOREIGN KEY (`staffing_event_id`) REFERENCES `education_ecm`.`hr`.`staffing_event`(`staffing_event_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_tertiary_onboarding_completed_by_employee_id` FOREIGN KEY (`tertiary_onboarding_completed_by_employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ADD CONSTRAINT `fk_hr_onboarding_task_prerequisite_onboarding_task_id` FOREIGN KEY (`prerequisite_onboarding_task_id`) REFERENCES `education_ecm`.`hr`.`onboarding_task`(`onboarding_task_id`);
ALTER TABLE `education_ecm`.`hr`.`training_record` ADD CONSTRAINT `fk_hr_training_record_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `education_ecm`.`hr`.`org_unit`(`org_unit_id`);
ALTER TABLE `education_ecm`.`hr`.`training_record` ADD CONSTRAINT `fk_hr_training_record_employee_id` FOREIGN KEY (`employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`training_record` ADD CONSTRAINT `fk_hr_training_record_renewal_training_record_id` FOREIGN KEY (`renewal_training_record_id`) REFERENCES `education_ecm`.`hr`.`training_record`(`training_record_id`);
ALTER TABLE `education_ecm`.`hr`.`accrual_plan` ADD CONSTRAINT `fk_hr_accrual_plan_superseded_accrual_plan_id` FOREIGN KEY (`superseded_accrual_plan_id`) REFERENCES `education_ecm`.`hr`.`accrual_plan`(`accrual_plan_id`);
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ADD CONSTRAINT `fk_hr_benefit_program_superseded_benefit_program_id` FOREIGN KEY (`superseded_benefit_program_id`) REFERENCES `education_ecm`.`hr`.`benefit_program`(`benefit_program_id`);
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` ADD CONSTRAINT `fk_hr_compensation_plan_superseded_compensation_plan_id` FOREIGN KEY (`superseded_compensation_plan_id`) REFERENCES `education_ecm`.`hr`.`compensation_plan`(`compensation_plan_id`);
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ADD CONSTRAINT `fk_hr_pay_grade_parent_pay_grade_id` FOREIGN KEY (`parent_pay_grade_id`) REFERENCES `education_ecm`.`hr`.`pay_grade`(`pay_grade_id`);
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ADD CONSTRAINT `fk_hr_payroll_run_employee_id` FOREIGN KEY (`employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ADD CONSTRAINT `fk_hr_payroll_run_org_unit_id` FOREIGN KEY (`org_unit_id`) REFERENCES `education_ecm`.`hr`.`org_unit`(`org_unit_id`);
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ADD CONSTRAINT `fk_hr_payroll_run_original_run_id` FOREIGN KEY (`original_run_id`) REFERENCES `education_ecm`.`hr`.`payroll_run`(`payroll_run_id`);
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ADD CONSTRAINT `fk_hr_payroll_run_payroll_calendar_id` FOREIGN KEY (`payroll_calendar_id`) REFERENCES `education_ecm`.`hr`.`payroll_calendar`(`payroll_calendar_id`);
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ADD CONSTRAINT `fk_hr_payroll_run_adjustment_payroll_run_id` FOREIGN KEY (`adjustment_payroll_run_id`) REFERENCES `education_ecm`.`hr`.`payroll_run`(`payroll_run_id`);
ALTER TABLE `education_ecm`.`hr`.`job_family` ADD CONSTRAINT `fk_hr_job_family_competency_framework_id` FOREIGN KEY (`competency_framework_id`) REFERENCES `education_ecm`.`hr`.`competency_framework`(`competency_framework_id`);
ALTER TABLE `education_ecm`.`hr`.`job_family` ADD CONSTRAINT `fk_hr_job_family_parent_job_family_id` FOREIGN KEY (`parent_job_family_id`) REFERENCES `education_ecm`.`hr`.`job_family`(`job_family_id`);
ALTER TABLE `education_ecm`.`hr`.`pay_period` ADD CONSTRAINT `fk_hr_pay_period_employee_id` FOREIGN KEY (`employee_id`) REFERENCES `education_ecm`.`hr`.`employee`(`employee_id`);
ALTER TABLE `education_ecm`.`hr`.`pay_period` ADD CONSTRAINT `fk_hr_pay_period_payroll_calendar_id` FOREIGN KEY (`payroll_calendar_id`) REFERENCES `education_ecm`.`hr`.`payroll_calendar`(`payroll_calendar_id`);
ALTER TABLE `education_ecm`.`hr`.`pay_period` ADD CONSTRAINT `fk_hr_pay_period_prior_pay_period_id` FOREIGN KEY (`prior_pay_period_id`) REFERENCES `education_ecm`.`hr`.`pay_period`(`pay_period_id`);
ALTER TABLE `education_ecm`.`hr`.`payroll_calendar` ADD CONSTRAINT `fk_hr_payroll_calendar_prior_payroll_calendar_id` FOREIGN KEY (`prior_payroll_calendar_id`) REFERENCES `education_ecm`.`hr`.`payroll_calendar`(`payroll_calendar_id`);
...
ALTER TABLE `education_ecm`.`hr`.`employee` ALTER COLUMN `job_profile_code` SET TAGS ('dbx_business_glossary_term' = 'Job Profile Code');
...
ALTER TABLE `education_ecm`.`hr`.`employee` ALTER COLUMN `position_number` SET TAGS ('dbx_business_glossary_term' = 'Position Number');
...
ALTER TABLE `education_ecm`.`hr`.`position` ALTER COLUMN `grant_account_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `education_ecm`.`hr`.`position` ALTER COLUMN `job_family_id` SET TAGS ('dbx_business_glossary_term' = 'Job Family ID');
...
ALTER TABLE `education_ecm`.`hr`.`job_profile` ALTER COLUMN `job_family_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `education_ecm`.`hr`.`job_profile` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Workday HCM Job Profile System ID');
...
ALTER TABLE `education_ecm`.`hr`.`job_profile` ALTER COLUMN `cip_code` SET TAGS ('dbx_business_glossary_term' = 'Classification of Instructional Programs (CIP) Code');
ALTER TABLE `education_ecm`.`hr`.`job_profile` ALTER COLUMN `cip_code` SET TAGS ('dbx_value_regex' = '^[0-9]{2}.[0-9]{4}$');
...
ALTER TABLE `education_ecm`.`hr`.`org_unit` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Manager ID');
ALTER TABLE `education_ecm`.`hr`.`org_unit` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`org_unit` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `education_ecm`.`hr`.`org_unit` ALTER COLUMN `cip_code` SET TAGS ('dbx_business_glossary_term' = 'Classification of Instructional Programs (CIP) Code');
ALTER TABLE `education_ecm`.`hr`.`org_unit` ALTER COLUMN `cip_code` SET TAGS ('dbx_value_regex' = '^[0-9]{2}.[0-9]{4}$');
...
ALTER TABLE `education_ecm`.`hr`.`org_unit` ALTER COLUMN `primary_campus_location` SET TAGS ('dbx_business_glossary_term' = 'Primary Campus Location');
...
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
...
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `compensation_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Compensation Plan ID');
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `compensation_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `compensation_plan_id` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `pay_grade_id` SET TAGS ('dbx_business_glossary_term' = 'Pay Grade ID');
...
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Workday Compensation Event ID');
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `event_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`hr`.`employee_compensation` ALTER COLUMN `event_id` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `education_ecm`.`hr`.`benefit_plan` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
...
ALTER TABLE `education_ecm`.`hr`.`benefit_enrollment` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
...
ALTER TABLE `education_ecm`.`hr`.`benefit_enrollment` ALTER COLUMN `benefit_program_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Program ID');
...
ALTER TABLE `education_ecm`.`hr`.`benefit_enrollment` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Event ID');
...
ALTER TABLE `education_ecm`.`hr`.`payroll_result` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
...
ALTER TABLE `education_ecm`.`hr`.`payroll_result` ALTER COLUMN `payroll_run_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Run ID');
...
ALTER TABLE `education_ecm`.`hr`.`payroll_result` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
...
ALTER TABLE `education_ecm`.`hr`.`payroll_result` ALTER COLUMN `fund_code` SET TAGS ('dbx_business_glossary_term' = 'Fund Code');
...
ALTER TABLE `education_ecm`.`hr`.`absence_event` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
...
ALTER TABLE `education_ecm`.`hr`.`absence_event` ALTER COLUMN `accrual_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Accrual Plan ID');
...
ALTER TABLE `education_ecm`.`hr`.`recruitment_requisition` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
...
ALTER TABLE `education_ecm`.`hr`.`recruitment_requisition` ALTER COLUMN `job_family_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `education_ecm`.`hr`.`recruitment_requisition` ALTER COLUMN `budget_account_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Account Code');
...
ALTER TABLE `education_ecm`.`hr`.`hr_applicant` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
...
ALTER TABLE `education_ecm`.`hr`.`hr_applicant` ALTER COLUMN `quaternary_hr_prior_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Prior Employee ID');
ALTER TABLE `education_ecm`.`hr`.`hr_applicant` ALTER COLUMN `quaternary_hr_prior_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`hr_applicant` ALTER COLUMN `quaternary_hr_prior_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `education_ecm`.`hr`.`performance_review` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
...
ALTER TABLE `education_ecm`.`hr`.`performance_review` ALTER COLUMN `training_record_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `education_ecm`.`hr`.`grievance` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`hr`.`grievance` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_id` SET TAGS ('dbx_business_glossary_term' = 'Grievance ID');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee ID');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_investigator_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Investigator Employee ID');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_investigator_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_investigator_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_respondent_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Respondent Employee ID');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_respondent_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_respondent_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `ada_accommodation_involved` SET TAGS ('dbx_business_glossary_term' = 'Americans with Disabilities Act (ADA) Accommodation Involved Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `appeal_decision_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Decision Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `appeal_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `appeal_status` SET TAGS ('dbx_business_glossary_term' = 'Grievance Appeal Status');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `appeal_status` SET TAGS ('dbx_value_regex' = 'not_appealed|appeal_filed|appeal_under_review|appeal_upheld|appeal_denied|arbitration_requested');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `closure_date` SET TAGS ('dbx_business_glossary_term' = 'Grievance Closure Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `disciplinary_action_type` SET TAGS ('dbx_business_glossary_term' = 'Disciplinary Action Type');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `disciplinary_action_type` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `eeoc_charge_filed` SET TAGS ('dbx_business_glossary_term' = 'Equal Employment Opportunity Commission (EEOC) Charge Filed Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `eeoc_charge_number` SET TAGS ('dbx_business_glossary_term' = 'Equal Employment Opportunity Commission (EEOC) Charge Number');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `eeoc_charge_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `external_counsel_engaged` SET TAGS ('dbx_business_glossary_term' = 'External Counsel Engaged Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `filing_date` SET TAGS ('dbx_business_glossary_term' = 'Grievance Filing Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `filing_date` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `filing_method` SET TAGS ('dbx_business_glossary_term' = 'Grievance Filing Method');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `filing_method` SET TAGS ('dbx_value_regex' = 'written|verbal|online_portal|email|anonymous');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `finding_summary` SET TAGS ('dbx_business_glossary_term' = 'Investigation Finding Summary');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `finding_summary` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `fmla_related` SET TAGS ('dbx_business_glossary_term' = 'Family and Medical Leave Act (FMLA) Related Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_status` SET TAGS ('dbx_business_glossary_term' = 'Grievance Status');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_type` SET TAGS ('dbx_business_glossary_term' = 'Grievance Type');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `grievance_type` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `hr_case_number` SET TAGS ('dbx_business_glossary_term' = 'Human Resources (HR) Case Number');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `hr_case_number` SET TAGS ('dbx_value_regex' = '^GRV-[0-9]{4}-[0-9]{6}$');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `hr_case_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `incident_date` SET TAGS ('dbx_business_glossary_term' = 'Incident Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `incident_date` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `investigation_close_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Close Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `investigation_finding` SET TAGS ('dbx_business_glossary_term' = 'Investigation Finding');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `investigation_finding` SET TAGS ('dbx_value_regex' = 'substantiated|unsubstantiated|inconclusive|no_jurisdiction|withdrawn_prior_to_finding');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `investigation_finding` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `investigation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Start Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `is_anonymous` SET TAGS ('dbx_business_glossary_term' = 'Anonymous Filing Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `is_union_grievance` SET TAGS ('dbx_business_glossary_term' = 'Union Grievance Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `legal_hold_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Hold Flag');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `mediation_outcome` SET TAGS ('dbx_business_glossary_term' = 'Mediation Outcome');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `mediation_outcome` SET TAGS ('dbx_value_regex' = 'settled|not_settled|withdrawn|not_applicable');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `mediation_outcome` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `mediation_requested` SET TAGS ('dbx_business_glossary_term' = 'Mediation Requested Indicator');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Grievance Priority Level');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `protected_class_basis` SET TAGS ('dbx_business_glossary_term' = 'Protected Class Basis');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `protected_class_basis` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `record_retention_date` SET TAGS ('dbx_business_glossary_term' = 'Record Retention Expiration Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Grievance Resolution Date');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `resolution_type` SET TAGS ('dbx_business_glossary_term' = 'Grievance Resolution Type');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `resolution_type` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `settlement_amount` SET TAGS ('dbx_business_glossary_term' = 'Settlement Amount');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `settlement_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `subtype` SET TAGS ('dbx_business_glossary_term' = 'Grievance Subtype');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `subtype` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `suspension_days` SET TAGS ('dbx_business_glossary_term' = 'Suspension Duration Days');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `suspension_days` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `union_contract_article` SET TAGS ('dbx_business_glossary_term' = 'Union Contract Article Reference');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `education_ecm`.`hr`.`grievance` ALTER COLUMN `workday_case_reference` SET TAGS ('dbx_business_glossary_term' = 'Workday Human Capital Management (HCM) Case ID');
ALTER TABLE `education_ecm`.`hr`.`time_entry` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`time_entry` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
...
ALTER TABLE `education_ecm`.`hr`.`time_entry` ALTER COLUMN `pay_period_id` SET TAGS ('dbx_business_glossary_term' = 'Pay Period Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`time_entry` ALTER COLUMN `payroll_run_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Run Identifier (ID)');
...
ALTER TABLE `education_ecm`.`hr`.`time_entry` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
...
ALTER TABLE `education_ecm`.`hr`.`time_entry` ALTER COLUMN `fund_code` SET TAGS ('dbx_business_glossary_term' = 'Fund Code');
...
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `onboarding_task_id` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Task Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `enterprise_application_id` SET TAGS ('dbx_business_glossary_term' = 'Enterprise Application Id (Foreign Key)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `position_id` SET TAGS ('dbx_business_glossary_term' = 'Position Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `quaternary_onboarding_approved_by_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `quaternary_onboarding_approved_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `quaternary_onboarding_approved_by_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `quinary_onboarding_waived_by_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Waived By Employee Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `quinary_onboarding_waived_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `quinary_onboarding_waived_by_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `staffing_event_id` SET TAGS ('dbx_business_glossary_term' = 'Staffing Event Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `tertiary_onboarding_completed_by_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Completed By Employee Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `tertiary_onboarding_completed_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `tertiary_onboarding_completed_by_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `prerequisite_onboarding_task_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `access_level` SET TAGS ('dbx_business_glossary_term' = 'System Access Level');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `approved_date` SET TAGS ('dbx_business_glossary_term' = 'Task Approved Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `assigned_date` SET TAGS ('dbx_business_glossary_term' = 'Task Assigned Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `assigned_to_role` SET TAGS ('dbx_business_glossary_term' = 'Assigned To Role');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `certificate_issued_flag` SET TAGS ('dbx_business_glossary_term' = 'Certificate Issued Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `completed_date` SET TAGS ('dbx_business_glossary_term' = 'Task Completed Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `compliance_framework` SET TAGS ('dbx_business_glossary_term' = 'Compliance Framework');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `document_received_date` SET TAGS ('dbx_business_glossary_term' = 'Document Received Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `document_received_flag` SET TAGS ('dbx_business_glossary_term' = 'Document Received Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `document_required` SET TAGS ('dbx_business_glossary_term' = 'Document Required Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `document_type` SET TAGS ('dbx_business_glossary_term' = 'Required Document Type');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Task Due Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `is_compliance_required` SET TAGS ('dbx_business_glossary_term' = 'Is Compliance Required Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `is_mandatory` SET TAGS ('dbx_business_glossary_term' = 'Is Mandatory Task Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `requires_approval` SET TAGS ('dbx_business_glossary_term' = 'Requires Approval Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `started_date` SET TAGS ('dbx_business_glossary_term' = 'Task Started Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `system_access_required` SET TAGS ('dbx_business_glossary_term' = 'System Access Required Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_instructions` SET TAGS ('dbx_business_glossary_term' = 'Task Instructions');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_name` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Task Name');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_notes` SET TAGS ('dbx_business_glossary_term' = 'Task Notes');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_number` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Task Number');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_priority` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Task Priority');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_priority` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_status` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Task Status');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `task_type` SET TAGS ('dbx_business_glossary_term' = 'Onboarding Task Type');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `training_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `training_module_name` SET TAGS ('dbx_business_glossary_term' = 'Training Module Name');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `training_required` SET TAGS ('dbx_business_glossary_term' = 'Training Required Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `waived_date` SET TAGS ('dbx_business_glossary_term' = 'Task Waived Date');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `waived_flag` SET TAGS ('dbx_business_glossary_term' = 'Task Waived Flag');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Task Waiver Reason');
ALTER TABLE `education_ecm`.`hr`.`onboarding_task` ALTER COLUMN `workday_task_reference` SET TAGS ('dbx_business_glossary_term' = 'Workday Task Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`training_record` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`hr`.`training_record` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_record_id` SET TAGS ('dbx_business_glossary_term' = 'Training Record Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Employee Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Training Program Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `renewal_training_record_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `attempts_count` SET TAGS ('dbx_business_glossary_term' = 'Training Attempts Count');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `certificate_issue_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Issue Date');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `certificate_issued_flag` SET TAGS ('dbx_business_glossary_term' = 'Certificate Issued Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `certificate_number` SET TAGS ('dbx_business_glossary_term' = 'Certificate Number');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `completion_date` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Date');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `compliance_framework` SET TAGS ('dbx_business_glossary_term' = 'Compliance Framework');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `compliance_requirement_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Requirement Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `contact_hours` SET TAGS ('dbx_business_glossary_term' = 'Training Contact Hours');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Training Cost Amount');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `credit_hours` SET TAGS ('dbx_business_glossary_term' = 'Training Credit Hours');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `delivery_method` SET TAGS ('dbx_business_glossary_term' = 'Training Delivery Method');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `delivery_method` SET TAGS ('dbx_value_regex' = 'online|in_person|hybrid|webinar|self_paced|instructor_led');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Training Due Date');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `employee_comments` SET TAGS ('dbx_business_glossary_term' = 'Employee Comments');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `enrollment_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Date');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Training Expiration Date');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `ferpa_training_flag` SET TAGS ('dbx_business_glossary_term' = 'Family Educational Rights and Privacy Act (FERPA) Training Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `funding_source` SET TAGS ('dbx_business_glossary_term' = 'Funding Source');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `instructor_name` SET TAGS ('dbx_business_glossary_term' = 'Instructor Name');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `passed_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Passed Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `passing_score` SET TAGS ('dbx_business_glossary_term' = 'Passing Score Threshold');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `safety_training_flag` SET TAGS ('dbx_business_glossary_term' = 'Safety Training Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `score` SET TAGS ('dbx_business_glossary_term' = 'Training Score');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Training Start Date');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `title_ix_training_flag` SET TAGS ('dbx_business_glossary_term' = 'Title IX Training Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_category` SET TAGS ('dbx_business_glossary_term' = 'Training Category');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_location` SET TAGS ('dbx_business_glossary_term' = 'Training Location');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_provider` SET TAGS ('dbx_business_glossary_term' = 'Training Provider');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_status` SET TAGS ('dbx_business_glossary_term' = 'Training Status');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_status` SET TAGS ('dbx_value_regex' = 'assigned|in_progress|completed|failed|waived|expired');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_title` SET TAGS ('dbx_business_glossary_term' = 'Training Title');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_type` SET TAGS ('dbx_business_glossary_term' = 'Training Type');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `training_type` SET TAGS ('dbx_value_regex' = 'required|elective|professional_development|compliance|safety|orientation');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `waiver_granted_flag` SET TAGS ('dbx_business_glossary_term' = 'Waiver Granted Flag');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `education_ecm`.`hr`.`training_record` ALTER COLUMN `workday_training_reference` SET TAGS ('dbx_business_glossary_term' = 'Workday Training Identifier (ID)');
ALTER TABLE `education_ecm`.`hr`.`accrual_plan` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`accrual_plan` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`accrual_plan` ALTER COLUMN `accrual_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Accrual Plan Identifier');
ALTER TABLE `education_ecm`.`hr`.`accrual_plan` ALTER COLUMN `superseded_accrual_plan_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `benefit_program_id` SET TAGS ('dbx_business_glossary_term' = 'Benefit Program Identifier');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `superseded_benefit_program_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `carrier_policy_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `employer_contribution_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `employer_contribution_percentage` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `program_administrator_contact` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`benefit_program` ALTER COLUMN `program_administrator_contact` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` ALTER COLUMN `compensation_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Compensation Plan Identifier');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` ALTER COLUMN `superseded_compensation_plan_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` ALTER COLUMN `base_salary_maximum` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` ALTER COLUMN `base_salary_midpoint` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`compensation_plan` ALTER COLUMN `base_salary_minimum` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `pay_grade_id` SET TAGS ('dbx_business_glossary_term' = 'Pay Grade Identifier');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `parent_pay_grade_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `hourly_rate_maximum` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `hourly_rate_minimum` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `maximum_annual_salary` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `midpoint_annual_salary` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `minimum_annual_salary` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`pay_grade` ALTER COLUMN `step_increment_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`hr`.`payroll_run` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`payroll_run` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ALTER COLUMN `payroll_run_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Run Identifier');
ALTER TABLE `education_ecm`.`hr`.`payroll_run` ALTER COLUMN `adjustment_payroll_run_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`job_family` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`job_family` SET TAGS ('dbx_subdomain' = 'workforce_administration');
ALTER TABLE `education_ecm`.`hr`.`job_family` ALTER COLUMN `job_family_id` SET TAGS ('dbx_business_glossary_term' = 'Job Family Identifier');
ALTER TABLE `education_ecm`.`hr`.`pay_period` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`pay_period` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`pay_period` ALTER COLUMN `pay_period_id` SET TAGS ('dbx_business_glossary_term' = 'Pay Period Identifier');
ALTER TABLE `education_ecm`.`hr`.`pay_period` ALTER COLUMN `prior_pay_period_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`payroll_calendar` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`payroll_calendar` SET TAGS ('dbx_subdomain' = 'compensation_payroll');
ALTER TABLE `education_ecm`.`hr`.`payroll_calendar` ALTER COLUMN `payroll_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Payroll Calendar Identifier');
ALTER TABLE `education_ecm`.`hr`.`payroll_calendar` ALTER COLUMN `prior_payroll_calendar_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`hr`.`competency_framework` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`hr`.`competency_framework` SET TAGS ('dbx_subdomain' = 'talent_acquisition');
ALTER TABLE `education_ecm`.`hr`.`competency_framework` ALTER COLUMN `competency_framework_id` SET TAGS ('dbx_business_glossary_term' = 'Competency Framework Identifier');
