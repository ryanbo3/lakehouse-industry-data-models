-- Schema for Domain: instruction | Business: Education | Version: v1_ecm
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`course_section` (
    ...
    `llc_program_id` BIGINT COMMENT 'Foreign key linking to studentlife.llc_program. Business justification: Living-Learning Community programs require specific course enrollments (honors seminars, themed first-year experiences). Registrar systems restrict section access to LLC participants, enforce curricul',
    ...
    `statement_id` BIGINT COMMENT 'Foreign key linking to billing.billing_statement. Business justification: Billing statements itemize charges by enrolled course section. Registrar and bursar offices use this link for enrollment verification, statement accuracy audits, and processing drop/add adjustments th',
    ...
    `tuition_charge_id` BIGINT COMMENT 'Foreign key linking to billing.tuition_charge. Business justification: Course sections generate tuition charges upon enrollment. Bursar offices require this link for drop/add refund calculations, enrollment verification on billing statements, and Title IV return-of-funds',
    ...
    `college_code` STRING COMMENT 'Code identifying the college or school offering this section. Higher-level organizational unit above department.. Valid values are `^[A-Z]{2,4}$`',
    ...
    `fee_amount` DECIMAL(18,2) COMMENT 'Additional course-specific fees in US dollars (e.g., lab fees, materials fees, technology fees) charged for this section.',
    ...
    `linked_sections` STRING COMMENT 'Comma-separated list of CRNs for sections that must be taken concurrently (e.g., lecture and lab). Students must register for all linked sections together.',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`section_meeting` (
    ...
    `scorm_package_id` BIGINT COMMENT 'Identifier of SCORM-compliant learning content associated with this meeting. Used for tracking standardized e-learning modules.',
    ...
    `building_code` STRING COMMENT 'Code identifying the building where the meeting is held. Null for online or arranged meetings.. Valid values are `^[A-Z0-9]{2,10}$`',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`faculty_assignment` (
    ...
    `campus_event_id` BIGINT COMMENT 'Foreign key linking to studentlife.campus_event. Business justification: Faculty receive workload credit for event facilitation (symposia, guest lectures, orientation sessions). HR systems track compensable event assignments for contract compliance, overload pay calculatio',
    `compliance_training_completion_id` BIGINT COMMENT 'Foreign key linking to compliance.training_completion. Business justification: Faculty must complete mandatory compliance training (Title IX, FERPA, Clery, research ethics) before teaching assignments are finalized. HR and academic departments verify training completion as prere',
    ...
    `identity_account_id` BIGINT COMMENT 'Foreign key linking to technology.identity_account. Business justification: Faculty assignments trigger access provisioning to LMS, email, and campus systems. Business needs this link for automated role-based access control, entitlement assignment, and timely deprovisioning w',
    ...
    `instruction_assignment_id` BIGINT COMMENT 'The native assignment identifier from Ellucian Banner Student Information System. Used for cross-system reconciliation and data lineage tracking.',
    ...
    `grant_account_code` STRING COMMENT 'The financial account or grant code to which this faculty assignments costs should be charged, if grant-funded. Null if not applicable.',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`lms_course_shell` (
    ...
    `enterprise_application_id` BIGINT COMMENT 'Foreign key linking to technology.enterprise_application. Business justification: LMS platforms (Canvas, Blackboard, Moodle) are enterprise applications. Business needs this link for vendor management, license tracking, support ticket routing, integration planning, and contract ren',
    `it_service_id` BIGINT COMMENT 'Foreign key linking to technology.it_service. Business justification: LMS course shells are hosted on IT services (Canvas service, Blackboard service). Business needs this link for SLA monitoring, incident impact analysis, capacity planning, service outage notifications',
    `course_id` BIGINT COMMENT 'Native Canvas system identifier for the course shell. This is the internal Canvas ID used in API calls and Canvas URLs.',
    `tertiary_lms_blueprint_course_id` BIGINT COMMENT 'Reference to the parent blueprint course if this course shell is associated with a Canvas blueprint. Blueprint courses serve as templates that push content and settings to associated child courses. Null if not a blueprint-associated course.',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`instruction_assignment` (
    `instruction_assignment_id` BIGINT COMMENT 'Unique identifier for the assignment record. Primary key for the assignment entity.',
    ...
    `enterprise_application_id` BIGINT COMMENT 'Reference to the external tool configuration in the LMS. Used for LTI launches and grade passback from external assessment platforms.',
    `identity_account_id` BIGINT COMMENT 'Reference to the faculty or staff member who last modified the assignment. Tracks change history and supports collaborative course design.',
    ...
    `tuition_charge_id` BIGINT COMMENT 'Foreign key linking to billing.tuition_charge. Business justification: Some institutions assess assignment-specific fees (lab fees, proctoring fees, course material charges). This link supports itemized billing for assignment-level charges and enables refund processing w',
    `wellness_program_id` BIGINT COMMENT 'Foreign key linking to studentlife.wellness_program. Business justification: Wellness programs offered for academic credit assign coursework (stress management journals, nutrition logs). LMS assignment records must link to wellness program participation for grade calculation, ',
    ...
    CONSTRAINT pk_instruction_assignment PRIMARY KEY(`instruction_assignment_id`)
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`instruction_submission` (
    `instruction_submission_id` BIGINT COMMENT 'Unique identifier for the student submission record. Primary key for the submission entity.',
    `ada_accommodation_id` BIGINT COMMENT 'Foreign key linking to compliance.ada_accommodation. Business justification: Students with ADA accommodations receive modified submission requirements (extended deadlines, alternative formats). Instructors and LMS systems must track which accommodations apply to each submissio',
    ...
    `instruction_assignment_id` BIGINT COMMENT 'Reference to the assignment for which this submission was made. Links to the assignment entity in the instruction domain.',
    ...
    `wellness_participation_id` BIGINT COMMENT 'Foreign key linking to studentlife.wellness_participation. Business justification: Students submit wellness program artifacts (reflection essays, activity logs) through LMS. Tracking submission compliance is required for mandatory wellness requirements tied to conduct sanctions, ath',
    ...
    CONSTRAINT pk_instruction_submission PRIMARY KEY(`instruction_submission_id`)
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`grade_entry` (
    ...
    `instruction_assignment_id` BIGINT COMMENT 'Identifier of the specific assignment or assessment for which this grade was awarded. Links to the assignment definition.',
    ...
    `slo_alignment` STRING COMMENT 'Mapping of this grade entry to specific Student Learning Outcomes (SLOs) assessed by this assignment. Stored as a structured text representation linking to SLO identifiers.',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`final_grade` (
    ...
    `studentlife_housing_assignment_id` BIGINT COMMENT 'Foreign key linking to studentlife.housing_assignment. Business justification: Housing eligibility is tied to academic standing (minimum GPA requirements). Residential life systems check final grades for contract renewal decisions, scholarship housing continuation, and LLC parti',
    `tuition_charge_id` BIGINT COMMENT 'Foreign key linking to billing.tuition_charge. Business justification: Grade-dependent refund policies (no refund if credit earned) and SAP billing adjustments require linking final grades to original tuition charges. Financial aid offices use this for satisfactory acade',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`rubric` (
    ...
    `identity_account_id` BIGINT COMMENT 'Identifier of the user who most recently modified this rubric. Supports audit trail and change management.',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`slo_assessment` (
    ...
    `instruction_assignment_id` BIGINT COMMENT 'External identifier for the Canvas LMS assignment or activity associated with this SLO assessment.',
...
CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`scorm_package` (
    `scorm_package_id` BIGINT COMMENT 'Unique identifier for the SCORM package. Primary key for the scorm_package product.',
    `course_section_id` BIGINT COMMENT 'Reference to the Canvas LMS (Learning Management System) course section where this SCORM package is deployed. Links the package to the instructional delivery context.',
    `employee_id` BIGINT COMMENT 'Reference to the user (faculty, instructional designer, or administrator) who uploaded or created the SCORM package record. Used for accountability and support routing.',
    `enterprise_application_id` BIGINT COMMENT 'Foreign key linking to technology.enterprise_application. Business justification: SCORM packages are delivered via enterprise learning platforms (separate from or integrated with LMS). Business needs this link for content licensing, vendor support, platform migration planning, and ',
    `finance_vendor_id` BIGINT COMMENT 'Foreign key linking to finance.vendor. Business justification: SCORM packages are purchased digital content from vendors. Required for procurement spend analysis, vendor performance evaluation, license renewal tracking, and educational technology budget reporting',
    `identity_account_id` BIGINT COMMENT 'Reference to the user who last modified the SCORM package record. Used for change management and audit purposes.',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to finance.purchase_order. Business justification: SCORM packages are procured assets requiring PO tracking for spend accountability, contract compliance verification, asset lifecycle management, and procurement audits. Essential for IT procurement re',
    `accessibility_features` STRING COMMENT 'Comma-separated list or textual description of accessibility features included in the SCORM package such as closed captions, screen reader compatibility, keyboard navigation, and alternative text. Used for ADA (Americans with Disabilities Act) compliance and student accommodation.',
    `assessment_type` STRING COMMENT 'Type of assessment embedded within the SCORM package. Indicates whether the package includes quizzes, exams, or other evaluation mechanisms and their pedagogical purpose.. Valid values are `formative|summative|diagnostic|none`',
    `average_completion_rate_percent` DECIMAL(18,2) COMMENT 'Historical average percentage of students who complete the SCORM package across all deployments. Used for content effectiveness evaluation and instructional design improvement.',
    `average_mastery_score` DECIMAL(18,2) COMMENT 'Historical average score achieved by students on assessments within the SCORM package. Used for content difficulty calibration and SLO (Student Learning Outcome) assessment.',
    `cip_code` STRING COMMENT 'Six-digit CIP (Classification of Instructional Programs) code identifying the instructional program area to which this SCORM package belongs. Used for IPEDS (Integrated Postsecondary Education Data System) reporting and federal compliance.. Valid values are `^d{2}.d{4}$`',
    `completion_threshold_percent` DECIMAL(18,2) COMMENT 'Minimum percentage of content that a student must complete for the SCORM package to be marked as complete. Typically ranges from 0 to 100 percent. Used for SLO (Student Learning Outcome) tracking and satisfactory academic progress.',
    `cost_per_student` DECIMAL(18,2) COMMENT 'Licensing or usage cost per student for proprietary SCORM packages. Zero for OER (Open Educational Resources). Used for COA (Cost of Attendance) calculations and budget planning.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this SCORM package record was first created in the institutional data system. Used for audit trail and data lineage tracking.',
    `estimated_duration_minutes` STRING COMMENT 'Expected time in minutes for a typical student to complete the SCORM package. Used for course planning, workload estimation, and credit hour compliance.',
    `file_size_mb` DECIMAL(18,2) COMMENT 'Total file size of the SCORM package in megabytes. Used for storage planning, bandwidth estimation, and student download time considerations.',
    `is_mobile_compatible` BOOLEAN COMMENT 'Boolean flag indicating whether the SCORM package is optimized for mobile devices (smartphones and tablets). Used for student accessibility and BYOD (Bring Your Own Device) support.',
    `is_oer` BOOLEAN COMMENT 'Boolean flag indicating whether this SCORM package is an OER (Open Educational Resource) available under an open license. Used for cost-of-attendance reduction initiatives and institutional OER (Open Educational Resources) reporting.',
    `language_code` STRING COMMENT 'ISO 639-1 two-letter language code (optionally with ISO 3166-1 country code) indicating the primary language of instruction for the SCORM package. Used for accessibility and international student support.. Valid values are `^[a-z]{2}(-[A-Z]{2})?$`',
    `last_reviewed_date` DATE COMMENT 'Date when the SCORM package was last reviewed for quality, accuracy, and alignment with current curriculum standards. Used for content lifecycle management and QA (Quality Assurance) tracking.',
    `launch_url` STRING COMMENT 'The URL endpoint used to launch the SCORM package within the LMS (Learning Management System). This is the entry point for student interaction with the content.',
    `learning_objectives` STRING COMMENT 'Detailed list or description of SLO (Student Learning Outcomes) that students are expected to achieve upon completion of the SCORM package. Used for assessment mapping and accreditation reporting.',
    `license_type` STRING COMMENT 'The intellectual property license under which the SCORM package is distributed. Determines usage rights, redistribution permissions, and attribution requirements. [ENUM-REF-CANDIDATE: proprietary|CC BY|CC BY-SA|CC BY-NC|CC BY-NC-SA|CC0|public domain — 7 candidates stripped; promote to reference product]',
    `mastery_score_threshold` DECIMAL(18,2) COMMENT 'Minimum score a student must achieve on assessments within the SCORM package to be considered to have mastered the content. Expressed as a percentage or raw score depending on package configuration. Used for competency-based education and SLO (Student Learning Outcome) assessment.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this SCORM package record was last modified in the institutional data system. Used for change tracking and data quality monitoring.',
    `next_review_date` DATE COMMENT 'Scheduled date for the next quality assurance review of the SCORM package. Used for proactive content maintenance and compliance with institutional review cycles.',
    `notes` STRING COMMENT 'Free-text field for internal notes, implementation guidance, known issues, or special instructions related to the SCORM package. Used by instructional designers and faculty for operational context.',
    `package_description` STRING COMMENT 'Detailed textual description of the SCORM package content, learning objectives, and instructional approach. Used for faculty selection and student preview.',
    `package_format` STRING COMMENT 'File format or compression method used for the SCORM package distribution. Common formats include ZIP archives and IMS Common Cartridge (imscc).. Valid values are `zip|imscc|tar.gz|other`',
    `package_identifier` STRING COMMENT 'Unique business identifier or code for the SCORM package, often assigned by the content provider or institutional catalog system. Used for inventory tracking and content management.',
    `package_status` STRING COMMENT 'Current lifecycle status of the SCORM package. Indicates whether the package is available for deployment, retired, or undergoing quality assurance review.. Valid values are `active|inactive|archived|under review|deprecated|pilot`',
    `package_title` STRING COMMENT 'Human-readable title of the SCORM package as displayed in the LMS (Learning Management System). This is the primary identifier for instructors and students.',
    `package_version` STRING COMMENT 'Version number or identifier assigned by the content provider to track updates and revisions to the SCORM package. Used for change management and quality assurance.',
    `plugin_name` STRING COMMENT 'Name and version of the browser plugin required to run the SCORM package, if applicable. Used for student technical guidance and IT support.',
    `prerequisite_knowledge` STRING COMMENT 'Textual description of prior knowledge, skills, or coursework that students should have before engaging with this SCORM package. Used for advising and course sequencing.',
    `requires_plugin` BOOLEAN COMMENT 'Boolean flag indicating whether the SCORM package requires browser plugins (e.g., Flash, Java) to function. Used for technical support planning and student device readiness assessment.',
    `standard_version` STRING COMMENT 'The technical standard and version that this package conforms to. Determines compatibility with LMS (Learning Management System) and tracking capabilities.. Valid values are `SCORM 1.2|SCORM 2004 3rd Edition|SCORM 2004 4th Edition|xAPI|cmi5|AICC`',
    `subject_area` STRING COMMENT 'Academic discipline or subject area that the SCORM package covers. May align with CIP (Classification of Instructional Programs) codes or institutional taxonomy. Used for content discovery and curriculum mapping.',
    `target_audience` STRING COMMENT 'Intended student or learner population for the SCORM package. Used for content recommendation and catalog filtering.. Valid values are `undergraduate|graduate|professional|continuing education|faculty development|all`',
    `upload_date` DATE COMMENT 'Date when the SCORM package was uploaded to the Canvas LMS (Learning Management System) or institutional content repository. Used for version control and audit purposes.',
    `usage_count` STRING COMMENT 'Total number of times the SCORM package has been deployed or launched across all course sections. Used for ROI (Return on Investment) analysis and content popularity tracking.',
    CONSTRAINT pk_scorm_package PRIMARY KEY(`scorm_package_id`)
) COMMENT 'SCORM (Sharable Content Object Reference Model) or xAPI/LTI content package deployed within a Canvas LMS course shell. Captures package title, standard version (SCORM 1.2, SCORM 2004, xAPI/Tin Can, cmi5), content provider, upload date, completion threshold, mastery score threshold, launch URL, package status, and OER (Open Educational Resource) flag. Represents the reusable digital learning object inventory available for instructional deployment.';

CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`grade_change_log` (
    `grade_change_log_id` BIGINT COMMENT 'Unique identifier for the grade change audit record. Primary key for the grade change log entity.',
    `grade_appeal_case_id` BIGINT COMMENT 'Reference identifier for the formal grade appeal case if this change resulted from a student grade appeal process. Null if not appeal-related.',
    `audit_finding_id` BIGINT COMMENT 'Foreign key linking to compliance.audit_finding. Business justification: Grade changes are frequent subjects of internal and external audits. Audit findings identify improper changes, missing approvals, or policy violations. Linking findings to specific grade changes enabl',
    `conduct_case_id` BIGINT COMMENT 'Reference identifier for the academic integrity investigation or hearing if this grade change resulted from an academic misconduct case resolution. Null if not integrity-related.',
    `course_section_id` BIGINT COMMENT 'Identifier of the course section in which the grade was changed. Links to the specific class offering where the student was enrolled.',
    `employee_id` BIGINT COMMENT 'Identifier of the registrar, dean, department chair, or other authorized administrator who approved the grade change. Links to the employee or faculty master record.',
    `instruction_assignment_id` BIGINT COMMENT 'Identifier of the specific assignment or assessment whose grade was changed. Null if the change applies to the final course grade rather than an individual assignment.',
    `original_change_log_grade_change_log_id` BIGINT COMMENT 'If this record is a reversal, references the grade_change_log_id of the original change being reversed. Null if not a reversal.',
    `profile_id` BIGINT COMMENT 'Unique identifier of the student whose grade was changed. Links to the student master record.',
    `instructor_id` BIGINT COMMENT 'Identifier of the faculty member who initiated the grade change request. Links to the faculty master record.',
    `academic_standing_impact` STRING COMMENT 'Describes the impact of this grade change on the students academic standing status, if any. Indicates transitions between good standing, probation, suspension, or dismissal.. Valid values are `none|probation_to_good_standing|good_standing_to_probation|probation_to_suspension|suspension_to_probation|dismissal_to_reinstatement`',
    `accreditation_reportable` BOOLEAN COMMENT 'Indicates whether this grade change must be included in accreditation reporting or audit documentation for regional or programmatic accreditation bodies (HLC, SACSCOC, ABET, AACSB, etc.). True if reportable.',
    `approval_timestamp` TIMESTAMP COMMENT 'Date and time when the grade change was officially approved by the authorized administrator. Represents the point at which the change became effective.',
    `approving_authority_role` STRING COMMENT 'The organizational role or title of the person who approved the grade change, indicating their level of authority in the approval hierarchy.. Valid values are `registrar|dean|department_chair|program_director|provost|academic_affairs`',
    `banner_transaction_ref` STRING COMMENT 'The unique transaction identifier generated by Ellucian Banner SIS when the grade change was processed in the source system. Enables traceability to the system of record.',
    `change_justification` STRING COMMENT 'Detailed narrative explanation provided by the requesting party describing the specific circumstances and rationale for the grade change. Required for audit trail and compliance purposes.',
    `change_reason_code` STRING COMMENT 'Standardized code indicating the category of reason for the grade change: clerical error, calculation error, late submission accepted, grade appeal outcome, incomplete grade conversion, academic integrity resolution, instructor discretion, or administrative adjustment. [ENUM-REF-CANDIDATE: clerical_error|calculation_error|late_submission|grade_appeal|incomplete_conversion|academic_integrity|instructor_discretion|administrative_adjustment — 8 candidates stripped; promote to reference product]',
    `change_status` STRING COMMENT 'Current workflow status of the grade change request: pending approval, approved and applied, rejected, reversed (undone), or under review by committee.. Valid values are `pending|approved|rejected|reversed|under_review`',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this audit record was first created in the data warehouse. Represents when the grade change event was captured in the analytics system.',
    `degree_audit_impact` BOOLEAN COMMENT 'Indicates whether this grade change affects the students degree audit or progress toward graduation requirements. True if degree audit must be re-evaluated.',
    `effective_date` DATE COMMENT 'The date on which the grade change officially takes effect for transcript and GPA calculation purposes. May differ from approval timestamp for retroactive changes.',
    `gpa_recalculation_required` BOOLEAN COMMENT 'Indicates whether this grade change requires recalculation of the students cumulative, term, or major GPA. True if recalculation is needed.',
    `grade_type` STRING COMMENT 'Classification of the grade being changed: final course grade, midterm grade, individual assignment, quiz, exam, project, or participation grade. [ENUM-REF-CANDIDATE: final|midterm|assignment|quiz|exam|project|participation — 7 candidates stripped; promote to reference product]',
    `lms_gradebook_sync_flag` BOOLEAN COMMENT 'Indicates whether the grade change has been synchronized back to the Canvas LMS gradebook. True if synced, False if pending or not applicable.',
    `lms_sync_timestamp` TIMESTAMP COMMENT 'Date and time when the grade change was synchronized to the Canvas LMS gradebook. Null if not yet synced or not applicable.',
    `modified_timestamp` TIMESTAMP COMMENT 'Date and time when this audit record was last updated in the data warehouse. Tracks any subsequent modifications to the grade change record.',
    `notification_sent_flag` BOOLEAN COMMENT 'Indicates whether the student has been officially notified of the grade change via email or student portal. True if notification has been sent.',
    `notification_timestamp` TIMESTAMP COMMENT 'Date and time when the student notification about the grade change was sent. Null if notification has not been sent.',
    `original_grade` STRING COMMENT 'The grade value before the change was applied. May be letter grade (A, B+, C), numeric score, or special code (I for Incomplete, W for Withdrawal, P for Pass).',
    `original_grade_points` DECIMAL(18,2) COMMENT 'The numeric grade point value associated with the original grade on the institutions GPA (Grade Point Average) scale, typically 0.00 to 4.00.',
    `request_timestamp` TIMESTAMP COMMENT 'Date and time when the grade change request was submitted by the instructor. Represents the initiation of the grade change workflow.',
    `reversal_flag` BOOLEAN COMMENT 'Indicates whether this record represents a reversal of a previous grade change. True if this is a reversal transaction.',
    `revised_grade` STRING COMMENT 'The new grade value after the change was applied. May be letter grade, numeric score, or special code.',
    `revised_grade_points` DECIMAL(18,2) COMMENT 'The numeric grade point value associated with the revised grade on the institutions GPA scale.',
    `sap_recalculation_required` BOOLEAN COMMENT 'Indicates whether this grade change requires recalculation of the students Satisfactory Academic Progress status for Title IV financial aid eligibility. True if SAP recalculation is needed.',
    `supporting_documentation_url` STRING COMMENT 'URL or file path to supporting documentation for the grade change (e.g., scanned forms, email correspondence, appeal decision letters) stored in the document management system.',
    `term_code` STRING COMMENT 'Six-digit academic term code identifying the semester or session in which the grade change occurred (e.g., 202401 for Spring 2024).. Valid values are `^[0-9]{6}$`',
    `transcript_notation_required` BOOLEAN COMMENT 'Indicates whether a special notation must be added to the students official transcript to document this grade change. True if notation is required per institutional policy.',
    CONSTRAINT pk_grade_change_log PRIMARY KEY(`grade_change_log_id`)
) COMMENT 'Audit record of every change made to a students final or assignment grade, capturing original grade, revised grade, change reason code, change justification narrative, requesting instructor, approving registrar or dean, approval timestamp, and Banner grade change transaction ID. Supports FERPA compliance, academic integrity investigations, and accreditation audit trails. Required for Title IV SAP recalculations when grades are retroactively changed.';

CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`incomplete_contract` (
    `incomplete_contract_id` BIGINT COMMENT 'Unique identifier for the incomplete contract agreement. Primary key for the incomplete contract entity.',
    `room_id` BIGINT COMMENT 'Foreign key linking to facility.room. Business justification: Incomplete contracts for lab/studio courses require continued access to specific rooms for work completion. Real process: facilities must grant card access and schedule availability for students compl',
    `course_section_id` BIGINT COMMENT 'Unique identifier for the course section in which the incomplete grade was assigned.',
    `employee_id` BIGINT COMMENT 'Unique identifier for the academic authority (department chair, dean, registrar) who approved the extension or contract modification.',
    `identity_account_id` BIGINT COMMENT 'Unique identifier for the system user who created this incomplete contract record.',
    `instructor_id` BIGINT COMMENT 'Unique identifier for the faculty member who assigned the incomplete grade and is party to this contract.',
    `modified_by_user_identity_account_id` BIGINT COMMENT 'Unique identifier for the system user who last modified this incomplete contract record.',
    `policy_id` BIGINT COMMENT 'Foreign key linking to compliance.policy. Business justification: Incomplete grade contracts must comply with institutional academic policies governing eligibility criteria, deadline extensions, default grades, and approval workflows. Registrars and academic affairs',
    `profile_id` BIGINT COMMENT 'Unique identifier for the student who received the incomplete grade and entered into this contract.',
    `tuition_charge_id` BIGINT COMMENT 'Foreign key linking to billing.tuition_charge. Business justification: Incomplete contracts trigger conditional billing holds and affect refund eligibility. Linking contracts to original tuition charges enables automated hold placement, refund processing upon contract re',
    `advisor_notification_date` DATE COMMENT 'Date when the academic advisor was notified of the incomplete contract.',
    `advisor_notified_flag` BOOLEAN COMMENT 'Indicates whether the students academic advisor has been notified of the incomplete contract.',
    `completion_deadline` DATE COMMENT 'Final date by which the student must complete all required work to resolve the incomplete grade.',
    `contract_notes` STRING COMMENT 'Additional notes, comments, or special conditions documented in the incomplete contract agreement.',
    `contract_number` STRING COMMENT 'Human-readable business identifier for the incomplete contract (format: IC-YYYY-NNNNNN).. Valid values are `^IC-[0-9]{4}-[0-9]{6}$`',
    `contract_signed_date` DATE COMMENT 'Date when both the student and instructor signed the incomplete contract agreement.',
    `contract_status` STRING COMMENT 'Current lifecycle status of the incomplete contract agreement.. Valid values are `pending|active|completed|expired|extended|cancelled`',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this incomplete contract record was first created in the system.',
    `credit_hours` DECIMAL(18,2) COMMENT 'Number of credit hours associated with the course section for which the incomplete was assigned.',
    `default_grade` STRING COMMENT 'The grade that will be automatically assigned if the student fails to complete the required work by the deadline.. Valid values are `^[A-F]$|^W$|^WF$`',
    `department_code` STRING COMMENT 'Code identifying the academic department offering the course section for which the incomplete was assigned.. Valid values are `^[A-Z]{2,4}$`',
    `extended_deadline` DATE COMMENT 'New completion deadline if an extension was granted.',
    `extension_approval_date` DATE COMMENT 'Date when the extension request was approved.',
    `extension_approved_flag` BOOLEAN COMMENT 'Indicates whether the extension request was approved by the instructor or academic authority.',
    `extension_reason` STRING COMMENT 'Documented reason or justification provided by the student for requesting a deadline extension.',
    `extension_request_date` DATE COMMENT 'Date when the student submitted a request for deadline extension.',
    `extension_requested_flag` BOOLEAN COMMENT 'Indicates whether the student has requested an extension to the original completion deadline.',
    `financial_aid_hold_flag` BOOLEAN COMMENT 'Indicates whether a financial aid hold has been placed on the students account due to this unresolved incomplete.',
    `graduation_hold_flag` BOOLEAN COMMENT 'Indicates whether this unresolved incomplete prevents the student from graduating.',
    `instructor_signature_timestamp` TIMESTAMP COMMENT 'Timestamp when the instructor electronically or physically signed the incomplete contract.',
    `lms_assignment_reference` STRING COMMENT 'Reference to specific Canvas LMS assignments or coursework that must be completed to resolve the incomplete.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this incomplete contract record was last modified.',
    `original_grade` STRING COMMENT 'The original incomplete grade assigned (typically I for Incomplete or IP for In Progress).. Valid values are `^I$|^IP$`',
    `original_grade_date` DATE COMMENT 'Date when the incomplete grade was originally assigned by the instructor.',
    `registration_hold_flag` BOOLEAN COMMENT 'Indicates whether a registration hold prevents the student from enrolling in future terms until the incomplete is resolved.',
    `required_work_description` STRING COMMENT 'Detailed description of the coursework, assignments, exams, or projects the student must complete to resolve the incomplete grade.',
    `resolution_date` DATE COMMENT 'Date when the incomplete contract was resolved and the final grade was posted.',
    `resolution_method` STRING COMMENT 'Method by which the incomplete contract was resolved.. Valid values are `work_completed|deadline_expired|administrative_change|student_withdrawal`',
    `resolved_grade` STRING COMMENT 'The final grade assigned upon resolution of the incomplete contract, either through completion of work or expiration of deadline.. Valid values are `^[A-F]$|^W$|^WF$|^P$|^NP$`',
    `sap_impact_flag` BOOLEAN COMMENT 'Indicates whether this incomplete contract impacts the students Satisfactory Academic Progress status for financial aid eligibility.',
    `sis_contract_ref` STRING COMMENT 'External identifier for this incomplete contract in the source Student Information System (e.g., Ellucian Banner).',
    `student_signature_timestamp` TIMESTAMP COMMENT 'Timestamp when the student electronically or physically signed the incomplete contract.',
    `term_code` STRING COMMENT 'Six-digit code identifying the academic term in which the incomplete grade was originally assigned (format: YYYYTT where TT is term type).. Valid values are `^[0-9]{6}$`',
    CONSTRAINT pk_incomplete_contract PRIMARY KEY(`incomplete_contract_id`)
) COMMENT 'Formal agreement between a student and instructor documenting the conditions under which an Incomplete (I) grade will be resolved, capturing original incomplete grade date, completion deadline, required work description, default grade if not completed, extension request history, resolution status, resolved grade, and resolution date. Supports academic standing monitoring, financial aid SAP compliance, and registrar grade finalization workflows.';

CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`instruction_early_alert` (
    `instruction_early_alert_id` BIGINT COMMENT 'Unique identifier for the early alert record. Primary key.',
    `account_hold_id` BIGINT COMMENT 'Foreign key linking to billing.account_hold. Business justification: Early alerts for financial distress or non-payment often result in account holds restricting registration. Student success offices use this link to track intervention outcomes, coordinate payment plan',
    `course_id` BIGINT COMMENT 'External identifier for the course in Canvas LMS where the alert was triggered.',
    `conduct_case_id` BIGINT COMMENT 'Foreign key linking to studentlife.conduct_case. Business justification: Early alert systems trigger conduct investigations when behavioral concerns are flagged. Academic intervention platforms routinely reference conduct records to coordinate holistic student support and ',
    `course_section_id` BIGINT COMMENT 'Identifier of the course section in which the alert was raised.',
    `enterprise_application_id` BIGINT COMMENT 'Foreign key linking to technology.enterprise_application. Business justification: Early alert systems (Starfish, Navigate, EAB) are distinct enterprise applications. Business needs this link for vendor support routing, data integration troubleshooting, reporting consolidation, and ',
    `instructor_id` BIGINT COMMENT 'Identifier of the advisor assigned to follow up on the alert.',
    `employee_id` BIGINT COMMENT 'Identifier of the user (instructor, advisor, or system) who raised the alert.',
    `profile_id` BIGINT COMMENT 'Identifier of the student for whom the early alert was raised.',
    `title_ix_case_id` BIGINT COMMENT 'Foreign key linking to compliance.title_ix_case. Business justification: Early alerts for behavioral concerns, safety issues, or interpersonal conflicts often relate to active Title IX cases. Faculty and advisors need visibility to coordinate supportive measures and avoid ',
    `absences_count` STRING COMMENT 'Number of absences recorded at the time the alert was raised.',
    `alert_date` DATE COMMENT 'Date when the early alert was raised.',
    `alert_description` STRING COMMENT 'Detailed description of the concern or issue that prompted the alert.',
    `alert_timestamp` TIMESTAMP COMMENT 'Precise timestamp when the early alert was created in the system.',
    `alert_type` STRING COMMENT 'Category of the early alert indicating the nature of the concern (attendance, academic performance, engagement, behavioral, financial, personal).. Valid values are `attendance|academic_performance|engagement|behavioral|financial|personal`',
    `attendance_concern_flag` BOOLEAN COMMENT 'Indicates whether the alert is related to attendance issues.',
    `banner_alert_ref` STRING COMMENT 'External identifier for the alert in Ellucian Banner SIS if the alert originated there.',
    `behavioral_concern_flag` BOOLEAN COMMENT 'Indicates whether the alert is related to behavioral or conduct issues.',
    `contact_method` STRING COMMENT 'Method used to contact the student regarding the alert.. Valid values are `email|phone|in_person|video_conference|text_message|portal_message`',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the early alert record was first created in the system.',
    `current_grade` STRING COMMENT 'Current grade in the course at the time the alert was raised.. Valid values are `^[A-F][+-]?$|^W$|^I$|^P$|^NP$`',
    `current_grade_percentage` DECIMAL(18,2) COMMENT 'Current grade as a percentage at the time the alert was raised.',
    `dfw_risk_score` DECIMAL(18,2) COMMENT 'Predictive risk score indicating likelihood of receiving a D, F, or withdrawing from the course.',
    `engagement_concern_flag` BOOLEAN COMMENT 'Indicates whether the alert is related to low engagement in course activities.',
    `final_course_grade` STRING COMMENT 'Final grade the student received in the course after the alert and intervention.. Valid values are `^[A-F][+-]?$|^W$|^I$|^P$|^NP$`',
    `first_contact_date` DATE COMMENT 'Date of the first contact or intervention attempt with the student.',
    `grade_concern_flag` BOOLEAN COMMENT 'Indicates whether the alert is related to academic performance or grade concerns.',
    `intervention_actions_taken` STRING COMMENT 'Description of intervention actions taken in response to the alert (e.g., advisor meeting, tutoring referral, counseling referral).',
    `intervention_status` STRING COMMENT 'Current status of the intervention process for the alert.. Valid values are `open|in_progress|resolved|closed|escalated|no_action_needed`',
    `lms_activity_score` DECIMAL(18,2) COMMENT 'Engagement score from the LMS based on logins, assignment submissions, and participation.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when the early alert record was last modified.',
    `referral_completed_flag` BOOLEAN COMMENT 'Indicates whether the student completed the referral to the support service.',
    `referral_service_type` STRING COMMENT 'Type of support service to which the student was referred (e.g., tutoring, counseling, financial aid, disability services, career services). [ENUM-REF-CANDIDATE: tutoring|counseling|financial_aid|disability_services|career_services|academic_advising|health_services — promote to reference product]',
    `resolution_date` DATE COMMENT 'Date when the alert was resolved or closed.',
    `resolution_notes` STRING COMMENT 'Notes describing the resolution or outcome of the alert and intervention.',
    `resolution_status` STRING COMMENT 'Final resolution status of the alert indicating the outcome of the intervention.. Valid values are `resolved|unresolved|partially_resolved|student_withdrew|no_response`',
    `severity_level` STRING COMMENT 'Severity level of the alert indicating urgency of intervention (low, moderate, high, critical).. Valid values are `low|moderate|high|critical`',
    `student_response_date` DATE COMMENT 'Date when the student responded to the intervention outreach.',
    `student_response_received` BOOLEAN COMMENT 'Indicates whether the student responded to the intervention outreach.',
    `student_retained_flag` BOOLEAN COMMENT 'Indicates whether the student was retained in the course or institution following the intervention.',
    `term_code` STRING COMMENT 'Academic term code in which the alert was raised (e.g., 202401 for Spring 2024).. Valid values are `^[0-9]{6}$`',
    `title_iii_reportable` BOOLEAN COMMENT 'Indicates whether this alert is reportable under Title III grant requirements for at-risk student interventions.',
    `title_iv_reportable` BOOLEAN COMMENT 'Indicates whether this alert is reportable under Title IV financial aid satisfactory academic progress (SAP) requirements.',
    `trigger_source` STRING COMMENT 'Source that triggered the alert (manual by instructor, manual by advisor, automated LMS threshold, automated SIS rule, DFW risk model, attendance threshold, grade threshold). [ENUM-REF-CANDIDATE: manual_instructor|manual_advisor|automated_lms|automated_sis|dfw_risk_model|attendance_threshold|grade_threshold — 7 candidates stripped; promote to reference product]',
    CONSTRAINT pk_instruction_early_alert PRIMARY KEY(`instruction_early_alert_id`)
) COMMENT 'Early alert or academic intervention flag raised by an instructor or advisor for a student in a course section, capturing alert type (attendance, academic performance, engagement, behavioral), severity level, trigger source (manual, automated LMS threshold, DFW risk model), alert date, assigned advisor, intervention actions taken, resolution status, and resolution date. Supports student success initiatives, retention programs, and Title III/IV reporting on at-risk student interventions.';

CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`faculty_training_requirement` (
    `faculty_training_requirement_id` BIGINT COMMENT 'Unique identifier for this faculty training requirement record. Primary key.',
    `faculty_assignment_id` BIGINT COMMENT 'Foreign key linking to the faculty assignment record that is subject to this training requirement',
    `training_program_id` BIGINT COMMENT 'Foreign key linking to the compliance training program that is required for this faculty assignment',
    `assignment_date` DATE COMMENT 'The date on which this training requirement was assigned to this faculty assignment. Typically aligns with faculty assignment start date or the date a new training program becomes mandatory.',
    `assignment_reason` STRING COMMENT 'The business or regulatory reason why this specific training program is required for this faculty assignment. Examples: role_based (all instructors), course_content (FERPA for courses with student records access), department_policy (college-specific requirement), accreditation (required by accreditor), regulatory (federal mandate), new_hire (onboarding requirement), annual_refresh (recertification).',
    `attempt_count` STRING COMMENT 'Number of attempts the faculty member has made to complete this training requirement. Some training programs limit the number of attempts or require remediation after multiple failures.',
    `certificate_issued_date` DATE COMMENT 'Date on which a completion certificate was issued for this training requirement. Null if training does not issue certificates or if not yet completed.',
    `certificate_number` STRING COMMENT 'Unique certificate number issued upon successful completion of this training requirement. Used for verification and audit purposes. Null if no certificate issued.',
    `completion_date` DATE COMMENT 'The date on which the faculty member successfully completed this training program for this assignment. Null if not yet completed. Used for compliance reporting and recertification scheduling.',
    `completion_status` STRING COMMENT 'Current status of this training requirement for this faculty assignment. Values: not_started (assigned but not accessed), in_progress (started but not completed), completed (successfully finished with passing score), overdue (past due date without completion), waived (requirement waived by authorized personnel), exempt (faculty member exempt from this requirement).',
    `due_date` DATE COMMENT 'The date by which this training must be completed for this specific faculty assignment. Calculated from assignment start date plus training program completion deadline, or set manually by compliance office for specific circumstances.',
    `last_reminder_date` DATE COMMENT 'Date of the most recent reminder notification sent for this training requirement. Used to schedule next reminder and avoid over-communication.',
    `notes` STRING COMMENT 'Free-text notes regarding this specific training requirement. May include special circumstances, accommodations provided, technical issues encountered, or follow-up actions required.',
    `notification_sent_date` DATE COMMENT 'Date on which the initial notification email was sent to the faculty member informing them of this training requirement. Used to track communication and calculate reminder schedules.',
    `recertification_due_date` DATE COMMENT 'Date by which this training must be retaken to maintain compliance, calculated from completion date plus training program recertification frequency. Null if training does not require recertification or not yet completed.',
    `reminder_count` STRING COMMENT 'Number of reminder notifications sent to the faculty member regarding this incomplete training requirement. Used to track escalation and compliance follow-up efforts.',
    `score_achieved` DECIMAL(18,2) COMMENT 'Percentage score achieved by the faculty member on the training assessment for this requirement. Null if not yet completed or if training does not include an assessment. Must meet or exceed training program passing score.',
    `waiver_approval_date` DATE COMMENT 'Date on which the waiver was officially approved. Null if no waiver granted.',
    `waiver_approved_by` BIGINT COMMENT 'Employee ID of the administrator who approved the waiver for this training requirement. Null if no waiver granted. Used for audit trail and accountability.',
    `waiver_flag` BOOLEAN COMMENT 'Boolean indicator of whether a waiver has been granted for this training requirement. True if the requirement has been waived by an authorized administrator (e.g., dean, compliance officer), false otherwise.',
    `waiver_reason` STRING COMMENT 'Free-text explanation of why a waiver was granted for this training requirement. Required when waiver_flag is true. Examples: prior completion at another institution, equivalent professional certification, subject matter expert exemption.',
    CONSTRAINT pk_faculty_training_requirement PRIMARY KEY(`faculty_training_requirement_id`)
) COMMENT 'This association product represents the compliance training requirement between a faculty assignment and a mandatory training program. It captures the assignment of specific compliance training obligations to faculty members in their instructional roles, tracking completion status, deadlines, waivers, and the regulatory or policy basis for the requirement. Each record links one faculty assignment to one training program with attributes that exist only in the context of this compliance obligation.. Existence Justification: In higher education compliance operations, faculty assignments require completion of multiple mandatory training programs (Title IX, FERPA, Clery Act, research ethics, LMS training) based on role, course content, and regulatory requirements. Each training program is required across multiple faculty assignments spanning departments, colleges, and terms. HR and academic affairs actively manage these training requirements as operational records, tracking completion status, due dates, waivers, and generating compliance reports for accreditation and federal audits.';

CREATE OR REPLACE TABLE `education_ecm`.`instruction`.`grade_appeal_case` (
    `grade_appeal_case_id` BIGINT COMMENT 'Primary key for grade_appeal_case',
    `course_section_id` BIGINT COMMENT 'Identifier of the course section for which the grade is being appealed.',
    `employee_id` BIGINT COMMENT 'Identifier of the individual or committee responsible for reviewing and deciding the appeal (department chair, dean, appeal committee).',
    `instructor_id` BIGINT COMMENT 'Identifier of the instructor who assigned the disputed grade.',
    `profile_id` BIGINT COMMENT 'Identifier of the student who initiated the grade appeal.',
    `conduct_case_id` BIGINT COMMENT 'Identifier of a related academic integrity case if the grade appeal is connected to or resulted from an academic misconduct finding.',
    `term_id` BIGINT COMMENT 'Identifier of the academic term in which the disputed grade was assigned.',
    `escalated_grade_appeal_case_id` BIGINT COMMENT 'Self-referencing FK on grade_appeal_case (escalated_grade_appeal_case_id)',
    `appeal_case_number` STRING COMMENT 'Human-readable business identifier for the grade appeal case, typically formatted with prefix and sequential number for external reference and tracking.',
    `appeal_fee_amount` DECIMAL(18,2) COMMENT 'Monetary fee charged to the student for filing the grade appeal, if applicable per institutional policy. Amount in institutional currency.',
    `appeal_fee_paid` BOOLEAN COMMENT 'Indicates whether the required appeal fee has been paid by the student.',
    `appeal_fee_waived` BOOLEAN COMMENT 'Indicates whether the appeal fee was waived due to financial hardship or other institutional policy considerations.',
    `appeal_level` STRING COMMENT 'The hierarchical level at which the appeal is currently being reviewed: instructor level (informal resolution), department level, college/school level, university level, or external review.',
    `appeal_reason` STRING COMMENT 'Detailed explanation provided by the student describing the grounds for the grade appeal and the specific circumstances or errors being contested.',
    `appeal_status` STRING COMMENT 'Current lifecycle status of the grade appeal case indicating its position in the review and decision workflow.',
    `appeal_type` STRING COMMENT 'Classification of the grade appeal based on the nature of the dispute: grade calculation error, grading policy violation, procedural irregularity, alleged bias or discrimination, extenuating circumstances, or other grounds.',
    `closed_timestamp` TIMESTAMP COMMENT 'Date and time when the grade appeal case was officially closed after final decision and all administrative actions were completed.',
    `confidentiality_level` STRING COMMENT 'Level of confidentiality required for this appeal case based on the nature of the issues involved (standard, elevated for sensitive matters, restricted for cases involving discrimination or harassment allegations).',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this grade appeal case record was first created in the system.',
    `decision_outcome` STRING COMMENT 'Final outcome of the appeal: approved (grade changed as requested), partially approved (grade changed but not to requested level), denied (original grade upheld), remanded (sent back for additional review), or withdrawn (student withdrew appeal).',
    `decision_rationale` STRING COMMENT 'Detailed explanation of the reasoning behind the appeal decision, including findings of fact and application of grading policies.',
    `decision_timestamp` TIMESTAMP COMMENT 'Date and time when the final decision on the grade appeal was rendered by the reviewing authority or appeal committee.',
    `department_recommendation` STRING COMMENT 'Recommendation provided by the academic department regarding the appeal, typically from the department chair or designated review committee.',
    `escalation_count` STRING COMMENT 'Number of times the appeal has been escalated to a higher review level after an unfavorable decision at a lower level.',
    `final_grade` STRING COMMENT 'The final grade determined after the appeal process is complete. May differ from both original and requested grades based on appeal committee decision.',
    `grade_change_effective_date` DATE COMMENT 'Date on which the grade change becomes effective in the students academic record, if the appeal is approved.',
    `hearing_location` STRING COMMENT 'Physical or virtual location where the appeal hearing will be conducted (room number, building name, video conference link).',
    `hearing_scheduled_date` DATE COMMENT 'Date on which a formal hearing or review meeting is scheduled for the grade appeal case, if applicable based on appeal type and institutional policy.',
    `initial_review_completed_timestamp` TIMESTAMP COMMENT 'Date and time when the initial administrative review of the appeal was completed to determine if it meets procedural requirements.',
    `instructor_response` STRING COMMENT 'Written response from the instructor addressing the students appeal claims and providing their perspective on the grading decision.',
    `instructor_response_timestamp` TIMESTAMP COMMENT 'Date and time when the instructor submitted their response to the grade appeal.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Date and time when this grade appeal case record was most recently updated.',
    `notes` STRING COMMENT 'Additional notes, comments, or administrative observations related to the appeal case processing and review.',
    `original_grade` STRING COMMENT 'The grade originally assigned by the instructor that is being appealed. Typically letter grade (A through F with optional plus/minus) or special codes (I=Incomplete, W=Withdrawal, P=Pass, N=No Pass, C=Credit).',
    `priority_flag` BOOLEAN COMMENT 'Indicates whether this appeal case requires expedited review due to time-sensitive circumstances such as graduation eligibility, financial aid status, or academic standing.',
    `requested_grade` STRING COMMENT 'The grade the student is requesting through the appeal process. May be null if student is requesting grade review without specifying desired outcome.',
    `student_notified_timestamp` TIMESTAMP COMMENT 'Date and time when the student was officially notified of the appeal decision.',
    `submission_deadline` DATE COMMENT 'The deadline date by which the grade appeal must be submitted according to institutional policy, typically calculated from grade posting date.',
    `submission_timestamp` TIMESTAMP COMMENT 'Date and time when the student formally submitted the grade appeal case for review.',
    `supporting_documentation_provided` BOOLEAN COMMENT 'Indicates whether the student has submitted supporting documentation (assignments, exams, correspondence, medical records, etc.) to substantiate the appeal.',
    `transcript_updated` BOOLEAN COMMENT 'Indicates whether the students official transcript has been updated to reflect the grade change resulting from a successful appeal.',
    CONSTRAINT pk_grade_appeal_case PRIMARY KEY(`grade_appeal_case_id`)
) COMMENT 'Master reference table for grade_appeal_case. Referenced by appeal_case_id.';
...
ALTER TABLE `education_ecm`.`instruction`.`section_meeting` ADD CONSTRAINT `fk_instruction_section_meeting_scorm_package_id` FOREIGN KEY (`scorm_package_id`) REFERENCES `education_ecm`.`instruction`.`scorm_package`(`scorm_package_id`);
...
ALTER TABLE `education_ecm`.`instruction`.`faculty_assignment` ADD CONSTRAINT `fk_instruction_faculty_assignment_instruction_assignment_id` FOREIGN KEY (`instruction_assignment_id`) REFERENCES `education_ecm`.`instruction`.`instruction_assignment`(`instruction_assignment_id`);
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ADD CONSTRAINT `fk_instruction_instruction_assignment_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ADD CONSTRAINT `fk_instruction_instruction_assignment_rubric_id` FOREIGN KEY (`rubric_id`) REFERENCES `education_ecm`.`instruction`.`rubric`(`rubric_id`);
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ADD CONSTRAINT `fk_instruction_instruction_submission_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ADD CONSTRAINT `fk_instruction_instruction_submission_instruction_assignment_id` FOREIGN KEY (`instruction_assignment_id`) REFERENCES `education_ecm`.`instruction`.`instruction_assignment`(`instruction_assignment_id`);
...
ALTER TABLE `education_ecm`.`instruction`.`grade_entry` ADD CONSTRAINT `fk_instruction_grade_entry_instruction_assignment_id` FOREIGN KEY (`instruction_assignment_id`) REFERENCES `education_ecm`.`instruction`.`instruction_assignment`(`instruction_assignment_id`);
...
ALTER TABLE `education_ecm`.`instruction`.`slo_assessment` ADD CONSTRAINT `fk_instruction_slo_assessment_instruction_assignment_id` FOREIGN KEY (`instruction_assignment_id`) REFERENCES `education_ecm`.`instruction`.`instruction_assignment`(`instruction_assignment_id`);
...
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ADD CONSTRAINT `fk_instruction_scorm_package_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ADD CONSTRAINT `fk_instruction_grade_change_log_grade_appeal_case_id` FOREIGN KEY (`grade_appeal_case_id`) REFERENCES `education_ecm`.`instruction`.`grade_appeal_case`(`grade_appeal_case_id`);
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ADD CONSTRAINT `fk_instruction_grade_change_log_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ADD CONSTRAINT `fk_instruction_grade_change_log_instruction_assignment_id` FOREIGN KEY (`instruction_assignment_id`) REFERENCES `education_ecm`.`instruction`.`instruction_assignment`(`instruction_assignment_id`);
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ADD CONSTRAINT `fk_instruction_grade_change_log_original_change_log_grade_change_log_id` FOREIGN KEY (`original_change_log_grade_change_log_id`) REFERENCES `education_ecm`.`instruction`.`grade_change_log`(`grade_change_log_id`);
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ADD CONSTRAINT `fk_instruction_incomplete_contract_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ADD CONSTRAINT `fk_instruction_instruction_early_alert_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ADD CONSTRAINT `fk_instruction_faculty_training_requirement_faculty_assignment_id` FOREIGN KEY (`faculty_assignment_id`) REFERENCES `education_ecm`.`instruction`.`faculty_assignment`(`faculty_assignment_id`);
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ADD CONSTRAINT `fk_instruction_grade_appeal_case_course_section_id` FOREIGN KEY (`course_section_id`) REFERENCES `education_ecm`.`instruction`.`course_section`(`course_section_id`);
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ADD CONSTRAINT `fk_instruction_grade_appeal_case_escalated_grade_appeal_case_id` FOREIGN KEY (`escalated_grade_appeal_case_id`) REFERENCES `education_ecm`.`instruction`.`grade_appeal_case`(`grade_appeal_case_id`);
...
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `llc_program_id` SET TAGS ('dbx_business_glossary_term' = 'Llc Program Id (Foreign Key)');
...
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `statement_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Statement Id (Foreign Key)');
...
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `tuition_charge_id` SET TAGS ('dbx_business_glossary_term' = 'Tuition Charge Id (Foreign Key)');
...
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `college_code` SET TAGS ('dbx_business_glossary_term' = 'College Code');
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `college_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2,4}$');
...
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Course Fee Amount');
...
ALTER TABLE `education_ecm`.`instruction`.`course_section` ALTER COLUMN `linked_sections` SET TAGS ('dbx_business_glossary_term' = 'Linked Section Identifiers');
...
ALTER TABLE `education_ecm`.`instruction`.`section_meeting` ALTER COLUMN `scorm_package_id` SET TAGS ('dbx_business_glossary_term' = 'Sharable Content Object Reference Model (SCORM) Content Identifier (ID)');
...
ALTER TABLE `education_ecm`.`instruction`.`section_meeting` ALTER COLUMN `building_code` SET TAGS ('dbx_business_glossary_term' = 'Building Code');
ALTER TABLE `education_ecm`.`instruction`.`section_meeting` ALTER COLUMN `building_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
...
ALTER TABLE `education_ecm`.`instruction`.`faculty_assignment` ALTER COLUMN `campus_event_id` SET TAGS ('dbx_business_glossary_term' = 'Campus Event Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`faculty_assignment` ALTER COLUMN `compliance_training_completion_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Training Completion Id (Foreign Key)');
...
ALTER TABLE `education_ecm`.`instruction`.`faculty_assignment` ALTER COLUMN `identity_account_id` SET TAGS ('dbx_business_glossary_term' = 'Instructor Identity Account Id (Foreign Key)');
...
ALTER TABLE `education_ecm`.`instruction`.`faculty_assignment` ALTER COLUMN `instruction_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Ellucian Banner Assignment Identifier (ID)');
...
ALTER TABLE `education_ecm`.`instruction`.`faculty_assignment` ALTER COLUMN `grant_account_code` SET TAGS ('dbx_business_glossary_term' = 'Grant Account Code');
...
ALTER TABLE `education_ecm`.`instruction`.`lms_course_shell` ALTER COLUMN `enterprise_application_id` SET TAGS ('dbx_business_glossary_term' = 'Lms Platform Application Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`lms_course_shell` ALTER COLUMN `it_service_id` SET TAGS ('dbx_business_glossary_term' = 'Lms Service Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`lms_course_shell` ALTER COLUMN `course_id` SET TAGS ('dbx_business_glossary_term' = 'Canvas Course ID');
ALTER TABLE `education_ecm`.`instruction`.`lms_course_shell` ALTER COLUMN `tertiary_lms_blueprint_course_id` SET TAGS ('dbx_business_glossary_term' = 'Blueprint Course ID');
...
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` SET TAGS ('dbx_subdomain' = 'course_delivery');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `instruction_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Assignment Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `course_section_id` SET TAGS ('dbx_business_glossary_term' = 'Course Section Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `enterprise_application_id` SET TAGS ('dbx_business_glossary_term' = 'External Tool Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `identity_account_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `instructor_id` SET TAGS ('dbx_business_glossary_term' = 'Final Grader Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `map_id` SET TAGS ('dbx_business_glossary_term' = 'Curriculum Map Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Room Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `rubric_id` SET TAGS ('dbx_business_glossary_term' = 'Rubric Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `slo_id` SET TAGS ('dbx_business_glossary_term' = 'Slo Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `tuition_charge_id` SET TAGS ('dbx_business_glossary_term' = 'Tuition Charge Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `wellness_program_id` SET TAGS ('dbx_business_glossary_term' = 'Wellness Program Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `allowed_attempts` SET TAGS ('dbx_business_glossary_term' = 'Allowed Attempts');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `allowed_file_extensions` SET TAGS ('dbx_business_glossary_term' = 'Allowed File Extensions');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `assignment_description` SET TAGS ('dbx_business_glossary_term' = 'Assignment Description');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `assignment_name` SET TAGS ('dbx_business_glossary_term' = 'Assignment Name');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `assignment_status` SET TAGS ('dbx_business_glossary_term' = 'Assignment Status');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `assignment_status` SET TAGS ('dbx_value_regex' = 'draft|published|unpublished|deleted');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `assignment_type` SET TAGS ('dbx_business_glossary_term' = 'Assignment Type');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `available_from_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Available From Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `canvas_assignment_ref` SET TAGS ('dbx_business_glossary_term' = 'Canvas Learning Management System (LMS) Assignment Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Assignment Created Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Due Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `external_tool_url` SET TAGS ('dbx_business_glossary_term' = 'External Tool Uniform Resource Locator (URL)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `grade_group_students_individually` SET TAGS ('dbx_business_glossary_term' = 'Grade Group Students Individually Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `grader_count` SET TAGS ('dbx_business_glossary_term' = 'Grader Count');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `grading_type` SET TAGS ('dbx_business_glossary_term' = 'Grading Type');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `grading_type` SET TAGS ('dbx_value_regex' = 'points|percentage|letter_grade|pass_fail|not_graded|gpa_scale');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `group_category_ref` SET TAGS ('dbx_business_glossary_term' = 'Group Category Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `group_ref` SET TAGS ('dbx_business_glossary_term' = 'Assignment Group Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_anonymous_grading` SET TAGS ('dbx_business_glossary_term' = 'Anonymous Grading Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_anonymous_peer_review` SET TAGS ('dbx_business_glossary_term' = 'Anonymous Peer Review Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_group_assignment` SET TAGS ('dbx_business_glossary_term' = 'Group Assignment Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_omit_from_final_grade` SET TAGS ('dbx_business_glossary_term' = 'Omit From Final Grade Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_only_visible_to_overrides` SET TAGS ('dbx_business_glossary_term' = 'Only Visible to Overrides Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_peer_review_enabled` SET TAGS ('dbx_business_glossary_term' = 'Peer Review Enabled Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_published` SET TAGS ('dbx_business_glossary_term' = 'Published Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `is_rubric_for_grading` SET TAGS ('dbx_business_glossary_term' = 'Rubric Used for Grading Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `lock_date` SET TAGS ('dbx_business_glossary_term' = 'Assignment Lock Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `moderated_grading_enabled` SET TAGS ('dbx_business_glossary_term' = 'Moderated Grading Enabled Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Assignment Modified Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `peer_review_count` SET TAGS ('dbx_business_glossary_term' = 'Peer Review Count');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `peer_review_due_date` SET TAGS ('dbx_business_glossary_term' = 'Peer Review Due Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `points_possible` SET TAGS ('dbx_business_glossary_term' = 'Points Possible');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `position` SET TAGS ('dbx_business_glossary_term' = 'Assignment Position');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `submission_type` SET TAGS ('dbx_business_glossary_term' = 'Submission Type');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `turnitin_enabled` SET TAGS ('dbx_business_glossary_term' = 'Turnitin Plagiarism Detection Enabled Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_assignment` ALTER COLUMN `vericite_enabled` SET TAGS ('dbx_business_glossary_term' = 'VeriCite Plagiarism Detection Enabled Indicator');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` SET TAGS ('dbx_subdomain' = 'academic_performance');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `instruction_submission_id` SET TAGS ('dbx_business_glossary_term' = 'Submission ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `ada_accommodation_id` SET TAGS ('dbx_business_glossary_term' = 'Ada Accommodation Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `course_section_id` SET TAGS ('dbx_business_glossary_term' = 'Course Section ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `instruction_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Assignment ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `instructor_id` SET TAGS ('dbx_business_glossary_term' = 'Grader ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Student ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `wellness_participation_id` SET TAGS ('dbx_business_glossary_term' = 'Wellness Participation Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `anonymous_ref` SET TAGS ('dbx_business_glossary_term' = 'Anonymous ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `attempt_number` SET TAGS ('dbx_business_glossary_term' = 'Attempt Number');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `body` SET TAGS ('dbx_business_glossary_term' = 'Submission Body');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `cached_due_date` SET TAGS ('dbx_business_glossary_term' = 'Cached Due Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `canvas_submission_ref` SET TAGS ('dbx_business_glossary_term' = 'Canvas Submission ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `comments_count` SET TAGS ('dbx_business_glossary_term' = 'Submission Comments Count');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `created_at` SET TAGS ('dbx_business_glossary_term' = 'Created At Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `entered_grade` SET TAGS ('dbx_business_glossary_term' = 'Entered Grade');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `entered_score` SET TAGS ('dbx_business_glossary_term' = 'Entered Score');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `extra_attempts` SET TAGS ('dbx_business_glossary_term' = 'Extra Attempts');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `grade` SET TAGS ('dbx_business_glossary_term' = 'Grade');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `graded_at` SET TAGS ('dbx_business_glossary_term' = 'Graded At Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `grading_period_ref` SET TAGS ('dbx_business_glossary_term' = 'Grading Period ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `group_ref` SET TAGS ('dbx_business_glossary_term' = 'Group ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `history_count` SET TAGS ('dbx_business_glossary_term' = 'Submission History Count');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `is_anonymous_grading` SET TAGS ('dbx_business_glossary_term' = 'Is Anonymous Grading Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `is_excused` SET TAGS ('dbx_business_glossary_term' = 'Is Excused Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `is_late` SET TAGS ('dbx_business_glossary_term' = 'Is Late Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `is_missing` SET TAGS ('dbx_business_glossary_term' = 'Is Missing Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `last_activity_at` SET TAGS ('dbx_business_glossary_term' = 'Last Activity At Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `late_policy_status` SET TAGS ('dbx_business_glossary_term' = 'Late Policy Status');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `late_policy_status` SET TAGS ('dbx_value_regex' = 'none|late|missing|extended');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `originality_score` SET TAGS ('dbx_business_glossary_term' = 'Originality Score');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `points_deducted` SET TAGS ('dbx_business_glossary_term' = 'Points Deducted');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `posted_at` SET TAGS ('dbx_business_glossary_term' = 'Posted At Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `preview_url` SET TAGS ('dbx_business_glossary_term' = 'Preview URL');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `redo_request` SET TAGS ('dbx_business_glossary_term' = 'Redo Request Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `rubric_assessment_ref` SET TAGS ('dbx_business_glossary_term' = 'Rubric Assessment ID');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `score` SET TAGS ('dbx_business_glossary_term' = 'Score');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `seconds_late` SET TAGS ('dbx_business_glossary_term' = 'Seconds Late');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `submission_type` SET TAGS ('dbx_business_glossary_term' = 'Submission Type');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `submitted_at` SET TAGS ('dbx_business_glossary_term' = 'Submitted At Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `turnitin_data` SET TAGS ('dbx_business_glossary_term' = 'Turnitin Data');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `updated_at` SET TAGS ('dbx_business_glossary_term' = 'Updated At Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `url` SET TAGS ('dbx_business_glossary_term' = 'Submission URL');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `workflow_state` SET TAGS ('dbx_business_glossary_term' = 'Workflow State');
ALTER TABLE `education_ecm`.`instruction`.`instruction_submission` ALTER COLUMN `workflow_state` SET TAGS ('dbx_value_regex' = 'submitted|unsubmitted|graded|pending_review');
ALTER TABLE `education_ecm`.`instruction`.`grade_entry` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`instruction`.`grade_entry` SET TAGS ('dbx_subdomain' = 'academic_performance');
...
ALTER TABLE `education_ecm`.`instruction`.`grade_entry` ALTER COLUMN `instruction_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Assignment ID');
...
ALTER TABLE `education_ecm`.`instruction`.`grade_entry` ALTER COLUMN `slo_alignment` SET TAGS ('dbx_business_glossary_term' = 'Student Learning Outcome (SLO) Alignment');
...
ALTER TABLE `education_ecm`.`instruction`.`final_grade` SET TAGS ('dbx_subdomain' = 'academic_performance');
...
ALTER TABLE `education_ecm`.`instruction`.`final_grade` ALTER COLUMN `studentlife_housing_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Housing Assignment Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`final_grade` ALTER COLUMN `tuition_charge_id` SET TAGS ('dbx_business_glossary_term' = 'Tuition Charge Id (Foreign Key)');
...
ALTER TABLE `education_ecm`.`instruction`.`attendance_record` SET TAGS ('dbx_subdomain' = 'academic_performance');
...
ALTER TABLE `education_ecm`.`instruction`.`rubric` SET TAGS ('dbx_subdomain' = 'course_delivery');
...
ALTER TABLE `education_ecm`.`instruction`.`rubric` ALTER COLUMN `identity_account_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
...
ALTER TABLE `education_ecm`.`instruction`.`slo_assessment` SET TAGS ('dbx_subdomain' = 'academic_performance');
...
ALTER TABLE `education_ecm`.`instruction`.`slo_assessment` ALTER COLUMN `instruction_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Learning Management System (LMS) Assignment ID');
...
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` SET TAGS ('dbx_subdomain' = 'course_delivery');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `scorm_package_id` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Package ID');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `course_section_id` SET TAGS ('dbx_business_glossary_term' = 'Course Section ID');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `enterprise_application_id` SET TAGS ('dbx_business_glossary_term' = 'Hosting Application Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `finance_vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Vendor Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `identity_account_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `accessibility_features` SET TAGS ('dbx_business_glossary_term' = 'Accessibility Features');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Assessment Type');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `assessment_type` SET TAGS ('dbx_value_regex' = 'formative|summative|diagnostic|none');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `average_completion_rate_percent` SET TAGS ('dbx_business_glossary_term' = 'Average Completion Rate Percentage');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `average_mastery_score` SET TAGS ('dbx_business_glossary_term' = 'Average Mastery Score');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `cip_code` SET TAGS ('dbx_business_glossary_term' = 'Classification of Instructional Programs (CIP) Code');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `cip_code` SET TAGS ('dbx_value_regex' = '^d{2}.d{4}$');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `completion_threshold_percent` SET TAGS ('dbx_business_glossary_term' = 'Completion Threshold Percentage');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `cost_per_student` SET TAGS ('dbx_business_glossary_term' = 'Cost Per Student');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `cost_per_student` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `estimated_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Estimated Duration in Minutes');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `file_size_mb` SET TAGS ('dbx_business_glossary_term' = 'File Size in Megabytes (MB)');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `is_mobile_compatible` SET TAGS ('dbx_business_glossary_term' = 'Mobile Compatible Flag');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `is_mobile_compatible` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `is_mobile_compatible` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `is_oer` SET TAGS ('dbx_business_glossary_term' = 'Open Educational Resource (OER) Flag');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Language Code');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `language_code` SET TAGS ('dbx_value_regex' = '^[a-z]{2}(-[A-Z]{2})?$');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `last_reviewed_date` SET TAGS ('dbx_business_glossary_term' = 'Last Reviewed Date');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `launch_url` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Launch Uniform Resource Locator (URL)');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `learning_objectives` SET TAGS ('dbx_business_glossary_term' = 'Learning Objectives');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `license_type` SET TAGS ('dbx_business_glossary_term' = 'Content License Type');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `mastery_score_threshold` SET TAGS ('dbx_business_glossary_term' = 'Mastery Score Threshold');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Modified Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Internal Notes');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_description` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Package Description');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_format` SET TAGS ('dbx_business_glossary_term' = 'Package Format');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_format` SET TAGS ('dbx_value_regex' = 'zip|imscc|tar.gz|other');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_identifier` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Package Identifier');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_status` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Package Status');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_status` SET TAGS ('dbx_value_regex' = 'active|inactive|archived|under review|deprecated|pilot');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_title` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Package Title');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `package_version` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Package Version');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `plugin_name` SET TAGS ('dbx_business_glossary_term' = 'Required Plugin Name');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `prerequisite_knowledge` SET TAGS ('dbx_business_glossary_term' = 'Prerequisite Knowledge');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `requires_plugin` SET TAGS ('dbx_business_glossary_term' = 'Requires Plugin Flag');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `standard_version` SET TAGS ('dbx_business_glossary_term' = 'SCORM (Sharable Content Object Reference Model) Standard Version');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `standard_version` SET TAGS ('dbx_value_regex' = 'SCORM 1.2|SCORM 2004 3rd Edition|SCORM 2004 4th Edition|xAPI|cmi5|AICC');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `subject_area` SET TAGS ('dbx_business_glossary_term' = 'Subject Area');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `target_audience` SET TAGS ('dbx_business_glossary_term' = 'Target Audience');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `target_audience` SET TAGS ('dbx_value_regex' = 'undergraduate|graduate|professional|continuing education|faculty development|all');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `upload_date` SET TAGS ('dbx_business_glossary_term' = 'Package Upload Date');
ALTER TABLE `education_ecm`.`instruction`.`scorm_package` ALTER COLUMN `usage_count` SET TAGS ('dbx_business_glossary_term' = 'Usage Count');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` SET TAGS ('dbx_subdomain' = 'academic_performance');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `grade_change_log_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Change Log ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `grade_appeal_case_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Appeal Case ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `conduct_case_id` SET TAGS ('dbx_business_glossary_term' = 'Academic Integrity Case ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `course_section_id` SET TAGS ('dbx_business_glossary_term' = 'Course Section ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approving Authority ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `instruction_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Assignment ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `original_change_log_grade_change_log_id` SET TAGS ('dbx_business_glossary_term' = 'Original Change Log ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Student ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `profile_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `profile_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `instructor_id` SET TAGS ('dbx_business_glossary_term' = 'Requesting Instructor ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `academic_standing_impact` SET TAGS ('dbx_business_glossary_term' = 'Academic Standing Impact');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `academic_standing_impact` SET TAGS ('dbx_value_regex' = 'none|probation_to_good_standing|good_standing_to_probation|probation_to_suspension|suspension_to_probation|dismissal_to_reinstatement');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `accreditation_reportable` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Reportable Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `approving_authority_role` SET TAGS ('dbx_business_glossary_term' = 'Approving Authority Role');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `approving_authority_role` SET TAGS ('dbx_value_regex' = 'registrar|dean|department_chair|program_director|provost|academic_affairs');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `banner_transaction_ref` SET TAGS ('dbx_business_glossary_term' = 'Banner Transaction ID');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `change_justification` SET TAGS ('dbx_business_glossary_term' = 'Change Justification Narrative');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `change_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Change Reason Code');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `change_status` SET TAGS ('dbx_business_glossary_term' = 'Grade Change Status');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `change_status` SET TAGS ('dbx_value_regex' = 'pending|approved|rejected|reversed|under_review');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `degree_audit_impact` SET TAGS ('dbx_business_glossary_term' = 'Degree Audit Impact Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `gpa_recalculation_required` SET TAGS ('dbx_business_glossary_term' = 'Grade Point Average (GPA) Recalculation Required Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `grade_type` SET TAGS ('dbx_business_glossary_term' = 'Grade Type');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `lms_gradebook_sync_flag` SET TAGS ('dbx_business_glossary_term' = 'Learning Management System (LMS) Gradebook Sync Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `lms_sync_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Learning Management System (LMS) Sync Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Modified Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `notification_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Student Notification Sent Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Student Notification Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `original_grade` SET TAGS ('dbx_business_glossary_term' = 'Original Grade');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `original_grade_points` SET TAGS ('dbx_business_glossary_term' = 'Original Grade Points');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `request_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Request Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `reversal_flag` SET TAGS ('dbx_business_glossary_term' = 'Reversal Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `revised_grade` SET TAGS ('dbx_business_glossary_term' = 'Revised Grade');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `revised_grade_points` SET TAGS ('dbx_business_glossary_term' = 'Revised Grade Points');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `sap_recalculation_required` SET TAGS ('dbx_business_glossary_term' = 'Satisfactory Academic Progress (SAP) Recalculation Required Flag');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `supporting_documentation_url` SET TAGS ('dbx_business_glossary_term' = 'Supporting Documentation URL');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `term_code` SET TAGS ('dbx_business_glossary_term' = 'Term Code');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `term_code` SET TAGS ('dbx_value_regex' = '^[0-9]{6}$');
ALTER TABLE `education_ecm`.`instruction`.`grade_change_log` ALTER COLUMN `transcript_notation_required` SET TAGS ('dbx_business_glossary_term' = 'Transcript Notation Required Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` SET TAGS ('dbx_subdomain' = 'course_delivery');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `incomplete_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Incomplete Contract Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Access Room Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `course_section_id` SET TAGS ('dbx_business_glossary_term' = 'Course Section Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approving Authority Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `identity_account_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `instructor_id` SET TAGS ('dbx_business_glossary_term' = 'Instructor Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `modified_by_user_identity_account_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `modified_by_user_identity_account_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `modified_by_user_identity_account_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Student Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `tuition_charge_id` SET TAGS ('dbx_business_glossary_term' = 'Tuition Charge Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `advisor_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Advisor Notification Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `advisor_notified_flag` SET TAGS ('dbx_business_glossary_term' = 'Academic Advisor Notified Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `completion_deadline` SET TAGS ('dbx_business_glossary_term' = 'Work Completion Deadline Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `contract_notes` SET TAGS ('dbx_business_glossary_term' = 'Contract Notes');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `contract_number` SET TAGS ('dbx_business_glossary_term' = 'Incomplete Contract Number');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `contract_number` SET TAGS ('dbx_value_regex' = '^IC-[0-9]{4}-[0-9]{6}$');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `contract_signed_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Signed Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `contract_status` SET TAGS ('dbx_business_glossary_term' = 'Incomplete Contract Status');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `contract_status` SET TAGS ('dbx_value_regex' = 'pending|active|completed|expired|extended|cancelled');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `credit_hours` SET TAGS ('dbx_business_glossary_term' = 'Credit Hours (CR)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `default_grade` SET TAGS ('dbx_business_glossary_term' = 'Default Grade');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `default_grade` SET TAGS ('dbx_value_regex' = '^[A-F]$|^W$|^WF$');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `department_code` SET TAGS ('dbx_business_glossary_term' = 'Academic Department Code');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `department_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2,4}$');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `extended_deadline` SET TAGS ('dbx_business_glossary_term' = 'Extended Completion Deadline Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `extension_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Extension Approval Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `extension_approved_flag` SET TAGS ('dbx_business_glossary_term' = 'Extension Approved Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `extension_reason` SET TAGS ('dbx_business_glossary_term' = 'Extension Request Reason');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `extension_request_date` SET TAGS ('dbx_business_glossary_term' = 'Extension Request Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `extension_requested_flag` SET TAGS ('dbx_business_glossary_term' = 'Extension Requested Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `financial_aid_hold_flag` SET TAGS ('dbx_business_glossary_term' = 'Financial Aid Hold Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `graduation_hold_flag` SET TAGS ('dbx_business_glossary_term' = 'Graduation Hold Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `instructor_signature_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Instructor Signature Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `lms_assignment_reference` SET TAGS ('dbx_business_glossary_term' = 'Learning Management System (LMS) Assignment Reference');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Modified Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `original_grade` SET TAGS ('dbx_business_glossary_term' = 'Original Incomplete Grade');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `original_grade` SET TAGS ('dbx_value_regex' = '^I$|^IP$');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `original_grade_date` SET TAGS ('dbx_business_glossary_term' = 'Original Grade Assignment Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `registration_hold_flag` SET TAGS ('dbx_business_glossary_term' = 'Registration Hold Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `required_work_description` SET TAGS ('dbx_business_glossary_term' = 'Required Work Description');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Resolution Date');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `resolution_method` SET TAGS ('dbx_business_glossary_term' = 'Resolution Method');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `resolution_method` SET TAGS ('dbx_value_regex' = 'work_completed|deadline_expired|administrative_change|student_withdrawal');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `resolved_grade` SET TAGS ('dbx_business_glossary_term' = 'Resolved Final Grade');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `resolved_grade` SET TAGS ('dbx_value_regex' = '^[A-F]$|^W$|^WF$|^P$|^NP$');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `sap_impact_flag` SET TAGS ('dbx_business_glossary_term' = 'Satisfactory Academic Progress (SAP) Impact Flag');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `sis_contract_ref` SET TAGS ('dbx_business_glossary_term' = 'Student Information System (SIS) Contract Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `student_signature_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Student Signature Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `term_code` SET TAGS ('dbx_business_glossary_term' = 'Academic Term Code');
ALTER TABLE `education_ecm`.`instruction`.`incomplete_contract` ALTER COLUMN `term_code` SET TAGS ('dbx_value_regex' = '^[0-9]{6}$');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` SET TAGS ('dbx_subdomain' = 'academic_performance');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `instruction_early_alert_id` SET TAGS ('dbx_business_glossary_term' = 'Early Alert Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `account_hold_id` SET TAGS ('dbx_business_glossary_term' = 'Account Hold Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `course_id` SET TAGS ('dbx_business_glossary_term' = 'Canvas Course Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `conduct_case_id` SET TAGS ('dbx_business_glossary_term' = 'Conduct Case Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `course_section_id` SET TAGS ('dbx_business_glossary_term' = 'Course Section Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `enterprise_application_id` SET TAGS ('dbx_business_glossary_term' = 'Alert System Application Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `instructor_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Advisor Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Raised By User Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Student Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `title_ix_case_id` SET TAGS ('dbx_business_glossary_term' = 'Title Ix Case Id (Foreign Key)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `absences_count` SET TAGS ('dbx_business_glossary_term' = 'Absences Count');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `alert_date` SET TAGS ('dbx_business_glossary_term' = 'Alert Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `alert_description` SET TAGS ('dbx_business_glossary_term' = 'Alert Description');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `alert_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Alert Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `alert_type` SET TAGS ('dbx_business_glossary_term' = 'Alert Type');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `alert_type` SET TAGS ('dbx_value_regex' = 'attendance|academic_performance|engagement|behavioral|financial|personal');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `attendance_concern_flag` SET TAGS ('dbx_business_glossary_term' = 'Attendance Concern Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `banner_alert_ref` SET TAGS ('dbx_business_glossary_term' = 'Banner Alert Identifier (ID)');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `behavioral_concern_flag` SET TAGS ('dbx_business_glossary_term' = 'Behavioral Concern Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `contact_method` SET TAGS ('dbx_business_glossary_term' = 'Contact Method');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `contact_method` SET TAGS ('dbx_value_regex' = 'email|phone|in_person|video_conference|text_message|portal_message');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `current_grade` SET TAGS ('dbx_business_glossary_term' = 'Current Grade');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `current_grade` SET TAGS ('dbx_value_regex' = '^[A-F][+-]?$|^W$|^I$|^P$|^NP$');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `current_grade_percentage` SET TAGS ('dbx_business_glossary_term' = 'Current Grade Percentage');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `dfw_risk_score` SET TAGS ('dbx_business_glossary_term' = 'DFW (D-grade F-grade Withdrawal) Risk Score');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `engagement_concern_flag` SET TAGS ('dbx_business_glossary_term' = 'Engagement Concern Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `final_course_grade` SET TAGS ('dbx_business_glossary_term' = 'Final Course Grade');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `final_course_grade` SET TAGS ('dbx_value_regex' = '^[A-F][+-]?$|^W$|^I$|^P$|^NP$');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `first_contact_date` SET TAGS ('dbx_business_glossary_term' = 'First Contact Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `grade_concern_flag` SET TAGS ('dbx_business_glossary_term' = 'Grade Concern Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `intervention_actions_taken` SET TAGS ('dbx_business_glossary_term' = 'Intervention Actions Taken');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `intervention_status` SET TAGS ('dbx_business_glossary_term' = 'Intervention Status');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `intervention_status` SET TAGS ('dbx_value_regex' = 'open|in_progress|resolved|closed|escalated|no_action_needed');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `lms_activity_score` SET TAGS ('dbx_business_glossary_term' = 'LMS (Learning Management System) Activity Score');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `referral_completed_flag` SET TAGS ('dbx_business_glossary_term' = 'Referral Completed Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `referral_service_type` SET TAGS ('dbx_business_glossary_term' = 'Referral Service Type');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Resolution Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `resolution_notes` SET TAGS ('dbx_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `resolution_status` SET TAGS ('dbx_business_glossary_term' = 'Resolution Status');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `resolution_status` SET TAGS ('dbx_value_regex' = 'resolved|unresolved|partially_resolved|student_withdrew|no_response');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `severity_level` SET TAGS ('dbx_value_regex' = 'low|moderate|high|critical');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `student_response_date` SET TAGS ('dbx_business_glossary_term' = 'Student Response Date');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `student_response_received` SET TAGS ('dbx_business_glossary_term' = 'Student Response Received');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `student_retained_flag` SET TAGS ('dbx_business_glossary_term' = 'Student Retained Flag');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `term_code` SET TAGS ('dbx_business_glossary_term' = 'Term Code');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `term_code` SET TAGS ('dbx_value_regex' = '^[0-9]{6}$');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `title_iii_reportable` SET TAGS ('dbx_business_glossary_term' = 'Title III Reportable');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `title_iv_reportable` SET TAGS ('dbx_business_glossary_term' = 'Title IV Reportable');
ALTER TABLE `education_ecm`.`instruction`.`instruction_early_alert` ALTER COLUMN `trigger_source` SET TAGS ('dbx_business_glossary_term' = 'Trigger Source');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` SET TAGS ('dbx_subdomain' = 'course_delivery');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` SET TAGS ('dbx_association_edges' = 'instruction.faculty_assignment,compliance.training_program');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `faculty_training_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Faculty Training Requirement ID');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `faculty_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Faculty Training Requirement - Faculty Assignment Id');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `training_program_id` SET TAGS ('dbx_business_glossary_term' = 'Faculty Training Requirement - Compliance Training Id');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `assignment_date` SET TAGS ('dbx_business_glossary_term' = 'Training Assignment Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `assignment_reason` SET TAGS ('dbx_business_glossary_term' = 'Assignment Reason');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `attempt_count` SET TAGS ('dbx_business_glossary_term' = 'Attempt Count');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `certificate_issued_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Issued Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `certificate_number` SET TAGS ('dbx_business_glossary_term' = 'Certificate Number');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `completion_date` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `completion_status` SET TAGS ('dbx_business_glossary_term' = 'Completion Status');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Training Due Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `last_reminder_date` SET TAGS ('dbx_business_glossary_term' = 'Last Reminder Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Requirement Notes');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `notification_sent_date` SET TAGS ('dbx_business_glossary_term' = 'Notification Sent Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `recertification_due_date` SET TAGS ('dbx_business_glossary_term' = 'Recertification Due Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `reminder_count` SET TAGS ('dbx_business_glossary_term' = 'Reminder Count');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `score_achieved` SET TAGS ('dbx_business_glossary_term' = 'Score Achieved');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `waiver_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Waiver Approval Date');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `waiver_approved_by` SET TAGS ('dbx_business_glossary_term' = 'Waiver Approved By');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `waiver_flag` SET TAGS ('dbx_business_glossary_term' = 'Waiver Flag');
ALTER TABLE `education_ecm`.`instruction`.`faculty_training_requirement` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` SET TAGS ('dbx_subdomain' = 'academic_performance');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `grade_appeal_case_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Appeal Case Identifier');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `profile_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `profile_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `escalated_grade_appeal_case_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `final_grade` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `final_grade` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `original_grade` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `original_grade` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `requested_grade` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `education_ecm`.`instruction`.`grade_appeal_case` ALTER COLUMN `requested_grade` SET TAGS ('dbx_pii_identifier' = 'true');
