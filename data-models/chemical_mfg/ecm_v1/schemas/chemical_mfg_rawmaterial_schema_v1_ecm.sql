-- Schema for Domain: rawmaterial | Business: Chemical Mfg | Version: v1_ecm
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_master` (
    ...
    `formula_id` BIGINT COMMENT 'Foreign key linking to formulation.formula. Business justification: Regulatory Ingredient‑to‑Formula mapping report requires linking each raw material to its primary formula for compliance, cost allocation, and traceability.',
    ...
    `rawmaterial_sds_record_id` BIGINT COMMENT 'Identifier linking to the materials SDS/MSDS record in the document repository.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` (
    ...
    `other_information` STRING COMMENT 'Any additional SDS details not covered in other sections.',
    ...
    `supplier_contact` STRING COMMENT 'Contact details (phone or email) for the SDS supplier.',
    `supplier_name` STRING COMMENT 'Legal name of the supplier that issued the SDS.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_ghs_classification` (
    ...
    `material_name` STRING COMMENT 'Human‑readable name of the raw material.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_specification` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Needed for Specification Ownership audit, tracking which employee owns each material specification for accountability and change control.',
    ...
    `material_code` STRING COMMENT 'Business identifier of the raw material to which this specification applies (e.g., SAP material number).',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_qualification` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Required for Quality Engineer Assignment report, linking each material qualification to the responsible quality engineer (employee) who oversees the qualification.',
    `material_master_id` BIGINT COMMENT 'Identifier of the raw material being qualified.',
    `project_id` BIGINT COMMENT 'Foreign key linking to research.project. Business justification: Material qualification activities are initiated by specific R&D projects; linking enables qualification status tracking per project.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: Needed to associate each Certificate of Analysis with the receiving customer for compliance, quality assurance, and audit trails.',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Ensures COA Signatory Compliance report can verify the employee who authorized the certificate of analysis, meeting GMP and regulatory traceability.',
    ...
    `supplier_lot_number` STRING COMMENT 'Lot number assigned by the supplier for the shipped raw material.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`reach_registration` (
    ...
    `cas_number` STRING COMMENT 'Unique Chemical Abstracts Service identifier for the substance.. Valid values are `^d{2,7}-d{2}-d$`',
    ...
    `registration_holder` STRING COMMENT 'Name of the organisation that holds the registration (lead or member).',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` (
    `tsca_inventory_id` BIGINT COMMENT 'System-generated unique identifier for each TSCA inventory compliance record.',
    `cas_registry_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.cas_registry. Business justification: Each TSCA inventory record pertains to a chemical substance identified by CAS; linking to cas_registry consolidates substance data and removes redundant fields.',
    `cdr_obligation_flag` BOOLEAN COMMENT 'True if the substance is required to be reported under EPA CDR.',
    `cdr_reporting_year` STRING COMMENT 'Calendar year for which CDR data must be submitted.',
    `cdr_volume` DECIMAL(18,2) COMMENT 'Quantity of the substance manufactured or imported, reported to EPA.',
    `consent_order_flag` BOOLEAN COMMENT 'True if a consent order applies to the substance.',
    `consent_order_number` STRING COMMENT 'Identifier of the EPA consent order, when applicable.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when the TSCA inventory record was created in the system.',
    `effective_date` DATE COMMENT 'Date from which the records data is considered effective for reporting.',
    `exemption_description` STRING COMMENT 'Narrative description of the exemption rationale.',
    `exemption_type` STRING COMMENT 'Type of TSCA exemption that applies to the substance.. Valid values are `active|inactive|exempt|polymer_exempt|none`',
    `expiration_date` DATE COMMENT 'Date after which the record is no longer valid, if applicable.',
    `last_review_date` DATE COMMENT 'Date of the most recent compliance review of the TSCA record.',
    `listing_status_effective_date` DATE COMMENT 'Date when the current TSCA listing status became effective.',
    `listing_status_expiration_date` DATE COMMENT 'Date when the current TSCA listing status expires, if applicable.',
    `pmn_filed_flag` BOOLEAN COMMENT 'True if a PMN has been filed for the substance; otherwise false.',
    `pmn_number` STRING COMMENT 'Identifier of the EPA PMN filing for the substance, when required.',
    `processing_use_code` STRING COMMENT 'EPA code describing the primary processing or use of the substance.',
    `processing_use_description` STRING COMMENT 'Human‑readable description of the processing use code.',
    `record_status` STRING COMMENT 'Indicates whether the record is active (current) or retained for historical purposes (archived).. Valid values are `current|archived`',
    `risk_evaluation_status` STRING COMMENT 'Current status of EPA risk evaluation for the substance.. Valid values are `prioritized|under_evaluation|completed|not_started`',
    `risk_evaluation_year` STRING COMMENT 'Year in which the risk evaluation status was assigned.',
    `section5_order_flag` BOOLEAN COMMENT 'True if the substance is subject to a Section 5 order under TSCA.',
    `snur_applicable_flag` BOOLEAN COMMENT 'Indicates whether the substance is subject to an EPA SNUR.',
    `source_system` STRING COMMENT 'Originating operational system (e.g., SAP MM, LIMS) that supplied the TSCA data.',
    `test_rule_description` STRING COMMENT 'Brief description of the applicable test rule.',
    `test_rule_flag` BOOLEAN COMMENT 'True if a specific EPA test rule applies to the substance.',
    `tsca_listing_status` STRING COMMENT 'Current TSCA listing status of the substance as defined by EPA.. Valid values are `active|inactive|exempt|polymer_exempt`',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time of the most recent update to the TSCA inventory record.',
    `volume_band` STRING COMMENT 'Categorical band indicating the scale of reported volume.. Valid values are `low|medium|high`',
    `volume_units` STRING COMMENT 'Units of measure for the reported CDR volume.. Valid values are `kg|lb|metric_ton|gallons`',
    CONSTRAINT pk_tsca_inventory PRIMARY KEY(`tsca_inventory_id`)
) COMMENT 'TSCA (Toxic Substances Control Act) inventory compliance records for chemical substances subject to US EPA regulation. Each record captures the substance CAS number, TSCA inventory listing status (active, inactive, exempt, polymer exemption), PMN (Pre-Manufacture Notice) number if applicable, significant new use rule (SNUR) applicability, CDR (Chemical Data Reporting) obligation, CDR reporting year and volume, manufactured/imported volume bands, processing and use codes, EPA risk evaluation status (prioritized, under evaluation, completed), consent orders, test rules, and Section 5 order applicability. This product is the SSOT for US EPA TSCA-specific compliance detail — the consolidated cross-jurisdictional compliance view is owned by material_regulatory_status. Supports EPA TSCA compliance, CDR submissions, PMN filings, and import/export certification requirements for US operations.';

CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` (
    `material_hazard_profile_id` BIGINT COMMENT 'System-generated unique identifier for each material hazard profile record.',
    `material_master_id` BIGINT COMMENT 'Foreign key reference to the master material record for which this hazard profile applies.',
    `aquatic_toxicity_ec50_mg_per_l` DECIMAL(18,2) COMMENT 'Effective concentration 50% for aquatic organisms (mg/L).',
    `aquatic_toxicity_lc50_mg_per_l` DECIMAL(18,2) COMMENT 'Lethal concentration 50% for aquatic organisms (mg/L).',
    `autoignition_temperature_celsius` DECIMAL(18,2) COMMENT 'Minimum temperature at which the material will spontaneously ignite without an external flame.',
    `boiling_point_celsius` DECIMAL(18,2) COMMENT 'Temperature at which the material transitions from liquid to vapor at standard pressure.',
    `carcinogen_iarc_class` STRING COMMENT 'International Agency for Research on Cancer (IARC) carcinogen class.. Valid values are `1|2A|2B|3|4`',
    `carcinogen_ntp_class` STRING COMMENT 'National Toxicology Program (NTP) carcinogen class.. Valid values are `A|B|C|D`',
    `cas_number` STRING COMMENT 'Unique registry number assigned by CAS to identify the chemical substance.. Valid values are `^[0-9]{2,7}-[0-9]{2}-[0-9]$`',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the hazard profile record was first created in the system.',
    `dot_hazard_class` STRING COMMENT 'U.S. Department of Transportation hazard class for transport (e.g., 1, 2.1, 3, 4, 5.1, 6, 8, 9). [ENUM-REF-CANDIDATE: 1|2.1|2.2|2.3|3|4|5.1|5.2|6|7|8|9 — promote to reference product]',
    `dot_packing_group` STRING COMMENT 'DOT packing group (I, II, III) indicating the degree of danger for transport.. Valid values are `I|II|III`',
    `effective_date` DATE COMMENT 'Date when the hazard profile becomes effective.',
    `epa_waste_code` STRING COMMENT 'EPA waste code (e.g., D001, F003) identifying the hazardous waste category.',
    `expiration_date` DATE COMMENT 'Date when the hazard profile expires or is superseded.',
    `flash_point_celsius` DECIMAL(18,2) COMMENT 'Lowest temperature at which the material can form an ignitable mixture in air.',
    `hap_designation` BOOLEAN COMMENT 'True if the material is designated as a Hazardous Air Pollutant under the Clean Air Act.',
    `idlh_value` DECIMAL(18,2) COMMENT 'Concentration (ppm) at which the material is immediately dangerous to life or health.',
    `marine_pollutant_flag` BOOLEAN COMMENT 'Indicates whether the material is classified as a marine pollutant under MARPOL.',
    `nfpa_flammability_rating` STRING COMMENT 'NFPA 704 flammability rating (0‑4) indicating the materials fire hazard.. Valid values are `0|1|2|3|4`',
    `nfpa_health_rating` STRING COMMENT 'NFPA 704 health hazard rating (0‑4) indicating the degree of health risk.. Valid values are `0|1|2|3|4`',
    `nfpa_reactivity_rating` STRING COMMENT 'NFPA 704 reactivity rating (0‑4) indicating the materials chemical instability.. Valid values are `0|1|2|3|4`',
    `nfpa_special_hazards` STRING COMMENT 'Codes for special hazards (e.g., W for water reactive, OX for oxidizer) as defined by NFPA.',
    `notes` STRING COMMENT 'Free‑form comments or special handling instructions for the material hazard profile.',
    `occupational_exposure_limit_pel` DECIMAL(18,2) COMMENT 'OSHA permissible exposure limit (ppm) for the material.',
    `occupational_exposure_limit_tlv` DECIMAL(18,2) COMMENT 'ACGIH threshold limit value (ppm) for the material.',
    `profile_status` STRING COMMENT 'Current lifecycle status of the hazard profile record.. Valid values are `active|inactive|archived`',
    `proper_shipping_name` STRING COMMENT 'Official shipping name required on transport documents.',
    `rcra_classification` STRING COMMENT 'Resource Conservation and Recovery Act classification for waste handling.. Valid values are `exempt|non-exempt|conditionally_exempt`',
    `reproductive_toxin_flag` BOOLEAN COMMENT 'Indicates whether the material is classified as a reproductive toxin.',
    `source_system` STRING COMMENT 'Originating system that supplied the hazard data (e.g., SAP, Intelex EHS, LabWare LIMS).. Valid values are `SAP|EHS|LIMS`',
    `specific_gravity` DECIMAL(18,2) COMMENT 'Ratio of the materials density to that of water at 4 °C.',
    `tri_reportable_flag` BOOLEAN COMMENT 'Indicates if the material is reportable under the EPA Toxics Release Inventory.',
    `un_number` STRING COMMENT 'Four‑digit UN identification number for hazardous materials.. Valid values are `^d{4}$`',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to the hazard profile record.',
    `vapor_pressure_kpa` DECIMAL(18,2) COMMENT 'Equilibrium vapor pressure of the material at 20 °C.',
    `voc_content_percent` DECIMAL(18,2) COMMENT 'Weight percent of VOCs present in the material.',
    CONSTRAINT pk_material_hazard_profile PRIMARY KEY(`material_hazard_profile_id`)
) COMMENT 'Consolidated non-GHS hazard and regulatory classification profile for each raw material, capturing hazard frameworks OUTSIDE the GHS/CLP system. Includes NFPA 704 diamond ratings (health, flammability, instability, special), DOT hazard class and packing group for transport, UN number, proper shipping name, marine pollutant flag, EPA hazardous waste codes (D, F, K, P, U series), RCRA classification, TRI (Toxics Release Inventory) reportable flag, HAP (Hazardous Air Pollutant) designation, VOC content percentage, OSHA PEL, ACGIH TLV, IDLH value, carcinogen classification (IARC, NTP, OSHA), reproductive toxin flag, and aquatic toxicity data (LC50, EC50). This product is the SSOT for non-GHS hazard data — GHS-specific classification (hazard classes, H-codes, P-codes, pictograms, signal words) is owned exclusively by ghs_classification. Supports EHS risk assessment, transport compliance, environmental reporting, and occupational exposure management.';
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_composition` (
    ...
    `component_cas_number` STRING COMMENT 'CAS Registry Number of the component substance.. Valid values are `^d{2,7}-d{2}-d$`',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_regulatory_status` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the regulatory affairs manager responsible for this material’s compliance.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` (
    `distribution_agreement_id` BIGINT COMMENT 'Primary key for the distribution_agreement association',
    `distributor_id` BIGINT COMMENT 'Foreign key linking to the distributor record',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to the material master record',
    `moq_kg` DECIMAL(18,2) COMMENT 'Minimum order quantity in kilograms for this material‑distributor pair',
    `price_per_unit` DECIMAL(18,2) COMMENT 'Unit price agreed for the material with this distributor',
    `reach_compliance_required` BOOLEAN COMMENT 'Indicates if REACH compliance must be demonstrated for this material when sold by the distributor',
    `tsca_compliance_required` BOOLEAN COMMENT 'Indicates if TSCA compliance must be demonstrated for this material when sold by the distributor',
    CONSTRAINT pk_distribution_agreement PRIMARY KEY(`distribution_agreement_id`)
) COMMENT 'Represents the contractual relationship between a raw material (material_master) and a distributor. Each record captures the specific terms under which a distributor is authorized to sell a material, including price, MOQ, and regulatory compliance flags.. Existence Justification: In the chemical manufacturing business, each raw material can be sold by multiple authorized distributors, and each distributor can sell multiple raw materials. The relationship is managed through a distribution agreement that captures price, minimum order quantity, and compliance requirements, and is created, updated, and deleted by business users.';

CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` (
    `rawmaterial_lot_allocation_id` BIGINT COMMENT 'Primary key for the lot_allocation association',
    `invoice_line_id` BIGINT COMMENT 'Foreign key linking to the invoice line',
    `lot_record_id` BIGINT COMMENT 'Foreign key linking to the raw material lot record',
    `expiry_date` DATE COMMENT 'Expiry date of the allocated lot at the time of invoicing',
    `lot_number` STRING COMMENT 'Internal identifier of the allocated lot',
    `quantity` DECIMAL(18,2) COMMENT 'Quantity of the lot allocated to the invoice line',
    `unit_price` DECIMAL(18,2) COMMENT 'Unit price applied for the allocated lot portion',
    CONSTRAINT pk_rawmaterial_lot_allocation PRIMARY KEY(`rawmaterial_lot_allocation_id`)
) COMMENT 'Represents the allocation of a specific raw material lot to a particular invoice line. Each record captures the quantity of the lot used, the unit price applied, the lot identifier, and the lot expiry date, enabling full traceability from procurement through billing.. Existence Justification: A raw material lot can be split across multiple customer invoice lines, and a single invoice line can be fulfilled using material from multiple lots. The allocation of specific lot quantities, unit prices, and expiry dates is actively recorded to satisfy traceability and regulatory requirements.';
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ADD CONSTRAINT `fk_rawmaterial_tsca_inventory_cas_registry_id` FOREIGN KEY (`cas_registry_id`) REFERENCES `chemical_mfg_ecm`.`rawmaterial`.`cas_registry`(`cas_registry_id`);
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ADD CONSTRAINT `fk_rawmaterial_material_hazard_profile_material_master_id` FOREIGN KEY (`material_master_id`) REFERENCES `chemical_mfg_ecm`.`rawmaterial`.`material_master`(`material_master_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ADD CONSTRAINT `fk_rawmaterial_distribution_agreement_material_master_id` FOREIGN KEY (`material_master_id`) REFERENCES `chemical_mfg_ecm`.`rawmaterial`.`material_master`(`material_master_id`);
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ADD CONSTRAINT `fk_rawmaterial_rawmaterial_lot_allocation_lot_record_id` FOREIGN KEY (`lot_record_id`) REFERENCES `chemical_mfg_ecm`.`rawmaterial`.`lot_record`(`lot_record_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_master` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_master` ALTER COLUMN `formula_id` SET TAGS ('dbx_business_glossary_term' = 'Formula Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_master` ALTER COLUMN `rawmaterial_sds_record_id` SET TAGS ('dbx_business_glossary_term' = 'Safety Data Sheet Document ID');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`cas_registry` SET TAGS ('dbx_subdomain' = 'regulatory_compliance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` ALTER COLUMN `cas_number` SET TAGS ('dbx_business_glossary_term' = 'Chemical Abstracts Service (CAS) Number');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` ALTER COLUMN `cas_number` SET TAGS ('dbx_value_regex' = '^d{2,7}-d{2}-d$');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` ALTER COLUMN `other_information` SET TAGS ('dbx_business_glossary_term' = 'Other Information');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` ALTER COLUMN `supplier_contact` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contact Information');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`sds_record` ALTER COLUMN `supplier_name` SET TAGS ('dbx_business_glossary_term' = 'Supplier Name');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_ghs_classification` ALTER COLUMN `material_name` SET TAGS ('dbx_business_glossary_term' = 'Material Name');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_specification` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_specification` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Owner Employee Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_specification` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_specification` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_specification` ALTER COLUMN `material_code` SET TAGS ('dbx_business_glossary_term' = 'Material Code');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`supplier_material` SET TAGS ('dbx_subdomain' = 'supplier_management');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_qualification` SET TAGS ('dbx_subdomain' = 'supplier_management');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_qualification` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Quality Engineer Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_qualification` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_qualification` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_qualification` ALTER COLUMN `project_id` SET TAGS ('dbx_business_glossary_term' = 'Rd Project Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`incoming_inspection` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`incoming_inspection` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Inspector ID');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`incoming_inspection` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`incoming_inspection` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`lot_record` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Authorized Signatory Employee Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`coa_document` ALTER COLUMN `supplier_lot_number` SET TAGS ('dbx_business_glossary_term' = 'Supplier Lot Number');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`reach_registration` ALTER COLUMN `cas_number` SET TAGS ('dbx_business_glossary_term' = 'CAS Registry Number (CAS_NUMBER)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`reach_registration` ALTER COLUMN `cas_number` SET TAGS ('dbx_value_regex' = '^d{2,7}-d{2}-d$');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`reach_registration` ALTER COLUMN `registration_holder` SET TAGS ('dbx_business_glossary_term' = 'Registration Holder (REG_HOLDER)');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` SET TAGS ('dbx_subdomain' = 'regulatory_compliance');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `tsca_inventory_id` SET TAGS ('dbx_business_glossary_term' = 'TSCA Inventory Record Identifier');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `cas_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Cas Registry Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `cdr_obligation_flag` SET TAGS ('dbx_business_glossary_term' = 'Chemical Data Reporting (CDR) Obligation');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `cdr_reporting_year` SET TAGS ('dbx_business_glossary_term' = 'CDR Reporting Year');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `cdr_volume` SET TAGS ('dbx_business_glossary_term' = 'CDR Reported Volume');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `consent_order_flag` SET TAGS ('dbx_business_glossary_term' = 'Consent Order Indicator');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `consent_order_number` SET TAGS ('dbx_business_glossary_term' = 'Consent Order Number');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Record Effective Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `exemption_description` SET TAGS ('dbx_business_glossary_term' = 'Exemption Description');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `exemption_type` SET TAGS ('dbx_business_glossary_term' = 'Exemption Type');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `exemption_type` SET TAGS ('dbx_value_regex' = 'active|inactive|exempt|polymer_exempt|none');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Record Expiration Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `listing_status_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Listing Status Effective Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `listing_status_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Listing Status Expiration Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `pmn_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'PMN Filed Indicator');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `pmn_number` SET TAGS ('dbx_business_glossary_term' = 'Pre‑Manufacture Notice (PMN) Number');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `processing_use_code` SET TAGS ('dbx_business_glossary_term' = 'Processing Use Code');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `processing_use_description` SET TAGS ('dbx_business_glossary_term' = 'Processing Use Description');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `record_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `record_status` SET TAGS ('dbx_value_regex' = 'current|archived');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `risk_evaluation_status` SET TAGS ('dbx_business_glossary_term' = 'Risk Evaluation Status');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `risk_evaluation_status` SET TAGS ('dbx_value_regex' = 'prioritized|under_evaluation|completed|not_started');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `risk_evaluation_year` SET TAGS ('dbx_business_glossary_term' = 'Risk Evaluation Year');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `section5_order_flag` SET TAGS ('dbx_business_glossary_term' = 'Section 5 Order Indicator');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `snur_applicable_flag` SET TAGS ('dbx_business_glossary_term' = 'Significant New Use Rule (SNUR) Applicability');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `test_rule_description` SET TAGS ('dbx_business_glossary_term' = 'Test Rule Description');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `test_rule_flag` SET TAGS ('dbx_business_glossary_term' = 'Test Rule Indicator');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `tsca_listing_status` SET TAGS ('dbx_business_glossary_term' = 'TSCA Listing Status');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `tsca_listing_status` SET TAGS ('dbx_value_regex' = 'active|inactive|exempt|polymer_exempt');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `volume_band` SET TAGS ('dbx_business_glossary_term' = 'Volume Band');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `volume_band` SET TAGS ('dbx_value_regex' = 'low|medium|high');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `volume_units` SET TAGS ('dbx_business_glossary_term' = 'Volume Units');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`tsca_inventory` ALTER COLUMN `volume_units` SET TAGS ('dbx_value_regex' = 'kg|lb|metric_ton|gallons');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` SET TAGS ('dbx_subdomain' = 'regulatory_compliance');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `material_hazard_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Material Hazard Profile Identifier');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Identifier');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `aquatic_toxicity_ec50_mg_per_l` SET TAGS ('dbx_business_glossary_term' = 'Aquatic Toxicity EC50 (mg/L)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `aquatic_toxicity_lc50_mg_per_l` SET TAGS ('dbx_business_glossary_term' = 'Aquatic Toxicity LC50 (mg/L)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `autoignition_temperature_celsius` SET TAGS ('dbx_business_glossary_term' = 'Autoignition Temperature (°C)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `boiling_point_celsius` SET TAGS ('dbx_business_glossary_term' = 'Boiling Point (°C)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `carcinogen_iarc_class` SET TAGS ('dbx_business_glossary_term' = 'IARC Carcinogen Classification');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `carcinogen_iarc_class` SET TAGS ('dbx_value_regex' = '1|2A|2B|3|4');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `carcinogen_ntp_class` SET TAGS ('dbx_business_glossary_term' = 'NTP Carcinogen Classification');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `carcinogen_ntp_class` SET TAGS ('dbx_value_regex' = 'A|B|C|D');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `cas_number` SET TAGS ('dbx_business_glossary_term' = 'Chemical Abstracts Service (CAS) Number');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `cas_number` SET TAGS ('dbx_value_regex' = '^[0-9]{2,7}-[0-9]{2}-[0-9]$');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `dot_hazard_class` SET TAGS ('dbx_business_glossary_term' = 'DOT Hazard Class');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `dot_packing_group` SET TAGS ('dbx_business_glossary_term' = 'DOT Packing Group');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `dot_packing_group` SET TAGS ('dbx_value_regex' = 'I|II|III');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `epa_waste_code` SET TAGS ('dbx_business_glossary_term' = 'EPA Hazardous Waste Code');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `flash_point_celsius` SET TAGS ('dbx_business_glossary_term' = 'Flash Point (°C)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `hap_designation` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Air Pollutant Designation');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `idlh_value` SET TAGS ('dbx_business_glossary_term' = 'Immediately Dangerous to Life or Health (IDLH) Value');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `marine_pollutant_flag` SET TAGS ('dbx_business_glossary_term' = 'Marine Pollutant Flag');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_flammability_rating` SET TAGS ('dbx_business_glossary_term' = 'NFPA Flammability Rating');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_flammability_rating` SET TAGS ('dbx_value_regex' = '0|1|2|3|4');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_health_rating` SET TAGS ('dbx_business_glossary_term' = 'NFPA Health Hazard Rating');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_health_rating` SET TAGS ('dbx_value_regex' = '0|1|2|3|4');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_health_rating` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_health_rating` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_reactivity_rating` SET TAGS ('dbx_business_glossary_term' = 'NFPA Reactivity Rating');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_reactivity_rating` SET TAGS ('dbx_value_regex' = '0|1|2|3|4');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `nfpa_special_hazards` SET TAGS ('dbx_business_glossary_term' = 'NFPA Special Hazard Symbols');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Additional Notes');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `occupational_exposure_limit_pel` SET TAGS ('dbx_business_glossary_term' = 'Occupational Exposure Limit – Permissible Exposure Limit (PEL)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `occupational_exposure_limit_tlv` SET TAGS ('dbx_business_glossary_term' = 'Occupational Exposure Limit – Threshold Limit Value (TLV)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `profile_status` SET TAGS ('dbx_business_glossary_term' = 'Hazard Profile Status');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `profile_status` SET TAGS ('dbx_value_regex' = 'active|inactive|archived');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `proper_shipping_name` SET TAGS ('dbx_business_glossary_term' = 'Proper Shipping Name');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `rcra_classification` SET TAGS ('dbx_business_glossary_term' = 'RCRA Classification');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `rcra_classification` SET TAGS ('dbx_value_regex' = 'exempt|non-exempt|conditionally_exempt');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `reproductive_toxin_flag` SET TAGS ('dbx_business_glossary_term' = 'Reproductive Toxicant Flag');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `source_system` SET TAGS ('dbx_value_regex' = 'SAP|EHS|LIMS');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `specific_gravity` SET TAGS ('dbx_business_glossary_term' = 'Specific Gravity');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `tri_reportable_flag` SET TAGS ('dbx_business_glossary_term' = 'TRI Reportable Flag');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `un_number` SET TAGS ('dbx_business_glossary_term' = 'United Nations (UN) Number');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `un_number` SET TAGS ('dbx_value_regex' = '^d{4}$');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `vapor_pressure_kpa` SET TAGS ('dbx_business_glossary_term' = 'Vapor Pressure (kPa)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_hazard_profile` ALTER COLUMN `voc_content_percent` SET TAGS ('dbx_business_glossary_term' = 'Volatile Organic Compounds (VOC) Content Percentage');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_composition` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_composition` SET TAGS ('dbx_subdomain' = 'quality_assurance');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_composition` ALTER COLUMN `component_cas_number` SET TAGS ('dbx_business_glossary_term' = 'Component CAS Number (CAS)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_composition` ALTER COLUMN `component_cas_number` SET TAGS ('dbx_value_regex' = '^d{2,7}-d{2}-d$');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_regulatory_status` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Manager ID (RMI)');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_regulatory_status` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`material_regulatory_status` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` SET TAGS ('dbx_subdomain' = 'supplier_management');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` SET TAGS ('dbx_association_edges' = 'rawmaterial.material_master,sales.distributor');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `distribution_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Agreement - Distribution Agreement Id');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `distributor_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Agreement - Distributor Id');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Agreement - Material Master Id');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `moq_kg` SET TAGS ('dbx_business_glossary_term' = 'Minimum Order Quantity');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `moq_kg` SET TAGS ('dbx_operational' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `price_per_unit` SET TAGS ('dbx_business_glossary_term' = 'Agreed Unit Price');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `price_per_unit` SET TAGS ('dbx_financial' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `reach_compliance_required` SET TAGS ('dbx_business_glossary_term' = 'REACH Compliance Required');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `reach_compliance_required` SET TAGS ('dbx_regulatory' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `tsca_compliance_required` SET TAGS ('dbx_business_glossary_term' = 'TSCA Compliance Required');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`distribution_agreement` ALTER COLUMN `tsca_compliance_required` SET TAGS ('dbx_regulatory' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` SET TAGS ('dbx_subdomain' = 'supplier_management');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` SET TAGS ('dbx_association_edges' = 'rawmaterial.lot_record,billing.invoice_line');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `rawmaterial_lot_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Allocation - Lot Allocation Id');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `invoice_line_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Allocation - Invoice Line Id');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `lot_record_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Allocation - Lot Record Id');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Lot Expiry Date');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `lot_number` SET TAGS ('dbx_business_glossary_term' = 'Lot Number');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `quantity` SET TAGS ('dbx_business_glossary_term' = 'Allocated Quantity');
ALTER TABLE `chemical_mfg_ecm`.`rawmaterial`.`rawmaterial_lot_allocation` ALTER COLUMN `unit_price` SET TAGS ('dbx_business_glossary_term' = 'Allocated Unit Price');
