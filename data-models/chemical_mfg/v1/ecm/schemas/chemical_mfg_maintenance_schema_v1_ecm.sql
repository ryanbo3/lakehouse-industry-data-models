-- Schema for Domain: maintenance | Business: Chemical Mfg | Version: v1_ecm
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` (
    ...
    `plant_id` BIGINT COMMENT 'Reference to the manufacturing plant or facility where this equipment is physically installed and operated.',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Assigns a sales rep to each sold equipment; used in after‑sales support KPIs and regional sales performance dashboards.',
    `territory_id` BIGINT COMMENT 'Foreign key linking to sales.territory. Business justification: Maps equipment to the sales territory where it is installed; supports regional service planning and territory performance metrics.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`functional_location` (
    ...
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Functional location belongs to a plant; plant_code is redundant after FK.',
    ...
    `parent_floc_code` STRING COMMENT 'The floc_code of the immediate parent node in the functional location hierarchy. Enables recursive traversal of the plant topology from plant down to sub-system level. Null for top-level plant nodes. Corresponds to SAP PM IFLOT-TPLMA.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Work Order Creation Report requires tracking which employee created each work order for audit and compliance.',
    ...
    `freight_order_id` BIGINT COMMENT 'Foreign key linking to logistics.freight_order. Business justification: Freight Order Maintenance Request creates a work order to service carrier equipment or resolve issues tied to a specific freight order.',
    ...
    `shipment_id` BIGINT COMMENT 'Foreign key linking to logistics.shipment. Business justification: Shipment‑Delay Work Order process requires linking a work order to the affected shipment for root‑cause analysis and delay reporting.',
    ...
    `planner_group_id` BIGINT COMMENT 'Foreign key linking to maintenance.planner_group. Business justification: Work order is planned by a planner group; code string replaced by FK.',
    ...
    `quote_id` BIGINT COMMENT 'Foreign key linking to sales.quote. Business justification: Links installation work orders to the original sales quote; enables cost reconciliation and quote‑to‑service reporting.',
    ...
    `notification_id` BIGINT COMMENT 'Reference to the originating maintenance notification or trouble report that triggered creation of this work order. Links work order to the initial problem report or service request.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`work_order_operation` (
    ...
    `plant_id` BIGINT COMMENT 'FK to maintenance.plant',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`notification` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the operator, inspector, or technician who created the notification. Supports accountability and follow-up communication.',
    ...
    `catalog_code_id` BIGINT COMMENT 'Reference to the catalog code describing the corrective activity or repair action required (e.g., replace seal, recalibrate instrument, clean filter).',
    `notification_catalog_code_id` BIGINT COMMENT 'Reference to the catalog code describing the type of damage or defect observed (e.g., corrosion, wear, leakage, electrical failure). Supports Pareto analysis of failure modes.',
    `notification_cause_code_maintenance_catalog_catalog_code_id` BIGINT COMMENT 'Reference to the catalog code identifying the root cause of the failure (e.g., material fatigue, operator error, design flaw, inadequate maintenance). Critical for Root Cause Investigation (RCI) and Corrective and Preventive Action (CAPA) workflows.',
    `planner_group_id` BIGINT COMMENT 'Foreign key linking to maintenance.planner_group. Business justification: Notification is planned by a planner group; replace string with FK.',
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Notification is tied to a plant; replace plant code with FK.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` (
    ...
    `maintenance_strategy_id` BIGINT COMMENT 'Reference to the maintenance strategy defining cycle packages and intervals (e.g., 3-month inspection, 6-month service, 12-month overhaul, 5-year major turnaround). Used for strategy_plan type.',
    `planner_group_id` BIGINT COMMENT 'Reference to the maintenance planner group responsible for scheduling, coordinating, and managing the PM plan execution.',
    `plant_id` BIGINT COMMENT 'Reference to the manufacturing plant or facility where the PM plan is executed. Supports multi-site maintenance management.',
    `rbi_assessment_id` BIGINT COMMENT 'Reference to the RBI assessment that optimized the inspection interval for this PM plan based on probability of failure and consequence of failure analysis. Supports API 580/581 RBI methodology.',
    `strategy_id` BIGINT COMMENT 'Foreign key linking to pricing.pricing_strategy. Business justification: Preventive maintenance plans are aligned with pricing strategies to ensure maintenance cost recovery is reflected in product pricing.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`schedule_call` (
    ...
    `planner_group_id` BIGINT COMMENT 'Foreign key linking to maintenance.planner_group. Business justification: Schedule call is assigned to a planner group; replace string with FK.',
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Schedule call occurs at a plant; replace plant code string with FK.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_point` (
    ...
    `market_price_index_id` BIGINT COMMENT 'Foreign key linking to pricing.market_price_index. Business justification: Raw‑material measurement points are tied to market price indices for dynamic cost adjustments in pricing calculations.',
    `planner_group_id` BIGINT COMMENT 'Foreign key linking to maintenance.planner_group. Business justification: Measurement point is managed by a planner group; replace string with FK.',
    ...
    `plant_code` STRING COMMENT 'Code identifying the manufacturing plant or facility where the measurement point is located. Supports multi-site maintenance management.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_reading` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the person or system that captured the measurement reading. For manual readings, this is the employee ID; for automatic readings, this is the sensor or SCADA system identifier.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` (
    ...
    `calibration_standard_id` BIGINT COMMENT 'Identifier of the reference standard or master instrument used for calibration. Ensures traceability to national or international standards.',
    ...
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Calibration record is linked to a plant for traceability; replace plant code with FK.',
    `employee_id` BIGINT COMMENT 'Reference to the technician or calibration specialist who performed the calibration. Required for accountability and competency tracking.',
    `reviewer_employee_id` BIGINT COMMENT 'Reference to the quality assurance reviewer who verified and approved the calibration record.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` (
    `asset_change_request_id` BIGINT COMMENT 'Unique identifier for the Management of Change (MOC) request record. Primary key for the asset change request entity.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: MOC requests are budgeted and expensed; assigning a cost center enables tracking of change‑related expenditures.',
    `equipment_id` BIGINT COMMENT 'Primary equipment identifier for the asset being modified or affected by the change request.',
    `functional_location_id` BIGINT COMMENT 'Identifier of the functional location within the plant hierarchy where the change will occur, used for asset management and maintenance planning.',
    `management_of_change_id` BIGINT COMMENT 'FK to ehs.management_of_change.management_of_change_id — Asset/equipment change requests must link to master MOC record for OSHA PSM 1910.119 compliance. Enables unified MOC tracking across all change types.',
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Asset change request (MOC) is associated with a plant; replace code with FK.',
    `employee_id` BIGINT COMMENT 'Identifier of the employee who originated and submitted the MOC request.',
    `tertiary_asset_closed_by_employee_id` BIGINT COMMENT 'Identifier of the employee who formally closed the MOC after verifying all requirements were met.',
    `actual_completion_date` DATE COMMENT 'Actual date when all implementation activities were completed and the change was fully operational.',
    `actual_start_date` DATE COMMENT 'Actual date when implementation activities commenced.',
    `affected_equipment_list` STRING COMMENT 'Comma-separated list or narrative description of all equipment, vessels, reactors, pumps, and instrumentation affected by the proposed change.',
    `affected_pid_list` STRING COMMENT 'List of P&ID drawing numbers that will require revision as a result of the proposed change.',
    `approval_required_roles` STRING COMMENT 'List of organizational roles or positions required to approve the MOC before implementation can proceed (e.g., Plant Manager, EHS Manager, Process Engineer, Maintenance Manager).',
    `approval_status` STRING COMMENT 'Current state of the approval workflow indicating whether all required approvals have been obtained.. Valid values are `pending|partial|approved|rejected`',
    `budget_code` STRING COMMENT 'Budget or cost center code to which the change implementation costs will be charged.',
    `capex_opex_classification` STRING COMMENT 'Financial classification of the change costs as capital expenditure, operating expenditure, or a combination of both.. Valid values are `capex|opex|mixed`',
    `change_description` STRING COMMENT 'Detailed narrative description of the proposed change including scope, rationale, technical specifications, and expected outcomes.',
    `change_justification` STRING COMMENT 'Business and technical justification for the proposed change including safety improvements, efficiency gains, regulatory compliance, or operational necessity.',
    `change_title` STRING COMMENT 'Short descriptive title summarizing the proposed change for quick identification and reference.',
    `change_type` STRING COMMENT 'Classification of the type of change being requested: equipment modification, piping alteration, instrumentation upgrade, chemical substitution, procedure revision, or operating condition adjustment.. Valid values are `equipment|piping|instrumentation|chemical|procedure|operating_condition`',
    `closure_date` DATE COMMENT 'Date when the MOC was formally closed after all implementation activities, PSSR, training, documentation updates, and post-implementation review were completed.',
    `cost_currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the estimated cost amount.. Valid values are `^[A-Z]{3}$`',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when the MOC request record was first created in the system.',
    `environmental_impact_description` STRING COMMENT 'Assessment of environmental impacts including emissions changes, waste generation, spill potential, and regulatory compliance implications.',
    `estimated_cost` DECIMAL(18,2) COMMENT 'Total estimated cost for implementing the change including materials, labor, engineering, downtime, and contractor expenses.',
    `final_approval_timestamp` TIMESTAMP COMMENT 'Date and time when the final approval was granted and the MOC was authorized for implementation.',
    `implementation_plan` STRING COMMENT 'Detailed plan describing how the change will be executed including sequence of activities, resource requirements, safety precautions, and coordination needs.',
    `initiator_department` STRING COMMENT 'Department or functional area of the initiating employee, used for routing and accountability.',
    `initiator_name` STRING COMMENT 'Full name of the employee who initiated the MOC request for reference and communication purposes.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Date and time when the MOC request record was last updated, used for audit trail and change tracking.',
    `moc_number` STRING COMMENT 'Business identifier for the MOC request, externally visible and used for tracking and reference across systems and documentation.. Valid values are `^MOC-[0-9]{6,10}$`',
    `moc_status` STRING COMMENT 'Current lifecycle status of the MOC request from initiation through closure. Tracks workflow progression through review, approval, implementation, and PSSR completion stages. [ENUM-REF-CANDIDATE: draft|submitted|under_review|approved|rejected|implementation|pssr_pending|closed|cancelled — 9 candidates stripped; promote to reference product]',
    `operational_impact_description` STRING COMMENT 'Description of impacts on production operations including downtime requirements, capacity changes, quality effects, and process performance.',
    `planned_completion_date` DATE COMMENT 'Target date for completing all implementation activities and closing out the MOC.',
    `planned_start_date` DATE COMMENT 'Target date for beginning implementation of the approved change.',
    `post_implementation_review_date` DATE COMMENT 'Date when the post-implementation review was conducted.',
    `post_implementation_review_findings` STRING COMMENT 'Summary of findings from the post-implementation review including effectiveness assessment, lessons learned, and any corrective actions required.',
    `post_implementation_review_required_flag` BOOLEAN COMMENT 'Indicates whether a formal post-implementation review is required to verify the change achieved its objectives and did not introduce unintended consequences.',
    `priority_level` STRING COMMENT 'Business priority assigned to the change request based on safety impact, operational urgency, regulatory deadlines, or strategic importance.. Valid values are `critical|high|medium|low`',
    `psm_covered_process_flag` BOOLEAN COMMENT 'Indicates whether the change affects a process covered under OSHA PSM regulations (29 CFR 1910.119), requiring enhanced review and documentation.',
    `pssr_completion_date` DATE COMMENT 'Date when the PSSR was completed and the equipment or process was cleared for startup.',
    `pssr_required_flag` BOOLEAN COMMENT 'Indicates whether a Pre-Startup Safety Review is required before the modified equipment or process can be returned to service, as mandated by OSHA PSM for covered processes.',
    `rejection_reason` STRING COMMENT 'Explanation provided when the MOC request is rejected, including specific concerns or deficiencies that must be addressed.',
    `risk_assessment_method` STRING COMMENT 'Type of risk assessment methodology applied: Process Hazard Analysis, Hazard and Operability Study, What-If Analysis, Failure Mode and Effects Analysis, Layer of Protection Analysis, or not required.. Valid values are `pha|hazop|what_if|fmea|lopa|not_required`',
    `risk_assessment_required_flag` BOOLEAN COMMENT 'Indicates whether a formal risk assessment (PHA, HAZOP, or similar) is required for this change based on its scope and potential impact.',
    `risk_level` STRING COMMENT 'Overall risk level determined from the risk assessment, considering safety, environmental, and operational impacts.. Valid values are `high|medium|low|negligible`',
    `safety_impact_description` STRING COMMENT 'Detailed description of potential safety impacts including hazards introduced, hazards eliminated, and mitigation measures required.',
    `sop_update_completion_date` DATE COMMENT 'Date when all required SOP revisions were completed and approved.',
    `sop_update_required_flag` BOOLEAN COMMENT 'Indicates whether existing SOPs must be revised to reflect the implemented change.',
    `training_completion_status` STRING COMMENT 'Current status of required employee training activities associated with the MOC implementation.. Valid values are `not_started|in_progress|completed|not_required`',
    `training_required_flag` BOOLEAN COMMENT 'Indicates whether employee training is required as a result of the change to procedures, equipment operation, or safety protocols.',
    CONSTRAINT pk_asset_change_request PRIMARY KEY(`asset_change_request_id`)
) COMMENT 'Management of Change (MOC) request record for proposed changes to process equipment, piping, instrumentation, chemicals, procedures, or operating conditions. Captures change type, description, initiator, affected equipment and P&IDs, risk assessment, required approvals, action items with responsible parties and due dates, PSSR tasks, implementation status, and post-implementation review. Supports PSM MOC requirements (OSHA 29 CFR 1910.119) and integrates with EHS workflows. Tracks full MOC lifecycle from initiation through action item completion and closure.';
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the user or operator who first reported the breakdown event to the maintenance system.',
    ...
    `planner_group_id` BIGINT COMMENT 'Foreign key linking to maintenance.planner_group. Business justification: Breakdown event is managed by a planner group; replace string with FK.',
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Breakdown event occurs at a plant; replace plant code with FK.',
    ...
    `affected_production_unit` STRING COMMENT 'Identifier of the production unit, process line, or manufacturing cell that was impacted by the equipment breakdown.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` (
    `inspection_round_id` BIGINT COMMENT 'Unique identifier for the inspection round record. Primary key for the inspection round entity.',
    `checklist_template_id` BIGINT COMMENT 'Reference to the structured checklist template used during this inspection round. Defines the inspection points, parameters, and acceptance criteria.',
    `equipment_id` BIGINT COMMENT 'Reference to the specific equipment, reactor, vessel, pump, or instrument inspected during this round.',
    `work_order_id` BIGINT COMMENT 'Reference to the primary maintenance work order created to address findings from this inspection round. Links inspection findings to corrective maintenance actions.',
    `functional_location_id` BIGINT COMMENT 'Reference to the functional location where the inspection round was performed. Represents the hierarchical location structure in the plant.',
    `employee_id` BIGINT COMMENT 'Reference to the user or employee who performed the inspection round. Links to workforce/employee master data.',
    `inspection_route_id` BIGINT COMMENT 'Reference to the predefined inspection route or path that defines the sequence of equipment and checkpoints to be inspected during this round.',
    `work_center_id` BIGINT COMMENT 'Reference to the maintenance work center responsible for this inspection round. Identifies the organizational unit performing the inspection.',
    `actual_completion_timestamp` TIMESTAMP COMMENT 'Date and time when the inspection round was completed. Used to calculate inspection duration and verify compliance with scheduled intervals.',
    `actual_start_timestamp` TIMESTAMP COMMENT 'Date and time when the inspector actually began the inspection round. Captures the real-world start of the inspection activity.',
    `approval_required_flag` BOOLEAN COMMENT 'Indicates whether the inspection round results require formal approval by a supervisor, engineer, or regulatory authority before being finalized.',
    `approval_timestamp` TIMESTAMP COMMENT 'Date and time when the inspection round results were formally approved. Part of the audit trail for compliance documentation.',
    `cancellation_reason` STRING COMMENT 'Explanation for why the inspection round was cancelled if status is cancelled. Captures reasons such as equipment unavailability, safety concerns, or rescheduling.',
    `compliance_due_date` DATE COMMENT 'Regulatory or statutory deadline by which this inspection must be completed to maintain compliance. Overdue inspections may result in regulatory violations or insurance coverage issues.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this inspection round record was first created in the system. Part of the audit trail for data lineage and compliance.',
    `deviations_found_count` STRING COMMENT 'Number of deviations, anomalies, or out-of-specification conditions identified during the inspection round. Triggers follow-up actions and work orders.',
    `highest_severity_level` STRING COMMENT 'The most severe classification level among all deviations found during the inspection round. Critical findings require immediate action; high findings require urgent attention; medium and low findings are tracked for planned maintenance.. Valid values are `critical|high|medium|low|none`',
    `inspection_findings_summary` STRING COMMENT 'High-level summary text describing the overall findings, observations, and conclusions from the inspection round. Provides executive-level overview of inspection results.',
    `inspection_interval_days` STRING COMMENT 'Defined interval in days between successive inspection rounds for this equipment or route. Based on regulatory requirements, manufacturer recommendations, or risk-based inspection methodology.',
    `inspection_status` STRING COMMENT 'Current lifecycle status of the inspection round. Tracks progression from scheduled through execution to completion or cancellation.. Valid values are `scheduled|in_progress|completed|cancelled|on_hold`',
    `inspector_notes` STRING COMMENT 'Free-form text field for inspector comments, observations, and contextual information not captured in structured checklist fields. Provides qualitative insights and situational context.',
    `inspector_qualification_code` STRING COMMENT 'Code representing the qualification or certification level of the inspector performing the round. Ensures inspections are performed by appropriately trained personnel per OSHA PSM and API requirements.. Valid values are `^[A-Z0-9]{2,10}$`',
    `insurance_carrier_code` STRING COMMENT 'Code identifying the insurance carrier for whom this inspection is performed or to whom results must be reported.. Valid values are `^[A-Z0-9]{2,10}$`',
    `insurance_inspection_flag` BOOLEAN COMMENT 'Indicates whether this inspection round is required by or performed for insurance carrier compliance. Insurance carriers often mandate periodic inspections of pressure vessels, boilers, and critical equipment.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Date and time when this inspection round record was last updated. Tracks the most recent change for audit and data quality purposes.',
    `moc_reference_number` STRING COMMENT 'Reference number of the MOC workflow initiated as a result of this inspection round findings. Links inspection to change management process.. Valid values are `^MOC-[A-Z0-9]{6,15}$`',
    `moc_required_flag` BOOLEAN COMMENT 'Indicates whether findings from this inspection round require a Management of Change (MOC) process before corrective actions can be implemented, per PSM requirements for process equipment modifications.',
    `next_scheduled_date` DATE COMMENT 'Calculated date for the next inspection round based on the inspection interval and completion date of this round. Used for forward planning and scheduling.',
    `plant_code` STRING COMMENT 'Four-character code identifying the manufacturing plant or facility where the inspection round was performed.. Valid values are `^[A-Z0-9]{4}$`',
    `psm_mechanical_integrity_flag` BOOLEAN COMMENT 'Indicates whether this inspection round is part of the PSM mechanical integrity program required by OSHA for process equipment handling highly hazardous chemicals.',
    `regulatory_requirement_code` STRING COMMENT 'Code identifying the specific regulatory or statutory requirement that mandates this inspection (e.g., OSHA PSM, API 510, API 570, API 653, insurance carrier requirement, local jurisdiction code).. Valid values are `^[A-Z0-9_]{2,20}$`',
    `round_number` STRING COMMENT 'Business identifier for the inspection round. Externally visible unique number used for tracking and reference in maintenance systems and compliance documentation.. Valid values are `^[A-Z0-9]{6,20}$`',
    `round_type` STRING COMMENT 'Classification of the inspection round activity. Operator rounds are routine operational checks; predictive inspections use condition monitoring; statutory/regulatory inspections meet legal requirements; PSM mechanical integrity inspections support Process Safety Management programs; safety device inspections verify critical safety equipment.. Valid values are `operator_round|predictive_inspection|statutory_inspection|regulatory_inspection|psm_mechanical_integrity|safety_device_inspection`',
    `safety_critical_flag` BOOLEAN COMMENT 'Indicates whether this inspection round covers safety-critical equipment or systems that are subject to Process Safety Management (PSM) mechanical integrity requirements.',
    `scheduled_date` DATE COMMENT 'Planned date when the inspection round is scheduled to be performed. Used for planning and resource allocation.',
    `total_checkpoints_completed` STRING COMMENT 'Actual number of inspection checkpoints completed during the round. Used to track inspection progress and completeness.',
    `total_checkpoints_planned` STRING COMMENT 'Total number of inspection checkpoints or measurement points planned for this round based on the route and checklist definition.',
    `work_order_generated_flag` BOOLEAN COMMENT 'Indicates whether one or more maintenance work orders were automatically generated as a result of deviations or findings from this inspection round.',
    CONSTRAINT pk_inspection_round PRIMARY KEY(`inspection_round_id`)
) COMMENT 'Operator or maintenance inspection round record capturing structured field inspection activities performed on equipment, instruments, safety devices, and process systems at defined intervals. Tracks round type (operator rounds, predictive inspection, statutory/regulatory inspection, PSM mechanical integrity inspection), route definition, inspection date/time, inspector qualification, checklist findings, measured values, deviations noted, severity classification, and follow-up action status with work order generation. Distinct from calibration_record (which covers instrument accuracy verification) and from PM schedule_call (which triggers full maintenance work orders). Supports PSM mechanical integrity programs, OSHA Process Safety compliance, API 510/570/653 inspection documentation, and insurance carrier inspection requirements.';
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the person assigned as confined space attendant who remains outside the space to monitor entrants and initiate rescue if needed.',
    `alt5_permit_closed_by_user_employee_id` BIGINT COMMENT 'Reference to the authorized person who closed the permit after verifying work completion and area safety.',
    ...
    `plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Permit to work is issued for a specific plant; replace plant code with FK.',
    `primary_permit_employee_id` BIGINT COMMENT 'Reference to the authorized person who issued and approved the permit. Must be a qualified individual with authority to authorize work in hazardous areas.',
    `quaternary_permit_safety_officer_user_employee_id` BIGINT COMMENT 'Reference to the Environment Health and Safety (EHS) officer who reviewed and approved the permit from a safety compliance perspective.',
    `quinary_permit_fire_watch_user_employee_id` BIGINT COMMENT 'Reference to the person assigned as fire watch for hot work operations. Must be trained in fire prevention and emergency response.',
    `tertiary_permit_area_authority_user_employee_id` BIGINT COMMENT 'Reference to the operations supervisor or area owner who has authority over the work location and must approve work in their area.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` (
    `catalog_code_id` BIGINT COMMENT 'Primary key for catalog_code',
    `api_689_reference` STRING COMMENT 'Cross-reference to API 689 (Collection and Exchange of Reliability and Maintenance Data for Equipment) standard. Used for alignment with American Petroleum Institute reliability standards.',
    `capa_trigger_flag` BOOLEAN COMMENT 'Indicates whether this catalog code automatically triggers a CAPA (Corrective and Preventive Action) workflow for quality management and continuous improvement. Aligns with GMP (Good Manufacturing Practice) and ISO 9001 requirements.',
    `catalog_code_code` STRING COMMENT 'The unique alphanumeric code within the catalog type and code group. This is the actual code value used in PM (Plant Maintenance) notifications and work orders for structured failure analysis. Example: CORR for corrosion, WEAR for wear, FRAC for fracture.',
    `catalog_code_status` STRING COMMENT 'Current lifecycle status of the catalog code. Active codes are available for use in maintenance transactions. Deprecated codes are retained for historical reporting but not available for new entries.. Valid values are `active|inactive|deprecated|pending_approval`',
    `catalog_type` STRING COMMENT 'The type of catalog this code belongs to: damage codes (what failed), cause codes (why it failed), activity codes (what was done), object part codes (which component), or symptom codes (how failure manifested). Aligns with SAP PM catalog profile structure.. Valid values are `damage|cause|activity|object_part|symptom`',
    `code_group` STRING COMMENT 'Hierarchical grouping identifier for the catalog code. Enables multi-level classification and rollup reporting. For example, mechanical failures, electrical failures, instrumentation failures. Corresponds to SAP PM code group.',
    `code_long_text` STRING COMMENT 'Detailed description of the catalog code providing full definition, usage guidance, and examples. Used for training and reference documentation.',
    `code_short_text` STRING COMMENT 'Brief description of the catalog code, typically 20-40 characters. Used in dropdown lists and compact displays in maintenance transactions.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this catalog code entry was created in the system. Used for audit trail and version control.',
    `effective_end_date` DATE COMMENT 'The date after which this catalog code is no longer valid for new transactions. Null for codes with no planned end date. Historical transactions retain the code for reporting continuity.',
    `effective_start_date` DATE COMMENT 'The date from which this catalog code becomes valid and available for use in maintenance transactions. Supports versioned catalog management.',
    `ehs_reportable_flag` BOOLEAN COMMENT 'Indicates whether failures coded with this catalog entry require EHS (Environment Health and Safety) incident reporting to regulatory authorities (EPA, OSHA, ECHA).',
    `equipment_class` STRING COMMENT 'The equipment class or category this catalog code is applicable to (e.g., rotating equipment, static equipment, instrumentation, electrical, piping). Enables filtering of relevant codes by equipment type.',
    `estimated_downtime_hours` DECIMAL(18,2) COMMENT 'Typical equipment downtime duration in hours associated with this failure mode or activity code. Used for production impact planning and scheduling.',
    `estimated_repair_cost` DECIMAL(18,2) COMMENT 'Typical repair cost in base currency associated with this failure mode or activity code. Used for maintenance budgeting and cost analysis.',
    `hierarchy_level` STRING COMMENT 'The depth level in the catalog hierarchy. Level 1 is top-level, level 2 is sub-category, etc. Used for reporting aggregation and navigation.',
    `iso_14224_reference` STRING COMMENT 'Cross-reference to ISO 14224 (Petroleum, petrochemical and natural gas industries - Collection and exchange of reliability and maintenance data for equipment) failure mode taxonomy. Enables benchmarking against industry standards.',
    `language_code` STRING COMMENT 'Two-letter ISO 639-1 language code for the catalog code text. Supports multi-language catalog maintenance for global operations.. Valid values are `^[A-Z]{2}$`',
    `last_used_date` DATE COMMENT 'The most recent date this catalog code was used in a maintenance transaction. Used to identify obsolete codes for catalog cleanup.',
    `moc_trigger_flag` BOOLEAN COMMENT 'Indicates whether this failure mode or activity code automatically triggers a MOC (Management of Change) workflow for process equipment changes. Required for PSM mechanical integrity compliance.',
    `modified_by` STRING COMMENT 'User ID of the person who last modified this catalog code entry. Used for audit trail and change tracking.',
    `modified_timestamp` TIMESTAMP COMMENT 'Date and time when this catalog code entry was last modified. Used for audit trail and version control.',
    `parent_code_group` STRING COMMENT 'Reference to the parent code group in the hierarchical catalog structure. Enables multi-level rollup and drill-down analysis. Null for top-level groups.',
    `plant_section` STRING COMMENT 'The plant section or process area this catalog code is primarily used in (e.g., reactor area, distillation, packaging, utilities). Supports location-specific failure analysis.',
    `priority_indicator` STRING COMMENT 'Default priority level associated with this catalog code. Used to auto-populate priority on PM notifications when this code is selected. Critical codes trigger immediate response.. Valid values are `critical|high|medium|low`',
    `psm_critical_flag` BOOLEAN COMMENT 'Indicates whether this catalog code is associated with PSM (Process Safety Management) critical equipment or failure modes requiring special handling, documentation, and regulatory reporting per OSHA PSM requirements.',
    `rci_required_flag` BOOLEAN COMMENT 'Indicates whether failures coded with this catalog entry mandate a formal RCI (Root Cause Investigation) or RCA (Root Cause Analysis) process. Typically set for critical or recurring failure modes.',
    `sort_sequence` STRING COMMENT 'Display order sequence number for this catalog code within its code group. Controls the presentation order in dropdown lists and reports.',
    `usage_count` STRING COMMENT 'The number of times this catalog code has been used in PM notifications and work orders. Used for Pareto analysis of failure modes and catalog optimization.',
    `created_by` STRING COMMENT 'User ID of the person who created this catalog code entry. Used for audit trail and data governance.',
    CONSTRAINT pk_catalog_code PRIMARY KEY(`catalog_code_id`)
) COMMENT 'Maintenance catalog code master defining the standardized taxonomy of damage codes, cause codes, activity codes, and object part codes used across PM notifications and work orders for structured failure analysis. Each catalog entry is a first-class reference entity with its own identity (catalog type + code group + code), versioned lifecycle, hierarchical grouping structure, and cross-references to industry failure mode standards (ISO 14224, API 689). Supports RCA/RCI investigations, Pareto analysis of failure modes, reliability reporting, and consistent coding across all maintenance transactions. Aligns with SAP PM Catalog Profile (T352/T353/T357).';
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`task_list` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Task List Management process logs the employee who authored the task list for accountability and change control.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_standard` (
    ...
    `calibration_method` STRING COMMENT 'Method by which the standard is applied or maintained.',
    `compliance_standard` STRING COMMENT 'Regulatory or quality framework that the calibration standard complies with.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the calibration standard record was first created in the system.',
    `calibration_standard_description` STRING COMMENT 'Free‑text description providing additional context about the standard.',
    `effective_date` DATE COMMENT 'Date when the calibration standard becomes valid for use.',
    `expiration_date` DATE COMMENT 'Date after which the calibration standard must no longer be used (nullable).',
    `instrument_category` STRING COMMENT 'Category of equipment that uses this calibration standard.',
    `is_primary_standard` BOOLEAN COMMENT 'Indicates whether this standard is a primary reference (True) or a secondary reference (False).',
    `last_calibrated_date` DATE COMMENT 'Date when the standard was last used to calibrate equipment.',
    `manufacturer` STRING COMMENT 'Name of the organization that produced the calibration standard.',
    `model_number` STRING COMMENT 'Model identifier assigned by the manufacturer.',
    `calibration_standard_name` STRING COMMENT 'Human‑readable name of the calibration standard (e.g., "Standard Temperature 100°C").',
    `next_due_date` DATE COMMENT 'Planned date for the next calibration activity using this standard.',
    `nominal_value` DECIMAL(18,2) COMMENT 'The target measurement value defined by the standard.',
    `notes` STRING COMMENT 'Free‑form field for any supplemental information or remarks.',
    `regulatory_approval_status` STRING COMMENT 'Current approval state of the standard with respect to regulatory bodies.',
    `serial_number` STRING COMMENT 'Unique serial number of the physical standard artifact, if applicable.',
    `standard_code` STRING COMMENT 'Business code or catalogue number that uniquely identifies the standard within the organization.',
    `standard_type` STRING COMMENT 'Category of measurement the standard applies to.',
    `calibration_standard_status` STRING COMMENT 'Current lifecycle status of the standard.',
    `tolerance_minus` DECIMAL(18,2) COMMENT 'Maximum allowable deviation below the nominal value.',
    `tolerance_plus` DECIMAL(18,2) COMMENT 'Maximum allowable deviation above the nominal value.',
    `unit_of_measure` STRING COMMENT 'Unit in which the nominal value and tolerances are expressed.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to the calibration standard record.',
    `version` STRING COMMENT 'Version identifier for the calibration standard (e.g., "v2.1").',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`plant` (
    ...
    `plant_area_sq_m` DECIMAL(18,2) COMMENT 'Total built‑up area of the plant in square metres.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_route` (
    `inspection_route_id` BIGINT COMMENT 'Primary key for inspection_route',
    `superseded_inspection_route_id` BIGINT COMMENT 'Self-referencing FK on inspection_route (superseded_inspection_route_id)',
    `assigned_team` STRING COMMENT 'Name of the maintenance team responsible for executing the route.',
    `compliance_requirements` STRING COMMENT 'Regulatory or internal standards that this inspection route must satisfy.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when the inspection route record was first created in the system.',
    `inspection_route_description` STRING COMMENT 'Detailed free‑text description of the route, including purpose and special instructions.',
    `distance_meters` STRING COMMENT 'Total linear distance covered by the route, measured in meters.',
    `effective_end_date` DATE COMMENT 'Date after which the inspection route is no longer valid (nullable for open‑ended).',
    `effective_start_date` DATE COMMENT 'Date from which the inspection route becomes valid for scheduling.',
    `equipment_category` STRING COMMENT 'Primary class of equipment inspected along the route.',
    `estimated_duration_minutes` STRING COMMENT 'Typical time required to complete the full inspection route, expressed in minutes.',
    `frequency_days` STRING COMMENT 'Number of days between required executions of this inspection route.',
    `is_mandatory` BOOLEAN COMMENT 'Indicates whether execution of this route is mandatory for regulatory compliance (true) or optional (false).',
    `last_inspection_date` DATE COMMENT 'Date when the route was most recently completed.',
    `next_scheduled_date` DATE COMMENT 'Planned date for the next execution of the route.',
    `notes` STRING COMMENT 'Free‑form field for additional comments, observations, or special instructions.',
    `priority` STRING COMMENT 'Priority level used for planning and resource allocation.',
    `route_code` STRING COMMENT 'Unique alphanumeric code used to reference the inspection route in work orders and schedules.',
    `route_name` STRING COMMENT 'Human‑readable name describing the inspection route.',
    `route_type` STRING COMMENT 'Category of the route indicating its purpose (e.g., routine, critical, safety, regulatory).',
    `safety_level` STRING COMMENT 'Risk classification of the route based on potential hazards encountered.',
    `inspection_route_status` STRING COMMENT 'Current lifecycle status of the inspection route.',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time of the most recent modification to the inspection route record.',
    CONSTRAINT pk_inspection_route PRIMARY KEY(`inspection_route_id`)
) COMMENT 'Master reference table for inspection_route. Referenced by route_id.';

CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`checklist_template` (
    `checklist_template_id` BIGINT COMMENT 'Primary key for checklist_template',
    `employee_id` BIGINT COMMENT 'Identifier of the user who owns the checklist template.',
    `superseded_checklist_template_id` BIGINT COMMENT 'Self-referencing FK on checklist_template (superseded_checklist_template_id)',
    `applicable_equipment_type` STRING COMMENT 'Type of equipment to which the checklist template applies.',
    `approval_status` STRING COMMENT 'Current approval state of the checklist template.',
    `approved_by` STRING COMMENT 'User identifier of the person who approved the checklist template.',
    `approved_timestamp` TIMESTAMP COMMENT 'Date and time when the checklist template was approved for use.',
    `attachment_count` STRING COMMENT 'Number of supporting documents attached to the checklist template.',
    `checklist_template_category` STRING COMMENT 'Category that classifies the checklist (e.g., safety, operational, inspection).',
    `checklist_template_code` STRING COMMENT 'Unique business code used to reference the checklist template in maintenance processes.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when the checklist template record was first created.',
    `deprecation_date` DATE COMMENT 'Date on which the checklist template was deprecated.',
    `checklist_template_description` STRING COMMENT 'Detailed description of the purpose and scope of the checklist template.',
    `document_url` STRING COMMENT 'Web address or file path to the electronic version of the checklist.',
    `effective_end_date` DATE COMMENT 'Date after which the checklist template is no longer valid (nullable).',
    `effective_start_date` DATE COMMENT 'Date from which the checklist template becomes valid.',
    `frequency_interval` STRING COMMENT 'Numeric interval associated with the frequency type (e.g., every 3 days).',
    `frequency_type` STRING COMMENT 'Recurrence pattern for executing the checklist.',
    `is_electronic` BOOLEAN COMMENT 'True if the checklist template is stored/evaluated electronically.',
    `is_mandatory` BOOLEAN COMMENT 'True if the checklist must be performed for the associated equipment.',
    `is_regulatory_required` BOOLEAN COMMENT 'True if the checklist is mandated by regulatory standards (e.g., OSHA, EPA).',
    `is_safety_critical` BOOLEAN COMMENT 'Indicates whether the checklist addresses safety‑critical activities.',
    `last_review_date` DATE COMMENT 'Date when the checklist template was last reviewed.',
    `checklist_template_name` STRING COMMENT 'Human‑readable name of the checklist template.',
    `notes` STRING COMMENT 'Free‑form notes or comments about the checklist template.',
    `owner_department` STRING COMMENT 'Organizational department responsible for the checklist template.',
    `required_by_role` STRING COMMENT 'Job role responsible for completing the checklist.',
    `review_interval_days` STRING COMMENT 'Number of days between mandatory reviews of the checklist template.',
    `checklist_template_status` STRING COMMENT 'Current lifecycle state of the checklist template.',
    `updated_by` STRING COMMENT 'User identifier who performed the latest update.',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time of the most recent update to the checklist template.',
    `version_number` STRING COMMENT 'Version identifier for the checklist template (e.g., v1.0, v2.1).',
    `created_by` STRING COMMENT 'User identifier who created the checklist template record.',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` (
    `planner_group_id` BIGINT COMMENT 'Primary key for planner_group',
    `functional_location_id` BIGINT COMMENT 'Identifier of the main plant or facility where the planner group operates.',
    `parent_planner_group_id` BIGINT COMMENT 'Self-referencing FK on planner_group (parent_planner_group_id)',
    `average_experience_years` DECIMAL(18,2) COMMENT 'Mean years of experience across all members of the planner group.',
    `planner_group_code` STRING COMMENT 'Unique business code assigned to the planner group for external reference.',
    `cost_center_code` STRING COMMENT 'Internal cost‑center code used for budgeting and charge‑back of the planner groups activities.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the planner group record was initially created in the system.',
    `planner_group_description` STRING COMMENT 'Detailed description of the planner groups purpose and scope.',
    `effective_from` DATE COMMENT 'Calendar date when the planner group starts being valid for planning activities.',
    `effective_until` DATE COMMENT 'Calendar date when the planner group ceases to be valid; null if open‑ended.',
    `is_shift_based` BOOLEAN COMMENT 'True if the planner group operates on a shift schedule; otherwise false.',
    `member_count` STRING COMMENT 'Total count of individual planners assigned to the group.',
    `planner_group_name` STRING COMMENT 'Descriptive name of the planner group used in maintenance planning.',
    `shift_pattern` STRING COMMENT 'Pattern of work shifts assigned to the planner group.',
    `planner_group_status` STRING COMMENT 'Current operational status of the planner group.',
    `planner_group_type` STRING COMMENT 'Classification of the planner group based on its functional focus.',
    `updated_by` STRING COMMENT 'User identifier of the person who performed the most recent update.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent modification to the planner group record.',
    `created_by` STRING COMMENT 'User identifier of the person who initially created the planner group record.',
    CONSTRAINT pk_planner_group PRIMARY KEY(`planner_group_id`)
) COMMENT 'Master reference table for planner_group. Referenced by planner_group_id.';

CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` (
    `rbi_assessment_id` BIGINT COMMENT 'Primary key for rbi_assessment',
    `employee_id` BIGINT COMMENT 'Internal employee identifier for the assessor.',
    `equipment_id` BIGINT COMMENT 'Identifier of the equipment or asset that the assessment evaluates.',
    `prior_rbi_assessment_id` BIGINT COMMENT 'Self-referencing FK on rbi_assessment (prior_rbi_assessment_id)',
    `approved_timestamp` TIMESTAMP COMMENT 'Timestamp when the assessment was formally approved.',
    `assessment_code` STRING COMMENT 'Unique business code assigned to the assessment for tracking across systems.',
    `assessment_date` DATE COMMENT 'Calendar date on which the RBI assessment was performed.',
    `assessment_method` STRING COMMENT 'Method or standard used to conduct the RBI assessment.',
    `assessment_name` STRING COMMENT 'Human‑readable name or title of the RBI assessment.',
    `assessment_type` STRING COMMENT 'Category of the assessment indicating its purpose or trigger.',
    `assessor_name` STRING COMMENT 'Full name of the engineer or specialist who performed the RBI assessment.',
    `comments` STRING COMMENT 'Free‑form notes or observations recorded by the assessor.',
    `confidentiality_level` STRING COMMENT 'Classification of the assessment record for data handling purposes.',
    `consequence_score` DECIMAL(18,2) COMMENT 'Numeric score representing the potential impact of a failure scenario.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the assessment record was first created in the system.',
    `is_critical_equipment` BOOLEAN COMMENT 'True if the assessed equipment is classified as critical to plant safety or production.',
    `likelihood_score` DECIMAL(18,2) COMMENT 'Numeric score representing the probability of the failure scenario occurring.',
    `next_review_date` DATE COMMENT 'Planned date for the next RBI reassessment of the same equipment.',
    `plant_location` STRING COMMENT 'Code or name of the plant/facility where the equipment resides.',
    `regulatory_reference` STRING COMMENT 'Regulatory or industry standard that drives the assessment requirements.',
    `risk_category` STRING COMMENT 'Risk classification based on the risk score.',
    `risk_score` DECIMAL(18,2) COMMENT 'Composite numeric risk score derived from consequence and likelihood, expressed on a 0‑100 scale.',
    `rbi_assessment_status` STRING COMMENT 'Current lifecycle state of the assessment.',
    `supporting_document_count` STRING COMMENT 'Number of attached supporting documents (e.g., inspection reports, photos).',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent modification to the assessment record.',
    `version_number` STRING COMMENT 'Sequential version of the assessment document.',
    CONSTRAINT pk_rbi_assessment PRIMARY KEY(`rbi_assessment_id`)
) COMMENT 'Master reference table for rbi_assessment. Referenced by rbi_assessment_id.';
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` ADD CONSTRAINT `fk_maintenance_equipment_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`functional_location` ADD CONSTRAINT `fk_maintenance_functional_location_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ADD CONSTRAINT `fk_maintenance_work_order_planner_group_id` FOREIGN KEY (`planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order_operation` ADD CONSTRAINT `fk_maintenance_work_order_operation_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ADD CONSTRAINT `fk_maintenance_notification_catalog_code_id` FOREIGN KEY (`catalog_code_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`catalog_code`(`catalog_code_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ADD CONSTRAINT `fk_maintenance_notification_notification_catalog_code_id` FOREIGN KEY (`notification_catalog_code_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`catalog_code`(`catalog_code_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ADD CONSTRAINT `fk_maintenance_notification_notification_cause_code_maintenance_catalog_catalog_code_id` FOREIGN KEY (`notification_cause_code_maintenance_catalog_catalog_code_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`catalog_code`(`catalog_code_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ADD CONSTRAINT `fk_maintenance_notification_planner_group_id` FOREIGN KEY (`planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ADD CONSTRAINT `fk_maintenance_notification_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ADD CONSTRAINT `fk_maintenance_pm_plan_planner_group_id` FOREIGN KEY (`planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ADD CONSTRAINT `fk_maintenance_pm_plan_rbi_assessment_id` FOREIGN KEY (`rbi_assessment_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`rbi_assessment`(`rbi_assessment_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`schedule_call` ADD CONSTRAINT `fk_maintenance_schedule_call_planner_group_id` FOREIGN KEY (`planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`schedule_call` ADD CONSTRAINT `fk_maintenance_schedule_call_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_point` ADD CONSTRAINT `fk_maintenance_measurement_point_planner_group_id` FOREIGN KEY (`planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ADD CONSTRAINT `fk_maintenance_calibration_record_calibration_standard_id` FOREIGN KEY (`calibration_standard_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`calibration_standard`(`calibration_standard_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ADD CONSTRAINT `fk_maintenance_calibration_record_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ADD CONSTRAINT `fk_maintenance_asset_change_request_equipment_id` FOREIGN KEY (`equipment_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`equipment`(`equipment_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ADD CONSTRAINT `fk_maintenance_asset_change_request_functional_location_id` FOREIGN KEY (`functional_location_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`functional_location`(`functional_location_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ADD CONSTRAINT `fk_maintenance_asset_change_request_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ADD CONSTRAINT `fk_maintenance_breakdown_event_planner_group_id` FOREIGN KEY (`planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ADD CONSTRAINT `fk_maintenance_breakdown_event_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ADD CONSTRAINT `fk_maintenance_inspection_round_checklist_template_id` FOREIGN KEY (`checklist_template_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`checklist_template`(`checklist_template_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ADD CONSTRAINT `fk_maintenance_inspection_round_equipment_id` FOREIGN KEY (`equipment_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`equipment`(`equipment_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ADD CONSTRAINT `fk_maintenance_inspection_round_work_order_id` FOREIGN KEY (`work_order_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`work_order`(`work_order_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ADD CONSTRAINT `fk_maintenance_inspection_round_functional_location_id` FOREIGN KEY (`functional_location_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`functional_location`(`functional_location_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ADD CONSTRAINT `fk_maintenance_inspection_round_inspection_route_id` FOREIGN KEY (`inspection_route_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`inspection_route`(`inspection_route_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ADD CONSTRAINT `fk_maintenance_permit_to_work_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`plant`(`plant_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` ADD CONSTRAINT `fk_maintenance_equipment_allocation_equipment_id` FOREIGN KEY (`equipment_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`equipment`(`equipment_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_standard` ADD CONSTRAINT `fk_maintenance_calibration_standard_reference_calibration_standard_id` FOREIGN KEY (`reference_calibration_standard_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`calibration_standard`(`calibration_standard_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_route` ADD CONSTRAINT `fk_maintenance_inspection_route_superseded_inspection_route_id` FOREIGN KEY (`superseded_inspection_route_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`inspection_route`(`inspection_route_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`checklist_template` ADD CONSTRAINT `fk_maintenance_checklist_template_superseded_checklist_template_id` FOREIGN KEY (`superseded_checklist_template_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`checklist_template`(`checklist_template_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` ADD CONSTRAINT `fk_maintenance_planner_group_functional_location_id` FOREIGN KEY (`functional_location_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`functional_location`(`functional_location_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` ADD CONSTRAINT `fk_maintenance_planner_group_parent_planner_group_id` FOREIGN KEY (`parent_planner_group_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`planner_group`(`planner_group_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` ADD CONSTRAINT `fk_maintenance_rbi_assessment_equipment_id` FOREIGN KEY (`equipment_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`equipment`(`equipment_id`);
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` ADD CONSTRAINT `fk_maintenance_rbi_assessment_prior_rbi_assessment_id` FOREIGN KEY (`prior_rbi_assessment_id`) REFERENCES `chemical_mfg_ecm`.`maintenance`.`rbi_assessment`(`rbi_assessment_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Rep Employee Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`functional_location` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`functional_location` ALTER COLUMN `parent_floc_code` SET TAGS ('dbx_business_glossary_term' = 'Parent Functional Location Code');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Employee Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `freight_order_id` SET TAGS ('dbx_business_glossary_term' = 'Freight Order Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Logistics Shipment Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `quote_id` SET TAGS ('dbx_business_glossary_term' = 'Quote Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order` ALTER COLUMN `notification_id` SET TAGS ('dbx_business_glossary_term' = 'Maintenance Notification ID');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order_operation` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`work_order_operation` ALTER COLUMN `plant_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reported By User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `catalog_code_id` SET TAGS ('dbx_business_glossary_term' = 'Activity Code ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `notification_catalog_code_id` SET TAGS ('dbx_business_glossary_term' = 'Damage Code ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `notification_cause_code_maintenance_catalog_catalog_code_id` SET TAGS ('dbx_business_glossary_term' = 'Cause Code ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`notification` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` SET TAGS ('dbx_subdomain' = 'maintenance_planning');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ALTER COLUMN `maintenance_strategy_id` SET TAGS ('dbx_business_glossary_term' = 'Maintenance Strategy ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ALTER COLUMN `rbi_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Risk-Based Inspection (RBI) Assessment ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`pm_plan` ALTER COLUMN `strategy_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Strategy Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`schedule_call` SET TAGS ('dbx_subdomain' = 'maintenance_planning');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`schedule_call` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`schedule_call` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_point` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_point` ALTER COLUMN `market_price_index_id` SET TAGS ('dbx_business_glossary_term' = 'Market Price Index Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_point` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_point` ALTER COLUMN `plant_code` SET TAGS ('dbx_business_glossary_term' = 'Plant Code');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_reading` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_reading` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reader ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_reading` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`measurement_reading` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `calibration_standard_id` SET TAGS ('dbx_business_glossary_term' = 'Calibration Standard Identifier (ID)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Technician Identifier (ID)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Identifier (ID)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_record` ALTER COLUMN `reviewer_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` SET TAGS ('dbx_subdomain' = 'asset_registry');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `asset_change_request_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Change Request ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `equipment_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `functional_location_id` SET TAGS ('dbx_business_glossary_term' = 'Functional Location ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Initiator Employee ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `tertiary_asset_closed_by_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Closed By Employee ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `tertiary_asset_closed_by_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `tertiary_asset_closed_by_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `actual_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Completion Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `actual_start_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Start Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `affected_equipment_list` SET TAGS ('dbx_business_glossary_term' = 'Affected Equipment List');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `affected_pid_list` SET TAGS ('dbx_business_glossary_term' = 'Affected Piping and Instrumentation Diagram (P&ID) List');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `approval_required_roles` SET TAGS ('dbx_business_glossary_term' = 'Approval Required Roles');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'pending|partial|approved|rejected');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `budget_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `capex_opex_classification` SET TAGS ('dbx_business_glossary_term' = 'Capital Expenditure (CapEx) / Operating Expenditure (OpEx) Classification');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `capex_opex_classification` SET TAGS ('dbx_value_regex' = 'capex|opex|mixed');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `change_description` SET TAGS ('dbx_business_glossary_term' = 'Change Description');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `change_justification` SET TAGS ('dbx_business_glossary_term' = 'Change Justification');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `change_title` SET TAGS ('dbx_business_glossary_term' = 'Change Title');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `change_type` SET TAGS ('dbx_business_glossary_term' = 'Change Type');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `change_type` SET TAGS ('dbx_value_regex' = 'equipment|piping|instrumentation|chemical|procedure|operating_condition');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `closure_date` SET TAGS ('dbx_business_glossary_term' = 'Closure Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `environmental_impact_description` SET TAGS ('dbx_business_glossary_term' = 'Environmental Impact Description');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Cost');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `final_approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Final Approval Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `implementation_plan` SET TAGS ('dbx_business_glossary_term' = 'Implementation Plan');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `initiator_department` SET TAGS ('dbx_business_glossary_term' = 'Initiator Department');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `initiator_name` SET TAGS ('dbx_business_glossary_term' = 'Initiator Name');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `moc_number` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Number');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `moc_number` SET TAGS ('dbx_value_regex' = '^MOC-[0-9]{6,10}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `moc_status` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Status');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `operational_impact_description` SET TAGS ('dbx_business_glossary_term' = 'Operational Impact Description');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `planned_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Completion Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `planned_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Start Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `post_implementation_review_date` SET TAGS ('dbx_business_glossary_term' = 'Post-Implementation Review Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `post_implementation_review_findings` SET TAGS ('dbx_business_glossary_term' = 'Post-Implementation Review Findings');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `post_implementation_review_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Post-Implementation Review Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `psm_covered_process_flag` SET TAGS ('dbx_business_glossary_term' = 'Process Safety Management (PSM) Covered Process Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `pssr_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Pre-Startup Safety Review (PSSR) Completion Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `pssr_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Pre-Startup Safety Review (PSSR) Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `rejection_reason` SET TAGS ('dbx_business_glossary_term' = 'Rejection Reason');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `risk_assessment_method` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Method');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `risk_assessment_method` SET TAGS ('dbx_value_regex' = 'pha|hazop|what_if|fmea|lopa|not_required');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `risk_assessment_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `risk_level` SET TAGS ('dbx_value_regex' = 'high|medium|low|negligible');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `safety_impact_description` SET TAGS ('dbx_business_glossary_term' = 'Safety Impact Description');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `sop_update_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Standard Operating Procedure (SOP) Update Completion Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `sop_update_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Standard Operating Procedure (SOP) Update Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `training_completion_status` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Status');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `training_completion_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|completed|not_required');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`asset_change_request` ALTER COLUMN `training_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reported By User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`breakdown_event` ALTER COLUMN `affected_production_unit` SET TAGS ('dbx_business_glossary_term' = 'Affected Production Unit');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspection_round_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Round ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `checklist_template_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Checklist Template ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `equipment_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Work Order ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `functional_location_id` SET TAGS ('dbx_business_glossary_term' = 'Functional Location ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Inspector User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspection_route_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Route ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `work_center_id` SET TAGS ('dbx_business_glossary_term' = 'Work Center ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `actual_completion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Actual Inspection Completion Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `actual_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Actual Inspection Start Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `approval_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Approval Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Approval Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Inspection Cancellation Reason');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `compliance_due_date` SET TAGS ('dbx_business_glossary_term' = 'Compliance Due Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `deviations_found_count` SET TAGS ('dbx_business_glossary_term' = 'Deviations Found Count');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `highest_severity_level` SET TAGS ('dbx_business_glossary_term' = 'Highest Severity Level');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `highest_severity_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low|none');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspection_findings_summary` SET TAGS ('dbx_business_glossary_term' = 'Inspection Findings Summary');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspection_interval_days` SET TAGS ('dbx_business_glossary_term' = 'Inspection Interval Days');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspection_status` SET TAGS ('dbx_business_glossary_term' = 'Inspection Round Status');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspection_status` SET TAGS ('dbx_value_regex' = 'scheduled|in_progress|completed|cancelled|on_hold');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspector_notes` SET TAGS ('dbx_business_glossary_term' = 'Inspector Notes');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspector_qualification_code` SET TAGS ('dbx_business_glossary_term' = 'Inspector Qualification Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `inspector_qualification_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `insurance_carrier_code` SET TAGS ('dbx_business_glossary_term' = 'Insurance Carrier Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `insurance_carrier_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `insurance_inspection_flag` SET TAGS ('dbx_business_glossary_term' = 'Insurance Carrier Inspection Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `moc_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Reference Number');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `moc_reference_number` SET TAGS ('dbx_value_regex' = '^MOC-[A-Z0-9]{6,15}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `moc_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `next_scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Next Scheduled Inspection Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `plant_code` SET TAGS ('dbx_business_glossary_term' = 'Plant Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `plant_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{4}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `psm_mechanical_integrity_flag` SET TAGS ('dbx_business_glossary_term' = 'Process Safety Management (PSM) Mechanical Integrity Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `regulatory_requirement_code` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Requirement Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `regulatory_requirement_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,20}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `round_number` SET TAGS ('dbx_business_glossary_term' = 'Inspection Round Number');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `round_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,20}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `round_type` SET TAGS ('dbx_business_glossary_term' = 'Inspection Round Type');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `round_type` SET TAGS ('dbx_value_regex' = 'operator_round|predictive_inspection|statutory_inspection|regulatory_inspection|psm_mechanical_integrity|safety_device_inspection');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `safety_critical_flag` SET TAGS ('dbx_business_glossary_term' = 'Safety Critical Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Inspection Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `total_checkpoints_completed` SET TAGS ('dbx_business_glossary_term' = 'Total Checkpoints Completed');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `total_checkpoints_planned` SET TAGS ('dbx_business_glossary_term' = 'Total Checkpoints Planned');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_round` ALTER COLUMN `work_order_generated_flag` SET TAGS ('dbx_business_glossary_term' = 'Work Order Generated Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Attendant User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `alt5_permit_closed_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Closed By User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `alt5_permit_closed_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `alt5_permit_closed_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `primary_permit_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Issuing Authority User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `primary_permit_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `primary_permit_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `quaternary_permit_safety_officer_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Safety Officer User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `quaternary_permit_safety_officer_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `quaternary_permit_safety_officer_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `quinary_permit_fire_watch_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Fire Watch User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `quinary_permit_fire_watch_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `quinary_permit_fire_watch_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `tertiary_permit_area_authority_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Area Authority User ID');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `tertiary_permit_area_authority_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`permit_to_work` ALTER COLUMN `tertiary_permit_area_authority_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` SET TAGS ('dbx_subdomain' = 'maintenance_planning');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `catalog_code_id` SET TAGS ('dbx_business_glossary_term' = 'Catalog Code Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `api_689_reference` SET TAGS ('dbx_business_glossary_term' = 'API 689 Reference Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `capa_trigger_flag` SET TAGS ('dbx_business_glossary_term' = 'Corrective and Preventive Action (CAPA) Trigger Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `catalog_code_code` SET TAGS ('dbx_business_glossary_term' = 'Catalog Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `catalog_code_status` SET TAGS ('dbx_business_glossary_term' = 'Catalog Code Status');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `catalog_code_status` SET TAGS ('dbx_value_regex' = 'active|inactive|deprecated|pending_approval');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `catalog_type` SET TAGS ('dbx_business_glossary_term' = 'Catalog Type');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `catalog_type` SET TAGS ('dbx_value_regex' = 'damage|cause|activity|object_part|symptom');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `code_group` SET TAGS ('dbx_business_glossary_term' = 'Code Group');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `code_long_text` SET TAGS ('dbx_business_glossary_term' = 'Code Long Text');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `code_short_text` SET TAGS ('dbx_business_glossary_term' = 'Code Short Text');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `ehs_reportable_flag` SET TAGS ('dbx_business_glossary_term' = 'Environment Health and Safety (EHS) Reportable Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `equipment_class` SET TAGS ('dbx_business_glossary_term' = 'Equipment Class');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `estimated_downtime_hours` SET TAGS ('dbx_business_glossary_term' = 'Estimated Downtime Hours');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `estimated_repair_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Repair Cost');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `hierarchy_level` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Level');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `iso_14224_reference` SET TAGS ('dbx_business_glossary_term' = 'ISO 14224 Reference Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `language_code` SET TAGS ('dbx_business_glossary_term' = 'Language Code');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `language_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `last_used_date` SET TAGS ('dbx_business_glossary_term' = 'Last Used Date');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `moc_trigger_flag` SET TAGS ('dbx_business_glossary_term' = 'Management of Change (MOC) Trigger Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `modified_by` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `parent_code_group` SET TAGS ('dbx_business_glossary_term' = 'Parent Code Group');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `plant_section` SET TAGS ('dbx_business_glossary_term' = 'Plant Section');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `priority_indicator` SET TAGS ('dbx_business_glossary_term' = 'Priority Indicator');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `priority_indicator` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `psm_critical_flag` SET TAGS ('dbx_business_glossary_term' = 'Process Safety Management (PSM) Critical Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `rci_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Investigation (RCI) Required Flag');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `sort_sequence` SET TAGS ('dbx_business_glossary_term' = 'Sort Sequence');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `usage_count` SET TAGS ('dbx_business_glossary_term' = 'Usage Count');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`catalog_code` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By User');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`task_list` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`task_list` SET TAGS ('dbx_subdomain' = 'maintenance_planning');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`task_list` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Employee Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`task_list` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`task_list` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` SET TAGS ('dbx_subdomain' = 'asset_registry');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` SET TAGS ('dbx_association_edges' = 'logistics.shipment,maintenance.equipment');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` ALTER COLUMN `equipment_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment Allocation - Equipment Allocation Id');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` ALTER COLUMN `equipment_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment Allocation - Equipment Id');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` ALTER COLUMN `shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Equipment Allocation - Logistics Shipment Id');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` ALTER COLUMN `load_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Load Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`equipment_allocation` ALTER COLUMN `unload_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Unload Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_procedure` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_procedure` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_standard` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_standard` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_standard` ALTER COLUMN `calibration_standard_id` SET TAGS ('dbx_business_glossary_term' = 'Calibration Standard Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`calibration_standard` ALTER COLUMN `reference_calibration_standard_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` SET TAGS ('dbx_subdomain' = 'asset_registry');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `emergency_contact_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `emergency_contact_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `emergency_contact_phone` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `emergency_contact_phone` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `manager_email` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `manager_email` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `manager_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `manager_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `manager_phone` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`plant` ALTER COLUMN `manager_phone` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_route` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_route` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_route` ALTER COLUMN `inspection_route_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Route Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`inspection_route` ALTER COLUMN `superseded_inspection_route_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`checklist_template` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`checklist_template` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`checklist_template` ALTER COLUMN `checklist_template_id` SET TAGS ('dbx_business_glossary_term' = 'Checklist Template Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`checklist_template` ALTER COLUMN `superseded_checklist_template_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` SET TAGS ('dbx_subdomain' = 'maintenance_planning');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` ALTER COLUMN `planner_group_id` SET TAGS ('dbx_business_glossary_term' = 'Planner Group Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`planner_group` ALTER COLUMN `parent_planner_group_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` SET TAGS ('dbx_subdomain' = 'maintenance_planning');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` ALTER COLUMN `rbi_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Rbi Assessment Identifier');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` ALTER COLUMN `prior_rbi_assessment_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`maintenance`.`rbi_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_pii_name' = 'true');
