-- Schema for Domain: product | Business: Chemical Mfg | Version: v1_mvm
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`chemical_product` (
    ...
    `facility_id` BIGINT COMMENT 'Foreign key linking to ehs.facility. Business justification: Chemical products are manufactured at specific facilities. Facility-level TRI, RMP, and EPCRA regulatory reporting requires knowing which facility produces each chemical product. A chemical manufactur',
    ...
    `hazmat_storage_rule_id` BIGINT COMMENT 'Foreign key linking to inventory.hazmat_storage_rule. Business justification: Each chemical products GHS hazard class and packing group maps to a specific hazmat storage rule governing segregation, temperature limits, and ventilation requirements. This link enables automated s',
    `hierarchy_id` BIGINT COMMENT 'Foreign key linking to product.hierarchy. Business justification: chemical_product has denormalized `product_line` and `business_unit` STRING fields. The hierarchy table is the authoritative multi-level product classification for commercial reporting, portfolio mana',
    ...
    `maintenance_plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Chemical products are manufactured/stored at specific plants. Plant assignment drives site-specific regulatory compliance (EPA/OSHA permits), emergency response planning, and production capacity repor',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`sku` (
    ...
    `grade_id` BIGINT COMMENT 'Foreign key linking to product.grade. Business justification: sku has a denormalized `grade` STRING field. The grade table is the authoritative record for quality grades (Technical Grade, USP, ACS Reagent, etc.). A SKU is defined in part by its grade — e.g., a 2',
    `maintenance_plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: SKUs are produced and packaged at specific plants. Plant-level SKU availability drives ATP (available-to-promise) calculations, shipping point determination, and hazmat compliance documentation. Chemi',
    `packaging_config_id` BIGINT COMMENT 'Foreign key linking to product.packaging_config. Business justification: A SKU is fundamentally defined by its packaging configuration — a 200L drum vs. a 1000L IBC vs. a 25kg bag are different SKUs. packaging_config is the authoritative master for approved packaging forma',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`product_specification` (
    ...
    `grade_id` BIGINT COMMENT 'Foreign key linking to product.grade. Business justification: product_specification defines technical specifications for a chemical product, grade, or SKU (per its description). It already has chemical_product_id. Adding nullable grade_id enables grade-specific ',
    `sds_id` BIGINT COMMENT 'Foreign key linking to product.sds. Business justification: product_specification has an `sds_section_reference` STRING field that references a specific section within an SDS document. Adding sds_id FK normalizes the reference to the SDS master record, while k',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: product_specifications description explicitly states it covers a chemical product, grade, or SKU. Adding nullable sku_id enables SKU-level specifications — customer-specific specs (customer_specifi',
    ...
    `tds_id` BIGINT COMMENT 'Foreign key linking to product.tds. Business justification: product_specification defines technical parameters with test methods and acceptance criteria. These specifications are often derived from or aligned with the Technical Data Sheet (TDS). Adding nullabl',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`tds` (
    ...
    `sds_id` BIGINT COMMENT 'Foreign key linking to product.sds. Business justification: tds (Technical Data Sheet) has a denormalized `sds_document_number` STRING field that references the associated SDS document. The sds table is the authoritative SDS master record. TDS and SDS are comp',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`sds` (
    ...
    `sds_record_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.sds_record. Business justification: Product SDS authoring in chemical manufacturing requires reference to raw material SDS records for hazard data (toxicology, ecological info, physical-chemical properties). SDS authors explicitly cross',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`product_ghs_classification` (
    ...
    `rawmaterial_ghs_classification_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.rawmaterial_ghs_classification. Business justification: GHS/CLP mixture classification rules require product hazard classifications to be derived from and traceable to component raw material GHS classifications. EHS and regulatory teams use this link for S',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`regulatory_status` (
    ...
    `material_regulatory_status_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.material_regulatory_status. Business justification: Product regulatory compliance status (TSCA, REACH, jurisdiction) is directly informed by constituent raw material regulatory status. Regulatory affairs teams link product compliance records to raw mat',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`packaging_config` (
    ...
    `equipment_id` BIGINT COMMENT 'Foreign key linking to maintenance.equipment. Business justification: Packaging configurations (drum fill, IBC, bulk tanker) are tied to specific filling/packaging equipment lines. In chemical manufacturing, equipment constraints determine which packaging configs can ru',
    `maintenance_plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Packaging configurations are plant-specific — a 55-gallon drum fill line or bulk loading station exists only at certain plants. This drives order routing, production scheduling, and ensures hazmat-cer',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`composition` (
    ...
    `cas_registry_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.cas_registry. Business justification: Each product composition component must reference the authoritative CAS registry record for REACH/TSCA regulatory reporting, SDS authoring, and hazard classification. component_cas_number is a denorma',
    `lot_record_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.lot_record. Business justification: GMP batch traceability and recall management require linking each product composition record to the specific raw material lot consumed during manufacturing. Regulatory audits and recall investigations',
    ...
    `material_specification_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.material_specification. Business justification: Product formulation approval requires each raw material component to meet a specific material specification version. Change control and quality management processes track which specification was in ef',
    `supplier_material_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.supplier_material. Business justification: Approved Supplier Lists (ASL) in chemical manufacturing require product compositions to specify not just the raw material but the approved supplier source. Change control processes and supplier qualif',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`reach_dossier` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: REACH registration costs (dossier preparation, SIEF membership, testing) are charged to regulatory affairs cost centers in chemical mfg. Required for REACH compliance budget tracking and cost allocati',
    `internal_order_id` BIGINT COMMENT 'Foreign key linking to finance.internal_order. Business justification: REACH dossier preparation is managed as a discrete internal order project in chemical mfg, enabling budget control and actual cost settlement for each substance registration. Regulatory project cost t',
    `reach_registration_id` BIGINT COMMENT 'Foreign key linking to rawmaterial.reach_registration. Business justification: ECHA REACH compliance requires product dossiers to reference the upstream substance registration of constituent raw materials. Regulatory affairs teams link product REACH dossiers to raw material REAC',
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supply.vendor. Business justification: REACH regulatory compliance requires identifying which vendor is the lead registrant or SIEF co-registrant for each substance dossier. Chemical manufacturers must track registrant identity for ECHA su',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`order` (
    `order_id` BIGINT COMMENT 'System-generated unique identifier for the sales order record.',
    ...
    `customer_allocation_id` BIGINT COMMENT 'Foreign key linking to planning.customer_allocation. Business justification: During allocation periods in chemical manufacturing (supply constraints, force majeure, raw material shortages), product orders are placed against customer allocations. The order must reference the al',
    ...
    `maintenance_plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Product orders are fulfilled from specific plants. The existing plain column plant_code is a denormalized plant reference. Proper FK to maintenance.plant enables capacity planning, regulatory compli',
    ...
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to supply.purchase_order. Business justification: Back-to-back and make-to-order scenarios in chemical manufacturing require linking a customer sales order to the supply purchase order it triggered. Role-prefix supply_ distinguishes this from line_',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`line_item` (
    ...
    `contract_price_id` BIGINT COMMENT 'Foreign key linking to supply.contract_price. Business justification: Contract price compliance auditing and revenue recognition in chemical manufacturing require linking each order line item to the specific contract_price record governing its pricing. Auditors and comm',
    ...
    `cost_element_id` BIGINT COMMENT 'Foreign key linking to finance.cost_element. Business justification: Cost element classification on order line items is required in chemical mfg controlling to distinguish material costs, freight surcharges, and hazmat handling fees. Enables cost-element-level variance',
    `customer_allocation_id` BIGINT COMMENT 'Foreign key linking to planning.customer_allocation. Business justification: In chemical manufacturing during supply constraints, order line items are fulfilled against customer allocations. The line item must reference the allocation it consumes to track allocation utilizatio',
    ...
    `grade_id` BIGINT COMMENT 'Foreign key linking to product.grade. Business justification: line_item has a denormalized `grade` STRING field capturing the quality grade of the ordered product. The grade table is the authoritative record for quality grades and purity tiers. Adding grade_id F',
    `internal_order_id` BIGINT COMMENT 'Foreign key linking to finance.internal_order. Business justification: In chemical mfg, campaign or project-based sales line items are settled against internal orders for cost tracking and budget consumption monitoring. Required for internal order settlement reporting an',
    `lot_id` BIGINT COMMENT 'Foreign key linking to inventory.lot. Business justification: Chemical mfg sales order line items are fulfilled from specific lots for GMP batch traceability, CoA attachment (coa_required_flag already on line_item), and REACH/FDA regulatory compliance. Domain ex',
    `maintenance_plant_id` BIGINT COMMENT 'Foreign key linking to maintenance.plant. Business justification: Line items have a plain plant column (denormalized plant code). Each line item is fulfilled from a specific plant, driving inventory allocation, hazmat compliance documentation, and shipping point d',
    ...
    `order_id` BIGINT COMMENT 'Identifier of the parent sales order to which this line belongs.',
    ...
    `po_line_id` BIGINT COMMENT 'Foreign key linking to supply.po_line. Business justification: Back-to-back order processing in chemical manufacturing requires tracing each sales order line item to the specific purchase order line that fulfills it. This is essential for delivery commitment trac',
    `production_schedule_id` BIGINT COMMENT 'Foreign key linking to planning.production_schedule. Business justification: In chemical manufacturing, order line items are pegged to specific production schedule entries for ATP/CTP (available-to-promise/capable-to-promise) confirmation. This peg enables delivery date commit',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Product-line P&L reporting in chemical mfg requires each order line item to be attributed to a profit center (e.g., specialty chemicals vs. agrochemicals). Enables contribution margin analysis by prod',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: A line item represents a specific orderable SKU configuration. The sku table is the authoritative record for each orderable/shippable product configuration. line_item already has chemical_product_id f',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`order_confirmation` (
    ...
    `lot_id` BIGINT COMMENT 'Foreign key linking to inventory.lot. Business justification: Order confirmations in chemical manufacturing must reference the specific lot shipped to support CoA/CoC document generation (coa_attached, coc_attached already on order_confirmation) and customer-fac',
    `order_id` BIGINT COMMENT 'Identifier of the original sales order that this confirmation relates to.',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Confirmed orders in chemical mfg are attributed to profit centers for segment revenue reporting. Finance controllers require profit center on order confirmations to produce product-line revenue report',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` (
    ...
    `lot_id` BIGINT COMMENT 'Foreign key linking to inventory.lot. Business justification: Outbound deliveries in chemical manufacturing must reference the specific lot shipped for GMP batch traceability, CoA/CoC document generation, and REACH/FDA regulatory compliance. batch_numbers is a d',
    `order_confirmation_id` BIGINT COMMENT 'Foreign key linking to product.order_confirmation. Business justification: In the order fulfillment lifecycle, an outbound delivery is created to execute the physical shipment after an order confirmation is issued. Linking outbound_delivery to order_confirmation enables trac',
    `order_id` BIGINT COMMENT 'Identifier of the sales order that triggered this delivery.',
    `sds_id` BIGINT COMMENT 'Foreign key linking to product.sds. Business justification: Outbound deliveries of hazardous chemicals must include the current SDS for regulatory compliance (OSHA HazCom 2012, EU CLP). Chemical manufacturers attach SDS to delivery documents for customer recei',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`status_event` (
    ...
    `line_item_id` BIGINT COMMENT 'Foreign key linking to product.line_item. Business justification: status_event is an immutable event log for order fulfillment milestones. While it already references product_order_id for order-level events, many fulfillment events (partial shipment, line cancellati',
    `order_confirmation_id` BIGINT COMMENT 'Foreign key linking to product.order_confirmation. Business justification: status_event captures every status transition in the sales order lifecycle. Order confirmation events (confirmation issued, confirmation revised, deviation flagged) are key milestones that should refe',
    `order_id` BIGINT COMMENT 'Identifier of the sales order to which this status event belongs.',
    `outbound_delivery_id` BIGINT COMMENT 'Foreign key linking to product.outbound_delivery. Business justification: status_event tracks every fulfillment milestone. Delivery-level events (goods issued, in-transit, delivered, delivery exception) are key milestones that should reference the specific outbound_delivery',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to maintenance.work_order. Business justification: Order status events are triggered by maintenance work orders — PSM-covered equipment breakdowns cause order delays, and work order completion triggers order resumption. Chemical manufacturers track th',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`product_hold` (
    ...
    `account_id` BIGINT COMMENT 'Identifier of the customer who placed the order.',
    `line_item_id` BIGINT COMMENT 'Foreign key linking to product.line_item. Business justification: A product hold can be placed at the order header level (product_order_id already exists) or at the individual line item level for granular hold management. In chemical manufacturing, holds are often p',
    `lot_id` BIGINT COMMENT 'Foreign key linking to inventory.lot. Business justification: Product holds in chemical manufacturing are frequently triggered by lot-level quality failures, contamination events, or regulatory issues (FDA, REACH). Linking product_hold to the specific lot enable',
    `order_id` BIGINT COMMENT 'Identifier of the sales order to which this hold is applied.',
    `safety_incident_id` BIGINT COMMENT 'Foreign key linking to ehs.safety_incident. Business justification: Product holds in chemical manufacturing are frequently triggered by safety incidents (process safety events, contamination, exposure). Linking product_hold to safety_incident enables EHS-to-commercial',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to maintenance.work_order. Business justification: Product holds in chemical manufacturing are triggered by quality investigations that generate maintenance work orders (equipment contamination, off-spec production). The hold is released when the work',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`returns_order` (
    ...
    `case_id` BIGINT COMMENT 'Foreign key linking to customer.case. Business justification: In chemical manufacturing, a returns order is typically created as a resolution action for a customer quality or complaint case. Linking returns_order to case enables CAPA closure tracking, credit mem',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Returns in chemical mfg trigger credit memo postings to returns/allowances GL accounts. Direct FK required for automated reversal journal entries and returns accrual reporting. Auditors expect returns',
    `line_item_id` BIGINT COMMENT 'Foreign key linking to product.line_item. Business justification: A customer return (RMA) is typically initiated against a specific line item from the original sales order, not just the order header. returns_order already has product_order_id for the order header re',
    ...
    `order_id` BIGINT COMMENT 'System identifier of the original sales order linked to this return.',
    `outbound_delivery_id` BIGINT COMMENT 'Foreign key linking to product.outbound_delivery. Business justification: A customer return (RMA) is typically initiated against a specific outbound delivery — the customer received goods via a specific delivery and is returning them. returns_order already has product_order',
    `waste_disposal_record_id` BIGINT COMMENT 'Foreign key linking to ehs.waste_disposal_record. Business justification: Returned hazardous chemical products require RCRA-compliant waste disposal. The returns_order triggers a waste disposal workflow; linking to waste_disposal_record enables traceability from customer re',
...
CREATE OR REPLACE TABLE `chemical_mfg_ecm`.`product`.`delivery_line` (
    `delivery_line_id` BIGINT COMMENT 'System-generated unique identifier for this delivery line record.',
    `line_item_id` BIGINT COMMENT 'Foreign key linking to the sales order line item being fulfilled by this delivery.',
    `outbound_delivery_id` BIGINT COMMENT 'Foreign key linking to the outbound delivery document that physically ships this line item.',
    `actual_delivery_date` DATE COMMENT 'Date when this specific line item was actually delivered. In split deliveries, different line items on the same outbound delivery document may have different actual delivery dates due to multi-stop routes or partial unloading.',
    `batch_number` STRING COMMENT 'Specific production batch number assigned to this delivery line. In chemical manufacturing, batch traceability is regulatory-critical. A single order line may be fulfilled from multiple batches across multiple deliveries, making this a relationship-level attribute.',
    `coa_attached` BOOLEAN COMMENT 'Indicates whether a COA document is attached for this specific delivery line and batch combination. COA attachment is per-batch and per-delivery, not per-order-line or per-delivery globally.',
    `coc_attached` BOOLEAN COMMENT 'Indicates whether a COC document is attached for this specific delivery line and batch combination. COC attachment is per-batch and per-delivery, not per-order-line or per-delivery globally.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this delivery line record was created in the system, typically during delivery document creation or wave planning.',
    `delivered_quantity` DECIMAL(18,2) COMMENT 'Actual quantity of the line item shipped on this specific delivery. Critical for partial fulfillment tracking and cannot be stored on either parent table because one line item can have different quantities across multiple deliveries.',
    `delivery_line_status` STRING COMMENT 'Lifecycle status of this specific delivery line execution. Tracks the operational state of fulfilling this line item on this delivery, independent of the overall delivery status or order line status.',
    `gross_weight_kg` DECIMAL(18,2) COMMENT 'Gross weight including packaging for this delivery line in kilograms. Required for hazmat documentation and freight calculation at the line level.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent modification to this delivery line record, tracking changes during picking, packing, or shipment execution.',
    `net_weight_kg` DECIMAL(18,2) COMMENT 'Net weight of the delivered quantity for this line in kilograms. Required for hazmat documentation and freight calculation at the line level.',
    `number_of_packages` STRING COMMENT 'Count of physical packages (drums, bags, totes) for this delivery line. Required for hazmat documentation and warehouse handling.',
    `packing_type` STRING COMMENT 'Type of packaging used for this delivery line (drums, IBC totes, bulk tanker, bags). Packing decisions are made at delivery execution time and may vary for the same line item across different deliveries.',
    `temperature_deviation_flag` BOOLEAN COMMENT 'Indicates whether this specific delivery line experienced temperature excursions during transport. Critical for temperature-sensitive chemicals and belongs to the delivery execution of this specific line, not to the delivery or line item globally.',
    `unit_of_measure` STRING COMMENT 'Unit of measure for the delivered quantity on this delivery line (kg, L, MT, drums). May differ from the order line UOM due to conversion during fulfillment.',
    CONSTRAINT pk_delivery_line PRIMARY KEY(`delivery_line_id`)
) COMMENT 'This association product represents the physical execution of a sales order line item through an outbound delivery in chemical manufacturing. It captures the many-to-many relationship where a single outbound delivery can ship multiple order line items (consolidated shipment), and a single order line item can be split across multiple deliveries due to hazmat transport limits, tank capacity constraints, batch availability, or partial fulfillment. Each record links one outbound delivery to one line item with attributes that exist only in the context of this specific delivery execution: delivered quantity, batch numbers assigned, COA/COC attachment status per line, and temperature deviation tracking.. Existence Justification: In chemical manufacturing operations, the relationship between outbound deliveries and sales order line items is genuinely many-to-many. A single outbound delivery routinely consolidates multiple order line items from one or more sales orders into a single shipment (many line items to one delivery). Conversely, a single order line item is frequently split across multiple deliveries due to hazmat transport quantity limits, production batch availability, tank truck capacity constraints, or customer-requested partial fulfillment (one line item to many deliveries). This is the standard SAP SD/EWM delivery item concept.';

-- ========= FOREIGN KEYS =========
ALTER TABLE `chemical_mfg_ecm`.`product`.`chemical_product` ADD CONSTRAINT `fk_product_chemical_product_hierarchy_id` FOREIGN KEY (`hierarchy_id`) REFERENCES `chemical_mfg_ecm`.`product`.`hierarchy`(`hierarchy_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`sku` ADD CONSTRAINT `fk_product_sku_grade_id` FOREIGN KEY (`grade_id`) REFERENCES `chemical_mfg_ecm`.`product`.`grade`(`grade_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`sku` ADD CONSTRAINT `fk_product_sku_packaging_config_id` FOREIGN KEY (`packaging_config_id`) REFERENCES `chemical_mfg_ecm`.`product`.`packaging_config`(`packaging_config_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ADD CONSTRAINT `fk_product_product_specification_grade_id` FOREIGN KEY (`grade_id`) REFERENCES `chemical_mfg_ecm`.`product`.`grade`(`grade_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ADD CONSTRAINT `fk_product_product_specification_sds_id` FOREIGN KEY (`sds_id`) REFERENCES `chemical_mfg_ecm`.`product`.`sds`(`sds_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ADD CONSTRAINT `fk_product_product_specification_sku_id` FOREIGN KEY (`sku_id`) REFERENCES `chemical_mfg_ecm`.`product`.`sku`(`sku_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ADD CONSTRAINT `fk_product_product_specification_tds_id` FOREIGN KEY (`tds_id`) REFERENCES `chemical_mfg_ecm`.`product`.`tds`(`tds_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`tds` ADD CONSTRAINT `fk_product_tds_sds_id` FOREIGN KEY (`sds_id`) REFERENCES `chemical_mfg_ecm`.`product`.`sds`(`sds_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ADD CONSTRAINT `fk_product_order_sds_id` FOREIGN KEY (`sds_id`) REFERENCES `chemical_mfg_ecm`.`product`.`sds`(`sds_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ADD CONSTRAINT `fk_product_line_item_grade_id` FOREIGN KEY (`grade_id`) REFERENCES `chemical_mfg_ecm`.`product`.`grade`(`grade_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ADD CONSTRAINT `fk_product_line_item_order_id` FOREIGN KEY (`order_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order`(`order_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ADD CONSTRAINT `fk_product_line_item_sku_id` FOREIGN KEY (`sku_id`) REFERENCES `chemical_mfg_ecm`.`product`.`sku`(`sku_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`order_confirmation` ADD CONSTRAINT `fk_product_order_confirmation_order_id` FOREIGN KEY (`order_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order`(`order_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ADD CONSTRAINT `fk_product_outbound_delivery_order_confirmation_id` FOREIGN KEY (`order_confirmation_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order_confirmation`(`order_confirmation_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ADD CONSTRAINT `fk_product_outbound_delivery_order_id` FOREIGN KEY (`order_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order`(`order_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ADD CONSTRAINT `fk_product_outbound_delivery_sds_id` FOREIGN KEY (`sds_id`) REFERENCES `chemical_mfg_ecm`.`product`.`sds`(`sds_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ADD CONSTRAINT `fk_product_status_event_line_item_id` FOREIGN KEY (`line_item_id`) REFERENCES `chemical_mfg_ecm`.`product`.`line_item`(`line_item_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ADD CONSTRAINT `fk_product_status_event_order_confirmation_id` FOREIGN KEY (`order_confirmation_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order_confirmation`(`order_confirmation_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ADD CONSTRAINT `fk_product_status_event_order_id` FOREIGN KEY (`order_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order`(`order_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ADD CONSTRAINT `fk_product_status_event_outbound_delivery_id` FOREIGN KEY (`outbound_delivery_id`) REFERENCES `chemical_mfg_ecm`.`product`.`outbound_delivery`(`outbound_delivery_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ADD CONSTRAINT `fk_product_product_hold_line_item_id` FOREIGN KEY (`line_item_id`) REFERENCES `chemical_mfg_ecm`.`product`.`line_item`(`line_item_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ADD CONSTRAINT `fk_product_product_hold_order_id` FOREIGN KEY (`order_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order`(`order_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ADD CONSTRAINT `fk_product_returns_order_line_item_id` FOREIGN KEY (`line_item_id`) REFERENCES `chemical_mfg_ecm`.`product`.`line_item`(`line_item_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ADD CONSTRAINT `fk_product_returns_order_order_id` FOREIGN KEY (`order_id`) REFERENCES `chemical_mfg_ecm`.`product`.`order`(`order_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ADD CONSTRAINT `fk_product_returns_order_outbound_delivery_id` FOREIGN KEY (`outbound_delivery_id`) REFERENCES `chemical_mfg_ecm`.`product`.`outbound_delivery`(`outbound_delivery_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ADD CONSTRAINT `fk_product_delivery_line_line_item_id` FOREIGN KEY (`line_item_id`) REFERENCES `chemical_mfg_ecm`.`product`.`line_item`(`line_item_id`);
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ADD CONSTRAINT `fk_product_delivery_line_outbound_delivery_id` FOREIGN KEY (`outbound_delivery_id`) REFERENCES `chemical_mfg_ecm`.`product`.`outbound_delivery`(`outbound_delivery_id`);
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`chemical_product` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`chemical_product` ALTER COLUMN `facility_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`chemical_product` ALTER COLUMN `hazmat_storage_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Hazmat Storage Rule Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`chemical_product` ALTER COLUMN `hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`chemical_product` ALTER COLUMN `maintenance_plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`sku` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`sku` ALTER COLUMN `grade_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`sku` ALTER COLUMN `maintenance_plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`sku` ALTER COLUMN `packaging_config_id` SET TAGS ('dbx_business_glossary_term' = 'Packaging Config Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`grade` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ALTER COLUMN `grade_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ALTER COLUMN `sds_id` SET TAGS ('dbx_business_glossary_term' = 'Sds Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_specification` ALTER COLUMN `tds_id` SET TAGS ('dbx_business_glossary_term' = 'Tds Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`tds` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`tds` ALTER COLUMN `sds_id` SET TAGS ('dbx_business_glossary_term' = 'Sds Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`sds` ALTER COLUMN `sds_record_id` SET TAGS ('dbx_business_glossary_term' = 'Source Sds Record Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_ghs_classification` ALTER COLUMN `rawmaterial_ghs_classification_id` SET TAGS ('dbx_business_glossary_term' = 'Rawmaterial Ghs Classification Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`regulatory_status` ALTER COLUMN `material_regulatory_status_id` SET TAGS ('dbx_business_glossary_term' = 'Material Regulatory Status Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`hierarchy` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`packaging_config` SET TAGS ('dbx_subdomain' = 'portfolio_management');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`packaging_config` ALTER COLUMN `equipment_id` SET TAGS ('dbx_business_glossary_term' = 'Filling Equipment Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`packaging_config` ALTER COLUMN `maintenance_plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`composition` SET TAGS ('dbx_subdomain' = 'regulatory_compliance');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`composition` ALTER COLUMN `cas_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Component Cas Registry Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`composition` ALTER COLUMN `lot_record_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Record Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`composition` ALTER COLUMN `material_specification_id` SET TAGS ('dbx_business_glossary_term' = 'Material Specification Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`composition` ALTER COLUMN `supplier_material_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Material Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`reach_dossier` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`reach_dossier` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`reach_dossier` ALTER COLUMN `reach_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Reach Registration Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`reach_dossier` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Order Identifier');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Identifier (CUSTOMER_ID)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Identifier (CONTRACT_ID)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `customer_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Allocation Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `distributor_id` SET TAGS ('dbx_business_glossary_term' = 'Distributor Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `forecast_version_id` SET TAGS ('dbx_business_glossary_term' = 'Forecast Version Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `maintenance_plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `opportunity_id` SET TAGS ('dbx_business_glossary_term' = 'Opportunity Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `price_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Price Agreement Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `site_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Address Identifier (BILL_ADDR_ID)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `quote_id` SET TAGS ('dbx_business_glossary_term' = 'Quote Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Territory Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `sds_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Document Identifier (COMPL_DOC_ID)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `shipping_address_site_id` SET TAGS ('dbx_business_glossary_term' = 'Shipping Address Identifier (SHIP_ADDR_ID)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `shipping_address_site_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `shipping_address_site_id` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Purchase Order Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Created Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code (CURR_CODE)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `discount_amount` SET TAGS ('dbx_business_glossary_term' = 'Discount Amount (DISC_AMT)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `distribution_channel` SET TAGS ('dbx_business_glossary_term' = 'Distribution Channel (DIST_CHAN)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `division` SET TAGS ('dbx_business_glossary_term' = 'Division Code (DIVISION)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `exchange_rate` SET TAGS ('dbx_business_glossary_term' = 'Exchange Rate (EXCH_RATE)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `freight_terms` SET TAGS ('dbx_business_glossary_term' = 'Freight Terms (FREIGHT_TERMS)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `freight_terms` SET TAGS ('dbx_value_regex' = 'prepaid|collect|third_party');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `gross_amount` SET TAGS ('dbx_business_glossary_term' = 'Gross Order Amount (GROSS_AMT)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `hazardous_material_flag` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Material Flag (HAZ_MAT_FLAG)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `incoterms` SET TAGS ('dbx_business_glossary_term' = 'Incoterms (INCO_TERM)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `incoterms` SET TAGS ('dbx_value_regex' = 'EXW|FCA|FOB|CFR|CIF|DDP');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `is_backorder` SET TAGS ('dbx_business_glossary_term' = 'Backorder Indicator (BACKORDER_FLAG)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `is_split` SET TAGS ('dbx_business_glossary_term' = 'Order Split Indicator (SPLIT_FLAG)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `line_count` SET TAGS ('dbx_business_glossary_term' = 'Order Line Count (LINE_COUNT)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `net_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Order Amount (NET_AMT)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_number` SET TAGS ('dbx_business_glossary_term' = 'Sales Order Number (SO_NUMBER)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_source` SET TAGS ('dbx_business_glossary_term' = 'Order Source (ORDER_SOURCE)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_source` SET TAGS ('dbx_value_regex' = 'SAP|CRM|EDI');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_status` SET TAGS ('dbx_business_glossary_term' = 'Order Status (SO_STATUS)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_status` SET TAGS ('dbx_value_regex' = 'draft|open|confirmed|fulfilled|closed|cancelled');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Order Creation Timestamp (SO_CREATED_TS)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_type` SET TAGS ('dbx_business_glossary_term' = 'Order Type (SO_TYPE)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `order_type` SET TAGS ('dbx_value_regex' = 'standard|rush|blanket|consignment|return');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `payment_terms` SET TAGS ('dbx_business_glossary_term' = 'Payment Terms (PAY_TERM)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Order Priority (PRIORITY)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `requested_delivery_date` SET TAGS ('dbx_business_glossary_term' = 'Requested Delivery Date (REQ_DEL_DATE)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `sales_org` SET TAGS ('dbx_business_glossary_term' = 'Sales Organization Code (SALES_ORG)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `sales_region` SET TAGS ('dbx_business_glossary_term' = 'Sales Region (SALES_REGION)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `shipping_method` SET TAGS ('dbx_business_glossary_term' = 'Shipping Method (SHIP_METHOD)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `shipping_method` SET TAGS ('dbx_value_regex' = 'ground|air|sea|rail');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `special_handling_instructions` SET TAGS ('dbx_business_glossary_term' = 'Special Handling Instructions (SPECIAL_HANDLING)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `status_reason` SET TAGS ('dbx_business_glossary_term' = 'Order Status Reason (SO_STATUS_REASON)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount (TAX_AMT)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `total_volume_m3` SET TAGS ('dbx_business_glossary_term' = 'Total Order Volume (VOL_M3)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `total_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Total Order Weight (WT_KG)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Updated Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `contract_price_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Price Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `cost_element_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Element Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `customer_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Allocation Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `grade_id` SET TAGS ('dbx_business_glossary_term' = 'Grade Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `maintenance_plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Order Identifier');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `po_line_id` SET TAGS ('dbx_business_glossary_term' = 'Po Line Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `production_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Production Schedule Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`line_item` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`order_confirmation` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`order_confirmation` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order_confirmation` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `chemical_mfg_ecm`.`product`.`order_confirmation` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ALTER COLUMN `order_confirmation_id` SET TAGS ('dbx_business_glossary_term' = 'Order Confirmation Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Order ID');
ALTER TABLE `chemical_mfg_ecm`.`product`.`outbound_delivery` ALTER COLUMN `sds_id` SET TAGS ('dbx_business_glossary_term' = 'Sds Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ALTER COLUMN `line_item_id` SET TAGS ('dbx_business_glossary_term' = 'Line Item Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ALTER COLUMN `order_confirmation_id` SET TAGS ('dbx_business_glossary_term' = 'Order Confirmation Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Order Identifier');
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ALTER COLUMN `outbound_delivery_id` SET TAGS ('dbx_business_glossary_term' = 'Outbound Delivery Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`status_event` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ALTER COLUMN `line_item_id` SET TAGS ('dbx_business_glossary_term' = 'Line Item Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Order Identifier (SO_ID)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ALTER COLUMN `safety_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Safety Incident Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`product_hold` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ALTER COLUMN `case_id` SET TAGS ('dbx_business_glossary_term' = 'Case Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ALTER COLUMN `line_item_id` SET TAGS ('dbx_business_glossary_term' = 'Line Item Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Original Sales Order ID');
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ALTER COLUMN `outbound_delivery_id` SET TAGS ('dbx_business_glossary_term' = 'Outbound Delivery Id (Foreign Key)');
ALTER TABLE `chemical_mfg_ecm`.`product`.`returns_order` ALTER COLUMN `waste_disposal_record_id` SET TAGS ('dbx_business_glossary_term' = 'Waste Disposal Record Id (Foreign Key)');
...
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` SET TAGS ('dbx_subdomain' = 'sales_fulfillment');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` SET TAGS ('dbx_association_edges' = 'product.outbound_delivery,product.line_item');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `delivery_line_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Line Identifier');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `line_item_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Line - Line Item Id');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `outbound_delivery_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Line - Outbound Delivery Id');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `actual_delivery_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Delivery Date');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `batch_number` SET TAGS ('dbx_business_glossary_term' = 'Batch Number');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `coa_attached` SET TAGS ('dbx_business_glossary_term' = 'Certificate of Analysis Attached');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `coc_attached` SET TAGS ('dbx_business_glossary_term' = 'Certificate of Conformance Attached');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `delivered_quantity` SET TAGS ('dbx_business_glossary_term' = 'Delivered Quantity');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `delivery_line_status` SET TAGS ('dbx_business_glossary_term' = 'Delivery Line Status');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `gross_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Gross Weight in Kilograms');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `net_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Net Weight in Kilograms');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `number_of_packages` SET TAGS ('dbx_business_glossary_term' = 'Number of Packages');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `packing_type` SET TAGS ('dbx_business_glossary_term' = 'Packing Type');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `temperature_deviation_flag` SET TAGS ('dbx_business_glossary_term' = 'Temperature Deviation Flag');
ALTER TABLE `chemical_mfg_ecm`.`product`.`delivery_line` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure');
