-- Schema for Domain: loyalty | Business: Restaurants | Version: v1_mvm
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`member` (
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: member.current_tier is a denormalized STRING storing the tier name/code. Replacing it with a FK to the tier reference catalog (current_tier_id -> loyalty.tier.tier_id) normalizes this relationship and',
    ...
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Every loyalty member is enrolled in a specific loyalty program. The program governs the members points rules, tier structure, earning rates, and redemption thresholds. Adding program_id to member est',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`tier` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Tier structures (Gold/Silver/Bronze) are brand-specific in multi-brand operators. Brand managers configure and audit tier ladders per brand. Tier eligibility reporting and brand-level loyalty analytic',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Tier configuration change management requires tracking which employee modified tier thresholds, multipliers, or qualification criteria — essential for loyalty program governance and compliance audits.',
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Loyalty tiers (Bronze, Silver, Gold, Platinum) are defined within the context of a specific loyalty program. A tier belongs to one program. Adding program_id to tier establishes this parent-child rela',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`tier_history` (
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: tier_history records every tier change event and stores new_tier_code as a STRING. Replacing this with a FK (new_tier_tier_id -> loyalty.tier.tier_id) normalizes the reference to the tier catalog for ',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Manual tier override authorization requires tracking which manager authorized the override — critical for fraud prevention and compliance in loyalty operations. override_authorized_by is a denormalize',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`points_ledger` (
    ...
    `accrual_rule_id` BIGINT COMMENT 'Foreign key linking to loyalty.accrual_rule. Business justification: Each points accrual entry in the ledger is generated by a specific accrual rule (e.g., 2x points on weekends, bonus points for specific menu categories). Adding accrual_rule_id to points_ledger links ',
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Points ledger entries represent financial transactions (earn/burn) that must be allocated to a cost center for restaurant-level P&L and loyalty program cost reporting. Foodservice finance teams requir',
    `enrollment_event_id` BIGINT COMMENT 'Foreign key linking to loyalty.enrollment_event. Business justification: Enrollment events award initial_points_awarded to new members, which generates a points_ledger entry. Adding enrollment_event_id to points_ledger links the welcome/enrollment bonus ledger entry to the',
    `financial_period_id` BIGINT COMMENT 'Foreign key linking to finance.financial_period. Business justification: Points ledger entries drive deferred revenue balances that must be reported by financial period for period-end close. Foodservice finance teams require loyalty points deferred revenue period-end clos',
    ...
    `offer_redemption_id` BIGINT COMMENT 'Foreign key linking to loyalty.offer_redemption. Business justification: Offer redemptions can trigger points accrual events (e.g., bonus points for redeeming an offer). Adding offer_redemption_id to points_ledger links the ledger entry to the specific offer redemption tha',
    ...
    `redemption_id` BIGINT COMMENT 'Foreign key linking to loyalty.redemption. Business justification: When a member redeems a reward (burn event), a points_ledger entry is created recording the points deduction. Adding redemption_id to points_ledger links the ledger debit entry to the specific redempt',
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: Points accrual channel normalization: points_ledger.source_channel is a plain-text denormalized attribute. FK to order.channel normalizes channel taxonomy for channel-level points accrual reporting an',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`reward` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Rewards are brand-scoped in multi-brand restaurant groups. Brand-level reward catalog management, franchise royalty cost allocation for reward redemptions, and brand marketing reporting all require li',
    ...
    `stock_item_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_item. Business justification: Reward definitions for free-item rewards must reference the inventory stock_item to calculate COGS and theoretical food cost impact. Foodservice operators track reward fulfillment cost per stock item ',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Rewards carry cost_of_goods_sold and monetary_value attributes requiring a GL account mapping for reward cost accrual and promotional expense posting. Foodservice finance teams map each reward type to',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Reward catalog change control requires linking reward modifications to the responsible employee for accountability and audit. Foodservice loyalty teams track who changed reward point costs or availabi',
    ...
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Rewards in the loyalty catalog belong to a specific loyalty program. A rewards availability, point cost, and redemption rules are governed by the program it belongs to. Adding program_id to reward es',
    `promotion_id` BIGINT COMMENT 'Foreign key linking to marketing.promotion. Business justification: Loyalty rewards (e.g., free item) are often the fulfillment mechanism for a specific marketing promotion. Linking reward to promotion enables promotion cost reporting that includes loyalty reward COGS',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`redemption` (
    ...
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: Redemption channel normalization: redemption.channel is a plain-text denormalized attribute. Replacing with FK to order.channel normalizes the channel taxonomy, enabling channel-level redemption rate ',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Redemption discount amounts must be allocated to a cost center for restaurant-level P&L and promotional discount variance reporting. Foodservice finance teams track redemption discount cost center al',
    ...
    `financial_period_id` BIGINT COMMENT 'Foreign key linking to finance.financial_period. Business justification: Redemption discount expenses must be assigned to a financial period for revenue recognition and discount expense period-end reporting. Foodservice finance teams require redemption discount period-end',
    ...
    `promotion_id` BIGINT COMMENT 'Foreign key linking to marketing.promotion. Business justification: Loyalty reward redemptions co-occur with active marketing promotions (e.g., BOGO + points). Linking redemption to promotion enables transaction-level reconciliation of loyalty redemptions against prom',
    `stock_item_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_item. Business justification: When a free-item loyalty reward is redeemed, the specific stock_item consumed must be recorded for inventory depletion, COGS tracking, and food cost variance reporting. Foodservice operators require t',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Accrual rule governance requires tracking which manager approved each rule before activation — a standard compliance and audit requirement in loyalty operations. approved_by is a denormalized plain-te',
    `area_management_id` BIGINT COMMENT 'Foreign key linking to restaurant.area_management. Business justification: Accrual rules are scoped to geographic territories for test-market earn-rate pilots and regional loyalty promotions. accrual_rule.geographic_scope is a plain-text denormalization of the area_managemen',
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Accrual rules (earn rates) are configured per brand — QSR and casual dining brands within the same operator have different points-per-dollar rates. Brand-level loyalty P&L and earn-rate governance aud',
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Accrual rules define how points are earned and the corresponding deferred revenue liability must be posted to a specific GL account. Finance teams map each accrual rule to a GL account for points lia',
    `menu_item_id` BIGINT COMMENT 'Foreign key linking to menu.menu_item. Business justification: Points accrual rule configuration: operators define which specific menu items trigger bonus point earning (e.g., premium items earn 2x points). The loyalty engine must resolve menu_item_id to apply it',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`offer` (
    ...
    `area_management_id` BIGINT COMMENT 'Foreign key linking to restaurant.area_management. Business justification: Offers are geographically targeted to specific territories/areas for regional promotions. offer.geographic_restriction is a plain-text denormalization of the area_management territory. Area marketing ',
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Offers are brand-specific in multi-brand operators. Brand marketing teams create and own offers; brand-level offer performance reporting, franchise co-op fund accounting, and brand standard compliance',
    ...
    `menu_item_id` BIGINT COMMENT 'Foreign key linking to menu.menu_item. Business justification: Free-item offer fulfillment: when a loyalty offer type is free item, the POS and OLO systems must resolve the exact menu_item to comp. Role-prefix free_ distinguishes this from other menu_item FKs',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: Supplier contracts in foodservice commonly include trade spend / promotional funding clauses that obligate the supplier to fund specific loyalty offers. Finance and procurement teams must reconcile of',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Offers represent promotional discount liabilities with estimated_cost_per_redemption. Finance teams map each offer type to a GL account for promotional expense accrual and discount liability reporting',
    `program_id` BIGINT COMMENT 'Foreign key linking to loyalty.program. Business justification: Loyalty offers (targeted discounts, bonus points, free items) are distributed within the context of a specific loyalty program. Adding program_id to offer links each offer to its governing program, en',
    `promotion_id` BIGINT COMMENT 'Foreign key linking to marketing.promotion. Business justification: Loyalty offers are frequently the loyalty-side execution of a specific marketing promotion. Linking offer to promotion enables promotion ROI reporting that reconciles loyalty offer redemption costs ag',
    ...
    `supplier_id` BIGINT COMMENT 'Foreign key linking to supply.supply_supplier. Business justification: Supplier-funded co-marketing offers (vendor trade spend) are a standard QSR loyalty business process — a beverage or food supplier sponsors a double-points or free-item offer on their products. Procur',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Offer redemption discount costs must be allocated to a cost center for restaurant-level P&L reporting and promotional budget variance analysis. Foodservice finance teams track promotional spend by cos',
    `financial_period_id` BIGINT COMMENT 'Foreign key linking to finance.financial_period. Business justification: Offer redemption discount expenses must be assigned to a financial period for period-end close and promotional expense reporting. Foodservice finance teams require period-level promotional expense acc',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Offer redemptions generate discount_amount transactions that must be posted to a GL account for promotional expense recognition. This is the core promotional discount GL posting process; a foodservi',
    ...
    `menu_item_id` BIGINT COMMENT 'Foreign key linking to menu.menu_item. Business justification: Offer redemption reporting and COGS reconciliation: finance and marketing track which menu items were discounted or comped via offer redemptions to calculate true food cost impact. A foodservice loyal',
    `offer_assignment_id` BIGINT COMMENT 'Foreign key linking to loyalty.offer_assignment. Business justification: An offer_redemption is the fulfillment of a specific offer_assignment — a member was assigned an offer and then redeemed it. Adding offer_assignment_id to offer_redemption closes the lifecycle loop fr',
    ...
    `promotion_id` BIGINT COMMENT 'Foreign key linking to marketing.promotion. Business justification: Offer redemptions in loyalty are the loyalty-side execution of marketing promotions. Linking offer_redemption to promotion enables promotion-level redemption reporting that reconciles marketing promot',
    `channel_id` BIGINT COMMENT 'Foreign key linking to order.channel. Business justification: Offer redemption channel normalization: offer_redemption.redemption_channel is a plain-text attribute. FK to order.channel normalizes channel taxonomy for offer-channel performance reporting (e.g., wh',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`program` (
    ...
    `brand_id` BIGINT COMMENT 'Foreign key linking to restaurant.brand. Business justification: Each loyalty program is owned and operated by a specific brand (e.g., McDonalds Rewards belongs to McDonalds brand). Brand-level loyalty program reporting, franchise disclosure, and multi-brand oper',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Loyalty program deferred revenue liability (outstanding points balance) must be reported under the owning legal entity for statutory financial reporting and IFRS 15 revenue recognition. A foodservice ',
...
CREATE OR REPLACE TABLE `restaurants_ecm`.`loyalty`.`enrollment_event` (
    ...
    `financial_period_id` BIGINT COMMENT 'Foreign key linking to finance.financial_period. Business justification: Enrollment bonus deferred revenue must be recognized in the correct financial period for period-end close reporting. Foodservice finance teams require period assignment for enrollment-driven liability',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` ADD CONSTRAINT `fk_loyalty_member_tier_id` FOREIGN KEY (`tier_id`) REFERENCES `restaurants_ecm`.`loyalty`.`tier`(`tier_id`);
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` ADD CONSTRAINT `fk_loyalty_member_program_id` FOREIGN KEY (`program_id`) REFERENCES `restaurants_ecm`.`loyalty`.`program`(`program_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ADD CONSTRAINT `fk_loyalty_tier_program_id` FOREIGN KEY (`program_id`) REFERENCES `restaurants_ecm`.`loyalty`.`program`(`program_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier_history` ADD CONSTRAINT `fk_loyalty_tier_history_tier_id` FOREIGN KEY (`tier_id`) REFERENCES `restaurants_ecm`.`loyalty`.`tier`(`tier_id`);
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ADD CONSTRAINT `fk_loyalty_points_ledger_accrual_rule_id` FOREIGN KEY (`accrual_rule_id`) REFERENCES `restaurants_ecm`.`loyalty`.`accrual_rule`(`accrual_rule_id`);
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ADD CONSTRAINT `fk_loyalty_points_ledger_enrollment_event_id` FOREIGN KEY (`enrollment_event_id`) REFERENCES `restaurants_ecm`.`loyalty`.`enrollment_event`(`enrollment_event_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ADD CONSTRAINT `fk_loyalty_points_ledger_offer_redemption_id` FOREIGN KEY (`offer_redemption_id`) REFERENCES `restaurants_ecm`.`loyalty`.`offer_redemption`(`offer_redemption_id`);
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ADD CONSTRAINT `fk_loyalty_points_ledger_redemption_id` FOREIGN KEY (`redemption_id`) REFERENCES `restaurants_ecm`.`loyalty`.`redemption`(`redemption_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ADD CONSTRAINT `fk_loyalty_reward_program_id` FOREIGN KEY (`program_id`) REFERENCES `restaurants_ecm`.`loyalty`.`program`(`program_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ADD CONSTRAINT `fk_loyalty_offer_program_id` FOREIGN KEY (`program_id`) REFERENCES `restaurants_ecm`.`loyalty`.`program`(`program_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ADD CONSTRAINT `fk_loyalty_offer_redemption_offer_assignment_id` FOREIGN KEY (`offer_assignment_id`) REFERENCES `restaurants_ecm`.`loyalty`.`offer_assignment`(`offer_assignment_id`);
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Current Tier Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Preferred Location ID');
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` ALTER COLUMN `member_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Guest ID');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`member` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` SET TAGS ('dbx_subdomain' = 'member_enrollment');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier ID');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By Employee Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier_history` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier_history` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'New Tier Tier Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier_history` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Override Authorized By Employee Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier_history` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `restaurants_ecm`.`loyalty`.`tier_history` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` SET TAGS ('dbx_subdomain' = 'points_accrual');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `accrual_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Accrual Rule Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `enrollment_event_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Event Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `financial_period_id` SET TAGS ('dbx_business_glossary_term' = 'Financial Period Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `offer_redemption_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Redemption Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `redemption_id` SET TAGS ('dbx_business_glossary_term' = 'Redemption Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`points_ledger` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Source Channel Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` SET TAGS ('dbx_subdomain' = 'offer_rewards');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `stock_item_id` SET TAGS ('dbx_business_glossary_term' = 'Free Item Stock Item Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By Employee Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`reward` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` SET TAGS ('dbx_subdomain' = 'points_accrual');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Redemption Channel Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `financial_period_id` SET TAGS ('dbx_business_glossary_term' = 'Financial Period Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `stock_item_id` SET TAGS ('dbx_business_glossary_term' = 'Redeemed Stock Item Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Restaurant ID');
ALTER TABLE `restaurants_ecm`.`loyalty`.`redemption` ALTER COLUMN `redemption_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Restaurant ID');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` SET TAGS ('dbx_subdomain' = 'points_accrual');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `area_management_id` SET TAGS ('dbx_business_glossary_term' = 'Area Management Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`accrual_rule` ALTER COLUMN `menu_item_id` SET TAGS ('dbx_business_glossary_term' = 'Menu Item Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` SET TAGS ('dbx_subdomain' = 'offer_rewards');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `offer_id` SET TAGS ('dbx_business_glossary_term' = 'Offer ID');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `area_management_id` SET TAGS ('dbx_business_glossary_term' = 'Area Management Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `menu_item_id` SET TAGS ('dbx_business_glossary_term' = 'Free Menu Item Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Funding Supplier Contract Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `site_id` SET TAGS ('dbx_business_glossary_term' = 'Site Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Supplier Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_assignment` SET TAGS ('dbx_subdomain' = 'offer_rewards');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` SET TAGS ('dbx_subdomain' = 'offer_rewards');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `financial_period_id` SET TAGS ('dbx_business_glossary_term' = 'Financial Period Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `menu_item_id` SET TAGS ('dbx_business_glossary_term' = 'Menu Item Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `offer_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Assignment Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `promotion_id` SET TAGS ('dbx_business_glossary_term' = 'Promotion Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`offer_redemption` ALTER COLUMN `channel_id` SET TAGS ('dbx_business_glossary_term' = 'Redemption Channel Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`program` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`program` ALTER COLUMN `brand_id` SET TAGS ('dbx_business_glossary_term' = 'Brand Id (Foreign Key)');
ALTER TABLE `restaurants_ecm`.`loyalty`.`program` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`enrollment_event` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `restaurants_ecm`.`loyalty`.`enrollment_event` ALTER COLUMN `financial_period_id` SET TAGS ('dbx_business_glossary_term' = 'Financial Period Id (Foreign Key)');
