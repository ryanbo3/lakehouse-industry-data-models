-- Schema for Domain: flight | Business: Airlines | Version: v1_ecm
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`scheduled_flight` (
    ...
    `foreign_carrier_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.foreign_carrier_permit. Business justification: Scheduled international services must comply with bilateral permits specifying traffic rights, frequencies, capacity limits, and route authorities. Link required for schedule compliance validation, sl',
    ...
    `aircraft_configuration_code` STRING COMMENT 'Code identifying the specific cabin configuration and seat layout for the scheduled aircraft (e.g., 3-class, 2-class, all-economy). Links to detailed seat maps and cabin class definitions.',
    ...
    `flight_number` STRING COMMENT 'The published flight number consisting of airline code and numeric identifier (e.g., AA1234, BA456). This is the externally-known identifier for the scheduled flight.. Valid values are `^[A-Z0-9]{2}[0-9]{1,4}[A-Z]?$`',
    ...
    `marketing_carrier_code` STRING COMMENT 'Two-character IATA airline code of the carrier marketing and selling the flight. May differ from operating carrier in codeshare arrangements.. Valid values are `^[A-Z0-9]{2}$`',
    ...
    `season_code` STRING COMMENT 'IATA season identifier for this schedule (e.g., S24 for Summer 2024, W24 for Winter 2024/2025). IATA defines two seasons per year: summer (last Sunday in March to last Saturday in October) and winter (last Sunday in October to last Saturday in March).. Valid values are `^(S|W)[0-9]{2}$`',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`flight_leg` (
    ...
    `occurrence_id` BIGINT COMMENT 'Foreign key linking to safety.occurrence. Business justification: Flight legs frequently generate safety occurrences (bird strikes, turbulence injuries, system malfunctions). Operations Control Center and Safety Management System require direct flight_leg→occurrence',
    ...
    `cabin_configuration_code` STRING COMMENT 'Code identifying the specific cabin layout and seating configuration of the aircraft operating this flight leg (e.g., number of first, business, premium economy, and economy seats).. Valid values are `^[A-Z0-9]{2,10}$`',
    ...
    `flight_number` STRING COMMENT 'The actual flight number operated on this leg, including carrier code and numeric identifier (e.g., AA1234, BA456).. Valid values are `^[A-Z0-9]{2,3}[0-9]{1,4}[A-Z]?$`',
    ...
    `operating_carrier_code` STRING COMMENT 'Two or three-letter IATA airline code of the carrier actually operating the flight (may differ from marketing carrier in codeshare scenarios).. Valid values are `^[A-Z0-9]{2,3}$`',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`dispatch_release` (
    ...
    `dangerous_goods_id` BIGINT COMMENT 'Foreign key linking to cargo.dangerous_goods. Business justification: Dispatchers must review and acknowledge dangerous goods declarations during dispatch release preparation per IATA DGR and regulatory requirements. This link enables verification that DG cargo is prope',
    `employee_id` BIGINT COMMENT 'Reference to the certified flight dispatcher who prepared and authorized this dispatch release.',
    `flight_leg_id` BIGINT COMMENT 'Reference to the specific flight leg that this dispatch release authorizes for departure.',
    `flight_plan_id` BIGINT COMMENT 'Foreign key linking to flight.flight_plan. Business justification: Dispatch release is based on and references a specific flight plan version. The dispatcher reviews the flight plan and issues the release authorizing departure. This is a core operational relationship',
    `member_id` BIGINT COMMENT 'Reference to the pilot-in-command (captain) who accepted and co-signed this dispatch release.',
    `operational_approval_id` BIGINT COMMENT 'Foreign key linking to compliance.operational_approval. Business justification: Dispatch releases must reference specific operational approvals (ETOPS, RNP, CAT II/III) authorizing the planned operation. Critical for pre-flight compliance verification, regulatory audit trails, an',
    ...
    `additional_fuel_kg` DECIMAL(18,2) COMMENT 'Extra fuel added at captain or dispatcher discretion beyond regulatory minimums for operational flexibility, measured in kilograms.',
    `aircraft_registration` STRING COMMENT 'Tail number (registration) of the aircraft assigned to this flight at time of dispatch release issuance.. Valid values are `^[A-Z]-[A-Z0-9]{4}$|^N[0-9]{1,5}[A-Z]{0,2}$`',
    ...
    `release_number` STRING COMMENT 'Official externally-known dispatch release document number assigned by flight operations control. Unique business identifier for regulatory traceability.. Valid values are `^[A-Z0-9]{6,20}$`',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`flight_plan` (
    `flight_plan_id` BIGINT COMMENT 'Unique identifier for the flight plan record. Primary key.',
    ...
    `employee_id` BIGINT COMMENT 'Reference to the flight dispatcher or flight planning officer who generated this flight plan.',
    ...
    `additional_fuel_kg` STRING COMMENT 'Extra fuel in kilograms added at the discretion of the pilot-in-command or dispatcher for operational reasons (weather, delays, etc.).',
    `aircraft_registration` STRING COMMENT 'Tail number or registration mark of the aircraft assigned to this flight plan.. Valid values are `^[A-Z0-9-]{5,10}$`',
    ...
    CONSTRAINT pk_flight_plan PRIMARY KEY(`flight_plan_id`)
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`fuel_uplift` (
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor. Business justification: Fuel uplifts are performed by contracted into-plane agents (vendors). Airlines track vendor performance, invoicing, and SLA compliance per uplift. Current fueling_company_code is denormalized; proper ',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`acars_message` (
    `acars_message_id` BIGINT COMMENT 'Unique identifier for the ACARS message record. Primary key.',
    `flight_leg_id` BIGINT COMMENT 'Reference to the flight leg during which this ACARS message was transmitted or received.',
    `oooi_event_id` BIGINT COMMENT 'Foreign key linking to flight.oooi_event. Business justification: ACARS messages with oooi_event_type populated are specifically OOOI event reports (Out-Off-On-In gate/runway timestamps). When an ACARS message IS an OOOI event report, it should FK to the oooi_event ',
    `acknowledgement_required_flag` BOOLEAN COMMENT 'Indicates whether the message requires an acknowledgement response from the receiving party.',
    `acknowledgement_timestamp` TIMESTAMP COMMENT 'UTC timestamp when the acknowledgement was received for this message. Null if no acknowledgement required or not yet received.',
    `aircraft_registration` STRING COMMENT 'Tail number or registration identifier of the aircraft that transmitted or received the message.. Valid values are `^[A-Z0-9]{5,7}$`',
    `altitude_feet` STRING COMMENT 'Aircraft altitude in feet above mean sea level at the time of message transmission. Populated for position report and engine data messages.',
    `arrival_airport_code` STRING COMMENT 'Three-letter IATA code of the arrival airport for the flight leg associated with this message.. Valid values are `^[A-Z]{3}$`',
    `block_number` STRING COMMENT 'Block identifier used in ACARS protocol for message segmentation and reassembly when messages exceed single transmission size.. Valid values are `^[A-Z0-9]{1,3}$`',
    `created_timestamp` TIMESTAMP COMMENT 'UTC timestamp when this ACARS message record was first created in the data system.',
    `data_link_service_provider` STRING COMMENT 'Communication service provider or medium used for ACARS message transmission (SITA network, ARINC network, satellite communication, VHF radio, or HF radio).. Valid values are `SITA|ARINC|SATCOM|VHF|HF`',
    `departure_airport_code` STRING COMMENT 'Three-letter IATA code of the departure airport for the flight leg associated with this message.. Valid values are `^[A-Z]{3}$`',
    `error_code` STRING COMMENT 'Error code indicating transmission or processing failure, if applicable. Null for successful messages.',
    `flight_phase` STRING COMMENT 'Phase of flight during which the ACARS message was transmitted, derived from OOOI events and flight profile data. [ENUM-REF-CANDIDATE: pre_departure|taxi_out|takeoff|climb|cruise|descent|approach|landing|taxi_in|post_arrival — 10 candidates stripped; promote to reference product]',
    `ground_speed_knots` STRING COMMENT 'Aircraft ground speed in knots at the time of message transmission. Populated for position report messages.',
    `message_content` STRING COMMENT 'Full text payload of the ACARS message containing the transmitted data, parameters, or free-text communication.',
    `message_direction` STRING COMMENT 'Direction of message transmission: uplink (ground to aircraft) or downlink (aircraft to ground).. Valid values are `uplink|downlink`',
    `message_label` STRING COMMENT 'Two-character ARINC 618 message label code that identifies the message category and routing (e.g., H1 for OOOI, 44 for free text, B6 for position report).. Valid values are `^[A-Z0-9]{2}$`',
    `message_sequence_number` STRING COMMENT 'Sequential number assigned to the message within the flight leg or transmission session for ordering and duplicate detection.',
    `message_size_bytes` STRING COMMENT 'Size of the ACARS message payload in bytes.',
    `message_type` STRING COMMENT 'Classification of the ACARS message indicating its purpose: OOOI (Out-Off-On-In gate events), engine data, position report, crew free-text, CPDLC (Controller-Pilot Data Link Communications), ATIS (Automated Terminal Information Service), weather uplink, PIREP (Pilot Report), maintenance alert, or fuel report. [ENUM-REF-CANDIDATE: OOOI|engine_data|position_report|free_text|cpdlc|atis|weather_uplink|pirep|maintenance_alert|fuel_report — 10 candidates stripped; promote to reference product]',
    `oooi_event_type` STRING COMMENT 'Specific OOOI gate event captured in this message: OUT (pushback from gate), OFF (wheels up/takeoff), ON (wheels down/landing), IN (arrival at gate). Populated only for OOOI message types.. Valid values are `OUT|OFF|ON|IN`',
    `position_latitude` DECIMAL(18,2) COMMENT 'Latitude coordinate of the aircraft position at the time of message transmission, in decimal degrees. Populated for position report messages.',
    `position_longitude` DECIMAL(18,2) COMMENT 'Longitude coordinate of the aircraft position at the time of message transmission, in decimal degrees. Populated for position report messages.',
    `priority_level` STRING COMMENT 'Priority classification of the message indicating urgency of processing and delivery (urgent for safety-critical, high for operational, normal for routine, low for informational).. Valid values are `urgent|high|normal|low`',
    `processing_status` STRING COMMENT 'Current processing state of the ACARS message in the ground system workflow.. Valid values are `received|processed|acknowledged|failed|pending`',
    `receipt_timestamp` TIMESTAMP COMMENT 'UTC timestamp when the ACARS message was received by the ground station or aircraft system.',
    `retry_count` STRING COMMENT 'Number of transmission retry attempts for this message due to delivery failures or acknowledgement timeouts.',
    `transmission_timestamp` TIMESTAMP COMMENT 'UTC timestamp when the ACARS message was transmitted from the aircraft or ground station.',
    `updated_timestamp` TIMESTAMP COMMENT 'UTC timestamp when this ACARS message record was last updated in the data system.',
    CONSTRAINT pk_acars_message PRIMARY KEY(`acars_message_id`)
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`delay_record` (
    ...
    `consumer_protection_case_id` BIGINT COMMENT 'Foreign key linking to compliance.consumer_protection_case. Business justification: Delays exceeding regulatory thresholds (3+ hours EU261, tarmac delay rules) trigger consumer protection obligations. Link enables compensation eligibility determination, regulatory reporting, and trac',
    ...
    `employee_id` BIGINT COMMENT 'User identifier of the supervisor or quality assurance staff member who verified and approved this delay record. Used for data quality control and compliance auditing.',
    ...
    `secondary_delay_code` STRING COMMENT 'Two-digit IATA delay code representing a secondary contributing cause of the delay per AHM 730 standard. Nullable if only one cause exists.. Valid values are `^[0-9]{2}$`',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`diversion` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Diversion decisions require named authority for regulatory accountability (FAA/EASA reporting). Operations Control Center duty managers or dispatchers make diversion calls; tracking the specific emplo',
    ...
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Diversions often require mandatory regulatory filings (safety reports, incident notifications to aviation authorities). Link tracks filing completeness, ensures timely submission, and maintains audit ',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`flight_irop_event` (
    `flight_irop_event_id` BIGINT COMMENT 'Unique identifier for the irregular operations event record. Primary key for the flight IROP event entity.',
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: IROP events (irregular operations) have designated coordinators managing recovery. Current event_coordinator_name is denormalized. Airlines track coordinator accountability for performance reviews, re',
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: IROP events trigger significant financial impacts (recovery operations, passenger compensation, rebooking costs, crew expenses) that must be posted to GL for financial reporting, variance analysis, an',
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Major IROP events (weather disruptions, system failures affecting multiple flights) trigger regulatory reporting obligations to aviation authorities. Link required for filing management, demonstrating',
    ...
    `additional_affected_stations` STRING COMMENT 'Comma-separated list of additional IATA airport codes for stations impacted by the irregular operations event beyond the primary affected station.',
    ...
    CONSTRAINT pk_flight_irop_event PRIMARY KEY(`flight_irop_event_id`)
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`cancellation` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Flight cancellations require named decision authority for financial accountability (revenue impact tracking), regulatory reporting (DOT on-time performance), and union grievance defense. Current cance',
    `consumer_protection_case_id` BIGINT COMMENT 'Foreign key linking to compliance.consumer_protection_case. Business justification: Flight cancellations triggering passenger compensation thresholds (EU261, DOT rules) generate consumer protection cases. Link required for regulatory reporting, compensation tracking, and demonstratin',
    `flight_irop_event_id` BIGINT COMMENT 'Reference to the broader irregular operations event that triggered this cancellation, if part of a multi-flight disruption scenario.',
    `flight_leg_id` BIGINT COMMENT 'Reference to the cancelled flight leg in the flight operations system.',
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Cancellations trigger GL postings for revenue adjustments, passenger compensation expense, and operational cost write-offs. Financial reporting and variance analysis require linking cancellation event',
    ...
    `maintenance_defect_code` STRING COMMENT 'ATA chapter or MEL (Minimum Equipment List) code for the maintenance defect that caused the cancellation, if maintenance-related.. Valid values are `^[A-Z0-9]{2,6}$`',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`operational_flight_plan` (
    `operational_flight_plan_id` BIGINT COMMENT 'Unique identifier for the operational flight plan record. Primary key.',
    `dispatch_release_id` BIGINT COMMENT 'Foreign key linking to flight.dispatch_release. Business justification: The OFP is generated from and linked to a specific dispatch release. Currently has dispatch_release_number as STRING, which should be replaced with proper FK. The dispatch release number can be retrie',
    `flight_leg_id` BIGINT COMMENT 'Reference to the flight leg this operational flight plan supports.',
    `flight_plan_id` BIGINT COMMENT 'Foreign key linking to flight.flight_plan. Business justification: The OFP (operational flight plan loaded into FMS) is derived from the filed flight_plan. This links the operational version to the planning version, enabling tracking of how the filed plan was adapted',
    `operational_approval_id` BIGINT COMMENT 'Foreign key linking to compliance.operational_approval. Business justification: Operational flight plans must reference applicable operational approvals (ETOPS authorization, special airport approvals, RNP specifications) to validate flight legality. Critical for dispatch complia',
    `operational_standard_id` BIGINT COMMENT 'Foreign key linking to route.operational_standard. Business justification: OFP generation requires direct reference to route-specific turnaround standards for accurate crew duty time calculations, minimum ground time validation, and fuel planning. Dispatchers validate planne',
    `employee_id` BIGINT COMMENT 'Reference to the flight dispatcher who prepared and released this operational flight plan.',
    `additional_fuel_kg` DECIMAL(18,2) COMMENT 'Additional fuel carried at the discretion of the pilot in command or dispatcher beyond regulatory minimums in kilograms.',
    `alternate_airport_1_code` STRING COMMENT 'IATA or ICAO code of the primary alternate airport designated in the operational flight plan.. Valid values are `^[A-Z]{3,4}$`',
    `alternate_airport_2_code` STRING COMMENT 'IATA or ICAO code of the secondary alternate airport designated in the operational flight plan.. Valid values are `^[A-Z]{3,4}$`',
    `alternate_fuel_kg` DECIMAL(18,2) COMMENT 'Fuel required to fly from destination to the designated alternate airport in kilograms.',
    `contingency_fuel_kg` DECIMAL(18,2) COMMENT 'Contingency fuel carried to account for unforeseen factors such as wind, routing changes, or ATC delays in kilograms.',
    `cost_index` STRING COMMENT 'Cost index value used by the FMS to optimize speed and fuel burn trade-offs. Higher values favor speed over fuel economy.',
    `crew_accepted_timestamp` TIMESTAMP COMMENT 'Timestamp when the flight crew formally accepted the operational flight plan. Represents the final crew-accepted version distinct from the initial computerized flight plan filed with ATC.',
    `estimated_block_time_minutes` STRING COMMENT 'Estimated block time from gate departure to gate arrival in minutes.',
    `estimated_time_enroute_minutes` STRING COMMENT 'Estimated time enroute from takeoff to landing at destination in minutes.',
    `etops_alternate_code` STRING COMMENT 'IATA or ICAO code of the ETOPS alternate airport for extended-range twin-engine operations.. Valid values are `^[A-Z]{3,4}$`',
    `etops_approval_indicator` BOOLEAN COMMENT 'Indicates whether this flight operates under ETOPS approval for extended-range twin-engine operations.',
    `etops_rule_time_minutes` STRING COMMENT 'ETOPS rule time in minutes (e.g., 120, 180, 207) indicating maximum diversion time to an alternate airport.',
    `final_reserve_fuel_kg` DECIMAL(18,2) COMMENT 'Final reserve fuel required for 30 minutes holding at 1500 feet above alternate airport in kilograms.',
    `flight_planning_system_code` STRING COMMENT 'Code identifying the flight planning system that generated this operational flight plan (e.g., Jeppesen, Lido, NavBlue).',
    `minimum_equipment_list_items` STRING COMMENT 'List of MEL items or deferred maintenance items affecting this flight. Comma-separated MEL reference numbers.',
    `notam_summary` STRING COMMENT 'Summary of relevant NOTAMs affecting the route, departure, destination, and alternate airports.',
    `ofp_version_number` STRING COMMENT 'Version number of the operational flight plan. Increments when the plan is revised after initial dispatch.',
    `plan_generated_timestamp` TIMESTAMP COMMENT 'Timestamp when the operational flight plan was generated by the flight planning system.',
    `plan_status` STRING COMMENT 'Current lifecycle status of the operational flight plan. Tracks progression from draft through crew acceptance to completion. [ENUM-REF-CANDIDATE: draft|released|accepted|active|completed|cancelled|superseded — 7 candidates stripped; promote to reference product]',
    `planned_cruise_altitude_ft` STRING COMMENT 'Initial planned cruise altitude in feet above mean sea level.',
    `planned_cruise_speed_mach` DECIMAL(18,2) COMMENT 'Planned cruise speed expressed as Mach number.',
    `planned_landing_weight_kg` DECIMAL(18,2) COMMENT 'Planned landing weight of the aircraft in kilograms at destination.',
    `planned_takeoff_weight_kg` DECIMAL(18,2) COMMENT 'Planned takeoff weight of the aircraft in kilograms including fuel, payload, and crew.',
    `record_created_timestamp` TIMESTAMP COMMENT 'Timestamp when this operational flight plan record was first created in the system.',
    `record_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this operational flight plan record was last updated in the system.',
    `route_description` STRING COMMENT 'Full textual description of the flight route including waypoints, airways, and navigation fixes as filed with ATC and loaded into the FMS.',
    `step_climb_profile` STRING COMMENT 'Planned step-climb altitude profile showing waypoints and altitudes for fuel-efficient cruise climbs.',
    `taxi_fuel_kg` DECIMAL(18,2) COMMENT 'Fuel expected to be consumed during taxi operations before takeoff in kilograms.',
    `total_fuel_required_kg` DECIMAL(18,2) COMMENT 'Total fuel required for the flight including trip, contingency, alternate, reserve, additional, and taxi fuel in kilograms.',
    `trip_fuel_kg` DECIMAL(18,2) COMMENT 'Planned fuel required from takeoff to landing at destination in kilograms.',
    `weather_deviation_indicator` BOOLEAN COMMENT 'Indicates whether the operational flight plan includes route deviations due to weather conditions.',
    `zero_fuel_weight_kg` DECIMAL(18,2) COMMENT 'Zero fuel weight of the aircraft in kilograms. Total weight excluding usable fuel.',
    CONSTRAINT pk_operational_flight_plan PRIMARY KEY(`operational_flight_plan_id`)
) COMMENT 'The operational flight plan (OFP) as loaded into the FMS and used by the crew in flight. Contains the final agreed route, step-climb profile, fuel burn by segment, estimated time en-route, waypoint ETAs, alternate fuel requirements, zero fuel weight, takeoff weight, landing weight, and performance data. Represents the final crew-accepted version distinct from the initial computerized flight_plan filed with ATC. Sourced from Jeppesen Flight Planning System.';
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`weight_balance` (
    ...
    `operational_flight_plan_id` BIGINT COMMENT 'Foreign key linking to flight.operational_flight_plan. Business justification: W&B loadsheet and OFP are coordinated operational documents. The OFP uses W&B data (ZFW, TOW, CG) for performance calculations and FMS loading. This FK links the loadsheet to the operational plan it s',
    ...
    `loadmaster_license_number` STRING COMMENT 'Professional license or certification number of the loadmaster who prepared the loadsheet.',
    `loadmaster_name` STRING COMMENT 'Name of the loadmaster or load control agent who prepared the weight and balance loadsheet.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`flight_performance` (
    `flight_performance_id` BIGINT COMMENT 'Unique identifier for the flight performance record. Primary key.',
    `aircraft_type_id` BIGINT COMMENT 'Foreign key linking to fleet.aircraft_type. Business justification: Performance analysis requires aircraft type baseline data (typical fuel burn, cruise performance) to calculate variance indices and identify anomalies. Fleet performance benchmarking compares actual f',
    `emissions_report_id` BIGINT COMMENT 'Foreign key linking to compliance.emissions_report. Business justification: Flight-level fuel burn and CO2 emissions aggregate into regulatory emissions reports for CORSIA, EU ETS, and carbon schemes. Link essential for emissions accounting, verification audits, and demonstra',
    `flight_leg_id` BIGINT COMMENT 'Reference to the completed flight leg for which performance data was captured.',
    `flight_plan_id` BIGINT COMMENT 'Foreign key linking to flight.flight_plan. Business justification: Post-flight performance data should reference the flight plan that was filed/used for the flight. This allows comparing planned vs actual performance metrics (fuel burn, block time, cruise altitude, e',
    `operational_flight_plan_id` BIGINT COMMENT 'Foreign key linking to flight.operational_flight_plan. Business justification: Flight_performance captures post-flight data from FDR/FMS/ACARS. It should reference the specific OFP (operational flight plan loaded into FMS) that was executed. This is more specific than the filed ',
    `actual_block_time_minutes` STRING COMMENT 'The actual block time in minutes from Out to In (OOOI) as recorded during flight operations.',
    `actual_fuel_burn_kg` DECIMAL(18,2) COMMENT 'The actual fuel consumed during the flight in kilograms, captured from FDR/FMS/ACARS data.',
    `aircraft_registration` STRING COMMENT 'Tail number or registration identifier of the aircraft that operated this flight.. Valid values are `^[A-Z0-9-]{5,10}$`',
    `anomaly_description` STRING COMMENT 'Textual description of any performance anomalies flagged during this flight, including threshold violations and engineering notes.',
    `anomaly_flag` BOOLEAN COMMENT 'Boolean flag indicating whether any performance anomalies were detected during this flight (e.g., excessive fuel burn, engine parameter deviations).',
    `atc_delay_minutes` STRING COMMENT 'Total delay in minutes attributed to ATC restrictions, holding patterns, or routing changes during this flight.',
    `average_cruise_speed_mach` DECIMAL(18,2) COMMENT 'The average cruise speed expressed as a Mach number during the cruise phase of flight.',
    `average_ground_speed_knots` STRING COMMENT 'The average ground speed in knots during the flight, accounting for wind effects.',
    `block_time_variance_minutes` STRING COMMENT 'The difference between planned and actual block time in minutes. Positive indicates longer than planned, negative indicates shorter.',
    `co2_emissions_kg` DECIMAL(18,2) COMMENT 'Calculated CO2 emissions in kilograms for this flight based on actual fuel burn and fuel type emission factors.',
    `corsia_eligible_flag` BOOLEAN COMMENT 'Boolean flag indicating whether this flight is eligible for CORSIA carbon reporting based on route and regulatory criteria.',
    `cruise_altitude_feet` STRING COMMENT 'The actual cruise altitude achieved during flight in feet above mean sea level, captured from FMS data.',
    `data_quality_score` DECIMAL(18,2) COMMENT 'A quality score from 0.00 to 1.00 indicating the completeness and reliability of the performance data captured for this flight.',
    `data_source` STRING COMMENT 'The primary source system from which performance data was captured: FDR (Flight Data Recorder), FMS (Flight Management System), ACARS (Aircraft Communications Addressing and Reporting System), or MANUAL entry.. Valid values are `FDR|FMS|ACARS|MANUAL`',
    `destination_airport_code` STRING COMMENT 'Three-letter IATA code of the arrival airport for this flight leg.. Valid values are `^[A-Z]{3}$`',
    `engine_1_avg_egt_celsius` DECIMAL(18,2) COMMENT 'Average Exhaust Gas Temperature for engine 1 during cruise phase in degrees Celsius, captured from FDR.',
    `engine_1_avg_fuel_flow_kg_per_hour` DECIMAL(18,2) COMMENT 'Average fuel flow rate for engine 1 in kilograms per hour during cruise phase.',
    `engine_1_avg_n1_percent` DECIMAL(18,2) COMMENT 'Average N1 (low-pressure spool speed) for engine 1 during cruise phase as a percentage of maximum RPM.',
    `engine_2_avg_egt_celsius` DECIMAL(18,2) COMMENT 'Average Exhaust Gas Temperature for engine 2 during cruise phase in degrees Celsius, captured from FDR.',
    `engine_2_avg_fuel_flow_kg_per_hour` DECIMAL(18,2) COMMENT 'Average fuel flow rate for engine 2 in kilograms per hour during cruise phase.',
    `engine_2_avg_n1_percent` DECIMAL(18,2) COMMENT 'Average N1 (low-pressure spool speed) for engine 2 during cruise phase as a percentage of maximum RPM.',
    `flight_number` STRING COMMENT 'The flight number for this performance record, typically carrier code plus numeric identifier.. Valid values are `^[A-Z0-9]{2,3}[0-9]{1,4}[A-Z]?$`',
    `flight_phase_coverage` STRING COMMENT 'Comma-separated list of flight phases for which performance data was successfully captured (e.g., TAXI_OUT, TAKEOFF, CLIMB, CRUISE, DESCENT, LANDING, TAXI_IN).',
    `fuel_efficiency_index` DECIMAL(18,2) COMMENT 'Normalized fuel efficiency metric comparing actual fuel burn to planned fuel burn. Values near 1.0 indicate on-plan performance.',
    `fuel_variance_kg` DECIMAL(18,2) COMMENT 'The difference between planned and actual fuel burn in kilograms. Positive indicates over-burn, negative indicates under-burn.',
    `operating_date` DATE COMMENT 'The date on which the flight operated, used for performance tracking and reporting.',
    `origin_airport_code` STRING COMMENT 'Three-letter IATA code of the departure airport for this flight leg.. Valid values are `^[A-Z]{3}$`',
    `performance_category` STRING COMMENT 'Overall performance classification for this flight based on fuel efficiency, block time adherence, and engine performance metrics.. Valid values are `EXCELLENT|GOOD|ACCEPTABLE|BELOW_TARGET|POOR`',
    `planned_block_time_minutes` STRING COMMENT 'The scheduled block time in minutes from gate departure to gate arrival as planned during scheduling.',
    `planned_cruise_altitude_feet` STRING COMMENT 'The planned cruise altitude in feet as filed in the flight plan.',
    `planned_fuel_kg` DECIMAL(18,2) COMMENT 'The planned fuel load in kilograms as calculated during flight planning and dispatch release.',
    `record_created_timestamp` TIMESTAMP COMMENT 'Timestamp when this flight performance record was first created in the system.',
    `record_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this flight performance record was last updated or modified.',
    `remarks` STRING COMMENT 'Free-text remarks or notes from flight operations, engineering, or performance analysts regarding this flight performance record.',
    `weather_impact_flag` BOOLEAN COMMENT 'Boolean flag indicating whether significant weather conditions (turbulence, icing, convective activity) impacted flight performance.',
    `wind_component_knots` STRING COMMENT 'The average wind component in knots experienced during cruise. Positive values indicate tailwind, negative values indicate headwind.',
    CONSTRAINT pk_flight_performance PRIMARY KEY(`flight_performance_id`)
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`booking_segment` (
    ...
    `booking_class` STRING COMMENT 'Single-letter booking class (RBD - Revenue Booking Designator) for this passenger on this flight segment. Determines fare level, upgrade eligibility, and mileage accrual.',
    `cabin_class` STRING COMMENT 'The physical cabin class in which the passenger is seated for this flight segment.',
    ...
    `fare_basis` STRING COMMENT 'The fare basis code indicating the specific fare rules, restrictions, and conditions applied to this passengers ticket for this segment.',
    `seat_assignment` STRING COMMENT 'The specific seat assigned to the passenger on this flight leg (e.g., 12A, 34F). May be pre-assigned at booking, selected during check-in, or assigned at gate.',
    ...
    `ticket_number` STRING COMMENT 'The 13-digit IATA ticket number issued for this passenger on this flight segment. Links to ticketing and revenue accounting systems.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`flight_allotment` (
    `flight_allotment_id` BIGINT COMMENT 'Primary key for flight_allotment',
    `cargo_allotment_id` BIGINT COMMENT 'Unique identifier for this flight allotment record. Primary key.',
    `freight_forwarder_id` BIGINT COMMENT 'Foreign key linking to the freight forwarder holding the allotment. References the cargo agent master record.',
    `scheduled_flight_id` BIGINT COMMENT 'Foreign key linking to the scheduled flight on which capacity is allocated. References the master scheduled flight record.',
    `allocated_volume_cbm` DECIMAL(18,2) COMMENT 'The contracted cargo volume capacity in cubic meters that the freight forwarder has secured on this scheduled flight. Volume constraint applies in addition to weight.',
    `allocated_weight_kg` DECIMAL(18,2) COMMENT 'The contracted cargo weight capacity in kilograms that the freight forwarder has secured on this scheduled flight. This is the maximum weight the forwarder can book against this allotment.',
    `allotment_status` STRING COMMENT 'Current lifecycle status of the allotment: active (currently in effect), suspended (temporarily inactive), expired (validity period ended), cancelled (terminated before expiry).',
    `created_date` TIMESTAMP COMMENT 'Timestamp when this allotment record was created in the system.',
    `last_modified_date` TIMESTAMP COMMENT 'Timestamp when this allotment record was last updated.',
    `minimum_utilization_threshold_pct` DECIMAL(18,2) COMMENT 'The minimum percentage of allocated capacity that the freight forwarder must utilize to maintain the allotment. Failure to meet this threshold may result in capacity reduction or allotment cancellation.',
    `priority_level` STRING COMMENT 'The booking priority level assigned to this allotment, determining precedence during capacity allocation and overbooking scenarios: platinum (highest priority), gold, silver, standard.',
    `rate_class` STRING COMMENT 'The pricing tier or rate class applicable to this allotment: contract (negotiated contract rate), published (standard published tariff), spot (market-driven spot rate), promotional (temporary promotional rate).',
    `validity_end_date` DATE COMMENT 'The last date on which this allotment contract is valid. After this date, the allotment expires and capacity reverts to general sale.',
    `validity_start_date` DATE COMMENT 'The first date on which this allotment contract becomes effective. The forwarder can book cargo against this allotment starting from this date.',
    CONSTRAINT pk_flight_allotment PRIMARY KEY(`flight_allotment_id`)
) COMMENT 'This association product represents the contractual capacity allocation between a scheduled flight and a freight forwarder. It captures the pre-booked cargo space (weight and volume) that a forwarder has secured on a specific scheduled flight, along with commercial terms, validity periods, and utilization thresholds. Each record links one scheduled_flight to one freight_forwarder with attributes that exist only in the context of this capacity contract.. Existence Justification: In airline cargo operations, freight forwarders negotiate capacity allotments on multiple scheduled flights to secure space for their shipments (e.g., a forwarder holds allotments on LAX-JFK, LAX-ORD, SFO-JFK). Simultaneously, each scheduled flight has capacity allocated to multiple forwarders to maximize load factor and revenue. The allotment itself is a contractual business entity with specific attributes (allocated weight/volume, validity periods, rate class, utilization thresholds) that belong to neither the flight nor the forwarder alone, but to the relationship between them.';

CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`cargo_segment` (
    `cargo_segment_id` BIGINT COMMENT 'Unique identifier for this cargo segment record. Primary key.',
    `awb_id` BIGINT COMMENT 'Foreign key linking to the air waybill being transported on this segment',
    `flight_leg_id` BIGINT COMMENT 'Foreign key linking to the specific flight leg on which this cargo segment was transported',
    `compartment_code` STRING COMMENT 'Code identifying the specific cargo compartment or hold position where this AWB was loaded on this flight leg (e.g., FWD, AFT, BULK). Used for weight and balance calculations.',
    `handling_station_code` STRING COMMENT 'Three-letter IATA airport code for the station where this cargo was handled for this segment (loading, transfer, or offloading). May differ from flight origin/destination for transfer cargo.',
    `loaded_timestamp` TIMESTAMP COMMENT 'Date and time when this AWB was physically loaded onto the aircraft for this flight leg. Used for operational performance tracking and load reconciliation.',
    `loading_priority` STRING COMMENT 'Priority level for loading this AWB on this specific flight leg, which may differ from overall AWB priority based on connection times, special handling requirements, or operational constraints.',
    `manifested_timestamp` TIMESTAMP COMMENT 'Date and time when this AWB was added to the cargo manifest for this specific flight leg. Used for operational cutoff tracking and regulatory compliance.',
    `offloaded_timestamp` TIMESTAMP COMMENT 'Date and time when this AWB was physically offloaded from the aircraft after this flight leg. Used for transfer coordination and delivery tracking.',
    `proration_factor` DECIMAL(18,2) COMMENT 'Revenue allocation factor for this segment, used to distribute total AWB revenue across multiple flight legs for interline and codeshare accounting. Typically based on segment distance or weight-distance.',
    `segment_chargeable_weight_kg` DECIMAL(18,2) COMMENT 'Chargeable weight for this specific segment, used for proration calculations and revenue allocation to this flight leg.',
    `segment_sequence` STRING COMMENT 'Sequential order of this flight leg within the AWBs complete routing (1 for first leg, 2 for second leg, etc.). Critical for multi-leg shipment tracking and transfer coordination.',
    `segment_status` STRING COMMENT 'Current operational status of this cargo segment within its lifecycle: booked (reserved on flight), manifested (included in flight manifest), loaded (physically loaded on aircraft), departed (flight departed), arrived (flight arrived), offloaded (cargo removed from aircraft), transferred (handed over to next segment).',
    `segment_weight_kg` DECIMAL(18,2) COMMENT 'Actual weight of this AWB carried on this specific flight leg, in kilograms. May differ from total AWB weight if shipment is split or consolidated during transit.',
    `transfer_timestamp` TIMESTAMP COMMENT 'Date and time when this AWB was transferred between flight legs at an intermediate station. Null for origin and final destination segments. Critical for tracking transfer performance and connection integrity.',
    `uld_number` STRING COMMENT 'Identifier of the Unit Load Device (container or pallet) on which this AWB was loaded for this flight leg. Null for bulk cargo. Used for load planning and tracking.',
    CONSTRAINT pk_cargo_segment PRIMARY KEY(`cargo_segment_id`)
) COMMENT 'This association product represents the operational segment-level tracking of air waybill shipments on specific flight legs. It captures the core operational reality of air cargo routing: multi-leg shipments travel on multiple flight_legs, and each flight_leg carries multiple AWBs. Each record links one flight_leg to one AWB with segment-specific attributes including weight carried per leg, proration factors for revenue sharing, transfer handling details, and segment sequencing. This is the atomic unit of cargo operations and revenue accounting.. Existence Justification: In airline cargo operations, multi-leg shipments routinely travel on multiple flight_legs (e.g., LAX→LHR via JFK requires two flight_legs), and each flight_leg carries multiple AWBs. This is the fundamental operational reality of air cargo routing. Airlines actively manage segment-level data including weight carried per leg, proration factors for revenue sharing among carriers, transfer handling at intermediate stations, and segment sequencing for operational coordination.';

CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` (
    `dispatch_exemption_invocation_id` BIGINT COMMENT 'Unique identifier for this specific invocation of an exemption within a dispatch release. Primary key.',
    `dispatch_release_id` BIGINT COMMENT 'Foreign key linking to the dispatch release document that invokes this exemption',
    `exemption_waiver_id` BIGINT COMMENT 'Foreign key linking to the regulatory exemption or waiver being invoked',
    `applicability_justification` STRING COMMENT 'Dispatchers explanation of why this specific exemption is applicable to this flight (e.g., ETOPS diversion time waiver applies due to North Atlantic routing with limited alternate airports). Required for regulatory audit defense.',
    `application_timestamp` TIMESTAMP COMMENT 'Date and time when the dispatcher applied this exemption to the dispatch release during the authorization workflow. Captures the operational moment of invocation.',
    `approval_status` STRING COMMENT 'Status of the exemption invocation approval within the dispatch release workflow (approved, conditionally_approved, rejected, pending_review). Tracks whether the dispatchers use of this exemption was validated.',
    `condition_compliance_notes` STRING COMMENT 'Detailed notes documenting how the flight meets all conditions attached to the exemption (e.g., Aircraft equipped with required ETOPS equipment per exemption condition 3a; crew ETOPS-qualified per condition 2b).',
    `conditions_accepted_flag` BOOLEAN COMMENT 'Indicates whether the dispatcher explicitly acknowledged and accepted all conditions attached to this exemption as applicable to this specific flight. Required for regulatory compliance attestation.',
    `dispatcher_acknowledgment` STRING COMMENT 'Textual acknowledgment or attestation statement by the dispatcher confirming understanding of the exemptions conditions and applicability to this flight. May include dispatcher comments on specific condition compliance.',
    `expiry_verification_timestamp` TIMESTAMP COMMENT 'Timestamp when the dispatcher verified that the exemption was still valid (not expired) at the time of dispatch release issuance. Critical for audit trail demonstrating due diligence.',
    `invocation_sequence_number` STRING COMMENT 'Sequential order in which this exemption was applied within the dispatch release workflow. Useful for understanding dispatcher decision-making sequence during release preparation.',
    CONSTRAINT pk_dispatch_exemption_invocation PRIMARY KEY(`dispatch_exemption_invocation_id`)
) COMMENT 'This association product represents the operational invocation of regulatory exemptions and waivers within specific dispatch release documents. Each record captures when a dispatcher explicitly invokes a regulatory relief instrument to authorize a flight departure under non-standard conditions, including the dispatchers acknowledgment of conditions, verification of applicability, and compliance attestation required for FAA/EASA audit trails per 14 CFR Part 121.. Existence Justification: In airline dispatch operations, a single dispatch release can invoke multiple regulatory exemptions simultaneously (e.g., ETOPS diversion time relief + MEL extension + special airport procedure waiver), and each exemption applies to many dispatch releases over its validity period. Dispatchers actively manage which exemptions are invoked per release, documenting acknowledgment of conditions, verification of applicability, and compliance attestation to create an auditable regulatory trail required by FAA 14 CFR Part 121.687.';

CREATE OR REPLACE TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` (
    `codeshare_flight_execution_id` BIGINT COMMENT 'Unique identifier for this codeshare flight execution record. Primary key.',
    `flight_leg_id` BIGINT COMMENT 'Foreign key linking to the specific operational flight leg instance on which this codeshare agreement is executed.',
    `partnership_id` BIGINT COMMENT 'Foreign key linking to the route partnership agreement that governs this codeshare execution.',
    `codeshare_flight_number_used` STRING COMMENT 'The specific marketing carrier flight number assigned and used for this codeshare execution, including carrier code and numeric identifier. This is the flight number passengers see on their tickets.',
    `created_timestamp` TIMESTAMP COMMENT 'UTC timestamp when this codeshare flight execution record was created in the system, typically during flight planning or schedule publication.',
    `execution_status` STRING COMMENT 'Operational status of this codeshare execution instance. Tracks whether the codeshare arrangement is active for this specific flight or has been cancelled/suspended due to operational or commercial reasons.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'UTC timestamp when this codeshare flight execution record was last modified, tracking changes to settlement status, passenger counts, or operational status.',
    `partner_notification_timestamp` TIMESTAMP COMMENT 'UTC timestamp when the partner carrier was notified of operational details for this codeshare flight execution (schedule changes, delays, cancellations). Critical for partner carrier customer service operations.',
    `passenger_count` STRING COMMENT 'Number of passengers booked on this specific codeshare flight number for this flight leg execution. Used for revenue allocation and capacity management.',
    `prorate_calculation_basis` STRING COMMENT 'The method or formula used to calculate revenue allocation for this specific flight execution (e.g., IATA standard prorate, special prorate agreement percentage, block-space fixed amount). Documents how revenue_share_amount was derived.',
    `revenue_share_amount` DECIMAL(18,2) COMMENT 'The calculated revenue amount allocated to the partner carrier for this specific flight leg execution under the partnership agreement terms. Used for settlement reconciliation.',
    `seat_allocation_count` STRING COMMENT 'Number of seats allocated to the marketing carrier for this specific flight execution under block-space agreements. Null for free-flow codeshare arrangements.',
    `settlement_status` STRING COMMENT 'Current status of revenue settlement processing for this codeshare flight execution. Tracks lifecycle from initial calculation through final settlement between carriers.',
    CONSTRAINT pk_codeshare_flight_execution PRIMARY KEY(`codeshare_flight_execution_id`)
) COMMENT 'This association product represents the operational execution of a codeshare agreement on a specific flight leg. It captures the commercial partnership arrangement applied to each operated flight, including the marketing carriers flight number used, revenue allocation calculations, settlement tracking, and partner notification status. Each record links one flight_leg to one route_partnership agreement with attributes that exist only in the context of this specific codeshare execution instance.. Existence Justification: In airline codeshare operations, a single flight leg can be marketed under multiple partner carrier flight numbers simultaneously (e.g., UA123 operated by United is also sold as LH7500 by Lufthansa and AC9876 by Air Canada under different partnership agreements). Conversely, each route partnership agreement covers hundreds or thousands of flight legs over time. The business actively manages each codeshare execution instance with specific revenue calculations, settlement tracking, passenger counts, and partner notifications that belong to neither the flight leg nor the partnership agreement alone.';
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ADD CONSTRAINT `fk_flight_dispatch_release_flight_plan_id` FOREIGN KEY (`flight_plan_id`) REFERENCES `airlines_ecm`.`flight`.`flight_plan`(`flight_plan_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ADD CONSTRAINT `fk_flight_flight_plan_flight_leg_id` FOREIGN KEY (`flight_leg_id`) REFERENCES `airlines_ecm`.`flight`.`flight_leg`(`flight_leg_id`);
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ADD CONSTRAINT `fk_flight_flight_plan_scheduled_flight_id` FOREIGN KEY (`scheduled_flight_id`) REFERENCES `airlines_ecm`.`flight`.`scheduled_flight`(`scheduled_flight_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ADD CONSTRAINT `fk_flight_acars_message_flight_leg_id` FOREIGN KEY (`flight_leg_id`) REFERENCES `airlines_ecm`.`flight`.`flight_leg`(`flight_leg_id`);
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ADD CONSTRAINT `fk_flight_acars_message_oooi_event_id` FOREIGN KEY (`oooi_event_id`) REFERENCES `airlines_ecm`.`flight`.`oooi_event`(`oooi_event_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ADD CONSTRAINT `fk_flight_cancellation_flight_irop_event_id` FOREIGN KEY (`flight_irop_event_id`) REFERENCES `airlines_ecm`.`flight`.`flight_irop_event`(`flight_irop_event_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ADD CONSTRAINT `fk_flight_operational_flight_plan_dispatch_release_id` FOREIGN KEY (`dispatch_release_id`) REFERENCES `airlines_ecm`.`flight`.`dispatch_release`(`dispatch_release_id`);
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ADD CONSTRAINT `fk_flight_operational_flight_plan_flight_leg_id` FOREIGN KEY (`flight_leg_id`) REFERENCES `airlines_ecm`.`flight`.`flight_leg`(`flight_leg_id`);
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ADD CONSTRAINT `fk_flight_operational_flight_plan_flight_plan_id` FOREIGN KEY (`flight_plan_id`) REFERENCES `airlines_ecm`.`flight`.`flight_plan`(`flight_plan_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` ADD CONSTRAINT `fk_flight_weight_balance_operational_flight_plan_id` FOREIGN KEY (`operational_flight_plan_id`) REFERENCES `airlines_ecm`.`flight`.`operational_flight_plan`(`operational_flight_plan_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ADD CONSTRAINT `fk_flight_flight_performance_flight_leg_id` FOREIGN KEY (`flight_leg_id`) REFERENCES `airlines_ecm`.`flight`.`flight_leg`(`flight_leg_id`);
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ADD CONSTRAINT `fk_flight_flight_performance_flight_plan_id` FOREIGN KEY (`flight_plan_id`) REFERENCES `airlines_ecm`.`flight`.`flight_plan`(`flight_plan_id`);
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ADD CONSTRAINT `fk_flight_flight_performance_operational_flight_plan_id` FOREIGN KEY (`operational_flight_plan_id`) REFERENCES `airlines_ecm`.`flight`.`operational_flight_plan`(`operational_flight_plan_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ADD CONSTRAINT `fk_flight_flight_allotment_scheduled_flight_id` FOREIGN KEY (`scheduled_flight_id`) REFERENCES `airlines_ecm`.`flight`.`scheduled_flight`(`scheduled_flight_id`);
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ADD CONSTRAINT `fk_flight_cargo_segment_flight_leg_id` FOREIGN KEY (`flight_leg_id`) REFERENCES `airlines_ecm`.`flight`.`flight_leg`(`flight_leg_id`);
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ADD CONSTRAINT `fk_flight_dispatch_exemption_invocation_dispatch_release_id` FOREIGN KEY (`dispatch_release_id`) REFERENCES `airlines_ecm`.`flight`.`dispatch_release`(`dispatch_release_id`);
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ADD CONSTRAINT `fk_flight_codeshare_flight_execution_flight_leg_id` FOREIGN KEY (`flight_leg_id`) REFERENCES `airlines_ecm`.`flight`.`flight_leg`(`flight_leg_id`);
...
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `foreign_carrier_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Foreign Carrier Permit Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `aircraft_configuration_code` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Configuration Code');
...
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `flight_number` SET TAGS ('dbx_business_glossary_term' = 'Flight Number');
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `flight_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2}[0-9]{1,4}[A-Z]?$');
...
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `marketing_carrier_code` SET TAGS ('dbx_business_glossary_term' = 'Marketing Carrier Code');
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `marketing_carrier_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `season_code` SET TAGS ('dbx_business_glossary_term' = 'IATA Season Code');
ALTER TABLE `airlines_ecm`.`flight`.`scheduled_flight` ALTER COLUMN `season_code` SET TAGS ('dbx_value_regex' = '^(S|W)[0-9]{2}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `occurrence_id` SET TAGS ('dbx_business_glossary_term' = 'Occurrence Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `cabin_configuration_code` SET TAGS ('dbx_business_glossary_term' = 'Cabin Configuration Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `cabin_configuration_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `flight_number` SET TAGS ('dbx_business_glossary_term' = 'Flight Number');
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `flight_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,3}[0-9]{1,4}[A-Z]?$');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `operating_carrier_code` SET TAGS ('dbx_business_glossary_term' = 'Operating Carrier Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_leg` ALTER COLUMN `operating_carrier_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,3}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `dangerous_goods_id` SET TAGS ('dbx_business_glossary_term' = 'Dg Declaration Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Dispatcher Identifier (ID)');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Plan Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `operational_approval_id` SET TAGS ('dbx_business_glossary_term' = 'Operational Approval Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `additional_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Additional Fuel (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Registration Number');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_value_regex' = '^[A-Z]-[A-Z0-9]{4}$|^N[0-9]{1,5}[A-Z]{0,2}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `release_number` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Release Number');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_release` ALTER COLUMN `release_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,20}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Plan Identifier (ID)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `aircraft_type_id` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Type Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Planned By Dispatcher Identifier (ID)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `flight_leg_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Leg Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `member_id` SET TAGS ('dbx_business_glossary_term' = 'Accepted By Pilot-In-Command (PIC) Identifier (ID)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `scheduled_flight_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Identifier (ID)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `additional_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Additional Fuel (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Registration Number');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]{5,10}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `alternate_airport_1_icao` SET TAGS ('dbx_business_glossary_term' = 'Alternate Airport 1 ICAO Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `alternate_airport_1_icao` SET TAGS ('dbx_value_regex' = '^[A-Z]{4}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `alternate_airport_2_icao` SET TAGS ('dbx_business_glossary_term' = 'Alternate Airport 2 ICAO Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `alternate_airport_2_icao` SET TAGS ('dbx_value_regex' = '^[A-Z]{4}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `alternate_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Alternate Fuel (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `arrival_airport_icao` SET TAGS ('dbx_business_glossary_term' = 'Arrival Airport ICAO Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `arrival_airport_icao` SET TAGS ('dbx_value_regex' = '^[A-Z]{4}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `contingency_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Contingency Fuel (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `cost_index` SET TAGS ('dbx_business_glossary_term' = 'Cost Index (CI)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `cruise_altitude_ft` SET TAGS ('dbx_business_glossary_term' = 'Cruise Altitude (Feet)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `cruise_speed_kts` SET TAGS ('dbx_business_glossary_term' = 'Cruise Speed (Knots)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `departure_airport_icao` SET TAGS ('dbx_business_glossary_term' = 'Departure Airport ICAO Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `departure_airport_icao` SET TAGS ('dbx_value_regex' = '^[A-Z]{4}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `etops_entry_point` SET TAGS ('dbx_business_glossary_term' = 'Extended-range Twin-engine Operational Performance Standards (ETOPS) Entry Point');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `etops_exit_point` SET TAGS ('dbx_business_glossary_term' = 'Extended-range Twin-engine Operational Performance Standards (ETOPS) Exit Point');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `etops_qualified_flag` SET TAGS ('dbx_business_glossary_term' = 'Extended-range Twin-engine Operational Performance Standards (ETOPS) Qualified Flag');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `filed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Filed Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `final_reserve_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Final Reserve Fuel (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `landing_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Landing Weight (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `operational_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Operational Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `plan_status` SET TAGS ('dbx_business_glossary_term' = 'Flight Plan Status');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `plan_status` SET TAGS ('dbx_value_regex' = 'filed|amended|operational|cancelled|rejected|superseded');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `planned_block_hours` SET TAGS ('dbx_business_glossary_term' = 'Planned Block Hours');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `planned_flight_time_hours` SET TAGS ('dbx_business_glossary_term' = 'Planned Flight Time (Hours)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `planned_taxi_in_minutes` SET TAGS ('dbx_business_glossary_term' = 'Planned Taxi-In Time (Minutes)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `planned_taxi_out_minutes` SET TAGS ('dbx_business_glossary_term' = 'Planned Taxi-Out Time (Minutes)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `route_of_flight` SET TAGS ('dbx_business_glossary_term' = 'Route of Flight');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `step_climb_schedule` SET TAGS ('dbx_business_glossary_term' = 'Step Climb Schedule');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `takeoff_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Takeoff Weight (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `total_fuel_required_kg` SET TAGS ('dbx_business_glossary_term' = 'Total Fuel Required (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `trip_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Trip Fuel (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Plan Version Number');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `wind_data_source` SET TAGS ('dbx_business_glossary_term' = 'Wind Data Source');
ALTER TABLE `airlines_ecm`.`flight`.`flight_plan` ALTER COLUMN `zero_fuel_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Zero Fuel Weight (Kilograms)');
ALTER TABLE `airlines_ecm`.`flight`.`oooi_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`oooi_event` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`fuel_uplift` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`fuel_uplift` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Fueling Vendor Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `acars_message_id` SET TAGS ('dbx_business_glossary_term' = 'ACARS (Aircraft Communications Addressing and Reporting System) Message ID');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `flight_leg_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Leg ID');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `oooi_event_id` SET TAGS ('dbx_business_glossary_term' = 'Oooi Event Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `acknowledgement_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgement Required Flag');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `acknowledgement_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgement Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Registration Number');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{5,7}$');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `altitude_feet` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Altitude in Feet');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `arrival_airport_code` SET TAGS ('dbx_business_glossary_term' = 'Arrival Airport IATA Code');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `arrival_airport_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `block_number` SET TAGS ('dbx_business_glossary_term' = 'ACARS Message Block Number');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `block_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{1,3}$');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `data_link_service_provider` SET TAGS ('dbx_business_glossary_term' = 'Data Link Service Provider');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `data_link_service_provider` SET TAGS ('dbx_value_regex' = 'SITA|ARINC|SATCOM|VHF|HF');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `departure_airport_code` SET TAGS ('dbx_business_glossary_term' = 'Departure Airport IATA Code');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `departure_airport_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `error_code` SET TAGS ('dbx_business_glossary_term' = 'Transmission Error Code');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `flight_phase` SET TAGS ('dbx_business_glossary_term' = 'Flight Phase');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `ground_speed_knots` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Ground Speed in Knots');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_content` SET TAGS ('dbx_business_glossary_term' = 'ACARS Message Content Payload');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_direction` SET TAGS ('dbx_business_glossary_term' = 'Message Direction');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_direction` SET TAGS ('dbx_value_regex' = 'uplink|downlink');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_label` SET TAGS ('dbx_business_glossary_term' = 'ACARS Message Label');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_label` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2}$');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Message Sequence Number');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_size_bytes` SET TAGS ('dbx_business_glossary_term' = 'Message Size in Bytes');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `message_type` SET TAGS ('dbx_business_glossary_term' = 'ACARS Message Type');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `oooi_event_type` SET TAGS ('dbx_business_glossary_term' = 'OOOI (Out-Off-On-In) Event Type');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `oooi_event_type` SET TAGS ('dbx_value_regex' = 'OUT|OFF|ON|IN');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `position_latitude` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Position Latitude');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `position_latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `position_latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `position_longitude` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Position Longitude');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `position_longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `position_longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Message Priority Level');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'urgent|high|normal|low');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `processing_status` SET TAGS ('dbx_business_glossary_term' = 'Message Processing Status');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `processing_status` SET TAGS ('dbx_value_regex' = 'received|processed|acknowledged|failed|pending');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `receipt_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Message Receipt Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `retry_count` SET TAGS ('dbx_business_glossary_term' = 'Message Retry Count');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `transmission_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Message Transmission Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`acars_message` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` SET TAGS ('dbx_subdomain' = 'disruption_management');
...
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` ALTER COLUMN `consumer_protection_case_id` SET TAGS ('dbx_business_glossary_term' = 'Consumer Protection Case Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Delay Verified By User Identifier (ID)');
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` ALTER COLUMN `secondary_delay_code` SET TAGS ('dbx_business_glossary_term' = 'Secondary Delay Code');
ALTER TABLE `airlines_ecm`.`flight`.`delay_record` ALTER COLUMN `secondary_delay_code` SET TAGS ('dbx_value_regex' = '^[0-9]{2}$');
ALTER TABLE `airlines_ecm`.`flight`.`diversion` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`diversion` SET TAGS ('dbx_subdomain' = 'disruption_management');
...
ALTER TABLE `airlines_ecm`.`flight`.`diversion` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Decision Authority Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`diversion` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`diversion` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`flight`.`diversion` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` SET TAGS ('dbx_subdomain' = 'disruption_management');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `flight_irop_event_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Irregular Operations (IROP) Event ID');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `station_id` SET TAGS ('dbx_business_glossary_term' = 'Affected Station Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Event Coordinator Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `actual_passengers_affected` SET TAGS ('dbx_business_glossary_term' = 'Actual Passengers Affected');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `actual_total_delay_minutes` SET TAGS ('dbx_business_glossary_term' = 'Actual Total Delay Minutes');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `additional_affected_stations` SET TAGS ('dbx_business_glossary_term' = 'Additional Affected Stations');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `atc_coordination_notes` SET TAGS ('dbx_business_glossary_term' = 'Air Traffic Control (ATC) Coordination Notes');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `cdm_session_reference` SET TAGS ('dbx_business_glossary_term' = 'Collaborative Decision Making (CDM) Session Reference');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `estimated_financial_impact_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Financial Impact Amount');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `estimated_financial_impact_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `estimated_passengers_affected` SET TAGS ('dbx_business_glossary_term' = 'Estimated Passengers Affected');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `estimated_total_delay_minutes` SET TAGS ('dbx_business_glossary_term' = 'Estimated Total Delay Minutes');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_code` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Event Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{3,10}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_description` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Event Description');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Event End Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Event Start Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_status` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Event Status');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_status` SET TAGS ('dbx_value_regex' = 'active|monitoring|resolved|closed');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_type` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Event Type');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `event_type` SET TAGS ('dbx_value_regex' = 'weather|atc_ground_stop|airport_closure|aircraft_aog|crew_shortage|security');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `flights_cancelled_count` SET TAGS ('dbx_business_glossary_term' = 'Flights Cancelled Count');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `flights_delayed_count` SET TAGS ('dbx_business_glossary_term' = 'Flights Delayed Count');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `flights_diverted_count` SET TAGS ('dbx_business_glossary_term' = 'Flights Diverted Count');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `notam_reference` SET TAGS ('dbx_business_glossary_term' = 'Notice to Air Missions (NOTAM) Reference');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `passenger_compensation_triggered_flag` SET TAGS ('dbx_business_glossary_term' = 'Passenger Compensation Triggered Flag');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `passenger_compensation_triggered_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `passenger_compensation_triggered_flag` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `recovery_plan_description` SET TAGS ('dbx_business_glossary_term' = 'Recovery Plan Description');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `recovery_plan_status` SET TAGS ('dbx_business_glossary_term' = 'Recovery Plan Status');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `recovery_plan_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|approved|executing|completed');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `regulatory_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Required Flag');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `regulatory_notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `responsible_department` SET TAGS ('dbx_business_glossary_term' = 'Responsible Department');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `root_cause_description` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Description');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Severity Level');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `severity_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `airlines_ecm`.`flight`.`flight_irop_event` ALTER COLUMN `weather_condition_summary` SET TAGS ('dbx_business_glossary_term' = 'Weather Condition Summary');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` SET TAGS ('dbx_subdomain' = 'disruption_management');
...
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Authority Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `consumer_protection_case_id` SET TAGS ('dbx_business_glossary_term' = 'Consumer Protection Case Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `flight_irop_event_id` SET TAGS ('dbx_business_glossary_term' = 'IROP (Irregular Operations) Event ID');
...
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `maintenance_defect_code` SET TAGS ('dbx_business_glossary_term' = 'Maintenance Defect Code');
ALTER TABLE `airlines_ecm`.`flight`.`cancellation` ALTER COLUMN `maintenance_defect_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,6}$');
...
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `operational_flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Operational Flight Plan (OFP) ID');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `dispatch_release_id` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Release Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `flight_leg_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Leg ID');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Plan Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `operational_approval_id` SET TAGS ('dbx_business_glossary_term' = 'Operational Approval Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `operational_standard_id` SET TAGS ('dbx_business_glossary_term' = 'Turnaround Standard Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Dispatcher ID');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `additional_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Additional Fuel Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `alternate_airport_1_code` SET TAGS ('dbx_business_glossary_term' = 'Alternate Airport 1 Code');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `alternate_airport_1_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3,4}$');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `alternate_airport_2_code` SET TAGS ('dbx_business_glossary_term' = 'Alternate Airport 2 Code');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `alternate_airport_2_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3,4}$');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `alternate_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Alternate Fuel Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `contingency_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Contingency Fuel Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `cost_index` SET TAGS ('dbx_business_glossary_term' = 'Cost Index (CI)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `crew_accepted_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Crew Accepted Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `estimated_block_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Estimated Block Time Minutes');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `estimated_time_enroute_minutes` SET TAGS ('dbx_business_glossary_term' = 'Estimated Time Enroute (ETE) Minutes');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `etops_alternate_code` SET TAGS ('dbx_business_glossary_term' = 'Extended-range Twin-engine Operational Performance Standards (ETOPS) Alternate Code');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `etops_alternate_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3,4}$');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `etops_approval_indicator` SET TAGS ('dbx_business_glossary_term' = 'Extended-range Twin-engine Operational Performance Standards (ETOPS) Approval Indicator');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `etops_rule_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Extended-range Twin-engine Operational Performance Standards (ETOPS) Rule Time Minutes');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `final_reserve_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Final Reserve Fuel Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `flight_planning_system_code` SET TAGS ('dbx_business_glossary_term' = 'Flight Planning System Code');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `minimum_equipment_list_items` SET TAGS ('dbx_business_glossary_term' = 'Minimum Equipment List (MEL) Items');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `notam_summary` SET TAGS ('dbx_business_glossary_term' = 'Notice to Air Missions (NOTAM) Summary');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `ofp_version_number` SET TAGS ('dbx_business_glossary_term' = 'Operational Flight Plan (OFP) Version Number');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `plan_generated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Plan Generated Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `plan_status` SET TAGS ('dbx_business_glossary_term' = 'Plan Status');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `planned_cruise_altitude_ft` SET TAGS ('dbx_business_glossary_term' = 'Planned Cruise Altitude (Feet)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `planned_cruise_speed_mach` SET TAGS ('dbx_business_glossary_term' = 'Planned Cruise Speed (Mach)');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `planned_landing_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Planned Landing Weight Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `planned_takeoff_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Planned Takeoff Weight Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `record_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `route_description` SET TAGS ('dbx_business_glossary_term' = 'Route Description');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `step_climb_profile` SET TAGS ('dbx_business_glossary_term' = 'Step Climb Profile');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `taxi_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Taxi Fuel Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `total_fuel_required_kg` SET TAGS ('dbx_business_glossary_term' = 'Total Fuel Required Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `trip_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Trip Fuel Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `weather_deviation_indicator` SET TAGS ('dbx_business_glossary_term' = 'Weather Deviation Indicator');
ALTER TABLE `airlines_ecm`.`flight`.`operational_flight_plan` ALTER COLUMN `zero_fuel_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Zero Fuel Weight (ZFW) Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` ALTER COLUMN `operational_flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Operational Flight Plan Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` ALTER COLUMN `loadmaster_license_number` SET TAGS ('dbx_business_glossary_term' = 'Loadmaster License Number');
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` ALTER COLUMN `loadmaster_license_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`weight_balance` ALTER COLUMN `loadmaster_name` SET TAGS ('dbx_business_glossary_term' = 'Loadmaster Name');
...
ALTER TABLE `airlines_ecm`.`flight`.`status` SET TAGS ('dbx_subdomain' = 'operational_execution');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `flight_performance_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Performance ID');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `aircraft_type_id` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Type Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `emissions_report_id` SET TAGS ('dbx_business_glossary_term' = 'Emissions Report Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `flight_leg_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Leg ID');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Plan Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `operational_flight_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Operational Flight Plan Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `actual_block_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Actual Block Time (minutes)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `actual_fuel_burn_kg` SET TAGS ('dbx_business_glossary_term' = 'Actual Fuel Burn (kg)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Registration Number');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `aircraft_registration` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]{5,10}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `anomaly_description` SET TAGS ('dbx_business_glossary_term' = 'Anomaly Description');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `anomaly_flag` SET TAGS ('dbx_business_glossary_term' = 'Performance Anomaly Flag');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `atc_delay_minutes` SET TAGS ('dbx_business_glossary_term' = 'Air Traffic Control (ATC) Delay (minutes)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `average_cruise_speed_mach` SET TAGS ('dbx_business_glossary_term' = 'Average Cruise Speed (Mach)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `average_ground_speed_knots` SET TAGS ('dbx_business_glossary_term' = 'Average Ground Speed (knots)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `block_time_variance_minutes` SET TAGS ('dbx_business_glossary_term' = 'Block Time Variance (minutes)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `co2_emissions_kg` SET TAGS ('dbx_business_glossary_term' = 'Carbon Dioxide (CO2) Emissions (kg)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `corsia_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Carbon Offsetting and Reduction Scheme for International Aviation (CORSIA) Eligible Flag');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `cruise_altitude_feet` SET TAGS ('dbx_business_glossary_term' = 'Cruise Altitude (feet)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `data_source` SET TAGS ('dbx_business_glossary_term' = 'Data Source');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `data_source` SET TAGS ('dbx_value_regex' = 'FDR|FMS|ACARS|MANUAL');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `destination_airport_code` SET TAGS ('dbx_business_glossary_term' = 'Destination Airport Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `destination_airport_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `engine_1_avg_egt_celsius` SET TAGS ('dbx_business_glossary_term' = 'Engine 1 Average Exhaust Gas Temperature (EGT) (Celsius)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `engine_1_avg_fuel_flow_kg_per_hour` SET TAGS ('dbx_business_glossary_term' = 'Engine 1 Average Fuel Flow (kg/hour)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `engine_1_avg_n1_percent` SET TAGS ('dbx_business_glossary_term' = 'Engine 1 Average N1 (percent)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `engine_2_avg_egt_celsius` SET TAGS ('dbx_business_glossary_term' = 'Engine 2 Average Exhaust Gas Temperature (EGT) (Celsius)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `engine_2_avg_fuel_flow_kg_per_hour` SET TAGS ('dbx_business_glossary_term' = 'Engine 2 Average Fuel Flow (kg/hour)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `engine_2_avg_n1_percent` SET TAGS ('dbx_business_glossary_term' = 'Engine 2 Average N1 (percent)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `flight_number` SET TAGS ('dbx_business_glossary_term' = 'Flight Number');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `flight_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,3}[0-9]{1,4}[A-Z]?$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `flight_phase_coverage` SET TAGS ('dbx_business_glossary_term' = 'Flight Phase Coverage');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `fuel_efficiency_index` SET TAGS ('dbx_business_glossary_term' = 'Fuel Efficiency Index');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `fuel_variance_kg` SET TAGS ('dbx_business_glossary_term' = 'Fuel Variance (kg)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `operating_date` SET TAGS ('dbx_business_glossary_term' = 'Operating Date');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `origin_airport_code` SET TAGS ('dbx_business_glossary_term' = 'Origin Airport Code');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `origin_airport_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `performance_category` SET TAGS ('dbx_business_glossary_term' = 'Performance Category');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `performance_category` SET TAGS ('dbx_value_regex' = 'EXCELLENT|GOOD|ACCEPTABLE|BELOW_TARGET|POOR');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `planned_block_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Planned Block Time (minutes)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `planned_cruise_altitude_feet` SET TAGS ('dbx_business_glossary_term' = 'Planned Cruise Altitude (feet)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `planned_fuel_kg` SET TAGS ('dbx_business_glossary_term' = 'Planned Fuel (kg)');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `record_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `remarks` SET TAGS ('dbx_business_glossary_term' = 'Performance Remarks');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `weather_impact_flag` SET TAGS ('dbx_business_glossary_term' = 'Weather Impact Flag');
ALTER TABLE `airlines_ecm`.`flight`.`flight_performance` ALTER COLUMN `wind_component_knots` SET TAGS ('dbx_business_glossary_term' = 'Wind Component (knots)');
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` SET TAGS ('dbx_subdomain' = 'commercial_integration');
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` SET TAGS ('dbx_association_edges' = 'flight.flight_leg,passenger.profile');
...
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` ALTER COLUMN `booking_class` SET TAGS ('dbx_business_glossary_term' = 'Booking Class');
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` ALTER COLUMN `cabin_class` SET TAGS ('dbx_business_glossary_term' = 'Cabin Class');
...
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` ALTER COLUMN `fare_basis` SET TAGS ('dbx_business_glossary_term' = 'Fare Basis Code');
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` ALTER COLUMN `seat_assignment` SET TAGS ('dbx_business_glossary_term' = 'Seat Assignment');
...
ALTER TABLE `airlines_ecm`.`flight`.`booking_segment` ALTER COLUMN `ticket_number` SET TAGS ('dbx_business_glossary_term' = 'Ticket Number');
...
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` SET TAGS ('dbx_subdomain' = 'commercial_integration');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` SET TAGS ('dbx_association_edges' = 'flight.scheduled_flight,cargo.freight_forwarder');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `flight_allotment_id` SET TAGS ('dbx_business_glossary_term' = 'flight_allotment Identifier');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `cargo_allotment_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Allotment Identifier');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `freight_forwarder_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Allotment - Freight Forwarder Id');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `scheduled_flight_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Allotment - Scheduled Flight Id');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `allocated_volume_cbm` SET TAGS ('dbx_business_glossary_term' = 'Allocated Volume in Cubic Meters');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `allocated_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Allocated Weight in Kilograms');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `allotment_status` SET TAGS ('dbx_business_glossary_term' = 'Allotment Status');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `created_date` SET TAGS ('dbx_business_glossary_term' = 'Allotment Created Date');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `last_modified_date` SET TAGS ('dbx_business_glossary_term' = 'Allotment Last Modified Date');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `minimum_utilization_threshold_pct` SET TAGS ('dbx_business_glossary_term' = 'Minimum Utilization Threshold Percentage');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `rate_class` SET TAGS ('dbx_business_glossary_term' = 'Rate Class');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `validity_end_date` SET TAGS ('dbx_business_glossary_term' = 'Allotment Validity End Date');
ALTER TABLE `airlines_ecm`.`flight`.`flight_allotment` ALTER COLUMN `validity_start_date` SET TAGS ('dbx_business_glossary_term' = 'Allotment Validity Start Date');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` SET TAGS ('dbx_subdomain' = 'commercial_integration');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` SET TAGS ('dbx_association_edges' = 'flight.flight_leg,cargo.awb');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `cargo_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Segment ID');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `awb_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Segment - Awb Id');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `flight_leg_id` SET TAGS ('dbx_business_glossary_term' = 'Cargo Segment - Flight Leg Id');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `compartment_code` SET TAGS ('dbx_business_glossary_term' = 'Aircraft Compartment Code');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `handling_station_code` SET TAGS ('dbx_business_glossary_term' = 'Handling Station Code');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `loaded_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Load Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `loading_priority` SET TAGS ('dbx_business_glossary_term' = 'Loading Priority');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `manifested_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Manifest Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `offloaded_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Offload Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `proration_factor` SET TAGS ('dbx_business_glossary_term' = 'Proration Factor');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `segment_chargeable_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Segment Chargeable Weight');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `segment_sequence` SET TAGS ('dbx_business_glossary_term' = 'Segment Sequence Number');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `segment_status` SET TAGS ('dbx_business_glossary_term' = 'Segment Status');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `segment_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Segment Weight');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `transfer_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Transfer Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`cargo_segment` ALTER COLUMN `uld_number` SET TAGS ('dbx_business_glossary_term' = 'Unit Load Device Number');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` SET TAGS ('dbx_subdomain' = 'operational_execution');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` SET TAGS ('dbx_association_edges' = 'flight.dispatch_release,compliance.exemption_waiver');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `dispatch_exemption_invocation_id` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Exemption Invocation ID');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `dispatch_release_id` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Exemption Invocation - Dispatch Release Id');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `exemption_waiver_id` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Exemption Invocation - Exemption Waiver Id');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `applicability_justification` SET TAGS ('dbx_business_glossary_term' = 'Applicability Justification');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `application_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Application Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `condition_compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Condition Compliance Notes');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `conditions_accepted_flag` SET TAGS ('dbx_business_glossary_term' = 'Conditions Accepted Flag');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `dispatcher_acknowledgment` SET TAGS ('dbx_business_glossary_term' = 'Dispatcher Acknowledgment');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `expiry_verification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Expiry Verification Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`dispatch_exemption_invocation` ALTER COLUMN `invocation_sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Invocation Sequence Number');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` SET TAGS ('dbx_subdomain' = 'commercial_integration');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` SET TAGS ('dbx_association_edges' = 'flight.flight_leg,route.route_partnership');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `codeshare_flight_execution_id` SET TAGS ('dbx_business_glossary_term' = 'Codeshare Flight Execution ID');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `flight_leg_id` SET TAGS ('dbx_business_glossary_term' = 'Codeshare Flight Execution - Flight Leg Id');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `partnership_id` SET TAGS ('dbx_business_glossary_term' = 'Codeshare Flight Execution - Route Codeshare Agreement Id');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `codeshare_flight_number_used` SET TAGS ('dbx_business_glossary_term' = 'Codeshare Flight Number Used');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `execution_status` SET TAGS ('dbx_business_glossary_term' = 'Codeshare Execution Status');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `partner_notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Partner Notification Timestamp');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `passenger_count` SET TAGS ('dbx_business_glossary_term' = 'Codeshare Passenger Count');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `prorate_calculation_basis` SET TAGS ('dbx_business_glossary_term' = 'Prorate Calculation Basis');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `revenue_share_amount` SET TAGS ('dbx_business_glossary_term' = 'Revenue Share Amount');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `revenue_share_amount` SET TAGS ('dbx_financial' = 'true');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `seat_allocation_count` SET TAGS ('dbx_business_glossary_term' = 'Seat Allocation Count');
ALTER TABLE `airlines_ecm`.`flight`.`codeshare_flight_execution` ALTER COLUMN `settlement_status` SET TAGS ('dbx_business_glossary_term' = 'Settlement Status');
