-- Schema for Domain: finance | Business: Airlines | Version: v1_ecm
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`journal_entry` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the customer master record for accounts receivable postings (e.g., interline billing receivables, corporate travel account receivables, cargo customer invoices). Populated for document types involving customer transactions.',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor/supplier master record for accounts payable postings (e.g., fuel suppliers, MRO providers, catering vendors, airport authorities, ACMI lessors). Populated for document types involving vendor transactions.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`cost_centre` (
    ...
    `employee_id` BIGINT COMMENT 'Employee identifier of the manager accountable for this cost centres budget, expenditure, and performance. Used for budget approval workflows, variance reporting escalation, and management accountability in SAP CO. Links to HR employee master.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`accounts_payable` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: AP invoices for employee expenses (training, relocation, travel advances) require employee linkage for expense management, reimbursement tracking, and payroll integration. Role prefix distinguishes fr',
    `goods_receipt_id` BIGINT COMMENT 'Foreign key linking to procurement.goods_receipt. Business justification: Three-way match (PO-GR-Invoice) requires AP to reference specific goods receipt for invoice verification, payment release authorization, and procurement compliance. Mandatory for accounts payable proc',
    ...
    `purchase_order_id` BIGINT COMMENT 'Reference to the originating purchase order document against which this vendor invoice is matched in the three-way match process (PO, goods receipt, invoice). Maps to SAP MM purchasing document.',
    `repair_order_id` BIGINT COMMENT 'Foreign key linking to procurement.repair_order. Business justification: MRO repair invoices must reference the originating repair order for cost validation, warranty claim processing, maintenance cost tracking, and airworthiness compliance. Essential for MRO financial con',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor master record representing the supplier of goods or services (e.g., MRO supplier, fuel vendor, airport authority, GDS provider, catering contractor, ground handling agent). Maps to SAP Vendor Master (LFA1).',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`accounts_receivable` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Staff travel tickets (ID90, ZED fares) sold to employees create AR entries requiring employee linkage for payroll deduction processing, travel entitlement tracking, and revenue recognition per airline',
    `ticket_id` BIGINT COMMENT 'Foreign key linking to revenue.ticket. Business justification: Accounts receivable for ticket sales must be linked to the ticket for revenue recognition and cash collection tracking. This FK enables reconciliation of ticket revenue with cash receipts.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`budget_plan` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the senior manager, CFO, or board member who approved this budget plan. Supports segregation of duties and audit trail requirements.',
    ...
    `primary_budget_employee_id` BIGINT COMMENT 'Reference to the employee or manager responsible for owning and managing this budget plan. Supports accountability tracking and approval workflow routing in SAP BPC.',
    `procurement_category_id` BIGINT COMMENT 'Foreign key linking to procurement.procurement_category. Business justification: Budget planning allocates spend targets by procurement category for category management, variance reporting, and strategic sourcing alignment. Core to procurement financial planning and category strat',
    `profit_centre_id` BIGINT COMMENT 'Reference to the SAP CO-PA profit centre associated with this budget plan. Supports profitability analysis by route, fleet type, or business unit.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` (
    ...
    `flight_leg_assignment_id` BIGINT COMMENT 'Foreign key linking to crew.flight_leg_assignment. Business justification: Fuel costs attributed to crew assignments for pilot fuel efficiency tracking, performance evaluation, and fuel conservation KPIs. Real operational metric used in pilot performance reviews and fuel man',
    ...
    `fuel_contract_id` BIGINT COMMENT 'Foreign key linking to procurement.fuel_contract. Business justification: Fuel cost accounting requires linking to the pricing contract for variance analysis, invoice reconciliation, and hedge effectiveness testing. Essential for fuel cost management and financial reporting',
    ...
    `fuel_uplift_order_id` BIGINT COMMENT 'Foreign key linking to procurement.fuel_uplift_order. Business justification: Fuel cost accounting must reconcile to operational fuel uplift orders for quantity/price variance analysis, dispute resolution, and invoice-to-delivery matching. Critical for fuel cost control and sup',
    ...
    `hedge_contract_id` BIGINT COMMENT 'Foreign key linking to finance.hedge_contract. Business justification: Each fuel cost transaction can be linked to ONE hedge contract for fuel price hedging. The fuel_cost_transaction table has hedging_contract_reference (STRING) which should be replaced with a FK to the',
    ...
    `supplier_invoice_number` STRING COMMENT 'Invoice number issued by the fuel supplier or into-plane agent for this uplift. Used for three-way matching (PO / goods receipt / invoice) in SAP S/4HANA MM/FI and for dispute resolution.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`lease_contract` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Lease contracts (aircraft, facilities, equipment) managed by specific employees require relationship manager tracking for accountability, renewal negotiations, and contract administration per airline ',
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to procurement.vendor. Business justification: Lease contracts must link to vendor master for lessor payment terms, performance tracking, vendor consolidation reporting, and IFRS 16 disclosure requirements. Essential for lease accounting and vendo',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`lease_liability_schedule` (
    ...
    `journal_entry_document_number` STRING COMMENT 'SAP S/4HANA FI accounting document number assigned when the IFRS 16 journal entry is posted. Provides a direct audit trail from the amortisation schedule row to the posted ledger document.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`interline_billing` (
    ...
    `flight_date` DATE COMMENT 'Scheduled departure date of the flight segment associated with this interline billing transaction. Key date for revenue recognition and settlement timing.',
    `flight_number` STRING COMMENT 'Flight number associated with the interline segment being billed. Combines airline code and numeric flight identifier.. Valid values are `^[A-Z0-9]{2}[0-9]{1,4}$`',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`treasury_position` (
    `treasury_position_id` BIGINT COMMENT 'Unique surrogate identifier for each treasury cash and liquidity position record in the silver layer lakehouse. Primary key for this data product.',
    `cash_pool_id` BIGINT COMMENT 'Identifier of the notional or physical cash pool to which this bank account belongs. Supports zero-balance sweeping, notional pooling, and group-level liquidity optimisation across legal entities.',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Each treasury position belongs to ONE legal entity (company code). The treasury_position table has legal_entity_code (STRING) which should be replaced with a FK to the company_code master. Treasury po',
    `cost_centre_id` BIGINT COMMENT 'Foreign key linking to finance.cost_centre. Business justification: Each treasury position can be assigned to ONE cost center for cost allocation. The treasury_position table has cost_centre_code (STRING) which should be replaced with a FK to the cost_centre master.',
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Each treasury position is linked to ONE GL account. The treasury_position table has gl_account_code (STRING) which should be replaced with a FK to the general_ledger master. This links treasury positi',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Treasury positions managed by specific treasury officers require accountability tracking for cash management, investment decisions, and regulatory compliance (SOX, internal controls) per airline treas',
    `balance_amount` DECIMAL(18,2) COMMENT 'The gross cash or investment balance in the transaction currency for this position record. Positive values represent assets/inflows; negative values represent liabilities/outflows or overdraft positions. Core monetary field for daily cash management.',
    `balance_amount_usd` DECIMAL(18,2) COMMENT 'The balance amount translated to US Dollars (USD) using the applicable FX rate on the position date. Used for consolidated group-level cash reporting, CASK analysis, and DOT financial reporting in USD.',
    `balance_type` STRING COMMENT 'Classifies the nature of the balance recorded: actual (confirmed bank balance), forecast (projected inflow/outflow), committed (contractually obligated but not yet settled), projected (model-driven estimate), opening (start-of-day balance), closing (end-of-day balance). Supports daily cash management and IROP contingency liquidity planning.. Valid values are `actual|forecast|committed|projected|opening|closing`',
    `bank_account_iban` STRING COMMENT 'The International Bank Account Number (IBAN) for the bank account associated with this position, where applicable (primarily European and SEPA-zone accounts). Classified as confidential financial data.. Valid values are `^[A-Z]{2}[0-9]{2}[A-Z0-9]{1,30}$`',
    `bank_account_name` STRING COMMENT 'The descriptive name of the bank account as registered with the counterparty bank (e.g., Airlines Operating Account USD - JPMorgan NYC). Used for human-readable identification in cash management dashboards.',
    `bank_account_number` STRING COMMENT 'The bank account number associated with this treasury position. Classified as confidential financial data. Used for bank account reconciliation and cash pooling identification.',
    `bank_statement_reference` STRING COMMENT 'The bank-assigned statement or transaction reference number from the SWIFT MT940 or ISO 20022 camt.053 bank statement. Used for bank account reconciliation matching in SAP S/4HANA FI-BL.',
    `bsp_settlement_period` STRING COMMENT 'The IATA BSP or ARC settlement period (YYYY-MM format) to which this interline or agency settlement cash flow belongs. Populated only when interline_settlement_flag is True. Used for BSP reconciliation and interline billing dispute resolution.. Valid values are `^[0-9]{4}-[0-9]{2}$`',
    `counterparty_bank_code` STRING COMMENT 'The BIC (Bank Identifier Code) / SWIFT code of the counterparty bank holding this position. Used for FX exposure reporting, counterparty credit risk monitoring, and debt covenant compliance.',
    `counterparty_bank_name` STRING COMMENT 'Full legal name of the counterparty bank or financial institution holding this treasury position (e.g., JPMorgan Chase Bank N.A.). Used in FX exposure and counterparty risk reports.',
    `country_code` STRING COMMENT 'Three-letter ISO 3166-1 alpha-3 country code of the jurisdiction where the bank account is domiciled (e.g., USA, GBR, DEU). Used for regulatory reporting, capital controls monitoring, and country-level FX exposure aggregation.. Valid values are `^[A-Z]{3}$`',
    `covenant_breach_flag` BOOLEAN COMMENT 'Indicates whether the current balance_amount is below the covenant_threshold_amount, triggering a potential debt covenant breach. True = covenant breach condition; False = compliant. Drives automated treasury alerts.',
    `covenant_threshold_amount` DECIMAL(18,2) COMMENT 'The minimum cash or liquidity balance required under debt covenant agreements with lenders, in the transaction currency. Used for debt covenant monitoring and breach early-warning reporting. Null if no covenant applies to this account.',
    `created_timestamp` TIMESTAMP COMMENT 'The timestamp when this treasury position record was first created in the data platform, in ISO 8601 format (yyyy-MM-ddTHH:mm:ss.SSSXXX). Supports audit trail and SOX compliance.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the denomination of this treasury position (e.g., USD, EUR, GBP). Drives FX exposure calculation and multi-currency cash reporting.. Valid values are `^[A-Z]{3}$`',
    `facility_limit_amount` DECIMAL(18,2) COMMENT 'The total approved credit limit of the bank facility (revolving credit line, overdraft facility) in the transaction currency. Used for bank facility utilisation monitoring and debt covenant compliance reporting.',
    `facility_utilised_amount` DECIMAL(18,2) COMMENT 'The amount currently drawn down against the bank facility limit on the position date. The difference between facility_limit_amount and this field represents available headroom. Used for IROP contingency liquidity planning.',
    `fx_rate` DECIMAL(18,2) COMMENT 'The exchange rate applied to translate the position currency to USD on the position date. Sourced from the treasury systems daily FX rate feed. Used for FX exposure reporting and IAS 21 translation.',
    `fx_rate_type` STRING COMMENT 'Identifies the type of FX rate used for translation: spot (real-time market rate), average (period average rate), closing (period-end rate), budget (planning rate), hedge (contractual hedge rate). Determines the accounting treatment under IAS 21.. Valid values are `spot|average|closing|budget|hedge`',
    `hedge_designation` STRING COMMENT 'Indicates whether this position is designated as a hedging instrument under IFRS 9 hedge accounting: fair_value_hedge, cash_flow_hedge, net_investment_hedge, or not_designated. Drives hedge effectiveness testing and P&L reclassification.. Valid values are `fair_value_hedge|cash_flow_hedge|net_investment_hedge|not_designated`',
    `ifrs16_lease_flag` BOOLEAN COMMENT 'Indicates whether this treasury position relates to an IFRS 16 lease liability payment obligation (e.g., aircraft dry lease or wet lease financing). True = IFRS 16 lease-related cash flow; False = non-lease treasury position.',
    `instrument_type` STRING COMMENT 'The specific financial instrument type underlying this treasury position. [ENUM-REF-CANDIDATE: demand_deposit|term_deposit|money_market_fund|commercial_paper|treasury_bill|revolving_credit|overdraft|certificate_of_deposit|repo|fx_forward — promote to reference product]',
    `interest_rate` DECIMAL(18,2) COMMENT 'The annual interest rate (as a decimal, e.g., 0.045 = 4.5%) applicable to this treasury position for time deposits, money market funds, or revolving credit facilities. Used for interest income/expense accrual and yield analysis.',
    `interline_settlement_flag` BOOLEAN COMMENT 'Indicates whether this treasury position is associated with an interline billing settlement (IATA BSP or ARC settlement cycle). True = interline settlement cash flow; False = standard treasury position. Supports interline prorate reconciliation.',
    `irop_reserve_flag` BOOLEAN COMMENT 'Indicates whether this treasury position is designated as an IROP (Irregular Operations) contingency liquidity reserve. True = reserved for IROP events (e.g., mass rebooking, hotel accommodation, compensation payouts); False = general operating liquidity.',
    `liquidity_category` STRING COMMENT 'Classifies the type of liquid asset or instrument: cash_and_equivalents (demand deposits), money_market_fund, commercial_paper, time_deposit, bank_facility (revolving credit facility drawdown), restricted_cash (escrowed or pledged funds). Aligns with IAS 7 cash and cash equivalents classification.. Valid values are `cash_and_equivalents|money_market_fund|commercial_paper|time_deposit|bank_facility|restricted_cash`',
    `maturity_date` DATE COMMENT 'The date on which a short-term investment (time deposit, commercial paper, money market fund) matures and principal is returned. Null for demand deposits and revolving facilities. Critical for maturity profile and liquidity ladder reporting.',
    `notes` STRING COMMENT 'Free-text notes or commentary entered by the treasury analyst regarding this position record (e.g., reason for manual adjustment, covenant waiver details, IROP reserve activation rationale). Supports audit trail and operational transparency.',
    `position_date` DATE COMMENT 'The business date for which this cash or liquidity position is recorded. Used as the primary time dimension for daily cash management, bank reconciliation, and FX exposure reporting.',
    `position_reference` STRING COMMENT 'Externally-known business identifier for the treasury position record, sourced from SAP S/4HANA FI-TR or the banks statement reference. Used for reconciliation and audit trail across systems.',
    `position_status` STRING COMMENT 'Current lifecycle state of the treasury position record: open (pending confirmation), confirmed (bank-confirmed balance), reconciled (matched to bank statement), disputed (discrepancy under investigation), closed (period-end finalised), cancelled (voided entry).. Valid values are `open|confirmed|reconciled|disputed|closed|cancelled`',
    `reconciliation_status` STRING COMMENT 'Current reconciliation state of this position against the bank statement: unreconciled (not yet matched), matched (fully reconciled to bank statement), partially_matched (partial match with outstanding difference), exception (reconciliation exception requiring investigation).. Valid values are `unreconciled|matched|partially_matched|exception`',
    `source_system` STRING COMMENT 'Identifies the operational system of record from which this treasury position record was sourced: SAP_S4HANA (FI-TR module), BANK_DIRECT (bank portal feed), SWIFT_MT940 (bank statement message), KYRIBA (treasury management system), MANUAL (manually entered adjustment).. Valid values are `SAP_S4HANA|BANK_DIRECT|SWIFT_MT940|KYRIBA|MANUAL`',
    `updated_timestamp` TIMESTAMP COMMENT 'The timestamp when this treasury position record was last modified in the data platform, in ISO 8601 format (yyyy-MM-ddTHH:mm:ss.SSSXXX). Tracks balance revisions, reconciliation updates, and status changes for audit compliance.',
    `value_date` DATE COMMENT 'The date on which the funds are actually available or settled in the bank account, which may differ from the position date due to clearing cycles or float. Critical for accurate liquidity planning.',
    CONSTRAINT pk_treasury_position PRIMARY KEY(`treasury_position_id`)
) COMMENT 'Treasury cash and liquidity position records tracking the airlines cash balances, short-term investments (money market funds, commercial paper, time deposits), foreign currency exposures, and bank facility utilisation across all bank accounts, currencies, and legal entities. Stores position date, bank account, currency, balance type (actual/forecast/committed), liquidity category, counterparty bank, and maturity profile. Supports daily cash management, bank account reconciliation, FX exposure reporting, debt covenant monitoring, and IROP (Irregular Operations) contingency liquidity planning.';

CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`hedge_contract` (
    `hedge_contract_id` BIGINT COMMENT 'Unique identifier for the hedge contract record. Primary key.',
    `fuel_contract_id` BIGINT COMMENT 'Foreign key linking to procurement.fuel_contract. Business justification: Fuel hedging programs must link hedge instruments to specific fuel supply contracts for hedge effectiveness testing, P&L attribution, and IFRS 9 hedge accounting documentation. Required for fuel risk ',
    `collateral_currency` STRING COMMENT 'ISO 4217 three-letter currency code in which collateral is posted. Null if no collateral posted.. Valid values are `^[A-Z]{3}$`',
    `collateral_posted_amount` DECIMAL(18,2) COMMENT 'Amount of cash or securities posted as collateral to the counterparty under Credit Support Annex (CSA) terms. Null if no collateral arrangement exists.',
    `contract_number` STRING COMMENT 'Externally-known unique business identifier for the hedge contract, used in treasury operations and external reporting.. Valid values are `^[A-Z0-9]{8,20}$`',
    `contract_status` STRING COMMENT 'Current lifecycle status of the hedge contract, indicating whether it is in force, expired, or terminated early.. Valid values are `active|matured|terminated|settled|cancelled|suspended`',
    `counterparty_identifier` STRING COMMENT 'Unique identifier for the counterparty, typically LEI (Legal Entity Identifier) or internal counterparty code.. Valid values are `^[A-Z0-9]{8,20}$`',
    `counterparty_name` STRING COMMENT 'Legal name of the financial institution or counterparty with whom the hedge contract is executed.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this hedge contract record was first created in the system.',
    `cumulative_gain_loss` DECIMAL(18,2) COMMENT 'Cumulative realized and unrealized gain or loss on the hedge contract since inception, used for hedge accounting reserve calculations.',
    `currency_pair` STRING COMMENT 'ISO 4217 currency pair for foreign exchange hedges (e.g., USD/EUR, GBP/USD). Null for non-FX hedges.. Valid values are `^[A-Z]{3}/[A-Z]{3}$`',
    `documentation_complete_flag` BOOLEAN COMMENT 'Indicates whether all required IFRS 9 hedge accounting documentation (hedge designation memo, effectiveness testing methodology) has been completed and approved.',
    `effective_date` DATE COMMENT 'Date from which the hedge contract becomes active and hedge accounting treatment begins.',
    `effectiveness_test_date` DATE COMMENT 'Date on which the most recent hedge effectiveness test was performed.',
    `fair_value_amount` DECIMAL(18,2) COMMENT 'Current fair value (mark-to-market) of the hedge instrument as of the last valuation date, used for balance sheet reporting.',
    `fair_value_currency` STRING COMMENT 'ISO 4217 three-letter currency code in which the fair value is denominated.. Valid values are `^[A-Z]{3}$`',
    `fair_value_date` DATE COMMENT 'Date as of which the fair value was calculated, typically month-end or quarter-end for financial reporting.',
    `hedge_designation` STRING COMMENT 'Formal hedge accounting designation under IFRS 9, determining how gains and losses are recognized in financial statements.. Valid values are `cash_flow|fair_value|net_investment|undesignated`',
    `hedge_effectiveness_test_result` STRING COMMENT 'Result of the most recent prospective or retrospective hedge effectiveness test under IFRS 9, determining continued eligibility for hedge accounting.. Valid values are `highly_effective|effective|ineffective|not_tested`',
    `hedge_ratio` DECIMAL(18,2) COMMENT 'Ratio of the hedging instrument quantity to the hedged item quantity, used in hedge effectiveness calculations (e.g., 1.0000 for perfect hedge).',
    `hedged_item_description` STRING COMMENT 'Textual description of the specific forecasted transaction or recognized asset/liability being hedged (e.g., Q3 2024 jet fuel purchases for transatlantic routes).',
    `instrument_type` STRING COMMENT 'Type of financial derivative instrument used for hedging. Determines the contractual structure and settlement mechanism.. Valid values are `swap|option|forward|collar|future|cap`',
    `isda_master_agreement_flag` BOOLEAN COMMENT 'Indicates whether this hedge contract is governed by an ISDA Master Agreement with the counterparty.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this hedge contract record was last updated, capturing any changes to contract terms, valuations, or status.',
    `maturity_date` DATE COMMENT 'Date on which the hedge contract expires or settles, marking the end of the hedging relationship.',
    `notional_amount` DECIMAL(18,2) COMMENT 'Principal amount or volume of the underlying exposure being hedged (e.g., barrels of fuel, currency units, interest principal).',
    `notional_unit` STRING COMMENT 'Unit of measure for the notional amount (barrels for fuel, currency code for FX, basis points for interest rate). [ENUM-REF-CANDIDATE: barrels|gallons|metric_tons|usd|eur|gbp|basis_points — 7 candidates stripped; promote to reference product]',
    `premium_currency` STRING COMMENT 'ISO 4217 three-letter currency code in which the premium was paid.. Valid values are `^[A-Z]{3}$`',
    `premium_paid` DECIMAL(18,2) COMMENT 'Upfront premium paid to acquire the hedge instrument (primarily for options and collars). Zero for forwards and swaps with no upfront cost.',
    `settlement_date` DATE COMMENT 'Actual date on which cash settlement or physical delivery occurred. Null if contract has not yet settled.',
    `strike_price` DECIMAL(18,2) COMMENT 'Agreed price or rate at which the hedge instrument can be exercised (for options) or settled (for forwards/swaps). Null for instruments without a fixed strike.',
    `termination_date` DATE COMMENT 'Date on which the hedge contract was terminated early, if applicable. Null for contracts that matured naturally or are still active.',
    `termination_reason` STRING COMMENT 'Business reason for early termination of the hedge contract (e.g., hedged item no longer expected, counterparty credit event, portfolio rebalancing). Null if not terminated.',
    `trade_date` DATE COMMENT 'Date on which the hedge contract was executed and agreed between the airline and counterparty.',
    `underlying_commodity_code` STRING COMMENT 'Standardized code for the specific commodity being hedged (e.g., JET for jet fuel, BRENT for Brent crude). Null for non-commodity hedges.. Valid values are `^[A-Z0-9]{2,10}$`',
    `underlying_type` STRING COMMENT 'Category of the underlying asset or rate being hedged (fuel commodity, currency pair, interest rate benchmark, or carbon allowance).. Valid values are `jet_fuel|crude_oil|foreign_exchange|interest_rate|carbon_credit`',
    CONSTRAINT pk_hedge_contract PRIMARY KEY(`hedge_contract_id`)
) COMMENT 'Financial hedging instrument master records for the airlines fuel price hedging, foreign currency hedging, and interest rate hedging programmes. Captures instrument type (swap, option, forward, collar), underlying commodity or currency, notional amount, strike price, premium paid, hedge designation (cash flow / fair value), effectiveness test results, and maturity date. Supports IFRS 9 hedge accounting documentation.';
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`tax_transaction` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Tax transactions related to employee benefits, payroll taxes, expatriate tax equalization require employee linkage for compliance reporting, tax certificate generation, and employee tax statement prep',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor or supplier party associated with this tax transaction, applicable for withholding tax on vendor payments and input VAT/GST recoveries.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`fixed_asset` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Assets assigned to employees (laptops, uniforms, tools, crew equipment) require custodian tracking for asset accountability, loss prevention, and disposal authorization per airline asset management po',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`cash_pool` (
    `cash_pool_id` BIGINT COMMENT 'Primary key for cash_pool',
    `bank_id` BIGINT COMMENT 'Reference to the financial institution providing the cash pooling service and managing the pool accounts.',
    `company_code_id` BIGINT COMMENT 'Reference to the legal entity that acts as the pool leader or header account holder, responsible for overall pool management and bank relationship.',
    `cost_centre_id` BIGINT COMMENT 'Reference to the cost center responsible for treasury management costs and interest income/expense allocation related to this cash pool.',
    `employee_id` BIGINT COMMENT 'Reference to the employee responsible for managing and monitoring this cash pool arrangement.',
    `parent_cash_pool_id` BIGINT COMMENT 'Self-referencing FK on cash_pool (parent_cash_pool_id)',
    `auto_sweep_enabled_flag` BOOLEAN COMMENT 'Indicates whether automatic cash sweeping is enabled to transfer excess balances from participant accounts to the master account.',
    `base_interest_rate` DECIMAL(18,2) COMMENT 'Reference interest rate (in decimal form, e.g., 0.0525 for 5.25%) used as the foundation for pool interest calculations, typically linked to a benchmark rate such as LIBOR, SOFR, or EURIBOR.',
    `created_by_user_code` STRING COMMENT 'User identifier of the person who created this cash pool record.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this cash pool record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO currency code for the cash pool. Multi-currency pools may require separate pool records per currency.',
    `effective_date` DATE COMMENT 'Date when the cash pool arrangement becomes active and operational for treasury transactions.',
    `expiry_date` DATE COMMENT 'Date when the cash pool arrangement terminates or requires renewal. Nullable for open-ended arrangements.',
    `ifrs_16_applicable_flag` BOOLEAN COMMENT 'Indicates whether IFRS 16 lease accounting considerations apply to this cash pool, relevant when pool is linked to lease liability financing arrangements.',
    `interest_calculation_method` STRING COMMENT 'Day count convention used for calculating interest on pooled balances and compensating participating accounts.',
    `interest_spread_bps` STRING COMMENT 'Spread in basis points (1 bp = 0.01%) added to or subtracted from the base rate for credit or debit positions within the pool.',
    `last_review_date` DATE COMMENT 'Date when the cash pool arrangement was last reviewed for performance, compliance, and alignment with treasury policy.',
    `master_account_number` STRING COMMENT 'The primary bank account number serving as the header or concentration account for the cash pool structure.',
    `maximum_pool_limit` DECIMAL(18,2) COMMENT 'Maximum aggregate exposure or balance limit for the cash pool arrangement, set by the bank or internal treasury policy.',
    `minimum_balance_threshold` DECIMAL(18,2) COMMENT 'Minimum aggregate balance required to be maintained in the cash pool to avoid penalties or maintain favorable interest terms.',
    `modified_by_user_code` STRING COMMENT 'User identifier of the person who last modified this cash pool record.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this cash pool record was last modified.',
    `netting_enabled_flag` BOOLEAN COMMENT 'Indicates whether intercompany netting is enabled within the cash pool to offset payables and receivables among participating entities.',
    `next_review_date` DATE COMMENT 'Scheduled date for the next periodic review of the cash pool arrangement.',
    `notes` STRING COMMENT 'Free-text field for additional comments, special terms, or operational notes related to the cash pool arrangement.',
    `overdraft_facility_flag` BOOLEAN COMMENT 'Indicates whether the cash pool includes an overdraft facility allowing negative aggregate balances up to an approved limit.',
    `overdraft_limit` DECIMAL(18,2) COMMENT 'Maximum overdraft amount permitted for the cash pool when overdraft facility is enabled. Nullable if no overdraft facility exists.',
    `pool_code` STRING COMMENT 'Externally-known unique business identifier for the cash pool arrangement, used in treasury operations and intercompany settlements.',
    `pool_name` STRING COMMENT 'Human-readable name of the cash pool arrangement, typically reflecting the participating entities or geographic scope.',
    `pool_status` STRING COMMENT 'Current lifecycle status of the cash pool arrangement indicating operational state and availability for treasury operations.',
    `pool_type` STRING COMMENT 'Classification of the cash pooling structure: notional (interest compensation without physical transfer), physical (actual fund transfers), hybrid (combination), zero balancing (daily sweep to zero), target balancing (sweep to target level), or header structure (master account with sub-accounts).',
    `pooling_frequency` STRING COMMENT 'Frequency at which cash concentration or interest calculation occurs within the pool structure.',
    `regulatory_approval_number` STRING COMMENT 'Reference number for regulatory approval or notification required for cross-border or multi-jurisdictional cash pooling arrangements.',
    `sweep_threshold_amount` DECIMAL(18,2) COMMENT 'Minimum balance threshold that triggers an automatic sweep transaction from a participant account to the master account.',
    `target_balance_amount` DECIMAL(18,2) COMMENT 'Target balance to be maintained in participant accounts after sweep operations in target balancing pool structures.',
    `tax_jurisdiction_code` STRING COMMENT 'Three-letter country code representing the primary tax jurisdiction governing the cash pool for withholding tax and interest reporting purposes.',
    `withholding_tax_rate` DECIMAL(18,2) COMMENT 'Applicable withholding tax rate (in decimal form, e.g., 0.15 for 15%) on interest income generated within the cash pool, based on tax jurisdiction.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`legal_entity` (
    ...
    `parent_entity_id` BIGINT COMMENT 'Reference to the immediate parent legal entity in the corporate ownership hierarchy. Null if this is the ultimate parent.',
    `ultimate_parent_entity_id` BIGINT COMMENT 'Reference to the top-level parent legal entity in the corporate ownership hierarchy. Null if this is the ultimate parent.',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`finance`.`bank` (
    `bank_id` BIGINT COMMENT 'Primary key for bank',
    `correspondent_bank_id` BIGINT COMMENT 'Self-referencing FK on bank (correspondent_bank_id)',
    `account_opening_date` DATE COMMENT 'Date when the airline first established a banking relationship with this bank.',
    `address_line_1` STRING COMMENT 'Primary street address line of the banks headquarters or main branch office.',
    `address_line_2` STRING COMMENT 'Secondary address line for additional location details such as building name, floor, or suite number.',
    `bank_code` STRING COMMENT 'Unique bank identification code assigned by the national banking authority or central bank. Used for domestic payment routing and interbank settlements.',
    `bank_name` STRING COMMENT 'Full legal name of the bank as registered with the banking authority.',
    `bank_status` STRING COMMENT 'Current operational status of the bank in the airlines treasury and payment systems.',
    `bank_type` STRING COMMENT 'Classification of the bank based on its primary business function and regulatory category.',
    `city` STRING COMMENT 'City or municipality where the banks headquarters or main branch is located.',
    `country_code` STRING COMMENT 'Three-letter ISO country code representing the country where the bank is domiciled and regulated.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this bank record was first created in the system.',
    `credit_rating` STRING COMMENT 'Current credit rating assigned to the bank by major rating agencies, used for counterparty risk assessment in treasury operations.',
    `currency_code` STRING COMMENT 'Three-letter ISO currency code representing the primary currency in which the bank operates and settles transactions.',
    `email_address` STRING COMMENT 'Primary email address for treasury operations, payment inquiries, and interline settlement communications with the bank.',
    `iban_prefix` STRING COMMENT 'Country-specific IBAN prefix used for international account number validation. Consists of 2-letter country code and 2-digit check digits.',
    `is_preferred_vendor` BOOLEAN COMMENT 'Indicates whether this bank is designated as a preferred banking partner for the airlines treasury operations, offering negotiated rates and priority service.',
    `is_sanctions_compliant` BOOLEAN COMMENT 'Indicates whether the bank has been verified as compliant with international sanctions regulations (OFAC, UN, EU) and is approved for airline treasury operations.',
    `last_transaction_date` DATE COMMENT 'Date of the most recent financial transaction processed through this bank.',
    `notes` STRING COMMENT 'Free-text field for additional information, special instructions, or operational notes related to the bank relationship.',
    `phone_number` STRING COMMENT 'Primary contact telephone number for the banks treasury or corporate banking department.',
    `postal_code` STRING COMMENT 'Postal or ZIP code for the banks address.',
    `regulatory_authority` STRING COMMENT 'Name of the primary banking regulatory authority or central bank that supervises and licenses this bank.',
    `relationship_manager_email` STRING COMMENT 'Email address of the banks relationship manager for treasury correspondence and payment instructions.',
    `relationship_manager_name` STRING COMMENT 'Name of the banks relationship manager or account officer assigned to the airlines corporate banking account.',
    `relationship_manager_phone` STRING COMMENT 'Direct phone number of the banks relationship manager for urgent treasury matters and payment escalations.',
    `routing_number` STRING COMMENT 'Nine-digit routing transit number (RTN) used for domestic electronic funds transfers and automated clearing house (ACH) transactions. Primarily used in United States banking.',
    `sanctions_screening_date` DATE COMMENT 'Date when the bank was last screened against international sanctions lists for compliance verification.',
    `state_province` STRING COMMENT 'State, province, or administrative region where the bank is located.',
    `swift_code` STRING COMMENT 'International bank identifier code (BIC) used for cross-border wire transfers and interline billing settlements. Format: 8 or 11 alphanumeric characters.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this bank record was last modified in the system.',
    `website_url` STRING COMMENT 'Official website URL of the bank for reference and online banking portal access.',
    CONSTRAINT pk_bank PRIMARY KEY(`bank_id`)
) COMMENT 'Master reference table for bank. Referenced by bank_id.';
...
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ADD CONSTRAINT `fk_finance_budget_plan_profit_centre_id` FOREIGN KEY (`profit_centre_id`) REFERENCES `airlines_ecm`.`finance`.`cost_centre`(`cost_centre_id`);
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` ADD CONSTRAINT `fk_finance_fuel_cost_transaction_hedge_contract_id` FOREIGN KEY (`hedge_contract_id`) REFERENCES `airlines_ecm`.`finance`.`hedge_contract`(`hedge_contract_id`);
...
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ADD CONSTRAINT `fk_finance_treasury_position_cash_pool_id` FOREIGN KEY (`cash_pool_id`) REFERENCES `airlines_ecm`.`finance`.`cash_pool`(`cash_pool_id`);
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ADD CONSTRAINT `fk_finance_treasury_position_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `airlines_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ADD CONSTRAINT `fk_finance_treasury_position_cost_centre_id` FOREIGN KEY (`cost_centre_id`) REFERENCES `airlines_ecm`.`finance`.`cost_centre`(`cost_centre_id`);
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ADD CONSTRAINT `fk_finance_treasury_position_general_ledger_id` FOREIGN KEY (`general_ledger_id`) REFERENCES `airlines_ecm`.`finance`.`general_ledger`(`general_ledger_id`);
...
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ADD CONSTRAINT `fk_finance_cash_pool_bank_id` FOREIGN KEY (`bank_id`) REFERENCES `airlines_ecm`.`finance`.`bank`(`bank_id`);
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ADD CONSTRAINT `fk_finance_cash_pool_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `airlines_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ADD CONSTRAINT `fk_finance_cash_pool_cost_centre_id` FOREIGN KEY (`cost_centre_id`) REFERENCES `airlines_ecm`.`finance`.`cost_centre`(`cost_centre_id`);
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ADD CONSTRAINT `fk_finance_cash_pool_parent_cash_pool_id` FOREIGN KEY (`parent_cash_pool_id`) REFERENCES `airlines_ecm`.`finance`.`cash_pool`(`cash_pool_id`);
ALTER TABLE `airlines_ecm`.`finance`.`legal_entity` ADD CONSTRAINT `fk_finance_legal_entity_parent_entity_id` FOREIGN KEY (`parent_entity_id`) REFERENCES `airlines_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
ALTER TABLE `airlines_ecm`.`finance`.`legal_entity` ADD CONSTRAINT `fk_finance_legal_entity_ultimate_parent_entity_id` FOREIGN KEY (`ultimate_parent_entity_id`) REFERENCES `airlines_ecm`.`finance`.`legal_entity`(`legal_entity_id`);
...
ALTER TABLE `airlines_ecm`.`finance`.`bank` ADD CONSTRAINT `fk_finance_bank_correspondent_bank_id` FOREIGN KEY (`correspondent_bank_id`) REFERENCES `airlines_ecm`.`finance`.`bank`(`bank_id`);
...
ALTER TABLE `airlines_ecm`.`finance`.`general_ledger` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`journal_entry` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`journal_entry` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Customer ID');
ALTER TABLE `airlines_ecm`.`finance`.`journal_entry` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`journal_entry` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`journal_entry` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
...
ALTER TABLE `airlines_ecm`.`finance`.`cost_centre` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`cost_centre` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Manager ID');
ALTER TABLE `airlines_ecm`.`finance`.`cost_centre` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`cost_centre` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Expense Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `goods_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Goods Receipt Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order ID');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `repair_order_id` SET TAGS ('dbx_business_glossary_term' = 'Repair Order Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_payable` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
...
ALTER TABLE `airlines_ecm`.`finance`.`accounts_receivable` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`accounts_receivable` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Staff Travel Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_receivable` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_receivable` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`accounts_receivable` ALTER COLUMN `ticket_id` SET TAGS ('dbx_business_glossary_term' = 'Ticket Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` SET TAGS ('dbx_subdomain' = 'planning_control');
...
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Approver ID');
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `primary_budget_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Owner ID');
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `primary_budget_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `primary_budget_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `procurement_category_id` SET TAGS ('dbx_business_glossary_term' = 'Procurement Category Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`budget_plan` ALTER COLUMN `profit_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Centre ID');
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` SET TAGS ('dbx_subdomain' = 'planning_control');
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` ALTER COLUMN `flight_leg_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Leg Assignment Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` ALTER COLUMN `fuel_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Fuel Contract Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` ALTER COLUMN `fuel_uplift_order_id` SET TAGS ('dbx_business_glossary_term' = 'Fuel Uplift Order Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` ALTER COLUMN `hedge_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Hedge Contract Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`fuel_cost_transaction` ALTER COLUMN `supplier_invoice_number` SET TAGS ('dbx_business_glossary_term' = 'Supplier Invoice Number');
...
ALTER TABLE `airlines_ecm`.`finance`.`lease_contract` SET TAGS ('dbx_subdomain' = 'lease_management');
...
ALTER TABLE `airlines_ecm`.`finance`.`lease_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Manager Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`lease_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`lease_contract` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`finance`.`lease_contract` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`finance`.`lease_liability_schedule` SET TAGS ('dbx_subdomain' = 'lease_management');
...
ALTER TABLE `airlines_ecm`.`finance`.`lease_liability_schedule` ALTER COLUMN `journal_entry_document_number` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Document Number');
...
ALTER TABLE `airlines_ecm`.`finance`.`interline_billing` SET TAGS ('dbx_subdomain' = 'lease_management');
...
ALTER TABLE `airlines_ecm`.`finance`.`interline_billing` ALTER COLUMN `flight_date` SET TAGS ('dbx_business_glossary_term' = 'Flight Date');
ALTER TABLE `airlines_ecm`.`finance`.`interline_billing` ALTER COLUMN `flight_number` SET TAGS ('dbx_business_glossary_term' = 'Flight Number');
ALTER TABLE `airlines_ecm`.`finance`.`interline_billing` ALTER COLUMN `flight_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2}[0-9]{1,4}$');
...
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` SET TAGS ('dbx_subdomain' = 'treasury_risk');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `treasury_position_id` SET TAGS ('dbx_business_glossary_term' = 'Treasury Position ID');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `cash_pool_id` SET TAGS ('dbx_business_glossary_term' = 'Cash Pool Identifier');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `cost_centre_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Centre Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Treasury Officer Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `balance_amount` SET TAGS ('dbx_business_glossary_term' = 'Balance Amount');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `balance_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `balance_amount_usd` SET TAGS ('dbx_business_glossary_term' = 'Balance Amount in USD');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `balance_amount_usd` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `balance_type` SET TAGS ('dbx_business_glossary_term' = 'Balance Type');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `balance_type` SET TAGS ('dbx_value_regex' = 'actual|forecast|committed|projected|opening|closing');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_iban` SET TAGS ('dbx_business_glossary_term' = 'Bank Account International Bank Account Number (IBAN)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_iban` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}[0-9]{2}[A-Z0-9]{1,30}$');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_iban` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_iban` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_name` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Name');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_name` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_number` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Number');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_account_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bank_statement_reference` SET TAGS ('dbx_business_glossary_term' = 'Bank Statement Reference');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bsp_settlement_period` SET TAGS ('dbx_business_glossary_term' = 'Billing and Settlement Plan (BSP) Settlement Period');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `bsp_settlement_period` SET TAGS ('dbx_value_regex' = '^[0-9]{4}-[0-9]{2}$');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `counterparty_bank_code` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Bank Code (BIC/SWIFT)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `counterparty_bank_name` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Bank Name');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `country_code` SET TAGS ('dbx_business_glossary_term' = 'Country Code (ISO 3166-1 Alpha-3)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `covenant_breach_flag` SET TAGS ('dbx_business_glossary_term' = 'Debt Covenant Breach Flag');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `covenant_threshold_amount` SET TAGS ('dbx_business_glossary_term' = 'Debt Covenant Threshold Amount');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `covenant_threshold_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code (ISO 4217)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `facility_limit_amount` SET TAGS ('dbx_business_glossary_term' = 'Bank Facility Limit Amount');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `facility_limit_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `facility_utilised_amount` SET TAGS ('dbx_business_glossary_term' = 'Bank Facility Utilised Amount');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `facility_utilised_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `fx_rate` SET TAGS ('dbx_business_glossary_term' = 'Foreign Exchange (FX) Rate');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `fx_rate_type` SET TAGS ('dbx_business_glossary_term' = 'Foreign Exchange (FX) Rate Type');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `fx_rate_type` SET TAGS ('dbx_value_regex' = 'spot|average|closing|budget|hedge');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `hedge_designation` SET TAGS ('dbx_business_glossary_term' = 'Hedge Designation');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `hedge_designation` SET TAGS ('dbx_value_regex' = 'fair_value_hedge|cash_flow_hedge|net_investment_hedge|not_designated');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `ifrs16_lease_flag` SET TAGS ('dbx_business_glossary_term' = 'IFRS 16 Lease Liability Flag');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `instrument_type` SET TAGS ('dbx_business_glossary_term' = 'Financial Instrument Type');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `interest_rate` SET TAGS ('dbx_business_glossary_term' = 'Interest Rate (Annual Percentage)');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `interline_settlement_flag` SET TAGS ('dbx_business_glossary_term' = 'Interline Settlement Flag');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `irop_reserve_flag` SET TAGS ('dbx_business_glossary_term' = 'Irregular Operations (IROP) Contingency Reserve Flag');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `liquidity_category` SET TAGS ('dbx_business_glossary_term' = 'Liquidity Category');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `liquidity_category` SET TAGS ('dbx_value_regex' = 'cash_and_equivalents|money_market_fund|commercial_paper|time_deposit|bank_facility|restricted_cash');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `maturity_date` SET TAGS ('dbx_business_glossary_term' = 'Maturity Date');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Treasury Position Notes');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `position_date` SET TAGS ('dbx_business_glossary_term' = 'Treasury Position Date');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `position_reference` SET TAGS ('dbx_business_glossary_term' = 'Treasury Position Reference Number');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `position_status` SET TAGS ('dbx_business_glossary_term' = 'Treasury Position Status');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `position_status` SET TAGS ('dbx_value_regex' = 'open|confirmed|reconciled|disputed|closed|cancelled');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_business_glossary_term' = 'Bank Reconciliation Status');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_value_regex' = 'unreconciled|matched|partially_matched|exception');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `source_system` SET TAGS ('dbx_value_regex' = 'SAP_S4HANA|BANK_DIRECT|SWIFT_MT940|KYRIBA|MANUAL');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `airlines_ecm`.`finance`.`treasury_position` ALTER COLUMN `value_date` SET TAGS ('dbx_business_glossary_term' = 'Value Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` SET TAGS ('dbx_subdomain' = 'treasury_risk');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedge_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Hedge Contract Identifier (ID)');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `fuel_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Fuel Contract Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `collateral_currency` SET TAGS ('dbx_business_glossary_term' = 'Collateral Currency Code');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `collateral_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `collateral_posted_amount` SET TAGS ('dbx_business_glossary_term' = 'Collateral Posted Amount');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `collateral_posted_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `contract_number` SET TAGS ('dbx_business_glossary_term' = 'Hedge Contract Number');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `contract_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,20}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `contract_status` SET TAGS ('dbx_business_glossary_term' = 'Hedge Contract Status');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `contract_status` SET TAGS ('dbx_value_regex' = 'active|matured|terminated|settled|cancelled|suspended');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `counterparty_identifier` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Identifier');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `counterparty_identifier` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,20}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `counterparty_identifier` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `counterparty_name` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Name');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `counterparty_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `cumulative_gain_loss` SET TAGS ('dbx_business_glossary_term' = 'Cumulative Gain or Loss');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `cumulative_gain_loss` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `currency_pair` SET TAGS ('dbx_business_glossary_term' = 'Currency Pair');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `currency_pair` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}/[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `documentation_complete_flag` SET TAGS ('dbx_business_glossary_term' = 'Hedge Documentation Complete Flag');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `effectiveness_test_date` SET TAGS ('dbx_business_glossary_term' = 'Effectiveness Test Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `fair_value_amount` SET TAGS ('dbx_business_glossary_term' = 'Fair Value Amount');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `fair_value_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `fair_value_currency` SET TAGS ('dbx_business_glossary_term' = 'Fair Value Currency Code');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `fair_value_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `fair_value_date` SET TAGS ('dbx_business_glossary_term' = 'Fair Value Valuation Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedge_designation` SET TAGS ('dbx_business_glossary_term' = 'Hedge Accounting Designation');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedge_designation` SET TAGS ('dbx_value_regex' = 'cash_flow|fair_value|net_investment|undesignated');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedge_effectiveness_test_result` SET TAGS ('dbx_business_glossary_term' = 'Hedge Effectiveness Test Result');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedge_effectiveness_test_result` SET TAGS ('dbx_value_regex' = 'highly_effective|effective|ineffective|not_tested');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedge_ratio` SET TAGS ('dbx_business_glossary_term' = 'Hedge Ratio');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `hedged_item_description` SET TAGS ('dbx_business_glossary_term' = 'Hedged Item Description');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `instrument_type` SET TAGS ('dbx_business_glossary_term' = 'Hedge Instrument Type');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `instrument_type` SET TAGS ('dbx_value_regex' = 'swap|option|forward|collar|future|cap');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `isda_master_agreement_flag` SET TAGS ('dbx_business_glossary_term' = 'International Swaps and Derivatives Association (ISDA) Master Agreement Flag');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `maturity_date` SET TAGS ('dbx_business_glossary_term' = 'Maturity Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `notional_amount` SET TAGS ('dbx_business_glossary_term' = 'Notional Amount');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `notional_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `notional_unit` SET TAGS ('dbx_business_glossary_term' = 'Notional Unit of Measure');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `premium_currency` SET TAGS ('dbx_business_glossary_term' = 'Premium Currency Code');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `premium_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `premium_paid` SET TAGS ('dbx_business_glossary_term' = 'Premium Paid');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `premium_paid` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `strike_price` SET TAGS ('dbx_business_glossary_term' = 'Strike Price');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `strike_price` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Termination Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `termination_reason` SET TAGS ('dbx_business_glossary_term' = 'Termination Reason');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `trade_date` SET TAGS ('dbx_business_glossary_term' = 'Trade Date');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `underlying_commodity_code` SET TAGS ('dbx_business_glossary_term' = 'Underlying Commodity Code');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `underlying_commodity_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,10}$');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `underlying_type` SET TAGS ('dbx_business_glossary_term' = 'Underlying Asset Type');
ALTER TABLE `airlines_ecm`.`finance`.`hedge_contract` ALTER COLUMN `underlying_type` SET TAGS ('dbx_value_regex' = 'jet_fuel|crude_oil|foreign_exchange|interest_rate|carbon_credit');
ALTER TABLE `airlines_ecm`.`finance`.`tax_transaction` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `airlines_ecm`.`finance`.`tax_transaction` SET TAGS ('dbx_subdomain' = 'planning_control');
...
ALTER TABLE `airlines_ecm`.`finance`.`tax_transaction` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Tax Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`tax_transaction` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`tax_transaction` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`tax_transaction` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
...
ALTER TABLE `airlines_ecm`.`finance`.`fixed_asset` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`fixed_asset` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Custodian Employee Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`finance`.`fixed_asset` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`fixed_asset` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`finance`.`company_code` SET TAGS ('dbx_subdomain' = 'accounting_operations');
...
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` SET TAGS ('dbx_subdomain' = 'treasury_risk');
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ALTER COLUMN `cash_pool_id` SET TAGS ('dbx_business_glossary_term' = 'Cash Pool Identifier');
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ALTER COLUMN `parent_cash_pool_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`cash_pool` ALTER COLUMN `master_account_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`legal_entity` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `airlines_ecm`.`finance`.`legal_entity` SET TAGS ('dbx_subdomain' = 'treasury_risk');
...
ALTER TABLE `airlines_ecm`.`finance`.`bank` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `airlines_ecm`.`finance`.`bank` SET TAGS ('dbx_subdomain' = 'treasury_risk');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `bank_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Identifier');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `correspondent_bank_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `address_line_1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `address_line_2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `email_address` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `email_address` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `phone_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `relationship_manager_email` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `relationship_manager_email` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `relationship_manager_phone` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `relationship_manager_phone` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `airlines_ecm`.`finance`.`bank` ALTER COLUMN `state_province` SET TAGS ('dbx_pii_address' = 'true');
