-- Schema for Domain: crew | Business: Airlines | Version: v1_mvm
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`member` (
    ...
    `carrier_id` BIGINT COMMENT 'Foreign key linking to route.carrier. Business justification: In codeshare, wet-lease, and multi-carrier group operations, crew members are employed by a specific carrier. Linking member to carrier enables AOC compliance tracking, crew cost attribution to the co',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`qualification` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to safety.audit. Business justification: IOSA and regulatory authority audits (CASA, FAA, EASA) directly audit crew qualification records for currency and validity. Linking a qualification record to the audit that reviewed it supports audit ',
    ...
    `training_event_id` BIGINT COMMENT 'Foreign key linking to crew.training_event. Business justification: A crew qualification (type rating, proficiency check result) is typically produced or validated by a specific training event (simulator check, OPC, PC). Linking qualification.training_event_id → train',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`base` (
    ...
    `carrier_id` BIGINT COMMENT 'Foreign key linking to route.carrier. Business justification: Crew bases operate under a specific carriers AOC (Air Operator Certificate). Linking base to carrier enables regulatory compliance reporting (which AOC governs each base), wet-lease crew management, ',
    `station_id` BIGINT COMMENT 'Foreign key linking to airport.station. Business justification: Every crew base is physically located at an airport station. Crew base management, slot coordination, and ground handling reports require a formal FK linking the base to its station. icao_code on base',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`pairing` (
    ...
    `block_time_standard_id` BIGINT COMMENT 'Foreign key linking to route.block_time_standard. Business justification: Block time standards define planned block times used in FTL legality calculations for pairings. Linking pairing to block_time_standard enables validation of pairing total_block_hours against route sta',
    ...
    `fleet_assignment_id` BIGINT COMMENT 'Foreign key linking to route.fleet_assignment. Business justification: Crew pairings must be validated against fleet assignments to ensure crew qualifications match the assigned aircraft type on each route-season combination. This link enables crew planners to directly v',
    ...
    `route_id` BIGINT COMMENT 'Foreign key linking to route.route. Business justification: Crew cost-per-route reporting and network crew planning require pairings to be directly attributed to routes. Crew planners build pairings for specific route operations; this link enables route-level ',
    `schedule_season_id` BIGINT COMMENT 'Foreign key linking to route.schedule_season. Business justification: Crew pairings are constructed per IATA schedule season. Linking pairing to schedule_season enables season-level crew cost analysis, pairing reuse across equivalent seasons, and alignment with the netw',
    `seasonal_schedule_id` BIGINT COMMENT 'Foreign key linking to route.seasonal_schedule. Business justification: Pairings are constructed to cover specific seasonal schedule operations. Linking pairing to seasonal_schedule enables crew planners to trace which scheduled service pattern a pairing covers, supportin',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`duty_period` (
    ...
    `station_id` BIGINT COMMENT 'Foreign key linking to airport.station. Business justification: FTL compliance reporting and crew positioning require duty periods to be formally anchored to their start station. Aviation regulators and crew control systems track where each duty period originates.',
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Duty periods drive FDP pay, augmented crew allowances, and split-duty payments posted to GL. Airline payroll-to-GL reconciliation requires duty_period→GL traceability. Aviation finance controllers exp',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`roster` (
    ...
    `schedule_season_id` BIGINT COMMENT 'Foreign key linking to route.schedule_season. Business justification: Crew rosters are published per bid period aligned to IATA schedule seasons. Linking roster to schedule_season enables season-level crew utilization reporting, headcount planning, and union compliance ',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`roster_activity` (
    ...
    `station_id` BIGINT COMMENT 'Foreign key linking to airport.station. Business justification: Roster activities (standby, positioning, training) are performed at specific stations. Crew scheduling and IROP management reports require a proper FK to station for filtering and joining. station_cod',
    ...
    `training_event_id` BIGINT COMMENT 'Foreign key linking to crew.training_event. Business justification: A roster_activity of activity_type=training should reference the specific training_event record it corresponds to. roster_activity already carries a training_type field indicating training activitie',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`flight_leg_assignment` (
    ...
    `cabin_configuration_id` BIGINT COMMENT 'Foreign key linking to fleet.cabin_configuration. Business justification: Cabin crew door/exit assignments and safety briefings depend on the specific cabin configuration of the operating aircraft. Regulators (EASA OPS, FAR 121) require crew complement verification against ',
    `fare_class_bucket_id` BIGINT COMMENT 'Foreign key linking to inventory.fare_class_bucket. Business justification: Deadhead crew are booked into specific fare class buckets (e.g., ZB crew booking class) that consume inventory. Crew travel management and revenue management must track which fare_class_bucket is cons',
    ...
    `flight_inventory_id` BIGINT COMMENT 'Foreign key linking to inventory.flight_inventory. Business justification: Deadhead crew consume revenue seats tracked in flight_inventory (operational_seats_held). Crew controllers and revenue management must reconcile seats blocked for deadhead/positioning crew against ava',
    ...
    `gate_assignment_id` BIGINT COMMENT 'Foreign key linking to airport.gate_assignment. Business justification: Crew briefing and gate reporting processes require crew assignments to reference the specific gate where the aircraft is parked. Crew control systems consume gate_assignment data to direct crew to the',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`ftl_legality_check` (
    ...
    `occurrence_id` BIGINT COMMENT 'Foreign key linking to safety.occurrence. Business justification: FTL violations are a primary causal factor in fatigue-related safety occurrences. EASA ORO.FTL and FAR Part 117 require linking FTL breaches to safety events. ftl_legality_check has no occurrence link',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`training_event` (
    ...
    `accounts_payable_id` BIGINT COMMENT 'Foreign key linking to finance.accounts_payable. Business justification: External type-rating schools, simulator providers, and approved training organizations invoice airlines via AP. Linking training_event to AP enables three-way matching of training bookings to vendor i',
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to safety.audit. Business justification: Regulatory audits (IOSA, national authority) audit specific training events for syllabus compliance, instructor qualification, and recency. Linking training_event to the audit that reviewed it enables',
    `cabin_configuration_id` BIGINT COMMENT 'Foreign key linking to fleet.cabin_configuration. Business justification: Cabin crew training (door drills, evacuation, galley familiarization) is configuration-specific. EASA OPS 1.1005 and FAR 121.417 mandate training records reference the exact cabin configuration. Aviat',
    ...
    `flight_inventory_id` BIGINT COMMENT 'Foreign key linking to inventory.flight_inventory. Business justification: Line training and Operating Experience (OE) events occur on revenue flights, consuming a crew seat in flight_inventory. Airlines must track which revenue flight inventory record supports each line che',
    `member_id` BIGINT COMMENT 'Identifier of the crew member who participated in this training event.',
    `route_id` BIGINT COMMENT 'Foreign key linking to route.route. Business justification: Route checks and line checks (EASA/FAA regulatory requirement) are conducted on specific routes under examiner observation. Linking training_event to route enables tracking of which routes were used f',
    `aircraft_id` BIGINT COMMENT 'Foreign key linking to fleet.aircraft. Business justification: Line checks and OE (operating experience) flights require recording the specific tail number used. Regulatory authorities (FAA, EASA) require tail-specific traceability for line check records. The exi',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`medical_certificate` (
    ...
    `accounts_payable_id` BIGINT COMMENT 'Foreign key linking to finance.accounts_payable. Business justification: Aviation medical examiners invoice airlines for mandatory crew medical examinations. Linking medical_certificate to AP enables matching of examination records to vendor invoices — required for crew co',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`licence` (
    ...
    `accounts_payable_id` BIGINT COMMENT 'Foreign key linking to finance.accounts_payable. Business justification: Airlines pay regulatory fees for crew licence renewals, type rating endorsements, and language proficiency tests via AP. Linking licence to AP supports crew compliance cost tracking and AP invoice mat',
    `medical_certificate_id` BIGINT COMMENT 'Foreign key linking to crew.medical_certificate. Business justification: Aviation licences (ATPL, CPL) are intrinsically linked to the holders medical certificate — ICAO Annex 1 and national CAA regulations require a valid medical to exercise licence privileges. The licen',
...
CREATE OR REPLACE TABLE `airlines_ecm`.`crew`.`absence` (
    ...
    `general_ledger_id` BIGINT COMMENT 'Foreign key linking to finance.general_ledger. Business justification: Crew absences trigger sick pay, statutory leave payments, and replacement crew costs posted to GL. Airline HR-finance integration requires absence→GL traceability for sick pay accruals, insurance reco',
...
ALTER TABLE `airlines_ecm`.`crew`.`qualification` ADD CONSTRAINT `fk_crew_qualification_training_event_id` FOREIGN KEY (`training_event_id`) REFERENCES `airlines_ecm`.`crew`.`training_event`(`training_event_id`);
...
ALTER TABLE `airlines_ecm`.`crew`.`roster_activity` ADD CONSTRAINT `fk_crew_roster_activity_training_event_id` FOREIGN KEY (`training_event_id`) REFERENCES `airlines_ecm`.`crew`.`training_event`(`training_event_id`);
...
ALTER TABLE `airlines_ecm`.`crew`.`licence` ADD CONSTRAINT `fk_crew_licence_medical_certificate_id` FOREIGN KEY (`medical_certificate_id`) REFERENCES `airlines_ecm`.`crew`.`medical_certificate`(`medical_certificate_id`);
...
ALTER TABLE `airlines_ecm`.`crew`.`member` SET TAGS ('dbx_subdomain' = 'crew_records');
...
ALTER TABLE `airlines_ecm`.`crew`.`member` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`qualification` SET TAGS ('dbx_subdomain' = 'crew_records');
...
ALTER TABLE `airlines_ecm`.`crew`.`qualification` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`qualification` ALTER COLUMN `training_event_id` SET TAGS ('dbx_business_glossary_term' = 'Training Event Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`base` SET TAGS ('dbx_subdomain' = 'crew_records');
...
ALTER TABLE `airlines_ecm`.`crew`.`base` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Operating Carrier Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`base` ALTER COLUMN `station_id` SET TAGS ('dbx_business_glossary_term' = 'Station Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`pairing` SET TAGS ('dbx_subdomain' = 'flight_scheduling');
...
ALTER TABLE `airlines_ecm`.`crew`.`pairing` ALTER COLUMN `block_time_standard_id` SET TAGS ('dbx_business_glossary_term' = 'Block Time Standard Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`pairing` ALTER COLUMN `fleet_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Fleet Assignment Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`pairing` ALTER COLUMN `route_id` SET TAGS ('dbx_business_glossary_term' = 'Route Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`pairing` ALTER COLUMN `schedule_season_id` SET TAGS ('dbx_business_glossary_term' = 'Schedule Season Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`pairing` ALTER COLUMN `seasonal_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Seasonal Schedule Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`duty_period` SET TAGS ('dbx_subdomain' = 'flight_scheduling');
...
ALTER TABLE `airlines_ecm`.`crew`.`duty_period` ALTER COLUMN `station_id` SET TAGS ('dbx_business_glossary_term' = 'Duty Start Station Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`duty_period` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'Duty Period General Ledger Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`roster` SET TAGS ('dbx_subdomain' = 'flight_scheduling');
...
ALTER TABLE `airlines_ecm`.`crew`.`roster` ALTER COLUMN `schedule_season_id` SET TAGS ('dbx_business_glossary_term' = 'Schedule Season Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`roster_activity` SET TAGS ('dbx_subdomain' = 'flight_scheduling');
...
ALTER TABLE `airlines_ecm`.`crew`.`roster_activity` ALTER COLUMN `station_id` SET TAGS ('dbx_business_glossary_term' = 'Station Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`roster_activity` ALTER COLUMN `training_event_id` SET TAGS ('dbx_business_glossary_term' = 'Training Event Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`flight_leg_assignment` SET TAGS ('dbx_subdomain' = 'flight_scheduling');
...
ALTER TABLE `airlines_ecm`.`crew`.`flight_leg_assignment` ALTER COLUMN `cabin_configuration_id` SET TAGS ('dbx_business_glossary_term' = 'Cabin Configuration Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`flight_leg_assignment` ALTER COLUMN `fare_class_bucket_id` SET TAGS ('dbx_business_glossary_term' = 'Deadhead Fare Class Bucket Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`flight_leg_assignment` ALTER COLUMN `flight_inventory_id` SET TAGS ('dbx_business_glossary_term' = 'Flight Inventory Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`flight_leg_assignment` ALTER COLUMN `gate_assignment_id` SET TAGS ('dbx_business_glossary_term' = 'Gate Assignment Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`ftl_legality_check` SET TAGS ('dbx_subdomain' = 'compliance_training');
...
ALTER TABLE `airlines_ecm`.`crew`.`ftl_legality_check` ALTER COLUMN `occurrence_id` SET TAGS ('dbx_business_glossary_term' = 'Occurrence Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`training_event` SET TAGS ('dbx_subdomain' = 'compliance_training');
...
ALTER TABLE `airlines_ecm`.`crew`.`training_event` ALTER COLUMN `accounts_payable_id` SET TAGS ('dbx_business_glossary_term' = 'Training Event Accounts Payable Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`training_event` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`training_event` ALTER COLUMN `cabin_configuration_id` SET TAGS ('dbx_business_glossary_term' = 'Cabin Configuration Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`training_event` ALTER COLUMN `flight_inventory_id` SET TAGS ('dbx_business_glossary_term' = 'Line Training Flight Inventory Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`training_event` ALTER COLUMN `route_id` SET TAGS ('dbx_business_glossary_term' = 'Route Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`training_event` ALTER COLUMN `aircraft_id` SET TAGS ('dbx_business_glossary_term' = 'Training Aircraft Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`medical_certificate` SET TAGS ('dbx_subdomain' = 'crew_records');
...
ALTER TABLE `airlines_ecm`.`crew`.`medical_certificate` ALTER COLUMN `accounts_payable_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Certificate Accounts Payable Id (Foreign Key)');
...
ALTER TABLE `airlines_ecm`.`crew`.`licence` SET TAGS ('dbx_subdomain' = 'crew_records');
ALTER TABLE `airlines_ecm`.`crew`.`licence` ALTER COLUMN `licence_id` SET TAGS ('dbx_business_glossary_term' = 'Licence ID');
ALTER TABLE `airlines_ecm`.`crew`.`licence` ALTER COLUMN `accounts_payable_id` SET TAGS ('dbx_business_glossary_term' = 'Licence Accounts Payable Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`licence` ALTER COLUMN `medical_certificate_id` SET TAGS ('dbx_business_glossary_term' = 'Medical Certificate Id (Foreign Key)');
ALTER TABLE `airlines_ecm`.`crew`.`licence` ALTER COLUMN `medical_certificate_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `airlines_ecm`.`crew`.`licence` ALTER COLUMN `medical_certificate_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `airlines_ecm`.`crew`.`absence` SET TAGS ('dbx_subdomain' = 'compliance_training');
...
ALTER TABLE `airlines_ecm`.`crew`.`absence` ALTER COLUMN `general_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'Absence General Ledger Id (Foreign Key)');
