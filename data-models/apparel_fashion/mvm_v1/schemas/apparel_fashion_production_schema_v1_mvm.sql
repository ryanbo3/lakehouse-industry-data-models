-- Schema for Domain: production | Business: Apparel Fashion | Version: v1_mvm
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`work_order` (
    ...
    `colorway_id` BIGINT COMMENT 'Foreign key linking to product.colorway. Business justification: Work orders are issued per colorway — fabric sourcing, dyeing, and quality inspection are colorway-specific. Existing style_id covers style level only; colorway_id is required for colorway-level produ',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Work orders in apparel are placed at SKU level (style+color+size) for size-ratio production planning, costing, and finished goods receipt. The plain sku column is a denormalized reference; replacing',
    ...
    `supplier_factory_id` BIGINT COMMENT 'FK to supplier.factory.factory_id — MUST-HAVE: Links production work orders to the factory master for capacity planning, compliance verification, and production allocation decisions.',
    `tech_pack_id` BIGINT COMMENT 'Foreign key linking to product.tech_pack. Business justification: Work orders must reference the specific tech pack version issued to the factory to ensure production conforms to approved specifications. Apparel compliance and quality audits require traceability fro',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`order_confirmation` (
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Order confirmations (goods receipt postings, activity confirmations) directly trigger FI journal entries in SAP PP/CO integration — Production Order Settlement and Goods Receipt Posting processes.',
    `lot_id` BIGINT COMMENT 'Foreign key linking to production.lot. Business justification: An order_confirmation records actual goods output (yield_quantity, scrap_quantity, rework_quantity) against a work_order_operation. In CMT apparel manufacturing, confirmations are posted at the lot/ba',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`factory_capacity` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Factory capacity blocks are budgeted and tracked against cost centers in the Capacity Cost Planning process — finance allocates labor and overhead budgets to capacity units. Apparel finance teams re',
    `season_id` BIGINT COMMENT 'Identifier of the merchandise season for which capacity is reserved or allocated.',
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: Integrated capacity planning requires linking internal factory capacity records to supplier factory profiles. Planners must verify compliance certifications and audit status on the supplier_factory re',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`schedule` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Production schedules are planned and tracked within cost centers for labor and overhead budget allocation — the Production Schedule Cost Center Planning process. Finance teams use this link for capa',
    ...
    `line_id` BIGINT COMMENT 'Reference to the specific production line or work center within the factory allocated for this schedule. Represents the capacity slot assignment.',
    ...
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: Production schedules in apparel are executed at supplier factories. Linking schedule to supplier_factory enables capacity booking confirmation, compliance status checks at scheduling time, and deliver',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` (
    ...
    `bulk_fabric_receipt_id` BIGINT COMMENT 'Foreign key linking to production.bulk_fabric_receipt. Business justification: T&A calendars track fabric receipt as a critical milestone (milestone_type = FABRIC_RECEIPT, FABRIC_INSPECTION_COMPLETE). When a T&A milestone represents a fabric receipt event, it must reference ',
    `buy_plan_id` BIGINT COMMENT 'Foreign key linking to merchandising.buy_plan. Business justification: TNA milestones track critical path events (ex-factory, in-DC, sample approval) anchored to buy plan dates. Apparel merchandisers monitor TNA milestone status against buy plan commitments for on-time d',
    `cut_order_id` BIGINT COMMENT 'Foreign key linking to production.cut_order. Business justification: T&A calendars in apparel production track cutting milestones (fabric cut date, cut completion) as critical path events. When a T&A milestone represents a cutting event (milestone_type = CUT_START, ',
    `lot_id` BIGINT COMMENT 'Foreign key linking to production.lot. Business justification: T&A calendars track lot-level production milestones (sewing start, sewing end, finishing, packing, inspection). When a T&A milestone represents a lot-level event (milestone_type = SEW_START, SEW_EN',
    `pp_sample_id` BIGINT COMMENT 'Foreign key linking to production.pp_sample. Business justification: Time and Action (T&A) calendars in apparel production explicitly track sample submission and approval milestones (proto sample, fit sample, PP sample, TOP sample). When a T&A milestone represents a sa',
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: TNA milestones carry season_code as a plain denormalized column but no FK to merchandising.season. Apparel seasonal T&A reporting and critical path management require proper referential integrity to t',
    ...
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: TNA milestone ownership in apparel is assigned to specific supplier factories (e.g., fabric booking, PP sample submission, ex-factory). Linking milestones to supplier_factory enables delay root-cause ',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`pp_sample` (
    ...
    `ap_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ap_invoice. Business justification: Pre-production sample charges are invoiced by factories and tracked as AP items — the Sample Cost Management process in apparel product development. Finance teams reconcile sample_cost against AP in',
    `buy_plan_id` BIGINT COMMENT 'Foreign key linking to merchandising.buy_plan. Business justification: Pre-production samples are submitted and approved against specific buy plan commitments before bulk production is authorized. Apparel T&A calendars require sample approval status to be tracked per buy',
    `colorway_id` BIGINT COMMENT 'Foreign key linking to product.colorway. Business justification: PP samples are submitted and approved per colorway — fabric color, print, and trim approval are colorway-specific steps in the critical path. The plain colorway_code and colorway_name columns are deno',
    ...
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: PP samples carry season_code as a plain denormalized attribute but no FK to merchandising.season. Apparel PLM and production workflows require season-level sample approval tracking and reporting for r',
    `style_id` BIGINT COMMENT 'Reference to the parent style or product design that this sample represents.',
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: PP sample approval tracking requires knowing the specific factory that produced the sample, not just the vendor. Quality teams use factory-level sample history for production readiness decisions and r',
    `tech_pack_id` BIGINT COMMENT 'Foreign key linking to product.tech_pack. Business justification: PP samples are evaluated against a specific tech pack version — the sample review validates construction, measurements, and labeling per tech pack specs. tech_pack_version (plain column) is a denormal',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt` (
    ...
    `sourcing_purchase_order_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order. Business justification: Bulk fabric receipts are goods receipts against sourcing POs. Direct FK enables AP invoice matching and PO receipt reconciliation — a mandatory apparel procurement process where fabric received at fac',
    `sourcing_purchase_order_line_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order_line. Business justification: Bulk fabric receipts fulfill specific sourcing PO lines for material procurement. Direct FK enables three-way AP matching (PO line → goods receipt → invoice) — a standard apparel finance process for f',
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: Fabric traceability and compliance audits require identifying the specific supplier factory that produced or shipped bulk fabric, not just the vendor entity. This supports material traceability report',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`cut_order` (
    ...
    `bulk_fabric_receipt_id` BIGINT COMMENT 'Foreign key linking to production.bulk_fabric_receipt. Business justification: A cut_order authorizes the cutting of fabric panels from a specific bulk fabric receipt. This FK establishes the direct traceability link between fabric consumption (cut_order) and fabric sourcing (bu',
    ...
    `sourcing_purchase_order_line_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order_line. Business justification: Cut orders fulfill specific sourcing PO lines by style/colorway/size. Direct FK enables cut-to-PO traceability — a standard apparel production control process where cut quantities are validated agains',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`routing` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Routings define standard costs per operation (SAM × labor rate) assigned to cost centers — the Standard Cost Routing process in SAP PP/CO. Apparel costing teams use routing-to-cost-center links for ',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`lot` (
    ...
    `bulk_fabric_receipt_id` BIGINT COMMENT 'Foreign key linking to production.bulk_fabric_receipt. Business justification: A production lot (batch) is manufactured from fabric received in a specific bulk_fabric_receipt. This FK enables full material traceability from fabric receipt through cutting and sewing to finished l',
    `buy_plan_line_id` BIGINT COMMENT 'Foreign key linking to merchandising.buy_plan_line. Business justification: Lots are the physical production output at SKU/size level; buy plan lines are the SKU-level purchase commitments. Apparel operations require lot-to-buy-plan-line traceability for receipt confirmation,',
    ...
    `line_id` BIGINT COMMENT 'Identifier of the manufacturing line or assembly line where this lot was produced.',
    ...
    `schedule_id` BIGINT COMMENT 'Foreign key linking to production.schedule. Business justification: A production lot (batch) is manufactured under a specific master production schedule slot. This FK links the actual production batch (lot) to the planned schedule (schedule), enabling schedule adheren',
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: Lots carry season_code as a plain denormalized attribute but no FK to merchandising.season. End-of-season inventory reconciliation, markdown planning, and carryover analysis require lot-to-season trac',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Production lots track finished goods at SKU level for inventory reconciliation, quality traceability, and carton labeling. The plain sku column is denormalized; sku_id FK enables lot-to-SKU traceabi',
    `sourcing_purchase_order_line_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order_line. Business justification: Production lots fulfill specific sourcing PO lines (by SKU, size, color). This link enables PO line fulfillment tracking — a core apparel operations report showing ordered vs produced quantity by PO l',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`marker` (
    ...
    `size_scale_id` BIGINT COMMENT 'Foreign key linking to product.size_scale. Business justification: Markers are created for a specific size scale — the nesting of pattern pieces depends on which size run is being cut. Linking marker to size_scale enables marker efficiency analysis by size scale, gra',
    ...
    `creation_method` STRING COMMENT 'Method or system used to create the marker layout, indicating whether it was manually created, designed using CAD software, or generated automatically.',
    ...
    `length_cm` DECIMAL(18,2) COMMENT 'Total length of the marker layout measured in centimeters, representing the fabric length required for one marker repeat.',
    ...
    `notes` STRING COMMENT 'Additional notes, instructions, or comments related to the marker layout, cutting process, or special handling requirements.',
    ...
    `version` STRING COMMENT 'Version number of the marker layout, used to track revisions and improvements over time.',
    `width_cm` DECIMAL(18,2) COMMENT 'Width of the marker layout measured in centimeters, corresponding to the usable fabric width.',
    CONSTRAINT pk_marker PRIMARY KEY(`marker_id`)
) COMMENT 'Master reference table for marker. Referenced by marker_id.';

CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`line` (
    `line_id` BIGINT COMMENT 'Primary key for production_line',
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: Production lines in apparel are dedicated to specific product categories (e.g., knitwear, denim, footwear). The plain product_category column is denormalized; category_id FK enables factory capacity p',
    ...
    `length_meters` DECIMAL(18,2) COMMENT 'Physical length of the production line measured in meters, relevant for layout planning and material flow optimization.',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`production`.`delivery_window` (
    ...
    `collection_plan_id` BIGINT COMMENT 'Foreign key linking to merchandising.collection_plan. Business justification: Collection plans define first_delivery_date and last_delivery_date that production delivery windows must operationalize. Apparel planners trace delivery windows to collection plans for seasonal delive',
    ...
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: Delivery windows have season_code as a plain denormalized attribute but no FK to merchandising.season. Apparel capacity and receipt planning requires referential integrity between delivery windows and',
    ...
    `wholesale_account_id` BIGINT COMMENT 'Foreign key linking to customer.wholesale_account. Business justification: Wholesale capacity allocation: delivery windows in apparel-fashion are negotiated with and reserved for specific wholesale buyers. Operations teams track which wholesale accounts seasonal buying comm',
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`order_confirmation` ADD CONSTRAINT `fk_production_order_confirmation_lot_id` FOREIGN KEY (`lot_id`) REFERENCES `apparel_fashion_ecm`.`production`.`lot`(`lot_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`schedule` ADD CONSTRAINT `fk_production_schedule_line_id` FOREIGN KEY (`line_id`) REFERENCES `apparel_fashion_ecm`.`production`.`line`(`line_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ADD CONSTRAINT `fk_production_production_tna_milestone_bulk_fabric_receipt_id` FOREIGN KEY (`bulk_fabric_receipt_id`) REFERENCES `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt`(`bulk_fabric_receipt_id`);
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ADD CONSTRAINT `fk_production_production_tna_milestone_cut_order_id` FOREIGN KEY (`cut_order_id`) REFERENCES `apparel_fashion_ecm`.`production`.`cut_order`(`cut_order_id`);
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ADD CONSTRAINT `fk_production_production_tna_milestone_lot_id` FOREIGN KEY (`lot_id`) REFERENCES `apparel_fashion_ecm`.`production`.`lot`(`lot_id`);
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ADD CONSTRAINT `fk_production_production_tna_milestone_pp_sample_id` FOREIGN KEY (`pp_sample_id`) REFERENCES `apparel_fashion_ecm`.`production`.`pp_sample`(`pp_sample_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`cut_order` ADD CONSTRAINT `fk_production_cut_order_bulk_fabric_receipt_id` FOREIGN KEY (`bulk_fabric_receipt_id`) REFERENCES `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt`(`bulk_fabric_receipt_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ADD CONSTRAINT `fk_production_lot_bulk_fabric_receipt_id` FOREIGN KEY (`bulk_fabric_receipt_id`) REFERENCES `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt`(`bulk_fabric_receipt_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ADD CONSTRAINT `fk_production_lot_line_id` FOREIGN KEY (`line_id`) REFERENCES `apparel_fashion_ecm`.`production`.`line`(`line_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ADD CONSTRAINT `fk_production_lot_schedule_id` FOREIGN KEY (`schedule_id`) REFERENCES `apparel_fashion_ecm`.`production`.`schedule`(`schedule_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ADD CONSTRAINT `fk_production_line_feeds_production_line_id` FOREIGN KEY (`feeds_production_line_id`) REFERENCES `apparel_fashion_ecm`.`production`.`line`(`line_id`);
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ADD CONSTRAINT `fk_production_line_production_factory_id` FOREIGN KEY (`production_factory_id`) REFERENCES `apparel_fashion_ecm`.`production`.`production_factory`(`production_factory_id`);
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ADD CONSTRAINT `fk_production_line_work_center_id` FOREIGN KEY (`work_center_id`) REFERENCES `apparel_fashion_ecm`.`production`.`work_center`(`work_center_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_order` SET TAGS ('dbx_subdomain' = 'manufacturing_execution');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_order` ALTER COLUMN `colorway_id` SET TAGS ('dbx_business_glossary_term' = 'Colorway Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_order` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_order` ALTER COLUMN `style_id` SET TAGS ('dbx_business_glossary_term' = 'Style ID');
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_order` ALTER COLUMN `tech_pack_id` SET TAGS ('dbx_business_glossary_term' = 'Tech Pack Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_order_operation` SET TAGS ('dbx_subdomain' = 'manufacturing_execution');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`order_confirmation` SET TAGS ('dbx_subdomain' = 'manufacturing_execution');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`order_confirmation` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`order_confirmation` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_factory` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`factory_capacity` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`factory_capacity` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`factory_capacity` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`schedule` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`schedule` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`schedule` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Production Line ID');
ALTER TABLE `apparel_fashion_ecm`.`production`.`schedule` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season ID');
ALTER TABLE `apparel_fashion_ecm`.`production`.`schedule` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `bulk_fabric_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Bulk Fabric Receipt Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `buy_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Buy Plan Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `cut_order_id` SET TAGS ('dbx_business_glossary_term' = 'Cut Order Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `pp_sample_id` SET TAGS ('dbx_business_glossary_term' = 'Pp Sample Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`production_tna_milestone` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `buy_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Buy Plan Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `colorway_id` SET TAGS ('dbx_business_glossary_term' = 'Colorway Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `style_id` SET TAGS ('dbx_business_glossary_term' = 'Style ID');
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`pp_sample` ALTER COLUMN `tech_pack_id` SET TAGS ('dbx_business_glossary_term' = 'Tech Pack Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt` SET TAGS ('dbx_subdomain' = 'manufacturing_execution');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt` ALTER COLUMN `sourcing_purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt` ALTER COLUMN `sourcing_purchase_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Line Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`bulk_fabric_receipt` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`cut_order` SET TAGS ('dbx_subdomain' = 'manufacturing_execution');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`cut_order` ALTER COLUMN `bulk_fabric_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Bulk Fabric Receipt Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`cut_order` ALTER COLUMN `sourcing_purchase_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Line Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`routing` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`routing` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`work_center` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` SET TAGS ('dbx_subdomain' = 'manufacturing_execution');
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot ID');
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `bulk_fabric_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Bulk Fabric Receipt Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `buy_plan_line_id` SET TAGS ('dbx_business_glossary_term' = 'Buy Plan Line Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Production Line ID');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Schedule Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`lot` ALTER COLUMN `sourcing_purchase_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Line Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`marker` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`marker` ALTER COLUMN `size_scale_id` SET TAGS ('dbx_business_glossary_term' = 'Size Scale Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` SET TAGS ('dbx_subdomain' = 'factory_planning');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Production Line Identifier');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ALTER COLUMN `feeds_production_line_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ALTER COLUMN `standard_labor_cost_per_hour` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `apparel_fashion_ecm`.`production`.`line` ALTER COLUMN `standard_overhead_rate` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `apparel_fashion_ecm`.`production`.`delivery_window` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `apparel_fashion_ecm`.`production`.`delivery_window` SET TAGS ('dbx_subdomain' = 'factory_planning');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`delivery_window` ALTER COLUMN `collection_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Plan Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`production`.`delivery_window` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`production`.`delivery_window` ALTER COLUMN `wholesale_account_id` SET TAGS ('dbx_business_glossary_term' = 'Wholesale Account Id (Foreign Key)');
