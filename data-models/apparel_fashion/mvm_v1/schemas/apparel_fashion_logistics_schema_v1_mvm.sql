-- Schema for Domain: logistics | Business: Apparel Fashion | Version: v1_mvm
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`shipment` (
    ...
    `collection_id` BIGINT COMMENT 'Foreign key linking to product.collection. Business justification: Seasonal collection-level OTIF reporting and shipment planning are standard apparel logistics KPIs. Merchants and logistics teams track on-time delivery by collection to ensure floor-set readiness. Wi',
    `contract_id` BIGINT COMMENT 'Foreign key linking to logistics.contract. Business justification: Every shipment in apparel logistics is executed under a specific carrier or 3PL contract that governs rates, service levels, and terms. Linking shipment directly to contract enables contract utilizati',
    ...
    `letter_of_credit_id` BIGINT COMMENT 'Foreign key linking to finance.letter_of_credit. Business justification: Letters of credit are drawn upon presentation of shipping documents for a specific shipment. Apparel importers link LCs to shipments to verify latest shipment date compliance, track document presentat',
    `lot_id` BIGINT COMMENT 'Foreign key linking to production.lot. Business justification: Shipments carry goods from specific production lots. Direct lot-to-shipment traceability is critical for quality recalls — if a lot fails post-shipment QC, logistics must immediately identify the ship',
    `material_id` BIGINT COMMENT 'Foreign key linking to product.material. Business justification: Inbound raw material shipments from fabric/trim suppliers to factories must be traceable to the specific material record for production planning, restricted substance compliance, and sustainability ce',
    ...
    `schedule_id` BIGINT COMMENT 'Foreign key linking to production.schedule. Business justification: Production schedules drive ex-factory dates and shipment booking windows. Linking shipment to schedule enables OTIF monitoring — logistics planners compare actual shipment dates against planned ex-fac',
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: Season-level OTIF and delivery performance reporting: apparel shipments must be attributed to a season for on-time delivery KPI tracking by collection. Merchandising and logistics teams jointly review',
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to customer.address. Business justification: DTC last-mile delivery and carrier label generation require the customers validated shipping address directly on the shipment record. Apparel brands use this for address verification, carrier routing',
    ...
    `tna_calendar_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_tna_calendar. Business justification: In apparel fashion, each shipment is executed against a TNA calendars planned ex-factory and in-DC dates. Linking shipment to sourcing_tna_calendar enables direct OTIF measurement against the seasona',
    `wholesale_account_id` BIGINT COMMENT 'Foreign key linking to customer.wholesale_account. Business justification: Wholesale OTIF performance reporting and account-level logistics SLA tracking are standard KPIs in apparel wholesale operations. A direct wholesale_account_id on shipment enables account-level on-time',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` (
    ...
    `customs_entry_id` BIGINT COMMENT 'Foreign key linking to logistics.customs_entry. Business justification: Shipment milestones include customs clearance events (customs_cleared_flag, duty_amount, hs_code fields exist on shipment_milestone). Linking directly to the customs_entry record enables traceability ',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Milestone events (customs clearance, port arrival, delivery) trigger financial accruals posted to GL accounts — duty accruals, demurrage charges, and freight cost recognition. Apparel importers post m',
    ...
    `sourcing_tna_milestone_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_tna_milestone. Business justification: In apparel supply chain, logistics milestones (ex-factory, port departure, DC arrival) directly confirm or trigger sourcing TNA milestones. This link is core to critical path tracking and delay report',
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: Ex-factory and goods-ready milestones in apparel logistics must reference the originating supplier factory for OTIF factory performance reporting and CBP country-of-origin audit trails. A supply chain',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to production.work_order. Business justification: Shipment milestones (ex-factory, in-transit, delivered) are tracked against work orders in apparel TNA management. Production and logistics teams jointly monitor ex-factory milestone compliance per wo',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` (
    ...
    `collection_id` BIGINT COMMENT 'Foreign key linking to product.collection. Business justification: Freight bookings in apparel are made by collection/season to secure carrier capacity for seasonal deliveries. collection_season_code is a denormalized plain attribute; replacing it with a proper FK to',
    `contract_id` BIGINT COMMENT 'Foreign key linking to logistics.contract. Business justification: Freight bookings are made under specific carrier contracts or rate agreements. The freight_booking has a rate_agreement_reference string field that is a natural candidate for normalization to a FK to ',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Freight bookings are charged to cost centers for logistics budget tracking and variance reporting. Apparel supply chain finance teams monitor freight spend vs. cost center budget; this link enables fr',
    `delivery_window_id` BIGINT COMMENT 'Foreign key linking to production.delivery_window. Business justification: Freight bookings are made to meet specific delivery windows. The freight booking references the delivery window to ensure space is allocated within the correct ship window — a standard apparel logisti',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Freight booking costs require GL account assignment for freight expense accrual at booking stage. Apparel brands accrue freight charges before carrier invoice receipt; the GL account drives automated ',
    ...
    `letter_of_credit_id` BIGINT COMMENT 'Foreign key linking to finance.letter_of_credit. Business justification: Freight bookings under LC payment terms must reference the LC to verify latest shipment date compliance. Apparel sourcing teams check that the booking date does not exceed the LCs latest shipment dat',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Freight costs are allocated to profit centers for brand/channel P&L reporting. Apparel companies with multi-brand structures allocate inbound freight by profit center to compute accurate landed cost a',
    ...
    `sample_request_id` BIGINT COMMENT 'Foreign key linking to sourcing.sample_request. Business justification: Apparel sample shipments (proto, fit, PP samples) are booked via freight bookings (courier/air). Linking freight_booking to sample_request enables sample tracking, freight cost allocation to the sampl',
    `schedule_id` BIGINT COMMENT 'Foreign key linking to production.schedule. Business justification: Freight bookings are made based on the production schedules planned ex-factory date. Freight forwarders reference the schedule to confirm space allocation timing. This direct link enables automated b',
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: Seasonal freight capacity planning: freight_booking.collection_season_code is a denormalized plain attribute. Proper FK to merchandising.season enables season-level freight booking analysis, capacity ',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to production.work_order. Business justification: Freight bookings in apparel are made against specific work orders to coordinate ex-factory pickup timing and confirm quantities. Logistics planners and freight forwarders reference the work order to a',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`customs_entry` (
    ...
    `production_factory_id` BIGINT COMMENT 'Foreign key linking to production.production_factory. Business justification: Customs entries require country-of-origin declarations referencing the production factory. For brands with owned factories, production_factory is distinct from supplier_factory (already linked). Direc',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Customs entries drive duty costs allocated to profit centers for brand/channel P&L and landed cost reporting. Apparel importers assign customs entries to profit centers so duty expenses flow into the ',
    ...
    `sourcing_purchase_order_line_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order_line. Business justification: CBP customs entries in apparel are filed at line level (each entry line = one PO line with its own HS code, value, COO). Line-level FK enables duty drawback claims, trade compliance audits, and entry ',
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supplier.vendor. Business justification: Customs entries in apparel fashion must be attributable to the vendor for duty chargeback billing, vendor compliance reporting, and importer-of-record reconciliation. Finance teams run vendor-level du',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to production.work_order. Business justification: Customs entries cover goods produced under specific work orders. Direct link enables duty drawback claims and compliance audits tracing from production order to customs clearance — required by CBP and',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`hs_code` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: HS codes determine duty type and are mapped to GL accounts for duty expense classification — e.g., Section 301 surcharges post to a different GL than standard MFN duties. Apparel customs teams maintai',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` (
    ...
    `bom_id` BIGINT COMMENT 'Foreign key linking to product.bom. Business justification: Landed cost analysis and customs audit require tracing each duty calculation back to the BOM that defined material composition, country of origin, and HS codes. Apparel costing teams routinely reconci',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Duty costs are allocated to profit centers for landed cost and product-line P&L reporting. Apparel brands allocate import duties by brand/channel profit center to compute accurate landed cost per unit',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: SKU-level landed cost reporting and margin analysis require linking each duty calculation to the specific SKU imported. Apparel finance teams calculate IMU and LDP cost at SKU level; without this FK, ',
    ...
    `sourcing_purchase_order_line_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order_line. Business justification: Apparel duty calculations are performed at line level — each PO line has its own HS code, COO, and quantity. Line-level FK enables CBP entry line reconciliation, FTA/GSP claims per SKU, and landed cos',
    `supplier_factory_id` BIGINT COMMENT 'Foreign key linking to supplier.supplier_factory. Business justification: CBP duty assessment in apparel requires factory-level country-of-origin determination — GSP eligibility, Section 301 rates, and antidumping duties are factory-specific. Duty calculations must referenc',
    ...
    `vendor_cost_quote_id` BIGINT COMMENT 'Foreign key linking to sourcing.vendor_cost_quote. Business justification: Apparel landed cost management requires reconciling actual duty calculations against the vendor cost quotes duty_cost field. This FK enables LDP variance reporting and quote accuracy audits — a stand',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to production.work_order. Business justification: Landed duty paid (LDP) cost calculations in apparel are tracked per work order for margin analysis and costing. Finance and sourcing teams need duty_calculation linked to work_order to report total la',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`carrier` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Carriers are mapped to freight expense GL accounts for automated AP invoice coding. Apparel AP teams configure carrier-to-GL mappings so freight invoices are automatically posted to the correct expens',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`routing_guide` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to logistics.contract. Business justification: Routing guides define approved carrier assignments and are executed under specific carrier contracts. The routing_guide has a carrier_contract_number string field that directly maps to the contract ma',
    ...
    `production_factory_id` BIGINT COMMENT 'Foreign key linking to production.production_factory. Business justification: Routing guides in apparel specify factory-specific shipping instructions — carrier, mode, and lane from a specific origin factory to destination DC. Vendor compliance programs require factory-level ro',
    ...
    `season_id` BIGINT COMMENT 'Foreign key linking to merchandising.season. Business justification: Seasonal routing guide activation: routing guides in apparel are published per season to reflect seasonal lane rates, carrier contracts, and delivery windows. routing_guide.season_code is a denormaliz',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`distribution_center` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Distribution centers incur facility operating costs (rent, utilities, labor) posted to specific GL accounts. Apparel DC finance teams map each DC to its primary GL account for facility cost posting, l',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`third_party_logistics` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: 3PL costs are tracked against cost centers for warehousing and fulfillment budget management. Apparel companies with outsourced DC operations assign each 3PL provider to a cost center for spend monito',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: 3PL providers are mapped to GL accounts for warehousing and fulfillment expense posting. Apparel AP teams configure 3PL-to-GL mappings so that 3PL invoices are automatically coded to the correct expen',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` (
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to logistics.contract. Business justification: SLA agreements are typically embedded within or referenced by formal logistics contracts. The sla_agreement has a contract_reference_number string field that is a natural candidate for normalization t',
    `distribution_center_id` BIGINT COMMENT 'Foreign key linking to logistics.distribution_center. Business justification: SLA agreements in apparel logistics are frequently scoped to specific distribution centers (e.g., DC-level OTIF targets, inbound receiving SLAs, outbound fulfillment SLAs). The sla_agreement has origi',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: SLA penalty and bonus amounts must be posted to GL accounts as financial line items. Apparel brands track carrier/3PL SLA breach penalties as AP debit memos and bonus credits; the GL account on the SL',
    `lane_id` BIGINT COMMENT 'Foreign key linking to logistics.lane. Business justification: SLA agreements in freight logistics are commonly scoped to specific trade lanes (origin-destination pairs). A lane-level SLA defines performance commitments for a specific routing corridor (e.g., Shan',
    ...
    `production_factory_id` BIGINT COMMENT 'Foreign key linking to production.production_factory. Business justification: SLA agreements in apparel logistics are scoped to specific production factories — e.g., on-time ex-factory SLAs or inbound DC receiving SLAs tied to a factorys shipments. Factory-level SLA tracking a',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`packing_list` (
    ...
    `customs_entry_id` BIGINT COMMENT 'Foreign key linking to logistics.customs_entry. Business justification: A packing list is a primary supporting document for customs clearance. Each packing list is associated with a specific customs entry that declares the shipment contents to customs authorities. This FK',
    ...
    `lot_id` BIGINT COMMENT 'Foreign key linking to production.lot. Business justification: Packing lists in apparel are generated at the production lot level — each lot of finished garments is packed and documented. Linking packing_list to lot enables AQL quality traceability, recall manage',
    ...
    `sourcing_purchase_order_line_id` BIGINT COMMENT 'Foreign key linking to sourcing.sourcing_purchase_order_line. Business justification: Apparel packing lists are built at SKU/line level — each carton maps to specific PO lines. A direct FK to sourcing_purchase_order_line enables line-level ASN matching, DC receiving reconciliation, and',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`lane` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Lanes are assigned to cost centers for freight budget allocation and trade-lane-level variance analysis. Apparel supply chain finance teams budget freight by trade lane mapped to cost centers; this li',
    `distribution_center_id` BIGINT COMMENT 'Identifier for the destination location (warehouse, distribution center, store, or port) where shipments end.',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Trade lanes are mapped to freight expense GL accounts for automated cost posting. Apparel logistics finance teams configure lane-to-GL mappings so that shipment costs on each lane are automatically co',
    `lane_distribution_center_id` BIGINT COMMENT 'Identifier for the origin location (warehouse, distribution center, or port) where shipments begin.',
    `production_factory_id` BIGINT COMMENT 'Foreign key linking to production.production_factory. Business justification: Logistics lanes in apparel are defined between origin production factories and destination DCs. Factory-specific lane management enables accurate rate negotiation and transit time planning per factory',
...
CREATE OR REPLACE TABLE `apparel_fashion_ecm`.`logistics`.`contract` (
    ...
    `carrier_id` BIGINT COMMENT 'Foreign key linking to logistics.carrier. Business justification: A logistics contract in the apparel supply chain is primarily a carrier contract — it governs the terms, rates, and service levels with a specific carrier. The contract table has no carrier_id FK curr',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Logistics contracts are budgeted and tracked against cost centers for freight and 3PL spend management. Apparel supply chain finance monitors contract spend vs. cost center budget; this link enables c',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Logistics contracts (carrier, 3PL) are linked to GL accounts for contract accrual and commitment accounting. Apparel finance teams post contract-level freight and warehousing accruals to specific GL a',
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ADD CONSTRAINT `fk_logistics_shipment_contract_id` FOREIGN KEY (`contract_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`contract`(`contract_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` ADD CONSTRAINT `fk_logistics_shipment_milestone_customs_entry_id` FOREIGN KEY (`customs_entry_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`customs_entry`(`customs_entry_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ADD CONSTRAINT `fk_logistics_freight_booking_contract_id` FOREIGN KEY (`contract_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`contract`(`contract_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`routing_guide` ADD CONSTRAINT `fk_logistics_routing_guide_contract_id` FOREIGN KEY (`contract_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`contract`(`contract_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ADD CONSTRAINT `fk_logistics_sla_agreement_contract_id` FOREIGN KEY (`contract_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`contract`(`contract_id`);
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ADD CONSTRAINT `fk_logistics_sla_agreement_distribution_center_id` FOREIGN KEY (`distribution_center_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`distribution_center`(`distribution_center_id`);
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ADD CONSTRAINT `fk_logistics_sla_agreement_lane_id` FOREIGN KEY (`lane_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`lane`(`lane_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`packing_list` ADD CONSTRAINT `fk_logistics_packing_list_customs_entry_id` FOREIGN KEY (`customs_entry_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`customs_entry`(`customs_entry_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`lane` ADD CONSTRAINT `fk_logistics_lane_lane_distribution_center_id` FOREIGN KEY (`lane_distribution_center_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`distribution_center`(`distribution_center_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`contract` ADD CONSTRAINT `fk_logistics_contract_carrier_id` FOREIGN KEY (`carrier_id`) REFERENCES `apparel_fashion_ecm`.`logistics`.`carrier`(`carrier_id`);
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `collection_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `letter_of_credit_id` SET TAGS ('dbx_business_glossary_term' = 'Letter Of Credit Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `material_id` SET TAGS ('dbx_business_glossary_term' = 'Material Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Schedule Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Shipping Address Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `tna_calendar_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Tna Calendar Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment` ALTER COLUMN `wholesale_account_id` SET TAGS ('dbx_business_glossary_term' = 'Wholesale Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` ALTER COLUMN `customs_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Customs Entry Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` ALTER COLUMN `sourcing_tna_milestone_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Tna Milestone Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`shipment_milestone` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `collection_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `delivery_window_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Window Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `letter_of_credit_id` SET TAGS ('dbx_business_glossary_term' = 'Letter Of Credit Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `sample_request_id` SET TAGS ('dbx_business_glossary_term' = 'Sample Request Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Schedule Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`freight_booking` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`customs_entry` ALTER COLUMN `production_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Production Factory Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`customs_entry` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`customs_entry` ALTER COLUMN `sourcing_purchase_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Line Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`customs_entry` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`customs_entry` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`hs_code` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `bom_id` SET TAGS ('dbx_business_glossary_term' = 'Bom Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `sourcing_purchase_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Line Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `supplier_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `vendor_cost_quote_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Cost Quote Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`duty_calculation` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`carrier` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`routing_guide` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`routing_guide` ALTER COLUMN `production_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Origin Production Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`routing_guide` ALTER COLUMN `season_id` SET TAGS ('dbx_business_glossary_term' = 'Season Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`distribution_center` SET TAGS ('dbx_subdomain' = 'network_partners');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`distribution_center` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`third_party_logistics` SET TAGS ('dbx_subdomain' = 'network_partners');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`third_party_logistics` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`third_party_logistics` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` SET TAGS ('dbx_subdomain' = 'network_partners');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ALTER COLUMN `distribution_center_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Center Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ALTER COLUMN `lane_id` SET TAGS ('dbx_business_glossary_term' = 'Lane Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`sla_agreement` ALTER COLUMN `production_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Production Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`packing_list` SET TAGS ('dbx_subdomain' = 'freight_movement');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`packing_list` ALTER COLUMN `customs_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Customs Entry Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`packing_list` ALTER COLUMN `lot_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`packing_list` ALTER COLUMN `sourcing_purchase_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Purchase Order Line Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`lane` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`lane` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`lane` ALTER COLUMN `production_factory_id` SET TAGS ('dbx_business_glossary_term' = 'Origin Production Factory Id (Foreign Key)');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`contract` SET TAGS ('dbx_subdomain' = 'network_partners');
...
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`contract` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`contract` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `apparel_fashion_ecm`.`logistics`.`contract` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
