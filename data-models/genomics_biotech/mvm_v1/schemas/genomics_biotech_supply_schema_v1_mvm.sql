-- Schema for Domain: supply | Business: Genomics Biotech | Version: v1_mvm
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: Custom genomics projects (bespoke sequencing reagents, custom arrays) require customer-specific procurement where POs are raised against a specific customer account for cost allocation, project billin',
    ...
    `plant_id` BIGINT COMMENT 'Foreign key linking to supply.plant. Business justification: purchase_order has plant_code as a STRING attribute, which is a denormalized reference to the supply.plant master. In SAP MM, every purchase order is issued for a specific plant. Replacing plant_code ',
    `submission_id` BIGINT COMMENT 'Foreign key linking to regulatory.submission. Business justification: GMP procurement of regulated genomics/IVD materials requires POs to reference the regulatory submission authorizing the product for the target jurisdiction. Regulatory affairs and procurement teams ve',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: In SAP MM procurement, purchase orders are frequently released against a supplier contract or scheduling agreement. purchase_order currently has no FK to supplier_contract. Adding supplier_contract_id',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`po_line` (
    ...
    `plant_id` BIGINT COMMENT 'Foreign key linking to supply.plant. Business justification: po_line has plant_code as a STRING attribute, which is a denormalized reference to the supply.plant master. Each PO line item is destined for a specific plant in SAP MM. Replacing plant_code with a pr',
    `purchase_order_id` BIGINT COMMENT 'Reference to the parent purchase order header under which this line item is grouped.',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: Individual PO line items in SAP MM can reference specific contract line items (outline agreements). po_line currently has no FK to supplier_contract. Adding supplier_contract_id at the line level enab',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` (
    ...
    `coa_id` BIGINT COMMENT 'Foreign key linking to reagent.coa. Business justification: GMP goods receipts in genomics biotech are accompanied by Certificates of Analysis that must be validated before material release. A proper FK to reagent.coa enables automated CoA validation at receip',
    `device_identifier_id` BIGINT COMMENT 'Foreign key linking to regulatory.device_identifier. Business justification: GMP receiving inspection for regulated IVD/genomics products requires UDI device identifier verification at goods receipt. FDA UDI rule and EU MDR mandate UDI scanning/verification during receiving. T',
    ...
    `po_line_id` BIGINT COMMENT 'Foreign key linking to supply.po_line. Business justification: A goods receipt in SAP MM is posted against a specific PO line item, not just the PO header. goods_receipt already has purchase_order_id but lacks po_line_id, making it impossible to trace which speci',
    `production_batch_id` BIGINT COMMENT 'Foreign key linking to manufacturing.production_batch. Business justification: When a genomics manufacturing production batch is completed, a goods receipt posts it into inventory. GMP traceability requires linking the goods receipt to the production batch for batch disposition,',
    ...
    `warehouse_location_id` BIGINT COMMENT 'Foreign key linking to supply.warehouse_location. Business justification: When materials are received (goods receipt posted), they are physically placed in a specific warehouse location (bin/rack). goods_receipt has storage_location_id (reagent domain) and plant_id but no F',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`inventory_stock` (
    ...
    `account_id` BIGINT COMMENT 'Foreign key linking to customer.account. Business justification: Genomics biotech companies place reagents and consumables on consignment at strategic customer accounts (hospitals, core labs). inventory_stock must identify which customer account holds consignment s',
    ...
    `warehouse_location_id` BIGINT COMMENT 'Foreign key linking to supply.warehouse_location. Business justification: inventory_stock tracks stock positions at plant and storage location level but lacks a direct FK to warehouse_location. In a genomics biotech supply chain with cold-chain and GMP zone requirements, kn',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to manufacturing.work_order. Business justification: Inventory stock reservations in genomics biotech are made against specific work orders for material staging. MRP reservation management requires linking reserved stock to the work order that triggered',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`stock_movement` (
    ...
    `inbound_delivery_id` BIGINT COMMENT 'Foreign key linking to supply.inbound_delivery. Business justification: stock_movement currently stores delivery_number as a denormalized STRING reference to the inbound delivery document. Replacing this with a proper FK inbound_delivery_id to supply.inbound_delivery.inbo',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to manufacturing.work_order. Business justification: Material consumption stock movements (goods issues) in genomics biotech are posted against work orders for cost accounting and GMP material traceability. production_order_number on stock_movement is a',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`batch` (
    ...
    `device_identifier_id` BIGINT COMMENT 'Foreign key linking to regulatory.device_identifier. Business justification: FDA UDI rule and EU MDR require lot/batch-level UDI-PI traceability linking each batch to its UDI device identifier. material has device_identifier_id for product-level UDI, but batch-level UDI tracea',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`inbound_delivery` (
    ...
    `submission_id` BIGINT COMMENT 'Foreign key linking to regulatory.submission. Business justification: Import clearance for regulated genomics/IVD products (sequencing reagents, diagnostic kits) requires referencing the import license or regulatory submission. inbound_delivery already tracks customs_cl',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` (
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Genomics supplier contracts are negotiated at SKU level (specific pack sizes, configurations, cold-chain requirements). covered_catalog_item_id covers catalog-level contracts; SKU-level contracts capt',
    `material_id` BIGINT COMMENT 'Foreign key linking to supply.material. Business justification: A supplier contract in genomics biotech governs the procurement of specific materials (reagents, raw chemicals, biologicals). supplier_contract currently references covered_catalog_item_id (product do',
    `plant_id` BIGINT COMMENT 'Foreign key linking to supply.plant. Business justification: supplier_contract has plant_code as a STRING attribute, which is a denormalized reference to the supply.plant master. Replacing plant_code with a proper FK plant_id normalizes the relationship and ena',
    `submission_id` BIGINT COMMENT 'Foreign key linking to regulatory.submission. Business justification: Supplier contracts for regulated genomics/IVD materials must reference the regulatory submission authorizing the contracted product for the target market. Regulatory affairs reviews submission status ',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`supplier_qualification` (
    ...
    `catalog_item_id` BIGINT COMMENT 'Foreign key linking to product.catalog_item. Business justification: Supplier qualifications in genomics are scoped to specific catalog items (e.g., Supplier X qualified for Sequencing Kit Y). approved_material_scope is a text denormalization of catalog_item. Structure',
    `establishment_registration_id` BIGINT COMMENT 'Foreign key linking to regulatory.establishment_registration. Business justification: GMP supplier qualification explicitly verifies and records the suppliers establishment registration as part of the qualification dossier. The qualification record must directly reference which establ',
...
CREATE OR REPLACE TABLE `genomics_biotech_ecm`.`supply`.`plant` (
    ...
    `establishment_registration_id` BIGINT COMMENT 'Foreign key linking to regulatory.establishment_registration. Business justification: Each manufacturing/distribution plant must be registered with FDA/competent authorities as an establishment. The plant record must reference its establishment registration for GMP compliance tracking,',
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` ADD CONSTRAINT `fk_supply_purchase_order_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`plant`(`plant_id`);
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` ADD CONSTRAINT `fk_supply_purchase_order_supplier_contract_id` FOREIGN KEY (`supplier_contract_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`supplier_contract`(`supplier_contract_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`po_line` ADD CONSTRAINT `fk_supply_po_line_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`plant`(`plant_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`po_line` ADD CONSTRAINT `fk_supply_po_line_supplier_contract_id` FOREIGN KEY (`supplier_contract_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`supplier_contract`(`supplier_contract_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ADD CONSTRAINT `fk_supply_goods_receipt_po_line_id` FOREIGN KEY (`po_line_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`po_line`(`po_line_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ADD CONSTRAINT `fk_supply_goods_receipt_warehouse_location_id` FOREIGN KEY (`warehouse_location_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`warehouse_location`(`warehouse_location_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`inventory_stock` ADD CONSTRAINT `fk_supply_inventory_stock_warehouse_location_id` FOREIGN KEY (`warehouse_location_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`warehouse_location`(`warehouse_location_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`stock_movement` ADD CONSTRAINT `fk_supply_stock_movement_inbound_delivery_id` FOREIGN KEY (`inbound_delivery_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`inbound_delivery`(`inbound_delivery_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` ADD CONSTRAINT `fk_supply_supplier_contract_material_id` FOREIGN KEY (`material_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`material`(`material_id`);
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` ADD CONSTRAINT `fk_supply_supplier_contract_plant_id` FOREIGN KEY (`plant_id`) REFERENCES `genomics_biotech_ecm`.`supply`.`plant`(`plant_id`);
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`material` SET TAGS ('dbx_subdomain' = 'inventory_control');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` SET TAGS ('dbx_subdomain' = 'procurement_operations');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` ALTER COLUMN `submission_id` SET TAGS ('dbx_business_glossary_term' = 'Submission Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`purchase_order` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`po_line` SET TAGS ('dbx_subdomain' = 'procurement_operations');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`po_line` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`po_line` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ALTER COLUMN `coa_id` SET TAGS ('dbx_business_glossary_term' = 'Coa Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ALTER COLUMN `device_identifier_id` SET TAGS ('dbx_business_glossary_term' = 'Device Identifier Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ALTER COLUMN `po_line_id` SET TAGS ('dbx_business_glossary_term' = 'Po Line Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ALTER COLUMN `production_batch_id` SET TAGS ('dbx_business_glossary_term' = 'Production Batch Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`goods_receipt` ALTER COLUMN `warehouse_location_id` SET TAGS ('dbx_business_glossary_term' = 'Warehouse Location Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`inventory_stock` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`inventory_stock` ALTER COLUMN `warehouse_location_id` SET TAGS ('dbx_business_glossary_term' = 'Warehouse Location Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`inventory_stock` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`stock_movement` ALTER COLUMN `inbound_delivery_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Delivery Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`stock_movement` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`batch` SET TAGS ('dbx_subdomain' = 'inventory_control');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`batch` ALTER COLUMN `device_identifier_id` SET TAGS ('dbx_business_glossary_term' = 'Device Identifier Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`inbound_delivery` SET TAGS ('dbx_subdomain' = 'procurement_operations');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`inbound_delivery` ALTER COLUMN `submission_id` SET TAGS ('dbx_business_glossary_term' = 'Submission Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Covered Sku Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` ALTER COLUMN `material_id` SET TAGS ('dbx_business_glossary_term' = 'Material Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_contract` ALTER COLUMN `submission_id` SET TAGS ('dbx_business_glossary_term' = 'Submission Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse_location` SET TAGS ('dbx_subdomain' = 'warehouse_facilities');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_qualification` ALTER COLUMN `catalog_item_id` SET TAGS ('dbx_business_glossary_term' = 'Catalog Item Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`supplier_qualification` ALTER COLUMN `establishment_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Establishment Registration Id (Foreign Key)');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` SET TAGS ('dbx_subdomain' = 'warehouse_facilities');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `plant_id` SET TAGS ('dbx_business_glossary_term' = 'Plant Identifier');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `establishment_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Establishment Registration Id (Foreign Key)');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `parent_plant_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `address` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `address` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `manager_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `manager_name` SET TAGS ('dbx_pii_name' = 'true');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`plant` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` SET TAGS ('dbx_subdomain' = 'warehouse_facilities');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `address_line1` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `address_line2` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `address_line2` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `genomics_biotech_ecm`.`supply`.`warehouse` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
