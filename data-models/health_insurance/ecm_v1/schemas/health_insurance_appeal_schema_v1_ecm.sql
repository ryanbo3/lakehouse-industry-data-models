-- Schema for Domain: appeal | Business: Health Insurance | Version: v1_ecm
...
-- ========= TABLES =========
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` (
    `appeal_grievance_id` BIGINT COMMENT 'Unique surrogate key for the grievance record.',
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the internal user or case manager assigned to the grievance.',
    ...
    `party_id` BIGINT COMMENT 'Unique identifier of the party who filed the grievance (member_id, provider_id, etc.).',
    ...
    `header_id` BIGINT COMMENT 'Identifier of the claim associated with the grievance, if applicable.',
    ...
    `related_encounter_number` BIGINT COMMENT 'Identifier of the clinical encounter linked to the grievance, if applicable.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`case` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Appeal case assignment process requires linking each case to the employee handling it for tracking case manager workload and regulatory reporting.',
    `header_id` BIGINT COMMENT 'Identifier of the claim that triggered the appeal.',
    `compliance_regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Each appeal case is subject to a regulatory obligation (e.g., timeliness); linking enables monitoring of compliance with that obligation.',
    `contract_provider_contract_id` BIGINT COMMENT 'Foreign key linking to contract.provider_contract. Business justification: Appeal adjudication must reference the governing provider contract to apply correct fee schedule, risk‑share and compliance rules per CMS regulations.',
    ...
    `member_risk_score_id` BIGINT COMMENT 'Foreign key linking to risk.member_risk_score. Business justification: Appeal case review report requires the members risk score to assess fraud likelihood and high‑cost patterns, a standard underwriting‑risk analysis step.',
    ...
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Provider Appeal Summary Report tracks which provider is the respondent for each appeal case.',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to credential.credential_record. Business justification: Required for the Credential Verification for Appeal report that checks provider credential status when a claim denial triggers an appeal.',
    ...
    `related_diagnosis_code` STRING COMMENT 'ICD diagnosis code linked to the appealed service.',
    `related_drug_ndc` STRING COMMENT 'National Drug Code of the medication involved, if a pharmacy appeal.',
    `related_service_code` STRING COMMENT 'CPT code of the service at issue in the appeal.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`adverse_determination` (
    ...
    `audit_finding_id` BIGINT COMMENT 'Foreign key linking to compliance.audit_finding. Business justification: Adverse determinations are audited for compliance; linking each determination to its audit finding supports audit trail and corrective actions.',
    ...
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Adverse Determination Provider Report needs a direct FK to provider for audit and risk analysis.',
    ...
    `network_status` STRING COMMENT 'Network relationship of the provider for the denied service.. Valid values are `in_network|out_of_network|preferred|non_preferred`',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`review` (
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to credential.credential_record. Business justification: Appeal reviewers must reference the providers credential record to assess if credentialing caused the denial, per the Appeal Review Credential Check process.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`external_review` (
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to credential.credential_record. Business justification: External review organizations need the providers credential record to validate eligibility during independent appeal assessments.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`timeline` (
    ...
    `org_unit_id` BIGINT COMMENT 'Internal team or workgroup responsible for the appeal.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`appeal_document` (
    `appeal_document_id` BIGINT COMMENT 'Unique identifier for the appeal document.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` (
    `appeal_communication_id` BIGINT COMMENT 'System-generated unique identifier for each appeal communication record.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` (
    ...
    `compliance_regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Coverage disputes may trigger regulatory reporting obligations; linking captures the governing obligation for each dispute.',
    `party_id` BIGINT COMMENT 'System identifier of the party that initiated the dispute.',
    `enrollment_eligibility_span_id` BIGINT COMMENT 'Foreign key linking to enrollment.enrollment_eligibility_span. Business justification: Eligibility disputes reference the exact eligibility span; needed for the Eligibility Dispute Analysis report required by CMS.',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to credential.credential_record. Business justification: Coverage disputes often arise from credentialing issues; linking to credential_record enables the Dispute Credential Impact analysis.',
    ...
    `disputed_benefit_code` STRING COMMENT 'Code of the benefit, service, or procedure that is the subject of the dispute.. Valid values are `^[A-Z0-9]{5}$`',
    ...
    `other_carrier_code` BIGINT COMMENT 'System identifier of the secondary carrier.',
    `other_carrier_name` STRING COMMENT 'Name of the secondary carrier involved in a COB dispute.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` (
    `appeal_regulatory_filing_id` BIGINT COMMENT 'System generated unique identifier for the appeal regulatory filing record.',
    `compliance_regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Regulatory filing of appeal data must reference the specific regulatory obligation it satisfies, required for compliance reporting.',
    `group_id` BIGINT COMMENT 'Foreign key linking to employer.employer_group. Business justification: Regulatory filings must include employer identifier to satisfy state/federal reporting of appeal outcomes.',
    `identity_id` BIGINT COMMENT 'Unique identifier of the member who initiated the appeal.',
    `provider_id` BIGINT COMMENT 'Unique identifier of the provider associated with the appeal (if applicable).',
    `acknowledgment_date` TIMESTAMP COMMENT 'Timestamp when the regulators acknowledgment was received.',
    `acknowledgment_received` BOOLEAN COMMENT 'Indicates whether an acknowledgment of receipt was received from the regulator.',
    `corrective_action_plan` STRING COMMENT 'Plan outlining steps to remediate deficiencies noted by the regulator.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the filing record was first created in the data lake.',
    `currency_code` STRING COMMENT 'Three‑letter ISO 4217 currency code for the filing amount.',
    `deficiency_description` STRING COMMENT 'Text description of the issues identified by the regulator.',
    `deficiency_notice` BOOLEAN COMMENT 'Flag indicating whether the regulator issued a deficiency notice for the filing.',
    `filing_amount` DECIMAL(18,2) COMMENT 'Monetary amount associated with the filing (e.g., filing fee).',
    `filing_date` TIMESTAMP COMMENT 'Timestamp when the filing was initially submitted to the regulator.',
    `filing_method` STRING COMMENT 'Technical method used to submit the filing (e.g., EDI transaction, web portal upload).. Valid values are `EDI|WebPortal|ManualUpload|Fax`',
    `filing_number` STRING COMMENT 'External filing number assigned by the regulatory body for tracking the appeal submission.',
    `filing_status` STRING COMMENT 'Current processing status of the filing within the regulatory workflow.. Valid values are `pending|submitted|accepted|rejected|closed`',
    `filing_type` STRING COMMENT 'Category of the regulatory filing (e.g., appeal, grievance, external review, state hearing).. Valid values are `appeal|grievance|external_review|state_hearing`',
    `filing_version` STRING COMMENT 'Version identifier for the filing document (e.g., v1, v2).',
    `notes` STRING COMMENT 'Free‑form comments or additional information about the filing.',
    `regulatory_body` STRING COMMENT 'The governing agency or organization to which the filing is submitted.. Valid values are `CMS|State_DOI|NCQA|URAC|HHS`',
    `reporting_period` STRING COMMENT 'The calendar or fiscal period the filing covers (e.g., 2023Q1).',
    `resubmission_date` TIMESTAMP COMMENT 'Timestamp of the resubmitted filing, if applicable.',
    `resubmission_required` BOOLEAN COMMENT 'Indicates whether a resubmission of the filing is required.',
    `submission_method` STRING COMMENT 'Channel through which the filing was delivered to the regulator.. Valid values are `Electronic|Paper|Phone|Email`',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`iro_organization` (
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to vendor.vendor. Business justification: REQUIRED: IROs are external vendors; linking enables compliance reports and performance metrics for each IRO contract.',
...
CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`penalty` (
    `penalty_id` BIGINT COMMENT 'Unique system-generated identifier for each appeal penalty record.',
    `compliance_regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Penalty assessments arise from violations of specific regulatory obligations; linking ensures traceability for audit and reporting.',
    `health_plan_id` BIGINT COMMENT 'Reference to the plan entity that incurred the penalty.',
    `identity_id` BIGINT COMMENT 'Member whose claim or appeal triggered the penalty, when relevant.',
    `provider_id` BIGINT COMMENT 'Provider linked to the penalty event, when relevant.',
    `parent_penalty_id` BIGINT COMMENT 'Self-referencing FK on penalty (parent_penalty_id)',
    `amount` DECIMAL(18,2) COMMENT 'Gross monetary value assessed before any adjustments.',
    `appeal_deadline` DATE COMMENT 'Last date the plan or member may contest the penalty.',
    `appeal_filed_date` DATE COMMENT 'Date the appeal against the penalty was received.',
    `appeal_filed_flag` BOOLEAN COMMENT 'True if an appeal was submitted, otherwise false.',
    `appeal_outcome` STRING COMMENT 'Result of the appeal after review.. Valid values are `upheld|reversed|modified|withdrawn`',
    `assessment_timestamp` TIMESTAMP COMMENT 'Exact date and time the penalty became effective.',
    `audit_trail` STRING COMMENT 'Path or identifier for detailed audit records related to the penalty.',
    `compliance_flag` STRING COMMENT 'Indicates whether the underlying process was in compliance when the penalty was issued.. Valid values are `compliant|non_compliant|pending`',
    `currency_code` STRING COMMENT 'Three‑letter code representing the currency of the penalty.. Valid values are `^[A-Z]{3}$`',
    `due_date` DATE COMMENT 'Deadline for payment to avoid additional sanctions.',
    `effective_from` DATE COMMENT 'Start date of the penaltys enforceability.',
    `effective_until` DATE COMMENT 'End date after which the penalty is no longer applicable.',
    `interest_amount` DECIMAL(18,2) COMMENT 'Additional monetary charge accrued on the penalty.',
    `notes` STRING COMMENT 'Additional comments or contextual information.',
    `payment_date` DATE COMMENT 'Date on which the penalty amount was settled.',
    `payment_status` STRING COMMENT 'Indicates whether the penalty has been paid or remains outstanding.. Valid values are `unpaid|paid|partial|settled`',
    `penalty_category` STRING COMMENT 'High‑level grouping of the penalty purpose.. Valid values are `financial|operational|clinical|administrative`',
    `penalty_number` STRING COMMENT 'Business-assigned identifier used in regulatory filings and internal tracking.',
    `penalty_status` STRING COMMENT 'Workflow state of the penalty record.. Valid values are `assessed|appealed|paid|waived|reversed|closed`',
    `penalty_type` STRING COMMENT 'Classification of the penalty based on its source or nature.. Valid values are `civil_money|state_fine|auto_approval|other`',
    `reason_code` STRING COMMENT 'Compact code used by the organization to categorize penalty causes.. Valid values are `^[A-Z0-9]{3,10}$`',
    `reason_description` STRING COMMENT 'Narrative explanation of why the penalty was assessed.',
    `record_audit_created` TIMESTAMP COMMENT 'Date‑time the row was initially loaded.',
    `record_audit_updated` TIMESTAMP COMMENT 'Date‑time of the latest modification.',
    `regulatory_body` STRING COMMENT 'Government or oversight entity that issued the penalty.. Valid values are `CMS|DOI|State|Federal|Other`',
    `regulatory_reference` STRING COMMENT 'Legal or regulatory provision that the penalty addresses.',
    `severity` STRING COMMENT 'Risk or impact rating assigned to the penalty.. Valid values are `low|medium|high|critical`',
    `source_record_reference` STRING COMMENT 'Primary key of the penalty record in the source system.',
    `source_system` STRING COMMENT 'Name of the operational system that supplied the penalty data.',
    `total_amount` DECIMAL(18,2) COMMENT 'Net amount payable after interest and any reductions.',
    CONSTRAINT pk_penalty PRIMARY KEY(`penalty_id`)
) COMMENT 'Tracks regulatory penalties, sanctions, and financial consequences imposed on the health plan for non-compliance with appeal processing requirements — including CMS Civil Money Penalties (CMPs), state DOI fines, and auto-approval penalties for missed turnaround times.';

CREATE OR REPLACE TABLE `health_insurance_ecm`.`appeal`.`evidence` (
    `evidence_id` BIGINT COMMENT 'Unique identifier for the appeal evidence record.',
    `provider_id` BIGINT COMMENT 'Foreign key linking to provider.provider_provider. Business justification: Evidence Source Attribution Report links evidence submitted by a provider to the provider entity.',
    `case_id` BIGINT COMMENT 'Identifier of the appeal to which this evidence is attached.',
    `pa_request_id` BIGINT COMMENT 'Identifier of the prior authorization related to the evidence.',
    `header_id` BIGINT COMMENT 'Identifier of the claim associated with the evidence, if applicable.',
    `parent_evidence_id` BIGINT COMMENT 'Self-referencing FK on evidence (parent_evidence_id)',
    `author_name` STRING COMMENT 'Full name of the author of the evidence.',
    `author_npi` STRING COMMENT 'National Provider Identifier of the author of the evidence, if applicable.',
    `checksum` STRING COMMENT 'Checksum (e.g., SHA-256) for file integrity verification.',
    `clinical_code_icd10` STRING COMMENT 'Associated ICD-10 diagnosis code(s) referenced in the evidence.',
    `confidentiality_flag` BOOLEAN COMMENT 'Indicates if the evidence contains confidential information requiring restricted handling.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the evidence record was created in the system.',
    `drg_code` STRING COMMENT 'Diagnosis Related Group code associated with the evidence.',
    `effective_date` DATE COMMENT 'Date when the evidence becomes effective for the appeal review.',
    `evidence_description` STRING COMMENT 'Detailed description of the evidence content and purpose.',
    `evidence_number` STRING COMMENT 'Business identifier assigned to the evidence by the source system.',
    `evidence_status` STRING COMMENT 'Current processing status of the evidence.. Valid values are `pending|reviewed|accepted|rejected|withdrawn`',
    `evidence_type` STRING COMMENT 'Category of the evidence submitted for the appeal. [ENUM-REF-CANDIDATE: medical_record|clinical_guideline|peer_reviewed_literature|expert_opinion|lab_result|imaging|authorization|other — 8 candidates stripped; promote to reference product]',
    `expiration_date` DATE COMMENT 'Date after which the evidence is no longer considered valid.',
    `file_format` STRING COMMENT 'File format of the digital evidence document. [ENUM-REF-CANDIDATE: pdf|docx|txt|jpg|png|tiff|xml|json — 8 candidates stripped; promote to reference product]',
    `file_size_bytes` BIGINT COMMENT 'Size of the evidence file in bytes.',
    `guideline_version` STRING COMMENT 'Version of the clinical guideline cited in the evidence.',
    `ndc_code` STRING COMMENT 'National Drug Code if the evidence pertains to a medication.',
    `notes` STRING COMMENT 'Free-text notes or comments about the evidence.',
    `phi_flag` BOOLEAN COMMENT 'Indicates whether the evidence includes protected health information.',
    `procedure_code_cpt` STRING COMMENT 'Associated CPT procedure code(s) referenced in the evidence.',
    `received_timestamp` TIMESTAMP COMMENT 'Date and time when the evidence was received by the processing system.',
    `related_encounter_number` BIGINT COMMENT 'Identifier of the clinical encounter linked to the evidence.',
    `relevance_score` DECIMAL(18,2) COMMENT 'Numeric score indicating the relevance of the evidence to the appeal (0-100).',
    `source_entity_type` STRING COMMENT 'Type of entity that provided the evidence.. Valid values are `member|provider|external|internal`',
    `source_system` STRING COMMENT 'Name of the source system that supplied the evidence.',
    `source_system_record_code` STRING COMMENT 'Identifier of the original record in the source system.',
    `storage_path` STRING COMMENT 'Logical storage location or URI where the evidence file is stored.',
    `submission_timestamp` TIMESTAMP COMMENT 'Date and time when the evidence was submitted to the appeal process.',
    `title` STRING COMMENT 'Brief title describing the evidence.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to the evidence record.',
    `version_number` STRING COMMENT 'Version of the evidence document, incremented on updates.',
    CONSTRAINT pk_evidence PRIMARY KEY(`evidence_id`)
) COMMENT 'Master record for clinical and non-clinical evidence submitted or gathered during the appeal review process — medical records, clinical guidelines cited, peer-reviewed literature, and expert opinions. Distinct from appeal_document (which tracks document logistics) by focusing on the evidentiary substance and its weight in the review decision.';

-- ========= FOREIGN KEYS =========
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ADD CONSTRAINT `fk_appeal_appeal_grievance_case_id` FOREIGN KEY (`case_id`) REFERENCES `health_insurance_ecm`.`appeal`.`case`(`case_id`);
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ADD CONSTRAINT `fk_appeal_appeal_document_case_id` FOREIGN KEY (`case_id`) REFERENCES `health_insurance_ecm`.`appeal`.`case`(`case_id`);
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ADD CONSTRAINT `fk_appeal_appeal_communication_case_id` FOREIGN KEY (`case_id`) REFERENCES `health_insurance_ecm`.`appeal`.`case`(`case_id`);
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ADD CONSTRAINT `fk_appeal_penalty_parent_penalty_id` FOREIGN KEY (`parent_penalty_id`) REFERENCES `health_insurance_ecm`.`appeal`.`penalty`(`penalty_id`);
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ADD CONSTRAINT `fk_appeal_evidence_case_id` FOREIGN KEY (`case_id`) REFERENCES `health_insurance_ecm`.`appeal`.`case`(`case_id`);
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ADD CONSTRAINT `fk_appeal_evidence_parent_evidence_id` FOREIGN KEY (`parent_evidence_id`) REFERENCES `health_insurance_ecm`.`appeal`.`evidence`(`evidence_id`);
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` SET TAGS ('dbx_subdomain' = 'case_management');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `appeal_grievance_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Grievance ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `case_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Case Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To User ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `employee_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Event Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Filing Party Identifier');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `party_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `party_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `premium_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Premium Invoice Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Related Claim ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `acknowledgment_date` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `appeal_grievance_status` SET TAGS ('dbx_business_glossary_term' = 'Grievance Status');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `appeal_grievance_status` SET TAGS ('dbx_value_regex' = 'open|in_review|closed|withdrawn|escalated');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `appeal_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Appeal Reference Number');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `case_notes` SET TAGS ('dbx_business_glossary_term' = 'Case Notes');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `escalation_level` SET TAGS ('dbx_business_glossary_term' = 'Escalation Level');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `escalation_level` SET TAGS ('dbx_value_regex' = 'none|level_1|level_2|level_3');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `external_review_outcome` SET TAGS ('dbx_business_glossary_term' = 'External Review Outcome');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `external_review_outcome` SET TAGS ('dbx_value_regex' = 'favorable|unfavorable|pending|not_requested');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `external_review_requested` SET TAGS ('dbx_business_glossary_term' = 'External Review Requested');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `filing_channel` SET TAGS ('dbx_business_glossary_term' = 'Filing Channel');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `filing_channel` SET TAGS ('dbx_value_regex' = 'portal|call_center|email|mail|in_person');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `filing_date` SET TAGS ('dbx_business_glossary_term' = 'Filing Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `filing_party_type` SET TAGS ('dbx_business_glossary_term' = 'Filing Party Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `filing_party_type` SET TAGS ('dbx_value_regex' = 'member|provider|employer|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `grievance_number` SET TAGS ('dbx_business_glossary_term' = 'Grievance Number');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `grievance_type` SET TAGS ('dbx_business_glossary_term' = 'Grievance Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `grievance_type` SET TAGS ('dbx_value_regex' = 'quality_of_care|access|billing|customer_service|provider_behavior|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `investigation_end_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation End Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `investigation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Start Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `is_appeal` SET TAGS ('dbx_business_glossary_term' = 'Is Appeal');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `is_confidential` SET TAGS ('dbx_business_glossary_term' = 'Is Confidential');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Grievance Priority');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'high|medium|low');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `regulatory_classification` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Classification');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `regulatory_classification` SET TAGS ('dbx_value_regex' = 'medicare_ctm|state_doi|aca|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `related_encounter_number` SET TAGS ('dbx_business_glossary_term' = 'Related Encounter ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Resolution Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `resolution_detail` SET TAGS ('dbx_business_glossary_term' = 'Resolution Detail');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `resolution_type` SET TAGS ('dbx_business_glossary_term' = 'Resolution Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `resolution_type` SET TAGS ('dbx_value_regex' = 'overturned|upheld|settled|withdrawn|no_action');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `state_code` SET TAGS ('dbx_business_glossary_term' = 'State Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `state_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Grievance Title');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `zip_code` SET TAGS ('dbx_business_glossary_term' = 'ZIP Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `zip_code` SET TAGS ('dbx_value_regex' = '^d{5}(-d{4})?$');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `zip_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_grievance` ALTER COLUMN `zip_code` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Employee Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `compliance_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `contract_provider_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Contract Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `member_risk_score_id` SET TAGS ('dbx_business_glossary_term' = 'Member Risk Score Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `related_diagnosis_code` SET TAGS ('dbx_business_glossary_term' = 'Related Diagnosis Code (ICD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `related_diagnosis_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `related_diagnosis_code` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `related_drug_ndc` SET TAGS ('dbx_business_glossary_term' = 'Related Drug NDC');
ALTER TABLE `health_insurance_ecm`.`appeal`.`case` ALTER COLUMN `related_service_code` SET TAGS ('dbx_business_glossary_term' = 'Related Service Code (CPT)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`adverse_determination` SET TAGS ('dbx_subdomain' = 'case_management');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`adverse_determination` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`adverse_determination` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`adverse_determination` ALTER COLUMN `network_status` SET TAGS ('dbx_business_glossary_term' = 'Network Status');
ALTER TABLE `health_insurance_ecm`.`appeal`.`adverse_determination` ALTER COLUMN `network_status` SET TAGS ('dbx_value_regex' = 'in_network|out_of_network|preferred|non_preferred');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`review` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`external_review` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`timeline` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`timeline` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Group ID');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` SET TAGS ('dbx_subdomain' = 'correspondence_records');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `appeal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Document ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `case_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal ID (APPEAL_ID)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID (MEMBER_ID)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID (PROV_ID)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `appeal_document_description` SET TAGS ('dbx_business_glossary_term' = 'Document Description (DESC)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `appeal_document_status` SET TAGS ('dbx_business_glossary_term' = 'Document Status (STATUS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `appeal_document_status` SET TAGS ('dbx_value_regex' = 'received|under_review|approved|rejected|closed');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `audit_trail` SET TAGS ('dbx_business_glossary_term' = 'Audit Trail (AUDIT)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `considered_in_decision` SET TAGS ('dbx_business_glossary_term' = 'Considered In Decision (CONSIDERED)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp (CREATED_TS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `document_number` SET TAGS ('dbx_business_glossary_term' = 'Document Number');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `document_type` SET TAGS ('dbx_business_glossary_term' = 'Document Type (DOC_TYPE)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `document_type` SET TAGS ('dbx_value_regex' = 'eob|denial_letter|medical_record|provider_note|transcript|peer_review');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `file_size_bytes` SET TAGS ('dbx_business_glossary_term' = 'File Size (BYTES)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `format` SET TAGS ('dbx_business_glossary_term' = 'Document Format (FMT)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `format` SET TAGS ('dbx_value_regex' = 'pdf|tiff|jpg|docx');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `is_confidential` SET TAGS ('dbx_business_glossary_term' = 'Confidential Flag (CONFIDENTIAL)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `is_redacted` SET TAGS ('dbx_business_glossary_term' = 'Redaction Flag (REDACTED)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `last_accessed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Accessed Timestamp (LAST_ACC_TS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `phi_classification` SET TAGS ('dbx_business_glossary_term' = 'PHI Classification (PHI_CLASS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `phi_classification` SET TAGS ('dbx_value_regex' = 'none|partial|full');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `receipt_date` SET TAGS ('dbx_business_glossary_term' = 'Receipt Date (REC_DATE)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `redaction_status` SET TAGS ('dbx_business_glossary_term' = 'Redaction Status (REDACT_STATUS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `redaction_status` SET TAGS ('dbx_value_regex' = 'not_redacted|partial|full');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `retention_period_years` SET TAGS ('dbx_business_glossary_term' = 'Retention Period (YEARS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `source` SET TAGS ('dbx_business_glossary_term' = 'Document Source (SRC)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `storage_path` SET TAGS ('dbx_business_glossary_term' = 'Storage Path (STORAGE_PATH)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `storage_path` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `storage_path` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Document Title');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp (UPDATED_TS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_document` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number (VER)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` SET TAGS ('dbx_subdomain' = 'correspondence_records');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `appeal_communication_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Communication ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `case_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Case Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `accessibility_accommodation` SET TAGS ('dbx_business_glossary_term' = 'Accessibility Accommodation');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `accessibility_accommodation` SET TAGS ('dbx_value_regex' = 'large_print|braille|interpreter|none');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `appeal_communication_status` SET TAGS ('dbx_business_glossary_term' = 'Communication Status');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `appeal_communication_status` SET TAGS ('dbx_value_regex' = 'pending|sent|delivered|failed|returned|closed');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `attachment_count` SET TAGS ('dbx_business_glossary_term' = 'Attachment Count');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `attachment_indicator` SET TAGS ('dbx_business_glossary_term' = 'Attachment Indicator');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `body_text` SET TAGS ('dbx_business_glossary_term' = 'Communication Body Text');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `channel` SET TAGS ('dbx_business_glossary_term' = 'Communication Channel');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `channel` SET TAGS ('dbx_value_regex' = 'mail|phone|secure_portal|fax|email');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `communication_category` SET TAGS ('dbx_business_glossary_term' = 'Communication Category');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `communication_category` SET TAGS ('dbx_value_regex' = 'member_notice|provider_notice|grievance|legal|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `communication_number` SET TAGS ('dbx_business_glossary_term' = 'Communication Number');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `communication_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Communication Event Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `communication_type` SET TAGS ('dbx_business_glossary_term' = 'Communication Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `communication_type` SET TAGS ('dbx_value_regex' = 'acknowledgment|extension_notice|decision_letter|notice_of_rights|information_request|response');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Compliance Notes');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `created_by_user_role` SET TAGS ('dbx_business_glossary_term' = 'Created By User Role');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `created_by_user_role` SET TAGS ('dbx_value_regex' = 'admin|agent|system|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `delivery_confirmation_date` SET TAGS ('dbx_business_glossary_term' = 'Delivery Confirmation Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `delivery_confirmation_flag` SET TAGS ('dbx_business_glossary_term' = 'Delivery Confirmation Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `direction` SET TAGS ('dbx_business_glossary_term' = 'Communication Direction');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `direction` SET TAGS ('dbx_value_regex' = 'outbound|inbound');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `escalation_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalation Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `is_confidential` SET TAGS ('dbx_business_glossary_term' = 'Confidential Communication Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `is_physical_mail` SET TAGS ('dbx_business_glossary_term' = 'Physical Mail Indicator');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `language` SET TAGS ('dbx_business_glossary_term' = 'Communication Language');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `last_modified_by_user_role` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Role');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `last_modified_by_user_role` SET TAGS ('dbx_value_regex' = 'admin|agent|system|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Communication Notes');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `party_type` SET TAGS ('dbx_business_glossary_term' = 'Party Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `party_type` SET TAGS ('dbx_value_regex' = 'member|provider|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `priority_flag` SET TAGS ('dbx_business_glossary_term' = 'Priority Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'high|medium|low');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `received_date` SET TAGS ('dbx_business_glossary_term' = 'Received Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `record_audit_created` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Created Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `record_audit_updated` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Updated Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `regulatory_basis` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Basis');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `regulatory_basis` SET TAGS ('dbx_value_regex' = 'ACA|ERISA|state_DIO|CMS|HIPAA');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `regulatory_notice_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notice Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `response_due_date` SET TAGS ('dbx_business_glossary_term' = 'Response Due Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `response_received_date` SET TAGS ('dbx_business_glossary_term' = 'Response Received Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `response_received_flag` SET TAGS ('dbx_business_glossary_term' = 'Response Received Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `returned_mail_date` SET TAGS ('dbx_business_glossary_term' = 'Returned Mail Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `returned_mail_flag` SET TAGS ('dbx_business_glossary_term' = 'Returned Mail Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `sent_date` SET TAGS ('dbx_business_glossary_term' = 'Sent Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_communication` ALTER COLUMN `subject` SET TAGS ('dbx_business_glossary_term' = 'Communication Subject');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `compliance_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `party_id` SET TAGS ('dbx_business_glossary_term' = 'Disputing Party Identifier');
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `enrollment_eligibility_span_id` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Span Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Credential Record Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `disputed_benefit_code` SET TAGS ('dbx_business_glossary_term' = 'Disputed Benefit Code (CPT/HCPCS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `disputed_benefit_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{5}$');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `other_carrier_code` SET TAGS ('dbx_business_glossary_term' = 'Other Carrier Identifier');
ALTER TABLE `health_insurance_ecm`.`appeal`.`coverage_dispute` ALTER COLUMN `other_carrier_name` SET TAGS ('dbx_business_glossary_term' = 'Other Carrier Name');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `appeal_regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Regulatory Filing ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `compliance_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Employer Group Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `identity_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `identity_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `provider_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `provider_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `acknowledgment_date` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `acknowledgment_received` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgment Received');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `corrective_action_plan` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Plan');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `deficiency_description` SET TAGS ('dbx_business_glossary_term' = 'Deficiency Description');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `deficiency_notice` SET TAGS ('dbx_business_glossary_term' = 'Deficiency Notice');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_amount` SET TAGS ('dbx_business_glossary_term' = 'Filing Amount');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_date` SET TAGS ('dbx_business_glossary_term' = 'Filing Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_method` SET TAGS ('dbx_business_glossary_term' = 'Filing Method');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_method` SET TAGS ('dbx_value_regex' = 'EDI|WebPortal|ManualUpload|Fax');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_number` SET TAGS ('dbx_business_glossary_term' = 'Filing Number');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_status` SET TAGS ('dbx_business_glossary_term' = 'Filing Status');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_status` SET TAGS ('dbx_value_regex' = 'pending|submitted|accepted|rejected|closed');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_type` SET TAGS ('dbx_business_glossary_term' = 'Filing Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_type` SET TAGS ('dbx_value_regex' = 'appeal|grievance|external_review|state_hearing');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `filing_version` SET TAGS ('dbx_business_glossary_term' = 'Filing Version');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_value_regex' = 'CMS|State_DOI|NCQA|URAC|HHS');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `reporting_period` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `resubmission_date` SET TAGS ('dbx_business_glossary_term' = 'Resubmission Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `resubmission_required` SET TAGS ('dbx_business_glossary_term' = 'Resubmission Required');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `submission_method` SET TAGS ('dbx_business_glossary_term' = 'Submission Method');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `submission_method` SET TAGS ('dbx_value_regex' = 'Electronic|Paper|Phone|Email');
ALTER TABLE `health_insurance_ecm`.`appeal`.`appeal_regulatory_filing` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`iro_organization` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`iro_organization` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`iro_organization` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Id (Foreign Key)');
...
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` SET TAGS ('dbx_subdomain' = 'compliance_oversight');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Penalty ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `compliance_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan ID (HPID)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `identity_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID (MID)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `identity_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `identity_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider ID (PID)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `provider_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `provider_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `parent_penalty_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `amount` SET TAGS ('dbx_business_glossary_term' = 'Penalty Amount (PA)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `appeal_deadline` SET TAGS ('dbx_business_glossary_term' = 'Appeal Deadline (AD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `appeal_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Date (AFD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `appeal_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Flag (AFF)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `appeal_outcome` SET TAGS ('dbx_business_glossary_term' = 'Appeal Outcome (AO)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `appeal_outcome` SET TAGS ('dbx_value_regex' = 'upheld|reversed|modified|withdrawn');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `assessment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Penalty Assessment Timestamp (PAT)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `audit_trail` SET TAGS ('dbx_business_glossary_term' = 'Audit Trail (AT)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Compliance Flag (CF)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `compliance_flag` SET TAGS ('dbx_value_regex' = 'compliant|non_compliant|pending');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code (CC)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Penalty Due Date (PDD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `effective_from` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date (EFD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `effective_until` SET TAGS ('dbx_business_glossary_term' = 'Effective Until Date (EUD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `interest_amount` SET TAGS ('dbx_business_glossary_term' = 'Interest Amount (IA)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `interest_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Penalty Notes (PN)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `payment_date` SET TAGS ('dbx_business_glossary_term' = 'Penalty Payment Date (PPD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `payment_status` SET TAGS ('dbx_business_glossary_term' = 'Payment Status (PST)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `payment_status` SET TAGS ('dbx_value_regex' = 'unpaid|paid|partial|settled');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_category` SET TAGS ('dbx_business_glossary_term' = 'Penalty Category (PC)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_category` SET TAGS ('dbx_value_regex' = 'financial|operational|clinical|administrative');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_number` SET TAGS ('dbx_business_glossary_term' = 'Penalty Reference Number (PRN)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_status` SET TAGS ('dbx_business_glossary_term' = 'Penalty Status (PS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_status` SET TAGS ('dbx_value_regex' = 'assessed|appealed|paid|waived|reversed|closed');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_type` SET TAGS ('dbx_business_glossary_term' = 'Penalty Type (PT)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `penalty_type` SET TAGS ('dbx_value_regex' = 'civil_money|state_fine|auto_approval|other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Penalty Reason Code (PRC)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `reason_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{3,10}$');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Penalty Reason Description (PRD)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `record_audit_created` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Created (RAC)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `record_audit_updated` SET TAGS ('dbx_business_glossary_term' = 'Record Audit Updated (RAU)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body (RB)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_value_regex' = 'CMS|DOI|State|Federal|Other');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `regulatory_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reference (RR)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `severity` SET TAGS ('dbx_business_glossary_term' = 'Penalty Severity (PSV)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `severity` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `source_record_reference` SET TAGS ('dbx_business_glossary_term' = 'Source Record ID (SRI)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System (SS)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `total_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Penalty Amount (TPA)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`penalty` ALTER COLUMN `total_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` SET TAGS ('dbx_subdomain' = 'review_processing');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `evidence_id` SET TAGS ('dbx_business_glossary_term' = 'Appeal Evidence ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `provider_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Provider Provider Id (Foreign Key)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `case_id` SET TAGS ('dbx_business_glossary_term' = 'Related Appeal ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `pa_request_id` SET TAGS ('dbx_business_glossary_term' = 'Related Authorization ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `header_id` SET TAGS ('dbx_business_glossary_term' = 'Related Claim ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `parent_evidence_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `author_name` SET TAGS ('dbx_business_glossary_term' = 'Author Name');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `author_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `author_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `author_npi` SET TAGS ('dbx_business_glossary_term' = 'Author NPI');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `author_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `author_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `checksum` SET TAGS ('dbx_business_glossary_term' = 'Checksum');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `clinical_code_icd10` SET TAGS ('dbx_business_glossary_term' = 'ICD-10 Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `confidentiality_flag` SET TAGS ('dbx_business_glossary_term' = 'Confidentiality Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `drg_code` SET TAGS ('dbx_business_glossary_term' = 'DRG Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `evidence_description` SET TAGS ('dbx_business_glossary_term' = 'Evidence Description');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `evidence_number` SET TAGS ('dbx_business_glossary_term' = 'Evidence Number');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `evidence_status` SET TAGS ('dbx_business_glossary_term' = 'Evidence Status');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `evidence_status` SET TAGS ('dbx_value_regex' = 'pending|reviewed|accepted|rejected|withdrawn');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `evidence_type` SET TAGS ('dbx_business_glossary_term' = 'Evidence Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `file_format` SET TAGS ('dbx_business_glossary_term' = 'File Format');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `file_size_bytes` SET TAGS ('dbx_business_glossary_term' = 'File Size (bytes)');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `guideline_version` SET TAGS ('dbx_business_glossary_term' = 'Guideline Version');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `ndc_code` SET TAGS ('dbx_business_glossary_term' = 'NDC Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `phi_flag` SET TAGS ('dbx_business_glossary_term' = 'PHI Flag');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `procedure_code_cpt` SET TAGS ('dbx_business_glossary_term' = 'CPT Code');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `received_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Received Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `related_encounter_number` SET TAGS ('dbx_business_glossary_term' = 'Related Encounter ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `relevance_score` SET TAGS ('dbx_business_glossary_term' = 'Relevance Score');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `source_entity_type` SET TAGS ('dbx_business_glossary_term' = 'Source Entity Type');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `source_entity_type` SET TAGS ('dbx_value_regex' = 'member|provider|external|internal');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `source_system_record_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Record ID');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `storage_path` SET TAGS ('dbx_business_glossary_term' = 'Storage Path');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `submission_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Submission Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `title` SET TAGS ('dbx_business_glossary_term' = 'Evidence Title');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `health_insurance_ecm`.`appeal`.`evidence` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
