-- Schema for Domain: partner | Business: Telecommunication | Version: v1_mvm
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`agreement` (
    ...
    `cycle_id` BIGINT COMMENT 'Foreign key linking to billing.billing_cycle. Business justification: Partner agreements define settlement_frequency aligned to billing cycles. Linking agreement to billing_cycle enables automated billing cycle assignment for agreement-based invoicing — used in wholesal',
    `partner_id` BIGINT COMMENT 'Reference to the partner organization that is party to this agreement.',
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Wholesale and MVNO host agreements in telecom are often referenced in or governed by regulatory filings (wholesale access filings, reference interconnect offers). Linking agreement to regulatory_filin',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`mvno_profile` (
    ...
    `billing_account_id` BIGINT COMMENT 'Foreign key linking to billing.billing_account. Business justification: Each MVNO operates under a dedicated billing account for wholesale charges. Linking mvno_profile to billing_account enables wholesale invoice generation and account management for MVNO partners — fund',
    `cycle_id` BIGINT COMMENT 'Foreign key linking to billing.billing_cycle. Business justification: MVNO wholesale billing is cycle-driven; the MVNO profile must reference its billing cycle for wholesale invoice generation. Telecom MVNO billing teams always align MVNO wholesale invoicing to a specif',
    `enterprise_contract_id` BIGINT COMMENT 'Foreign key linking to enterprise.enterprise_contract. Business justification: MVNO profiles are governed by host agreements formalized as enterprise contracts (enterprise_contract.mvno_host_agreement_flag). MVNO onboarding, regulatory compliance, and billing arrangement setup a',
    ...
    `msisdn_range_id` BIGINT COMMENT 'Foreign key linking to inventory.msisdn_range. Business justification: MVNO profiles are allocated specific MSISDN number ranges from inventory. mvno_profile currently stores msisdn_range_start/msisdn_range_end as denormalized text. A proper FK to inventory.msisdn_range ',
    `partner_id` BIGINT COMMENT 'Reference to the partner entity representing this MVNO in the partner master data.',
    `qos_profile_id` BIGINT COMMENT 'Foreign key linking to network.network_qos_profile. Business justification: MVNO QoS enforcement: mvno_profile.qos_profile is a plain text denormalization of the network QoS profile applied to MVNO traffic. Replacing it with a proper FK to network_qos_profile enables automate',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`revenue_share_plan` (
    ...
    `cycle_id` BIGINT COMMENT 'Foreign key linking to billing.billing_cycle. Business justification: Revenue share plans are calculated and settled per billing cycle. Linking revenue_share_plan to billing_cycle enables automated revenue share calculation triggers aligned to billing periods — essentia',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Revenue share plans in telecom are constrained by wholesale access and interconnect pricing regulatory obligations. Linking revenue_share_plan to the governing regulatory_obligation enables compliance',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`settlement_run` (
    ...
    `cycle_id` BIGINT COMMENT 'Foreign key linking to billing.billing_cycle. Business justification: Settlement runs are executed per billing cycle period. Linking settlement_run to billing_cycle enables cycle-aligned settlement reporting and automated settlement triggers — standard in telecom wholes',
    `invoice_id` BIGINT COMMENT 'Foreign key linking to billing.invoice. Business justification: Settlement runs produce a net payable invoice to/from the partner. Linking settlement_run to invoice enables accounts payable/receivable traceability for partner settlement — telecom finance teams req',
    ...
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Wholesale and roaming settlement runs in telecom are often tied to regulatory filings (interconnect settlement reports, wholesale billing filings). Linking settlement_run to regulatory_filing supports',
    `revenue_share_plan_id` BIGINT COMMENT 'Foreign key linking to partner.revenue_share_plan. Business justification: A settlement_run executes financial settlement calculations against a specific revenue share plan. While the revenue_share_plan is reachable via settlement_agreement_id → agreement → revenue_share_pla',
    ...
    `sla_breach_event_id` BIGINT COMMENT 'Foreign key linking to assurance.sla_breach_event. Business justification: Settlement runs that process SLA penalty payments must reference the originating breach event. Settlement operations teams reconcile breach-triggered penalty amounts against settlement runs for audit,',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`settlement_line` (
    ...
    `aggregated_usage_id` BIGINT COMMENT 'Foreign key linking to usage.aggregated_usage. Business justification: Settlement lines are derived from aggregated usage volumes. Linking settlement_line to aggregated_usage enables wholesale billing teams to trace each settlement line item back to the specific aggregat',
    ...
    `billing_charge_id` BIGINT COMMENT 'Foreign key linking to billing.billing_charge. Business justification: Settlement lines are reconciled against specific billing charges (wholesale, interconnect). Linking settlement_line to billing_charge enables charge-level settlement reconciliation — a standard teleco',
    ...
    `iot_deployment_id` BIGINT COMMENT 'Foreign key linking to enterprise.iot_deployment. Business justification: IoT roaming settlement lines must reference the specific enterprise IoT deployment generating the traffic. Telecom operators produce per-deployment IoT settlement reports for enterprise customers. set',
    ...
    `roaming_agreement_id` BIGINT COMMENT 'Foreign key linking to partner.roaming_agreement. Business justification: settlement_line records for roaming traffic (identified by roaming_country_code, tap_file_reference, rap_file_reference fields) are governed by specific bilateral roaming agreements. settlement_line a',
    ...
    `sla_breach_event_id` BIGINT COMMENT 'Foreign key linking to assurance.sla_breach_event. Business justification: A settlement line item representing a penalty credit or charge must be traceable to the specific SLA breach event that triggered it. Settlement analysts and auditors require this link for breach-to-pa',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to workforce.work_order. Business justification: Partner settlement lines for managed services or subcontracted field work must reference the specific work order to reconcile billed labor against actual field delivery. Telecom finance teams require ',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`onboarding_request` (
    ...
    `enterprise_contract_id` BIGINT COMMENT 'Foreign key linking to enterprise.enterprise_contract. Business justification: Partner onboarding requests for MVNO and wholesale partners are initiated under a pre-negotiated enterprise contract framework. onboarding_request tracks contract_effective_date and contract_status, r',
    `opportunity_id` BIGINT COMMENT 'Foreign key linking to sales.opportunity. Business justification: Partner onboarding requests are triggered by converted sales opportunities (new MVNO, dealer, or reseller deals). Linking onboarding_request to opportunity enables end-to-end sales-to-activation funne',
    `partner_id` BIGINT COMMENT 'Reference to the partner entity being onboarded. Links to the partner master record.',
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Onboarding requests carry regulatory_approval_date and regulatory_approval_status, confirming regulatory approval is tracked. Linking to the specific regulatory_filing that granted approval provides t',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to workforce.work_order. Business justification: Partner technical onboarding (MVNO, dealer, roaming partner) triggers field work orders for network integration, equipment installation, and connectivity testing. Linking onboarding_request to the pro',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`dealer` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to partner.agreement. Business justification: Dealers operate under formal commercial agreements with the telecommunications company. dealer currently carries contract_effective_date, contract_expiry_date, and contract_renewal_terms as inline str',
    `commission_plan_id` BIGINT COMMENT 'Foreign key linking to sales.commission_plan. Business justification: Dealers operate under a specific commission plan governing their earnings per activation, residual rates, and clawback terms. Telecom partner finance teams require this direct link for dealer commissi',
    ...
    `enterprise_contract_id` BIGINT COMMENT 'Foreign key linking to enterprise.enterprise_contract. Business justification: Large corporate dealers (retail chains, enterprise resellers) have commercial terms governed by enterprise contracts covering discounts, SLA commitments, and minimum volumes. Dealer performance manage',
    ...
    `partner_id` BIGINT COMMENT 'Foreign key linking to partner.partner. Business justification: Dealers are authorized indirect sales channel participants who are also registered as third-party partners in the partner master registry. The partner table is the SSOT for all third-party relationshi',
...
CREATE OR REPLACE TABLE `telecommunication_ecm`.`partner`.`partner_dispute` (
    ...
    `anomaly_id` BIGINT COMMENT 'Foreign key linking to usage.anomaly. Business justification: Partner disputes arising from revenue assurance anomalies or roaming fraud detections must reference the originating anomaly record. Dispute resolution teams require traceability from the partner disp',
    ...
    `managed_service_id` BIGINT COMMENT 'Foreign key linking to enterprise.managed_service. Business justification: Partner disputes frequently arise from managed service delivery failures — MVNO partners dispute charges tied to specific managed service instances. Linking partner_dispute to managed_service enables ',
    ...
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Escalated partner disputes in telecom (wholesale, roaming) may be filed with regulators. Linking partner_dispute to regulatory_filing supports regulatory escalation tracking and compliance reporting o',
    `roaming_file_id` BIGINT COMMENT 'Foreign key linking to usage.roaming_file. Business justification: Partner disputes about TAP/RAP file discrepancies must formally reference the specific roaming file in dispute. roaming_file has dispute_flag and dispute_reason_code confirming disputes occur at file ',
    `sales_contract_id` BIGINT COMMENT 'Foreign key linking to sales.sales_contract. Business justification: Partner disputes frequently arise from sales contract terms — disputed pricing, ETF charges, or SLA breaches defined in the sales contract. Telecom dispute resolution teams require direct traceability',
    `settlement_line_id` BIGINT COMMENT 'Foreign key linking to partner.settlement_line. Business justification: Disputes in the partner domain are frequently raised at the line-item level — a partner disputes a specific charge, rate, or usage record within a settlement run. partner_dispute already has settlemen',
    ...
    `work_order_id` BIGINT COMMENT 'Foreign key linking to workforce.work_order. Business justification: Partner disputes frequently arise from failed, delayed, or substandard field work (missed SLA, repeat visits, incomplete installations). Linking partner_dispute to the causative work_order is essentia',
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` ADD CONSTRAINT `fk_partner_settlement_run_revenue_share_plan_id` FOREIGN KEY (`revenue_share_plan_id`) REFERENCES `telecommunication_ecm`.`partner`.`revenue_share_plan`(`revenue_share_plan_id`);
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ADD CONSTRAINT `fk_partner_settlement_line_roaming_agreement_id` FOREIGN KEY (`roaming_agreement_id`) REFERENCES `telecommunication_ecm`.`partner`.`roaming_agreement`(`roaming_agreement_id`);
...
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` ADD CONSTRAINT `fk_partner_dealer_agreement_id` FOREIGN KEY (`agreement_id`) REFERENCES `telecommunication_ecm`.`partner`.`agreement`(`agreement_id`);
...
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` ADD CONSTRAINT `fk_partner_dealer_partner_id` FOREIGN KEY (`partner_id`) REFERENCES `telecommunication_ecm`.`partner`.`partner`(`partner_id`);
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ADD CONSTRAINT `fk_partner_partner_dispute_settlement_line_id` FOREIGN KEY (`settlement_line_id`) REFERENCES `telecommunication_ecm`.`partner`.`settlement_line`(`settlement_line_id`);
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`agreement` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`agreement` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`agreement` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `billing_account_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Account Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `enterprise_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Enterprise Contract Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `msisdn_range_id` SET TAGS ('dbx_business_glossary_term' = 'Msisdn Range Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `msisdn_range_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `msisdn_range_id` SET TAGS ('dbx_pii_phone' = 'true');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`mvno_profile` ALTER COLUMN `qos_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Network Qos Profile Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`roaming_agreement` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`sla_definition` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`revenue_share_plan` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`revenue_share_plan` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`revenue_share_plan` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` ALTER COLUMN `cycle_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` ALTER COLUMN `revenue_share_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Revenue Share Plan Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_run` ALTER COLUMN `sla_breach_event_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Breach Event Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ALTER COLUMN `aggregated_usage_id` SET TAGS ('dbx_business_glossary_term' = 'Aggregated Usage Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ALTER COLUMN `billing_charge_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Charge Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ALTER COLUMN `iot_deployment_id` SET TAGS ('dbx_business_glossary_term' = 'Iot Deployment Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ALTER COLUMN `roaming_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Roaming Agreement Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ALTER COLUMN `sla_breach_event_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Breach Event Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`settlement_line` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`onboarding_request` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`onboarding_request` ALTER COLUMN `enterprise_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Enterprise Contract Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`onboarding_request` ALTER COLUMN `opportunity_id` SET TAGS ('dbx_business_glossary_term' = 'Opportunity Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`onboarding_request` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`onboarding_request` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Agreement Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` ALTER COLUMN `commission_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Commission Plan Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` ALTER COLUMN `enterprise_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Enterprise Contract Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`dealer` ALTER COLUMN `partner_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_contact` SET TAGS ('dbx_subdomain' = 'partner_registry');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` SET TAGS ('dbx_subdomain' = 'financial_settlement');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `anomaly_id` SET TAGS ('dbx_business_glossary_term' = 'Anomaly Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `managed_service_id` SET TAGS ('dbx_business_glossary_term' = 'Managed Service Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `roaming_file_id` SET TAGS ('dbx_business_glossary_term' = 'Roaming File Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `sales_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Contract Id (Foreign Key)');
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `settlement_line_id` SET TAGS ('dbx_business_glossary_term' = 'Settlement Line Id (Foreign Key)');
...
ALTER TABLE `telecommunication_ecm`.`partner`.`partner_dispute` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
