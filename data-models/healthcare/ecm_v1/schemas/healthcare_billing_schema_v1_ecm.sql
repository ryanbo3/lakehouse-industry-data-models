-- Schema for Domain: billing | Business: Healthcare | Version: v1_ecm
-- Generated on: 2026-05-04 15:51:52
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`charge` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the system user who voided the charge. Used for audit trail and compliance tracking.',
    ...
    `treatment_consent_id` BIGINT COMMENT 'Foreign key linking to consent.treatment_consent. Business justification: Medical necessity validation and compliance audits require linking procedure charges to documented treatment consent. Payers deny claims without proof of informed consent for procedures. Essential for',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`invoice` (
    ...
    `cda_document_id` BIGINT COMMENT 'Foreign key linking to interoperability.cda_document. Business justification: Claims submission workflow: invoices are transmitted as CDA documents (e.g., CMS-1500, UB-04 electronic equivalents) to payers and HIEs for adjudication. Tracking which CDA document represents each in',
    ...
    `fhir_resource_log_id` BIGINT COMMENT 'Foreign key linking to interoperability.fhir_resource_log. Business justification: FHIR-based claims submission: invoices transmitted as FHIR Claim resources require tracking which fhir_resource_log entry represents the submission. Enables monitoring submission status, retry logic, ',
    ...
    `public_health_report_id` BIGINT COMMENT 'Foreign key linking to interoperability.public_health_report. Business justification: Hospital IQR and IPPS reporting: invoices with specific DRGs/conditions are reported to CMS for quality measures and payment determination. Linking invoices to public_health_report submissions enables',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`coding_assignment` (
    ...
    `public_health_report_id` BIGINT COMMENT 'Foreign key linking to interoperability.public_health_report. Business justification: Quality measure reporting: coded diagnoses/procedures drive CMS quality measure submissions (IQR, PQRS, MIPS). Linking coding_assignment to public_health_report enables validation that correct codes w',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`payment` (
    ...
    `payment_era_transaction_ar_transaction_id` BIGINT COMMENT 'The transaction identifier from the Electronic Remittance Advice (ERA) 835 file received from the insurance payer, linking this payment to the payers remittance details.',
    `payment_last_modified_by_user_employee_id` BIGINT COMMENT 'The identifier of the user or system account that last modified the payment record.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`adjustment` (
    ...
    `adjustment_employee_id` BIGINT COMMENT 'Reference to the user or authority who approved the adjustment, required for high-value or sensitive adjustments.',
    `adjustment_last_modified_by_user_employee_id` BIGINT COMMENT 'Reference to the user who last modified this adjustment record.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`statement` (
    ...
    `statement_last_modified_by_user_employee_id` BIGINT COMMENT 'The system user or process identifier that last modified this statement record.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`collection_account` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`billing_coverage` (
    ...
    `member_mpi_record_id` BIGINT COMMENT 'Unique member identifier assigned by the insurance payer to the patient for this coverage. Used for claims submission and eligibility verification.',
    `mpi_record_id` BIGINT COMMENT 'Identifier of the patient who holds this insurance coverage. Links to the patient master record.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`write_off` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`rac_audit` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`charity_care_application` (
    ...
    `patient_identity_match_id` BIGINT COMMENT 'Foreign key linking to interoperability.patient_identity_match. Business justification: Financial assistance via HIE: charity care applications submitted through patient portals or HIE require patient identity matching to link external applications to internal MPI records. Tracking which',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`refund` (
    ...
    `refund_employee_id` BIGINT COMMENT 'Reference to the user or role who approved the refund transaction, used for audit trail and accountability.',
    `refund_last_modified_by_user_employee_id` BIGINT COMMENT 'Reference to the user who last modified the refund record in the system.',
    `refund_voided_by_user_employee_id` BIGINT COMMENT 'Reference to the user who voided or cancelled the refund, used for audit trail purposes.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`billing_network_participation` (
    `billing_network_participation_id` BIGINT COMMENT 'Primary key for billing_network_participation',
    `billing_coverage_id` BIGINT COMMENT 'Foreign key linking to the insurance coverage record representing the payer plan in this network relationship',
    `care_site_id` BIGINT COMMENT 'Foreign key linking to the care delivery site participating in this payer network',
    `insurance_network_participation_id` BIGINT COMMENT 'Unique identifier for this payer-facility network participation record. Primary key.',
    `authorization_required_flag` BOOLEAN COMMENT 'Indicates whether this payer requires prior authorization for services delivered at this specific care site. Authorization requirements vary by payer-facility contract terms. Moved from coverage product where it was incorrectly stored as a plan-level attribute rather than a payer-facility relationship attribute.',
    `contract_status` STRING COMMENT 'Current lifecycle status of this payer-facility network participation contract. Active indicates contract is in force and network benefits apply.',
    `contract_type` STRING COMMENT 'Classification of the network participation contract type. Determines scope of services covered and patient access rules.',
    `contracted_rate_schedule` STRING COMMENT 'Reference to the fee schedule or rate table governing reimbursement for services provided at this care site under this payer contract. Different payer-facility contracts have different negotiated rate schedules.',
    `created_date` TIMESTAMP COMMENT 'Timestamp when this network participation record was initially created in the system.',
    `effective_date` DATE COMMENT 'Date when this payer-facility network participation contract becomes effective. Determines when in-network benefits and contracted rates apply. This is the contract effective date, distinct from the patient coverage effective date stored in the coverage product.',
    `last_updated_date` TIMESTAMP COMMENT 'Timestamp when this network participation record was last modified. Used for change tracking and audit purposes.',
    `network_status` STRING COMMENT 'Indicates whether this care site is in-network, out-of-network, or has partial network status for this specific payer plan. Determines patient cost-sharing and reimbursement rates. Moved from coverage product where it was incorrectly stored as a coverage-level attribute rather than a payer-facility relationship attribute.',
    `referral_required_flag` BOOLEAN COMMENT 'Indicates whether this payer requires a PCP referral for services at this specific care site. Referral requirements vary by payer-facility contract and service type. Moved from coverage product where it was incorrectly stored as a plan-level attribute rather than a payer-facility relationship attribute.',
    `termination_date` DATE COMMENT 'Date when this payer-facility network participation contract terminates. Null indicates an active ongoing contract. This is the contract termination date, distinct from the patient coverage termination date stored in the coverage product.',
    `tier_level` STRING COMMENT 'Tier designation for this care site within the payers tiered network structure. Affects patient cost-sharing levels. Null if payer does not use tiered networks.',
    CONSTRAINT pk_billing_network_participation PRIMARY KEY(`billing_network_participation_id`)
) COMMENT 'This association product represents the contractual network participation relationship between insurance coverage plans and care delivery sites. It captures payer-facility network contracts that determine whether a care site is in-network or out-of-network for a specific payer plan, along with authorization requirements, referral rules, and contract effective periods. Each record links one coverage (representing a payer plan) to one care site with contract terms that exist only in the context of this payer-facility network relationship. This is the operational SSOT for network status determination during patient registration, eligibility verification, and claims adjudication workflows.. Existence Justification: In healthcare revenue cycle operations, payer organizations establish network contracts with multiple care delivery sites across an integrated delivery network, and each care site participates in network agreements with multiple payers. Network participation is an actively managed contractual relationship with specific terms (in-network status, authorization requirements, contracted rates, effective dates) that vary by each unique payer-facility pair. Revenue cycle staff query network participation during patient registration to determine coverage implications, and contract management teams maintain these relationships as payers and facilities negotiate, renew, or terminate network agreements.';

CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`invoice_line_item` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`study_service_coverage` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` (
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ADD CONSTRAINT `fk_billing_charge_billing_coverage_id` FOREIGN KEY (`billing_coverage_id`) REFERENCES `healthcare_ecm`.`billing`.`billing_coverage`(`billing_coverage_id`);
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ADD CONSTRAINT `fk_billing_payment_billing_coverage_id` FOREIGN KEY (`billing_coverage_id`) REFERENCES `healthcare_ecm`.`billing`.`billing_coverage`(`billing_coverage_id`);
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_billing_coverage_id` FOREIGN KEY (`billing_coverage_id`) REFERENCES `healthcare_ecm`.`billing`.`billing_coverage`(`billing_coverage_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_charity_care_application_id` FOREIGN KEY (`charity_care_application_id`) REFERENCES `healthcare_ecm`.`billing`.`charity_care_application`(`charity_care_application_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_collection_account_id` FOREIGN KEY (`collection_account_id`) REFERENCES `healthcare_ecm`.`billing`.`collection_account`(`collection_account_id`);
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_rac_audit_id` FOREIGN KEY (`rac_audit_id`) REFERENCES `healthcare_ecm`.`billing`.`rac_audit`(`rac_audit_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`patient_account` ADD CONSTRAINT `fk_billing_patient_account_charity_care_application_id` FOREIGN KEY (`charity_care_application_id`) REFERENCES `healthcare_ecm`.`billing`.`charity_care_application`(`charity_care_application_id`);
...
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ADD CONSTRAINT `fk_billing_collection_account_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `healthcare_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ADD CONSTRAINT `fk_billing_collection_account_patient_account_id` FOREIGN KEY (`patient_account_id`) REFERENCES `healthcare_ecm`.`billing`.`patient_account`(`patient_account_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ADD CONSTRAINT `fk_billing_write_off_charity_care_application_id` FOREIGN KEY (`charity_care_application_id`) REFERENCES `healthcare_ecm`.`billing`.`charity_care_application`(`charity_care_application_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ADD CONSTRAINT `fk_billing_write_off_collection_account_id` FOREIGN KEY (`collection_account_id`) REFERENCES `healthcare_ecm`.`billing`.`collection_account`(`collection_account_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ADD CONSTRAINT `fk_billing_write_off_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `healthcare_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ADD CONSTRAINT `fk_billing_write_off_original_write_off_id` FOREIGN KEY (`original_write_off_id`) REFERENCES `healthcare_ecm`.`billing`.`write_off`(`write_off_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ADD CONSTRAINT `fk_billing_write_off_rac_audit_id` FOREIGN KEY (`rac_audit_id`) REFERENCES `healthcare_ecm`.`billing`.`rac_audit`(`rac_audit_id`);
...
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ADD CONSTRAINT `fk_billing_rac_audit_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `healthcare_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `healthcare_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_payment_id` FOREIGN KEY (`payment_id`) REFERENCES `healthcare_ecm`.`billing`.`payment`(`payment_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_rac_audit_id` FOREIGN KEY (`rac_audit_id`) REFERENCES `healthcare_ecm`.`billing`.`rac_audit`(`rac_audit_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_original_refund_id` FOREIGN KEY (`original_refund_id`) REFERENCES `healthcare_ecm`.`billing`.`refund`(`refund_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ADD CONSTRAINT `fk_billing_invoice_coverage_billing_billing_coverage_id` FOREIGN KEY (`billing_coverage_id`) REFERENCES `healthcare_ecm`.`billing`.`billing_coverage`(`billing_coverage_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ADD CONSTRAINT `fk_billing_invoice_coverage_billing_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `healthcare_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ADD CONSTRAINT `fk_billing_billing_network_participation_billing_coverage_id` FOREIGN KEY (`billing_coverage_id`) REFERENCES `healthcare_ecm`.`billing`.`billing_coverage`(`billing_coverage_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ADD CONSTRAINT `fk_billing_invoice_line_item_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `healthcare_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ADD CONSTRAINT `fk_billing_study_service_coverage_cdm_entry_id` FOREIGN KEY (`cdm_entry_id`) REFERENCES `healthcare_ecm`.`billing`.`cdm_entry`(`cdm_entry_id`);
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ADD CONSTRAINT `fk_billing_site_cdm_pricing_cdm_entry_id` FOREIGN KEY (`cdm_entry_id`) REFERENCES `healthcare_ecm`.`billing`.`cdm_entry`(`cdm_entry_id`);
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` SET TAGS ('dbx_subdomain' = 'revenue_capture');
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `case_cart_id` SET TAGS ('dbx_business_glossary_term' = 'Charge Capture - Case Cart Id');
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Voided By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `fulfillment_id` SET TAGS ('dbx_business_glossary_term' = 'Order Fulfillment Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Consent Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`billing`.`charge` ALTER COLUMN `ndc_code` SET TAGS ('dbx_business_glossary_term' = 'National Drug Code (NDC)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` SET TAGS ('dbx_subdomain' = 'revenue_capture');
...
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department Identifier');
...
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` ALTER COLUMN `ndc_code` SET TAGS ('dbx_business_glossary_term' = 'National Drug Code (NDC)');
ALTER TABLE `healthcare_ecm`.`billing`.`cdm_entry` ALTER COLUMN `ndc_code` SET TAGS ('dbx_value_regex' = '^[0-9]{11}$');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` SET TAGS ('dbx_subdomain' = 'revenue_capture');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `ar_account_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Account Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `cda_document_id` SET TAGS ('dbx_business_glossary_term' = 'Cda Document Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `fhir_resource_log_id` SET TAGS ('dbx_business_glossary_term' = 'Fhir Resource Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `provider_location_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Location Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `public_health_report_id` SET TAGS ('dbx_business_glossary_term' = 'Public Health Report Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `public_health_report_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `public_health_report_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `drg_code` SET TAGS ('dbx_business_glossary_term' = 'Diagnosis-Related Group (DRG) Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `drg_code` SET TAGS ('dbx_value_regex' = '^[0-9]{3}$');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice` ALTER COLUMN `drg_weight` SET TAGS ('dbx_business_glossary_term' = 'Diagnosis-Related Group (DRG) Weight');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line` SET TAGS ('dbx_subdomain' = 'revenue_capture');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line` ALTER COLUMN `authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Number');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line` ALTER COLUMN `ndc_code` SET TAGS ('dbx_business_glossary_term' = 'National Drug Code (NDC)');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line` ALTER COLUMN `ndc_code` SET TAGS ('dbx_value_regex' = '^[0-9]{11}$');
...
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line` ALTER COLUMN `rendering_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Rendering Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line` ALTER COLUMN `rendering_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
...
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` SET TAGS ('dbx_subdomain' = 'revenue_capture');
...
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `cda_document_id` SET TAGS ('dbx_business_glossary_term' = 'Cda Document Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Coder ID');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `public_health_report_id` SET TAGS ('dbx_business_glossary_term' = 'Public Health Report Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `public_health_report_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `public_health_report_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_business_glossary_term' = 'Secondary Diagnosis Codes');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `secondary_diagnosis_codes` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`coding_assignment` ALTER COLUMN `secondary_procedure_codes` SET TAGS ('dbx_business_glossary_term' = 'Secondary Procedure Codes');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment` SET TAGS ('dbx_subdomain' = 'payment_processing');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Era Message Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `direct_message_id` SET TAGS ('dbx_business_glossary_term' = 'Notification Direct Message Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `ar_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Transaction Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Posted By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `payment_era_transaction_ar_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Electronic Remittance Advice (ERA) Transaction Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `payment_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `payment_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`payment` ALTER COLUMN `payment_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` SET TAGS ('dbx_subdomain' = 'payment_processing');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approving User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `ar_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Transaction Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `charity_care_application_id` SET TAGS ('dbx_business_glossary_term' = 'Charity Care Application Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `collection_account_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Account Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Era Message Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `rac_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Recovery Audit Contractor (RAC) Audit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `restriction_request_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Restriction Request Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `sanction_id` SET TAGS ('dbx_business_glossary_term' = 'Sanction Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`adjustment` ALTER COLUMN `contractual_payer_name` SET TAGS ('dbx_business_glossary_term' = 'Contractual Payer Name');
...
ALTER TABLE `healthcare_ecm`.`billing`.`patient_account` SET TAGS ('dbx_subdomain' = 'payment_processing');
...
ALTER TABLE `healthcare_ecm`.`billing`.`patient_account` ALTER COLUMN `ar_account_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Account Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`patient_account` ALTER COLUMN `charity_care_application_id` SET TAGS ('dbx_business_glossary_term' = 'Charity Care Application Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`patient_account` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`billing`.`patient_account` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Agency ID');
...
ALTER TABLE `healthcare_ecm`.`billing`.`statement` SET TAGS ('dbx_subdomain' = 'payment_processing');
...
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `statement_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `statement_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `statement_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`statement` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Agency Identifier (ID)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` SET TAGS ('dbx_subdomain' = 'financial_resolution');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_account_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Account Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `ar_account_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Account Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Care Site Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `guarantor_id` SET TAGS ('dbx_business_glossary_term' = 'Guarantor Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `patient_account_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Account Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Agency Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `adjustment_amount` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `aging_days` SET TAGS ('dbx_business_glossary_term' = 'Aging Days');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `closed_date` SET TAGS ('dbx_business_glossary_term' = 'Closed Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_account_number` SET TAGS ('dbx_business_glossary_term' = 'Collection Account Number');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_account_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_account_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_agency_name` SET TAGS ('dbx_business_glossary_term' = 'Collection Agency Name');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Collection Fee Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_notes` SET TAGS ('dbx_business_glossary_term' = 'Collection Notes');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_status` SET TAGS ('dbx_business_glossary_term' = 'Collection Status');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_type` SET TAGS ('dbx_business_glossary_term' = 'Collection Type');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `collection_type` SET TAGS ('dbx_value_regex' = 'internal|external|legal|pre_collection|bad_debt');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `contact_attempt_count` SET TAGS ('dbx_business_glossary_term' = 'Contact Attempt Count');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `interest_amount` SET TAGS ('dbx_business_glossary_term' = 'Interest Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `judgment_amount` SET TAGS ('dbx_business_glossary_term' = 'Judgment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `last_contact_date` SET TAGS ('dbx_business_glossary_term' = 'Last Contact Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `last_contact_method` SET TAGS ('dbx_business_glossary_term' = 'Last Contact Method');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `last_contact_method` SET TAGS ('dbx_value_regex' = 'phone|mail|email|sms|in_person|portal');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `last_payment_amount` SET TAGS ('dbx_business_glossary_term' = 'Last Payment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `last_payment_date` SET TAGS ('dbx_business_glossary_term' = 'Last Payment Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `legal_action_date` SET TAGS ('dbx_business_glossary_term' = 'Legal Action Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `legal_action_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Action Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `monthly_payment_amount` SET TAGS ('dbx_business_glossary_term' = 'Monthly Payment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `outstanding_balance` SET TAGS ('dbx_business_glossary_term' = 'Outstanding Balance');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `payment_plan_end_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Plan End Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `payment_plan_flag` SET TAGS ('dbx_business_glossary_term' = 'Payment Plan Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `payment_plan_start_date` SET TAGS ('dbx_business_glossary_term' = 'Payment Plan Start Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `principal_amount` SET TAGS ('dbx_business_glossary_term' = 'Principal Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `recall_date` SET TAGS ('dbx_business_glossary_term' = 'Recall Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `recall_reason` SET TAGS ('dbx_business_glossary_term' = 'Recall Reason');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `recovered_amount` SET TAGS ('dbx_business_glossary_term' = 'Recovered Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `referral_date` SET TAGS ('dbx_business_glossary_term' = 'Referral Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `referral_reason` SET TAGS ('dbx_business_glossary_term' = 'Referral Reason');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `referred_balance` SET TAGS ('dbx_business_glossary_term' = 'Referred Balance');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `settlement_amount` SET TAGS ('dbx_business_glossary_term' = 'Settlement Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `settlement_flag` SET TAGS ('dbx_business_glossary_term' = 'Settlement Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `write_off_amount` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `write_off_date` SET TAGS ('dbx_business_glossary_term' = 'Bad Debt Write-Off Date');
ALTER TABLE `healthcare_ecm`.`billing`.`collection_account` ALTER COLUMN `write_off_reason` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Reason');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` SET TAGS ('dbx_subdomain' = 'payment_processing');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `billing_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `billing_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `billing_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `fhir_resource_log_id` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Fhir Resource Log Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `formulary_id` SET TAGS ('dbx_business_glossary_term' = 'Formulary Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `group_id` SET TAGS ('dbx_business_glossary_term' = 'Provider Group Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `member_mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Member ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `member_mpi_record_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `member_mpi_record_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `org_provider_id` SET TAGS ('dbx_business_glossary_term' = 'Org Provider Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_business_glossary_term' = 'Subscriber ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `subscriber_id` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `vendor_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Contract Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `active_flag` SET TAGS ('dbx_business_glossary_term' = 'Coverage Active Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `authorization_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Required Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `bin_number` SET TAGS ('dbx_business_glossary_term' = 'Bank Identification Number (BIN)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coinsurance_percentage` SET TAGS ('dbx_business_glossary_term' = 'Coinsurance Percentage');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coordination_of_benefits_order` SET TAGS ('dbx_business_glossary_term' = 'Coordination of Benefits (COB) Order');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coordination_of_benefits_order` SET TAGS ('dbx_value_regex' = 'Primary|Secondary|Tertiary');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `copay_amount` SET TAGS ('dbx_business_glossary_term' = 'Copayment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coverage_status` SET TAGS ('dbx_business_glossary_term' = 'Coverage Status');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coverage_status` SET TAGS ('dbx_value_regex' = 'Active|Inactive|Suspended|Terminated|Pending');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coverage_type` SET TAGS ('dbx_business_glossary_term' = 'Coverage Type');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `coverage_type` SET TAGS ('dbx_value_regex' = 'Medical|Dental|Vision|Pharmacy|Behavioral Health|Long-Term Care');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `deductible_amount` SET TAGS ('dbx_business_glossary_term' = 'Deductible Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `deductible_met_amount` SET TAGS ('dbx_business_glossary_term' = 'Deductible Met Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Effective Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `eligibility_verification_count` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Verification Count');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `employer_name` SET TAGS ('dbx_business_glossary_term' = 'Employer Name');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `last_verification_date` SET TAGS ('dbx_business_glossary_term' = 'Last Eligibility Verification Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `last_verification_method` SET TAGS ('dbx_business_glossary_term' = 'Last Eligibility Verification Method');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `last_verification_method` SET TAGS ('dbx_value_regex' = 'Real-Time 270/271|Manual|Batch|Portal');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `last_verification_status` SET TAGS ('dbx_business_glossary_term' = 'Last Eligibility Verification Status');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `last_verification_status` SET TAGS ('dbx_value_regex' = 'Verified Active|Verified Inactive|Verification Failed|Pending');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `network_status` SET TAGS ('dbx_business_glossary_term' = 'Network Status');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `network_status` SET TAGS ('dbx_value_regex' = 'In-Network|Out-of-Network|Both');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `out_of_pocket_maximum` SET TAGS ('dbx_business_glossary_term' = 'Out-of-Pocket Maximum Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `out_of_pocket_met_amount` SET TAGS ('dbx_business_glossary_term' = 'Out-of-Pocket Met Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `payer_response_code` SET TAGS ('dbx_business_glossary_term' = 'Payer Response Code');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `payer_response_message` SET TAGS ('dbx_business_glossary_term' = 'Payer Response Message');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `pcn_number` SET TAGS ('dbx_business_glossary_term' = 'Processor Control Number (PCN)');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `plan_year_end_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Year End Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `plan_year_start_date` SET TAGS ('dbx_business_glossary_term' = 'Plan Year Start Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `policy_number` SET TAGS ('dbx_business_glossary_term' = 'Policy Number');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `referral_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Referral Required Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `rx_group_number` SET TAGS ('dbx_business_glossary_term' = 'Pharmacy Benefit Group Number');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `subscriber_relationship` SET TAGS ('dbx_business_glossary_term' = 'Subscriber Relationship');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `subscriber_relationship` SET TAGS ('dbx_value_regex' = 'Self|Spouse|Child|Other');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Termination Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_coverage` ALTER COLUMN `verification_source_system` SET TAGS ('dbx_business_glossary_term' = 'Verification Source System');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` SET TAGS ('dbx_subdomain' = 'financial_resolution');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_id` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `ar_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Transaction Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `charity_care_application_id` SET TAGS ('dbx_business_glossary_term' = 'Charity Care Application Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `collection_account_id` SET TAGS ('dbx_business_glossary_term' = 'Collection Account Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `cpt_code_id` SET TAGS ('dbx_business_glossary_term' = 'Cpt Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `drg_id` SET TAGS ('dbx_business_glossary_term' = 'Drg Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `financial_assistance_id` SET TAGS ('dbx_business_glossary_term' = 'Financial Assistance Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `guarantor_id` SET TAGS ('dbx_business_glossary_term' = 'Guarantor Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `original_write_off_id` SET TAGS ('dbx_business_glossary_term' = 'Original Write-Off Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approving User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `rac_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Recovery Audit Contractor (RAC) Audit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `restriction_request_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Restriction Request Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `tertiary_write_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `tertiary_write_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `tertiary_write_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `amount` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `appeal_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `appeal_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `appeal_status` SET TAGS ('dbx_business_glossary_term' = 'Appeal Status');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `appeal_status` SET TAGS ('dbx_value_regex' = 'pending|approved|denied|withdrawn');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `approval_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Approval Required Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `bad_debt_referral_date` SET TAGS ('dbx_business_glossary_term' = 'Bad Debt Referral Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `discount_percentage` SET TAGS ('dbx_business_glossary_term' = 'Discount Percentage');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `fpl_percentage` SET TAGS ('dbx_business_glossary_term' = 'Federal Poverty Level (FPL) Percentage');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Notes');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `original_balance` SET TAGS ('dbx_business_glossary_term' = 'Original Balance');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `posting_date` SET TAGS ('dbx_business_glossary_term' = 'Posting Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Reason Code');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Reason Description');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `remaining_balance` SET TAGS ('dbx_business_glossary_term' = 'Remaining Balance');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `revenue_code` SET TAGS ('dbx_business_glossary_term' = 'Revenue Code');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `reversal_date` SET TAGS ('dbx_business_glossary_term' = 'Reversal Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `reversal_flag` SET TAGS ('dbx_business_glossary_term' = 'Reversal Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `reversal_reason` SET TAGS ('dbx_business_glossary_term' = 'Reversal Reason');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `service_date` SET TAGS ('dbx_business_glossary_term' = 'Service Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_category` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Category');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_category` SET TAGS ('dbx_value_regex' = 'patient_responsibility|insurance_denial|uncompensated_care|policy_adjustment|audit_adjustment');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_date` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Date');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_number` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Number');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_status` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Status');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_status` SET TAGS ('dbx_value_regex' = 'pending|approved|posted|reversed|under_review');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_type` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Type');
ALTER TABLE `healthcare_ecm`.`billing`.`write_off` ALTER COLUMN `write_off_type` SET TAGS ('dbx_value_regex' = 'bad_debt|charity_care|small_balance|administrative|rac_recoupment|contractual_dispute');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` SET TAGS ('dbx_subdomain' = 'payment_processing');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `tertiary_payment_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `tertiary_payment_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`payment_plan` ALTER COLUMN `tertiary_payment_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` SET TAGS ('dbx_subdomain' = 'financial_resolution');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `rac_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Recovery Audit Contractor (RAC) Audit ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Care Site Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Audit Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Reviewer User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `cda_document_id` SET TAGS ('dbx_business_glossary_term' = 'Response Cda Document Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `tertiary_rac_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `tertiary_rac_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `tertiary_rac_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit ID');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_decision_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Decision Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_level` SET TAGS ('dbx_business_glossary_term' = 'Appeal Level');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_level` SET TAGS ('dbx_value_regex' = 'redetermination|reconsideration|alj_hearing|mac_review|federal_court');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_outcome_amount` SET TAGS ('dbx_business_glossary_term' = 'Appeal Outcome Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_status` SET TAGS ('dbx_business_glossary_term' = 'Appeal Status');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `appeal_status` SET TAGS ('dbx_value_regex' = 'pending|upheld|overturned|partially_favorable|withdrawn|dismissed');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_notes` SET TAGS ('dbx_business_glossary_term' = 'Audit Notes');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_number` SET TAGS ('dbx_business_glossary_term' = 'Audit Number');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Period End Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Period Start Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_request_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Request Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_status` SET TAGS ('dbx_business_glossary_term' = 'Audit Status');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `audit_type` SET TAGS ('dbx_business_glossary_term' = 'Audit Type');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `compliance_resolution_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Resolution Status');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `compliance_resolution_status` SET TAGS ('dbx_value_regex' = 'open|corrective_action_plan_submitted|corrective_action_implemented|validated|closed');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `contractor_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Contractor Jurisdiction');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `contractor_name` SET TAGS ('dbx_business_glossary_term' = 'Audit Contractor Name');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `contractor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `contractor_name` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `corrective_action_description` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Description');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `corrective_action_plan_date` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Plan Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `extrapolation_flag` SET TAGS ('dbx_business_glossary_term' = 'Extrapolation Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `extrapolation_sample_size` SET TAGS ('dbx_business_glossary_term' = 'Extrapolation Sample Size');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `extrapolation_universe_size` SET TAGS ('dbx_business_glossary_term' = 'Extrapolation Universe Size');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `finding_amount` SET TAGS ('dbx_business_glossary_term' = 'Finding Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `finding_type` SET TAGS ('dbx_business_glossary_term' = 'Finding Type');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `findings_issued_date` SET TAGS ('dbx_business_glossary_term' = 'Findings Issued Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `interest_amount` SET TAGS ('dbx_business_glossary_term' = 'Interest Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `overpayment_amount` SET TAGS ('dbx_business_glossary_term' = 'Overpayment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `records_requested_count` SET TAGS ('dbx_business_glossary_term' = 'Records Requested Count');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `records_submitted_count` SET TAGS ('dbx_business_glossary_term' = 'Records Submitted Count');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `recoupment_amount` SET TAGS ('dbx_business_glossary_term' = 'Recoupment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `recoupment_date` SET TAGS ('dbx_business_glossary_term' = 'Recoupment Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `recoupment_method` SET TAGS ('dbx_business_glossary_term' = 'Recoupment Method');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `recoupment_method` SET TAGS ('dbx_value_regex' = 'offset|direct_payment|installment_plan|settlement|waived');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `response_due_date` SET TAGS ('dbx_business_glossary_term' = 'Response Due Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `response_submitted_date` SET TAGS ('dbx_business_glossary_term' = 'Response Submitted Date');
ALTER TABLE `healthcare_ecm`.`billing`.`rac_audit` ALTER COLUMN `underpayment_amount` SET TAGS ('dbx_business_glossary_term' = 'Underpayment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` SET TAGS ('dbx_subdomain' = 'financial_resolution');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `charity_care_application_id` SET TAGS ('dbx_business_glossary_term' = 'Charity Care Application ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approver User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `guarantor_id` SET TAGS ('dbx_business_glossary_term' = 'Guarantor ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `patient_identity_match_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identity Match Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `primary_charity_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `primary_charity_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `primary_charity_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `tertiary_charity_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `tertiary_charity_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `tertiary_charity_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit ID');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `appeal_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `appeal_flag` SET TAGS ('dbx_business_glossary_term' = 'Appeal Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `appeal_status` SET TAGS ('dbx_business_glossary_term' = 'Appeal Status');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `appeal_status` SET TAGS ('dbx_value_regex' = 'pending|approved|denied|withdrawn');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_date` SET TAGS ('dbx_business_glossary_term' = 'Application Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_method` SET TAGS ('dbx_business_glossary_term' = 'Application Method');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_method` SET TAGS ('dbx_value_regex' = 'online_portal|paper_form|phone|in_person|mail|fax');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_number` SET TAGS ('dbx_business_glossary_term' = 'Application Number');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_source` SET TAGS ('dbx_business_glossary_term' = 'Application Source');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_source` SET TAGS ('dbx_value_regex' = 'patient_initiated|staff_initiated|screening_tool|presumptive_eligibility|third_party');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `application_status` SET TAGS ('dbx_business_glossary_term' = 'Application Status');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'pending|approved|denied|conditional');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `approved_discount_percentage` SET TAGS ('dbx_business_glossary_term' = 'Approved Discount Percentage');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `coverage_period_months` SET TAGS ('dbx_business_glossary_term' = 'Coverage Period Months');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `denial_date` SET TAGS ('dbx_business_glossary_term' = 'Denial Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `denial_reason` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `documentation_status` SET TAGS ('dbx_business_glossary_term' = 'Documentation Status');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `documentation_status` SET TAGS ('dbx_value_regex' = 'not_submitted|submitted|verified|incomplete|rejected');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `documentation_type` SET TAGS ('dbx_business_glossary_term' = 'Documentation Type');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `family_size` SET TAGS ('dbx_business_glossary_term' = 'Family Size');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `fap_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Financial Assistance Policy (FAP) Notification Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `fpl_percentage` SET TAGS ('dbx_business_glossary_term' = 'Federal Poverty Level (FPL) Percentage');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `household_income` SET TAGS ('dbx_business_glossary_term' = 'Household Income');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `household_income` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `household_income` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `irs_501r_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'IRS 501(r) Compliance Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `language_preference` SET TAGS ('dbx_business_glossary_term' = 'Language Preference');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `medicaid_application_date` SET TAGS ('dbx_business_glossary_term' = 'Medicaid Application Date');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `medicaid_pending_flag` SET TAGS ('dbx_business_glossary_term' = 'Medicaid Pending Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Application Notes');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `plain_language_summary_provided_flag` SET TAGS ('dbx_business_glossary_term' = 'Plain Language Summary Provided Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `presumptive_eligibility_basis` SET TAGS ('dbx_business_glossary_term' = 'Presumptive Eligibility Basis');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `presumptive_eligibility_flag` SET TAGS ('dbx_business_glossary_term' = 'Presumptive Eligibility Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `program_type` SET TAGS ('dbx_business_glossary_term' = 'Program Type');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `program_type` SET TAGS ('dbx_value_regex' = 'full_charity|partial_charity|sliding_scale|medicaid_presumptive|emergency_medicaid|prompt_pay_discount');
ALTER TABLE `healthcare_ecm`.`billing`.`charity_care_application` ALTER COLUMN `screening_score` SET TAGS ('dbx_business_glossary_term' = 'Screening Score');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` SET TAGS ('dbx_subdomain' = 'payment_processing');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_id` SET TAGS ('dbx_business_glossary_term' = 'Refund Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `ar_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Transaction Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Care Site Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `demographics_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `guarantor_id` SET TAGS ('dbx_business_glossary_term' = 'Guarantor Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payment_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `icd_code_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Diagnosis Icd Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `icd_code_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `icd_code_id` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `rac_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Recovery Audit Contractor (RAC) Audit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approval Authority User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_voided_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Voided By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_voided_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_voided_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `original_refund_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `amount` SET TAGS ('dbx_business_glossary_term' = 'Refund Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Refund Approval Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `check_number` SET TAGS ('dbx_business_glossary_term' = 'Check Number');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `cleared_date` SET TAGS ('dbx_business_glossary_term' = 'Refund Cleared Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `cms_credit_balance_report_flag` SET TAGS ('dbx_business_glossary_term' = 'Centers for Medicare and Medicaid Services (CMS) Credit Balance Report Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `cms_report_date` SET TAGS ('dbx_business_glossary_term' = 'Centers for Medicare and Medicaid Services (CMS) Report Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_last_four` SET TAGS ('dbx_business_glossary_term' = 'Credit Card Last Four Digits');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_last_four` SET TAGS ('dbx_value_regex' = '^[0-9]{4}$');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_last_four` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_last_four` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_type` SET TAGS ('dbx_business_glossary_term' = 'Credit Card Type');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_type` SET TAGS ('dbx_value_regex' = 'visa|mastercard|amex|discover|other');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_type` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `credit_card_type` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `disbursement_date` SET TAGS ('dbx_business_glossary_term' = 'Refund Disbursement Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `eft_trace_number` SET TAGS ('dbx_business_glossary_term' = 'Electronic Funds Transfer (EFT) Trace Number');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Refund Notes');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `original_payment_amount` SET TAGS ('dbx_business_glossary_term' = 'Original Payment Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line_1` SET TAGS ('dbx_business_glossary_term' = 'Payee Address Line 1');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line_1` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line_1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line_2` SET TAGS ('dbx_business_glossary_term' = 'Payee Address Line 2');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line_2` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line_2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_city` SET TAGS ('dbx_business_glossary_term' = 'Payee City');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_city` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_country_code` SET TAGS ('dbx_business_glossary_term' = 'Payee Country Code');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_name` SET TAGS ('dbx_business_glossary_term' = 'Payee Name');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Payee Postal Code');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_state` SET TAGS ('dbx_business_glossary_term' = 'Payee State');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_state` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_state` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `payee_state` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Refund Reason Code');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Refund Reason Description');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `reconciliation_date` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Status');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_value_regex' = 'pending|reconciled|unreconciled|exception');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_category` SET TAGS ('dbx_business_glossary_term' = 'Refund Category');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_category` SET TAGS ('dbx_value_regex' = 'overpayment|duplicate|credit_balance|rac_ordered|patient_request|payer_request');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_method` SET TAGS ('dbx_business_glossary_term' = 'Refund Method');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_method` SET TAGS ('dbx_value_regex' = 'check|eft|wire|credit_card_reversal|cash|offset');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_number` SET TAGS ('dbx_business_glossary_term' = 'Refund Number');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_status` SET TAGS ('dbx_business_glossary_term' = 'Refund Status');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_type` SET TAGS ('dbx_business_glossary_term' = 'Refund Type');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `refund_type` SET TAGS ('dbx_value_regex' = 'patient|guarantor|payer|vendor');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `request_date` SET TAGS ('dbx_business_glossary_term' = 'Refund Request Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `void_date` SET TAGS ('dbx_business_glossary_term' = 'Void Date');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `void_flag` SET TAGS ('dbx_business_glossary_term' = 'Void Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`refund` ALTER COLUMN `void_reason` SET TAGS ('dbx_business_glossary_term' = 'Void Reason');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` SET TAGS ('dbx_subdomain' = 'revenue_capture');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` SET TAGS ('dbx_association_edges' = 'billing.invoice,billing.coverage');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `invoice_coverage_billing_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Coverage Billing - Invoice Coverage Billing Id');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Coverage Billing - Coverage Id');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Coverage Billing - Invoice Id');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `adjudication_date` SET TAGS ('dbx_business_glossary_term' = 'Claim Adjudication Date');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `allowed_amount` SET TAGS ('dbx_business_glossary_term' = 'Allowed Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `check_eft_number` SET TAGS ('dbx_business_glossary_term' = 'Check or EFT Number');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `claim_filing_indicator_code` SET TAGS ('dbx_business_glossary_term' = 'Claim Filing Indicator Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `claim_status` SET TAGS ('dbx_business_glossary_term' = 'Claim Status');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `clearinghouse_trace_number` SET TAGS ('dbx_business_glossary_term' = 'Clearinghouse Trace Number');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `contractual_adjustment` SET TAGS ('dbx_business_glossary_term' = 'Contractual Adjustment');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `coordination_of_benefits_order` SET TAGS ('dbx_business_glossary_term' = 'Coordination of Benefits Order');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `coverage_status_at_billing` SET TAGS ('dbx_business_glossary_term' = 'Coverage Status at Billing');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Claim Denial Reason Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `denial_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Claim Denial Reason Description');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `paid_amount` SET TAGS ('dbx_business_glossary_term' = 'Paid Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `patient_responsibility` SET TAGS ('dbx_business_glossary_term' = 'Patient Responsibility');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `payer_claim_control_number` SET TAGS ('dbx_business_glossary_term' = 'Payer Claim Control Number');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `remittance_advice_number` SET TAGS ('dbx_business_glossary_term' = 'Remittance Advice Number');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Claim Submission Date');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_coverage_billing` ALTER COLUMN `submitted_amount` SET TAGS ('dbx_business_glossary_term' = 'Submitted Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` SET TAGS ('dbx_subdomain' = 'revenue_capture');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` SET TAGS ('dbx_association_edges' = 'billing.coverage,facility.care_site');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `billing_network_participation_id` SET TAGS ('dbx_business_glossary_term' = 'billing_network_participation Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Network Participation - Coverage Id');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Network Participation - Care Site Id');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `insurance_network_participation_id` SET TAGS ('dbx_business_glossary_term' = 'Network Participation Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `authorization_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Authorization Required Indicator');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `contract_status` SET TAGS ('dbx_business_glossary_term' = 'Contract Status');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `contract_type` SET TAGS ('dbx_business_glossary_term' = 'Network Contract Type');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `contracted_rate_schedule` SET TAGS ('dbx_business_glossary_term' = 'Contracted Rate Schedule Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `created_date` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Effective Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `last_updated_date` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `network_status` SET TAGS ('dbx_business_glossary_term' = 'Network Status');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `referral_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Referral Required Indicator');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Contract Termination Date');
ALTER TABLE `healthcare_ecm`.`billing`.`billing_network_participation` ALTER COLUMN `tier_level` SET TAGS ('dbx_business_glossary_term' = 'Network Tier Level');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` SET TAGS ('dbx_subdomain' = 'revenue_capture');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` SET TAGS ('dbx_association_edges' = 'billing.invoice,supply.material_master');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `invoice_line_item_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Line Item Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Line Item - Invoice Id');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Line Item - Material Master Id');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `allowed_amount` SET TAGS ('dbx_business_glossary_term' = 'Allowed Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Denial Reason Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `hcpcs_code` SET TAGS ('dbx_business_glossary_term' = 'HCPCS Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `line_amount` SET TAGS ('dbx_business_glossary_term' = 'Line Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `line_number` SET TAGS ('dbx_business_glossary_term' = 'Line Number');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `line_status` SET TAGS ('dbx_business_glossary_term' = 'Line Status');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `modifier_codes` SET TAGS ('dbx_business_glossary_term' = 'Modifier Codes');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `ndc_code` SET TAGS ('dbx_business_glossary_term' = 'NDC Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `ndc_quantity` SET TAGS ('dbx_business_glossary_term' = 'NDC Quantity');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `ndc_unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'NDC Unit of Measure');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `paid_amount` SET TAGS ('dbx_business_glossary_term' = 'Paid Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `quantity_billed` SET TAGS ('dbx_business_glossary_term' = 'Quantity Billed');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `revenue_code` SET TAGS ('dbx_business_glossary_term' = 'Revenue Code');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `service_date` SET TAGS ('dbx_business_glossary_term' = 'Service Date');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `udi` SET TAGS ('dbx_business_glossary_term' = 'Unique Device Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`invoice_line_item` ALTER COLUMN `unit_price` SET TAGS ('dbx_business_glossary_term' = 'Unit Price');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` SET TAGS ('dbx_subdomain' = 'revenue_capture');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` SET TAGS ('dbx_association_edges' = 'billing.cdm_entry,research.research_study');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `study_service_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Study Service Coverage Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `cdm_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Study Service Coverage - Cdm Entry Id');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Study Service Coverage - Research Study Id');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `billing_notes` SET TAGS ('dbx_business_glossary_term' = 'Study Service Billing Notes');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `coverage_determination` SET TAGS ('dbx_business_glossary_term' = 'Coverage Determination');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `created_date` SET TAGS ('dbx_business_glossary_term' = 'Record Created Date');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Effective Date');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Expiration Date');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `last_updated_by` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated By');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `last_updated_date` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Date');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `reimbursement_rate` SET TAGS ('dbx_business_glossary_term' = 'Study-Specific Reimbursement Rate');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `sponsor_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Sponsor Approval Date');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `standard_of_care_flag` SET TAGS ('dbx_business_glossary_term' = 'Standard of Care Flag');
ALTER TABLE `healthcare_ecm`.`billing`.`study_service_coverage` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Record Created By');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` SET TAGS ('dbx_subdomain' = 'revenue_capture');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` SET TAGS ('dbx_association_edges' = 'billing.cdm_entry,facility.care_site');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `site_cdm_pricing_id` SET TAGS ('dbx_business_glossary_term' = 'Site CDM Pricing Identifier');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Site Cdm Pricing - Care Site Id');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `cdm_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Site Cdm Pricing - Cdm Entry Id');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Configuration Updated By User');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `active_flag` SET TAGS ('dbx_business_glossary_term' = 'Site Activation Status');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Site Configuration Effective Date');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Site Configuration Expiration Date');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `last_updated_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Last Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `payer_contract_reference` SET TAGS ('dbx_business_glossary_term' = 'Payer Contract Reference');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `site_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Site-Specific Cost');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `site_specific_price` SET TAGS ('dbx_business_glossary_term' = 'Site-Specific Charge Amount');
ALTER TABLE `healthcare_ecm`.`billing`.`site_cdm_pricing` ALTER COLUMN `site_specific_revenue_code` SET TAGS ('dbx_business_glossary_term' = 'Site-Specific Revenue Code');
