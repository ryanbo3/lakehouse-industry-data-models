-- Schema for Domain: order | Business: Healthcare | Version: v1_ecm
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`clinical_order` (
    ...
    `clinician_id` BIGINT COMMENT 'FK to provider.clinician.clinician_id — Every clinical order must identify the ordering provider for clinical accountability, privilege verification, and regulatory compliance.',
    ...
    `primary_clinical_clinician_id` BIGINT COMMENT 'Reference to the clinician who entered or authorized this order via Computerized Physician Order Entry (CPOE). Corresponds to the ordering provider NPI-linked record in the provider master.',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Procedures, surgeries, and high-risk treatments require documented informed consent. Pre-procedure verification workflows mandate linking orders to the authorizing consent record. Core HIPAA and state',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`order_status_history` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the user (clinician, pharmacist, nurse, or system account) who triggered this lifecycle event. Distinct from the ordering provider — captures who made the change, not who originally placed the order. Core field for HIPAA audit accountability.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`referral_order` (
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Specialty referrals (behavioral health, substance use, HIV care) require specific consent for information sharing under 42 CFR Part 2 and state laws. Referral authorization workflow validates consent ',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`set` (
    ...
    `form_template_id` BIGINT COMMENT 'Foreign key linking to consent.consent_form_template. Business justification: Clinical order sets (surgical protocols, chemotherapy regimens) specify required consent forms. Protocol-driven consent management ensures correct consent template is presented when order set is activ',
    ...
    `protocol_id` BIGINT COMMENT 'Reference to the parent order set or protocol in a hierarchical protocol structure, enabling nested care pathway hierarchies (e.g., a sub-protocol within a broader sepsis management protocol). Null for top-level order sets with no parent.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`routing` (
    ...
    `previous_destination_department_org_unit_id` BIGINT COMMENT 'Identifier of the department from which the order was rerouted. Populated only for rerouted orders to maintain routing history.',
    ...
    `hipaa_security_risk_id` BIGINT COMMENT 'Foreign key linking to compliance.hipaa_security_risk. Business justification: Order routing systems handle ePHI and are subject to security risk assessments (interface vulnerabilities, access control weaknesses, encryption gaps). Security risk register tracks routing system ris',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`fulfillment` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.audit. Business justification: Order fulfillment processes are audited for compliance (specimen handling, turnaround times, charge capture). Audit scope includes fulfillment operations for quality and regulatory compliance verifica',
    ...
    `measure_id` BIGINT COMMENT 'Foreign key linking to quality.measure. Business justification: Order fulfillment timeliness is a direct quality indicator (door-to-balloon time for STEMI, sepsis bundle completion within 3 hours). Quality measure calculation requires fulfillment timestamps to det',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`verbal_order` (
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Verbal orders in emergencies document whether consent was obtained, waived under emergency exception, or deferred. Emergency treatment documentation workflow links orders to consent status for regulat',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`cpoe_alert` (
    ...
    `business_associate_agreement_id` BIGINT COMMENT 'Foreign key linking to compliance.business_associate_agreement. Business justification: Clinical decision support vendors providing alerts are business associates. BAA covers alert service and PHI access for HIPAA compliance and third-party risk management.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`order_authorization` (
    `order_authorization_id` BIGINT COMMENT 'Unique identifier for the order authorization record. Primary key for this entity.',
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supply.vendor. Business justification: Prior authorizations for devices may specify approved manufacturer/vendor (e.g., payer approves Medtronic but not Stryker for spinal hardware). Links authorization to supply chain vendor for procureme',
    ...
    `denial_reason_code` STRING COMMENT 'Standardized code indicating the reason for denial or partial approval. Typically uses payer-specific or industry-standard denial reason codes.',
    `denial_reason_description` STRING COMMENT 'Detailed textual explanation of the denial or partial approval reason provided by the payer.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`reconciliation` (
...
) COMMENT 'Transactional record capturing medication and order reconciliation events at care transitions (admission, transfer, discharge) as required by TJC NPSG.03.06.01. Captures reconciliation type, reconciling provider NPI, reconciliation datetime, encounter transition event, orders reviewed, orders continued, orders modified, orders discontinued, reconciliation completion status, and discrepancy details. Supports transitions of care quality measures, medication safety, and CMS Conditions of Participation compliance.';

CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`diet_order` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`therapy_order` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`alert_rule` (
    `alert_rule_id` BIGINT COMMENT 'Primary key for alert_rule',
    `approved_by_user_employee_id` BIGINT COMMENT 'Identifier of the clinical or administrative user who approved this alert rule for production use.',
    `employee_id` BIGINT COMMENT 'Identifier of the user or system account that created this alert rule record.',
    `last_modified_by_user_employee_id` BIGINT COMMENT 'Identifier of the user or system account that last modified this alert rule record.',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`order`.`routing_rule` (
    ...
    `fallback_routing_rule_id` BIGINT COMMENT 'Identifier of an alternative routing rule to apply if this rule cannot be executed due to capacity, availability, or other constraints.',
...
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ADD CONSTRAINT `fk_order_order_status_history_cpoe_alert_id` FOREIGN KEY (`cpoe_alert_id`) REFERENCES `healthcare_ecm`.`order`.`cpoe_alert`(`cpoe_alert_id`);
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ADD CONSTRAINT `fk_order_referral_order_order_authorization_id` FOREIGN KEY (`order_authorization_id`) REFERENCES `healthcare_ecm`.`order`.`order_authorization`(`order_authorization_id`);
...
ALTER TABLE `healthcare_ecm`.`order`.`routing` ADD CONSTRAINT `fk_order_routing_routing_rule_id` FOREIGN KEY (`routing_rule_id`) REFERENCES `healthcare_ecm`.`order`.`routing_rule`(`routing_rule_id`);
...
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ADD CONSTRAINT `fk_order_verbal_order_clinical_order_id` FOREIGN KEY (`clinical_order_id`) REFERENCES `healthcare_ecm`.`order`.`clinical_order`(`clinical_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ADD CONSTRAINT `fk_order_cpoe_alert_alert_rule_id` FOREIGN KEY (`alert_rule_id`) REFERENCES `healthcare_ecm`.`order`.`alert_rule`(`alert_rule_id`);
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ADD CONSTRAINT `fk_order_cpoe_alert_clinical_order_id` FOREIGN KEY (`clinical_order_id`) REFERENCES `healthcare_ecm`.`order`.`clinical_order`(`clinical_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ADD CONSTRAINT `fk_order_order_authorization_clinical_order_id` FOREIGN KEY (`clinical_order_id`) REFERENCES `healthcare_ecm`.`order`.`clinical_order`(`clinical_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ADD CONSTRAINT `fk_order_diet_order_clinical_order_id` FOREIGN KEY (`clinical_order_id`) REFERENCES `healthcare_ecm`.`order`.`clinical_order`(`clinical_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ADD CONSTRAINT `fk_order_diet_order_superseded_diet_order_id` FOREIGN KEY (`superseded_diet_order_id`) REFERENCES `healthcare_ecm`.`order`.`diet_order`(`diet_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ADD CONSTRAINT `fk_order_therapy_order_clinical_order_id` FOREIGN KEY (`clinical_order_id`) REFERENCES `healthcare_ecm`.`order`.`clinical_order`(`clinical_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ADD CONSTRAINT `fk_order_therapy_order_order_authorization_id` FOREIGN KEY (`order_authorization_id`) REFERENCES `healthcare_ecm`.`order`.`order_authorization`(`order_authorization_id`);
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ADD CONSTRAINT `fk_order_therapy_order_set_id` FOREIGN KEY (`set_id`) REFERENCES `healthcare_ecm`.`order`.`set`(`set_id`);
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ADD CONSTRAINT `fk_order_therapy_order_superseded_therapy_order_id` FOREIGN KEY (`superseded_therapy_order_id`) REFERENCES `healthcare_ecm`.`order`.`therapy_order`(`therapy_order_id`);
ALTER TABLE `healthcare_ecm`.`order`.`alert_rule` ADD CONSTRAINT `fk_order_alert_rule_parent_alert_rule_id` FOREIGN KEY (`parent_alert_rule_id`) REFERENCES `healthcare_ecm`.`order`.`alert_rule`(`alert_rule_id`);
ALTER TABLE `healthcare_ecm`.`order`.`routing_rule` ADD CONSTRAINT `fk_order_routing_rule_fallback_routing_rule_id` FOREIGN KEY (`fallback_routing_rule_id`) REFERENCES `healthcare_ecm`.`order`.`routing_rule`(`routing_rule_id`);
ALTER TABLE `healthcare_ecm`.`order`.`routing_rule` ADD CONSTRAINT `fk_order_routing_rule_parent_routing_rule_id` FOREIGN KEY (`parent_routing_rule_id`) REFERENCES `healthcare_ecm`.`order`.`routing_rule`(`routing_rule_id`);
...
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `attestation_id` SET TAGS ('dbx_business_glossary_term' = 'Attestation Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `primary_clinical_clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider ID');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `training_id` SET TAGS ('dbx_business_glossary_term' = 'Training Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `patient_mrn` SET TAGS ('dbx_business_glossary_term' = 'Patient Medical Record Number (MRN)');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `patient_mrn` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`clinical_order` ALTER COLUMN `patient_mrn` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `cpoe_alert_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Decision Support (CDS) Alert ID');
...
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modifying User ID');
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Receiving Department ID');
...
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Health Level Seven (HL7) Message ID');
...
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `source_system_event_message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Source System Event ID');
...
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `modifying_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Modifying Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`order_status_history` ALTER COLUMN `modifying_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Care Coordinator ID');
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `order_authorization_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Authorization ID');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Referral Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `receiving_facility_name` SET TAGS ('dbx_business_glossary_term' = 'Receiving Facility Name');
...
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `referring_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Referring Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`referral_order` ALTER COLUMN `referring_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
...
ALTER TABLE `healthcare_ecm`.`order`.`set` SET TAGS ('dbx_subdomain' = 'order_management');
...
ALTER TABLE `healthcare_ecm`.`order`.`set` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Centers for Medicare and Medicaid Services (CMS) Measure ID');
ALTER TABLE `healthcare_ecm`.`order`.`set` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`set` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`set` ALTER COLUMN `form_template_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Form Template Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`set` ALTER COLUMN `protocol_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Protocol ID');
ALTER TABLE `healthcare_ecm`.`order`.`set` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`set_item` SET TAGS ('dbx_subdomain' = 'order_management');
...
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `diagnosis_criteria` SET TAGS ('dbx_business_glossary_term' = 'Diagnosis Criteria');
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `diagnosis_criteria` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `diagnosis_criteria` SET TAGS ('dbx_pii_health' = 'true');
...
ALTER TABLE `healthcare_ecm`.`order`.`set_item` ALTER COLUMN `order_code` SET TAGS ('dbx_business_glossary_term' = 'Order Code');
...
ALTER TABLE `healthcare_ecm`.`order`.`routing` SET TAGS ('dbx_subdomain' = 'fulfillment_operations');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `routing_id` SET TAGS ('dbx_business_glossary_term' = 'Order Routing ID');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `business_associate_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Business Associate Agreement Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Manual Route User ID');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Interface Message ID');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Destination Department ID');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `previous_destination_department_org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Destination Department ID');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `routing_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Routing Rule ID');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `hipaa_security_risk_id` SET TAGS ('dbx_business_glossary_term' = 'Security Risk Hipaa Security Risk Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`routing` ALTER COLUMN `source_department_org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Source Department ID');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` SET TAGS ('dbx_subdomain' = 'fulfillment_operations');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Updated By User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Source System Fulfillment Identifier (ID)');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `scheduling_appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Appointment Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `subject_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Subject Enrollment Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`fulfillment` ALTER COLUMN `performing_department_code` SET TAGS ('dbx_business_glossary_term' = 'Performing Department Code');
...
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` SET TAGS ('dbx_subdomain' = 'authorization_compliance');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `verbal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Verbal Order ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Facility ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `clinical_order_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Waiver Authorized By ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `receiving_clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Receiving Clinician ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `subject_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Subject Enrollment Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit ID');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `authentication_datetime` SET TAGS ('dbx_business_glossary_term' = 'Authentication Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `authentication_deadline_datetime` SET TAGS ('dbx_business_glossary_term' = 'Authentication Deadline Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `co_signature_datetime` SET TAGS ('dbx_business_glossary_term' = 'Co-Signature Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `co_signature_deadline_datetime` SET TAGS ('dbx_business_glossary_term' = 'Co-Signature Deadline Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `co_signer_npi` SET TAGS ('dbx_business_glossary_term' = 'Co-Signer National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `co_signer_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `co_signer_role` SET TAGS ('dbx_business_glossary_term' = 'Co-Signer Role');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `co_signer_role` SET TAGS ('dbx_value_regex' = 'attending_physician|supervising_physician|pharmacist|charge_nurse|clinical_supervisor|other');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `controlled_substance_flag` SET TAGS ('dbx_business_glossary_term' = 'Controlled Substance Flag');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `created_datetime` SET TAGS ('dbx_business_glossary_term' = 'Created Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `dea_schedule` SET TAGS ('dbx_business_glossary_term' = 'Drug Enforcement Administration (DEA) Schedule');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `dea_schedule` SET TAGS ('dbx_value_regex' = 'I|II|III|IV|V');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `mrn` SET TAGS ('dbx_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `mrn` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `mrn` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_content` SET TAGS ('dbx_business_glossary_term' = 'Order Content');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_content` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_number` SET TAGS ('dbx_business_glossary_term' = 'Order Number');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_received_datetime` SET TAGS ('dbx_business_glossary_term' = 'Order Received Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_status` SET TAGS ('dbx_business_glossary_term' = 'Order Status');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_status` SET TAGS ('dbx_value_regex' = 'pending_authentication|authenticated|overdue|waived|cancelled');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `order_type` SET TAGS ('dbx_business_glossary_term' = 'Order Type');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `overdue_flag` SET TAGS ('dbx_business_glossary_term' = 'Overdue Flag');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Order Priority');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'stat|urgent|routine');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `read_back_confirmed_flag` SET TAGS ('dbx_business_glossary_term' = 'Read-Back Confirmed Flag');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `read_back_datetime` SET TAGS ('dbx_business_glossary_term' = 'Read-Back Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `receiving_clinician_npi` SET TAGS ('dbx_business_glossary_term' = 'Receiving Clinician National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `receiving_clinician_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `regulatory_basis` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Basis');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `updated_datetime` SET TAGS ('dbx_business_glossary_term' = 'Updated Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `verbal_order_type` SET TAGS ('dbx_business_glossary_term' = 'Verbal Order Type');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `verbal_order_type` SET TAGS ('dbx_value_regex' = 'verbal|telephone|co_signature|countersignature');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `waiver_datetime` SET TAGS ('dbx_business_glossary_term' = 'Waiver Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `waiver_flag` SET TAGS ('dbx_business_glossary_term' = 'Waiver Flag');
ALTER TABLE `healthcare_ecm`.`order`.`verbal_order` ALTER COLUMN `waiver_reason` SET TAGS ('dbx_business_glossary_term' = 'Waiver Reason');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` SET TAGS ('dbx_subdomain' = 'order_management');
...
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `coverage_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `form_template_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Form Template Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `imaging_modality` SET TAGS ('dbx_business_glossary_term' = 'Imaging Modality');
...
ALTER TABLE `healthcare_ecm`.`order`.`standing_order` ALTER COLUMN `medication_name` SET TAGS ('dbx_business_glossary_term' = 'Medication Name');
...
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` SET TAGS ('dbx_subdomain' = 'authorization_compliance');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `cpoe_alert_id` SET TAGS ('dbx_business_glossary_term' = 'Computerized Physician Order Entry (CPOE) Alert ID');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Alert Rule ID');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `business_associate_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Business Associate Agreement Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `clinical_order_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Order ID');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider ID');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `coverage_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit ID');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_acknowledged_flag` SET TAGS ('dbx_business_glossary_term' = 'Alert Acknowledged Flag');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_display_duration_seconds` SET TAGS ('dbx_business_glossary_term' = 'Alert Display Duration Seconds');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_fire_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Alert Fire Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_message` SET TAGS ('dbx_business_glossary_term' = 'Alert Message');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_priority` SET TAGS ('dbx_business_glossary_term' = 'Alert Priority');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_priority` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_rule_version` SET TAGS ('dbx_business_glossary_term' = 'Alert Rule Version');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_severity` SET TAGS ('dbx_business_glossary_term' = 'Alert Severity');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_severity` SET TAGS ('dbx_value_regex' = 'hard_stop|soft_stop|informational|warning');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_source_system` SET TAGS ('dbx_business_glossary_term' = 'Alert Source System');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_suppressed_flag` SET TAGS ('dbx_business_glossary_term' = 'Alert Suppressed Flag');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_title` SET TAGS ('dbx_business_glossary_term' = 'Alert Title');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_trigger_detail` SET TAGS ('dbx_business_glossary_term' = 'Alert Trigger Detail');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_type` SET TAGS ('dbx_business_glossary_term' = 'Alert Type');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `alert_type` SET TAGS ('dbx_value_regex' = 'drug_drug_interaction|drug_allergy|duplicate_order|dose_range_check|contraindication|formulary_substitution');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `allergy_substance` SET TAGS ('dbx_business_glossary_term' = 'Allergy Substance');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `clinical_context` SET TAGS ('dbx_business_glossary_term' = 'Clinical Context');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `duplicate_order_timeframe_hours` SET TAGS ('dbx_business_glossary_term' = 'Duplicate Order Timeframe Hours');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `formulary_status` SET TAGS ('dbx_business_glossary_term' = 'Formulary Status');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `formulary_status` SET TAGS ('dbx_value_regex' = 'formulary|non_formulary|restricted|prior_auth_required');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `interacting_medication_1` SET TAGS ('dbx_business_glossary_term' = 'Interacting Medication 1');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `interacting_medication_2` SET TAGS ('dbx_business_glossary_term' = 'Interacting Medication 2');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `ordered_dose` SET TAGS ('dbx_business_glossary_term' = 'Ordered Dose');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `ordered_dose_unit` SET TAGS ('dbx_business_glossary_term' = 'Ordered Dose Unit');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `override_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Override Reason Code');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `override_reason_text` SET TAGS ('dbx_business_glossary_term' = 'Override Reason Text');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `patient_age_at_alert` SET TAGS ('dbx_business_glossary_term' = 'Patient Age at Alert');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `patient_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Patient Weight Kilograms');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `provider_response` SET TAGS ('dbx_business_glossary_term' = 'Provider Response');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `provider_response` SET TAGS ('dbx_value_regex' = 'accepted|overridden|modified|cancelled|deferred');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `recommended_dose_max` SET TAGS ('dbx_business_glossary_term' = 'Recommended Dose Maximum');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `recommended_dose_min` SET TAGS ('dbx_business_glossary_term' = 'Recommended Dose Minimum');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `response_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Response Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `suggested_alternative_medication` SET TAGS ('dbx_business_glossary_term' = 'Suggested Alternative Medication');
ALTER TABLE `healthcare_ecm`.`order`.`cpoe_alert` ALTER COLUMN `suppression_reason` SET TAGS ('dbx_business_glossary_term' = 'Suppression Reason');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` SET TAGS ('dbx_subdomain' = 'authorization_compliance');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `order_authorization_id` SET TAGS ('dbx_business_glossary_term' = 'Order Authorization Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Approved Vendor Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `business_associate_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Business Associate Agreement Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `clinical_order_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `cpt_code_id` SET TAGS ('dbx_business_glossary_term' = 'Procedure Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `icd_code_id` SET TAGS ('dbx_business_glossary_term' = 'Diagnosis Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `insurance_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Insurance Coverage Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `appeal_decision_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Decision Date');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `appeal_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Appeal Filed Date');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `appeal_status` SET TAGS ('dbx_business_glossary_term' = 'Authorization Appeal Status');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `appeal_status` SET TAGS ('dbx_value_regex' = 'not_appealed|appeal_pending|appeal_approved|appeal_denied|appeal_withdrawn');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `approved_quantity` SET TAGS ('dbx_business_glossary_term' = 'Approved Service Quantity');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `approved_quantity_unit` SET TAGS ('dbx_business_glossary_term' = 'Approved Quantity Unit of Measure');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `approved_quantity_unit` SET TAGS ('dbx_value_regex' = 'visits|units|sessions|days|procedures|treatments');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Payer Authorization Number');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `authorization_status` SET TAGS ('dbx_business_glossary_term' = 'Authorization Decision Status');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `authorization_type` SET TAGS ('dbx_business_glossary_term' = 'Authorization Type');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `authorization_type` SET TAGS ('dbx_value_regex' = 'prior_authorization|concurrent_review|retrospective_review|pre_certification|referral_authorization');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `clinical_notes` SET TAGS ('dbx_business_glossary_term' = 'Authorization Clinical Notes');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `clinical_notes` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `decision_datetime` SET TAGS ('dbx_business_glossary_term' = 'Authorization Decision Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `denial_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Authorization Denial Reason Code');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `denial_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Authorization Denial Reason Description');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Authorization Effective Date');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Authorization Expiration Date');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('dbx_business_glossary_term' = 'Patient Medical Record Number (MRN)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `patient_mrn` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `peer_to_peer_conducted` SET TAGS ('dbx_business_glossary_term' = 'Peer-to-Peer Review Conducted Flag');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `peer_to_peer_date` SET TAGS ('dbx_business_glossary_term' = 'Peer-to-Peer Review Date');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Authorization Request Priority');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'routine|urgent|stat|emergent');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `request_datetime` SET TAGS ('dbx_business_glossary_term' = 'Authorization Request Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `requested_quantity` SET TAGS ('dbx_business_glossary_term' = 'Requested Service Quantity');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `requesting_provider_name` SET TAGS ('dbx_business_glossary_term' = 'Requesting Provider Name');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `requesting_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Requesting Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `requesting_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `reviewer_name` SET TAGS ('dbx_business_glossary_term' = 'Payer Reviewer Name');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `service_category` SET TAGS ('dbx_business_glossary_term' = 'Service Category');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `servicing_facility_name` SET TAGS ('dbx_business_glossary_term' = 'Servicing Facility Name');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `servicing_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Servicing Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `servicing_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System Name');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `submission_method` SET TAGS ('dbx_business_glossary_term' = 'Authorization Submission Method');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `submission_method` SET TAGS ('dbx_value_regex' = 'electronic|fax|phone|portal|mail');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `turnaround_time_hours` SET TAGS ('dbx_business_glossary_term' = 'Authorization Turnaround Time in Hours');
ALTER TABLE `healthcare_ecm`.`order`.`order_authorization` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` SET TAGS ('dbx_subdomain' = 'authorization_compliance');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_id` SET TAGS ('dbx_business_glossary_term' = 'Order Reconciliation ID');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Reconciling Provider ID');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Source System ID');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit ID');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `allergy_review_indicator` SET TAGS ('dbx_business_glossary_term' = 'Allergy Review Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `attestation_datetime` SET TAGS ('dbx_business_glossary_term' = 'Attestation Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `attestation_signature` SET TAGS ('dbx_business_glossary_term' = 'Attestation Signature');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `completion_indicator` SET TAGS ('dbx_business_glossary_term' = 'Completion Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `compliance_indicator` SET TAGS ('dbx_business_glossary_term' = 'Compliance Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `created_datetime` SET TAGS ('dbx_business_glossary_term' = 'Created Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `datetime` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `discrepancy_description` SET TAGS ('dbx_business_glossary_term' = 'Discrepancy Description');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `discrepancy_identified_indicator` SET TAGS ('dbx_business_glossary_term' = 'Discrepancy Identified Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `discrepancy_severity` SET TAGS ('dbx_business_glossary_term' = 'Discrepancy Severity');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `discrepancy_severity` SET TAGS ('dbx_value_regex' = 'high|medium|low|none');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `documentation_location` SET TAGS ('dbx_business_glossary_term' = 'Documentation Location');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `drug_interaction_check_indicator` SET TAGS ('dbx_business_glossary_term' = 'Drug Interaction Check Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Duration in Minutes');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `formulary_check_indicator` SET TAGS ('dbx_business_glossary_term' = 'Formulary Check Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `home_medication_list_reviewed_indicator` SET TAGS ('dbx_business_glossary_term' = 'Home Medication List Reviewed Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `last_updated_datetime` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `next_provider_communication_indicator` SET TAGS ('dbx_business_glossary_term' = 'Next Provider Communication Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `non_compliance_reason` SET TAGS ('dbx_business_glossary_term' = 'Non-Compliance Reason');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `orders_added_count` SET TAGS ('dbx_business_glossary_term' = 'Orders Added Count');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `orders_continued_count` SET TAGS ('dbx_business_glossary_term' = 'Orders Continued Count');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `orders_discontinued_count` SET TAGS ('dbx_business_glossary_term' = 'Orders Discontinued Count');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `orders_modified_count` SET TAGS ('dbx_business_glossary_term' = 'Orders Modified Count');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `orders_reviewed_count` SET TAGS ('dbx_business_glossary_term' = 'Orders Reviewed Count');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `patient_communication_indicator` SET TAGS ('dbx_business_glossary_term' = 'Patient Communication Indicator');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_method` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Method');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_method` SET TAGS ('dbx_value_regex' = 'cpoe|paper|verbal|hybrid');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Status');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_status` SET TAGS ('dbx_value_regex' = 'completed|incomplete|in_progress|not_required|deferred');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_type` SET TAGS ('dbx_business_glossary_term' = 'Reconciliation Type');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciliation_type` SET TAGS ('dbx_value_regex' = 'admission|transfer|discharge|pre_procedure|post_procedure|outpatient_visit');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciling_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Reconciling Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `reconciling_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `source_medication_list_type` SET TAGS ('dbx_business_glossary_term' = 'Source Medication List Type');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `source_medication_list_type` SET TAGS ('dbx_value_regex' = 'patient_reported|pharmacy_records|prior_emr|pcp_records|family_reported|medication_bottles');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `healthcare_ecm`.`order`.`reconciliation` ALTER COLUMN `transition_event` SET TAGS ('dbx_business_glossary_term' = 'Transition Event');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` SET TAGS ('dbx_data_type' = 'Master');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` SET TAGS ('dbx_subdomain' = 'specialty_requests');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `diet_order_id` SET TAGS ('dbx_business_glossary_term' = 'Diet Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `clinical_order_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Order Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `demographics_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Dietitian Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `icd_code_id` SET TAGS ('dbx_business_glossary_term' = 'Icd10 Indication Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Nutrition Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Department Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `subject_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Subject Enrollment Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `superseded_diet_order_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `allergen_exclusions` SET TAGS ('dbx_business_glossary_term' = 'Allergen Exclusions');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `calorie_target` SET TAGS ('dbx_business_glossary_term' = 'Daily Calorie Target');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `clinical_indication` SET TAGS ('dbx_business_glossary_term' = 'Clinical Indication');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `diet_type` SET TAGS ('dbx_business_glossary_term' = 'Diet Type');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `diet_type_code` SET TAGS ('dbx_business_glossary_term' = 'Diet Type Code');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `feeding_route` SET TAGS ('dbx_business_glossary_term' = 'Feeding Route');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `feeding_route` SET TAGS ('dbx_value_regex' = 'oral|enteral|parenteral|nasogastric|gastrostomy|jejunostomy');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `fluid_consistency` SET TAGS ('dbx_business_glossary_term' = 'Fluid Consistency');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `fluid_consistency` SET TAGS ('dbx_value_regex' = 'thin|nectar-thick|honey-thick|pudding-thick');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `fluid_restriction_ml` SET TAGS ('dbx_business_glossary_term' = 'Fluid Restriction in Milliliters (mL)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `food_preferences` SET TAGS ('dbx_business_glossary_term' = 'Food Preferences');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `meal_schedule` SET TAGS ('dbx_business_glossary_term' = 'Meal Schedule');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `mrn` SET TAGS ('dbx_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `mrn` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `mrn` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `npo_reason` SET TAGS ('dbx_business_glossary_term' = 'Nothing by Mouth (NPO) Reason');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `npo_status` SET TAGS ('dbx_business_glossary_term' = 'Nothing by Mouth (NPO) Status');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `protein_target_grams` SET TAGS ('dbx_business_glossary_term' = 'Daily Protein Target in Grams');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `source_system_order_reference` SET TAGS ('dbx_business_glossary_term' = 'Source System Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `special_instructions` SET TAGS ('dbx_business_glossary_term' = 'Special Instructions');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `supplement_frequency` SET TAGS ('dbx_business_glossary_term' = 'Supplement Frequency');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `supplement_name` SET TAGS ('dbx_business_glossary_term' = 'Nutritional Supplement Name');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `texture_modification` SET TAGS ('dbx_business_glossary_term' = 'Texture Modification');
ALTER TABLE `healthcare_ecm`.`order`.`diet_order` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` SET TAGS ('dbx_data_type' = 'Master');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` SET TAGS ('dbx_subdomain' = 'specialty_requests');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `therapy_order_id` SET TAGS ('dbx_business_glossary_term' = 'Therapy Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Facility Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `clinical_order_id` SET TAGS ('dbx_business_glossary_term' = 'Parent Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `cpt_code_id` SET TAGS ('dbx_business_glossary_term' = 'Cpt Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `icd_code_id` SET TAGS ('dbx_business_glossary_term' = 'Icd10 Diagnosis Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `mpi_record_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_authorization_id` SET TAGS ('dbx_business_glossary_term' = 'Authorization Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Performing Department Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `scheduling_appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Appointment Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `set_id` SET TAGS ('dbx_business_glossary_term' = 'Order Set Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `subject_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Subject Enrollment Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Therapy Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Consent Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `superseded_therapy_order_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `authorization_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Authorization Required Flag');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `body_site` SET TAGS ('dbx_business_glossary_term' = 'Body Site');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `cancelled_datetime` SET TAGS ('dbx_business_glossary_term' = 'Cancelled Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `clinical_indication` SET TAGS ('dbx_business_glossary_term' = 'Clinical Indication');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `completed_datetime` SET TAGS ('dbx_business_glossary_term' = 'Completed Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `duration_unit` SET TAGS ('dbx_business_glossary_term' = 'Duration Unit');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `duration_unit` SET TAGS ('dbx_value_regex' = 'minutes|hours');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `duration_value` SET TAGS ('dbx_business_glossary_term' = 'Duration Value');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'Therapy End Date');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `frequency_code` SET TAGS ('dbx_business_glossary_term' = 'Frequency Code');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `frequency_description` SET TAGS ('dbx_business_glossary_term' = 'Frequency Description');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `is_recurring` SET TAGS ('dbx_business_glossary_term' = 'Is Recurring Order');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `laterality` SET TAGS ('dbx_business_glossary_term' = 'Laterality');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `laterality` SET TAGS ('dbx_value_regex' = 'left|right|bilateral');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_datetime` SET TAGS ('dbx_business_glossary_term' = 'Order Date and Time');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_mode` SET TAGS ('dbx_business_glossary_term' = 'Order Mode');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_mode` SET TAGS ('dbx_value_regex' = 'inpatient|outpatient|home_health|telehealth');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_number` SET TAGS ('dbx_business_glossary_term' = 'Therapy Order Number');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_status` SET TAGS ('dbx_business_glossary_term' = 'Therapy Order Status');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `order_status` SET TAGS ('dbx_value_regex' = 'draft|active|on-hold|completed|cancelled|discontinued');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `patient_instructions` SET TAGS ('dbx_business_glossary_term' = 'Patient Instructions');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Order Priority');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'routine|urgent|stat|asap');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `sessions_completed` SET TAGS ('dbx_business_glossary_term' = 'Sessions Completed');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `sessions_remaining` SET TAGS ('dbx_business_glossary_term' = 'Sessions Remaining');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `source_system_order_reference` SET TAGS ('dbx_business_glossary_term' = 'Source System Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `special_instructions` SET TAGS ('dbx_business_glossary_term' = 'Special Instructions');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `start_date` SET TAGS ('dbx_business_glossary_term' = 'Therapy Start Date');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `therapy_service_code` SET TAGS ('dbx_business_glossary_term' = 'Therapy Service Code');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `therapy_service_description` SET TAGS ('dbx_business_glossary_term' = 'Therapy Service Description');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `therapy_type` SET TAGS ('dbx_business_glossary_term' = 'Therapy Type');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `therapy_type` SET TAGS ('dbx_value_regex' = 'physical_therapy|occupational_therapy|speech_therapy|respiratory_therapy');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `total_sessions_ordered` SET TAGS ('dbx_business_glossary_term' = 'Total Sessions Ordered');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `treatment_goal` SET TAGS ('dbx_business_glossary_term' = 'Treatment Goal');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `treatment_goal` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `treatment_goal` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`therapy_order` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`order`.`alert_rule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`order`.`alert_rule` SET TAGS ('dbx_subdomain' = 'authorization_compliance');
ALTER TABLE `healthcare_ecm`.`order`.`alert_rule` ALTER COLUMN `alert_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Alert Rule Identifier');
ALTER TABLE `healthcare_ecm`.`order`.`alert_rule` ALTER COLUMN `parent_alert_rule_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `healthcare_ecm`.`order`.`routing_rule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`order`.`routing_rule` SET TAGS ('dbx_subdomain' = 'fulfillment_operations');
ALTER TABLE `healthcare_ecm`.`order`.`routing_rule` ALTER COLUMN `routing_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Routing Rule Identifier');
ALTER TABLE `healthcare_ecm`.`order`.`routing_rule` ALTER COLUMN `parent_routing_rule_id` SET TAGS ('dbx_self_ref_fk' = 'true');
