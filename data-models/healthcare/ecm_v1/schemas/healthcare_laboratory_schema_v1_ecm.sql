-- Schema for Domain: laboratory | Business: Healthcare | Version: v1_ecm
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`lab_order` (
    ...
    `compliance_program_id` BIGINT COMMENT 'Foreign key linking to compliance.compliance_program. Business justification: Lab orders must comply with regulatory programs (CLIA, CAP accreditation). Healthcare operations require tracking which compliance program governs each test order for regulatory reporting and audit tr',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Lab orders for genetic testing, HIV, substance abuse screening, and research protocols require documented patient consent before specimen collection. Real-world lab workflows verify consent status at ',
    ...
    `training_id` BIGINT COMMENT 'Foreign key linking to compliance.training. Business justification: Lab ordering requires documented training on test selection, clinical indications, specimen requirements. CLIA and medical staff bylaws require competency verification; linking orders to training enab',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`specimen` (
    ...
    `message_log_id` BIGINT COMMENT 'Foreign key linking to interoperability.message_log. Business justification: Specimen collection events trigger ADT messages or specimen tracking notifications to reference labs and LIS systems. Links specimen to transmission event for chain-of-custody audit and send-out lab c',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Specimen collection for biobanking, research studies, genetic testing, and tissue retention requires documented consent. Pathology specimens used in teaching or future research require explicit patien',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`test_result` (
    ...
    `hedis_measure_id` BIGINT COMMENT 'Foreign key linking to quality.hedis_measure. Business justification: Lab results (HbA1c, LDL, BMI) directly determine HEDIS measure numerator compliance. Quality reporting systems link test results to specific HEDIS measures for performance calculation and gap closure ',
    ...
    `payer_id` BIGINT COMMENT 'Foreign key linking to insurance.payer. Business justification: Payers mandate critical value notification protocols and result reporting requirements. Utilization review, quality reporting programs, and value-based care contracts require payer-linked result data ',
    `phi_access_log_id` BIGINT COMMENT 'Foreign key linking to compliance.phi_access_log. Business justification: Test results are PHI requiring access logging per HIPAA. Healthcare operations link results to access logs for breach investigations, inappropriate access detection, and audit trail compliance.',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Results disclosure for genetic tests, HIV, substance abuse screening requires consent verification before release. Patient portal access to sensitive lab results, results sharing with third parties, a',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`pathology_report` (
    ...
    `mortality_review_id` BIGINT COMMENT 'Foreign key linking to quality.mortality_review. Business justification: Pathology findings (cancer diagnoses, unexpected autopsy findings, discrepancies between clinical and pathologic diagnoses) are reviewed in mortality review committees. Mortality review workflows refe',
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Pathology specimens used in research, teaching, tumor registries, or biobanking require documented consent. Tissue retention beyond diagnostic use, molecular profiling for research, and future contact',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` (
    ...
    `improvement_initiative_id` BIGINT COMMENT 'Foreign key linking to quality.improvement_initiative. Business justification: HAI culture data drives infection prevention improvement initiatives (CLABSI reduction, CAUTI prevention, SSI reduction). Quality teams link culture surveillance data to improvement projects for basel',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` (
    ...
    `record_id` BIGINT COMMENT 'Foreign key linking to consent.consent_record. Business justification: Blood product administration requires documented consent. Directed donations (family member to patient), autologous transfusions (patients own blood), and Jehovahs Witness refusals all require conse',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`qc_run` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`instrument` (
    ...
    `accreditation_survey_id` BIGINT COMMENT 'Foreign key linking to quality.accreditation_survey. Business justification: Laboratory instruments are inspected during accreditation surveys (CAP, TJC, state health departments). Surveyors document instrument maintenance, calibration, and QC compliance as part of laboratory ',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`test_catalog` (
    ...
    `monitoring_activity_id` BIGINT COMMENT 'Foreign key linking to compliance.monitoring_activity. Business justification: Test catalog monitored for appropriate utilization, medical necessity compliance, and overutilization. Utilization management programs require linking catalog entries to monitoring activities for inte',
    ...
    `regulatory_change_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_change. Business justification: Test catalog updated in response to regulatory changes (new CPT codes, PAMA requirements, FDA clearances). Linking catalog changes to regulatory drivers enables impact tracking and compliance document',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`lab_charge` (
    ...
    `measure_result_id` BIGINT COMMENT 'Foreign key linking to quality.measure_result. Business justification: Lab charges provide utilization data for efficiency/cost quality measures in VBP programs. Quality systems track lab utilization costs (cost per case, unnecessary testing) as part of efficiency domain',
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`molecular_test` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` (
...
CREATE OR REPLACE TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` (
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ADD CONSTRAINT `fk_laboratory_test_result_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ADD CONSTRAINT `fk_laboratory_pathology_report_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ADD CONSTRAINT `fk_laboratory_microbiology_culture_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ADD CONSTRAINT `fk_laboratory_blood_bank_unit_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ADD CONSTRAINT `fk_laboratory_point_of_care_test_instrument_id` FOREIGN KEY (`instrument_id`) REFERENCES `healthcare_ecm`.`laboratory`.`instrument`(`instrument_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ADD CONSTRAINT `fk_laboratory_point_of_care_test_previous_result_point_of_care_test_id` FOREIGN KEY (`previous_result_point_of_care_test_id`) REFERENCES `healthcare_ecm`.`laboratory`.`point_of_care_test`(`point_of_care_test_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ADD CONSTRAINT `fk_laboratory_point_of_care_test_qc_run_id` FOREIGN KEY (`qc_run_id`) REFERENCES `healthcare_ecm`.`laboratory`.`qc_run`(`qc_run_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ADD CONSTRAINT `fk_laboratory_point_of_care_test_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ADD CONSTRAINT `fk_laboratory_point_of_care_test_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ADD CONSTRAINT `fk_laboratory_qc_run_instrument_id` FOREIGN KEY (`instrument_id`) REFERENCES `healthcare_ecm`.`laboratory`.`instrument`(`instrument_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ADD CONSTRAINT `fk_laboratory_qc_run_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ADD CONSTRAINT `fk_laboratory_qc_run_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ADD CONSTRAINT `fk_laboratory_instrument_clia_certificate_id` FOREIGN KEY (`clia_certificate_id`) REFERENCES `healthcare_ecm`.`laboratory`.`clia_certificate`(`clia_certificate_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ADD CONSTRAINT `fk_laboratory_test_catalog_clia_certificate_id` FOREIGN KEY (`clia_certificate_id`) REFERENCES `healthcare_ecm`.`laboratory`.`clia_certificate`(`clia_certificate_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ADD CONSTRAINT `fk_laboratory_test_catalog_instrument_id` FOREIGN KEY (`instrument_id`) REFERENCES `healthcare_ecm`.`laboratory`.`instrument`(`instrument_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ADD CONSTRAINT `fk_laboratory_lab_charge_lab_order_id` FOREIGN KEY (`lab_order_id`) REFERENCES `healthcare_ecm`.`laboratory`.`lab_order`(`lab_order_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ADD CONSTRAINT `fk_laboratory_lab_charge_specimen_id` FOREIGN KEY (`specimen_id`) REFERENCES `healthcare_ecm`.`laboratory`.`specimen`(`specimen_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ADD CONSTRAINT `fk_laboratory_lab_charge_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ADD CONSTRAINT `fk_laboratory_lab_charge_test_result_id` FOREIGN KEY (`test_result_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_result`(`test_result_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ADD CONSTRAINT `fk_laboratory_molecular_test_instrument_id` FOREIGN KEY (`instrument_id`) REFERENCES `healthcare_ecm`.`laboratory`.`instrument`(`instrument_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ADD CONSTRAINT `fk_laboratory_molecular_test_lab_order_id` FOREIGN KEY (`lab_order_id`) REFERENCES `healthcare_ecm`.`laboratory`.`lab_order`(`lab_order_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ADD CONSTRAINT `fk_laboratory_molecular_test_reagent_lot_id` FOREIGN KEY (`reagent_lot_id`) REFERENCES `healthcare_ecm`.`laboratory`.`reagent_lot`(`reagent_lot_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ADD CONSTRAINT `fk_laboratory_molecular_test_specimen_id` FOREIGN KEY (`specimen_id`) REFERENCES `healthcare_ecm`.`laboratory`.`specimen`(`specimen_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ADD CONSTRAINT `fk_laboratory_molecular_test_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ADD CONSTRAINT `fk_laboratory_reagent_lot_instrument_id` FOREIGN KEY (`instrument_id`) REFERENCES `healthcare_ecm`.`laboratory`.`instrument`(`instrument_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ADD CONSTRAINT `fk_laboratory_test_coverage_policy_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ADD CONSTRAINT `fk_laboratory_study_test_requirement_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ADD CONSTRAINT `fk_laboratory_lab_fee_schedule_line_test_catalog_id` FOREIGN KEY (`test_catalog_id`) REFERENCES `healthcare_ecm`.`laboratory`.`test_catalog`(`test_catalog_id`);
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ADD CONSTRAINT `fk_laboratory_instrument_policy_compliance_instrument_id` FOREIGN KEY (`instrument_id`) REFERENCES `healthcare_ecm`.`laboratory`.`instrument`(`instrument_id`);
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `interface_channel_id` SET TAGS ('dbx_business_glossary_term' = 'Interface Channel Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `monitoring_activity_id` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Activity Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `scheduling_appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Scheduling Appointment Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `training_id` SET TAGS ('dbx_business_glossary_term' = 'Training Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `diagnosis_code` SET TAGS ('dbx_business_glossary_term' = 'ICD-10 (International Classification of Diseases 10th Revision) Diagnosis Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `diagnosis_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `diagnosis_code` SET TAGS ('dbx_pii_health' = 'true');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `order_set_name` SET TAGS ('dbx_business_glossary_term' = 'Order Set Name');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `performing_lab_location` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Location');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `reference_lab_name` SET TAGS ('dbx_business_glossary_term' = 'Reference Laboratory Name');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_order` ALTER COLUMN `standing_order` SET TAGS ('dbx_business_glossary_term' = 'Standing Order Flag');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Collector Identifier');
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Message Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `scheduling_appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Scheduling Appointment Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `receiving_lab_location` SET TAGS ('dbx_business_glossary_term' = 'Receiving Laboratory Location');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`specimen` ALTER COLUMN `storage_location` SET TAGS ('dbx_business_glossary_term' = 'Storage Location');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `fhir_resource_log_id` SET TAGS ('dbx_business_glossary_term' = 'Fhir Resource Log Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `hedis_measure_id` SET TAGS ('dbx_business_glossary_term' = 'Hedis Measure Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `phi_access_log_id` SET TAGS ('dbx_business_glossary_term' = 'Phi Access Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Verifying Technologist Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `tertiary_test_amending_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Amending User Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `tertiary_test_amending_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `tertiary_test_amending_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `clia_number` SET TAGS ('dbx_business_glossary_term' = 'Clinical Laboratory Improvement Amendments (CLIA) Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `clia_number` SET TAGS ('dbx_value_regex' = '^[0-9]{2}D[0-9]{7}$');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_result` ALTER COLUMN `performing_lab_facility` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Facility');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`reference_range` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`reference_range` ALTER COLUMN `instrument_platform` SET TAGS ('dbx_business_glossary_term' = 'Instrument Platform');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `cda_document_id` SET TAGS ('dbx_business_glossary_term' = 'Cda Document Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `mortality_review_id` SET TAGS ('dbx_business_glossary_term' = 'Mortality Review Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `phi_access_log_id` SET TAGS ('dbx_business_glossary_term' = 'Phi Access Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `quality_peer_review_id` SET TAGS ('dbx_business_glossary_term' = 'Peer Review Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `clia_number` SET TAGS ('dbx_business_glossary_term' = 'CLIA Number');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`pathology_report` ALTER COLUMN `performing_laboratory` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `improvement_initiative_id` SET TAGS ('dbx_business_glossary_term' = 'Improvement Initiative Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Message Log Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `osha_exposure_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Osha Exposure Incident Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `patient_safety_event_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Safety Event Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Performing Technologist Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`microbiology_culture` ALTER COLUMN `substance_use_consent_id` SET TAGS ('dbx_business_glossary_term' = 'Substance Use Consent Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`susceptibility_result` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`susceptibility_result` ALTER COLUMN `loinc_code` SET TAGS ('dbx_business_glossary_term' = 'Logical Observation Identifiers Names and Codes (LOINC) Code');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`susceptibility_result` ALTER COLUMN `performing_lab_code` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`susceptibility_result` ALTER COLUMN `performing_lab_name` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Name');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`susceptibility_result` ALTER COLUMN `snomed_code` SET TAGS ('dbx_business_glossary_term' = 'Systematized Nomenclature of Medicine Clinical Terms (SNOMED CT) Code');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` SET TAGS ('dbx_subdomain' = 'transfusion_services');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `record_id` SET TAGS ('dbx_business_glossary_term' = 'Consent Record Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `issued_to_location` SET TAGS ('dbx_business_glossary_term' = 'Issued to Clinical Location');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `reserved_for_patient_mrn` SET TAGS ('dbx_business_glossary_term' = 'Reserved for Patient Medical Record Number (MRN)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `reserved_for_patient_mrn` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`blood_bank_unit` ALTER COLUMN `reserved_for_patient_mrn` SET TAGS ('dbx_pii_identifier' = 'true');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` SET TAGS ('dbx_subdomain' = 'transfusion_services');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Message Log Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `patient_safety_event_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Safety Event Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Crossmatch Technologist Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_business_glossary_term' = 'Treatment Consent Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`transfusion_event` ALTER COLUMN `treatment_consent_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `point_of_care_test_id` SET TAGS ('dbx_business_glossary_term' = 'Point Of Care Test Identifier');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Performing Location ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `clinician_id` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `demographics_id` SET TAGS ('dbx_business_glossary_term' = 'Patient ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Operator ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Device ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `message_log_id` SET TAGS ('dbx_business_glossary_term' = 'Message Log Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `clinical_order_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `previous_result_point_of_care_test_id` SET TAGS ('dbx_business_glossary_term' = 'Previous Result ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `qc_run_id` SET TAGS ('dbx_business_glossary_term' = 'Qc Run Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Test Catalog Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Encounter ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `abnormal_flag` SET TAGS ('dbx_business_glossary_term' = 'Abnormal Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `abnormal_flag` SET TAGS ('dbx_value_regex' = 'normal|low|high|critical_low|critical_high|abnormal');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `clia_waived_flag` SET TAGS ('dbx_business_glossary_term' = 'Clinical Laboratory Improvement Amendments (CLIA) Waived Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `collection_datetime` SET TAGS ('dbx_business_glossary_term' = 'Collection Date and Time');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `corrected_result_flag` SET TAGS ('dbx_business_glossary_term' = 'Corrected Result Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `critical_value_flag` SET TAGS ('dbx_business_glossary_term' = 'Critical Value Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `device_type` SET TAGS ('dbx_business_glossary_term' = 'Device Type');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `ehr_transmission_datetime` SET TAGS ('dbx_business_glossary_term' = 'Electronic Health Record (EHR) Transmission Date and Time');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `ehr_transmission_status` SET TAGS ('dbx_business_glossary_term' = 'Electronic Health Record (EHR) Transmission Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `ehr_transmission_status` SET TAGS ('dbx_value_regex' = 'transmitted|pending|failed|not_required');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `mrn` SET TAGS ('dbx_business_glossary_term' = 'Medical Record Number (MRN)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `mrn` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `mrn` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `operator_competency_date` SET TAGS ('dbx_business_glossary_term' = 'Operator Competency Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `operator_competency_status` SET TAGS ('dbx_business_glossary_term' = 'Operator Competency Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `operator_competency_status` SET TAGS ('dbx_value_regex' = 'competent|training|expired|not_assessed');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `operator_name` SET TAGS ('dbx_business_glossary_term' = 'Operator Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `operator_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `performing_location_name` SET TAGS ('dbx_business_glossary_term' = 'Performing Location Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `qc_datetime` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Date and Time');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `qc_lot_number` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Lot Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `qc_status` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `qc_status` SET TAGS ('dbx_value_regex' = 'passed|failed|not_performed|pending');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `reference_range_high` SET TAGS ('dbx_business_glossary_term' = 'Reference Range High');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `reference_range_low` SET TAGS ('dbx_business_glossary_term' = 'Reference Range Low');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `result_comment` SET TAGS ('dbx_business_glossary_term' = 'Result Comment');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `result_datetime` SET TAGS ('dbx_business_glossary_term' = 'Result Date and Time');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `result_numeric` SET TAGS ('dbx_business_glossary_term' = 'Result Numeric Value');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `result_unit` SET TAGS ('dbx_business_glossary_term' = 'Result Unit of Measure');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `result_value` SET TAGS ('dbx_business_glossary_term' = 'Result Value');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `specimen_source` SET TAGS ('dbx_business_glossary_term' = 'Specimen Source');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `specimen_type` SET TAGS ('dbx_business_glossary_term' = 'Specimen Type');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `test_category` SET TAGS ('dbx_business_glossary_term' = 'Test Category');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `test_datetime` SET TAGS ('dbx_business_glossary_term' = 'Test Date and Time');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `test_status` SET TAGS ('dbx_business_glossary_term' = 'Test Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `test_status` SET TAGS ('dbx_value_regex' = 'preliminary|final|corrected|cancelled|entered_in_error');
ALTER TABLE `healthcare_ecm`.`laboratory`.`point_of_care_test` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` SET TAGS ('dbx_subdomain' = 'quality_operations');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_run_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Run Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `accreditation_survey_id` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Survey Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Instrument Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Performing Technologist Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `training_id` SET TAGS ('dbx_business_glossary_term' = 'Training Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `comments` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Comments');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `corrective_action_taken` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Taken');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `corrective_action_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Corrective Action Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `expected_mean` SET TAGS ('dbx_business_glossary_term' = 'Expected Mean Value');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `expected_standard_deviation` SET TAGS ('dbx_business_glossary_term' = 'Expected Standard Deviation (SD)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `observed_result` SET TAGS ('dbx_business_glossary_term' = 'Observed Quality Control (QC) Result');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pass_fail_indicator` SET TAGS ('dbx_business_glossary_term' = 'Pass or Fail Indicator');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_attestation_date` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Attestation Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_corrective_action_plan` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Corrective Action Plan');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_event_code` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Event Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_graded_result` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Graded Result');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_graded_result` SET TAGS ('dbx_value_regex' = 'acceptable|unacceptable|pending|not_graded');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_peer_group_mean` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Peer Group Mean');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_peer_group_standard_deviation` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Peer Group Standard Deviation (SD)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_program_name` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Program Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_sample_number` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Sample Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_submitted_result` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Submitted Result');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `pt_z_score` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing (PT) Z-Score');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_level` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Level');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_material_lot_number` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Material Lot Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_run_status` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Run Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_run_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Run Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_type` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Type');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `qc_type` SET TAGS ('dbx_value_regex' = 'internal_qc|proficiency_testing|reagent_lot_validation|calibration_verification|instrument_maintenance_qc|competency_assessment');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `reagent_storage_temperature` SET TAGS ('dbx_business_glossary_term' = 'Reagent Storage Temperature');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `result_unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Result Unit of Measure');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `reviewed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Reviewed Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `test_code` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Test Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`qc_run` ALTER COLUMN `westgard_rule_evaluation` SET TAGS ('dbx_business_glossary_term' = 'Westgard Rule Evaluation');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` SET TAGS ('dbx_subdomain' = 'quality_operations');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `accreditation_survey_id` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Survey Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `clia_certificate_id` SET TAGS ('dbx_business_glossary_term' = 'Clia Certificate Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Technician ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `fixed_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Fixed Asset Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `osha_safety_program_id` SET TAGS ('dbx_business_glossary_term' = 'Osha Safety Program Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `training_id` SET TAGS ('dbx_business_glossary_term' = 'Training Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument` ALTER COLUMN `service_contract_number` SET TAGS ('dbx_business_glossary_term' = 'Service Contract Number');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `clia_certificate_id` SET TAGS ('dbx_business_glossary_term' = 'Clia Certificate Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `compliance_program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `monitoring_activity_id` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Activity Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Preferred Instrument Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `measure_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Measure Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `regulatory_change_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change Id (Foreign Key)');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_catalog` ALTER COLUMN `performing_lab_location` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Location');
...
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` SET TAGS ('dbx_subdomain' = 'revenue_management');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `lab_charge_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Charge Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `billing_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Performing Facility Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_id` SET TAGS ('dbx_business_glossary_term' = 'Charge Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `chart_of_accounts_id` SET TAGS ('dbx_business_glossary_term' = 'Chart Of Accounts Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `cpt_code_id` SET TAGS ('dbx_business_glossary_term' = 'Cpt Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `demographics_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `lab_order_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `measure_result_id` SET TAGS ('dbx_business_glossary_term' = 'Measure Result Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `specimen_id` SET TAGS ('dbx_business_glossary_term' = 'Specimen Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Test Catalog Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `test_result_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Result Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Visit Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `billing_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Billing Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `billing_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `cdm_code` SET TAGS ('dbx_business_glossary_term' = 'Charge Description Master (CDM) Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Charge Created Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_entry_method` SET TAGS ('dbx_business_glossary_term' = 'Charge Entry Method');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_entry_method` SET TAGS ('dbx_value_regex' = 'automatic|manual|interface|batch');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_submitted_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Charge Submitted Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Charge Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_voided_by` SET TAGS ('dbx_business_glossary_term' = 'Charge Voided By User');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_voided_reason` SET TAGS ('dbx_business_glossary_term' = 'Charge Voided Reason');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `charge_voided_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Charge Voided Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_1` SET TAGS ('dbx_business_glossary_term' = 'Primary Diagnosis Code (ICD-10)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_1` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_1` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_2` SET TAGS ('dbx_business_glossary_term' = 'Secondary Diagnosis Code (ICD-10)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_2` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_2` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_3` SET TAGS ('dbx_business_glossary_term' = 'Tertiary Diagnosis Code (ICD-10)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_3` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_3` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_4` SET TAGS ('dbx_business_glossary_term' = 'Quaternary Diagnosis Code (ICD-10)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_4` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `diagnosis_code_4` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `insurance_authorization_number` SET TAGS ('dbx_business_glossary_term' = 'Insurance Authorization Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `ordering_provider_name` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Ordering Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `ordering_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `performing_lab_section` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Section');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `performing_provider_npi` SET TAGS ('dbx_business_glossary_term' = 'Performing Provider National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `performing_provider_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `point_of_care_indicator` SET TAGS ('dbx_business_glossary_term' = 'Point of Care (POC) Testing Indicator');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `reference_lab_indicator` SET TAGS ('dbx_business_glossary_term' = 'Reference Laboratory Indicator');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `reference_lab_name` SET TAGS ('dbx_business_glossary_term' = 'Reference Laboratory Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `service_location_code` SET TAGS ('dbx_business_glossary_term' = 'Service Location Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_charge` ALTER COLUMN `stat_surcharge_amount` SET TAGS ('dbx_business_glossary_term' = 'STAT Surcharge Amount');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` SET TAGS ('dbx_subdomain' = 'quality_operations');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `clia_certificate_id` SET TAGS ('dbx_business_glossary_term' = 'Clinical Laboratory Improvement Amendments (CLIA) Certificate ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `accreditation_program_id` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Program Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `accreditation_status_id` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Status Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `care_site_id` SET TAGS ('dbx_business_glossary_term' = 'Facility ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `cms_condition_status_id` SET TAGS ('dbx_business_glossary_term' = 'Cms Condition Status Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `regulatory_change_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `trading_partner_id` SET TAGS ('dbx_business_glossary_term' = 'Trading Partner Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `accrediting_organization` SET TAGS ('dbx_business_glossary_term' = 'Accrediting Organization');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `annual_fee_amount` SET TAGS ('dbx_business_glossary_term' = 'Annual CLIA Fee Amount');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `annual_fee_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `annual_test_volume` SET TAGS ('dbx_business_glossary_term' = 'Annual Test Volume');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `application_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Application Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `certificate_number` SET TAGS ('dbx_business_glossary_term' = 'CLIA Certificate Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `certificate_number` SET TAGS ('dbx_value_regex' = '^[0-9]{2}D[0-9]{7}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `certificate_status` SET TAGS ('dbx_business_glossary_term' = 'Certificate Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `certificate_status` SET TAGS ('dbx_value_regex' = 'active|expired|suspended|revoked|pending_renewal|inactive');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `certificate_type` SET TAGS ('dbx_business_glossary_term' = 'CLIA Certificate Type');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `certificate_type` SET TAGS ('dbx_value_regex' = 'certificate_of_waiver|provider_performed_microscopy|certificate_of_registration|certificate_of_compliance|certificate_of_accreditation');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `deficiency_count` SET TAGS ('dbx_business_glossary_term' = 'Deficiency Count');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Effective Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Expiration Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `fee_payment_status` SET TAGS ('dbx_business_glossary_term' = 'Fee Payment Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `fee_payment_status` SET TAGS ('dbx_value_regex' = 'current|overdue|delinquent|waived|not_applicable');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `fee_schedule_category` SET TAGS ('dbx_business_glossary_term' = 'Fee Schedule Category');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `inspection_outcome` SET TAGS ('dbx_business_glossary_term' = 'Last Inspection Outcome');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `inspection_outcome` SET TAGS ('dbx_value_regex' = 'compliant|deficiencies_cited|conditional|not_applicable');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `issuing_agency` SET TAGS ('dbx_business_glossary_term' = 'Issuing Agency Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `issuing_state` SET TAGS ('dbx_business_glossary_term' = 'Issuing State Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `issuing_state` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_license_number` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Director License Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_license_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_license_number` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_license_state` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Director License State');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_license_state` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_name` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Director Full Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_npi` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Director National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_npi` SET TAGS ('dbx_value_regex' = '^[0-9]{10}$');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_npi` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_director_npi` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `laboratory_type` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Type');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `last_fee_payment_date` SET TAGS ('dbx_business_glossary_term' = 'Last Fee Payment Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `last_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Last Inspection Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `last_proficiency_testing_date` SET TAGS ('dbx_business_glossary_term' = 'Last Proficiency Testing Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `next_inspection_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Inspection Due Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Certificate Notes');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `plan_of_correction_due_date` SET TAGS ('dbx_business_glossary_term' = 'Plan of Correction Due Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `plan_of_correction_status` SET TAGS ('dbx_business_glossary_term' = 'Plan of Correction Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `plan_of_correction_status` SET TAGS ('dbx_value_regex' = 'not_required|pending|submitted|approved|rejected|overdue');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `proficiency_testing_enrollment` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing Enrollment Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `proficiency_testing_outcome` SET TAGS ('dbx_business_glossary_term' = 'Last Proficiency Testing Outcome');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `proficiency_testing_outcome` SET TAGS ('dbx_value_regex' = 'successful|unsuccessful|pending|not_applicable');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `proficiency_testing_provider` SET TAGS ('dbx_business_glossary_term' = 'Proficiency Testing Provider Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `renewal_date` SET TAGS ('dbx_business_glossary_term' = 'Certificate Renewal Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `renewal_status` SET TAGS ('dbx_business_glossary_term' = 'Certificate Renewal Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `renewal_status` SET TAGS ('dbx_value_regex' = 'not_due|pending|submitted|approved|denied|overdue');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `sanction_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Sanction Effective Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `sanction_lifted_date` SET TAGS ('dbx_business_glossary_term' = 'Sanction Lifted Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `sanction_type` SET TAGS ('dbx_business_glossary_term' = 'Sanction Type Description');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `sanctions_imposed` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Imposed Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `specialty_codes` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Specialty Codes');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `testing_complexity_level` SET TAGS ('dbx_business_glossary_term' = 'Testing Complexity Level');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `testing_complexity_level` SET TAGS ('dbx_value_regex' = 'waived|moderate|high|provider_performed_microscopy');
ALTER TABLE `healthcare_ecm`.`laboratory`.`clia_certificate` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `molecular_test_id` SET TAGS ('dbx_business_glossary_term' = 'Molecular Test Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `cda_document_id` SET TAGS ('dbx_business_glossary_term' = 'Cda Document Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `claim_id` SET TAGS ('dbx_business_glossary_term' = 'Claim Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `demographics_id` SET TAGS ('dbx_business_glossary_term' = 'Patient Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `genetic_testing_consent_id` SET TAGS ('dbx_business_glossary_term' = 'Genetic Testing Consent Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Health Plan Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `health_plan_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Instrument Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `lab_order_id` SET TAGS ('dbx_business_glossary_term' = 'Laboratory (Lab) Order Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `loinc_code_id` SET TAGS ('dbx_business_glossary_term' = 'Loinc Code Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `payer_id` SET TAGS ('dbx_business_glossary_term' = 'Payer Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Research Study Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `specimen_id` SET TAGS ('dbx_business_glossary_term' = 'Specimen Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Test Catalog Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `snomed_concept_id` SET TAGS ('dbx_business_glossary_term' = 'Variant Snomed Concept Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `visit_id` SET TAGS ('dbx_business_glossary_term' = 'Encounter Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `accession_number` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Accession Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `allele_frequency` SET TAGS ('dbx_business_glossary_term' = 'Variant Allele Frequency (VAF)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `amended` SET TAGS ('dbx_business_glossary_term' = 'Amended Result Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `amendment_reason` SET TAGS ('dbx_business_glossary_term' = 'Amendment Reason');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `amendment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Amendment Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `associated_drug` SET TAGS ('dbx_business_glossary_term' = 'Associated Drug or Therapy');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `bioinformatics_pipeline_version` SET TAGS ('dbx_business_glossary_term' = 'Bioinformatics Pipeline Version');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `clinical_indication` SET TAGS ('dbx_business_glossary_term' = 'Clinical Indication');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `companion_diagnostic` SET TAGS ('dbx_business_glossary_term' = 'Companion Diagnostic Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `copy_number_variation` SET TAGS ('dbx_business_glossary_term' = 'Copy Number Variation (CNV)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `coverage_percentage` SET TAGS ('dbx_business_glossary_term' = 'Target Coverage Percentage');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `critical_value` SET TAGS ('dbx_business_glossary_term' = 'Critical Value Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `critical_value_notified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Critical Value Notification Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `diagnosis_code` SET TAGS ('dbx_business_glossary_term' = 'Diagnosis Code (ICD-10)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `diagnosis_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `diagnosis_code` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `fda_cleared` SET TAGS ('dbx_business_glossary_term' = 'FDA (Food and Drug Administration) Cleared Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `laboratory_developed_test` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Developed Test (LDT) Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `medical_director_name` SET TAGS ('dbx_business_glossary_term' = 'Laboratory Medical Director Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `medical_director_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `medical_director_name` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `medical_director_npi` SET TAGS ('dbx_business_glossary_term' = 'Medical Director National Provider Identifier (NPI)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `medical_director_npi` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `medical_director_npi` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `methodology` SET TAGS ('dbx_business_glossary_term' = 'Testing Methodology');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `performing_lab_clia_number` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory CLIA (Clinical Laboratory Improvement Amendments) Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `performing_lab_name` SET TAGS ('dbx_business_glossary_term' = 'Performing Laboratory Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `quality_score` SET TAGS ('dbx_business_glossary_term' = 'Variant Quality Score');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `read_depth` SET TAGS ('dbx_business_glossary_term' = 'Sequencing Read Depth');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `reference_genome` SET TAGS ('dbx_business_glossary_term' = 'Reference Genome Build');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `reference_genome` SET TAGS ('dbx_value_regex' = 'GRCh37|GRCh38|hg19|hg38');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `report_narrative` SET TAGS ('dbx_business_glossary_term' = 'Molecular Test Report Narrative');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `result_interpretation` SET TAGS ('dbx_business_glossary_term' = 'Result Interpretation');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `result_interpretation` SET TAGS ('dbx_value_regex' = 'detected|not_detected|positive|negative|indeterminate|inconclusive');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `result_reported_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Result Reported Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `specimen_received_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Specimen Received Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `target_gene` SET TAGS ('dbx_business_glossary_term' = 'Target Gene or Pathogen');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_category` SET TAGS ('dbx_business_glossary_term' = 'Molecular Test Category');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_category` SET TAGS ('dbx_value_regex' = 'oncology|infectious_disease|pharmacogenomics|hereditary|prenatal|hematology');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_performed_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Test Performed Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_priority` SET TAGS ('dbx_business_glossary_term' = 'Test Priority Level');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_priority` SET TAGS ('dbx_value_regex' = 'routine|urgent|stat|asap');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_status` SET TAGS ('dbx_business_glossary_term' = 'Molecular Test Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_type` SET TAGS ('dbx_business_glossary_term' = 'Molecular Test Type');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `test_type` SET TAGS ('dbx_value_regex' = 'diagnostic|screening|confirmatory|monitoring|companion_diagnostic|research');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `therapeutic_implications` SET TAGS ('dbx_business_glossary_term' = 'Therapeutic Implications');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `turnaround_time_hours` SET TAGS ('dbx_business_glossary_term' = 'Turnaround Time (Hours)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `variant_classification` SET TAGS ('dbx_business_glossary_term' = 'Variant Classification (ACMG)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `variant_classification` SET TAGS ('dbx_value_regex' = 'pathogenic|likely_pathogenic|VUS|likely_benign|benign');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `variant_detected` SET TAGS ('dbx_business_glossary_term' = 'Variant Detected Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`molecular_test` ALTER COLUMN `variant_nomenclature` SET TAGS ('dbx_business_glossary_term' = 'Variant Nomenclature (HGVS)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` SET TAGS ('dbx_subdomain' = 'quality_operations');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `reagent_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Reagent Lot Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Instrument Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `ndc_drug_id` SET TAGS ('dbx_business_glossary_term' = 'Ndc Drug Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `osha_safety_program_id` SET TAGS ('dbx_business_glossary_term' = 'Osha Safety Program Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Validated By Technologist Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `room_id` SET TAGS ('dbx_business_glossary_term' = 'Storage Room Id (Foreign Key)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor Identifier (ID)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `cost_per_unit` SET TAGS ('dbx_business_glossary_term' = 'Cost Per Unit');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `cost_per_unit` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `disposal_date` SET TAGS ('dbx_business_glossary_term' = 'Disposal Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `disposal_method` SET TAGS ('dbx_business_glossary_term' = 'Disposal Method');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `fda_clearance_number` SET TAGS ('dbx_business_glossary_term' = 'Food and Drug Administration (FDA) Clearance Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `fda_cleared_flag` SET TAGS ('dbx_business_glossary_term' = 'Food and Drug Administration (FDA) Cleared Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `hazardous_material_flag` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Material Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `in_use_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'In-Use Expiration Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `in_use_stability_days` SET TAGS ('dbx_business_glossary_term' = 'In-Use Stability Days');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `light_sensitivity_flag` SET TAGS ('dbx_business_glossary_term' = 'Light Sensitivity Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `lot_number` SET TAGS ('dbx_business_glossary_term' = 'Lot Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `lot_status` SET TAGS ('dbx_business_glossary_term' = 'Lot Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `lot_status` SET TAGS ('dbx_value_regex' = 'unopened|in_use|depleted|expired|quarantined|rejected');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `opened_date` SET TAGS ('dbx_business_glossary_term' = 'Opened Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `purchase_order_number` SET TAGS ('dbx_business_glossary_term' = 'Purchase Order (PO) Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `qc_validation_date` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Validation Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `qc_validation_status` SET TAGS ('dbx_business_glossary_term' = 'Quality Control (QC) Validation Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `qc_validation_status` SET TAGS ('dbx_value_regex' = 'passed|failed|pending|not_required');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `quantity_on_hand` SET TAGS ('dbx_business_glossary_term' = 'Quantity On Hand');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `quantity_received` SET TAGS ('dbx_business_glossary_term' = 'Quantity Received');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `quarantine_date` SET TAGS ('dbx_business_glossary_term' = 'Quarantine Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `quarantine_reason` SET TAGS ('dbx_business_glossary_term' = 'Quarantine Reason');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `receipt_date` SET TAGS ('dbx_business_glossary_term' = 'Receipt Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `reconstitution_date` SET TAGS ('dbx_business_glossary_term' = 'Reconstitution Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `reconstitution_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Reconstitution Required Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `reorder_threshold` SET TAGS ('dbx_business_glossary_term' = 'Reorder Threshold');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `safety_data_sheet_url` SET TAGS ('dbx_business_glossary_term' = 'Safety Data Sheet (SDS) Uniform Resource Locator (URL)');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `storage_temperature_requirement` SET TAGS ('dbx_business_glossary_term' = 'Storage Temperature Requirement');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `test_method_code` SET TAGS ('dbx_business_glossary_term' = 'Test Method Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `test_method_name` SET TAGS ('dbx_business_glossary_term' = 'Test Method Name');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `total_lot_cost` SET TAGS ('dbx_business_glossary_term' = 'Total Lot Cost');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `total_lot_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`reagent_lot` ALTER COLUMN `unit_of_measure` SET TAGS ('dbx_business_glossary_term' = 'Unit of Measure');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` SET TAGS ('dbx_association_edges' = 'laboratory.test_catalog,insurance.coverage_policy');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `test_coverage_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Test Coverage Policy Identifier');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `coverage_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Test Coverage Policy - Coverage Policy Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Test Coverage Policy - Test Catalog Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `age_restrictions` SET TAGS ('dbx_business_glossary_term' = 'Age Restrictions');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `copay_amount` SET TAGS ('dbx_business_glossary_term' = 'Copay Amount');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `coverage_determination` SET TAGS ('dbx_business_glossary_term' = 'Coverage Determination');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `coverage_notes` SET TAGS ('dbx_business_glossary_term' = 'Coverage Notes');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `coverage_percentage` SET TAGS ('dbx_business_glossary_term' = 'Coverage Percentage');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `diagnosis_code_requirements` SET TAGS ('dbx_business_glossary_term' = 'Diagnosis Code Requirements');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `diagnosis_code_requirements` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `diagnosis_code_requirements` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Effective Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `frequency_limitations` SET TAGS ('dbx_business_glossary_term' = 'Frequency Limitations');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `last_updated_date` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `medical_necessity_criteria` SET TAGS ('dbx_business_glossary_term' = 'Medical Necessity Criteria');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `medical_necessity_criteria` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `medical_necessity_criteria` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `place_of_service_restrictions` SET TAGS ('dbx_business_glossary_term' = 'Place of Service Restrictions');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `prior_authorization_required` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Required Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`test_coverage_policy` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Termination Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` SET TAGS ('dbx_subdomain' = 'diagnostic_testing');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` SET TAGS ('dbx_association_edges' = 'laboratory.test_catalog,research.research_study');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `study_test_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Study Test Requirement ID');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `research_study_id` SET TAGS ('dbx_business_glossary_term' = 'Study Test Requirement - Research Study Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Study Test Requirement - Test Catalog Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `collection_instructions` SET TAGS ('dbx_business_glossary_term' = 'Collection Instructions');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `collection_timepoint` SET TAGS ('dbx_business_glossary_term' = 'Collection Timepoint');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `end_date` SET TAGS ('dbx_business_glossary_term' = 'End Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `expected_frequency` SET TAGS ('dbx_business_glossary_term' = 'Expected Frequency');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `protocol_amendment_number` SET TAGS ('dbx_business_glossary_term' = 'Protocol Amendment Number');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `protocol_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Protocol Required Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `requirement_status` SET TAGS ('dbx_business_glossary_term' = 'Requirement Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `sponsor_covered_flag` SET TAGS ('dbx_business_glossary_term' = 'Sponsor Covered Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `standard_of_care_flag` SET TAGS ('dbx_business_glossary_term' = 'Standard of Care Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`study_test_requirement` ALTER COLUMN `visit_schedule` SET TAGS ('dbx_business_glossary_term' = 'Visit Schedule');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` SET TAGS ('dbx_subdomain' = 'revenue_management');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` SET TAGS ('dbx_association_edges' = 'laboratory.test_catalog,insurance.fee_schedule');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `lab_fee_schedule_line_id` SET TAGS ('dbx_business_glossary_term' = 'Lab Fee Schedule Line Identifier');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `fee_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Lab Fee Schedule Line - Fee Schedule Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `test_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Lab Fee Schedule Line - Test Catalog Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `authorization_required` SET TAGS ('dbx_business_glossary_term' = 'Prior Authorization Required Flag');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `bundling_indicator` SET TAGS ('dbx_business_glossary_term' = 'Bundling Payment Indicator');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `contracted_rate_amount` SET TAGS ('dbx_business_glossary_term' = 'Contracted Rate Amount');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `coverage_limitation` SET TAGS ('dbx_business_glossary_term' = 'Coverage Limitation Description');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Line Item Effective Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `lab_fee_schedule_line_status` SET TAGS ('dbx_business_glossary_term' = 'Line Item Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `medical_necessity_codes` SET TAGS ('dbx_business_glossary_term' = 'Medical Necessity Diagnosis Codes');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `medical_necessity_codes` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `medical_necessity_codes` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `modifier_codes` SET TAGS ('dbx_business_glossary_term' = 'Required Billing Modifiers');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `place_of_service_code` SET TAGS ('dbx_business_glossary_term' = 'Place of Service Code');
ALTER TABLE `healthcare_ecm`.`laboratory`.`lab_fee_schedule_line` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Line Item Termination Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` SET TAGS ('dbx_subdomain' = 'quality_operations');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` SET TAGS ('dbx_association_edges' = 'laboratory.instrument,compliance.policy');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `instrument_policy_compliance_id` SET TAGS ('dbx_business_glossary_term' = 'Instrument Policy Compliance Identifier');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assessor Employee Identifier');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `compliance_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Instrument Policy Compliance - Policy Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `instrument_id` SET TAGS ('dbx_business_glossary_term' = 'Instrument Policy Compliance - Instrument Id');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `attestation_status` SET TAGS ('dbx_business_glossary_term' = 'Attestation Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Policy Effective Date for Instrument');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `last_assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Last Compliance Assessment Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Compliance Review Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `non_compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Non-Compliance Notes');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `waiver_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Waiver Expiration Date');
ALTER TABLE `healthcare_ecm`.`laboratory`.`instrument_policy_compliance` ALTER COLUMN `waiver_justification` SET TAGS ('dbx_business_glossary_term' = 'Waiver Justification');
ALTER TABLE `healthcare_ecm`.`laboratory`.`organism` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `healthcare_ecm`.`laboratory`.`organism` SET TAGS ('dbx_subdomain' = 'quality_operations');
