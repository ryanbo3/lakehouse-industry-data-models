-- Schema for Domain: grant | Business: Ngo | Version: v1_mvm
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`award` (
    ...
    `emergency_id` BIGINT COMMENT 'Foreign key linking to field.emergency. Business justification: Awards are issued in direct response to declared emergencies (CERF, flash appeals, BHA emergency grants). Linking award to emergency is essential for emergency portfolio tracking, donor accountability',
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Awards must be associated with the active fiscal period for period-end accruals, exchange rate application, and donor report generation. Finance teams use this link during period-end close to identify',
    `funding_source_id` BIGINT COMMENT 'Foreign key linking to grant.funding_source. Business justification: An award is funded by a specific funding source (e.g., USAID cooperative agreement, BHA grant mechanism). The funding_source table is the reference master for all donor funding mechanisms. Adding fund',
    ...
    `program_id` BIGINT COMMENT 'Foreign key linking to program.program. Business justification: NGO grant portfolio management: awards fund programs, and program-level grant portfolio reporting requires direct award-to-program linkage. NGO finance directors need to aggregate all awards funding a',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`proposal` (
    ...
    `budget_id` BIGINT COMMENT 'Foreign key linking to finance.budget. Business justification: During proposal development, a preliminary budget is created in the finance system for cost proposal validation and indirect cost rate application. Business development officers reference this budget ',
    ...
    `partner_org_id` BIGINT COMMENT 'Foreign key linking to partnership.partner_org. Business justification: Proposals are frequently developed with a consortium lead partner. This FK replaces the denormalized consortium_lead_organization text field with a proper reference to partner_org, enabling business d',
    ...
    `emergency_id` BIGINT COMMENT 'Foreign key linking to field.emergency. Business justification: Proposals are written in response to specific declared emergencies. Linking proposal to emergency supports emergency response pipeline tracking, go/no-go decisions tied to emergency severity, and dono',
    `funding_source_id` BIGINT COMMENT 'Foreign key linking to grant.funding_source. Business justification: A proposal is submitted in response to a specific funding opportunity from a funding source (e.g., a USAID NOFO, BHA solicitation). Linking proposal to funding_source allows tracking which funding mec',
    ...
    `staff_member_id` BIGINT COMMENT 'Foreign key linking to workforce.staff_member. Business justification: BD pipeline management and win/loss analysis require linking proposals to the staff member who led writing. Workload reporting, capacity planning, and staff performance reviews reference proposal auth',
    `logframe_id` BIGINT COMMENT 'Foreign key linking to mel.mel_logframe. Business justification: Proposals reference donor-mandated logframes during design phase. Proposal writers must align technical approach with donors results framework and indicator requirements specified in RFP logframe tem',
    `prospect_id` BIGINT COMMENT 'Foreign key linking to donor.prospect. Business justification: NGO business development pipeline tracks proposals against prospect records. Win/loss analysis by prospect cultivation stage, proposal conversion rates by prospect rating, and solicitation amount accu',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`award_budget` (
    ...
    `staff_member_id` BIGINT COMMENT 'Foreign key linking to workforce.staff_member. Business justification: Award budget approval requires a traceable internal approver for audit trails, segregation-of-duties compliance, and donor reporting. approved_by is a plain-text denormalized staff reference. Linkin',
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Award budgets are structured by budget period aligned to fiscal periods. Finance teams match award budget periods to fiscal periods for period-end budget vs. actual reporting, exchange rate applicatio',
    ...
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: The donor-facing award_budget and internal finance grant_budget represent two sides of the same grant financial plan. Finance teams reconcile these during budget revisions and closeout. Linking them e',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`award_budget_line` (
    ...
    `commodity_id` BIGINT COMMENT 'Foreign key linking to supply.commodity. Business justification: Award budget lines for commodity procurement are tied to specific commodity types for donor budget compliance and pipeline planning. NGO supply chain and finance teams track which commodities are budg',
    ...
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: Award budget lines (donor-approved line items) must reconcile to the internal grant_budget. This link enables variance analysis between donor-approved amounts and internal budget tracking, supporting ',
    ...
    `project_site_id` BIGINT COMMENT 'Foreign key linking to field.project_site. Business justification: Budget lines are allocated to specific project sites for implementation. Site-level budget tracking is required for donor financial reporting, audit compliance (NICRA, single audit), and operational c',
    `reporting_period_id` BIGINT COMMENT 'Foreign key linking to mel.reporting_period. Business justification: Integrated financial-results reporting requires matching budget line expenditures to MEL reporting periods. NGO finance and MEL teams produce period-based burn rate vs. results achieved reports — a co',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`grant_amendment` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to partnership.partnership_agreement. Business justification: Grant amendments that change scope, budget, or timeline require corresponding updates to partnership agreements. This FK identifies which partnership agreement must be amended when a grant amendment i',
    ...
    `constituent_id` BIGINT COMMENT 'Foreign key linking to donor.constituent. Business justification: Amendments require donor prior approval and are communicated directly to the donor constituent. NGO compliance teams run all amendments pending approval from donor X reports. The existing approved_b',
    `donor_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.donor_requirement. Business justification: Grant amendments (scope changes, no-cost extensions, budget modifications) frequently trigger new or modified donor requirements — e.g., prior approval requirements, revised reporting obligations. Lin',
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: Grant amendments modify budget amounts, period of performance, and scope. The grant_budget must be updated when an amendment is approved. This link enables amendment-driven budget revision tracking, a',
    `logframe_id` BIGINT COMMENT 'Foreign key linking to mel.mel_logframe. Business justification: Amendments revise logframes when scope, geography, or results chain changes. Formal amendments update intervention logic, assumptions, and indicator hierarchies in donor-approved logframe, requiring v',
    `staff_member_id` BIGINT COMMENT 'Foreign key linking to workforce.staff_member. Business justification: Grant amendment preparation is a named compliance workflow requiring staff accountability for audit trails. Amendments affecting budget, scope, or timeline must be traceable to the preparing staff mem',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`subaward` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to partnership.partnership_agreement. Business justification: Every subaward must be executed under a governing partnership agreement. This FK enables compliance officers to verify each subaward has a corresponding legal agreement — a mandatory audit requirement',
    ...
    `budget_line_id` BIGINT COMMENT 'Foreign key linking to finance.budget_line. Business justification: Subaward costs are charged to specific budget lines. Finance tracks subaward expenditures at the line-item level for donor reporting, allowability verification, and budget line burn rate monitoring. A',
    `component_id` BIGINT COMMENT 'Foreign key linking to program.component. Business justification: NGO sub-award management: subawards implement specific program components, and component-level sub-award tracking is required for donor financial reporting and component budget monitoring. Grant manag',
    `emergency_id` BIGINT COMMENT 'Foreign key linking to field.emergency. Business justification: Subawards to implementing partners are frequently issued for emergency response operations. Linking subaward to emergency supports emergency-specific partner portfolio management, FSRS/FFATA complianc',
    `finance_fund_id` BIGINT COMMENT 'Foreign key linking to finance.finance_fund. Business justification: Each subaward is funded from a specific restricted fund. Finance tracks fund-level drawdowns per subaward for restriction compliance, fund balance reporting, and single audit requirements. A nonprofit',
    `grant_amendment_id` BIGINT COMMENT 'Foreign key linking to grant.grant_amendment. Business justification: A subaward may be created or modified as a direct result of a grant amendment (e.g., a no-cost extension amendment extends the subaward period, or a budget modification amendment changes the subaward ',
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: Subaward disbursements are drawn from a specific grant budget. Finance teams monitor subaward spend against the grant_budget for burn rate analysis, remaining obligation tracking, and donor financial ',
    ...
    `staff_member_id` BIGINT COMMENT 'Foreign key linking to workforce.staff_member. Business justification: Subaward management requires a designated NGO staff member responsible for partner monitoring, site visits, and compliance oversight. Subaward monitoring reports and FFATA/FSRS filings reference the r',
    ...
    `single_audit_id` BIGINT COMMENT 'Foreign key linking to compliance.single_audit. Business justification: 2 CFR 200 Uniform Guidance requires NGOs to track which subawards are covered by or triggered a Single Audit. The subaward already has `single_audit_required_flag`, confirming this business relationsh',
    `warehouse_id` BIGINT COMMENT 'Foreign key linking to supply.warehouse. Business justification: Subawards to partner organizations designate specific warehouses for the partners commodity storage and distribution operations. NGO supply chain managers track subaward warehouse assignments for par',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`donor_condition` (
    ...
    `grant_amendment_id` BIGINT COMMENT 'Foreign key linking to grant.grant_amendment. Business justification: Donor conditions (Special Award Conditions / SACs) are frequently introduced or modified through grant amendments. Linking donor_condition to the grant_amendment that created or last modified it provi',
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: Donor conditions include financial thresholds, prior approval requirements, and budget line restrictions. Compliance officers verify financial conditions against the active grant_budget. This link ena',
    ...
    `indicator_target_id` BIGINT COMMENT 'Foreign key linking to mel.indicator_target. Business justification: Donor special award conditions frequently require achieving a specific indicator target value by a deadline (e.g., 80% of target X by Year 2). Linking donor_condition to indicator_target enables com',
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Donor-imposed special award conditions (SACs) directly correspond to underlying compliance obligations (regulatory, statutory, or policy-based). NGO compliance officers map each donor_condition to the',
    `org_unit_id` BIGINT COMMENT 'Foreign key linking to workforce.org_unit. Business justification: Donor compliance condition tracking requires linking each condition to the responsible organizational unit for escalation, workload management, and compliance dashboards. responsible_department is a',
    `partner_org_id` BIGINT COMMENT 'Foreign key linking to partnership.partner_org. Business justification: Donor conditions frequently assign compliance obligations to specific implementing partners (e.g., mandatory training before fund release, safeguarding certification). This FK enables compliance manag',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`donor_report` (
    ...
    `constituent_id` BIGINT COMMENT 'Foreign key linking to donor.constituent. Business justification: Donor reports are submitted to a specific donor/constituent for stewardship and compliance. NGOs must track all reports sent to donor X for relationship management and audit trails. The existing awa',
    `country_office_id` BIGINT COMMENT 'Foreign key linking to field.country_office. Business justification: Donor reports are submitted by and attributed to specific country offices. Country office-level donor report portfolio management is essential for compliance monitoring, country director oversight, an',
    `donor_fund_id` BIGINT COMMENT 'Foreign key linking to donor.donor_fund. Business justification: Restricted fund reporting is a core NGO compliance requirement: donors require reports tied to their specific restricted fund. donor_report currently links to award but not to the specific donor_fund ',
    `emergency_id` BIGINT COMMENT 'Foreign key linking to field.emergency. Business justification: Emergency-specific donor reports (CERF, ECHO, BHA) must reference the declared emergency. Linking donor_report to emergency enables emergency-level reporting compliance tracking, aggregation of all do',
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Donor reports are tied to specific fiscal periods for financial statement alignment. The fiscal_period determines exchange rates applied, which transactions are included, and period-end close status. ',
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: Donor financial reports are prepared against a specific grant budget version. Cumulative expenditure, budget variance, and financial figures reported must reconcile to the grant_budget record. A nonpr',
    ...
    `reporting_period_id` BIGINT COMMENT 'Foreign key linking to mel.reporting_period. Business justification: Donor reports are submitted against specific MEL reporting periods. Linking enables automated submission deadline tracking, period-based results aggregation, and ensures donor reports align with the o',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`funding_source` (
    ...
    `constituent_id` BIGINT COMMENT 'Foreign key linking to donor.constituent. Business justification: A funding source IS a donor constituent entity. NGO donor relationship management and IATI reporting require linking funding sources to constituent records for portfolio analysis, giving history, and ',
...
CREATE OR REPLACE TABLE `ngo_ecm`.`grant`.`cost_share_commitment` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to partnership.partnership_agreement. Business justification: Cost share commitments from implementing partners are formalized within partnership agreements. This FK links each cost share commitment to its governing agreement, enabling financial auditors to veri',
    `staff_member_id` BIGINT COMMENT 'Foreign key linking to workforce.staff_member. Business justification: Cost share commitments require an authorized internal approver for donor compliance and single-audit purposes. approved_by_name is a denormalized staff name. Linking to staff_member enables approval',
    `award_id` BIGINT COMMENT 'Reference to the grant award that requires this cost-share commitment.',
    `budget_line_id` BIGINT COMMENT 'Foreign key linking to finance.budget_line. Business justification: Cost share commitments map to specific budget lines to demonstrate proper categorization and allowability per donor requirements. Auditors verify this mapping during single audits (2 CFR 200.306). Fin',
    `budget_plan_id` BIGINT COMMENT 'Foreign key linking to program.budget_plan. Business justification: NGO cost-share compliance: cost-share commitments must be matched against specific budget plan versions for audit verification. Donors and auditors require evidence that cost-share contributions align',
    `commodity_id` BIGINT COMMENT 'Foreign key linking to supply.commodity. Business justification: In-kind cost share commitments in NGOs frequently involve specific commodities (donated food, NFIs, medicines). Linking to commodity enables in-kind cost share valuation and donor reporting — the co',
    ...
    `donor_requirement_id` BIGINT COMMENT 'Foreign key linking to compliance.donor_requirement. Business justification: Cost share commitments are frequently mandated by specific donor requirements in the award agreement. Linking cost_share_commitment to the originating donor_requirement allows compliance teams to veri',
    `finance_fund_id` BIGINT COMMENT 'Foreign key linking to finance.finance_fund. Business justification: Cost share contributions are sourced from specific restricted or unrestricted funds. Finance tracks which fund provides the cost share to ensure fund-level compliance, proper restriction treatment, an',
    `grant_amendment_id` BIGINT COMMENT 'Foreign key linking to grant.grant_amendment. Business justification: Cost share commitments can be created or revised through grant amendments (e.g., an amendment that changes the cost share percentage or required amount). Linking cost_share_commitment to the authorizi',
    `grant_budget_id` BIGINT COMMENT 'Foreign key linking to finance.grant_budget. Business justification: Cost share commitments are tracked against a specific grant budget that holds the required cost share amount and percentage. Compliance officers verify committed vs. required cost share against the gr',
    `indicator_id` BIGINT COMMENT 'Foreign key linking to mel.indicator. Business justification: Cost share contributions are often tied to specific program indicators to demonstrate that in-kind or cash contributions advance measurable results. Donors require cost-share verification reports show',
    ...
    `project_site_id` BIGINT COMMENT 'Foreign key linking to field.project_site. Business justification: Cost share commitments (in-kind labor, materials, volunteer hours) are made at specific project sites. Site-level cost share tracking is required for donor audit verification, in-kind valuation, and c',
    `reporting_period_id` BIGINT COMMENT 'Foreign key linking to mel.reporting_period. Business justification: Cost share commitments must be verified and reported within official MEL reporting periods. Linking to reporting_period replaces denormalized start/end date columns and aligns cost-share verification ',
    `volunteer_deployment_id` BIGINT COMMENT 'Foreign key linking to volunteer.volunteer_deployment. Business justification: Cost-share commitments sourced from volunteer labor must reference the specific deployment generating the in-kind contribution. cost_share_commitment already has volunteer_hourly_rate and volunteer_ho',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `ngo_ecm`.`grant`.`award` ADD CONSTRAINT `fk_grant_award_funding_source_id` FOREIGN KEY (`funding_source_id`) REFERENCES `ngo_ecm`.`grant`.`funding_source`(`funding_source_id`);
...
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ADD CONSTRAINT `fk_grant_proposal_funding_source_id` FOREIGN KEY (`funding_source_id`) REFERENCES `ngo_ecm`.`grant`.`funding_source`(`funding_source_id`);
...
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ADD CONSTRAINT `fk_grant_subaward_grant_amendment_id` FOREIGN KEY (`grant_amendment_id`) REFERENCES `ngo_ecm`.`grant`.`grant_amendment`(`grant_amendment_id`);
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ADD CONSTRAINT `fk_grant_donor_condition_grant_amendment_id` FOREIGN KEY (`grant_amendment_id`) REFERENCES `ngo_ecm`.`grant`.`grant_amendment`(`grant_amendment_id`);
...
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ADD CONSTRAINT `fk_grant_cost_share_commitment_grant_amendment_id` FOREIGN KEY (`grant_amendment_id`) REFERENCES `ngo_ecm`.`grant`.`grant_amendment`(`grant_amendment_id`);
...
ALTER TABLE `ngo_ecm`.`grant`.`award` ALTER COLUMN `emergency_id` SET TAGS ('dbx_business_glossary_term' = 'Emergency Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`award` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`award` ALTER COLUMN `funding_source_id` SET TAGS ('dbx_business_glossary_term' = 'Funding Source Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`award` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Program Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`proposal` SET TAGS ('dbx_subdomain' = 'award_management');
...
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `budget_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Program Identifier (ID)');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Consortium Lead Partner Org Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `emergency_id` SET TAGS ('dbx_business_glossary_term' = 'Emergency Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `funding_source_id` SET TAGS ('dbx_business_glossary_term' = 'Funding Source Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Lead Proposal Writer Staff Member Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `logframe_id` SET TAGS ('dbx_business_glossary_term' = 'Mel Logframe Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`proposal` ALTER COLUMN `prospect_id` SET TAGS ('dbx_business_glossary_term' = 'Prospect Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`award_budget` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`award_budget` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`award_budget` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `ngo_ecm`.`grant`.`award_budget` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`award_budget` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`award_budget_line` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`award_budget_line` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`award_budget_line` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`award_budget_line` ALTER COLUMN `reporting_period_id` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Identifier (ID)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `constituent_id` SET TAGS ('dbx_business_glossary_term' = 'Constituent Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `donor_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Donor Requirement Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `logframe_id` SET TAGS ('dbx_business_glossary_term' = 'Mel Logframe Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Prepared By Staff Member Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`grant_amendment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `ngo_ecm`.`grant`.`subaward` SET TAGS ('dbx_subdomain' = 'compliance_reporting');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `subaward_id` SET TAGS ('dbx_business_glossary_term' = 'Subaward Identifier (ID)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Identifier (ID)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `budget_line_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Line Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `component_id` SET TAGS ('dbx_business_glossary_term' = 'Component Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `emergency_id` SET TAGS ('dbx_business_glossary_term' = 'Emergency Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `grant_amendment_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Amendment Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Subaward Manager Staff Member Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `single_audit_id` SET TAGS ('dbx_business_glossary_term' = 'Single Audit Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`subaward` ALTER COLUMN `warehouse_id` SET TAGS ('dbx_business_glossary_term' = 'Warehouse Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` SET TAGS ('dbx_subdomain' = 'compliance_reporting');
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ALTER COLUMN `grant_amendment_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Amendment Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ALTER COLUMN `indicator_target_id` SET TAGS ('dbx_business_glossary_term' = 'Indicator Target Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ALTER COLUMN `org_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Org Unit Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_condition` ALTER COLUMN `partner_org_id` SET TAGS ('dbx_business_glossary_term' = 'Partner Org Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` SET TAGS ('dbx_subdomain' = 'compliance_reporting');
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `constituent_id` SET TAGS ('dbx_business_glossary_term' = 'Constituent Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `country_office_id` SET TAGS ('dbx_business_glossary_term' = 'Country Office Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `donor_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Donor Fund Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `emergency_id` SET TAGS ('dbx_business_glossary_term' = 'Emergency Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`donor_report` ALTER COLUMN `reporting_period_id` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`funding_source` SET TAGS ('dbx_subdomain' = 'compliance_reporting');
...
ALTER TABLE `ngo_ecm`.`grant`.`funding_source` ALTER COLUMN `constituent_id` SET TAGS ('dbx_business_glossary_term' = 'Constituent Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` SET TAGS ('dbx_subdomain' = 'compliance_reporting');
...
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Partnership Agreement Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Staff Member Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `staff_member_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `award_id` SET TAGS ('dbx_business_glossary_term' = 'Grant ID');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `budget_line_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Line Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `budget_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Budget Plan Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `commodity_id` SET TAGS ('dbx_business_glossary_term' = 'Commodity Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `donor_requirement_id` SET TAGS ('dbx_business_glossary_term' = 'Donor Requirement Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `finance_fund_id` SET TAGS ('dbx_business_glossary_term' = 'Finance Fund Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `grant_amendment_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Amendment Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `grant_budget_id` SET TAGS ('dbx_business_glossary_term' = 'Grant Budget Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `indicator_id` SET TAGS ('dbx_business_glossary_term' = 'Indicator Id (Foreign Key)');
...
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `project_site_id` SET TAGS ('dbx_business_glossary_term' = 'Project Site Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `reporting_period_id` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Id (Foreign Key)');
ALTER TABLE `ngo_ecm`.`grant`.`cost_share_commitment` ALTER COLUMN `volunteer_deployment_id` SET TAGS ('dbx_business_glossary_term' = 'Volunteer Deployment Id (Foreign Key)');
