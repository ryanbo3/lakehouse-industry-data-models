-- Schema for Domain: distribution | Business: Energy Utilities | Version: v1_mvm
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`feeder` (
    ...
    `ems_node_id` BIGINT COMMENT 'Foreign key linking to grid.ems_node. Business justification: Feeders are modeled as branch elements in the EMS network model with a head-end bus node. State estimation, contingency analysis, and load flow studies require correlating feeder-level distribution da',
    `distribution_substation_id` BIGINT COMMENT 'FK to distribution.substation.substation_id — Every distribution feeder emanates from a substation bus. This is the foundational topological relationship — load flow analysis, fault isolation, and FLISR all require knowing which substation source',
    ...
    `parent_feeder_id` BIGINT COMMENT 'Reference to an alternate feeder that can provide backup service to customers on this feeder through normally-open tie switches.',
    `pnode_id` BIGINT COMMENT 'Foreign key linking to transmission.pnode. Business justification: In RTO/ISO markets, each distribution feeders load is settled at a transmission pricing node (pnode). Distribution planners and settlement teams require this link for LMP-based load forecasting, cong',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Feeders are capital assets requiring asset registry entries for depreciation, FERC accounting, RAB inclusion, and lifecycle management. A utility asset manager would expect every feeder to have a regi',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` (
    ...
    `bes_facility_id` BIGINT COMMENT 'Foreign key linking to compliance.bes_facility. Business justification: Distribution substations meeting NERC BES criteria are registered as BES facilities. NERC CIP compliance scoping requires linking the physical substation to its BES facility registration. A NERC compl',
    ...
    `model_id` BIGINT COMMENT 'Foreign key linking to forecast.model. Business justification: Distribution capacity planning and DER hosting capacity studies require knowing which forecast model governs each substations load projections. Planners and engineers reference this link in annual lo',
    ...
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Distribution substations are major capital assets requiring asset registry entries for depreciation schedules, FERC account coding, RAB inclusion, and valuation for rate cases. Every utility asset man',
    `service_territory_id` BIGINT COMMENT 'Foreign key linking to distribution.service_territory. Business justification: Distribution substations are the primary infrastructure assets within a utilitys franchise service territory. Each substation serves customers within a defined geographic franchise area. Adding servi',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`transformer` (
    ...
    `feeder_id` BIGINT COMMENT 'FK to distribution.feeder.feeder_id — Distribution transformers are assigned to feeders. This FK supports transformer loading studies, voltage complaint investigation, and outage impact assessment (how many transformers/customers affected',
    `load_zone_id` BIGINT COMMENT 'Foreign key linking to distribution.load_zone. Business justification: Distribution transformers serve customers within specific load zones — the load zone determines the voltage regulation targets, VVO optimization parameters, and load balancing objectives that govern t',
    `network_segment_id` BIGINT COMMENT 'Foreign key linking to distribution.network_segment. Business justification: Distribution transformers are physically installed on specific network segments (line sections) of the distribution feeder. The network segment determines the transformers electrical location within ',
    `primary_transformer_feeder_id` BIGINT COMMENT 'FK to distribution.feeder.feeder_id — Distribution transformers are installed on feeders. This relationship is essential for load analysis (aggregating transformer loads to feeder level), outage impact assessment, and capacity planning.',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Distribution transformers are individually tracked capital assets in the registry for condition monitoring, depreciation, replacement planning, and warranty tracking. asset_tag and serial_number are d',
    `scada_point_id` BIGINT COMMENT 'Foreign key linking to grid.scada_point. Business justification: SCADA-monitored distribution transformers (scada_monitored_flag=true) require a FK to the SCADA point for real-time load monitoring, overload alarm management, and DMS integration. This link enables t',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`sectionalizing_device` (
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Sectionalizing devices (reclosers, switches) are subject to specific maintenance and inspection obligations under state PUC rules and NERC standards. A compliance officer tracking device-level obligat',
    `planned_outage_window_id` BIGINT COMMENT 'Foreign key linking to outage.planned_outage_window. Business justification: Sectionalizing devices (reclosers, switches, fuses) require planned outage windows for maintenance, testing, and replacement. Linking the device to its planned outage window supports asset maintenance',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Sectionalizing devices (reclosers, switches, fuses) are capital assets tracked in the registry for depreciation, condition assessment, and maintenance planning. serial_number is denormalized from regi',
    `feeder_id` BIGINT COMMENT 'Identifier of the distribution feeder circuit where this device is installed. Links device to circuit topology for load flow and reliability analysis.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`service_drop` (
    ...
    `transformer_id` BIGINT COMMENT 'FK to distribution.transformer.transformer_id — Service drops connect to distribution transformers. This FK is essential for outage impact assessment (transformer outage → which service drops/customers affected) and voltage complaint investigation.',
    ...
    `primary_service_transformer_id` BIGINT COMMENT 'Reference to the distribution transformer from which this service drop originates. Null for services fed directly from secondary mains.',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Service drops are capital assets tracked in the registry for depreciation, replacement programs, and FERC accounting. Utilities register service drop infrastructure for lifecycle management and capita',
    `service_point_id` BIGINT COMMENT 'Foreign key linking to metering.service_point. Business justification: Field crew work orders, asset management, and service connection/disconnection operations require linking the physical service drop conductor to the metering service point it terminates at. Utility fi',
    `service_territory_id` BIGINT COMMENT 'FK to distribution.service_territory',
    `to_transformer_id` BIGINT COMMENT 'FK to distribution.distribution_transformer.distribution_transformer_id — Service drops connect to distribution transformers (or secondary network). The description mentions pole or transformer association. This FK is critical for customer-to-network tracing and outage im',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`network_segment` (
    ...
    `ems_node_id` BIGINT COMMENT 'Foreign key linking to grid.ems_node. Business justification: Distribution network segments are modeled as EMS branches/nodes for state estimation observability and contingency analysis at the transmission-distribution boundary. Distribution planners and EMS ope',
    ...
    `feeder_id` BIGINT COMMENT 'Reference to the parent distribution feeder that this segment belongs to. Links segment to the primary circuit originating from a substation.',
    `service_territory_id` BIGINT COMMENT 'Foreign key linking to distribution.service_territory. Business justification: Network segments are physical conductor sections within the distribution network that fall within a specific service territory. The network_segment table already has a service_territory_code STRING co',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`load_zone` (
    ...
    `control_area_id` BIGINT COMMENT 'Foreign key linking to grid.control_area. Business justification: Distribution load zones roll up into a balancing authority control area for AGC load forecasting, demand response dispatch, and NERC BAL compliance reporting. A domain expert expects load zones to be ',
    ...
    `pnode_id` BIGINT COMMENT 'Foreign key linking to transmission.pnode. Business justification: Distribution load zones are mapped to transmission pnodes for LMP-based wholesale settlement, congestion cost allocation, and load forecasting in RTO/ISO markets. FERC settlement and distribution plan',
    `feeder_id` BIGINT COMMENT 'FK to distribution.feeder.feeder_id — Load zones are associated with feeders. The description mentions associated feeders. This is a many-to-many but the primary association is critical for load balancing operations.',
    `service_territory_id` BIGINT COMMENT 'Foreign key linking to distribution.service_territory. Business justification: Load zones are logical groupings of network segments used for load balancing and DMS/ADMS operations — they exist within the geographic boundaries of a service territory. Adding service_territory_id t',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` (
    ...
    `control_center_id` BIGINT COMMENT 'Foreign key linking to grid.control_center. Business justification: Distribution switching orders for operations affecting transmission-connected substations require authorization and coordination from the grid control center. Control centers issue safety clearances a',
    `dr_event_id` BIGINT COMMENT 'Foreign key linking to demand.dr_event. Business justification: Switching orders are issued in coordination with DR events for load transfer during outage restoration and planned maintenance. Linking switching orders to triggering DR events is required for post-ev',
    `energy_schedule_id` BIGINT COMMENT 'Foreign key linking to trading.energy_schedule. Business justification: Switching orders that de-energize feeders with DERs require coordinated energy schedule curtailments. ADMS/ETRM integration mandates this link so operators can identify and revise active energy schedu',
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Switching orders for planned outages must comply with specific regulatory obligations governing advance customer notification, safety clearance standards, and work authorization. A compliance officer ',
    `planned_outage_window_id` BIGINT COMMENT 'Foreign key linking to outage.planned_outage_window. Business justification: Planned maintenance outages require distribution switching orders to de-energize and re-energize equipment. Linking switching orders to their planned outage window is standard OMS/WMS integration, ena',
    `state_estimation_run_id` BIGINT COMMENT 'Foreign key linking to grid.state_estimation_run. Business justification: Distribution switching orders reference state estimation snapshots to validate network topology safety before and after switching. The pre/post topology snapshots on the switching order require a link',
    `storm_event_id` BIGINT COMMENT 'Foreign key linking to outage.storm_event. Business justification: Storm-driven switching orders are issued during major storm events for network reconfiguration and restoration. Post-storm switching analysis, NERC OE-417 reporting, and mutual aid cost tracking requi',
    `transmission_switching_order_id` BIGINT COMMENT 'Foreign key linking to transmission.transmission_switching_order. Business justification: Distribution switching operations at the T/D interface must be coordinated with transmission switching orders per NERC TOP/GOP coordination requirements and safety clearance protocols. Transmission op',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` (
    `flisr_event_id` BIGINT COMMENT 'Unique identifier for the FLISR automation event record. Primary key.',
    `cause_id` BIGINT COMMENT 'Foreign key linking to outage.cause. Business justification: FLISR events are triggered by specific fault types requiring standardized cause classification for IEEE 1366 reliability reporting and NERC/PUC regulatory filings. The outage.cause lookup provides the',
    ...
    `dr_event_id` BIGINT COMMENT 'Foreign key linking to demand.dr_event. Business justification: FLISR events trigger DR dispatch for load relief during fault isolation and restoration. Linking FLISR events to coordinated DR events is required for post-event reliability analysis, SAIDI/SAIFI repo',
    `energy_schedule_id` BIGINT COMMENT 'Foreign key linking to trading.energy_schedule. Business justification: FLISR events isolating feeders with DERs force schedule deviations that must be reported to the ISO/RTO. This link enables automatic identification of affected energy schedules during fault isolation,',
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: FLISR events carry a puc_reportable_flag indicating they are subject to specific PUC reporting obligations. A compliance officer managing automated restoration reporting expects each FLISR event to re',
    `network_segment_id` BIGINT COMMENT 'Identifier of the distribution network segment (feeder section) identified by FLISR algorithm as containing the fault. References GIS utility network topology.',
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: FLISR events with puc_reportable_flag=true must be included in PUC regulatory reliability filings. Automated restoration events are specifically tracked in state reliability reports. A regulatory affa',
    ...
    `scada_point_id` BIGINT COMMENT 'Foreign key linking to grid.scada_point. Business justification: FLISR events are triggered by SCADA fault indicator alarms. The plain string triggering_fault_indicator_code is a denormalized SCADA reference. Role-prefix triggering_ used because the event has a',
    `weather_input_id` BIGINT COMMENT 'Foreign key linking to forecast.weather_input. Business justification: FLISR event performance is analyzed against weather forecast inputs for storm hardening studies and reliability reporting. weather_condition is a denormalized plain attribute; linking to forecast.weat',
    ...
    CONSTRAINT pk_flisr_event PRIMARY KEY(`flisr_event_id`)
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` (
    ...
    `capacitor_bank_id` BIGINT COMMENT 'Foreign key linking to distribution.capacitor_bank. Business justification: VVO (Volt-VAR Optimization) control actions executed by GE PowerOn ADMS primarily target capacitor banks to regulate reactive power and voltage profiles on distribution feeders. A volt_var_action reco',
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: IEEE 1547-2018 requires smart inverter DERs to participate in volt-var optimization. VVO actions that dispatch DER reactive power control must reference the specific DER registry record for DERMS comm',
    ...
    `dr_event_id` BIGINT COMMENT 'Foreign key linking to demand.dr_event. Business justification: VVO actions are coordinated with DR dispatch for combined CVR+DR demand reduction programs. Linking volt_var_action to dr_event supports combined CVR-DR performance reporting, energy savings verificat',
    `energy_schedule_id` BIGINT COMMENT 'Foreign key linking to trading.energy_schedule. Business justification: VVO actions that curtail DER output must reference the active energy schedule to trigger ISO notification of forced curtailment. ADMS/ETRM integration requires this link for real-time DER dispatch coo',
    `feeder_id` BIGINT COMMENT 'Identifier of the distribution feeder circuit on which the VVO action was performed.',
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Volt-VAR optimization programs are often mandated by state PUC energy efficiency obligations or voltage quality standards. A compliance officer tracking VVO program compliance expects volt_var_action ',
    `load_zone_id` BIGINT COMMENT 'Identifier of the distribution control zone where the VVO action was executed. Links to the geographic/electrical zone managed by ADMS.',
    `scada_point_id` BIGINT COMMENT 'Foreign key linking to grid.scada_point. Business justification: VVO actions are executed via SCADA commands to field devices. The plain string scada_command_code is a denormalized SCADA reference. A proper FK enables DMS-SCADA command audit trails, VVO performan',
    `state_estimation_run_id` BIGINT COMMENT 'Foreign key linking to grid.state_estimation_run. Business justification: VVO actions are triggered by state estimation results identifying voltage violations. Linking to the triggering state estimation run enables VVO performance analysis, voltage violation audit trails, a',
    `tertiary_load_zone_id` BIGINT COMMENT 'FK to distribution.load_zone.load_zone_id — VVO control actions are dispatched per control zone. The load_zone FK identifies which zone the optimization action targets, enabling zone-level VVO performance analysis.',
    `volt_load_zone_id` BIGINT COMMENT 'FK to distribution.load_zone.load_zone_id — VVO actions are executed within a control zone. The description mentions control zone. Critical for volt-VAR optimization tracking.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`capacitor_bank` (
    ...
    `ems_node_id` BIGINT COMMENT 'Foreign key linking to grid.ems_node. Business justification: Capacitor banks are modeled as shunt reactive elements in the EMS network model. Their reactive power contribution is tracked in state estimation for power flow accuracy. This link enables VVO coordin',
    ...
    `network_segment_id` BIGINT COMMENT 'Foreign key linking to distribution.network_segment. Business justification: Capacitor banks are physically installed on specific network segments (conductors/line sections) of the distribution network. The network segment determines the electrical location of the capacitor ba',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Capacitor banks are capital assets requiring registry entries for depreciation, valuation, and maintenance planning. serial_number is denormalized from registry. Utility asset managers track all field',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` (
    ...
    `transmission_outage_id` BIGINT COMMENT 'Foreign key linking to transmission.transmission_outage. Business justification: NERC and state PUC reliability reporting requires linking distribution outage events to causal upstream transmission outages for root cause analysis and regulatory filings. Distribution reliability en',
    `cause_id` BIGINT COMMENT 'Foreign key linking to outage.cause. Business justification: IEEE 1366 reliability reporting (SAIDI/SAIFI/CAIDI) and PUC regulatory filings require standardized cause classification. The outage.cause lookup provides IEEE/NERC/PUC cause codes. cause_code and cau',
    `control_center_id` BIGINT COMMENT 'Foreign key linking to grid.control_center. Business justification: Reliability events meeting NERC OE-417 thresholds are reported through and coordinated by the grid control center. The control center manages restoration coordination and regulatory submissions. This ',
    ...
    `dr_event_id` BIGINT COMMENT 'Foreign key linking to demand.dr_event. Business justification: Reliability events (outages) trigger DR dispatch; linking reliability events to coordinated DR events is required for IEEE 1366 reporting with DR contribution, PUC regulatory filings, and post-event a',
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Distribution reliability events must be reported under specific regulatory obligations (IEEE 1366 metrics, state PUC reliability rules). The existing regulatory_filing FK captures the filing, but the ',
    ...
    `planned_outage_window_id` BIGINT COMMENT 'Foreign key linking to outage.planned_outage_window. Business justification: IEEE 1366 reliability calculations exclude planned interruptions from SAIDI/SAIFI indices. Linking reliability events to planned outage windows enables proper exclusion logic in regulatory filings and',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.registry. Business justification: Reliability events involve failed equipment traceable to the asset registry for condition management, failure rate analysis, and replacement prioritization. Links the outage/reliability domain to asse',
    ...
    `service_territory_id` BIGINT COMMENT 'Foreign key linking to distribution.service_territory. Business justification: Distribution reliability events (SAIDI/SAIFI/CAIDI contributors) are reported to the PUC by service territory. The puc_reportable_flag and puc_report_date fields confirm regulatory reporting requireme',
    ...
    `transformer_id` BIGINT COMMENT 'Foreign key linking to distribution.transformer. Business justification: Distribution reliability interruption events are frequently caused by transformer failures (overloads, insulation failures, oil leaks, lightning strikes). The distribution_reliability_event already li',
    `violation_id` BIGINT COMMENT 'Foreign key linking to compliance.violation. Business justification: A distribution reliability event exceeding regulatory duration or frequency thresholds directly triggers a compliance violation. Utilities track which outage events resulted in PUC violations for pena',
    `weather_input_id` BIGINT COMMENT 'Foreign key linking to forecast.weather_input. Business justification: Reliability events are correlated with weather forecast inputs for IEEE 1366 major event day classification, storm hardening analysis, and PUC regulatory reporting. weather_condition is a denormalized',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` (
    ...
    `dr_event_id` BIGINT COMMENT 'Foreign key linking to demand.dr_event. Business justification: Feeder load readings are the primary SCADA data source for validating DR curtailment during events. Linking readings to DR events supports feeder-level M&V, settlement dispute resolution, and ISO/RTO ',
    `ems_node_id` BIGINT COMMENT 'Foreign key linking to grid.ems_node. Business justification: Feeder load readings feed into state estimation as pseudo-measurements at specific EMS nodes. Linking to the EMS node enables state estimation observability tracking, load flow validation, and distrib',
    ...
    `flisr_event_id` BIGINT COMMENT 'Identifier linking this reading to a FLISR automation event if the reading was captured during an automated switching sequence. Null if no FLISR event was active.',
    `model_id` BIGINT COMMENT 'Foreign key linking to forecast.model. Business justification: Feeder load readings are used to validate and calibrate forecast models — the accuracy assessment process (MAPE, RMSE tracking) requires linking actual feeder readings to the specific forecast model t',
    ...
    `load_zone_id` BIGINT COMMENT 'Foreign key linking to distribution.load_zone. Business justification: Feeder load readings are the primary data source for load zone demand analysis, load balancing, and DMS/ADMS state estimation. Load zones are logical groupings of network segments used for load balanc',
    ...
    `planned_outage_window_id` BIGINT COMMENT 'Foreign key linking to outage.planned_outage_window. Business justification: Pre- and post-planned-outage load readings are required to verify restoration and confirm load levels before re-energization. Linking feeder load readings to planned outage windows supports work manag',
    ...
    `scada_point_id` BIGINT COMMENT 'Foreign key linking to grid.scada_point. Business justification: Feeder load readings are sourced from SCADA telemetry points. Linking to the source scada_point enables data lineage traceability, SCADA data quality validation, and PI historian tag reconciliation re',
    `storm_event_id` BIGINT COMMENT 'Foreign key linking to outage.storm_event. Business justification: Feeder load readings during storm events are essential for post-storm load restoration sequencing, storm cost estimation, and NERC reportable event analysis. Linking load readings to storm events enab',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to interconnection.interconnection_agreement. Business justification: The physical distribution DER interconnection point is authorized by a formal interconnection agreement. Utilities must link the physical grid asset record to the binding legal agreement for complianc',
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: The distribution interconnection point is the physical POI for a registered DER asset. Interconnection engineers and DERMS operators must link the physical grid connection point to the DER registry re',
    `der_system_id` BIGINT COMMENT 'Foreign key linking to interconnection.der_system. Business justification: The physical distribution interconnection point must reference the registered DER system for DERMS dispatch, IEEE 1547 compliance monitoring, and DER performance reporting. This links the physical gri',
    ...
    `ems_node_id` BIGINT COMMENT 'Foreign key linking to grid.ems_node. Business justification: DER interconnection points inject power at nodes modeled in the EMS for state estimation and contingency analysis. IEEE 1547 compliance, hosting capacity studies, and FERC Order 2222 aggregation requi',
    ...
    `load_zone_id` BIGINT COMMENT 'Foreign key linking to distribution.load_zone. Business justification: DER interconnection points directly impact load zone hosting capacity, voltage profiles, and reactive power balance. The hosting_capacity_impact_mw field on der_interconnection_point quantifies this i',
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: DER interconnections are governed by specific regulatory obligations including IEEE 1547 compliance, state interconnection rules, and NEM program requirements. A compliance officer managing DER interc',
    `event_id` BIGINT COMMENT 'Foreign key linking to outage.event. Business justification: DER interconnection points must disconnect during outage events per IEEE 1547 anti-islanding requirements. Tracking which outage event triggered DER disconnection is required for interconnection agree',
    `pnode_id` BIGINT COMMENT 'Foreign key linking to transmission.pnode. Business justification: FERC Order 2222 and wholesale DER market participation require mapping DER interconnection points to transmission pnodes for energy settlement, interconnection impact studies, and hosting capacity ana',
    ...
    `feeder_id` BIGINT COMMENT 'Reference to the distribution feeder circuit to which this DER is connected. Critical for hosting capacity analysis, reverse power flow monitoring, and feeder-level DER aggregation.',
    `network_segment_id` BIGINT COMMENT 'FK to distribution.network_segment.network_segment_id — DER interconnection points connect at specific network segments. Segment-level assignment is needed for detailed hosting capacity analysis and voltage impact studies.',
    `rate_case_id` BIGINT COMMENT 'Foreign key linking to compliance.rate_case. Business justification: NEM tariffs and DER export compensation rates are established in rate cases. DER interconnection points under NEM programs are directly governed by rate case decisions setting compensation levels. A r',
    ...
    `service_territory_id` BIGINT COMMENT 'Foreign key linking to distribution.service_territory. Business justification: DER interconnection points are subject to franchise territory regulations, NEM program eligibility, and interconnection queue management that are all governed at the service territory level. The nem_e',
    ...
    `transformer_id` BIGINT COMMENT 'Foreign key linking to distribution.transformer. Business justification: DER interconnection points connect to the distribution secondary network through distribution transformers. The transformer is the physical point where the DERs inverter output is injected into the g',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`service_territory` (
    ...
    `control_area_id` BIGINT COMMENT 'Foreign key linking to grid.control_area. Business justification: Service territories map to a balancing authority control area for NERC compliance reporting, load obligation tracking, and regulatory filings. A distribution planner or regulatory analyst expects serv',
    ...
    `market_participant_id` BIGINT COMMENT 'Foreign key linking to trading.market_participant. Business justification: Each service territory is operated by a utility that participates in wholesale markets as a market participant. This link is used in regulatory reporting, load-serving obligation tracking, and ISO/RTO',
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Service territories are subject to jurisdiction-specific regulatory obligations including franchise agreement requirements, reliability performance standards, and reporting mandates. A compliance mana',
    `planning_period_id` BIGINT COMMENT 'Foreign key linking to forecast.planning_period. Business justification: Service territories operate under regulatory planning periods governing IRP cycles, rate case schedules, and reliability reporting periods. Linking territories to planning periods enables tracking of ',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`distribution`.`switching_step` (
    `switching_step_id` BIGINT COMMENT 'Unique identifier for this switching step. Primary key for the association.',
    `distribution_switching_order_id` BIGINT COMMENT 'Foreign key linking to the parent switching order that contains this step',
    `sectionalizing_device_id` BIGINT COMMENT 'Foreign key linking to the sectionalizing device that is the target of this switching step',
    `actual_timestamp` TIMESTAMP COMMENT 'Actual date and time when this switching step was executed. Used for audit trail and performance analysis.',
    `confirmed_device_state` STRING COMMENT 'Actual confirmed device state after operation execution. May differ from expected state if operation failed.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this switching step record was created in the system. Part of audit trail.',
    `failure_reason` STRING COMMENT 'Free-text explanation of why this step failed to execute or complete. Populated only when step_completion_status is FAILED.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Date and time when this switching step record was last modified. Part of audit trail.',
    `operation_type` STRING COMMENT 'Type of switching operation to be performed on the device. Critical for safety procedures and device state management.',
    `operator_code` BIGINT COMMENT 'Identifier of the operator who executed this switching step. Critical for NERC CIP compliance and accountability.',
    `operator_confirmation_flag` BOOLEAN COMMENT 'Indicates whether the operator has confirmed successful execution and device state verification for this step. Required for NERC CIP compliance.',
    `post_operation_position` STRING COMMENT 'Expected device position after successful execution of this step. Used for confirmation and validation.',
    `pre_operation_position` STRING COMMENT 'Required device position before this step can be executed. Used for safety verification and operational validation.',
    `safety_clearance_verified` BOOLEAN COMMENT 'Indicates whether required safety clearances were verified before step execution. Critical for worker safety compliance.',
    `scada_command_sent` BOOLEAN COMMENT 'Indicates whether a SCADA remote control command was sent for this step (for automated devices). False for manual operations.',
    `scheduled_timestamp` TIMESTAMP COMMENT 'Planned date and time for execution of this switching step. Part of the switching order schedule.',
    `step_completion_status` STRING COMMENT 'Current execution status of this switching step. Tracks progression through the step lifecycle.',
    `step_sequence_number` STRING COMMENT 'Sequential order of this step within the parent switching order. Determines execution order for safety and operational correctness.',
    CONSTRAINT pk_switching_step PRIMARY KEY(`switching_step_id`)
) COMMENT 'This association product represents the atomic operational unit of a distribution switching order - each individual device operation performed as part of a switching sequence. It captures the step-by-step execution of switching orders on sectionalizing devices, including operation type, device state transitions, timing, operator confirmation, and step completion status. Each record links one switching order to one sectionalizing device operation with attributes that exist only in the context of this specific switching action. Critical for NERC CIP compliance, safety clearance tracking, FLISR automation audit trails, and post-event analysis.. Existence Justification: In distribution operations, a switching order executes a sequence of individual device operations (switching steps), where each step operates on one sectionalizing device. One switching order operates on multiple devices in sequence (e.g., open recloser A, close switch B, open fuse C), and one device is operated on by many different switching orders over time (planned maintenance, emergency restoration, FLISR automation). The switching step is the atomic operational unit tracked for NERC CIP compliance, safety clearance, and audit trails.';
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder` ADD CONSTRAINT `fk_distribution_feeder_parent_feeder_id` FOREIGN KEY (`parent_feeder_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`feeder`(`feeder_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` ADD CONSTRAINT `fk_distribution_distribution_substation_service_territory_id` FOREIGN KEY (`service_territory_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`service_territory`(`service_territory_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ADD CONSTRAINT `fk_distribution_transformer_load_zone_id` FOREIGN KEY (`load_zone_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`load_zone`(`load_zone_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ADD CONSTRAINT `fk_distribution_transformer_network_segment_id` FOREIGN KEY (`network_segment_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`network_segment`(`network_segment_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ADD CONSTRAINT `fk_distribution_transformer_primary_transformer_feeder_id` FOREIGN KEY (`primary_transformer_feeder_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`feeder`(`feeder_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_drop` ADD CONSTRAINT `fk_distribution_service_drop_primary_service_transformer_id` FOREIGN KEY (`primary_service_transformer_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`transformer`(`transformer_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_drop` ADD CONSTRAINT `fk_distribution_service_drop_to_transformer_id` FOREIGN KEY (`to_transformer_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`transformer`(`transformer_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`network_segment` ADD CONSTRAINT `fk_distribution_network_segment_service_territory_id` FOREIGN KEY (`service_territory_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`service_territory`(`service_territory_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`load_zone` ADD CONSTRAINT `fk_distribution_load_zone_service_territory_id` FOREIGN KEY (`service_territory_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`service_territory`(`service_territory_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ADD CONSTRAINT `fk_distribution_flisr_event_distribution_substation_id` FOREIGN KEY (`distribution_substation_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`distribution_substation`(`distribution_substation_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ADD CONSTRAINT `fk_distribution_flisr_event_feeder_id` FOREIGN KEY (`feeder_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`feeder`(`feeder_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ADD CONSTRAINT `fk_distribution_flisr_event_network_segment_id` FOREIGN KEY (`network_segment_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`network_segment`(`network_segment_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ADD CONSTRAINT `fk_distribution_volt_var_action_capacitor_bank_id` FOREIGN KEY (`capacitor_bank_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`capacitor_bank`(`capacitor_bank_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ADD CONSTRAINT `fk_distribution_volt_var_action_tertiary_load_zone_id` FOREIGN KEY (`tertiary_load_zone_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`load_zone`(`load_zone_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`capacitor_bank` ADD CONSTRAINT `fk_distribution_capacitor_bank_network_segment_id` FOREIGN KEY (`network_segment_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`network_segment`(`network_segment_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ADD CONSTRAINT `fk_distribution_distribution_reliability_event_service_territory_id` FOREIGN KEY (`service_territory_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`service_territory`(`service_territory_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ADD CONSTRAINT `fk_distribution_distribution_reliability_event_transformer_id` FOREIGN KEY (`transformer_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`transformer`(`transformer_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ADD CONSTRAINT `fk_distribution_feeder_load_reading_flisr_event_id` FOREIGN KEY (`flisr_event_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`flisr_event`(`flisr_event_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ADD CONSTRAINT `fk_distribution_feeder_load_reading_load_zone_id` FOREIGN KEY (`load_zone_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`load_zone`(`load_zone_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ADD CONSTRAINT `fk_distribution_der_interconnection_point_load_zone_id` FOREIGN KEY (`load_zone_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`load_zone`(`load_zone_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ADD CONSTRAINT `fk_distribution_der_interconnection_point_service_territory_id` FOREIGN KEY (`service_territory_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`service_territory`(`service_territory_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ADD CONSTRAINT `fk_distribution_der_interconnection_point_transformer_id` FOREIGN KEY (`transformer_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`transformer`(`transformer_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ADD CONSTRAINT `fk_distribution_switching_step_distribution_switching_order_id` FOREIGN KEY (`distribution_switching_order_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`distribution_switching_order`(`distribution_switching_order_id`);
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ADD CONSTRAINT `fk_distribution_switching_step_sectionalizing_device_id` FOREIGN KEY (`sectionalizing_device_id`) REFERENCES `energy_utilities_ecm`.`distribution`.`sectionalizing_device`(`sectionalizing_device_id`);
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder` ALTER COLUMN `ems_node_id` SET TAGS ('dbx_business_glossary_term' = 'Ems Node Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder` ALTER COLUMN `parent_feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Backup Feeder Identifier');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder` ALTER COLUMN `pnode_id` SET TAGS ('dbx_business_glossary_term' = 'Pnode Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` ALTER COLUMN `bes_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Bes Facility Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` ALTER COLUMN `model_id` SET TAGS ('dbx_business_glossary_term' = 'Forecast Model Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_substation` ALTER COLUMN `service_territory_id` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ALTER COLUMN `load_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Load Zone Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ALTER COLUMN `network_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Network Segment Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`transformer` ALTER COLUMN `scada_point_id` SET TAGS ('dbx_business_glossary_term' = 'Scada Point Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`sectionalizing_device` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`sectionalizing_device` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`sectionalizing_device` ALTER COLUMN `planned_outage_window_id` SET TAGS ('dbx_business_glossary_term' = 'Planned Outage Window Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`sectionalizing_device` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`sectionalizing_device` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Feeder ID');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_drop` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_drop` ALTER COLUMN `primary_service_transformer_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Transformer Identifier');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_drop` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_drop` ALTER COLUMN `service_point_id` SET TAGS ('dbx_business_glossary_term' = 'Service Point Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`network_segment` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`network_segment` ALTER COLUMN `ems_node_id` SET TAGS ('dbx_business_glossary_term' = 'Ems Node Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`network_segment` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Feeder Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`network_segment` ALTER COLUMN `service_territory_id` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`load_zone` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`load_zone` ALTER COLUMN `control_area_id` SET TAGS ('dbx_business_glossary_term' = 'Control Area Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`load_zone` ALTER COLUMN `pnode_id` SET TAGS ('dbx_business_glossary_term' = 'Pnode Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`load_zone` ALTER COLUMN `service_territory_id` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` SET TAGS ('dbx_subdomain' = 'operational_switching');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `control_center_id` SET TAGS ('dbx_business_glossary_term' = 'Control Center Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `dr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `energy_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Energy Schedule Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `planned_outage_window_id` SET TAGS ('dbx_business_glossary_term' = 'Planned Outage Window Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `state_estimation_run_id` SET TAGS ('dbx_business_glossary_term' = 'State Estimation Run Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `storm_event_id` SET TAGS ('dbx_business_glossary_term' = 'Storm Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_switching_order` ALTER COLUMN `transmission_switching_order_id` SET TAGS ('dbx_business_glossary_term' = 'Transmission Switching Order Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` SET TAGS ('dbx_subdomain' = 'operational_switching');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `flisr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Fault Location, Isolation, and Service Restoration (FLISR) Event ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `cause_id` SET TAGS ('dbx_business_glossary_term' = 'Cause Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `distribution_substation_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Substation ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `dr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `energy_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Energy Schedule Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Outage Event ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Feeder ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `network_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Faulted Distribution Segment ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `storm_event_id` SET TAGS ('dbx_business_glossary_term' = 'Storm Event ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `scada_point_id` SET TAGS ('dbx_business_glossary_term' = 'Triggering Scada Point Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `weather_input_id` SET TAGS ('dbx_business_glossary_term' = 'Weather Input Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `caidi_contribution_minutes` SET TAGS ('dbx_business_glossary_term' = 'Customer Average Interruption Duration Index (CAIDI) Contribution in Minutes');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `communication_failure_flag` SET TAGS ('dbx_business_glossary_term' = 'Communication Failure Flag');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `customers_affected_initial` SET TAGS ('dbx_business_glossary_term' = 'Customers Affected Initial Count');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `customers_affected_post_restoration` SET TAGS ('dbx_business_glossary_term' = 'Customers Affected Post-Restoration Count');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `customers_restored_count` SET TAGS ('dbx_business_glossary_term' = 'Customers Restored Count');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `der_impacted_count` SET TAGS ('dbx_business_glossary_term' = 'Distributed Energy Resource (DER) Impacted Count');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `event_status` SET TAGS ('dbx_business_glossary_term' = 'FLISR Event Status');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `event_status` SET TAGS ('dbx_value_regex' = 'initiated|in_progress|completed|failed|partially_completed|aborted');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'FLISR Event Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `failure_reason` SET TAGS ('dbx_business_glossary_term' = 'FLISR Failure Reason');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `fault_current_amps` SET TAGS ('dbx_business_glossary_term' = 'Fault Current in Amperes');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `fault_type` SET TAGS ('dbx_business_glossary_term' = 'Fault Type Classification');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `fault_type` SET TAGS ('dbx_value_regex' = 'permanent|temporary|transient');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `flisr_algorithm_version` SET TAGS ('dbx_business_glossary_term' = 'FLISR Algorithm Version');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `flisr_event_number` SET TAGS ('dbx_business_glossary_term' = 'FLISR Event Number');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `flisr_outcome_status` SET TAGS ('dbx_business_glossary_term' = 'FLISR Outcome Status');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `flisr_outcome_status` SET TAGS ('dbx_value_regex' = 'full_success|partial_success|failed|aborted_operator|aborted_system');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `isolation_devices_operated` SET TAGS ('dbx_business_glossary_term' = 'Isolation Devices Operated');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `isolation_time_seconds` SET TAGS ('dbx_business_glossary_term' = 'Isolation Time in Seconds');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `load_transferred_kw` SET TAGS ('dbx_business_glossary_term' = 'Load Transferred in Kilowatts (kW)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `major_event_exclusion_flag` SET TAGS ('dbx_business_glossary_term' = 'Major Event Day (MED) Exclusion Flag');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `operator_override_flag` SET TAGS ('dbx_business_glossary_term' = 'Operator Override Flag');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `puc_reportable_flag` SET TAGS ('dbx_business_glossary_term' = 'Public Utility Commission (PUC) Reportable Flag');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `record_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `restoration_devices_operated` SET TAGS ('dbx_business_glossary_term' = 'Restoration Devices Operated');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `restoration_time_seconds` SET TAGS ('dbx_business_glossary_term' = 'Restoration Time in Seconds');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `saidi_contribution_minutes` SET TAGS ('dbx_business_glossary_term' = 'System Average Interruption Duration Index (SAIDI) Contribution in Minutes');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `saifi_contribution` SET TAGS ('dbx_business_glossary_term' = 'System Average Interruption Frequency Index (SAIFI) Contribution');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `scada_alarm_count` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Control and Data Acquisition (SCADA) Alarm Count');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System Identifier');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `switching_operations_count` SET TAGS ('dbx_business_glossary_term' = 'Switching Operations Count');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `topology_validation_status` SET TAGS ('dbx_business_glossary_term' = 'Network Topology Validation Status');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `topology_validation_status` SET TAGS ('dbx_value_regex' = 'valid|invalid|uncertain|not_checked');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`flisr_event` ALTER COLUMN `voltage_sag_percent` SET TAGS ('dbx_business_glossary_term' = 'Voltage Sag Percentage');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` SET TAGS ('dbx_subdomain' = 'operational_switching');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `capacitor_bank_id` SET TAGS ('dbx_business_glossary_term' = 'Capacitor Bank Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `dr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `energy_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Energy Schedule Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `load_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Control Zone ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `scada_point_id` SET TAGS ('dbx_business_glossary_term' = 'Scada Point Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`volt_var_action` ALTER COLUMN `state_estimation_run_id` SET TAGS ('dbx_business_glossary_term' = 'State Estimation Run Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`capacitor_bank` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`capacitor_bank` ALTER COLUMN `ems_node_id` SET TAGS ('dbx_business_glossary_term' = 'Ems Node Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`capacitor_bank` ALTER COLUMN `network_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Network Segment Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`capacitor_bank` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` SET TAGS ('dbx_subdomain' = 'operational_switching');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `transmission_outage_id` SET TAGS ('dbx_business_glossary_term' = 'Causal Transmission Outage Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `cause_id` SET TAGS ('dbx_business_glossary_term' = 'Cause Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `control_center_id` SET TAGS ('dbx_business_glossary_term' = 'Control Center Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `dr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Event Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `planned_outage_window_id` SET TAGS ('dbx_business_glossary_term' = 'Planned Outage Window Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Registry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `service_territory_id` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `transformer_id` SET TAGS ('dbx_business_glossary_term' = 'Transformer Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `violation_id` SET TAGS ('dbx_business_glossary_term' = 'Violation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`distribution_reliability_event` ALTER COLUMN `weather_input_id` SET TAGS ('dbx_business_glossary_term' = 'Weather Input Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` SET TAGS ('dbx_subdomain' = 'operational_switching');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `dr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `ems_node_id` SET TAGS ('dbx_business_glossary_term' = 'Ems Node Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `flisr_event_id` SET TAGS ('dbx_business_glossary_term' = 'FLISR (Fault Location, Isolation, and Service Restoration) Event ID');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `model_id` SET TAGS ('dbx_business_glossary_term' = 'Forecast Model Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `load_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Load Zone Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `planned_outage_window_id` SET TAGS ('dbx_business_glossary_term' = 'Planned Outage Window Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `scada_point_id` SET TAGS ('dbx_business_glossary_term' = 'Scada Point Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`feeder_load_reading` ALTER COLUMN `storm_event_id` SET TAGS ('dbx_business_glossary_term' = 'Storm Event Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Interconnection Agreement Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `der_system_id` SET TAGS ('dbx_business_glossary_term' = 'Der System Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `ems_node_id` SET TAGS ('dbx_business_glossary_term' = 'Ems Node Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `load_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Load Zone Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Outage Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `pnode_id` SET TAGS ('dbx_business_glossary_term' = 'Pnode Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Feeder Identifier');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `rate_case_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Case Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `service_territory_id` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`der_interconnection_point` ALTER COLUMN `transformer_id` SET TAGS ('dbx_business_glossary_term' = 'Transformer Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_territory` SET TAGS ('dbx_subdomain' = 'network_assets');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_territory` ALTER COLUMN `control_area_id` SET TAGS ('dbx_business_glossary_term' = 'Control Area Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_territory` ALTER COLUMN `market_participant_id` SET TAGS ('dbx_business_glossary_term' = 'Market Participant Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_territory` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`service_territory` ALTER COLUMN `planning_period_id` SET TAGS ('dbx_business_glossary_term' = 'Planning Period Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` SET TAGS ('dbx_subdomain' = 'operational_switching');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` SET TAGS ('dbx_association_edges' = 'distribution.distribution_switching_order,distribution.sectionalizing_device');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `switching_step_id` SET TAGS ('dbx_business_glossary_term' = 'Switching Step Identifier');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `distribution_switching_order_id` SET TAGS ('dbx_business_glossary_term' = 'Switching Step - Distribution Switching Order Id');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `sectionalizing_device_id` SET TAGS ('dbx_business_glossary_term' = 'Switching Step - Sectionalizing Device Id');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `sectionalizing_device_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `sectionalizing_device_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `actual_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Actual Operation Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `confirmed_device_state` SET TAGS ('dbx_business_glossary_term' = 'Confirmed Device State');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `failure_reason` SET TAGS ('dbx_business_glossary_term' = 'Step Failure Reason');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `operation_type` SET TAGS ('dbx_business_glossary_term' = 'Operation Type');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `operator_code` SET TAGS ('dbx_business_glossary_term' = 'Operator Identifier');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `operator_confirmation_flag` SET TAGS ('dbx_business_glossary_term' = 'Operator Confirmation Flag');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `post_operation_position` SET TAGS ('dbx_business_glossary_term' = 'Post-Operation Device Position');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `pre_operation_position` SET TAGS ('dbx_business_glossary_term' = 'Pre-Operation Device Position');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `safety_clearance_verified` SET TAGS ('dbx_business_glossary_term' = 'Safety Clearance Verified');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `scada_command_sent` SET TAGS ('dbx_business_glossary_term' = 'SCADA Command Sent Flag');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `scheduled_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Scheduled Operation Timestamp');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `step_completion_status` SET TAGS ('dbx_business_glossary_term' = 'Step Completion Status');
ALTER TABLE `energy_utilities_ecm`.`distribution`.`switching_step` ALTER COLUMN `step_sequence_number` SET TAGS ('dbx_business_glossary_term' = 'Step Sequence Number');
