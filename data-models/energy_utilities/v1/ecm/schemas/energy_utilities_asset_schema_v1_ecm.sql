-- Schema for Domain: asset | Business: Energy Utilities | Version: v1_ecm
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`registry` (
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supply.vendor. Business justification: Assets have OEM manufacturers tracked as vendors for warranty claims, technical support, and spare parts procurement. Formalizes manufacturer field into vendor master for supplier performance tracking',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`hierarchy` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the user or system account that created this hierarchy relationship record. Used for audit trail and data governance. Links to enterprise identity management system.',
    `hierarchy_last_modified_by_user_employee_id` BIGINT COMMENT 'Identifier of the user or system account that last modified this hierarchy relationship record. Used for audit trail and change tracking. Links to enterprise identity management system.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`class` (
    ...
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Asset classes define standard material types for procurement planning and inventory stocking strategies. Utilities standardize materials by asset class for economies of scale and interoperability—crit',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`work_order` (
    ...
    `asset_registry_id` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Every work order is executed against a specific asset. This is the most fundamental FK in any EAM system — work orders without asset references are unroutable and uncosted.',
    `crew_id` BIGINT COMMENT 'Reference to the internal utility crew or work group assigned to execute this work order. May be null if work is assigned to an external contractor.',
    ...
    `contractor_worker_id` BIGINT COMMENT 'Reference to the external contractor or service provider assigned to execute this work order. May be null if work is performed by internal crews.',
    `cost_center_id` BIGINT COMMENT 'Financial cost center code to which this work order cost is allocated. Used for departmental budgeting, OPEX vs CAPEX classification, and regulatory reporting.',
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: Maintenance and repair work orders issued against DER assets for O&M cost tracking, warranty claim documentation, and reliability metrics (SAIDI/SAIFI). Critical for performance guarantee compliance i',
    `employee_id` BIGINT COMMENT 'Reference to the supervisor or manager responsible for approving and overseeing this work order. Used for approval workflow and accountability.',
    ...
    `gl_account_id` BIGINT COMMENT 'General ledger account code for financial posting of work order costs. Aligns with FERC Uniform System of Accounts for utility financial reporting.',
    `application_id` BIGINT COMMENT 'Foreign key linking to interconnection.application. Business justification: Work orders are created to perform interconnection-related construction (meter installation, service upgrades, POI modifications) and inspection support activities. Essential for tracking utility labo',
    `maintenance_plan_id` BIGINT COMMENT 'FK to asset.maintenance_plan.maintenance_plan_id — Preventive work orders are generated from maintenance plans. This FK enables tracking of PM compliance rates and plan execution history.',
    `purchase_order_id` BIGINT COMMENT 'Foreign key linking to supply.purchase_order. Business justification: Work orders often trigger direct material procurement for non-stock items or emergency purchases. Links work order to originating PO for cost tracking, capex/opex classification, and regulatory cost r',
    ...
    `registry_id` BIGINT COMMENT 'Reference to the utility asset (generation unit, transformer, transmission tower, distribution pole, switchgear, protection relay, substation equipment, fleet vehicle, or facility) that is the subject of this work order.',
    ...
    `line_id` BIGINT COMMENT 'Foreign key linking to transmission.line. Business justification: Maintenance work orders on transmission lines (conductor replacement, tower repair, insulator replacement, vegetation management) require direct linkage to the specific line for work planning, outage ',
    ...
    `work_maintenance_plan_id` BIGINT COMMENT 'Reference to the preventive maintenance schedule or plan that generated this work order. Used for PM compliance tracking and maintenance strategy optimization.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` (
    ...
    `asset_class_id` BIGINT COMMENT 'FK to asset.asset_class.asset_class_id — Maintenance plans are often defined at the asset class level (e.g., all 132kV power transformers get DGA every 2 years). This FK enables class-level maintenance strategy inheritance.',
    `asset_registry_id` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Maintenance plans are defined for specific assets or asset classes. The asset-level FK enables automatic work order generation for the correct asset.',
    `class_id` BIGINT COMMENT 'Reference to the asset class or equipment category for which this maintenance plan is defined. Used for template-based maintenance strategies applied across multiple assets of the same type.',
    ...
    `cost_center_id` BIGINT COMMENT 'Financial cost center to which maintenance costs (labor, materials, contractors) are charged. Used for OPEX tracking, departmental budgeting, and RAB (Regulatory Asset Base) cost allocation.',
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: Preventive maintenance schedules for DER equipment (inverter inspections, battery health checks, panel cleaning) required for warranty compliance, performance guarantee tracking in PPAs, and interconn',
    ...
    `employee_id` BIGINT COMMENT 'Reference to the maintenance planner or engineer responsible for managing this maintenance plan, including plan updates, work order review, and execution oversight.',
    `registry_id` BIGINT COMMENT 'Reference to the specific asset (generation unit, transformer, transmission tower, distribution pole, switchgear, protection relay, substation equipment, fleet vehicle, or facility) to which this maintenance plan applies. Links to the asset registry.',
    `crew_id` BIGINT COMMENT 'Reference to the maintenance crew or work center assigned to execute work orders generated by this plan. Used for resource capacity planning and crew scheduling.',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: Recurring maintenance services (vegetation management, transformer oil analysis, substation cleaning) are often contracted. Links maintenance plan to service contract for cost forecasting, contractor ',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Preventive maintenance plans specify standard materials (oils, filters, consumables) for bill-of-materials planning and auto-generation of material requisitions. Essential for MRP planning, inventory ',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`failure_event` (
    ...
    `asset_registry_id` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Failure events must reference the failed asset for reliability analysis, MTBF calculations, and NERC reporting. Core FK for RCM analysis.',
    ...
    `transformer_id` BIGINT COMMENT 'Foreign key linking to distribution.transformer. Business justification: Transformer failures are major reliability events requiring detailed tracking for warranty claims, manufacturer defect analysis, fleet health assessment, and replacement prioritization. Essential for ',
    `dr_event_id` BIGINT COMMENT 'Foreign key linking to demand.dr_event. Business justification: Asset failures may be triggered by DR events (transformer overload during rapid load restoration, relay misoperation during curtailment) or cause DR event cancellation. Reliability engineers analyze c',
    `event_id` BIGINT COMMENT 'Reference to the associated outage event if the failure caused a customer outage. Links to outage management system.',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Failure events identify specific failed components for reliability analysis, warranty claims, and defect tracking. Links failure to material master for supplier quality management, mean-time-between-f',
    ...
    `primary_work_order_id` BIGINT COMMENT 'FK to asset.work_order.work_order_id — Failure events typically generate corrective work orders. Linking failure to repair enables cost-of-failure analysis and RCM.',
    `registry_id` BIGINT COMMENT 'Reference to the asset that experienced the failure. Links to the asset registry in Ventyx Asset Suite.',
    `self_report_id` BIGINT COMMENT 'Foreign key linking to compliance.self_report. Business justification: Certain asset failures trigger mandatory self-reporting to NERC (bulk electric system events) or state regulators (major outages, safety incidents). Direct link enables regulatory incident reporting w',
    ...
    `weather_condition` STRING COMMENT 'Weather conditions at the time of failure (e.g., storm, lightning, high wind, ice) if weather-related.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`capital_project` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Capital projects track costs by cost center for budget control, variance analysis, and FERC account allocation in rate base filings. New FK needed; removes denormalized cost_center string.',
    `cpcn_application_id` BIGINT COMMENT 'Foreign key linking to compliance.cpcn_application. Business justification: Major capital projects require Certificate of Public Convenience and Necessity approval from state regulators. Tracking which CPCN application authorized each project is essential for regulatory compl',
    `distribution_substation_id` BIGINT COMMENT 'Foreign key linking to distribution.distribution_substation. Business justification: Substation capital projects (transformer additions, bus upgrades, protection modernization, capacity expansion) are major utility investments requiring detailed tracking for FERC/PUC reporting, RAB in',
    `feeder_id` BIGINT COMMENT 'Foreign key linking to distribution.feeder. Business justification: Capital projects for feeder upgrades (reconductoring, capacity expansion, automation, undergrounding) require feeder attribution for investment planning, rate case justification, RAB tracking, and rel',
    ...
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supplier_contract. Business justification: Large capital projects often use frame contracts for multi-year procurement of standardized equipment (transformers, poles, conductors). Links project to master supply agreement for pricing validation',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Capital projects specify primary equipment being installed (transformer model, circuit breaker type) for asset registry creation and depreciation setup. Links project to material master for standardiz',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Capital projects allocate to profit centers for business unit P&L tracking, rate base segmentation by service territory, and regulatory jurisdiction reporting. New FK needed for financial consolidatio',
    `employee_id` BIGINT COMMENT 'Employee identifier for the project manager, linking to the workforce management system.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` (
    ...
    `asset_class_id` BIGINT COMMENT 'FK to asset.asset_class.asset_class_id — Depreciation methods and useful life defaults are driven by asset class. This FK enables class-level depreciation policy inheritance.',
    `asset_registry_id` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Depreciation schedules are defined per asset. Without this FK, depreciation charges cannot be allocated to specific assets for RAB valuation and financial reporting.',
    `class_id` BIGINT COMMENT 'Foreign key linking to asset.asset_class. Business justification: depreciation_schedule has both asset_registry_id (for asset-specific schedules) and asset_class_code (STRING) for class-level depreciation schedules. The asset_class_code should be normalized to FK as',
    `employee_id` BIGINT COMMENT 'User ID of the person or system account that created this depreciation schedule record.',
    `last_modified_by_user_employee_id` BIGINT COMMENT 'User ID of the person or system account that last modified this depreciation schedule record.',
    `registry_id` BIGINT COMMENT 'Reference to the utility asset (generation unit, transformer, transmission tower, distribution pole, switchgear, protection relay, substation equipment, fleet vehicle, or facility) to which this depreciation schedule applies.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` (
    ...
    `asset_class_id` BIGINT COMMENT 'FK to asset.asset_class.asset_class_id — Spare parts are stocked for specific asset classes (e.g., bushings for 132kV transformers). This FK enables parts applicability lookup during work order planning.',
    `class_id` BIGINT COMMENT 'Foreign key linking to asset.asset_class. Business justification: spare_parts_inventory has asset_class_applicability (STRING) which appears to store which asset class(es) the spare part is applicable to. This should be normalized to FK asset_class_id → asset_class.',
    ...
    `employee_id` BIGINT COMMENT 'System user identifier of the person who created the spare parts inventory record in the enterprise asset management system.',
    ...
    `spare_part_applicability` BIGINT COMMENT 'FK to asset.asset_class.asset_class_id — Spare parts are stocked for specific asset classes. This FK enables identification of which parts serve which equipment types for inventory optimization.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`inspection` (
    ...
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: Regular inspections of DER installations for safety compliance, performance verification, and interconnection agreement requirements. Mandated by most jurisdictions for NEM programs, feed-in tariffs, ',
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the supervisor or engineer who reviewed and approved the inspection findings and recommendations.',
    `inspection_employee_id` BIGINT COMMENT 'Identifier of the employee, contractor, or assessor who performed the inspection.',
    ...
    `work_order_id` BIGINT COMMENT 'Identifier of the work order in Ventyx Asset Suite that authorized or scheduled this inspection activity.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`valuation` (
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the user who approved this valuation for regulatory submission or financial reporting. Provides audit trail for governance.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`retirement` (
    `retirement_id` BIGINT COMMENT 'Unique identifier for the asset retirement event record. Primary key for the asset retirement data product.',
    `capital_project_id` BIGINT COMMENT 'Reference to the capital project under which the asset retirement was executed, if part of a planned capital program. Links to capital project management for CAPEX tracking and program reporting.',
    `vendor_id` BIGINT COMMENT 'Reference to the vendor or contractor who performed the asset disposal or environmental remediation services. Links to vendor master for procurement and payment tracking.',
    `transformer_id` BIGINT COMMENT 'Foreign key linking to distribution.transformer. Business justification: Transformer retirements must be tracked for RAB removal, gain/loss on disposal accounting, salvage value recovery, environmental compliance (PCB disposal), and replacement asset linkage. Required for ',
    `environmental_permit_id` BIGINT COMMENT 'Foreign key linking to compliance.environmental_permit. Business justification: Asset retirements must comply with environmental permits governing decommissioning, hazardous material disposal, and site remediation. Tracking which permit governs each retirement ensures environment',
    `feeder_id` BIGINT COMMENT 'Foreign key linking to distribution.feeder. Business justification: Feeder retirements (decommissioning, reconfiguration, voltage conversion) require tracking for RAB removal, regulatory accounting (FERC Form 1), salvage value recovery, and system planning updates. Es',
    `irp_scenario_id` BIGINT COMMENT 'Foreign key linking to forecast.irp_scenario. Business justification: Asset retirements are planned within IRP scenarios. Retirement schedules are key IRP inputs and outputs. Tracking which IRP scenario drove a retirement decision is required for regulatory filings, rat',
    `employee_id` BIGINT COMMENT 'Reference to the user who approved the asset retirement transaction. Used for audit trail and SOX compliance.',
    `retirement_employee_id` BIGINT COMMENT 'Reference to the user who initiated the asset retirement transaction. Used for audit trail and accountability.',
    `registry_id` BIGINT COMMENT 'Reference to the asset being retired. Links to the master asset registry for generation units, transmission towers, distribution poles, transformers, switchgear, protection relays, substation equipment, fleet vehicles, and facilities.',
    `retirement_replacement_asset_asset_registry_id` BIGINT COMMENT 'Reference to the new asset that replaced the retired asset, if applicable. Links to the master asset registry for tracking asset replacement programs and capital project execution.',
    `work_order_id` BIGINT COMMENT 'Reference to the work order that executed the asset retirement and decommissioning activities. Links to Ventyx Asset Suite work management for labor, materials, and cost tracking.',
    `approval_date` DATE COMMENT 'The date on which the asset retirement was approved by management or regulatory authority. Part of the retirement lifecycle audit trail.',
    `asset_class_code` STRING COMMENT 'FERC or utility-specific asset class code for the retired asset. Used for financial reporting, depreciation studies, and regulatory rate case filings.',
    `cost_center_code` STRING COMMENT 'The cost center to which retirement and decommissioning costs are charged. Used for Operations and Maintenance (O&M) cost tracking and financial reporting.',
    `created_timestamp` TIMESTAMP COMMENT 'The timestamp when this asset retirement record was first created in the system. Part of the audit trail for data lineage and compliance.',
    `decommissioning_cost` DECIMAL(18,2) COMMENT 'The total cost incurred to decommission and remove the asset from service, including labor, equipment, environmental remediation, and disposal fees. Expressed in the utilitys functional currency.',
    `depreciation_reserve_released` DECIMAL(18,2) COMMENT 'The accumulated depreciation reserve amount released upon asset retirement. Used for financial reconciliation and regulatory reporting.',
    `disposal_date` DATE COMMENT 'The date on which the physical disposal or transfer of the retired asset was completed. May differ from retirement date if asset remained in inventory or storage after retirement from service.',
    `disposal_method` STRING COMMENT 'The method by which the retired asset was disposed of or removed from inventory. Values include scrap (recycled or scrapped for materials), sale (sold to third party), transfer (transferred to another entity or business unit), donation (donated to external organization), write-off (financial write-off with no physical disposal), environmental disposal (specialized disposal for hazardous materials such as PCB transformers or asbestos-containing equipment).. Valid values are `scrap|sale|transfer|donation|write_off|environmental_disposal`',
    `environmental_compliance_flag` BOOLEAN COMMENT 'Boolean indicator of whether the asset retirement and disposal process complied with all applicable environmental regulations. True indicates full compliance with EPA standards for hazardous material disposal (e.g., PCB transformers, asbestos-containing equipment, SF6 gas handling).',
    `environmental_disposal_permit_number` STRING COMMENT 'Reference number for the environmental disposal permit or certification obtained for hazardous material disposal. Required for assets containing PCBs, asbestos, lead, or other regulated substances.',
    `functional_location_code` STRING COMMENT 'The functional location or network position where the retired asset was installed. Used for geographic and network topology analysis of retirement patterns.',
    `gain_loss_on_disposal` DECIMAL(18,2) COMMENT 'The calculated financial gain or loss on asset disposal, computed as salvage value realized minus net book value at retirement minus decommissioning cost. Positive values indicate gain, negative values indicate loss.',
    `gl_account_code` STRING COMMENT 'The general ledger account code to which the retirement transaction is posted. Used for financial accounting and FERC Uniform System of Accounts compliance.',
    `insurance_claim_reference` STRING COMMENT 'Reference number for any insurance claim filed related to the asset retirement, particularly for catastrophic failure or damage events. Used for insurance recovery tracking.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'The timestamp when this asset retirement record was last modified. Part of the audit trail for data lineage and compliance.',
    `net_book_value_at_retirement` DECIMAL(18,2) COMMENT 'The net book value of the asset at the time of retirement, calculated as original cost less accumulated depreciation. Used to calculate gain or loss on disposal and for regulatory asset base (RAB) adjustments.',
    `notes` STRING COMMENT 'Free-form text field for additional notes, comments, or special circumstances related to the asset retirement. Used for capturing context not covered by structured fields.',
    `rab_removal_amount` DECIMAL(18,2) COMMENT 'The monetary amount removed from the regulatory asset base upon asset retirement. Used for rate case filings and regulatory reporting to state Public Utility Commissions (PUCs) and FERC.',
    `rab_removal_date` DATE COMMENT 'The date on which the asset was removed from the regulatory asset base for rate-making purposes. Critical for regulatory compliance and revenue requirement calculations. May differ from retirement date based on regulatory approval timing.',
    `reason_code` STRING COMMENT 'Categorical code indicating the primary reason for asset retirement. Values include end of life (asset reached expected useful life), catastrophic failure (unrecoverable damage or failure), obsolescence (technology or standards obsolete), regulatory mandate (compliance requirement), network reconfiguration (grid modernization or topology change), replacement (planned upgrade or substitution), performance degradation (no longer meets operational standards). [ENUM-REF-CANDIDATE: end_of_life|catastrophic_failure|obsolescence|regulatory_mandate|network_reconfiguration|replacement|performance_degradation — 7 candidates stripped; promote to reference product]',
    `reason_description` STRING COMMENT 'Detailed narrative explanation of the specific circumstances and business justification for the asset retirement. Provides context beyond the categorical reason code for audit, regulatory reporting, and asset management analysis.',
    `regulatory_approval_date` DATE COMMENT 'The date on which regulatory approval for asset retirement was granted by FERC, state PUC, or other governing body. Null if regulatory approval was not required.',
    `regulatory_approval_required_flag` BOOLEAN COMMENT 'Boolean indicator of whether regulatory approval was required prior to asset retirement. True for assets subject to Certificate of Public Convenience and Necessity (CPCN) or other regulatory oversight.',
    `regulatory_notification_reference` STRING COMMENT 'Reference number or identifier for the regulatory notification or filing submitted to FERC, state PUC, or other governing body regarding the asset retirement. Required for major transmission and generation asset retirements.',
    `retirement_category` STRING COMMENT 'High-level categorization of the retired asset by utility functional area. Used for portfolio analysis and regulatory reporting segmentation.. Valid values are `generation|transmission|distribution|substation|fleet|facility`',
    `retirement_date` DATE COMMENT 'The date on which the asset was officially retired from service. This is the principal business event timestamp for the retirement transaction and marks the end of the assets operational lifecycle.',
    `retirement_number` STRING COMMENT 'Business identifier for the retirement transaction. Externally-known reference number used in regulatory filings, financial reporting, and audit trails.',
    `retirement_status` STRING COMMENT 'Current lifecycle status of the retirement transaction. Values include planned (scheduled for future retirement), in progress (decommissioning underway), completed (retirement and disposal finalized), cancelled (retirement cancelled), pending approval (awaiting regulatory or management approval).. Valid values are `planned|in_progress|completed|cancelled|pending_approval`',
    `salvage_value_realized` DECIMAL(18,2) COMMENT 'The actual monetary value recovered from the disposal of the retired asset through sale, scrap, or other means. Expressed in the utilitys functional currency. Used for financial reconciliation and variance analysis against estimated salvage value.',
    `source_system_code` STRING COMMENT 'Code identifying the source system from which the retirement record originated. Primary sources are SAP S/4HANA FI-AA for financial asset retirement and Ventyx Asset Suite for operational retirement tracking.. Valid values are `SAP_S4HANA|VENTYX|MANUAL`',
    `source_system_record_code` STRING COMMENT 'The unique identifier of this retirement record in the source system. Used for data lineage, reconciliation, and troubleshooting.',
    CONSTRAINT pk_retirement PRIMARY KEY(`retirement_id`)
) COMMENT 'Record of asset decommissioning, disposal, or retirement events. Captures retirement date, retirement reason (end of life, failure, regulatory, network reconfiguration, replacement), disposal method (scrap, sale, transfer, write-off), salvage value realised, environmental disposal compliance flag (e.g., PCB transformer disposal, asbestos), decommissioning cost, RAB removal date, and regulatory notification reference. Sourced from SAP S/4HANA FI-AA asset retirement and Ventyx Asset Suite.';
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`warranty` (
    ...
    `vendor_id` BIGINT COMMENT 'Foreign key linking to supply.vendor. Business justification: Warranties are contractual agreements with vendors (OEMs or third-party providers). Formalizes manufacturer_name into vendor master for warranty claim processing, vendor performance evaluation, and tr',
    ...
    `employee_id` BIGINT COMMENT 'Identifier of the user who created the warranty record in the system. Used for audit trail and data governance.',
    `warranty_last_modified_by_user_employee_id` BIGINT COMMENT 'Identifier of the user who last modified the warranty record. Used for audit trail and change tracking.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` (
    `asset_risk_assessment_id` BIGINT COMMENT 'Unique identifier for the asset risk assessment record.',
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Risk assessments determine asset replacement priorities and capital investment decisions worth millions. Regulatory prudency reviews require documented qualified assessor credentials. This link provid',
    `asset_registry_id` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Risk assessments evaluate specific assets. This FK is essential for risk-based maintenance prioritization and IRP investment justification.',
    `obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.obligation. Business justification: Risk assessments evaluate compliance with regulatory standards (NERC TPL transmission planning, distribution reliability requirements, FERC dam safety). Linking assessments to obligations enables comp',
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: Risk scoring for DER assets considering technology obsolescence, performance degradation rates, grid integration challenges, and stranded asset risk. Supports investment planning, rate case prudency d',
    `irp_scenario_id` BIGINT COMMENT 'Foreign key linking to forecast.irp_scenario. Business justification: Risk assessments inform IRP scenario development. High-risk assets identified in assessments feed into IRP capacity expansion and replacement plans. Essential for risk-informed integrated resource pla',
    `registry_id` BIGINT COMMENT 'Reference to the utility asset being assessed in the asset registry.',
    `assessment_confidence_level` STRING COMMENT 'Confidence level in the risk assessment results based on data quality, methodology rigor, and subject matter expert validation.. Valid values are `high|medium|low`',
    `assessment_date` DATE COMMENT 'Date when the risk assessment was performed.',
    `assessment_notes` STRING COMMENT 'Free-text notes and observations from the risk assessment including assumptions, limitations, and special considerations.',
    `assessment_status` STRING COMMENT 'Current lifecycle status of the risk assessment record.. Valid values are `draft|submitted|under_review|approved|superseded|archived`',
    `assessment_type` STRING COMMENT 'Type of risk assessment performed indicating the trigger or purpose of the assessment.. Valid values are `initial|periodic|condition-triggered|event-triggered|pre-investment|post-incident`',
    `benefit_cost_ratio` DECIMAL(18,2) COMMENT 'Cost-benefit ratio calculated as risk reduction benefit divided by intervention cost estimate, used for investment justification and IRP (Integrated Resource Plan) prioritization.',
    `consequence_of_failure_rating` STRING COMMENT 'Qualitative consequence of failure rating band derived from the aggregate CoF score.. Valid values are `negligible|minor|moderate|major|severe|catastrophic`',
    `consequence_of_failure_score` DECIMAL(18,2) COMMENT 'Aggregate consequence of failure score combining safety, environmental, reliability, financial, and regulatory impact categories.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this risk assessment record was first created in the lakehouse.',
    `data_quality_score` DECIMAL(18,2) COMMENT 'Quality score for the input data used in the risk assessment including asset condition data, failure history, loading data, and environmental factors. Scale 0-100.',
    `environmental_consequence_score` DECIMAL(18,2) COMMENT 'Impact score for environmental consequences of asset failure including emissions, spills, contamination, and EPA violations.',
    `financial_consequence_score` DECIMAL(18,2) COMMENT 'Impact score for direct financial consequences including replacement cost, emergency repair cost, lost revenue, and regulatory penalties.',
    `intervention_cost_estimate` DECIMAL(18,2) COMMENT 'Estimated cost in USD to execute the recommended intervention including materials, labor, outage costs, and project management.',
    `intervention_priority` STRING COMMENT 'Priority level for executing the recommended intervention based on risk-informed maintenance planning.. Valid values are `immediate|urgent|high|medium|low|deferred`',
    `irp_inclusion_flag` BOOLEAN COMMENT 'Indicates whether this risk assessment and recommended intervention are included in the utilitys Integrated Resource Plan for PUC review and approval.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this risk assessment record was last modified in the lakehouse.',
    `nerc_compliance_flag` BOOLEAN COMMENT 'Indicates whether this risk assessment is required for NERC CIP (Critical Infrastructure Protection) compliance or other NERC reliability standards.',
    `net_present_value` DECIMAL(18,2) COMMENT 'Net present value in USD of the recommended intervention considering risk reduction benefits, intervention costs, and discount rate over the planning horizon.',
    `next_assessment_due_date` DATE COMMENT 'Scheduled date for the next periodic risk assessment based on asset criticality and risk band.',
    `overall_risk_score` DECIMAL(18,2) COMMENT 'Composite risk score calculated as the product or weighted combination of probability of failure and consequence of failure scores per the selected risk methodology.',
    `payback_period_years` DECIMAL(18,2) COMMENT 'Estimated payback period in years for the intervention investment based on annual risk reduction benefits.',
    `probability_of_failure_rating` STRING COMMENT 'Qualitative probability of failure rating band derived from the quantitative PoF score.. Valid values are `very_low|low|medium|high|very_high|critical`',
    `probability_of_failure_score` DECIMAL(18,2) COMMENT 'Quantitative probability of failure score based on asset age, condition, loading, and environmental factors. Scale typically 0-100 or 0-1 depending on methodology.',
    `puc_prudency_evidence_flag` BOOLEAN COMMENT 'Indicates whether this risk assessment serves as prudency review evidence for capital investment justification to the Public Utility Commission.',
    `rab_impact_flag` BOOLEAN COMMENT 'Indicates whether the recommended intervention will impact the Regulatory Asset Base valuation and rate base calculations.',
    `recommended_intervention` STRING COMMENT 'Recommended risk mitigation intervention strategy based on the risk assessment results. [ENUM-REF-CANDIDATE: replace|refurbish|upgrade|increase_monitoring|condition_assessment|accept_risk|no_action — 7 candidates stripped; promote to reference product]',
    `regulatory_consequence_score` DECIMAL(18,2) COMMENT 'Impact score for regulatory and compliance consequences including NERC CIP violations, FERC penalties, PUC sanctions, and service level agreement breaches.',
    `reliability_consequence_score` DECIMAL(18,2) COMMENT 'Impact score for system reliability consequences measured by customer interruptions, SAIDI (System Average Interruption Duration Index), SAIFI (System Average Interruption Frequency Index), and load at risk.',
    `review_date` DATE COMMENT 'Date when the risk assessment was reviewed and approved.',
    `reviewer_name` STRING COMMENT 'Name of the senior engineer or manager who reviewed and approved the risk assessment.',
    `risk_assessment_asset` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Risk assessments evaluate specific assets. The FK is required for risk-based maintenance prioritization and investment planning.',
    `risk_assessment_to_asset` BIGINT COMMENT 'FK to asset.asset_registry.asset_registry_id — Risk assessments are performed on specific assets. The FK enables risk-ranked asset registers and investment prioritization for IRP submissions.',
    `risk_band` STRING COMMENT 'Overall risk classification band derived from the overall risk score used for prioritization and investment planning.. Valid values are `critical|high|medium|low|negligible`',
    `risk_methodology` STRING COMMENT 'Industry-standard risk assessment methodology applied (risk matrix, Monte Carlo simulation, Weibull analysis, Fault Tree Analysis, Bow-Tie, Failure Mode and Effects Analysis).. Valid values are `risk_matrix|monte_carlo|weibull_analysis|fault_tree|bow_tie|fmea`',
    `risk_rank` STRING COMMENT 'Relative ranking of this asset risk within the asset portfolio or asset class for prioritization purposes.',
    `risk_reduction_benefit` DECIMAL(18,2) COMMENT 'Estimated monetary benefit in USD from risk reduction achieved by the recommended intervention, calculated as avoided consequence cost weighted by probability reduction.',
    `safety_consequence_score` DECIMAL(18,2) COMMENT 'Impact score for public and worker safety consequences of asset failure including potential injuries, fatalities, and OSHA reportable incidents.',
    `source_system_code` STRING COMMENT 'Code identifying the source system that generated this risk assessment record (e.g., Ventyx Asset Suite Risk module, enterprise risk management system).',
    `source_system_record_code` STRING COMMENT 'Unique identifier of this risk assessment record in the source system for traceability and reconciliation.',
    CONSTRAINT pk_asset_risk_assessment PRIMARY KEY(`asset_risk_assessment_id`)
) COMMENT 'Risk-based assessment record for utility assets evaluating probability of failure (PoF), consequence of failure (CoF), and overall asset risk score using industry-standard methodologies. Captures assessment date, risk methodology (risk matrix, Monte Carlo simulation, Weibull analysis), probability of failure rating based on age, condition, loading, and environment, consequence categories (safety, environment, reliability/SAIDI-SAIFI impact, financial, regulatory/compliance), overall risk score, risk band (critical, high, medium, low), recommended intervention (replace, refurbish, increase monitoring, accept risk), cost-benefit analysis of intervention, and risk-informed maintenance priority ranking. Supports risk-based asset management (RBAM) per ISO 55000, IRP investment justification, and PUC prudency review evidence. Sourced from Ventyx Asset Suite Risk module and enterprise risk management systems.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` (
    `work_order_material_issue_id` BIGINT COMMENT 'Unique identifier for the material issue transaction record. Primary key for the association.',
    `employee_id` BIGINT COMMENT 'Reference to the storeroom clerk, technician, or employee who physically issued the material from inventory. Used for accountability and audit purposes.',
    `spare_parts_inventory_id` BIGINT COMMENT 'Foreign key linking to the spare parts inventory item that was issued. Identifies which material master record was consumed.',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to the work order that consumed the material. Identifies which maintenance activity the spare part was issued against.',
    `cost_center_charged` STRING COMMENT 'Financial cost center code to which this material consumption cost is allocated. May differ from the work order cost center if material is charged to a different organizational unit.',
    `gl_account_code` STRING COMMENT 'General ledger account code for financial posting of the material cost. Aligns with chart of accounts for O&M expense or capital project accounting.',
    `issue_date` DATE COMMENT 'Calendar date when the material was issued from inventory to the work order. Used for inventory transaction history and cost period allocation.',
    `issue_timestamp` TIMESTAMP COMMENT 'Precise date and time when the material issue transaction was recorded in the EAM system. Provides audit trail and chronological sequencing of material movements.',
    `material_document_number` STRING COMMENT 'Unique document identifier for the material issue transaction in the EAM or ERP system (e.g., SAP Material Document, Maximo Issue Record). Provides audit trail and cross-system reconciliation.',
    `quantity_issued` DECIMAL(18,2) COMMENT 'Quantity of the spare part or material issued from inventory to the work order. Measured in the unit of measure defined in the spare parts inventory record. This is the primary consumption metric.',
    `quantity_returned` DECIMAL(18,2) COMMENT 'Quantity of the spare part or material returned to inventory after work order completion (unused or excess material). Used to adjust actual consumption and inventory levels.',
    `reservation_number` STRING COMMENT 'Material reservation or requisition number that pre-allocated this inventory item to the work order. Links the issue transaction to the planning phase reservation record.',
    `return_date` DATE COMMENT 'Calendar date when unused or excess material was returned to inventory from the work order. Null if no return occurred.',
    `unit_cost_at_issue` DECIMAL(18,2) COMMENT 'Standard or moving average cost per unit at the time the material was issued. Captures the cost basis for work order cost allocation, which may differ from current inventory unit cost due to price changes over time.',
    `work_order_line_number` STRING COMMENT 'Line item sequence number within the work order material list. Used for organizing and referencing multiple material issues on the same work order.',
    CONSTRAINT pk_work_order_material_issue PRIMARY KEY(`work_order_material_issue_id`)
) COMMENT 'This association product represents the Material Issue Transaction between work_order and spare_parts_inventory. It captures the consumption of spare parts, materials, and consumables against maintenance work orders in utility asset management operations. Each record links one work order to one spare part inventory item with transactional attributes including quantities issued and returned, costs at time of issue, timestamps, material document references, and cost allocation codes. This is a core operational entity in EAM/CMMS systems (Ventyx Asset Suite, SAP PM, Maximo) that enables material cost tracking, inventory consumption analysis, and work order cost rollup.. Existence Justification: In utility asset management operations, maintenance work orders consume multiple spare parts and materials (transformers, insulators, cables, tools, consumables), and each spare part in inventory is issued to multiple work orders over its lifecycle. Material issue transactions are actively created by storeroom clerks and technicians during work execution, with each transaction recording quantities, costs, timestamps, and document references. This is a recognized operational entity in EAM systems called Material Issue, Parts Consumption, or Material Document.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` (
    `asset_program_enrollment_id` BIGINT COMMENT 'Primary key for asset_program_enrollment',
    `dr_program_id` BIGINT COMMENT 'Foreign key linking to the demand response program',
    `program_id` BIGINT COMMENT 'Unique identifier for this asset-program enrollment record. Primary key.',
    `registry_id` BIGINT COMMENT 'Foreign key linking to the enrolled asset in the asset registry',
    `average_performance_percentage` DECIMAL(18,2) COMMENT 'Rolling average of actual load reduction achieved versus committed enrolled capacity across all dispatch events for this asset-program combination. Used for performance tier assignment.',
    `baseline_methodology` STRING COMMENT 'The specific baseline calculation methodology applied to this asset for this program. May differ from program default if asset has unique operational characteristics requiring customized M&V approach.',
    `cumulative_dispatch_hours_ytd` DECIMAL(18,2) COMMENT 'Year-to-date cumulative hours this asset has been dispatched under this program. Used to monitor compliance with max_event_hours_per_year program limits.',
    `enrolled_capacity_kw` DECIMAL(18,2) COMMENT 'The committed load reduction capacity in kilowatts for this specific asset under this DR program. Aggregated across all asset enrollments to calculate program-level enrolled_capacity_actual_mw.',
    `enrollment_date` DATE COMMENT 'Date when this specific asset was enrolled into this DR program. Used for eligibility calculations and incentive accrual start date.',
    `enrollment_end_date` DATE COMMENT 'Date when this assets enrollment in this DR program ended or is scheduled to end. Null for active ongoing enrollments.',
    `incentive_rate` DECIMAL(18,2) COMMENT 'The specific incentive payment rate ($/kW or $/kWh) applicable to this asset for this program. May vary from program base rate based on performance tier, asset criticality, or negotiated terms.',
    `last_dispatch_date` DATE COMMENT 'Date of the most recent DR event dispatch where this asset was called to reduce load under this program. Used for performance tracking and compliance monitoring.',
    `participation_status` STRING COMMENT 'Current status of this assets participation in this DR program. Active indicates asset is eligible for dispatch. Suspended indicates temporary exclusion. Withdrawn indicates permanent removal. Pending indicates enrollment awaiting approval.',
    `performance_tier` STRING COMMENT 'Performance classification for this asset within this program based on historical dispatch compliance and load reduction achievement. Used to determine incentive rates and dispatch priority.',
    `withdrawal_reason` STRING COMMENT 'Business reason for asset withdrawal from program (e.g., asset retired, performance non-compliance, program requirements changed, operational constraints). Populated when participation_status is Withdrawn.',
    CONSTRAINT pk_asset_program_enrollment PRIMARY KEY(`asset_program_enrollment_id`)
) COMMENT 'This association product represents the enrollment relationship between physical utility assets and demand response programs. It captures the operational participation of specific assets (substations, feeders, transformers) in DR programs, including capacity commitments, performance tracking, and settlement parameters. Each record links one asset to one DR program with attributes that exist only in the context of this enrollment relationship.. Existence Justification: In energy utility operations, physical distribution assets (substations, feeders, transformers) are enrolled in multiple demand response programs simultaneously to serve different grid reliability and market participation objectives. A single substation may participate in ISO capacity markets, emergency DR programs, economic dispatch programs, and voltage support programs concurrently. Each enrollment has distinct capacity commitments, performance metrics, baseline methodologies, and settlement rates that are managed independently by the demand response operations team.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` (
    `asset_compliance_id` BIGINT COMMENT 'Primary key for asset_compliance',
    `compliance_obligation_id` BIGINT COMMENT 'Unique identifier for this asset-obligation compliance tracking record. Primary key.',
    `obligation_id` BIGINT COMMENT 'Foreign key linking to the regulatory compliance obligation being verified',
    `registry_id` BIGINT COMMENT 'Foreign key linking to the physical utility asset being tracked for compliance',
    `corrective_action_plan_id` BIGINT COMMENT 'Reference to the remediation or mitigation plan if this asset-obligation pair is in non-compliance or conditional status.',
    `applicability_determination_date` DATE COMMENT 'Date when it was determined that this obligation applies to this asset, based on asset characteristics (voltage class, criticality, location) and obligation scope.',
    `compliance_status` STRING COMMENT 'Current compliance status of this specific asset with respect to this specific obligation. Explicitly identified in detection reasoning as relationship data.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this asset-obligation compliance tracking record was created in the system.',
    `evidence_reference` STRING COMMENT 'Reference identifier or location of compliance evidence documentation for this asset-obligation pair (e.g., inspection report ID, test result document, audit file reference). Explicitly identified in detection reasoning as relationship data.',
    `last_audit_finding` STRING COMMENT 'Most recent audit finding specific to this assets compliance with this obligation (pass, minor-finding, major-finding, observation).',
    `last_compliance_verification_date` DATE COMMENT 'Date when compliance of this asset with this obligation was most recently verified through inspection, testing, or audit. Explicitly identified in detection reasoning as relationship data.',
    `next_compliance_due_date` DATE COMMENT 'Next scheduled date by which this asset must be verified for compliance with this obligation, based on the obligations compliance frequency and asset-specific factors. Explicitly identified in detection reasoning as relationship data.',
    `non_compliance_risk_score` STRING COMMENT 'Calculated risk score for this asset-obligation pair based on asset criticality, obligation VRF/VSL, and current compliance status. Used for audit prioritization.',
    `responsible_party` STRING COMMENT 'Name or identifier of the individual or organizational unit responsible for ensuring this assets compliance with this obligation. Explicitly identified in detection reasoning as relationship data.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this compliance record was last updated, reflecting changes in status, verification dates, or responsible parties.',
    CONSTRAINT pk_asset_compliance PRIMARY KEY(`asset_compliance_id`)
) COMMENT 'This association product represents the compliance tracking relationship between physical utility assets and regulatory obligations. It captures the compliance status, verification history, and evidence management for each asset-obligation pair. Each record links one asset to one regulatory obligation with attributes that track compliance verification dates, responsible parties, and audit evidence specific to that assets compliance with that particular obligation.. Existence Justification: In energy utilities operations, individual physical assets (transformers, transmission towers, substations, protection relays) are each subject to multiple regulatory obligations from different governing bodies (NERC CIP cybersecurity standards, EPA emissions limits, OSHA safety mandates, FERC reliability requirements). Simultaneously, each regulatory obligation applies to many assets across the utilitys asset base based on asset characteristics like voltage class, criticality score, and operational function. Compliance teams actively manage this many-to-many relationship by tracking verification dates, collecting evidence, assigning responsible parties, and preparing for audits on each asset-obligation pair.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` (
    `ppa_allocation_id` BIGINT COMMENT 'Unique identifier for this asset-PPA allocation record. Primary key.',
    `registry_id` BIGINT COMMENT 'Foreign key linking to the physical generation asset being allocated to this PPA contract',
    `ppa_contract_id` BIGINT COMMENT 'Foreign key linking to the renewable PPA contract receiving this asset allocation',
    `meter_id` BIGINT COMMENT 'Identifier of the revenue-quality meter used to measure generation from this asset for settlement under this PPA contract. Critical for multi-PPA scenarios where different meters or meter channels track output allocated to different contracts.',
    `allocation_notes` STRING COMMENT 'Free-text field capturing business context for this allocation, such as contract amendment references, reallocation justifications, or special settlement terms applicable to this asset-PPA pairing.',
    `allocation_percentage` DECIMAL(18,2) COMMENT 'Percentage of the assets generation capacity or output allocated to this specific PPA contract. Allows partial allocation where a single asset serves multiple contracts or where portfolio PPAs aggregate multiple assets. Sum of allocation percentages across all PPAs for a given asset may equal or be less than 100%.',
    `allocation_status` STRING COMMENT 'Current operational status of this asset-PPA allocation: active (currently generating and settling), pending (approved but not yet effective), suspended (temporarily halted due to maintenance or curtailment), terminated (ended early), expired (reached contract_end_date). Drives settlement processing and REC transfer eligibility.',
    `contract_end_date` DATE COMMENT 'Effective end date when this assets allocation to this PPA contract terminates. May differ from the PPA contracts overall effective_end_date if assets are removed from the contract early due to retirement, reallocation, or contract amendment.',
    `contract_start_date` DATE COMMENT 'Effective start date when this specific asset began contributing generation to this PPA contract. May differ from the PPA contracts overall effective_start_date if assets are added to the contract over time or if the asset was commissioned after contract inception.',
    `contracted_capacity_mw` DECIMAL(18,2) COMMENT 'Portion of the assets rated capacity (in MW) that is contractually committed to this specific PPA. For portfolio PPAs, this represents the assets contribution to the total contracted capacity. Used for capacity payment calculations and contract compliance tracking.',
    `contracted_price_per_mwh` DECIMAL(18,2) COMMENT 'Energy price in USD per MWh applicable to this specific asset-PPA pairing. May differ from the PPAs base_price_per_mwh if the contract has asset-specific pricing tiers, vintage-based pricing, or technology-specific rates. Used for settlement calculations.',
    `rec_allocation_flag` BOOLEAN COMMENT 'Indicates whether Renewable Energy Certificates (RECs) generated by this asset under this allocation are transferred to the utility per the PPAs rec_ownership terms. Supports scenarios where energy and RECs are sold separately or where only a portion of an assets RECs are allocated to a specific PPA.',
    CONSTRAINT pk_ppa_allocation PRIMARY KEY(`ppa_allocation_id`)
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` (
    `impact_assessment_id` BIGINT COMMENT 'Primary key for the asset_impact_assessment association',
    `employee_id` BIGINT COMMENT 'Reference to the utility engineer who performed the technical impact analysis for this asset-application combination. Used for accountability, quality assurance, and workload tracking.',
    `cluster_study_group_id` BIGINT COMMENT 'Reference to the cluster study group that includes this asset impact assessment, if applicable. Under FERC Order 2023, multiple interconnection applications that impact the same assets may be grouped into cluster studies for coordinated analysis and cost allocation.',
    `application_id` BIGINT COMMENT 'Foreign key linking to the interconnection application being reviewed',
    `registry_id` BIGINT COMMENT 'Foreign key linking to the utility asset being impacted by the interconnection application',
    `affected_capacity_kw` DECIMAL(18,2) COMMENT 'Amount of asset capacity in kilowatts that is affected or consumed by the proposed DER interconnection. Used to determine hosting capacity impacts and whether the asset has sufficient remaining capacity for additional interconnection applications in the queue.',
    `analysis_date` DATE COMMENT 'Date when the technical impact analysis for this asset-application combination was performed by the utility review engineer. Used to track review timeline compliance with regulatory deadlines and to determine whether re-analysis is required if grid conditions change.',
    `analysis_status` STRING COMMENT 'Current status of the impact analysis for this asset-application combination. Tracks progression through the technical review workflow and identifies when supplemental studies are required.',
    `estimated_cost` DECIMAL(18,2) COMMENT 'Estimated cost in USD to upgrade or replace this asset to accommodate the proposed DER interconnection. Used for cost allocation across applications in cluster studies and to determine applicant financial responsibility under interconnection agreements.',
    `impact_severity` STRING COMMENT 'Engineering assessment of the severity of impact the proposed DER has on this asset. Determines review tier escalation, supplemental study requirements, and priority for upgrade planning.',
    `impact_type` STRING COMMENT 'Category of technical impact the proposed DER has on this specific asset (thermal loading, voltage regulation, protection coordination, fault current contribution, stability, power quality). Determines the type of engineering analysis required and potential mitigation measures.',
    `mitigation_required` STRING COMMENT 'Description of mitigation measures required to address the impact on this asset, such as protection relay upgrades, voltage regulator adjustments, transformer replacement, or operational restrictions on DER output.',
    `upgrade_required_flag` BOOLEAN COMMENT 'Indicates whether this asset requires upgrade or replacement to accommodate the proposed DER interconnection. Drives cost allocation and determines whether the application triggers network upgrade costs that may be shared across multiple applications in a cluster study.',
    CONSTRAINT pk_impact_assessment PRIMARY KEY(`impact_assessment_id`)
) COMMENT 'This association product represents the technical impact assessment between a utility asset and an interconnection application. It captures the engineering analysis of how a proposed distributed energy resource (DER) interconnection affects specific grid assets, including thermal loading, voltage regulation, protection coordination, and required upgrades. Each record links one asset to one interconnection application with impact-specific attributes that exist only in the context of this relationship. This is the foundation for cluster study formation, cost allocation, and queue management under FERC Order 2023 and state interconnection rules.. Existence Justification: In utility interconnection operations, a single interconnection application routinely impacts multiple grid assets (transformer, feeder, substation, protection devices) that must be analyzed for thermal, voltage, and protection impacts. Conversely, a single asset can be affected by multiple pending interconnection applications in the queue, requiring coordinated analysis to determine cumulative impacts and shared upgrade costs. Utilities actively manage these asset-application impact assessments as part of the technical review process, tracking impact type, severity, upgrade requirements, and cost allocation for each asset-application combination.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`der_connection` (
    `der_connection_id` BIGINT COMMENT 'Unique identifier for this specific connection between a utility asset and a DER system. Primary key for the association.',
    `der_system_id` BIGINT COMMENT 'Foreign key linking to the distributed energy resource system that is physically connected to the utility asset',
    `registry_id` BIGINT COMMENT 'Foreign key linking to the utility asset (transformer, meter, disconnect switch, protection relay) that serves as a connection point for the DER system',
    `commissioning_date` DATE COMMENT 'Date when this specific DER-asset connection was commissioned and placed into operational service. Marks the point when the physical connection was energized and the DER system began operating in coordination with the utility asset.',
    `connection_capacity_kw` DECIMAL(18,2) COMMENT 'Maximum power capacity in kilowatts approved for this specific DER-asset connection point. May be lower than the DER system nameplate capacity if the utility asset has capacity constraints or if multiple DER systems share the same asset.',
    `connection_type` STRING COMMENT 'Classification of the physical connection point type between the DER system and the utility asset. Identifies the specific electrical interface and connection configuration used for the interconnection.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this connection record was created in the system.',
    `disconnect_switch_accessible_flag` BOOLEAN COMMENT 'Indicates whether the utility-accessible disconnect switch for this DER-asset connection is readily accessible to utility crews for maintenance and emergency operations. Critical for outage response and safety procedures.',
    `export_limit_kw` DECIMAL(18,2) COMMENT 'Maximum power export limit in kilowatts allowed for this specific DER-asset connection. May be constrained by asset capacity, feeder capacity, or interconnection agreement terms. Null if no export is permitted.',
    `last_inspection_date` DATE COMMENT 'Date of the most recent physical inspection or testing of this DER-asset connection point. Used for maintenance tracking and compliance verification.',
    `metering_configuration` STRING COMMENT 'Specification of the metering arrangement and measurement configuration for this specific DER-asset connection. Defines how energy flow is measured and recorded for billing, settlement, and grid monitoring purposes.',
    `next_inspection_due_date` DATE COMMENT 'Scheduled date for the next required inspection or testing of this DER-asset connection based on utility maintenance standards and regulatory requirements.',
    `operating_status` STRING COMMENT 'Current operational status of this specific DER-asset connection. Indicates whether the connection is actively energized, temporarily suspended, under maintenance, or permanently decommissioned. Used for outage management and operational coordination.',
    `poi_voltage_level_kv` DECIMAL(18,2) COMMENT 'Nominal voltage level in kilovolts at the specific point where the DER system connects to this utility asset. Critical for protection coordination and equipment specification.',
    `protection_scheme` STRING COMMENT 'Description of the protection coordination scheme and relay settings applied to this specific DER-asset connection. Captures the protective relay configuration, trip settings, and coordination with upstream/downstream protection devices to ensure safe operation and fault isolation.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this connection record was last updated.',
    CONSTRAINT pk_der_connection PRIMARY KEY(`der_connection_id`)
) COMMENT 'This association product represents the physical electrical connection between a utility asset and a distributed energy resource (DER) system. It captures the technical configuration, protection coordination, metering arrangement, and operational status of each connection point. Each record links one asset_registry entry to one der_system with attributes that exist only in the context of this specific interconnection configuration. Essential for distribution operations, protection coordination, outage management, and maintenance planning of DER-integrated grid infrastructure.. Existence Justification: In utility distribution operations, a single DER system (solar PV, battery storage, EV charger) physically connects to multiple utility assets at the point of interconnection - typically a service transformer, revenue meter, utility-accessible disconnect switch, and protection relay. Conversely, a single utility asset such as a distribution transformer or secondary service panel can serve multiple DER systems installed at different customer premises or even multiple DER systems at the same location. Each connection has unique technical attributes including metering configuration, protection coordination settings, connection capacity limits, and operational status that belong to the specific DER-asset pairing, not to either entity alone.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`asset`.`project_permit` (
    `project_permit_id` BIGINT COMMENT 'Unique identifier for this project-permit compliance relationship. Primary key.',
    `capital_project_id` BIGINT COMMENT 'Foreign key linking to the capital project requiring environmental permit compliance',
    `environmental_permit_id` BIGINT COMMENT 'Foreign key linking to the environmental permit governing this project',
    `compliance_status` STRING COMMENT 'Current compliance status of the capital project with respect to this specific permit and condition. Tracks whether the project meets permit requirements.',
    `environmental_permit_number` STRING COMMENT 'Reference number for environmental permits required for the capital project, issued by EPA or state environmental agencies. [Moved from capital_project: This attribute in capital_project appears to reference a single permit, but in reality capital projects require multiple permits. This attribute should be removed from capital_project as the relationship is properly modeled through the project_permit association, which can link to multiple permits and track permit-specific compliance details.]',
    `mitigation_measure_description` STRING COMMENT 'Description of specific mitigation measures, controls, or corrective actions implemented for this project to comply with this permit condition. May include engineering controls, monitoring protocols, or operational restrictions.',
    `next_inspection_date` DATE COMMENT 'Scheduled date for the next regulatory inspection or compliance verification for this project-permit combination.',
    `notes` STRING COMMENT 'Additional notes, comments, or documentation references related to permit compliance for this specific project.',
    `permit_application_date` DATE COMMENT 'Date when the permit application was submitted for this specific project. Relevant for project-specific permits or permit modifications required for the capital project.',
    `permit_approval_date` DATE COMMENT 'Date when the permit was approved or modified to cover this capital project. Critical for tracking project schedule dependencies on regulatory approvals.',
    `permit_condition_number` STRING COMMENT 'Specific permit condition number or reference that applies to this capital project. Each permit may have multiple conditions, and projects must track compliance with specific applicable conditions.',
    `responsible_party` STRING COMMENT 'Name or identifier of the individual or team responsible for ensuring compliance with this permit condition for this specific project. May be project manager, environmental coordinator, or compliance officer.',
    `verification_date` DATE COMMENT 'Date when compliance with this permit condition was last verified by environmental compliance staff or regulatory inspectors.',
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ADD CONSTRAINT `fk_asset_work_order_asset_registry_id` FOREIGN KEY (`asset_registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ADD CONSTRAINT `fk_asset_work_order_work_maintenance_plan_id` FOREIGN KEY (`work_maintenance_plan_id`) REFERENCES `energy_utilities_ecm`.`asset`.`maintenance_plan`(`maintenance_plan_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ADD CONSTRAINT `fk_asset_maintenance_plan_asset_class_id` FOREIGN KEY (`asset_class_id`) REFERENCES `energy_utilities_ecm`.`asset`.`class`(`class_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ADD CONSTRAINT `fk_asset_maintenance_plan_asset_registry_id` FOREIGN KEY (`asset_registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ADD CONSTRAINT `fk_asset_failure_event_asset_registry_id` FOREIGN KEY (`asset_registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ADD CONSTRAINT `fk_asset_failure_event_primary_work_order_id` FOREIGN KEY (`primary_work_order_id`) REFERENCES `energy_utilities_ecm`.`asset`.`work_order`(`work_order_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ADD CONSTRAINT `fk_asset_depreciation_schedule_asset_class_id` FOREIGN KEY (`asset_class_id`) REFERENCES `energy_utilities_ecm`.`asset`.`class`(`class_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ADD CONSTRAINT `fk_asset_depreciation_schedule_asset_registry_id` FOREIGN KEY (`asset_registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` ADD CONSTRAINT `fk_asset_spare_parts_inventory_asset_class_id` FOREIGN KEY (`asset_class_id`) REFERENCES `energy_utilities_ecm`.`asset`.`class`(`class_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ADD CONSTRAINT `fk_asset_inspection_work_order_id` FOREIGN KEY (`work_order_id`) REFERENCES `energy_utilities_ecm`.`asset`.`work_order`(`work_order_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ADD CONSTRAINT `fk_asset_retirement_capital_project_id` FOREIGN KEY (`capital_project_id`) REFERENCES `energy_utilities_ecm`.`asset`.`capital_project`(`capital_project_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ADD CONSTRAINT `fk_asset_retirement_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ADD CONSTRAINT `fk_asset_retirement_retirement_replacement_asset_asset_registry_id` FOREIGN KEY (`retirement_replacement_asset_asset_registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ADD CONSTRAINT `fk_asset_retirement_work_order_id` FOREIGN KEY (`work_order_id`) REFERENCES `energy_utilities_ecm`.`asset`.`work_order`(`work_order_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ADD CONSTRAINT `fk_asset_asset_risk_assessment_asset_registry_id` FOREIGN KEY (`asset_registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ADD CONSTRAINT `fk_asset_asset_risk_assessment_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ADD CONSTRAINT `fk_asset_work_order_material_issue_spare_parts_inventory_id` FOREIGN KEY (`spare_parts_inventory_id`) REFERENCES `energy_utilities_ecm`.`asset`.`spare_parts_inventory`(`spare_parts_inventory_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ADD CONSTRAINT `fk_asset_work_order_material_issue_work_order_id` FOREIGN KEY (`work_order_id`) REFERENCES `energy_utilities_ecm`.`asset`.`work_order`(`work_order_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ADD CONSTRAINT `fk_asset_asset_program_enrollment_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ADD CONSTRAINT `fk_asset_asset_compliance_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ADD CONSTRAINT `fk_asset_ppa_allocation_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ADD CONSTRAINT `fk_asset_impact_assessment_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ADD CONSTRAINT `fk_asset_der_connection_registry_id` FOREIGN KEY (`registry_id`) REFERENCES `energy_utilities_ecm`.`asset`.`registry`(`registry_id`);
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ADD CONSTRAINT `fk_asset_project_permit_capital_project_id` FOREIGN KEY (`capital_project_id`) REFERENCES `energy_utilities_ecm`.`asset`.`capital_project`(`capital_project_id`);
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`registry` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`registry` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Manufacturer Vendor Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` ALTER COLUMN `hierarchy_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` ALTER COLUMN `hierarchy_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`hierarchy` ALTER COLUMN `hierarchy_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`class` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`class` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Standard Material Master Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` SET TAGS ('dbx_subdomain' = 'maintenance_operations');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `crew_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Crew ID');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `contractor_worker_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Contractor ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Supervisor ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Circuit ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Interconnection Application Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `purchase_order_id` SET TAGS ('dbx_business_glossary_term' = 'Material Purchase Order Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Transmission Line Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order` ALTER COLUMN `work_maintenance_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Preventive Maintenance (PM) Schedule ID');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` SET TAGS ('dbx_subdomain' = 'maintenance_operations');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `class_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Class ID');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Planner ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `crew_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Crew ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Service Supplier Contract Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`maintenance_plan` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Standard Material Master Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` SET TAGS ('dbx_subdomain' = 'maintenance_operations');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `transformer_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Transformer Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `dr_event_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `event_id` SET TAGS ('dbx_business_glossary_term' = 'Outage Event ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Failed Component Material Master Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `self_report_id` SET TAGS ('dbx_business_glossary_term' = 'Self Report Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`failure_event` ALTER COLUMN `weather_condition` SET TAGS ('dbx_business_glossary_term' = 'Weather Condition');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` SET TAGS ('dbx_subdomain' = 'project_finance');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `cpcn_application_id` SET TAGS ('dbx_business_glossary_term' = 'Cpcn Application Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `distribution_substation_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Substation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Feeder Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Master Supplier Contract Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Equipment Material Master Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Project Manager Employee ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`capital_project` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `class_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Class Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`depreciation_schedule` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset ID');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` SET TAGS ('dbx_subdomain' = 'maintenance_operations');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` ALTER COLUMN `class_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Class Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`spare_parts_inventory` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` SET TAGS ('dbx_subdomain' = 'maintenance_operations');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `inspection_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Inspector ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `inspection_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `inspection_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`inspection` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order ID');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`valuation` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`valuation` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`valuation` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`valuation` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` SET TAGS ('dbx_subdomain' = 'asset_registry');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Retirement ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Capital Project ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Disposal Vendor ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `transformer_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Transformer Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `environmental_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Environmental Permit Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `feeder_id` SET TAGS ('dbx_business_glossary_term' = 'Feeder Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `irp_scenario_id` SET TAGS ('dbx_business_glossary_term' = 'Irp Scenario Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Retirement Approved By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Retirement Initiated By User ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_replacement_asset_asset_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Replacement Asset ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Retirement Approval Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `asset_class_code` SET TAGS ('dbx_business_glossary_term' = 'Asset Class Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `decommissioning_cost` SET TAGS ('dbx_business_glossary_term' = 'Decommissioning Cost');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `depreciation_reserve_released` SET TAGS ('dbx_business_glossary_term' = 'Depreciation Reserve Released');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `disposal_date` SET TAGS ('dbx_business_glossary_term' = 'Disposal Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `disposal_method` SET TAGS ('dbx_business_glossary_term' = 'Disposal Method');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `disposal_method` SET TAGS ('dbx_value_regex' = 'scrap|sale|transfer|donation|write_off|environmental_disposal');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `environmental_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Environmental Compliance Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `environmental_disposal_permit_number` SET TAGS ('dbx_business_glossary_term' = 'Environmental Disposal Permit Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `functional_location_code` SET TAGS ('dbx_business_glossary_term' = 'Functional Location Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `gain_loss_on_disposal` SET TAGS ('dbx_business_glossary_term' = 'Gain or Loss on Disposal');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `insurance_claim_reference` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Reference');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `net_book_value_at_retirement` SET TAGS ('dbx_business_glossary_term' = 'Net Book Value (NBV) at Retirement');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Retirement Notes');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `rab_removal_amount` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Asset Base (RAB) Removal Amount');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `rab_removal_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Asset Base (RAB) Removal Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Retirement Reason Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Retirement Reason Description');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `regulatory_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Approval Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `regulatory_approval_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Approval Required Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `regulatory_notification_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Reference');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_category` SET TAGS ('dbx_business_glossary_term' = 'Asset Retirement Category');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_category` SET TAGS ('dbx_value_regex' = 'generation|transmission|distribution|substation|fleet|facility');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_date` SET TAGS ('dbx_business_glossary_term' = 'Asset Retirement Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_number` SET TAGS ('dbx_business_glossary_term' = 'Asset Retirement Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_status` SET TAGS ('dbx_business_glossary_term' = 'Retirement Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `retirement_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|cancelled|pending_approval');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `salvage_value_realized` SET TAGS ('dbx_business_glossary_term' = 'Salvage Value Realized');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `source_system_code` SET TAGS ('dbx_value_regex' = 'SAP_S4HANA|VENTYX|MANUAL');
ALTER TABLE `energy_utilities_ecm`.`asset`.`retirement` ALTER COLUMN `source_system_record_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Record ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Warranty Provider Vendor Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `warranty_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `warranty_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`warranty` ALTER COLUMN `warranty_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` SET TAGS ('dbx_subdomain' = 'asset_registry');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `asset_risk_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Risk Assessment ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assessor Employee Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `irp_scenario_id` SET TAGS ('dbx_business_glossary_term' = 'Irp Scenario Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Registry ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_confidence_level` SET TAGS ('dbx_business_glossary_term' = 'Assessment Confidence Level');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_confidence_level` SET TAGS ('dbx_value_regex' = 'high|medium|low');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_notes` SET TAGS ('dbx_business_glossary_term' = 'Assessment Notes');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Assessment Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_status` SET TAGS ('dbx_value_regex' = 'draft|submitted|under_review|approved|superseded|archived');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Assessment Type');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `assessment_type` SET TAGS ('dbx_value_regex' = 'initial|periodic|condition-triggered|event-triggered|pre-investment|post-incident');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `benefit_cost_ratio` SET TAGS ('dbx_business_glossary_term' = 'Benefit Cost Ratio');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `consequence_of_failure_rating` SET TAGS ('dbx_business_glossary_term' = 'Consequence of Failure (CoF) Rating');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `consequence_of_failure_rating` SET TAGS ('dbx_value_regex' = 'negligible|minor|moderate|major|severe|catastrophic');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `consequence_of_failure_score` SET TAGS ('dbx_business_glossary_term' = 'Consequence of Failure (CoF) Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `environmental_consequence_score` SET TAGS ('dbx_business_glossary_term' = 'Environmental Consequence Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `financial_consequence_score` SET TAGS ('dbx_business_glossary_term' = 'Financial Consequence Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `intervention_cost_estimate` SET TAGS ('dbx_business_glossary_term' = 'Intervention Cost Estimate');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `intervention_priority` SET TAGS ('dbx_business_glossary_term' = 'Intervention Priority');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `intervention_priority` SET TAGS ('dbx_value_regex' = 'immediate|urgent|high|medium|low|deferred');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `irp_inclusion_flag` SET TAGS ('dbx_business_glossary_term' = 'Integrated Resource Plan (IRP) Inclusion Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `nerc_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'North American Electric Reliability Corporation (NERC) Compliance Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `net_present_value` SET TAGS ('dbx_business_glossary_term' = 'Net Present Value (NPV)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `next_assessment_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Assessment Due Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `overall_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Overall Risk Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `payback_period_years` SET TAGS ('dbx_business_glossary_term' = 'Payback Period Years');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `probability_of_failure_rating` SET TAGS ('dbx_business_glossary_term' = 'Probability of Failure (PoF) Rating');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `probability_of_failure_rating` SET TAGS ('dbx_value_regex' = 'very_low|low|medium|high|very_high|critical');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `probability_of_failure_score` SET TAGS ('dbx_business_glossary_term' = 'Probability of Failure (PoF) Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `puc_prudency_evidence_flag` SET TAGS ('dbx_business_glossary_term' = 'Public Utility Commission (PUC) Prudency Evidence Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `rab_impact_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Asset Base (RAB) Impact Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `recommended_intervention` SET TAGS ('dbx_business_glossary_term' = 'Recommended Intervention');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `regulatory_consequence_score` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Consequence Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `reliability_consequence_score` SET TAGS ('dbx_business_glossary_term' = 'Reliability Consequence Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `reviewer_name` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Name');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `risk_band` SET TAGS ('dbx_business_glossary_term' = 'Risk Band');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `risk_band` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low|negligible');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `risk_methodology` SET TAGS ('dbx_business_glossary_term' = 'Risk Methodology');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `risk_methodology` SET TAGS ('dbx_value_regex' = 'risk_matrix|monte_carlo|weibull_analysis|fault_tree|bow_tie|fmea');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `risk_rank` SET TAGS ('dbx_business_glossary_term' = 'Risk Rank');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `risk_reduction_benefit` SET TAGS ('dbx_business_glossary_term' = 'Risk Reduction Benefit');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `safety_consequence_score` SET TAGS ('dbx_business_glossary_term' = 'Safety Consequence Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `source_system_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_risk_assessment` ALTER COLUMN `source_system_record_code` SET TAGS ('dbx_business_glossary_term' = 'Source System Record ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` SET TAGS ('dbx_subdomain' = 'maintenance_operations');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` SET TAGS ('dbx_association_edges' = 'asset.work_order,asset.spare_parts_inventory');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `work_order_material_issue_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Material Issue ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Issued By Employee ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `spare_parts_inventory_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Material Issue - Spare Parts Inventory Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Material Issue - Work Order Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `cost_center_charged` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Charged');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'GL Account Code');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `issue_date` SET TAGS ('dbx_business_glossary_term' = 'Issue Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `issue_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Issue Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `material_document_number` SET TAGS ('dbx_business_glossary_term' = 'Material Document Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `quantity_issued` SET TAGS ('dbx_business_glossary_term' = 'Quantity Issued');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `quantity_returned` SET TAGS ('dbx_business_glossary_term' = 'Quantity Returned');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `reservation_number` SET TAGS ('dbx_business_glossary_term' = 'Reservation Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `return_date` SET TAGS ('dbx_business_glossary_term' = 'Return Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `unit_cost_at_issue` SET TAGS ('dbx_business_glossary_term' = 'Unit Cost at Issue');
ALTER TABLE `energy_utilities_ecm`.`asset`.`work_order_material_issue` ALTER COLUMN `work_order_line_number` SET TAGS ('dbx_business_glossary_term' = 'Work Order Line Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` SET TAGS ('dbx_subdomain' = 'project_finance');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` SET TAGS ('dbx_association_edges' = 'asset.asset_registry,demand.dr_program');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `asset_program_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'asset_program_enrollment Identifier');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `dr_program_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Program Enrollment - Dr Program Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Program Enrollment ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Program Enrollment - Asset Registry Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `average_performance_percentage` SET TAGS ('dbx_business_glossary_term' = 'Average Performance Percentage');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `baseline_methodology` SET TAGS ('dbx_business_glossary_term' = 'Baseline Methodology');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `cumulative_dispatch_hours_ytd` SET TAGS ('dbx_business_glossary_term' = 'Cumulative Dispatch Hours YTD');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `enrolled_capacity_kw` SET TAGS ('dbx_business_glossary_term' = 'Enrolled Capacity (kW)');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `enrollment_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `enrollment_end_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment End Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `incentive_rate` SET TAGS ('dbx_business_glossary_term' = 'Incentive Rate');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `last_dispatch_date` SET TAGS ('dbx_business_glossary_term' = 'Last Dispatch Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `participation_status` SET TAGS ('dbx_business_glossary_term' = 'Participation Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `performance_tier` SET TAGS ('dbx_business_glossary_term' = 'Performance Tier');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_program_enrollment` ALTER COLUMN `withdrawal_reason` SET TAGS ('dbx_business_glossary_term' = 'Withdrawal Reason');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` SET TAGS ('dbx_subdomain' = 'project_finance');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` SET TAGS ('dbx_association_edges' = 'asset.asset_registry,compliance.obligation');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `asset_compliance_id` SET TAGS ('dbx_business_glossary_term' = 'asset_compliance Identifier');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `compliance_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Compliance Record ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Compliance - Obligation Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Compliance - Asset Registry Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `corrective_action_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Remediation Plan ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `applicability_determination_date` SET TAGS ('dbx_business_glossary_term' = 'Applicability Determination Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `evidence_reference` SET TAGS ('dbx_business_glossary_term' = 'Evidence Reference');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `last_audit_finding` SET TAGS ('dbx_business_glossary_term' = 'Last Audit Finding');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `last_compliance_verification_date` SET TAGS ('dbx_business_glossary_term' = 'Last Compliance Verification Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `next_compliance_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Compliance Due Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `non_compliance_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Non-Compliance Risk Score');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `responsible_party` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party');
ALTER TABLE `energy_utilities_ecm`.`asset`.`asset_compliance` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` SET TAGS ('dbx_subdomain' = 'project_finance');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` SET TAGS ('dbx_association_edges' = 'asset.asset_registry,renewable.ppa_contract');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `ppa_allocation_id` SET TAGS ('dbx_business_glossary_term' = 'Asset PPA Allocation ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Ppa Allocation - Asset Registry Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `ppa_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Ppa Allocation - Renewable Ppa Contract Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `meter_id` SET TAGS ('dbx_business_glossary_term' = 'Settlement Meter ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `allocation_notes` SET TAGS ('dbx_business_glossary_term' = 'Allocation Notes');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `allocation_percentage` SET TAGS ('dbx_business_glossary_term' = 'Allocation Percentage');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `allocation_status` SET TAGS ('dbx_business_glossary_term' = 'Allocation Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `contract_end_date` SET TAGS ('dbx_business_glossary_term' = 'Allocation End Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `contract_start_date` SET TAGS ('dbx_business_glossary_term' = 'Allocation Start Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `contracted_capacity_mw` SET TAGS ('dbx_business_glossary_term' = 'Allocated Contracted Capacity');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `contracted_price_per_mwh` SET TAGS ('dbx_business_glossary_term' = 'Asset-Specific Contract Price');
ALTER TABLE `energy_utilities_ecm`.`asset`.`ppa_allocation` ALTER COLUMN `rec_allocation_flag` SET TAGS ('dbx_business_glossary_term' = 'REC Allocation Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` SET TAGS ('dbx_subdomain' = 'project_finance');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` SET TAGS ('dbx_association_edges' = 'asset.asset_registry,interconnection.application');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `impact_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Impact Assessment - Asset Impact Assessment Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Analyzed By');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `cluster_study_group_id` SET TAGS ('dbx_business_glossary_term' = 'Cluster Study ID');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Impact Assessment - Interconnection Application Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Impact Assessment - Asset Registry Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `affected_capacity_kw` SET TAGS ('dbx_business_glossary_term' = 'Affected Capacity');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `analysis_date` SET TAGS ('dbx_business_glossary_term' = 'Analysis Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `analysis_status` SET TAGS ('dbx_business_glossary_term' = 'Analysis Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `estimated_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Upgrade Cost');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `impact_severity` SET TAGS ('dbx_business_glossary_term' = 'Impact Severity');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `impact_type` SET TAGS ('dbx_business_glossary_term' = 'Impact Type');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `mitigation_required` SET TAGS ('dbx_business_glossary_term' = 'Mitigation Required');
ALTER TABLE `energy_utilities_ecm`.`asset`.`impact_assessment` ALTER COLUMN `upgrade_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Upgrade Required Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` SET TAGS ('dbx_subdomain' = 'project_finance');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` SET TAGS ('dbx_association_edges' = 'asset.asset_registry,interconnection.der_system');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `der_connection_id` SET TAGS ('dbx_business_glossary_term' = 'Asset-DER Connection Identifier');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `der_system_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Der Connection - Der System Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Asset Der Connection - Asset Registry Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `commissioning_date` SET TAGS ('dbx_business_glossary_term' = 'Connection Commissioning Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `connection_capacity_kw` SET TAGS ('dbx_business_glossary_term' = 'Connection Capacity');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `connection_type` SET TAGS ('dbx_business_glossary_term' = 'Connection Type');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `disconnect_switch_accessible_flag` SET TAGS ('dbx_business_glossary_term' = 'Disconnect Switch Accessible Flag');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `export_limit_kw` SET TAGS ('dbx_business_glossary_term' = 'Export Limit');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `last_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Connection Last Inspection Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `metering_configuration` SET TAGS ('dbx_business_glossary_term' = 'Metering Configuration');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `next_inspection_due_date` SET TAGS ('dbx_business_glossary_term' = 'Connection Next Inspection Due Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `operating_status` SET TAGS ('dbx_business_glossary_term' = 'Connection Operating Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `poi_voltage_level_kv` SET TAGS ('dbx_business_glossary_term' = 'Point of Interconnection Voltage Level');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `protection_scheme` SET TAGS ('dbx_business_glossary_term' = 'Protection Scheme');
ALTER TABLE `energy_utilities_ecm`.`asset`.`der_connection` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` SET TAGS ('dbx_subdomain' = 'project_finance');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` SET TAGS ('dbx_association_edges' = 'asset.capital_project,compliance.environmental_permit');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `project_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Project Permit Identifier');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `capital_project_id` SET TAGS ('dbx_business_glossary_term' = 'Project Permit - Capital Project Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `environmental_permit_id` SET TAGS ('dbx_business_glossary_term' = 'Project Permit - Compliance Environmental Permit Id');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `environmental_permit_number` SET TAGS ('dbx_business_glossary_term' = 'Environmental Permit Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `mitigation_measure_description` SET TAGS ('dbx_business_glossary_term' = 'Mitigation Measure Description');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `next_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Next Inspection Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Compliance Notes');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `permit_application_date` SET TAGS ('dbx_business_glossary_term' = 'Permit Application Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `permit_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Permit Approval Date');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `permit_condition_number` SET TAGS ('dbx_business_glossary_term' = 'Permit Condition Number');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `responsible_party` SET TAGS ('dbx_business_glossary_term' = 'Responsible Party');
ALTER TABLE `energy_utilities_ecm`.`asset`.`project_permit` ALTER COLUMN `verification_date` SET TAGS ('dbx_business_glossary_term' = 'Compliance Verification Date');
