-- Schema for Domain: billing | Business: Energy Utilities | Version: v1_ecm
-- Generated on: 2026-05-04 21:10:19
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`rate_schedule` (
    ...
    `irp_scenario_id` BIGINT COMMENT 'Foreign key linking to forecast.irp_scenario. Business justification: Rate schedules are designed to recover costs projected in IRP scenarios. Regulatory rate cases reference specific IRP scenarios for cost recovery justification. Utilities track which IRP scenario info',
    `lmp_price_id` BIGINT COMMENT 'Foreign key linking to trading.lmp_price. Business justification: Dynamic retail tariffs indexed to real-time or day-ahead LMP require direct price reference for automated rate calculation, regulatory cost recovery validation, and customer bill transparency in real-',
    `peak_demand_id` BIGINT COMMENT 'Foreign key linking to forecast.peak_demand. Business justification: Demand charge rate schedules are designed to recover capacity costs allocated based on peak demand forecasts. Rate design documents reference specific peak demand forecasts (by customer class and seas',
    ...
    `incentive_program_id` BIGINT COMMENT 'Foreign key linking to renewable.incentive_program. Business justification: Rate schedules incorporate renewable incentive program terms for green tariff pricing, renewable energy riders, and regulatory compliance. Tariff design requires linking incentive program rates to bil',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`rate_component` (
    ...
    `energy_price_id` BIGINT COMMENT 'Foreign key linking to forecast.energy_price. Business justification: Energy charge rate components often include fuel adjustment clauses indexed to forecasted energy/fuel prices. Tariff sheets reference specific price forecast scenarios for automatic rate adjustments. ',
    `fuel_price_assumption_id` BIGINT COMMENT 'Foreign key linking to forecast.fuel_price_assumption. Business justification: Fuel adjustment clause rate components are calculated using specific fuel price assumptions filed with regulators. Utilities must track which fuel price forecast version was used for each rate compone',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` (
    ...
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: Service agreements track DER installations at customer premises for net metering billing, export compensation, and demand response programs. Essential for calculating NEM credits, managing export limi',
    `meter_point_id` BIGINT COMMENT 'Foreign key linking to metering.meter_point. Business justification: Service agreements govern specific service points (the contract-to-location binding). Rate schedules, service voltage, and billing cycles are service-point-specific. This link is essential for service',
    `nem_account_id` BIGINT COMMENT 'Foreign key linking to renewable.nem_account. Business justification: Service agreements have 1:1 relationship with NEM accounts for net metering customers. Required to link billing terms, rate schedules, and payment arrangements to specific NEM credit tracking, true-up',
    `premise_id` BIGINT COMMENT 'Reference to the premise (service location) where energy is delivered under this agreement.',
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Special contract service agreements and non-standard tariff arrangements require regulatory approval and disclosure in filings. Links agreements to the filing where they were approved or reported for ',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`invoice` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Invoices post revenue to specific GL accounts for financial statement preparation and regulatory reporting. Every billed revenue transaction requires GL account assignment per FERC Uniform System of A',
    ...
    `trade_id` BIGINT COMMENT 'Foreign key linking to trading.trade. Business justification: Bilateral wholesale energy trades delivered outside ISO markets generate direct customer invoices requiring trade reference for settlement reconciliation, revenue recognition, and regulatory audit tra',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`invoice_line` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Revenue allocation to cost centers enables internal performance reporting, cost-of-service studies for rate cases, and regulatory reporting by functional area. Essential for FERC Form 1 preparation an',
    ...
    `invoice_rate_component_id` BIGINT COMMENT 'Foreign key linking to billing.rate_component. Business justification: Each invoice line item is calculated using a specific rate component definition from the rate schedule. Currently invoice_line has rate_component_code (STRING) which is a denormalized reference. Addin',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Revenue attribution to profit centers supports segment reporting, FERC functional classification (generation/transmission/distribution), and business unit profitability analysis. Required for regulato',
    `sox_control_test_id` BIGINT COMMENT 'Foreign key linking to compliance.sox_control_test. Business justification: SOX control testing samples specific invoice line items to verify billing accuracy controls and revenue recognition. Links the tested line item to the control test for audit evidence, sample documenta',
    `trade_id` BIGINT COMMENT 'Foreign key linking to trading.trade. Business justification: Individual invoice line items may represent specific wholesale trade settlements, particularly for bilateral contracts with retail pass-through pricing where each trade leg generates a separate billin',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`payment` (
    ...
    `payment_invoice_id` BIGINT COMMENT 'Reference to the specific invoice being paid. Null for account-level payments or refunds not tied to a specific invoice.',
    ...
    `payment_invoice` BIGINT COMMENT 'FK to billing.invoice.invoice_id — Payments are applied against specific invoices. Critical for AR balance calculation and cash application.',
    ...
    `service_agreement` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Payments may be applied at the account/service agreement level when not tied to a specific invoice (e.g., prepayments, deposits).',
    ...
    `to_sa` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Payments are received for a customer service agreement. Needed when payments are applied at account level rather than invoice level.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`payment_arrangement` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the customer service representative or collections agent who approved the payment arrangement terms.',
    `application_id` BIGINT COMMENT 'Foreign key linking to interconnection.application. Business justification: High interconnection costs (impact studies, network upgrades, inspection fees) often require payment arrangements. Business process: Customers unable to pay upfront interconnection fees negotiate inst',
    ...
    `arrangement_to_sa` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Payment arrangements are established for a specific service agreement with past-due balances.',
    ...
    `service_agreement` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Payment arrangements are established for a specific service agreement/account.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`determinant` (
    ...
    `billing_period_id` BIGINT COMMENT 'Reference to the billing period for which these determinants apply. Defines the start and end dates of the consumption window used for rate calculation.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`adjustment` (
    ...
    `employee_id` BIGINT COMMENT 'User ID of the person or system process that initiated the adjustment transaction. Required for audit trails and segregation of duties controls.',
    `adjustment_employee_id` BIGINT COMMENT 'User ID of the person who approved the adjustment. Required for SOX audit trails and segregation of duties controls.',
    `adjustment_invoice_id` BIGINT COMMENT 'Reference to the source invoice affected by this adjustment. Nullable for account-level adjustments not tied to a specific invoice.',
    ...
    `bill_dispute_id` BIGINT COMMENT 'Reference to the customer dispute or complaint case that resulted in this adjustment. Null if not dispute-related. Links to customer service case management system.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Adjustments allocated to cost centers for variance analysis, budget tracking, and regulatory cost recovery. Essential for identifying systematic billing errors by department and supporting rate case c',
    `dr_incentive_payment_id` BIGINT COMMENT 'Foreign key linking to demand.dr_incentive_payment. Business justification: DR incentive payments are frequently posted as billing adjustments when credited after initial invoice generation. GL reconciliation, audit trails, and financial reporting require linking adjustments ',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Billing adjustments (credits, rebates, corrections) post to specific GL accounts for accurate financial statements and audit trails. Reusing existing unlinked gl_account_code as denormalized reference',
    ...
    `adjustment_invoice` BIGINT COMMENT 'FK to billing.invoice.invoice_id — Adjustments are applied against specific invoices or billing periods. Critical for AR reconciliation and SOX controls.',
    ...
    `service_agreement` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Adjustments are posted to a service agreement. Required for account-level balance tracking.',
    ...
    `to_sa` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Adjustments are applied to a service agreement when not tied to a specific invoice (e.g., account-level credits, NEM credits).',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`deposit` (
    ...
    `employee_id` BIGINT COMMENT 'User ID of the system user or CSR who last modified the deposit record. Supports SOX audit requirements and internal control documentation for financial transactions.',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Customer deposits held in specific liability GL accounts per regulatory requirements and GAAP. Link enables proper balance sheet classification, interest accrual tracking, and regulatory reporting of ',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` (
    ...
    `bill_dispute_id` BIGINT COMMENT 'Reference to a billing dispute or complaint case opened by the customer in response to this dunning notice.',
    ...
    `disconnect_order_id` BIGINT COMMENT 'Reference to the service disconnection work order issued as a result of this dunning notice, if escalated to disconnect.',
    `dunning_run_id` BIGINT COMMENT 'Identifier for the batch dunning process or collections run that generated this notice. Used for audit and reconciliation.',
    ...
    `in_collections_case` BIGINT COMMENT 'FK to billing.collections_case.collections_case_id — Dunning notices are issued within the context of a collections case. This FK tracks the escalation lifecycle from first notice through disconnect warning. Note: if merge recommendation is accepted, th',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`collections` (
    ...
    `employee_id` BIGINT COMMENT 'Reference to the collections agent or team member assigned to manage this case.',
    ...
    `service_agreement` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Collections cases are opened against a delinquent service agreement/account. Critical for collections workflow and disconnect processing.',
    ...
    `to_sa` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Collections cases are opened for a delinquent service agreement. This FK is essential for collections workflow and disconnect processing.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`write_off` (
    ...
    `employee_id` BIGINT COMMENT 'Foreign key linking to workforce.employee. Business justification: Write-off approval requires explicit employee authorization tracking for SOX compliance, audit trail, and segregation of duties. Energy utilities must document approval authority per FERC/PUC requirem',
    `collections_id` BIGINT COMMENT 'Reference to the collections case that led to this write-off decision, if applicable.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Write-offs allocated to cost centers for performance tracking, collections effectiveness analysis, and regulatory cost recovery. Supports identification of high-risk customer segments and justificatio',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Bad debt write-offs post to specific expense GL accounts for financial reporting and regulatory cost recovery. Link supports allowance for doubtful accounts reconciliation, FERC reporting, and rate ca',
    ...
    `service_territory_code` STRING COMMENT 'The geographic service territory or operating district in which the customer account was located.. Valid values are `^[A-Z0-9]{2,6}$`',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`budget_plan` (
    ...
    `load_id` BIGINT COMMENT 'Foreign key linking to forecast.load. Business justification: Budget billing plans calculate levelized monthly payments based on forecasted annual usage for each customer class. Customer service systems reference specific load forecasts (by rate class and season',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`cycle` (
    ...
    `employee_id` BIGINT COMMENT 'User ID of the supervisor who approved the billing run for invoice release. Null if approval not required or not yet approved.',
    `cycle_employee_id` BIGINT COMMENT 'User ID or system account that initiated or executed the billing run. Used for audit trail and accountability.',
    ...
    `service_territory_code` STRING COMMENT 'Geographic service territory identifier this billing cycle serves (e.g., EAST, WEST, METRO1). Used for regional billing operations.. Valid values are `^[A-Z0-9]{2,6}$`',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`billing_run` (
    ...
    `employee_id` BIGINT COMMENT 'User identifier or system account that initiated or executed the billing run, used for audit trail and accountability in billing operations.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`revenue_recognition` (
    ...
    `audit_id` BIGINT COMMENT 'Foreign key linking to compliance.audit. Business justification: External and internal audits test revenue recognition entries for compliance with accounting standards and regulatory requirements. Links revenue entries to the audit engagement where they were tested',
    ...
    `invoice_id` BIGINT COMMENT 'FK to billing.invoice.invoice_id — Revenue recognition entries may reference the source invoice for billed revenue. Supports ASC 606 compliance and audit trail.',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Revenue recognition entries create or reference journal entries for GL posting. Direct linkage ensures audit trail from billing subledger to general ledger, required for SOX compliance, external audit',
    ...
    `revenue_invoice_id` BIGINT COMMENT 'Reference to the invoice document from which this revenue recognition entry was derived. Null for unbilled revenue accruals.',
    `sox_control_id` BIGINT COMMENT 'Foreign key linking to compliance.sox_control. Business justification: Revenue recognition is a key SOX control area for financial statement accuracy. Links specific revenue entries to the SOX control they satisfy, enabling control testing, evidence collection, and audit',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` (
    `bill_dispute_id` BIGINT COMMENT 'Unique identifier for the bill dispute record. Primary key.',
    `account_id` BIGINT COMMENT 'Reference to the customer account that initiated the dispute.',
    `bill_invoice_id` BIGINT COMMENT 'Reference to the invoice being disputed by the customer.',
    `credit_memo_id` BIGINT COMMENT 'Reference to the credit memo document issued to the customer account if a credit was granted as part of the resolution.',
    `employee_id` BIGINT COMMENT 'User ID of the customer service representative or analyst currently assigned to investigate and resolve the dispute.',
    `grid_reliability_event_id` BIGINT COMMENT 'Foreign key linking to grid.grid_reliability_event. Business justification: Customers dispute bills due to outages or voltage issues caused by grid reliability events. Real business process: dispute investigation requires referencing the causal grid event to validate customer',
    `invoice_id` BIGINT COMMENT 'FK to billing.invoice.invoice_id — Bill disputes reference a specific invoice being contested. This FK is essential for dispute resolution workflow and PUC complaint tracking.',
    `meter_point_id` BIGINT COMMENT 'Foreign key linking to metering.meter_point. Business justification: Disputes often involve meter accuracy, AMI communication failures, or service point configuration issues. This link identifies the physical metering location for field investigation, meter test schedu',
    `adjustment_amount` DECIMAL(18,2) COMMENT 'Monetary amount credited or debited to the customer account as a result of the dispute resolution. Positive values represent credits to customer; negative values represent additional charges.',
    `assigned_to_team` STRING COMMENT 'Name or code of the team or department responsible for handling the dispute (e.g., Billing Operations, Meter Services, Revenue Protection).',
    `bill_dispute_to_invoice` BIGINT COMMENT 'FK to billing.invoice.invoice_id — Bill disputes are filed against a specific invoice or charge. This FK is essential for dispute investigation and resolution tracking.',
    `bill_invoice` BIGINT COMMENT 'FK to billing.invoice.invoice_id — Bill disputes are raised against a specific invoice. Critical for dispute resolution workflow and PUC complaint tracking.',
    `created_by_user` STRING COMMENT 'User ID of the system user or customer service representative who created the dispute record.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this dispute record was first created in the system.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the disputed amount.. Valid values are `^[A-Z]{3}$`',
    ...
    `dispute_number` STRING COMMENT 'Externally visible unique dispute case number assigned by the Customer Information System (CIS) for tracking and customer communication.',
    `dispute_open_date` DATE COMMENT 'Date when the dispute was formally opened and logged in the system. Represents the business event timestamp for dispute initiation.',
    `dispute_open_timestamp` TIMESTAMP COMMENT 'Precise timestamp when the dispute record was created in the CIS, including time-of-day for SLA tracking.',
    `dispute_reason_code` STRING COMMENT 'Standardized code categorizing the primary reason for the dispute as reported by the customer or identified during investigation. [ENUM-REF-CANDIDATE: estimated_read|meter_malfunction|rate_error|billing_calculation_error|unauthorized_charge|duplicate_charge|service_quality|high_bill_complaint|tariff_misapplication|proration_error|other — 11 candidates stripped; promote to reference product]',
    `dispute_reason_description` STRING COMMENT 'Detailed free-text explanation of the dispute reason provided by the customer or customer service representative.',
    `dispute_source_channel` STRING COMMENT 'Channel through which the customer initiated the dispute. [ENUM-REF-CANDIDATE: call_center|web_portal|mobile_app|email|mail|in_person|field_service|automated_system — 8 candidates stripped; promote to reference product]',
    `dispute_status` STRING COMMENT 'Current lifecycle status of the dispute case in the resolution workflow. [ENUM-REF-CANDIDATE: open|under_investigation|pending_customer_response|pending_meter_test|pending_field_inspection|resolved|closed|escalated_to_puc|withdrawn — 9 candidates stripped; promote to reference product]',
    `dispute_to_sa` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Bill disputes are associated with a service agreement for customer service tracking and PUC complaint reporting.',
    `disputed_amount` DECIMAL(18,2) COMMENT 'Total monetary amount in dispute as claimed by the customer. Represents the portion of the invoice the customer contests.',
    `field_inspection_date` DATE COMMENT 'Date when the field inspection was conducted at the customer premise.',
    `field_inspection_required_flag` BOOLEAN COMMENT 'Indicates whether a field technician visit to the premise is required to investigate the dispute.',
    `investigation_notes` STRING COMMENT 'Detailed notes and findings documented during the dispute investigation process, including meter test results, field inspection outcomes, and billing recalculations.',
    `investigation_start_date` DATE COMMENT 'Date when formal investigation of the dispute began.',
    `meter_test_required_flag` BOOLEAN COMMENT 'Indicates whether a physical meter accuracy test is required as part of the dispute investigation.',
    `meter_test_result` STRING COMMENT 'Outcome of the meter accuracy test if performed, indicating whether the meter was found to be within acceptable tolerance.. Valid values are `passed|failed|inconclusive|not_performed`',
    `modified_by_user` STRING COMMENT 'User ID of the system user who last modified the dispute record.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this dispute record was last updated.',
    `preventable_flag` BOOLEAN COMMENT 'Indicates whether the dispute was caused by a preventable error or process failure on the utilitys part, used for quality metrics and process improvement.',
    `priority_level` STRING COMMENT 'Priority classification assigned to the dispute based on factors such as disputed amount, customer segment, regulatory risk, or service impact.. Valid values are `low|medium|high|critical`',
    `puc_complaint_number` STRING COMMENT 'Official complaint case number assigned by the Public Utility Commission if the dispute was escalated to regulatory review.',
    `puc_escalation_date` DATE COMMENT 'Date when the dispute was formally escalated to the Public Utility Commission.',
    `regulatory_escalation_flag` BOOLEAN COMMENT 'Indicates whether the dispute has been escalated to the Public Utility Commission (PUC) or other regulatory body for formal complaint review.',
    `resolution_date` DATE COMMENT 'Date when the dispute was formally resolved and closed.',
    `resolution_notes` STRING COMMENT 'Detailed explanation of the resolution decision, including justification for credit issuance, bill upholding, or other actions taken.',
    `resolution_timestamp` TIMESTAMP COMMENT 'Precise timestamp when the dispute status was changed to resolved or closed.',
    `resolution_type` STRING COMMENT 'Classification of how the dispute was resolved, indicating the action taken to close the case. [ENUM-REF-CANDIDATE: credit_issued|bill_upheld|partial_adjustment|rebill_issued|payment_plan_offered|service_correction|meter_replaced|dispute_withdrawn — 8 candidates stripped; promote to reference product]',
    `root_cause_category` STRING COMMENT 'Classification of the underlying root cause identified during investigation, used for process improvement and quality tracking. [ENUM-REF-CANDIDATE: meter_issue|billing_system_error|rate_application_error|customer_misunderstanding|data_entry_error|estimation_error|system_integration_issue|process_failure|other — 9 candidates stripped; promote to reference product]',
    `service_agreement` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — Bill disputes are associated with a service agreement/account. Required for customer service case management.',
    `sla_breach_flag` BOOLEAN COMMENT 'Indicates whether the dispute resolution exceeded the target SLA timeframe.',
    `sla_target_resolution_date` DATE COMMENT 'Target date by which the dispute must be resolved according to internal service level agreements or regulatory requirements.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`nem_credit` (
    `nem_credit_id` BIGINT COMMENT 'Primary key for nem_credit',
    `account_id` BIGINT COMMENT 'Reference to the customer account that earned this NEM credit. Links to the customer billing account with behind-the-meter generation.',
    `billing_service_agreement_id` BIGINT COMMENT 'Foreign key linking to billing.billing_service_agreement. Business justification: NEM credits are earned under a specific service agreement that has NEM tariff terms and interconnection agreement details. Currently nem_credit has account_id and premise_id but no direct link to the ',
    `compliance_rec_certificate_id` BIGINT COMMENT 'Foreign key linking to compliance.rec_certificate. Business justification: Net energy metering credits for customer-sited generation are often tied to renewable energy certificates for RPS compliance tracking. Links NEM export credits to the REC issued for that generation fo',
    `der_interconnection_point_id` BIGINT COMMENT 'Foreign key linking to distribution.der_interconnection_point. Business justification: NEM credits must reference the specific DER interconnection point for export metering validation, tariff rate application (location-specific export rates), interconnection agreement compliance, and IE',
    `der_registry_id` BIGINT COMMENT 'Foreign key linking to renewable.der_registry. Business justification: NEM credits originate from specific DER installations and must track source system for generation verification, capacity validation, and program eligibility. Required for REC tracking, interconnection',
    `dr_program_id` BIGINT COMMENT 'Foreign key linking to demand.dr_program. Business justification: Emerging VPP programs allow DR participation for NEM customers with battery storage or controllable DER. Billing systems must track both NEM export credits and DR incentive payments for same customer.',
    `registry_id` BIGINT COMMENT 'Foreign key linking to asset.asset_registry. Business justification: Net metering credits are generated by specific distributed generation assets (solar, wind) registered in asset_registry. Interconnection compliance, regulatory reporting, credit validation, and capaci',
    `invoice_id` BIGINT COMMENT 'Reference to the billing invoice on which this NEM credit was applied or carried forward.',
    `lmp_price_id` BIGINT COMMENT 'Foreign key linking to trading.lmp_price. Business justification: NEM export compensation rates in market-based programs reference real-time or day-ahead LMP at premise location for avoided cost valuation; required for tariff compliance, customer bill calculation, a',
    `nem_for_agreement` BIGINT COMMENT 'FK to billing.billing_service_agreement.billing_service_agreement_id — NEM credits are earned by a specific service agreement with behind-the-meter generation. This FK links credits to the customers NEM billing arrangement.',
    `net_energy_metering_id` BIGINT COMMENT 'Foreign key linking to metering.net_energy_metering. Business justification: NEM credits are generated from specific DER installations (net_energy_metering records). This link tracks which solar/storage system produced the export, supports RPS compliance reporting, interconnec',
    `original_credit_nem_credit_id` BIGINT COMMENT 'Reference to the original NEM credit transaction that this record adjusts or corrects, establishing audit trail for credit modifications.',
    `premise_id` BIGINT COMMENT 'Reference to the service premise where the behind-the-meter generation equipment (solar, battery storage) is installed.',
    `rate_schedule_id` BIGINT COMMENT 'Reference to the underlying retail rate schedule that determines the credit valuation methodology for exported energy.',
    `rec_holding_id` BIGINT COMMENT 'Foreign key linking to trading.rec_holding. Business justification: Net metering programs bundling RECs with energy credits require tracking which REC inventory supports customer credits for RPS compliance verification, renewable portfolio standard reporting, and envi',
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: NEM credits and distributed generation exports must be reported in RPS compliance filings and renewable energy program reports. Links NEM credits to the regulatory filing where they were reported for ',
    `ppa_contract_id` BIGINT COMMENT 'Foreign key linking to renewable.ppa_contract. Business justification: Some NEM arrangements involve PPA contracts for pricing export compensation above standard tariff rates. Business model for community solar, virtual net metering, and third-party owned systems where P',
    `adjustment_flag` BOOLEAN COMMENT 'Indicates whether this credit transaction is an adjustment or correction to a previously posted NEM credit.',
    `adjustment_reason_code` STRING COMMENT 'Code identifying the reason for adjustment to the NEM credit (e.g., meter read correction, tariff change, billing error correction).',
    `billing_cycle_code` STRING COMMENT 'Code identifying the billing cycle schedule during which this NEM credit was calculated and posted.',
    `created_by_user` STRING COMMENT 'User ID or system process identifier that created this NEM credit record, supporting audit and compliance requirements.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this NEM credit record was first created in the system.',
    `credit_amount` DECIMAL(18,2) COMMENT 'Total dollar value of NEM credits earned during this period, calculated as exported_energy_kwh multiplied by credit_rate_per_kwh.',
    `credit_applied_amount` DECIMAL(18,2) COMMENT 'Dollar amount of NEM credits applied to offset current billing period charges, reducing the customers net invoice amount.',
    `credit_expiration_date` DATE COMMENT 'Date on which unused NEM credits from this transaction will expire if not applied, per state NEM program rules (typically 12 months).',
    `credit_period_end_date` DATE COMMENT 'End date of the billing period during which the excess energy was exported to the grid and NEM credits were earned.',
    `credit_period_start_date` DATE COMMENT 'Start date of the billing period during which the excess energy was exported to the grid and NEM credits were earned.',
    `credit_rate_per_kwh` DECIMAL(18,2) COMMENT 'Dollar amount per kWh credited to the customer for exported energy, as defined by the applicable NEM tariff schedule.',
    `credit_status` STRING COMMENT 'Current lifecycle status of the NEM credit transaction in the billing and accounts receivable system.. Valid values are `earned|applied|expired|forfeited|pending|disputed`',
    `credit_transaction_number` STRING COMMENT 'Business identifier for the NEM credit transaction, used for customer communication and dispute resolution.',
    `credit_type` STRING COMMENT 'Classification of the NEM credit based on the value stream being compensated (energy, capacity, environmental attributes).. Valid values are `energy_export|capacity_credit|renewable_energy_certificate|ancillary_service`',
    `cumulative_credit_balance` DECIMAL(18,2) COMMENT 'Running total of unused NEM credits available on the customer account after this transaction, including prior period carryover.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for all monetary amounts in this credit transaction (typically USD).. Valid values are `^[A-Z]{3}$`',
    `dispute_flag` BOOLEAN COMMENT 'Indicates whether the customer has disputed the NEM credit calculation or application, triggering investigation and resolution workflow.',
    `dispute_reason_code` STRING COMMENT 'Code identifying the reason for customer dispute of the NEM credit (e.g., incorrect meter reading, wrong tariff applied, calculation error).',
    `exported_energy_kwh` DECIMAL(18,2) COMMENT 'Total kilowatt-hours of excess energy exported from the customers behind-the-meter generation to the utility grid during the credit period.',
    `generation_technology_type` STRING COMMENT 'Type of behind-the-meter distributed energy resource (DER) technology generating the exported energy.. Valid values are `solar_pv|battery_storage|wind|combined_heat_power|fuel_cell|other`',
    `gl_account_code` STRING COMMENT 'General ledger account code for posting NEM credit transactions to the financial accounting system, supporting revenue recognition and regulatory reporting.',
    `installed_capacity_kw` DECIMAL(18,2) COMMENT 'Nameplate capacity in kilowatts of the customers behind-the-meter generation equipment registered for NEM participation.',
    `interconnection_agreement_number` STRING COMMENT 'Unique identifier of the utility interconnection agreement authorizing the customers DER to export energy to the grid.',
    `jurisdiction_code` STRING COMMENT 'State or regulatory jurisdiction code governing the NEM program rules and tariff provisions applicable to this credit.',
    `meter_number` STRING COMMENT 'Identifier of the bidirectional meter measuring both imported and exported energy at the customer premise.',
    `modified_by_user` STRING COMMENT 'User ID or system process identifier that last modified this NEM credit record, supporting audit and compliance requirements.',
    `modified_timestamp` TIMESTAMP COMMENT 'Date and time when this NEM credit record was last modified or updated.',
    `nem_tariff_code` STRING COMMENT 'Code identifying the specific NEM tariff schedule version applicable to this credit transaction (e.g., NEM 1.0, NEM 2.0, NEM 3.0).',
    `nem_tariff_version` STRING COMMENT 'Version identifier of the NEM tariff schedule, reflecting regulatory changes and grandfathering provisions.',
    `posted_timestamp` TIMESTAMP COMMENT 'Date and time when the NEM credit transaction was posted to the customer account in the billing system.',
    `regulatory_account_code` STRING COMMENT 'Regulatory accounting code for tracking NEM program costs and benefits in rate case filings and PUC compliance reports.',
    `renewable_energy_certificate_flag` BOOLEAN COMMENT 'Indicates whether renewable energy certificates associated with the exported energy are retained by the customer or transferred to the utility.',
    `rps_compliance_eligible_flag` BOOLEAN COMMENT 'Indicates whether the exported energy qualifies for the utilitys RPS compliance obligations under state renewable energy mandates.',
    `service_territory_code` STRING COMMENT 'Utility service territory code where the customer premise and DER interconnection are located.',
    `tou_period_code` STRING COMMENT 'Time-of-use period classification for exported energy, affecting credit rate when NEM tariff includes TOU differentiation.',
    `true_up_period_flag` BOOLEAN COMMENT 'Indicates whether this credit transaction is part of an annual true-up reconciliation period where unused credits are settled or forfeited.',
    `true_up_settlement_date` DATE COMMENT 'Date of the annual true-up settlement when cumulative NEM credits are reconciled and excess credits are either paid out or forfeited per tariff rules.',
    CONSTRAINT pk_nem_credit PRIMARY KEY(`nem_credit_id`)
) COMMENT 'Transactional record of NEM (Net Energy Metering) credits earned by a customer with behind-the-meter generation (rooftop solar, battery storage) for excess energy exported to the grid. Captures credit period, exported kWh, credit rate ($/kWh), credit amount, cumulative credit balance, credit expiration date, and applicable NEM tariff version. Sourced from Oracle CC&B NEM billing module. Supports state RPS and NEM regulatory compliance reporting.';
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`assistance_program` (
    ...
    `application_id` BIGINT COMMENT 'Foreign key linking to interconnection.application. Business justification: Equity-focused interconnection programs (SGIP equity resiliency budget, community solar for low-income) coordinate with utility financial assistance programs. Business process: Eligibility verificatio',
    ...
    `regulatory_filing_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_filing. Business justification: Low-income assistance programs require annual reporting to regulatory commissions on enrollment, costs, and effectiveness. Links the program to the regulatory filing where program performance and cost',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` (
    `billing_program_enrollment_id` BIGINT COMMENT 'Unique identifier for the customer program enrollment record. Primary key for the program enrollment entity.',
    `account_id` BIGINT COMMENT 'Reference to the customer account enrolled in the program. Links to the customer account master entity.',
    `employee_id` BIGINT COMMENT 'User identifier of the utility employee who approved the program enrollment application. Used for audit trail and accountability.',
    `billing_last_modified_by_user_employee_id` BIGINT COMMENT 'User identifier of the person who last modified this program enrollment record. Used for audit trail and accountability.',
    `program_id` BIGINT COMMENT 'Foreign key linking to compliance.program. Business justification: Customer program enrollments (demand response, energy efficiency, low-income assistance) are tracked under compliance programs for regulatory reporting and program administration. Links enrollment to ',
    `demand_dr_enrollment_id` BIGINT COMMENT 'Foreign key linking to demand.dr_enrollment. Business justification: Generic program enrollment table tracks all utility program participation including DR alongside low-income assistance, budget billing, etc. Customer service, program administration, and unified custo',
    `green_tariff_subscription_id` BIGINT COMMENT 'Foreign key linking to renewable.green_tariff_subscription. Business justification: Green tariff subscriptions are a specific type of program enrollment where customers purchase renewable energy blocks or percentages. Direct relationship required for billing premium charges, allocati',
    `payment_arrangement_id` BIGINT COMMENT 'Reference to the associated payment arrangement for arrearage management program enrollments. Links to payment arrangement master entity.',
    `premise_id` BIGINT COMMENT 'Reference to the service premise associated with this program enrollment. Links to the premise master entity.',
    `incentive_program_id` BIGINT COMMENT 'Foreign key linking to renewable.incentive_program. Business justification: Customers enroll in renewable incentive programs through utility billing systems for solar rebates, green tariffs, and DER programs. Enrollment tracking links customer accounts to program eligibility,',
    `sgip_enrollment_id` BIGINT COMMENT 'Foreign key linking to interconnection.sgip_enrollment. Business justification: SGIP (Self-Generation Incentive Program) enrollment for battery storage requires coordinated enrollment in specific utility rate schedules (TOU rates for optimal dispatch). Business process: Program a',
    `application_date` DATE COMMENT 'Date the customer submitted the program enrollment application. Used for tracking application processing time and eligibility determination.',
    `approval_date` DATE COMMENT 'Date on which the program enrollment application was approved by the utility. Used for processing time metrics and audit trail.',
    `auto_renew_flag` BOOLEAN COMMENT 'Indicates whether the program enrollment automatically renews upon expiration (True) or requires manual re-enrollment (False). Used for enrollment lifecycle management.',
    `benefit_amount` DECIMAL(18,2) COMMENT 'Monthly or per-billing-cycle discount or credit amount provided to the customer under this program enrollment. Used for rate calculation and revenue impact analysis. Expressed in local currency.',
    `benefit_percentage` DECIMAL(18,2) COMMENT 'Percentage discount applied to eligible charges under this program enrollment (e.g., 20% discount on energy charges for CARE customers). Used when benefit is calculated as a percentage rather than fixed amount.',
    `budget_billing_amount` DECIMAL(18,2) COMMENT 'Fixed monthly payment amount for budget billing program enrollments. Calculated based on historical usage and periodically adjusted. Expressed in local currency.',
    `budget_billing_review_date` DATE COMMENT 'Next scheduled date for budget billing amount review and adjustment. Typically occurs annually or semi-annually to true-up actual vs. budgeted usage.',
    `consent_date` DATE COMMENT 'Date on which customer consent or authorization was obtained. Used for compliance tracking and audit trail.',
    `consent_method` STRING COMMENT 'Method by which customer consent was obtained: written signature, electronic acceptance, verbal authorization (recorded), or implied consent.. Valid values are `written|electronic|verbal|implied`',
    `consent_received_flag` BOOLEAN COMMENT 'Indicates whether customer consent or authorization was received for program enrollment (True/False). Required for regulatory compliance and audit purposes.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this program enrollment record was first created in the system. Used for audit trail and data lineage tracking.',
    `discount_type` STRING COMMENT 'Type of discount calculation method applied: fixed amount (flat monthly credit), percentage (percent off eligible charges), tiered (discount varies by usage tier), usage-based (discount calculated from consumption).. Valid values are `fixed_amount|percentage|tiered|usage_based`',
    `disenrollment_date` DATE COMMENT 'Date the customer was disenrolled or removed from the program. Null for active enrollments. Used for churn analysis and program retention metrics.',
    `disenrollment_initiated_by` STRING COMMENT 'Party that initiated the disenrollment: customer (voluntary opt-out), utility (administrative removal), automatic (system-triggered expiration), regulator (compliance action).. Valid values are `customer|utility|automatic|regulator`',
    `disenrollment_reason` STRING COMMENT 'Reason for program disenrollment: customer request (voluntary withdrawal), ineligibility (no longer qualifies), non-payment (payment arrangement breach), moved (service transfer), deceased, program ended (utility discontinued), failed re-certification (did not complete renewal). [ENUM-REF-CANDIDATE: customer_request|ineligibility|non_payment|moved|deceased|program_ended|failed_recertification — 7 candidates stripped; promote to reference product]',
    `effective_end_date` DATE COMMENT 'Date on which the program enrollment expires or terminates. Null for open-ended enrollments. Used for automatic disenrollment processing and re-certification scheduling.',
    `effective_start_date` DATE COMMENT 'Date from which the program benefits and terms become effective for billing and rate calculation purposes. May differ from enrollment date due to billing cycle alignment.',
    `eligibility_verification_date` DATE COMMENT 'Date on which eligibility verification was completed and approved. Used for audit trail and compliance reporting to regulatory authorities.',
    `eligibility_verification_method` STRING COMMENT 'Method used to verify customer eligibility for the program: income documentation (pay stubs/tax returns), categorical eligibility (participation in other assistance programs), third-party verification (government database check), self-certification, or automatic enrollment.. Valid values are `income_documentation|categorical_eligibility|third_party_verification|self_certification|automatic_enrollment`',
    `eligibility_verification_status` STRING COMMENT 'Status of the eligibility verification process for income-qualified or regulated programs: not verified (initial state), verified (eligibility confirmed), pending verification (under review), verification failed (did not meet criteria), exempt (no verification required).. Valid values are `not_verified|verified|pending_verification|verification_failed|exempt`',
    `enrollment_channel` STRING COMMENT 'Channel through which the customer enrolled in the program: online portal, phone call center, mail application, in-person at office, automatic enrollment by utility, or third-party agency.. Valid values are `online|phone|mail|in_person|automatic|third_party`',
    `enrollment_date` DATE COMMENT 'Date the customer was officially enrolled in the program and benefits became effective. Key business event timestamp for program participation lifecycle.',
    `enrollment_number` STRING COMMENT 'Business-facing unique enrollment number assigned to this program participation record. Used for customer communication and service inquiries.',
    `enrollment_status` STRING COMMENT 'Current lifecycle status of the program enrollment: pending (application submitted), active (enrolled and receiving benefits), suspended (temporarily inactive), expired (term ended), cancelled (customer or utility terminated), denied (application rejected).. Valid values are `pending|active|suspended|expired|cancelled|denied`',
    `expiration_date` DATE COMMENT 'Scheduled expiration date for time-limited program enrollments requiring periodic re-certification (e.g., annual CARE/FERA re-qualification). Triggers re-certification workflow.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this program enrollment record was last updated. Used for change tracking and audit trail.',
    `last_recertification_date` DATE COMMENT 'Date of the most recent successful re-certification or eligibility verification. Used to calculate next re-certification due date and track compliance history.',
    `notes` STRING COMMENT 'Free-text notes or comments regarding the program enrollment. Used for documenting special circumstances, customer requests, or processing exceptions.',
    `priority_flag` BOOLEAN COMMENT 'Indicates whether this enrollment has priority status for special handling (True/False). Used for life support, medical baseline, or critical care programs requiring expedited service.',
    `program_code` STRING COMMENT 'Standardized code identifying the utility billing program type (e.g., CARE, FERA, BUDGET, AUTOPAY, PAPERLESS, ARREARAGE, TOU, CPP, NEM, DR).',
    `program_name` STRING COMMENT 'Full descriptive name of the utility billing program (e.g., California Alternate Rates for Energy, Family Electric Rate Assistance, Budget Billing Plan, Auto-Pay Enrollment, Paperless Billing, Arrearage Management Program).',
    `program_type` STRING COMMENT 'High-level classification of the program category: rate assistance programs (CARE/FERA), payment plans (budget billing/arrearage management), billing convenience (auto-pay/paperless), energy efficiency, demand response, net metering, or other. [ENUM-REF-CANDIDATE: rate_assistance|payment_plan|billing_convenience|energy_efficiency|demand_response|net_metering|other — 7 candidates stripped; promote to reference product]',
    `puc_approval_number` STRING COMMENT 'PUC decision or resolution number authorizing this program. Required for regulatory compliance and audit documentation.',
    `rate_schedule_code` STRING COMMENT 'Rate schedule or tariff code applicable to this program enrollment. Used for billing calculation and rate application. May differ from standard rate for discount programs.',
    `recertification_due_date` DATE COMMENT 'Date by which the customer must complete re-certification to maintain program eligibility. Applicable to income-qualified and regulatory programs requiring periodic eligibility verification.',
    `regulatory_program_code` STRING COMMENT 'Regulatory or compliance program code assigned by the Public Utility Commission (PUC) or governing body. Used for regulatory reporting and cost recovery tracking.',
    CONSTRAINT pk_billing_program_enrollment PRIMARY KEY(`billing_program_enrollment_id`)
) COMMENT 'Transactional master entity representing a customer account enrollment in a utility billing program such as CARE, FERA, budget billing, auto-pay, paperless billing, or arrearage management. Captures enrollment date, program type, eligibility verification status, benefit amount, enrollment expiration date, re-certification due date, and disenrollment reason. Managed within Oracle CC&B and SAP IS-U. SSOT for all customer program participation records in the billing domain.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` (
    `ar_ledger_id` BIGINT COMMENT 'Primary key for ar_ledger',
    `account_id` BIGINT COMMENT 'Reference to the customer account associated with this AR ledger entry. Links to the customer account master.',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: AR ledger entries directly map to GL accounts for subledger-to-GL reconciliation. Essential for month-end close, external audits, SOX compliance, and ensuring billing subledger balances tie to general',
    `ar_invoice_id` BIGINT COMMENT 'Reference to the invoice document that originated this AR ledger entry, if applicable. Null for non-invoice postings such as payments or adjustments.',
    `collections_id` BIGINT COMMENT 'Reference to the associated collections case if this AR ledger entry is part of an active collections effort. Null if no collections case exists.',
    `invoice_id` BIGINT COMMENT 'FK to billing.invoice.invoice_id — AR ledger postings reference specific invoices for open item management. Required for FICA clearing and SOX controls.',
    `payment_arrangement_id` BIGINT COMMENT 'Reference to the payment arrangement plan if this AR ledger entry is covered by an installment agreement. Null if no payment arrangement exists.',
    `employee_id` BIGINT COMMENT 'User ID of the authorized approver who approved this write-off. Null for non-write-off entries. Required for SOX internal controls and audit trail.',
    `primary_invoice_id` BIGINT COMMENT 'FK to billing.invoice.invoice_id — AR ledger entries reference the source invoice document for open item management and clearing.',
    `aging_bucket` STRING COMMENT 'Classification of this AR ledger entry based on the number of days past due. Used for aging reports and collections prioritization. Calculated from posting date and due date.. Valid values are `current|1_30_days|31_60_days|61_90_days|over_90_days`',
    `ar_to_sa` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — AR ledger entries are posted for a specific service agreement. This FK is the backbone of the customer AR sub-ledger.',
    `bad_debt_gl_account_code` STRING COMMENT 'The general ledger account code for bad debt expense associated with this write-off. Null for non-write-off entries. Used for financial statement preparation.',
    `clearing_date` DATE COMMENT 'The date on which this AR ledger entry was cleared (fully paid or offset). Null for open items.',
    `created_timestamp` TIMESTAMP COMMENT 'The date and time when this AR ledger entry record was first created in the system. Used for audit trail and data lineage tracking.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the amounts in this AR ledger entry. Typically USD for US utilities.. Valid values are `^[A-Z]{3}$`',
    `debit_credit_indicator` STRING COMMENT 'Indicates whether this AR ledger entry is a debit (increases receivable balance) or credit (decreases receivable balance). Invoices and adjustments are typically debits; payments and refunds are credits.. Valid values are `debit|credit`',
    `dispute_date` DATE COMMENT 'The date on which the customer initiated a dispute for this AR ledger entry. Null if no dispute exists.',
    `dispute_flag` BOOLEAN COMMENT 'Indicates whether this AR ledger entry is under dispute by the customer. True if disputed; false otherwise. Disputed items may be excluded from collections activity.',
    `dispute_reason_code` STRING COMMENT 'Standardized code indicating the reason for the customer dispute. Examples include billing error, meter reading issue, rate dispute. Null if no dispute exists.',
    `document_date` DATE COMMENT 'The business date of the underlying transaction or event that gave rise to this AR ledger entry. May differ from posting date for backdated or future-dated transactions.',
    `document_number` STRING COMMENT 'The externally-known unique document number for this AR ledger entry as recorded in the financial system. Used for audit trail and reconciliation.',
    `document_type` STRING COMMENT 'Type of financial document represented by this AR ledger entry. Determines the nature of the posting and its impact on the customer balance. [ENUM-REF-CANDIDATE: invoice|payment|adjustment|write-off|refund|credit_memo|debit_memo|reversal — 8 candidates stripped; promote to reference product]',
    `due_date` DATE COMMENT 'The date by which payment is expected for this AR ledger entry. Used for aging calculations and collections prioritization. Null for non-invoice postings.',
    `fiscal_period` STRING COMMENT 'The fiscal period (month) within the fiscal year in which this AR ledger entry was posted. Used for monthly financial reporting and variance analysis.',
    `fiscal_year` STRING COMMENT 'The fiscal year in which this AR ledger entry was posted. Used for period-based financial reporting and year-end closing processes.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'The date and time when this AR ledger entry record was last updated in the system. Used for audit trail and change tracking.',
    `notes` STRING COMMENT 'Free-form text field for additional comments, explanations, or special handling instructions related to this AR ledger entry. Used for audit trail and customer service reference.',
    `original_amount` DECIMAL(18,2) COMMENT 'The original posted amount of this AR ledger entry in the transaction currency. Represents the gross amount before any partial payments or adjustments.',
    `outstanding_amount` DECIMAL(18,2) COMMENT 'The remaining unpaid balance of this AR ledger entry in the transaction currency. Equals original amount minus any payments or offsets applied. Zero for cleared items.',
    `payment_method_code` STRING COMMENT 'The payment method used for this AR ledger entry if it represents a payment. Examples include check, credit card, ACH, cash. Null for non-payment entries.',
    `payment_reference_number` STRING COMMENT 'External reference number for the payment transaction, such as check number, credit card authorization code, or ACH trace number. Null for non-payment entries.',
    `posting_date` DATE COMMENT 'The date on which this AR ledger entry was posted to the financial system. Used for period-based financial reporting and aging analysis.',
    `posting_status` STRING COMMENT 'Current lifecycle status of the AR ledger entry. Open items represent outstanding receivables; cleared items have been fully paid or offset.. Valid values are `open|cleared|partially_cleared|reversed|disputed`',
    `recovery_tracking_status` STRING COMMENT 'Status of efforts to recover a written-off receivable. Tracks whether the utility is still pursuing collection or has abandoned recovery efforts. Not applicable for non-write-off entries.. Valid values are `not_applicable|pending_recovery|recovered|abandoned`',
    `revenue_class` STRING COMMENT 'Classification of revenue type for regulatory and financial reporting purposes. Examples include residential, commercial, industrial, street lighting.',
    `revenue_recognition_date` DATE COMMENT 'The date on which revenue associated with this AR ledger entry is recognized for financial reporting purposes. May differ from posting date based on revenue recognition policies.',
    `reversal_flag` BOOLEAN COMMENT 'Indicates whether this AR ledger entry is a reversal of a previous posting. True for reversal entries; false otherwise. Used for audit trail and error correction tracking.',
    `reversed_document_number` STRING COMMENT 'The document number of the original AR ledger entry that this reversal entry is offsetting. Null for non-reversal entries. Establishes audit trail linkage.',
    `service_agreement` BIGINT COMMENT 'FK to billing.service_agreement.service_agreement_id — AR ledger postings are recorded against a service agreement/account. SSOT for customer AR balance.',
    `service_territory_code` STRING COMMENT 'Code identifying the geographic service territory or regulatory jurisdiction associated with this AR ledger entry. Used for regulatory reporting and revenue allocation.',
    `write_off_amount` DECIMAL(18,2) COMMENT 'The amount written off as uncollectable bad debt. Null for non-write-off entries. Represents the reduction in accounts receivable and corresponding bad debt expense.',
    `write_off_date` DATE COMMENT 'The date on which this receivable was written off as uncollectable. Null for non-write-off entries. Used for bad debt expense tracking and PUC reporting.',
    `write_off_flag` BOOLEAN COMMENT 'Indicates whether this AR ledger entry represents a write-off of uncollectable debt. True for write-off postings; false otherwise.',
    `write_off_reason_code` STRING COMMENT 'Standardized code indicating the reason for writing off this receivable. Used for bad debt analysis and regulatory reporting. Null for non-write-off entries.. Valid values are `bankruptcy|skip|hardship|deceased|small_balance|statute_of_limitations`',
    CONSTRAINT pk_ar_ledger PRIMARY KEY(`ar_ledger_id`)
) COMMENT 'Transactional entity representing the accounts receivable sub-ledger for customer billing activity, capturing all open item postings including invoiced amounts, payments applied, adjustments, write-offs, balance carry-forwards, and bad debt entries. Stores posting date, document type (invoice, payment, adjustment, write-off, refund), debit/credit indicator, amount, clearing status, clearing date, and GL account. For write-off postings: captures write-off date, write-off amount, reason (bankruptcy, skip, hardship, deceased), approval authority, bad debt GL account, recovery tracking status, and associated collections case reference. Sourced from SAP IS-U FICA open item management and SAP S/4HANA AR sub-ledger. SSOT for customer AR balance, write-off tracking, and bad debt management supporting SOX financial controls and PUC uncollectable expense reporting.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`refund` (
    `refund_id` BIGINT COMMENT 'Unique identifier for the refund transaction. Primary key for the refund product.',
    `account_id` BIGINT COMMENT 'Reference to the customer account that received the refund. Links to the billing account in the customer domain.',
    `bill_dispute_id` BIGINT COMMENT 'Reference to the customer dispute or complaint case that triggered this refund. Links to customer service resolution tracking.',
    `credit_memo_id` BIGINT COMMENT 'Reference to the credit memo document that authorized this refund. Links to the billing adjustment that created the refundable credit.',
    `deposit_id` BIGINT COMMENT 'FK to billing.deposit.deposit_id — Deposit refunds are a primary refund type. This FK links the refund disbursement back to the originating deposit for financial reconciliation. Note: not all refunds are deposit-related, so this is a n',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Refunds post to specific GL accounts to reverse revenue or record expense. Link enables proper financial statement presentation, cash flow tracking, and regulatory reporting of customer refunds and ov',
    `invoice_id` BIGINT COMMENT 'Reference to the original invoice that generated the credit or overpayment leading to this refund. May be null for deposit returns.',
    `payment_id` BIGINT COMMENT 'Foreign key linking to billing.payment. Business justification: Refunds can be issued for overpayments, and linking to the original payment enables full audit trail and reconciliation. Currently refund has account_id and invoice_id but no link to the payment being',
    `employee_id` BIGINT COMMENT 'System user identifier of the employee who authorized the refund. Required for SOX audit trail and segregation of duties compliance.',
    `refund_modified_by_user_employee_id` BIGINT COMMENT 'System user identifier of the employee who last modified this refund record. Required for SOX audit trail and change tracking.',
    `work_order_id` BIGINT COMMENT 'Foreign key linking to asset.work_order. Business justification: Refunds often result from service quality issues, meter inaccuracies, or outage credits documented in work orders. Customer service, audit trails, and regulatory compliance require traceability from r',
    `amount` DECIMAL(18,2) COMMENT 'The gross monetary value of the refund issued to the customer before any adjustments or withholdings.',
    `approval_date` DATE COMMENT 'The date when the refund was formally approved by an authorized user. Establishes the point of financial commitment.',
    `approval_status` STRING COMMENT 'Current state of the refund approval workflow. Tracks whether the refund has passed required authorization controls.. Valid values are `pending|approved|rejected|escalated`',
    `approval_threshold_level` STRING COMMENT 'The authorization level required for this refund based on amount thresholds and business rules. Supports tiered approval workflows.. Valid values are `auto|supervisor|manager|director|executive`',
    `bank_account_number` STRING COMMENT 'The customer bank account number for ACH or wire transfer refunds. Encrypted at rest per PCI-DSS requirements.',
    `bank_routing_number` STRING COMMENT 'The ABA routing transit number for the customer financial institution. Required for ACH and wire transfer processing.',
    `cleared_date` DATE COMMENT 'The date when the refund payment was confirmed as cleared or cashed by the customer or their bank. Used for cash flow reconciliation.',
    `cost_center_code` STRING COMMENT 'The organizational cost center responsible for this refund. Used for internal financial reporting and budget tracking.',
    `created_timestamp` TIMESTAMP COMMENT 'The system timestamp when this refund record was first created in the billing system. Used for audit trail and data lineage.',
    `currency_code` STRING COMMENT 'Three-letter ISO 4217 currency code for the refund transaction. Typically USD for US-based utilities.. Valid values are `^[A-Z]{3}$`',
    `disbursement_date` DATE COMMENT 'The date when the refund payment was physically issued or electronically transmitted to the customer or their financial institution.',
    `last_modified_timestamp` TIMESTAMP COMMENT 'The system timestamp when this refund record was last updated. Tracks the most recent change for audit and reconciliation purposes.',
    `net_refund_amount` DECIMAL(18,2) COMMENT 'The final amount disbursed to the customer after all adjustments, withholdings, and offsets have been applied.',
    `notes` STRING COMMENT 'Free-form text field for additional context, special instructions, or audit notes related to the refund transaction.',
    `originating_credit_source` STRING COMMENT 'The upstream transaction or business process that created the credit balance leading to this refund. Enables traceability to root cause.. Valid values are `payment|deposit|adjustment|rate_change|meter_correction|billing_reversal`',
    `payee_address_line1` STRING COMMENT 'Primary street address line for the refund recipient. Used for check mailing and payment verification.',
    `payee_address_line2` STRING COMMENT 'Secondary address line for apartment, suite, or unit number. Optional address component for refund delivery.',
    `payee_city` STRING COMMENT 'City name for the refund recipient address. Required for payment processing and tax reporting.',
    `payee_country_code` STRING COMMENT 'Three-letter ISO country code for the refund recipient address. Used for international payment processing and compliance.. Valid values are `^[A-Z]{3}$`',
    `payee_name` STRING COMMENT 'The legal name of the individual or entity receiving the refund. May differ from account holder for third-party payments.',
    `payee_postal_code` STRING COMMENT 'ZIP or postal code for the refund recipient address. Required for payment delivery and address validation.',
    `payee_state_province` STRING COMMENT 'State or province code for the refund recipient address. Used for tax jurisdiction determination and regulatory reporting.',
    `payment_instrument_number` STRING COMMENT 'The check number, ACH trace number, wire confirmation number, or other instrument identifier for the disbursement. Used for reconciliation.',
    `reason_code` STRING COMMENT 'Detailed code identifying the specific reason for the refund. Used for root cause analysis and process improvement tracking.',
    `reason_description` STRING COMMENT 'Human-readable explanation of why the refund was issued. Provides context for customer service and audit purposes.',
    `refund_date` DATE COMMENT 'The business date when the refund was officially issued or approved for disbursement. Principal business event timestamp for this transaction.',
    `refund_method` STRING COMMENT 'The disbursement instrument or mechanism used to deliver the refund to the customer. Determines processing time and cost.. Valid values are `check|ach|wire_transfer|account_credit|debit_card|prepaid_card`',
    `refund_number` STRING COMMENT 'Business-facing unique refund document number used for customer communication and audit trails. Externally visible identifier.',
    `refund_status` STRING COMMENT 'Current lifecycle status of the refund transaction. Tracks progression from approval through disbursement and clearing. [ENUM-REF-CANDIDATE: pending|approved|rejected|issued|cleared|cancelled|voided — 7 candidates stripped; promote to reference product]',
    `refund_type` STRING COMMENT 'Classification of the refund based on the originating business reason. Determines accounting treatment and regulatory reporting category.. Valid values are `overpayment|deposit_return|billing_error|credit_balance|rate_adjustment|meter_error`',
    `regulatory_reporting_flag` BOOLEAN COMMENT 'Indicates whether this refund must be included in regulatory filings to state PUC or FERC. True if reportable under utility commission rules.',
    `revenue_class` STRING COMMENT 'The customer class or rate category associated with this refund. Used for regulatory reporting and revenue analysis.. Valid values are `residential|commercial|industrial|agricultural|public_authority`',
    `service_territory_code` STRING COMMENT 'The geographic service territory or jurisdiction where the refunded service was provided. Used for regulatory compliance reporting.',
    `sox_control_flag` BOOLEAN COMMENT 'Indicates whether this refund is subject to SOX internal control testing and audit procedures. True for material transactions requiring enhanced controls.',
    `tax_amount` DECIMAL(18,2) COMMENT 'Portion of the refund amount representing sales tax, utility tax, or other regulatory taxes being returned to the customer.',
    CONSTRAINT pk_refund PRIMARY KEY(`refund_id`)
) COMMENT 'Transactional record of a refund issued to a customer for overpayment, deposit return, credit balance, or billing error correction. Captures refund date, amount, method (check, ACH, account credit), reason code, originating credit source, approval status, and disbursement confirmation. Sourced from SAP IS-U FICA and Oracle CC&B. Supports cash disbursement controls and SOX compliance.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` (
    `service_agreement_incentive_enrollment_id` BIGINT COMMENT 'Unique identifier for this enrollment record. Primary key for the association.',
    `billing_service_agreement_id` BIGINT COMMENT 'Foreign key to the billing service agreement participating in this enrollment.',
    `incentive_program_id` BIGINT COMMENT 'Foreign key linking to the renewable incentive program in which the service agreement is enrolled',
    `service_agreement_billing_service_agreement_id` BIGINT COMMENT 'Foreign key linking to the billing service agreement enrolled in the incentive program',
    `application_reference_number` STRING COMMENT 'External reference number from the program application or approval process for this enrollment.',
    `approved_incentive_amount` DECIMAL(18,2) COMMENT 'Total incentive amount approved for this service agreement under this specific program in USD. Explicitly identified in detection phase relationship data.',
    `compliance_status` STRING COMMENT 'Current compliance status of this enrollment indicating whether the service agreement meets ongoing program requirements. Explicitly identified in detection phase relationship data.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this enrollment record was created in the system.',
    `effective_end_date` DATE COMMENT 'Date when incentive payments cease for this enrollment. Null for active enrollments with no scheduled end date.',
    `effective_start_date` DATE COMMENT 'Date when incentive payments begin for this enrollment. May differ from enrollment_date due to approval processing time.',
    `enrollment_date` DATE COMMENT 'Date when the service agreement was enrolled into this specific incentive program. Explicitly identified in detection phase relationship data.',
    `enrollment_status` STRING COMMENT 'Current lifecycle status of this enrollment indicating whether it is actively receiving incentive payments.',
    `last_compliance_check_date` DATE COMMENT 'Date of the most recent compliance verification for this enrollment.',
    `next_payment_due_date` DATE COMMENT 'Scheduled date for the next incentive payment for this enrollment. Null for one-time payments already disbursed.',
    `payment_schedule_type` STRING COMMENT 'Type of payment schedule for this enrollment: one_time, monthly, quarterly, annual, or per_kwh_generated. Explicitly identified in detection phase relationship data.',
    `total_paid_amount` DECIMAL(18,2) COMMENT 'Cumulative amount paid to date for this specific enrollment in USD. Tracks actual payments made under this program-agreement combination. Explicitly identified in detection phase relationship data.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this enrollment record was last modified.',
    CONSTRAINT pk_service_agreement_incentive_enrollment PRIMARY KEY(`service_agreement_incentive_enrollment_id`)
) COMMENT 'This association product represents the enrollment relationship between a billing service agreement and a renewable incentive program. It captures the operational enrollment of a specific service agreement into a specific incentive program, tracking approval, payment, and compliance status. Each record links one billing service agreement to one incentive program with attributes that exist only in the context of this enrollment relationship.. Existence Justification: In energy utilities operations, a single billing service agreement can simultaneously participate in multiple renewable incentive programs (e.g., a residential solar customer enrolled in both a solar rebate program and a green tariff program and a low-income assistance program). Conversely, each incentive program serves many service agreements across the utilitys customer base. The business actively manages these enrollments as operational entities, tracking approval amounts, payment schedules, compliance status, and payment history for each program-agreement combination.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`credit_memo` (
    `credit_memo_id` BIGINT COMMENT 'Primary key for credit_memo',
    `account_id` BIGINT COMMENT 'Account to which the credit memo is applied.',
    `invoice_id` BIGINT COMMENT 'Identifier of the invoice that the credit memo offsets.',
    `profile_id` BIGINT COMMENT 'Identifier of the customer receiving the credit.',
    `reversal_of_memo_id` BIGINT COMMENT 'If this memo reverses a prior credit memo, reference to that memo.',
    `original_credit_memo_id` BIGINT COMMENT 'Self-referencing FK on credit_memo (original_credit_memo_id)',
    `adjustment_type` STRING COMMENT 'Category of financial adjustment represented by the credit memo.',
    `amount_gross` DECIMAL(18,2) COMMENT 'Total credit amount before taxes, discounts, or adjustments.',
    `amount_net` DECIMAL(18,2) COMMENT 'Final credit amount after tax and discount adjustments.',
    `approval_status` STRING COMMENT 'Current approval state of the credit memo.',
    ...
    `batch_number` STRING COMMENT 'Identifier of the batch run that created or processed the credit memo.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the credit memo record was first created in the system.',
    `currency_code` STRING COMMENT 'Three‑letter ISO 4217 code of the currency for the credit memo.',
    `discount_amount` DECIMAL(18,2) COMMENT 'Any discount applied to the credit memo.',
    `due_date` DATE COMMENT 'Date by which the credit must be applied or settled.',
    `external_reference_code` STRING COMMENT 'Identifier linking the credit memo to an external system (e.g., SAP, Oracle).',
    `is_refund` BOOLEAN COMMENT 'True if the credit memo results in a cash refund to the customer.',
    `memo_date` TIMESTAMP COMMENT 'Date and time when the credit memo was issued to the customer.',
    `memo_number` STRING COMMENT 'External reference number assigned to the credit memo for customer and accounting purposes.',
    `notes` STRING COMMENT 'Additional free‑form comments or remarks about the credit memo.',
    `original_invoice_number` STRING COMMENT 'External invoice number associated with the original charge.',
    `payment_channel` STRING COMMENT 'Channel through which the payment or refund was executed.',
    `payment_method` STRING COMMENT 'Method used to process any refund associated with the credit memo.',
    `processing_status` STRING COMMENT 'Internal status of the credit memo processing workflow.',
    `reason_code` STRING COMMENT 'Standard code indicating why the credit memo was issued (e.g., overpayment, service credit).',
    `reason_description` STRING COMMENT 'Free‑text description of the reason for the credit.',
    `reversal_flag` BOOLEAN COMMENT 'True if the credit memo is a reversal of a previous transaction.',
    `settlement_date` DATE COMMENT 'Date the credit memo was settled against the account.',
    `credit_memo_status` STRING COMMENT 'Current processing state of the credit memo.',
    `tax_amount` DECIMAL(18,2) COMMENT 'Tax component of the credit memo.',
    `tax_code` STRING COMMENT 'Applicable tax code used for tax calculation on the credit.',
    `tax_exempt_flag` BOOLEAN COMMENT 'Indicates whether the credit memo is exempt from tax.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent modification to the credit memo record.',
    `write_off_flag` BOOLEAN COMMENT 'True if the credit memo amount was written off as uncollectible.',
    `created_by` BIGINT COMMENT 'Identifier of the employee who created the credit memo record.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`billing_period` (
    ...
    `adjustment_flag` BOOLEAN COMMENT 'Indicates whether the period includes post‑billing adjustments.',
    `billing_cycle_code` STRING COMMENT 'Code that identifies the recurring billing cycle associated with the period.',
    `budgeted_amount` DECIMAL(18,2) COMMENT 'Financial budget allocated for the period before actual usage is recorded.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the billing period record was first created in the system.',
    `currency_code` STRING COMMENT 'Three‑letter ISO 4217 code of the currency used for monetary values in the period.',
    `days_in_period` STRING COMMENT 'Total count of calendar days covered by the billing period.',
    `billing_period_description` STRING COMMENT 'Free‑form text describing special characteristics of the period.',
    `effective_from` DATE COMMENT 'Date from which the billing period definition is effective for reporting.',
    `effective_until` DATE COMMENT 'Date until which the billing period definition remains effective; null for open‑ended periods.',
    `fiscal_quarter` STRING COMMENT 'Quarter of the fiscal year for the billing period.',
    `fiscal_year` STRING COMMENT 'Four‑digit fiscal year to which the billing period belongs.',
    `is_pro_rata` BOOLEAN COMMENT 'Flag indicating whether the period is calculated on a pro‑rata basis.',
    `period_end_date` DATE COMMENT 'Calendar date on which the billing period ends.',
    `period_name` STRING COMMENT 'Human‑readable name for the billing period, e.g., "Jan 2024".',
    `period_start_date` DATE COMMENT 'Calendar date on which the billing period begins.',
    `period_type` STRING COMMENT 'Classification of the period purpose, such as regular billing, adjustment, forecast, or reversal.',
    `settlement_date` DATE COMMENT 'Date on which financial settlement for the period is performed.',
    `billing_period_status` STRING COMMENT 'Current lifecycle status of the billing period.',
...
CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` (
    `disconnect_order_id` BIGINT COMMENT 'Primary key for disconnect_order',
    `account_id` BIGINT COMMENT 'Unique identifier of the customer associated with the disconnect order.',
    `employee_id` BIGINT COMMENT 'Identifier of the field technician assigned to execute the disconnect.',
    `meter_id` BIGINT COMMENT 'Unique identifier of the electric meter associated with the service point.',
    `service_point_id` BIGINT COMMENT 'Identifier of the physical location where service is delivered.',
    `superseded_disconnect_order_id` BIGINT COMMENT 'Self-referencing FK on disconnect_order (superseded_disconnect_order_id)',
    `actual_disconnect_date` DATE COMMENT 'Date on which the service was actually disconnected.',
    `adjustment_amount` DECIMAL(18,2) COMMENT 'Sum of all adjustments (taxes, fees, discounts) applied to the gross amount.',
    `approval_date` DATE COMMENT 'Date on which the disconnect order received formal approval.',
    `approved_by` STRING COMMENT 'Name or identifier of the employee who approved the disconnect order.',
    `cancellation_reason` STRING COMMENT 'Reason provided if the disconnect order was cancelled.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the disconnect order record was first persisted in the lakehouse.',
    `currency_code` STRING COMMENT 'Three‑letter ISO 4217 code of the currency used for the order.',
    `deposit_amount` DECIMAL(18,2) COMMENT 'Security deposit held for the account at the time of disconnect.',
    `disconnect_reason` STRING COMMENT 'Reason why the service disconnect is being requested or mandated.',
    `disconnect_type` STRING COMMENT 'Classification of the disconnect (e.g., voluntary customer request vs. involuntary for safety).',
    `dispute_flag` BOOLEAN COMMENT 'True if the disconnect order is under a customer dispute.',
    `estimated_duration_minutes` STRING COMMENT 'Projected time in minutes required to complete the disconnect.',
    ...
    `order_number` STRING COMMENT 'External reference number assigned to the disconnect order for customer‑facing communication.',
    `order_timestamp` TIMESTAMP COMMENT 'Date‑time when the disconnect order was created or submitted by the customer.',
    `priority` STRING COMMENT 'Urgency level assigned to the disconnect order.',
    `regulatory_flag` BOOLEAN COMMENT 'Indicates whether the order is subject to special regulatory reporting (e.g., SOX, NERC).',
    `reversal_flag` BOOLEAN COMMENT 'Indicates whether the disconnect order was later reversed or reinstated.',
    `scheduled_disconnect_date` DATE COMMENT 'Planned calendar date for the disconnection to occur.',
    `disconnect_order_status` STRING COMMENT 'Current processing state of the disconnect order.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent modification to the disconnect order record.',
    `write_off_flag` BOOLEAN COMMENT 'True if the outstanding balance was written off as uncollectible.',
    CONSTRAINT pk_disconnect_order PRIMARY KEY(`disconnect_order_id`)
) COMMENT 'Master reference table for disconnect_order. Referenced by disconnect_order_id.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`dunning_run` (
    `dunning_run_id` BIGINT COMMENT 'Primary key for dunning_run',
    `account_id` BIGINT COMMENT 'Identifier of the primary customer account targeted by the dunning run.',
    `employee_id` BIGINT COMMENT 'User responsible for initiating or overseeing the dunning run.',
    `last_run_id` BIGINT COMMENT 'Identifier of the previous dunning run that this run may be a retry of.',
    `dunning_schedule_id` BIGINT COMMENT 'Reference to the schedule definition that triggered the run.',
    `previous_dunning_run_id` BIGINT COMMENT 'Self-referencing FK on dunning_run (previous_dunning_run_id)',
    `accounting_period` STRING COMMENT 'Fiscal period (e.g., 2023Q4) for which the dunning run is accounted.',
    `adjustment_amount` DECIMAL(18,2) COMMENT 'Total of discounts, fees, or write‑offs applied during the run.',
    `batch_number` STRING COMMENT 'External batch identifier used by the processing engine.',
    `contact_method` STRING COMMENT 'Preferred communication channel used for dunning (letter, email, phone, sms).',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the dunning run record was first created in the data lake.',
    `currency_code` STRING COMMENT 'Three‑letter ISO 4217 code of the currency for monetary values.',
    `deposit_flag` BOOLEAN COMMENT 'True if the run processes deposit‑related dunning actions.',
    `dunning_run_description` STRING COMMENT 'Free‑form text describing the purpose or notes for the run.',
    `due_date` DATE COMMENT 'Date by which the outstanding balances should be collected.',
    `dunning_level` STRING COMMENT 'Highest escalation level applied in this run (e.g., 1‑3).',
    `dunning_strategy_code` STRING COMMENT 'Code representing the collection strategy (e.g., standard, aggressive).',
    `end_timestamp` TIMESTAMP COMMENT 'Timestamp when the dunning run completed or was terminated.',
    `error_count` STRING COMMENT 'Count of error records encountered during processing.',
    `failure_count` STRING COMMENT 'Number of accounts that failed processing.',
    `financial_period` STRING COMMENT 'Financial reporting period aligned with the runs revenue recognition.',
    `gl_account` STRING COMMENT 'GL account code to which dunning write‑offs are posted.',
    `gross_amount` DECIMAL(18,2) COMMENT 'Sum of original invoice amounts before any adjustments.',
    `net_amount` DECIMAL(18,2) COMMENT 'Final amount due after adjustments; used for accounting postings.',
    `notes` STRING COMMENT 'Additional operational comments captured during execution.',
    `payment_plan_flag` BOOLEAN COMMENT 'Indicates whether the run includes accounts on a payment arrangement.',
    `priority_code` STRING COMMENT 'Business priority assigned to the run for scheduling purposes.',
    `reconciliation_status` STRING COMMENT 'Status of financial reconciliation for the runs postings.',
    `region_code` STRING COMMENT 'Geographic region identifier (e.g., three‑letter ISO region) associated with the run.',
    `retry_count` STRING COMMENT 'Number of times the run has been re‑executed after failure.',
    `run_comment` STRING COMMENT 'Additional free‑form comment captured at run completion.',
    `run_number` STRING COMMENT 'Human‑readable sequential number assigned to the dunning run.',
    `run_type` STRING COMMENT 'Indicates whether the run was generated automatically by the system or initiated manually.',
    `scheduled_timestamp` TIMESTAMP COMMENT 'Planned start time for the dunning run as defined by scheduling logic.',
    `start_timestamp` TIMESTAMP COMMENT 'Timestamp when the dunning run processing began.',
    `dunning_run_status` STRING COMMENT 'Current lifecycle status of the dunning run.',
    `success_count` STRING COMMENT 'Number of accounts successfully processed without errors.',
    `tax_code` STRING COMMENT 'Tax classification applied to adjustments within the run.',
    `territory_code` STRING COMMENT 'Sub‑regional sales or service territory linked to the accounts in the run.',
    `total_accounts` STRING COMMENT 'Number of distinct accounts included in this dunning run.',
    `total_outstanding_amount` DECIMAL(18,2) COMMENT 'Aggregate amount overdue across all accounts in the run.',
    `updated_by` BIGINT COMMENT 'System user identifier who last modified the dunning run record.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to the dunning run record.',
    `created_by` BIGINT COMMENT 'System user identifier who created the dunning run record.',
    CONSTRAINT pk_dunning_run PRIMARY KEY(`dunning_run_id`)
) COMMENT 'Master reference table for dunning_run. Referenced by dunning_run_id.';

CREATE OR REPLACE TABLE `energy_utilities_ecm`.`billing`.`dunning_schedule` (
    `dunning_schedule_id` BIGINT COMMENT 'Primary key for dunning_schedule',
    `superseded_dunning_schedule_id` BIGINT COMMENT 'Self-referencing FK on dunning_schedule (superseded_dunning_schedule_id)',
    `action_type` STRING COMMENT 'Method used to contact the customer for this dunning step.',
    `amount_threshold` DECIMAL(18,2) COMMENT 'Minimum outstanding amount that must be met before this dunning step applies.',
    `compliance_code` STRING COMMENT 'Regulatory or internal compliance code associated with this dunning schedule (e.g., SOX control reference).',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the dunning schedule record was first created in the system.',
    `days_before_action` STRING COMMENT 'Number of days after the invoice due date before this dunning action is triggered.',
    `dunning_schedule_description` STRING COMMENT 'Free‑form description of the schedule purpose and any special handling notes.',
    `dunning_level` STRING COMMENT 'Ordinal level of the dunning step (e.g., 1 = first reminder, 2 = second reminder).',
    `effective_from` DATE COMMENT 'Date on which the dunning schedule becomes effective.',
    `effective_until` DATE COMMENT 'Date on which the dunning schedule expires; null for open‑ended schedules.',
    `escalation_level` STRING COMMENT 'Numeric indicator of how far the dunning process has escalated.',
    `grace_period_days` STRING COMMENT 'Additional days granted after the action before further escalation.',
    `interest_rate` DECIMAL(18,2) COMMENT 'Interest rate applied to the overdue balance for this dunning step, expressed as a percentage.',
    `max_attempts` STRING COMMENT 'Maximum number of times this dunning step may be repeated before moving to the next level.',
    `notes` STRING COMMENT 'Additional internal comments or audit notes related to the schedule.',
    `notification_method` STRING COMMENT 'Channel used to deliver the dunning notice.',
    `penalty_fee` DECIMAL(18,2) COMMENT 'Fixed penalty fee charged when this dunning step is executed.',
    `schedule_name` STRING COMMENT 'Human‑readable name of the dunning schedule used for reporting and UI display.',
    `schedule_type` STRING COMMENT 'Classification of the schedule indicating whether it follows a standard, custom, or regulatory pattern.',
    `dunning_schedule_status` STRING COMMENT 'Current lifecycle status of the schedule.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to the dunning schedule record.',
    `waive_interest_flag` BOOLEAN COMMENT 'Indicates whether interest is waived for this dunning step (true = waived).',
    `waive_penalty_flag` BOOLEAN COMMENT 'Indicates whether the penalty fee is waived for this dunning step (true = waived).',
    CONSTRAINT pk_dunning_schedule PRIMARY KEY(`dunning_schedule_id`)
) COMMENT 'Master reference table for dunning_schedule. Referenced by schedule_id.';
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` ADD CONSTRAINT `fk_billing_invoice_line_invoice_rate_component_id` FOREIGN KEY (`invoice_rate_component_id`) REFERENCES `energy_utilities_ecm`.`billing`.`rate_component`(`rate_component_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`payment` ADD CONSTRAINT `fk_billing_payment_payment_invoice_id` FOREIGN KEY (`payment_invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`determinant` ADD CONSTRAINT `fk_billing_determinant_billing_period_id` FOREIGN KEY (`billing_period_id`) REFERENCES `energy_utilities_ecm`.`billing`.`billing_period`(`billing_period_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_adjustment_invoice_id` FOREIGN KEY (`adjustment_invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ADD CONSTRAINT `fk_billing_adjustment_bill_dispute_id` FOREIGN KEY (`bill_dispute_id`) REFERENCES `energy_utilities_ecm`.`billing`.`bill_dispute`(`bill_dispute_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` ADD CONSTRAINT `fk_billing_dunning_notice_bill_dispute_id` FOREIGN KEY (`bill_dispute_id`) REFERENCES `energy_utilities_ecm`.`billing`.`bill_dispute`(`bill_dispute_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` ADD CONSTRAINT `fk_billing_dunning_notice_disconnect_order_id` FOREIGN KEY (`disconnect_order_id`) REFERENCES `energy_utilities_ecm`.`billing`.`disconnect_order`(`disconnect_order_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` ADD CONSTRAINT `fk_billing_dunning_notice_dunning_run_id` FOREIGN KEY (`dunning_run_id`) REFERENCES `energy_utilities_ecm`.`billing`.`dunning_run`(`dunning_run_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ADD CONSTRAINT `fk_billing_bill_dispute_bill_invoice_id` FOREIGN KEY (`bill_invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ADD CONSTRAINT `fk_billing_bill_dispute_credit_memo_id` FOREIGN KEY (`credit_memo_id`) REFERENCES `energy_utilities_ecm`.`billing`.`credit_memo`(`credit_memo_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ADD CONSTRAINT `fk_billing_bill_dispute_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ADD CONSTRAINT `fk_billing_nem_credit_billing_service_agreement_id` FOREIGN KEY (`billing_service_agreement_id`) REFERENCES `energy_utilities_ecm`.`billing`.`billing_service_agreement`(`billing_service_agreement_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ADD CONSTRAINT `fk_billing_nem_credit_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ADD CONSTRAINT `fk_billing_nem_credit_nem_for_agreement` FOREIGN KEY (`nem_for_agreement`) REFERENCES `energy_utilities_ecm`.`billing`.`billing_service_agreement`(`billing_service_agreement_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ADD CONSTRAINT `fk_billing_nem_credit_original_credit_nem_credit_id` FOREIGN KEY (`original_credit_nem_credit_id`) REFERENCES `energy_utilities_ecm`.`billing`.`nem_credit`(`nem_credit_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ADD CONSTRAINT `fk_billing_nem_credit_rate_schedule_id` FOREIGN KEY (`rate_schedule_id`) REFERENCES `energy_utilities_ecm`.`billing`.`rate_schedule`(`rate_schedule_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ADD CONSTRAINT `fk_billing_billing_program_enrollment_payment_arrangement_id` FOREIGN KEY (`payment_arrangement_id`) REFERENCES `energy_utilities_ecm`.`billing`.`payment_arrangement`(`payment_arrangement_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ADD CONSTRAINT `fk_billing_ar_ledger_ar_invoice_id` FOREIGN KEY (`ar_invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ADD CONSTRAINT `fk_billing_ar_ledger_collections_id` FOREIGN KEY (`collections_id`) REFERENCES `energy_utilities_ecm`.`billing`.`collections`(`collections_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ADD CONSTRAINT `fk_billing_ar_ledger_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ADD CONSTRAINT `fk_billing_ar_ledger_payment_arrangement_id` FOREIGN KEY (`payment_arrangement_id`) REFERENCES `energy_utilities_ecm`.`billing`.`payment_arrangement`(`payment_arrangement_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ADD CONSTRAINT `fk_billing_ar_ledger_primary_invoice_id` FOREIGN KEY (`primary_invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_bill_dispute_id` FOREIGN KEY (`bill_dispute_id`) REFERENCES `energy_utilities_ecm`.`billing`.`bill_dispute`(`bill_dispute_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_credit_memo_id` FOREIGN KEY (`credit_memo_id`) REFERENCES `energy_utilities_ecm`.`billing`.`credit_memo`(`credit_memo_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_deposit_id` FOREIGN KEY (`deposit_id`) REFERENCES `energy_utilities_ecm`.`billing`.`deposit`(`deposit_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ADD CONSTRAINT `fk_billing_refund_payment_id` FOREIGN KEY (`payment_id`) REFERENCES `energy_utilities_ecm`.`billing`.`payment`(`payment_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ADD CONSTRAINT `fk_billing_service_agreement_incentive_enrollment_billing_service_agreement_id` FOREIGN KEY (`billing_service_agreement_id`) REFERENCES `energy_utilities_ecm`.`billing`.`billing_service_agreement`(`billing_service_agreement_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ADD CONSTRAINT `fk_billing_service_agreement_incentive_enrollment_service_agreement_billing_service_agreement_id` FOREIGN KEY (`service_agreement_billing_service_agreement_id`) REFERENCES `energy_utilities_ecm`.`billing`.`billing_service_agreement`(`billing_service_agreement_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` ADD CONSTRAINT `fk_billing_credit_memo_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `energy_utilities_ecm`.`billing`.`invoice`(`invoice_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` ADD CONSTRAINT `fk_billing_credit_memo_reversal_of_memo_id` FOREIGN KEY (`reversal_of_memo_id`) REFERENCES `energy_utilities_ecm`.`billing`.`credit_memo`(`credit_memo_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` ADD CONSTRAINT `fk_billing_credit_memo_original_credit_memo_id` FOREIGN KEY (`original_credit_memo_id`) REFERENCES `energy_utilities_ecm`.`billing`.`credit_memo`(`credit_memo_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_period` ADD CONSTRAINT `fk_billing_billing_period_previous_billing_period_id` FOREIGN KEY (`previous_billing_period_id`) REFERENCES `energy_utilities_ecm`.`billing`.`billing_period`(`billing_period_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ADD CONSTRAINT `fk_billing_disconnect_order_superseded_disconnect_order_id` FOREIGN KEY (`superseded_disconnect_order_id`) REFERENCES `energy_utilities_ecm`.`billing`.`disconnect_order`(`disconnect_order_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` ADD CONSTRAINT `fk_billing_dunning_run_last_run_id` FOREIGN KEY (`last_run_id`) REFERENCES `energy_utilities_ecm`.`billing`.`dunning_run`(`dunning_run_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` ADD CONSTRAINT `fk_billing_dunning_run_dunning_schedule_id` FOREIGN KEY (`dunning_schedule_id`) REFERENCES `energy_utilities_ecm`.`billing`.`dunning_schedule`(`dunning_schedule_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` ADD CONSTRAINT `fk_billing_dunning_run_previous_dunning_run_id` FOREIGN KEY (`previous_dunning_run_id`) REFERENCES `energy_utilities_ecm`.`billing`.`dunning_run`(`dunning_run_id`);
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_schedule` ADD CONSTRAINT `fk_billing_dunning_schedule_superseded_dunning_schedule_id` FOREIGN KEY (`superseded_dunning_schedule_id`) REFERENCES `energy_utilities_ecm`.`billing`.`dunning_schedule`(`dunning_schedule_id`);
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`rate_schedule` ALTER COLUMN `irp_scenario_id` SET TAGS ('dbx_business_glossary_term' = 'Irp Scenario Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`rate_schedule` ALTER COLUMN `lmp_price_id` SET TAGS ('dbx_business_glossary_term' = 'Lmp Price Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`rate_schedule` ALTER COLUMN `peak_demand_id` SET TAGS ('dbx_business_glossary_term' = 'Peak Demand Forecast Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`rate_schedule` ALTER COLUMN `incentive_program_id` SET TAGS ('dbx_business_glossary_term' = 'Renewable Incentive Program Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `energy_price_id` SET TAGS ('dbx_business_glossary_term' = 'Energy Price Forecast Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`rate_component` ALTER COLUMN `fuel_price_assumption_id` SET TAGS ('dbx_business_glossary_term' = 'Fuel Price Assumption Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` SET TAGS ('dbx_subdomain' = 'invoice_processing');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` ALTER COLUMN `meter_point_id` SET TAGS ('dbx_business_glossary_term' = 'Meter Point Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` ALTER COLUMN `nem_account_id` SET TAGS ('dbx_business_glossary_term' = 'Nem Account Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_service_agreement` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice` SET TAGS ('dbx_subdomain' = 'invoice_processing');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice` ALTER COLUMN `trade_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` SET TAGS ('dbx_subdomain' = 'invoice_processing');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `invoice_rate_component_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Component Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `sox_control_test_id` SET TAGS ('dbx_business_glossary_term' = 'Sox Control Test Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`invoice_line` ALTER COLUMN `trade_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`payment` ALTER COLUMN `payment_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`payment_arrangement` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`payment_arrangement` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`payment_arrangement` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`payment_arrangement` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Interconnection Application Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`determinant` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`determinant` ALTER COLUMN `billing_period_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Period ID');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `adjustment_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `bill_dispute_id` SET TAGS ('dbx_business_glossary_term' = 'Dispute Case Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Cost Center Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `dr_incentive_payment_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Incentive Payment Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`adjustment` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Gl Account Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`deposit` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`deposit` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`deposit` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`deposit` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Deposit Gl Account Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` ALTER COLUMN `bill_dispute_id` SET TAGS ('dbx_business_glossary_term' = 'Dispute Case ID');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` ALTER COLUMN `disconnect_order_id` SET TAGS ('dbx_business_glossary_term' = 'Disconnect Order ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_notice` ALTER COLUMN `dunning_run_id` SET TAGS ('dbx_business_glossary_term' = 'Dunning Run ID');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`collections` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Collector ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`collections` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`collections` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Employee Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Write Off Cost Center Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Write Off Gl Account Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`write_off` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,6}$');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`budget_plan` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`budget_plan` ALTER COLUMN `load_id` SET TAGS ('dbx_business_glossary_term' = 'Forecast Load Forecast Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` SET TAGS ('dbx_subdomain' = 'invoice_processing');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `cycle_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Operator Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `cycle_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `cycle_employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`cycle` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{2,6}$');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_run` SET TAGS ('dbx_subdomain' = 'invoice_processing');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Operator Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_run` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`revenue_recognition` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`revenue_recognition` ALTER COLUMN `audit_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`revenue_recognition` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Journal Entry Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`revenue_recognition` ALTER COLUMN `revenue_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`revenue_recognition` ALTER COLUMN `sox_control_id` SET TAGS ('dbx_business_glossary_term' = 'Sox Control Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` SET TAGS ('dbx_subdomain' = 'payment_collections');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `bill_dispute_id` SET TAGS ('dbx_business_glossary_term' = 'Bill Dispute Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `bill_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `credit_memo_id` SET TAGS ('dbx_business_glossary_term' = 'Credit Memo Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `grid_reliability_event_id` SET TAGS ('dbx_business_glossary_term' = 'Grid Reliability Event Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `meter_point_id` SET TAGS ('dbx_business_glossary_term' = 'Meter Point Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `adjustment_amount` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `assigned_to_team` SET TAGS ('dbx_business_glossary_term' = 'Assigned To Team');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `created_by_user` SET TAGS ('dbx_business_glossary_term' = 'Created By User');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `customer_satisfaction_rating` SET TAGS ('dbx_business_glossary_term' = 'Customer Satisfaction Rating');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_number` SET TAGS ('dbx_business_glossary_term' = 'Dispute Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_open_date` SET TAGS ('dbx_business_glossary_term' = 'Dispute Open Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_open_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Dispute Open Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Dispute Reason Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Dispute Reason Description');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_source_channel` SET TAGS ('dbx_business_glossary_term' = 'Dispute Source Channel');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `dispute_status` SET TAGS ('dbx_business_glossary_term' = 'Dispute Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `disputed_amount` SET TAGS ('dbx_business_glossary_term' = 'Disputed Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `field_inspection_date` SET TAGS ('dbx_business_glossary_term' = 'Field Inspection Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `field_inspection_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Field Inspection Required Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `investigation_notes` SET TAGS ('dbx_business_glossary_term' = 'Investigation Notes');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `investigation_start_date` SET TAGS ('dbx_business_glossary_term' = 'Investigation Start Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `meter_test_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Meter Test Required Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `meter_test_result` SET TAGS ('dbx_business_glossary_term' = 'Meter Test Result');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `meter_test_result` SET TAGS ('dbx_value_regex' = 'passed|failed|inconclusive|not_performed');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `preventable_flag` SET TAGS ('dbx_business_glossary_term' = 'Preventable Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `puc_complaint_number` SET TAGS ('dbx_business_glossary_term' = 'Public Utility Commission (PUC) Complaint Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `puc_escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Public Utility Commission (PUC) Escalation Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `regulatory_escalation_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Escalation Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Resolution Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `resolution_notes` SET TAGS ('dbx_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `resolution_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Resolution Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `resolution_type` SET TAGS ('dbx_business_glossary_term' = 'Resolution Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `root_cause_category` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Category');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `sla_breach_flag` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Breach Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`bill_dispute` ALTER COLUMN `sla_target_resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Target Resolution Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `nem_credit_id` SET TAGS ('dbx_business_glossary_term' = 'Nem Credit Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `billing_service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Service Agreement Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `compliance_rec_certificate_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Rec Certificate Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `der_interconnection_point_id` SET TAGS ('dbx_business_glossary_term' = 'Der Interconnection Point Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `der_registry_id` SET TAGS ('dbx_business_glossary_term' = 'Der Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `dr_program_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Program Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `registry_id` SET TAGS ('dbx_business_glossary_term' = 'Generation Asset Registry Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `lmp_price_id` SET TAGS ('dbx_business_glossary_term' = 'Lmp Price Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `net_energy_metering_id` SET TAGS ('dbx_business_glossary_term' = 'Net Energy Metering Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `original_credit_nem_credit_id` SET TAGS ('dbx_business_glossary_term' = 'Original Credit ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `rate_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Schedule ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `rec_holding_id` SET TAGS ('dbx_business_glossary_term' = 'Rec Holding Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `ppa_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Renewable Ppa Contract Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `adjustment_flag` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `adjustment_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Adjustment Reason Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `billing_cycle_code` SET TAGS ('dbx_business_glossary_term' = 'Billing Cycle Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `created_by_user` SET TAGS ('dbx_business_glossary_term' = 'Created By User');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_amount` SET TAGS ('dbx_business_glossary_term' = 'Credit Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_applied_amount` SET TAGS ('dbx_business_glossary_term' = 'Credit Applied Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Credit Expiration Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Credit Period End Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Credit Period Start Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_rate_per_kwh` SET TAGS ('dbx_business_glossary_term' = 'Credit Rate per Kilowatt-Hour (kWh)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_status` SET TAGS ('dbx_business_glossary_term' = 'Credit Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_status` SET TAGS ('dbx_value_regex' = 'earned|applied|expired|forfeited|pending|disputed');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_transaction_number` SET TAGS ('dbx_business_glossary_term' = 'Credit Transaction Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_type` SET TAGS ('dbx_business_glossary_term' = 'Credit Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `credit_type` SET TAGS ('dbx_value_regex' = 'energy_export|capacity_credit|renewable_energy_certificate|ancillary_service');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `cumulative_credit_balance` SET TAGS ('dbx_business_glossary_term' = 'Cumulative Credit Balance');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `dispute_flag` SET TAGS ('dbx_business_glossary_term' = 'Dispute Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `dispute_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Dispute Reason Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `exported_energy_kwh` SET TAGS ('dbx_business_glossary_term' = 'Exported Energy Kilowatt-Hours (kWh)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `generation_technology_type` SET TAGS ('dbx_business_glossary_term' = 'Generation Technology Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `generation_technology_type` SET TAGS ('dbx_value_regex' = 'solar_pv|battery_storage|wind|combined_heat_power|fuel_cell|other');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `installed_capacity_kw` SET TAGS ('dbx_business_glossary_term' = 'Installed Capacity Kilowatts (kW)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `interconnection_agreement_number` SET TAGS ('dbx_business_glossary_term' = 'Interconnection Agreement Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `jurisdiction_code` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `meter_number` SET TAGS ('dbx_business_glossary_term' = 'Meter Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `modified_by_user` SET TAGS ('dbx_business_glossary_term' = 'Modified By User');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `nem_tariff_code` SET TAGS ('dbx_business_glossary_term' = 'Net Energy Metering (NEM) Tariff Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `nem_tariff_version` SET TAGS ('dbx_business_glossary_term' = 'Net Energy Metering (NEM) Tariff Version');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `posted_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Posted Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `regulatory_account_code` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Account Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `renewable_energy_certificate_flag` SET TAGS ('dbx_business_glossary_term' = 'Renewable Energy Certificate (REC) Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `rps_compliance_eligible_flag` SET TAGS ('dbx_business_glossary_term' = 'Renewable Portfolio Standard (RPS) Compliance Eligible Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `tou_period_code` SET TAGS ('dbx_business_glossary_term' = 'Time of Use (TOU) Period Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `true_up_period_flag` SET TAGS ('dbx_business_glossary_term' = 'True-Up Period Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`nem_credit` ALTER COLUMN `true_up_settlement_date` SET TAGS ('dbx_business_glossary_term' = 'True-Up Settlement Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`assistance_program` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`assistance_program` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`assistance_program` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Interconnection Application Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`assistance_program` ALTER COLUMN `regulatory_filing_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Filing Id (Foreign Key)');
...
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `billing_program_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Program Enrollment Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `billing_last_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `billing_last_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `billing_last_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Program Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `demand_dr_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Dr Enrollment Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `green_tariff_subscription_id` SET TAGS ('dbx_business_glossary_term' = 'Green Tariff Subscription Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `payment_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Arrangement Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `premise_id` SET TAGS ('dbx_business_glossary_term' = 'Premise Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `incentive_program_id` SET TAGS ('dbx_business_glossary_term' = 'Renewable Incentive Program Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `sgip_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Sgip Enrollment Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `application_date` SET TAGS ('dbx_business_glossary_term' = 'Application Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `auto_renew_flag` SET TAGS ('dbx_business_glossary_term' = 'Auto-Renew Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `benefit_amount` SET TAGS ('dbx_business_glossary_term' = 'Benefit Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `benefit_percentage` SET TAGS ('dbx_business_glossary_term' = 'Benefit Percentage');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `budget_billing_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Billing Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `budget_billing_review_date` SET TAGS ('dbx_business_glossary_term' = 'Budget Billing Review Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `consent_date` SET TAGS ('dbx_business_glossary_term' = 'Consent Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `consent_method` SET TAGS ('dbx_business_glossary_term' = 'Consent Method');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `consent_method` SET TAGS ('dbx_value_regex' = 'written|electronic|verbal|implied');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `consent_received_flag` SET TAGS ('dbx_business_glossary_term' = 'Consent Received Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `discount_type` SET TAGS ('dbx_business_glossary_term' = 'Discount Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `discount_type` SET TAGS ('dbx_value_regex' = 'fixed_amount|percentage|tiered|usage_based');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `disenrollment_date` SET TAGS ('dbx_business_glossary_term' = 'Disenrollment Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `disenrollment_initiated_by` SET TAGS ('dbx_business_glossary_term' = 'Disenrollment Initiated By');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `disenrollment_initiated_by` SET TAGS ('dbx_value_regex' = 'customer|utility|automatic|regulator');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `disenrollment_reason` SET TAGS ('dbx_business_glossary_term' = 'Disenrollment Reason');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `eligibility_verification_date` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Verification Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `eligibility_verification_method` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Verification Method');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `eligibility_verification_method` SET TAGS ('dbx_value_regex' = 'income_documentation|categorical_eligibility|third_party_verification|self_certification|automatic_enrollment');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `eligibility_verification_status` SET TAGS ('dbx_business_glossary_term' = 'Eligibility Verification Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `eligibility_verification_status` SET TAGS ('dbx_value_regex' = 'not_verified|verified|pending_verification|verification_failed|exempt');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `enrollment_channel` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Channel');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `enrollment_channel` SET TAGS ('dbx_value_regex' = 'online|phone|mail|in_person|automatic|third_party');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `enrollment_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `enrollment_number` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `enrollment_status` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `enrollment_status` SET TAGS ('dbx_value_regex' = 'pending|active|suspended|expired|cancelled|denied');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Expiration Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `last_recertification_date` SET TAGS ('dbx_business_glossary_term' = 'Last Re-certification Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `priority_flag` SET TAGS ('dbx_business_glossary_term' = 'Priority Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `program_code` SET TAGS ('dbx_business_glossary_term' = 'Program Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `program_name` SET TAGS ('dbx_business_glossary_term' = 'Program Name');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `program_type` SET TAGS ('dbx_business_glossary_term' = 'Program Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `puc_approval_number` SET TAGS ('dbx_business_glossary_term' = 'Public Utility Commission (PUC) Approval Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `rate_schedule_code` SET TAGS ('dbx_business_glossary_term' = 'Rate Schedule Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `recertification_due_date` SET TAGS ('dbx_business_glossary_term' = 'Re-certification Due Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_program_enrollment` ALTER COLUMN `regulatory_program_code` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Program Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` SET TAGS ('dbx_subdomain' = 'payment_collections');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `ar_ledger_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Ledger Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Gl Account Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `ar_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `collections_id` SET TAGS ('dbx_business_glossary_term' = 'Collections Case ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `payment_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Arrangement ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Approval User ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `aging_bucket` SET TAGS ('dbx_business_glossary_term' = 'Aging Bucket');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `aging_bucket` SET TAGS ('dbx_value_regex' = 'current|1_30_days|31_60_days|61_90_days|over_90_days');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `bad_debt_gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'Bad Debt General Ledger (GL) Account Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `clearing_date` SET TAGS ('dbx_business_glossary_term' = 'Clearing Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `debit_credit_indicator` SET TAGS ('dbx_business_glossary_term' = 'Debit/Credit Indicator');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `debit_credit_indicator` SET TAGS ('dbx_value_regex' = 'debit|credit');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `dispute_date` SET TAGS ('dbx_business_glossary_term' = 'Dispute Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `dispute_flag` SET TAGS ('dbx_business_glossary_term' = 'Dispute Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `dispute_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Dispute Reason Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `document_date` SET TAGS ('dbx_business_glossary_term' = 'Document Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `document_number` SET TAGS ('dbx_business_glossary_term' = 'Document Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `document_type` SET TAGS ('dbx_business_glossary_term' = 'Document Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Due Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `fiscal_period` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `original_amount` SET TAGS ('dbx_business_glossary_term' = 'Original Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `outstanding_amount` SET TAGS ('dbx_business_glossary_term' = 'Outstanding Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `payment_method_code` SET TAGS ('dbx_business_glossary_term' = 'Payment Method Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `payment_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Payment Reference Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `posting_date` SET TAGS ('dbx_business_glossary_term' = 'Posting Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `posting_status` SET TAGS ('dbx_business_glossary_term' = 'Posting Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `posting_status` SET TAGS ('dbx_value_regex' = 'open|cleared|partially_cleared|reversed|disputed');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `recovery_tracking_status` SET TAGS ('dbx_business_glossary_term' = 'Recovery Tracking Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `recovery_tracking_status` SET TAGS ('dbx_value_regex' = 'not_applicable|pending_recovery|recovered|abandoned');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `revenue_class` SET TAGS ('dbx_business_glossary_term' = 'Revenue Class');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `revenue_recognition_date` SET TAGS ('dbx_business_glossary_term' = 'Revenue Recognition Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `reversal_flag` SET TAGS ('dbx_business_glossary_term' = 'Reversal Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `reversed_document_number` SET TAGS ('dbx_business_glossary_term' = 'Reversed Document Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `write_off_amount` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `write_off_date` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `write_off_flag` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `write_off_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Write-Off Reason Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`ar_ledger` ALTER COLUMN `write_off_reason_code` SET TAGS ('dbx_value_regex' = 'bankruptcy|skip|hardship|deceased|small_balance|statute_of_limitations');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` SET TAGS ('dbx_subdomain' = 'payment_collections');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_id` SET TAGS ('dbx_business_glossary_term' = 'Refund Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Account Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bill_dispute_id` SET TAGS ('dbx_business_glossary_term' = 'Dispute Case Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `credit_memo_id` SET TAGS ('dbx_business_glossary_term' = 'Credit Memo Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Refund Gl Account Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payment_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `employee_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_modified_by_user_employee_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Identifier (ID)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_modified_by_user_employee_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_modified_by_user_employee_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `work_order_id` SET TAGS ('dbx_business_glossary_term' = 'Work Order Id (Foreign Key)');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `amount` SET TAGS ('dbx_business_glossary_term' = 'Refund Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'pending|approved|rejected|escalated');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `approval_threshold_level` SET TAGS ('dbx_business_glossary_term' = 'Approval Threshold Level');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `approval_threshold_level` SET TAGS ('dbx_value_regex' = 'auto|supervisor|manager|director|executive');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bank_account_number` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bank_account_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bank_account_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bank_routing_number` SET TAGS ('dbx_business_glossary_term' = 'Bank Routing Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bank_routing_number` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `bank_routing_number` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `cleared_date` SET TAGS ('dbx_business_glossary_term' = 'Cleared Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `cost_center_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `disbursement_date` SET TAGS ('dbx_business_glossary_term' = 'Disbursement Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `net_refund_amount` SET TAGS ('dbx_business_glossary_term' = 'Net Refund Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `originating_credit_source` SET TAGS ('dbx_business_glossary_term' = 'Originating Credit Source');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `originating_credit_source` SET TAGS ('dbx_value_regex' = 'payment|deposit|adjustment|rate_change|meter_correction|billing_reversal');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Payee Address Line 1');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line1` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Payee Address Line 2');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line2` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_city` SET TAGS ('dbx_business_glossary_term' = 'Payee City');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_city` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_country_code` SET TAGS ('dbx_business_glossary_term' = 'Payee Country Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_country_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_country_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_name` SET TAGS ('dbx_business_glossary_term' = 'Payee Name');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Payee Postal Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_postal_code` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_state_province` SET TAGS ('dbx_business_glossary_term' = 'Payee State or Province');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_state_province` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payee_state_province` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payment_instrument_number` SET TAGS ('dbx_business_glossary_term' = 'Payment Instrument Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `payment_instrument_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `reason_code` SET TAGS ('dbx_business_glossary_term' = 'Refund Reason Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `reason_description` SET TAGS ('dbx_business_glossary_term' = 'Refund Reason Description');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_date` SET TAGS ('dbx_business_glossary_term' = 'Refund Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_method` SET TAGS ('dbx_business_glossary_term' = 'Refund Method');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_method` SET TAGS ('dbx_value_regex' = 'check|ach|wire_transfer|account_credit|debit_card|prepaid_card');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_number` SET TAGS ('dbx_business_glossary_term' = 'Refund Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_status` SET TAGS ('dbx_business_glossary_term' = 'Refund Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_type` SET TAGS ('dbx_business_glossary_term' = 'Refund Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `refund_type` SET TAGS ('dbx_value_regex' = 'overpayment|deposit_return|billing_error|credit_balance|rate_adjustment|meter_error');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `regulatory_reporting_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `revenue_class` SET TAGS ('dbx_business_glossary_term' = 'Revenue Class');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `revenue_class` SET TAGS ('dbx_value_regex' = 'residential|commercial|industrial|agricultural|public_authority');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `service_territory_code` SET TAGS ('dbx_business_glossary_term' = 'Service Territory Code');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `sox_control_flag` SET TAGS ('dbx_business_glossary_term' = 'Sarbanes-Oxley (SOX) Control Flag');
ALTER TABLE `energy_utilities_ecm`.`billing`.`refund` ALTER COLUMN `tax_amount` SET TAGS ('dbx_business_glossary_term' = 'Tax Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` SET TAGS ('dbx_subdomain' = 'revenue_recognition');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` SET TAGS ('dbx_association_edges' = 'billing.billing_service_agreement,renewable.incentive_program');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `service_agreement_incentive_enrollment_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Incentive Enrollment ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `billing_service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Service Agreement ID');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `incentive_program_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Incentive Enrollment - Renewable Incentive Program Id');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `service_agreement_billing_service_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Service Agreement Incentive Enrollment - Billing Service Agreement Id');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `application_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Application Reference Number');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `approved_incentive_amount` SET TAGS ('dbx_business_glossary_term' = 'Approved Incentive Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Compliance Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `enrollment_date` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `enrollment_status` SET TAGS ('dbx_business_glossary_term' = 'Enrollment Status');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `last_compliance_check_date` SET TAGS ('dbx_business_glossary_term' = 'Last Compliance Check Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `next_payment_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Payment Due Date');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `payment_schedule_type` SET TAGS ('dbx_business_glossary_term' = 'Payment Schedule Type');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `total_paid_amount` SET TAGS ('dbx_business_glossary_term' = 'Total Paid Amount');
ALTER TABLE `energy_utilities_ecm`.`billing`.`service_agreement_incentive_enrollment` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` SET TAGS ('dbx_subdomain' = 'payment_collections');
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` ALTER COLUMN `credit_memo_id` SET TAGS ('dbx_business_glossary_term' = 'Credit Memo Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`credit_memo` ALTER COLUMN `original_credit_memo_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_period` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_period` SET TAGS ('dbx_subdomain' = 'invoice_processing');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_period` ALTER COLUMN `billing_period_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Period Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`billing_period` ALTER COLUMN `previous_billing_period_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` SET TAGS ('dbx_subdomain' = 'invoice_processing');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `disconnect_order_id` SET TAGS ('dbx_business_glossary_term' = 'Disconnect Order Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `superseded_disconnect_order_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `adjustment_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `adjustment_amount` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `deposit_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `deposit_amount` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `gross_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `gross_amount` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `net_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`disconnect_order` ALTER COLUMN `net_amount` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` SET TAGS ('dbx_subdomain' = 'payment_collections');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` ALTER COLUMN `dunning_run_id` SET TAGS ('dbx_business_glossary_term' = 'Dunning Run Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_run` ALTER COLUMN `previous_dunning_run_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_schedule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_schedule` SET TAGS ('dbx_subdomain' = 'payment_collections');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_schedule` ALTER COLUMN `dunning_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Dunning Schedule Identifier');
ALTER TABLE `energy_utilities_ecm`.`billing`.`dunning_schedule` ALTER COLUMN `superseded_dunning_schedule_id` SET TAGS ('dbx_self_ref_fk' = 'true');
