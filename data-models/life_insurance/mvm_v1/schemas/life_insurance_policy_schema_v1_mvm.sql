-- Schema for Domain: policy | Business: Life Insurance | Version: v1_mvm
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`in_force_policy` (
    ...
    `application_id` BIGINT COMMENT 'Foreign key linking to underwriting.application. Business justification: Every issued policy originates from an underwriting application. Policy admin systems must trace back to the originating application for contestability period tracking, replacement regulation complian',
    `cohort_definition_id` BIGINT COMMENT 'Foreign key linking to actuarial.cohort_definition. Business justification: LDTI (ASC 944) and IFRS17 require every in-force policy to be assigned to a measurement cohort for LFPB/CSM calculation and DAC amortization. Actuaries and finance teams depend on this link for quarte',
    `coi_rate_schedule_id` BIGINT COMMENT 'Foreign key linking to product.coi_rate_schedule. Business justification: UL/VUL/IUL in-force policies require a direct reference to the COI rate schedule applied for monthly deduction calculations. This link enables COI charge audit trails, rate action impact analysis on t',
    `form_id` BIGINT COMMENT 'Foreign key linking to product.form. Business justification: in_force_policy carries policy_form_number as a plain text column — a denormalized reference to product.form. Linking to form enables regulatory compliance verification that the correct approved form ',
    `hierarchy_node_id` BIGINT COMMENT 'Foreign key linking to agent.hierarchy_node. Business justification: Life insurance carriers assign each policy to a distribution hierarchy node (territory/region/branch) for override commission roll-up, production reporting, and market conduct analysis. A domain exper',
    `ifrs17_contract_group_id` BIGINT COMMENT 'Foreign key linking to finance.ifrs17_contract_group. Business justification: IFRS 17 requires each insurance contract to be assigned to a contract group for measurement (GMM, PAA, or VFA). Direct FK from in_force_policy to ifrs17_contract_group is a fundamental IFRS 17 operati',
    `jurisdiction_license_id` BIGINT COMMENT 'Foreign key linking to compliance.jurisdiction_license. Business justification: Life insurance policies may only be issued in jurisdictions where the company holds a valid license. Market conduct exams and regulatory audits verify that each policy was issued in a licensed jurisdi',
    `pbr_model_segment_id` BIGINT COMMENT 'Foreign key linking to actuarial.pbr_model_segment. Business justification: NAIC VM-20/VM-21 Principle-Based Reserving requires each policy to be assigned to a PBR model segment that governs reserve methodology, credibility weighting, and stochastic scenario application. Actu',
    `policy_form_approval_id` BIGINT COMMENT 'Foreign key linking to compliance.policy_form_approval. Business justification: Every life insurance policy must be issued on a DOI-approved form. Market conduct exams verify that each in-force policy was issued on an approved form for its jurisdiction. policy_form_number is a de',
    ...
    `risk_class_id` BIGINT COMMENT 'Foreign key linking to underwriting.risk_class. Business justification: The in_force_policy carries denormalized `risk_class` and `underwriting_class` text columns that are authoritative data owned by underwriting.risk_class. A FK enforces referential integrity, drives CO',
    `schedule_id` BIGINT COMMENT 'Foreign key linking to commission.schedule. Business justification: At policy issue, carriers assign a specific commission schedule to lock in renewal commission rates and chargeback periods for the policys lifetime. Producer compensation audits and renewal commissio',
    `state_regulation_id` BIGINT COMMENT 'Foreign key linking to compliance.state_regulation. Business justification: Every life insurance policy is governed by the state regulation of its issuing jurisdiction. Market conduct exams and compliance audits require tracing each policy to its governing state regulation fo',
    `underwriting_class_id` BIGINT COMMENT 'Foreign key linking to product.underwriting_class. Business justification: in_force_policy carries underwriting_class as a plain text column. Linking to the underwriting_class entity enables experience studies by class, reserve adequacy reporting, and reinsurance cession dec',
    `separate_account_id` BIGINT COMMENT 'Foreign key linking to investment.separate_account. Business justification: Variable life and variable annuity policies are legally and regulatorily required to identify their backing separate account (SEC registration, prospectus delivery, NAIC separate account reporting). R',
    `agency_id` BIGINT COMMENT 'Foreign key linking to agent.agency. Business justification: Policies are placed through agencies, not just individual producers. Carriers track the writing agency for agency-level commission statements, E&O coverage verification at issue, agency production rep',
    `appointment_id` BIGINT COMMENT 'Foreign key linking to agent.appointment. Business justification: State DOI regulations require carriers to verify the writing producer held an active appointment in the state of issue at policy issuance. Linking the policy to the specific appointment record support',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`value` (
    ...
    `crediting_strategy_id` BIGINT COMMENT 'Foreign key linking to product.crediting_strategy. Business justification: Policy value records track index_credit_amount and interest_credited_amount computed per crediting strategy. Actuarial valuation, in-force illustration, and regulatory reporting require an audit trail',
    ...
    `separate_account_id` BIGINT COMMENT 'Foreign key linking to investment.separate_account. Business justification: Variable policy account value (av_amount, fund_allocation_total, index_credit_amount) is directly derived from the separate accounts NAV and unit values. Daily policy valuation, statutory reserve cal',
    `unit_value_id` BIGINT COMMENT 'Foreign key linking to investment.unit_value. Business justification: Variable policy account value (av_amount), interest_credited_amount, and index_credit_amount are computed directly from unit values. Daily policy valuation processing, policyholder statements, and FIN',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`rider` (
    `rider_id` BIGINT COMMENT 'Unique system identifier for the rider record. Primary key.',
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Rider additions generate endorsement documents that are legal amendments to the base policy contract, required for state DOI filing and policyholder delivery. Direct FK links the rider record to its e',
    `mortality_table_id` BIGINT COMMENT 'Foreign key linking to actuarial.mortality_table. Business justification: Mortality-sensitive riders (term, ADB, guaranteed insurability) use rider-specific mortality tables that may differ from the base policy table. Actuarial reserve calculations and COI rate setting for ',
    `policy_form_approval_id` BIGINT COMMENT 'Foreign key linking to compliance.policy_form_approval. Business justification: Riders must be issued on DOI-approved forms, just like base policies. Market conduct exams review whether riders were issued on approved forms in each jurisdiction. acord_form_number on policy_rider i',
    ...
    `pricing_basis_id` BIGINT COMMENT 'Foreign key linking to actuarial.pricing_basis. Business justification: Riders (LTC, ADB, waiver of premium, term) have distinct pricing bases with separate COI rates, benefit pricing, and expense loadings. Actuarial pricing and rider reserve calculations require the ride',
    ...
    `risk_class_id` BIGINT COMMENT 'Foreign key linking to underwriting.risk_class. Business justification: Riders (waiver of premium, LTC, disability income) carry their own underwriting risk classification separate from the base policy. The policy_rider has a denormalized `underwriting_class` text column.',
    `separate_account_id` BIGINT COMMENT 'Foreign key linking to investment.separate_account. Business justification: Variable riders (GMDB, GMWB, GMIB) calculate charges and guarantee bases against the separate accounts NAV. Rider reserve calculations, hedge effectiveness testing, and NAIC AG43/VM-21 reserve report',
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: Rider additions, modifications, and terminations are post-issue servicing transactions processed through service requests (request_type=RIDER_ADD, RIDER_TERMINATE, RIDER_MODIFY). The service_req',
    `treaty_id` BIGINT COMMENT 'Reference to the reinsurance treaty under which this rider is ceded. Null if rider is not reinsured.',
    `underwriting_class_id` BIGINT COMMENT 'Foreign key linking to product.underwriting_class. Business justification: Riders (DI, LTC) carry independent underwriting classes separate from the base policy. Linking policy_rider to underwriting_class enables rider-specific experience studies, independent reinsurance ces',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`status_history` (
    ...
    `grace_period_id` BIGINT COMMENT 'Foreign key linking to billing.grace_period. Business justification: State insurance regulators require traceability from a policy status transition (e.g., grace, lapse) to the specific billing grace period event that triggered it. This FK enables the regulatory audit ',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Grace period notices, lapse notifications, and reinstatement eligibility communications are mandated by specific state regulatory obligations. status_history has regulatory_reporting_flag indicating r',
    `state_regulation_id` BIGINT COMMENT 'Foreign key linking to compliance.state_regulation. Business justification: Grace period lengths, free-look cancellation windows, and lapse notification requirements are state-regulated. status_history tracks these events and has state_jurisdiction_code as a denormalized refe',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Policy status changes (lapse, termination) trigger reserve release journal entries. Finance teams reconcile reserve releases to specific lapse events for statutory reserve adequacy reporting and LDTI ',
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Policy status changes (lapse, grace period, reinstatement eligibility) trigger mandatory regulatory notices (grace period notices, lapse notices) required by state insurance codes. Direct FK links the',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`policy_beneficiary` (
    ...
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Beneficiary designation changes require executed designation forms as legal documents governing death benefit distribution. Direct FK links the beneficiary record to its governing designation document',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` (
    ...
    `application_id` BIGINT COMMENT 'Foreign key linking to underwriting.application. Business justification: Reinstatement requiring evidence of insurability generates a new underwriting application. The reinstatement record must reference this application to track the underwriting submission, decision, and ',
    `assumption_set_id` BIGINT COMMENT 'Foreign key linking to actuarial.assumption_set. Business justification: Reinstatement underwriting uses actuarial assumption sets to determine evidence of insurability thresholds, back-premium interest rates, and contestability period restart rules. Actuarial and complian',
    `cession_id` BIGINT COMMENT 'Foreign key linking to reinsurance.reinsurance_cession. Business justification: Policy reinstatement requires reinstating the associated reinsurance cession, which may have lapsed or been terminated. Reinsurance operations must coordinate cession reinstatement with the reinsurer.',
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Reinstatement applications require health statements, evidence of insurability forms, and back-premium receipts as regulatory documents. Insurers must retain these for contestability period compliance',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Reinstatements collect back premiums and fees, generating journal entries for premium income and fee revenue recognition. Finance reconciliation of reinstatement collections to GL postings is required',
    ...
    `risk_assessment_id` BIGINT COMMENT 'Foreign key linking to underwriting.underwriting_risk_assessment. Business justification: Reinstatement underwriting produces a formal risk assessment. The reinstatement record carries denormalized `underwriting_decision` and `underwriting_decision_date` fields that belong on the risk asse',
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: A policy reinstatement application is initiated through a service request (request_type=REINSTATEMENT). Linking policy_reinstatement to the originating service_request provides full traceability fro',
    `state_regulation_id` BIGINT COMMENT 'Foreign key linking to compliance.state_regulation. Business justification: Reinstatement eligibility windows, evidence of insurability requirements, and back-premium interest rules are state-regulated. Compliance reporting on reinstatement practices during market conduct exa',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`surrender` (
    ...
    `cession_id` BIGINT COMMENT 'Foreign key linking to reinsurance.reinsurance_cession. Business justification: Surrender processing requires terminating the associated reinsurance cession and notifying the reinsurer. Operations teams must identify which cession to close when a policy surrenders. A reinsurance ',
    `charge_schedule_id` BIGINT COMMENT 'Foreign key linking to product.charge_schedule. Business justification: Surrender transactions apply surrender charges, MVA, and free withdrawal amounts defined in charge_schedule. Linking surrender to charge_schedule provides the audit trail of which schedule version was',
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Surrender transactions generate IRS Form 1099-R, surrender request forms, and disbursement confirmations required for tax reporting and regulatory compliance. Direct FK enables tax document retrieval,',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Surrender transactions generate specific GL postings (CSV disbursement, surrender charge revenue, taxable gain recognition). Finance reconciliation of surrender proceeds to GL requires tracing each su',
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to annuity.annuity_contract. Business justification: Surrendered life policies frequently roll into annuities via 1035 tax-free exchanges. Surrender processing tracks the resulting annuity contract for tax reporting (Form 1099-R coordination), customer ',
    `separate_account_id` BIGINT COMMENT 'Foreign key linking to investment.separate_account. Business justification: Surrendering a variable life/annuity policy requires unit redemption from the specific separate account. The surrender processing system must reference the separate account to calculate MVA, process u',
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: A surrender or partial withdrawal transaction is initiated through a service request (request_type=SURRENDER or PARTIAL_WITHDRAWAL). Linking surrender to the originating service_request provides e',
    `state_regulation_id` BIGINT COMMENT 'Foreign key linking to compliance.state_regulation. Business justification: Nonforfeiture options, surrender charge schedules, and minimum cash surrender value requirements are state-regulated. The surrender table tracks naic_compliance_flag and nonforfeiture_option_type. A F',
    `valuation_id` BIGINT COMMENT 'Foreign key linking to investment.valuation. Business justification: Surrender value (gross_amount, csv_amount_used, net_proceeds) is computed from the current market valuation of the policys investment holdings. The specific valuation snapshot used at surrender must ',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`loan` (
    ...
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Policy loans require executed loan agreement documents and disclosure forms as legal instruments. Regulators and auditors require direct traceability from the loan record to its governing document. Di',
    `in_force_policy_id` BIGINT COMMENT 'Reference to the underlying life insurance policy against which this loan is issued.',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Policy loan disbursements, interest accruals, and repayments generate journal entries for interest income recognition and loan balance reconciliation. Finance teams must trace each loan transaction to',
    `valuation_id` BIGINT COMMENT 'Foreign key linking to investment.valuation. Business justification: Maximum policy loan amount is determined by the CSV, which is derived from the investment valuation at loan origination. Recording the specific valuation snapshot used supports loan origination audit ',
    ...
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: A policy loan origination is initiated through a service request (request_type=LOAN_REQUEST). Linking loan to the originating service_request provides traceability from the policyholders loan appli',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`dividend` (
    ...
    `experience_study_id` BIGINT COMMENT 'Foreign key linking to actuarial.experience_study. Business justification: Dividend factors (mortality_factor, interest_factor, expense_factor on dividend record) are directly derived from experience study results. Actuarial teams set annual dividend scales based on experien',
    ...
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Dividend declarations and payments generate journal entries for policyholder dividend expense, cash disbursements, and PUA purchases. Finance requires direct traceability from dividend records to GL p',
    ...
    `valuation_run_id` BIGINT COMMENT 'Foreign key linking to actuarial.valuation_run. Business justification: Participating policy dividend declarations are based on annual actuarial valuation results (mortality, interest, expense experience). Linking dividend to valuation_run enables actuarial teams to trace',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` (
    ...
    `assumption_set_id` BIGINT COMMENT 'Foreign key linking to actuarial.assumption_set. Business justification: IRC 7702 guideline premium and cash value accumulation test calculations require specific actuarial assumptions (interest rates, mortality). The assumption_set governs these inputs beyond just the mor',
    ...
    `insured_id` BIGINT COMMENT 'Foreign key linking to policyholder.insured. Business justification: IRC 7702 and MEC testing requires insured age, risk class, and mortality data directly from the insured record. insured_age_at_test is a denormalized copy of insured data. This FK enables actuarial co',
    `irc7702_parameter_id` BIGINT COMMENT 'Foreign key linking to product.irc7702_parameter. Business justification: IRC 7702 CVAT/GPT compliance tests use guideline_single_premium, guideline_level_premium, seven_pay_premium_limit, and corridor_percentage defined in irc7702_parameter. Linking tax_compliance_test to ',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: IRC 7702 compliance testing and MEC 7-pay testing are specific regulatory obligations under the Internal Revenue Code. Linking each tax_compliance_test to its governing regulatory_obligation enables c',
    `separate_account_id` BIGINT COMMENT 'Foreign key linking to investment.separate_account. Business justification: IRC 7702/7702A compliance tests for variable life require the separate accounts unit values and NAV to compute CVAT/GPT corridor tests and account value. Regulators and auditors require traceability ',
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: IRC 7702 and MEC test results require mandatory policyholder notification documents (MEC notices, corridor test disclosures) under IRS regulations. Direct FK links the compliance test record to its ge',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: MEC reclassification events (IRC 7702 test failures) generate journal entries for deferred tax adjustments and IRS Form 1099-R reporting. Finance must trace MEC determination events to their GL impact',
    `valuation_run_id` BIGINT COMMENT 'Foreign key linking to actuarial.valuation_run. Business justification: IRC 7702 and MEC compliance tests are executed within actuarial valuation cycles. Linking tax_compliance_test to valuation_run enables audit traceability of which valuation run produced the guideline ',
    `value_id` BIGINT COMMENT 'Foreign key linking to policy.value. Business justification: IRC Section 7702A MEC testing is performed against a specific policy value snapshot as of the test date. Linking tax_compliance_test to the policy.value record used in the test provides authoritative ',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`fund_allocation` (
    ...
    `crediting_strategy_id` BIGINT COMMENT 'Foreign key linking to product.crediting_strategy. Business justification: Each IUL/FIA fund allocation segment is governed by a specific crediting strategy defining cap, floor, participation, and spread rates. Linking fund_allocation to crediting_strategy is essential for s',
    ...
    `scenario_set_id` BIGINT COMMENT 'Foreign key linking to actuarial.scenario_set. Business justification: Indexed and variable fund allocations (IUL, VA) are stress-tested against economic scenario sets for AG43/VM-21 stochastic reserve requirements. Linking fund_allocation to scenario_set enables actuari',
    ...
    `separate_account_id` BIGINT COMMENT 'Foreign key linking to investment.separate_account. Business justification: Variable life/annuity fund allocations must reference the investment.separate_account for daily unit-value-based crediting, NAV computation, and SEC/FINRA regulatory reporting. The existing separate_a',
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: Fund allocation changes (rebalancing, strategy changes, index segment elections) for universal life and variable annuity policies are initiated through service requests (request_type=FUND_REALLOCATIO',
    `suitability_review_id` BIGINT COMMENT 'Foreign key linking to compliance.suitability_review. Business justification: Variable and indexed product fund allocation changes require suitability review under FINRA Reg BI and state variable product regulations. Linking fund_allocation to the suitability_review that approv',
    `valuation_id` BIGINT COMMENT 'Foreign key linking to investment.valuation. Business justification: Each fund allocations dollar amount (allocation_amount) is computed from the current market valuation of the underlying investment. Linking to the specific valuation snapshot used at allocation_effec',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`assignment` (
    ...
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: An absolute or collateral assignment of a life insurance policy is a post-issue servicing transaction initiated through a service request (request_type=ASSIGNMENT). Linking assignment to the origina',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`service_request` (
    ...
    `application_id` BIGINT COMMENT 'Foreign key linking to underwriting.application. Business justification: Service requests for coverage increases, reinstatements, or conversion exercises that require evidence of insurability spawn a new underwriting application. Linking service_request to the resulting un',
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Service requests (beneficiary changes, address updates, endorsements) generate primary confirmation documents required for audit trails and regulatory compliance. Operations teams need direct linkage ',
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to annuity.annuity_contract. Business justification: Service requests for 1035 exchanges and policy-to-annuity conversions track the resulting annuity contract for workflow completion verification, customer notifications, and regulatory documentation. E',
    `suitability_review_id` BIGINT COMMENT 'Foreign key linking to compliance.suitability_review. Business justification: Service requests involving 1035 exchanges, replacements, and annuity conversions trigger mandatory suitability reviews under NAIC Suitability in Annuity Transactions Model Regulation and state rules. ',
    `workflow_id` BIGINT COMMENT 'Foreign key linking to document.workflow. Business justification: Service requests initiate document processing workflows (imaging, indexing, NIGO resolution, approval routing). Direct FK from service_request to workflow enables SLA tracking, work queue management, ',
...
CREATE OR REPLACE TABLE `life_insurance_ecm`.`policy`.`conversion` (
    ...
    `application_id` BIGINT COMMENT 'Foreign key linking to underwriting.application. Business justification: Conversions requiring evidence of insurability (e.g., term-to-perm beyond guaranteed conversion window) generate a new underwriting application. Linking conversion to the underwriting application supp',
    ...
    `document_id` BIGINT COMMENT 'Foreign key linking to document.document. Business justification: Term-to-permanent conversions generate conversion application forms, new policy contracts, and evidence-of-insurability waiver documents required for regulatory filing and new policy issuance. Direct ',
    `journal_entry_id` BIGINT COMMENT 'Foreign key linking to finance.journal_entry. Business justification: Policy conversions (term-to-permanent) trigger journal entries for reserve transfers, DAC reclassification, and premium income reclassification. Finance must trace conversion events to GL impact for G',
    `original_conversion_id` BIGINT COMMENT 'Self-referencing FK on conversion (original_conversion_id)',
    `policy_form_approval_id` BIGINT COMMENT 'Foreign key linking to compliance.policy_form_approval. Business justification: Policy conversions result in a new policy issued on a new approved form. Compliance requires verifying the converted-to product form was approved in the issuing jurisdiction at conversion time. No exi',
    ...
    `contract_id` BIGINT COMMENT 'Foreign key linking to annuity.annuity_contract. Business justification: Policy conversion to annuity (e.g., term-to-annuity or whole life conversion privilege) is a named life insurance business process requiring direct traceability to the resulting annuity contract for c',
    `service_request_id` BIGINT COMMENT 'Foreign key linking to policy.service_request. Business justification: A term-to-permanent policy conversion is a post-issue servicing transaction initiated through a service request (request_type=CONVERSION). Linking conversion to the originating service_request provi',
    `suitability_review_id` BIGINT COMMENT 'Foreign key linking to compliance.suitability_review. Business justification: Policy conversions, especially to annuity or variable products, require suitability review under NAIC model regulations and state rules. Linking conversion to suitability_review enables compliance tra',
    `plan_id` BIGINT COMMENT 'Foreign key linking to product.plan. Business justification: Conversion records the target plan a policy converts to. Linking conversion to the target plan_id enables conversion privilege validation against plan eligibility rules, new policy issuance workflow, ',
    `decision_id` BIGINT COMMENT 'Foreign key linking to underwriting.decision. Business justification: Conversions with evidence of insurability requirements produce a formal underwriting decision (approve/decline/rate). The conversion table has denormalized decision context but no FK to the authoritat',
...
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ADD CONSTRAINT `fk_policy_rider_in_force_policy_id` FOREIGN KEY (`in_force_policy_id`) REFERENCES `life_insurance_ecm`.`policy`.`in_force_policy`(`in_force_policy_id`);
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ADD CONSTRAINT `fk_policy_rider_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ADD CONSTRAINT `fk_policy_policy_reinstatement_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ADD CONSTRAINT `fk_policy_surrender_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`loan` ADD CONSTRAINT `fk_policy_loan_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ADD CONSTRAINT `fk_policy_tax_compliance_test_value_id` FOREIGN KEY (`value_id`) REFERENCES `life_insurance_ecm`.`policy`.`value`(`value_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ADD CONSTRAINT `fk_policy_fund_allocation_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`assignment` ADD CONSTRAINT `fk_policy_assignment_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ADD CONSTRAINT `fk_policy_conversion_service_request_id` FOREIGN KEY (`service_request_id`) REFERENCES `life_insurance_ecm`.`policy`.`service_request`(`service_request_id`);
...
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Application Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `cohort_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Cohort Definition Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `coi_rate_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Coi Rate Schedule Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `form_id` SET TAGS ('dbx_business_glossary_term' = 'Form Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `hierarchy_node_id` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Node Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `ifrs17_contract_group_id` SET TAGS ('dbx_business_glossary_term' = 'Ifrs17 Contract Group Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `jurisdiction_license_id` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction License Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `pbr_model_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Pbr Model Segment Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `policy_form_approval_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Form Approval Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `risk_class_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Class Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Schedule Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `state_regulation_id` SET TAGS ('dbx_business_glossary_term' = 'State Regulation Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `underwriting_class_id` SET TAGS ('dbx_business_glossary_term' = 'Underwriting Class Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `separate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Variable Separate Account Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `agency_id` SET TAGS ('dbx_business_glossary_term' = 'Writing Agency Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`in_force_policy` ALTER COLUMN `appointment_id` SET TAGS ('dbx_business_glossary_term' = 'Writing Appointment Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`value` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`value` ALTER COLUMN `crediting_strategy_id` SET TAGS ('dbx_business_glossary_term' = 'Crediting Strategy Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`value` ALTER COLUMN `separate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Separate Account Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`value` ALTER COLUMN `unit_value_id` SET TAGS ('dbx_business_glossary_term' = 'Unit Value Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` SET TAGS ('dbx_subdomain' = 'contract_administration');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `rider_id` SET TAGS ('dbx_business_glossary_term' = 'Rider Identifier (ID)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Rider Endorsement Document Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `mortality_table_id` SET TAGS ('dbx_business_glossary_term' = 'Mortality Table Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `policy_form_approval_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Form Approval Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `portfolio_id` SET TAGS ('dbx_business_glossary_term' = 'Portfolio Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `pricing_basis_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Basis Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `in_force_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Identifier (ID)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `rider_definition_id` SET TAGS ('dbx_business_glossary_term' = 'Rider Definition Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `risk_class_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Class Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `separate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Separate Account Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `treaty_id` SET TAGS ('dbx_business_glossary_term' = 'Reinsurance Treaty Identifier (ID)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `underwriting_class_id` SET TAGS ('dbx_business_glossary_term' = 'Underwriting Class Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `attained_age` SET TAGS ('dbx_business_glossary_term' = 'Attained Age');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `benefit_period_months` SET TAGS ('dbx_business_glossary_term' = 'Benefit Period (Months)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `benefit_trigger_condition` SET TAGS ('dbx_business_glossary_term' = 'Benefit Trigger Condition');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `cash_value` SET TAGS ('dbx_business_glossary_term' = 'Rider Cash Value');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `cost_of_insurance_rate` SET TAGS ('dbx_business_glossary_term' = 'Cost of Insurance (COI) Rate');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Rider Effective Date');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `elimination_period_days` SET TAGS ('dbx_business_glossary_term' = 'Elimination Period (Days)');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Rider Expiry Date');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `face_amount` SET TAGS ('dbx_business_glossary_term' = 'Rider Face Amount');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `guaranteed_insurability_flag` SET TAGS ('dbx_business_glossary_term' = 'Guaranteed Insurability Flag');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `issue_age` SET TAGS ('dbx_business_glossary_term' = 'Issue Age');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `loan_outstanding_amount` SET TAGS ('dbx_business_glossary_term' = 'Rider Loan Outstanding Amount');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `premium_amount` SET TAGS ('dbx_business_glossary_term' = 'Rider Premium Amount');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `premium_frequency` SET TAGS ('dbx_business_glossary_term' = 'Rider Premium Frequency');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `premium_frequency` SET TAGS ('dbx_value_regex' = 'annual|semi_annual|quarterly|monthly|single');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `premium_mode` SET TAGS ('dbx_business_glossary_term' = 'Rider Premium Mode');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `premium_mode` SET TAGS ('dbx_value_regex' = 'level|increasing|decreasing|flexible');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `rating_percentage` SET TAGS ('dbx_business_glossary_term' = 'Rating Percentage');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `reinsurance_ceded_flag` SET TAGS ('dbx_business_glossary_term' = 'Reinsurance Ceded Flag');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `rider_status` SET TAGS ('dbx_business_glossary_term' = 'Rider Status');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `rider_status` SET TAGS ('dbx_value_regex' = 'active|suspended|terminated|lapsed|pending|matured');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `state_of_issue` SET TAGS ('dbx_business_glossary_term' = 'State of Issue');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `state_of_issue` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `surrender_charge` SET TAGS ('dbx_business_glossary_term' = 'Rider Surrender Charge');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `tax_qualified_flag` SET TAGS ('dbx_business_glossary_term' = 'Tax Qualified Flag');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Rider Termination Date');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `version` SET TAGS ('dbx_business_glossary_term' = 'Rider Version');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `version` SET TAGS ('dbx_value_regex' = '^[A-Z0-9.]{1,10}$');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `waiver_of_premium_flag` SET TAGS ('dbx_business_glossary_term' = 'Waiver of Premium Flag');
ALTER TABLE `life_insurance_ecm`.`policy`.`rider` ALTER COLUMN `withdrawal_percentage` SET TAGS ('dbx_business_glossary_term' = 'Withdrawal Percentage');
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` ALTER COLUMN `grace_period_id` SET TAGS ('dbx_business_glossary_term' = 'Grace Period Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` ALTER COLUMN `state_regulation_id` SET TAGS ('dbx_business_glossary_term' = 'State Regulation Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Status Journal Entry Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`status_history` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Status Notice Document Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_beneficiary` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Beneficiary Designation Document Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Application Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `assumption_set_id` SET TAGS ('dbx_business_glossary_term' = 'Assumption Set Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `cession_id` SET TAGS ('dbx_business_glossary_term' = 'Reinsurance Cession Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Reinstatement Document Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Reinstatement Journal Entry Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `risk_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Underwriting Risk Assessment Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`policy_reinstatement` ALTER COLUMN `state_regulation_id` SET TAGS ('dbx_business_glossary_term' = 'State Regulation Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `cession_id` SET TAGS ('dbx_business_glossary_term' = 'Reinsurance Cession Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `charge_schedule_id` SET TAGS ('dbx_business_glossary_term' = 'Charge Schedule Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Surrender Document Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Surrender Journal Entry Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Resulting Annuity Contract Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `separate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Separate Account Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `state_regulation_id` SET TAGS ('dbx_business_glossary_term' = 'State Regulation Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`surrender` ALTER COLUMN `valuation_id` SET TAGS ('dbx_business_glossary_term' = 'Surrender Valuation Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`loan` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`loan` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Loan Document Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`loan` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Loan Journal Entry Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`loan` ALTER COLUMN `valuation_id` SET TAGS ('dbx_business_glossary_term' = 'Origination Valuation Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`loan` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`dividend` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`dividend` ALTER COLUMN `experience_study_id` SET TAGS ('dbx_business_glossary_term' = 'Experience Study Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`dividend` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Dividend Journal Entry Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`dividend` ALTER COLUMN `valuation_run_id` SET TAGS ('dbx_business_glossary_term' = 'Valuation Run Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `assumption_set_id` SET TAGS ('dbx_business_glossary_term' = 'Assumption Set Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `insured_id` SET TAGS ('dbx_business_glossary_term' = 'Insured Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `irc7702_parameter_id` SET TAGS ('dbx_business_glossary_term' = 'Irc7702 Parameter Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `separate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Separate Account Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Tax Compliance Document Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Tax Compliance Journal Entry Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `valuation_run_id` SET TAGS ('dbx_business_glossary_term' = 'Valuation Run Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`tax_compliance_test` ALTER COLUMN `value_id` SET TAGS ('dbx_business_glossary_term' = 'Value Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` SET TAGS ('dbx_subdomain' = 'contract_administration');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ALTER COLUMN `crediting_strategy_id` SET TAGS ('dbx_business_glossary_term' = 'Crediting Strategy Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ALTER COLUMN `scenario_set_id` SET TAGS ('dbx_business_glossary_term' = 'Scenario Set Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ALTER COLUMN `separate_account_id` SET TAGS ('dbx_business_glossary_term' = 'Separate Account Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ALTER COLUMN `suitability_review_id` SET TAGS ('dbx_business_glossary_term' = 'Suitability Review Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`fund_allocation` ALTER COLUMN `valuation_id` SET TAGS ('dbx_business_glossary_term' = 'Valuation Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`assignment` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`service_request` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`service_request` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Application Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`service_request` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Document Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`service_request` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Resulting Annuity Contract Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`service_request` ALTER COLUMN `suitability_review_id` SET TAGS ('dbx_business_glossary_term' = 'Suitability Review Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`service_request` ALTER COLUMN `workflow_id` SET TAGS ('dbx_business_glossary_term' = 'Workflow Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` SET TAGS ('dbx_subdomain' = 'financial_transactions');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `application_id` SET TAGS ('dbx_business_glossary_term' = 'Application Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `document_id` SET TAGS ('dbx_business_glossary_term' = 'Conversion Document Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `journal_entry_id` SET TAGS ('dbx_business_glossary_term' = 'Conversion Journal Entry Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `original_conversion_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `policy_form_approval_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Form Approval Id (Foreign Key)');
...
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `contract_id` SET TAGS ('dbx_business_glossary_term' = 'Resulting Annuity Contract Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `service_request_id` SET TAGS ('dbx_business_glossary_term' = 'Service Request Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `suitability_review_id` SET TAGS ('dbx_business_glossary_term' = 'Suitability Review Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `plan_id` SET TAGS ('dbx_business_glossary_term' = 'Target Plan Id (Foreign Key)');
ALTER TABLE `life_insurance_ecm`.`policy`.`conversion` ALTER COLUMN `decision_id` SET TAGS ('dbx_business_glossary_term' = 'Underwriting Decision Id (Foreign Key)');
