-- Schema for Domain: store | Business: Grocery | Version: v1_ecm
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`store_location` (
    ...
    `primary_store_manager_associate_id` BIGINT COMMENT 'FK to workforce.associate',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`department` (
    ...
    `associate_id` BIGINT COMMENT 'Identifier of the employee assigned as the department manager responsible for operations, staffing, and performance.',
    ...
    `store_location_id` BIGINT COMMENT 'Identifier of the store where this department is located.',
    ...
    `gl_account_code` STRING COMMENT 'General ledger account code assigned to this department for financial accounting and cost allocation.',
    ...
    `profit_center_code` STRING COMMENT 'Profit center code assigned to this department for profitability analysis and management reporting.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`operating_hours` (
    ...
    `store_location_id` BIGINT COMMENT 'Identifier of the store location to which these operating hours apply. Links to the store master data.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`service_capability` (
    ...
    `store_location_id` BIGINT COMMENT 'Identifier of the store location where this service capability is offered.',
    ...
    `sla_target_time_minutes` STRING COMMENT 'Target service level agreement time in minutes for fulfilling customer orders or transactions using this service capability. Used for performance monitoring and customer expectations.',
    ...
    `vendor_partner_name` STRING COMMENT 'Name of the third-party vendor or partner that provides or supports this service capability (e.g., Coinstar, Western Union, Starbucks, Instacart). Null if operated directly by the retailer.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`remodel_project` (
    `remodel_project_id` BIGINT COMMENT 'Unique identifier for the store remodel, expansion, or refresh project.',
    `store_location_id` BIGINT COMMENT 'Identifier of the store location undergoing remodel or expansion.',
    `actual_completion_date` DATE COMMENT 'Actual date when remodel project was completed and store resumed normal operations.',
    `actual_spend_amount` DECIMAL(18,2) COMMENT 'Total actual expenditure incurred on the remodel project in USD. Tracked for budget variance analysis and capital planning accuracy.',
    `actual_start_date` DATE COMMENT 'Actual date when remodel construction or implementation activities commenced.',
    `approval_date` DATE COMMENT 'Date when the remodel project was formally approved by executive leadership or capital planning committee.',
    `cancellation_date` DATE COMMENT 'Date when the remodel project was cancelled or terminated before completion.',
    `cancellation_reason` STRING COMMENT 'Explanation for why the remodel project was cancelled (e.g., budget constraints, store closure decision, market conditions, permitting issues).',
    `capital_budget_amount` DECIMAL(18,2) COMMENT 'Total approved capital budget allocated for the remodel project in USD. Includes construction, equipment, fixtures, and technology costs.',
    `closure_end_date` DATE COMMENT 'Date when store reopens to customers after remodel-related closure period.',
    `closure_start_date` DATE COMMENT 'Date when store closure begins if full closure is required for the remodel project.',
    `comp_sales_exclusion_end_date` DATE COMMENT 'Date when the store is reintroduced into comp sales calculations after remodel completion and stabilization period.',
    `comp_sales_exclusion_flag` BOOLEAN COMMENT 'Indicates whether this store should be excluded from comp sales (same-store sales) calculations during the remodel period due to operational disruption. True means exclude from comp sales reporting; false means include in comp sales.',
    `comp_sales_exclusion_start_date` DATE COMMENT 'Date when the store begins to be excluded from comp sales calculations due to remodel disruption.',
    `contractor_name` STRING COMMENT 'Name of the primary general contractor or construction firm responsible for executing the remodel project.',
    `contractor_reference_number` STRING COMMENT 'External reference number or contract identifier for the contractor agreement associated with this remodel project.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this remodel project record was first created in the system.',
    `departments_impacted` STRING COMMENT 'Comma-separated list of store departments affected by the remodel project (e.g., produce, deli, bakery, pharmacy, fuel center, front end).',
    `fiscal_year` STRING COMMENT 'Fiscal year in which the remodel project is planned and budgeted (e.g., 2024).',
    `inspection_completion_date` DATE COMMENT 'Date when final building inspection was completed and approved by local authorities, allowing store to resume operations.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when this remodel project record was last modified in the system.',
    `notes` STRING COMMENT 'Free-form notes and comments about the remodel project for internal tracking and communication.',
    `payback_period_months` STRING COMMENT 'Expected number of months for the remodel investment to be recovered through incremental sales and margin improvements.',
    `permit_issue_date` DATE COMMENT 'Date when the building permit was issued by local authorities for the remodel project.',
    `permit_number` STRING COMMENT 'Local government building permit number issued for the remodel construction work.',
    `planned_completion_date` DATE COMMENT 'Scheduled date for remodel project to be completed and store to resume normal operations.',
    `planned_start_date` DATE COMMENT 'Scheduled date for remodel project to begin construction or implementation activities.',
    `priority_rank` STRING COMMENT 'Relative priority ranking of this remodel project within the annual capital plan portfolio. Lower numbers indicate higher priority.',
    `project_manager_name` STRING COMMENT 'Name of the internal project manager or construction manager responsible for overseeing the remodel project.',
    `project_name` STRING COMMENT 'Descriptive name of the remodel project for business communication and reporting.',
    `project_number` STRING COMMENT 'Business-assigned unique project number for tracking and reference in capital planning systems.. Valid values are `^[A-Z0-9]{8,20}$`',
    `project_status` STRING COMMENT 'Current lifecycle status of the remodel project. Planning indicates initial scoping and budgeting; approved means capital budget has been authorized; in_progress indicates active construction; on_hold reflects temporary suspension; completed means project is finished and store is operational; cancelled indicates project was terminated before completion.. Valid values are `planning|approved|in_progress|on_hold|completed|cancelled`',
    `project_type` STRING COMMENT 'Classification of the remodel project scope. Full remodel includes complete store renovation; department refresh targets specific departments (produce, pharmacy, deli); technology upgrade focuses on POS, self-checkout, or digital signage; ADA compliance addresses accessibility requirements; expansion adds square footage; format conversion changes store format (e.g., conventional to fresh format).. Valid values are `full_remodel|department_refresh|technology_upgrade|ada_compliance|expansion|format_conversion`',
    `roi_target_percentage` DECIMAL(18,2) COMMENT 'Target return on investment percentage for the remodel project, used in capital planning and project approval decisions.',
    `scope_description` STRING COMMENT 'Detailed narrative description of the remodel project scope, including specific work to be performed, fixtures to be replaced, technology to be installed, and layout changes.',
    `square_footage_after` STRING COMMENT 'Total retail square footage of the store after the remodel or expansion project is completed.',
    `square_footage_before` STRING COMMENT 'Total retail square footage of the store before the remodel or expansion project.',
    `store_closure_required_flag` BOOLEAN COMMENT 'Indicates whether the store must be fully closed to customers during the remodel period. True means full closure; false means store remains open with partial operations.',
    CONSTRAINT pk_remodel_project PRIMARY KEY(`remodel_project_id`)
) COMMENT 'Tracks store remodel, expansion, or refresh projects including project ID, store location, project type (full remodel, department refresh, technology upgrade, ADA compliance), planned start date, planned completion date, actual completion date, capital budget, contractor reference, and project status. Supports capital planning, store closure scheduling, and comp sales exclusion flagging during remodel periods.';

CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`site_selection` (
    `site_selection_id` BIGINT COMMENT 'Unique identifier for the site selection evaluation record. Primary key for the site selection entity.',
    `format_id` BIGINT COMMENT 'Foreign key linking to store.store_format. Business justification: Site selection records need to specify the intended store format; FK provides this relationship.',
    `annual_lease_cost` DECIMAL(18,2) COMMENT 'Estimated annual lease cost in US dollars including base rent and common area maintenance (CAM). Null if the site is a purchase.',
    `approval_stage` STRING COMMENT 'Current stage in the site selection approval workflow. Tracks progression through the capital allocation and real estate decision process. [ENUM-REF-CANDIDATE: initial_review|feasibility_study|due_diligence|executive_review|approved|rejected|on_hold — 7 candidates stripped; promote to reference product]',
    `candidate_address_line1` STRING COMMENT 'Primary street address line for the candidate site location.',
    `candidate_address_line2` STRING COMMENT 'Secondary address line for suite, building, or unit information for the candidate site.',
    `candidate_city` STRING COMMENT 'City name for the candidate site location.',
    `candidate_country_code` STRING COMMENT 'Three-letter ISO country code for the candidate site location.. Valid values are `^[A-Z]{3}$`',
    `candidate_postal_code` STRING COMMENT 'Postal or ZIP code for the candidate site location.. Valid values are `^[0-9]{5}(-[0-9]{4})?$`',
    `candidate_state_province` STRING COMMENT 'Two-letter state or province code for the candidate site location.. Valid values are `^[A-Z]{2}$`',
    `cannibalization_risk_score` DECIMAL(18,2) COMMENT 'Quantitative score (0-100) representing the estimated risk that the new store will negatively impact sales at existing nearby stores. Higher scores indicate greater cannibalization risk.',
    `competitor_count_within_trade_area` STRING COMMENT 'Number of direct competitor stores within the defined trade area. Used for competitive intensity assessment.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the site selection record was first created in the system. Used for pipeline tracking and audit trail.',
    `decision_date` DATE COMMENT 'Date when the final go/no-go decision was made for the candidate site. Null if still under evaluation.',
    `decision_outcome` STRING COMMENT 'Final decision outcome for the site selection evaluation. Approved sites move to the store opening pipeline.. Valid values are `approved|rejected|deferred`',
    `distance_to_nearest_competitor_miles` DECIMAL(18,2) COMMENT 'Distance in miles from the candidate site to the nearest competitor store. Used for competitive positioning analysis.',
    `distance_to_nearest_own_store_miles` DECIMAL(18,2) COMMENT 'Distance in miles from the candidate site to the nearest existing company-owned store. Used to assess cannibalization risk and market coverage gaps.',
    `environmental_assessment_status` STRING COMMENT 'Status of Phase I and Phase II environmental site assessments. Required for EPA (Environmental Protection Agency) compliance and risk mitigation, particularly for fuel center sites.. Valid values are `completed|in_progress|not_started|issues_identified`',
    `environmental_issues_flag` BOOLEAN COMMENT 'Indicates whether environmental concerns or contamination were identified during site assessment. True if issues exist that require remediation or further investigation.',
    `estimated_annual_sales_volume` DECIMAL(18,2) COMMENT 'Projected annual sales revenue in US dollars for the candidate location based on trade area analysis and comparable store performance. Used for ROI (Return on Investment) and capital allocation decisions.',
    `estimated_build_out_cost` DECIMAL(18,2) COMMENT 'Projected cost in US dollars for construction, fixtures, equipment, and store build-out. Used for total capital investment calculation.',
    `evaluation_notes` STRING COMMENT 'Free-text notes capturing qualitative observations, risks, opportunities, and strategic considerations for the site evaluation.',
    `last_updated_timestamp` TIMESTAMP COMMENT 'Timestamp when the site selection record was last modified. Used for change tracking and data freshness monitoring.',
    `latitude` DECIMAL(18,2) COMMENT 'Geographic latitude coordinate of the candidate site in decimal degrees. Used for proximity analysis and trade area mapping.',
    `lease_or_purchase_indicator` STRING COMMENT 'Indicates whether the site will be leased, purchased, or developed as a build-to-suit arrangement. Impacts capital requirements and financial modeling.. Valid values are `lease|purchase|build_to_suit`',
    `lease_term_years` STRING COMMENT 'Length of the proposed lease term in years. Null if the site is a purchase. Used for lease obligation tracking and financial planning.',
    `longitude` DECIMAL(18,2) COMMENT 'Geographic longitude coordinate of the candidate site in decimal degrees. Used for proximity analysis and trade area mapping.',
    `market_region` STRING COMMENT 'Geographic market or region designation for the candidate site. Used for regional portfolio planning and market penetration analysis.',
    `parking_spaces_count` STRING COMMENT 'Number of available parking spaces at the candidate site. Critical for customer convenience and local zoning compliance.',
    `permitting_status` STRING COMMENT 'Current status of building and operational permits required for store opening. Includes construction, occupancy, and health department permits.. Valid values are `approved|pending|not_started|denied`',
    `planned_square_footage` STRING COMMENT 'Total planned retail and operational square footage for the candidate store location.',
    `projected_opening_date` DATE COMMENT 'Target date for store opening if the site is approved. Used for capital planning and operational readiness scheduling.',
    `purchase_price` DECIMAL(18,2) COMMENT 'Proposed purchase price in US dollars for the site and property. Null if the site is a lease.',
    `real_estate_broker` STRING COMMENT 'Name of the real estate broker or firm representing the site opportunity. Used for vendor relationship tracking.',
    `site_code` STRING COMMENT 'Business identifier for the candidate site in the real estate pipeline. Used for tracking and reference across real estate and capital planning systems.. Valid values are `^[A-Z0-9]{6,12}$`',
    `target_store_format` STRING COMMENT 'Planned store format type for the candidate location. Determines size, assortment, and service capabilities. MFC (Micro-Fulfillment Center) represents automated fulfillment facilities. [ENUM-REF-CANDIDATE: supermarket|hypermarket|neighborhood_market|express|pharmacy|fuel_center|mfc — 7 candidates stripped; promote to reference product]',
    `trade_area_household_density` DECIMAL(18,2) COMMENT 'Number of households per square mile within the trade area. Indicates market density and potential customer base.',
    `trade_area_median_income` DECIMAL(18,2) COMMENT 'Median household income in US dollars within the trade area. Key demographic indicator for assortment planning and pricing strategy.',
    `trade_area_population` STRING COMMENT 'Total population count within the defined trade area radius for the candidate site. Used for demand forecasting and market sizing.',
    `traffic_count_daily` STRING COMMENT 'Average daily vehicle traffic count on the primary road adjacent to the candidate site. Key visibility and accessibility metric.',
    `zoning_status` STRING COMMENT 'Current status of zoning approval for the intended retail use. Critical gate for site viability and timeline planning.. Valid values are `approved|pending|not_started|variance_required|denied`',
    CONSTRAINT pk_site_selection PRIMARY KEY(`site_selection_id`)
) COMMENT 'Site selection evaluation record for candidate new store locations in the real estate pipeline. Captures site ID, candidate address and coordinates, target store format, trade area demographics (population, median income, household density), estimated annual sales volume, proximity to existing stores and competitors, cannibalization risk score, lease/purchase terms summary, zoning and permitting status, environmental assessment status, approval stage, and decision date. Supports real estate strategy, new store opening pipeline management, and capital allocation decisions.';

CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`location_event` (
    `location_event_id` BIGINT COMMENT 'Primary key for location_event',
    `associate_id` BIGINT COMMENT 'Identifier of the employee who initially reported or logged the event. Links to employee master data.',
    `store_location_id` BIGINT COMMENT 'Identifier of the store location where the event occurred. Links to the store master data.',
    `supplier_id` BIGINT COMMENT 'Identifier of the external vendor or contractor involved in the event response or resolution (e.g., HVAC repair company, security firm, cleaning service). Null if no external vendor was involved.',
    `affected_departments` STRING COMMENT 'Comma-separated list of store departments impacted by the event (e.g., Produce, Meat, Deli, Pharmacy, Bakery, Frozen Foods, Dairy). Used to assess scope of operational disruption.',
    `comp_sales_adjustment_flag` BOOLEAN COMMENT 'Indicates whether this event should trigger an adjustment to comparable store sales (comp sales) reporting. True if the event significantly impacted sales and should be excluded from comp sales calculations.',
    `created_timestamp` TIMESTAMP COMMENT 'Date and time when this event record was first created in the system. Represents the initial logging of the event.',
    `customer_communication_sent_flag` BOOLEAN COMMENT 'Indicates whether customer-facing communication was issued regarding the event (e.g., store closure notice, service disruption alert). True if communication was sent.',
    `duration_minutes` STRING COMMENT 'Total duration of the event in minutes, calculated from start to end timestamp. Used for operational impact analysis and SLA tracking.',
    `estimated_sales_impact_usd` DECIMAL(18,2) COMMENT 'Estimated financial impact on store sales revenue in US dollars due to the event. Used for insurance claims, comp sales adjustments, and financial reporting. Negative values indicate lost revenue.',
    `event_category` STRING COMMENT 'High-level grouping of the event type for reporting and analysis purposes. Operational events affect store operations, regulatory events involve compliance inspections, safety events involve hazards, emergency events require immediate response, promotional events are customer-facing, and maintenance events involve planned repairs.. Valid values are `operational|regulatory|safety|emergency|promotional|maintenance`',
    `event_end_timestamp` TIMESTAMP COMMENT 'Date and time when the store event ended or is scheduled to end. Null if the event is ongoing or end time is not yet determined.',
    `event_number` STRING COMMENT 'Business-facing unique event number assigned to this location event for tracking and reference purposes. Format: EVT-YYYYMMDD-NNNN.. Valid values are `^EVT-[0-9]{8}-[0-9]{4}$`',
    `event_start_timestamp` TIMESTAMP COMMENT 'Date and time when the store event began or is scheduled to begin. Represents the actual or planned start of the event occurrence.',
    `event_status` STRING COMMENT 'Current lifecycle status of the store event. Tracks progression from initial scheduling through resolution and closure.. Valid values are `scheduled|in_progress|resolved|closed|cancelled`',
    `event_type` STRING COMMENT 'Classification of the store event. Indicates the nature of the operational event. [ENUM-REF-CANDIDATE: grand_opening|temporary_closure|power_outage|refrigeration_failure|health_inspection|osha_visit|fire_marshal_inspection|bomb_threat|active_shooter_lockdown|community_event|remodel|weather_closure|emergency_closure|equipment_failure|pest_control|security_incident — promote to reference product]. Valid values are `grand_opening|temporary_closure|power_outage|refrigeration_failure|health_inspection|osha_visit`',
    `follow_up_action_items` STRING COMMENT 'List of follow-up actions, preventive measures, or corrective tasks identified as a result of the event. Used for continuous improvement and risk mitigation.',
    `impact_severity` STRING COMMENT 'Severity classification of the event impact on store operations. Critical events cause complete store closure or major safety hazards, major events significantly disrupt operations, minor events have limited impact, and informational events are for record-keeping only.. Valid values are `critical|major|minor|informational`',
    `inspection_result` STRING COMMENT 'Outcome of a regulatory inspection event. Passed indicates full compliance, failed indicates violations found, conditional indicates minor issues requiring follow-up, and pending indicates results not yet finalized.. Valid values are `passed|failed|conditional|pending`',
    `insurance_claim_number` STRING COMMENT 'Reference number for any insurance claim filed as a result of the event. Used to track financial recovery and claim status.',
    `media_coverage_flag` BOOLEAN COMMENT 'Indicates whether the event received media attention or press coverage. True if the event was reported in news media or social media.',
    `notes` STRING COMMENT 'Additional free-form notes, comments, or observations about the event that do not fit into other structured fields. Used for supplementary context and documentation.',
    `notification_sent_flag` BOOLEAN COMMENT 'Indicates whether required notifications were sent to corporate headquarters, regional management, or regulatory agencies. True if notifications were completed per protocol.',
    `notification_timestamp` TIMESTAMP COMMENT 'Date and time when the event notification was sent to relevant stakeholders. Used for compliance tracking and response time analysis.',
    `regulatory_agency` STRING COMMENT 'Name of the regulatory or governing body involved in the event (e.g., FDA, USDA, DEA, State Board of Pharmacy, OSHA, EPA, Fire Marshal, Health Department). Applicable for inspection and compliance events.',
    `resolution_notes` STRING COMMENT 'Detailed notes describing how the event was resolved, actions taken, and final outcome. Captures the response and remediation activities performed.',
    `root_cause_category` STRING COMMENT 'High-level classification of the underlying cause of the event. Used for trend analysis and preventive action planning.. Valid values are `equipment_failure|weather|human_error|external_threat|regulatory|planned`',
    `root_cause_description` STRING COMMENT 'Detailed narrative description of the root cause of the event. Provides context for understanding why the event occurred and what factors contributed.',
    `store_closure_flag` BOOLEAN COMMENT 'Indicates whether the event resulted in a complete store closure. True if the store was closed to customers during the event, False otherwise.',
    `updated_timestamp` TIMESTAMP COMMENT 'Date and time when this event record was last modified. Used for audit trail and change tracking purposes.',
    `violation_count` STRING COMMENT 'Number of violations or deficiencies identified during a regulatory inspection event. Zero indicates no violations found.',
    `violation_details` STRING COMMENT 'Detailed description of violations or deficiencies identified during a regulatory inspection, including citation codes and corrective action requirements.',
    CONSTRAINT pk_location_event PRIMARY KEY(`location_event_id`)
) COMMENT 'Record of significant operational events at a store location such as grand openings, temporary closures (weather, emergency, remodel), power outages, refrigeration failures, health department inspections, OSHA visits, fire marshal inspections, bomb threats, active shooter lockdowns, or special community events. Captures event type, start datetime, end datetime, duration, impact severity (critical/major/minor), affected departments, estimated sales impact, root cause category, resolution notes, and follow-up action items. Supports operational continuity planning, insurance claims, comp sales adjustment flagging, and regulatory compliance reporting.';
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`energy_management` (
    `energy_management_id` BIGINT COMMENT 'Unique identifier for each energy management record.',
    `store_location_id` BIGINT COMMENT 'Identifier of the store to which the energy data applies.',
    `prior_period_energy_management_id` BIGINT COMMENT 'Self-referencing FK on energy_management (prior_period_energy_management_id)',
    `average_demand_kw` DECIMAL(18,2) COMMENT 'Average demand over the reporting period, in kilowatts.',
    `carbon_emission_kg` DECIMAL(18,2) COMMENT 'Estimated CO₂ emissions associated with the energy consumption, in kilograms.',
    `consumption_amount` DECIMAL(18,2) COMMENT 'Quantity of energy consumed during the period.',
    `consumption_unit` STRING COMMENT 'Unit of measure for the consumption amount.. Valid values are `kwh|mwh|gallons|therms`',
    `consumption_variance_percent` DECIMAL(18,2) COMMENT 'Percentage difference between actual consumption and target.',
    `cost_per_unit` DECIMAL(18,2) COMMENT 'Cost per consumption unit (e.g., USD per kWh).',
    `cost_usd` DECIMAL(18,2) COMMENT 'Monetary cost of the energy consumed, expressed in US dollars.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when the record was first created in the system.',
    `data_quality_score` DECIMAL(18,2) COMMENT 'Score (0‑1) indicating the quality of the measurement data.',
    `energy_efficiency_rating` STRING COMMENT 'Rating of the stores energy efficiency based on internal benchmarks.. Valid values are `A|B|C|D|E|F`',
    `energy_management_status` STRING COMMENT 'Current lifecycle status of the energy record.. Valid values are `active|inactive|archived`',
    `energy_source` STRING COMMENT 'Primary source of energy for the measured consumption.. Valid values are `electricity|natural_gas|propane|solar`',
    `epa_reporting_flag` BOOLEAN COMMENT 'Indicates whether this record is required for EPA environmental reporting.',
    `last_calibration_date` DATE COMMENT 'Date when the meter was last calibrated.',
    `load_factor` DECIMAL(18,2) COMMENT 'Ratio of average demand to peak demand, indicating utilization efficiency.',
    `maintenance_date` TIMESTAMP COMMENT 'Timestamp of the most recent maintenance activity on the meter.',
    `measurement_method` STRING COMMENT 'Method used to obtain the energy reading.. Valid values are `submeter|utility_bill|estimated`',
    `measurement_timestamp` TIMESTAMP COMMENT 'Exact date‑time when the energy measurement was recorded.',
    `meter_code` STRING COMMENT 'Unique identifier of the energy meter or sub‑meter that generated the reading.',
    `meter_location` STRING COMMENT 'Physical location of the meter within the store (e.g., HVAC zone, refrigeration room).',
    `notes` STRING COMMENT 'Free‑form comments or observations related to the measurement.',
    `peak_demand_kw` DECIMAL(18,2) COMMENT 'Maximum demand recorded during the period, expressed in kilowatts.',
    `period_end_date` DATE COMMENT 'End date of the reporting period for the measurement.',
    `period_start_date` DATE COMMENT 'Start date of the reporting period for the measurement.',
    `reading_type` STRING COMMENT 'Indicates whether the reading is a direct measurement or an estimate.. Valid values are `actual|estimated`',
    `reporting_period` STRING COMMENT 'Fiscal or calendar period label (e.g., 2023-Q1) for regulatory reporting.',
    `sensor_status` STRING COMMENT 'Current operational status of the energy sensor.. Valid values are `ok|faulty|maintenance`',
    `source_system` STRING COMMENT 'Name of the source system that supplied the measurement (e.g., WMS, utility provider).',
    `sustainability_target_flag` BOOLEAN COMMENT 'True if the record is compared against a sustainability target.',
    `target_consumption_kwh` DECIMAL(18,2) COMMENT 'Energy consumption target for the reporting period, in kilowatt‑hours.',
    `updated_timestamp` TIMESTAMP COMMENT 'Timestamp of the most recent update to the record.',
    `utility_type` STRING COMMENT 'Category of utility being measured (e.g., HVAC, refrigeration, lighting).. Valid values are `hvac|refrigeration|lighting|water|gas`',
    CONSTRAINT pk_energy_management PRIMARY KEY(`energy_management_id`)
) COMMENT 'Store energy consumption monitoring and management records tracking HVAC, refrigeration, lighting, and utility usage by store location for sustainability reporting and cost optimization.';
...
CREATE OR REPLACE TABLE `grocery_ecm`.`store`.`banner` (
    ...
    `banner_logo_url` STRING COMMENT 'Web address of the banners logo image for digital assets.',
...
ALTER TABLE `grocery_ecm`.`store`.`department` ADD CONSTRAINT `fk_store_department_store_location_id` FOREIGN KEY (`store_location_id`) REFERENCES `grocery_ecm`.`store`.`store_location`(`store_location_id`);
ALTER TABLE `grocery_ecm`.`store`.`operating_hours` ADD CONSTRAINT `fk_store_operating_hours_store_location_id` FOREIGN KEY (`store_location_id`) REFERENCES `grocery_ecm`.`store`.`store_location`(`store_location_id`);
ALTER TABLE `grocery_ecm`.`store`.`service_capability` ADD CONSTRAINT `fk_store_service_capability_store_location_id` FOREIGN KEY (`store_location_id`) REFERENCES `grocery_ecm`.`store`.`store_location`(`store_location_id`);
...
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ADD CONSTRAINT `fk_store_remodel_project_store_location_id` FOREIGN KEY (`store_location_id`) REFERENCES `grocery_ecm`.`store`.`store_location`(`store_location_id`);
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ADD CONSTRAINT `fk_store_site_selection_format_id` FOREIGN KEY (`format_id`) REFERENCES `grocery_ecm`.`store`.`format`(`format_id`);
ALTER TABLE `grocery_ecm`.`store`.`location_event` ADD CONSTRAINT `fk_store_location_event_store_location_id` FOREIGN KEY (`store_location_id`) REFERENCES `grocery_ecm`.`store`.`store_location`(`store_location_id`);
...
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ADD CONSTRAINT `fk_store_energy_management_store_location_id` FOREIGN KEY (`store_location_id`) REFERENCES `grocery_ecm`.`store`.`store_location`(`store_location_id`);
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ADD CONSTRAINT `fk_store_energy_management_prior_period_energy_management_id` FOREIGN KEY (`prior_period_energy_management_id`) REFERENCES `grocery_ecm`.`store`.`energy_management`(`energy_management_id`);
...
ALTER TABLE `grocery_ecm`.`store`.`store_location` ALTER COLUMN `primary_store_manager_associate_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `grocery_ecm`.`store`.`format` SET TAGS ('dbx_subdomain' = 'organizational_structure');
...
ALTER TABLE `grocery_ecm`.`store`.`region` SET TAGS ('dbx_subdomain' = 'organizational_structure');
...
ALTER TABLE `grocery_ecm`.`store`.`district` SET TAGS ('dbx_subdomain' = 'organizational_structure');
...
ALTER TABLE `grocery_ecm`.`store`.`department` SET TAGS ('dbx_subdomain' = 'store_operations');
ALTER TABLE `grocery_ecm`.`store`.`department` ALTER COLUMN `department_id` SET TAGS ('dbx_business_glossary_term' = 'Department ID');
ALTER TABLE `grocery_ecm`.`store`.`department` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Department Manager Employee ID');
...
ALTER TABLE `grocery_ecm`.`store`.`department` ALTER COLUMN `store_location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
...
ALTER TABLE `grocery_ecm`.`store`.`department` ALTER COLUMN `gl_account_code` SET TAGS ('dbx_business_glossary_term' = 'General Ledger (GL) Account Code');
...
ALTER TABLE `grocery_ecm`.`store`.`department` ALTER COLUMN `profit_center_code` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Code');
...
ALTER TABLE `grocery_ecm`.`store`.`operating_hours` ALTER COLUMN `store_location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
...
ALTER TABLE `grocery_ecm`.`store`.`service_capability` ALTER COLUMN `store_location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
...
ALTER TABLE `grocery_ecm`.`store`.`service_capability` ALTER COLUMN `sla_target_time_minutes` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Target Time in Minutes');
...
ALTER TABLE `grocery_ecm`.`store`.`service_capability` ALTER COLUMN `vendor_partner_name` SET TAGS ('dbx_business_glossary_term' = 'Vendor Partner Name');
ALTER TABLE `grocery_ecm`.`store`.`mfc_profile` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `grocery_ecm`.`store`.`mfc_profile` SET TAGS ('dbx_subdomain' = 'real_estate');
...
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` SET TAGS ('dbx_subdomain' = 'store_operations');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `remodel_project_id` SET TAGS ('dbx_business_glossary_term' = 'Remodel Project ID');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `store_location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `actual_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Completion Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `actual_spend_amount` SET TAGS ('dbx_business_glossary_term' = 'Actual Spend Amount');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `actual_spend_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `actual_start_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Start Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Project Approval Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `cancellation_date` SET TAGS ('dbx_business_glossary_term' = 'Project Cancellation Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `capital_budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Capital Budget Amount');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `capital_budget_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `closure_end_date` SET TAGS ('dbx_business_glossary_term' = 'Closure End Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `closure_start_date` SET TAGS ('dbx_business_glossary_term' = 'Closure Start Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `comp_sales_exclusion_end_date` SET TAGS ('dbx_business_glossary_term' = 'Comparable Store Sales (Comp Sales) Exclusion End Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `comp_sales_exclusion_flag` SET TAGS ('dbx_business_glossary_term' = 'Comparable Store Sales (Comp Sales) Exclusion Flag');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `comp_sales_exclusion_start_date` SET TAGS ('dbx_business_glossary_term' = 'Comparable Store Sales (Comp Sales) Exclusion Start Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `contractor_name` SET TAGS ('dbx_business_glossary_term' = 'Contractor Name');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `contractor_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `contractor_name` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `contractor_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Contractor Reference Number');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `departments_impacted` SET TAGS ('dbx_business_glossary_term' = 'Departments Impacted');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `fiscal_year` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Year');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `inspection_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Final Inspection Completion Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Project Notes');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `payback_period_months` SET TAGS ('dbx_business_glossary_term' = 'Payback Period in Months');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `payback_period_months` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `permit_issue_date` SET TAGS ('dbx_business_glossary_term' = 'Permit Issue Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `permit_number` SET TAGS ('dbx_business_glossary_term' = 'Building Permit Number');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `planned_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Completion Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `planned_start_date` SET TAGS ('dbx_business_glossary_term' = 'Planned Start Date');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `priority_rank` SET TAGS ('dbx_business_glossary_term' = 'Priority Rank');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_manager_name` SET TAGS ('dbx_business_glossary_term' = 'Project Manager Name');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_name` SET TAGS ('dbx_business_glossary_term' = 'Project Name');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_number` SET TAGS ('dbx_business_glossary_term' = 'Project Number');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{8,20}$');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_status` SET TAGS ('dbx_business_glossary_term' = 'Project Status');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_status` SET TAGS ('dbx_value_regex' = 'planning|approved|in_progress|on_hold|completed|cancelled');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_type` SET TAGS ('dbx_business_glossary_term' = 'Project Type');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `project_type` SET TAGS ('dbx_value_regex' = 'full_remodel|department_refresh|technology_upgrade|ada_compliance|expansion|format_conversion');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `roi_target_percentage` SET TAGS ('dbx_business_glossary_term' = 'Return on Investment (ROI) Target Percentage');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `roi_target_percentage` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `scope_description` SET TAGS ('dbx_business_glossary_term' = 'Scope Description');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `square_footage_after` SET TAGS ('dbx_business_glossary_term' = 'Square Footage After Remodel');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `square_footage_before` SET TAGS ('dbx_business_glossary_term' = 'Square Footage Before Remodel');
ALTER TABLE `grocery_ecm`.`store`.`remodel_project` ALTER COLUMN `store_closure_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Store Closure Required Flag');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` SET TAGS ('dbx_subdomain' = 'real_estate');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `site_selection_id` SET TAGS ('dbx_business_glossary_term' = 'Site Selection ID');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `format_id` SET TAGS ('dbx_business_glossary_term' = 'Store Format Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `annual_lease_cost` SET TAGS ('dbx_business_glossary_term' = 'Annual Lease Cost');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `annual_lease_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `approval_stage` SET TAGS ('dbx_business_glossary_term' = 'Approval Stage');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Candidate Address Line 1');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_address_line1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Candidate Address Line 2');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_address_line2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_city` SET TAGS ('dbx_business_glossary_term' = 'Candidate City');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_country_code` SET TAGS ('dbx_business_glossary_term' = 'Candidate Country Code');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Candidate Postal Code');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_postal_code` SET TAGS ('dbx_value_regex' = '^[0-9]{5}(-[0-9]{4})?$');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_state_province` SET TAGS ('dbx_business_glossary_term' = 'Candidate State or Province');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_state_province` SET TAGS ('dbx_value_regex' = '^[A-Z]{2}$');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `candidate_state_province` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `cannibalization_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Cannibalization Risk Score');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `competitor_count_within_trade_area` SET TAGS ('dbx_business_glossary_term' = 'Competitor Count Within Trade Area');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `decision_date` SET TAGS ('dbx_business_glossary_term' = 'Decision Date');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `decision_outcome` SET TAGS ('dbx_business_glossary_term' = 'Decision Outcome');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `decision_outcome` SET TAGS ('dbx_value_regex' = 'approved|rejected|deferred');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `distance_to_nearest_competitor_miles` SET TAGS ('dbx_business_glossary_term' = 'Distance to Nearest Competitor (Miles)');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `distance_to_nearest_own_store_miles` SET TAGS ('dbx_business_glossary_term' = 'Distance to Nearest Own Store (Miles)');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `environmental_assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Environmental Assessment Status');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `environmental_assessment_status` SET TAGS ('dbx_value_regex' = 'completed|in_progress|not_started|issues_identified');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `environmental_issues_flag` SET TAGS ('dbx_business_glossary_term' = 'Environmental Issues Flag');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `estimated_annual_sales_volume` SET TAGS ('dbx_business_glossary_term' = 'Estimated Annual Sales Volume');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `estimated_annual_sales_volume` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `estimated_build_out_cost` SET TAGS ('dbx_business_glossary_term' = 'Estimated Build-Out Cost');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `estimated_build_out_cost` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `evaluation_notes` SET TAGS ('dbx_business_glossary_term' = 'Evaluation Notes');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `latitude` SET TAGS ('dbx_business_glossary_term' = 'Latitude');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `latitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `latitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `lease_or_purchase_indicator` SET TAGS ('dbx_business_glossary_term' = 'Lease or Purchase Indicator');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `lease_or_purchase_indicator` SET TAGS ('dbx_value_regex' = 'lease|purchase|build_to_suit');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `lease_term_years` SET TAGS ('dbx_business_glossary_term' = 'Lease Term (Years)');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `longitude` SET TAGS ('dbx_business_glossary_term' = 'Longitude');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `longitude` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `longitude` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `market_region` SET TAGS ('dbx_business_glossary_term' = 'Market Region');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `parking_spaces_count` SET TAGS ('dbx_business_glossary_term' = 'Parking Spaces Count');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `permitting_status` SET TAGS ('dbx_business_glossary_term' = 'Permitting Status');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `permitting_status` SET TAGS ('dbx_value_regex' = 'approved|pending|not_started|denied');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `planned_square_footage` SET TAGS ('dbx_business_glossary_term' = 'Planned Square Footage');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `projected_opening_date` SET TAGS ('dbx_business_glossary_term' = 'Projected Opening Date');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `purchase_price` SET TAGS ('dbx_business_glossary_term' = 'Purchase Price');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `purchase_price` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `real_estate_broker` SET TAGS ('dbx_business_glossary_term' = 'Real Estate Broker');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `site_code` SET TAGS ('dbx_business_glossary_term' = 'Site Code');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `site_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,12}$');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `target_store_format` SET TAGS ('dbx_business_glossary_term' = 'Target Store Format');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `trade_area_household_density` SET TAGS ('dbx_business_glossary_term' = 'Trade Area Household Density');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `trade_area_median_income` SET TAGS ('dbx_business_glossary_term' = 'Trade Area Median Income');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `trade_area_population` SET TAGS ('dbx_business_glossary_term' = 'Trade Area Population');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `traffic_count_daily` SET TAGS ('dbx_business_glossary_term' = 'Traffic Count (Daily)');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `zoning_status` SET TAGS ('dbx_business_glossary_term' = 'Zoning Status');
ALTER TABLE `grocery_ecm`.`store`.`site_selection` ALTER COLUMN `zoning_status` SET TAGS ('dbx_value_regex' = 'approved|pending|not_started|variance_required|denied');
ALTER TABLE `grocery_ecm`.`store`.`location_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`store`.`location_event` SET TAGS ('dbx_subdomain' = 'store_operations');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `location_event_id` SET TAGS ('dbx_business_glossary_term' = 'Location Event Identifier');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `associate_id` SET TAGS ('dbx_business_glossary_term' = 'Reported By Employee ID');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `store_location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'External Vendor ID');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `affected_departments` SET TAGS ('dbx_business_glossary_term' = 'Affected Departments');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `comp_sales_adjustment_flag` SET TAGS ('dbx_business_glossary_term' = 'Comparable Sales (Comp Sales) Adjustment Flag');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `customer_communication_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Customer Communication Sent Flag');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Duration Minutes');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `estimated_sales_impact_usd` SET TAGS ('dbx_business_glossary_term' = 'Estimated Sales Impact (USD)');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `estimated_sales_impact_usd` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_category` SET TAGS ('dbx_business_glossary_term' = 'Event Category');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_category` SET TAGS ('dbx_value_regex' = 'operational|regulatory|safety|emergency|promotional|maintenance');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_end_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Event End Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_number` SET TAGS ('dbx_business_glossary_term' = 'Event Number');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_number` SET TAGS ('dbx_value_regex' = '^EVT-[0-9]{8}-[0-9]{4}$');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Event Start Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_status` SET TAGS ('dbx_business_glossary_term' = 'Event Status');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_status` SET TAGS ('dbx_value_regex' = 'scheduled|in_progress|resolved|closed|cancelled');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_type` SET TAGS ('dbx_business_glossary_term' = 'Event Type');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `event_type` SET TAGS ('dbx_value_regex' = 'grand_opening|temporary_closure|power_outage|refrigeration_failure|health_inspection|osha_visit');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `follow_up_action_items` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Action Items');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `impact_severity` SET TAGS ('dbx_business_glossary_term' = 'Impact Severity');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `impact_severity` SET TAGS ('dbx_value_regex' = 'critical|major|minor|informational');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `inspection_result` SET TAGS ('dbx_business_glossary_term' = 'Inspection Result');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `inspection_result` SET TAGS ('dbx_value_regex' = 'passed|failed|conditional|pending');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `insurance_claim_number` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Number');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `insurance_claim_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `media_coverage_flag` SET TAGS ('dbx_business_glossary_term' = 'Media Coverage Flag');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `notification_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Notification Sent Flag');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `notification_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Notification Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `regulatory_agency` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Agency');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `resolution_notes` SET TAGS ('dbx_business_glossary_term' = 'Resolution Notes');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `root_cause_category` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Category');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `root_cause_category` SET TAGS ('dbx_value_regex' = 'equipment_failure|weather|human_error|external_threat|regulatory|planned');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `root_cause_description` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Description');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `store_closure_flag` SET TAGS ('dbx_business_glossary_term' = 'Store Closure Flag');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `violation_count` SET TAGS ('dbx_business_glossary_term' = 'Violation Count');
ALTER TABLE `grocery_ecm`.`store`.`location_event` ALTER COLUMN `violation_details` SET TAGS ('dbx_business_glossary_term' = 'Violation Details');
ALTER TABLE `grocery_ecm`.`store`.`health_inspection` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`store`.`health_inspection` SET TAGS ('dbx_subdomain' = 'store_operations');
...
ALTER TABLE `grocery_ecm`.`store`.`lease` SET TAGS ('dbx_subdomain' = 'real_estate');
...
ALTER TABLE `grocery_ecm`.`store`.`energy_management` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` SET TAGS ('dbx_subdomain' = 'real_estate');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_management_id` SET TAGS ('dbx_business_glossary_term' = 'Energy Management Record ID');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `store_location_id` SET TAGS ('dbx_business_glossary_term' = 'Store ID');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `prior_period_energy_management_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `average_demand_kw` SET TAGS ('dbx_business_glossary_term' = 'Average Demand (kW)');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `carbon_emission_kg` SET TAGS ('dbx_business_glossary_term' = 'Carbon Emission (kg)');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `consumption_amount` SET TAGS ('dbx_business_glossary_term' = 'Energy Consumption Amount');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `consumption_unit` SET TAGS ('dbx_business_glossary_term' = 'Energy Consumption Unit');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `consumption_unit` SET TAGS ('dbx_value_regex' = 'kwh|mwh|gallons|therms');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `consumption_variance_percent` SET TAGS ('dbx_business_glossary_term' = 'Consumption Variance (%)');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `cost_per_unit` SET TAGS ('dbx_business_glossary_term' = 'Cost Per Unit');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `cost_usd` SET TAGS ('dbx_business_glossary_term' = 'Energy Cost (USD)');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `data_quality_score` SET TAGS ('dbx_business_glossary_term' = 'Data Quality Score');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_efficiency_rating` SET TAGS ('dbx_business_glossary_term' = 'Energy Efficiency Rating');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_efficiency_rating` SET TAGS ('dbx_value_regex' = 'A|B|C|D|E|F');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_management_status` SET TAGS ('dbx_business_glossary_term' = 'Record Status');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_management_status` SET TAGS ('dbx_value_regex' = 'active|inactive|archived');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_source` SET TAGS ('dbx_business_glossary_term' = 'Energy Source');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `energy_source` SET TAGS ('dbx_value_regex' = 'electricity|natural_gas|propane|solar');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `epa_reporting_flag` SET TAGS ('dbx_business_glossary_term' = 'EPA Reporting Flag');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `last_calibration_date` SET TAGS ('dbx_business_glossary_term' = 'Last Calibration Date');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `load_factor` SET TAGS ('dbx_business_glossary_term' = 'Load Factor');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `maintenance_date` SET TAGS ('dbx_business_glossary_term' = 'Meter Maintenance Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `measurement_method` SET TAGS ('dbx_business_glossary_term' = 'Measurement Method');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `measurement_method` SET TAGS ('dbx_value_regex' = 'submeter|utility_bill|estimated');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `measurement_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Measurement Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `meter_code` SET TAGS ('dbx_business_glossary_term' = 'Meter Identifier');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `meter_location` SET TAGS ('dbx_business_glossary_term' = 'Meter Location');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `peak_demand_kw` SET TAGS ('dbx_business_glossary_term' = 'Peak Demand (kW)');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Period End Date');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Period Start Date');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `reading_type` SET TAGS ('dbx_business_glossary_term' = 'Reading Type');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `reading_type` SET TAGS ('dbx_value_regex' = 'actual|estimated');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `reporting_period` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `sensor_status` SET TAGS ('dbx_business_glossary_term' = 'Sensor Status');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `sensor_status` SET TAGS ('dbx_value_regex' = 'ok|faulty|maintenance');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `source_system` SET TAGS ('dbx_business_glossary_term' = 'Source System');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `sustainability_target_flag` SET TAGS ('dbx_business_glossary_term' = 'Sustainability Target Flag');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `target_consumption_kwh` SET TAGS ('dbx_business_glossary_term' = 'Target Consumption (kWh)');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `utility_type` SET TAGS ('dbx_business_glossary_term' = 'Utility Type');
ALTER TABLE `grocery_ecm`.`store`.`energy_management` ALTER COLUMN `utility_type` SET TAGS ('dbx_value_regex' = 'hvac|refrigeration|lighting|water|gas');
ALTER TABLE `grocery_ecm`.`store`.`banner` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `grocery_ecm`.`store`.`banner` SET TAGS ('dbx_subdomain' = 'organizational_structure');
...
ALTER TABLE `grocery_ecm`.`store`.`division` SET TAGS ('dbx_subdomain' = 'organizational_structure');
