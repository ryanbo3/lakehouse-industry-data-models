-- Schema for Domain: vendor | Business: Grocery | Version: v1_mvm
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`supplier_contact` (
    ...
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to vendor.supplier_site. Business justification: Supplier contacts are often site-specific — a quality manager at a manufacturing plant, an EDI coordinator at a distribution warehouse, or a compliance officer at a specific facility. The existing off',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`trade_agreement` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Trade agreement rebates, payment terms, and slotting fees are accrued against specific fiscal periods for GAAP compliance. Finance teams align trade agreement activation to fiscal periods for rebate l',
    ...
    `item_hierarchy_id` BIGINT COMMENT 'Foreign key linking to product.item_hierarchy. Business justification: Trade agreements in grocery retail are routinely negotiated at the category level (e.g., all dairy, all private label beverages). Buyers and category managers need to link contracts to the item hierar',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`vendor_item` (
    ...
    `trade_agreement_id` BIGINT COMMENT 'Foreign key linking to vendor.trade_agreement. Business justification: A vendor item is governed by a specific trade agreement (master supply agreement, private-label co-manufacturing agreement, or DSD agreement). The existing contract_reference_number (STRING) is a deno',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`cost_schedule` (
    ...
    `cost_price_id` BIGINT COMMENT 'Foreign key linking to pricing.cost_price. Business justification: Cost schedules are the vendor-side source documents that produce cost_price records in the retailers pricing system. Linking them enables the procurement-to-pricing cost traceability pipeline — a nam',
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Vendor cost schedules (bracket pricing, allowances) are activated and accrued per fiscal period. Finance teams reconcile vendor cost changes against fiscal periods during period-close. Grocery retail ',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Cost schedules drive COGS and inventory cost postings in grocery retail. The GL account for cost-of-goods must be known at the cost schedule level to enable automated journal entry generation during g',
    `supplier_id` BIGINT COMMENT 'Reference to the supplier or vendor providing the item at this cost.',
    `trade_agreement_id` BIGINT COMMENT 'Foreign key linking to vendor.trade_agreement. Business justification: Cost schedules (wholesale COGS pricing) are negotiated and established under trade agreements. The existing contract_reference_number (STRING) on cost_schedule is a denormalized reference to the gover',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`trade_allowance` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Trade allowances are accrued to specific cost centers for departmental P&L reporting in grocery retail. The existing cost_center_code is a denormalized plain-text field; replacing it with a proper FK ',
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Trade allowances (slotting fees, promotional funds) must be accrued and settled within specific fiscal periods per GAAP. Finance teams track allowance accruals by fiscal period for period-close report',
    ...
    `reward_offer_id` BIGINT COMMENT 'Foreign key linking to loyalty.reward_offer. Business justification: Trade allowances directly fund specific loyalty reward offers (e.g., a CPG suppliers promotional allowance pays for a bonus-points event on their SKUs). Retail-grocery trade promotion managers must l',
    `payment_run_id` BIGINT COMMENT 'Foreign key linking to finance.payment_run. Business justification: Trade allowance payments in grocery retail are disbursed via payment runs (ACH or check batches). Linking the allowance to its settlement payment run enables cash management reconciliation and audit o',
    `supplier_id` BIGINT COMMENT 'Reference to the CPG supplier or vendor providing the trade allowance.',
    `tpr_id` BIGINT COMMENT 'Foreign key linking to pricing.tpr. Business justification: Vendor-funded TPRs (tpr.vendor_funded_flag=true) must be traceable to the specific trade_allowance authorizing and funding the discount. This supports vendor TPR reconciliation and post-event settleme',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`performance_scorecard` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Vendor performance scorecards are evaluated per fiscal period to trigger rebate accruals, contract renegotiations, and shrink recovery postings. Finance teams align scorecard periods to fiscal periods',
    `item_hierarchy_id` BIGINT COMMENT 'Foreign key linking to product.item_hierarchy. Business justification: Supplier performance scorecards in grocery retail are scoped to product categories (e.g., dairy OTIF, produce fill rate). Category managers need category-level supplier scorecards for quarterly busine',
    `node_id` BIGINT COMMENT 'Foreign key linking to fulfillment.node. Business justification: Grocery DC-level OTIF and fill-rate reporting requires scorecards tied to the receiving fulfillment node. Buyers and DC managers run node-level supplier performance reports (on_time_delivery_rate, oti',
    `supplier_id` BIGINT COMMENT 'Reference to the supplier or vendor being evaluated in this scorecard period.',
    `trade_agreement_id` BIGINT COMMENT 'Foreign key linking to vendor.trade_agreement. Business justification: Performance scorecards measure supplier KPIs (fill rate, OTIF, invoice accuracy) against thresholds that are typically defined in a trade agreement. The scorecards contract_compliance_flag and contra',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`compliance_record` (
    ...
    `product_item_id` BIGINT COMMENT 'Foreign key linking to product.product_item. Business justification: Food safety compliance records (HACCP, organic certification, USDA) are tied to specific products, not just suppliers. Grocery food safety and quality teams need to link compliance records to specific',
    ...
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to vendor.supplier_site. Business justification: Compliance certifications (HACCP, FDA registration, organic certification, USDA inspection) are facility-specific, not just supplier-level. The existing facility_location (STRING) on compliance_record',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` (
    `trade_allowance_item_id` BIGINT COMMENT 'Primary key for the trade_allowance_item association',
    `trade_allowance_id` BIGINT COMMENT 'Foreign key linking to the parent trade allowance program that covers this vendor item.',
    `vendor_item_id` BIGINT COMMENT 'Foreign key linking to the specific vendor item (SKU) that qualifies under this trade allowance.',
    `accrued_amount` DECIMAL(18,2) COMMENT 'Total dollar amount of allowance accrued to date for this specific vendor item under the parent trade allowance program. Enables item-level accrual tracking and trade promotion ROI analysis by SKU, which cannot be derived from the header-level accrued_amount alone.',
    `effective_end_date` DATE COMMENT 'The date on which this vendor items qualification under the trade allowance program ends. May be equal to or earlier than the parent allowance promotion period end date.',
    `effective_start_date` DATE COMMENT 'The date from which this vendor item qualifies under the trade allowance program. May be equal to or later than the parent allowance promotion period start date, allowing item-level sub-period qualification.',
    `minimum_purchase_quantity` DECIMAL(18,2) COMMENT 'The minimum quantity of this specific vendor item that must be purchased to trigger the allowance for this item. Enables item-level qualifying thresholds that may differ from the header-level minimum_purchase_quantity on the parent trade_allowance.',
    `rate_value` DECIMAL(18,2) COMMENT 'The numeric allowance rate applicable to this specific vendor item under the parent allowance program. May differ from the header-level allowance_rate_value when item-level overrides are negotiated. Interpretation depends on the parent allowance_rate_type (percentage, fixed amount per case, etc.).',
    CONSTRAINT pk_trade_allowance_item PRIMARY KEY(`trade_allowance_item_id`)
) COMMENT 'This association product represents the Contract between a trade_allowance program and a specific vendor_item that qualifies under that program. It captures the item-level terms of a trade allowance agreement — including item-specific rates, qualifying quantities, accrual amounts, and effective dates — that exist only in the context of a specific allowance-item pairing. Each record links one trade_allowance to one vendor_item and is the authoritative source for deduction management, EDI 844/849 line-item processing, and trade promotion ROI analysis at the SKU level.. Existence Justification: In grocery retail trade promotion management, a single trade allowance program (e.g., a scan allowance or bill-back) routinely covers dozens or hundreds of specific vendor items (SKUs), and a single vendor item can simultaneously participate in multiple active trade allowance programs (e.g., an off-invoice allowance AND a performance rebate from the same supplier). The business actively manages this coverage list — adding or removing qualifying items, setting item-level rates, and tracking accruals per item — making this a genuine operational M:N relationship, not an analytical correlation. The detection phase correctly identified the qualifying_sku_list STRING anti-pattern on trade_allowance as direct evidence that this M:N is currently unresolved.';
...
ALTER TABLE `grocery_ecm`.`vendor`.`supplier_contact` ADD CONSTRAINT `fk_vendor_supplier_contact_supplier_site_id` FOREIGN KEY (`supplier_site_id`) REFERENCES `grocery_ecm`.`vendor`.`supplier_site`(`supplier_site_id`);
...
ALTER TABLE `grocery_ecm`.`vendor`.`vendor_item` ADD CONSTRAINT `fk_vendor_vendor_item_trade_agreement_id` FOREIGN KEY (`trade_agreement_id`) REFERENCES `grocery_ecm`.`vendor`.`trade_agreement`(`trade_agreement_id`);
...
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` ADD CONSTRAINT `fk_vendor_cost_schedule_trade_agreement_id` FOREIGN KEY (`trade_agreement_id`) REFERENCES `grocery_ecm`.`vendor`.`trade_agreement`(`trade_agreement_id`);
...
ALTER TABLE `grocery_ecm`.`vendor`.`performance_scorecard` ADD CONSTRAINT `fk_vendor_performance_scorecard_trade_agreement_id` FOREIGN KEY (`trade_agreement_id`) REFERENCES `grocery_ecm`.`vendor`.`trade_agreement`(`trade_agreement_id`);
...
ALTER TABLE `grocery_ecm`.`vendor`.`compliance_record` ADD CONSTRAINT `fk_vendor_compliance_record_supplier_site_id` FOREIGN KEY (`supplier_site_id`) REFERENCES `grocery_ecm`.`vendor`.`supplier_site`(`supplier_site_id`);
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ADD CONSTRAINT `fk_vendor_trade_allowance_item_trade_allowance_id` FOREIGN KEY (`trade_allowance_id`) REFERENCES `grocery_ecm`.`vendor`.`trade_allowance`(`trade_allowance_id`);
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ADD CONSTRAINT `fk_vendor_trade_allowance_item_vendor_item_id` FOREIGN KEY (`vendor_item_id`) REFERENCES `grocery_ecm`.`vendor`.`vendor_item`(`vendor_item_id`);
...
ALTER TABLE `grocery_ecm`.`vendor`.`supplier` SET TAGS ('dbx_subdomain' = 'supplier_management');
...
ALTER TABLE `grocery_ecm`.`vendor`.`supplier_contact` SET TAGS ('dbx_subdomain' = 'supplier_management');
...
ALTER TABLE `grocery_ecm`.`vendor`.`supplier_contact` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`supplier_site` SET TAGS ('dbx_subdomain' = 'supplier_management');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_agreement` SET TAGS ('dbx_subdomain' = 'trade_agreements');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_agreement` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_agreement` ALTER COLUMN `item_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Item Hierarchy Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`vendor_item` SET TAGS ('dbx_subdomain' = 'supplier_management');
...
ALTER TABLE `grocery_ecm`.`vendor`.`vendor_item` ALTER COLUMN `trade_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Agreement Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` SET TAGS ('dbx_subdomain' = 'trade_agreements');
...
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` ALTER COLUMN `cost_price_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Price Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier ID');
ALTER TABLE `grocery_ecm`.`vendor`.`cost_schedule` ALTER COLUMN `trade_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Agreement Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` SET TAGS ('dbx_subdomain' = 'trade_agreements');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` ALTER COLUMN `reward_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Reward Offer Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` ALTER COLUMN `payment_run_id` SET TAGS ('dbx_business_glossary_term' = 'Settlement Payment Run Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier ID');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance` ALTER COLUMN `tpr_id` SET TAGS ('dbx_business_glossary_term' = 'Tpr Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`performance_scorecard` SET TAGS ('dbx_subdomain' = 'compliance_performance');
...
ALTER TABLE `grocery_ecm`.`vendor`.`performance_scorecard` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`performance_scorecard` ALTER COLUMN `item_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Item Hierarchy Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`performance_scorecard` ALTER COLUMN `node_id` SET TAGS ('dbx_business_glossary_term' = 'Node Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`performance_scorecard` ALTER COLUMN `trade_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Agreement Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`compliance_requirement` SET TAGS ('dbx_subdomain' = 'compliance_performance');
...
ALTER TABLE `grocery_ecm`.`vendor`.`compliance_record` SET TAGS ('dbx_subdomain' = 'compliance_performance');
...
ALTER TABLE `grocery_ecm`.`vendor`.`compliance_record` ALTER COLUMN `product_item_id` SET TAGS ('dbx_business_glossary_term' = 'Product Item Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`vendor`.`compliance_record` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
ALTER TABLE `grocery_ecm`.`vendor`.`compliance_record` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` SET TAGS ('dbx_subdomain' = 'trade_agreements');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` SET TAGS ('dbx_association_edges' = 'vendor.trade_allowance,vendor.vendor_item');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `trade_allowance_item_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Allowance Item - Trade Allowance Item Id');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `trade_allowance_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Allowance Item - Trade Allowance Id');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `vendor_item_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Allowance Item - Vendor Item Id');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `accrued_amount` SET TAGS ('dbx_business_glossary_term' = 'Item Accrued Allowance Amount');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Item Allowance Effective End Date');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Item Allowance Effective Start Date');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `minimum_purchase_quantity` SET TAGS ('dbx_business_glossary_term' = 'Item Minimum Purchase Quantity');
ALTER TABLE `grocery_ecm`.`vendor`.`trade_allowance_item` ALTER COLUMN `rate_value` SET TAGS ('dbx_business_glossary_term' = 'Item Allowance Rate Value');
