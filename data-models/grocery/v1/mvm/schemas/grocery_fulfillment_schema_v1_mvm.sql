-- Schema for Domain: fulfillment | Business: Grocery | Version: v1_mvm
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`wave` (
    ...
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to vendor.supplier_site. Business justification: Grocery DC inbound receiving waves are organized around scheduled supplier site deliveries. Linking wave to supplier_site enables inbound wave planning tied to supplier delivery appointments, receivin',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`wms_task` (
    ...
    `fulfillment_order_line_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order_line. Business justification: A WMS pick/putaway task is executed against a specific fulfillment order line (the SKU-level unit of work). Linking wms_task directly to fulfillment_order_line enables operational traceability from ta',
    `lot_batch_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_batch. Business justification: WMS pick tasks for perishables must reference the specific lot/batch to enforce FIFO/FEFO rotation and FDA FSMA lot-level traceability for recalls. wms_task.lot_number is a denormalized plain attribut',
    `order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: A WMS task is directed work for a specific fulfillment order. While the path wms_task → fulfillment_order_line → fulfillment_order exists, a direct FK to fulfillment_order enables efficient order-leve',
    `receiving_record_id` BIGINT COMMENT 'Foreign key linking to inventory.receiving_record. Business justification: Putaway WMS tasks are directly triggered by receiving records — when goods arrive at a DC or store, a putaway task is created to move items from the receiving dock to storage. This fundamental WMS rec',
    ...
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to fulfillment.sla_policy. Business justification: wms_task has sla_target_completion_time as a plain attribute but no FK to sla_policy. Task-level SLA compliance tracking (pick task completion within policy window) is a core WMS KPI in grocery fulfil',
    ...
    `transfer_order_id` BIGINT COMMENT 'Foreign key linking to inventory.transfer_order. Business justification: Transfer order execution generates WMS pick and putaway tasks at source and destination locations. WMS tasks must reference the authorizing transfer_order for operational tracking, discrepancy resolut',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`pack_task` (
    ...
    `fulfillment_order_line_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order_line. Business justification: A pack task packs specific line items (SKUs) for an order. pack_task has product_order_line_id (cross-domain) but no in-domain link to fulfillment_order_line. Adding fulfillment_order_line_id enables ',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: A pack task is the packing execution step for a specific fulfillment order. pack_task currently has order_header_id (cross-domain) and wave_id but no in-domain link to fulfillment_order. Adding fulfil',
    `rx_fill_id` BIGINT COMMENT 'Foreign key linking to pharmacy.rx_fill. Business justification: Packing a prescription order requires the pack station to verify the specific rx_fill — Rx label must match dispensed fill, controlled substance documentation must be captured at pack time, and pharma',
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to fulfillment.sla_policy. Business justification: Pack tasks are governed by SLA policies defining pack time windows (pack_sla_minutes in sla_policy). pack_task has sla_target_pack_time and sla_compliance_flag but no sla_policy_id FK. Adding this FK ',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`shipment` (
    ...
    `delivery_route_id` BIGINT COMMENT 'Reference to the optimized delivery route that includes this shipment, used for last-mile delivery planning and driver dispatch.',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: A shipment is the physical dispatch of goods for a fulfillment order. shipment has order_header_id (cross-domain) and wave_id but no direct in-domain link to fulfillment_order. Adding fulfillment_orde',
    ...
    `shopper_id` BIGINT COMMENT 'Foreign key linking to customer.shopper. Business justification: Last-mile grocery delivery requires direct shopper reference on shipment for proof-of-delivery capture, signature verification, delivery exception handling, and customer notification triggers. Driver ',
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to fulfillment.sla_policy. Business justification: Shipments are governed by SLA policies that define delivery windows, breach thresholds, and compensation rules. shipment has no sla_policy_id currently. Adding this FK enables direct SLA compliance ev',
    ...
    `supplier_site_id` BIGINT COMMENT 'Foreign key linking to vendor.supplier_site. Business justification: Inbound shipment receiving requires knowing the originating supplier site for OTIF measurement, receiving discrepancy resolution, and cold-chain compliance verification. Grocery DC receiving teams mus',
    `transfer_order_id` BIGINT COMMENT 'Foreign key linking to inventory.transfer_order. Business justification: DC-to-store replenishment shipments execute a transfer order. Grocery supply chain operations require shipment-to-transfer-order linkage for end-to-end stock movement tracking, discrepancy resolution,',
    ...
    `wholesale_account_id` BIGINT COMMENT 'Foreign key linking to customer.wholesale_account. Business justification: Grocery wholesale shipments to business accounts require direct wholesale_account reference for BOL generation, account-level delivery SLA tracking, freight cost allocation, and wholesale delivery sch',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` (
    ...
    `node_id` BIGINT COMMENT 'Foreign key linking to fulfillment.node. Business justification: Pickup staging occurs at a specific fulfillment node (store backroom or MFC). pickup_staging has store_location_id (cross-domain to store domain) but no in-domain FK to the fulfillment node. Adding fu',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: Pickup staging is the physical execution stage of a curbside or click-and-collect fulfillment order. pickup_staging has order_header_id (cross-domain) but no in-domain link to fulfillment_order. Addin',
    `pharmacy_location_id` BIGINT COMMENT 'Foreign key linking to pharmacy.pharmacy_location. Business justification: When prescription_included_flag=true on pickup_staging, the staging area must coordinate with a specific pharmacy_location for pharmacist-to-customer prescription handoff, HIPAA-compliant counseling, ',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`order` (
    `order_id` BIGINT COMMENT 'Unique identifier for the fulfillment order. This is the primary key for the fulfillment domain representation of an order released for physical execution.',
    ...
    `channel_price_id` BIGINT COMMENT 'Foreign key linking to pricing.channel_price. Business justification: Channel-specific pricing compliance: grocery fulfillment orders are channel-specific (delivery, pickup, in-store). Linking to channel_price enables channel-level price audit, margin reporting by fulfi',
    ...
    `pharmacy_location_id` BIGINT COMMENT 'Foreign key linking to pharmacy.pharmacy_location. Business justification: When a fulfillment order includes prescription items (prescription_flag=true), the dispensing pharmacy_location must be identified for pharmacist verification, DEA-regulated controlled substance hando',
    ...
    `shopper_id` BIGINT COMMENT 'Foreign key linking to customer.shopper. Business justification: Grocery omnichannel fulfillment requires direct shopper reference on fulfillment_order for substitution approval workflows, delivery exception notifications, and SLA breach alerts. Fulfillment operati',
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to fulfillment.sla_policy. Business justification: Fulfillment orders are governed by SLA policies that define pick, pack, and dispatch timing requirements. fulfillment_order has sla_due_timestamp but no sla_policy_id FK. Adding this FK enables direct',
    `store_cluster_id` BIGINT COMMENT 'Foreign key linking to assortment.store_cluster. Business justification: Assortment-aware order routing: the store cluster determines which SKUs are authorized for fulfillment, substitution depth, and private-label penetration targets. Grocery omnichannel operations route ',
    ...
    `wholesale_account_id` BIGINT COMMENT 'Foreign key linking to customer.wholesale_account. Business justification: Grocery wholesale fulfillment orders must be tied to the wholesale account for credit limit validation, account-level SLA tracking, minimum order enforcement, and B2B billing reconciliation. Wholesale',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` (
    ...
    `cost_price_id` BIGINT COMMENT 'Foreign key linking to pricing.cost_price. Business justification: Shrink and margin impact reporting: when fulfillment lines are short-picked, damaged, or substituted, the cost price of the affected item is required for shrink/loss accounting, margin impact analysis',
    ...
    `lot_batch_id` BIGINT COMMENT 'Foreign key linking to inventory.lot_batch. Business justification: Order lines for perishable and regulated items (deli, pharmacy, alcohol) require lot-level traceability for FDA FSMA compliance and recall management. fulfillment_order_line.lot_number is a denormaliz',
    `markdown_id` BIGINT COMMENT 'Foreign key linking to pricing.markdown. Business justification: Perishable clearance and shrink reporting: grocery fulfillment of markdown items (clearance, near-expiry) must reference the active markdown record to confirm correct price application, track sell-thr',
    `new_item_intro_id` BIGINT COMMENT 'Foreign key linking to assortment.new_item_intro. Business justification: New item introduction velocity tracking: grocery merchants track first-receipt fulfillment events against new_item_intro records to measure launch performance, confirm first_ship_date actuals, and val',
    `offer_item_id` BIGINT COMMENT 'Foreign key linking to promotion.offer_item. Business justification: Fulfillment order line execution must enforce offer_item-level rules: max quantity, substitution eligibility, vendor-funded flags, and planogram placement. The promo_offer_id on fulfillment_order_line',
    `order_id` BIGINT COMMENT 'Reference to the parent fulfillment order header. Links this line to the overall order being fulfilled.',
    ...
    `retail_price_id` BIGINT COMMENT 'Foreign key linking to pricing.retail_price. Business justification: Price-at-time-of-pick audit and substitution price validation: grocery fulfillment must trace which retail price record governed each order line to support price accuracy audits, substitution price co',
    ...
    `slot_location_id` BIGINT COMMENT 'Foreign key linking to fulfillment.slot_location. Business justification: fulfillment_order_line has pick_location as a STRING field representing the physical slot where the item is to be picked. Normalizing this to a FK to slot_location replaces the free-text string with a',
    ...
    `tpr_id` BIGINT COMMENT 'Foreign key linking to pricing.tpr. Business justification: Vendor-funded TPR reconciliation: when a TPR is active at pick time, grocery fulfillment must reference the TPR record to validate the discounted price applied, support vendor deduction claims, and pr',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`delivery_route` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Route-level costs (fuel, driver labor, vehicle wear) must be allocated to cost centers for P&L reporting. fulfillment_delivery_route has route_cost_amount and fuel_cost_amount; linking to cost_center ',
    ...
    `sla_policy_id` BIGINT COMMENT 'Foreign key linking to fulfillment.sla_policy. Business justification: Delivery routes are governed by SLA policies defining on-time delivery thresholds, dispatch windows, and breach escalation rules. fulfillment_delivery_route has sla_compliance_flag but no sla_policy_i',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`slot_location` (
    ...
    `node_id` BIGINT COMMENT 'Foreign key linking to fulfillment.node. Business justification: A slot location physically exists within a fulfillment node (DC, MFC, or store backroom). slot_location has dc_facility_id (cross-domain to supply) but no in-domain FK to the fulfillment node. Adding ',
    `planogram_id` BIGINT COMMENT 'Foreign key linking to assortment.planogram. Business justification: Planogram-to-slot compliance mapping: slot locations are physically assigned to planogram positions for slotting optimization and planogram compliance audits. The existing planogram_slot_reference pla',
    ...
    `supplier_id` BIGINT COMMENT 'Foreign key linking to vendor.supplier. Business justification: Grocery DCs designate supplier-dedicated slot locations for DSD deliveries and vendor-managed inventory sections. Slot-to-supplier assignment enables DSD receiving dock management, vendor compliance a',
    `temperature_zone_id` BIGINT COMMENT 'Foreign key linking to inventory.temperature_zone. Business justification: Pick slot locations in grocery fulfillment centers have specific temperature requirements (frozen, refrigerated, ambient). Slot locations must reference the governing temperature_zone for cold-chain c',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`sla_policy` (
    ...
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: Tier-based SLA differentiation is a named grocery omnichannel process: Platinum/Gold loyalty members receive priority fulfillment SLAs. sla_policy.customer_tier is a denormalized tier reference; repla',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`node` (
    ...
    `pharmacy_location_id` BIGINT COMMENT 'Foreign key linking to pharmacy.pharmacy_location. Business justification: Fulfillment nodes with pharmacy_licensed_flag=true correspond to a specific licensed pharmacy_location for prescription order routing, DEA compliance reporting, and pharmacist verification workflows. ',
    `price_zone_id` BIGINT COMMENT 'Foreign key linking to pricing.price_zone. Business justification: Zone-based pricing compliance for omnichannel fulfillment: each fulfillment node operates within a price zone that determines applicable retail prices. Linking node to price_zone enables zone-price co',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Fulfillment nodes are financial reporting units assigned to profit centers for revenue and cost attribution in retail-grocery. Enables node-level profitability reporting, same-store sales comparisons,',
    `store_cluster_id` BIGINT COMMENT 'Foreign key linking to assortment.store_cluster. Business justification: Node-to-cluster assortment availability: a fulfillment node serves stores belonging to a specific store cluster, which governs which assortment depth and planogram templates apply. Grocery order manag',
    `temperature_zone_id` BIGINT COMMENT 'Foreign key linking to inventory.temperature_zone. Business justification: Fulfillment nodes (DCs, MFCs, dark stores) must be certified against defined temperature zones for cold-chain compliance (FDA, USDA). Grocery operations require node-to-temperature-zone mapping for co',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`tote` (
    ...
    `delivery_route_id` BIGINT COMMENT 'Reference to the delivery route this tote is assigned to for last-mile delivery. Links tote to carrier and route optimization.',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: A tote is packed and dispatched for a specific fulfillment order. tote has order_header_id (cross-domain) but no in-domain link to fulfillment_order. Adding fulfillment_order_id creates the in-domain ',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`fulfillment`.`vehicle` (
    ...
    `temperature_zone_id` BIGINT COMMENT 'Foreign key linking to inventory.temperature_zone. Business justification: Delivery vehicles with cold-chain capability must be certified against specific temperature zones (frozen, refrigerated). Grocery last-mile delivery requires vehicle-to-temperature-zone mapping for co',
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ADD CONSTRAINT `fk_fulfillment_wms_task_fulfillment_order_line_id` FOREIGN KEY (`fulfillment_order_line_id`) REFERENCES `grocery_ecm`.`fulfillment`.`fulfillment_order_line`(`fulfillment_order_line_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ADD CONSTRAINT `fk_fulfillment_wms_task_order_id` FOREIGN KEY (`order_id`) REFERENCES `grocery_ecm`.`fulfillment`.`order`(`order_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ADD CONSTRAINT `fk_fulfillment_wms_task_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `grocery_ecm`.`fulfillment`.`sla_policy`(`sla_policy_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ADD CONSTRAINT `fk_fulfillment_pack_task_fulfillment_order_line_id` FOREIGN KEY (`fulfillment_order_line_id`) REFERENCES `grocery_ecm`.`fulfillment`.`fulfillment_order_line`(`fulfillment_order_line_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ADD CONSTRAINT `fk_fulfillment_pack_task_order_id` FOREIGN KEY (`order_id`) REFERENCES `grocery_ecm`.`fulfillment`.`order`(`order_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ADD CONSTRAINT `fk_fulfillment_pack_task_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `grocery_ecm`.`fulfillment`.`sla_policy`(`sla_policy_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ADD CONSTRAINT `fk_fulfillment_shipment_delivery_route_id` FOREIGN KEY (`delivery_route_id`) REFERENCES `grocery_ecm`.`fulfillment`.`delivery_route`(`delivery_route_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ADD CONSTRAINT `fk_fulfillment_shipment_order_id` FOREIGN KEY (`order_id`) REFERENCES `grocery_ecm`.`fulfillment`.`order`(`order_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ADD CONSTRAINT `fk_fulfillment_shipment_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `grocery_ecm`.`fulfillment`.`sla_policy`(`sla_policy_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` ADD CONSTRAINT `fk_fulfillment_pickup_staging_node_id` FOREIGN KEY (`node_id`) REFERENCES `grocery_ecm`.`fulfillment`.`node`(`node_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` ADD CONSTRAINT `fk_fulfillment_pickup_staging_order_id` FOREIGN KEY (`order_id`) REFERENCES `grocery_ecm`.`fulfillment`.`order`(`order_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ADD CONSTRAINT `fk_fulfillment_order_carrier_id` FOREIGN KEY (`carrier_id`) REFERENCES `grocery_ecm`.`fulfillment`.`carrier`(`carrier_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ADD CONSTRAINT `fk_fulfillment_order_node_id` FOREIGN KEY (`node_id`) REFERENCES `grocery_ecm`.`fulfillment`.`node`(`node_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ADD CONSTRAINT `fk_fulfillment_order_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `grocery_ecm`.`fulfillment`.`sla_policy`(`sla_policy_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ADD CONSTRAINT `fk_fulfillment_order_wave_id` FOREIGN KEY (`wave_id`) REFERENCES `grocery_ecm`.`fulfillment`.`wave`(`wave_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ADD CONSTRAINT `fk_fulfillment_fulfillment_order_line_order_id` FOREIGN KEY (`order_id`) REFERENCES `grocery_ecm`.`fulfillment`.`order`(`order_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ADD CONSTRAINT `fk_fulfillment_fulfillment_order_line_slot_location_id` FOREIGN KEY (`slot_location_id`) REFERENCES `grocery_ecm`.`fulfillment`.`slot_location`(`slot_location_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ADD CONSTRAINT `fk_fulfillment_delivery_route_carrier_id` FOREIGN KEY (`carrier_id`) REFERENCES `grocery_ecm`.`fulfillment`.`carrier`(`carrier_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ADD CONSTRAINT `fk_fulfillment_delivery_route_node_id` FOREIGN KEY (`node_id`) REFERENCES `grocery_ecm`.`fulfillment`.`node`(`node_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ADD CONSTRAINT `fk_fulfillment_delivery_route_sla_policy_id` FOREIGN KEY (`sla_policy_id`) REFERENCES `grocery_ecm`.`fulfillment`.`sla_policy`(`sla_policy_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ADD CONSTRAINT `fk_fulfillment_delivery_route_vehicle_id` FOREIGN KEY (`vehicle_id`) REFERENCES `grocery_ecm`.`fulfillment`.`vehicle`(`vehicle_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ADD CONSTRAINT `fk_fulfillment_delivery_route_wave_id` FOREIGN KEY (`wave_id`) REFERENCES `grocery_ecm`.`fulfillment`.`wave`(`wave_id`);
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` ADD CONSTRAINT `fk_fulfillment_slot_location_node_id` FOREIGN KEY (`node_id`) REFERENCES `grocery_ecm`.`fulfillment`.`node`(`node_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`tote` ADD CONSTRAINT `fk_fulfillment_tote_delivery_route_id` FOREIGN KEY (`delivery_route_id`) REFERENCES `grocery_ecm`.`fulfillment`.`delivery_route`(`delivery_route_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`tote` ADD CONSTRAINT `fk_fulfillment_tote_order_id` FOREIGN KEY (`order_id`) REFERENCES `grocery_ecm`.`fulfillment`.`order`(`order_id`);
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`wave` SET TAGS ('dbx_subdomain' = 'warehouse_operations');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`wave` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` SET TAGS ('dbx_subdomain' = 'warehouse_operations');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `fulfillment_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Line Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `lot_batch_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Batch Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `receiving_record_id` SET TAGS ('dbx_business_glossary_term' = 'Receiving Record Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`wms_task` ALTER COLUMN `transfer_order_id` SET TAGS ('dbx_business_glossary_term' = 'Transfer Order Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` SET TAGS ('dbx_subdomain' = 'warehouse_operations');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ALTER COLUMN `fulfillment_order_line_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Line Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ALTER COLUMN `order_header_id` SET TAGS ('dbx_business_glossary_term' = 'Order ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ALTER COLUMN `rx_fill_id` SET TAGS ('dbx_business_glossary_term' = 'Rx Fill Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`pack_task` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` SET TAGS ('dbx_subdomain' = 'delivery_dispatch');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `delivery_route_id` SET TAGS ('dbx_business_glossary_term' = 'Route ID');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `shopper_id` SET TAGS ('dbx_business_glossary_term' = 'Shopper Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `supplier_site_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Site Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `transfer_order_id` SET TAGS ('dbx_business_glossary_term' = 'Transfer Order Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`shipment` ALTER COLUMN `wholesale_account_id` SET TAGS ('dbx_business_glossary_term' = 'Wholesale Account Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` SET TAGS ('dbx_subdomain' = 'order_execution');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` ALTER COLUMN `node_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`pickup_staging` ALTER COLUMN `pharmacy_location_id` SET TAGS ('dbx_business_glossary_term' = 'Pharmacy Location Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` SET TAGS ('dbx_subdomain' = 'order_execution');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `channel_price_id` SET TAGS ('dbx_business_glossary_term' = 'Channel Price Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `node_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `order_header_id` SET TAGS ('dbx_business_glossary_term' = 'Source Order ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `pharmacy_location_id` SET TAGS ('dbx_business_glossary_term' = 'Pharmacy Location Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `prescription_id` SET TAGS ('dbx_business_glossary_term' = 'Prescription Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `prescription_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `prescription_id` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `promo_campaign_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Campaign Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `shopper_id` SET TAGS ('dbx_business_glossary_term' = 'Shopper Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `store_cluster_id` SET TAGS ('dbx_business_glossary_term' = 'Store Cluster Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `wave_id` SET TAGS ('dbx_business_glossary_term' = 'Wave ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `wholesale_account_id` SET TAGS ('dbx_business_glossary_term' = 'Wholesale Account Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `alcohol_flag` SET TAGS ('dbx_business_glossary_term' = 'Alcohol Flag');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Reason');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `cancellation_reason` SET TAGS ('dbx_value_regex' = 'customer_request|inventory_unavailable|address_issue|payment_failure|fraud_detected|system_error');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `cancellation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Cancellation Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `carrier_service_level` SET TAGS ('dbx_business_glossary_term' = 'Carrier Service Level');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `carrier_service_level` SET TAGS ('dbx_value_regex' = 'same_day|next_day|two_day|standard|economy');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `cold_chain_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Cold Chain Required Flag');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `delivery_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Delivery Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `dispatch_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `exception_code` SET TAGS ('dbx_business_glossary_term' = 'Exception Code');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `exception_notes` SET TAGS ('dbx_business_glossary_term' = 'Exception Notes');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_channel` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Channel');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_channel` SET TAGS ('dbx_value_regex' = 'dc|mfc|store_backroom|dsd|cross_dock|vendor_direct');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_order_number` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Number');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_order_number` SET TAGS ('dbx_value_regex' = '^FO[0-9]{10}$');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_status` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Status');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_type` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Type');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `fulfillment_type` SET TAGS ('dbx_value_regex' = 'home_delivery|curbside_pickup|in_store_pickup|ship_to_store|locker_pickup');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `hazmat_flag` SET TAGS ('dbx_business_glossary_term' = 'Hazardous Materials (HAZMAT) Flag');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `oms_release_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Order Management System (OMS) Release Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `pack_complete_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Pack Complete Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `package_count` SET TAGS ('dbx_business_glossary_term' = 'Package Count');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `pick_complete_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Pick Complete Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `pick_start_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Pick Start Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `prescription_flag` SET TAGS ('dbx_business_glossary_term' = 'Prescription Flag');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `prescription_flag` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `prescription_flag` SET TAGS ('dbx_pii_health' = 'true');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `priority_code` SET TAGS ('dbx_business_glossary_term' = 'Priority Code');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `priority_code` SET TAGS ('dbx_value_regex' = 'urgent|high|standard|low');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `short_pick_count` SET TAGS ('dbx_business_glossary_term' = 'Short Pick Count');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `sla_due_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Due Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `special_instructions` SET TAGS ('dbx_business_glossary_term' = 'Special Instructions');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `substitution_count` SET TAGS ('dbx_business_glossary_term' = 'Substitution Count');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `temperature_zone` SET TAGS ('dbx_business_glossary_term' = 'Temperature Zone');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `temperature_zone` SET TAGS ('dbx_value_regex' = 'ambient|chilled|frozen|multi_temp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `total_line_count` SET TAGS ('dbx_business_glossary_term' = 'Total Line Count');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `total_unit_count` SET TAGS ('dbx_business_glossary_term' = 'Total Unit Count');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `total_volume_m3` SET TAGS ('dbx_business_glossary_term' = 'Total Volume in Cubic Meters (M3)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `total_weight_kg` SET TAGS ('dbx_business_glossary_term' = 'Total Weight in Kilograms (KG)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `tracking_number` SET TAGS ('dbx_business_glossary_term' = 'Tracking Number');
ALTER TABLE `grocery_ecm`.`fulfillment`.`order` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` SET TAGS ('dbx_subdomain' = 'order_execution');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `cost_price_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Price Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `lot_batch_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Batch Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `markdown_id` SET TAGS ('dbx_business_glossary_term' = 'Markdown Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `new_item_intro_id` SET TAGS ('dbx_business_glossary_term' = 'New Item Intro Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `offer_item_id` SET TAGS ('dbx_business_glossary_term' = 'Offer Item Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order ID');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `retail_price_id` SET TAGS ('dbx_business_glossary_term' = 'Retail Price Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `slot_location_id` SET TAGS ('dbx_business_glossary_term' = 'Slot Location Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`fulfillment_order_line` ALTER COLUMN `tpr_id` SET TAGS ('dbx_business_glossary_term' = 'Tpr Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`carrier` SET TAGS ('dbx_subdomain' = 'delivery_dispatch');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` SET TAGS ('dbx_subdomain' = 'delivery_dispatch');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `delivery_route_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Delivery Route ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `carrier_id` SET TAGS ('dbx_business_glossary_term' = 'Carrier ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `node_id` SET TAGS ('dbx_business_glossary_term' = 'Facility ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `sla_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Policy Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `vehicle_id` SET TAGS ('dbx_business_glossary_term' = 'Vehicle ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `wave_id` SET TAGS ('dbx_business_glossary_term' = 'Wave ID');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `actual_distance_miles` SET TAGS ('dbx_business_glossary_term' = 'Actual Distance Miles');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `actual_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Actual Duration Minutes');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `actual_end_time` SET TAGS ('dbx_business_glossary_term' = 'Actual End Time');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `actual_start_time` SET TAGS ('dbx_business_glossary_term' = 'Actual Start Time');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `completed_stops` SET TAGS ('dbx_business_glossary_term' = 'Completed Stops');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `dispatch_time` SET TAGS ('dbx_business_glossary_term' = 'Dispatch Time');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `estimated_duration_minutes` SET TAGS ('dbx_business_glossary_term' = 'Estimated Duration Minutes');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `failed_stops` SET TAGS ('dbx_business_glossary_term' = 'Failed Stops');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `fuel_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Fuel Cost Amount');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `fuel_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Route Notes');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `on_time_delivery_percentage` SET TAGS ('dbx_business_glossary_term' = 'On-Time Delivery Percentage');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `optimization_source` SET TAGS ('dbx_business_glossary_term' = 'Optimization Source');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `optimization_source` SET TAGS ('dbx_value_regex' = 'jda_tms|manual|blue_yonder|third_party');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `planned_end_time` SET TAGS ('dbx_business_glossary_term' = 'Planned End Time');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `planned_start_time` SET TAGS ('dbx_business_glossary_term' = 'Planned Start Time');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Route Cost Amount');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_date` SET TAGS ('dbx_business_glossary_term' = 'Route Date');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_number` SET TAGS ('dbx_business_glossary_term' = 'Route Number');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_status` SET TAGS ('dbx_business_glossary_term' = 'Route Status');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|partial|cancelled');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_type` SET TAGS ('dbx_business_glossary_term' = 'Route Type');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `route_type` SET TAGS ('dbx_value_regex' = 'home_delivery|curbside_pickup|store_transfer|vendor_return');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `service_level` SET TAGS ('dbx_business_glossary_term' = 'Service Level');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `service_level` SET TAGS ('dbx_value_regex' = 'same_day|next_day|two_day|standard');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `sla_compliance_flag` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Compliance Flag');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `temperature_zone` SET TAGS ('dbx_business_glossary_term' = 'Temperature Zone');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `temperature_zone` SET TAGS ('dbx_value_regex' = 'ambient|refrigerated|frozen|multi_temp');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `total_distance_miles` SET TAGS ('dbx_business_glossary_term' = 'Total Distance Miles');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `total_order_count` SET TAGS ('dbx_business_glossary_term' = 'Total Order Count');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `total_stops` SET TAGS ('dbx_business_glossary_term' = 'Total Stops');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `total_volume_cubic_feet` SET TAGS ('dbx_business_glossary_term' = 'Total Volume Cubic Feet');
ALTER TABLE `grocery_ecm`.`fulfillment`.`delivery_route` ALTER COLUMN `total_weight_lbs` SET TAGS ('dbx_business_glossary_term' = 'Total Weight Pounds');
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` SET TAGS ('dbx_subdomain' = 'warehouse_operations');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` ALTER COLUMN `node_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Node Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` ALTER COLUMN `planogram_id` SET TAGS ('dbx_business_glossary_term' = 'Planogram Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`slot_location` ALTER COLUMN `temperature_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Temperature Zone Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`sla_policy` SET TAGS ('dbx_subdomain' = 'order_execution');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`sla_policy` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`node` SET TAGS ('dbx_subdomain' = 'warehouse_operations');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`node` ALTER COLUMN `pharmacy_location_id` SET TAGS ('dbx_business_glossary_term' = 'Pharmacy Location Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`node` ALTER COLUMN `price_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Price Zone Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`node` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`node` ALTER COLUMN `store_cluster_id` SET TAGS ('dbx_business_glossary_term' = 'Store Cluster Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`fulfillment`.`node` ALTER COLUMN `temperature_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Temperature Zone Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`tote` SET TAGS ('dbx_subdomain' = 'warehouse_operations');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`tote` ALTER COLUMN `delivery_route_id` SET TAGS ('dbx_business_glossary_term' = 'Delivery Route Identifier (ID)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`tote` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`vehicle` SET TAGS ('dbx_subdomain' = 'delivery_dispatch');
...
ALTER TABLE `grocery_ecm`.`fulfillment`.`vehicle` ALTER COLUMN `temperature_zone_id` SET TAGS ('dbx_business_glossary_term' = 'Temperature Zone Id (Foreign Key)');
