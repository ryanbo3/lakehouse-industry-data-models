-- Schema for Domain: loyalty | Business: Grocery | Version: v1_mvm
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`tier` (
    ...
    `program_config_id` BIGINT COMMENT 'Foreign key linking to loyalty.program_config. Business justification: tier is the reference master defining the loyalty program tier structure (Member, Silver, Gold, Platinum). program_config stores the complete configuration and business rules for each loyalty program.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`tier_history` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Tier evaluation events (upgrades/downgrades) are assessed against fiscal periods for loyalty program P&L reporting and period-close reconciliation. Finance teams require tier-change costs bucketed by ',
    `membership_id` BIGINT COMMENT 'Reference to the loyalty program member who experienced this tier change event.',
    `tier_id` BIGINT COMMENT 'Foreign key linking to loyalty.tier. Business justification: tier_history currently stores new_tier_code and new_tier_name as denormalized STRING columns. Adding new_tier_id as a FK to loyalty.tier.tier_id normalizes this relationship, allowing a JOIN to retrie',
    ...
    `transaction_id` BIGINT COMMENT 'Foreign key linking to payment.payment_transaction. Business justification: Tier changes are often triggered by cumulative spend on payment transactions; linking supports tier‑evaluation reports and automated tier upgrades.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`points_account` (
    ...
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Points accounts represent deferred revenue / breakage liability on the balance sheet under ASC 606/IFRS 15. Each account type maps to a GL account for liability recognition. Retail-grocery finance tea',
    ...
    `program_config_id` BIGINT COMMENT 'Foreign key linking to loyalty.program_config. Business justification: points_account is the master ledger account for a loyalty membership. It has account_type, expiration_policy_code, earn_multiplier, and currency_code — all of which are governed by the program_config ',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`points_transaction` (
    ...
    `digital_coupon_id` BIGINT COMMENT 'Foreign key linking to promotion.digital_coupon. Business justification: When a digital coupon is redeemed, bonus loyalty points may be awarded. points_transaction already has promo_offer_id but digital coupon-specific points attribution (e.g., manufacturer-funded coupon b',
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Points earn/burn transactions must be assigned to fiscal periods for deferred revenue accounting under ASC 606 (performance obligation tracking). Period-end close requires all points transactions buck',
    ...
    `order_header_id` BIGINT COMMENT 'Foreign key linking to product.order_header. Business justification: Loyalty points earn reconciliation requires linking each points_transaction to the originating purchase order. Grocery retailers audit points earned per transaction against order totals; finance and l',
    `points_account_id` BIGINT COMMENT 'Foreign key linking to loyalty.points_account. Business justification: points_transaction is the granular ledger recording every loyalty currency movement. points_account is the master ledger account holding the running balance. Every points_transaction must be posted ag',
    ...
    `reward_offer_id` BIGINT COMMENT 'Foreign key linking to loyalty.reward_offer. Business justification: points_transaction already has promo_offer_id linking to the promotion domains promo_offer. However, reward_offer is a loyalty-domain concept representing loyalty-specific reward offers (bonus points',
    ...
    `trade_allowance_id` BIGINT COMMENT 'Foreign key linking to vendor.trade_allowance. Business justification: Bonus points transactions funded by supplier trade allowances (e.g., double-points weekends, category-specific earn events) must be attributed to the specific trade allowance for co-op settlement, all',
    `transaction_id` BIGINT COMMENT 'Foreign key linking to payment.payment_transaction. Business justification: Required for Points‑Earned per Transaction report, linking each points transaction to its originating payment transaction for audit and financial reconciliation.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`reward_offer` (
    ...
    `category_id` BIGINT COMMENT 'Foreign key linking to assortment.category. Business justification: Grocery loyalty programs routinely configure category-level earn offers (e.g., 2x points on all organic produce). A category_id FK on reward_offer enables category-level offer targeting, margin impa',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Reward offer costs are charged to specific cost centers for departmental P&L and budget tracking. The denormalized cost_center_code plain attribute on reward_offer should be replaced with a proper FK.',
    `new_item_intro_id` BIGINT COMMENT 'Foreign key linking to assortment.new_item_intro. Business justification: New item trial incentive programs are a named grocery loyalty business process: bonus points are awarded to members who purchase newly introduced items. Linking reward_offer to new_item_intro enables ',
    `price_rule_id` BIGINT COMMENT 'Foreign key linking to pricing.price_rule. Business justification: Grocery loyalty reward offers must comply with pricing guardrails (margin_floor_percent, price_ceiling, stackable_flag priority) defined in price_rule. Loyalty offer governance process requires reward',
    ...
    `promo_offer_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_offer. Business justification: Loyalty reward offers in grocery are frequently backed by or derived from promotional offers (e.g., a bonus points offer tied to a specific promo_offer). Offer reconciliation, vendor funding attributi',
    `retail_price_id` BIGINT COMMENT 'Foreign key linking to pricing.retail_price. Business justification: Loyalty pricing analysts validate that reward_offer discount_value does not breach the margin floor anchored to the shelf retail price. Grocery operations require reward offers to reference the govern',
    `segment_id` BIGINT COMMENT 'Foreign key linking to customer.segment. Business justification: Reward offers are targeted to specific customer segments (e.g., Gold-tier shoppers, organic-affinity segment). The targeting_segment_criteria column is a denormalized text representation of the segmen',
    `store_cluster_id` BIGINT COMMENT 'Foreign key linking to assortment.store_cluster. Business justification: Grocery loyalty offers are cluster-targeted (urban vs. rural stores receive different fuel or produce rewards). A store_cluster_id FK on reward_offer enables cluster-level offer deployment and ROI rep',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to supply.supply_supplier_contract. Business justification: Vendor-funded loyalty programs: CPG suppliers fund bonus-point offers under specific supplier contracts that define allowance rates and funding terms. Grocery trade promotion teams must link each vend',
    ...
    `tender_type_id` BIGINT COMMENT 'Foreign key linking to payment.tender_type. Business justification: Grocery retailers run tender-linked loyalty promotions (e.g., 2x points when paying with store credit card, fuel discount only with loyalty Visa). The reward_offer must reference the eligible tend',
    `trade_agreement_id` BIGINT COMMENT 'Foreign key linking to vendor.trade_agreement. Business justification: Supplier-funded loyalty reward offers (co-op bonus points, double-points events) are authorized and governed by a specific trade agreement. Category managers need to trace which trade agreement funds ',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`member_offer` (
    ...
    `order_header_id` BIGINT COMMENT 'Foreign key linking to product.order_header. Business justification: Offer redemption attribution reporting requires knowing which specific order triggered a member_offer redemption status change. Grocery loyalty teams track offer-to-order conversion rates and validate',
    ...
    `promo_offer_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_offer. Business justification: member_offer has promo_campaign_id but no offer-level FK. In grocery loyalty, member offers are assigned at the promo_offer level (specific discount mechanics, eligible SKUs). Personalization targetin',
    `retail_price_id` BIGINT COMMENT 'Foreign key linking to pricing.retail_price. Business justification: Personalized member offer discount_amount is computed relative to the current retail_price.srp_amount. Grocery loyalty personalization and member offer ROI reporting require joining member_offer to re',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` (
    ...
    `fiscal_period_id` BIGINT COMMENT 'Foreign key linking to finance.fiscal_period. Business justification: Redemption events must be mapped to fiscal periods for loyalty liability reduction reporting and period-close reconciliation. Finance teams run period-end reports bucketing redemptions by fiscal perio',
    `gift_card_id` BIGINT COMMENT 'Foreign key linking to payment.gift_card. Business justification: Grocery loyalty programs allow points redemption for gift card issuance (points-to-gift-card redemption type). When redemption_type is GIFT_CARD, the loyalty_redemption must reference the specific g',
    `member_offer_id` BIGINT COMMENT 'Foreign key linking to loyalty.member_offer. Business justification: loyalty_redemption already references reward_offer_id (the offer template), but not the specific member_offer instance (the personalized assignment of that offer to the member). Adding member_offer_id',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to fulfillment.fulfillment_order. Business justification: Omnichannel redemption reconciliation: loyalty analysts and fulfillment ops must join redemptions to fulfillment_order to validate that delivery/pickup channel redemptions correspond to fulfilled orde',
    `points_transaction_id` BIGINT COMMENT 'Foreign key linking to loyalty.points_transaction. Business justification: loyalty_redemption captures each redemption event. When a member redeems points, a corresponding points_transaction record is created to debit the points balance. Adding points_transaction_id to loyal',
    ...
    `promo_offer_id` BIGINT COMMENT 'Foreign key linking to promotion.promo_offer. Business justification: loyalty_redemption already has promo_campaign_id but lacks offer-level granularity. Grocery loyalty redemption reporting and offer performance dashboards require knowing which specific promo_offer was',
    ...
    `transaction_id` BIGINT COMMENT 'External transaction identifier from the Point of Sale (POS) or Order Management System (OMS) where the redemption occurred. Links to NCR POS or Salesforce Commerce Cloud transaction.',
...
CREATE OR REPLACE TABLE `grocery_ecm`.`loyalty`.`program_config` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Multi-banner grocery operators (e.g., Kroger, Albertsons) operate loyalty programs under distinct legal entities for regulatory compliance, tax reporting, and financial consolidation. Program config m',
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier` ADD CONSTRAINT `fk_loyalty_tier_program_config_id` FOREIGN KEY (`program_config_id`) REFERENCES `grocery_ecm`.`loyalty`.`program_config`(`program_config_id`);
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier_history` ADD CONSTRAINT `fk_loyalty_tier_history_tier_id` FOREIGN KEY (`tier_id`) REFERENCES `grocery_ecm`.`loyalty`.`tier`(`tier_id`);
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_account` ADD CONSTRAINT `fk_loyalty_points_account_program_config_id` FOREIGN KEY (`program_config_id`) REFERENCES `grocery_ecm`.`loyalty`.`program_config`(`program_config_id`);
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ADD CONSTRAINT `fk_loyalty_points_transaction_points_account_id` FOREIGN KEY (`points_account_id`) REFERENCES `grocery_ecm`.`loyalty`.`points_account`(`points_account_id`);
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ADD CONSTRAINT `fk_loyalty_points_transaction_reward_offer_id` FOREIGN KEY (`reward_offer_id`) REFERENCES `grocery_ecm`.`loyalty`.`reward_offer`(`reward_offer_id`);
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ADD CONSTRAINT `fk_loyalty_loyalty_redemption_member_offer_id` FOREIGN KEY (`member_offer_id`) REFERENCES `grocery_ecm`.`loyalty`.`member_offer`(`member_offer_id`);
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ADD CONSTRAINT `fk_loyalty_loyalty_redemption_points_transaction_id` FOREIGN KEY (`points_transaction_id`) REFERENCES `grocery_ecm`.`loyalty`.`points_transaction`(`points_transaction_id`);
...
ALTER TABLE `grocery_ecm`.`loyalty`.`membership` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier` SET TAGS ('dbx_subdomain' = 'member_enrollment');
ALTER TABLE `grocery_ecm`.`loyalty`.`tier` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Tier ID');
ALTER TABLE `grocery_ecm`.`loyalty`.`tier` ALTER COLUMN `program_config_id` SET TAGS ('dbx_business_glossary_term' = 'Program Config Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier_history` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier_history` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier_history` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'New Tier Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`tier_history` ALTER COLUMN `transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Transaction Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_account` SET TAGS ('dbx_subdomain' = 'points_rewards');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_account` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_account` ALTER COLUMN `program_config_id` SET TAGS ('dbx_business_glossary_term' = 'Program Config Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` SET TAGS ('dbx_subdomain' = 'points_rewards');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `digital_coupon_id` SET TAGS ('dbx_business_glossary_term' = 'Digital Coupon Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `order_header_id` SET TAGS ('dbx_business_glossary_term' = 'Order Header Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `points_account_id` SET TAGS ('dbx_business_glossary_term' = 'Points Account Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `reward_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Reward Offer Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `trade_allowance_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Allowance Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`points_transaction` ALTER COLUMN `transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Payment Transaction Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` SET TAGS ('dbx_subdomain' = 'points_rewards');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `new_item_intro_id` SET TAGS ('dbx_business_glossary_term' = 'New Item Intro Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `price_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Price Rule Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `promo_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Offer Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `retail_price_id` SET TAGS ('dbx_business_glossary_term' = 'Retail Price Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `segment_id` SET TAGS ('dbx_business_glossary_term' = 'Segment Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `store_cluster_id` SET TAGS ('dbx_business_glossary_term' = 'Store Cluster Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supply Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `tender_type_id` SET TAGS ('dbx_business_glossary_term' = 'Tender Type Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`reward_offer` ALTER COLUMN `trade_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Trade Agreement Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`member_offer` SET TAGS ('dbx_subdomain' = 'points_rewards');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`member_offer` ALTER COLUMN `order_header_id` SET TAGS ('dbx_business_glossary_term' = 'Order Header Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`member_offer` ALTER COLUMN `promo_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Offer Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`member_offer` ALTER COLUMN `retail_price_id` SET TAGS ('dbx_business_glossary_term' = 'Retail Price Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` SET TAGS ('dbx_subdomain' = 'points_rewards');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `fiscal_period_id` SET TAGS ('dbx_business_glossary_term' = 'Fiscal Period Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `gift_card_id` SET TAGS ('dbx_business_glossary_term' = 'Gift Card Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `member_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Member Offer Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Order Id (Foreign Key)');
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `points_transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Points Transaction Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `promo_offer_id` SET TAGS ('dbx_business_glossary_term' = 'Promo Offer Id (Foreign Key)');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`loyalty_redemption` ALTER COLUMN `transaction_id` SET TAGS ('dbx_business_glossary_term' = 'Transaction ID');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`household_member` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`program_config` SET TAGS ('dbx_subdomain' = 'member_enrollment');
...
ALTER TABLE `grocery_ecm`.`loyalty`.`program_config` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
