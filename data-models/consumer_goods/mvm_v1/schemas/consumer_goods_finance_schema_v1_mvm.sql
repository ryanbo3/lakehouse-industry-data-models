-- Schema for Domain: finance | Business: Consumer Goods | Version: v1_mvm
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`cost_center` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Cost centers in SAP CO are always assigned to a company code. The existing company_code STRING column should be normalized to a proper FK referencing the company_code master table. Removing the denorm',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: In SAP CO, cost centers are assigned to profit centers for P&L reporting and segment accounting. The existing profit_center_code STRING column should be normalized to a proper FK referencing the profi',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`profit_center` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Profit centers in SAP CO are assigned to a company code. The existing company_code STRING column should be normalized to a proper FK referencing the company_code master table.',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`journal_entry` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Every journal entry in SAP FI is posted to a specific company code. The existing company_code STRING column should be normalized to a proper FK referencing the company_code master table.',
    `compliance_obligation_id` BIGINT COMMENT 'Foreign key linking to regulatory.compliance_obligation. Business justification: Finance posts accrual journal entries for compliance obligation costs (penalty reserves, fee accruals, audit cost provisions). Linking journal_entry to compliance_obligation enables compliance cost ac',
    ...
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: Journal entries are generated from sales invoices in SD-FI integration. Consumer goods companies require direct invoice traceability on GL postings for revenue audit, deduction management, and financi',
    `order_id` BIGINT COMMENT 'Foreign key linking to sales.order. Business justification: Journal entries reference originating sales orders for order-to-cash audit trails and SOX compliance. Consumer goods finance teams require direct order traceability on GL postings for revenue audit, d',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: In SAP FI, journal entry line items can carry their own company code (especially in intercompany scenarios where lines post to different entities). Normalizing the existing company_code STRING to a FK',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Journal entry line items carry cost center assignments for CO cost allocation. The existing cost_center_code STRING column should be normalized to a proper FK referencing the cost_center master table.',
    ...
    `material_id` BIGINT COMMENT 'Foreign key linking to product.material. Business justification: Journal entry lines for inventory movements, GR/IR, and COGS postings reference material masters in consumer goods ERP. journal_entry_line.material_number is a denormalized signal. This FK enables mat',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Journal entry line items carry profit center assignments for segment reporting. The existing profit_center_code STRING column should be normalized to a proper FK referencing the profit_center master t',
    `stock_movement_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_movement. Business justification: Every goods movement (GR, GI, transfer posting) in Consumer Goods generates journal entry lines for inventory valuation. Linking journal_entry_line to stock_movement enables inventory-to-GL reconcilia',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`ar_invoice` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: AR invoices are issued by a specific legal entity (company code). Normalizing the existing company_code STRING to a FK ensures referential integrity with the company_code master.',
    ...
    `order_id` BIGINT COMMENT 'Foreign key linking to sales.order. Business justification: AR invoices originate from sales orders; direct FK enables order-to-cash reconciliation and DSO analysis by order. Consumer goods AR teams trace deductions and disputes back to originating orders. sal',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`ar_payment` (
    ...
    `ar_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ar_invoice. Business justification: AR payments are applied against AR invoices — this is the core accounts receivable cash application relationship. ar_payment currently has invoice_id pointing to sales.invoice (cross-domain), but the ',
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to finance.bank_account. Business justification: AR payments are deposited into a specific bank account. The existing bank_account_code BIGINT column is already the correct data type and semantically maps to bank_account.bank_account_id. Normalizing',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: AR payments are received by a specific company code. Normalizing the existing company_code STRING to a FK ensures referential integrity with the company_code master.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: AR payments are attributed to cost centers for revenue and cash allocation in SAP CO. The existing cost_center_code STRING column should be normalized to a proper FK referencing the cost_center master',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: AP invoices are processed under a specific company code in SAP FI. Normalizing the existing company_code STRING to a FK ensures referential integrity with the company_code master.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: AP invoices are attributed to cost centers for expense allocation in SAP CO. The existing cost_center_code STRING column should be normalized to a proper FK referencing the cost_center master table.',
    ...
    `inbound_receipt_id` BIGINT COMMENT 'Foreign key linking to distribution.inbound_receipt. Business justification: AP invoices are matched against inbound receipts in the 3-way match process (PO + goods receipt + invoice). Consumer goods AP teams require this link to validate supplier invoices against DC receipts,',
    `po_line_id` BIGINT COMMENT 'Foreign key linking to procurement.po_line. Business justification: Line-level 3-way match in Consumer Goods AP requires matching AP invoice lines to specific PO lines. Current ap_invoice only links at PO header level. Line-level matching is standard for quantity/pric',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: AP invoices are attributed to profit centers for cost allocation and P&L reporting. The existing profit_center_code STRING column should be normalized to a proper FK referencing the profit_center mast',
    ...
    `supplier_invoice_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_invoice. Business justification: 3-way match reconciliation (PO/GR/Invoice) in Consumer Goods AP processing requires finance ap_invoice to reference the procurement-side supplier_invoice record. Enables payment reconciliation, disput',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`ap_payment` (
    ...
    `ap_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ap_invoice. Business justification: AP payments settle AP invoices — this is the core accounts payable payment application relationship. ap_payment currently has invoice_id pointing to sales.invoice (cross-domain, which appears semantic',
    `bank_account_id` BIGINT COMMENT 'Foreign key linking to finance.bank_account. Business justification: AP payments are disbursed from a specific bank account. The existing bank_account_code BIGINT column is already the correct data type and semantically maps to bank_account.bank_account_id. Normalizing',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: AP payments are executed from a specific company code. Normalizing the existing company_code STRING to a FK ensures referential integrity with the company_code master.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: AP payments are attributed to cost centers for cash flow and expense tracking in SAP CO. The existing cost_center_code STRING column should be normalized to a proper FK referencing the cost_center mas',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: AP payments are attributed to profit centers for cash flow and P&L reporting. The existing profit_center_code STRING column should be normalized to a proper FK referencing the profit_center master tab',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`cogs_allocation` (
    ...
    `hierarchy_id` BIGINT COMMENT 'Foreign key linking to product.hierarchy. Business justification: COGS allocation by product hierarchy supports category and division P&L reporting in consumer goods. This FK enables hierarchy-level COGS roll-up, category margin analysis, and IBP-to-finance reconcil',
    ...
    `invoice_id` BIGINT COMMENT 'Foreign key linking to sales.invoice. Business justification: COGS is recognized when revenue is recognized at invoice. Consumer goods gross margin reporting requires linking COGS allocations to sales invoices for product-level profitability analysis, trade prom',
    ...
    `standard_cost_id` BIGINT COMMENT 'Foreign key linking to finance.standard_cost. Business justification: COGS allocation records reference the standard cost estimate used as the basis for cost allocation. In CPG manufacturing, COGS allocations are derived from standard cost estimates (SAP CK11N). This in',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`standard_cost` (
    ...
    `approved_supplier_list_id` BIGINT COMMENT 'Foreign key linking to procurement.approved_supplier_list. Business justification: Standard cost setting in Consumer Goods uses ASL pricing (price_validity dates, quality ratings) as the basis for raw material cost estimates. Links the cost estimate to the specific approved supplier',
    ...
    `product_bom_id` BIGINT COMMENT 'Foreign key linking to product.product_bom. Business justification: Standard cost estimates in consumer goods are built from specific BOM versions. standard_cost.bom_usage (plain attr) signals this dependency. This FK enables cost roll-up traceability from standard co',
    `supplier_contract_id` BIGINT COMMENT 'Foreign key linking to procurement.supplier_contract. Business justification: In Consumer Goods, standard cost estimates for raw materials and packaging are set using contracted supplier prices. Cost accountants reference the active supplier contract during annual standard cost',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`finance_budget` (
    ...
    `category_id` BIGINT COMMENT 'Foreign key linking to product.category. Business justification: Consumer goods budget planning is structured by product category (e.g., Hair Care A&P budget, Skin Care trade spend). finance_budget.product_line_code is a denormalized signal. This FK enables categor',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Financial budgets are planned at the company code level in SAP CO. Normalizing the existing company_code STRING to a FK ensures referential integrity with the company_code master.',
    `compliance_obligation_id` BIGINT COMMENT 'Foreign key linking to regulatory.compliance_obligation. Business justification: Finance teams create dedicated budget lines to fund specific compliance obligations (REACH registration fees, FDA submission fees, GMP audit costs). Linking finance_budget to compliance_obligation ena',
    ...
    `distribution_facility_id` BIGINT COMMENT 'Foreign key linking to distribution.distribution_facility. Business justification: Distribution center budgets (labor, utilities, freight, maintenance) are planned at facility level. Consumer goods finance teams require this link to associate budget records with specific DCs for bud',
    `gl_account_id` BIGINT COMMENT 'Foreign key linking to finance.gl_account. Business justification: Financial budgets are planned at the GL account level (cost element budgeting in SAP CO). The existing gl_account_code STRING column should be normalized to a proper FK referencing the gl_account mast',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Financial budgets in SAP CO are planned at the profit center level for P&L reporting. The existing profit_center_code STRING column should be normalized to a proper FK referencing the profit_center ma',
    `spend_category_id` BIGINT COMMENT 'Foreign key linking to procurement.spend_category. Business justification: Procurement spend-vs-budget reporting in Consumer Goods requires aligning finance budget lines to procurement spend categories. Enables category managers to track actual spend against approved budget ',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` (
    ...
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Fixed assets in SAP AM are registered under a specific company code. Normalizing the existing company_code STRING to a FK ensures referential integrity with the company_code master.',
    ...
    `distribution_facility_id` BIGINT COMMENT 'Foreign key linking to distribution.distribution_facility. Business justification: DC buildings, racking, conveyors, and MHE are fixed assets assigned to distribution facilities. Consumer goods asset managers require this link for facility-level asset register reporting, depreciatio',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Fixed assets are assigned to profit centers for segment reporting and P&L allocation in SAP AM. The existing profit_center_code STRING column should be normalized to a proper FK referencing the profit',
    `warehouse_id` BIGINT COMMENT 'Foreign key linking to inventory.warehouse. Business justification: Consumer Goods warehouses contain fixed assets (racking, forklifts, refrigeration units). Linking fixed_asset to warehouse enables asset register reporting by warehouse, depreciation allocation to war',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Intercompany transactions are attributed to cost centers for CO cost allocation and intercompany reconciliation. The existing cost_center_code STRING column should be normalized to a proper FK referen',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Intercompany transactions are attributed to profit centers for segment elimination and consolidation reporting. The existing profit_center_code STRING column should be normalized to a proper FK refere',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Intercompany transactions originate from a sending legal entity (company code). Normalizing sending_company_code STRING to a FK with a meaningful prefix sending_ distinguishes it from the receiving ',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Intercompany transactions in consumer goods involve transfer pricing for specific SKUs between legal entities. intercompany_transaction.transfer_price and transfer_pricing_method require SKU-level ref',
    `stock_movement_id` BIGINT COMMENT 'Foreign key linking to inventory.stock_movement. Business justification: Consumer Goods intercompany stock transfers generate both a stock movement and an intercompany financial transaction. This link enables intercompany reconciliation, transfer pricing validation, and el',
...
CREATE OR REPLACE TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` (
    ...
    `ar_invoice_id` BIGINT COMMENT 'Foreign key linking to finance.ar_invoice. Business justification: Revenue recognition events are triggered by AR invoice billing under ASC 606. The revenue_recognition table already links to sales.invoice (cross-domain) but lacks the in-domain link to ar_invoice whi',
    `company_code_id` BIGINT COMMENT 'Foreign key linking to finance.company_code. Business justification: Revenue recognition events are scoped to a specific legal entity (company code) for ASC 606 compliance. Normalizing the existing company_code STRING to a FK ensures referential integrity.',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Revenue recognition events can be attributed to cost centers for detailed cost-of-sales accounting. The existing cost_center_code STRING column should be normalized to a proper FK referencing the cost',
    `distribution_shipment_id` BIGINT COMMENT 'Foreign key linking to distribution.distribution_shipment. Business justification: Under ASC 606/IFRS 15, revenue is recognized when control transfers — typically at shipment or delivery. Consumer goods finance teams require this link to tie revenue recognition events to specific di',
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Revenue recognition events are attributed to profit centers for segment revenue reporting under ASC 606. The existing profit_center_code STRING column should be normalized to a proper FK referencing t',
    `registration_id` BIGINT COMMENT 'Foreign key linking to regulatory.regulatory_registration. Business justification: Under ASC 606/IFRS 15, revenue for regulated consumer goods products cannot be recognized until the product holds valid market registration. Linking revenue_recognition to regulatory_registration enab',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: ASC 606/IFRS 15 revenue recognition requires performance obligation tracking at SKU level in consumer goods. revenue_recognition.sku (plain string) is a denormalized signal. This FK enables SKU-level ',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `consumer_goods_ecm`.`finance`.`cost_center` ADD CONSTRAINT `fk_finance_cost_center_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`cost_center` ADD CONSTRAINT `fk_finance_cost_center_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`profit_center` ADD CONSTRAINT `fk_finance_profit_center_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry` ADD CONSTRAINT `fk_finance_journal_entry_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ADD CONSTRAINT `fk_finance_journal_entry_line_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ADD CONSTRAINT `fk_finance_journal_entry_line_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ADD CONSTRAINT `fk_finance_journal_entry_line_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_invoice` ADD CONSTRAINT `fk_finance_ar_invoice_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ADD CONSTRAINT `fk_finance_ar_payment_ar_invoice_id` FOREIGN KEY (`ar_invoice_id`) REFERENCES `consumer_goods_ecm`.`finance`.`ar_invoice`(`ar_invoice_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ADD CONSTRAINT `fk_finance_ar_payment_bank_account_id` FOREIGN KEY (`bank_account_id`) REFERENCES `consumer_goods_ecm`.`finance`.`bank_account`(`bank_account_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ADD CONSTRAINT `fk_finance_ar_payment_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ADD CONSTRAINT `fk_finance_ar_payment_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ADD CONSTRAINT `fk_finance_ap_invoice_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ADD CONSTRAINT `fk_finance_ap_payment_ap_invoice_id` FOREIGN KEY (`ap_invoice_id`) REFERENCES `consumer_goods_ecm`.`finance`.`ap_invoice`(`ap_invoice_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ADD CONSTRAINT `fk_finance_ap_payment_bank_account_id` FOREIGN KEY (`bank_account_id`) REFERENCES `consumer_goods_ecm`.`finance`.`bank_account`(`bank_account_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ADD CONSTRAINT `fk_finance_ap_payment_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ADD CONSTRAINT `fk_finance_ap_payment_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ADD CONSTRAINT `fk_finance_ap_payment_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cogs_allocation` ADD CONSTRAINT `fk_finance_cogs_allocation_standard_cost_id` FOREIGN KEY (`standard_cost_id`) REFERENCES `consumer_goods_ecm`.`finance`.`standard_cost`(`standard_cost_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ADD CONSTRAINT `fk_finance_finance_budget_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ADD CONSTRAINT `fk_finance_finance_budget_gl_account_id` FOREIGN KEY (`gl_account_id`) REFERENCES `consumer_goods_ecm`.`finance`.`gl_account`(`gl_account_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ADD CONSTRAINT `fk_finance_finance_budget_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` ADD CONSTRAINT `fk_finance_fixed_asset_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` ADD CONSTRAINT `fk_finance_fixed_asset_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ADD CONSTRAINT `fk_finance_intercompany_transaction_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ADD CONSTRAINT `fk_finance_intercompany_transaction_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ADD CONSTRAINT `fk_finance_intercompany_transaction_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ADD CONSTRAINT `fk_finance_revenue_recognition_ar_invoice_id` FOREIGN KEY (`ar_invoice_id`) REFERENCES `consumer_goods_ecm`.`finance`.`ar_invoice`(`ar_invoice_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ADD CONSTRAINT `fk_finance_revenue_recognition_company_code_id` FOREIGN KEY (`company_code_id`) REFERENCES `consumer_goods_ecm`.`finance`.`company_code`(`company_code_id`);
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ADD CONSTRAINT `fk_finance_revenue_recognition_cost_center_id` FOREIGN KEY (`cost_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`cost_center`(`cost_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ADD CONSTRAINT `fk_finance_revenue_recognition_profit_center_id` FOREIGN KEY (`profit_center_id`) REFERENCES `consumer_goods_ecm`.`finance`.`profit_center`(`profit_center_id`);
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cost_center` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cost_center` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cost_center` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`profit_center` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`profit_center` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`gl_account` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ledger` SET TAGS ('dbx_subdomain' = 'accounting_structure');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry` SET TAGS ('dbx_subdomain' = 'ledger_transactions');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry` ALTER COLUMN `compliance_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Invoice Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Order Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` SET TAGS ('dbx_subdomain' = 'ledger_transactions');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `material_id` SET TAGS ('dbx_business_glossary_term' = 'Material Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`journal_entry_line` ALTER COLUMN `stock_movement_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Movement Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_invoice` SET TAGS ('dbx_subdomain' = 'receivables_payables');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_invoice` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_invoice` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Order Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` SET TAGS ('dbx_subdomain' = 'receivables_payables');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ALTER COLUMN `ar_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Invoice Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ar_payment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` SET TAGS ('dbx_subdomain' = 'receivables_payables');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ALTER COLUMN `inbound_receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Inbound Receipt Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ALTER COLUMN `po_line_id` SET TAGS ('dbx_business_glossary_term' = 'Po Line Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_invoice` ALTER COLUMN `supplier_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Invoice Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` SET TAGS ('dbx_subdomain' = 'receivables_payables');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `ap_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ap Invoice Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_business_glossary_term' = 'Bank Account Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`ap_payment` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cogs_allocation` SET TAGS ('dbx_subdomain' = 'cost_planning');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cogs_allocation` ALTER COLUMN `hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Hierarchy Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cogs_allocation` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Sales Invoice Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`cogs_allocation` ALTER COLUMN `standard_cost_id` SET TAGS ('dbx_business_glossary_term' = 'Standard Cost Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`standard_cost` SET TAGS ('dbx_subdomain' = 'cost_planning');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`standard_cost` ALTER COLUMN `approved_supplier_list_id` SET TAGS ('dbx_business_glossary_term' = 'Approved Supplier List Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`standard_cost` ALTER COLUMN `product_bom_id` SET TAGS ('dbx_business_glossary_term' = 'Product Bom Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`standard_cost` ALTER COLUMN `supplier_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Contract Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` SET TAGS ('dbx_subdomain' = 'cost_planning');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Category Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `compliance_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Obligation Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `distribution_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Facility Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `gl_account_id` SET TAGS ('dbx_business_glossary_term' = 'Gl Account Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`finance_budget` ALTER COLUMN `spend_category_id` SET TAGS ('dbx_business_glossary_term' = 'Spend Category Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` SET TAGS ('dbx_subdomain' = 'cost_planning');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` ALTER COLUMN `distribution_facility_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Facility Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`fixed_asset` ALTER COLUMN `warehouse_id` SET TAGS ('dbx_business_glossary_term' = 'Warehouse Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` SET TAGS ('dbx_subdomain' = 'ledger_transactions');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Sending Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`intercompany_transaction` ALTER COLUMN `stock_movement_id` SET TAGS ('dbx_business_glossary_term' = 'Stock Movement Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` SET TAGS ('dbx_subdomain' = 'ledger_transactions');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `ar_invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Ar Invoice Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `company_code_id` SET TAGS ('dbx_business_glossary_term' = 'Company Code Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `distribution_shipment_id` SET TAGS ('dbx_business_glossary_term' = 'Distribution Shipment Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `registration_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Registration Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`revenue_recognition` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` SET TAGS ('dbx_subdomain' = 'ledger_transactions');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `master_bank_account_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `master_bank_account_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `master_bank_account_id` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `bank_account_status` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `consumer_goods_ecm`.`finance`.`bank_account` ALTER COLUMN `bank_account_status` SET TAGS ('dbx_pii_financial' = 'true');
...
ALTER TABLE `consumer_goods_ecm`.`finance`.`company_code` SET TAGS ('dbx_subdomain' = 'accounting_structure');
