-- Schema for Domain: service | Business: Legal | Version: v1_ecm
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`legal_service` (
    ...
    `legal_practice_area_id` BIGINT COMMENT 'Foreign key linking to service.practice_area. Business justification: Each legal service belongs to a practice area. Normalize by linking to practice_area master taxonomy. The practice_area STRING column is a code/name reference.',
    ...
    `legal_pricing_model_id` BIGINT COMMENT 'Foreign key linking to service.pricing_model. Business justification: Legal services reference a pricing model. Normalize by linking to pricing_model master. The pricing_model STRING column is a code/name reference.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`pricing_model` (
    ...
    `last_modified_by_user_timekeeper_id` BIGINT COMMENT 'Identifier of the user who last modified this pricing model record.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`rate_card` (
    ...
    `rate_practice_area_id` BIGINT COMMENT 'Foreign key linking to service.practice_area. Business justification: Rate cards are scoped by practice area. Normalize by linking to practice_area master. Remove practice_area_name as it can be retrieved via JOIN to practice_area.name.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`package` (
    ...
    `last_modified_by_user_timekeeper_id` BIGINT COMMENT 'Reference to the user who most recently modified this service package record.',
    `package_approved_by_partner_timekeeper_id` BIGINT COMMENT 'Reference to the partner who provided final approval for this service package to be offered to clients.',
    `package_timekeeper_id` BIGINT COMMENT 'Reference to the partner responsible for package design, pricing approval, and client relationship ownership.',
    ...
    `superseded_by_package_id` BIGINT COMMENT 'Reference to the newer service package that replaces this one, enabling version lineage tracking and client migration paths.',
    `timekeeper_id` BIGINT COMMENT 'Reference to the user who created this service package record in the system.',
    `upsell_package_id` BIGINT COMMENT 'Reference to a higher-tier service package that this package can be upgraded to, enabling structured upsell pathways.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`jurisdiction_coverage` (
    ...
    `jurisdiction_legal_service_id` BIGINT COMMENT 'Reference to the legal service offering for which this jurisdiction coverage applies.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`offering_version` (
    ...
    `created_by_user_timekeeper_id` BIGINT COMMENT 'Reference to the user who created this service offering version record. Links to workforce/user master data for audit purposes.',
    `last_modified_by_user_timekeeper_id` BIGINT COMMENT 'Reference to the user who last modified this service offering version record. Links to workforce/user master data for audit purposes.',
    ...
    `superseded_version_offering_version_id` BIGINT COMMENT 'Reference to the previous service offering version that this version replaces. Nullable for initial versions. Enables version lineage tracking.',
    `timekeeper_id` BIGINT COMMENT 'Reference to the partner or senior lawyer who approved this service offering version for use. Links to workforce/partner master data.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`matter_service_mapping` (
    ...
    `last_modified_by_user_timekeeper_id` BIGINT COMMENT 'Identifier of the user who last modified this service mapping record.',
    ...
    `matter_legal_service_id` BIGINT COMMENT 'Identifier of the legal service offering (e.g., M&A advisory, litigation, IP counsel) under which work is delivered for this matter.',
    `matter_pricing_model_id` BIGINT COMMENT 'Foreign key linking to service.pricing_model. Business justification: Matter service mappings reference a pricing model. Normalize by linking to pricing_model master. The pricing_model STRING column is a code/name reference.',
    `matter_sla_template_id` BIGINT COMMENT 'Identifier of the SLA template defining performance commitments, response times, and deliverable timelines for this service.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`scope_amendment` (
    ...
    `modified_by_user_timekeeper_id` BIGINT COMMENT 'Reference to the user who last modified this amendment record.',
    ...
    `superseded_by_amendment_scope_amendment_id` BIGINT COMMENT 'Reference to a subsequent amendment that supersedes or replaces this amendment. Null if this is the current active amendment.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`eligibility_rule` (
    ...
    `last_modified_by_timekeeper_id` BIGINT COMMENT 'Reference to the user (timekeeper, business development manager, or system administrator) who most recently modified this eligibility rule record.',
    ...
    `timekeeper_id` BIGINT COMMENT 'Reference to the user (timekeeper, business development manager, or system administrator) who created this eligibility rule record.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`lpm_template` (
    ...
    `lpm_legal_service_id` BIGINT COMMENT 'Reference to the legal service offering (e.g., M&A advisory, litigation, IP counsel) that this LPM template is designed for.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`delivery_configuration` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`service`.`office` (
    `office_id` BIGINT COMMENT 'Primary key for office',
    `billing_office_id` BIGINT COMMENT 'Identifier of the legal entity used for billing and invoicing clients served by this office.',
    `employee_id` BIGINT COMMENT 'Identifier of the partner responsible for managing the office operations and client relationships.',
    `parent_office_id` BIGINT COMMENT 'Identifier of the parent or regional office to which this office reports, enabling hierarchical office structures.',
    `accreditation_status` STRING COMMENT 'Professional accreditations or certifications held by the office, such as ISO certifications or industry-specific quality standards.',
    ...
    `address_line_2` STRING COMMENT 'Secondary address line for suite, floor, or unit information.',
    `attorney_headcount` STRING COMMENT 'Total number of licensed attorneys currently employed at this office.',
    `bar_association_code` STRING COMMENT 'Identifier or registration number with the local bar association or law society governing the offices legal practice.',
    `city` STRING COMMENT 'City or municipality where the office is located.',
    `client_service_hours` STRING COMMENT 'Standard business hours during which the office is available for client meetings and consultations.',
    `closure_date` DATE COMMENT 'Date when the office ceased operations, applicable only for closed or merged offices.',
    `cost_center_code` STRING COMMENT 'Financial accounting code used to track expenses and allocate costs for this office in the firms general ledger.',
    `country_code` STRING COMMENT 'Three-letter ISO 3166-1 alpha-3 country code identifying the country where the office is located.',
    `created_timestamp` TIMESTAMP COMMENT 'Timestamp when this office record was first created in the system.',
    `email_address` STRING COMMENT 'General email address for the office, used for client communications and business correspondence.',
    `fax_number` STRING COMMENT 'Fax number for the office, used for document transmission and court filings where required.',
    `is_conflict_check_required` BOOLEAN COMMENT 'Indicates whether this office requires independent conflict-of-interest checks due to ethical walls or separate practice groups.',
    `jurisdiction_code` STRING COMMENT 'Legal jurisdiction code or identifier for the primary regulatory authority governing this offices legal practice.',
    `languages_supported` STRING COMMENT 'Comma-separated list of languages in which the office can provide legal services, supporting multilingual client needs.',
    `modified_timestamp` TIMESTAMP COMMENT 'Timestamp when this office record was last modified in the system.',
    `notes` STRING COMMENT 'Free-text field for additional information, special considerations, or operational notes about the office.',
    `office_code` STRING COMMENT 'Externally-known unique business identifier for the office, used in client communications, billing systems, and matter assignments.',
    `office_name` STRING COMMENT 'Full legal name of the office location, used in official correspondence and client-facing materials.',
    `office_size_sqft` DECIMAL(18,2) COMMENT 'Total physical space of the office measured in square feet, used for facilities management and cost allocation.',
    `office_type` STRING COMMENT 'Classification of the office based on its operational role and scope within the firms organizational structure.',
    `opening_date` DATE COMMENT 'Date when the office commenced operations and began serving clients.',
    `operational_status` STRING COMMENT 'Current operational state of the office in its lifecycle, indicating whether it is actively serving clients or undergoing transition.',
    `phone_number` STRING COMMENT 'Primary contact telephone number for the office, used for client inquiries and business communications.',
    `postal_code` STRING COMMENT 'Postal or ZIP code for the office address.',
    `practice_area_focus` STRING COMMENT 'Primary practice areas or legal specializations offered by this office, such as corporate law, litigation, intellectual property, employment law, or regulatory compliance.',
    `profit_center_code` STRING COMMENT 'Financial accounting code used to track revenue and profitability for this office in management reporting.',
    `security_clearance_level` STRING COMMENT 'Highest government security clearance level held by the office, relevant for offices handling classified government matters.',
    `state_province` STRING COMMENT 'State, province, or administrative region where the office is located.',
    `support_staff_headcount` STRING COMMENT 'Total number of non-attorney support staff (paralegals, legal assistants, administrative staff) employed at this office.',
    `time_zone` STRING COMMENT 'IANA time zone identifier for the office location, used for scheduling and deadline management across jurisdictions.',
    `website_url` STRING COMMENT 'Web address for the office-specific website or landing page, if applicable.',
...
ALTER TABLE `legal_ecm`.`service`.`legal_service` ADD CONSTRAINT `fk_service_legal_service_legal_practice_area_id` FOREIGN KEY (`legal_practice_area_id`) REFERENCES `legal_ecm`.`service`.`practice_area`(`practice_area_id`);
...
ALTER TABLE `legal_ecm`.`service`.`sla_template` ADD CONSTRAINT `fk_service_sla_template_offering_version_id` FOREIGN KEY (`offering_version_id`) REFERENCES `legal_ecm`.`service`.`offering_version`(`offering_version_id`);
...
ALTER TABLE `legal_ecm`.`service`.`package` ADD CONSTRAINT `fk_service_package_superseded_by_package_id` FOREIGN KEY (`superseded_by_package_id`) REFERENCES `legal_ecm`.`service`.`package`(`package_id`);
ALTER TABLE `legal_ecm`.`service`.`package` ADD CONSTRAINT `fk_service_package_upsell_package_id` FOREIGN KEY (`upsell_package_id`) REFERENCES `legal_ecm`.`service`.`package`(`package_id`);
ALTER TABLE `legal_ecm`.`service`.`jurisdiction_coverage` ADD CONSTRAINT `fk_service_jurisdiction_coverage_jurisdiction_legal_service_id` FOREIGN KEY (`jurisdiction_legal_service_id`) REFERENCES `legal_ecm`.`service`.`legal_service`(`legal_service_id`);
...
ALTER TABLE `legal_ecm`.`service`.`offering_version` ADD CONSTRAINT `fk_service_offering_version_legal_service_id` FOREIGN KEY (`legal_service_id`) REFERENCES `legal_ecm`.`service`.`legal_service`(`legal_service_id`);
ALTER TABLE `legal_ecm`.`service`.`offering_version` ADD CONSTRAINT `fk_service_offering_version_practice_area_id` FOREIGN KEY (`practice_area_id`) REFERENCES `legal_ecm`.`service`.`practice_area`(`practice_area_id`);
ALTER TABLE `legal_ecm`.`service`.`offering_version` ADD CONSTRAINT `fk_service_offering_version_pricing_model_id` FOREIGN KEY (`pricing_model_id`) REFERENCES `legal_ecm`.`service`.`pricing_model`(`pricing_model_id`);
ALTER TABLE `legal_ecm`.`service`.`offering_version` ADD CONSTRAINT `fk_service_offering_version_superseded_version_offering_version_id` FOREIGN KEY (`superseded_version_offering_version_id`) REFERENCES `legal_ecm`.`service`.`offering_version`(`offering_version_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_legal_service_id` FOREIGN KEY (`legal_service_id`) REFERENCES `legal_ecm`.`service`.`legal_service`(`legal_service_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_matter_legal_service_id` FOREIGN KEY (`matter_legal_service_id`) REFERENCES `legal_ecm`.`service`.`legal_service`(`legal_service_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_matter_pricing_model_id` FOREIGN KEY (`matter_pricing_model_id`) REFERENCES `legal_ecm`.`service`.`pricing_model`(`pricing_model_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_matter_sla_template_id` FOREIGN KEY (`matter_sla_template_id`) REFERENCES `legal_ecm`.`service`.`sla_template`(`sla_template_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_package_id` FOREIGN KEY (`package_id`) REFERENCES `legal_ecm`.`service`.`package`(`package_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_practice_area_id` FOREIGN KEY (`practice_area_id`) REFERENCES `legal_ecm`.`service`.`practice_area`(`practice_area_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_pricing_model_id` FOREIGN KEY (`pricing_model_id`) REFERENCES `legal_ecm`.`service`.`pricing_model`(`pricing_model_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_sla_template_id` FOREIGN KEY (`sla_template_id`) REFERENCES `legal_ecm`.`service`.`sla_template`(`sla_template_id`);
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ADD CONSTRAINT `fk_service_matter_service_mapping_tier_id` FOREIGN KEY (`tier_id`) REFERENCES `legal_ecm`.`service`.`tier`(`tier_id`);
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ADD CONSTRAINT `fk_service_scope_amendment_superseded_by_amendment_scope_amendment_id` FOREIGN KEY (`superseded_by_amendment_scope_amendment_id`) REFERENCES `legal_ecm`.`service`.`scope_amendment`(`scope_amendment_id`);
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ADD CONSTRAINT `fk_service_lpm_template_lpm_legal_service_id` FOREIGN KEY (`lpm_legal_service_id`) REFERENCES `legal_ecm`.`service`.`legal_service`(`legal_service_id`);
...
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ADD CONSTRAINT `fk_service_delivery_configuration_delivery_model_id` FOREIGN KEY (`delivery_model_id`) REFERENCES `legal_ecm`.`service`.`delivery_model`(`delivery_model_id`);
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ADD CONSTRAINT `fk_service_delivery_configuration_legal_service_id` FOREIGN KEY (`legal_service_id`) REFERENCES `legal_ecm`.`service`.`legal_service`(`legal_service_id`);
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ADD CONSTRAINT `fk_service_practice_area_delivery_configuration_delivery_configuration_id` FOREIGN KEY (`delivery_configuration_id`) REFERENCES `legal_ecm`.`service`.`delivery_configuration`(`delivery_configuration_id`);
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ADD CONSTRAINT `fk_service_practice_area_delivery_configuration_delivery_model_id` FOREIGN KEY (`delivery_model_id`) REFERENCES `legal_ecm`.`service`.`delivery_model`(`delivery_model_id`);
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ADD CONSTRAINT `fk_service_practice_area_delivery_configuration_practice_area_id` FOREIGN KEY (`practice_area_id`) REFERENCES `legal_ecm`.`service`.`practice_area`(`practice_area_id`);
ALTER TABLE `legal_ecm`.`service`.`office` ADD CONSTRAINT `fk_service_office_parent_office_id` FOREIGN KEY (`parent_office_id`) REFERENCES `legal_ecm`.`service`.`office`(`office_id`);
...
ALTER TABLE `legal_ecm`.`service`.`practice_area` SET TAGS ('dbx_subdomain' = 'offering_catalog');
...
ALTER TABLE `legal_ecm`.`service`.`practice_area` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`service`.`legal_service` SET TAGS ('dbx_subdomain' = 'offering_catalog');
...
ALTER TABLE `legal_ecm`.`service`.`legal_service` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Owner Partner Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`legal_service` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`legal_service` ALTER COLUMN `legal_practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`service`.`legal_service` ALTER COLUMN `eligible_client_segments` SET TAGS ('dbx_business_glossary_term' = 'Eligible Client Segments');
...
ALTER TABLE `legal_ecm`.`service`.`legal_service` ALTER COLUMN `legal_pricing_model_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Model Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`service`.`tier` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`tier` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Service Tier Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`tier` ALTER COLUMN `appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Threshold Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`service`.`pricing_model` SET TAGS ('dbx_subdomain' = 'commercial_terms');
...
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Partner Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `legal_ecm`.`service`.`pricing_model` ALTER COLUMN `applicable_practice_areas` SET TAGS ('dbx_business_glossary_term' = 'Applicable Practice Areas');
...
ALTER TABLE `legal_ecm`.`service`.`rate_card` SET TAGS ('dbx_subdomain' = 'commercial_terms');
...
ALTER TABLE `legal_ecm`.`service`.`rate_card` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`rate_card` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`rate_card` ALTER COLUMN `rate_practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`service`.`rate_card` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Timekeeper Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`service`.`sla_template` SET TAGS ('dbx_subdomain' = 'commercial_terms');
...
ALTER TABLE `legal_ecm`.`service`.`sla_template` ALTER COLUMN `offering_version_id` SET TAGS ('dbx_business_glossary_term' = 'Service Offering ID');
...
ALTER TABLE `legal_ecm`.`service`.`sla_template` ALTER COLUMN `client_tier` SET TAGS ('dbx_business_glossary_term' = 'Client Tier');
ALTER TABLE `legal_ecm`.`service`.`sla_template` ALTER COLUMN `client_tier` SET TAGS ('dbx_value_regex' = 'platinum|gold|silver|bronze|standard');
...
ALTER TABLE `legal_ecm`.`service`.`package` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `package_id` SET TAGS ('dbx_business_glossary_term' = 'Service Package ID');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Threshold Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `package_approved_by_partner_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Partner ID');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `package_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Lead Partner ID');
...
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `superseded_by_package_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded By Package ID');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `upsell_package_id` SET TAGS ('dbx_business_glossary_term' = 'Upsell Package ID');
...
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `sla_resolution_time_days` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Resolution Time (Days)');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `sla_response_time_hours` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Response Time (Hours)');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `target_client_segment` SET TAGS ('dbx_business_glossary_term' = 'Target Client Segment');
ALTER TABLE `legal_ecm`.`service`.`package` ALTER COLUMN `target_client_segment` SET TAGS ('dbx_value_regex' = 'enterprise|mid_market|startup|government|non_profit|individual');
...
ALTER TABLE `legal_ecm`.`service`.`jurisdiction_coverage` SET TAGS ('dbx_subdomain' = 'offering_catalog');
...
ALTER TABLE `legal_ecm`.`service`.`jurisdiction_coverage` ALTER COLUMN `jurisdiction_legal_service_id` SET TAGS ('dbx_business_glossary_term' = 'Service ID');
...
ALTER TABLE `legal_ecm`.`service`.`jurisdiction_coverage` ALTER COLUMN `practice_area_restrictions` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Restrictions');
...
ALTER TABLE `legal_ecm`.`service`.`offering_version` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`service`.`offering_version` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `offering_version_id` SET TAGS ('dbx_business_glossary_term' = 'Service Offering Version Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `created_by_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `created_by_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `created_by_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `legal_service_id` SET TAGS ('dbx_business_glossary_term' = 'Service Offering Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `pricing_model_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Model Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `superseded_version_offering_version_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded Version Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Partner Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `active_matters_count` SET TAGS ('dbx_business_glossary_term' = 'Active Matters Count');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `approval_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Approval Required Flag');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `archive_flag` SET TAGS ('dbx_business_glossary_term' = 'Archive Flag');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `change_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Change Reason Code');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `change_reason_description` SET TAGS ('dbx_business_glossary_term' = 'Change Reason Description');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `deliverables_list` SET TAGS ('dbx_business_glossary_term' = 'Deliverables List');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `engagement_letter_template_reference` SET TAGS ('dbx_business_glossary_term' = 'Engagement Letter Template Reference');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `estimated_hours_max` SET TAGS ('dbx_business_glossary_term' = 'Estimated Hours Maximum');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `estimated_hours_min` SET TAGS ('dbx_business_glossary_term' = 'Estimated Hours Minimum');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `exclusions_description` SET TAGS ('dbx_business_glossary_term' = 'Exclusions Description');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `jurisdiction_scope` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Scope');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `matter_type_applicability` SET TAGS ('dbx_business_glossary_term' = 'Matter Type Applicability');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `prerequisites_description` SET TAGS ('dbx_business_glossary_term' = 'Prerequisites Description');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `pricing_model` SET TAGS ('dbx_business_glossary_term' = 'Pricing Model');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `rate_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Rate Currency Code');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `rate_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `regulatory_compliance_notes` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Compliance Notes');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `rfp_template_reference` SET TAGS ('dbx_business_glossary_term' = 'Request for Proposal (RFP) Template Reference');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `service_scope_description` SET TAGS ('dbx_business_glossary_term' = 'Service Scope Description');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `service_type` SET TAGS ('dbx_business_glossary_term' = 'Service Type');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `sla_delivery_time_days` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Delivery Time Days');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `sla_response_time_hours` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Response Time Hours');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `sla_terms_description` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Terms Description');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `standard_rate_amount` SET TAGS ('dbx_business_glossary_term' = 'Standard Rate Amount');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `standard_rate_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `target_client_segment` SET TAGS ('dbx_business_glossary_term' = 'Target Client Segment');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `version_name` SET TAGS ('dbx_business_glossary_term' = 'Version Name');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `version_number` SET TAGS ('dbx_value_regex' = '^[0-9]+.[0-9]+(.[0-9]+)?$');
ALTER TABLE `legal_ecm`.`service`.`offering_version` ALTER COLUMN `version_status` SET TAGS ('dbx_business_glossary_term' = 'Version Status');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` SET TAGS ('dbx_subdomain' = 'engagement_execution');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `matter_service_mapping_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Service Mapping ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Conflict Clearance ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `guideline_id` SET TAGS ('dbx_business_glossary_term' = 'Billing Guideline ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `last_modified_by_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `letter_of_engagement_id` SET TAGS ('dbx_business_glossary_term' = 'Engagement Letter ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `matter_legal_service_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Service ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `matter_pricing_model_id` SET TAGS ('dbx_business_glossary_term' = 'Pricing Model Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `matter_sla_template_id` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Template ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `package_id` SET TAGS ('dbx_business_glossary_term' = 'Service Package ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_internal' = 'true');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `primary_matter_approved_by_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Timekeeper ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `tertiary_matter_lead_associate_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Lead Associate ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `tier_id` SET TAGS ('dbx_business_glossary_term' = 'Service Tier Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `amendment_count` SET TAGS ('dbx_business_glossary_term' = 'Amendment Count');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'draft|pending_approval|approved|rejected|suspended|terminated');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `billing_frequency` SET TAGS ('dbx_business_glossary_term' = 'Billing Frequency');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `billing_frequency` SET TAGS ('dbx_value_regex' = 'monthly|quarterly|milestone|on_completion|ad_hoc');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `budget_amount` SET TAGS ('dbx_business_glossary_term' = 'Budget Amount');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `budget_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Budget Currency Code');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `budget_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Effective End Date');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Start Date');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `estimated_hours` SET TAGS ('dbx_business_glossary_term' = 'Estimated Hours');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `is_primary_service` SET TAGS ('dbx_business_glossary_term' = 'Is Primary Service');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `last_amendment_date` SET TAGS ('dbx_business_glossary_term' = 'Last Amendment Date');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `pricing_model` SET TAGS ('dbx_business_glossary_term' = 'Pricing Model');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `pricing_model` SET TAGS ('dbx_value_regex' = 'hourly|fixed_fee|contingency|afa_capped|afa_blended|retainer');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `rfp_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Request for Proposal (RFP) Reference Number');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `service_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Service Completion Date');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `service_scope_description` SET TAGS ('dbx_business_glossary_term' = 'Service Scope Description');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `service_start_date` SET TAGS ('dbx_business_glossary_term' = 'Service Start Date');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `service_tier` SET TAGS ('dbx_business_glossary_term' = 'Service Tier');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `service_tier` SET TAGS ('dbx_value_regex' = 'standard|premium|enterprise|custom');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `utbms_activity_code` SET TAGS ('dbx_business_glossary_term' = 'Uniform Task-Based Management System (UTBMS) Activity Code');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `utbms_activity_code` SET TAGS ('dbx_value_regex' = '^[A-Z][0-9]{3}$');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `utbms_task_code` SET TAGS ('dbx_business_glossary_term' = 'Uniform Task-Based Management System (UTBMS) Task Code');
ALTER TABLE `legal_ecm`.`service`.`matter_service_mapping` ALTER COLUMN `utbms_task_code` SET TAGS ('dbx_value_regex' = '^[A-Z][0-9]{3}$');
ALTER TABLE `legal_ecm`.`service`.`delivery_model` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `legal_ecm`.`service`.`delivery_model` SET TAGS ('dbx_subdomain' = 'offering_catalog');
...
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Owner Partner ID');
...
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `applicable_practice_areas` SET TAGS ('dbx_business_glossary_term' = 'Applicable Practice Areas');
...
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `secondary_delivery_geography` SET TAGS ('dbx_business_glossary_term' = 'Secondary Delivery Geography');
ALTER TABLE `legal_ecm`.`service`.`delivery_model` ALTER COLUMN `secondary_delivery_geography` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
...
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` SET TAGS ('dbx_subdomain' = 'engagement_execution');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `scope_amendment_id` SET TAGS ('dbx_business_glossary_term' = 'Service Scope Amendment ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By Partner ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `client_engagement_scope_id` SET TAGS ('dbx_business_glossary_term' = 'Engagement Scope ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `modified_by_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `modified_by_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `modified_by_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `client_contact_id` SET TAGS ('dbx_business_glossary_term' = 'Requested By Contact ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `superseded_by_amendment_scope_amendment_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded By Amendment ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_description` SET TAGS ('dbx_business_glossary_term' = 'Amendment Description');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_document_reference` SET TAGS ('dbx_business_glossary_term' = 'Amendment Document Reference');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_number` SET TAGS ('dbx_business_glossary_term' = 'Amendment Number');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_reason_code` SET TAGS ('dbx_business_glossary_term' = 'Amendment Reason Code');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_reason_code` SET TAGS ('dbx_value_regex' = 'scope_creep|client_request|regulatory_change|cost_overrun|timeline_extension|service_upgrade');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_status` SET TAGS ('dbx_business_glossary_term' = 'Amendment Status');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_status` SET TAGS ('dbx_value_regex' = 'draft|pending_approval|approved|rejected|executed|superseded');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_type` SET TAGS ('dbx_business_glossary_term' = 'Amendment Type');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `amendment_type` SET TAGS ('dbx_value_regex' = 'scope_expansion|scope_reduction|fee_renegotiation|sla_modification|afa_restructuring|service_addition');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `billing_impact_notes` SET TAGS ('dbx_business_glossary_term' = 'Billing Impact Notes');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `client_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Client Approval Date');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `execution_date` SET TAGS ('dbx_business_glossary_term' = 'Execution Date');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `expiry_date` SET TAGS ('dbx_business_glossary_term' = 'Expiry Date');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `fee_adjustment_amount` SET TAGS ('dbx_business_glossary_term' = 'Fee Adjustment Amount');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `fee_adjustment_currency` SET TAGS ('dbx_business_glossary_term' = 'Fee Adjustment Currency');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `fee_adjustment_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `fee_impact_type` SET TAGS ('dbx_business_glossary_term' = 'Fee Impact Type');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `fee_impact_type` SET TAGS ('dbx_value_regex' = 'increase|decrease|no_change|restructure');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `indemnity_exposure_impact` SET TAGS ('dbx_business_glossary_term' = 'Professional Indemnity Exposure Impact');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `indemnity_exposure_impact` SET TAGS ('dbx_value_regex' = 'increased|decreased|unchanged|under_review');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `original_scope_reference` SET TAGS ('dbx_business_glossary_term' = 'Original Scope Reference');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `request_date` SET TAGS ('dbx_business_glossary_term' = 'Request Date');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `requested_by_party` SET TAGS ('dbx_business_glossary_term' = 'Requested By Party');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `requested_by_party` SET TAGS ('dbx_value_regex' = 'client|firm|mutual');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `revised_fee_arrangement_type` SET TAGS ('dbx_business_glossary_term' = 'Revised Fee Arrangement Type');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `revised_sla_terms` SET TAGS ('dbx_business_glossary_term' = 'Revised Service Level Agreement (SLA) Terms');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `risk_assessment_notes` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Notes');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `risk_assessment_notes` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `scope_change_summary` SET TAGS ('dbx_business_glossary_term' = 'Scope Change Summary');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `sla_change_indicator` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Change Indicator');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `wip_adjustment_amount` SET TAGS ('dbx_business_glossary_term' = 'Work in Progress (WIP) Adjustment Amount');
ALTER TABLE `legal_ecm`.`service`.`scope_amendment` ALTER COLUMN `wip_impact_assessment` SET TAGS ('dbx_business_glossary_term' = 'Work in Progress (WIP) Impact Assessment');
ALTER TABLE `legal_ecm`.`service`.`eligibility_rule` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`service`.`eligibility_rule` SET TAGS ('dbx_subdomain' = 'commercial_terms');
...
ALTER TABLE `legal_ecm`.`service`.`eligibility_rule` ALTER COLUMN `last_modified_by_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`service`.`eligibility_rule` ALTER COLUMN `primary_eligibility_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Rule Owner Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`eligibility_rule` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`service`.`eligibility_rule` ALTER COLUMN `practice_area_filter` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Filter');
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` SET TAGS ('dbx_subdomain' = 'engagement_execution');
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Owner Partner ID');
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `knowledge_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Knowledge Asset Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `lpm_legal_service_id` SET TAGS ('dbx_business_glossary_term' = 'Service ID');
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `document_template_references` SET TAGS ('dbx_business_glossary_term' = 'Document Template References');
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `fee_arrangement_type` SET TAGS ('dbx_business_glossary_term' = 'Alternative Fee Arrangement (AFA) Type');
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `fee_arrangement_type` SET TAGS ('dbx_value_regex' = 'Hourly|Fixed Fee|Contingency|Capped Fee|Blended Rate|Success Fee');
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `practice_group_code` SET TAGS ('dbx_business_glossary_term' = 'Practice Group Code');
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `practice_group_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9_]{2,10}$');
...
ALTER TABLE `legal_ecm`.`service`.`lpm_template` ALTER COLUMN `sla_target_days` SET TAGS ('dbx_business_glossary_term' = 'Service Level Agreement (SLA) Target (Days)');
...
ALTER TABLE `legal_ecm`.`service`.`line` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`line` ALTER COLUMN `line_id` SET TAGS ('dbx_business_glossary_term' = 'Service Line Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`line` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Identifier (ID)');
ALTER TABLE `legal_ecm`.`service`.`line` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`line` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `legal_ecm`.`service`.`line` ALTER COLUMN `practice_area_taxonomy` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Taxonomy');
...
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` SET TAGS ('dbx_association_edges' = 'service.legal_service,service.delivery_model');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `delivery_configuration_id` SET TAGS ('dbx_business_glossary_term' = 'Service Delivery Configuration ID');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `delivery_model_id` SET TAGS ('dbx_business_glossary_term' = 'Service Delivery Configuration - Delivery Model Id');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `legal_service_id` SET TAGS ('dbx_business_glossary_term' = 'Service Delivery Configuration - Legal Service Id');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `configuration_status` SET TAGS ('dbx_business_glossary_term' = 'Configuration Status');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `cost_allocation_percentage` SET TAGS ('dbx_business_glossary_term' = 'Cost Allocation Percentage');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `created_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Created Date');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Effective End Date');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Effective Start Date');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `last_modified_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Last Modified Date');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `preferred_flag` SET TAGS ('dbx_business_glossary_term' = 'Preferred Delivery Configuration Flag');
ALTER TABLE `legal_ecm`.`service`.`delivery_configuration` ALTER COLUMN `sla_override` SET TAGS ('dbx_business_glossary_term' = 'SLA Override Configuration');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` SET TAGS ('dbx_association_edges' = 'service.practice_area,service.delivery_model');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `practice_area_delivery_configuration_id` SET TAGS ('dbx_business_glossary_term' = 'practice_area_delivery_configuration Identifier');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `delivery_configuration_id` SET TAGS ('dbx_business_glossary_term' = 'Configuration Identifier');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `delivery_model_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Delivery Configuration - Delivery Model Id');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Delivery Configuration - Practice Area Id');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `capacity_allocation_fte` SET TAGS ('dbx_business_glossary_term' = 'Capacity Allocation in FTE');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `client_demand_rating` SET TAGS ('dbx_business_glossary_term' = 'Client Demand Rating');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `configuration_status` SET TAGS ('dbx_business_glossary_term' = 'Configuration Status');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `cost_efficiency_score` SET TAGS ('dbx_business_glossary_term' = 'Cost Efficiency Score');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Configuration Created Timestamp');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `effective_end_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Effective End Date');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `effective_start_date` SET TAGS ('dbx_business_glossary_term' = 'Configuration Effective Start Date');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Configuration Last Updated Timestamp');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Configuration Priority Level');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `quality_rating` SET TAGS ('dbx_business_glossary_term' = 'Quality Rating');
ALTER TABLE `legal_ecm`.`service`.`practice_area_delivery_configuration` ALTER COLUMN `utilization_target_percentage` SET TAGS ('dbx_business_glossary_term' = 'Utilization Target Percentage');
ALTER TABLE `legal_ecm`.`service`.`office` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`service`.`office` SET TAGS ('dbx_subdomain' = 'offering_catalog');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `office_id` SET TAGS ('dbx_business_glossary_term' = 'Office Identifier');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `address_line_1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `address_line_1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `address_line_2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `address_line_2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `email_address` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `email_address` SET TAGS ('dbx_pii_email' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `fax_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `fax_number` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `phone_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `phone_number` SET TAGS ('dbx_pii_phone' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `state_province` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`service`.`office` ALTER COLUMN `state_province` SET TAGS ('dbx_pii_address' = 'true');
