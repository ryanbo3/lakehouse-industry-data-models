-- Schema for Domain: risk | Business: Legal | Version: v1_ecm
-- Generated on: 2026-05-07 11:59:02
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`register` (
    ...
    `matter_id` BIGINT COMMENT 'Reference to the legal matter or case to which this risk is associated, where the risk is matter-level rather than enterprise-level. Null for firm-wide enterprise risks.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`assessment` (
    ...
    `attorney_profile_id` BIGINT COMMENT 'Reference to the timekeeper or workforce member who conducted and is responsible for this risk assessment. Supports accountability tracking and assessor workload reporting.',
    `assessment_treatment_owner_attorney_profile_id` BIGINT COMMENT 'Reference to the timekeeper or workforce member accountable for implementing the risk treatment actions. Distinct from the assessor; supports accountability and escalation workflows.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`risk_control` (
    ...
    `quaternary_risk_approved_by_attorney_profile_id` BIGINT COMMENT 'Reference to the timekeeper or senior risk officer who formally approved this control for inclusion in the risk register and Statement of Applicability. Supports governance and segregation of duties requirements.',
    ...
    `tertiary_risk_remediation_owner_attorney_profile_id` BIGINT COMMENT 'Reference to the timekeeper or workforce member accountable for completing the remediation action. Null if no remediation is required. Supports escalation workflows and accountability tracking.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`data_breach_incident` (
    ...
    `cyber_risk_event_id` BIGINT COMMENT 'FK to risk.cyber_risk_event.cyber_risk_event_id — A data breach may be triggered by or discovered through a cybersecurity/operational event. This link connects the privacy incident to its root cause event.',
    ...
    `primary_cyber_risk_event_id` BIGINT COMMENT 'FK to risk.cyber_risk_event.cyber_risk_event_id — A data breach is a specific type of risk event with additional privacy-specific regulatory attributes. The parent event should be linkable for consolidated event reporting.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`cyber_risk_event` (
    ...
    `assigned_to_user_timekeeper_id` BIGINT COMMENT 'Reference to the timekeeper or staff member currently responsible for managing and resolving the cyber risk event.',
    ...
    `escalated_to_user_timekeeper_id` BIGINT COMMENT 'Reference to the senior timekeeper or executive to whom the cyber risk event was escalated for decision-making or oversight.',
    ...
    `timekeeper_id` BIGINT COMMENT 'Reference to the timekeeper or staff member who first reported or identified the cyber risk event.',
    ...
    `data_breach_incident_id` BIGINT COMMENT 'Reference to the associated data breach incident record if this cyber risk event triggered privacy-specific regulatory notification obligations.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`sanctions_screening` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`reputational_risk` (
    ...
    `matter_id` BIGINT COMMENT 'Identifier of the matter associated with this reputational risk flag.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`appetite` (
    ...
    `appetite_reviewed_by_attorney_profile_id` BIGINT COMMENT 'Foreign key reference to the individual (partner, risk officer, or executive) who last reviewed this risk appetite threshold.',
    ...
    `last_updated_by_timekeeper_id` BIGINT COMMENT 'Foreign key reference to the user or system that last updated this risk appetite threshold record.',
    `office_id` BIGINT COMMENT 'Foreign key reference to the office if this threshold applies at office level (nullable for firm-wide or practice group thresholds).',
    ...
    `superseded_by_threshold_appetite_id` BIGINT COMMENT 'Foreign key reference to the risk appetite threshold record that supersedes this one (nullable for current active thresholds).',
    `timekeeper_id` BIGINT COMMENT 'Foreign key reference to the user or system that created this risk appetite threshold record.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`third_party_risk` (
    ...
    `vendor_name` STRING COMMENT 'Legal name of the third-party vendor, Alternative Legal Service Provider (ALSP), technology vendor, or outsourced service provider.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`risk`.`escalation` (
    ...
    `last_updated_by_timekeeper_id` BIGINT COMMENT 'Reference to the user who last updated the escalation record.',
    ...
    `timekeeper_id` BIGINT COMMENT 'Reference to the individual or role who initiated the escalation.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `legal_ecm`.`risk`.`register` ADD CONSTRAINT `fk_risk_register_appetite_id` FOREIGN KEY (`appetite_id`) REFERENCES `legal_ecm`.`risk`.`appetite`(`appetite_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`assessment` ADD CONSTRAINT `fk_risk_assessment_appetite_id` FOREIGN KEY (`appetite_id`) REFERENCES `legal_ecm`.`risk`.`appetite`(`appetite_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ADD CONSTRAINT `fk_risk_indemnity_exposure_escalation_id` FOREIGN KEY (`escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ADD CONSTRAINT `fk_risk_pi_claim_escalation_id` FOREIGN KEY (`escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`data_breach_incident` ADD CONSTRAINT `fk_risk_data_breach_incident_cyber_risk_event_id` FOREIGN KEY (`cyber_risk_event_id`) REFERENCES `legal_ecm`.`risk`.`cyber_risk_event`(`cyber_risk_event_id`);
ALTER TABLE `legal_ecm`.`risk`.`data_breach_incident` ADD CONSTRAINT `fk_risk_data_breach_incident_escalation_id` FOREIGN KEY (`escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
ALTER TABLE `legal_ecm`.`risk`.`data_breach_incident` ADD CONSTRAINT `fk_risk_data_breach_incident_primary_cyber_risk_event_id` FOREIGN KEY (`primary_cyber_risk_event_id`) REFERENCES `legal_ecm`.`risk`.`cyber_risk_event`(`cyber_risk_event_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ADD CONSTRAINT `fk_risk_cyber_risk_event_category_id` FOREIGN KEY (`category_id`) REFERENCES `legal_ecm`.`risk`.`category`(`category_id`);
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ADD CONSTRAINT `fk_risk_cyber_risk_event_escalation_id` FOREIGN KEY (`escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ADD CONSTRAINT `fk_risk_cyber_risk_event_register_id` FOREIGN KEY (`register_id`) REFERENCES `legal_ecm`.`risk`.`register`(`register_id`);
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ADD CONSTRAINT `fk_risk_cyber_risk_event_risk_control_id` FOREIGN KEY (`risk_control_id`) REFERENCES `legal_ecm`.`risk`.`risk_control`(`risk_control_id`);
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ADD CONSTRAINT `fk_risk_sanctions_screening_register_id` FOREIGN KEY (`register_id`) REFERENCES `legal_ecm`.`risk`.`register`(`register_id`);
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ADD CONSTRAINT `fk_risk_sanctions_screening_reputational_risk_id` FOREIGN KEY (`reputational_risk_id`) REFERENCES `legal_ecm`.`risk`.`reputational_risk`(`reputational_risk_id`);
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ADD CONSTRAINT `fk_risk_reputational_risk_category_id` FOREIGN KEY (`category_id`) REFERENCES `legal_ecm`.`risk`.`category`(`category_id`);
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ADD CONSTRAINT `fk_risk_reputational_risk_escalation_id` FOREIGN KEY (`escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ADD CONSTRAINT `fk_risk_reputational_risk_register_id` FOREIGN KEY (`register_id`) REFERENCES `legal_ecm`.`risk`.`register`(`register_id`);
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` ADD CONSTRAINT `fk_risk_matter_risk_profile_appetite_id` FOREIGN KEY (`appetite_id`) REFERENCES `legal_ecm`.`risk`.`appetite`(`appetite_id`);
ALTER TABLE `legal_ecm`.`risk`.`appetite` ADD CONSTRAINT `fk_risk_appetite_category_id` FOREIGN KEY (`category_id`) REFERENCES `legal_ecm`.`risk`.`category`(`category_id`);
ALTER TABLE `legal_ecm`.`risk`.`appetite` ADD CONSTRAINT `fk_risk_appetite_superseded_by_threshold_appetite_id` FOREIGN KEY (`superseded_by_threshold_appetite_id`) REFERENCES `legal_ecm`.`risk`.`appetite`(`appetite_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ADD CONSTRAINT `fk_risk_third_party_risk_category_id` FOREIGN KEY (`category_id`) REFERENCES `legal_ecm`.`risk`.`category`(`category_id`);
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ADD CONSTRAINT `fk_risk_third_party_risk_escalation_id` FOREIGN KEY (`escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ADD CONSTRAINT `fk_risk_third_party_risk_register_id` FOREIGN KEY (`register_id`) REFERENCES `legal_ecm`.`risk`.`register`(`register_id`);
ALTER TABLE `legal_ecm`.`risk`.`escalation` ADD CONSTRAINT `fk_risk_escalation_category_id` FOREIGN KEY (`category_id`) REFERENCES `legal_ecm`.`risk`.`category`(`category_id`);
ALTER TABLE `legal_ecm`.`risk`.`escalation` ADD CONSTRAINT `fk_risk_escalation_register_id` FOREIGN KEY (`register_id`) REFERENCES `legal_ecm`.`risk`.`register`(`register_id`);
ALTER TABLE `legal_ecm`.`risk`.`escalation` ADD CONSTRAINT `fk_risk_escalation_prior_escalation_id` FOREIGN KEY (`prior_escalation_id`) REFERENCES `legal_ecm`.`risk`.`escalation`(`escalation_id`);
...
ALTER TABLE `legal_ecm`.`risk`.`register` ALTER COLUMN `appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Threshold Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`risk`.`register` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`register` ALTER COLUMN `practice_group_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Group ID');
ALTER TABLE `legal_ecm`.`risk`.`register` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`risk`.`register` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Owner ID');
ALTER TABLE `legal_ecm`.`risk`.`register` ALTER COLUMN `register_risk_owner_attorney_profile_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `legal_ecm`.`risk`.`assessment` ALTER COLUMN `appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Threshold Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`assessment` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Assessor ID');
ALTER TABLE `legal_ecm`.`risk`.`assessment` ALTER COLUMN `assessment_treatment_owner_attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Treatment Owner ID');
ALTER TABLE `legal_ecm`.`risk`.`assessment` ALTER COLUMN `assessment_treatment_owner_attorney_profile_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`assessment` ALTER COLUMN `assessment_treatment_owner_attorney_profile_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `legal_ecm`.`risk`.`assessment` ALTER COLUMN `reviewer_attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer ID');
...
ALTER TABLE `legal_ecm`.`risk`.`risk_control` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Control Owner ID');
ALTER TABLE `legal_ecm`.`risk`.`risk_control` ALTER COLUMN `quaternary_risk_approved_by_attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By ID');
ALTER TABLE `legal_ecm`.`risk`.`risk_control` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Item ID');
ALTER TABLE `legal_ecm`.`risk`.`risk_control` ALTER COLUMN `tertiary_risk_remediation_owner_attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Remediation Owner ID');
...
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` SET TAGS ('dbx_subdomain' = 'professional_indemnity');
...
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Timekeeper ID');
...
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `insurer_name` SET TAGS ('dbx_business_glossary_term' = 'Professional Indemnity (PI) Insurer Name');
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `insurer_name` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `policy_number` SET TAGS ('dbx_business_glossary_term' = 'Professional Indemnity (PI) Policy Number');
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `policy_number` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `policy_year` SET TAGS ('dbx_business_glossary_term' = 'Professional Indemnity (PI) Policy Year');
ALTER TABLE `legal_ecm`.`risk`.`indemnity_exposure` ALTER COLUMN `policy_year` SET TAGS ('dbx_value_regex' = '^[0-9]{4}-[0-9]{4}$');
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` SET TAGS ('dbx_subdomain' = 'professional_indemnity');
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Invoice Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Coverage Counsel Timekeeper ID');
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Timekeeper ID');
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `data_breach_reference` SET TAGS ('dbx_business_glossary_term' = 'Data Breach Incident Reference');
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `insurer_name` SET TAGS ('dbx_business_glossary_term' = 'PI Insurer Name');
...
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `policy_number` SET TAGS ('dbx_business_glossary_term' = 'PI Insurance Policy Number');
ALTER TABLE `legal_ecm`.`risk`.`pi_claim` ALTER COLUMN `policy_year` SET TAGS ('dbx_business_glossary_term' = 'PI Insurance Policy Year');
...
ALTER TABLE `legal_ecm`.`risk`.`data_breach_incident` SET TAGS ('dbx_subdomain' = 'incident_management');
...
ALTER TABLE `legal_ecm`.`risk`.`data_breach_incident` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` SET TAGS ('dbx_subdomain' = 'incident_management');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `cyber_risk_event_id` SET TAGS ('dbx_business_glossary_term' = 'Cyber Risk Event ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Affected Trust Account Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `assigned_to_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned To User ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `assigned_to_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `assigned_to_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `escalated_to_user_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Escalated To User ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `escalated_to_user_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `escalated_to_user_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `matter_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `profile_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Reported By User ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `actual_loss_amount` SET TAGS ('dbx_business_glossary_term' = 'Actual Loss Amount');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `actual_loss_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_client_count` SET TAGS ('dbx_business_glossary_term' = 'Affected Client Count');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_practice_group` SET TAGS ('dbx_business_glossary_term' = 'Affected Practice Group');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_practice_group` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_record_count` SET TAGS ('dbx_business_glossary_term' = 'Affected Record Count');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_system` SET TAGS ('dbx_business_glossary_term' = 'Affected System');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_system` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `affected_user_count` SET TAGS ('dbx_business_glossary_term' = 'Affected User Count');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `containment_action` SET TAGS ('dbx_business_glossary_term' = 'Containment Action');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `data_breach_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Data Breach Incident ID');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `detection_date` SET TAGS ('dbx_business_glossary_term' = 'Detection Date');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `detection_source` SET TAGS ('dbx_business_glossary_term' = 'Detection Source');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `downtime_hours` SET TAGS ('dbx_business_glossary_term' = 'Downtime Hours');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `estimated_loss_amount` SET TAGS ('dbx_business_glossary_term' = 'Estimated Loss Amount');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `estimated_loss_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_date` SET TAGS ('dbx_business_glossary_term' = 'Event Date');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Event Reference Number');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_reference_number` SET TAGS ('dbx_value_regex' = '^CRE-[0-9]{4}-[0-9]{6}$');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_status` SET TAGS ('dbx_business_glossary_term' = 'Event Status');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_status` SET TAGS ('dbx_value_regex' = 'detected|investigating|contained|remediated|closed|escalated');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Event Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `event_type` SET TAGS ('dbx_business_glossary_term' = 'Event Type');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `external_party_involved` SET TAGS ('dbx_business_glossary_term' = 'External Party Involved');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `external_party_involved` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `insurance_claim_filed` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Filed');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `insurance_claim_reference` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Reference');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `insurance_claim_reference` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `is_near_miss` SET TAGS ('dbx_business_glossary_term' = 'Is Near Miss');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `is_reportable_breach` SET TAGS ('dbx_business_glossary_term' = 'Is Reportable Breach');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `iso_control_reference` SET TAGS ('dbx_business_glossary_term' = 'ISO (International Organization for Standardization) Control Reference');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `iso_control_reference` SET TAGS ('dbx_value_regex' = '^A.[0-9]{1,2}.[0-9]{1,2}(.[0-9]{1,2})?$');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `lessons_learned` SET TAGS ('dbx_business_glossary_term' = 'Lessons Learned');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `loss_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Loss Currency Code');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `loss_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `recovery_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Recovery Cost Amount');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `recovery_cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `regulatory_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `regulatory_notification_required` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Required');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `remediation_action` SET TAGS ('dbx_business_glossary_term' = 'Remediation Action');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `resolution_date` SET TAGS ('dbx_business_glossary_term' = 'Resolution Date');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `root_cause_analysis` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Analysis');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
ALTER TABLE `legal_ecm`.`risk`.`cyber_risk_event` ALTER COLUMN `severity_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low|informational');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` SET TAGS ('dbx_subdomain' = 'compliance_screening');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `sanctions_screening_id` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Screening Identifier');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Officer ID');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `contract_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Agreement Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `reputational_risk_id` SET TAGS ('dbx_business_glossary_term' = 'Reputational Risk Flag Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `trust_disbursement_id` SET TAGS ('dbx_business_glossary_term' = 'Screened Trust Disbursement Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `receipt_id` SET TAGS ('dbx_business_glossary_term' = 'Screened Trust Receipt Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `adverse_media_flag` SET TAGS ('dbx_business_glossary_term' = 'Adverse Media Flag');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `aml_risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Anti-Money Laundering (AML) Risk Rating');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `aml_risk_rating` SET TAGS ('dbx_value_regex' = 'low|medium|high|very_high');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `clearance_conditions` SET TAGS ('dbx_business_glossary_term' = 'Clearance Conditions');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `clearance_conditions` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `clearance_date` SET TAGS ('dbx_business_glossary_term' = 'Clearance Date');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `clearance_decision` SET TAGS ('dbx_business_glossary_term' = 'Clearance Decision');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `clearance_decision` SET TAGS ('dbx_value_regex' = 'approved|declined|conditional_approval');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `enhanced_due_diligence_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Enhanced Due Diligence (EDD) Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `escalation_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalation Flag');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `false_positive_flag` SET TAGS ('dbx_business_glossary_term' = 'False Positive Flag');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `false_positive_reason` SET TAGS ('dbx_business_glossary_term' = 'False Positive Reason');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `match_count` SET TAGS ('dbx_business_glossary_term' = 'Match Count');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `match_score` SET TAGS ('dbx_business_glossary_term' = 'Match Score');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `match_type` SET TAGS ('dbx_business_glossary_term' = 'Match Type');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `match_type` SET TAGS ('dbx_value_regex' = 'exact|fuzzy|alias|phonetic|no_match');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `matched_list_entry_name` SET TAGS ('dbx_business_glossary_term' = 'Matched List Entry Name');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `matched_list_entry_reference` SET TAGS ('dbx_business_glossary_term' = 'Matched List Entry Reference');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `pep_category` SET TAGS ('dbx_business_glossary_term' = 'Politically Exposed Person (PEP) Category');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `pep_category` SET TAGS ('dbx_value_regex' = 'domestic|foreign|international_organization|family_member|close_associate');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `pep_flag` SET TAGS ('dbx_business_glossary_term' = 'Politically Exposed Person (PEP) Flag');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `rescreening_due_date` SET TAGS ('dbx_business_glossary_term' = 'Rescreening Due Date');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `review_notes` SET TAGS ('dbx_business_glossary_term' = 'Review Notes');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `review_notes` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `review_outcome` SET TAGS ('dbx_business_glossary_term' = 'Review Outcome');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `review_outcome` SET TAGS ('dbx_value_regex' = 'cleared|rejected|pending_review|requires_enhanced_due_diligence|approved_with_conditions');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_country_of_residence` SET TAGS ('dbx_business_glossary_term' = 'Screened Entity Country of Residence');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_date_of_birth` SET TAGS ('dbx_business_glossary_term' = 'Screened Entity Date of Birth');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_date_of_birth` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_date_of_birth` SET TAGS ('dbx_pii_dob' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_name` SET TAGS ('dbx_business_glossary_term' = 'Screened Entity Name');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_national_identifier` SET TAGS ('dbx_business_glossary_term' = 'Screened Entity National ID');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_national_identifier` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_national_identifier` SET TAGS ('dbx_pii_national_id' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_type` SET TAGS ('dbx_business_glossary_term' = 'Screened Entity Type');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screened_entity_type` SET TAGS ('dbx_value_regex' = 'individual|organization|beneficial_owner|counterparty|matter_participant|trust');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_date` SET TAGS ('dbx_business_glossary_term' = 'Screening Date');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_list_checked` SET TAGS ('dbx_business_glossary_term' = 'Screening List Checked');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Screening Reference Number');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_status` SET TAGS ('dbx_business_glossary_term' = 'Screening Status');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_status` SET TAGS ('dbx_value_regex' = 'completed|in_progress|pending_review|failed');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_system_name` SET TAGS ('dbx_business_glossary_term' = 'Screening System Name');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_system_version` SET TAGS ('dbx_business_glossary_term' = 'Screening System Version');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Screening Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_trigger_event` SET TAGS ('dbx_business_glossary_term' = 'Screening Trigger Event');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `screening_trigger_event` SET TAGS ('dbx_value_regex' = 'new_client_intake|matter_opening|periodic_review|regulatory_update|risk_event');
ALTER TABLE `legal_ecm`.`risk`.`sanctions_screening` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` SET TAGS ('dbx_subdomain' = 'compliance_screening');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `reputational_risk_id` SET TAGS ('dbx_business_glossary_term' = 'Reputational Risk Identifier');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Approving Partner ID');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Monitored Trust Account Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `board_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Board Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `closure_date` SET TAGS ('dbx_business_glossary_term' = 'Closure Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `closure_reason` SET TAGS ('dbx_business_glossary_term' = 'Closure Reason');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `conditions_imposed` SET TAGS ('dbx_business_glossary_term' = 'Conditions Imposed');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `conditions_imposed` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `counterparty_name` SET TAGS ('dbx_business_glossary_term' = 'Counterparty Name');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `counterparty_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `disposition` SET TAGS ('dbx_business_glossary_term' = 'Disposition');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `disposition` SET TAGS ('dbx_value_regex' = 'accepted_with_conditions|declined|monitored|escalated_to_board|no_action_required');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `external_counsel_engaged_flag` SET TAGS ('dbx_business_glossary_term' = 'External Counsel Engaged Flag');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `flag_raised_by` SET TAGS ('dbx_business_glossary_term' = 'Flag Raised By');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `flag_raised_date` SET TAGS ('dbx_business_glossary_term' = 'Flag Raised Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `flag_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Flag Reference Number');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `flag_reference_number` SET TAGS ('dbx_value_regex' = '^RRF-[0-9]{8}$');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `flag_source` SET TAGS ('dbx_business_glossary_term' = 'Flag Source');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `flag_source_detail` SET TAGS ('dbx_business_glossary_term' = 'Flag Source Detail');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `impact_assessment` SET TAGS ('dbx_business_glossary_term' = 'Impact Assessment');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `impact_assessment` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `industry_sector` SET TAGS ('dbx_business_glossary_term' = 'Industry Sector');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `insurance_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Insurance Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `insurance_notification_flag` SET TAGS ('dbx_business_glossary_term' = 'Insurance Notification Flag');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `last_modified_by` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `media_exposure_flag` SET TAGS ('dbx_business_glossary_term' = 'Media Exposure Flag');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `mitigation_actions` SET TAGS ('dbx_business_glossary_term' = 'Mitigation Actions');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `mitigation_actions` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `monitoring_frequency` SET TAGS ('dbx_business_glossary_term' = 'Monitoring Frequency');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `monitoring_frequency` SET TAGS ('dbx_value_regex' = 'continuous|monthly|quarterly|semi_annual|annual|as_needed');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `regulatory_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `regulatory_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `regulatory_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reference Number');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `review_status` SET TAGS ('dbx_business_glossary_term' = 'Review Status');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `review_status` SET TAGS ('dbx_value_regex' = 'pending_review|under_review|reviewed|escalated|closed');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `reviewed_by` SET TAGS ('dbx_business_glossary_term' = 'Reviewed By');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `risk_description` SET TAGS ('dbx_business_glossary_term' = 'Risk Description');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `risk_description` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `severity_level` SET TAGS ('dbx_business_glossary_term' = 'Severity Level');
ALTER TABLE `legal_ecm`.`risk`.`reputational_risk` ALTER COLUMN `severity_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` SET TAGS ('dbx_subdomain' = 'compliance_screening');
...
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` ALTER COLUMN `appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Threshold Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` ALTER COLUMN `practice_group_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Group ID');
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Partner ID');
ALTER TABLE `legal_ecm`.`risk`.`matter_risk_profile` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Primary Trust Account Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`risk`.`appetite` SET TAGS ('dbx_data_type' = 'reference_data');
ALTER TABLE `legal_ecm`.`risk`.`appetite` SET TAGS ('dbx_subdomain' = 'risk_governance');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Appetite Identifier');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Approved By ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `appetite_reviewed_by_attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewed By ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `last_updated_by_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Updated By ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `office_id` SET TAGS ('dbx_business_glossary_term' = 'Office ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `practice_group_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Group ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `superseded_by_threshold_appetite_id` SET TAGS ('dbx_business_glossary_term' = 'Superseded By Threshold ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Created By ID');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `approving_governance_body` SET TAGS ('dbx_business_glossary_term' = 'Approving Governance Body');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `approving_governance_body` SET TAGS ('dbx_value_regex' = 'risk_committee|managing_partner|board|executive_committee|practice_group_leader');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `automatic_escalation_flag` SET TAGS ('dbx_business_glossary_term' = 'Automatic Escalation Flag');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `client_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Client Notification Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `effective_from_date` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `effective_to_date` SET TAGS ('dbx_business_glossary_term' = 'Effective To Date');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `escalation_trigger_score` SET TAGS ('dbx_business_glossary_term' = 'Escalation Trigger Score');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `financial_exposure_limit` SET TAGS ('dbx_business_glossary_term' = 'Financial Exposure Limit');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `financial_exposure_limit` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `insurance_coverage_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Insurance Coverage Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `mandatory_mitigation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Mandatory Mitigation Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `maximum_tolerable_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Maximum Tolerable Risk Score');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `minimum_acceptable_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Minimum Acceptable Risk Score');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `organizational_level` SET TAGS ('dbx_business_glossary_term' = 'Organizational Level');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `organizational_level` SET TAGS ('dbx_value_regex' = 'firm_wide|practice_group|office|matter');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `regulator_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulator Notification Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `review_frequency` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `review_frequency` SET TAGS ('dbx_value_regex' = 'monthly|quarterly|semi_annually|annually|ad_hoc');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `risk_score_scale` SET TAGS ('dbx_business_glossary_term' = 'Risk Score Scale');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `risk_score_scale` SET TAGS ('dbx_value_regex' = '1-5|1-10|1-25|1-100');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `risk_subcategory` SET TAGS ('dbx_business_glossary_term' = 'Risk Subcategory');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `risk_tolerance_band` SET TAGS ('dbx_business_glossary_term' = 'Risk Tolerance Band');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `risk_tolerance_band` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `threshold_rationale` SET TAGS ('dbx_business_glossary_term' = 'Threshold Rationale');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `threshold_reference_code` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Threshold Reference Code');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `threshold_reference_code` SET TAGS ('dbx_value_regex' = '^RAT-[A-Z0-9]{6,12}$');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `threshold_status` SET TAGS ('dbx_business_glossary_term' = 'Threshold Status');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `threshold_status` SET TAGS ('dbx_value_regex' = 'draft|active|under_review|superseded|expired');
ALTER TABLE `legal_ecm`.`risk`.`appetite` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
...
ALTER TABLE `legal_ecm`.`risk`.`mitigation_action` ALTER COLUMN `practice_group_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Group ID');
ALTER TABLE `legal_ecm`.`risk`.`mitigation_action` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Action Owner ID');
...
ALTER TABLE `legal_ecm`.`risk`.`mitigation_action` ALTER COLUMN `reviewer_attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer ID');
...
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` SET TAGS ('dbx_subdomain' = 'compliance_screening');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `third_party_risk_id` SET TAGS ('dbx_business_glossary_term' = 'Third Party Risk ID');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `contract_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Contract ID');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Owner ID');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Trust Banking Account Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `vendor_id` SET TAGS ('dbx_business_glossary_term' = 'Vendor ID');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `assessment_frequency` SET TAGS ('dbx_business_glossary_term' = 'Assessment Frequency');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `assessment_frequency` SET TAGS ('dbx_value_regex' = 'annual|semi_annual|quarterly|monthly|ad_hoc');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `client_data_flag` SET TAGS ('dbx_business_glossary_term' = 'Client Data Access Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `contractual_security_obligations` SET TAGS ('dbx_business_glossary_term' = 'Contractual Security Obligations');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `cybersecurity_incident_count` SET TAGS ('dbx_business_glossary_term' = 'Cybersecurity Incident Count');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `data_access_level` SET TAGS ('dbx_business_glossary_term' = 'Data Access Level');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `data_access_level` SET TAGS ('dbx_value_regex' = 'none|public|internal|confidential|restricted');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `dpa_execution_date` SET TAGS ('dbx_business_glossary_term' = 'Data Processing Agreement (DPA) Execution Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `dpa_signed_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Processing Agreement (DPA) Signed Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `due_diligence_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Due Diligence Completion Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `due_diligence_status` SET TAGS ('dbx_business_glossary_term' = 'Due Diligence Status');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `due_diligence_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|completed|failed|expired');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `gdpr_compliant_flag` SET TAGS ('dbx_business_glossary_term' = 'General Data Protection Regulation (GDPR) Compliant Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `inherent_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Inherent Risk Score');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `iso_27001_certificate_number` SET TAGS ('dbx_business_glossary_term' = 'ISO 27001 Certificate Number');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `iso_27001_certified_flag` SET TAGS ('dbx_business_glossary_term' = 'ISO 27001 Certification Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `iso_27001_expiry_date` SET TAGS ('dbx_business_glossary_term' = 'ISO 27001 Certificate Expiry Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `last_assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Last Assessment Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Updated Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `lpp_access_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Professional Privilege (LPP) Access Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `monitoring_status` SET TAGS ('dbx_business_glossary_term' = 'Ongoing Monitoring Status');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `monitoring_status` SET TAGS ('dbx_value_regex' = 'active|inactive|enhanced|suspended');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `next_assessment_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Assessment Due Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `onboarding_date` SET TAGS ('dbx_business_glossary_term' = 'Vendor Onboarding Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `pii_access_flag` SET TAGS ('dbx_business_glossary_term' = 'Personally Identifiable Information (PII) Access Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `reputational_risk_flag` SET TAGS ('dbx_business_glossary_term' = 'Reputational Risk Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `reputational_risk_notes` SET TAGS ('dbx_business_glossary_term' = 'Reputational Risk Notes');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `residual_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Score');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `risk_status` SET TAGS ('dbx_business_glossary_term' = 'Risk Status');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `risk_status` SET TAGS ('dbx_value_regex' = 'acceptable|under_review|requires_mitigation|unacceptable|suspended');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `risk_tier` SET TAGS ('dbx_business_glossary_term' = 'Risk Tier');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `risk_tier` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `sanctions_screening_date` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Screening Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `sanctions_screening_status` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Screening Status');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `sanctions_screening_status` SET TAGS ('dbx_value_regex' = 'cleared|flagged|pending|not_screened');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `service_category` SET TAGS ('dbx_business_glossary_term' = 'Service Category');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `service_description` SET TAGS ('dbx_business_glossary_term' = 'Service Description');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `soc2_certified_flag` SET TAGS ('dbx_business_glossary_term' = 'SOC 2 Certification Flag');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Vendor Termination Date');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `vendor_name` SET TAGS ('dbx_business_glossary_term' = 'Vendor Name');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `vendor_reference_code` SET TAGS ('dbx_business_glossary_term' = 'Vendor Reference Code');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `vendor_status` SET TAGS ('dbx_business_glossary_term' = 'Vendor Status');
ALTER TABLE `legal_ecm`.`risk`.`third_party_risk` ALTER COLUMN `vendor_status` SET TAGS ('dbx_value_regex' = 'active|inactive|suspended|terminated|under_review');
ALTER TABLE `legal_ecm`.`risk`.`escalation` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`risk`.`escalation` SET TAGS ('dbx_subdomain' = 'risk_governance');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalation_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Escalation ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Decision Maker ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `category_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Category Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `last_updated_by_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Last Updated By ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Escalating Party ID');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `prior_escalation_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `board_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Board Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `board_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Board Notification Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `closure_date` SET TAGS ('dbx_business_glossary_term' = 'Closure Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `conditions_imposed` SET TAGS ('dbx_business_glossary_term' = 'Conditions Imposed');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `decision_date` SET TAGS ('dbx_business_glossary_term' = 'Decision Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `decision_maker_name` SET TAGS ('dbx_business_glossary_term' = 'Decision Maker Name');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `decision_outcome` SET TAGS ('dbx_business_glossary_term' = 'Decision Outcome');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `decision_outcome` SET TAGS ('dbx_value_regex' = 'accept|mitigate|transfer|avoid|defer|escalate_further');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `decision_rationale` SET TAGS ('dbx_business_glossary_term' = 'Decision Rationale');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalating_party_name` SET TAGS ('dbx_business_glossary_term' = 'Escalating Party Name');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalating_party_role` SET TAGS ('dbx_business_glossary_term' = 'Escalating Party Role');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalation_status` SET TAGS ('dbx_business_glossary_term' = 'Escalation Status');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalation_status` SET TAGS ('dbx_value_regex' = 'open|under_review|decision_made|closed|deferred');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `escalation_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Escalation Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `estimated_financial_impact` SET TAGS ('dbx_business_glossary_term' = 'Estimated Financial Impact');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `follow_up_action_reference` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Action Reference');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `follow_up_action_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Action Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `insurance_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Insurance Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `insurance_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Insurance Notification Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `last_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Updated Timestamp');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `lpp_applies_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Professional Privilege (LPP) Applies Flag');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `pii_involved_flag` SET TAGS ('dbx_business_glossary_term' = 'Personally Identifiable Information (PII) Involved Flag');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `priority` SET TAGS ('dbx_business_glossary_term' = 'Escalation Priority');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `priority` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `receiving_governance_body` SET TAGS ('dbx_business_glossary_term' = 'Receiving Governance Body');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `reference_number` SET TAGS ('dbx_business_glossary_term' = 'Escalation Reference Number');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `reference_number` SET TAGS ('dbx_value_regex' = '^ESC-[0-9]{6,10}$');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `regulatory_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `review_due_date` SET TAGS ('dbx_business_glossary_term' = 'Review Due Date');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `risk_severity_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Severity Level');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `risk_severity_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `trigger` SET TAGS ('dbx_business_glossary_term' = 'Escalation Trigger');
ALTER TABLE `legal_ecm`.`risk`.`escalation` ALTER COLUMN `trigger_description` SET TAGS ('dbx_business_glossary_term' = 'Escalation Trigger Description');
