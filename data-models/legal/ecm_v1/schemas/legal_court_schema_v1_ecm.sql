...
CREATE OR REPLACE TABLE `legal_ecm`.`court`.`service_of_process` (
    ...
    `regulatory_breach_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_breach. Business justification: Service of process in breach litigation initiates defense timeline and regulatory notification obligations; essential for breach incident tracking and compliance reporting.',
    `trust_disbursement_id` BIGINT COMMENT 'Foreign key linking to trust.trust_disbursement. Business justification: Process server fees are typically paid from client trust accounts as litigation costs. Firms must link service of process events to trust disbursements for client billing, cost recovery tracking, and ',
...
CREATE OR REPLACE TABLE `legal_ecm`.`court`.`adr_proceeding` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`court`.`arbitral_award` (
    ...
    `arbitral_adr_proceeding_id` BIGINT COMMENT 'Reference to the parent arbitration proceeding under which this award was issued.',
    `arbitral_award_proceeding` BIGINT COMMENT 'FK to court.adr_proceeding.adr_proceeding_id — Arbitral awards are issued in the context of an ADR proceeding. Required for enforcement workflow and matter closure.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ADD CONSTRAINT `fk_court_arbitral_award_adr_proceeding_id` FOREIGN KEY (`adr_proceeding_id`) REFERENCES `legal_ecm`.`court`.`adr_proceeding`(`adr_proceeding_id`);
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ADD CONSTRAINT `fk_court_arbitral_award_arbitral_adr_proceeding_id` FOREIGN KEY (`arbitral_adr_proceeding_id`) REFERENCES `legal_ecm`.`court`.`adr_proceeding`(`adr_proceeding_id`);
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ADD CONSTRAINT `fk_court_arbitral_award_arbitral_award_proceeding` FOREIGN KEY (`arbitral_award_proceeding`) REFERENCES `legal_ecm`.`court`.`adr_proceeding`(`adr_proceeding_id`);

-- ========= TAGS =========
ALTER SCHEMA `legal_ecm`.`court` SET TAGS ('dbx_division' = 'operations');
ALTER SCHEMA `legal_ecm`.`court` SET TAGS ('dbx_domain' = 'court');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` SET TAGS ('dbx_subdomain' = 'court_core');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_of_process_id` SET TAGS ('dbx_business_glossary_term' = 'Service of Process ID');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `form_template_id` SET TAGS ('dbx_business_glossary_term' = 'Form Template Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `indemnity_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Indemnity Claim Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Proof of Service Document ID');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `privacy_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Data Breach Incident Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client Profile Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `regulatory_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Breach Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `trust_disbursement_id` SET TAGS ('dbx_business_glossary_term' = 'Trust Disbursement Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `attempted_service_count` SET TAGS ('dbx_business_glossary_term' = 'Attempted Service Count');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `central_authority_reference` SET TAGS ('dbx_business_glossary_term' = 'Central Authority Reference');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `compliance_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Compliance Deadline Date');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `created_by_user_code` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `created_by_user_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `created_by_user_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `document_type` SET TAGS ('dbx_business_glossary_term' = 'Document Type');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `ecf_filing_reference` SET TAGS ('dbx_business_glossary_term' = 'Electronic Case Filing (ECF) Filing Reference');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `esi_ediscovery_flag` SET TAGS ('dbx_business_glossary_term' = 'Electronically Stored Information (ESI) / eDiscovery Flag');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `hague_convention_flag` SET TAGS ('dbx_business_glossary_term' = 'Hague Convention Flag');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `jurisdiction_code` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Code');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `modified_by_user_code` SET TAGS ('dbx_business_glossary_term' = 'Modified By User ID');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `modified_by_user_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `modified_by_user_code` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `objection_date` SET TAGS ('dbx_business_glossary_term' = 'Objection Date');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `objection_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Objection Filed Flag');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `objection_reason` SET TAGS ('dbx_business_glossary_term' = 'Objection Reason');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `pacer_case_number` SET TAGS ('dbx_business_glossary_term' = 'Public Access to Court Electronic Records (PACER) Case Number');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `party_served_name` SET TAGS ('dbx_business_glossary_term' = 'Party Served Name');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `party_served_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `party_served_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `party_served_role` SET TAGS ('dbx_business_glossary_term' = 'Party Served Role');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `process_server_license_number` SET TAGS ('dbx_business_glossary_term' = 'Process Server License Number');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `process_server_name` SET TAGS ('dbx_business_glossary_term' = 'Process Server Name');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `proof_of_service_filed_date` SET TAGS ('dbx_business_glossary_term' = 'Proof of Service Filed Date');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `response_due_date` SET TAGS ('dbx_business_glossary_term' = 'Response Due Date');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_address_line1` SET TAGS ('dbx_business_glossary_term' = 'Service Address Line 1');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_address_line1` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_address_line1` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_address_line2` SET TAGS ('dbx_business_glossary_term' = 'Service Address Line 2');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_address_line2` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_address_line2` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_city` SET TAGS ('dbx_business_glossary_term' = 'Service City');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_city` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_city` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Service Cost Amount');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Service Cost Currency Code');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_country_code` SET TAGS ('dbx_business_glossary_term' = 'Service Country Code');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_country_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_date` SET TAGS ('dbx_business_glossary_term' = 'Service Date');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_method` SET TAGS ('dbx_business_glossary_term' = 'Service Method');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_notes` SET TAGS ('dbx_business_glossary_term' = 'Service Notes');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_postal_code` SET TAGS ('dbx_business_glossary_term' = 'Service Postal Code');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_postal_code` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_postal_code` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Service Reference Number');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_state_province` SET TAGS ('dbx_business_glossary_term' = 'Service State or Province');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_status` SET TAGS ('dbx_business_glossary_term' = 'Service Status');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_status` SET TAGS ('dbx_value_regex' = 'pending|completed|failed|returned_unserved|quashed');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_time` SET TAGS ('dbx_business_glossary_term' = 'Service Timestamp');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_validity_status` SET TAGS ('dbx_business_glossary_term' = 'Service Validity Status');
ALTER TABLE `legal_ecm`.`court`.`service_of_process` ALTER COLUMN `service_validity_status` SET TAGS ('dbx_value_regex' = 'valid|invalid|pending_validation|quashed|objected');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` SET TAGS ('dbx_subdomain' = 'court_core');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `adr_proceeding_id` SET TAGS ('dbx_business_glossary_term' = 'Alternative Dispute Resolution (ADR) Proceeding Identifier');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Trust Account Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `indemnity_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Indemnity Claim Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Identifier');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `practice_note_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Note Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client Profile Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `regulatory_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Breach Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `administering_institution` SET TAGS ('dbx_business_glossary_term' = 'Administering Institution');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `adr_type` SET TAGS ('dbx_business_glossary_term' = 'Alternative Dispute Resolution (ADR) Type');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `adr_type` SET TAGS ('dbx_value_regex' = 'arbitration|mediation|expert_determination|adjudication|conciliation|dispute_board');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `award_amount` SET TAGS ('dbx_business_glossary_term' = 'Award Amount');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `award_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `award_currency` SET TAGS ('dbx_business_glossary_term' = 'Award Currency');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `award_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `award_date` SET TAGS ('dbx_business_glossary_term' = 'Award Date');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `case_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Case Reference Number');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `claim_amount` SET TAGS ('dbx_business_glossary_term' = 'Claim Amount');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `claim_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `claim_currency` SET TAGS ('dbx_business_glossary_term' = 'Claim Currency');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `claim_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `claimant_party_name` SET TAGS ('dbx_business_glossary_term' = 'Claimant Party Name');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `claimant_party_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `commencement_date` SET TAGS ('dbx_business_glossary_term' = 'Commencement Date');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `confidentiality_level` SET TAGS ('dbx_business_glossary_term' = 'Confidentiality Level');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `confidentiality_level` SET TAGS ('dbx_value_regex' = 'public|confidential|highly_confidential|sealed');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `costs_awarded_to_claimant` SET TAGS ('dbx_business_glossary_term' = 'Costs Awarded to Claimant');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `costs_awarded_to_claimant` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `costs_awarded_to_respondent` SET TAGS ('dbx_business_glossary_term' = 'Costs Awarded to Respondent');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `costs_awarded_to_respondent` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `counterclaim_amount` SET TAGS ('dbx_business_glossary_term' = 'Counterclaim Amount');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `counterclaim_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `counterclaim_currency` SET TAGS ('dbx_business_glossary_term' = 'Counterclaim Currency');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `counterclaim_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `dispute_subject_matter` SET TAGS ('dbx_business_glossary_term' = 'Dispute Subject Matter');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `dispute_subject_matter` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `emergency_arbitrator_appointed_flag` SET TAGS ('dbx_business_glossary_term' = 'Emergency Arbitrator Appointed Flag');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `enforcement_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Jurisdiction');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `enforcement_status` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Status');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `first_hearing_date` SET TAGS ('dbx_business_glossary_term' = 'First Hearing Date');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `governing_law` SET TAGS ('dbx_business_glossary_term' = 'Governing Law');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `governing_rules` SET TAGS ('dbx_business_glossary_term' = 'Governing Rules');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `interim_measures_granted_flag` SET TAGS ('dbx_business_glossary_term' = 'Interim Measures Granted Flag');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `lead_counsel_firm` SET TAGS ('dbx_business_glossary_term' = 'Lead Counsel Firm');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `lead_counsel_firm` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `number_of_arbitrators` SET TAGS ('dbx_business_glossary_term' = 'Number of Arbitrators');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `opposing_counsel_firm` SET TAGS ('dbx_business_glossary_term' = 'Opposing Counsel Firm');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `opposing_counsel_firm` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `place_of_hearing` SET TAGS ('dbx_business_glossary_term' = 'Place of Hearing');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `proceeding_status` SET TAGS ('dbx_business_glossary_term' = 'Proceeding Status');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `record_created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `record_updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Updated Timestamp');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `respondent_party_name` SET TAGS ('dbx_business_glossary_term' = 'Respondent Party Name');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `respondent_party_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `seat_of_arbitration` SET TAGS ('dbx_business_glossary_term' = 'Seat of Arbitration');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `settlement_date` SET TAGS ('dbx_business_glossary_term' = 'Settlement Date');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `settlement_reached_flag` SET TAGS ('dbx_business_glossary_term' = 'Settlement Reached Flag');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `termination_date` SET TAGS ('dbx_business_glossary_term' = 'Termination Date');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `termination_reason` SET TAGS ('dbx_business_glossary_term' = 'Termination Reason');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `termination_reason` SET TAGS ('dbx_value_regex' = 'award_issued|settlement|withdrawal|lack_of_prosecution|jurisdictional_objection_sustained|other');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `tribunal_constitution_date` SET TAGS ('dbx_business_glossary_term' = 'Tribunal Constitution Date');
ALTER TABLE `legal_ecm`.`court`.`adr_proceeding` ALTER COLUMN `underlying_contract_type` SET TAGS ('dbx_business_glossary_term' = 'Underlying Contract Type');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` SET TAGS ('dbx_subdomain' = 'court_core');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `arbitral_award_id` SET TAGS ('dbx_business_glossary_term' = 'Arbitral Award Identifier (ID)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `account_id` SET TAGS ('dbx_business_glossary_term' = 'Trust Account Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `arbitral_adr_proceeding_id` SET TAGS ('dbx_business_glossary_term' = 'Arbitration Proceeding Identifier (ID)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `indemnity_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Indemnity Claim Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `indemnity_exposure_id` SET TAGS ('dbx_business_glossary_term' = 'Indemnity Exposure Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Identifier (ID)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `precedent_id` SET TAGS ('dbx_business_glossary_term' = 'Precedent Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client Profile Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `regulatory_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Breach Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `adr_mechanism` SET TAGS ('dbx_business_glossary_term' = 'Alternative Dispute Resolution (ADR) Mechanism');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `adr_mechanism` SET TAGS ('dbx_value_regex' = 'arbitration|mediation|conciliation|expert_determination|adjudication');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `arbitral_institution` SET TAGS ('dbx_business_glossary_term' = 'Arbitral Institution');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `award_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Award Reference Number');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `award_status` SET TAGS ('dbx_business_glossary_term' = 'Award Status');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `award_summary` SET TAGS ('dbx_business_glossary_term' = 'Award Summary');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `award_summary` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `award_type` SET TAGS ('dbx_business_glossary_term' = 'Award Type');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `award_type` SET TAGS ('dbx_value_regex' = 'final|partial|interim|consent|additional|correction');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `awarded_amount` SET TAGS ('dbx_business_glossary_term' = 'Awarded Amount');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `challenge_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Challenge Filed Flag');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `challenge_filing_date` SET TAGS ('dbx_business_glossary_term' = 'Challenge Filing Date');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `challenge_grounds` SET TAGS ('dbx_business_glossary_term' = 'Challenge Grounds');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `challenge_outcome` SET TAGS ('dbx_business_glossary_term' = 'Challenge Outcome');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `challenge_outcome` SET TAGS ('dbx_value_regex' = 'pending|dismissed|upheld|partially_upheld|withdrawn');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `confidentiality_flag` SET TAGS ('dbx_business_glossary_term' = 'Confidentiality Flag');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `cost_allocation_basis` SET TAGS ('dbx_business_glossary_term' = 'Cost Allocation Basis');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `costs_awarded_to_claimant` SET TAGS ('dbx_business_glossary_term' = 'Costs Awarded to Claimant');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `costs_awarded_to_respondent` SET TAGS ('dbx_business_glossary_term' = 'Costs Awarded to Respondent');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `currency_code` SET TAGS ('dbx_business_glossary_term' = 'Currency Code');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `document_reference` SET TAGS ('dbx_business_glossary_term' = 'Document Reference');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `enforcement_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Jurisdiction');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `enforcement_order_date` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Order Date');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `enforcement_order_reference` SET TAGS ('dbx_business_glossary_term' = 'Enforcement Order Reference');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `governing_law` SET TAGS ('dbx_business_glossary_term' = 'Governing Law');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `interest_calculation_method` SET TAGS ('dbx_business_glossary_term' = 'Interest Calculation Method');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `interest_calculation_method` SET TAGS ('dbx_value_regex' = 'simple|compound|statutory|contractual');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `interest_provision_flag` SET TAGS ('dbx_business_glossary_term' = 'Interest Provision Flag');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `interest_rate` SET TAGS ('dbx_business_glossary_term' = 'Interest Rate Percentage');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `interest_start_date` SET TAGS ('dbx_business_glossary_term' = 'Interest Start Date');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `issue_date` SET TAGS ('dbx_business_glossary_term' = 'Award Issue Date');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `matter_closure_trigger_flag` SET TAGS ('dbx_business_glossary_term' = 'Matter Closure Trigger Flag');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `new_york_convention_applicable_flag` SET TAGS ('dbx_business_glossary_term' = 'New York Convention Applicable Flag');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Internal Notes');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `notes` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `notification_date` SET TAGS ('dbx_business_glossary_term' = 'Award Notification Date');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `presiding_arbitrator_name` SET TAGS ('dbx_business_glossary_term' = 'Presiding Arbitrator Name');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `presiding_arbitrator_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `publication_permitted_flag` SET TAGS ('dbx_business_glossary_term' = 'Publication Permitted Flag');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `seat_of_arbitration` SET TAGS ('dbx_business_glossary_term' = 'Seat of Arbitration');
ALTER TABLE `legal_ecm`.`court`.`arbitral_award` ALTER COLUMN `tribunal_composition` SET TAGS ('dbx_business_glossary_term' = 'Arbitral Tribunal Composition');
