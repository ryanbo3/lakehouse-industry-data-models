-- Schema for Domain: compliance | Business: Legal | Version: v1_ecm
-- Generated on: 2026-05-07 11:58:59
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`compliance_control` (
    ...
    `compliance_control_framework_id` BIGINT COMMENT 'Reference to the parent compliance framework (e.g., SOX, ISO 27001, GDPR, SRA Code of Conduct) under which this control is defined. Links to the compliance_framework product.',
    `compliance_regulatory_obligation_id` BIGINT COMMENT 'Reference to the specific regulatory obligation or requirement that this control is designed to satisfy. Links to the compliance_obligation product.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`sar_filing` (
    ...
    `sar_aml_kyc_program_id` BIGINT COMMENT 'Foreign key linking to compliance.aml_kyc_program. Business justification: SAR filings are submitted under the firms AML/KYC programme. N:1 relationship (many SARs per program). Connects sar_filing to aml_kyc_program. No columns to remove - filing-specific attributes (sar_re',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`dpia` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`data_subject_request` (
    ...
    `data_privacy_register_id` BIGINT COMMENT 'FK to compliance.data_privacy_register.data_privacy_register_id — Data subject requests relate to specific processing activities documented in the RoPA. The DPO needs to trace which processing activity triggered the request to assess scope and exemptions.',
    ...
    `primary_data_privacy_register_id` BIGINT COMMENT 'FK to compliance.data_privacy_register.data_privacy_register_id — Data subject requests relate to specific processing activities documented in the RoPA. Linking DSRs to the relevant processing activity enables the DPO to assess scope and respond accurately.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`privacy_breach` (
    ...
    `last_modified_by_user_id` BIGINT COMMENT 'User identifier of the person who last modified this breach incident record. Audit trail field.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`regulatory_return` (
    ...
    `primary_regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Regulatory returns are filed to satisfy regulatory obligations. N:1 relationship (many returns per obligation). Resolves silo for regulatory_return. No visible columns to remove - regulatory_body and ',
    `regulatory_obligation_id` BIGINT COMMENT 'FK to compliance.regulatory_obligation.regulatory_obligation_id — Regulatory returns are filed to satisfy specific periodic reporting obligations. This FK enables tracking of which obligations have been satisfied through filing and which are overdue.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`compliance_control_test` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`regulatory_breach` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`training_programme` (
    ...
    `training_regulatory_obligation_id` BIGINT COMMENT 'Reference to the regulatory obligation that this training programme satisfies.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`sanctions_check` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`regulatory_change` (
    ...
    `implementation_owner_timekeeper_id` BIGINT COMMENT 'Foreign key linking to workforce.timekeeper. Business justification: Regulatory changes require a named timekeeper as implementation owner for tracking accountability, workload, and completion. The existing implementation_owner_employee_id(unlinked) should map to timek',
    ...
    `regulatory_change_to_obligation` BIGINT COMMENT 'FK to compliance.regulatory_obligation.regulatory_obligation_id — Regulatory changes impact existing obligations or create new ones. This FK enables impact assessment workflow.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`audit_engagement` (
    ...
    `control_framework_id` BIGINT COMMENT 'Foreign key reference to the control framework entity that this audit engagement is assessing compliance against.',
    `lead_auditor_timekeeper_id` BIGINT COMMENT 'Foreign key linking to workforce.timekeeper. Business justification: Internal compliance audits are conducted by firm timekeepers (often senior associates or compliance specialists). Linking lead auditor to timekeeper enables workload tracking, CPD credit allocation, a',
    ...
    `primary_control_framework_id` BIGINT COMMENT 'FK to compliance.control_framework.control_framework_id — Audit engagements are scoped against specific control frameworks (e.g., ISO 27001 surveillance audit, SOX controls audit). This FK defines audit scope and enables framework-level assurance reporting.',
    `responsible_officer_employee_timekeeper_id` BIGINT COMMENT 'Employee identifier for the responsible officer accountable for audit remediation.',
    `timekeeper_id` BIGINT COMMENT 'Employee identifier for the lead auditor if the audit is conducted internally or by a known staff member.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`audit_finding` (
    `audit_finding_id` BIGINT COMMENT 'Unique identifier for the audit finding record. Primary key.',
    `action_owner_timekeeper_id` BIGINT COMMENT 'Foreign key linking to workforce.timekeeper. Business justification: Audit findings require remediation by named timekeepers. Linking action owner to timekeeper enables tracking of open findings by owner, workload balancing, and performance review input for remediation',
    ...
    `timekeeper_id` BIGINT COMMENT 'Employee identifier of the action owner responsible for remediation.',
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`audit_scope` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` (
...
CREATE OR REPLACE TABLE `legal_ecm`.`compliance`.`audit_program` (
    `audit_program_id` BIGINT COMMENT 'Primary key for audit_program',
    `approved_by_employee_id` BIGINT COMMENT 'Identifier of the individual who approved this audit program for execution.',
    `department_id` BIGINT COMMENT 'Identifier of the department or business unit that owns or is subject to this audit program.',
    `employee_id` BIGINT COMMENT 'Identifier of the individual or role responsible for the overall management and governance of this audit program.',
    `lead_auditor_employee_id` BIGINT COMMENT 'Identifier of the lead auditor assigned to execute this audit program.',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ADD CONSTRAINT `fk_compliance_compliance_control_compliance_control_framework_id` FOREIGN KEY (`compliance_control_framework_id`) REFERENCES `legal_ecm`.`compliance`.`control_framework`(`control_framework_id`);
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ADD CONSTRAINT `fk_compliance_compliance_control_compliance_regulatory_obligation_id` FOREIGN KEY (`compliance_regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
...
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ADD CONSTRAINT `fk_compliance_aml_risk_assessment_aml_kyc_program_id` FOREIGN KEY (`aml_kyc_program_id`) REFERENCES `legal_ecm`.`compliance`.`aml_kyc_program`(`aml_kyc_program_id`);
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ADD CONSTRAINT `fk_compliance_aml_risk_assessment_regulatory_obligation_id` FOREIGN KEY (`regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
...
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ADD CONSTRAINT `fk_compliance_sar_filing_regulatory_breach_id` FOREIGN KEY (`regulatory_breach_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_breach`(`regulatory_breach_id`);
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ADD CONSTRAINT `fk_compliance_sar_filing_sar_aml_kyc_program_id` FOREIGN KEY (`sar_aml_kyc_program_id`) REFERENCES `legal_ecm`.`compliance`.`aml_kyc_program`(`aml_kyc_program_id`);
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ADD CONSTRAINT `fk_compliance_sar_filing_sanctions_check_id` FOREIGN KEY (`sanctions_check_id`) REFERENCES `legal_ecm`.`compliance`.`sanctions_check`(`sanctions_check_id`);
...
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ADD CONSTRAINT `fk_compliance_dpia_data_privacy_register_id` FOREIGN KEY (`data_privacy_register_id`) REFERENCES `legal_ecm`.`compliance`.`data_privacy_register`(`data_privacy_register_id`);
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ADD CONSTRAINT `fk_compliance_data_subject_request_policy_id` FOREIGN KEY (`policy_id`) REFERENCES `legal_ecm`.`compliance`.`policy`(`policy_id`);
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ADD CONSTRAINT `fk_compliance_data_subject_request_data_privacy_register_id` FOREIGN KEY (`data_privacy_register_id`) REFERENCES `legal_ecm`.`compliance`.`data_privacy_register`(`data_privacy_register_id`);
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ADD CONSTRAINT `fk_compliance_data_subject_request_primary_data_privacy_register_id` FOREIGN KEY (`primary_data_privacy_register_id`) REFERENCES `legal_ecm`.`compliance`.`data_privacy_register`(`data_privacy_register_id`);
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ADD CONSTRAINT `fk_compliance_data_subject_request_privacy_breach_id` FOREIGN KEY (`privacy_breach_id`) REFERENCES `legal_ecm`.`compliance`.`privacy_breach`(`privacy_breach_id`);
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ADD CONSTRAINT `fk_compliance_privacy_breach_data_privacy_register_id` FOREIGN KEY (`data_privacy_register_id`) REFERENCES `legal_ecm`.`compliance`.`data_privacy_register`(`data_privacy_register_id`);
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ADD CONSTRAINT `fk_compliance_privacy_breach_primary_data_privacy_register_id` FOREIGN KEY (`primary_data_privacy_register_id`) REFERENCES `legal_ecm`.`compliance`.`data_privacy_register`(`data_privacy_register_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ADD CONSTRAINT `fk_compliance_regulatory_return_primary_regulatory_obligation_id` FOREIGN KEY (`primary_regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ADD CONSTRAINT `fk_compliance_regulatory_return_regulatory_obligation_id` FOREIGN KEY (`regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ADD CONSTRAINT `fk_compliance_regulatory_return_regulatory_breach_id` FOREIGN KEY (`regulatory_breach_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_breach`(`regulatory_breach_id`);
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ADD CONSTRAINT `fk_compliance_compliance_control_test_audit_program_id` FOREIGN KEY (`audit_program_id`) REFERENCES `legal_ecm`.`compliance`.`audit_program`(`audit_program_id`);
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ADD CONSTRAINT `fk_compliance_compliance_control_test_compliance_control_id` FOREIGN KEY (`compliance_control_id`) REFERENCES `legal_ecm`.`compliance`.`compliance_control`(`compliance_control_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ADD CONSTRAINT `fk_compliance_regulatory_breach_compliance_control_id` FOREIGN KEY (`compliance_control_id`) REFERENCES `legal_ecm`.`compliance`.`compliance_control`(`compliance_control_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ADD CONSTRAINT `fk_compliance_regulatory_breach_regulatory_obligation_id` FOREIGN KEY (`regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ADD CONSTRAINT `fk_compliance_regulatory_breach_policy_id` FOREIGN KEY (`policy_id`) REFERENCES `legal_ecm`.`compliance`.`policy`(`policy_id`);
...
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ADD CONSTRAINT `fk_compliance_training_programme_policy_id` FOREIGN KEY (`policy_id`) REFERENCES `legal_ecm`.`compliance`.`policy`(`policy_id`);
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ADD CONSTRAINT `fk_compliance_training_programme_regulatory_obligation_id` FOREIGN KEY (`regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ADD CONSTRAINT `fk_compliance_training_programme_training_regulatory_obligation_id` FOREIGN KEY (`training_regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ADD CONSTRAINT `fk_compliance_sanctions_check_aml_kyc_program_id` FOREIGN KEY (`aml_kyc_program_id`) REFERENCES `legal_ecm`.`compliance`.`aml_kyc_program`(`aml_kyc_program_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ADD CONSTRAINT `fk_compliance_regulatory_change_control_framework_id` FOREIGN KEY (`control_framework_id`) REFERENCES `legal_ecm`.`compliance`.`control_framework`(`control_framework_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ADD CONSTRAINT `fk_compliance_regulatory_change_primary_regulatory_obligation_id` FOREIGN KEY (`primary_regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ADD CONSTRAINT `fk_compliance_regulatory_change_regulatory_change_to_obligation` FOREIGN KEY (`regulatory_change_to_obligation`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ADD CONSTRAINT `fk_compliance_regulatory_change_regulatory_obligation_id` FOREIGN KEY (`regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
...
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ADD CONSTRAINT `fk_compliance_audit_engagement_control_framework_id` FOREIGN KEY (`control_framework_id`) REFERENCES `legal_ecm`.`compliance`.`control_framework`(`control_framework_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ADD CONSTRAINT `fk_compliance_audit_engagement_primary_control_framework_id` FOREIGN KEY (`primary_control_framework_id`) REFERENCES `legal_ecm`.`compliance`.`control_framework`(`control_framework_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ADD CONSTRAINT `fk_compliance_audit_finding_audit_engagement_id` FOREIGN KEY (`audit_engagement_id`) REFERENCES `legal_ecm`.`compliance`.`audit_engagement`(`audit_engagement_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ADD CONSTRAINT `fk_compliance_audit_finding_compliance_control_id` FOREIGN KEY (`compliance_control_id`) REFERENCES `legal_ecm`.`compliance`.`compliance_control`(`compliance_control_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ADD CONSTRAINT `fk_compliance_audit_finding_policy_id` FOREIGN KEY (`policy_id`) REFERENCES `legal_ecm`.`compliance`.`policy`(`policy_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ADD CONSTRAINT `fk_compliance_audit_scope_audit_engagement_id` FOREIGN KEY (`audit_engagement_id`) REFERENCES `legal_ecm`.`compliance`.`audit_engagement`(`audit_engagement_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ADD CONSTRAINT `fk_compliance_audit_scope_regulatory_obligation_id` FOREIGN KEY (`regulatory_obligation_id`) REFERENCES `legal_ecm`.`compliance`.`regulatory_obligation`(`regulatory_obligation_id`);
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ADD CONSTRAINT `fk_compliance_policy_acknowledgement_policy_id` FOREIGN KEY (`policy_id`) REFERENCES `legal_ecm`.`compliance`.`policy`(`policy_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` ADD CONSTRAINT `fk_compliance_audit_program_control_framework_id` FOREIGN KEY (`control_framework_id`) REFERENCES `legal_ecm`.`compliance`.`control_framework`(`control_framework_id`);
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` ADD CONSTRAINT `fk_compliance_audit_program_parent_audit_program_id` FOREIGN KEY (`parent_audit_program_id`) REFERENCES `legal_ecm`.`compliance`.`audit_program`(`audit_program_id`);
...
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_obligation` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
...
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_obligation` ALTER COLUMN `control_framework_reference` SET TAGS ('dbx_business_glossary_term' = 'Control Framework Reference');
...
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_obligation` ALTER COLUMN `related_policy_reference` SET TAGS ('dbx_business_glossary_term' = 'Related Policy Reference');
...
ALTER TABLE `legal_ecm`.`compliance`.`control_framework` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
...
ALTER TABLE `legal_ecm`.`compliance`.`control_framework` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Framework Owner Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
...
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ALTER COLUMN `compliance_control_framework_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Framework Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ALTER COLUMN `compliance_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ALTER COLUMN `user_id` SET TAGS ('dbx_business_glossary_term' = 'Modified By User Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ALTER COLUMN `user_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ALTER COLUMN `user_id` SET TAGS ('dbx_pii' = 'true');
...
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Control Owner Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`compliance`.`aml_kyc_program` ALTER COLUMN `program_documentation_location` SET TAGS ('dbx_business_glossary_term' = 'Program Documentation Location');
ALTER TABLE `legal_ecm`.`compliance`.`aml_kyc_program` ALTER COLUMN `program_documentation_location` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `legal_ecm`.`compliance`.`aml_kyc_program` ALTER COLUMN `risk_assessment_findings` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Findings');
ALTER TABLE `legal_ecm`.`compliance`.`aml_kyc_program` ALTER COLUMN `risk_assessment_findings` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `legal_ecm`.`compliance`.`aml_kyc_program` ALTER COLUMN `risk_assessor_name` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessor Name');
ALTER TABLE `legal_ecm`.`compliance`.`aml_kyc_program` ALTER COLUMN `risk_assessor_name` SET TAGS ('dbx_confidential' = 'true');
...
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` SET TAGS ('dbx_subdomain' = 'financial_crime');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `aml_risk_assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Anti-Money Laundering (AML) Risk Assessment ID');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `aml_kyc_program_id` SET TAGS ('dbx_business_glossary_term' = 'Aml Kyc Program Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `license_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Ip License Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `adverse_media_screening_coverage` SET TAGS ('dbx_business_glossary_term' = 'Adverse Media Screening Coverage');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `adverse_media_screening_coverage` SET TAGS ('dbx_value_regex' = 'full|partial|none');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_methodology` SET TAGS ('dbx_business_glossary_term' = 'Assessment Methodology');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Assessment Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_reference_number` SET TAGS ('dbx_value_regex' = '^AML-[0-9]{4}-[0-9]{6}$');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Assessment Status');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_status` SET TAGS ('dbx_value_regex' = 'draft|in_review|approved|superseded|archived');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_type` SET TAGS ('dbx_business_glossary_term' = 'Assessment Type');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessment_type` SET TAGS ('dbx_value_regex' = 'firm_wide|matter_type|client_segment|geographic|product_service|delivery_channel');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessor_name` SET TAGS ('dbx_business_glossary_term' = 'Assessor Name');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `assessor_role` SET TAGS ('dbx_business_glossary_term' = 'Assessor Role');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `board_presentation_date` SET TAGS ('dbx_business_glossary_term' = 'Board Presentation Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `control_effectiveness_rating` SET TAGS ('dbx_business_glossary_term' = 'Control Effectiveness Rating');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `control_effectiveness_rating` SET TAGS ('dbx_value_regex' = 'ineffective|partially_effective|effective|highly_effective');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `data_sources` SET TAGS ('dbx_business_glossary_term' = 'Data Sources');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `effective_from_date` SET TAGS ('dbx_business_glossary_term' = 'Effective From Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `effective_until_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Until Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `external_audit_date` SET TAGS ('dbx_business_glossary_term' = 'External Audit Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `external_auditor_name` SET TAGS ('dbx_business_glossary_term' = 'External Auditor Name');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `findings_summary` SET TAGS ('dbx_business_glossary_term' = 'Findings Summary');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `high_risk_jurisdictions_identified` SET TAGS ('dbx_business_glossary_term' = 'High-Risk Jurisdictions Identified');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `inherent_risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Inherent Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `inherent_risk_rating` SET TAGS ('dbx_value_regex' = 'low|medium|high|very_high');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `inherent_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Inherent Risk Score');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `jurisdiction_scope` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Scope');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `key_risk_indicators` SET TAGS ('dbx_business_glossary_term' = 'Key Risk Indicators (KRI)');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `mlro_approval_date` SET TAGS ('dbx_business_glossary_term' = 'Money Laundering Reporting Officer (MLRO) Approval Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `mlro_approval_name` SET TAGS ('dbx_business_glossary_term' = 'Money Laundering Reporting Officer (MLRO) Approval Name');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `pep_exposure_flag` SET TAGS ('dbx_business_glossary_term' = 'Politically Exposed Person (PEP) Exposure Flag');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `recommendations` SET TAGS ('dbx_business_glossary_term' = 'Recommendations');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `regulatory_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Date');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `regulatory_submission_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Submission Flag');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `residual_risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `residual_risk_rating` SET TAGS ('dbx_value_regex' = 'low|medium|high|very_high');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `residual_risk_score` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Score');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `risk_appetite_alignment` SET TAGS ('dbx_business_glossary_term' = 'Risk Appetite Alignment');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `risk_appetite_alignment` SET TAGS ('dbx_value_regex' = 'within_appetite|near_limit|exceeds_appetite');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `risk_category` SET TAGS ('dbx_business_glossary_term' = 'Risk Category');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `risk_category` SET TAGS ('dbx_value_regex' = 'client|geographic|product_service|delivery_channel|transaction|other');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `risk_subcategory` SET TAGS ('dbx_business_glossary_term' = 'Risk Subcategory');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `sanctions_screening_coverage` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Screening Coverage');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `sanctions_screening_coverage` SET TAGS ('dbx_value_regex' = 'full|partial|none');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `transaction_monitoring_coverage` SET TAGS ('dbx_business_glossary_term' = 'Transaction Monitoring Coverage');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `transaction_monitoring_coverage` SET TAGS ('dbx_value_regex' = 'full|partial|none');
ALTER TABLE `legal_ecm`.`compliance`.`aml_risk_assessment` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
...
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Money Laundering Reporting Officer (MLRO) ID');
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ALTER COLUMN `regulatory_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Breach Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ALTER COLUMN `sar_aml_kyc_program_id` SET TAGS ('dbx_business_glossary_term' = 'Aml Kyc Program Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`sar_filing` ALTER COLUMN `sanctions_check_id` SET TAGS ('dbx_business_glossary_term' = 'Triggering Sanctions Check Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`compliance`.`data_privacy_register` SET TAGS ('dbx_subdomain' = 'privacy_protection');
...
ALTER TABLE `legal_ecm`.`compliance`.`data_privacy_register` ALTER COLUMN `dpia_necessity_assessment` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Impact Assessment (DPIA) Necessity Assessment');
...
ALTER TABLE `legal_ecm`.`compliance`.`data_privacy_register` ALTER COLUMN `dpia_risk_identification` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Impact Assessment (DPIA) Risk Identification');
...
ALTER TABLE `legal_ecm`.`compliance`.`dpia` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` SET TAGS ('dbx_subdomain' = 'privacy_protection');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `dpia_id` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Impact Assessment (DPIA) ID');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_privacy_register_id` SET TAGS ('dbx_business_glossary_term' = 'Data Privacy Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `approval_status` SET TAGS ('dbx_business_glossary_term' = 'Approval Status');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `approval_status` SET TAGS ('dbx_value_regex' = 'pending|approved|conditionally_approved|rejected');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `approved_by` SET TAGS ('dbx_business_glossary_term' = 'Approved By');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Completion Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_conducted_by` SET TAGS ('dbx_business_glossary_term' = 'Assessment Conducted By');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Assessment Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_reference_number` SET TAGS ('dbx_value_regex' = '^DPIA-[0-9]{4}-[0-9]{4}$');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_start_date` SET TAGS ('dbx_business_glossary_term' = 'Assessment Start Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Assessment Status');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `assessment_title` SET TAGS ('dbx_business_glossary_term' = 'Assessment Title');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_categories` SET TAGS ('dbx_business_glossary_term' = 'Data Categories');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_controller_name` SET TAGS ('dbx_business_glossary_term' = 'Data Controller Name');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_minimization_measures` SET TAGS ('dbx_business_glossary_term' = 'Data Minimization Measures');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_processor_name` SET TAGS ('dbx_business_glossary_term' = 'Data Processor Name');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_subject_categories` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Categories');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `data_subjects_consulted_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Subjects Consulted Flag');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `dpo_advice` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Advice');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `dpo_consultation_date` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Consultation Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `dpo_consulted_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Consulted Flag');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `last_review_date` SET TAGS ('dbx_business_glossary_term' = 'Last Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `lawful_basis` SET TAGS ('dbx_business_glossary_term' = 'Lawful Basis');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `lawful_basis` SET TAGS ('dbx_value_regex' = 'consent|contract|legal_obligation|vital_interests|public_task|legitimate_interests');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `mitigation_measures` SET TAGS ('dbx_business_glossary_term' = 'Mitigation Measures');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `necessity_assessment` SET TAGS ('dbx_business_glossary_term' = 'Necessity Assessment');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `processing_purpose` SET TAGS ('dbx_business_glossary_term' = 'Processing Purpose');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `proportionality_assessment` SET TAGS ('dbx_business_glossary_term' = 'Proportionality Assessment');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `residual_risk_level` SET TAGS ('dbx_business_glossary_term' = 'Residual Risk Level');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `residual_risk_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|very_high');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `retention_period` SET TAGS ('dbx_business_glossary_term' = 'Retention Period');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `review_frequency` SET TAGS ('dbx_business_glossary_term' = 'Review Frequency');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `review_frequency` SET TAGS ('dbx_value_regex' = 'annual|biannual|quarterly|on_change|continuous');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `risk_identification_summary` SET TAGS ('dbx_business_glossary_term' = 'Risk Identification Summary');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `risk_level` SET TAGS ('dbx_value_regex' = 'low|medium|high|very_high');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `special_category_data_flag` SET TAGS ('dbx_business_glossary_term' = 'Special Category Data Flag');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `supervisory_authority_consultation_date` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Consultation Date');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `supervisory_authority_consultation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Consultation Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `supervisory_authority_decision` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Decision');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `supervisory_authority_reference` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Reference');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `trigger_reason` SET TAGS ('dbx_business_glossary_term' = 'Trigger Reason');
ALTER TABLE `legal_ecm`.`compliance`.`dpia` ALTER COLUMN `vulnerable_data_subjects_flag` SET TAGS ('dbx_business_glossary_term' = 'Vulnerable Data Subjects Flag');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` SET TAGS ('dbx_subdomain' = 'privacy_protection');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_request_id` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Request ID');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Assigned Dpo Officer Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `client_contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Policy Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Response Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `privacy_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Related Privacy Breach Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `authorized_representative_flag` SET TAGS ('dbx_business_glossary_term' = 'Authorized Representative Flag');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `complaint_lodged_flag` SET TAGS ('dbx_business_glossary_term' = 'Complaint Lodged Flag');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `complaint_reference` SET TAGS ('dbx_business_glossary_term' = 'Complaint Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_address` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Address');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_address` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_address` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_identifier` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_identifier` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_subject_identifier` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_volume_pages` SET TAGS ('dbx_business_glossary_term' = 'Data Volume in Pages');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `data_volume_records` SET TAGS ('dbx_business_glossary_term' = 'Data Volume in Records');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `escalation_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalation Flag');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `escalation_reason` SET TAGS ('dbx_business_glossary_term' = 'Escalation Reason');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `exemption_applied` SET TAGS ('dbx_business_glossary_term' = 'Exemption Applied');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `extended_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Extended Deadline Date');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `extension_applied_flag` SET TAGS ('dbx_business_glossary_term' = 'Extension Applied Flag');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `extension_reason` SET TAGS ('dbx_business_glossary_term' = 'Extension Reason');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `identity_verification_date` SET TAGS ('dbx_business_glossary_term' = 'Identity Verification Date');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `identity_verification_method` SET TAGS ('dbx_business_glossary_term' = 'Identity Verification Method');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `identity_verification_status` SET TAGS ('dbx_business_glossary_term' = 'Identity Verification Status');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `identity_verification_status` SET TAGS ('dbx_value_regex' = 'not_started|pending|verified|failed|exemption_applied');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `lpp_exemption_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Professional Privilege (LPP) Exemption Flag');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Request Notes');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `outcome` SET TAGS ('dbx_business_glossary_term' = 'Request Outcome');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `outcome` SET TAGS ('dbx_value_regex' = 'fully_granted|partially_granted|refused|withdrawn_by_subject|no_data_held');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `processing_time_days` SET TAGS ('dbx_business_glossary_term' = 'Processing Time in Days');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `receipt_channel` SET TAGS ('dbx_business_glossary_term' = 'Receipt Channel');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `receipt_channel` SET TAGS ('dbx_value_regex' = 'email|postal_mail|web_portal|phone|in_person|third_party_representative');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `receipt_date` SET TAGS ('dbx_business_glossary_term' = 'Receipt Date');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `receipt_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Receipt Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `refusal_reason` SET TAGS ('dbx_business_glossary_term' = 'Refusal Reason');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `regulatory_jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Jurisdiction');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `representative_name` SET TAGS ('dbx_business_glossary_term' = 'Representative Name');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `representative_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `representative_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `representative_relationship` SET TAGS ('dbx_business_glossary_term' = 'Representative Relationship');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `representative_relationship` SET TAGS ('dbx_value_regex' = 'parent|guardian|attorney|executor|power_of_attorney|other');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `request_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Request Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `request_reference_number` SET TAGS ('dbx_value_regex' = '^DSR-[0-9]{8}-[A-Z0-9]{6}$');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `request_scope_description` SET TAGS ('dbx_business_glossary_term' = 'Request Scope Description');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `request_status` SET TAGS ('dbx_business_glossary_term' = 'Request Status');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `request_type` SET TAGS ('dbx_business_glossary_term' = 'Request Type');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `request_type` SET TAGS ('dbx_value_regex' = 'subject_access_request|right_to_erasure|right_to_rectification|right_to_data_portability|right_to_object|right_to_restrict_processing');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `response_date` SET TAGS ('dbx_business_glossary_term' = 'Response Date');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `response_method` SET TAGS ('dbx_business_glossary_term' = 'Response Method');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `response_method` SET TAGS ('dbx_value_regex' = 'email|postal_mail|secure_portal|encrypted_file|in_person|phone');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `statutory_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Statutory Deadline Date');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `systems_searched` SET TAGS ('dbx_business_glossary_term' = 'Systems Searched');
ALTER TABLE `legal_ecm`.`compliance`.`data_subject_request` ALTER COLUMN `third_party_data_flag` SET TAGS ('dbx_business_glossary_term' = 'Third Party Data Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` SET TAGS ('dbx_subdomain' = 'privacy_protection');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `privacy_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Privacy Breach Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Partner ID');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `user_id` SET TAGS ('dbx_business_glossary_term' = 'Created By User ID');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `user_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `user_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `last_modified_by_user_id` SET TAGS ('dbx_business_glossary_term' = 'Last Modified By User ID');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `last_modified_by_user_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `last_modified_by_user_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Notification Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `primary_data_privacy_register_id` SET TAGS ('dbx_business_glossary_term' = 'Data Privacy Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_breach_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Data Breach Incident Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Incident Owner ID');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `affected_data_categories` SET TAGS ('dbx_business_glossary_term' = 'Affected Data Categories');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `affected_data_categories` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `affected_system_name` SET TAGS ('dbx_business_glossary_term' = 'Affected System Name');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `affected_system_name` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_cause` SET TAGS ('dbx_business_glossary_term' = 'Breach Cause');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_cause` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_description` SET TAGS ('dbx_business_glossary_term' = 'Breach Description');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_description` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_location` SET TAGS ('dbx_business_glossary_term' = 'Breach Location');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_location` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_status` SET TAGS ('dbx_business_glossary_term' = 'Breach Status');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_status` SET TAGS ('dbx_value_regex' = 'reported|under_investigation|contained|remediated|closed');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_type` SET TAGS ('dbx_business_glossary_term' = 'Breach Type');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `breach_type` SET TAGS ('dbx_value_regex' = 'accidental_loss|accidental_destruction|accidental_alteration|unauthorised_disclosure|unauthorised_access|unlawful_processing');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `containment_date` SET TAGS ('dbx_business_glossary_term' = 'Containment Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `containment_measures_taken` SET TAGS ('dbx_business_glossary_term' = 'Containment Measures Taken');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `containment_measures_taken` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_records_count` SET TAGS ('dbx_business_glossary_term' = 'Data Records Count');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_subject_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Notification Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_subject_notification_method` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Notification Method');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_subject_notification_method` SET TAGS ('dbx_value_regex' = 'email|postal_mail|phone|public_communication|website_notice');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_subject_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Subject Notification Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `data_subjects_count` SET TAGS ('dbx_business_glossary_term' = 'Data Subjects Count');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `discovery_date` SET TAGS ('dbx_business_glossary_term' = 'Discovery Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `discovery_method` SET TAGS ('dbx_business_glossary_term' = 'Discovery Method');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `discovery_method` SET TAGS ('dbx_value_regex' = 'internal_audit|system_alert|employee_report|client_complaint|third_party_notification|regulatory_inquiry');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `dpo_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Notification Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `dpo_notified_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Notified Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `incident_date` SET TAGS ('dbx_business_glossary_term' = 'Incident Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `incident_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Incident Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `insurance_claim_filed_flag` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Filed Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `insurance_claim_reference` SET TAGS ('dbx_business_glossary_term' = 'Insurance Claim Reference');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `insurance_claim_reference` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `post_incident_review_completed_flag` SET TAGS ('dbx_business_glossary_term' = 'Post-Incident Review Completed Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `post_incident_review_date` SET TAGS ('dbx_business_glossary_term' = 'Post-Incident Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `post_incident_review_outcome` SET TAGS ('dbx_business_glossary_term' = 'Post-Incident Review Outcome');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `post_incident_review_outcome` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `preventive_controls_implemented` SET TAGS ('dbx_business_glossary_term' = 'Preventive Controls Implemented');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `preventive_controls_implemented` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `regulatory_penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Penalty Amount');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `regulatory_penalty_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `regulatory_penalty_currency` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Penalty Currency');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `regulatory_penalty_currency` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `remediation_actions_taken` SET TAGS ('dbx_business_glossary_term' = 'Remediation Actions Taken');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `remediation_actions_taken` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `remediation_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Remediation Completion Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `risk_assessment_rationale` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment Rationale');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `risk_assessment_rationale` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `risk_assessment_to_data_subjects` SET TAGS ('dbx_business_glossary_term' = 'Risk Assessment to Data Subjects');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `risk_assessment_to_data_subjects` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `special_category_data_involved_flag` SET TAGS ('dbx_business_glossary_term' = 'Special Category Data Involved Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `supervisory_authority_name` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Name');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `supervisory_authority_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Notification Date');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `supervisory_authority_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Notification Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`privacy_breach` ALTER COLUMN `supervisory_authority_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Supervisory Authority Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_return_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Return ID');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Return Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Preparer Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `primary_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Reported Regulatory Breach Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `accountant_name` SET TAGS ('dbx_business_glossary_term' = 'Accountant Name');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `accountant_registration_number` SET TAGS ('dbx_business_glossary_term' = 'Accountant Registration Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `approval_date` SET TAGS ('dbx_business_glossary_term' = 'Approval Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `approver_name` SET TAGS ('dbx_business_glossary_term' = 'Approver Name');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `approver_role` SET TAGS ('dbx_business_glossary_term' = 'Approver Role');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_description` SET TAGS ('dbx_business_glossary_term' = 'Breach Description');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_discovery_date` SET TAGS ('dbx_business_glossary_term' = 'Breach Discovery Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_indicator` SET TAGS ('dbx_business_glossary_term' = 'Breach Indicator');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_occurrence_date` SET TAGS ('dbx_business_glossary_term' = 'Breach Occurrence Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_severity` SET TAGS ('dbx_business_glossary_term' = 'Breach Severity');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_severity` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `breach_type` SET TAGS ('dbx_business_glossary_term' = 'Breach Type');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `modified_by` SET TAGS ('dbx_business_glossary_term' = 'Modified By');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `preparer_role` SET TAGS ('dbx_business_glossary_term' = 'Preparer Role');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `query_response_date` SET TAGS ('dbx_business_glossary_term' = 'Query Response Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `query_response_due_date` SET TAGS ('dbx_business_glossary_term' = 'Query Response Due Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_acknowledgement_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Acknowledgement Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_body` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Body');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_findings` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Findings');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_queries` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Queries');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `regulatory_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `remediation_actions` SET TAGS ('dbx_business_glossary_term' = 'Remediation Actions');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `remediation_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Remediation Completion Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `reporting_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period End Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `reporting_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Start Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `return_number` SET TAGS ('dbx_business_glossary_term' = 'Return Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `return_type` SET TAGS ('dbx_business_glossary_term' = 'Return Type');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `reviewer_name` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Name');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `reviewer_role` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Role');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `root_cause_analysis` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Analysis');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `submission_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Submission Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `submission_due_date` SET TAGS ('dbx_business_glossary_term' = 'Submission Due Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `submission_method` SET TAGS ('dbx_business_glossary_term' = 'Submission Method');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `submission_method` SET TAGS ('dbx_value_regex' = 'online_portal|email|postal_mail|electronic_filing_system|in_person');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `submission_status` SET TAGS ('dbx_business_glossary_term' = 'Submission Status');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_return` ALTER COLUMN `supporting_documents` SET TAGS ('dbx_business_glossary_term' = 'Supporting Documents');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `compliance_control_test_id` SET TAGS ('dbx_business_glossary_term' = 'Control Test Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `audit_program_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Program Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `compliance_control_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Control Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Tester Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `reviewer_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewer Identifier (ID)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `risk_control_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Control Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `compliance_framework` SET TAGS ('dbx_business_glossary_term' = 'Compliance Framework');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `control_deficiency_severity` SET TAGS ('dbx_business_glossary_term' = 'Control Deficiency Severity');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `control_deficiency_severity` SET TAGS ('dbx_value_regex' = 'none|deficiency|significant_deficiency|material_weakness');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `evidence_reference` SET TAGS ('dbx_business_glossary_term' = 'Evidence Reference');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `exception_details` SET TAGS ('dbx_business_glossary_term' = 'Exception Details');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `exception_rate_percentage` SET TAGS ('dbx_business_glossary_term' = 'Exception Rate Percentage');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `exceptions_identified_count` SET TAGS ('dbx_business_glossary_term' = 'Exceptions Identified Count');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `external_auditor_flag` SET TAGS ('dbx_business_glossary_term' = 'External Auditor Flag');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `population_size` SET TAGS ('dbx_business_glossary_term' = 'Population Size');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `remediation_due_date` SET TAGS ('dbx_business_glossary_term' = 'Remediation Due Date');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `remediation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Remediation Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `review_date` SET TAGS ('dbx_business_glossary_term' = 'Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `sample_size` SET TAGS ('dbx_business_glossary_term' = 'Sample Size');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `sampling_method` SET TAGS ('dbx_business_glossary_term' = 'Sampling Method');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_completion_date` SET TAGS ('dbx_business_glossary_term' = 'Test Completion Date');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_conclusion` SET TAGS ('dbx_business_glossary_term' = 'Test Conclusion');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_conclusion` SET TAGS ('dbx_value_regex' = 'effective|ineffective|partially_effective|not_tested|inconclusive');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_date` SET TAGS ('dbx_business_glossary_term' = 'Test Execution Date');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_methodology` SET TAGS ('dbx_business_glossary_term' = 'Test Methodology');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_objective` SET TAGS ('dbx_business_glossary_term' = 'Test Objective');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Test Period End Date');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Test Period Start Date');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_procedure_description` SET TAGS ('dbx_business_glossary_term' = 'Test Procedure Description');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Test Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_reference_number` SET TAGS ('dbx_value_regex' = '^[A-Z0-9-]+$');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_results_summary` SET TAGS ('dbx_business_glossary_term' = 'Test Results Summary');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_status` SET TAGS ('dbx_business_glossary_term' = 'Test Status');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|completed|under_review|approved|remediation_required');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `test_type` SET TAGS ('dbx_business_glossary_term' = 'Test Type');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `testing_frequency` SET TAGS ('dbx_business_glossary_term' = 'Testing Frequency');
ALTER TABLE `legal_ecm`.`compliance`.`compliance_control_test` ALTER COLUMN `testing_frequency` SET TAGS ('dbx_value_regex' = 'annual|semi_annual|quarterly|monthly|continuous|ad_hoc');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_breach_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Breach Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `compliance_control_id` SET TAGS ('dbx_business_glossary_term' = 'Failed Compliance Control Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Related Regulatory Obligation ID');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Officer Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Violated Policy Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `actual_remediation_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Remediation Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `affected_client_count` SET TAGS ('dbx_business_glossary_term' = 'Affected Client Count');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `affected_matter_count` SET TAGS ('dbx_business_glossary_term' = 'Affected Matter Count');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_category` SET TAGS ('dbx_business_glossary_term' = 'Breach Category');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_category` SET TAGS ('dbx_value_regex' = 'FINANCIAL|DATA_PRIVACY|CONDUCT|OPERATIONAL|REPORTING|CLIENT_FUNDS');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_description` SET TAGS ('dbx_business_glossary_term' = 'Breach Description');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_description` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Breach Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_reference_number` SET TAGS ('dbx_value_regex' = '^BRH-[0-9]{8}$');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_status` SET TAGS ('dbx_business_glossary_term' = 'Breach Status');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_status` SET TAGS ('dbx_value_regex' = 'IDENTIFIED|UNDER_INVESTIGATION|REMEDIATION_IN_PROGRESS|REMEDIATED|CLOSED|ESCALATED');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `breach_type` SET TAGS ('dbx_business_glossary_term' = 'Breach Type');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `client_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Client Notification Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `client_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Client Notification Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `discovery_date` SET TAGS ('dbx_business_glossary_term' = 'Discovery Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `discovery_method` SET TAGS ('dbx_business_glossary_term' = 'Discovery Method');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `dpo_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Notification Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `dpo_notified_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Notified Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `escalated_to_executive_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalated to Executive Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `financial_impact_amount` SET TAGS ('dbx_business_glossary_term' = 'Financial Impact Amount');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `financial_impact_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `financial_impact_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Financial Impact Currency Code');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `financial_impact_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `impact_assessment` SET TAGS ('dbx_business_glossary_term' = 'Impact Assessment');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `impact_assessment` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `notes` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `notified_regulator` SET TAGS ('dbx_business_glossary_term' = 'Notified Regulator');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `occurrence_date` SET TAGS ('dbx_business_glossary_term' = 'Occurrence Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `professional_indemnity_claim_flag` SET TAGS ('dbx_business_glossary_term' = 'Professional Indemnity Claim Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `professional_indemnity_claim_reference` SET TAGS ('dbx_business_glossary_term' = 'Professional Indemnity Claim Reference');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `professional_indemnity_claim_reference` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulator_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Regulator Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_notification_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_notification_deadline` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Deadline');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_notification_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Notification Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_penalty_amount` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Penalty Amount');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_penalty_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_penalty_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Penalty Currency Code');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `regulatory_penalty_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `remediation_owner` SET TAGS ('dbx_business_glossary_term' = 'Remediation Owner');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `remediation_owner_department` SET TAGS ('dbx_business_glossary_term' = 'Remediation Owner Department');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `remediation_plan` SET TAGS ('dbx_business_glossary_term' = 'Remediation Plan');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `remediation_plan` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `remediation_status` SET TAGS ('dbx_business_glossary_term' = 'Remediation Status');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `remediation_status` SET TAGS ('dbx_value_regex' = 'NOT_STARTED|IN_PROGRESS|ON_HOLD|COMPLETED|OVERDUE');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `root_cause` SET TAGS ('dbx_business_glossary_term' = 'Root Cause');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `root_cause` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `severity_rating` SET TAGS ('dbx_business_glossary_term' = 'Severity Rating');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `severity_rating` SET TAGS ('dbx_value_regex' = 'CRITICAL|HIGH|MEDIUM|LOW');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_breach` ALTER COLUMN `target_remediation_date` SET TAGS ('dbx_business_glossary_term' = 'Target Remediation Date');
ALTER TABLE `legal_ecm`.`compliance`.`indemnity_policy` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`compliance`.`indemnity_policy` SET TAGS ('dbx_subdomain' = 'audit_assurance');
...
ALTER TABLE `legal_ecm`.`compliance`.`indemnity_policy` ALTER COLUMN `practice_areas_covered` SET TAGS ('dbx_business_glossary_term' = 'Practice Areas Covered');
...
ALTER TABLE `legal_ecm`.`compliance`.`indemnity_claim` SET TAGS ('dbx_subdomain' = 'audit_assurance');
...
ALTER TABLE `legal_ecm`.`compliance`.`indemnity_claim` ALTER COLUMN `attorney_profile_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Partner Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`compliance`.`indemnity_claim` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Fee Earner Identifier (ID)');
...
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` SET TAGS ('dbx_subdomain' = 'audit_assurance');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `training_programme_id` SET TAGS ('dbx_business_glossary_term' = 'Training Programme Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Policy Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Content Owner Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `knowledge_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Knowledge Asset Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `practice_group_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Group Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `training_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation ID');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `accreditation_body` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Body');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `accreditation_reference` SET TAGS ('dbx_business_glossary_term' = 'Accreditation Reference');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `assessment_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Assessment Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `certificate_issued_flag` SET TAGS ('dbx_business_glossary_term' = 'Certificate Issued Flag');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_business_glossary_term' = 'Cost Currency Code');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `cost_currency_code` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `cost_per_participant` SET TAGS ('dbx_business_glossary_term' = 'Cost Per Participant');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `cost_per_participant` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `cpd_credits` SET TAGS ('dbx_business_glossary_term' = 'Continuing Professional Development (CPD) Credits');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `delivery_format` SET TAGS ('dbx_business_glossary_term' = 'Delivery Format');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `delivery_format` SET TAGS ('dbx_value_regex' = 'online|in-person|hybrid|on-demand|scheduled');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `delivery_method` SET TAGS ('dbx_business_glossary_term' = 'Delivery Method');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `delivery_method` SET TAGS ('dbx_value_regex' = 'e-learning|instructor-led|blended|webinar|workshop|self-study');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `duration_hours` SET TAGS ('dbx_business_glossary_term' = 'Duration Hours');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `external_url` SET TAGS ('dbx_business_glossary_term' = 'External URL');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `frequency` SET TAGS ('dbx_business_glossary_term' = 'Training Frequency');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `frequency` SET TAGS ('dbx_value_regex' = 'annual|biannual|quarterly|one-time|as-needed|triennial');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `internal_system_reference` SET TAGS ('dbx_business_glossary_term' = 'Internal System Reference');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `language` SET TAGS ('dbx_business_glossary_term' = 'Language');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `language` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `last_content_update_date` SET TAGS ('dbx_business_glossary_term' = 'Last Content Update Date');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `launch_date` SET TAGS ('dbx_business_glossary_term' = 'Launch Date');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `next_review_date` SET TAGS ('dbx_business_glossary_term' = 'Next Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `pass_mark_percentage` SET TAGS ('dbx_business_glossary_term' = 'Pass Mark Percentage');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `practice_group_scope` SET TAGS ('dbx_business_glossary_term' = 'Practice Group Scope');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `prerequisite_programmes` SET TAGS ('dbx_business_glossary_term' = 'Prerequisite Programmes');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `programme_code` SET TAGS ('dbx_business_glossary_term' = 'Programme Code');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `programme_code` SET TAGS ('dbx_value_regex' = '^[A-Z0-9]{6,20}$');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `programme_description` SET TAGS ('dbx_business_glossary_term' = 'Programme Description');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `programme_name` SET TAGS ('dbx_business_glossary_term' = 'Programme Name');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `programme_status` SET TAGS ('dbx_business_glossary_term' = 'Programme Status');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `programme_status` SET TAGS ('dbx_value_regex' = 'active|inactive|draft|under-review|retired|suspended');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `provider_name` SET TAGS ('dbx_business_glossary_term' = 'Provider Name');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `recurrence_interval_months` SET TAGS ('dbx_business_glossary_term' = 'Recurrence Interval Months');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `related_policy_reference` SET TAGS ('dbx_business_glossary_term' = 'Related Policy Reference');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `retirement_date` SET TAGS ('dbx_business_glossary_term' = 'Retirement Date');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `target_audience` SET TAGS ('dbx_business_glossary_term' = 'Target Audience');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `target_role_category` SET TAGS ('dbx_business_glossary_term' = 'Target Role Category');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `training_category` SET TAGS ('dbx_business_glossary_term' = 'Training Category');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `training_category` SET TAGS ('dbx_value_regex' = 'mandatory|recommended|optional|refresher|induction|specialized');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `version_effective_date` SET TAGS ('dbx_business_glossary_term' = 'Version Effective Date');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `version_number` SET TAGS ('dbx_business_glossary_term' = 'Version Number');
ALTER TABLE `legal_ecm`.`compliance`.`training_programme` ALTER COLUMN `version_number` SET TAGS ('dbx_value_regex' = '^[0-9]+.[0-9]+$');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` SET TAGS ('dbx_subdomain' = 'financial_crime');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `sanctions_check_id` SET TAGS ('dbx_business_glossary_term' = 'Sanctions Check Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `aml_kyc_program_id` SET TAGS ('dbx_business_glossary_term' = 'Aml Kyc Program Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `ownership_id` SET TAGS ('dbx_business_glossary_term' = 'Ip Ownership Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `license_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Ip License Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client ID');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Reviewing Officer Employee ID');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `disposition_date` SET TAGS ('dbx_business_glossary_term' = 'Disposition Date');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `disposition_rationale` SET TAGS ('dbx_business_glossary_term' = 'Disposition Rationale');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `escalation_recipient` SET TAGS ('dbx_business_glossary_term' = 'Escalation Recipient');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `escalation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalation Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `match_details` SET TAGS ('dbx_business_glossary_term' = 'Match Details');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `match_score` SET TAGS ('dbx_business_glossary_term' = 'Match Score');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `match_type` SET TAGS ('dbx_business_glossary_term' = 'Match Type');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `match_type` SET TAGS ('dbx_value_regex' = 'exact|fuzzy|alias|phonetic|no_match');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `regulatory_report_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Reference');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `reviewing_officer_name` SET TAGS ('dbx_business_glossary_term' = 'Reviewing Officer Name');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'low|medium|high|critical');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `sanctions_list_screened` SET TAGS ('dbx_business_glossary_term' = 'Sanctions List Screened');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `sanctions_list_screened` SET TAGS ('dbx_value_regex' = 'OFAC_SDN|HM_Treasury|EU_Consolidated|UN_Sanctions|Combined_All');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_date` SET TAGS ('dbx_business_glossary_term' = 'Screening Date');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_disposition` SET TAGS ('dbx_business_glossary_term' = 'Screening Disposition');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_disposition` SET TAGS ('dbx_value_regex' = 'cleared|escalated|blocked|pending_review|false_positive');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Screening Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_system_name` SET TAGS ('dbx_business_glossary_term' = 'Screening System Name');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_system_version` SET TAGS ('dbx_business_glossary_term' = 'Screening System Version');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Screening Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_trigger_event` SET TAGS ('dbx_business_glossary_term' = 'Screening Trigger Event');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `screening_trigger_event` SET TAGS ('dbx_value_regex' = 'client_onboarding|matter_opening|periodic_review|transaction_review|manual_request|system_alert');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_country_of_residence` SET TAGS ('dbx_business_glossary_term' = 'Subject Country of Residence');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_country_of_residence` SET TAGS ('dbx_value_regex' = '^[A-Z]{3}$');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_date_of_birth` SET TAGS ('dbx_business_glossary_term' = 'Subject Date of Birth');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_date_of_birth` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_date_of_birth` SET TAGS ('dbx_pii_dob' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_name` SET TAGS ('dbx_business_glossary_term' = 'Subject Name');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_name` SET TAGS ('dbx_pii_name' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_national_identifier` SET TAGS ('dbx_business_glossary_term' = 'Subject National Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_national_identifier` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_national_identifier` SET TAGS ('dbx_pii_national_id' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_type` SET TAGS ('dbx_business_glossary_term' = 'Subject Type');
ALTER TABLE `legal_ecm`.`compliance`.`sanctions_check` ALTER COLUMN `subject_type` SET TAGS ('dbx_value_regex' = 'individual|organisation|beneficial_owner|counterparty|related_party|trust');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `regulatory_change_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change ID');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `control_framework_id` SET TAGS ('dbx_business_glossary_term' = 'Impacted Control Framework Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `implementation_owner_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Implementation Owner Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `primary_regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation ID');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Implementation Owner Employee ID');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `change_description` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change Description');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `change_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Change Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `change_status` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change Status');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `change_status` SET TAGS ('dbx_value_regex' = 'proposed|published|effective|superseded|withdrawn');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `change_title` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change Title');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `change_type` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Change Type');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `consultation_response_deadline` SET TAGS ('dbx_business_glossary_term' = 'Consultation Response Deadline');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `consultation_response_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Consultation Response Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `dpo_review_date` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `dpo_review_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Data Protection Officer (DPO) Review Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Effective Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `governing_body` SET TAGS ('dbx_business_glossary_term' = 'Governing Body');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `impact_assessment_date` SET TAGS ('dbx_business_glossary_term' = 'Impact Assessment Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `impact_assessment_status` SET TAGS ('dbx_business_glossary_term' = 'Impact Assessment Status');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `impact_assessment_status` SET TAGS ('dbx_value_regex' = 'not_started|in_progress|completed|approved');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `impact_severity` SET TAGS ('dbx_business_glossary_term' = 'Impact Severity');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `impact_severity` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low|negligible');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `impacted_business_functions` SET TAGS ('dbx_business_glossary_term' = 'Impacted Business Functions');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `implementation_deadline` SET TAGS ('dbx_business_glossary_term' = 'Implementation Deadline');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `legal_counsel_review_date` SET TAGS ('dbx_business_glossary_term' = 'Legal Counsel Review Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `legal_counsel_review_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Legal Counsel Review Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `policy_update_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Policy Update Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `publication_date` SET TAGS ('dbx_business_glossary_term' = 'Publication Date');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `regulatory_obligation_created_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Created Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `related_policy_references` SET TAGS ('dbx_business_glossary_term' = 'Related Policy References');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `required_control_updates` SET TAGS ('dbx_business_glossary_term' = 'Required Control Updates');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `responsible_department` SET TAGS ('dbx_business_glossary_term' = 'Responsible Department');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `source_document_reference` SET TAGS ('dbx_business_glossary_term' = 'Source Document Reference');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `source_document_url` SET TAGS ('dbx_business_glossary_term' = 'Source Document Uniform Resource Locator (URL)');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `system_changes_description` SET TAGS ('dbx_business_glossary_term' = 'System Changes Description');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `system_changes_required_flag` SET TAGS ('dbx_business_glossary_term' = 'System Changes Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `training_completion_deadline` SET TAGS ('dbx_business_glossary_term' = 'Training Completion Deadline');
ALTER TABLE `legal_ecm`.`compliance`.`regulatory_change` ALTER COLUMN `training_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Training Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`policy` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`compliance`.`policy` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
...
ALTER TABLE `legal_ecm`.`compliance`.`policy` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Owner Employee ID');
...
ALTER TABLE `legal_ecm`.`compliance`.`policy` ALTER COLUMN `related_policy_references` SET TAGS ('dbx_business_glossary_term' = 'Related Policy References');
...
ALTER TABLE `legal_ecm`.`compliance`.`policy` ALTER COLUMN `superseded_policy_code` SET TAGS ('dbx_business_glossary_term' = 'Superseded Policy Code');
...
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` SET TAGS ('dbx_subdomain' = 'audit_assurance');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_engagement_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Engagement ID');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `control_framework_id` SET TAGS ('dbx_business_glossary_term' = 'Control Framework ID');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `lead_auditor_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Lead Auditor Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `practice_area_id` SET TAGS ('dbx_business_glossary_term' = 'Practice Area Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `responsible_officer_employee_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Responsible Officer Employee ID');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `responsible_officer_employee_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `responsible_officer_employee_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Lead Auditor Employee ID');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_documentation_location` SET TAGS ('dbx_business_glossary_term' = 'Audit Documentation Location');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Period End Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Period Start Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_scope` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_status` SET TAGS ('dbx_business_glossary_term' = 'Audit Status');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_status` SET TAGS ('dbx_value_regex' = 'planned|in_progress|fieldwork_complete|draft_issued|final_issued|closed');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_type` SET TAGS ('dbx_business_glossary_term' = 'Audit Type');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `audit_type` SET TAGS ('dbx_value_regex' = 'internal_compliance_review|external_regulatory_inspection|iso_27001_surveillance|aml_audit|gdpr_audit|sox_compliance_audit');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `auditor_type` SET TAGS ('dbx_business_glossary_term' = 'Auditor Type');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `auditor_type` SET TAGS ('dbx_value_regex' = 'internal|external|regulatory_body');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `critical_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Critical Findings Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `draft_report_date` SET TAGS ('dbx_business_glossary_term' = 'Draft Report Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `engagement_name` SET TAGS ('dbx_business_glossary_term' = 'Engagement Name');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `engagement_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Engagement Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `external_auditor_name` SET TAGS ('dbx_business_glossary_term' = 'External Auditor Name');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `fieldwork_end_date` SET TAGS ('dbx_business_glossary_term' = 'Fieldwork End Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `fieldwork_start_date` SET TAGS ('dbx_business_glossary_term' = 'Fieldwork Start Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `final_report_date` SET TAGS ('dbx_business_glossary_term' = 'Final Report Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `findings_closed_count` SET TAGS ('dbx_business_glossary_term' = 'Findings Closed Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `findings_open_count` SET TAGS ('dbx_business_glossary_term' = 'Findings Open Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `findings_overdue_count` SET TAGS ('dbx_business_glossary_term' = 'Findings Overdue Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `follow_up_audit_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Audit Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `follow_up_audit_scheduled_date` SET TAGS ('dbx_business_glossary_term' = 'Follow-Up Audit Scheduled Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `high_findings_count` SET TAGS ('dbx_business_glossary_term' = 'High Findings Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `low_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Low Findings Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `management_response_due_date` SET TAGS ('dbx_business_glossary_term' = 'Management Response Due Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `management_response_received_date` SET TAGS ('dbx_business_glossary_term' = 'Management Response Received Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `medium_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Medium Findings Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `overall_audit_opinion` SET TAGS ('dbx_business_glossary_term' = 'Overall Audit Opinion');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `overall_audit_opinion` SET TAGS ('dbx_value_regex' = 'satisfactory|satisfactory_with_observations|needs_improvement|unsatisfactory|adverse');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `regulatory_report_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Reference');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `regulatory_report_submission_date` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Submission Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `responsible_department` SET TAGS ('dbx_business_glossary_term' = 'Responsible Department');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `responsible_officer_name` SET TAGS ('dbx_business_glossary_term' = 'Responsible Officer Name');
ALTER TABLE `legal_ecm`.`compliance`.`audit_engagement` ALTER COLUMN `total_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Total Findings Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` SET TAGS ('dbx_subdomain' = 'audit_assurance');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `audit_finding_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Finding Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `action_owner_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Action Owner Timekeeper Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `audit_engagement_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Engagement Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `auditor_employee_timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Auditor Employee Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `auditor_employee_timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `auditor_employee_timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `compliance_control_id` SET TAGS ('dbx_business_glossary_term' = 'Compliance Control Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `ip_asset_id` SET TAGS ('dbx_business_glossary_term' = 'Ip Asset Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Finding Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_business_glossary_term' = 'Action Owner Employee Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `timekeeper_id` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Violated Policy Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `actual_closure_date` SET TAGS ('dbx_business_glossary_term' = 'Actual Closure Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `agreed_action` SET TAGS ('dbx_business_glossary_term' = 'Agreed Management Action');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `auditor_name` SET TAGS ('dbx_business_glossary_term' = 'Auditor Name');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `control_reference` SET TAGS ('dbx_business_glossary_term' = 'Control Reference');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `escalation_date` SET TAGS ('dbx_business_glossary_term' = 'Escalation Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `escalation_recipient` SET TAGS ('dbx_business_glossary_term' = 'Escalation Recipient');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `escalation_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalation Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `evidence_reference` SET TAGS ('dbx_business_glossary_term' = 'Evidence Reference');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_category` SET TAGS ('dbx_business_glossary_term' = 'Finding Category');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_category` SET TAGS ('dbx_value_regex' = 'control_gap|policy_breach|regulatory_non_compliance|process_deficiency|documentation_gap|system_weakness');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_description` SET TAGS ('dbx_business_glossary_term' = 'Finding Description');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_raised_date` SET TAGS ('dbx_business_glossary_term' = 'Finding Raised Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_reference_number` SET TAGS ('dbx_business_glossary_term' = 'Finding Reference Number');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_status` SET TAGS ('dbx_business_glossary_term' = 'Finding Status');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_status` SET TAGS ('dbx_value_regex' = 'open|in_progress|pending_verification|closed|overdue');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_title` SET TAGS ('dbx_business_glossary_term' = 'Finding Title');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_type` SET TAGS ('dbx_business_glossary_term' = 'Finding Type');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `finding_type` SET TAGS ('dbx_value_regex' = 'observation|recommendation|non_conformance|opportunity_for_improvement');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `impact_assessment` SET TAGS ('dbx_business_glossary_term' = 'Impact Assessment');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active Flag');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `management_response` SET TAGS ('dbx_business_glossary_term' = 'Management Response');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `recommended_action` SET TAGS ('dbx_business_glossary_term' = 'Recommended Action');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `regulatory_framework` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Framework');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `regulatory_report_reference` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Report Reference');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `regulatory_reporting_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Reporting Required Flag');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `related_policy_reference` SET TAGS ('dbx_business_glossary_term' = 'Related Policy Reference');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `responsible_department` SET TAGS ('dbx_business_glossary_term' = 'Responsible Department');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `risk_rating` SET TAGS ('dbx_value_regex' = 'very_high|high|medium|low|very_low');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `root_cause` SET TAGS ('dbx_business_glossary_term' = 'Root Cause Analysis');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `severity` SET TAGS ('dbx_business_glossary_term' = 'Finding Severity');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `severity` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `target_remediation_date` SET TAGS ('dbx_business_glossary_term' = 'Target Remediation Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `verification_date` SET TAGS ('dbx_business_glossary_term' = 'Verification Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `verification_notes` SET TAGS ('dbx_business_glossary_term' = 'Verification Notes');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `verification_status` SET TAGS ('dbx_business_glossary_term' = 'Verification Status');
ALTER TABLE `legal_ecm`.`compliance`.`audit_finding` ALTER COLUMN `verification_status` SET TAGS ('dbx_value_regex' = 'not_started|scheduled|in_progress|verified|failed_verification');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` SET TAGS ('dbx_subdomain' = 'audit_assurance');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` SET TAGS ('dbx_association_edges' = 'compliance.audit_engagement,compliance.regulatory_obligation');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `audit_scope_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `audit_engagement_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope - Audit Engagement Id');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Scope - Regulatory Obligation Id');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `audit_period_end_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Period End Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `audit_period_start_date` SET TAGS ('dbx_business_glossary_term' = 'Audit Period Start Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `auditor_notes` SET TAGS ('dbx_business_glossary_term' = 'Auditor Notes');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `exceptions_identified` SET TAGS ('dbx_business_glossary_term' = 'Exceptions Identified');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `obligation_compliance_status` SET TAGS ('dbx_business_glossary_term' = 'Obligation Compliance Status');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `obligation_findings_count` SET TAGS ('dbx_business_glossary_term' = 'Obligation Findings Count');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `obligation_risk_rating` SET TAGS ('dbx_business_glossary_term' = 'Obligation Risk Rating');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `obligation_test_results_summary` SET TAGS ('dbx_business_glossary_term' = 'Obligation Test Results Summary');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `sample_size` SET TAGS ('dbx_business_glossary_term' = 'Sample Size');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `scope_inclusion_date` SET TAGS ('dbx_business_glossary_term' = 'Scope Inclusion Date');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `scope_status` SET TAGS ('dbx_business_glossary_term' = 'Scope Status');
ALTER TABLE `legal_ecm`.`compliance`.`audit_scope` ALTER COLUMN `testing_approach` SET TAGS ('dbx_business_glossary_term' = 'Testing Approach');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` SET TAGS ('dbx_subdomain' = 'regulatory_oversight');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` SET TAGS ('dbx_association_edges' = 'client.client_profile,compliance.policy');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `policy_acknowledgement_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Acknowledgement Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Acknowledgement - Compliance Policy Id');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Acknowledgement - Client Profile Id');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acceptance_status` SET TAGS ('dbx_business_glossary_term' = 'Acceptance Status');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledged_by_contact_email` SET TAGS ('dbx_business_glossary_term' = 'Acknowledged By Contact Email');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledged_by_contact_email` SET TAGS ('dbx_pii' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledged_by_contact_name` SET TAGS ('dbx_business_glossary_term' = 'Acknowledged By Contact Name');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledged_by_contact_name` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledged_by_contact_name` SET TAGS ('dbx_pii_identifier' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledgement_date` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgement Date');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `acknowledgement_document_reference` SET TAGS ('dbx_business_glossary_term' = 'Acknowledgement Document Reference');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `attestation_method` SET TAGS ('dbx_business_glossary_term' = 'Attestation Method');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `next_acknowledgement_due_date` SET TAGS ('dbx_business_glossary_term' = 'Next Acknowledgement Due Date');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `reminder_sent_date` SET TAGS ('dbx_business_glossary_term' = 'Reminder Sent Date');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `required_for_engagement_flag` SET TAGS ('dbx_business_glossary_term' = 'Required for Engagement Flag');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Updated Timestamp');
ALTER TABLE `legal_ecm`.`compliance`.`policy_acknowledgement` ALTER COLUMN `version_acknowledged` SET TAGS ('dbx_business_glossary_term' = 'Policy Version Acknowledged');
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` SET TAGS ('dbx_subdomain' = 'audit_assurance');
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` ALTER COLUMN `audit_program_id` SET TAGS ('dbx_business_glossary_term' = 'Audit Program Identifier');
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` ALTER COLUMN `control_framework_id` SET TAGS ('dbx_business_glossary_term' = 'Target Control Framework Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` ALTER COLUMN `parent_audit_program_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `legal_ecm`.`compliance`.`audit_program` ALTER COLUMN `budget_amount` SET TAGS ('dbx_confidential' = 'true');
