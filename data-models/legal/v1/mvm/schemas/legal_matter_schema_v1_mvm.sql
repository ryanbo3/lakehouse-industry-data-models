-- Schema for Domain: matter | Business: Legal | Version: v1_mvm
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`tribunal` (
    ...
    `jurisdiction_id` BIGINT COMMENT 'Foreign key linking to matter.jurisdiction. Business justification: A tribunal operates within a specific legal jurisdiction. The tribunal table currently stores jurisdiction_geographic as a denormalized STRING. Adding jurisdiction_id FK to matter.jurisdiction normali',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`docket` (
    ...
    `indemnity_claim_id` BIGINT COMMENT 'Foreign key linking to compliance.indemnity_claim. Business justification: A docket is the court proceeding record for an indemnity claim against the firm. Linking docket directly to indemnity_claim enables claims handlers to navigate from the insurance claim to the active c',
    `indemnity_policy_id` BIGINT COMMENT 'Foreign key linking to compliance.indemnity_policy. Business justification: When a docket represents litigation against the firm, the specific indemnity policy covering that proceeding must be identified for insurer notification and coverage confirmation. Docket is the primar',
    `jurisdiction_id` BIGINT COMMENT 'Foreign key linking to matter.jurisdiction. Business justification: A docket is filed in a specific legal jurisdiction (e.g., Southern District of New York, California Superior Court). The docket table currently stores jurisdiction as a denormalized STRING. Adding jur',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: A docket may be opened specifically for a regulatory enforcement action tied to a named regulatory obligation (e.g., FCA enforcement, SEC investigation). Compliance teams need to link the court procee',
    `request_id` BIGINT COMMENT 'Foreign key linking to intake.request. Business justification: Matter lifecycle traceability: the intake request is the upstream trigger for docket creation. Linking docket to its originating intake request supports matter lifecycle audit trails, intake-to-litiga',
    `retainer_agreement_id` BIGINT COMMENT 'Foreign key linking to trust.retainer_agreement. Business justification: Active litigation dockets are funded by specific retainer agreements governing trust fund usage for that proceeding. Linking docket to retainer_agreement enables docket-level trust balance monitoring ',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`hearing` (
    `hearing_id` BIGINT COMMENT 'Primary key for hearing',
    `jurisdiction_id` BIGINT COMMENT 'Reference to the court or tribunal where the hearing is scheduled.',
    `docket_id` BIGINT COMMENT 'Foreign key linking to matter.docket. Business justification: A court hearing is scheduled on a specific docket. The hearing table currently has matter_id and judge_id but no direct docket_id FK, despite having pacer_case_number as a denormalized reference. Addi',
    ...
    `legal_document_id` BIGINT COMMENT 'Foreign key linking to document.legal_document. Business justification: Hearings produce and reference specific legal documents (transcripts, rulings, motions). Attorneys need direct navigation from a hearing record to its primary document for appellate preparation and ca',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: A hearing may be scheduled specifically to adjudicate a regulatory enforcement matter tied to a named regulatory obligation (e.g., FCA enforcement hearing, tribunal hearing on regulatory breach). Link',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`appearance` (
    ...
    `judge_id` BIGINT COMMENT 'Foreign key linking to matter.judge. Business justification: An attorney appearance is made before a specific judicial officer, arbitrator, or mediator. The appearance table currently stores judge_name as a denormalized STRING. Adding judge_id FK to matter.judg',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`filing` (
    ...
    `agreement_id` BIGINT COMMENT 'Foreign key linking to contract.contract_agreement. Business justification: Court filings of contract documents (settlement agreements, consent orders, regulatory contract filings) require tracing the filing back to the specific contract agreement. Legal professionals and cou',
    `amended_filing_id` BIGINT COMMENT 'Reference to the original filing that this filing amends or supersedes, if applicable.',
    `doc_version_id` BIGINT COMMENT 'Foreign key linking to document.doc_version. Business justification: Court filings must reference the exact document version submitted for ECF compliance, amendment tracking, and responding to court inquiries. filing already links to legal_document but not to the speci',
    `docket_id` BIGINT COMMENT 'Foreign key linking to matter.docket. Business justification: Every court filing is made on a specific docket. The filing table currently has tribunal_id and matter_id but no direct docket_id FK, despite having a docket_entry_number STRING field. Adding docket_i',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Court filings may be required to satisfy a specific regulatory obligation (e.g., mandatory regulatory disclosure filing, statutory notice filing). Linking filing to regulatory_obligation enables compl',
    `tribunal_id` BIGINT COMMENT 'Reference to the court, tribunal, or arbitral body where the document was filed.',
    `trust_disbursement_id` BIGINT COMMENT 'Foreign key linking to trust.trust_disbursement. Business justification: Court filing fees are routinely paid from client trust accounts via trust disbursements. Linking filing to the trust_disbursement that funded it enables trust accounting reconciliation and confirms wh',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`deadline` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to client.contact. Business justification: Critical deadlines often require specific client contact action (document production, settlement authority, filing approvals). This link enables targeted deadline notifications, tracks client responsi',
    `court_rule_id` BIGINT COMMENT 'Foreign key linking to matter.court_rule. Business justification: Matter deadlines are frequently governed by specific procedural court rules (e.g., FRCP Rule 26 disclosure deadlines, local rules for motion practice). The matter_deadline table currently stores court',
    ...
    `legal_document_id` BIGINT COMMENT 'Foreign key linking to document.legal_document. Business justification: Deadlines are triggered by or responsive to specific documents (e.g., complaint triggers answer deadline). matter_deadline.document_reference is a plain-text denormalization. A proper FK enables autom',
    `matter_id` BIGINT COMMENT 'Reference to the internal matter associated with this deadline.',
    `obligation_id` BIGINT COMMENT 'Foreign key linking to contract.obligation. Business justification: Contractual obligation due dates are formally docketed as matter deadlines for malpractice risk management. matter_deadline has regulatory_obligation_id (compliance domain) but no FK to contract.oblig',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`order` (
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: A court order (consent decree, injunction, compliance order) may impose or directly reference a regulatory obligation. Linking order to regulatory_obligation enables compliance teams to track court-im',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`court_rule` (
    ...
    `doc_template_id` BIGINT COMMENT 'Foreign key linking to document.doc_template. Business justification: Court rules mandate specific document templates and mandatory forms. court_rule.mandatory_form_reference is a plain-text denormalization. A proper FK enables automated template selection during filing',
    `jurisdiction_id` BIGINT COMMENT 'Foreign key linking to matter.jurisdiction. Business justification: Court rules and procedural rules apply within specific legal jurisdictions. The court_rule table currently stores jurisdiction_scope as a denormalized STRING. Adding jurisdiction_id FK to matter.juris',
    `legal_document_id` BIGINT COMMENT 'Foreign key linking to document.legal_document. Business justification: Court rules are stored as legal documents (local rules PDFs, standing orders). Linking court_rule to its stored document enables version-controlled access to rule text, supports deadline calculation v',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`judgment` (
    ...
    `escrow_arrangement_id` BIGINT COMMENT 'Foreign key linking to trust.escrow_arrangement. Business justification: Consent judgments and settlement judgments routinely establish or release escrow arrangements. Linking judgment to escrow_arrangement allows tracking of escrow funds tied to specific court judgments —',
    `judge_id` BIGINT COMMENT 'FK to court.judge',
    ...
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: A judgment in a regulatory enforcement matter directly resolves, modifies, or creates a regulatory obligation (e.g., consent decree imposing ongoing compliance requirements). Linking judgment to regul',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`matter` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to client.client_contact. Business justification: Every matter has a designated client-side contact responsible for receiving status updates, approving budgets, and signing off on strategy. This is a fundamental legal matter management requirement. m',
    `engagement_scope_id` BIGINT COMMENT 'Foreign key linking to client.engagement_scope. Business justification: Matters operate under specific engagement scopes (panel appointments, rate agreements, authorized practice areas). This link enforces billing compliance (rate caps, LEDES requirements), validates matt',
    ...
    `jurisdiction_id` BIGINT COMMENT 'Foreign key linking to matter.jurisdiction. Business justification: Every legal matter is governed by a specific legal jurisdiction (governing law). The matter table currently stores jurisdiction as a denormalized STRING. Adding jurisdiction_id FK to matter.jurisdicti',
    `legal_service_id` BIGINT COMMENT 'Foreign key linking to service.legal_service. Business justification: Matter opening requires classification against a service catalog entry to drive SLA obligations, conflict check requirements, KYC rules, billing code assignment, and service-line revenue reporting. Ev',
    `outside_counsel_guideline_id` BIGINT COMMENT 'Foreign key linking to client.outside_counsel_guideline. Business justification: Each matter must be governed by the clients OCG specifying rate caps, staffing restrictions, and budget approval thresholds. A direct FK enables matter-level OCG enforcement without joining through c',
    ...
    `referral_source_id` BIGINT COMMENT 'Foreign key linking to intake.referral_source. Business justification: Referral fee management and business development ROI: law firms must track which referral source originated each matter to calculate referral fees, comply with referral agreements, and measure referra',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`phase` (
    ...
    `lpm_template_id` BIGINT COMMENT 'Foreign key linking to service.lpm_template. Business justification: Matter phases are instantiated from LPM templates that define standard milestones, budgets, task counts, and resource requirements. Legal project management depends on this link to track template util',
    ...
    `register_id` BIGINT COMMENT 'Foreign key linking to risk.register. Business justification: Matter phases carry distinct risk profiles requiring phase-level risk register entries (e.g., discovery phase: e-discovery cost overrun risk; trial phase: adverse judgment risk). Phase-gate risk revie',
    `sla_template_id` BIGINT COMMENT 'Foreign key linking to service.sla_template. Business justification: Individual matter phases are governed by phase-specific SLA commitments (e.g., due diligence phase: 5-day turnaround). Linking phase to sla_template enables phase-level SLA breach monitoring, client r',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`task` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to client.contact. Business justification: Tasks often require specific client contact involvement (document requests, approval signatures, information gathering). This link enables targeted task assignment notifications, tracks client deliver',
    ...
    `legal_document_id` BIGINT COMMENT 'Foreign key linking to document.legal_document. Business justification: Matter tasks produce deliverable documents (drafted briefs, contracts, filings). Linking tasks to their output documents enables work product tracking, billing verification against deliverables, and c',
    ...
    `obligation_id` BIGINT COMMENT 'Foreign key linking to contract.obligation. Business justification: Tasks are created to fulfill specific contractual obligations — a named legal project management process. Linking task to obligation enables obligation fulfillment tracking, percentage completion repo',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`team` (
    ...
    `ethical_wall_id` BIGINT COMMENT 'Foreign key linking to conflict.ethical_wall. Business justification: Team member assignments must enforce specific ethical walls. Linking team→ethical_wall enables direct system enforcement queries: which wall restricts this timekeeper on this matter? Team already ha',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`budget` (
    ...
    `fee_arrangement_id` BIGINT COMMENT 'Foreign key linking to billing.fee_arrangement. Business justification: Budget vs. billing arrangement variance reporting is a core financial management process in legal operations. Firms compare matter budget performance (internal cost planning) against fee arrangement c',
    `matter_id` BIGINT COMMENT 'Reference to the parent matter for which this budget and rate schedule applies.',
    `outside_counsel_guideline_id` BIGINT COMMENT 'Foreign key linking to client.outside_counsel_guideline. Business justification: Matter budgets must be validated against OCG rate caps, budget approval thresholds, and billing format requirements. Legal operations and billing teams require this direct link to enforce OCG constrai',
    ...
    `rate_card_id` BIGINT COMMENT 'Foreign key linking to service.rate_card. Business justification: Budgets are constructed from rate cards that define approved hourly rates and billing parameters. Budget approval, variance analysis, and rate compliance reporting all require knowing which rate card ',
    `retainer_agreement_id` BIGINT COMMENT 'Foreign key linking to trust.retainer_agreement. Business justification: Matter budgets are funded by retainer agreements; linking budget to retainer_agreement enables real-time comparison of approved budget against available retainer balance. Legal finance teams use this ',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`matter_party` (
    ...
    `aml_kyc_program_id` BIGINT COMMENT 'Foreign key linking to compliance.aml_kyc_program. Business justification: Each matter party undergoes AML/KYC screening under a specific AML/KYC program. Linking matter_party to aml_kyc_program records which program governed the due diligence, enabling MLRO oversight, regul',
    ...
    `conflict_party_id` BIGINT COMMENT 'Foreign key linking to conflict.conflict_party. Business justification: A matter party IS the conflict_party screened during intake. Linking matter_party→conflict_party enables ongoing conflict monitoring: when a new matter opens involving the same party, the system can i',
    `kyc_screening_id` BIGINT COMMENT 'Foreign key linking to intake.kyc_screening. Business justification: AML/KYC regulatory compliance: law firms must confirm every matter party has a completed KYC screening before engagement proceeds. Linking matter_party directly to kyc_screening supports compliance re',
    ...
    `register_id` BIGINT COMMENT 'Foreign key linking to risk.register. Business justification: Parties (especially adverse parties, beneficial owners, PEPs) have associated risk register entries for AML, sanctions, and reputational risk. matter_party already carries kyc_risk_rating, pep_status,',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`hold` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to client.contact. Business justification: Legal holds identify client contacts as custodians of relevant data. This link enables custodian acknowledgement tracking, targeted hold notifications, and custodian interview scheduling. Essential fo',
    `data_breach_incident_id` BIGINT COMMENT 'Foreign key linking to risk.data_breach_incident. Business justification: Legal holds are routinely issued in direct response to data breach incidents (GDPR/CCPA preservation obligations, regulatory investigation holds). The hold record must reference the triggering data br',
    `docket_id` BIGINT COMMENT 'Foreign key linking to matter.docket. Business justification: A litigation hold is typically issued in connection with a specific court case/docket. The hold table currently stores court_case_number as a denormalized STRING. Adding docket_id FK to matter.docket ',
    `matter_id` BIGINT COMMENT 'Reference to the parent matter for which this hold was issued.',
    `policy_id` BIGINT COMMENT 'Foreign key linking to compliance.policy. Business justification: A litigation hold is governed by the firms data retention and preservation policy. Linking hold to the applicable compliance policy ensures holds are issued in accordance with firm policy, supports p',
    `register_id` BIGINT COMMENT 'Foreign key linking to risk.register. Business justification: Legal holds are risk mitigation instruments — they are issued in response to identified risks (litigation risk, regulatory investigation risk, spoliation risk). Linking holds to the risk register entr',
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: Litigation holds are frequently triggered by specific regulatory obligations (e.g., SEC document preservation rules, GDPR data subject access requests). Linking hold to regulatory_obligation enables c',
    `request_id` BIGINT COMMENT 'Foreign key linking to intake.request. Business justification: Legal hold audit trail and litigation readiness: a litigation hold is frequently triggered by an intake request identifying litigation risk. Linking hold to the originating intake request demonstrates',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`outcome` (
    ...
    `escrow_arrangement_id` BIGINT COMMENT 'Foreign key linking to trust.escrow_arrangement. Business justification: Matter outcomes (settlements, consent judgments) frequently trigger release or closure of escrow arrangements holding disputed funds. Linking outcome to escrow_arrangement enables tracking which escro',
    ...
    `legal_document_id` BIGINT COMMENT 'Foreign key linking to document.legal_document. Business justification: Outcomes such as settlement agreements, consent orders, and dismissal orders are formal legal documents requiring direct linkage for enforcement tracking and matter closure. Not all outcomes are judgm',
    `matter_id` BIGINT COMMENT 'Reference to the parent matter for which this outcome is recorded.',
    `pi_claim_id` BIGINT COMMENT 'Foreign key linking to risk.pi_claim. Business justification: Adverse matter outcomes (missed deadlines, wrong advice, procedural errors) are the direct predicate for professional indemnity claims. Legal risk managers link outcome records to PI claims for root-c',
    `regulatory_obligation_id` BIGINT COMMENT 'Foreign key linking to compliance.regulatory_obligation. Business justification: A matter outcome (regulatory settlement, consent order, acquittal) directly resolves or modifies a specific regulatory obligation. Compliance teams must track which regulatory obligations are closed o',
    `invoice_id` BIGINT COMMENT 'Foreign key linking to billing.invoice. Business justification: Matter outcomes with contingent_fee_triggered_flag=true generate a specific contingency fee invoice. Legal billing requires linking the outcome event to the triggered invoice for contingency fee audit',
    `trust_disbursement_id` BIGINT COMMENT 'Foreign key linking to trust.trust_disbursement. Business justification: Settlement and judgment outcomes trigger disbursement of funds from trust to opposing parties or clients. Linking outcome to the trust_disbursement that executed the settlement payment is essential fo',
...
CREATE OR REPLACE TABLE `legal_ecm`.`matter`.`assignment` (
    ...
    `clearance_id` BIGINT COMMENT 'Foreign key linking to conflict.clearance. Business justification: Attorney assignments require conflict clearance before proceeding. The sibling product team already carries conflict_clearance_id→clearance for the same business reason. Assignment lacks this link, ',
    `matter_id` BIGINT COMMENT 'Foreign key linking to the legal matter to which the timekeeper is assigned',
    `timekeeper_rate_id` BIGINT COMMENT 'Foreign key linking to billing.timekeeper_rate. Business justification: When a timekeeper is assigned to a matter, their applicable billing rate is determined by a timekeeper_rate record. assignment.billing_rate_override implies rate management at assignment level; linkin',
...
-- ========= FOREIGN KEYS =========
ALTER TABLE `legal_ecm`.`matter`.`tribunal` ADD CONSTRAINT `fk_matter_tribunal_jurisdiction_id` FOREIGN KEY (`jurisdiction_id`) REFERENCES `legal_ecm`.`matter`.`jurisdiction`(`jurisdiction_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`docket` ADD CONSTRAINT `fk_matter_docket_jurisdiction_id` FOREIGN KEY (`jurisdiction_id`) REFERENCES `legal_ecm`.`matter`.`jurisdiction`(`jurisdiction_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`hearing` ADD CONSTRAINT `fk_matter_hearing_docket_id` FOREIGN KEY (`docket_id`) REFERENCES `legal_ecm`.`matter`.`docket`(`docket_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`appearance` ADD CONSTRAINT `fk_matter_appearance_judge_id` FOREIGN KEY (`judge_id`) REFERENCES `legal_ecm`.`matter`.`judge`(`judge_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`filing` ADD CONSTRAINT `fk_matter_filing_docket_id` FOREIGN KEY (`docket_id`) REFERENCES `legal_ecm`.`matter`.`docket`(`docket_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_court_rule_id` FOREIGN KEY (`court_rule_id`) REFERENCES `legal_ecm`.`matter`.`court_rule`(`court_rule_id`);
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_docket_id` FOREIGN KEY (`docket_id`) REFERENCES `legal_ecm`.`matter`.`docket`(`docket_id`);
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_hearing_id` FOREIGN KEY (`hearing_id`) REFERENCES `legal_ecm`.`matter`.`hearing`(`hearing_id`);
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_judge_id` FOREIGN KEY (`judge_id`) REFERENCES `legal_ecm`.`matter`.`judge`(`judge_id`);
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_matter_id` FOREIGN KEY (`matter_id`) REFERENCES `legal_ecm`.`matter`.`matter`(`matter_id`);
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_order_id` FOREIGN KEY (`order_id`) REFERENCES `legal_ecm`.`matter`.`order`(`order_id`);
ALTER TABLE `legal_ecm`.`matter`.`deadline` ADD CONSTRAINT `fk_matter_deadline_tribunal_id` FOREIGN KEY (`tribunal_id`) REFERENCES `legal_ecm`.`matter`.`tribunal`(`tribunal_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`court_rule` ADD CONSTRAINT `fk_matter_court_rule_jurisdiction_id` FOREIGN KEY (`jurisdiction_id`) REFERENCES `legal_ecm`.`matter`.`jurisdiction`(`jurisdiction_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`matter` ADD CONSTRAINT `fk_matter_matter_jurisdiction_id` FOREIGN KEY (`jurisdiction_id`) REFERENCES `legal_ecm`.`matter`.`jurisdiction`(`jurisdiction_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`hold` ADD CONSTRAINT `fk_matter_hold_docket_id` FOREIGN KEY (`docket_id`) REFERENCES `legal_ecm`.`matter`.`docket`(`docket_id`);
...
ALTER TABLE `legal_ecm`.`matter`.`tribunal` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`tribunal` ALTER COLUMN `jurisdiction_id` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`jurisdiction` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`judge` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`docket` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`docket` ALTER COLUMN `indemnity_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Indemnity Claim Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`docket` ALTER COLUMN `indemnity_policy_id` SET TAGS ('dbx_business_glossary_term' = 'Indemnity Policy Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`docket` ALTER COLUMN `jurisdiction_id` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`docket` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`docket` ALTER COLUMN `request_id` SET TAGS ('dbx_business_glossary_term' = 'Request Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`docket` ALTER COLUMN `retainer_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Retainer Agreement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`hearing` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`hearing` ALTER COLUMN `docket_id` SET TAGS ('dbx_business_glossary_term' = 'Docket Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`hearing` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Document Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`hearing` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`appearance` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`appearance` ALTER COLUMN `judge_id` SET TAGS ('dbx_business_glossary_term' = 'Judge Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`filing` SET TAGS ('dbx_subdomain' = 'court_proceedings');
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `filing_id` SET TAGS ('dbx_business_glossary_term' = 'Filing Identifier');
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Contract Agreement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `doc_version_id` SET TAGS ('dbx_business_glossary_term' = 'Doc Version Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `docket_id` SET TAGS ('dbx_business_glossary_term' = 'Docket Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `tribunal_id` SET TAGS ('dbx_business_glossary_term' = 'Court Identifier');
ALTER TABLE `legal_ecm`.`matter`.`filing` ALTER COLUMN `trust_disbursement_id` SET TAGS ('dbx_business_glossary_term' = 'Trust Disbursement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`deadline` SET TAGS ('dbx_subdomain' = 'court_proceedings');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `deadline_id` SET TAGS ('dbx_business_glossary_term' = 'Deadline Identifier');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Client Contact Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `court_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Court Rule Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `docket_id` SET TAGS ('dbx_business_glossary_term' = 'Docket Identifier (ID)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `hearing_id` SET TAGS ('dbx_business_glossary_term' = 'Hearing Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `judge_id` SET TAGS ('dbx_business_glossary_term' = 'Judge Identifier (ID)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Identifier (ID)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `order_id` SET TAGS ('dbx_business_glossary_term' = 'Order Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `profile_id` SET TAGS ('dbx_business_glossary_term' = 'Client Profile Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Risk Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `tribunal_id` SET TAGS ('dbx_business_glossary_term' = 'Court Identifier (ID)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `adr_proceeding_flag` SET TAGS ('dbx_business_glossary_term' = 'Alternative Dispute Resolution (ADR) Proceeding Flag');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `adr_proceeding_type` SET TAGS ('dbx_business_glossary_term' = 'Alternative Dispute Resolution (ADR) Proceeding Type');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `adr_proceeding_type` SET TAGS ('dbx_value_regex' = 'arbitration|mediation|settlement_conference|early_neutral_evaluation');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `alert_days_before` SET TAGS ('dbx_business_glossary_term' = 'Alert Days Before');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `alert_sent_flag` SET TAGS ('dbx_business_glossary_term' = 'Alert Sent Flag');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `alert_sent_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Alert Sent Timestamp');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `calendar_entry_reference` SET TAGS ('dbx_business_glossary_term' = 'Calendar Entry Identifier (ID)');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `calendar_sync_status` SET TAGS ('dbx_business_glossary_term' = 'Calendar Synchronization Status');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `calendar_sync_status` SET TAGS ('dbx_value_regex' = 'synced|pending|failed|not_synced');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `calendar_sync_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Calendar Synchronization Timestamp');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `client_notified_date` SET TAGS ('dbx_business_glossary_term' = 'Client Notified Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `completion_date` SET TAGS ('dbx_business_glossary_term' = 'Completion Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `completion_notes` SET TAGS ('dbx_business_glossary_term' = 'Completion Notes');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `completion_status` SET TAGS ('dbx_business_glossary_term' = 'Completion Status');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `completion_status` SET TAGS ('dbx_value_regex' = 'pending|completed|missed|waived|extended');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `completion_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Completion Timestamp');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `court_case_number` SET TAGS ('dbx_business_glossary_term' = 'Court Case Number');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Created Timestamp');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Deadline Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `deadline_description` SET TAGS ('dbx_business_glossary_term' = 'Deadline Description');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `deadline_time` SET TAGS ('dbx_business_glossary_term' = 'Deadline Time');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `deadline_type` SET TAGS ('dbx_business_glossary_term' = 'Deadline Type');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `deadline_type` SET TAGS ('dbx_value_regex' = 'statute_of_limitations|court_ordered|regulatory_filing|contractual_notice|discovery_cutoff|motion_filing');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `due_date` SET TAGS ('dbx_business_glossary_term' = 'Due Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `due_time` SET TAGS ('dbx_business_glossary_term' = 'Due Time');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `ecf_filing_required_flag` SET TAGS ('dbx_business_glossary_term' = 'Electronic Case Filing (ECF) Filing Required Flag');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `escalation_flag` SET TAGS ('dbx_business_glossary_term' = 'Escalation Flag');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `extended_deadline_date` SET TAGS ('dbx_business_glossary_term' = 'Extended Deadline Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `extended_due_date` SET TAGS ('dbx_business_glossary_term' = 'Extended Due Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `extension_granted_date` SET TAGS ('dbx_business_glossary_term' = 'Extension Granted Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `extension_granted_flag` SET TAGS ('dbx_business_glossary_term' = 'Extension Granted Flag');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `extension_order_reference` SET TAGS ('dbx_business_glossary_term' = 'Extension Order Reference');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `filing_method` SET TAGS ('dbx_business_glossary_term' = 'Filing Method');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `filing_method` SET TAGS ('dbx_value_regex' = 'electronic|paper|hand_delivery|certified_mail');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `governing_rule_reference` SET TAGS ('dbx_business_glossary_term' = 'Governing Rule Reference');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `is_active` SET TAGS ('dbx_business_glossary_term' = 'Is Active');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `jurisdiction` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `last_modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Last Modified Timestamp');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `malpractice_insurance_notified` SET TAGS ('dbx_business_glossary_term' = 'Malpractice Insurance Notified');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `malpractice_risk_flag` SET TAGS ('dbx_business_glossary_term' = 'Malpractice Risk Flag');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `modified_by` SET TAGS ('dbx_business_glossary_term' = 'Modified By');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `modified_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Modified Timestamp');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Notes');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `opposing_counsel_name` SET TAGS ('dbx_business_glossary_term' = 'Opposing Counsel Name');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `pacer_case_number` SET TAGS ('dbx_business_glossary_term' = 'Public Access to Court Electronic Records (PACER) Case Number');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `priority_level` SET TAGS ('dbx_business_glossary_term' = 'Priority Level');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `priority_level` SET TAGS ('dbx_value_regex' = 'critical|high|medium|low');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `reminder_lead_time_days` SET TAGS ('dbx_business_glossary_term' = 'Reminder Lead Time Days');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `responsible_team` SET TAGS ('dbx_business_glossary_term' = 'Responsible Team');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `risk_level` SET TAGS ('dbx_business_glossary_term' = 'Risk Level');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `risk_level` SET TAGS ('dbx_value_regex' = 'high|medium|low');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `triggering_event` SET TAGS ('dbx_business_glossary_term' = 'Triggering Event');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `triggering_event_date` SET TAGS ('dbx_business_glossary_term' = 'Triggering Event Date');
ALTER TABLE `legal_ecm`.`matter`.`deadline` ALTER COLUMN `created_by` SET TAGS ('dbx_business_glossary_term' = 'Created By');
ALTER TABLE `legal_ecm`.`matter`.`order` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `legal_ecm`.`matter`.`order` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`order` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`court_rule` SET TAGS ('dbx_subdomain' = 'court_proceedings');
ALTER TABLE `legal_ecm`.`matter`.`court_rule` ALTER COLUMN `court_rule_id` SET TAGS ('dbx_business_glossary_term' = 'Court Rule ID');
ALTER TABLE `legal_ecm`.`matter`.`court_rule` ALTER COLUMN `doc_template_id` SET TAGS ('dbx_business_glossary_term' = 'Doc Template Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`court_rule` ALTER COLUMN `jurisdiction_id` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`court_rule` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Document Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`judgment` SET TAGS ('dbx_subdomain' = 'court_proceedings');
...
ALTER TABLE `legal_ecm`.`matter`.`judgment` ALTER COLUMN `escrow_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Escrow Arrangement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`judgment` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`matter` SET TAGS ('dbx_subdomain' = 'matter_management');
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Client Contact Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `engagement_scope_id` SET TAGS ('dbx_business_glossary_term' = 'Engagement Scope Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `jurisdiction_id` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `legal_service_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Service Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `outside_counsel_guideline_id` SET TAGS ('dbx_business_glossary_term' = 'Outside Counsel Guideline Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`matter` ALTER COLUMN `referral_source_id` SET TAGS ('dbx_business_glossary_term' = 'Referral Source Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`phase` SET TAGS ('dbx_subdomain' = 'matter_management');
ALTER TABLE `legal_ecm`.`matter`.`phase` ALTER COLUMN `phase_id` SET TAGS ('dbx_business_glossary_term' = 'Phase Identifier');
ALTER TABLE `legal_ecm`.`matter`.`phase` ALTER COLUMN `lpm_template_id` SET TAGS ('dbx_business_glossary_term' = 'Lpm Template Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`phase` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`matter`.`phase` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`phase` ALTER COLUMN `sla_template_id` SET TAGS ('dbx_business_glossary_term' = 'Sla Template Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`task` SET TAGS ('dbx_subdomain' = 'matter_management');
ALTER TABLE `legal_ecm`.`matter`.`task` ALTER COLUMN `task_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Task ID');
ALTER TABLE `legal_ecm`.`matter`.`task` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Client Contact Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`task` ALTER COLUMN `deadline_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Deadline Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`task` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Document Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`task` ALTER COLUMN `obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Obligation Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`team` SET TAGS ('dbx_subdomain' = 'matter_management');
...
ALTER TABLE `legal_ecm`.`matter`.`team` ALTER COLUMN `ethical_wall_id` SET TAGS ('dbx_business_glossary_term' = 'Ethical Wall Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`budget` SET TAGS ('dbx_subdomain' = 'matter_management');
ALTER TABLE `legal_ecm`.`matter`.`budget` ALTER COLUMN `budget_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Budget ID');
ALTER TABLE `legal_ecm`.`matter`.`budget` ALTER COLUMN `fee_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Fee Arrangement Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`budget` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`matter`.`budget` ALTER COLUMN `outside_counsel_guideline_id` SET TAGS ('dbx_business_glossary_term' = 'Outside Counsel Guideline Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`budget` ALTER COLUMN `rate_card_id` SET TAGS ('dbx_business_glossary_term' = 'Rate Card Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`budget` ALTER COLUMN `retainer_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Retainer Agreement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`matter_party` SET TAGS ('dbx_subdomain' = 'matter_management');
...
ALTER TABLE `legal_ecm`.`matter`.`matter_party` ALTER COLUMN `aml_kyc_program_id` SET TAGS ('dbx_business_glossary_term' = 'Aml Kyc Program Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`matter_party` ALTER COLUMN `conflict_party_id` SET TAGS ('dbx_business_glossary_term' = 'Conflict Party Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`matter_party` ALTER COLUMN `kyc_screening_id` SET TAGS ('dbx_business_glossary_term' = 'Kyc Screening Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`matter_party` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Register Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`hold` SET TAGS ('dbx_subdomain' = 'matter_management');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `hold_id` SET TAGS ('dbx_business_glossary_term' = 'Matter Hold ID');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Custodian Contact Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `data_breach_incident_id` SET TAGS ('dbx_business_glossary_term' = 'Data Breach Incident Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `docket_id` SET TAGS ('dbx_business_glossary_term' = 'Docket Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `policy_id` SET TAGS ('dbx_business_glossary_term' = 'Policy Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `register_id` SET TAGS ('dbx_business_glossary_term' = 'Register Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`hold` ALTER COLUMN `request_id` SET TAGS ('dbx_business_glossary_term' = 'Request Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`outcome` SET TAGS ('dbx_subdomain' = 'matter_management');
...
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `escrow_arrangement_id` SET TAGS ('dbx_business_glossary_term' = 'Escrow Arrangement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `legal_document_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Document Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `matter_id` SET TAGS ('dbx_business_glossary_term' = 'Matter ID');
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `pi_claim_id` SET TAGS ('dbx_business_glossary_term' = 'Pi Claim Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `regulatory_obligation_id` SET TAGS ('dbx_business_glossary_term' = 'Regulatory Obligation Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `invoice_id` SET TAGS ('dbx_business_glossary_term' = 'Triggered Invoice Id (Foreign Key)');
ALTER TABLE `legal_ecm`.`matter`.`outcome` ALTER COLUMN `trust_disbursement_id` SET TAGS ('dbx_business_glossary_term' = 'Trust Disbursement Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`assignment` SET TAGS ('dbx_subdomain' = 'matter_management');
...
ALTER TABLE `legal_ecm`.`matter`.`assignment` ALTER COLUMN `clearance_id` SET TAGS ('dbx_business_glossary_term' = 'Conflict Clearance Id (Foreign Key)');
...
ALTER TABLE `legal_ecm`.`matter`.`assignment` ALTER COLUMN `timekeeper_rate_id` SET TAGS ('dbx_business_glossary_term' = 'Timekeeper Rate Id (Foreign Key)');
