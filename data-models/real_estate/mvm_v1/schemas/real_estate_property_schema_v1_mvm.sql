-- Schema for Domain: property | Business: Real Estate | Version: v1_mvm
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`asset` (
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to reference.uom. Business justification: International portfolio reporting requires standardized UOM for asset-level GLA and NLA measurements (sqft vs sqm). NCREIF, MSCI, and INREV reporting mandates consistent area measurement units at the ',
    ...
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to reference.market_segment. Business justification: Investment portfolio reporting and NCREIF benchmarking require classifying each asset by market segment (core, value-add, opportunistic). Asset managers and fund accountants depend on this classificat',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`building` (
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to reference.uom. Business justification: Buildings carry both sqft and sqm area columns (gla_sqft/gla_sqm, nla_sqft/nla_sqm, total_rentable_sqft). International portfolio management and BOMA measurement standard compliance require a canonica',
    ...
    `dev_project_id` BIGINT COMMENT 'Foreign key linking to development.dev_project. Business justification: A building is physically created by a development project. Asset managers, warranty administrators, and construction closeout teams need to know which dev_project constructed a building for as-built d',
    `geographic_hierarchy_id` BIGINT COMMENT 'Foreign key linking to reference.geographic_hierarchy. Business justification: CoStar submarket assignment, market rent benchmarking, and leasing strategy reports require buildings to be directly classified within a geographic hierarchy node. Brokers and asset managers use subma',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`parcel` (
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to reference.uom. Business justification: Parcels carry lot_size_sqf, lot_size_sqm, and lot_size_acres simultaneously. Land valuation, FAR calculations, and regulatory submissions require a canonical UOM reference to determine the authoritati',
    ...
    `geographic_hierarchy_id` BIGINT COMMENT 'Foreign key linking to reference.geographic_hierarchy. Business justification: Parcel-level tax jurisdiction mapping, FEMA flood zone reporting, and GIS submarket analysis require direct geographic hierarchy classification. County assessors and environmental due diligence workfl',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`unit` (
    ...
    `address_id` BIGINT COMMENT 'Foreign key linking to property.address. Business justification: Individual units (apartment units, office suites, retail bays) can have their own distinct sub-addresses for mail delivery, legal identification, and tenant correspondence — e.g., Unit 4B, 123 Main S',
    ...
    `dev_project_id` BIGINT COMMENT 'Foreign key linking to development.dev_project. Business justification: Units are delivered by development projects. Property managers and warranty teams track which dev_project constructed or renovated each unit for punch list management, warranty claims, and TI cost cap',
    `floor_id` BIGINT COMMENT 'Foreign key linking to property.floor. Business justification: A unit resides on a specific floor within a building. The floor table is the authoritative master reference for floor records in the property domain. Adding floor_id FK normalizes the relationship and',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`space` (
    ...
    `dev_project_id` BIGINT COMMENT 'Foreign key linking to development.dev_project. Business justification: Commercial spaces are created or renovated by development/TI projects. Leasing teams and asset managers track which dev_project built out a space for TI cost capitalization, warranty management, and l',
    ...
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to reference.market_segment. Business justification: Commercial leasing strategy and CoStar availability reporting classify individual spaces by market segment (e.g., Class A office core vs. flex). Leasing teams and brokers use space-level market segmen',
    ...
    `unit_id` BIGINT COMMENT 'Reference to the suite or demised premises to which this space belongs. Enables grouping of individual spaces into a leasable suite for lease administration in Yardi Voyager.',
    `space_unit_id` BIGINT COMMENT 'Foreign key linking to property.unit. Business justification: Space is described as a sub-unit space within a building, supporting multi-tenant floor configurations and co-working arrangements. A space can be a subdivision of a leasable unit (e.g., a suite subdi',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`property_ownership_interest` (
    ...
    `investment_deal_id` BIGINT COMMENT 'Foreign key linking to investment.investment_deal. Business justification: Ownership interests in real property are created, transferred, or terminated through investment deals (acquisitions, dispositions, JV formations). Linking property_ownership_interest to the originatin',
    ...
    `transaction_type_id` BIGINT COMMENT 'Foreign key linking to reference.transaction_type. Business justification: Ownership interests are created through specific transaction types (acquisition, disposition, JV formation, 1031 exchange). Transfer tax calculations, SEC reporting, REIT qualifying income tests, and ',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`title_record` (
    ...
    `jurisdiction_id` BIGINT COMMENT 'Foreign key linking to compliance.jurisdiction. Business justification: Title records are recorded in specific jurisdictions (county recorders, state land registries). Normalizing the recording jurisdiction to a proper FK enables jurisdiction-based title search, complianc',
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Title records identify the legal entity holding title (grantee). Linking title_record to legal_entity supports entity-level title portfolio management, title vesting verification for financing, and le',
    ...
    `transaction_type_id` BIGINT COMMENT 'Foreign key linking to reference.transaction_type. Business justification: Title records are created for specific transaction types (sale, refinance, easement grant, foreclosure). Title insurance policy type, SNDA requirements, transfer tax applicability, and environmental r',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`inspection` (
    ...
    `demised_space_id` BIGINT COMMENT 'Foreign key linking to lease.lease_demised_space. Business justification: Move-in and move-out condition inspections must reference the exact demised premises record to document space condition at lease commencement and surrender. This supports security deposit adjudication',
    ...
    `floor_id` BIGINT COMMENT 'Foreign key linking to property.floor. Business justification: Inspections are frequently conducted at the floor level — fire safety inspections, structural assessments, ADA compliance reviews, and HVAC zone inspections all target a specific floor. The inspection',
    ...
    `listing_agreement_id` BIGINT COMMENT 'Foreign key linking to brokerage.listing_agreement. Business justification: Pre-listing inspections and due diligence inspections are triggered by listing agreement terms. Brokers and sellers must document condition reports as part of listing agreement obligations. inspection',
    `occupancy_id` BIGINT COMMENT 'Foreign key linking to tenant.tenant_occupancy. Business justification: Move-in/move-out inspections are tied to specific occupancy periods for security deposit dispute resolution and condition-at-handover documentation. Linking inspection directly to tenant_occupancy ena',
    `parcel_id` BIGINT COMMENT 'Foreign key linking to property.parcel. Business justification: Environmental inspections (Phase I/II ESA, soil classification, flood zone assessments) are conducted at the parcel level, not the building or unit level. The inspection table has an environmental_pha',
    ...
    `space_id` BIGINT COMMENT 'Foreign key linking to property.space. Business justification: Inspections can be conducted at the individual space level (e.g., a specific office suite, retail bay, or co-working space within a multi-tenant floor). The inspection table already supports asset, bu',
    `tenant_id` BIGINT COMMENT 'Foreign key linking to tenant.tenant. Business justification: Move-in and move-out inspections are tenant-specific events critical for security deposit adjudication and habitability compliance. Property managers must retrieve all inspections for a given tenant a',
    `ti_allowance_id` BIGINT COMMENT 'Foreign key linking to lease.ti_allowance. Business justification: TI disbursement milestone inspections are a standard landlord process — construction progress inspections gate allowance disbursements. Property managers and construction teams need this link to track',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`occupancy_record` (
    ...
    `geographic_hierarchy_id` BIGINT COMMENT 'Foreign key linking to reference.geographic_hierarchy. Business justification: Submarket-level occupancy reporting for CoStar, CBRE, and JLL market reports requires occupancy snapshots to be directly tagged to a geographic hierarchy node. Market analysts use submarket occupancy ',
    `lease_agreement_id` BIGINT COMMENT 'Foreign key linking to lease.lease_agreement. Business justification: Rent roll and WALT/WALE reporting require tying each occupancy snapshot to the specific lease driving it. Portfolio analysts and asset managers depend on this link for lease expiry roll analysis, same',
    `market_segment_id` BIGINT COMMENT 'Foreign key linking to reference.market_segment. Business justification: NCREIF and MSCI occupancy benchmarking reports require occupancy records to be classified by market segment (core, value-add, opportunistic). Fund performance attribution and investor reporting compar',
    ...
    `reporting_period_id` BIGINT COMMENT 'Foreign key linking to finance.reporting_period. Business justification: Occupancy records are time-series snapshots that must align to financial reporting periods for synchronized NOI and occupancy reporting. Investor reports, lender compliance packages, and REIT disclosu',
    `space_id` BIGINT COMMENT 'Foreign key linking to property.space. Business justification: Occupancy records can be captured at the individual space level, particularly for co-working environments, multi-tenant floor configurations, and flex space arrangements. The occupancy_record table al',
    `sublease_id` BIGINT COMMENT 'Foreign key linking to lease.sublease. Business justification: Sublease occupancy must be tracked separately from direct lease occupancy for accurate NOI reporting, portfolio analytics, and ASC 842 sublease income recognition. Asset managers need to distinguish s',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`amenity` (
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to reference.uom. Business justification: Amenities carry both area_sqf and area_sqm columns. LEED certification documentation, CAM cost allocation, and property condition assessments require a canonical UOM reference for amenity area to ensu',
    ...
    `demised_space_id` BIGINT COMMENT 'Foreign key linking to lease.lease_demised_space. Business justification: Dedicated amenities (exclusive parking stalls, private storage, rooftop access) are granted as part of specific demised premises in commercial leases. Linking amenity to lease_demised_space enables le',
    `dev_project_id` BIGINT COMMENT 'Foreign key linking to development.dev_project. Business justification: Amenities (fitness centers, lobbies, rooftop terraces) are installed as part of development projects. Asset managers and warranty teams track which dev_project installed each amenity for warranty clai',
    `floor_id` BIGINT COMMENT 'Foreign key linking to property.floor. Business justification: An amenity (pool, fitness center, parking structure) is physically located on a specific floor of a building. The floor table is the authoritative master for floor records. Adding floor_id FK normaliz',
    `replaced_amenity_id` BIGINT COMMENT 'Self-referencing FK on amenity (replaced_amenity_id)',
    `space_id` BIGINT COMMENT 'Foreign key linking to property.space. Business justification: Some amenities occupy or are associated with a specific leasable space — a fitness center occupying suite 100, a conference room within a specific space, or a rooftop terrace associated with a penthou',
    `unit_id` BIGINT COMMENT 'Foreign key linking to property.unit. Business justification: Some amenities are unit-specific rather than building-wide — in-unit washer/dryer, private balcony, dedicated parking stall, or private storage unit. The amenity table already has asset_id and buildin',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`floor` (
    ...
    `uom_id` BIGINT COMMENT 'Foreign key linking to reference.uom. Business justification: Floor plates carry both sqft and sqm area columns (gross_floor_area_sqft/sqm, net_leasable_area_sqft/sqm). BOMA floor measurement certification, lease administration, and international portfolio repor',
    `building_id` BIGINT COMMENT 'Reference to the parent building that contains this floor.',
    `dev_project_id` BIGINT COMMENT 'Foreign key linking to development.dev_project. Business justification: Floors are delivered in phases by development projects in high-rise construction. Project managers and asset managers track which dev_project constructed or renovated each floor for phased COO issuanc',
    `parent_floor_id` BIGINT COMMENT 'Self-referencing FK on floor (parent_floor_id)',
    `space_use_type_id` BIGINT COMMENT 'Foreign key linking to reference.space_use_type. Business justification: Building permit applications, zoning compliance certificates, and occupancy reporting require floor-level use type classification (office, retail, parking, mechanical). Fire egress planning and CAM po',
    ...
    `finish_type` STRING COMMENT 'Type of flooring material or finish installed (e.g., carpet, tile, hardwood, concrete, vinyl).',
    ...
    `load_capacity_kg_sqm` DECIMAL(18,2) COMMENT 'Maximum live load the floor structure can support, measured in kilograms per square meter.',
    `load_capacity_lbs_sqft` DECIMAL(18,2) COMMENT 'Maximum live load the floor structure can support, measured in pounds per square foot.',
    ...
    `plate_efficiency_pct` DECIMAL(18,2) COMMENT 'Ratio of net leasable area to gross floor area, expressed as a percentage. Indicates how efficiently the floor space is utilized for revenue-generating purposes.',
...
CREATE OR REPLACE TABLE `real_estate_ecm`.`property`.`site_coverage` (
    `site_coverage_id` BIGINT COMMENT 'Primary key for the site_coverage association',
    `building_id` BIGINT COMMENT 'Foreign key linking to the building whose footprint is being tracked on this parcel.',
    `parcel_id` BIGINT COMMENT 'Foreign key linking to the land parcel on which a portion of the building footprint sits.',
    `coverage_percentage` DECIMAL(18,2) COMMENT 'The percentage of the buildings total ground-floor footprint that falls on this specific parcel. All coverage_percentage values for a given building_id should sum to 100. Critical for FAR compliance calculations and tax apportionment across multiple parcels.',
    `coverage_sqft` DECIMAL(18,2) COMMENT 'The actual square footage of the buildings ground-floor footprint that falls on this parcel. Derived from GIS polygon intersection but stored as an operational record for tax apportionment, title encumbrance documentation, and zoning compliance reporting.',
    `coverage_status` STRING COMMENT 'Lifecycle status of this building-parcel coverage record. Drives whether the record is used in active tax apportionment and title reporting.',
    `effective_date` DATE COMMENT 'The date on which this building-parcel coverage relationship became legally effective. Typically corresponds to a lot split, lot merger, subdivision recording, or initial site plan approval. Required for historical title and tax apportionment tracking.',
    `expiration_date` DATE COMMENT 'The date on which this building-parcel coverage relationship ceased to be effective, if applicable (e.g., following a lot merger or parcel subdivision that changed the legal boundary). Null if the relationship is currently active.',
    `far` DECIMAL(18,2) COMMENT 'Floor Area Ratio (FAR) — the ratio of total building floor area to the area of the land parcel. Represents the maximum allowable development density per zoning regulations. Critical for development feasibility and air rights analysis. [Moved from building: FAR (Floor Area Ratio) is computed relative to a specific parcels land area, not the building in isolation. When a building spans multiple parcels, FAR must be calculated per parcel using coverage_sqft and the parcels lot_size_sqf. Storing a single FAR on the building record is ambiguous and incorrect in multi-parcel scenarios; it belongs on the site_coverage association where both building floor area and parcel land area are contextually available.]',
    `is_primary_parcel` BOOLEAN COMMENT 'Indicates whether this parcel is the primary tax-situs parcel for the building. Exactly one parcel per building should carry this flag as true. Used by tax and title teams to identify the principal parcel of record.',
    CONSTRAINT pk_site_coverage PRIMARY KEY(`site_coverage_id`)
) COMMENT 'This association product represents the Site Coverage relationship between a building and a land parcel. It captures the legally and operationally significant fact that a buildings physical footprint may span one or more legal parcels, and that a parcel may host one or more buildings. Each record links one building to one parcel and carries attributes — coverage percentage, covered square footage, primary parcel designation, and effective date — that exist only in the context of this specific building-parcel pairing. Used by tax assessment, title, GIS, and zoning compliance teams.. Existence Justification: In commercial real estate, a single building can physically span multiple legal parcels (e.g., a large office tower or campus straddling two or more APN-delineated lots), and a single parcel can contain multiple buildings (e.g., a mixed-use development or industrial park). This is an operationally managed relationship: tax assessors apportion assessed value across parcels, title companies track which parcels are encumbered by which structures, and GIS/site teams track coverage percentages for zoning compliance and FAR calculations. The relationship is actively created, updated, and queried by real estate operations, legal, and tax teams.';
...
ALTER TABLE `real_estate_ecm`.`property`.`unit` ADD CONSTRAINT `fk_property_unit_address_id` FOREIGN KEY (`address_id`) REFERENCES `real_estate_ecm`.`property`.`address`(`address_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`unit` ADD CONSTRAINT `fk_property_unit_floor_id` FOREIGN KEY (`floor_id`) REFERENCES `real_estate_ecm`.`property`.`floor`(`floor_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`space` ADD CONSTRAINT `fk_property_space_space_unit_id` FOREIGN KEY (`space_unit_id`) REFERENCES `real_estate_ecm`.`property`.`unit`(`unit_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ADD CONSTRAINT `fk_property_inspection_floor_id` FOREIGN KEY (`floor_id`) REFERENCES `real_estate_ecm`.`property`.`floor`(`floor_id`);
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ADD CONSTRAINT `fk_property_inspection_parcel_id` FOREIGN KEY (`parcel_id`) REFERENCES `real_estate_ecm`.`property`.`parcel`(`parcel_id`);
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ADD CONSTRAINT `fk_property_inspection_space_id` FOREIGN KEY (`space_id`) REFERENCES `real_estate_ecm`.`property`.`space`(`space_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ADD CONSTRAINT `fk_property_occupancy_record_space_id` FOREIGN KEY (`space_id`) REFERENCES `real_estate_ecm`.`property`.`space`(`space_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ADD CONSTRAINT `fk_property_amenity_floor_id` FOREIGN KEY (`floor_id`) REFERENCES `real_estate_ecm`.`property`.`floor`(`floor_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ADD CONSTRAINT `fk_property_amenity_space_id` FOREIGN KEY (`space_id`) REFERENCES `real_estate_ecm`.`property`.`space`(`space_id`);
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ADD CONSTRAINT `fk_property_amenity_unit_id` FOREIGN KEY (`unit_id`) REFERENCES `real_estate_ecm`.`property`.`unit`(`unit_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ADD CONSTRAINT `fk_property_site_coverage_building_id` FOREIGN KEY (`building_id`) REFERENCES `real_estate_ecm`.`property`.`building`(`building_id`);
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ADD CONSTRAINT `fk_property_site_coverage_parcel_id` FOREIGN KEY (`parcel_id`) REFERENCES `real_estate_ecm`.`property`.`parcel`(`parcel_id`);
...
ALTER TABLE `real_estate_ecm`.`property`.`asset` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Area Uom Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`asset` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`building` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Area Uom Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`building` ALTER COLUMN `dev_project_id` SET TAGS ('dbx_business_glossary_term' = 'Dev Project Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`building` ALTER COLUMN `geographic_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Geographic Hierarchy Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`parcel` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Area Uom Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`parcel` ALTER COLUMN `geographic_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Geographic Hierarchy Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`unit` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Address Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`unit` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `real_estate_ecm`.`property`.`unit` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
...
ALTER TABLE `real_estate_ecm`.`property`.`unit` ALTER COLUMN `dev_project_id` SET TAGS ('dbx_business_glossary_term' = 'Dev Project Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`unit` ALTER COLUMN `floor_id` SET TAGS ('dbx_business_glossary_term' = 'Floor Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`space` ALTER COLUMN `dev_project_id` SET TAGS ('dbx_business_glossary_term' = 'Dev Project Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`space` ALTER COLUMN `floor_id` SET TAGS ('dbx_business_glossary_term' = 'Floor ID');
ALTER TABLE `real_estate_ecm`.`property`.`space` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`space` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Suite ID');
ALTER TABLE `real_estate_ecm`.`property`.`space` ALTER COLUMN `space_unit_id` SET TAGS ('dbx_business_glossary_term' = 'Unit Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`property_ownership_interest` SET TAGS ('dbx_subdomain' = 'ownership_title');
...
ALTER TABLE `real_estate_ecm`.`property`.`property_ownership_interest` ALTER COLUMN `investment_deal_id` SET TAGS ('dbx_business_glossary_term' = 'Investment Deal Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`property_ownership_interest` ALTER COLUMN `transaction_type_id` SET TAGS ('dbx_business_glossary_term' = 'Transaction Type Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`title_record` SET TAGS ('dbx_subdomain' = 'ownership_title');
...
ALTER TABLE `real_estate_ecm`.`property`.`title_record` ALTER COLUMN `jurisdiction_id` SET TAGS ('dbx_business_glossary_term' = 'Jurisdiction Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`title_record` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`title_record` ALTER COLUMN `transaction_type_id` SET TAGS ('dbx_business_glossary_term' = 'Transaction Type Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`inspection` SET TAGS ('dbx_subdomain' = 'ownership_title');
...
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `demised_space_id` SET TAGS ('dbx_business_glossary_term' = 'Lease Demised Space Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `floor_id` SET TAGS ('dbx_business_glossary_term' = 'Floor Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `listing_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Listing Agreement Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `occupancy_id` SET TAGS ('dbx_business_glossary_term' = 'Tenant Occupancy Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `parcel_id` SET TAGS ('dbx_business_glossary_term' = 'Parcel Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `space_id` SET TAGS ('dbx_business_glossary_term' = 'Space Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `tenant_id` SET TAGS ('dbx_business_glossary_term' = 'Tenant Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`inspection` ALTER COLUMN `ti_allowance_id` SET TAGS ('dbx_business_glossary_term' = 'Ti Allowance Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` SET TAGS ('dbx_subdomain' = 'ownership_title');
...
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ALTER COLUMN `geographic_hierarchy_id` SET TAGS ('dbx_business_glossary_term' = 'Geographic Hierarchy Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ALTER COLUMN `lease_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Lease Agreement Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ALTER COLUMN `market_segment_id` SET TAGS ('dbx_business_glossary_term' = 'Market Segment Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ALTER COLUMN `reporting_period_id` SET TAGS ('dbx_business_glossary_term' = 'Reporting Period Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ALTER COLUMN `space_id` SET TAGS ('dbx_business_glossary_term' = 'Space Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`occupancy_record` ALTER COLUMN `sublease_id` SET TAGS ('dbx_business_glossary_term' = 'Sublease Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`amenity` SET TAGS ('dbx_subdomain' = 'asset_registry');
...
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Area Uom Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `demised_space_id` SET TAGS ('dbx_business_glossary_term' = 'Lease Demised Space Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `dev_project_id` SET TAGS ('dbx_business_glossary_term' = 'Dev Project Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `floor_id` SET TAGS ('dbx_business_glossary_term' = 'Floor Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `replaced_amenity_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `space_id` SET TAGS ('dbx_business_glossary_term' = 'Space Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`amenity` ALTER COLUMN `unit_id` SET TAGS ('dbx_business_glossary_term' = 'Unit Id (Foreign Key)');
...
ALTER TABLE `real_estate_ecm`.`property`.`floor` ALTER COLUMN `uom_id` SET TAGS ('dbx_business_glossary_term' = 'Area Uom Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`floor` ALTER COLUMN `dev_project_id` SET TAGS ('dbx_business_glossary_term' = 'Dev Project Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`floor` ALTER COLUMN `parent_floor_id` SET TAGS ('dbx_self_ref_fk' = 'true');
ALTER TABLE `real_estate_ecm`.`property`.`floor` ALTER COLUMN `space_use_type_id` SET TAGS ('dbx_business_glossary_term' = 'Space Use Type Id (Foreign Key)');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` SET TAGS ('dbx_data_type' = 'association_data');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` SET TAGS ('dbx_subdomain' = 'asset_registry');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` SET TAGS ('dbx_association_edges' = 'property.building,property.parcel');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `site_coverage_id` SET TAGS ('dbx_business_glossary_term' = 'Site Coverage - Site Coverage Id');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `building_id` SET TAGS ('dbx_business_glossary_term' = 'Site Coverage - Building Id');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `parcel_id` SET TAGS ('dbx_business_glossary_term' = 'Site Coverage - Parcel Id');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `coverage_percentage` SET TAGS ('dbx_business_glossary_term' = 'Coverage Percentage');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `coverage_sqft` SET TAGS ('dbx_business_glossary_term' = 'Coverage Square Footage');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `coverage_status` SET TAGS ('dbx_business_glossary_term' = 'Coverage Status');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `effective_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Effective Date');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `expiration_date` SET TAGS ('dbx_business_glossary_term' = 'Coverage Expiration Date');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `far` SET TAGS ('dbx_business_glossary_term' = 'Floor Area Ratio (FAR)');
ALTER TABLE `real_estate_ecm`.`property`.`site_coverage` ALTER COLUMN `is_primary_parcel` SET TAGS ('dbx_business_glossary_term' = 'Primary Parcel Flag');
