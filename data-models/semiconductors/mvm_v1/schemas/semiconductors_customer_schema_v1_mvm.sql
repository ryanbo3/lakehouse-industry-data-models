-- Schema for Domain: customer | Business: Semiconductors | Version: v1_mvm
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`account_hierarchy` (
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Parent-subsidiary account structures map to legal entity consolidation hierarchies for financial reporting, transfer pricing, and tax compliance in global semiconductor operations. Essential for inter',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`segment` (
    ...
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Market segments (automotive, mobile, IoT) align with profit center reporting structures for P&L analysis, strategic resource allocation, and segment profitability reporting in semiconductor business p',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`customer_design_win` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Design wins incur engineering support costs (FAE time, design tools, validation) tracked to specific cost centers for project accounting, resource planning, and design win ROI analysis.',
    ...
    `flow_id` BIGINT COMMENT 'Foreign key linking to process.process_flow. Business justification: A design win is committed to a specific process flow for yield forecasting, capacity planning, and NRE scoping. The existing process_node text field is a denormalization of process_flow data. Semico',
    ...
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Design wins specify exact materials/components (wafers, substrates, packaging materials) for production. Links to material master for BOM planning, cost estimation, supply chain readiness, and product',
    `primary_customer_account_id` BIGINT COMMENT 'Reference to the customer organization that owns the design win.',
    ...
    `sourcing_contract_id` BIGINT COMMENT 'Foreign key linking to supply.sourcing_contract. Business justification: Design wins often trigger or reference specific sourcing contracts for long-term component supply. Links to sourcing contract for supply assurance, contract compliance tracking, revenue-to-supply alig',
    ...
    `fab_tool_id` BIGINT COMMENT 'Foreign key linking to equipment.fab_tool. Business justification: Design wins specify target fabrication equipment for production planning. Critical for design-to-manufacturing handoff, capacity allocation, tool qualification requirements, and production ramp schedu',
    `technology_control_plan_id` BIGINT COMMENT 'Foreign key linking to compliance.technology_control_plan. Business justification: Design wins involving controlled technology (advanced process nodes, cryptographic IP, military-grade components) must reference applicable technology control plans. Sales and compliance teams verify ',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` (
    ...
    `contact_id` BIGINT COMMENT 'Foreign key linking to customer.contact. Business justification: A design registration is initiated and owned by a specific design engineer or technical contact at the customer account. The customer_design_win table already carries contact_id, and the same pattern ',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Design registrations consume engineering resources (FAE support, EDA tools, reference designs) charged to cost centers for activity-based costing and engineering capacity planning.',
    `customer_design_win_id` BIGINT COMMENT 'Foreign key linking to customer.customer_design_win. Business justification: In the semiconductor design-in lifecycle, a design registration is the formal precursor to a confirmed design win. Linking customer_design_registration to customer_design_win enables full traceability',
    `flow_id` BIGINT COMMENT 'Foreign key linking to process.process_flow. Business justification: Design registration specifies which process flow the customers design will be fabricated on — essential for tapeout scheduling, DFM rule application, and NRE budgeting. The existing process_node_nm',
    ...
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Design registrations specify target materials/packages during early design phase. Links to material master for early supply chain engagement, material qualification planning, and lead-time assessment.',
    `technology_control_plan_id` BIGINT COMMENT 'Foreign key linking to compliance.technology_control_plan. Business justification: Design registrations for advanced process nodes (sub-14nm) or controlled IP blocks require technology control plan coverage per EAR/ITAR. Export compliance officers must verify TCP applicability befor',
    `program_id` BIGINT COMMENT 'Foreign key linking to test.test_program. Business justification: Design registrations specify validation test programs for engineering samples and characterization. Design-in process requires linking customer projects to test programs for silicon validation, yield ',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`qualification_status` (
    ...
    `certification_id` BIGINT COMMENT 'Foreign key linking to compliance.certification. Business justification: Supplier/customer qualification in semiconductor supply chain references specific compliance certifications (ISO 9001, IATF 16949, ISO 14001). Quality engineers verify certification validity during qu',
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Customer qualification programs (AEC-Q100, PPAP, IATF 16949) incur costs for reliability testing, documentation, and certification tracked to quality/engineering cost centers for compliance budgeting.',
    ...
    `qualification_id` BIGINT COMMENT 'Foreign key linking to process.process_qualification. Business justification: PPAP and AEC-Q customer qualification records reference the underlying fab process qualification that was completed. Semiconductor quality engineers link customer qualification status (IATF/AEC-Q) to ',
    `reliability_test_run_id` BIGINT COMMENT 'Foreign key linking to test.reliability_test_run. Business justification: Qualification status (especially AEC-Q, automotive PPAP) is directly driven by reliability test run results (HTOL, TC, HAST). Quality engineers link qualification records to specific reliability test ',
    `program_id` BIGINT COMMENT 'Foreign key linking to test.test_program. Business justification: Customer qualification (AEC-Q100, PPAP, IATF) requires execution of specific test programs. Quality and reliability teams track which test program version was used to achieve qualification status for ',
    `fab_tool_id` BIGINT COMMENT 'Foreign key linking to equipment.fab_tool. Business justification: Customer qualifications (AEC-Q, PPAP, IATF-16949) often tied to specific fab tool capabilities and certifications. Real business process: customer audit trails require traceability to qualified manufa',
    `quality_qualification_program_id` BIGINT COMMENT 'Foreign key linking to quality.quality_qualification_program. Business justification: Customer qualification status directly tracks progress through formal quality qualification programs (AEC-Q, PPAP, IATF). Essential for customer approval workflows, automotive qualification reporting,',
    `tool_qualification_id` BIGINT COMMENT 'Foreign key linking to equipment.tool_qualification. Business justification: Customer qualification status links to tool qualification records for compliance traceability. Critical for automotive/aerospace customers requiring AEC-Q or PPAP certification - customer audits deman',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`nda_agreement` (
    ...
    `export_license_id` BIGINT COMMENT 'Foreign key linking to compliance.export_license. Business justification: NDAs governing technology transfer in semiconductor industry often require export license coverage for ITAR/EAR controlled technical data. Compliance officers need to verify NDA scope aligns with lice',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`account_team` (
    ...
    `cost_center_id` BIGINT COMMENT 'Foreign key linking to finance.cost_center. Business justification: Sales and FAE team members time is charged to cost centers for headcount planning, sales expense allocation, and territory profitability analysis in semiconductor sales operations.',
    `account_id` BIGINT COMMENT 'Identifier of the customer account to which the team member is assigned.',
    `territory_id` BIGINT COMMENT 'Foreign key linking to sales.territory. Business justification: Account teams are assigned to sales territories in semiconductor sales organizations. Territory assignment drives coverage model, quota allocation, commission calculations, and ensures proper account ',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`distributor_agreement` (
    ...
    `account_id` BIGINT COMMENT 'FK to customer.account.account_id — Distributor agreements reference the distributors account record. The distributor is a type of account.',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Distributor agreements define channel economics (discount rates, MDF, price protection) that impact profit center P&L, margin analysis, and channel profitability reporting by business unit.',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`sample_request` (
    `sample_request_id` BIGINT COMMENT 'Unique identifier for the sample request record.',
    ...
    `customer_design_registration_id` BIGINT COMMENT 'Foreign key linking to customer.customer_design_registration. Business justification: In semiconductor sales, engineering sample requests are almost always tied to a specific design project or design registration. A customer requests samples to evaluate a part for a registered design. ',
    `die_bank_id` BIGINT COMMENT 'Foreign key linking to inventory.die_bank. Business justification: Sample requests are often fulfilled from die bank inventory for engineering samples before final packaging. Essential for tracking die consumption for customer evaluation and linking sample requests t',
    `finished_good_id` BIGINT COMMENT 'Foreign key linking to inventory.finished_good. Business justification: Sample requests must track which finished goods inventory was allocated and shipped to fulfill the request. Critical for sample inventory management, cost tracking, and reconciling sample consumption ',
    `fab_tool_id` BIGINT COMMENT 'Foreign key linking to equipment.fab_tool. Business justification: Engineering sample requests specify which fab tool/line to use for prototype production. Critical for sample fulfillment planning, tool availability checking, process capability matching, and ensuring',
    `ic_catalog_id` BIGINT COMMENT 'Foreign key linking to product.ic_catalog. Business justification: Sample requests must specify which IC product is being requested for evaluation. Fulfillment process requires linking request to IC catalog entry to validate availability, export controls, and sample ',
    `inspection_lot_id` BIGINT COMMENT 'Foreign key linking to quality.inspection_lot. Business justification: Sample requests generate inspection lots for quality validation before shipment to customers. Links sample fulfillment to quality inspection results for traceability, customer complaint investigation,',
    ...
    `lot_process_run_id` BIGINT COMMENT 'Foreign key linking to process.lot_process_run. Business justification: A customer sample request is fulfilled by executing a specific lot process run. Semiconductor customer service and fab operations teams track which lot run produced the samples shipped to a customer —',
    `material_master_id` BIGINT COMMENT 'Foreign key linking to supply.material_master. Business justification: Sample requests often specify raw materials or components (wafers, substrates, packaging materials) needed for prototyping and customer evaluation. Links to material master for sample fulfillment, inv',
    ...
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Samples are fulfilled at SKU level (specific orderable part number with package/speed/voltage variant), not just IC catalog. Real-world sample requests specify exact MPN; fulfillment systems pick inve',
    ...
    `wbs_element_id` BIGINT COMMENT 'Foreign key linking to finance.wbs_element. Business justification: Sample programs tracked as projects with WBS elements for cost recovery, program management, and evaluation of sample-to-production conversion rates in semiconductor customer acquisition.',
...
CREATE OR REPLACE TABLE `semiconductors_ecm`.`customer`.`price_agreement` (
    ...
    `distributor_agreement_id` BIGINT COMMENT 'Foreign key linking to customer.distributor_agreement. Business justification: Price agreements (SPAs — Special Price Authorizations) in the semiconductor industry are frequently negotiated under the umbrella of a master distributor agreement. Linking price_agreement to distribu',
    ...
    `legal_entity_id` BIGINT COMMENT 'Foreign key linking to finance.legal_entity. Business justification: Price agreements are legal contracts executed by specific legal entities; required for contract management, intercompany pricing compliance, and transfer pricing documentation in global semiconductor ',
    ...
    `primary_price_account_id` BIGINT COMMENT 'Unique identifier of the customer to which the price agreement applies.',
    `profit_center_id` BIGINT COMMENT 'Foreign key linking to finance.profit_center. Business justification: Price agreements impact profit center revenue and margin; essential for pricing analytics, variance analysis, and profitability reporting by business unit in semiconductor pricing management.',
    `quote_id` BIGINT COMMENT 'Foreign key linking to sales.quote. Business justification: Price agreements in semiconductor sales often reference the originating quote that established negotiated terms. Required for pricing audit trails, compliance verification, and tracking quote-to-agree',
    `sku_id` BIGINT COMMENT 'Foreign key linking to product.sku. Business justification: Pricing is negotiated and tracked at SKU level (specific orderable MPN with package/speed/voltage variants), not just IC catalog. Real-world price agreements specify exact manufacturer part numbers. C',
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ADD CONSTRAINT `fk_customer_customer_design_win_primary_customer_account_id` FOREIGN KEY (`primary_customer_account_id`) REFERENCES `semiconductors_ecm`.`customer`.`account`(`account_id`);
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ADD CONSTRAINT `fk_customer_customer_design_registration_contact_id` FOREIGN KEY (`contact_id`) REFERENCES `semiconductors_ecm`.`customer`.`contact`(`contact_id`);
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ADD CONSTRAINT `fk_customer_customer_design_registration_customer_design_win_id` FOREIGN KEY (`customer_design_win_id`) REFERENCES `semiconductors_ecm`.`customer`.`customer_design_win`(`customer_design_win_id`);
...
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ADD CONSTRAINT `fk_customer_sample_request_contact_id` FOREIGN KEY (`contact_id`) REFERENCES `semiconductors_ecm`.`customer`.`contact`(`contact_id`);
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ADD CONSTRAINT `fk_customer_sample_request_customer_design_registration_id` FOREIGN KEY (`customer_design_registration_id`) REFERENCES `semiconductors_ecm`.`customer`.`customer_design_registration`(`customer_design_registration_id`);
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ADD CONSTRAINT `fk_customer_sample_request_account_id` FOREIGN KEY (`account_id`) REFERENCES `semiconductors_ecm`.`customer`.`account`(`account_id`);
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ADD CONSTRAINT `fk_customer_sample_request_address_id` FOREIGN KEY (`address_id`) REFERENCES `semiconductors_ecm`.`customer`.`address`(`address_id`);
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ADD CONSTRAINT `fk_customer_price_agreement_distributor_agreement_id` FOREIGN KEY (`distributor_agreement_id`) REFERENCES `semiconductors_ecm`.`customer`.`distributor_agreement`(`distributor_agreement_id`);
...
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ADD CONSTRAINT `fk_customer_price_agreement_primary_price_account_id` FOREIGN KEY (`primary_price_account_id`) REFERENCES `semiconductors_ecm`.`customer`.`account`(`account_id`);
...
ALTER TABLE `semiconductors_ecm`.`customer`.`contact` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`account_hierarchy` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`segment` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`segment` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`address` SET TAGS ('dbx_subdomain' = 'account_management');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` SET TAGS ('dbx_subdomain' = 'design_engagement');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `flow_id` SET TAGS ('dbx_business_glossary_term' = 'Process Flow Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `primary_customer_account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Identifier (CUST_ID)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `sourcing_contract_id` SET TAGS ('dbx_business_glossary_term' = 'Sourcing Contract Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `fab_tool_id` SET TAGS ('dbx_business_glossary_term' = 'Target Fab Tool Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_win` ALTER COLUMN `technology_control_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Technology Control Plan Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` SET TAGS ('dbx_subdomain' = 'design_engagement');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `contact_id` SET TAGS ('dbx_business_glossary_term' = 'Contact Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `customer_design_win_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Design Win Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `flow_id` SET TAGS ('dbx_business_glossary_term' = 'Process Flow Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `technology_control_plan_id` SET TAGS ('dbx_business_glossary_term' = 'Technology Control Plan Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`customer_design_registration` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Validation Test Program Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `certification_id` SET TAGS ('dbx_business_glossary_term' = 'Certification Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `qualification_id` SET TAGS ('dbx_business_glossary_term' = 'Process Qualification Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `reliability_test_run_id` SET TAGS ('dbx_business_glossary_term' = 'Qualification Reliability Test Run Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `program_id` SET TAGS ('dbx_business_glossary_term' = 'Qualification Test Program Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `fab_tool_id` SET TAGS ('dbx_business_glossary_term' = 'Qualified Fab Tool Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `quality_qualification_program_id` SET TAGS ('dbx_business_glossary_term' = 'Quality Qualification Program Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`qualification_status` ALTER COLUMN `tool_qualification_id` SET TAGS ('dbx_business_glossary_term' = 'Required Tool Qualification Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`nda_agreement` SET TAGS ('dbx_subdomain' = 'contract_terms');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`nda_agreement` ALTER COLUMN `export_license_id` SET TAGS ('dbx_business_glossary_term' = 'Export License Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`nda_agreement` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_internal' = 'true');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`account_team` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`account_team` ALTER COLUMN `territory_id` SET TAGS ('dbx_business_glossary_term' = 'Territory Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`distributor_agreement` SET TAGS ('dbx_subdomain' = 'contract_terms');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`distributor_agreement` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`credit_profile` SET TAGS ('dbx_subdomain' = 'contract_terms');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` SET TAGS ('dbx_data_type' = 'transactional_data');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` SET TAGS ('dbx_subdomain' = 'design_engagement');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `sample_request_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Sample Request Identifier');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_center_id` SET TAGS ('dbx_business_glossary_term' = 'Cost Center Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `customer_design_registration_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Design Registration Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `die_bank_id` SET TAGS ('dbx_business_glossary_term' = 'Die Bank Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `finished_good_id` SET TAGS ('dbx_business_glossary_term' = 'Finished Good Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `fab_tool_id` SET TAGS ('dbx_business_glossary_term' = 'Fulfillment Fab Tool Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `ic_catalog_id` SET TAGS ('dbx_business_glossary_term' = 'Ic Catalog Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `inspection_lot_id` SET TAGS ('dbx_business_glossary_term' = 'Inspection Lot Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `internal_order_id` SET TAGS ('dbx_business_glossary_term' = 'Internal Order Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `lot_process_run_id` SET TAGS ('dbx_business_glossary_term' = 'Lot Process Run Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `material_master_id` SET TAGS ('dbx_business_glossary_term' = 'Material Master Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `address_id` SET TAGS ('dbx_business_glossary_term' = 'Shipping Address Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `address_id` SET TAGS ('dbx_restricted' = 'true');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `address_id` SET TAGS ('dbx_pii_address' = 'true');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `supplier_id` SET TAGS ('dbx_business_glossary_term' = 'Supplier Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `wbs_element_id` SET TAGS ('dbx_business_glossary_term' = 'Wbs Element Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `application_description` SET TAGS ('dbx_business_glossary_term' = 'Application Description');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `approval_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Sample Approval Timestamp');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_adjustment` SET TAGS ('dbx_business_glossary_term' = 'Sample Cost Adjustment');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_adjustment` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_amount` SET TAGS ('dbx_business_glossary_term' = 'Sample Gross Cost');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_amount` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_currency` SET TAGS ('dbx_business_glossary_term' = 'Sample Cost Currency');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_currency` SET TAGS ('dbx_value_regex' = 'USD|EUR|JPY|CNY|KRW|GBP');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_currency` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_net` SET TAGS ('dbx_business_glossary_term' = 'Sample Net Cost');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `cost_net` SET TAGS ('dbx_confidential' = 'true');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `created_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Creation Timestamp');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `customer_sample_request_status` SET TAGS ('dbx_business_glossary_term' = 'Sample Request Status');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `customer_sample_request_status` SET TAGS ('dbx_value_regex' = 'pending|approved|rejected|fulfilled|cancelled');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `delivery_requested_date` SET TAGS ('dbx_business_glossary_term' = 'Requested Delivery Date');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `fulfillment_status` SET TAGS ('dbx_business_glossary_term' = 'Sample Fulfillment Status');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `fulfillment_status` SET TAGS ('dbx_value_regex' = 'not_fulfilled|shipped|delivered|returned');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `fulfillment_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Sample Fulfillment Timestamp');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `notes` SET TAGS ('dbx_business_glossary_term' = 'Sample Request Notes');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `quantity` SET TAGS ('dbx_business_glossary_term' = 'Requested Sample Quantity');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `request_number` SET TAGS ('dbx_business_glossary_term' = 'Sample Request Number');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `request_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Sample Request Timestamp');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `sample_purpose` SET TAGS ('dbx_business_glossary_term' = 'Sample Purpose');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `sample_purpose` SET TAGS ('dbx_value_regex' = 'evaluation|design_in|qualification|testing');
ALTER TABLE `semiconductors_ecm`.`customer`.`sample_request` ALTER COLUMN `updated_timestamp` SET TAGS ('dbx_business_glossary_term' = 'Record Update Timestamp');
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` SET TAGS ('dbx_data_type' = 'master_data');
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` SET TAGS ('dbx_subdomain' = 'contract_terms');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ALTER COLUMN `distributor_agreement_id` SET TAGS ('dbx_business_glossary_term' = 'Distributor Agreement Id (Foreign Key)');
...
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ALTER COLUMN `legal_entity_id` SET TAGS ('dbx_business_glossary_term' = 'Legal Entity Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ALTER COLUMN `primary_price_account_id` SET TAGS ('dbx_business_glossary_term' = 'Customer Identifier');
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ALTER COLUMN `profit_center_id` SET TAGS ('dbx_business_glossary_term' = 'Profit Center Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ALTER COLUMN `quote_id` SET TAGS ('dbx_business_glossary_term' = 'Quote Id (Foreign Key)');
ALTER TABLE `semiconductors_ecm`.`customer`.`price_agreement` ALTER COLUMN `sku_id` SET TAGS ('dbx_business_glossary_term' = 'Sku Id (Foreign Key)');
